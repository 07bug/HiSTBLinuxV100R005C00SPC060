...
ifeq ($(CFG_HI_ADVCA_SUPPORT),y)
	ifeq ($(CFG_HI_ADVCA_TYPE),CONAX)
		ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3796mv200 hi3716mv450),)
		    SIGNATURE_SCRIPT_DIR := Conax/3796mv200
		else
		    SIGNATURE_SCRIPT_DIR := Conax/3716mv420
		endif
		...
		ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3796mv200 hi3716mv450),)
		    SIGNATURE_SCRIPT_DIR := Verimatrix/ultra/3796mv200
		else
		    SIGNATURE_SCRIPT_DIR := Verimatrix/ultra/3798mv200
		endif
	else ifeq ($(CFG_HI_ADVCA_TYPE),IRDETO)
		ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3796mv200 hi3716mv450),)
		    SIGNATURE_SCRIPT_DIR := Irdeto/3796mv200
		else
		    SIGNATURE_SCRIPT_DIR := Irdeto/3798mv200
		endif
	else ifeq ($(CFG_HI_ADVCA_TYPE),OTHER)
		ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3798mv200),)
		    SIGNATURE_SCRIPT_DIR := Hisi
		endif
	else ifeq ($(CFG_HI_ADVCA_TYPE),SUMA)
	    SIGNATURE_SCRIPT_DIR := Suma
	else ifeq ($(CFG_HI_ADVCA_TYPE),NOVEL)
	    SIGNATURE_SCRIPT_DIR := Novel
	endif
else ifeq ($(CFG_HI_TEE_SUPPORT),y)
    ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3796mv200 hi3716mv450),)
...
ifneq ($(SIGNATURE_SCRIPT_DIR),none)
	...
	test ! -d $(CREATE_SIGNATURE_DIR)/CreateSignature/$(SIGNATURE_SCRIPT_DIR) || make -C $(CREATE_SIGNATURE_DIR)/CreateSignature/$(SIGNATURE_SCRIPT_DIR) all
...
ifeq ($(CFG_HI_NAND_FLASH_SUPPORT),y)
	-rm -rf $(HI_NAND_IMAGE_DIR)/*.sig $(HI_NAND_IMAGE_DIR)/FinalBoot* $(HI_NAND_IMAGE_DIR)/*signature* $(HI_NAND_IMAGE_DIR)/*enc
endif
ifeq ($(CFG_HI_EMMC_SUPPORT),y)
	-rm -rf $(HI_EMMC_IMAGE_DIR)/*.sig $(HI_EMMC_IMAGE_DIR)/FinalBoot* $(HI_EMMC_IMAGE_DIR)/*signature* $(HI_EMMC_IMAGE_DIR)/*enc
endif
ifeq ($(CFG_HI_SPI_SUPPORT)_$(CFG_HI_EMMC_SUPPORT),y_y)
	-rm -rf $(HI_SPI_EMMC_IMAGE_DIR)/*.sig $(HI_SPI_EMMC_IMAGE_DIR)/FinalBoot* $(HI_SPI_EMMC_IMAGE_DIR)/*signature* $(HI_SPI_EMMC_IMAGE_DIR)/*enc
endif
endif
#====================================================================================
#                   tools
...
ifneq ($(CFG_HI_TWOSTAGEBOOT_SUPPORT),y)
	...
	$(AT)make -C $(BOOT_DIR) all $(if $(HI_OUT_DIR),O=$(HI_OUT_DIR)/obj/source/boot_fbl,) CFG_HI_BOOT_FBL=y
	$(AT)make -C $(BOOT_DIR) all $(if $(HI_OUT_DIR),O=$(HI_OUT_DIR)/obj/source/boot_sbl,) CFG_HI_BOOT_SBL=y
endif
endif
...
rootbox: linux tee tools rootfs common msp higo gpu omx component
ifneq ($(CFG_HI_LOADER_APPLOADER),y)
...
ifeq ($(CFG_HI_ADVCA_TYPE),IRDETO)
	$(AT)make -C $(COMPONENT_DIR)/advca/irdeto/ all
endif

ifeq ($(CFG_HI_ADVCA_TYPE),VERIMATRIX_ULTRA)
ifeq ($(CFG_HI_DRM_SUPPORT),y)
ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3796mv200 hi3716mv450),)
	$(AT)make -C $(COMPONENT_DIR)/drm/vmx_ultra/ all
endif
endif
endif
...
ifeq ($(CFG_HI_ADVCA_TYPE),NAGRA)
  ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3716mv410 hi3716mv420 hi3716mv450 hi3796mv200),)
    HI_APPSQUASHFS_DIR := $(HI_INSTALL_DIR)/appsquashfs
    IMAGES += appsquashfs
    ifeq ($(CFG_HI_NAND_FLASH_SUPPORT),y)
      HI_DATAUBIFS_DIR := $(HI_INSTALL_DIR)/dataubifs
      IMAGES += dataubifs
      ifeq ($(CFG_HI_CHIP_TYPE),hi3796mv200)
        HI_TRUSTSTORAGEFS_DIR := $(HI_INSTALL_DIR)/trusted_storageubifs
        IMAGES += trusted_storageubifs
      endif
    endif
  endif
endif
...
ifeq ($(CFG_HI_SPI_SUPPORT),y)
	...
	$(AT)cp -f $(HI_IMAGE_DIR)/rootfs.squashfs $(HI_SPI_IMAGE_DIR)/
endif

ifeq ($(CFG_HI_ADVCA_TYPE),NAGRA)
  ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3716mv410 hi3716mv420 hi3716mv450 hi3796mv200),)
appsquashfs:
		@mkdir -p ${HI_APPSQUASHFS_DIR}
		@echo "This is a test file for initmrd." > ${HI_APPSQUASHFS_DIR}/README.TXT
		@chmod 755 ${HI_APPSQUASHFS_DIR}/README.TXT
		$(MKSQUASHFS) ${HI_APPSQUASHFS_DIR} $(HI_IMAGE_DIR)/app.squashfs \
			-no-fragments -noappend -noI -comp xz
ifeq ($(CFG_HI_NAND_FLASH_SUPPORT),y)
	$(AT)test -d $(HI_NAND_IMAGE_DIR) || mkdir -p $(HI_NAND_IMAGE_DIR)
	$(AT)cp -f $(HI_IMAGE_DIR)/app.squashfs $(HI_NAND_IMAGE_DIR)/
endif
ifeq ($(CFG_HI_EMMC_SUPPORT),y)
	$(AT)test -d $(HI_EMMC_IMAGE_DIR) || mkdir -p $(HI_EMMC_IMAGE_DIR)
	$(AT)cp -f $(HI_IMAGE_DIR)/app.squashfs $(HI_EMMC_IMAGE_DIR)/
endif
ifeq ($(CFG_HI_SPI_SUPPORT)_$(CFG_HI_EMMC_SUPPORT),y_y)
	$(AT)test -d $(HI_SPI_EMMC_IMAGE_DIR) || mkdir -p $(HI_SPI_EMMC_IMAGE_DIR)
	$(AT)cp -f $(HI_IMAGE_DIR)/app.squashfs $(HI_SPI_EMMC_IMAGE_DIR)/
endif
ifeq ($(CFG_HI_SPI_SUPPORT),y)
	$(AT)test -d $(HI_SPI_IMAGE_DIR) || mkdir -p $(HI_SPI_IMAGE_DIR)
	$(AT)cp -f $(HI_IMAGE_DIR)/app.squashfs $(HI_SPI_IMAGE_DIR)/
endif

    ifeq ($(CFG_HI_NAND_FLASH_SUPPORT),y)
dataubifs:
		@mkdir -p ${HI_DATAUBIFS_DIR}
		@echo "This is a test file for data ubifs." > ${HI_DATAUBIFS_DIR}/README.TXT
		@chmod 755 ${HI_DATAUBIFS_DIR}/README.TXT
		$(AT)test -d $(HI_NAND_IMAGE_DIR) || mkdir -p $(HI_NAND_IMAGE_DIR)
		$(MKUBIIMG) $(CFG_HI_NAND_PAGE_SIZE) $(CFG_HI_NAND_BLOCK_SIZE) ${HI_DATAUBIFS_DIR} ${CFG_HI_DATA_UBI_PARTITION_SIZE} $(HI_NAND_IMAGE_DIR)/data.ubifs

      ifeq ($(CFG_HI_CHIP_TYPE),hi3796mv200)
trusted_storageubifs:
		@mkdir -p $(HI_TRUSTSTORAGEFS_DIR)
		@echo "This is a test file for trusted storage ubifs." > $(HI_TRUSTSTORAGEFS_DIR)/README.TXT
		@chmod 755 $(HI_TRUSTSTORAGEFS_DIR)/README.TXT
		$(AT)test -d $(HI_NAND_IMAGE_DIR) || mkdir -p $(HI_NAND_IMAGE_DIR)
		$(MKUBIIMG) $(CFG_HI_NAND_PAGE_SIZE) $(CFG_HI_NAND_BLOCK_SIZE) $(HI_TRUSTSTORAGEFS_DIR) ${CFG_HI_TRUSTED_STORAGE_UBI_PARTITION_SIZE} $(HI_NAND_IMAGE_DIR)/trusted_storage.ubifs
      endif  # /*ifeq ($(CFG_HI_CHIP_TYPE),hi3796mv200)*/
    endif # /*ifeq ($(CFG_HI_NAND_FLASH_SUPPORT),y) */
  endif # /*ifeq ($(findstring $(CFG_HI_CHIP_TYPE), hi3716mv410 hi3716mv420 hi3716mv450 hi3796mv200),)*/
endif # /*ifeq ($(CFG_HI_ADVCA_TYPE),NAGRA) */
