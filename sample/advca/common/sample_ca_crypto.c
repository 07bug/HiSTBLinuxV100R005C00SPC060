...
static HI_VOID show_keyladderinfo(HI_VOID)
{
    ...
    else if (KEYLADDER_TYPE_DCAS == g_KladType)
    {
        printf("Keyladder type is DCAS\n");
    }
    else
    {
    ...
    else if (HI_UNF_ADVCA_ALG_TYPE_SM4 == g_kladAlg)
    {
        printf("Keyladder Alg  is SM4\n");
    }
    else
    {
        printf("Keyladder Alg  is unknow\n");
    }
    return;
}
...
static HI_S32 _configR2RKeyladder(HI_HANDLE hHandle)
{
    ...
    Ret  = HI_UNF_ADVCA_SetR2RAlg(g_kladAlg);
...
static HI_S32 _configDCASKeyladder(HI_HANDLE hHandle)
{
    HI_S32 Ret = 0;
    HI_U8 dcas_nouce1[KEY_LEN]          = {0};
    HI_U8 vendor_id[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    HI_U8 n1key[16]     = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

//    HI_UNF_ADVCA_KEYLADDER_LEV_E enStage = 0;

    printf("into _configDCASKeyladder\n");

    if (g_kladAlg == HI_UNF_ADVCA_ALG_TYPE_TDES)
    {
        Ret = HI_UNF_ADVCA_DCASOpen(HI_UNF_CIPHER_ALG_3DES);
    }
    else if (g_kladAlg == HI_UNF_ADVCA_ALG_TYPE_AES)
    {
        Ret = HI_UNF_ADVCA_DCASOpen(HI_UNF_CIPHER_ALG_AES);
    }
    else
    {
        Ret = HI_UNF_ADVCA_DCASOpen(HI_UNF_CIPHER_ALG_SM4);
    }
    if (HI_SUCCESS != Ret)
    {
        return HI_FAILURE;
    ...
    Ret = HI_UNF_ADVCA_SetDCASSessionKey(HI_UNF_ADVCA_DCAS_KEYLADDER_VENDORSYSID, vendor_id, dcas_nouce1);
    if (HI_SUCCESS != Ret)
    {
        return HI_FAILURE;
    }

    Ret = HI_UNF_ADVCA_SetDCASSessionKey(HI_UNF_ADVCA_KEYLADDER_LEV1, g_sessionkey1, dcas_nouce1);
    if (HI_SUCCESS != Ret)
    {
        return HI_FAILURE;
    }
    ...
    Ret = HI_UNF_ADVCA_SetDCASSessionKey(HI_UNF_ADVCA_DCAS_KEYLADDER_NONCE, n1key, g_dcas_nouce);
    if (HI_SUCCESS != Ret)
    {
        return HI_FAILURE;
    }
    printBuffer("dcas_nouce", g_dcas_nouce, 16);


    Ret = HI_UNF_ADVCA_SetDCASSessionKey(HI_UNF_ADVCA_KEYLADDER_LEV2, g_sessionkey2, dcas_nouce1);
    if (HI_SUCCESS != Ret)
    {
        return HI_FAILURE;
    }
...
static HI_S32 setConfigure(HI_HANDLE hHandle)
{
    ...
    {
        return _configDCASKeyladder(hHandle);
    }
