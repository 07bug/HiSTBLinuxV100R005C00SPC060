...
HI_VOID show_usage(HI_VOID)
{
    ...
    printf("name: reemarketid      Get ree market segment id\n");
    ...
    printf("name: bootenc          Get boot loader key generation status\n");
...
HI_S32 run_cmdline(HI_S32 argc, HI_CHAR** argv)
{
    ...
    else if (strncmp(argv[1], "sosmarketid", strlen("sosmarketid")) == 0)
    {
        memset(buf, 0x0, sizeof (buf));
        memset(&stOtpAttr, 0x0, sizeof (stOtpAttr));
        HI_UNF_ADVCA_GetOtpFuse(HI_UNF_ADVCA_OTP_SOS_MSID, &stOtpAttr);
    ...
    else if (strncmp(argv[1], "reemarketid", strlen("reemarketid")) == 0)
    {
        memset(buf, 0x0, sizeof (buf));
        memset(&stOtpAttr, 0x0, sizeof (stOtpAttr));
        HI_UNF_ADVCA_GetOtpFuse(HI_UNF_ADVCA_OTP_REE_MSID, &stOtpAttr);
