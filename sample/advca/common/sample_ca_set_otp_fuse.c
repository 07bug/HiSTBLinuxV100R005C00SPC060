...
HI_VOID show_usage(HI_VOID)
{
    ...
    printf("name: reemarketid         value: 0x12345678(4 bytes number)\n");
...
HI_S32 run_cmdline(HI_S32 argc, HI_CHAR** argv)
{
    ...
    else if (strncmp(argv[1], "sosmarketid", strlen("sosmarketid")) == 0)
    {
        ...
        Ret = HI_UNF_ADVCA_SetOtpFuse(HI_UNF_ADVCA_OTP_SOS_MSID, &stOtpAttr);
    }
    else if (strncmp(argv[1], "reemarketid", strlen("reemarketid")) == 0)
    {
        ...
        Ret = HI_UNF_ADVCA_SetOtpFuse(HI_UNF_ADVCA_OTP_REE_MSID, &stOtpAttr);
    }
    else if (strncmp(argv[1], "versionid", strlen("versionid")) == 0)
    {
        ...
        Ret = HI_UNF_ADVCA_GetVersionId(buf_from_otp);
        if( HI_SUCCESS != Ret)
        {
            printf("set %s  %s fail\n", argv[1], argv[2]);
            return HI_FAILURE;
