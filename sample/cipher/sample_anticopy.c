...
static const HI_U8 g_au8CustomerKey[16] = {0x3b, 0xa0, 0x78, 0xf8, 0xc6, 0x0d, 0xfc, 0x1d, 0x72, 0xf8, 0x3b, 0xec, 0x02, 0x35, 0xa2, 0x0d};
...
static HI_S32 Cipher_LoadStbRootKey(HI_HANDLE hCipher)
{
#if defined (CA_FRAMEWORK_V200_SUPPORT)
    HI_S32 ret =HI_FAILURE;
    HI_HANDLE hKlad;
    HI_UNF_KLAD_ATTR_S stAttr;
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CIPHER("HI_UNF_KLAD_Create falied!\n");
        HI_UNF_KLAD_DeInit();
        return HI_FAILURE;
    }

    ret = HI_UNF_KLAD_Attach(hKlad, hCipher);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CIPHER("HI_UNF_KLAD_Attach falied!\n");
        HI_UNF_KLAD_Destroy(hKlad);
        HI_UNF_KLAD_DeInit();
        return HI_FAILURE;
    }

    stAttr.enAlg       = HI_UNF_KLAD_ALG_TYPE_BUTT;
    stAttr.enEngine    = HI_UNF_KLAD_TARGET_ENGINE_FIXED;
    stAttr.u32KladType = HI_UNF_KLAD_TYPE_STBROOTKEY;
    ret = HI_UNF_KLAD_SetAttr(hKlad, &stAttr);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CIPHER("HI_UNF_KLAD_SetAttr falied!\n");
        HI_UNF_KLAD_Detach(hKlad, hCipher);
        HI_UNF_KLAD_Destroy(hKlad);
        HI_UNF_KLAD_DeInit();
        return HI_FAILURE;
    }

    ret = HI_UNF_KLAD_SendSTBRootkeyToCipher(hKlad);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CIPHER("HI_UNF_KLAD_SendSTBRootkeyToCipher falied!\n");
        HI_UNF_KLAD_Detach(hKlad, hCipher);
        HI_UNF_KLAD_Destroy(hKlad);
        HI_UNF_KLAD_DeInit();
        return HI_FAILURE;
...
//    stCipherAttr.enCipherType = HI_UNF_CIPHER_TYPE_COPY_AVOID;
    ...
    ret = Cipher_LoadStbRootKey(hTestchnid);
    if(HI_SUCCESS != ret)
    {
        HI_ERR_CIPHER("Cipher_LodStbRootKey failed.\n");
        goto __CIPHER_EXIT__;
    }
    ...
    ret = memcmp(g_au8DataOut, g_au8CustomerKey, sizeof(g_au8CustomerKey));
    if(0 != ret)
    {
        HI_ERR_CIPHER("error, customer key memcmp failed!\n");
        goto __CIPHER_EXIT__;
    }
