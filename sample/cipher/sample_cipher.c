...
HI_VOID CBC_SM4(HI_VOID)
{
    ...
    HI_HANDLE hTestchnid = 0;
    HI_UNF_CIPHER_ATTS_S stCipherAttr;
    HI_UNF_CIPHER_CTRL_EX_S stCtrl;
    HI_UNF_CIPHER_CTRL_SM4_S stSm4Param;
    ...
    HI_U8 sm4_src[32] = {"\xD6\xEC\x40\x8D\x54\x27\xC4\x0E\x21\xD9\x91\xAD\x5A\xA3\xC7\x71\x2C\x85\x59\x5D\x30\x72\xDE\x74\xF5\x6C\x1B\x4F\xE0\x2F\xAE\xCA"};
    HI_U8 sm4_dst[32] = {"\xC3\x34\x6B\xAA\x1D\x5D\xD1\x70\xAA\xD7\x67\x95\xDC\xB1\x99\x90\xB9\x69\xD3\xF3\x3D\xBA\xB6\xBE\x85\xE9\x1C\x8C\x7D\xB3\x2C\xB9"};
    ...
    stCtrl.enAlg = HI_UNF_CIPHER_ALG_SM4;
    stCtrl.bKeyByCA = 0;
    stCtrl.enWorkMode = HI_UNF_CIPHER_WORK_MODE_CBC;
    stCtrl.pParam = &stSm4Param;
    stSm4Param.stChangeFlags.bit1IV = CIPHER_IV_CHANGE_ONE_PKG;
    memcpy(stSm4Param.u32Key, sm4_key, 16);
    memcpy(stSm4Param.u32IV, sm4_IV, 16);

    /* For encrypt */
    s32Ret = HI_UNF_CIPHER_ConfigHandleEx(hTestchnid, &stCtrl);
    if(HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("Set config info failed.\n");
        goto __CIPHER_EXIT__;
    ...
    memset(pInputAddrVir, 0x0, u32TestDataLen);
    memcpy(pInputAddrVir, sm4_src, u32TestDataLen);
    printBuffer("CBC-SM4-128-ORI:", sm4_src, u32TestDataLen);
    ...
    printBuffer("CBC-SM4-128-ENC:", pOutputAddrVir, u32TestDataLen);

    /* compare */
    if ( 0 != memcmp(pOutputAddrVir, sm4_dst, u32TestDataLen) )
    {
        HI_ERR_CIPHER("Memcmp failed!\n");
        s32Ret = HI_FAILURE;
        goto __CIPHER_EXIT__;
   ...
   /* For decrypt */
    memcpy(pInputAddrVir, sm4_dst, u32TestDataLen);
    memset(pOutputAddrVir, 0x0, u32TestDataLen);
    ...
    printBuffer("CBC-SM4-128-DEC", pOutputAddrVir, u32TestDataLen);
    /* compare */
    if ( 0 != memcmp(pOutputAddrVir, sm4_src, u32TestDataLen) )
    {
        HI_ERR_CIPHER("Memcmp failed!\n");
        s32Ret = HI_FAILURE;
        goto __CIPHER_EXIT__;
...
HI_S32 CCM_AES128_3(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32TestDataLen = 24;
    HI_U32 u32InputAddrPhy = 0;
    HI_U32 u32OutPutAddrPhy = 0;
    HI_U32 u32AadAddrPhy = 0;
    HI_U8 *pInputAddrVir = HI_NULL;
