...
HI_S32 PKCS_PRI_ENC(HI_UNF_CIPHER_RSA_ENC_SCHEME_E enScheme)
{
    ...
    if (enScheme == HI_UNF_CIPHER_RSA_ENC_SCHEME_RSAES_PKCS1_V1_5)
    {
 ...
 //       stRsaEnc.stPriKey.pu8D = (HI_U8*)malloc(256);
        ...
        stRsaEnc.stPriKey.pu8D = HI_NULL;
        /*
        ...
        */
        ...
        memcpy(stRsaEnc.stPriKey.pu8N, N, 256);
        memcpy(stRsaEnc.stPriKey.pu8E, E, 256);
        memcpy(stRsaEnc.stPriKey.pu8P, P, 128);
        memcpy(stRsaEnc.stPriKey.pu8Q, Q, 128);

        stRsaEnc.stPriKey.u16NLen = 256;
        stRsaEnc.stPriKey.u16ELen = 256;
        stRsaEnc.stPriKey.u16PLen = 128;
        stRsaEnc.stPriKey.u16QLen = 128;
        stRsaEnc.stPriKey.u16DPLen = 128;
        stRsaEnc.stPriKey.u16DQLen = 128;
        stRsaEnc.stPriKey.u16QPLen = 128;
...
//        printBuffer ("D", stRsaEnc.stPriKey.pu8D, stRsaEnc.stPriKey.u16DLen);
        printBuffer ("P", stRsaEnc.stPriKey.pu8P, stRsaEnc.stPriKey.u16PLen);
        printBuffer ("Q", stRsaEnc.stPriKey.pu8Q, stRsaEnc.stPriKey.u16QLen);
 //       printBuffer ("DP", stRsaEnc.stPriKey.pu8DP, stRsaEnc.stPriKey.u16DPLen);
//        printBuffer ("DQ", stRsaEnc.stPriKey.pu8DQ, stRsaEnc.stPriKey.u16DQLen);
//        printBuffer ("QP", stRsaEnc.stPriKey.pu8QP, stRsaEnc.stPriKey.u16QPLen);

        memset(stRsaEnc.stPriKey.pu8DP, 0, 128);
        memset(stRsaEnc.stPriKey.pu8DQ, 0, 128);
        memset(stRsaEnc.stPriKey.pu8QP, 0, 128);
        printf("Compute CRT Params...\n");
        ret = HI_UNF_CIPHER_RsaComputeCrtParams(2048, 0x10001, stRsaEnc.stPriKey.pu8P,
                                        stRsaEnc.stPriKey.pu8Q, stRsaEnc.stPriKey.pu8DP,
                                        stRsaEnc.stPriKey.pu8DQ, stRsaEnc.stPriKey.pu8QP);

        if ( HI_SUCCESS != ret )
        {
            HI_ERR_CIPHER("HI_UNF_CIPHER_RsaComputeCrtParams failed, ret = %d\n", ret);
            return HI_FAILURE;
        }
        printBuffer ("DP", stRsaEnc.stPriKey.pu8DP, stRsaEnc.stPriKey.u16DPLen);
        printBuffer ("DQ", stRsaEnc.stPriKey.pu8DQ, stRsaEnc.stPriKey.u16DQLen);
    ...
    ret = HI_UNF_CIPHER_RsaPrivateEncrypt(&stRsaEnc, sha256_sum, sizeof(sha256_sum), u8Sign, &u32SignLen);
    if ( HI_SUCCESS != ret )
    {
        HI_ERR_CIPHER("HI_UNF_CIPHER_RsaPrivateEncrypt failed\n");
        return HI_FAILURE;
    }
    ...
    if ( HI_SUCCESS != ret )
    {
        HI_ERR_CIPHER("HI_UNF_CIPHER_RsaPublicDecrypt failed\n");
        return HI_FAILURE;
    }
...
int main(int argc, char* argv[])
{
    ...
    PKCS_PRI_ENC(HI_UNF_CIPHER_RSA_ENC_SCHEME_RSAES_PKCS1_V1_5);
