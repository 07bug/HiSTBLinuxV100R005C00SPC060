...
#include <string.h>
    ...
    {
#ifdef ANDROID44
        pBitmap->setConfig(ColorType, u32Width, u32Height);
        if(NULL!=pu8Buffer)
        {
...
#if defined(ANDROID5) || defined(ANDROIDN) || defined(ANDROIDO)
        if(NULL==pu8Buffer)
        {
    ...
    HI_S32 OsdManager::DrawCCText(HI_HANDLE hSurface, SURFACE_ATTR_S * pstSurfaceAttr,HI_RECT_S* pstRect, HI_CHAR* pTest,CCTEXT_ATTR_S * pstAttr)
    {
        ...
        Set_SkBitmap(&bitmap,pSurface->stAttr.u32Width,pSurface->stAttr.u32Height,pBitmapAddr,kRGBA_8888_SkColorType,kPremul_SkAlphaType);
        ...
        {
        paint.setTypeface(SkTypeface::MakeFromName("serif", SkFontStyle::FromOldStyle(SkTypeface::kItalic)));
        }
        else
        {
        paint.setTypeface(SkTypeface::MakeFromName("serif", SkFontStyle::FromOldStyle(SkTypeface::kNormal)));
        }
    ...
    HI_S32 OsdManager::GetCCTextSize(HI_CHAR* pTest,HI_S32 *pWidth, HI_S32 *pHeigh )
    {
        ...
        paint.setTextSize(ts);
        paint.setTypeface(SkTypeface::MakeFromName("monospace", SkFontStyle::FromOldStyle(SkTypeface::kNormal)));
        size_t byteLength = strlen(pTest);
