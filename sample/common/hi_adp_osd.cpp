...
{
    pstAdpAttr->enPixelFormat = PixelFormatAndroid2OSD(pstOsdAttr->s32PixelFormat);
    pstAdpAttr->u32Width      = pstOsdAttr->u32Width;
    pstAdpAttr->u32Height     = pstOsdAttr->u32Height;
}

static HI_VOID SurfaceAttrADP2OSD(HIADP_SURFACE_ATTR_S *pstAdpAttr, SURFACE_ATTR_S *pstOsdAttr)
{
    pstOsdAttr->s32PixelFormat   = PixelFormatOSD2Android(pstAdpAttr->enPixelFormat);
    pstOsdAttr->u32Width         = pstAdpAttr->u32Width;
    pstOsdAttr->u32Height        = pstAdpAttr->u32Height;
    pstOsdAttr->u32Stride        = pstAdpAttr->u32Width * GetBytesPerPixel(pstAdpAttr->enPixelFormat);
}
...
{
    OSD_CHECK_INIT();
    HI_UNUSED(EncodeType);

    return g_om->Videoframe2jpg(pstFrame, DstFile);
}
