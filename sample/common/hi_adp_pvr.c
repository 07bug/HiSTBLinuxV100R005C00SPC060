...
#if 0
static HI_BOOL s_bSupportSecAudio = HI_FALSE;
static HI_BOOL s_bUseSecTsBuffer = HI_FALSE;
#endif
...
HI_S32 PvrTeeRecordCallback(HI_UNF_PVR_DATA_ATTR_S *pstDataAttr,
                              HI_UNF_PVR_BUF_S *pstDstBuf,
                              HI_UNF_PVR_BUF_S *pstSrcBuf,
                              HI_U32 u32Offset,
                              HI_U32 u32DataSize,
                              HI_VOID *pUserData)
{
...
HI_S32 PvrTeePlayBackCallback(HI_UNF_PVR_DATA_ATTR_S *pstDataAttr,
                              HI_UNF_PVR_BUF_S *pstDstBuf,
                              HI_UNF_PVR_BUF_S *pstSrcBuf,
                              HI_U32 u32Offset,
                              HI_U32 u32DataSize,
                              HI_VOID *pUserData)
{
    ...
    if (HI_TRUE != s_bUseSecTsBuffer)
        return HI_SUCCESS;

    memset(&stCopyOperation, 0x00, sizeof(stCopyOperation));
    stCopyOperation.started = 1;
    stCopyOperation.params[1].value.a = pstDstBuf->u32PhyAddr;
    stCopyOperation.params[1].value.b = 0xA5A5A5A5;
    stCopyOperation.params[2].value.a = pstSrcBuf->u32PhyAddr;
    stCopyOperation.params[2].value.b = u32DataSize;
    stCopyOperation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INOUT,
                                            TEEC_VALUE_INOUT,
...
HI_VOID SearchFileTsSendThread(HI_VOID *args)
{
    ...
    while (!g_bStopTsThread)
    {
        ...
        Ret = HIADP_DMX_PushTsBuffer(hTsBuf, &stStreamBuf, 0, u32Readlen);
        if (Ret != HI_SUCCESS )
        {
...
HI_VOID PVR_RecProgInfoTrans(PVR_PROG_INFO_S *pstFileInfo, PMT_COMPACT_PROG *pstProgInfo)
{
    ...
    pstFileInfo->ProgID = pstProgInfo->ProgID;
    pstFileInfo->PmtPid = pstProgInfo->PmtPid;
    pstFileInfo->PcrPid = pstProgInfo->PcrPid;
    pstFileInfo->VideoType = pstProgInfo->VideoType;
    pstFileInfo->VElementNum = pstProgInfo->VElementNum;
    pstFileInfo->VElementPid = pstProgInfo->VElementPid;
    pstFileInfo->AudioType = pstProgInfo->AudioType;
    pstFileInfo->AElementNum = pstProgInfo->AElementNum;
    pstFileInfo->AElementPid = pstProgInfo->AElementPid;
    pstFileInfo->SubtType = pstProgInfo->SubtType;
    pstFileInfo->u16SubtitlingNum = pstProgInfo->u16SubtitlingNum;
    pstFileInfo->u16ClosedCaptionNum = pstProgInfo->u16ClosedCaptionNum;
    pstFileInfo->u16ARIBCCPid = pstProgInfo->u16ARIBCCPid;
    pstFileInfo->u16TtxNum = pstProgInfo->u16TtxNum;
    pstFileInfo->stSCTESubtInfo.u16SCTESubtPID = pstProgInfo->stSCTESubtInfo.u16SCTESubtPID;
    pstFileInfo->stSCTESubtInfo.u32LanguageCode = pstProgInfo->stSCTESubtInfo.u32LanguageCode;
    
    u32Max = PVR_PROG_MAX_AUDIO > PROG_MAX_AUDIO ? PROG_MAX_AUDIO : PVR_PROG_MAX_AUDIO;
    for (i = 0; i < u32Max; i++)
    {
        pstFileInfo->Audioinfo[i].u16AudioPid = pstProgInfo->Audioinfo[i].u16AudioPid;
        pstFileInfo->Audioinfo[i].u32AudioEncType = pstProgInfo->Audioinfo[i].u32AudioEncType;
    }

    u32Max = PVR_SUBTITLING_MAX > SUBTITLING_MAX ? SUBTITLING_MAX : PVR_SUBTITLING_MAX;
    for (i = 0; i < u32Max; i++)
    {
        u32Num = PVR_SUBTDES_INFO_MAX > SUBTDES_INFO_MAX ? SUBTDES_INFO_MAX : PVR_SUBTDES_INFO_MAX;
        pstFileInfo->SubtitingInfo[i].u8DesTag = pstProgInfo->SubtitingInfo[i].u8DesTag;
        pstFileInfo->SubtitingInfo[i].u8DesLength = pstProgInfo->SubtitingInfo[i].u8DesLength;
        pstFileInfo->SubtitingInfo[i].u8DesInfoCnt = pstProgInfo->SubtitingInfo[i].u8DesInfoCnt;
        pstFileInfo->SubtitingInfo[i].u16SubtitlingPID = pstProgInfo->SubtitingInfo[i].u16SubtitlingPID;
        for (j = 0; j < u32Num; j++)
        {
            pstFileInfo->SubtitingInfo[i].DesInfo[j].u16AncillaryPageID = pstProgInfo->SubtitingInfo[i].DesInfo[j].u16AncillaryPageID;
            pstFileInfo->SubtitingInfo[i].DesInfo[j].u16PageID = pstProgInfo->SubtitingInfo[i].DesInfo[j].u16AncillaryPageID;
            pstFileInfo->SubtitingInfo[i].DesInfo[j].u32LangCode = pstProgInfo->SubtitingInfo[i].DesInfo[j].u32LangCode;
            pstFileInfo->SubtitingInfo[i].DesInfo[j].u8SubtitleType = pstProgInfo->SubtitingInfo[i].DesInfo[j].u8SubtitleType;
        }
    }

    u32Max = PVR_CAPTION_SERVICE_MAX > CAPTION_SERVICE_MAX ? CAPTION_SERVICE_MAX : PVR_CAPTION_SERVICE_MAX;
    for (i = 0; i < u32Max; i++)
    {
        pstFileInfo->stClosedCaption[i].u32LangCode = pstProgInfo->stClosedCaption[i].u32LangCode;
        pstFileInfo->stClosedCaption[i].u8IsDigitalCC = pstProgInfo->stClosedCaption[i].u8IsDigitalCC;
        pstFileInfo->stClosedCaption[i].u8ServiceNumber = pstProgInfo->stClosedCaption[i].u8ServiceNumber;
        pstFileInfo->stClosedCaption[i].u8IsEasyReader = pstProgInfo->stClosedCaption[i].u8IsEasyReader;
        pstFileInfo->stClosedCaption[i].u8IsWideAspectRatio = pstProgInfo->stClosedCaption[i].u8IsWideAspectRatio;
    }
    u32Max = PVR_TTX_MAX > TTX_MAX ? TTX_MAX : PVR_TTX_MAX;
    for (i = 0; i < u32Max; i++)
    {
        u32Num = PVR_TTX_DES_MAX > TTX_DES_MAX ? TTX_DES_MAX : PVR_TTX_DES_MAX;
        pstFileInfo->stTtxInfo[i].u16TtxPID = pstProgInfo->stTtxInfo[i].u16TtxPID;
        pstFileInfo->stTtxInfo[i].u8DesTag = pstProgInfo->stTtxInfo[i].u8DesTag;
        pstFileInfo->stTtxInfo[i].u8DesLength = pstProgInfo->stTtxInfo[i].u8DesLength;
        pstFileInfo->stTtxInfo[i].u8DesInfoCnt = pstProgInfo->stTtxInfo[i].u8DesInfoCnt;
        for (j = 0; j < u32Num; j++)
        {
            pstFileInfo->stTtxInfo[i].stTtxDes[j].u32ISO639LanguageCode = pstProgInfo->stTtxInfo[i].stTtxDes[j].u32ISO639LanguageCode;
            pstFileInfo->stTtxInfo[i].stTtxDes[j].u8TtxType = pstProgInfo->stTtxInfo[i].stTtxDes[j].u8TtxType;
            pstFileInfo->stTtxInfo[i].stTtxDes[j].u8TtxMagazineNumber = pstProgInfo->stTtxInfo[i].stTtxDes[j].u8TtxMagazineNumber;
            pstFileInfo->stTtxInfo[i].stTtxDes[j].u8TtxPageNumber = pstProgInfo->stTtxInfo[i].stTtxDes[j].u8TtxPageNumber;
        }
    }
    return;
}
HI_VOID PVR_PlayProgInfoTrans(PMT_COMPACT_PROG *pstProgInfo, PVR_PROG_INFO_S *pstFileInfo)
{
    ...
    pstProgInfo->ProgID = pstFileInfo->ProgID;
    pstProgInfo->PmtPid = pstFileInfo->PmtPid;
    pstProgInfo->PcrPid = pstFileInfo->PcrPid;
    pstProgInfo->VideoType = pstFileInfo->VideoType;
    pstProgInfo->VElementNum = pstFileInfo->VElementNum;
    pstProgInfo->VElementPid = pstFileInfo->VElementPid;
    pstProgInfo->AudioType = pstFileInfo->AudioType;
    pstProgInfo->AElementNum = pstFileInfo->AElementNum;
    pstProgInfo->AElementPid = pstFileInfo->AElementPid;
    pstProgInfo->SubtType = pstFileInfo->SubtType;
    pstProgInfo->u16SubtitlingNum = pstFileInfo->u16SubtitlingNum;
    pstProgInfo->u16ClosedCaptionNum = pstFileInfo->u16ClosedCaptionNum;
    pstProgInfo->u16ARIBCCPid = pstFileInfo->u16ARIBCCPid;
    pstProgInfo->u16TtxNum = pstFileInfo->u16TtxNum;
    pstProgInfo->stSCTESubtInfo.u16SCTESubtPID = pstFileInfo->stSCTESubtInfo.u16SCTESubtPID;
    pstProgInfo->stSCTESubtInfo.u32LanguageCode = pstFileInfo->stSCTESubtInfo.u32LanguageCode;
    
    u32Max = PVR_PROG_MAX_AUDIO > PROG_MAX_AUDIO ? PROG_MAX_AUDIO : PVR_PROG_MAX_AUDIO;
    for (i = 0; i < u32Max; i++)
    {
        pstProgInfo->Audioinfo[i].u16AudioPid = pstFileInfo->Audioinfo[i].u16AudioPid;
        pstProgInfo->Audioinfo[i].u32AudioEncType = pstFileInfo->Audioinfo[i].u32AudioEncType;
    }
    ...
    {
        u32Num = PVR_SUBTDES_INFO_MAX > SUBTDES_INFO_MAX ? SUBTDES_INFO_MAX : PVR_SUBTDES_INFO_MAX;
        pstProgInfo->SubtitingInfo[i].u8DesTag = pstFileInfo->SubtitingInfo[i].u8DesTag;
        pstProgInfo->SubtitingInfo[i].u8DesLength = pstFileInfo->SubtitingInfo[i].u8DesLength;
        pstProgInfo->SubtitingInfo[i].u8DesInfoCnt = pstFileInfo->SubtitingInfo[i].u8DesInfoCnt;
        pstProgInfo->SubtitingInfo[i].u16SubtitlingPID = pstFileInfo->SubtitingInfo[i].u16SubtitlingPID;
        for (j = 0; j < u32Num; j++)
        {
            pstProgInfo->SubtitingInfo[i].DesInfo[j].u16AncillaryPageID = pstFileInfo->SubtitingInfo[i].DesInfo[j].u16AncillaryPageID;
            pstProgInfo->SubtitingInfo[i].DesInfo[j].u16PageID = pstFileInfo->SubtitingInfo[i].DesInfo[j].u16AncillaryPageID;
            pstProgInfo->SubtitingInfo[i].DesInfo[j].u32LangCode = pstFileInfo->SubtitingInfo[i].DesInfo[j].u32LangCode;
            pstProgInfo->SubtitingInfo[i].DesInfo[j].u8SubtitleType = pstFileInfo->SubtitingInfo[i].DesInfo[j].u8SubtitleType;
        }
    }
    ...
    for (i = 0; i < u32Max; i++)
    {
        pstProgInfo->stClosedCaption[i].u32LangCode = pstFileInfo->stClosedCaption[i].u32LangCode;
        pstProgInfo->stClosedCaption[i].u8IsDigitalCC = pstFileInfo->stClosedCaption[i].u8IsDigitalCC;
        pstProgInfo->stClosedCaption[i].u8ServiceNumber = pstFileInfo->stClosedCaption[i].u8ServiceNumber;
        pstProgInfo->stClosedCaption[i].u8IsEasyReader = pstFileInfo->stClosedCaption[i].u8IsEasyReader;
        pstProgInfo->stClosedCaption[i].u8IsWideAspectRatio = pstFileInfo->stClosedCaption[i].u8IsWideAspectRatio;
    }
    u32Max = PVR_TTX_MAX > TTX_MAX ? TTX_MAX : PVR_TTX_MAX;
    for (i = 0; i < u32Max; i++)
    {
        u32Num = PVR_TTX_DES_MAX > TTX_DES_MAX ? TTX_DES_MAX : PVR_TTX_DES_MAX;
        pstProgInfo->stTtxInfo[i].u16TtxPID = pstFileInfo->stTtxInfo[i].u16TtxPID;
        pstProgInfo->stTtxInfo[i].u8DesTag = pstFileInfo->stTtxInfo[i].u8DesTag;
        pstProgInfo->stTtxInfo[i].u8DesLength = pstFileInfo->stTtxInfo[i].u8DesLength;
        pstProgInfo->stTtxInfo[i].u8DesInfoCnt = pstFileInfo->stTtxInfo[i].u8DesInfoCnt;
        for (j = 0; j < u32Num; j++)
        {
            pstProgInfo->stTtxInfo[i].stTtxDes[j].u32ISO639LanguageCode = pstFileInfo->stTtxInfo[i].stTtxDes[j].u32ISO639LanguageCode;
            pstProgInfo->stTtxInfo[i].stTtxDes[j].u8TtxType = pstFileInfo->stTtxInfo[i].stTtxDes[j].u8TtxType;
            pstProgInfo->stTtxInfo[i].stTtxDes[j].u8TtxMagazineNumber = pstFileInfo->stTtxInfo[i].stTtxDes[j].u8TtxMagazineNumber;
            pstProgInfo->stTtxInfo[i].stTtxDes[j].u8TtxPageNumber = pstFileInfo->stTtxInfo[i].stTtxDes[j].u8TtxPageNumber;
        }
    }
    return;
}
...
{
    s_bUseSecTsBuffer = bSupport;
    s_bSupportSecAudio = bSupport;
    return;
}
#endif
                    ...
                    HI_BOOL bRewind, HI_BOOL bDoCipher, HI_U64 u64MaxSize, HI_U32 *pu32RecChn)
{
    ...
    {
#if 0
        HI_UNF_PVR_RegisterExtendCallback(u32RecChn, HI_UNF_PVR_EXTRA_WRITE_CALLBACK, PvrTeeRecordCallback, HI_NULL);
#endif
/*
...
*/
    ...
    PVR_RecProgInfoTrans(&stFileInfo, pstProgInfo);
...
HI_S32 PVR_RecStop(HI_U32 u32RecChnID)
{
    ...
    {
#if 0
        HI_UNF_PVR_UnRegisterExtendCallback(u32RecChnID, HI_UNF_PVR_EXTRA_WRITE_CALLBACK, PvrTeeRecordCallback);
#endif
    }
...
//    HI_U32 i;
    ...
    memset(&stAdecAttr, 0x00, sizeof(stAdecAttr));
    /*
...
HI_S32 PVR_StartLivePlay(HI_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    ...
    HI_CODEC_VIDEO_CMD_S  stVdecCmdPara;
    ...
    memset(&stVdecCmdPara, 0x00, sizeof(stVdecCmdPara));
    memset(&stTplayOpts, 0x00, sizeof(stTplayOpts));
...
HI_S32 PVR_StartPlayBack(const HI_CHAR *pszFileName, HI_U32 *pu32PlayChn, HI_HANDLE hAvplay)
{
    ...
    memset(&stTplayOpts, 0x00, sizeof(stTplayOpts));
    memset(&stVdecCmdPara, 0x00, sizeof(stVdecCmdPara));
    memset(&stProgInfoTmp, 0x00, sizeof(stProgInfoTmp));
    ...
    {
        PVR_PlayProgInfoTrans(&stProgInfoTmp, &stFileInfo);
...
#if 0
    if (HI_FALSE != s_bUseSecTsBuffer)
    {
        HI_UNF_DMX_TSBUF_ATTR_S stTsBuffAttr;
        stTsBuffAttr.enSecureMode = HI_UNF_DMX_SECURE_MODE_TEE;
        stTsBuffAttr.u32BufSize = 4*1024*1024;
        SAMPLE_RUN(HI_UNF_DMX_CreateTSBufferExt(PVR_DMX_PORT_ID_PLAYBACK, &stTsBuffAttr,&g_hTsBufForPlayBack), ret);
    }
    else
#endif
    {
        SAMPLE_RUN(HI_UNF_DMX_CreateTSBuffer(PVR_DMX_PORT_ID_PLAYBACK, 4 * 1024 * 1024, &g_hTsBufForPlayBack), ret);
    }
    if (ret != HI_SUCCESS)
    {
    ...
    if (HI_FALSE != s_bRegisterTeeExtra)
    {
        HI_UNF_PVR_RegisterExtendCallback(u32PlayChn, HI_UNF_PVR_EXTRA_READ_CALLBACK, PvrTeePlayBackCallback, HI_NULL);
    }
#endif
    if ((HI_FALSE != s_bRegisterExtra) && (HI_FALSE != s_bRegisterTeeExtra))
    {
...
HI_S32 PVR_AvplayInit(HI_HANDLE hWin, HI_HANDLE *phAvplay, HI_HANDLE* phSoundTrack)
{
    ...
    memset(&stAvplayAttr, 0x00, sizeof(stAvplayAttr));
    memset(&stSyncAttr, 0x00, sizeof(stSyncAttr));
...
#if 0
    if (HI_FALSE != s_bSupportSecAudio)
    {
        HI_UNF_AVPLAY_AUD_SECURITY_ATTR_S stAudioAttr;
        stAudioAttr.bEnable = HI_TRUE;
        Ret = HI_UNF_AVPLAY_SetAttr(hAvplay, HI_UNF_AVPLAY_ATTR_ID_AUD_SECURITY, &stAudioAttr);
        if (Ret != HI_SUCCESS)
        {
            PVR_SAMPLE_Printf("[%s_%d]call HI_UNF_AVPLAY_SetAttr failed.ret = 0x%08x\n", __FILE__, __LINE__, Ret);
            HI_UNF_AVPLAY_DeInit();
