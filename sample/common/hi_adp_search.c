...
typedef struct tagPMT_DESC_ISO639_S
{
    HI_U8 au8ISO639_Language_Code[HI_LANG_CODE_LEN];
    HI_U8 u8AudioType;
}PMT_DESC_ISO639_S;

typedef struct tagPMT_DESC_ISO639_LANGUAGE_S
{
    HI_U32 u32LanguageEleNum;
    PMT_DESC_ISO639_S astLanguageInfo[HI_MAX_LANGUAGE_ELENUM];
}PMT_DESC_ISO639_LANGUAGE_S;

static HI_U8 s_au8LangChTable[16][16] =
{
...
static long long getBits48 (HI_U8* pu8Buff, HI_S32 s32ByteOffset, HI_S32 s32StartBit, HI_S32 s32BitLen)
{
    HI_U8* u8Tmp = HI_NULL;
    ...
    if (s32BitLen > 48) return 0x7EFEFEFEFEFEFEFELL;
    if (pu8Buff == (unsigned char*) 0) return 0x7EFEFEFEFEFEFEFELL;
    if (0 == s32BitLen) return 0;

    u8Tmp = &pu8Buff[s32ByteOffset + (s32StartBit >> 3)];
    s32StartBit %= 8;
    tmp = (unsigned long long)(*(u8Tmp + 6)+ (*(u8Tmp + 5)<< 8) + (*(u8Tmp +4)<< 16) +
          ((unsigned long long)*(u8Tmp + 3) << 24) + ((unsigned long long) * (u8Tmp + 2) << 32) +
          ((unsigned long long) * (u8Tmp + 1) << 40) + ((unsigned long long) * (u8Tmp) << 48));
    s32StartBit = 56 - s32StartBit - s32BitLen;
    tmp  = tmp >> s32StartBit;
    mask = (1ULL << s32BitLen) - 1;
    v = tmp & mask;
    return v;
}

static unsigned long getBits (HI_U8* pu8Buff, HI_S32 s32ByteOffset, HI_S32 s32StartBit, HI_S32 s32BitLen)
{
    HI_U8* u8Tmp = HI_NULL;
    unsigned long v = 0;
    ...
    if (s32BitLen > 32) return (HI_U32) 0xFEFEFEFE;

    if (pu8Buff == (unsigned char*) 0) return (HI_U32) 0xFEFEFEFE;

    if (0 == s32BitLen) return 0;

    u8Tmp = &pu8Buff[s32ByteOffset + (s32StartBit >> 3)];
    s32StartBit %= 8;
    switch ((s32BitLen - 1) >> 3)
    {
        case 3:
            return (unsigned long) getBits48(u8Tmp, 0, s32StartBit, s32BitLen);
        case 2:
            tmp_long = (unsigned long)(*(u8Tmp + 3) + (*(u8Tmp + 2) << 8) + (*(u8Tmp + 1) << 16) + (*(u8Tmp) << 24));
            s32BitHigh = 32;
            break;
        case 1:
            tmp_long = (unsigned long)(*(u8Tmp + 2) + (*(u8Tmp + 1) << 8) + (*(u8Tmp) << 16));
            s32BitHigh = 24;
            break;
        case 0:
            tmp_long = (unsigned long)((*(u8Tmp) << 8) + * (u8Tmp + 1));
            s32BitHigh = 16;
            break;
        case - 1:
            return 0L;
    ...
    s32StartBit = s32BitHigh - s32StartBit - s32BitLen;
    tmp_long = tmp_long >> s32StartBit;
    mask = (1ULL << s32BitLen) - 1;
    v = tmp_long & mask;
    return v;
}

static HI_VOID ParseAudio_LanguageCodeTrans(const HI_U8 au8ISO639_Language_Code[3], HI_U8 u8StrBufLen, HI_U8 *pu8StrBuf)
{
    HI_U32 i = 0;
    HI_U8 u8FirstNibble = 0;
    HI_U8 u8SecondNibble = 0;

    if ((HI_NULL== pu8StrBuf) || (3 > u8StrBufLen))
    {
        return;
    }
    ...
    for (i = 0; i < 3; i++)
    {
        u8FirstNibble = (au8ISO639_Language_Code[i]&0xF0) >> 4;
        u8SecondNibble = au8ISO639_Language_Code[i]&0x0F;
        if ((0xf < u8FirstNibble) || (0xF < u8SecondNibble))
            return;
        pu8StrBuf[i] = s_au8LangChTable[u8SecondNibble][u8FirstNibble];
    }

    return;
}

static HI_VOID ParseAudioLunguage( const HI_U8* pu8DesData, HI_U32 u32BufLen, PMT_DESC_ISO639_LANGUAGE_S *pstData)
{
    HI_U8 *pu8Buff = (HI_U8 *)pu8DesData;
    HI_U8 u8Length = 0;
    HI_U8 u8StrCode[3] = {0};

    if ((HI_NULL== pu8Buff) || (HI_NULL== pstData))
    {
        return;
    }

    if (LANGUAGE_DESCRIPTOR != pu8Buff[0])
    {
        return;
    }

    if (u32BufLen != (pu8Buff[1] + 2))
    {
        return;
    }
    ...
    u8Length = pu8Buff[1];
    pu8Buff += 2;
    pstData->u32LanguageEleNum = 0;
    while(u8Length >= 4)
    {
        pstData->astLanguageInfo[pstData->u32LanguageEleNum].au8ISO639_Language_Code[0] = (HI_U8)getBits(pu8Buff, 0, 0, 8);
        pstData->astLanguageInfo[pstData->u32LanguageEleNum].au8ISO639_Language_Code[1] = (HI_U8)getBits(pu8Buff, 0, 8, 8);
        pstData->astLanguageInfo[pstData->u32LanguageEleNum].au8ISO639_Language_Code[2] = (HI_U8)getBits(pu8Buff, 0, 16, 8);
        pstData->astLanguageInfo[pstData->u32LanguageEleNum].u8AudioType = (HI_U8)getBits(pu8Buff, 0, 24, 8);
        memset(u8StrCode, 0x00, 3*sizeof(HI_U8));
        ParseAudio_LanguageCodeTrans(pstData->astLanguageInfo[pstData->u32LanguageEleNum].au8ISO639_Language_Code, HI_LANG_CODE_LEN, u8StrCode);
        pstData->astLanguageInfo[pstData->u32LanguageEleNum].au8ISO639_Language_Code[0] = u8StrCode[0];
        pstData->astLanguageInfo[pstData->u32LanguageEleNum].au8ISO639_Language_Code[1] = u8StrCode[1];
        pstData->astLanguageInfo[pstData->u32LanguageEleNum].au8ISO639_Language_Code[2] = u8StrCode[2];
        pstData->u32LanguageEleNum++;
        pu8Buff += 4;
        u8Length -= 4;
    }
    return;
...
static HI_VOID ParsePMTDesc( const HI_U8* pu8DesData, HI_S32 s32DescLength, PMT_TB* pstruProg, HI_BOOL bParseAudio)
{
    HI_U8 u8Tag = 0;
    HI_U8 u8Length = 0;
    HI_U8 u8CntIndex = 0;
    const HI_U8 *pu8Data = HI_NULL;
    PMT_DESC_ISO639_LANGUAGE_S stAudioLang;
    if ((HI_NULL == pu8DesData) || (HI_NULL == pstruProg))
    {
        return;
    }
    ...
    while (s32DescLength > 0)
    {
        ...
        switch (u8Tag)
        {
            ...
            {
                if (HI_FALSE == bParseAudio)
                    break;
                memset(&stAudioLang, 0x00, sizeof(stAudioLang));
                ParseAudioLunguage(pu8Data, u8Length + 2, &stAudioLang);
                pstruProg->Audioinfo[pstruProg->u16AudoNum].u8AudLang[0] = stAudioLang.astLanguageInfo[0].au8ISO639_Language_Code[0];
                pstruProg->Audioinfo[pstruProg->u16AudoNum].u8AudLang[1] = stAudioLang.astLanguageInfo[0].au8ISO639_Language_Code[1];
                pstruProg->Audioinfo[pstruProg->u16AudoNum].u8AudLang[2] = stAudioLang.astLanguageInfo[0].au8ISO639_Language_Code[2];
                if (3 == stAudioLang.astLanguageInfo[0].u8AudioType)
                {
                    pstruProg->Audioinfo[pstruProg->u16AudoNum].u16ADType = 1;
                }
                break;
            }
            ...
            {
                HI_U8 extension_tag;
                extension_tag = pu8DesData[0];
                if (SUPPLEMENTARY_AUDIO_DESCRIPTOR == extension_tag)
                {
                    if ((pu8DesData[1] >> 7) == 0x0)
                    {
                        pstruProg->Audioinfo[pstruProg->u16AudoNum].u16ADType = 1;
                    }
                }
                if (AC4_DESCRIPTOR == extension_tag)
                {
                    pstruProg->Audioinfo[pstruProg->u16AudoNum].u32AudioEncType = HA_AUDIO_ID_MS12_AC4;
                }
            }
            default:
                break;
...
static HI_BOOL IsDTSStream(const HI_U8 *buf, HI_U32 len)
{
    while (len >= 2)
    {
        HI_U32 tag      = buf[0];
        ...
        switch (tag)
        {
            case REGISTRATION_DESCRIPTOR:
                if ( (STREAM_TYPE_DTS1_AUDIO_IDENTIFY == htonl(*((HI_U32*)buf)))
                  || (STREAM_TYPE_DTS2_AUDIO_IDENTIFY == htonl(*((HI_U32*)buf)))
                  || (STREAM_TYPE_DTS3_AUDIO_IDENTIFY == htonl(*((HI_U32*)buf))))
                {
                    return HI_TRUE;
                }
                break;
...
static HI_BOOL IsDRAStream(const HI_U8 *buf, HI_U32 len)
{
    while (len >= 2)
    {
        HI_U32 tag      = buf[0];
...
HI_S32 SRH_ParsePMT ( const HI_U8 *pu8SectionData, HI_S32 s32Length, HI_U8 *pSectionStruct)
{
   ...
   // printf("pcr pid = %#x, deslength:%d, s32Length = %d\n", u16Pid, u16DesLen, s32Length);
    ...
    if (u16DesLen > 0)
    {
        ParsePMTDesc(pu8SectionData, u16DesLen, pPmtTb, HI_FALSE);
        ...
        case STREAM_TYPE_HEVC_VIDEO:
        {
           // printf("video stream type is %#x, pid=%#x\n", u8StreamType, u16Pid);
            if (pPmtTb->u16VideoNum < PROG_MAX_VIDEO)
            {
                ...
                {
                    pPmtTb->Videoinfo[pPmtTb->u16VideoNum].u32VideoEncType = HI_UNF_VCODEC_TYPE_AVS2;
                }
...
#endif
        {
           // printf("audio stream type is %#x, pid = %#x\n", u8StreamType, u16Pid);
            if(pPmtTb->u16AudoNum < PROG_MAX_AUDIO)
            {
                ...
                if (u16DesLen > 0)
                {
                    ParsePMTDesc(pu8SectionData, u16DesLen, pPmtTb, HI_TRUE);
                    pu8SectionData += u16DesLen;
                    s32Length -= u16DesLen;
                }
            ...
            if (u16DesLen > 0 )/* subtitling stream info */
            {
                ...
                else
                {
                    if (pPmtTb->u16AudoNum < PROG_MAX_AUDIO)
                    {
                        ...
                        else if (IsDTSStream(pu8SectionData, u16DesLen))
                        {
                            pPmtTb->Audioinfo[pPmtTb->u16AudoNum].u32AudioEncType = HA_AUDIO_ID_DTSHD;
                        }
                        else if (IsDRAStream(pu8SectionData, u16DesLen))
                        {
                            pPmtTb->Audioinfo[pPmtTb->u16AudoNum].u32AudioEncType = HA_AUDIO_ID_DRA;
                        }
                        else
                        {
                            pPmtTb->Audioinfo[pPmtTb->u16AudoNum].u32AudioEncType = HA_AUDIO_ID_MP3;
    ...
    for (i = 0; i < pat_tb.u16ProgNum; i++)
    {
        ...
        for(j=0; j<pmt_tb.u16AudoNum; j++)
        {
            ...
            if (j < PROG_MAX_AUDIO)
            {
                ...
                pProgTable->proginfo[u32Promnum].Audioinfo[j].u16ADType = pmt_tb.Audioinfo[j].u16ADType;
                pProgTable->proginfo[u32Promnum].Audioinfo[j].u8AudLang[0] = pmt_tb.Audioinfo[j].u8AudLang[0];
                pProgTable->proginfo[u32Promnum].Audioinfo[j].u8AudLang[1] = pmt_tb.Audioinfo[j].u8AudLang[1];
                pProgTable->proginfo[u32Promnum].Audioinfo[j].u8AudLang[2] = pmt_tb.Audioinfo[j].u8AudLang[2];
            }
