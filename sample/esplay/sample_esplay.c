...
HI_S32 main(HI_S32 argc, HI_CHAR* argv[])
{
    ...
    if (strcasecmp("null", argv[1]))
    {
        ...
        else if (!strcasecmp("avs2", argv[2]))
        {
            VdecType = HI_UNF_VCODEC_TYPE_AVS2;
        }
    ...
    if (strcasecmp("null", argv[3]))
    {
        ...
        else if (!strcasecmp("g726", argv[4]))
        {
            AdecType = HA_AUDIO_ID_G726;
        }
        else if (!strcasecmp("vorbis", argv[4]))
        {
            AdecType = HA_AUDIO_ID_VORBIS;
        }
        else if (!strcasecmp("opus", argv[4]))
        {
            AdecType = HA_AUDIO_ID_OPUS;
        }
        else if (!strcasecmp("adpcm", argv[4]))
        {
            AdecType = HA_AUDIO_ID_ADPCM;
        }
