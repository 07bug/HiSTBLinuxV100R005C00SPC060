...
******************************************************************************
  File Name     	: es20_eglCreateSyncKHR.c
...
int main(void)
{
    ...
    for(i = 0; i < 200; i++)
    {
        ...
        {
            ret = eglClientWaitSyncKHR(egl_display, egl_sync, EGL_SYNC_FLUSH_COMMANDS_BIT_KHR, 20000000);
            if(EGL_TIMEOUT_EXPIRED_KHR == ret)
            {
                fprintf(stderr, "fence TIMEOUT\n");
            }
        ...
        {
            fprintf(stderr, "fence can not be created\n");
        }
    }
