...
HI_S32 save_picture(HI_HANDLE hAvplay, HI_UNF_AVPLAY_I_FRAME_S *pIframe, HI_CHAR* FileName)
{
    HI_S32 Ret;
    ...
    MemSurfaceInfo.Width        = stCapPic.u32Width;
    MemSurfaceInfo.Height       = stCapPic.u32Height;
    MemSurfaceInfo.PixelFormat  = HIGO_PF_BUTT;
    switch (stCapPic.enVideoFormat)
    {
        ...
        case HI_UNF_FORMAT_YUV_SEMIPLANAR_400 :
            ...
            break;
        default :
            sample_printf("[%s] not support format %d\n", __FUNCTION__, stCapPic.enVideoFormat);
    }

    if (HIGO_PF_BUTT != MemSurfaceInfo.PixelFormat)
    {
        MemSurfaceInfo.Pitch[0]     = stCapPic.stVideoFrameAddr[0].u32YStride;
        MemSurfaceInfo.Pitch[1]     = stCapPic.stVideoFrameAddr[0].u32CStride;
        MemSurfaceInfo.pPhyAddr[0]  = (HI_CHAR*)HI_NULL + stCapPic.stVideoFrameAddr[0].u32YAddr;
        MemSurfaceInfo.pPhyAddr[1]  = (HI_CHAR*)HI_NULL + stCapPic.stVideoFrameAddr[0].u32CAddr;
        ...
        EncAttr.ExpectType   = HIGO_IMGTYPE_JPEG;
...
HI_S32 update_logo(HI_HANDLE hAvplay, HI_UNF_AVPLAY_I_FRAME_S *pIframe)
{
    ...
    Ret = HI_GO_EncodeToMem(MemSurface, pTmpBuf, MaxSize, &LogoSize, &EncAttr);
    if (HI_SUCCESS != Ret)
    {
    ...
    if(HI_SUCCESS != Ret)
    {
        sample_printf("ERR: HI_UNF_PDM_UpdateBaseParam, Ret = %#x\n", Ret);
    }

    Ret = HI_UNF_PDM_UpdateLogoContent(pTmpBuf, LogoSize);
    if(HI_SUCCESS != Ret)
    {
        sample_printf("ERR: HI_UNF_PDM_UpdateLogoContent, Ret = %#x\n", Ret);
        goto FREE_BUF;
    }

    stLogoParam.bLogoEnable = HI_TRUE;
    stLogoParam.u32LogoLen = LogoSize;

    Ret = HI_UNF_PDM_UpdateLogoParam(&stLogoParam);
    if(HI_SUCCESS != Ret)
    {
        sample_printf("ERR: HI_UNF_PDM_UpdateLogoParam, Ret = %#x\n", Ret);
        goto FREE_BUF;
    }
...
HI_S32 main(HI_S32 argc,HI_CHAR** argv)
{
    ...
    else
    {
        ...
        while (1)
        {
            ...
            if ('s' == InputCmd[0])
            {
                sprintf(FileName, "Iframe_picture.jpg");
                Ret = save_picture(hAvplay, &stIFrame, FileName);
                if (HI_SUCCESS != Ret)
                {
            ...
            {
                printf("update logo \n");
                Ret = update_logo(hAvplay, &stIFrame);
                if (HI_SUCCESS != Ret)
                {
