...
static HI_S32 setCsa2Cw(HI_HANDLE hKey)
{
    ...
    HI_UNF_KLAD_LEVEL_E enStage;

    printf("use csa2 keyladder\n");

    if (HI_UNF_KLAD_ALG_TYPE_TDES == g_kladAlg)
    {
        HI_UNF_ADVCA_SetDVBAlg(HI_UNF_KLAD_ALG_TYPE_TDES);
        printf("Keyladder Alg  is TDES\n");

    }
    else
    {
        HI_UNF_ADVCA_SetDVBAlg(HI_UNF_KLAD_ALG_TYPE_AES);
    ...
    if (enStage >= HI_UNF_KLAD_LEVEL2)
    {
        ...
        Ret |= HI_UNF_ADVCA_SetDVBSessionKey(HI_UNF_KLAD_LEVEL1, g_u8SessionKey1);
        if (HI_SUCCESS != Ret)
        {
    ...
    if (enStage >= HI_UNF_KLAD_LEVEL3)
    {
        ...
        Ret |= HI_UNF_ADVCA_SetDVBSessionKey(HI_UNF_KLAD_LEVEL2, g_u8SessionKey2);
        if (HI_SUCCESS != Ret)
        {
            printf("call HI_UNF_ADVCA_SetDVBSessionKey failed\n");
            return HI_FAILURE;
...
static HI_S32 setCsa3Cw(HI_HANDLE hKey)
{
    ...
    if (HI_UNF_KLAD_ALG_TYPE_TDES == g_kladAlg)
    {
        Ret = HI_UNF_ADVCA_SetCSA3Alg(HI_UNF_KLAD_ALG_TYPE_TDES);
        printf("Keyladder Alg  is TDES\n");
    }
    else
    {
        Ret = HI_UNF_ADVCA_SetCSA3Alg(HI_UNF_KLAD_ALG_TYPE_AES);
        printf("Keyladder Alg  is AES\n");
    }
    ...
    if (enStage >= HI_UNF_KLAD_LEVEL2)
    {
        ...
        Ret = HI_UNF_ADVCA_SetCSA3SessionKey(HI_UNF_KLAD_LEVEL1, g_u8SessionKey1);
        if (HI_SUCCESS != Ret)
        {
    ...
    if (enStage >= HI_UNF_KLAD_LEVEL3)
    {
        ...
        Ret = HI_UNF_ADVCA_SetCSA3SessionKey(HI_UNF_KLAD_LEVEL2, g_u8SessionKey2);
        if (HI_SUCCESS != Ret)
        {
            printf("call HI_UNF_ADVCA_SetCSA3SessionKey failed\n");
            return HI_FAILURE;
...
static HI_S32 setCw(HI_HANDLE hKlad,HI_HANDLE hKey)
{
    ...
    if(HI_UNF_KLAD_COMMON_CA_TYPE_CSA2 == g_kladAlg)
    {
        stKladAttr.enEngine = HI_UNF_KLAD_TARGET_ENGINE_PAYLOAD_CSA2;
    }
    else if(HI_UNF_KLAD_COMMON_CA_TYPE_CSA3 == g_kladAlg)
    {
    ...
    if (enStage >= HI_UNF_KLAD_LEVEL2)
    {
        ...
        memcpy(sessionkey.au8Key,g_u8SessionKey1,16);
    ...
    if (HI_SUCCESS != Ret)
    {
        printf("call HI_UNF_KLAD_SetContentKey Odd failed,ret = 0x%x\n",Ret);
        return HI_FAILURE;
    }
    Ret = HI_UNF_KLAD_SetContentKey(hKlad,&EvenContentkey);
    if (HI_SUCCESS != Ret)
    {
        printf("call HI_UNF_KLAD_SetContentKey Even failed,ret = 0x%x\n",Ret);
        return HI_FAILURE;
    }
...
static HI_S32 setClearCw(HI_HANDLE hKlad,HI_HANDLE hKey)
{
    ...
    get_key_value("CLEARODDKEY", g_u8ClearOddKey, 16);
    get_key_value("CLEAREVENKEY", g_u8ClearEvenKey, 16);

    printBuffer("Clear Odd key :", g_u8ClearOddKey, 16);
    printBuffer("Clear enen key:", g_u8ClearEvenKey, 16);
...
static HI_S32 AVPLAY_AttachDescramble(HI_HANDLE hAvplay,HI_HANDLE *phKey)
{
    ...
    if (HI_UNF_KLAD_COMMON_CA_TYPE_CSA2 == g_KladType)              //KEYLADDER_TYPE_CSA2
    {
        stDesramblerAttr.enDescramblerType = HI_UNF_DMX_DESCRAMBLER_TYPE_CSA2;
    }
    else if (HI_UNF_KLAD_COMMON_CA_TYPE_CSA3 == g_KladType)         //KEYLADDER_TYPE_CSA3
    {
...
HI_S32 main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    if (argc < 2)
    {
        ...
        printf("Example: %s cw_key_config.ini 618 6875 64 1080i_50 0\n", argv[0]);
        return HI_FAILURE;
    }
    ...
    Ret = HI_UNF_KLAD_Attach(hKlad,hDescramber);
    if(Ret != HI_SUCCESS)
    {
    ...
    Ret = AVPLAY_SetCW(hKlad, hDescramber);
    if (Ret != HI_SUCCESS)
    {
        printf("call AVPLAY_SetCW 1 failed. ret = 0x%x\n",Ret);
        goto KLAD_DETACH;
    }
    ...
    while(1)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            printf("call AVPLAY_SetCW 2 failed. ret = 0x%x\n",Ret);
            goto KLAD_DETACH;
        }
...
KLAD_DETACH:
    HI_UNF_KLAD_Detach(hKlad,hDescramber);
