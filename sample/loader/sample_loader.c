...
static HI_S32 UpdateDeviceInfo(LDDB_DEVICE_INFO_S* pstDeviceInfo)
{
    ...
    hFlashHandle = LOADER_FLASH_OpenByName(LOADER_D_FLASHMAP_DEVICEINFO);
    if (HI_INVALID_HANDLE == hFlashHandle)
    {
        LOADER_PrintFuncErr(LOADER_FLASH_OpenByName, s32Ret);
        return -1;
    }
    ...
    pcFlashData = malloc(stFlashInfo.u32RBlockSize);
    ...
    memset(pcFlashData, 0x00, stFlashInfo.u32RBlockSize);
   ...
   /* reserve MAC , but unused for loader */
    ...
    if (stFlashInfo.u32RBlockSize < (pcWritePosition - pcFlashData))
    {
    ...
    s32Ret = LOADER_FLASH_Write(hFlashHandle, 0ULL, (HI_U8*)pcFlashData, stFlashInfo.u32RBlockSize);
    if (HI_SUCCESS != s32Ret)
    {
