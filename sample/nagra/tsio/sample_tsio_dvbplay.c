...
HI_S32 HIADP_TSIO_Init(HI_U32 TunerID, HI_U32 ProgNum)
{
    HI_S32 Ret = HI_FAILURE;
    HI_UNF_TSIO_TSIPORT_ATTR_S  TsiAttrs;
    ...
    if (FIRST_TUNER == TunerID)
        TsiAttrs.enDmxPort = HI_DEMUX_PORT;
    else if (SECOND_TUNER == TunerID)
        TsiAttrs.enDmxPort = HI_DEMUX1_PORT;
    else if (THIRD_TUNER == TunerID)
        TsiAttrs.enDmxPort = HI_DEMUX2_PORT;
    else if (FOURTH_TUNER == TunerID)
        TsiAttrs.enDmxPort = HI_DEMUX3_PORT;

    Ret = HI_UNF_DMX_AttachTSPort(0, HI_UNF_DMX_PORT_TSIO_0);
    if (HI_SUCCESS != Ret)
    {
    ...
    Ret = HI_UNF_TSIO_CreateTSIPort(HI_UNF_TSIO_ANY_TSI_PORT, &TsiAttrs, &hPtHandle);
    if (HI_SUCCESS != Ret)
    {
        printf("call HI_UNF_TSIO_CreateTSIPort failed.\n");
        goto out2;
    }
    ...
    Ret = HI_UNF_TSIO_AttachTSIInput(hTsio, hPtHandle);
    if (Ret != HI_SUCCESS)
    {
        printf("call HI_UNF_TSIO_AttachTSIInput failed.\n");
        goto out4;
    }
...
out3:
    HI_UNF_TSIO_DestroyTSIPort(hPtHandle);
out2:
    HI_UNF_TSIO_DeInit();
out1:
