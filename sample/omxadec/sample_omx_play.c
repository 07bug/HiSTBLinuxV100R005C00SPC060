...
static HI_BOOL  g_bSpecialOmxDec = HI_FALSE;
...
OMX_STRING HA_OMX_DecTable[] =
{
    "OMX.hisi.audio.aacdec",
    "OMX.hisi.audio.mp3dec",
    "OMX.hisi.audio.ddpdec",
    "OMX.hisi.audio.dradec",
    "OMX.hisi.audio.dtsdec",
    "OMX.hisi.audio.vorbisdec",
    "OMX.hisi.audio.cookdec",
    "OMX.hisi.audio.ffmpegdec",
    "OMX.hisi.audio.opusdec",
    NULL,
};
            ...
            switch (*(p_input))
            {
                ...
                case 'T':
                    if (count + 1 < argc)
                    {
                        ...
                        else if (!strcasecmp("dra", p_argv[count + 1]))
                        {
                            pAppPriData->strDecoder = HA_OMX_DecTable[3];
                        }
                        else if (!strcasecmp("dts", p_argv[count + 1]))
                        {
                            pAppPriData->strDecoder = HA_OMX_DecTable[4];
                        }
                        else if (!strcasecmp("vorbis", p_argv[count + 1]))
                        {
                            pAppPriData->strDecoder = HA_OMX_DecTable[5];
                        }
                        else if (!strcasecmp("cook", p_argv[count + 1]))
                        {
                            pAppPriData->strDecoder = HA_OMX_DecTable[6];
							g_bSpecialOmxDec = HI_TRUE;
                        }
                        else if (!strcasecmp("ffmpeg", p_argv[count + 1]))
                        {
                            pAppPriData->strDecoder = HA_OMX_DecTable[7];
                        }
                        else if (!strcasecmp("opus", p_argv[count + 1]))
                        {
                            pAppPriData->strDecoder = HA_OMX_DecTable[8];
                        }
                        else
                        {
...
HI_VOID OMX_ProcSpcialOmxDec(OMX_HANDLETYPE audiodechandle)
{
    OMX_AUDIO_PARAM_RATYPE stRaType;
    OMX_ERRORTYPE  error;
    OMX_VERSIONTYPE version;

    version.nVersion = 16777219;

    stRaType.nSamplingRate = 44100;
    stRaType.nBitsPerFrame = 128;
    stRaType.nChannels = 2;
    stRaType.nVersion = version;
    stRaType.nSamplePerFrame = 2048;
    stRaType.nNumRegions = 37;
    stRaType.nCouplingStartRegion = 2;
    stRaType.nCouplingQuantBits = 4;

    error = OMX_SetParameter(audiodechandle, OMX_IndexParamAudioRa, (OMX_PTR)(&stRaType));
    if (error != OMX_ErrorNone)
    {
...
int main(int argc, char** argv)
{
    ...
    if(g_bSpecialOmxDec == HI_TRUE)
    {
       OMX_ProcSpcialOmxDec(audiodechandle);
    }
