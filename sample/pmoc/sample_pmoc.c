...
HI_S32  main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    HI_U32 u32PowerGpioNumber = 0;

    HI_BOOL bCECEnable = 0;

    HI_U32 u32TimeToDeepStandby = 0;

    if ((argc <= 1) || (argc > 13))
    {
        printf("Usage:  %s [TimerCount] [Display] [Running Count] [Scene] [IR Type] [Keyled Type] [Eth | USB | WiFi Type] [Reset] [WakeUp GPIO No.] [HDMI CEC] [TimeToDeepStandby] [PowerCtrl GPIO No.]\n"
               ...
               "\tScene: 0: standard; 1: eth forward(unsupported).           \n"
               ...
               "\tEth|USB: 0: Disable; 1: USB Wakeup; 2: Eth Single Packet; 3: Eth Magic Packet; 4: Eth Frame Filter. \n"
               "\tReset: 0: resume to ddr; 1: resume to reset system. \n"
               "\tWakUp GPIO No.: 0: disable gpio wake up ; other: gpio wake up number. \n"
               "\tHDMI CEC: 0: disable HDMI CEC; 1: enable HMDI CEC. \n "
               "\tTimeToDeepStandby: the time of eth type to deep standby type(in Second). \n "
               "\tPowerCtrl GPIO No.: 0: do not power off WiFi or Eth; other: the gpio number to power off WiFi or Eth. \n",
                argv[0]);
        printf("Example: %s 0 2 1 0 0 2 0 0 0 0 0 0\n", argv[0]);
        return HI_FAILURE;
    }
    ...
    if (argc > 10)
    {
        bCECEnable = strtol(argv[10],NULL,0);
    }

    if (argc > 11)
    {
        u32TimeToDeepStandby = strtol(argv[11],NULL,0);
    }

    if (argc > 12)
    {
        u32PowerGpioNumber = strtol(argv[12],NULL,0);
    }
    ...
    else if (2 == u8EthUsbStandbyMode)
    {
        ...
        wakeup.stNetwork.u32TimeToDeepStandby = u32TimeToDeepStandby;
    }
    ...
    wakeup.bCECWakeUpEnable = bCECEnable;
    ...
    ret = HI_UNF_PMOC_SetPwrOffGpio(u32PowerGpioNumber, HI_FALSE);
    if (HI_SUCCESS != ret)
    {
    ...
    for (i = 0; i < g_RunningCount; i++)
    {
        ...
        else if (actual == HI_UNF_PMOC_WKUP_CEC)
        {
            HI_INFO_PMOC("wakeup by CEC !!!\n");
        }
        else
        {
