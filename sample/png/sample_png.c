...
HI_S32 PNG_ShowSurface(PNG_RESULT_S *pstSurface, HI_U32 u32PosX, HI_U32 u32PosY)
{
    int fd;
    ...
    mapped_mem = mmap(NULL, fix.smem_len,
    ...
    image = HI_MMZ_Map(pstSurface->u32PhyAddr, 0);
    if(NULL == image)
    {
        munmap(mapped_mem, fix.smem_len);
        close(fd);
        return -1;
    }

    height = pstSurface->u32Height < var.yres ? pstSurface->u32Height : var.yres;
    rowbytes = pstSurface->u32RowBytes < fix.line_length ? pstSurface->u32RowBytes : fix.line_length;
    for (i = 0; i < height; i++)
    {
        memcpy(mapped_mem + i * fix.line_length, image + i * pstSurface->u32Stride, rowbytes);
    }

    printf("Please press Enter to show next picture!\n");
    getchar();
    HI_MMZ_Unmap(pstSurface->u32PhyAddr);
    munmap(mapped_mem, fix.smem_len);
    close(fd);
    return 0;
}
...
static HI_S32 PNG_Decode(HI_CHAR *pszFilepath, PNG_RESULT_S *pstDecResult)
{
    ...
    pstDecResult->u32PhyAddr = u32Phyaddr;
    pstDecResult->u32Width = width;

    PNG_ShowSurface(pstDecResult, 70, 20);
