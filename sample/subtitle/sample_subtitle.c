...
#define HI_SUBT_MAX_INTERVAL (50 * 1000) /* max interval time(ms) of the subtitle */
...
static HI_S32 StartSubtitle(PMT_COMPACT_TBL *pSubTitleProgTbl, HI_U8 u8ProgNo)
{
    ...
    if (0 == hUnfSO)
    {
        ...
        (HI_VOID)HI_UNF_SO_SetMaxInterval(hUnfSO, HI_SUBT_MAX_INTERVAL);
    }
...
static HI_S32 InjectTsTask(HI_VOID *args)
{
    ...
    while(g_bTaskQuit == HI_FALSE)
    {
        ...
        s32Ret = HIADP_DMX_PushTsBuffer(hTsBuffer, &StreamBuf, 0, Readlen);
        if (s32Ret != HI_SUCCESS )
        {
