...
{
    HI_UNF_DMX_DATA_S stSection[SUBT_FLTR_SECTION_NUM] = { {0} };
    ...
    while (HI_TRUE == s_bSubtDataThreadReady)
    {
        ...
        for (i = 0; i < DATA_RECV_MAX_NUM; i++)
        {
            ...
            if (HI_SUCCESS != HI_UNF_DMX_AcquireBuf(pstSubtData->hChannelID, SUBT_FLTR_SECTION_NUM, &u32SectionNum, stSection, 0))
            {
                pthread_mutex_unlock(&s_stSubtDataMutex);
                continue;
            }

            if ((u32SectionNum == 0) || (u32SectionNum > SUBT_FLTR_SECTION_NUM))
            {
                pthread_mutex_unlock(&s_stSubtDataMutex);
                continue;
            }
            ...
            {
                for (j = 0; j < u32SectionNum; j++)
                {
                    (HI_VOID)pstSubtData->stInstallParam.pfnCallback(pstSubtData->stInstallParam.u32UserData, stSection[j].pu8Data, stSection[j].u32Size);
                }
            }
