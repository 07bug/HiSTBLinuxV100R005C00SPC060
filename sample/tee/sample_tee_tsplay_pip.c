...
HI_VOID TsTthread(HI_VOID *args)
{
    ...
    {
        pthread_mutex_lock(pTsParam->pTsMutex);
        Ret = HI_UNF_DMX_GetTSBuffer(pTsParam->g_TsBuf, 188*50, &StreamBuf, 1000);
        if (Ret != HI_SUCCESS )
        {
            pthread_mutex_unlock(pTsParam->pTsMutex);
            continue;
        }
        ...
        Ret = HIADP_DMX_PushTsBuffer(pTsParam->g_TsBuf, &StreamBuf, 0, Readlen);
        if (Ret != HI_SUCCESS )
        {
...
HI_S32 main(HI_S32 argc, HI_CHAR *argv[])
{
    ...
    pthread_mutex_init(&TsMutex[0],NULL);
    pthread_mutex_init(&TsMutex[1],NULL);
    ...
    for (i = 0; i < AVPLAY_NUM; i++)
    {
        ...
        stTsParam[i].pTsMutex = &TsMutex[i];
        printf("g_TsBuf[%d] = %d\n", i, g_TsBuf[i]);
        ...
        pthread_mutex_unlock(&TsMutex[i]);
...
TSBUF_FREE:
    ...
    pthread_mutex_destroy(&TsMutex[0]);
    pthread_mutex_destroy(&TsMutex[1]);
