...
/*
** Add aspect ratio by sdk @2015.08.03
*/
...
/*
** Add CTS2.0 test case by sdk @2015.08.03
...
static HI_U32 hdmi_spd_info( HI_U32 u32Enable );
...
static HI_U32 hdmi_hdcp(HI_U32 hdcpmode,HI_U32 u32DynamicMode);
...
static HI_S32 hdmi_test_event(HI_UNF_HDMI_ID_E enHdmi, HI_U32 mode);
static HI_S32 hdmi_test_load(HI_UNF_HDMI_ID_E enHdmi, HI_U32 u32Mode ,HI_CHAR *pu8FileName);
...
static HI_U32 Hdmi_ParseArg(HI_U32 argc)
{
    ...
    if ( (0 == strcmp("help", (char *)args[0])) || (0 == strcmp("h", (char *)args[0])) )
    {
        ...
        printf("\t hdmi_spd_info     set spd infoframe\n");
        ...
        printf("\t hdmi_event        hdmi event test\n");
		printf("\t hdmi_load         load hdcp14 key or srm file\n");
    }
    ...
    else if (0 == strcmp("hdmi_spd_info", (char *)args[0]))
    {
        HI_U8 reversecolor_index;
        /* change video reverse color space: normal/reserve */
        if (1 >= argc)
        {
            printf("Usage:hdmi_spd_info argv\n");
            printf("argv: 0   disable\n");
            printf("argv: 1   enable\n");
            return RetError;
        }
        reversecolor_index = (HI_U8)Hdmi_String_to_Integer(args[1]);
        RetError = hdmi_spd_info(reversecolor_index);
    }
    ...
    else if (0 == strcmp("hdmi_hdcp", (char *)args[0]))
    {
        ...
        if (1 >= argc)
        {
            printf("Usage:hdmi_hdcp hdcpmode [b_dynamic]\n");
            printf("hdcpmode: \n");
            ...
            printf("b_dynamic: \n");
            printf("        0: static enable hdcp(default)\n");
            printf("        1: dynamic enable hdcp\n");

            return RetError;
        }
        RetError = hdmi_hdcp(Hdmi_String_to_Integer(args[1]),Hdmi_String_to_Integer(args[2]));
    }
    else if (0 == strcmp("hdmi_cecautoping", (char *)args[0]))
    {
        /* CEC command, to be define */
        RetError = hdmi_test_cec_autoping(enTest_Hdmi_ID);
    }
    else if (0 == strcmp("cec_enable", (char *)args[0]))
    {
        u32Arg = 0;
        hdmi_test_cec_enable(enTest_Hdmi_ID);
        if(argc >= 2)
        {
            u32Arg = Hdmi_String_to_Integer(args[1]);
        }
        HDMI_CEC_LogSet((u32Arg == 0) ? HI_FALSE : HI_TRUE);
    }
    else if (0 == strcmp("cec_disable", (char *)args[0]))
    {
        hdmi_test_cec_disable(enTest_Hdmi_ID);
    }
    else if (0 == strcmp("cec_setcm", (char *)args[0]))
    {
        HI_U8 index, u8Cmd;
        HDMI_TEST_CEC_CMD_S stCecCmd;

        if((argc < 2) || (argc >= 3 && argc < 5))
        {
            printf("Usage:cec_setcm cmd\n");
            printf("At present, we support:\n");
            printf("0   :       //0x36 <Standy> standy broadcast :ok\n");
            printf("1   :       //0x04 <Image View On> invoke TV out of standy :ok\n");
            printf("2   :       //0x0d <Text View On> invoke TV out of standy :ok\n");
            printf("3   :       //0x82 <Active Source> brocast message with Physical Address :ok\n");
            printf("4   :       //0x9f <Get CEC Version> :ok\n");
            printf("5   :       //0x91 <Get Menu Language> :ok\n");
            printf("6   :       //0x83 <Give Physical Address> :ok\n");
            printf("7   :       //0x8c <Give Device Vendor ID> :ok\n");
            printf("8   :       //0x87 <Device Vendor ID> brocast the vendor ID of this device :ok\n");
            printf("9   :       //0x46 <Give OSD Name> :ok\n");
            printf("10  :       //0x64 <Set OSD String>:Maybe no support\n");
            printf("11  :       //0x8f <Give Device Power Status> :ok\n");
            printf("12  :       //0x44 <User Cotrol pressed> 0x41 <Set volume up> :ok\n");
            printf("13  :       //0x44 <User Cotrol pressed> 0x42 <Set volume down> :ok\n");
            printf("14  :       //0x44 <User Cotrol pressed> 0x43 <Set volume mute> :ok\n");
            printf("15  :       //0x70 <System Audio Mode Request> :ok\n");
            printf("16  :       //0x8e <Menu Status> 0x0[Activated] 0x1[Deativated] :ok\n");
            printf("17  :       //0x9e <CEC version> 0x4[Version 1.3a] 0x5[Version 1.4]:ok\n");
            printf("18-254:     //reserved \n");
            printf("255 :       //user define: cec_setcm 255 DestAdd Opcode OperandLength Operandx\n");
            return RetError;
        }

        memset(&stCecCmd, 0, sizeof(stCecCmd));

        u8Cmd = Hdmi_String_to_Integer(args[1]);
        if(u8Cmd < HDMI_TEST_CEC_USER_DEFINE)
        {
            stCecCmd.enCmd = u8Cmd;
        }
        else
        {
            stCecCmd.enCmd = HDMI_TEST_CEC_USER_DEFINE;
            stCecCmd.u8DestAddr = Hdmi_String_to_Hexadecimal(args[2]);
            stCecCmd.u8Opcode = Hdmi_String_to_Hexadecimal(args[3]);
            stCecCmd.u8Len = Hdmi_String_to_Hexadecimal(args[4]);

            if(argc < (stCecCmd.u8Len + 5) || stCecCmd.u8Len > MAX_CEC_CMD_LEN)
            {
                printf("input data error!\n");
                return HI_FAILURE;
            }
            /* CEC command */
            for(index = 0; index < stCecCmd.u8Len; index ++)
            {
                stCecCmd.u8Data[index] = Hdmi_String_to_Hexadecimal(args[5 + index]);
            }
        }

        RetError = hdmi_test_cec_setcmd(enTest_Hdmi_ID, &stCecCmd);
    ...
    else if (0 == strcmp("hdmi_event", (char *)args[0]))
    {
        /* hdmi event callback test */
        if (1 >= argc)
        {
            printf("Usage: hdmi_event mode\n");
            printf("mode: \n");
            printf("   0: disable \n");
            printf("   1: enable\n");
            return RetError;
        }
        RetError = hdmi_test_event(enTest_Hdmi_ID, Hdmi_String_to_Integer(args[1]));
    }
	else if(0 == strcmp("hdmi_load", (char *)args[0]))
	{
		RetError = hdmi_test_load(enTest_Hdmi_ID, Hdmi_String_to_Integer(args[1]),(HI_CHAR *)args[2]);
	}
...
static HI_U32 hdmi_spd_info( HI_U32 u32Enable )
{
    HI_U8                             i = 0;
    HI_U32                            RetError = HI_SUCCESS;
    HI_UNF_HDMI_ATTR_S                attr = {0};
    HI_UNF_HDMI_INFOFRAME_S           stInfoFrame = {0};
    HI_UNF_HDMI_SPD_INFOFRAME_S       *pstSpdInfoframe = HI_NULL;

    pstSpdInfoframe = &(stInfoFrame.unInforUnit.stSPDInfoFrame);
    printf("u32Enable:%10d\n",u32Enable);
    HI_UNF_HDMI_GetAttr(enTest_Hdmi_ID, &attr);
    attr.bEnableSpdInfoFrame = u32Enable ? HI_TRUE : HI_FALSE;
    HI_UNF_HDMI_SetAttr(enTest_Hdmi_ID, &attr);

    memset(&stInfoFrame,0,sizeof(stInfoFrame));

    stInfoFrame.enInfoFrameType = HI_INFOFRAME_TYPE_SPD;
    pstSpdInfoframe->u8VendorName[0] = 'h';
    pstSpdInfoframe->u8VendorName[1] = 'i';
    pstSpdInfoframe->u8VendorName[2] = 's';
    pstSpdInfoframe->u8VendorName[3] = 'i';

    pstSpdInfoframe->u8SrcDevInfo    = 0x01;      /* Digital STB  */

    for(i=1;i<17;i++)
        pstSpdInfoframe->u8ProductDescription[i-1] = i;
    ...
    printf("GetSpdInfo [VendorName]: %c%c%c%c\n",
        pstSpdInfoframe->u8VendorName[0],
        pstSpdInfoframe->u8VendorName[1],
        pstSpdInfoframe->u8VendorName[2],
        pstSpdInfoframe->u8VendorName[3]);
    printf("GetSpdInfo [ProductDesp]:");

    for(i=0;i<16;i++)
        printf("%d ",pstSpdInfoframe->u8ProductDescription[i]);

    printf("\nGetSpdInfo [SrcDevInfo]:0x%x\n",pstSpdInfoframe->u8SrcDevInfo);
...
static HI_U32 hdmi_attachIntf(HI_U32 u32dispChnIndex)
{
    ...
    HI_UNF_HDMI_Start(stHdmiIntf.unIntf.enHdmi);
...
HI_U32 hdmi_hdcp(HI_U32 hdcpmode,HI_U32 u32DynamicMode)
{
    HI_U32                      RetError = HI_SUCCESS;
#ifndef HI_HDCP_SUPPORT
    sample_common_printf("no config to support hdcp!\n");
#else

#if defined(HI_HDMI_SUPPORT_2_0)
    if(u32DynamicMode)
    {
        HI_UNF_HDMI_HDCP_CAP_S      stHdcpCap = {0};
        HI_UNF_HDMI_HDCP_PARAM_S    stParam = {0};

        switch(hdcpmode)
        {
            case HI_HDMI_HDCP_MODE_DISABLE:
                stParam.bHdcpEnable= HI_FALSE;

                break;
            case HI_HDMI_HDCP_MODE_AUTO:
                stParam.bHdcpEnable = HI_TRUE;//Enable HDCP
                stParam.enHdcpMode= HI_UNF_HDMI_HDCP_MODE_AUTO;
                break;
            case HI_HDMI_HDCP_MODE_HDCP14:
                stParam.bHdcpEnable = HI_TRUE;//Enable HDCP
                stParam.enHdcpMode = HI_UNF_HDMI_HDCP_MODE_1_4;
                break;
            case HI_HDMI_HDCP_MODE_HDCP22:
                stParam.bHdcpEnable = HI_TRUE;//Enable HDCP
                stParam.enHdcpMode = HI_UNF_HDMI_HDCP_MODE_2_2;
                break;
            default:
                break;
        }

        RetError = HI_UNF_HDMI_GetHdcpCapability(enTest_Hdmi_ID,&stHdcpCap);
        sample_common_printf("bHdcp14Support = %d\nbHdcp22Support = %d\n",stHdcpCap.bHdcp14Support,stHdcpCap.bHdcp22Support);
        if(RetError != HI_SUCCESS)
        {
            sample_common_printf("get Hdcp cap fail!\n");
        }

        RetError = HI_UNF_HDMI_HdcpEnable(enTest_Hdmi_ID,&stParam);
        if(RetError != HI_SUCCESS)
        {
            sample_common_printf("Hdcp enable fail!\n");
        }
...
HI_VOID hdmi_test_event_callbakc(HI_UNF_HDMI_EVENT_TYPE_E event, HI_VOID *pPrivateData)
{
    g_u32EventNo++;
    printf("\n %d event : ",g_u32EventNo);
    switch(event)
    {
...
static HI_S32 hdmi_test_event(HI_UNF_HDMI_ID_E enHdmi, HI_U32 mode)
{
    ...
    g_stCallbackFunc.pfnHdmiEventCallback = hdmi_test_event_callbakc;
    g_stCallbackFunc.pPrivateData = &g_stHdmiArgs;

    if(1 == mode)
    {
        s32Ret = HI_UNF_HDMI_RegCallbackFunc(HI_UNF_HDMI_ID_0, &g_stCallbackFunc);
    ...
    {
        s32Ret = HI_UNF_HDMI_UnRegCallbackFunc(HI_UNF_HDMI_ID_0, &g_stCallbackFunc);
    }

    return s32Ret;
}

#define HDMI_MAX_SIZE_HDCP14_KEY		332
#define HDMI_MAX_SIZE_SRM_DATA			5116
static HI_S32 hdmi_test_load(HI_UNF_HDMI_ID_E enHdmi, HI_U32 u32Mode ,HI_CHAR *pu8FileName)
{
...
#ifdef HI_HDMI_SUPPORT_2_0
	HI_UNF_HDMI_SRM_S		stSrm;
#endif
	if( (!pu8FileName) || ((u32Mode!=1) && (u32Mode!=2)))
	{
		printf("cmd format error!\n");
		printf("[Usage] hdmi_load  mode file_path\n"
			   "[mode ] 1  --hdcp14 key.\n"
			   "[mode ] 2  --srm revoction id file.\n"
			   "[example1] hdmi_load 1 /mnt/key/encryptedkey_332bytes\n"
			   "[example2] hdmi_load 2 /mnt/key/hdcp1_test.srm\n");

		return HI_FAILURE;
	}

	pu8Data = (HI_U8 *)malloc(HDMI_MAX_SIZE_SRM_DATA);
	if(!pu8Data)
	{
	...
	u32Len = fread(pu8Data, 1, HDMI_MAX_SIZE_SRM_DATA, pFile);
    fclose(pFile);

	switch(u32Mode)
	{
		case 1:
			if (u32Len != HDMI_MAX_SIZE_HDCP14_KEY)
			{
				printf("%s is error hdcp14 key file,u32Len=%u!\n",pu8FileName,u32Len);
				s32Ret = HI_FAILURE;
			}
			else
			{
				stLoadKey.u32KeyLength = HDMI_MAX_SIZE_HDCP14_KEY;
				stLoadKey.pu8InputEncryptedKey = pu8Data;
				s32Ret = HI_UNF_HDMI_LoadHDCPKey(enHdmi, &stLoadKey);
			}
			break;

		case 2:
#ifdef HI_HDMI_SUPPORT_2_0
			stSrm.u32SrmLen  = u32Len;
			stSrm.pu8SrmData = pu8Data;
			s32Ret = HI_UNF_HDMI_SetSrm(enHdmi, &stSrm);
#else
			s32Ret = HI_FAILURE;
			printf("not support srm in hdmi1.4!\n");
#endif
			break;
		default:
			break;
	...
	printf("Load file %s %s\n",pu8FileName,(s32Ret==HI_SUCCESS)?"success" : "fail");
	free(pu8Data);
    return s32Ret;
#else
	printf("error,Makefile config no support HDCP!\n");
	return HI_FAILURE;
#endif
}
