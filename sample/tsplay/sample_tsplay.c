...
static HI_U32      g_bEndOfStream = HI_FALSE;
...
HI_S32 ProcEventFunc(HI_HANDLE hAvplay, HI_UNF_AVPLAY_EVENT_E enEvent, HI_VOID *para)
{
    switch(enEvent)
    {
        case HI_UNF_AVPLAY_EVENT_EOS :
            printf("end of stream!\n");
            g_bEndOfStream = HI_TRUE;
            break;
        default:
            break;
    }
...
HI_S32 main(HI_S32 argc, HI_CHAR *argv[])
{
    ...
    Ret = HI_UNF_AVPLAY_RegisterEvent64(hAvplay, HI_UNF_AVPLAY_EVENT_EOS, ProcEventFunc);
    if (Ret != HI_SUCCESS)
    {
        printf("call HI_UNF_AVPLAY_RegisterEvent failed.\n");
        goto AVPLAY_DESTROY;
    }
    ...
    while (1)
    {
        ...
        if ('s' == InputCmd[0])
        {
            ...
            HI_UNF_AVPLAY_FlushStream(hAvplay,HI_NULL);

            while(!g_bEndOfStream)
            {
                usleep(100 * 1000);
...
AVPLAY_DESTROY:
    HI_UNF_AVPLAY_UnRegisterEvent(hAvplay, HI_UNF_AVPLAY_EVENT_EOS);
