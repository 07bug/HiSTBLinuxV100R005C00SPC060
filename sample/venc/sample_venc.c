...
void* SampleSendFrameRoutine(void* args)
{
    ...
    HI_U8* u32TmpData = malloc(3840 * 2160 * 3);
    ...
    while (!g_bSampleSendFrameStop)
    {
        ...
        stFrameinfo_release.u32Pts                 = pts++;
#endif
        if(g_bSampleSendFrameStop != HI_TRUE)
        {
...
HI_S32 main(HI_S32 argc, HI_CHAR* argv[])
{
    ...
    for (i = 0; i < chn_num; i++)
    {
        ...
        if (stVeAttr.u32Width > 2160 || stVeAttr.u32Height > 2160)
        {
            stVeAttr.enCapLevel       = HI_UNF_VCODEC_CAP_LEVEL_4096x2160;
            stVeAttr.u32StrmBufSize   = 4096 * 2160 * 3 / 2;
            stVeAttr.u32TargetBitRate = 5 * 1024 * 1024;            
        }
        else if (stVeAttr.u32Width > 1920  || stVeAttr.u32Height > 1088)
        {
            stVeAttr.enCapLevel       = HI_UNF_VCODEC_CAP_LEVEL_2160x2160;
            stVeAttr.u32StrmBufSize   = 2160 * 2160 * 3 / 2;
            stVeAttr.u32TargetBitRate = 5 * 1024 * 1024;            
        }
        else if (stVeAttr.u32Width > 1280)
        {
