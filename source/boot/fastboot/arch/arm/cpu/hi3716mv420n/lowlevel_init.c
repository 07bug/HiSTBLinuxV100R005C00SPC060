	...
	unsigned int which)
{
	...
	if (which & INIT_REG_ITEM1) {
		if (!pm) {
			/* write random DDR CA */
			writel(readl(REG_RNG_DATA), REG_DDRCA_RANDOM0);
			writel(readl(REG_RNG_DATA), REG_DDRCA_RANDOM1);
		}
...
void reg_init_module(unsigned int param)
{
	...
	unsigned int ca_flag = (0x2a13c812 == readl(0xf8ab0060))? 0:1;
	...
	uart_early_put_hex(module_type);
