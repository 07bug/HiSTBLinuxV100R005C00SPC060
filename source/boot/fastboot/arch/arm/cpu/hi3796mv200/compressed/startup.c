...
#endif
/******************************************************************************/
#define UNCOMPRESSED_DATA_OFFSET  0x500000
const unsigned int IMAGE_ENTRY = TEXT_BASE + UNCOMPRESSED_DATA_OFFSET;
...
void start_armboot(void)
{
	...
	mmu_init((MEM_BASE_DDR + 0x4000), MEM_BASE_DDR, TEXT_BASE + 2 * UNCOMPRESSED_DATA_OFFSET);
	dcache_enable(0);

	if (input_data[0] == 0x5D) {
		uart_early_puts("Uncompress");
		ret = decompress(input_data, image_data_len, pdst);
		if(ret) {
			uart_early_puts("FAILED\r\n");
		}
		else
			uart_early_puts("Ok\r\n");
	} else {
