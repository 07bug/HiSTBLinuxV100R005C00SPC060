...
extern char *jtag_recv_buf;
extern char *jtag_send_buf;

void jtag_download_process(void)
{
	int ret = -1;
	unsigned int cmd_len = 0;
	unsigned short cksum;
	...
	do {
		do {
	        count = 0;
	        while(!jtag_check_cmd_set_flag()) {
	                udelay(1000);
	                count++;
	                if (count > 1000) {
	                        serial_puts("Waiting XHEAD\n");
	                        count = 0;
	                }
	        }
			...
			if (jtag_recv_buf[0] != XHEAD) {
				printf("Invalid XHEAD %p[%X]\n", jtag_recv_buf, jtag_recv_buf[0]);
				goto head_fail;
			}

			/* crc check */
			cksum = calc_crc16((unsigned char*)jtag_recv_buf,3);
			if (cksum == ((jtag_recv_buf[3] << 8) | jtag_recv_buf[4])) {
				/* init */
				cmd_len = (unsigned short)((jtag_recv_buf[1] << 8) | jtag_recv_buf[2]) + 3;
				if (cmd_len >= CONFIG_JTAG_RECV_BUF_SIZE) {
					printf("Invalid cmd length 0x%X\n", cmd_len);
					goto head_fail;
				}
			...
			printf("XHEAD CRC[0x%X] error, expected 0x%X\n", cksum, ((jtag_recv_buf[3] << 8) | jtag_recv_buf[4]));
head_fail:
			/* init */
			cmd_len = 0;
		...
		do {
			...
			if (jtag_recv_buf[0] != XCMD) {
				serial_puts("Invalid XCMD\n");
				goto cmd_fail;
			}

			/* crc check */
			cksum = calc_crc16((unsigned char*)jtag_recv_buf, cmd_len - 2);
			if (cksum == ((jtag_recv_buf[cmd_len-2] << 8) | jtag_recv_buf[cmd_len-1])) {
				/* SEND: ack wait result */
				serial_putc(ACK);
				/* Just break to run cmd, and set cmd done flag after the cmd has run */
				break;
			}

			printf("XCMD CRC[0x%X] error, expected 0x%X\n", cksum, ((jtag_recv_buf[cmd_len-2] << 8) | jtag_recv_buf[cmd_len-1]));
...
cmd_fail:
		...
		/* clean crc */
		jtag_recv_buf[cmd_len-1] = 0;
		jtag_recv_buf[cmd_len-2] = 0;

		/* cmd process */
		printf("Run '%s'\n", jtag_recv_buf + 1);
		jtag_reset_send_buffer();
		ret = run_command((jtag_recv_buf+1), 0);
		if (ret < 0) {
			/* SEND: end flag */
			jtag_pr_tool("[EOT](ERROR)\n");
			jtag_set_cmd_result(EINVAL);
			jtag_set_cmd_done_flag();
		} else{
			/* SEND: end flag */
...
{
	char *recovery_bootargs = getenv("recoverybootargs");
	char *recovery_bootargs_buf = NULL;

	if (recovery_bootargs) {
		recovery_bootargs_buf = (char *)malloc(strlen(recovery_bootargs));
		if (recovery_bootargs_buf) {
			strncpy(recovery_bootargs_buf, recovery_bootargs, strlen(recovery_bootargs));
			setenv("bootargs", recovery_bootargs_buf);
			free(recovery_bootargs_buf);
		} else {
...
#elif defined(CONFIG_SBL_VXX_UXX) //CONFIG_SBL_VXX_UXX is defined for VXX Uxxxx, not compatible with CONFIG_JTAG_PRG_SUPPORT
	if (CONFIG_START_MAGIC == readl(REG_START_FLAG)
		|| CONFIG_START_MAGIC == readl(REG_SBL_START_FLAG)) {
		return 1;
	}
#endif
...
void download_boot_normal(int (*handle)(void))
{
		if (is_bootstrap()) {
			...
			if (is_jtag_mode()) {
				if (0 != jtag_init()) {
					serial_puts("JTAG init fail\n");
					hang();
				}
				jtag_pr_tool("start download process.\n");
				jtag_set_cmd_result(0);
				jtag_set_cmd_done_flag();
				jtag_download_process();
			}
#endif
			/* clear flag */
			writel(SELF_BOOT_TYPE_NONE, REG_START_FLAG);
			serial_puts("start download process.\n");
			bootstraptype = readl(REG_USB_UART_FLAG);
...
void download_boot_ca(int (*handle)(void))
{
	...
	if (SELF_BOOT_TYPE_UART != readl(REG_USB_UART_FLAG)) {
		return;
	}
...
void download_boot(int (*handle)(void))
{
	if (_HI3798C_V200 == get_chipid() || _HI3798M_V200 == get_chipid()
		|| _HI3796M_V200_15X15 == get_chipid()
		|| _HI3796M_V200_21X21 == get_chipid() || _HI3798M_V300 == get_chipid()
		|| _HI3716M_V450_21X21 == get_chipid() || _HI3798M_V310 == get_chipid()
		|| _HI3798M_V300_H == get_chipid())
...
void selfboot_from_udisk(void)
{
	...
	bootstraptype = readl(REG_USB_UART_FLAG);
	writel(SELF_BOOT_TYPE_NONE, REG_USB_UART_FLAG);
