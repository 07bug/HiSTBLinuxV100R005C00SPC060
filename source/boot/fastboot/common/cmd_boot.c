...
int do_arm64_go(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	*cpu_ctrl_reg = *cpu_ctrl_reg | 1;	/* enable 64bit instruction mode */
	...
	printf("### Start warm reset to enter arm64 mode, entry 0x%X\n", *cpu_addr_reg);
...
extern int do_sys_srst(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
...
U_BOOT_CMD(
	sreset, 1, 0,	do_sys_srst,
	"Perform System soft RESET of the CPU",
	""
);
#endif
#else
