...
extern int is_atf_trustedcore_img(char *buf);
extern int is_optee_img(char *buf);
...
static int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	|| defined(CONFIG_ARCH_HI3796MV2X) || defined (CONFIG_ARCH_HI3798MV310)
...
#if defined(CONFIG_HI3798MV2X_FPGA) || defined(CONFIG_ARCH_HI3796MV2X)
#ifndef CONFIG_UNIFIED_BOOT
	/* Enable System counter, this must be set in secure CPU. */
	...
	if (2 == argc) {
		...
		if (is_tee_type()) {
			if (is_trustedcore_img(buf) || is_atf_trustedcore_img(buf)
			    || is_optee_img(buf) || is_atf_optee_img(buf)) {
			...
			else if (!memcmp((char *)buf, HI_ADVCA_MAGIC, HI_ADVCA_MAGIC_SIZE)
				&& (is_trustedcore_img(buf + HI_ADVCA_HEADER_SIZE)
					|| is_atf_trustedcore_img(buf + HI_ADVCA_HEADER_SIZE)
					|| is_optee_img(buf + HI_ADVCA_HEADER_SIZE)
					|| is_atf_optee_img(buf + HI_ADVCA_HEADER_SIZE))) {
				printf("Boot Hisilicon ADVCA SecureOS Image ...\n");
				return do_load_secure_os((ulong)buf, HI_ADVCA_HEADER_SIZE, 0, 0, NULL);
			}
#endif
		}
