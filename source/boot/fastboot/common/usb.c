...
int get_usb2_controller_count(void);
int get_usb3_controller_count(void);
/***********************************************************************
 * wait_ms
 */
...
int usb_init(void)
{
	...
	for (i = 0; i < get_usb2_controller_count(); i++) {
		/* init low_level USB */
		printf("USB%d:   ", i);
...
#ifdef CONFIG_USB_XHCI
	for (i = 0; i < get_usb3_controller_count(); i++) {
		/* init low_level USB */
		printf("USB%d:   ", i);
		if (usb_lowlevel_init_ex(i, &ctrl)) {
			puts("lowlevel init failed\n");
			continue;
		}
		/*
			...
			void *buffer, int transfer_len, int interval)
{
	...
	if (ctrl->xhci == CONFIG_XHCI_CONTROLLER_FLAG)
		return submit_int_msg_ex(dev, pipe, buffer, transfer_len, interval);
#endif
			...
			void *data, unsigned short size, int timeout)
{
	...
	struct xhci_ctrl *ctrl = dev->controller;
	if (ctrl->xhci == CONFIG_XHCI_CONTROLLER_FLAG) {
		if (submit_control_msg_ex(dev, pipe, data, size, setup_packet) < 0)
			return -1;
	} else {
#endif
...
int get_usb2_controller_count(void)
{
	if (_HI3798M_V100 == get_chipid()	|| _HI3796M_V100 == get_chipid()) {
		return 2;
	}
...
int get_usb3_controller_count(void)
{
	if (_HI3798C_V200 == get_chipid()) {
		return 2;
	} else if ( _HI3716M_V410  == get_chipid() 
		|| _HI3716M_V420 == get_chipid()) {
		return 0;
	}
