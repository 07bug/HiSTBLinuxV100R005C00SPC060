...
{
	ALLOC_CACHE_ALIGN_BUFFER(unsigned char, buffer, DEFAULT_SECTOR_SIZE);
	...
	for (i = 0; i < 4; i++, pt++) {
		...
		if (((pt->boot_ind & ~0x80) == 0) &&
			...
			switch(dev_desc->if_type) {
				...
				case IF_TYPE_SATA:
				case IF_TYPE_ATAPI:
					snprintf ((char *)info->name, sizeof((char *)info->name), "hd%c%d",
						'a' + dev_desc->dev, part_num);
					break;
				case IF_TYPE_SCSI:
					snprintf ((char *)info->name, sizeof((char *)info->name), "sd%c%d",
						'a' + dev_desc->dev, part_num);
					break;
				case IF_TYPE_USB:
					snprintf ((char *)info->name, sizeof((char *)info->name), "usbd%c%d",
						'a' + dev_desc->dev, part_num);
					break;
				case IF_TYPE_DOC:
					snprintf ((char *)info->name, sizeof((char *)info->name), "docd%c%d",
						'a' + dev_desc->dev, part_num);
					break;
				default:
					snprintf ((char *)info->name, sizeof((char *)info->name), "xx%c%d",
						'a' + dev_desc->dev, part_num);
					break;
			}
			/* sprintf(info->type, "%d, pt->sys_ind); */
			snprintf ((char *)info->type, sizeof((char *)info->type), "U-Boot");
			return 0;
		}
