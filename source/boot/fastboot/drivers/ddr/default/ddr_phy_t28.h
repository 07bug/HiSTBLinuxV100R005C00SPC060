...
#define DDR_PHY_TRAINCTRL4	0x114    /* control the data training */
...
/* RD DQ0-DQ3 [6:0] [14:8] [22:16] [30:24] delay value of the bit delay line
 on read path */
#define DDR_PHY_DXNRDQNBDL0(n)	(0x21C + (ddr_dmc_get_rank_idx() << 10) + ((n) << 7))
/* RD DQ4-DQ7 [6:0] [14:8] [22:16] [30:24] delay value of the bit delay line
 on read path */
#define DDR_PHY_DXNRDQNBDL1(n)	(0x220 + (ddr_dmc_get_rank_idx() << 10) + ((n) << 7))
/* [6:0]RD DM*/
#define DDR_PHY_DXNRDQNBDL2(n)	(0x224 + (ddr_dmc_get_rank_idx() << 10) + ((n) << 7))
/* [CUSTOM] */
...
#define DDR_PHY_DXNRDQNBDL0_RANK(m, n)	(0x21C + (m << 10) + ((n) << 7))
#define DDR_PHY_DXNRDQNBDL1_RANK(m, n)	(0x220 + (m << 10) + ((n) << 7))
#define DDR_PHY_DXNRDQNBDL2_RANK(m, n)	(0x224 + (m << 10) + ((n) << 7))
...
#define PHY_RDET_DQ_RANK_EN_MASK  0x1
...
#define PHY_RDET_DQ_RANK_EN_BIT  31 /* [31] 0: All rank share rank0 rdqnbdl (default), 1: Each rank owns its rdqnbdl */
...
#define DDR_PHY_VREF_DRAM_SET(base_phy, val, byte_index) \
	do { \
		unsigned int rank_num = (REG_READ(base_dmc + DDR_DMC_CFG_DDRMODE) >> DMC_CFG_RANK_BIT) & DMC_CFG_RANK_MASK; \
		...
		if (rank_num) { \
			unsigned int rank_ctrl = REG_READ(base_phy + DDR_PHY_TRAINCTRL0); \
			REG_WRITE((rank_ctrl & 0xfffffff0) | 0x1, base_phy + DDR_PHY_TRAINCTRL0); \
			REG_WRITE(PHY_PHYINITCTRL_DVREFT_SYNC \
			| PHY_PHYINITCTRL_INIT_EN, \
			base_phy + DDR_PHY_PHYINITCTRL); \
			while (1) { \
			...
			REG_WRITE(rank_ctrl, base_phy + DDR_PHY_TRAINCTRL0); \
		} \
		REG_WRITE(dvrftctrl & (~PHY_DVRFTCTRL_PDAEN_EN), \
			base_phy + DDR_PHY_DVRFTCTRL); \
	} while (0)
...
#define DDR_PHY_RDQS_SYNC_RDM(base_dmc, base_phy, delay, byte_index) \
	do { \
		if (DMC_CFG_DRAM_TYPE_DDR4 == \
			...
			if (val > old) \
				ddr_adjust_sync_rdq(base_phy, byte_index, val - old); \
		} \
	} while (0)
