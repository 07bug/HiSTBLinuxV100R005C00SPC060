...
#ifdef DDR_RANK_SWITCH_CONFIG
unsigned int ddr_dmc_get_rank_idx(void)
{
	/* only support DDR_REG_BASE_PHY0 */
	unsigned int dq_rank_en = (REG_READ(DDR_REG_BASE_PHY0 + DDR_PHY_TRAINCTRL4) >> PHY_RDET_DQ_RANK_EN_BIT) & PHY_RDET_DQ_RANK_EN_MASK;
	unsigned int mask = REG_READ(DDR_REG_BASE_SYSCTRL + SYSCTRL_DDR_TRAINING_CFG) & DDR_BYPASS_RANK_MASK;
...
#else
unsigned int ddr_dmc_get_rank_idx(void) { return 0;}
#endif
...
static void ddr_adjust_sync_rdq(unsigned int base_phy, unsigned int byte, unsigned int offset)
{
	unsigned int val = offset | (offset << 8) | (offset << 16) | (offset << 24);
	unsigned int rank = 1 - ddr_dmc_get_rank_idx(); /* sync another rank */
	unsigned int rdq03 = REG_READ(base_phy + DDR_PHY_DXNRDQNBDL0_RANK(rank, byte)) + val; /* MUST not overflow */
	unsigned int rdq47 = REG_READ(base_phy + DDR_PHY_DXNRDQNBDL1_RANK(rank, byte)) + val; /* MUST not overflow */
	unsigned int rdm = REG_READ(base_phy + DDR_PHY_DXNRDQNBDL2_RANK(rank, byte)) + offset; /* MUST not overflow */
	unsigned int dq_rank_en = (REG_READ(base_phy + DDR_PHY_TRAINCTRL4) >> PHY_RDET_DQ_RANK_EN_BIT) & PHY_RDET_DQ_RANK_EN_MASK;

	if (!dq_rank_en)
		return;

	DDR_DEBUG("rank[%x] byte[%x] offset[%x]", rank, byte, offset);

	REG_WRITE(rdq03, base_phy + DDR_PHY_DXNRDQNBDL0_RANK(rank, byte));
	REG_WRITE(rdq47, base_phy + DDR_PHY_DXNRDQNBDL1_RANK(rank, byte));
	REG_WRITE(rdm, base_phy + DDR_PHY_DXNRDQNBDL2_RANK(rank, byte));
}
