...
static void himci_set_timing(unsigned int timing)
{
	...
	if (timing == MMC_TIMING_UHS_DDR50) {
		...
		tmp_reg |= EMMC_CLK_MODE0_100M  | EMMC_DRV_PS_180_90;
		if (get_mmc_io_voltage() == EMMC_IO_VOL_1_8V) {
			tmp_reg |= EMMC_SAP_PS_225_112DOT5;
		} else {
...
static u32 himciv300_get_dll_taps()
{
	u32 regval;

	regval = himci_readl(REG_BASE_CRG + REG_PERI_CRG160);
	return (regval & 0xff);
}
