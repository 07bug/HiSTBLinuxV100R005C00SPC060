...
#include <asm/errno.h>
#include <asm/io.h>
#include <himciv300_reg.h>

#include "himciv300.h"

extern void set_to_emmc(void);
...
static int hi_mci_setup_data(struct himci_host *host, struct mmc_data *data)
{
	...
	flush_dcache_range((uint32_t)des, ALIGN((uint32_t)&des[des_cnt], ARCH_DMA_MINALIGN));
	return 0;
}
...
static int hi_mci_data_done(struct himci_host *host, unsigned int stat)
{
	HIMCI_DEBUG_FUN("Function Call: stat 0x%08x", stat);
	HIMCI_ASSERT(host);
	struct mmc_cmd *cmd = host->cmd;
	...
	if((host->tunning) && (stat & (HTO_INT_STATUS | DRTO_INT_STATUS
...
static int hi_mci_wait_data_complete(struct himci_host *host)
{
	...
	do {
		...
		if (reg_data & DTO_INT_STATUS) {
			...
			return hi_mci_data_done(host, reg_data);
		}
		udelay(10);
		wait_retry_count++;
	} while (wait_retry_count < retry_num);
			  ...
			  struct mmc_data *data)
{
	...
	if (data) {
		if (data->flags & MMC_DATA_WRITE) {
			flush_dcache_range((uint32_t)data->src, ALIGN((uint32_t)data->src + data->blocks*data->blocksize, ARCH_DMA_MINALIGN));
		}
		...
		if (data->flags & MMC_DATA_READ) {
			if((uint32_t)data->dest&(ARCH_DMA_MINALIGN-1))
				flush_dcache_range((uint32_t)data->dest&(~(ARCH_DMA_MINALIGN-1)), (uint32_t)data->dest&(~(ARCH_DMA_MINALIGN-1))+ARCH_DMA_MINALIGN);
			invalidate_dcache_range((uint32_t)data->dest&(~(ARCH_DMA_MINALIGN-1)), ALIGN((uint32_t)data->dest + data->blocks*data->blocksize, ARCH_DMA_MINALIGN));
		}
...
static void hi_mci_set_ios(struct mmc *mmc)
{
	...
	/* set cardthrctl reg */
	tmp_reg = himci_readl(host->base + MCI_CARDTHRCTL);
#if defined(CONFIG_ARCH_HI3798MV2X) || defined(CONFIG_ARCH_HI3796MV2X) ||defined(CONFIG_ARCH_HI3798MV310)
	tmp_reg = RW_THRESHOLD_SIZE;
#else
...
static int hi_mci_execute_tuning(struct mmc *mmc, unsigned int datastrobe)
{
	...
	if (datastrobe != 1) {
		err = himciv300_execute_tuning(mmc, datastrobe);
		return err;
	}
...
static int hi_mci_initialize(bd_t * bis)
{
	...
	if (mmc->host_caps & (MMC_MODE_HS | MMC_MODE_HS_52MHz))
		mmc->timing = MMC_TIMING_MMC_HS;
	else
		mmc->timing = MMC_TIMING_LEGACY;

#if defined(CONFIG_ARCH_HI3798MX) || defined(CONFIG_ARCH_HIFONE) || defined(CONFIG_ARCH_HI3798CV2X) || defined(CONFIG_ARCH_HI3798MV2X) || defined(CONFIG_ARCH_HI3796MV2X) || defined(CONFIG_ARCH_HI3798MV310)
	mmc->host_caps |= MMC_MODE_DDR_52MHz;
	if ((_HI3798M_V300 == get_chipid()) || (_HI3798M_V310 == get_chipid()) || (_HI3798M_V300_H == get_chipid()))
		mmc->host_caps |= MMC_MODE_DDR50_TUNING;
#endif
...
void check_ext_csd(struct mmc *mmc)
{
...
#if  defined(CONFIG_BOOT1_PARTION_ENABLE)
	boot_part = 0x1;
#elif defined(CONFIG_BOOT2_PARTION_ENABLE)
	boot_part = 0x2;
#else
	if (get_ca_vendor())
		boot_part = 0x1;
		...
		boot_part = 0x7;
#endif
#ifdef CONFIG_MMC_BOOT_MODE_BUS_WIDTH
	boot_bus_width = CONFIG_MMC_BOOT_MODE_BUS_WIDTH;
#else
	...
	|| defined(CONFIG_ARCH_HI3716MV410) || defined(CONFIG_ARCH_HI3798CV2X) || defined(CONFIG_ARCH_HI3798MV2X) || defined(CONFIG_ARCH_HI3796MV2X) || defined(CONFIG_ARCH_HI3798MV310)
