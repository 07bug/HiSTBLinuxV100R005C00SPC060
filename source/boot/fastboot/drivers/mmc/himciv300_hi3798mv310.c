...
static void himci_set_timing(unsigned int timing)
{
	...
	/*good for ddr50 tuning*/
	if(timing == MMC_TIMING_UHS_DDR50){
		tmp_reg = himci_readl(REG_BASE_CRG + REG_PERI_CRG231);
		tmp_reg |= EMMC_SAP_DLL_DSEL_STEP1;
		himci_writel(tmp_reg,REG_BASE_CRG + REG_PERI_CRG231);
	}
	else{
		tmp_reg = himci_readl(REG_BASE_CRG + REG_PERI_CRG231);
		tmp_reg &= ~EMMC_SAP_DLL_DSEL_MASK;
		himci_writel(tmp_reg,REG_BASE_CRG + REG_PERI_CRG231);
	}
	...
	if (timing == MMC_TIMING_MMC_HS) {
		...
		for (ix = 0; ioshare[ix] != 0xff; ix += 2) {
			...
			/* avoid that strength is modified when nand startup */
			if (DRV_IOSHARE_EMMC == (tmp_reg&DRV_IOSHARE_MASK)) {
