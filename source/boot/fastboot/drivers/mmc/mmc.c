...
int mmc_read(struct mmc *mmc, u64 src, uchar *dst, int size)
{
	...
	buffer = memalign(ARCH_DMA_MINALIGN, blklen);
...
int mmc_send_tuning(struct mmc* mmc, u32 opcode)
{
	...
	data_buf = memalign(ARCH_DMA_MINALIGN, size);
	if (!data_buf)
		return -ENOMEM;
...
int mmc_startup(struct mmc *mmc)
{
	...
	ALLOC_CACHE_ALIGN_BUFFER(char, ext_csd, 512);
	ALLOC_CACHE_ALIGN_BUFFER(char, test_ext_csd, 512);
...
SwitchDDR50:
	if (IS_SD(mmc)) {
		...
		for (idx=0; idx < sizeof(ext_csd_bits)/sizeof(ext_csd_bits[0]); idx++) {
			...
			if (mmc->host_caps & MMC_MODE_DDR50_TUNING) {
				if (mmc->execute_tuning)
					err = mmc->execute_tuning(mmc, 2);
				if (!err) {
