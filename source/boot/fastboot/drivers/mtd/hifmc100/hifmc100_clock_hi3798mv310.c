...
#define CRG224_SPI_NAND_FMC_CODEC_SEL_300M	(1 << 8)

#define HIFMC100_SPI_NAND_CLK_SEL_MASK		(0x7 << 5)
#define HIFMC100_SPI_NAND_CLK_SEL_12M		0x80
#define HIFMC100_SPI_NAND_CLK_SEL_75M		0x40
...
static void change_ecc_decoder_clksrc(void)
{
	u32 regval;
	/*Before changing ECC decode clock to 300MHz, enable ECC synchronous mode*/
	regval = readl(0xf9950004);
	regval &= ~(1 << 19);
	writel(regval, 0xf9950004);

	/*Use 300MHz clock to improve performance of ECC*/
	regval = readl(REG_BASE_CRG + HIFMC100_CRG224);
	regval |= CRG224_SPI_NAND_FMC_CODEC_SEL_300M;
