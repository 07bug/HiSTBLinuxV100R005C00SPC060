...
static int hifmc100_spinor_erase(struct spi_flash *flash, u32 offset, size_t len)
{
	int nrchip = 0;
...
static int hifmc100_spinor_dma_read(struct spi_flash *flash, u32 offset,
				    size_t len, void *buf)
{
	int size;
	int nrchip = 0;
...
static int hifmc100_spinor_dma_write(struct spi_flash *flash, u32 offset,
...
static int hifmc100_spinor_probe_device(struct hifmc_spinor *spinor)
{
	...
	for (nrchip = 0; nrchip < HIFMC100_OP_CFG_NUM_CS; nrchip++) {
		...
		if (!spinor->nr_chips) {
			...
			if (!flash_dev) {
				printf("Can't recognize this chip(cs%d).\n", nrchip);
				return -EINVAL;
			}
			...
			if (spi_info_ex.type == 0) {
				...
				spi_info_ex.chipsize  = (uint64_t)flash_dev->chipsize;
				spi_info_ex.erasesize = flash_dev->erasesize;
				spi_info_ex.pagesize  = 1;
				spi_info_ex.addrcycle = spinor->addr_cycle;
			
				if (flash_dev->id_len > sizeof(spi_info_ex.ids)) {
					printf("BUG! ID len out of range.\n");
					BUG();
				}
			
				spi_info_ex.id_length = flash_dev->id_len;
				memcpy(spi_info_ex.ids, flash_dev->id,
						flash_dev->id_len);
			
				strncpy(spi_info_ex.name, flash_dev->name,
	...
	spi_info_ex.numchips = spinor->nr_chips;
...
{
	int nrchip;
	struct hifmc_spinor *spinor = host->spinor;

	if (!spinor)
		return;

	host->set_ifmode(host, HIFMC_IFMODE_SPINOR);
	host->set_clock(host, 0, ENABLE);
...
struct spi_flash * hifmc100_spinor_probe(void)
{
	...
	add_shutdown(hifmc100_spinor_shutdown);

	return spiflash;
}

/*****************************************************************************/
static struct mtd_info_ex *hifmc100_spinor_getinfo(void)
{
	if (spi_info_ex.type == 0)
		return NULL;
...
/*****************************************************************************/
int hifmc100_spinor_init(struct spi_flash **spiflash,
			 struct mtd_info_ex **spiinfo)
{
	if (hifmc100_host_init())
		return -1;

	(*spiflash) = hifmc100_spinor_probe();
	(*spiinfo) = hifmc100_spinor_getinfo();
