...
#define ETH_GLB_REG( n )    (GLB_HOSTMAC_L32 + REG_BASE_SF_ETH + n*4)
#if 0
...
static int hieth_dev_probe_init(struct eth_device* dev, int port)
{
	...
	ld->iobase_phys = REG_BASE_SF_ETH;
...
int hieth_initialize(void)
{
	...
	if (_HI3798M_V310 == get_chipid() || _HI3798M_V300_H == get_chipid())
		return 0;
	...
	dev->iobase = REG_BASE_SF_ETH + 0x2000;
	dev->init = hieth_init;
	dev->halt = hieth_halt;
	dev->send = hieth_send;
	dev->recv = hieth_recv;
