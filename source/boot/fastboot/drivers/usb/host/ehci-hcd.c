...
int usb_set_negotiation_timeout(int sec)
{
	if (sec < 0 || sec > 20) {
		printf("\n** Set invalid timeout(%d) **\n", sec);
		return -1;
	}

	usb_negotiation_timeout = sec;
	return 0;
}
		   ...
		   int length, struct devrequest *req)
{
	...
	do {
		/* Invalidate dcache */
		invalidate_dcache_range((uint32_t)&ctrl->qh_list, ALIGN_END_ADDR(struct QH, &ctrl->qh_list, 1));
		invalidate_dcache_range((uint32_t)qh,ALIGN_END_ADDR(struct QH, qh, 1));
		invalidate_dcache_range((uint32_t)qtd,ALIGN_END_ADDR(struct qTD, qtd, 3));
	 ...
	 */
	invalidate_dcache_range((uint32_t)buffer,ALIGN((uint32_t)buffer + length, ARCH_DMA_MINALIGN));
		 ...
		 int length, struct devrequest *req)
{
	...
	case USB_REQ_GET_STATUS | ((USB_RT_PORT | USB_DIR_IN) << 8):
		...
		/* Udisk may not complete self init now, so try several times */
		while (!(reg & EHCI_PS_CS) && usb_negotiation_timeout) {
			mdelay(1000);
			usb_negotiation_timeout -= 1;
			reg = ehci_readl(status_reg);
		}
		if (reg & EHCI_PS_CS)
