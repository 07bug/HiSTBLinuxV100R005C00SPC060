...
int ext4_unsparse(struct mmc *mmc, u32 dev, const u8 *pbuf, u32 blk, u32 cnt)
{
	...
	for (i = 0; i < header->total_chunks; i++) {
		...
		case CHUNK_TYPE_FILL:
			...
			if (chunk->total_sz
			    != (header->chunk_hdr_sz + sizeof(uint32_t))) {
				printf("No.%d chunk size error.\n", i);
				print_chunk_info(chunk);
				return 1;
			}
			...
			fill_buf = (uint32_t *)malloc(header->blk_sz);
			if (!fill_buf) {
				printf("Malloc failed for: CHUNK_TYPE_FILL.\n");
				return 1;
			}

			fill_val = *(uint32_t *)pbuf;
			pbuf = (char *)pbuf + sizeof(uint32_t);

			for (j = 0; j < (header->blk_sz / sizeof(fill_val)); j++) {
				fill_buf[j] = fill_val;
			}

			for (j = 0; j < chunk->chunk_sz; j++) {
				num = mmc->block_dev.block_write(dev,
						blk,
						(header->blk_sz >> EMMC_BLKSIZE_SHIFT),
						fill_buf);
				if (num != (header->blk_sz >> EMMC_BLKSIZE_SHIFT)) {
					printf("fill data: No.%d blocks written: %s.\n",
					       num, "ERROR");
					free(fill_buf);
					return 1;
				}

				blk += (header->blk_sz >> EMMC_BLKSIZE_SHIFT);
			}
