				   ...
				   int dols)
{
	...
	while (1) {
		...
		for (i = 0; i < DIRENTSPERCLUST; i++) {
			...
			if (strncmp(filename, s_name, sizeof(s_name))
			    && strncmp(filename, l_name, sizeof(l_name))) {
				debug("Mismatch: |%s|%s|\n", s_name, l_name);
				dentptr++;
				continue;
			}
...

{
	...
	while (1) {
		...
		for (i = 0; i < DIRENTSPERBLOCK; i++) {
			...
			if (strncmp(fnamecopy, s_name, sizeof(fnamecopy))
			    && strncmp(fnamecopy, l_name, sizeof(fnamecopy))) {
...
long file_fat_read_at (const char *filename, void *buffer, unsigned long pos, unsigned long maxsize)
{
	printf("reading %s from %lu\n", filename, pos);
	return do_fat_read_at(filename, pos, buffer, maxsize, LS_NO);
}

static char fnamecopy[2048];
static char l_name[VFAT_MAXLEN_BYTES];
...
long file_fat_size(const char *filename)
{
	...
	size = (long)FAT2CPU32(dentptr->size);
	printf("\n%8ld        %s\n\n", size, l_name);
