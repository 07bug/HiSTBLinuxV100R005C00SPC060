				 ...
				 uchar *enetaddr)
{
	char enetvar[32];
	if (index)
		snprintf(enetvar, sizeof(enetvar), "%s%daddr", base_name, index);
	else
		snprintf(enetvar, sizeof(enetvar), "%saddr", base_name);
...
static int eth_mac_skip(int index)
{
	char enetvar[15];
	if (index)
		snprintf(enetvar, sizeof(enetvar), "eth%dmacskip", index);
	else
		snprintf(enetvar, sizeof(enetvar), "ethmacskip");

	return (getenv(enetvar) != NULL);
}
...
#ifndef CONFIG_OPTIMIZE_BOOTTIME
