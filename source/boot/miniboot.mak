...
CONFIG_EXTERN_FLAGS := \
        ...
        $(if $(CFG_HI_ADVCA_NOCS3_RPC_SERVER),CONFIG_NOCS3_RPC_SERVER=$(CFG_HI_ADVCA_NOCS3_RPC_SERVER),) \
        ...
        $(if $(CFG_HI_KEYPROVISION_SUPPORT),CONFIG_KEYPROVISION_SUPPORT=$(CFG_HI_KEYPROVISION_SUPPORT)) \
        ...
        $(if $(CFG_HI_ADVANCED_CHIP),CONFIG_ADVANCED_CHIP=$(CFG_HI_ADVANCED_CHIP),)

ifeq ($(CFG_HI_CHIP_TYPE)_$(CFG_HI_TEE_SUPPORT)_$(CFG_HI_ADVCA_TYPE), hi3798mv200_y_VERIMATRIX_ULTRA)
CONFIG_EXTERN_FLAGS += CONFIG_SBL_VXX_UXX=y
endif

ifeq ($(CFG_HI_TWOSTAGEBOOT_SUPPORT),y)
CONFIG_EXTERN_FLAGS += \
        $(if $(CFG_HI_BOOT_FBL),CONFIG_FBL=y,) \
        $(if $(CFG_HI_BOOT_SBL),CONFIG_SBL=y,) \
        $(if $(CFG_HI_SECONDBOOT_STARTADDR),CFG_SECONDBOOT_ADDR=$(CFG_HI_SECONDBOOT_STARTADDR),) \
        $(if $(CFG_HI_SECONDBOOT_SIZE),CFG_SECONDBOOT_SIZE=$(CFG_HI_SECONDBOOT_SIZE),) \
        $(if $(CFG_HI_SECONDBOOT_PARTITION_SIZE),CFG_SECONDBOOT_PARTITION_SIZE=$(CFG_HI_SECONDBOOT_PARTITION_SIZE),) \
        $(if $(CFG_HI_BOOT_CMD),CONFIG_BOOTCOMMAND="\"$(CFG_HI_BOOT_CMD)\"",)
endif
...
ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3716mv410 hi3716mv420 hi3798cv200 hi3798mv200 hi3798mv300 hi3796mv200 hi3716mv450),)
...
ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3716mv410 hi3716mv420),)
#+++++++++++++++++++++ADVCA NAGRA chipid different++++++++++++++++++++++++
ifeq ($(CFG_ADVCA_NAGRA),y)
BOOTCONFIG_FILE     := hi3716mv420n
else
...
ifneq ($(BOOT_ADVCA_PROGRAMMER),y)
    ifeq ($(CFG_HI_ADVCA_SUPPORT),y)
        ...
        CONFIG_EXTERN_FLAGS +=  CONFIG_CC_STACKPROTECTOR=y
...
ifeq ($(CFG_HI_ADVCA_TYPE),NAGRA)
	CONFIG_CA_TYPE=nagra
else ifeq ($(CFG_HI_ADVCA_TYPE),IRDETO)
	CONFIG_CA_TYPE=irdeto
else ifeq ($(CFG_HI_ADVCA_TYPE),CONAX)
	CONFIG_CA_TYPE=conax
else ifeq ($(CFG_HI_ADVCA_TYPE),VERIMATRIX_ULTRA)
	CONFIG_CA_TYPE=vmx_ultra
else
	CONFIG_CA_TYPE=null
endif
    MKFLAGS += CONFIG_CA_TYPE=${CONFIG_CA_TYPE}
endif
################################################################################
PHONYS += all product prepare miniboot $(OBJ_DIR)/copy_code $(AUXIMG)
...
ifneq ($(CFG_HI_TWOSTAGEBOOT_SUPPORT),y)
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_NAND_IMAGE_DIR)/miniboot.bin
else
ifeq ($(CFG_HI_BOOT_FBL),y)
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_NAND_IMAGE_DIR)/miniboot.bin
else
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_NAND_IMAGE_DIR)/secondboot.bin
endif
endif
endif
...
ifneq ($(CFG_HI_TWOSTAGEBOOT_SUPPORT),y)
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_SPI_IMAGE_DIR)/miniboot.bin
else
ifeq ($(CFG_HI_BOOT_FBL),y)
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_SPI_IMAGE_DIR)/miniboot.bin
else
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_SPI_IMAGE_DIR)/secondboot.bin
endif
endif
endif
...
ifneq ($(CFG_HI_TWOSTAGEBOOT_SUPPORT),y)
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_EMMC_IMAGE_DIR)/miniboot.bin
else
ifeq ($(CFG_HI_BOOT_FBL),y)
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_EMMC_IMAGE_DIR)/miniboot.bin
else
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_EMMC_IMAGE_DIR)/secondboot.bin
endif
endif
endif
...
ifneq ($(CFG_HI_TWOSTAGEBOOT_SUPPORT),y)
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_SPI_EMMC_IMAGE_DIR)/miniboot.bin
else
ifeq ($(CFG_HI_BOOT_FBL),y)
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_SPI_EMMC_IMAGE_DIR)/miniboot.bin
else
	$(AT)cp -f $(MINIBOOT)/$(TARGET) $(HI_SPI_EMMC_IMAGE_DIR)/secondboot.bin
endif
endif
endif
else
...
$(OBJ_DIR)/copy_code:
	...
	$(AT)rm -rf $(OBJ_DIR)/miniboot $(OBJ_DIR)/fastboot $(OBJ_DIR)/product
...
miniboot: $(if $(CONFIG_PRODUCT_WITH_BOOT),product,prepare)
...
ifdef CONFIG_UNIFIED_BOOT
    ...
    PROGRAM_MKFLAGS += CHIP_TYPE=$(CFG_HI_CHIP_TYPE)

ifneq ($(findstring $(CFG_HI_CHIP_TYPE), hi3796mv200 hi3716mv450),)
    PROGRAM_MKFLAGS += CONFIG_CA_TYPE=$(CONFIG_CA_TYPE)
endif    
...
ifeq ($(CFG_HI_NAND_FLASH_SUPPORT),y)
	$(AT)test -d $(HI_NAND_IMAGE_DIR) || mkdir -p $(HI_NAND_IMAGE_DIR)
	$(AT)cp -f $(FASTBOOT)/$(PROGRAMMER_TARGET) $(HI_NAND_IMAGE_DIR)/advca_programmer.bin
endif
ifeq ($(CFG_HI_SPI_SUPPORT),y)
	$(AT)test -d $(HI_SPI_IMAGE_DIR) || mkdir -p $(HI_SPI_IMAGE_DIR)
	$(AT)cp -f $(FASTBOOT)/$(PROGRAMMER_TARGET) $(HI_SPI_IMAGE_DIR)/advca_programmer.bin
endif
ifeq ($(CFG_HI_EMMC_SUPPORT),y)
	$(AT)test -d $(HI_EMMC_IMAGE_DIR) || mkdir -p $(HI_EMMC_IMAGE_DIR)
	$(AT)cp -f $(FASTBOOT)/$(PROGRAMMER_TARGET) $(HI_EMMC_IMAGE_DIR)/advca_programmer.bin
endif
ifeq ($(CFG_HI_SPI_SUPPORT)_$(CFG_HI_EMMC_SUPPORT),y_y)
	$(AT)test -d $(HI_SPI_EMMC_IMAGE_DIR) || mkdir -p $(HI_SPI_EMMC_IMAGE_DIR)
	$(AT)cp -f $(FASTBOOT)/$(PROGRAMMER_TARGET) $(HI_SPI_EMMC_IMAGE_DIR)/advca_programmer.bin
endif
else
	$(AT)test -d $(PREFIX) || mkdir -p $(PREFIX)
...
$(CONFIG_FILE): $(if $(OBJ_DIR), $(OBJ_DIR)/copy_code,) force
	...
	@$(CONFIG_EXPORT) "CONFIG_SBL_VXX_UXX=$(CFG_HI_TEE_VMX_ULTRA_SUPPORT)" \
		"Enable SBL" $@
	...
	@$(CONFIG_EXPORT) "CONFIG_ADVANCED_CHIP=$(CFG_HI_ADVANCED_CHIP)" \
		"Advanced Chip" $@
	@$(CONFIG_EXPORT) "CONFIG_COMPRESS_BOOT=y" \
		"Compress boot" $@
...
###################################################################################
prepare: $(SYSREG) $(if $(OBJ_DIR), $(OBJ_DIR)/copy_code,)
...
ifeq ($(CFG_HI_EMMC_SUPPORT),y)
	$(AT)rm -rf $(HI_EMMC_IMAGE_DIR)/*[Bb]oot.bin
endif
ifeq ($(CFG_HI_SPI_SUPPORT)_$(CFG_HI_EMMC_SUPPORT),y_y)
