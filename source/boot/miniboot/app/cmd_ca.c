...
#include <cmd-words.h>
#include <debug.h>
//#define SSVTS_SUPPORT  //SSVTS_SUPPORT is for ssvts test case
#ifdef SSVTS_SUPPORT
#include "ssv_stg2.h"
#include "nocs_bsd.h"
#endif
...
#ifdef SSVTS_SUPPORT
static struct cmd_word_t CA_NagraBSDSSV_show = {
    "nagra_ssv", CMDWORD_TYPE_KEYWORD, _T(" - bsd ssv test case, for example:....")
};
#endif
#if 0
...
#ifdef SSVTS_SUPPORT
static int CA_NagraBSDSSV(struct cmd_t *cmd)
{
    int ret = 0;
    TBsdInitParameters *pxInitParameters = NULL;
    TBsdTerminateParameters *pxTerminateParameters = NULL;
        pr_info("CA_NagraBSDSSV start \n");
        bsdInitialize(pxInitParameters);
        ssvAuthenticatePairingKeys(NULL,NULL,NULL,NULL);
        bsdTerminate(pxTerminateParameters);
        return ret;
    pr_error("Not support this command.\n");
    return -1;
}
CMD({&cw_ca, NULL}, {&CA_NagraBSDSSV_show, NULL, CA_NagraBSDSSV})
#endif
