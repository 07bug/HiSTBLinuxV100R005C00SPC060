...
int arm_start(void)
{
	...
	ret = uncompress(uncompbuf, &sz_uncompbuf, data_begin, data_end - data_begin);
	if(ret) {
		early_puts("FAILED(");
		early_puthex(ret);
		early_puts(")\n");
...
#ifdef CONFIG_CHECKED_AREA_START
	/* Skip the head area */
	entry = (entry_fn)((unsigned int)entry + CONFIG_CHECKED_AREA_START);
#endif
	entry();
