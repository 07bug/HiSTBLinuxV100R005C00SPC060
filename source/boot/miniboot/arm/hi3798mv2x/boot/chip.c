...
extern void *early_memcpy(void *dest, const void *src, unsigned int count);

/* for Hi3798MV200 riscure, auxcode set cpu freq to 200/400/600
 * to enhance robustness. Here to restore cpu freq to APLL.
 */
void copy_fbl_ext_area(void)
{
	/* copy ext area from ddr 8 to CONFIG_PARAM_AREA_POS,
	   FBL/SBL need. */
	early_memcpy((unsigned int *)(TEXT_BASE + CONFIG_PARAM_AREA_POS),
			(unsigned int *)CONFIG_EXT_AREA_ADDR, CONFIG_EXT_AREA_LEN);
}

char *get_fbl_ext_area(unsigned int *length)
{
	if (length)
		*length = CONFIG_EXT_AREA_LEN;
	return (char *)(TEXT_BASE + CONFIG_PARAM_AREA_POS);
}
#endif
