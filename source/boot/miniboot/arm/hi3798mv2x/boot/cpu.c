...
static int hi3798mv2x_boot_media(char **media)
{
	...
	default:
		...
		early_puts("Unknown boot mode:");
		early_puthex(boot_media);
		early_puts("\n");
		break;
	}
...
static void get_hi3798mv2x_clock(unsigned int *cpu, unsigned int *timer)
{
	unsigned long FREF = HI3798MV2X_OSC_FREQ;
...
static int hi3798mv2x_get_ca_type(void)
{
...
#endif
		.boot_media = hi3798mv2x_boot_media,
		.get_clock  = get_hi3798mv2x_clock,
		.get_ca_type = hi3798mv2x_get_ca_type,
	},
	{
		.name   = "Hi3798Mv300",
