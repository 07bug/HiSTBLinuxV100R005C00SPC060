...
void check_bootfromsd(void)
{
	if (BOOT_FROM_SD == get_boot_mode()) {
		reg_read_write((volatile unsigned int *)REG_IOSHARE_EBI_DQ7, 0x0, 0x3);
		reg_read_write((volatile unsigned int *)REG_IOSHARE_EBI_DQ6, 0x0, 0x1);
		reg_read_write((volatile unsigned int *)REG_IOSHARE_EBI_DQ5, 0x0, 0x1);
