...
static int hi3796mv200_phyinf[] = {/* array index is the hw config */
	[0] = ETH_PHY_MII,
	[1] = ETH_PHY_RGMII,
	[3] = ETH_PHY_UNKNOWN,
	[4] = ETH_PHY_RMII,
...
static void set_hi3796mv200_eth_phyintf(u32 regval)
{
	u32 phy0intf;
	u32 phy1intf;

	phy0intf = (regval >> 13) & 0x7;
	...
	snprintf(eth_config.phyintf, sizeof(eth_config.phyintf), "%s,%s",
		 ethphy_intfname(hi3796mv200_phyinf[phy0intf]),
		 ethphy_intfname(hi3796mv200_phyinf[phy1intf]));
}
...
static void set_eth_phymdio(u32 regval)
{
	...
	phy0mdio = (regval >> 8) & 0x1f;
	phy1mdio = (regval >> 24) & 0x1f;

	if (phy0mdio > 1 && phy0mdio != 0x1f) {
		printf(EXCEL_ERR
...
{
	/* The eth param is set up by tags in setup_eth_tags() of kern_tags.c */
}
...
void eth_config_init(void)
{
	 ...
	 * bit 8 -- 12: phymdio for mac0
	 * bit13 -- 15: phyintf for mac0
	 * bit16 -- 23: phyaddr for mac1
	 * bit24 -- 28: phymdio for mac1
	 * bit29 -- 31: phyintf for mac1
	 */
	...
	set_hi3796mv200_eth_phyintf(phyaddrmdio_val);
}
