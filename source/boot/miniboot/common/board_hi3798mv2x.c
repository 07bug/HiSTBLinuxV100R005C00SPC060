...
static void set_hi3798mv2x_eth_phyintf(u32 regval)
{
	u32 phy0intf;
	u32 phy1intf;

	phy0intf = (regval >> 13) & 0x1;

	if (phy0intf != 0 && phy0intf != 1) {
		printf("MAC0_IF register should be 0(MII), 1(RMII), "
		       "but current is %d\n",
		       phy0intf);
		return;
	}
...
void eth_config_init(void)
{
	...
	set_hi3798mv2x_eth_phyintf(phyaddrmdio_val);
}
