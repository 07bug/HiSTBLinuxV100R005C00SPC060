...
static int hi3716cv200_phyinf[] = {/* array index is the hw config */
	[0] = ETH_PHY_MII,
	[1] = ETH_PHY_RGMII,
	[3] = ETH_PHY_UNKNOWN,
	[4] = ETH_PHY_RMII,
...
static int hi3719mv100_phyinf[] = {/* array index is the hw config */
	[0] = ETH_PHY_MII,
	[1] = ETH_PHY_RMII,
	[3] = ETH_PHY_UNKNOWN,
};
...
static void set_hi3719mv100_eth_phyintf(void)
{
	uint32 regval;
	uint32 phy0intf;
	...
	snprintf(eth_config.phyintf, sizeof(eth_config.phyintf), "%s,%s",
		 ethphy_intfname(hi3719mv100_phyinf[phy0intf]),
		 "none");
}

static void set_hi3716cv200_eth_phyintf(void)
{
	uint32 regval;
	uint32 phy0intf;
	uint32 phy1intf;

	regval = readl(REG_BASE_SF + HIGMACV300_MAC0_IF);
	phy0intf = (regval >> 5) & 0x7;

	if (phy0intf != 0 && phy0intf != 1 && phy0intf != 4) {
		printf("MAC0_IF register should be 0(MII), 1(RGMII) or 4(RMII), "
		       "but current is %d\n",
		       phy0intf);
		return;
	}

	regval = readl(REG_BASE_SF + HIGMACV300_MAC1_IF);
	phy1intf = (regval >> 5) & 0x7;
	if (phy1intf != 0 && phy1intf != 1 && phy1intf != 4) {
		printf("MAC1_IF register should be 0(MII), 1(RGMII) or 4(RMII), "
		       "but current is %d\n",
		       phy1intf);
		return;
	}
	...
	snprintf(eth_config.phyintf, sizeof(eth_config.phyintf), "%s,%s",
		 ethphy_intfname(hi3716cv200_phyinf[phy0intf]),
		 ethphy_intfname(hi3716cv200_phyinf[phy1intf]));
}

static void set_hi3716mv400_eth_phyintf(void)
{
	uint32 regval;
	uint32 phy0intf;
	uint32 phy1intf;
	...
	if (phy0intf != 0 && phy0intf != 4) {
		printf("MAC0_IF register should be 0(MII) or 4(RMII), "
		       "but current is %d\n",
		       phy0intf);
		return;
	}
	...
	regval = readl(REG_BASE_SF + HIGMACV300_MAC1_IF);
	phy1intf = (regval >> 5) & 0x7;
	if (phy1intf != 0 && phy1intf != 4) {
		printf("MAC1_IF register should be 0(MII) or 4(RMII), "
		       "but current is %d\n",
		       phy1intf);
		return;
	}
...
void eth_config_init(void)
{
	...
	set_eth_phyaddr(phyaddrmdio_val);
	set_eth_phymdio(phyaddrmdio_val);
	set_hi3716cv200_eth_phyintf();
}
