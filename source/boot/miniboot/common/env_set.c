...
int env_save_to_flash(void)
{
	...
	const struct flash_info_t *finfo;
...
static int __maybe_unused load_backup_flash(enum flash_type_t flashtype, struct env_t *env)
{
	int ret;
	struct flash_intf_t *flashif;
...
static void env_init(void *ptr)
{
	...
	struct env_t *env;
	struct flash_intf_t * __maybe_unused flashif;
...
#ifndef CONFIG_FBL /* When use fbl, then set the default env */
