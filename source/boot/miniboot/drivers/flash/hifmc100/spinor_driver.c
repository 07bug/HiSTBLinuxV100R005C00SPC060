...
static int spinor_write_buf_reg(struct flash_regop *regop, u8 cmd, char *buf,
				int sz_buf)
{
...
static int spinor_read_buf_reg(struct flash_regop *regop, u8 cmd, char *buf,
			       int sz_buf)
{
	struct flash_regop_info info = {0};
...
static int spinor_quad_enable_mxic(struct flash_regop *regop)
{
	...
	spinor_write_buf_reg(regop, SPINOR_OP_WRSR, (char *)&status, 1);
...
static int spinor_quad_enable_spansion(struct flash_regop *regop)
{
	...
	spinor_read_buf_reg(regop, SPINOR_OP_RDSR2, (char *)&sr[1], 1);
	if (sr[1] & SPINOR_STATUS2_EQ)
		return 0;

	spinor_read_buf_reg(regop, SPINOR_OP_RDSR,  (char *)&sr[0], 1);

	sr[1] |= SPINOR_STATUS2_EQ;

	spinor_write_enable(regop);

	spinor_write_buf_reg(regop, SPINOR_OP_WRSR, (char *)sr, 2);
