...
struct mmc_dev_t *hi_mci_initialize(void)
{
	...
	if (mmc_dev->host_caps & (MMC_MODE_HS | MMC_MODE_HS_52MHz))
		mmc_dev->timing = MMC_TIMING_MMC_HS;
	else
		mmc_dev->timing = MMC_TIMING_LEGACY;
...
#endif
#if defined(CONFIG_ARCH_HIFONE) || defined(CONFIG_ARCH_HI3798CV2X) || defined(CONFIG_ARCH_HI3798MV2X)||defined(CONFIG_ARCH_HI3796MV2X)
