...
static int sd_dev_switch_timing(struct mmc_dev_t *mmc_dev)
{
	int ret;
	struct mmc_cmd cmd;
...
static int mmc_dev_switch_hs400(struct mmc_dev_t *mmc_dev)
{
	int ret;
...
static int mmc_dev_switch_timing(struct mmc_dev_t *mmc_dev, char ext_csd[512])
{
	int ret;
...
SwitchDDR50:
	if (IS_SD(mmc_dev)) {
		if (mmc_dev->card_caps & MMC_MODE_4BIT)
			return sd_dev_switch_timing(mmc_dev);
		...
		if (mmc_dev->card_caps&MMC_MODE_HS400) {
			...
			mmc_dev->speed = HS200_MAX_CLK;
#else
			mmc_dev->speed = 200000000;
#endif
#if defined(CONFIG_ARCH_HI3796MV2X)
	if((_HI3796M_V200_MASK & get_chipid())== _HI3796M_V200_15X15)
		mmc_dev->speed = HS200_CLK_150M;
	else
		mmc_dev->speed = HS200_CLK_187M;
#endif
	...
	if (mmc_dev->card_caps & MMC_MODE_HS400) {
		ret = mmc_dev_switch_hs400(mmc_dev);
		if (ret)
			return ret;
	}
...
static int mmc_get_ext_csd(struct mmc_dev_t *mmc_dev, char ext_csd[512])
{
	int ret;
	uint64 capacity;
...
int mmc_dev_start(struct mmc_dev_t *mmc_dev)
{
	...
	ret = mmc_get_ext_csd(mmc_dev, ext_csd);
	if (ret)
		return ret;
	...
	ret = mmc_dev_switch_timing(mmc_dev, ext_csd);
	if (ret)
		return ret;
