...
#define CA_OTP_V200_ANTIROLLBACK_NUMBER     (0x500)
...
#if defined(CHIP_TYPE_hi3798mv310)
    if (HI_UNF_ADVCA_ALG_TYPE_SM4 == g_R2RAlgType)
    {
...
static CA_KEYLADDER_INFO_S g_BLENCInfo= {0};
#if defined(CHIP_TYPE_hi3798mv310)
static CA_KEYLADDER_INFO_S g_BLDECInfo= {0};
#endif
...
HI_S32 CA_SetEncSwpkAlg(HI_UNF_ADVCA_ALG_TYPE_E enType)
{
#if defined(CHIP_TYPE_hi3798mv310)
...
HI_S32 CA_DecryptSwpk(HI_U32 AddrID, HI_U32 *pEncryptSwpk)
{
    ...
    BlDecCtrl.bits.key_addr = (AddrID << 1) & 0x7F;
#else
    BlDecCtrl.bits.key_addr = AddrID & 0x3F;
#endif
    DRV_ADVCA_WriteReg(CA_V300_BL_CTRL_DEC, BlDecCtrl.u32);
...
HI_S32 CA_GetBootMode(HI_UNF_ADVCA_FLASH_TYPE_E *penFlashType)
{
    ...
    bootMode = OTP_Read(OTP_INTERNAL_PV_0);
...
static HI_S32 CA_SetBootMode(HI_UNF_ADVCA_FLASH_TYPE_E enFlashType)
{
    ...
    PVLOCK_0.u32 = OTP_Read(OTP_INTERNAL_PVLOCK_0);
    if ((1 == PVLOCK_0.bits.boot_mode_sel_1_lock) && (1 == PVLOCK_0.bits.boot_mode_sel_0_lock))
    {
        PV_0.u32 = OTP_Read(OTP_INTERNAL_PV_0);
    ...
    ret = OTP_Write(OTP_INTERNAL_PV_0, PV_0.u32);
    if (HI_SUCCESS != ret)
    {
    ...
    ret = OTP_Write(OTP_INTERNAL_PVLOCK_0, PVLOCK_0.u32);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CA("Fail to write OTP!\n");
        return ret;
...
static HI_S32 CA_SetBootSelCtrl(HI_U32 u32BootSel)
{
    ...
    PVLOCK_1.u32 = OTP_Read(OTP_INTERNAL_PVLOCK_1);
    if (1 == PVLOCK_1.bits.bootsel_ctrl_lock)
    {
        PV_1.u32 = OTP_Read(OTP_INTERNAL_PV_1);
    ...
    if (u32BootSel == 1)
    {
        ...
        ret = OTP_Write(OTP_INTERNAL_PV_1, PV_1.u32);
        if (HI_SUCCESS != ret)
        {
    ...
    ret = OTP_Write(OTP_INTERNAL_PVLOCK_1, PVLOCK_1.u32);
    if (HI_SUCCESS != ret)
    {
...
HI_S32 CA_SetRSAKeyLockFlag(HI_UNF_ADVCA_OTP_ATTR_S *pstOtpFuseAttr)
{
    ...
    {
        u32Temp = OTP_Read(OTP_INTERNAL_DATALOCK_1);
        if(1 == (u32Temp & 1))
        {
            return HI_SUCCESS;
        }
        ...
        u32Temp = u32Temp | 1;
        ret = OTP_Write(OTP_INTERNAL_DATALOCK_1, u32Temp);
        if(HI_SUCCESS != ret)
        {
...
static HI_U32 ADVCA_BitCount(HI_U8 n)
{
    HI_U32 u32Count = 0;
...
HI_S32 CA_GetAntiRollBackVersion(HI_UNF_ADVCA_OTP_ATTR_S *pstOtpFuseAttr)
{
    HI_U32 u32VersionCount = 0;
    HI_S32 i;
    HI_U8 u8Value;
    HI_U32 u32Addr = 0;
    HI_U32 u32Length = 0;
    ...
    if(NULL == pstOtpFuseAttr)
    {
        HI_ERR_CA("pstOtpFuseAttr is Null\n");
        return HI_ERR_CA_INVALID_PARA;
    }

    u32Addr   = pstOtpFuseAttr->unOtpFuseAttr.stVersionNumber.u32OTPAddr;
    u32Length = pstOtpFuseAttr->unOtpFuseAttr.stVersionNumber.u32OTPLength;
    if ((u32Addr < CA_OTP_V200_ANTIROLLBACK_NUMBER) || (u32Addr + u32Length >= CA_OTP_V200_ANTIROLLBACK_NUMBER + ANTIROLLBACK_MAX_LEN))
    {
        HI_ERR_CA("AntiRollBack addr(%x) or Length(%x) is invalid\n", u32Addr, u32Length);
        return HI_FAILURE;
    }

    for (i = 0; i < u32Length; i++)
    {
        u8Value = OTP_ReadByte(u32Addr + i);
        u32VersionCount += ADVCA_BitCount(u8Value);
    }

    pstOtpFuseAttr->unOtpFuseAttr.stVersionNumber.u32VersionNumber = u32VersionCount;

    return HI_SUCCESS;
}

HI_S32 CA_SetAntiRollBackVersion(HI_UNF_ADVCA_OTP_ATTR_S *pstOtpFuseAttr)
{
    HI_S32 i;
    HI_U8 u8Value;
    HI_U32 u32Addr = 0;
    ...
    for (i = 0; i < u32Length; i++)
    {
        ...
        if (0xff != u8Value)
        {
            u8Value |= 1 << ADVCA_BitCount(u8Value);
            return OTP_SetByte(u32Addr + i, u8Value);
        }
    }

    HI_ERR_CA("antiroll back version num is full, set failed\n");
...
HI_S32 CA_GetRSAKeyLockFlag(HI_UNF_ADVCA_OTP_ATTR_S *pstOtpFuseAttr)
{
    ...
    u8Temp = OTP_ReadByte(OTP_INTERNAL_DATALOCK_1);
    if(0 == (u8Temp & 1))
    {
...
HI_S32 CA_EnableSCSActive(HI_UNF_ADVCA_FLASH_TYPE_E enFlashType)
{
    ...
    ret = OTP_Write(OTP_INTERNAL_ONEWAY_0, OneWay.u32);
    if(HI_SUCCESS != ret)
    {
    ...
    unDataLock.u32 = 0;
    unDataLock.u32 = OTP_Read(OTP_INTERNAL_DATALOCK_0);
    ...
    if(1 != unDataLock.bits.scs_en_bak_lock)
    {
        ...
        ret = OTP_SetByte(OTP_SCS_EN_BAK_ADDR, u8ScsBak);
        if(HI_SUCCESS != ret)
        {
        ...
        ret = OTP_Write(OTP_INTERNAL_DATALOCK_0, unDataLock.u32);
        if(HI_SUCCESS != ret)
        {
...
HI_S32 CA_GetSCSActiveFlag(HI_BOOL *pbSCSActiveFlag)
{
    ...
    OneWay_0.u32 = OTP_Read(OTP_INTERNAL_ONEWAY_0);
    u8ScsBak = OTP_ReadByte(OTP_SCS_EN_BAK_ADDR);
...
HI_S32 DRV_CA_OTP_V200_GetCSA2LadderLevel(CA_OTP_KEY_LEVEL_E *pLevel)
{
    ...
    PV_0.u32 = OTP_Read(CA_OTP_V200_INTERNAL_PV_0);
...
HI_S32 DRV_CA_OTP_V200_GetSPLadderLevel(CA_OTP_KEY_LEVEL_E *pLevel)
{
    ...
    PV_0.u32 = OTP_Read(CA_OTP_V200_INTERNAL_PV_0);
    PV_1.u32 = OTP_Read(CA_OTP_V200_INTERNAL_PV_1);
...
HI_S32 DRV_CA_OTP_V200_GetSecureChipId(CA_OTP_VENDOR_TYPE_E *pu32SecureChipId)
{
    ...
    u32SecureChipId = OTP_Read(CA_OTP_V200_INTERNAL_CA_VENDOR_ID);
