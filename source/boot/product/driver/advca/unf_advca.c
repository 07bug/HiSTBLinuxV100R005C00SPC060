...
HI_S32 HI_UNF_ADVCA_SetRSAKey(HI_U8 *pkey)
{
    ...
    u8Tmp = HI_UNF_OTP_ReadByte(OTP_INTERNAL_DATALOCK_1);	//0x14[0]: rsa_key_lock
    if(1 == (u8Tmp & 1))
    {
    ...
    {
        u8Tmp = HI_UNF_OTP_ReadByte(OTP_RSA_KEY_E_ADDR + index);
        if(0 != u8Tmp)
        {
    ...
    {
        u8Tmp = HI_UNF_OTP_ReadByte(OTP_RSA_KEY_N_ADDR + index);
        if(0 != u8Tmp)
        {
    ...
    for(index = 0; index < 16; index++)
    {
        ret = HI_UNF_OTP_WriteByte(OTP_RSA_KEY_E_ADDR + index, ptr[index]);
        if(HI_SUCCESS != ret)
        {
    ...
    for(index = 0; index < 256; index++)
    {
        ret = HI_UNF_OTP_WriteByte(OTP_RSA_KEY_N_ADDR + index, ptr[index]);
        if(HI_SUCCESS != ret)
        {
    ...
    for(index = 0; index < 16; index++)
    {
        ptr[index] = HI_UNF_OTP_ReadByte(OTP_RSA_KEY_E_ADDR + index);
    }

    ptr = au8KeyOut;
    for(index = 0; index < 256; index++)
    {
        ptr[index] = HI_UNF_OTP_ReadByte(OTP_RSA_KEY_N_ADDR + index);
    }
...
HI_S32 HI_UNF_ADVCA_SetOtpFuse(HI_UNF_ADVCA_OTP_FUSE_E enOtpFuse, HI_UNF_ADVCA_OTP_ATTR_S *pstOtpFuseAttr)
{
    ...
    switch(enOtpFuse)
    {
        ...
        case HI_UNF_ADVCA_OTP_ANTIROLLBACK_VERSION:
        {
            ret = CA_SetAntiRollBackVersion(pstOtpFuseAttr);
            break;
        }
        default:
        {
...
HI_S32 HI_UNF_ADVCA_GetOtpFuse(HI_UNF_ADVCA_OTP_FUSE_E enOtpFuse, HI_UNF_ADVCA_OTP_ATTR_S *pstOtpFuseAttr)
{
    ...
    switch(enOtpFuse)
    {
        ...
        case HI_UNF_ADVCA_OTP_ANTIROLLBACK_VERSION:
        {
            ret = CA_GetAntiRollBackVersion(pstOtpFuseAttr);
            break;
        }
        default:
        {
...
HI_S32 HI_UNF_ADVCA_SetEncryptSWPKAlg(HI_UNF_ADVCA_ALG_TYPE_E enType)
{
    return CA_SetEncSwpkAlg(enType);
}
