...
#if 0
#define cmd_tbl_t void
#define U_BOOT_CMD(a,s,d,f,g,h)
#define simple_strtoul(a, b, c)    0
#define get_timer(x)               1
#define CONFIG_SYS_HZ              1

static int cipher_test(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
static int cbc_mac(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
...
void cipher_self_test(void)
{
    HI_PRINT("cipher_self_test start ...\n");

    cipher_test(NULL, 0, 0, NULL);
    test_hash_all(NULL, 0, 0, NULL);
    rsa_sign_test(NULL, 0, 0, NULL);
    rsa_enc_test(NULL, 0, 0, NULL);
    rng_test(NULL, 0, 0, NULL);
    cbc_mac(NULL, 0, 0, NULL);
}
#endif
...
static const unsigned char hmac_sha2_sum[3][32] ={
    ...
    {0xe0, 0x18, 0xcb, 0x84, 0x21, 0x05, 0x82, 0x31, 0x43, 0xf2, 0xda, 0xd9, 0x89, 0x1a, 0x4e, 0x81,
     0x49, 0x32, 0xc8, 0x0b, 0x95, 0x1e, 0x83, 0x5e, 0xdf, 0x39, 0x7c, 0x3d, 0x28, 0xfa, 0x3b, 0xd5}
};
...
HI_S32 SHA1(HI_VOID)
{
    ...
    for(i = 0; i < 3; i++)
    {
        if(i == 2)
        {
            ...
            HI_UNF_CIPHER_HashUpdate(hHandle, u8Buf + 1, 1024);
...
HI_S32 SHA256(HI_VOID)
{
    ...
    for(i = 0; i < 3; i++)
    {
        ...
        HI_UNF_CIPHER_HashInit(&stHashAttr, &hHandle[i]);
    }
    ...
    for(i = 0; i < 3; i++)
    {
        if(i == 2)
        {
            ...
            HI_UNF_CIPHER_HashUpdate(hHandle[i], u8Buf, 1024);
        }
        else
        {
            HI_UNF_CIPHER_HashUpdate(hHandle[i], sha1_buf[i], sha1_buflen[i]);
        }
    }
    for(i = 0; i < 3; i++)
    {
        HI_UNF_CIPHER_HashFinal(hHandle[i], u8Hash);
    ...
    for(i = 0; i < 3; i++)
    {
        if(i == 2)
        {
            ...
            stHashAttr.eShaType = HI_UNF_CIPHER_HASH_TYPE_SHA256;
            stHashAttr.u32TotalDataLen = buflen;
            HI_UNF_CIPHER_HashInit(&stHashAttr, &hHandle[0]);
            HI_UNF_CIPHER_HashUpdate(hHandle[0], u8Buf, 1024);
            HI_UNF_CIPHER_HashFinal(hHandle[0], u8Hash);
        }
        else
        {
            stHashAttr.eShaType = HI_UNF_CIPHER_HASH_TYPE_SHA256;
            stHashAttr.u32TotalDataLen = sha1_buflen[i];
            HI_UNF_CIPHER_HashInit(&stHashAttr, &hHandle[0]);
            HI_UNF_CIPHER_HashUpdate(hHandle[0], sha1_buf[i], sha1_buflen[i]);
            HI_UNF_CIPHER_HashFinal(hHandle[0], u8Hash);
        }
...
HI_S32 HMAC_SHA256(HI_VOID)
{
    ...
    HI_HANDLE hHandle;
    HI_UNF_CIPHER_HASH_ATTS_S stHashAttr;
    HI_UNF_CIPHER_HASH_ATTS_EX_S stHashAttrEx;
    ...
    for(i = 0; i < 3; i++)
    {
        ...
        {
            memset(u8Buf, 'a', buflen = 1024);
            stHashAttrEx.eShaType = HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA256;
            stHashAttrEx.u32TotalDataLen = buflen;
            stHashAttrEx.pu8HMACKey = u8HMACSha2LongKey;
            stHashAttrEx.u32HMACKeyLen = sizeof(u8HMACSha2LongKey);
            ret = HI_UNF_CIPHER_HashInitEx(&stHashAttrEx, &hHandle);
            if(HI_SUCCESS != ret)
            {
                HI_ERR_CIPHER("hash init failed!\n");
                goto EXIT;
...
int Hash_Calc(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
{
    HI_U32 i;
    HI_U32 u32Random[16] = {0};

    HI_UNF_CIPHER_Init();

    for(i=0; i<16; i++)
    {
        HI_UNF_CIPHER_GetRandomNumber(&u32Random[i]);
        printf("Random: 0x%x\n", u32Random[i]);
    }

    HI_UNF_CIPHER_GetMultiRandomBytes((HI_U8*)u32Random, sizeof(u32Random));
    Cipher_printBuffer("Random", (HI_U8*)u32Random, sizeof(u32Random));
...
int cbc_mac(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U8 u8MacValue[16];
    HI_U8 IV[16];
...
//    HI_U8 result[16] = {0xdf, 0xa6, 0x67, 0x47, 0xde, 0x9a, 0xe6, 0x30, 0x30, 0xca, 0x32, 0x61, 0x14, 0x97, 0xc8, 0x27};//use soft key
    ...
    s32Ret = Setconfiginfo(hCipherHandle, HI_TRUE, HI_UNF_CIPHER_ALG_AES, HI_UNF_CIPHER_WORK_MODE_CBC,
        HI_UNF_CIPHER_KEY_AES_128BIT, key, IV);
    if(HI_SUCCESS != s32Ret)
    {
...
U_BOOT_CMD(cbc_mac_test, 1, 1, cbc_mac, "Calcluate cbc mac", "for example: cbc_mac_test");
