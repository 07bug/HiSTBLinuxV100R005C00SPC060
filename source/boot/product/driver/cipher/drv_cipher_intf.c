...
#define HASH_CheckHandle(handle)   \
    do \
    { \
        if((HI_ID_CIPHER != HI_HANDLE_GET_MODID(handle)) \
            || (0 != HI_HANDLE_GET_PriDATA(handle))) \
        ...
        if (HI_HANDLE_GET_CHNID(handle) >= HASH_CHANNAL_MAX_NUM) \
        { \
            HI_ERR_CIPHER("chan %d is too large, max: %d\n", HI_HANDLE_GET_CHNID(handle), HASH_CHANNAL_MAX_NUM); \
            return HI_ERR_CIPHER_INVALID_HANDLE; \
        } \
        if (HI_FALSE == g_stCipherHashData[HI_HANDLE_GET_CHNID(handle)].bIsUsed) \
        { \
            HI_ERR_CIPHER("chan %d is not open\n", HI_HANDLE_GET_CHNID(handle)); \
            return HI_ERR_CIPHER_INVALID_HANDLE; \
        } \
...
HI_S32 Cipher_CreateHandle(HI_HANDLE* phCipher, const HI_UNF_CIPHER_ATTS_S *pstCipherAttr)
{
    ...
    {
        case HI_UNF_CIPHER_TYPE_COPY_AVOID:
        {
        ...
        case HI_UNF_CIPHER_TYPE_NONSEC:
        {
            if (0 == g_stCipherOsrChn[CIPHER_PKGxN_CHAN_MIN].g_bSoftChnOpen)
            {
                i = CIPHER_PKGxN_CHAN_MIN;
            }
            else
            {
                i = CIPHER_INVALID_CHN;
        ...
        case HI_UNF_CIPHER_TYPE_NORMAL:
        {
            for(i = CIPHER_PKGxN_CHAN_MIN + 1; i < CIPHER_SOFT_CHAN_NUM; i++)
            {
        ...
        default:
        {
            HI_ERR_CIPHER("Invalid type 0x%x.\n", pstCipherAttr->enCipherType );
...
HI_S32 Cipher_DestroyHandle(HI_HANDLE hCipher)
{
    ...
    if((HI_ID_CIPHER != HI_HANDLE_GET_MODID(hCipher))
        || (0 != HI_HANDLE_GET_PriDATA(hCipher)))
    {
        HI_ERR_CIPHER("Invalid handle 0x%x!\n", hCipher);
        return HI_ERR_CIPHER_INVALID_HANDLE;
    }
    if (CIPHER_SOFT_CHAN_NUM <= HI_HANDLE_GET_CHNID(hCipher))
    {
        HI_ERR_CIPHER("chan %d is too large, max: %d\n", HI_HANDLE_GET_CHNID(hCipher), CIPHER_SOFT_CHAN_NUM);
        return HI_ERR_CIPHER_INVALID_HANDLE;
    }
...
HI_S32 Cipher_ConfigHandle(HI_HANDLE hCipher, HI_UNF_CIPHER_CTRL_S* pstCtrl)
{
    ...
    CIPHER_CheckHandle(configdata.CIHandle);
...
HI_S32 Cipher_Encrypt(HI_HANDLE hCipher, HI_U32 u32SrcPhyAddr, HI_U32 u32DestPhyAddr, HI_U32 u32ByteLength)
{
    ...
    if ((0 == chnid) || (HI_UNF_CIPHER_MIN_CRYPT_LEN > u32ByteLength)
    ...
    CIPHER_CheckHandle(CIdata.CIHandle);
...
{
    return HAL_Cipher_GetRandomNumber(pu8RandomByte, u32Bytes, u32TimeOutUs);
}
...
HI_S32 HashInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
    ...
    switch(pstHashAttr->eShaType)
    {
        ...
        case HI_UNF_CIPHER_HASH_TYPE_CBCMAC:
        {
            memset(pstHashInfo, 0, sizeof(HASH_INFO_S));
            pstHashInfo->enShaType = pstHashAttr->eShaType;
            ...
            stCipherCtrl.enCaType = HI_UNF_CIPHER_CA_TYPE_HCA;
    ...
    if(Ret == HI_SUCCESS)
    {
        *pHashHandle = HI_HANDLE_MAKEHANDLE(HI_ID_CIPHER, 0, u32SoftId);
...
HI_S32 HashUpdate(HI_HANDLE CIHashHandle, HI_U8 *pu8InputData, HI_U32 u32InputDataLen)
{
    HI_S32 Ret = HI_SUCCESS;
    ...
    if(pstHashInfo->enShaType == HI_UNF_CIPHER_HASH_TYPE_CBCMAC)
    {
        if(!bIsDstMmzInit)
        {
            #if 0
...
HI_S32 HashFinal(HI_HANDLE CIHashHandle, HI_U8 *pu8OutputHash)
{
    HI_S32 Ret = HI_SUCCESS;
    HASH_INFO_S *pstHashInfo = HI_NULL;
    CIPHER_HASH_DATA_S stHashData = {0};
    HI_U32 u32Tmp = 0;
    HI_HANDLE hHashHandle = 0;
...
static HI_S32 HmacKeyInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_U8 au8Hmackey[HASH_BLOCK_SIZE])
{
    HI_S32 ret = HI_SUCCESS;
    HI_HANDLE hHash = 0;
    ...
    ret = HashUpdate(hHash, pstHashAttr->pu8HMACKey, pstHashAttr->u32HMACKeyLen);
    if(ret != HI_SUCCESS)
    {
...
static HI_S32 HmacInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
    HI_S32 ret = HI_SUCCESS;
    ...
    pstHashInfo = (HASH_INFO_S *)&g_stCipherHashData[HI_HANDLE_GET_CHNID(*pHashHandle)];
...
static HI_S32 HmacFinal(HI_HANDLE CIHashHandle, HI_U8 *pu8Output)
{
    HI_S32 ret = HI_SUCCESS;
    HASH_INFO_S *pstHashInfo = HI_NULL;
    HI_HANDLE hHash = HI_INVALID_HANDLE;
    HI_UNF_CIPHER_HASH_ATTS_EX_S stHashAttr = {0};
    HI_U8 hash_sum_1[HASH256_SIGNATURE_SIZE]= {0};
    HI_U8 key_pad[HASH_BLOCK_SIZE] = {0};
    HI_U32 i;
    HI_HANDLE hHashHandle = HI_INVALID_HANDLE;
    ...
    ret = HashFinal(CIHashHandle, hash_sum_1);
    if(ret != HI_SUCCESS)
    {
...
HI_S32 Cipher_HashInit(HI_UNF_CIPHER_HASH_ATTS_EX_S * pstHashAttr, HI_HANDLE * pHashHandle)
{
    ...
    {
        HI_ERR_CIPHER("Invalid point null!\n");
        return HI_FAILURE;
    }
...
HI_S32 Cipher_HashUpdate(HI_HANDLE CIHashHandle, HI_U8 * pu8InputData, HI_U32 u32InputDataLen)
{
    ...
    {
        return HashUpdate(CIHashHandle, pu8InputData, u32InputDataLen);
    }
#ifdef HASH_HAMC_SUPPORT
    ...
    {
        return HmacUpdate(CIHashHandle, pu8InputData, u32InputDataLen);
    }
#endif
    return HI_FAILURE;
}
...
HI_S32 Cipher_HashFinal(HI_HANDLE CIHashHandle, HI_U8 * pu8OutputHash)
{
    ...
    if ((HI_UNF_CIPHER_HASH_TYPE_SHA1 == pstHashInfo->enShaType) || (HI_UNF_CIPHER_HASH_TYPE_SHA256 == pstHashInfo->enShaType) )
    {
        return HashFinal(CIHashHandle, pu8OutputHash);
    }
#ifdef HASH_HAMC_SUPPORT
    else if ((HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA1 == pstHashInfo->enShaType) || (HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA256 == pstHashInfo->enShaType))
    {
        return HmacFinal(CIHashHandle, pu8OutputHash);
    }
#endif
    return HI_FAILURE;
}
