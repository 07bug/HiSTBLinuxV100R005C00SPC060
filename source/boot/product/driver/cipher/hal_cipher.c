...
HI_S32 HAL_Cipher_SetKey(HI_U32 chnId, HI_UNF_CIPHER_CTRL_S* pCtrl)
{
    ...
    {
#if !defined(CHIP_TYPE_hi3796mv200) && !defined(CHIP_TYPE_hi3716mv450) && !defined(NOCS3_0_SUPPORT)
        ...
        else if(pCtrl->enCaType == HI_UNF_CIPHER_CA_TYPE_HCA)
        {
...
HI_S32 HAL_Cipher_AuthCbcMac(HI_U8 *pu8RefCbcMac, HI_U32 u32AppLen)
{
    HI_ERR_CIPHER("Error! Cipher auth cbc mac is unsupported now!\n");
    return HI_FAILURE;
}
...
HI_VOID HAL_Cipher_ClearCbcMacVerifyFlag(HI_VOID)
{
    ...
    CIPHER_WRITE_REG(CIPHER_REG_HL_APP_CBC_CTRL, u32RegValue);

    return;
}

HI_S32 HAL_Cipher_GetRandomNumber(HI_U8 *pu8RandomByte, HI_U32 u32Bytes, HI_U32 u32TimeOutUs)
{
#if defined (CHIP_TYPE_hi3798cv200) \
    || defined(CHIP_TYPE_hi3798mv200) \
    || defined(CHIP_TYPE_hi3798mv300) \
    ...
    while((i < u32Bytes) && (u32TimeOut < 1000))
    {
        ...
        CIPHER_READ_REG(HISEC_COM_TRNG_FIFO_DATA, u32Randnum);
        u32Cnt = MIN(u32Bytes - i, sizeof(HI_U32));
        memcpy(pu8RandomByte + i, &u32Randnum, u32Cnt);
        i += u32Cnt;
        u32TimeOut = 0;
    }
    ...
    while((i < u32Bytes) && (u32TimeOut < 1000))
    {
        ...
        CIPHER_READ_REG(REG_RNG_NUMBER_ADDR, u32Randnum);
        u32Cnt = MIN(u32Bytes - i, sizeof(HI_U32));
        memcpy(pu8RandomByte + i, &u32Randnum, u32Cnt);
        i += u32Cnt;
        u32TimeOut = 0;
