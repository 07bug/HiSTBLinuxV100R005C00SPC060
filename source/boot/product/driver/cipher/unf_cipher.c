...
{
        return Cipher_ConfigHandleEx(hCipher, pstExCtrl);
}
...
{
    HI_UNF_CIPHER_HASH_ATTS_EX_S stHashAttrEx = {0};

    if ((HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA1 == pstHashAttr->eShaType)
       || (HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA224 == pstHashAttr->eShaType)
       || (HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA256 == pstHashAttr->eShaType)
       || (HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA384 == pstHashAttr->eShaType)
       || (HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA512 == pstHashAttr->eShaType))
    {
        if (pstHashAttr->u32HMACKeyLen > sizeof(pstHashAttr->u8HMACKey))
        {
            HI_ERR_CIPHER("Hmac key length %d is invalid.\n", pstHashAttr->u32HMACKeyLen);
            return HI_ERR_CIPHER_INVALID_PARA;
        }
    }

    stHashAttrEx.eShaType        = pstHashAttr->eShaType;
    stHashAttrEx.u32TotalDataLen = pstHashAttr->u32TotalDataLen;
    stHashAttrEx.pu8HMACKey      = pstHashAttr->u8HMACKey;
    stHashAttrEx.u32HMACKeyLen   = pstHashAttr->u32HMACKeyLen;

    return Cipher_HashInit(&stHashAttrEx, pHashHandle);
}
...
HI_S32 HI_UNF_CIPHER_GetRandomNumber(HI_U32 *pu32RandomNumber)
{
    ...
    return Cipher_GetRandomNumber((HI_U8*)pu32RandomNumber, sizeof(HI_U32), 0);
}

HI_S32 HI_UNF_CIPHER_GetMultiRandomBytes(HI_U8 *pu8RandomByte, HI_U32 u32Bytes)
{
    if (NULL == pu8RandomByte)
    {
        HI_ERR_CIPHER("Input pointer is null.\n");
        return HI_ERR_CIPHER_INVALID_POINT;
    }

    return Cipher_GetRandomNumber(pu8RandomByte, u32Bytes, 0);
}
                             ...
                             HI_U8 *pu8OutSign, HI_U32 *pu32OutSignLen)
{
    ...
    CIPHER_RSA_DATA_S stRsaData;
    HI_UNF_CIPHER_HASH_TYPE_E enHashType;
    HI_UNF_CIPHER_HASH_ATTS_EX_S stHashAttr;
    HI_HANDLE HashHandle;
    HI_U8 *pHash;
...
static HI_S32 AES_Encrypt(HI_HANDLE hCipherHandle, HI_U8 *input, HI_U32 datalen, HI_U8 *output)
{
    ...
    u32InputAddrPhy = (HI_U32)devmemalign(16, MAX_DATA_LEN);
    if (0 == u32InputAddrPhy)
    {
        HI_ERR_CIPHER("mmz new for u32InputAddrPhy failed!\n");
        return HI_FAILURE;
    }
    pu8InputAddrVir = (HI_U8*)u32InputAddrPhy;

    u32OutPutAddrPhy = (HI_U32)devmemalign(16, MAX_DATA_LEN);
    if (0 == u32OutPutAddrPhy)
    {
        HI_ERR_CIPHER("mmz new for u32OutPutAddrPhy failed!\n");
        free(pu8InputAddrVir);
        return HI_FAILURE;
    }
    pu8OutputAddrVir = (HI_U8*)u32OutPutAddrPhy;
...
CIPHER_RELEASE_BUF:
    free(pu8InputAddrVir);
    pu8InputAddrVir = NULL;
    free(pu8OutputAddrVir);
    pu8OutputAddrVir = NULL;
    return Ret;
}
...
{
    return Cipher_GetHandleConfig(hCipherHandle, pstCtrl);
}
