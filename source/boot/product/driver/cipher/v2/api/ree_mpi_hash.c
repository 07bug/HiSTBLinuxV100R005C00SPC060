...
HI_S32 CIPHER_CbcMacInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_HANDLE *pHashHandle);
...
static HI_S32 CIPHER_HashInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
...
static HI_S32 CIPHER_HashUpdateBlock(HASH_INFO_S *pstHashInfo, HI_U8 *pu8InputData, HI_U32 u32InputDataLen)
{
    ...
    stHashData.u32DataPhy = (HI_U32)pu8InputData;
    ...
    cipher_memcpy_s(pstHashInfo->u32ShaVal, sizeof(stHashData.u32ShaVal),
...
static HI_S32 CIPHER_HashUpdate(HI_HANDLE hHashHandle, HI_U8 *pu8InputData, HI_U32 u32InputDataLen)
{
    ...
    if ( 0 < pstHashInfo->u8LastBlockSize )
    {
        ...
        ret = CIPHER_HashUpdateBlock(pstHashInfo, pstHashInfo->u8LastBlock, pstHashInfo->u8BlockSize);
        if(ret != HI_SUCCESS)
        {
            HI_ERR_CIPHER("Error, ioctl for hash update failed!\n");
            pstHashInfo->bIsUsed = HI_FALSE;
    ...
    /* save tail data */
    cipher_memset_s(pstHashInfo->u8LastBlock, HASH_BLOCK_SIZE, 0, HASH_BLOCK_SIZE);
    pstHashInfo->u8LastBlockSize = u32InputDataLen;
...
static HI_S32 CIPHER_HashFinal(HI_HANDLE hHashHandle, HI_U8 *pu8OutputHash)
{
    ...
    stHashData.u32DataPhy = (HI_U32)pstHashInfo->u8LastBlock;
    cipher_memcpy_s(stHashData.u32ShaVal, sizeof(stHashData.u32ShaVal),
        pstHashInfo->u32ShaVal, sizeof(pstHashInfo->u32ShaVal));
    stHashData.u32HardChn = SPACC_HASH_CHN;
...
static HI_S32 CIPHER_HmacKeyInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_U8 au8Hmackey[HASH_MAX_BLOCK_SIZE])
{
    HI_HANDLE hHash = 0;
...
static HI_S32 CIPHER_HmacInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
    HI_S32 ret = HI_SUCCESS;
...
static HI_S32 CIPHER_HmacFinal(HI_HANDLE hHashHandle, HI_U8 *pu8Output)
{
    ...
    HI_U8 hash_sum_1[HASH_RESULT_MAX_LEN] = {0};
    HI_U8 key_pad[HASH_MAX_BLOCK_SIZE] = {0};
    HI_U32 u32ShaSize = 0;
    HI_U32 i;

    pstHashInfo = &g_stCipherHashData[hHashHandle];

    cipher_memset_s(&stHashAttr, sizeof(stHashAttr), 0, sizeof(HI_UNF_CIPHER_HASH_ATTS_EX_S));
...
HI_S32 HI_MPI_CIPHER_HashInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
    HI_S32 ret = HI_FAILURE;
...
HI_S32 CIPHER_CbcMacInit(HI_UNF_CIPHER_HASH_ATTS_EX_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
    ...
    HI_UNF_CIPHER_CTRL_S stCipherCtrl = {{0}, {0}, 0};
    HI_HANDLE hCipher = 0;
    HI_U32 u32SoftId = 0;
    HASH_INFO_S *pstHashInfo = HI_NULL;
