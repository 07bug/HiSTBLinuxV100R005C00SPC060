...
HI_VOID HAL_RSA_Enable(HI_VOID)
{
    ...
    HAL_SET_BIT(Value, CIPHER_RSA_CRG_CLK_BIT); /* clock opened */
    HAL_CIPHER_WriteReg(g_u32RsaRegCrg,Value);
    cipher_msleep(1);

    HAL_CLEAR_BIT(Value, CIPHER_RSA_CRG_RST_BIT); /* cancel reset */
    HAL_CIPHER_WriteReg(g_u32RsaRegCrg,Value);
...
HI_VOID HAL_RSA_Disable(HI_VOID)
{
    ...
    HAL_SET_BIT(Value, CIPHER_RSA_CRG_RST_BIT); /* reset */
    HAL_CIPHER_WriteReg(g_u32RsaRegCrg,Value);
    cipher_msleep(1);

    HAL_CLEAR_BIT(Value, CIPHER_RSA_CRG_CLK_BIT); /* clock closed */
    HAL_CIPHER_WriteReg(g_u32RsaRegCrg,Value);
}
