...
HI_S32 DRV_CIPHER_Reset(HI_VOID)
{
    ...
    HAL_SET_BIT(Value, CIPHER_SPACC_CRG_CLK_BIT);   /* clock opened */
    HAL_CIPHER_WriteReg(pvirt,Value);

    HAL_SET_BIT(Value, CIPHER_SPACC_CRG_RST_BIT); /* reset */
    HAL_CIPHER_WriteReg(pvirt,Value);
    cipher_msleep(5);

    HAL_CLEAR_BIT(Value, CIPHER_SPACC_CRG_RST_BIT); /* cancel reset */
    HAL_CIPHER_WriteReg(pvirt,Value);
                                     ...
                                     HI_UNF_CIPHER_SM1_ROUND_E  enSm1Round)
{
    ...
    */
	if ((enSymcAlg == SYMC_ALG_DES) || (enSymcAlg == SYMC_ALG_3DES))
    {
        ...
        {
            if ((enSymcWidth != SYMC_DAT_WIDTH_64)
				 && (enSymcWidth != SYMC_DAT_WIDTH_8)
...
static HI_S32 DRV_CIPHER_ConfigChn(HI_U32 softChnId,  CIPHER_CONFIG_CTRL_EX *pConfig)
{
    ...
    {
        if(pConfig->enCaType == HI_UNF_CIPHER_CA_TYPE_BLPK)
        {
            ret = CA_DecryptSwpk(softChnId, pConfig->u32Key);
        }
        else if(pConfig->enCaType == HI_UNF_CIPHER_CA_TYPE_R2R)
        {
            ret = CA_DecryptCipher(softChnId, pConfig->u32Key);
        }
        else if(pConfig->enCaType == HI_UNF_CIPHER_CA_TYPE_HCA)
        {
            ret = CA_CryptHCA(softChnId);
        }
        else
        {
...
static HI_S32 DRV_CIPHER_SymcWaitDone(SPACC_SYMC_CHN_S *pstChn, HI_U32 u32TimeOut)
{
    ...
    HI_U32 src_addr;
    HI_U32 dst_addr;
    HI_U32 in_addr, in_size;
    HI_U32 out_addr, out_size;
...
static HI_S32 DRV_CIPHER_EncryptMulti(CIPHER_PKG_S *psPkg, HI_BOOL bIsDecrypt)
{
    ...
    for(i=0; i<psPkg->u32PkgNum; i++)
    {
        ...
        if (pstChn->pstNodeList[i].bOddKey)
        {
            ...
            /* 2. each node length must be a multiple of 64*/
...
HI_S32 DRV_CIPHER_Release(HI_VOID * inode, HI_VOID * file)
{
    HI_U32 i;
...
HI_S32 DRV_CIPHER_Open(HI_VOID * inode, HI_VOID * file)
{
    return HI_SUCCESS;
}
...
{
    HI_ERR_CIPHER("unsupport auth cbc mac\n");
    return HI_FAILURE;
}
