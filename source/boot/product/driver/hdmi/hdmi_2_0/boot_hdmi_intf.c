...
static HI_S32 HdmiDeviceInit(HDMI_DEVICE_S * pstHdmiDev)
{
    ...
    pstAppAttr->bEnableSpdInfoFrame  = HI_FALSE;
    pstAppAttr->bEnableMpegInfoFrame = HI_FALSE;
...
HI_S32 DRV_HDMI_Open(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    if ((HI_NULL == pstHdmiDev->pstHdmiHal) || (s32Ret != HI_SUCCESS))
    {
...
HI_S32 DRV_HDMI_Start(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_PhyPowerEnableSet, pstHdmiDev->pstHdmiHal, HI_TRUE);
...
HI_S32 DRV_HDMI_Stop(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_PhyPowerEnableSet, pstHdmiDev->pstHdmiHal, HI_FALSE);
    return HI_SUCCESS;
}
...
static HI_S32 HdmiVideoPathSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S* pstVOAttr)
{
    ...
    stVideoCfg.enOutQuantization= (pstVOAttr->enVideoTiming > HDMI_VIDEO_TIMING_640X480P_60000 ) ? HDMI_QUANTIZATION_FULL : HDMI_QUANTIZATION_LIMITED;
    stVideoCfg.enInQuantization= stVideoCfg.enOutQuantization; //we don't known VO's vaule.
...
static HI_S32 HdmiScrambleEnableSet(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bEnable)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_ScdcStatusGet, pstHdmiDev->pstHdmiHal, &stCurrentStatus);
...
static HI_S32 HdmiScrambleEnableSet(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bEnable)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_ScdcConfig, pstHdmiDev->pstHdmiHal, &stScdcConfig);
...
HI_S32 DRV_HDMI_AttrSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_ATTR_S *pstAttr)
{
    ...
    pstHdmiDev->stAttr.stVOAttr.enRGBQuantization = HDMI_QUANTIZATION_RANGE_DEFAULT;
    pstHdmiDev->stAttr.stVOAttr.enYCCQuantization = HDMI_YCC_QUANTIZATION_RANGE_LIMITED;
