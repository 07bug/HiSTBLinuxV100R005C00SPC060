...
static HI_S32 DRV_KLAD_SESSION_Create(HI_HANDLE* create_handle)
{
    ...
    {
        create = HI_KLAD_MAKEHANDLE(HI_ID_KLAD, i);
        ...
        if (found == HI_FALSE)
        {
            ...
            {
                session->session_key_flag[j] = HI_FALSE;
            }
...
HI_S32 DRV_KLAD_RunCACallBack(unsigned int cmd, void* arg)
{
    HI_S32 ret = HI_FAILURE;
    HI_HANDLE handle = 0;
