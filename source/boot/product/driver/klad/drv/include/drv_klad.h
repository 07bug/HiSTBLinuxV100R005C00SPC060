...
#define DRV_KLAD_CHECK_TARGET_MOD(mod) \
    do {\
        if ((mod) >= (DRV_KLAD_TARGET_BUTT)) \
        {\
            HI_ERR_KLAD("invalid MOD: %d\n", (mod)); \
...
#define DRV_KLAD_CHECK_ENGINE(engine) \
    do{ \
        if (engine >= HI_UNF_KLAD_TARGET_ENGINE_BUTT)\
        {\
            HI_ERR_KLAD("invalid klad engine: %d\n", (engine)); \
            return HI_ERR_KLAD_INVALID_ENGINE;\
        }\
    }while(0)

#define DRV_KLAD_CHECK_ATTR_FLAG(session) \
    do{ \
        if (HI_TRUE != session->attr_flag) \
        {\
            HI_ERR_KLAD("invalid session attr flag: %d\n", (session->attr_flag)); \
            return HI_ERR_KLAD_NOT_SET_ATTR; \
        }\
    }while(0)

#define DRV_KLAD_CHECK_SESSION_KEY_FLAG(session,index) \
    do{ \
        if (HI_TRUE != session->session_key_flag[index]) \
        {\
            HI_ERR_KLAD("invalid session Session flag: %d [index=%d]\n", (session->content_key_flag), (index)); \
            return HI_ERR_KLAD_NOT_SET_SESSION_KEY; \
        }\
    }while(0)

#define DRV_KLAD_CHECK_CONTENT_KEY_FLAG(session) \
    do{ \
        if (HI_TRUE != session->content_key_flag) \
        {\
            HI_ERR_KLAD("invalid session contentkey flag: %d\n", (session->content_key_flag)); \
...
#define HI_KLAD_GET_CATYPE(handle)    (((handle) >> 8) & 0x3f)
