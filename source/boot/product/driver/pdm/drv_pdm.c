...
static HI_VOID LoaderGetDefDispParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam)
{
    ...
    pstDispParam->enPixelFormat    = HI_GFX_PF_8888;
...
HI_VOID DRV_PDM_UpdateCustomTimingDisp1(HI_DB_TABLE_S *pstTable, HI_UNF_DISP_TIMING_S *pstTiming, HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_VFB, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->VFB), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_VBB, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->VBB), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_VACT, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->VACT), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_HFB, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->HFB), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_HBB, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->HBB), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_HACT, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->HACT), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_VPW, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->VPW), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_HPW, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->HPW), pstKey->u32ValueSize);
    }
...
HI_VOID DRV_PDM_UpdateCustomTimingDisp2(HI_DB_TABLE_S *pstTable, HI_UNF_DISP_TIMING_S *pstTiming, HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_IDV, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->IDV), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_IHS, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->IHS), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_IVS, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->IVS), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_CLKR, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->ClockReversal), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_DW, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->DataWidth), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_ITFMT, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->ItfFormat), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_DITE, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->DitherEnable), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_CLK0, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->ClkPara0), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_CLK1, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstTiming->ClkPara1), pstKey->u32ValueSize);
    }
...
static HI_S32 PDM_UpdateCustomTiming(HI_DB_TABLE_S *pstTable, HI_UNF_DISP_TIMING_S *pstTiming)
{
    ...
    DRV_PDM_UpdateCustomTimingDisp1(pstTable, pstTiming, &stKey);

    DRV_PDM_UpdateCustomTimingDisp2(pstTable, pstTiming, &stKey);
...
HI_S32 DRV_PDM_TransTVFormat(HI_UNF_ENC_FMT_E enSrcFmt, HI_UNF_ENC_FMT_E *penHdFmt, HI_UNF_ENC_FMT_E *penSdFmt)
{
    switch (enSrcFmt)
    {
...
HI_S32 DRV_PDM_TransVGAFormat(HI_UNF_ENC_FMT_E enSrcFmt, HI_UNF_ENC_FMT_E *penHdFmt, HI_UNF_ENC_FMT_E *penSdFmt)
{
    switch (enSrcFmt)
    {
...
HI_VOID PDM_TransFomat(HI_UNF_ENC_FMT_E enSrcFmt, HI_UNF_ENC_FMT_E *penHdFmt, HI_UNF_ENC_FMT_E *penSdFmt)
{
    ...
    Ret = DRV_PDM_TransTVFormat(enSrcFmt, penHdFmt, penSdFmt);
    if (Ret == HI_SUCCESS)
    {
        return;
    }

    Ret = DRV_PDM_TransVGAFormat(enSrcFmt, penHdFmt, penSdFmt);
    if (Ret == HI_SUCCESS)
    {
        return;
    }
...
HI_S32 PDM_MatchIntFormat(HI_UNF_ENC_FMT_E enSrcFmt, HI_UNF_ENC_FMT_E *penDstFmt)
{
...
HI_S32 PDM_MatchFormat(HI_UNF_ENC_FMT_E enSrcFmt, HI_UNF_ENC_FMT_E *penDstFmt)
{
    ...
    */
    ...
    Ret = PDM_MatchIntFormat(enSrcFmt, penDstFmt);
    if (HI_SUCCESS == Ret)
    {
        return HI_SUCCESS;
    }
...
HI_VOID DRV_PDM_GetCustomTimingDisp1(HI_DB_TABLE_S *pstTable, HI_UNF_DISP_TIMING_S *pstTiming, HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;
    ...
    if (HI_SUCCESS == Ret)
    {
        pstTiming->VFB = *(HI_U32*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_VBB, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->VBB = *(HI_U32*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_VACT, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->VACT = *(HI_U32*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_HFB, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->HFB = *(HI_U32*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_HBB, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->HBB = *(HI_U32*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_HACT, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->HACT = *(HI_U32*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_VPW, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->VPW = *(HI_U32*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_HPW, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->HPW = *(HI_U32*)(pstKey->pValue);
    }
...
HI_VOID DRV_PDM_GetCustomTimingDisp2(HI_DB_TABLE_S *pstTable, HI_UNF_DISP_TIMING_S *pstTiming, HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;
    ...
    if (HI_SUCCESS == Ret)
    {
        pstTiming->IDV = *(HI_BOOL*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_IHS, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->IHS = *(HI_BOOL*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_IVS, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->IVS = *(HI_BOOL*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_CLKR, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->ClockReversal = *(HI_BOOL*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_DW, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->DataWidth = *(HI_UNF_DISP_INTF_DATA_WIDTH_E*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_ITFMT, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->ItfFormat = *(HI_UNF_DISP_INTF_DATA_FMT_E*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_DITE, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->DitherEnable = *(HI_BOOL*)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_TIMING_CLK0, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstTiming->ClkPara0 = *(HI_U32*)(pstKey->pValue);
    }
...
static HI_S32 PDM_GetCustomTiming(HI_DB_TABLE_S *pstTable, HI_UNF_DISP_TIMING_S *pstTiming)
{
    ...
    DRV_PDM_GetCustomTimingDisp1(pstTable, pstTiming, &stKey);

    DRV_PDM_GetCustomTimingDisp2(pstTable, pstTiming, &stKey);
...
HI_S32 DRV_PDM_GetDispBaseParam(HI_UNF_DISP_E enDisp,HI_DISP_PARAM_S *pstDispParam, HI_DB_TABLE_S *pstTable,HI_DB_KEY_S *pstKey)
{
    HI_S32                  Ret;
    HI_U32                  BaseDBAddr;
    HI_DB_S                 stBaseDB;
    ...
    {
        Ret = HI_DB_GetTableByName(&stBaseDB, MCE_BASE_TABLENAME_DISP0, pstTable);
    }
    else
    {
        Ret = HI_DB_GetTableByName(&stBaseDB, MCE_BASE_TABLENAME_DISP1, pstTable);
    }
    ...
    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_FMT, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstDispParam->enFormat = *(HI_UNF_ENC_FMT_E *)(pstKey->pValue);

        if (HI_UNF_ENC_FMT_BUTT == pstDispParam->enFormat)
        {
            Ret = PDM_GetCustomTiming(pstTable, &(pstDispParam->stDispTiming));
        }
...
HI_VOID DRV_PDM_GetIntfParam(HI_UNF_DISP_E enDisp,HI_DISP_PARAM_S *pstDispParam, HI_DB_TABLE_S *pstTable,HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;
    ...
    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_YPBPR, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_YPBPR] = *(HI_UNF_DISP_INTF_S *)(pstKey->pValue);
...
HI_VOID PDM_GetDispColorAndBLightParam(HI_UNF_DISP_E enDisp,HI_DISP_PARAM_S *pstDispParam,HI_DB_TABLE_S *pstTable,HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;
...
HI_VOID PDM_GetDispRectParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam, HI_DB_TABLE_S *pstTable,HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;
    ...
    if (HI_SUCCESS == Ret)
    {
        pstDispParam->stOffsetInfo.u32Left = *(HI_U32 *)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_DISP_T, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstDispParam->stOffsetInfo.u32Top = *(HI_U32 *)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_DISP_R, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstDispParam->stOffsetInfo.u32Right = *(HI_U32 *)(pstKey->pValue);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_DISP_B, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstDispParam->stOffsetInfo.u32Bottom = *(HI_U32 *)(pstKey->pValue);
    }
...
HI_S32 HI_DRV_PDM_GetDispParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam)
{
    ...
    Ret = DRV_PDM_GetDispBaseParam(enDisp,pstDispParam,&stTable, &stKey);
    if (HI_SUCCESS != Ret)
    {
        HI_INFO_PDM("ERR: DRV_PDM_GetDispBaseParam, use default baseparam!\n");
        return HI_SUCCESS;
    }

    DRV_PDM_GetIntfParam(enDisp,pstDispParam, &stTable, &stKey);
    PDM_GetDispColorAndBLightParam(enDisp,pstDispParam, &stTable, &stKey);

    PDM_GetDispRectParam(enDisp,pstDispParam, &stTable, &stKey);

    Ret = HI_DB_GetKeyByName(&stTable, MCE_BASE_KEYNAME_PF, &stKey);
    if (HI_SUCCESS == Ret)
    {
        pstDispParam->enPixelFormat = *(HI_GFX_PF_E *)(stKey.pValue);
        if ((pstDispParam->enPixelFormat < HI_GFX_PF_4444) || (pstDispParam->enPixelFormat > HI_GFX_PF_0888))
        {
...
HI_S32 DRV_PDM_UpdateBaseDispParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam,HI_DB_TABLE_S *pstTable, HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;
    HI_DB_S stBaseDB;
    HI_U32 BaseDBAddr;
    ...
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->enFormat), pstKey->u32ValueSize);

        if (HI_UNF_ENC_FMT_BUTT == pstDispParam->enFormat)
        {
            Ret = PDM_UpdateCustomTiming(pstTable, &(pstDispParam->stDispTiming));
        }
    }
...
HI_VOID DRV_PDM_UpdateIntfParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam,HI_DB_TABLE_S *pstTable, HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;
    ...
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_HDMI]), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_YPBPR, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_YPBPR]), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_CVBS, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_CVBS]), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_RGB, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_RGB]), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_SVIDEO, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_SVIDEO]), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_BT1120, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_BT1120]), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_BT656, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_BT656]), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_LCD, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_LCD]), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_INTF_VGA, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stIntf[HI_UNF_DISP_INTF_TYPE_VGA]), pstKey->u32ValueSize);
    }
...
HI_VOID DRV_PDM_UpdateDispColorAndBLightParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam,HI_DB_TABLE_S *pstTable, HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;
    ...
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->u32HuePlus), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_SATU, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->u32Saturation), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_CONTR, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->u32Contrast), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_BRIG, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->u32Brightness), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_BGCOLOR, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stBgColor), pstKey->u32ValueSize);
    }

    Ret = HI_DB_GetKeyByName(pstTable, MCE_BASE_KEYNAME_ASPECT, pstKey);
    if (HI_SUCCESS == Ret)
    {
        memcpy(pstKey->pValue, &(pstDispParam->stAspectRatio), pstKey->u32ValueSize);
    }
...
HI_S32 HI_DRV_PDM_UpdateDispParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam)
{
    ...
    Ret = DRV_PDM_UpdateBaseDispParam(enDisp, pstDispParam, &stTable, &stKey);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PDM("ERR: DRV_PDM_UpdateBaseDispParam!");
        return Ret;
    }

    DRV_PDM_UpdateIntfParam(enDisp,pstDispParam, &stTable, &stKey);
    ...
    DRV_PDM_UpdateDispColorAndBLightParam(enDisp,pstDispParam, &stTable, &stKey);
...
HI_S32 DRV_PDM_GetMceParam(HI_MCE_PARAM_S *pstMceParam,HI_DB_S *pstPlayDB, HI_DB_TABLE_S *pstTable, HI_DB_KEY_S *pstKey)
{
    HI_S32 Ret;

    Ret = HI_DB_GetTableByName(pstPlayDB, MCE_PLAY_TABLENAME, pstTable);
    if (HI_SUCCESS != Ret)
    {
        HI_INFO_PDM("ERR: HI_DB_GetTableByName!\n");
        return HI_FAILURE;
    ...
    Ret = HI_DB_GetKeyByName(pstTable, MCE_PLAY_KEYNAME_FLAG, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstMceParam->u32CheckFlag = *(HI_U32 *)(pstKey->pValue);
    }
    else
    {
    ...
    Ret = HI_DB_GetKeyByName(pstTable, MCE_PLAY_KEYNAME_DATALEN, pstKey);
    if (HI_SUCCESS == Ret)
    {
        pstMceParam->u32PlayDataLen = *(HI_U32 *)(pstKey->pValue);
    }
    else
    {
...
HI_S32 HI_DRV_PDM_GetMceParam(HI_MCE_PARAM_S *pstMceParam)
{
    ...
    Ret = DRV_PDM_GetMceParam(pstMceParam, &stPlayDB, &stTable, &stKey);
    if(HI_SUCCESS != Ret)
    {
        HI_ERR_PDM("ERR: DRV_PDM_GetMceParam!\n");
        return HI_FAILURE;
    }
...
/* Release reserve memory in boot */
HI_S32 HI_DRV_PDM_ReleaseReserveMem(HI_U32 *pu32PhyAddr)
{
    if(pu32PhyAddr == HI_NULL)
    {
        HI_ERR_PDM("pu32PhyAddr is null.\n");
        return HI_FAILURE;
    }
    HI_MEM_Free(pu32PhyAddr);
    return HI_SUCCESS;
}

/* Alloc CAS private reserve memory in boot */
HI_S32 HI_DRV_PDM_GetCASReserveMem(const HI_CHAR *BufName, HI_U32 u32Len, HI_U32 *pu32PhyAddr)
{
    if (g_u32ResBufNum >= PDM_MAX_RESBUF_NUM)
    {
        HI_ERR_PDM("ERR: you have alloc the max num reserve mem!\n");
    ...
    if ((HI_NULL == BufName) || (HI_NULL == pu32PhyAddr) || (0 == u32Len))
    {
        HI_ERR_PDM("invalid input!\n");
        return HI_FAILURE;
    }

    if (0 != u32Len % 4096)
    {
        u32Len = u32Len/4096 * 4096 + 4096;
    }

    if (u32Len > (512*1024))
    {
        HI_ERR_PDM("ERR, u32Len is invalid, u32Len=%x\n", u32Len);
        return HI_FAILURE;
    }

    *pu32PhyAddr = 0x1F00000;

    strncpy(g_stResBuf[g_u32ResBufNum].Name, BufName, strlen(BufName));
    g_stResBuf[g_u32ResBufNum].PhyAddr = 0x1F00000;
    g_stResBuf[g_u32ResBufNum].Len = u32Len;
