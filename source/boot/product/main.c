...
#ifdef HI_BOOT_FBL
extern HI_S32 HI_FIRSTBOOT_SSA_Entry(HI_U32 u32Address, HI_U32 u32Length, HI_U32 u32PartitionSize);
#else
...
HI_S32 product_init(HI_VOID)
{
    ...
    || defined(HI_IDWORD_LOCK_SUPPORT)
    HI_OTP_LockIdWord();
#endif
...
#if defined(HI_ADVCA_VERIFY_ENABLE) && defined(HI_ADVCA_SUPPORT)
    HI_FIRSTBOOT_SSA_Entry(CFG_SECONDBOOT_ADDR, CFG_SECONDBOOT_SIZE, CFG_SECONDBOOT_PARTITION_SIZE);
#endif
...
* others:       :  NA
*****************************************************************************/
HI_S32 Logo_Main(HI_BOOL bNeedFastplay, HI_PDM_MEMINFO_S* pstMemInfo, HI_BOOL bDispParmValid)
{
    HI_S32           s32Ret = HI_SUCCESS;
...
#endif
    HI_BASE_GFX_LOGO_INFO InputBaseLogoInfo;
    ...
    if (HI_FALSE == bDispParmValid)
    {
        HI_INFO_MCE("HI_PDM_GetGrcParam err! DispParm is invalid\n");
        return HI_FAILURE;
    }
    ...
    || defined(CHIP_TYPE_hi3716mv420)  \
        ...
        if ((HI_UNF_DISP_INTF_TYPE_HDMI == g_stDispParam1.stIntf[HI_UNF_DISP_INTF_TYPE_HDMI].enIntfType)
            && (HI_UNF_HDMI_ID_0 == g_stDispParam1.stIntf[HI_UNF_DISP_INTF_TYPE_HDMI].unIntf.enHdmi))
        {
            HI_UNF_HDMI_Init();
        ...
        memset(&InputBaseLogoInfo, 0x0, sizeof(HI_BASE_GFX_LOGO_INFO));
        InputBaseLogoInfo.DispToSD      = (g_stDispParam0.enSrcDisp != HI_UNF_DISPLAY1) ? (HI_TRUE) : (HI_FALSE);
        InputBaseLogoInfo.LogoDataLen   = stLogoParam.u32LogoLen;
        InputBaseLogoInfo.LogoDataBuf   = u32LogoDataAddr;

        InputBaseLogoInfo.DispColorFmt[CONFIG_GFX_DISP_LOGO_SD] = g_stDispParam0.enPixelFormat;
        InputBaseLogoInfo.DispVirtScreenWidth[CONFIG_GFX_DISP_LOGO_SD]  = g_stDispParam0.u32VirtScreenWidth;
        InputBaseLogoInfo.DispVirtScreenHeight[CONFIG_GFX_DISP_LOGO_SD] = g_stDispParam0.u32VirtScreenHeight;

        InputBaseLogoInfo.DispColorFmt[CONFIG_GFX_DISP_LOGO_HD] = g_stDispParam1.enPixelFormat;
        InputBaseLogoInfo.DispVirtScreenWidth[CONFIG_GFX_DISP_LOGO_HD]  = g_stDispParam1.u32VirtScreenWidth;
        InputBaseLogoInfo.DispVirtScreenHeight[CONFIG_GFX_DISP_LOGO_HD] = g_stDispParam1.u32VirtScreenHeight;

        s32Ret = HI_GFX_ShowLogo(&InputBaseLogoInfo);
...
int fastapp_entry(int argc, char* argv[])
{
    int     LogoEnable = 1;
    char*   pLogo;
    HI_BOOL bDispParmValid = HI_FALSE;
...
#ifdef HI_ADVCA_NOCS3_RPC_SERVER
    extern HI_VOID bsd_startServer(HI_U32);
    HI_PRINT("\nbsd start server\n");
    bsd_startServer(0);
#endif
    ...
    HI_SSA_Entry();
    //return 0;
#endif // HI_ADVCA_VERIFY_ENABLE
#endif //HI_ADVCA_SUPPORT

    Ret = GetDispParam();
    if (HI_SUCCESS == Ret)
    {
        bDispParmValid = HI_TRUE;
    }

    HI_PQ_Init(bDispParmValid, &g_stDispParam0, &g_stDispParam1);
    ...
    if (1 == LogoEnable)
    {
        Logo_Main(HI_TRUE, HI_NULL, bDispParmValid);
    }
