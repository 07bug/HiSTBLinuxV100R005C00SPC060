...
HI_S32 ssaShowLogo(HI_VOID)
{
    ...
    if(HI_SUCCESS != s32Ret)
    {
        SSA_PrintFuncErr(HI_GFX_ShowLogo, s32Ret);
...
static HI_S32 ssaVerify(HI_VOID)
{
    HI_S32 s32Ret = HI_FAILURE;

    SSA_DBG_FuncEnter();

    SSA_INFO("verify: %s begin\n", SSA_PARTNAME_BOOTARGS);
    s32Ret = ssaVerifyImageByName(SSA_PARTNAME_BOOTARGS, HI_NULL_PTR, SSA_VERIFYMODE_COMPLETE);
    if (HI_SUCCESS != s32Ret)
    {
        SSA_PrintFuncErr(ssaVerifyImageByName, s32Ret);
        return s32Ret;
    }
    SSA_INFO("verify: %s end\n\n", SSA_PARTNAME_BOOTARGS);

    SSA_INFO("verify: %s begin\n", SSA_PARTNAME_TRUSTEDCORE);
    s32Ret = ssaVerifyImageByName(SSA_PARTNAME_TRUSTEDCORE, HI_NULL_PTR, SSA_VERIFYMODE_COMPLETE);
    if (HI_SUCCESS != s32Ret)
    {
        SSA_PrintFuncErr(ssaVerifyImageByName, s32Ret);
        return s32Ret;
    ...
    SSA_INFO("verify: %s begin\n", SSA_PARTNAME_KERNEL);
    s32Ret = ssaVerifyImageByName(SSA_PARTNAME_KERNEL, HI_NULL_PTR, SSA_VERIFYMODE_COMPLETE);
    if (HI_SUCCESS != s32Ret)
    {
        SSA_PrintFuncErr(ssaVerifyImageByName, s32Ret);
        return s32Ret;
    ...
    SSA_INFO("verify: %s begin\n", SSA_PARTNAME_ROOTFS);
    s32Ret = ssaVerifyImageByName(SSA_PARTNAME_ROOTFS, HI_NULL_PTR, SSA_VERIFYMODE_COMPLETE);
    if (HI_SUCCESS != s32Ret)
    {
        SSA_PrintFuncErr(ssaVerifyImageByName, s32Ret);
        return s32Ret;
...
static HI_VOID ssaReset(HI_VOID)
{
...
HI_S32 HI_SSA_Entry(HI_VOID)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        SSA_PrintFuncErr(ssaVerify, s32Ret);
        ssaReset();
    }
    else
    {
