...
extern HI_S32 cmdline_parts_init(HI_CHAR *bootargs);
extern HI_S32 get_part_info(HI_U8 partindex, HI_U64 *start, HI_U64 *size);

HI_S32 find_part_from_devname(HI_CHAR *media_name, HI_CHAR *bootargs,
...
HI_S32 emmc_raw_init(HI_CHAR *bootargs)
{
    HI_U8  aucBuf[512];
    HI_S32  dev_fd;
    HI_S32  tmp_fd = -1;
    ...
    g_stEmmcFlash.u64RawAreaSize  = (HI_U64)strtoull((HI_CHAR *)aucBuf, NULL, 10)
                                    * EMMC_SECTOR_SIZE - EMMC_RAW_AREA_START;

    return HI_SUCCESS;
}

static HI_S32 emmc_flash_probe(HI_VOID)
{
    HI_S32 dev;
#if defined (ANDROID)
                         ...
                         HI_U64 u64Length)
{
    ...
    memset(pstEmmcCB, 0, sizeof(EMMC_CB_S));
...
EMMC_CB_S *emmc_node_open(const HI_U8 *pu8Node)
{
    ...
    if (-1 == (fd = open((const HI_CHAR*)pu8Node, O_RDWR)))
    {
        HI_PRINT("no devices available.");
        return NULL;
    }
                      ...
                      HI_U64    u64Length)
{
    ...
    cmd.from = (HI_U32)(u64Start >> 9);
    cmd.nr   = (HI_U32)(u64Length >> 9);
    cmd.arg = 0x01;
