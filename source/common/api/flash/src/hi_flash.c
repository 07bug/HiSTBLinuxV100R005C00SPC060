...
static HI_FLASH_TYPE_E get_flashtype_by_bootargs(HI_CHAR *pszPartitionName)
{
    ...
    {
        HI_ERR_FLASH("Invalid pszPartitionName, should not include ' '.\n");
        return (HI_FLASH_TYPE_BUTT);
    }
            ...
            || (HI_FLASH_STAT_INSTALL == g_eDevStat[HI_FLASH_TYPE_NAND_0]))
    {
        ...
        if (fp)
        {
            ...
            for(i=0; fgets(line, sizeof(line), fp) != 0; i++)
            {
                ...
                gPartInfo[i].PartSize = (HI_U64)(HI_S64)strtoull((const HI_CHAR*)argv[1], (HI_CHAR**)NULL, 16);
                gPartInfo[i].BlockSize = (HI_U64)(HI_S64)strtol((const HI_CHAR*)argv[2], (HI_CHAR**)NULL,16); //erase size
                memset(gPartInfo[i].PartName, 0, sizeof(gPartInfo[i].PartName));
                strncpy(gPartInfo[i].PartName, (HI_CHAR*)(argv[3]+1), (strlen((HI_CHAR*)argv[3])-2));
...
static HI_S32 All_FLash_Init()
{
    ...
    if (!gInitFlag)
    {
        ...
        if( HI_SUCCESS == emmc_raw_init((HI_CHAR *)g_au8Bootargs))
        {
                                     ...
                                     HI_U64 u64Len)
{
    ...
    gFlashInfo[hFlash].fd = (HI_SLONG *)(HI_SLONG)(SPAN_PART_HANDLE + hFlash);
...
static HI_HANDLE HI_Flash_OpenByName_Emmc(HI_CHAR *pPartitionName)
{
    HI_U64 u64Address = 0;
    HI_U64 u64Len = 0;
    HI_U32 hFlash = 0;
    HI_CHAR *ptr;
    HI_CHAR media_name[20] = {0};
    EMMC_CB_S *pstEmmcCB;
    ...
    (HI_VOID)pthread_mutex_lock(&gFlashMutex);
...
#endif
        {
            ...
            if (find_part_from_devname(media_name, (HI_CHAR *)g_au8Bootargs,
        ...
        ptr = strstr((HI_CHAR*)g_au8Bootargs, "mmcblk0:");
        if(NULL != ptr)
        {
            memset(media_name, 0 , sizeof(media_name));
            strncpy(media_name, "mmcblk0", sizeof(media_name) - 1);
...
static HI_S32 check_flash_part(HI_FLASH_TYPE_E enFlashType,
...
static HI_HANDLE HI_Flash_OpenByName_Spi_Nand(HI_FLASH_TYPE_E enFlashType,
                                              HI_CHAR *pPartitionName)
{
       ...
       || strstr((char*)g_au8Bootargs, "hi_sfc:"))
    {
         ...
         * Warning 676: Possibly negative subscript (-1) in operator '['
         */
       if ((i = check_flash_part(enFlashType, pPartitionName)) < 0)
        {
        (HI_VOID)pthread_mutex_unlock(&gFlashMutex);
        return (HI_HANDLE)INVALID_FD;
        }
                                     ...
                                     HI_CHAR *pPartitionName)
{
    ...
    if( HI_FLASH_TYPE_EMMC_0 == enFlashType)
    {
        return HI_Flash_OpenByName_Emmc(pPartitionName);

    }

    if( (HI_FLASH_TYPE_SPI_0 == enFlashType) || (HI_FLASH_TYPE_NAND_0 == enFlashType))
    {
        return HI_Flash_OpenByName_Spi_Nand(enFlashType, pPartitionName);
    }
...
HI_HANDLE HI_Flash_OpenByName(HI_CHAR *pPartitionName)
{
    ...
    if((HI_FLASH_TYPE_EMMC_0 == enFlashType)
        && (NULL != (ptr = strstr((HI_CHAR*)g_au8Bootargs, "mmcblk0:"))))
    {
        memset(media_name, 0 , sizeof(media_name));
        strncpy(media_name, "mmcblk0", sizeof(media_name) - 1);
        media_name[sizeof(media_name) - 1] = '\0';
    ...
    if(strstr((HI_CHAR*)g_au8Bootargs, "hinand:")
       || strstr((HI_CHAR*)g_au8Bootargs, "hi_sfc:"))
    {
        if ((i = check_flash_part(enFlashType, pPartitionName) < 0))
        {
        (HI_VOID)pthread_mutex_unlock(&gFlashMutex);
...
static HI_S32 Compensate_Nand_Address(HI_U64 u64Address, HI_U64 *StartAddr)
{
    ...
    idx = (HI_S32)(u64Address >> BlockShift);
    ...
    *StartAddr += (HI_U64)(((HI_U32)idx) << BlockShift);
...
HI_S32 HI_Flash_Erase(HI_HANDLE hFlash, HI_U64 u64Address, HI_U64 u64Len)
{
    ...
    ret = (HI_S32)gFlashInfo[hFlash].pFlashopt->raw_erase((HI_S32)(HI_SLONG)gFlashInfo[hFlash].fd, (unsigned long long)StartAddr, u64Len, gFlashInfo[hFlash].OpenAddr, LimitLeng);
    HI_INFO_FLASH("end.\n");
    (HI_VOID)pthread_mutex_unlock(&gFlashMutex);
...
HI_S64 HI_Flash_Erase64(HI_HANDLE hFlash, HI_U64 u64Address, HI_U64 u64Len)
{
    ...
    ret = (HI_S64)gFlashInfo[hFlash].pFlashopt->raw_erase((HI_S32)(HI_SLONG)gFlashInfo[hFlash].fd, (unsigned long long)StartAddr, u64Len, gFlashInfo[hFlash].OpenAddr, LimitLeng);
    HI_INFO_FLASH("end.\n");
    (HI_VOID)pthread_mutex_unlock(&gFlashMutex);
...
HI_S32 HI_Flash_Read(HI_HANDLE hFlash, HI_U64 u64Address, HI_U8 *pBuf, HI_U32 u32Len, HI_U32 u32Flags)
{
    ...
    ret = gFlashInfo[hFlash].pFlashopt->raw_read((HI_S32)(HI_SLONG)gFlashInfo[hFlash].fd, (unsigned long long *)&StartAddr, pBuf, u32Len, gFlashInfo[hFlash].OpenAddr, LimitLeng, wtihoob, 1);
...
HI_S32 HI_Flash_Write(HI_HANDLE hFlash, HI_U64 u64Address, HI_U8 *pBuf, HI_U32 u32Len, HI_U32 u32Flags)
{
    ...
    if (erase)
    {
        ...
        ret = gFlashInfo[hFlash].pFlashopt->raw_erase((HI_SLONG)gFlashInfo[hFlash].fd, (unsigned long long)StartAddr, (unsigned long long)u32EraseLen, gFlashInfo[hFlash].OpenAddr, LimitLeng);
        if (0 >= ret)
        {
    ...
    ret = gFlashInfo[hFlash].pFlashopt->raw_write((HI_S32)(HI_SLONG)gFlashInfo[hFlash].fd, &StartAddr, pBuf, u32Len, gFlashInfo[hFlash].OpenAddr, LimitLeng, wtihoob);
