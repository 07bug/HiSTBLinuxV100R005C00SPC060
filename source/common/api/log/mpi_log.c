...
HI_S32 HI_MPI_LogInit(HI_VOID)
{
    ...
    if(-1 == g_s32DbgDevFd)
    {
        ...
        VirAddr = (HI_U32*)HI_MPI_MMZ_Map(PhyAddr, HI_FALSE);
        if(NULL ==  VirAddr)
        {
            (HI_VOID)MPILogDeviceClose(g_s32DbgDevFd);
...
HI_VOID HI_MPI_LogDeInit(HI_VOID)
{
    ...
    {
        g_LogModInit = 0;
        if (HI_SUCCESS != HI_MPI_MMZ_Unmap(g_u32LogInfoPhyAddr))
        {
            HI_LOG_UNLOCK();
            HI_WARN_LOG("log info HI_MPI_MMZ_Unmap failed\n");
            HI_LOG_LOCK();
        }
