...
HI_U32 stat_mmz_phyaddr_base = 0;
STAT_USERSPACE_S *p_stat_mmz_virtual_base = NULL;
...
HI_S32 HI_MPI_STAT_Init(HI_VOID)
{
    ...
    if (s_s32StatFd == -1)
    {
        ...
        {
            HI_STAT_UNLOCK();
            HI_ERR_STAT("ERROR: can not open %s device.\n", "/dev/"UMAP_DEVNAME_STAT);
            return HI_FAILURE;
        }

        ret = ioctl(s_s32StatFd, UMAPC_CMPI_STAT_GET_PHYADDR_BASE, &stat_mmz_phyaddr_base);
        if (-1 == ret)
        {
            HI_STAT_UNLOCK();
            HI_ERR_STAT("ioctl UMAPC_CMPI_STAT_REGISTER faild.\n");
            return HI_FAILURE;
        }

        p_stat_mmz_virtual_base = (STAT_USERSPACE_S *)HI_MPI_MMZ_Map(stat_mmz_phyaddr_base, HI_FALSE);
        if (HI_NULL == p_stat_mmz_virtual_base)
        {
            HI_STAT_UNLOCK();
            HI_ERR_STAT("HI_MPI_MMZ_Map stat_mmz_phyaddr_base:0x%x faild. \n", stat_mmz_phyaddr_base);
            return HI_FAILURE;
        }
...
HI_S32 HI_MPI_STAT_DeInit(HI_VOID)
{
    ...
    if (s_s32StatFd != -1)
    {
        ...
        if (HI_SUCCESS != HI_MPI_MMZ_Unmap(stat_mmz_phyaddr_base))
        {
            HI_ERR_STAT("HI_MPI_MMZ_Unmap faild.\n");
        }
        stat_mmz_phyaddr_base = 0;
        p_stat_mmz_virtual_base = NULL;
...
HI_S32 HI_MPI_STAT_ThreadReset(HI_STAT_HANDLE handle)
{
    if (NULL == handle)
    {
        HI_ERR_STAT("input Invalid parameter.\n");
        return HI_FAILURE;
    }
...
HI_S32 HI_MPI_STAT_ThreadRegister(char *name, HI_STAT_HANDLE *pHandle)
{
    ...
    if (NULL == p_stat_mmz_virtual_base)
    {
        HI_ERR_STAT("Init stat not init.\n");
        return HI_FAILURE;
    }

    if (NULL == name || NULL == pHandle)
    {
        HI_ERR_STAT("input Invalid parameter.\n");
        return HI_FAILURE;
    }
    ...
    *pHandle = (HI_STAT_HANDLE)HI_MALLOC(HI_ID_MEM, sizeof(HI_STAT_HANDLE_S));
    if (NULL == *pHandle)
    {
        HI_ERR_STAT("HI_MALLOC faild.\n");
        return HI_FAILURE;
    }

    HI_STAT_LOCK();

    ret = ioctl(s_s32StatFd, UMAPC_CMPI_STAT_REGISTER, &userspaceNum);
    if (ret != HI_SUCCESS || 0 > userspaceNum)
    {
        HI_STAT_UNLOCK();

        HI_ERR_STAT("ioctl of UMAPC_CMPI_STAT_REGISTER faild, ret:%x userspaceNum:0x%x\n", ret, userspaceNum);
    ...
    (*pHandle)->stat_thread_uvirtaddr = &p_stat_mmz_virtual_base[userspaceNum];
...
{
    if (NULL == pHandle || NULL == *pHandle || NULL == (*pHandle)->stat_thread_uvirtaddr)
    {
        HI_ERR_STAT("input Invalid parameter.\n");
        return HI_FAILURE;
    }
...
HI_S32 HI_MPI_STAT_Event(STAT_EVENT_E enEvent, HI_U32 Value)
{
    ...
    if(Ret != HI_SUCCESS)
    {
        ...
        HI_ERR_STAT("ioctl of UMAPC_CMPI_STAT_EVENT err, Ret:0x%x\n", Ret);
        return HI_FAILURE;
    }
