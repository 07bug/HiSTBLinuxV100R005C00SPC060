...
#if defined(CHIP_TYPE_hi3716mv410) || defined(CHIP_TYPE_hi3716mv420) || defined(CHIP_TYPE_hi3716mv450)
...
HI_S32 HI_SYS_GetChipAttr(HI_SYS_CHIP_ATTR_S *pstChipAttr)
{
    ...
    pstChipAttr->bHDR10Support = (HI_BOOL)u32Support;
...
HI_S32 HI_SYS_GetAttr(HI_SYS_ATTR_S *pstSysAttr)
{
    ...
    s32Ret = ioctl(s_s32SysFd, SYS_GET_SYS_ATTR, pstSysAttr);
...
HI_S32 HI_SYS_MapRegister(HI_U32 u32RegAddr, HI_U32 u32Length, HI_VOID *pVirAddr)
{
    ...
    if (HI_NULL == pVirAddr)
    {
        HI_ERR_SYS("HI_SYS_MapRegister Invalid param!\n");
        return HI_FAILURE;
    }

    HI_SYS_LOCK();

    pu32VirAddr = (HI_U32*)HI_MMAP(u32RegAddr, u32Length);
    if (HI_NULL == pu32VirAddr)
    {
        HI_ERR_SYS("HI_MMAP failed\n");
        HI_SYS_UNLOCK();
...
HI_S32 HI_SYS_UnmapRegister(HI_VOID * pVirAddr)
{
    ...
    if (HI_NULL == pVirAddr)
    {
        HI_ERR_SYS("HI_SYS_UnmapRegister Invalid param!\n");
        return HI_FAILURE;
    }

    HI_SYS_LOCK();

    s32Ret = HI_MUNMAP((void*)pVirAddr);
    if (HI_FAILURE == s32Ret)
    {
        HI_ERR_SYS("HI_MUNMAP failed\n");
        HI_SYS_UNLOCK();
...
HI_S32 HI_SYS_DisableVideoCodec(HI_SYS_VIDEO_CODEC_E enVideoCodec)
{
    ...
    s32Ret = ioctl(s_s32SysFd, SYS_DISABLE_VIDEOCODEC, &enVideoCodec);
