...
{
    return (long)HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, CMPI_LOG_Ioctl);
}
...
{
    return (long)HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, CMPI_Compat_LOG_Ioctl);
}
#endif
