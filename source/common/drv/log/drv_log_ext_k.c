...
#define LOG_MAX_LOGFILE_SIZE (120U)         /* 120M */
#define LOG_MIN_LOGFILE_SIZE (1U)           /* 1M */
#define LOG_DEFAULT_LOGFILE_SIZE (12U)      /* 12M */

#if defined(LOG_NETWORK_SUPPORT) || defined(LOG_UDISK_SUPPORT)
#define LOG_MAX_LOGFILE_CNT  (0x10000U)  /* 65536 */
...
static int g_DefaultLogSize = LOG_DEFAULT_LOGFILE_SIZE * 1024 * 1024;
...
HI_S32 HI_DRV_LOG_BufferRead(HI_U8 *Buf,  HI_U32 BufLen, HI_U32 *pCopyLen, HI_BOOL bKernelCopy)
{
    ...
    if(g_MsgBufInfo.u32WriteAddr == g_MsgBufInfo.u32ReadAddr)
    {
        ...
        else
        {
            ...
            if (HI_FALSE == wait_event_interruptible_timeout(g_MsgBufInfo.wqNoData,
                    (g_MsgBufInfo.u32WriteAddr != g_MsgBufInfo.u32ReadAddr), msecs_to_jiffies(500)))
            {
                return HI_FAILURE;
            }
        }
...
HI_S32 HI_DRV_LOG_BufferWrite(HI_U8 *Buf,  HI_U32 MsgLen, HI_U32 UserOrKer)
{
    ...
    if (MsgLen >= LogBufSize)
    {
        HI_ERR_LOG("print data length too long.\n");
        return HI_FAILURE;
    }
...
HI_S32 LogUdiskSave(const HI_S8* pFileName, HI_S8* pData, HI_U32 u32DataLen)
{
    ...
    struct kfilestat fileStat = {0};

    static HI_U64 u64FileIndex = 0;
    HI_S8 szCurFileName[MAX_FILENAME_LENTH] = {0};

    HI_OSAL_Snprintf(szCurFileName, sizeof(szCurFileName)-1, "%s%08llu", pFileName, u64FileIndex % LOG_MAX_LOGFILE_CNT);

    pFile = HI_DRV_FILE_Open(szCurFileName, 1);
    if(pFile == NULL)
    {
    ...
    s32Ret = HI_DRV_FILE_Stat(szCurFileName, &fileStat);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_LOG("HI_DRV_FILE_Stat failed, Ret[0x%x]\n", s32Ret);
        return s32Ret;
    }

    if (fileStat.size >= g_DefaultLogSize / 3)
    {
        u64FileIndex++;
        if (u64FileIndex >= 3) /* support max 3 files loop */
        {
            HI_OSAL_Snprintf(szCurFileName, sizeof(szCurFileName)-1, "%s%08llu", pFileName, u64FileIndex - 3);
            s32Ret = HI_DRV_FILE_Remove(szCurFileName);
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_LOG("HI_DRV_FILE_Remove failed, Ret[0x%x]\n", s32Ret);
                return s32Ret;
            }

            /* loop back(0~65535) */
            if ((LOG_MAX_LOGFILE_CNT + 2) == u64FileIndex)
            {
                u64FileIndex = 2;
            }
        }
...
HI_S32 HI_DRV_LOG_ProcRead(struct seq_file *s, HI_VOID *pArg)
{
    ...
    PROC_PRINT(s, "echo logsize=XXX > /proc/msp/log\n");
    PROC_PRINT(s, "                   the current logsize is (%d)MB, range(%dMB~%dMB)\n", (g_DefaultLogSize / 1024 / 1024), LOG_MIN_LOGFILE_SIZE, LOG_MAX_LOGFILE_SIZE);
                     ...
                     size_t count, loff_t *ppos)
{
    ...
    if ((*ppos >= MAX_FILENAME_LENTH) || (count >= MAX_FILENAME_LENTH))
        return -EFBIG;
    ...
    if (!HI_OSAL_Strncasecmp("help", m, 4))
    {
        ...
        HI_DRV_PROC_EchoHelper("Use 'echo \"logsize = xxx\" > /proc/msp/log' to set log size as xxx MB.\n");
        HI_DRV_PROC_EchoHelper("Use 'echo \"logsize = 12\" > /proc/msp/log' to set log size as 12 MB.\n");
        HI_DRV_PROC_EchoHelper("    example: 'echo logsize=12 > /proc/msp/log'\n");
    ...
    else if (!HI_OSAL_Strncasecmp("logsize", left, strlen("logsize")+1))
    {
        HI_U32 u32TotalLogSize = 0;
        if (strstr(right, "0x") || strstr(right, "0X"))
        {
            u32TotalLogSize = simple_strtoul(right, NULL ,16);
        }
        else
        {
            u32TotalLogSize = simple_strtoul(right, NULL, 10);
        }

        if (u32TotalLogSize > LOG_MAX_LOGFILE_SIZE || u32TotalLogSize < LOG_MIN_LOGFILE_SIZE)
        {
            HI_ERR_LOG("    Logsize(%dMB) is out of range(%dMB~%dMb)\n", u32TotalLogSize, LOG_MIN_LOGFILE_SIZE, LOG_MAX_LOGFILE_SIZE);
            goto out;
        }

        nRet = LOG_FILE_LOCK();

        g_DefaultLogSize = u32TotalLogSize * 1024 * 1024;

        LOG_FILE_UNLOCK();

        HI_INFO_LOG("set log file size as %d MB\n", g_DefaultLogSize);
