...
void flush_inner_cache(void *viraddr, unsigned int len)
{
	if ((NULL == viraddr) || (0 == len)) {
		HI_MMZ_WARN("%s failed, viraddr:%p len:0x%x!\n", __func__, \
							viraddr, len);
		return;
	}
...
static void flush_outer(hil_mmb_t *mmb)
{
	...
	if (!table) {
		HI_MMZ_WARN("%s: get pages failed!\n", __func__);
		return;
	}
...
static void dump_mem(void)
{
	...
	list_for_each_entry(p, &mmz_list, list) {
		...
		if (p->iommu)
			...
			mem_type = phy_name;
		HI_PRINT(SPLIT_LINE);
		HI_PRINT("|                   %s           |  ID  | ZONE  |  KVIRT  |  FLAGS  |  LENGTH(KB)  |       NAME        |\n", mem_type);
		HI_PRINT(SPLIT_LINE);
		for (n = rb_first(&p->root); n; n = rb_next(n)) {
			u32Number++;
		}
		HI_PRINT("|ZONE[%d]: (0x%08x, 0x%08x)   %d         %d        0x%08x      %-10u   \"%s%-14s|\n", nZoneCnt, \
		...
		HI_PRINT(SPLIT_LINE);
		if ((p)->iommu)
			HI_PRINT("|  SMMU Total Size  |    Iommu Used     |     Idle     |  Zone Number  |   BLock Number                 |\n");
		else
			HI_PRINT("|  MMZ Total Size  |     CMA Used      |     Idle     |  Zone Number  |   BLock Number                 |\n");
		HI_PRINT(SPLIT_LINE);
		free_size = mmz_total_size - used_size;
		HI_PRINT("|       %d%-8s       %d%-8s          %d%-8s         %d              %d                |\n",
...
result:
	if (NULL == mmb) {
		if (iommu)
			HI_MMZ_WARN("%s smmu:0x%x err args\n", __func__, addr);
		else
			HI_MMZ_WARN("%s phys:0x%x err args\n", __func__, addr);
		...
		for (n = rb_first(&zone->root); n; n = rb_next(n)) {
			...
			if (!iommu) {
				...
				if (zone->iommu) {
					m = rb_entry(n, hil_mmb_t, s_node);
					if (m->sec_smmu) {
						if ((m->sec_smmu <= sec_addr) && (sec_addr < (m->sec_smmu + m->length))) {
							mmb = m;
							goto result;
						}
					}
	...
	if (iommu) {
		while (*p) {
			...
			if (mmb->iommu_addr < entry->iommu_addr) {
				...
				HI_MMZ_WARN("%s: buffer already found.\n",
								__func__);
				BUG();
				return;
			}
			...
			HI_U32 align, const char *mmz_name, int flag)
{
	...
	if (IS_ERR(mmb->handle)) {
		HI_MMZ_WARN("%s :mem alloc failed !\n", __func__);
		goto handle_err;
	}
	...
	if (iommu) {
		...
		ret = ion_phys(client, mmb->handle, &phys,
							&len);
		if (ret) {
			HI_PRINT("%s :cannot get phys_addr!\n",__func__);
			goto err;
		}
...
static void *_mmb_map2kern(hil_mmb_t *mmb, int cached)
{
	...
	if (NULL != kdata) {
		if (kdata->map_cached != cached) {
			HI_MMZ_WARN("mmb<%s> already kernel-mapped one \
	...
	if (NULL == kdata) {
		HI_MMZ_WARN("%s : remap failed!\n", __func__);
		return NULL;
	}
	kdata->kvirt = _map2kern(mmb, cached);

	if (NULL == kdata->kvirt) {
		HI_MMZ_WARN("%s :remap failed in ion!\n",__func__);
		kfree(kdata);
		kdata = NULL;
		return NULL;
	}
...
int hil_mmb_unmap(hil_mmb_t *mmb, void *addr)
{
	...
	kdata = mmb->kdata;
	if (NULL == kdata) {
		HI_MMZ_WARN("%s :cannot find userdata!\n", __func__);
		up(&mmz_lock);
		return HI_FAILURE;
	}
	if (mmb->iommu)
...
int hil_mmb_free(hil_mmb_t *mmb)
{
	...
	if ((mmb->map_ref) || (mmb->phy_ref) || (mmb->cma_smmu_ref) ||
						(mmb->sec_smmu_ref)) {
		pr_debug("%s name=%s, mmz(0x%x) smmu(0x%x) is still used: \
...
hil_mmb_t *hil_mmb_getby_kvirt(void *virt)
{
	...
	list_for_each_entry(zone, &mmz_list, list) {
		...
		for (n = rb_first(&zone->root); n; n = rb_next(n)) {
			...
			if (((unsigned long)(m->kdata->kvirt) <= (unsigned long)virt) &&
			    ((unsigned long)virt < ((unsigned long)m->kdata->kvirt + m->length))) {
				mmb = m;
				goto end;
			}
		}
	...
	if (NULL == mmb)
		HI_MMZ_WARN("%s virt:%p cannot find mem\n", __func__, virt);
...
mmb_addr_t hil_mmb_cma_mapto_iommu(mmb_addr_t addr, int iommu)
{
	...
	if (iommu) {
		HI_MMZ_WARN("%s:err args,iommu must be 0,and addr must be \
				cma phy_addr\n", __func__);
	...
	if (MMB_ADDR_INVALID != mmb->iommu_addr) {
		HI_MMZ_WARN("%s :It's already mapped to iommu.\n",__func__);
		mmb->cma_smmu_ref++;
		mmb->phy_ref++;
...
int hil_mmb_cma_unmapfrom_iommu(mmb_addr_t addr, int iommu)
{
	...
	if (!iommu) {
		HI_MMZ_WARN("%s:err args,iommu must be 1,and addr must be \
				cma phy_addr\n", __func__);
		return HI_FAILURE;
	}
	...
	end_list_for_each_mmz()
	if (NULL == mmz) {
		HI_MMZ_WARN("%s:cannot find iommu zone!\n", __func__);
		up(&mmz_lock);
		return HI_FAILURE;
	}
...
int sec_mmb_put(u32 addr, int iommu)
{
	...
	mmb = hil_mmb_getby_phys((HI_U32)addr, (HI_U32)iommu);
	if (!mmb) {
		HI_MMZ_WARN("%s:err args, addr:0x%x iommu:0x%x!\n", __func__,
								addr, iommu);
		return HI_FAILURE;
	}
	down(&mmz_lock);
	...
	if (!mmb->sec_smmu_ref || !mmb->phy_ref) {
		HI_MMZ_WARN("%s: wrong operation.mmb->sec_smmu_ref :%d \
			mmb->phy_ref: %d \n", __func__, mmb->sec_smmu_ref,
			mmb->phy_ref);
		up(&mmz_lock);
		return HI_FAILURE;
	}
...
int mmb_put(u32 addr, int iommu)
{
	...
	if (mmb->phy_ref > 0) {
		...
		HI_MMZ_WARN("%s : phy_ref is wrong:ref:%d\n", __func__,
						mmb->phy_ref);
		up(&mmz_lock);
		return HI_FAILURE;
	}
	if (!mmb->phy_ref && !mmb->map_ref && !mmb->cma_smmu_ref &&
						!mmb->sec_smmu_ref) {
...
int mem_source_query(u32 iommu_addr, int *source)
{
        ...
        if (!source || !iommu_addr) {
                HI_MMZ_WARN("%s: source or iommu_addr should not be null!\n", __func__);
                return HI_FAILURE;
	}
...
#define CHECK_BUFF_OVERFLOW() \
	do { \
		if (m->count > MAX_MMZ_INFO_LEN) { \
			PRINTK_CA(KERN_ERR"mmz_info_buff overflow(0x%x), more than %u KB data!\n", (unsigned int)m->count, MAX_MMZ_INFO_LEN / 1024); \
			break; \
		} \
	} while (0)
...
static int iommu_zone_init(void)
{
	...
	if (NULL == iommu_zone) {
		HI_MMZ_WARN("%s :iommu zone can not be used!\n",__func__);
		return HI_FAILURE;
	}

	zone = hil_mmz_create("null", 0, 0, 0);
	if (NULL == zone) {
		HI_MMZ_WARN("%s :iommu zone created failed,iommu zone may not be used!\n",__func__);
		return HI_FAILURE;
	}
...
static int client_init(void)
{
	...
	if (NULL == common->client) {
		HI_MMZ_WARN("%s :ion client is created failed!\n",__func__);
		return HI_FAILURE;
	}
...
int HI_DRV_MMZ_Init(void)
{
	...
	ret = client_init();
	if (ret) {
		HI_MMZ_WARN("%s :client init is failed!\n",__func__);
		return HI_FAILURE;
	}

	ret = mmz_zone_init();
	if (ret) {
		HI_MMZ_WARN("%s :mmz zone init is failed!\n",__func__);
		return HI_FAILURE;
	}
	...
	ret = iommu_zone_init();
	if (ret) {
		HI_MMZ_WARN("%s :iommu zone init is failed!\n",__func__);
		return HI_FAILURE;
	}
#endif
