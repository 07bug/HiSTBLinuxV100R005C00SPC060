...
/*  define the mmz print function   */
#define	HI_MMZ_PRINT(LEVEL,fmt,args...)  do{\
	if(mmz_print_level > LEVEL)\
		HI_PRINT("%s(%d): " fmt, __FILE__, __LINE__, ##args);\
}while (0)
...
struct mmb_udata {
	...
	HI_U32 map_ref;
	int map_fd;
	unsigned int unmap_flag; /* indicate if normal unmapd or process killed by system*/
	struct {
