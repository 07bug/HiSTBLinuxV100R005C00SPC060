...
#if defined(CONFIG_ARCH_HI3796MV2X) && defined(CONFIG_TEE)
#include "sec_mmz.h"
#endif

#define HI_ZERO	0

HI_S32 HI_DRV_MMZ_AllocAndMap(const char *bufname, char *zone_name, HI_U32 size,
				int align, MMZ_BUFFER_S *psMBuf)
{
	mmb_addr_t phyaddr;
...
HI_S32 HI_DRV_MMZ_Alloc(const char *bufname, char *zone_name, HI_U32 size,
					int align, MMZ_BUFFER_S *psMBuf)
{
	mmb_addr_t phyaddr;
...
{
	if (!psMBuf || !psMBuf->pu8StartVirAddr) {
		HI_PRINT("%s: err arg\n",__func__);
		return HI_FAILURE;
	}
...
HI_S32 HI_DRV_MMZ_Buffer_Get(MMZ_BUFFER_S *psMBuf)
{
	...
	return mmb_buf_get(psMBuf->u32StartPhyAddr, 0);
}
...
HI_S32 HI_DRV_MMZ_Buffer_Put(MMZ_BUFFER_S *psMBuf)
{
	...
	return mmb_buf_put(psMBuf->u32StartPhyAddr, 0);
}

#ifdef HI_SMMU_SUPPORT
HI_S32 HI_DRV_SMMU_AllocAndMap(const HI_CHAR *bufname, HI_U32 size,
			       HI_U32 align, SMMU_BUFFER_S *pSmmuBuf)
{
	mmb_addr_t smmuaddr;
...
HI_S32 HI_DRV_SMMU_Alloc(const HI_CHAR *bufname, HI_U32 size, int align,
						SMMU_BUFFER_S *pSmmuBuf)
{
	mmb_addr_t smmuaddr;
...
HI_S32 HI_DRV_MMZ_UnmapFromSmmu(SMMU_BUFFER_S *psMBuf)
{
	...
	iommu_addr = psMBuf->u32StartSmmuAddr;
...
HI_S32 HI_DRV_SMMU_Buffer_Get(SMMU_BUFFER_S *pSmmuBuf)
{
	...
	return mmb_buf_get(pSmmuBuf->u32StartSmmuAddr, 1);
}
...
HI_S32 HI_DRV_SECSMMU_Release(const SMMU_BUFFER_S *pSecSmmuBuf)
{
	...
	if (MMB_ADDR_INVALID == smmuaddr) {
		pr_debug("%s: secmem cannot to be freed! secsmmu:0x%x \n", __func__,
...
HI_S32 HI_DRV_SECMMZ_Buffer_Get(MMZ_BUFFER_S *pSecMBuf)
{
	...
	return sec_mem_get(pSecMBuf->u32StartPhyAddr , 0);
}
...
HI_S32 HI_DRV_SECMMZ_Buffer_Put(MMZ_BUFFER_S *pSecMBuf)
{
	...
	return sec_mem_put(pSecMBuf->u32StartPhyAddr , 0);
}
...
HI_S32 HI_DRV_SECSMMU_Buffer_Get(SMMU_BUFFER_S *pSecSmmuBuf)
{
	...
	return sec_mem_get(pSecSmmuBuf->u32StartSmmuAddr , 1);
...
HI_S32 HI_DRV_SECSMMU_Buffer_Put(SMMU_BUFFER_S *pSecSmmuBuf)
{
	...
	return sec_mem_put(pSecSmmuBuf->u32StartSmmuAddr , 1);
}

HI_S32 HI_DRV_SECMMZ2_Alloc(const HI_CHAR *bufname, HI_U32 size, SMMU_BUFFER_S *pSecSmmuBuf)
{
#if defined(CONFIG_ARCH_HI3796MV2X) && defined(CONFIG_TEE)
#define HIL_SMMZ2_NAME_MAX	16

	char mmbname[HIL_SMMZ2_NAME_MAX];
	...
	memset(mmbname, 0x0, HIL_SMMZ2_NAME_MAX);
	memcpy(mmbname, bufname, (strlen(bufname) > HIL_SMMZ2_NAME_MAX)?
				HIL_SMMZ2_NAME_MAX:strlen(bufname));
	mmbname[HIL_SMMZ2_NAME_MAX - 1] = '\0';

	pSecSmmuBuf->u32Size = 0;
	pSecSmmuBuf->u32StartSmmuAddr = HI_NULL;

	pSecSmmuBuf->u32StartSmmuAddr = (HI_ULONG)HI_SEC_MMZ_New_And_Map_SecSmmu(size , "SEC-MMZ2", mmbname);
	if (!pSecSmmuBuf->u32StartSmmuAddr) {
		HI_PRINT("[%s] Alloc %s in SEC-MMZ2 zone failed\n", __FUNCTION__, bufname);
		return HI_FAILURE;
	}
...
HI_S32 HI_DRV_SECMMZ2_Release(const SMMU_BUFFER_S *pSecSmmuBuf)
{
#if defined(CONFIG_ARCH_HI3796MV2X) && defined(CONFIG_TEE)
	HI_S32 ret = HI_FAILURE;

	if (!pSecSmmuBuf || !pSecSmmuBuf->u32StartSmmuAddr || !pSecSmmuBuf->u32Size) {
		HI_PRINT("%s:invalidate params!\n", __func__);
		return HI_FAILURE;
	}

	ret = HI_SEC_MMZ_Delete_And_Unmap_SecSmmu(pSecSmmuBuf->u32StartSmmuAddr, pSecSmmuBuf->u32Size);
	if (ret) {
		HI_PRINT("free sec mmz2 failed!\n");
		return HI_FAILURE;
	}
#endif
	return HI_SUCCESS;
...
EXPORT_SYMBOL(HI_DRV_SECMMZ2_Alloc);
EXPORT_SYMBOL(HI_DRV_SECMMZ2_Release);
