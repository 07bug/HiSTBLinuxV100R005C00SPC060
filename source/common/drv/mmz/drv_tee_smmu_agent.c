...
#ifdef CONFIG_OPTEE
static int smmu_ops(struct smmu_ctrl_t *smmu_ctrl)
{
	int ret = 0;
	mmb_addr_t addr;
	int iommu;
	...
	if (!smmu_ctrl) {
		HI_MMZ_WARN("%s :parameters from tee is NULL!\n",__func__);
		return -1;
	}
	...
	case HISI_MEM_ALLOC:
	{
		if (smmu_ctrl->memtype) {
			addr = new_mmb(smmu_ctrl->name, smmu_ctrl->size,
							0x1000, "iommu");
			smmu_ctrl->normal_smmu = addr;
			smmu_ctrl->phys_addr = 0;
			iommu = 1;
		} else {
			addr = new_mmb(smmu_ctrl->name, smmu_ctrl->size, 0x1000,
									NULL);
	...
	case HISI_MEM_GET_MEMINFO:
	{
		if (smmu_ctrl->memtype) {
			iommu = 1;
			smmu_ctrl->tz_mblock_phys = hi_tee_smmumem_into_ta(
						smmu_ctrl->normal_smmu, 1);
			addr = smmu_ctrl->normal_smmu;
		} else {
			iommu = 0;
			smmu_ctrl->tz_mblock_phys = hi_tee_smmumem_into_ta(
...
static int smmu_agent_run(void *priv, void *virt, unsigned int size)
{
	struct smmu_ctrl_t *smmu_ctrl = NULL;

	if (!virt || !size || size < sizeof(*smmu_ctrl)) {
		HI_MMZ_WARN("%s :parameter from tee is null!\n",__func__);
		return -1;
	}
	smmu_ctrl = (struct smmu_ctrl_t *)virt;
	return smmu_ops(smmu_ctrl);
}


struct tee_agent_kernel_ops smmu_agent_ops = {
	.agent_name = "smmu_agent",
	.agent_id = SMMU_AGENT_ID,
	.mutex = __MUTEX_INITIALIZER(smmu_agent_ops.mutex),
	.tee_agent_run = smmu_agent_run,
};

int smmu_agent_register(void)
{
	return tee_agent_kernel_register(&smmu_agent_ops);
}

int smmu_agent_unregister(void)
{
	return tee_agent_kernel_unregister(&smmu_agent_ops);
}
#else
...
static int smmu_tee_agent_work(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ret) {
			HI_MMZ_WARN("%s send event response fail\n", agent_instance->agent_name);
			break;
		}
...
static int smmu_tee_agent_run(struct tee_agent_kernel_ops* agent_instance)
{
	...
	if (IS_ERR(shared_mem)) {
		HI_MMZ_WARN("%s :allocate agent buffer fail\n",__func__);
	...
	if (ret) {
		HI_MMZ_WARN("%s :register agent buffer fail\n",__func__);
		ret = -1;
		goto out;
	}
	...
	if (IS_ERR(agent_instance->agent_thread)) {
		HI_MMZ_WARN("%s :kthread creat fail\n",__func__);
...
static int smmu_tee_agent_init(struct tee_agent_kernel_ops* agent_instance)
{
	...
	if ((NULL == agent_instance) || (agent_instance->agent_id == 0) ||
		...
		HI_MMZ_WARN("%s :agent is invalid\n",__func__);
		return -1;;
	}
	...
	if (agent_instance->tee_agent_init)
		...
		HI_MMZ_WARN("tee_agent_init %s should not be null\n", agent_instance->agent_name);
		return -1;
	}

	if (ret) {
		HI_MMZ_WARN("tee_agent_init %s failed\n", agent_instance->agent_name);
		return -1;
	}
	...
	if (agent_instance->tee_agent_run)
		...
		HI_MMZ_WARN("tee_agent_run %s should not be null\n", agent_instance->agent_name);
		return -1;
	}

	if (ret) {
		HI_MMZ_WARN("tee_agent_run %s failed\n", agent_instance->agent_name);
...
static int smmu_agent_stop(struct tee_agent_kernel_ops* agent_instance)
{
	...
	if (ret) {
		HI_MMZ_WARN("%s  close agend failed!\n", __func__);
		return -1;
	}
