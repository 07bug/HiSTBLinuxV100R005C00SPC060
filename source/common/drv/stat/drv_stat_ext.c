...
static MMZ_BUFFER_S  g_stStatMmz = {0};
...
HI_S32 CMPI_STAT_Ioctl(struct inode *inode, struct file *file, HI_U32 cmd, HI_VOID * arg)
{
    ...
    switch (cmd)
    {
        case UMAPC_CMPI_STAT_GET_PHYADDR_BASE:
        {
            HI_U32 *p_stat_mmz_phyaddr_base = (HI_U32 * )arg;
            *p_stat_mmz_phyaddr_base = g_stat_thread_phyaddr_base;
            break;
        }

        case UMAPC_CMPI_STAT_REGISTER:
        {
            HI_S32 *userspace_num = (HI_U32 * )arg;
            ...
            for (i = 0; i < STAT_MAX_THREAD_USERSPACE; i++)
            {
                if (g_stat_thread_kvirt_base[i].bUsed == HI_FALSE)
                {
                    ...
                    *userspace_num = i;
                    ...
                    HI_WARN_STAT("g_stat_thread_phyaddr_base = %x, offset_addr = %x\n", (HI_U32)g_stat_thread_phyaddr_base, (*userspace_num)*sizeof(STAT_USERSPACE_S));
                    return 0;
                }
            }

            HI_ERR_STAT("no available stat resource, i:0x%x\n", i);
            *userspace_num = -1;
            return -1;
        }
        ...
        case UMAPC_CMPI_STAT_EVENT:
        {
            ...
            if ( pStatEvent->enEvent >= STAT_EVENT_BUTT )
            {
                return -1;
            }
...
{
    return (long)HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, CMPI_STAT_Ioctl);
}
...
HI_S32 HI_DRV_STAT_Init(HI_VOID)
{
    ...
    ret = HI_DRV_MMZ_AllocAndMap("STAT_MMZBuf", MMZ_OTHERS, STAT_USERSPACE_TOTAL_SIZE, 0, &g_stStatMmz);
    if (HI_SUCCESS != ret || HI_NULL == g_stStatMmz.pu8StartVirAddr
                          || 0 == g_stStatMmz.u32StartPhyAddr || g_stStatMmz.u32Size < STAT_USERSPACE_TOTAL_SIZE)
    {
        HI_ERR_STAT("AllocAndMap STAT MMZBuf error, ret[%#x], viraddr[%#x], phyaddr[%#x], size[%#x]\n",
                                          ret, g_stStatMmz.pu8StartVirAddr, g_stStatMmz.u32StartPhyAddr, g_stStatMmz.u32Size);
        return HI_FAILURE;
    }

    g_stat_thread_kvirt_base = (STAT_USERSPACE_S *)g_stStatMmz.pu8StartVirAddr;
    g_stat_thread_phyaddr_base = g_stStatMmz.u32StartPhyAddr;
    ...
    if (!item)
    {
        HI_ERR_STAT("cann't add moudle for stat.\n");
...
out:
    HI_DRV_MMZ_UnmapAndRelease(&g_stStatMmz);
    memset(&g_stStatMmz, 0x0, sizeof(MMZ_BUFFER_S));
    g_stat_thread_kvirt_base = HI_NULL;
    g_stat_thread_phyaddr_base = 0;
