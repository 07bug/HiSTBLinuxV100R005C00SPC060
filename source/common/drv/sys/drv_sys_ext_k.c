...
static HI_SYS_VIDEO_CODEC_E g_enVideoCodec = HI_SYS_VIDEO_MASK;
   ...
   switch (g_pstRegSysCtrl->SC_SYSID)
    {
        ...
        case 0x37980210:
            ...
            break;
        case 0x37980300:
            if (0x0 == g_pstRegPeri->PERI_SOC_FUSE_0.bits.chip_id)
            {
                ChipType    = HI_CHIP_TYPE_HI3798M;
            ...
            {
                ChipType    = HI_CHIP_TYPE_HI3798M_H;
                ChipVersion = HI_CHIP_VERSION_V300;
            }
            break;
        case 0x37960200:
            if (0x10 == g_pstRegPeri->PERI_SOC_FUSE_0.bits.chip_id)
            {
                ChipType    = HI_CHIP_TYPE_HI3716M;
                ChipVersion = HI_CHIP_VERSION_V450;
            }
            else
            {
                ChipType    = HI_CHIP_TYPE_HI3796M;
    ...
    if ((0x37160410 == g_pstRegSysCtrl->SC_SYSID) || (0x37160460 == g_pstRegSysCtrl->SC_SYSID))
    {
        switch (g_pstRegPeri->PERI_SOC_FUSE_0.bits.chip_id)
        {
            case 0x0:
...
HI_S32 HI_DRV_SYS_GetChipPackageType(HI_CHIP_PACKAGE_TYPE_E *penPackageType)
{
    ...
    if ((0x37986200 == g_pstRegSysCtrl->SC_SYSID) || (0x37980210 == g_pstRegSysCtrl->SC_SYSID) ||
        (0x37988200 == g_pstRegSysCtrl->SC_SYSID) || (0x37980300 == g_pstRegSysCtrl->SC_SYSID))
    {
...
HI_S32 HI_DRV_SYS_GetHdrSupport(HI_U32 *pu32Support)
{
   ...
   || defined(CHIP_TYPE_hi3798mv300) \
   || defined(CHIP_TYPE_hi3798mv310)
/* 98mv200 and 96mv200: bit27 0 means support hdr10, 1 means not support */
    switch(g_pstRegPeri->PERI_SOC_FUSE_0.bits.otp_hdr_ctrl & 0x1)
    {
        case 0x0:
            *pu32Support = 0x1;
            break;
...
HI_S32 HI_DRV_SYS_GetDolbyvisionSupport(HI_U32 *pu32Support)
{
   ...
   || defined(CHIP_TYPE_hi3796mv200) \
   || defined(CHIP_TYPE_hi3716mv450)
    /* 98mv200 and 96mv200: bit28 0 means support dolbyvision, 1 means not support */
    switch(g_pstRegPeri->PERI_SOC_FUSE_0.bits.otp_hdr_ctrl & 0x2)
    {
        case 0x0:
            *pu32Support = 0x1;
            break;
...
#else /* 98mv300 and 98mv310 don't support Dolbyvision */
    *pu32Support = 0x0;
#endif
    return HI_SUCCESS;
}
...
#if    defined(CHIP_TYPE_hi3796mv100)   \
...
#else  /* defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3716mv450) */
...
HI_S32 HI_DRV_SYS_Get_Attrs(HI_SYS_ATTR_S *attrs)
{
    if (HI_NULL == attrs)
    {
        return HI_FAILURE;
    ...
    attrs->bSMPSupport = HI_TRUE;
#else
    attrs->bSMPSupport = HI_FALSE;
#endif

    return HI_SUCCESS;
}

HI_S32 HI_DRV_SYS_DisableVideoCodec(HI_SYS_VIDEO_CODEC_E enVideoCodec)
{
    kSys_lock();

    g_enVideoCodec &= ~(enVideoCodec);
...
HI_S32 HI_DRV_SYS_GetVideoCodecSupport(HI_SYS_VIDEO_CODEC_E *penVideoCodec)
{
    if (HI_NULL == penVideoCodec)
    {
        return HI_FAILURE;
    }
    kSys_lock();

    *penVideoCodec = g_enVideoCodec;
...
/*pu32Value: 0:normal mode,other:special mode*/
...
/*bModeType: 0: External Amplifier, 1: Internal Amplifier*/
HI_S32 HI_DRV_SYS_GetAudioOPMode(HI_U32 *pModeType)
{
    HI_U32 u32RegValue = 0;

    if (HI_NULL == pModeType)
    {
        return HI_FAILURE;
    }
    ...
    HI_REG_READ((HI_VOID *)g_pstRegSysCtrl + 0x94, u32RegValue);
    if (u32RegValue>>27 & 0x01) /* SC_GEN5(0x94) 27bit; 0: External Amplifier, 1: Internal Amplifier*/
    {
        *pModeType = 1;
    }
    else
    ...
    HI_INFO_SYS("AudioOP: %s\n", (1 == *pModeType) ? "Internal Amplifier" : "External Amplifier");
    return HI_SUCCESS;

#else
    HI_UNUSED(u32RegValue);
    HI_ERR_SYS("only 3798mv300 support GetAudioOPMode mode!\n");
    return HI_FAILURE;
#endif
}
...
EXPORT_SYMBOL(HI_DRV_SYS_GetAudioOPMode);
EXPORT_SYMBOL(HI_DRV_SYS_DisableVideoCodec);
EXPORT_SYMBOL(HI_DRV_SYS_GetVideoCodecSupport);
