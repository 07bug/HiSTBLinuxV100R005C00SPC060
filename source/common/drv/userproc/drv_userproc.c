...
static inline int __full_name_hash(const unsigned char *name, unsigned int len)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
    return full_name_hash(name, strlen(name)) & 0x7fffffffU;
#else
...
{
    HI_U32 hash = __full_name_hash(pszName, strlen(pszName));
...
UMP_DIR_S * PROC_AddDir(const HI_CHAR* pszName, const HI_CHAR* pszParent, struct file *pstFile)
{
    ...
    pstDir->dir_name_hash = __full_name_hash(pszName, strlen(pszName));
...
UMP_ENTRY_S* PROC_AddEntry(const HI_DRV_USRMODEPROC_ENTRY_S* pstParam, HI_BOOL bUsrMode)
{
    ...
    pstEntry->entry_name_hash = __full_name_hash(pstParam->aszName, strlen(pstParam->aszName));
