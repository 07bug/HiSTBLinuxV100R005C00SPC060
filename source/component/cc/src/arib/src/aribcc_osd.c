...
static void _AribCC_CalcCharSize(HI_S32 CCType, HI_S32 s32IsIntervalInclude, HI_S32 *s32width, HI_S32 *s32height)
{
    ...
    */
    ...
    HI_S32 s32ActualFontWidth = 0, s32ActualFontHeight = 0;
    ...
    memset(&stText, 0, sizeof(HI_UNF_CC_TEXT_S));
    stText.pu16Text = buf;
    stText.u8TextLen = s32Length;
    stText.enFontStyle = HI_UNF_CC_FONTSTYLE_NORMAL;
    stText.u8AribFontSize = pstText->u8FontHeight;
    stText.enEdgetype = pstText->u8EdgeType;
    stText.enScaleType = (HI_UNF_CC_ARIB_SCALE_E)pstText->u8FontScale;
    stText.u8CharInterval = u8CharInterval;
    stText.u8LineInterval = u8LineInterval;
    stText.enPrtDirect = (HI_UNF_CC_ARIB_PRTDIRECT_E)pstText->stFormat.stPrtDirect;
    ARIBCC_GRAPHIC_GetTextSize(CCType, &stText, &s32ActualFontWidth, &s32ActualFontHeight);
    ...
    *s32width = s32ActualFontWidth;
    *s32height = s32ActualFontHeight;
...
void _AribCC_SetDefaultCursor(HI_S32 CCType)
{
    ...
    {
        if (pText->stWinAttr.u16width > char_width)
        {
    ...
    OSD_TRACE("Current Cursor: s32x=%d,s32y=%d,row=%d,column=%d\n",pText->stCursor.u16X,
           pText->stCursor.u16Y,pText->stCursor.u16Row,pText->stCursor.u16Column);
    return;
}
...
static HI_S32 _AribCC_DrawCaptionToWindow(HI_S32 CCType,HI_U16 *pu16Buf, HI_U16 u16BufLen,HI_U8 u8HasNonspace)
{
    ...
    HI_S32 x,y;
    HI_S32 char_width_with_interval = 0, char_height_with_interval = 0;
    ...
    _AribCC_CalcCharSize(CCType, FONTSIZE_WITH_INTERVAL, &char_width_with_interval, &char_height_with_interval);
    OSD_TRACE("u8FontWidth = %d, u8FontHeight = %d u8FontScale = %d u8CharInterval = %d u8LineInterval = %d char_width_with_interval = %d char_height_with_interval = %d",
                pstText->u8FontWidth,pstText->u8FontHeight, pstText->u8FontScale,
                pstText->u8CharInterval, pstText->u8LineInterval, char_width_with_interval, char_height_with_interval);
    if( char_width_with_interval == 0 || char_height_with_interval == 0 )
    {
        HI_ERR_CC("character size is zero:width=%d,height=%d\n",char_width_with_interval,char_height_with_interval);
        return HI_FAILURE;
    }
             ...
             pstText->u8WinScrollMode == 1 )
    {
        ...
        pstText->stCursor.u16MaxRow = pstText->stWinAttr.u16height/char_height_with_interval;
    ...
    OSD_TRACE("pstText->stWinAttr.u16x = %d,pstText->stWinAttr.u16y = %d pstText->stWinAttr.u16width = %d pstText->stWinAttr.u16height = %d\n",
            pstText->stWinAttr.u16x,pstText->stWinAttr.u16y, pstText->stWinAttr.u16width, pstText->stWinAttr.u16height);

    /*if( pstText->stCursor.u16Y == 0 && pstText->stFormat.stPrtDirect == CCARIB_PRINT_HORIZONTAL)
    {
        pstText->stCursor.u16Y += char_height_with_interval;
    }*/
    if( pstText->stCursor.u16X == pstText->stWinAttr.u16width && pstText->stFormat.stPrtDirect == CCARIB_PRINT_VERTICAL )
    {
#ifdef VERTICAL_CENTER_BASE_LINE
        pstText->stCursor.u16X = pstText->stWinAttr.u16width - (char_width_with_interval>>1);
#else
        pstText->stCursor.u16X = pstText->stWinAttr.u16width - char_width_with_interval;
#endif
    }
    ...
    while( u16BufLen > 0 )
    {
        ...
        empty_cols = (pstText->stWinAttr.u16width - pstText->stCursor.u16X)/char_width_with_interval;
        if( pstText->stFormat.stPrtDirect == CCARIB_PRINT_VERTICAL )
        {
            empty_cols = (pstText->stWinAttr.u16height - pstText->stCursor.u16Y)/char_height_with_interval;
            //empty_rows = (pstText->stCursor.u16X)/char_width_with_interval;
        }
...
#ifdef VERTICAL_CENTER_BASE_LINE
                if( pstText->stCursor.u16X >= ((char_width_with_interval*3)>>1) )
#else
                if( pstText->stCursor.u16X >= char_width_with_interval )
#endif
                {
                    pstText->stCursor.u16X -= char_width_with_interval;
                }
            ...
            else
            {
                ...
                if( pstText->u8ScrollType >= ARIBCC_LINESCROLL_FIX ||pstText->u8WinScrollMode == 1)
                {
                    ...
                    if( pstText->u8ScrollType == ARIBCC_LINESCROLL_FIX &&pstText->u8ScrolledRows == pstText->stCursor.u16MaxRow)
                    {
                        ...
                        ARIBCC_OSD_ClearScreen(CCType, HI_TRUE);
                        ARIBCC_OSD_Show(CCType);
        ...
        OSD_TRACE("pstText->stWinAttr.u16x = %d,pstText->stWinAttr.u16y = %d pstText->stWinAttr.u16width = %d pstText->stWinAttr.u16height = %d\n",
            pstText->stWinAttr.u16x,pstText->stWinAttr.u16y, pstText->stWinAttr.u16width, pstText->stWinAttr.u16height);
        OSD_TRACE("pstText->stCursor.u16X = %d,pstText->stCursor.u16Y = %d x = %d y = %d\n",
            pstText->stCursor.u16X,pstText->stCursor.u16Y, x, y);
        OSD_TRACE("stPrtDirect = %d, char_width_with_interval = %d char_height_with_interval = %d space_num = %d is_nonspace_char = %d\n",
                        pstText->stFormat.stPrtDirect, char_width_with_interval, char_height_with_interval, space_num, is_nonspace_char);
        ...
        if( space_num > 0 && pstText->stBgColor.u8Alpha != 0 )
        {
            ...
            {
                ARIBCC_GRAPHIC_FillWindow(CCType,x,y-char_height_with_interval,space_num*char_width_with_interval,char_height_with_interval,pstText->stBgColor);
            }
            else
            {
#ifdef VERTICAL_CENTER_BASE_LINE
                ARIBCC_GRAPHIC_FillWindow(CCType,x-(char_width_with_interval>>1),y,char_width_with_interval,space_num*char_height_with_interval,pstText->stBgColor);
#else
                ARIBCC_GRAPHIC_FillWindow(CCType,x,y,char_width_with_interval,space_num*char_height_with_interval,pstText->stBgColor);
#endif
            }
        }
        #endif
        ...
        {
#if 1
            //y -= char_height_with_interval;/*correct y position*/
#else
            HI_S32 scale_line_interval = (char_height_with_interval - net_height + 1)>>1;
...
#ifdef VERTICAL_CENTER_BASE_LINE
            x -= (net_width>>1);
            //y += ((char_height_with_interval - net_height)>>1);
#else
            x += (char_width_with_interval - net_width + 1)/2;
#endif
        }
        ...
        if( is_nonspace_char )
        {
            if( pstText->stFormat.stPrtDirect == CCARIB_PRINT_HORIZONTAL )
            {
                if( x >= char_width_with_interval )
                {
                    x -= char_width_with_interval;
                }
            }
            else
            {
                if( y >= char_height_with_interval )
                {
                    y -= char_height_with_interval;
                }
            }
            is_nonspace_char = 0;
        }
        ...
        else
        {
            if((pstText->stLastDrawCaption.u16Length == draw_length) && (pstText->u16LastRowNum != pstText->stCursor.u16Row))
            {
                ...
                if(i >= draw_length)
                {
                    ...
                    ARIBCC_GRAPHIC_ClearScreen(CCType, HI_FALSE);
                }
            }

            ARIBCC_GRAPHIC_TextOut(CCType, x, y, pu16Buf+counter, draw_length, pstText->u8CharInterval, pstText->u8LineInterval, HI_FALSE);
        ...
        if( pstText->stFormat.stPrtDirect == CCARIB_PRINT_HORIZONTAL )
        {
            _AribCC_DrawCharBoxHorizontal(CCType,x,y,char_width_with_interval*u16BufLen,char_height_with_interval,pstText->stTextBox);
        }
        else
        {
            _AribCC_DrawCharBoxVertical(CCType,x,y,char_width_with_interval,char_height_with_interval*u16BufLen,pstText->stTextBox);
        }

        if( char_width_with_interval > pstText->u8MaxCharWidth )
        {
            pstText->u8MaxCharWidth = char_width_with_interval;
        }
        if( char_height_with_interval > pstText->u8MaxCharHeight )
        {
            pstText->u8MaxCharHeight = char_height_with_interval;
        }
        ...
        if( pstText->stFormat.stPrtDirect == CCARIB_PRINT_HORIZONTAL )
        {
            pstText->stCursor.u16X += space_num * char_width_with_interval;
            pstText->stCursor.u16Column += space_num;
        }
        else if( pstText->stFormat.stPrtDirect == CCARIB_PRINT_VERTICAL )
        {
            pstText->stCursor.u16Y += space_num * char_height_with_interval;
            pstText->stCursor.u16Column += space_num;
        }
        OSD_TRACE("after draw %d chars x=%d,y=%d space_num = %d char_height_with_interval = %d\n",draw_length,pstText->stCursor.u16X,pstText->stCursor.u16Y, space_num, char_height_with_interval);
    }
...
static void _AribCC_HorizontalScrollThreadTask(void *parameters)
{
    ...
    for(i=0; i<scroll_buf_length; i++)
    {
        ...
        ARIBCC_GRAPHIC_FillRect(CCType,x,move_y,char_w,char_h,pText->stBgColor, HI_TRUE);
        ARIBCC_GRAPHIC_TextOut(CCType,x,draw_y,&(pText->stScrollBuf.astScrollText[i].u16text), 1, pText->u8CharInterval, pText->u8LineInterval, HI_TRUE);
...
*********************************************************************************/
void ARIBCC_OSD_ClearScreen(HI_S32 CCType, HI_BOOL bBlit)
{
    ARIBCC_TEXTATTR_S *pAribText = _AribCC_GetAribText(CCType);
    ...
    (HI_VOID)ARIBCC_GRAPHIC_ClearScreen(CCType, bBlit);
    ...
    ARIBCC_GRAPHIC_FillRect(CCType,pAribText->stWinAttr.u16x,pAribText->stWinAttr.u16y,pAribText->stWinAttr.u16width,
                             pAribText->stWinAttr.u16height,pAribText->stWinColor, bBlit);
...
void ARIBCC_OSD_SetWindowSize(HI_S32 CCType, HI_S32 width, HI_S32 height)
{
    ...
    HI_INFO_CC("\tSetWindowSize,x=%d,y=%d,width:%d,height:%d\n",pText->stWinAttr.u16x, pText->stWinAttr.u16y,width,height);
...
void ARIBCC_OSD_SetWindowPos(HI_S32 CCType, HI_S32 x, HI_S32 y)
{
    ...
    if( (pText->stWinAttr.u16x != x) || (pText->stWinAttr.u16y != y) )
    {
        ...
        ARIBCC_GRAPHIC_SetWindowSize(CCType, x, y, pText->stWinAttr.u16width, pText->stWinAttr.u16height);
    }
...
void ARIBCC_OSD_Show(HI_S32 CCType)
{
    ...
    if( pText->u8IsWinUpdate == 1 )
    {
        ...
        ARIBCC_GRAPHIC_FillRect(CCType,pText->stWinAttr.u16x,pText->stWinAttr.u16y,
                     pText->stWinAttr.u16width,pText->stWinAttr.u16height,pText->stClearColor, HI_TRUE);
#ifdef ARIBCC_SD_SHARE_ONEPLANE
