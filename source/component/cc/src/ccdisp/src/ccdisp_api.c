...
HI_S32 CCDISP_Window_FillRectByRGB(CCDISP_WINDOW_ID_E enWinID, HI_UNF_CC_COLOR_S stColor, HI_UNF_CC_RECT_S stRect,HI_U32 flag)
{
    ...
    /*
    in _DtvCC_ClearWindowOnScreen, we find the widest row width to fillrect.
    so we treat window width as Max Width(when 1920 *1080) i.e:1536.
    */
    stWinRectTmp.x = pstWindow->stWinRect.x;
    stWinRectTmp.y = pstWindow->stWinRect.y;
    stWinRectTmp.width = ((1536 - pstWindow->stWinRect.x) > pstWindow->stWinRect.width) ? (1536 - pstWindow->stWinRect.x) : pstWindow->stWinRect.width;
    stWinRectTmp.height = pstWindow->stWinRect.height;
    if (HI_FAILURE == CCDISP_CheckRect(stWinRectTmp, stRect))
    {
        HI_ERR_CC("window rect is (%d %d %d %d)\n", stWinRectTmp.x, stWinRectTmp.y, stWinRectTmp.width, stWinRectTmp.height);
                        ...
                        HI_U16* pu16Str, HI_S32 s32StrLen, HI_S32* ps32DrawWidth, HI_U32 flag)
{
    ...
    if(pstScreen->stOpt.pfnGetTextSize && pstScreen->stOpt.pfnDisplay)
    {
        ...
        if (s32StrLen <= 0)
        {
            HI_WARN_CC("text length is less than or equal to 0!\n");
            return HI_FAILURE;
        }
