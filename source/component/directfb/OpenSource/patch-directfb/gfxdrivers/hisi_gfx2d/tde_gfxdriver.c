...
#include "hisi_dfb_opentv5_layer.c"
#endif
...
static HI_U8* CreatePlatteSurface(void)
{
    HI_U8 *p;
    HI_U32 i;
    HI_U8 *pPhyaddr;

    pPhyaddr = (HI_U8 *)HI_MMZ_New(256 * 4, 4, NULL, "tde-test");
    if (pPhyaddr == HI_NULL)
    {
        return HI_NULL;
    }
    p = (HI_U8 *)HI_MMZ_Map((HI_U32)pPhyaddr, 1);
    if (p == HI_NULL)
    {
        HI_MMZ_Delete((HI_U32)pPhyaddr);
        return HI_NULL;
    }
    for (i = 0; i < 256; i++)
    {
        memset((HI_VOID *)(p + i * 4), 0xff, 3);
        *(p + i * 4 + 3) = i;
    }
...
static HI_VOID DestroyPlatteSurface(HI_U8* ClutPhyAddr)
{
    HI_MMZ_Unmap((HI_U32)ClutPhyAddr);
    HI_MMZ_Delete((HI_U32)ClutPhyAddr);
    ClutPhyAddr = HI_NULL;
    return ;
}


HI_U8 * pu8ClutPhyAddr = HI_NULL;
                                      ...
                                      CoreDFB *core)
{
     ...
     dfb_screens_hook_primary(device, driver_data,&hiPrimaryScreenFuncs,NULL,NULL);
     dfb_layers_hook_primary(device, driver_data,&hiPrimaryLayerFuncs,&hiOldPrimaryFuncs,&hiOldPrimaryDriverData );

     pu8ClutPhyAddr = CreatePlatteSurface();
 ...
 #ifdef CONFIG_GFX_DFB_OPTV5_SUPPORT
     dfb_layers_register(dfb_screens_at(DSCID_PRIMARY), driver_data, &video_layer_funcs);
 #endif
                     ...
                     void               *driver_data )
{
    if (pu8ClutPhyAddr)
    {
        DestroyPlatteSurface(pu8ClutPhyAddr);
    }
    HI_TDE2_Close();
