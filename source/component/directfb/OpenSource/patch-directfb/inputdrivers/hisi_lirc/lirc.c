...
LIRC_MAP_TO_INPUT gs_stIrcVirKeyToInputSymbols[LIRC_KEY_MAX_NUM] =
{
    ...
    {LIRC_KEY_SEARCH,        DIKI_UNKNOWN,      DIKS_EXIT},
    ...
    {LIRC_KEY_BTV,           DIKI_UNKNOWN,     DIKS_MENU},
	...
	{LIRC_KEY_F1,             DIKI_UNKNOWN,         DIKS_GREEN},
	{LIRC_KEY_F2,             DIKI_UNKNOWN,         DIKS_RED},
	{LIRC_KEY_F3,             DIKI_UNKNOWN,         DIKS_YELLOW},
	{LIRC_KEY_F4,             DIKI_UNKNOWN,         DIKS_BLUE},
...
#define REPEATKEY_FILTER_COUNT 3
#define REPEATKEY_RESEND_COUNT (3)
#define REPREATKEY_INTERVAL_MS (36)

#ifndef SUPPORT_LONG_PRESS_KEY
#define SUPPORT_LONG_PRESS_KEY
#endif
...
static void* LircEventThread(DirectThread *thread, void *driver_data)
{
    ...
    HI_U32 u32RepeatKeyDownCount = 0;

    HI_UNF_KEY_STATUS_E penPressStatus;
#ifdef SUPPORT_LONG_PRESS_KEY
    HI_U32  uRepeatKeyCount = 0;
    HI_BOOL bLongPressState = HI_FALSE;
    HI_BOOL bLongPressReady = HI_FALSE;
#endif
...
#ifdef SUPPORT_LONG_PRESS_KEY
        bLongPressState = HI_FALSE;
#endif
        if(HI_UNF_KEY_STATUS_DOWN == penPressStatus)
        {
            u32RepeatKeyDownCount = 0;
            stInputEvt.type = DIET_KEYPRESS;
        #ifdef SUPPORT_LONG_PRESS_KEY
            bLongPressReady = HI_TRUE;
        #endif
        }
...
#ifdef SUPPORT_LONG_PRESS_KEY
            bLongPressReady = HI_FALSE;
#endif
        }
        else
        {
#ifdef SUPPORT_LONG_PRESS_KEY
            if (HI_UNF_KEY_STATUS_HOLD == penPressStatus)
            {
                bLongPressState = HI_TRUE;
                uRepeatKeyCount = 0;

                ++u32RepeatKeyDownCount;
                if (u32RepeatKeyDownCount <= REPEATKEY_FILTER_COUNT){
                    continue;
                }

                while (HI_TRUE == bLongPressReady && uRepeatKeyCount++ < REPEATKEY_RESEND_COUNT)
                {
                    stInputEvt.type  = DIET_KEYPRESS;
                    stInputEvt.flags = DIEF_KEYCODE;
                    for (KeyCodeIndex = 0; KeyCodeIndex < LIRC_KEY_MAX_NUM; KeyCodeIndex++)
                    {
                        if (gs_stHardMapToVir[KeyCodeIndex].s32Code == LircKeyId)
                        {
                    ...
                    if (KeyCodeIndex == LIRC_KEY_MAX_NUM)
                    {
                        continue;
                    }
                    ...
                    usleep(REPREATKEY_INTERVAL_MS * 1000);
                }
            }
#else
            stInputEvt.type = DIET_KEYPRESS;
...
static DFBResult driver_open_device(CoreInputDevice *device, unsigned int number,InputDeviceInfo  *info, void **driver_data )
{
	...
	Ret =  HI_UNF_IR_SetRepKeyTimeoutAttr(80);
	if (HI_SUCCESS != Ret)
	{
		HI_UNF_IR_Close();
