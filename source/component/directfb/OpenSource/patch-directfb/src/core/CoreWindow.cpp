...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        case COREDFB_CALL_DIRECT:{
            ...
            Core_PushCalling();
            ret = real.Repaint( left, right, flags, timestamp );
            Core_PopCalling();
        ...
        case COREDFB_CALL_INDIRECT: {
            ...
            Core_PushCalling();
            ret = requestor.Repaint( left, right, flags, timestamp );
            Core_PopCalling();
...
)
{
    ...
    ret = (DFBResult) CoreWindow_Call( obj, (FusionCallExecFlags)(FCEF_ONEWAY), CoreWindow_Repaint, args, sizeof(CoreWindowRepaint), NULL, 0, NULL );
    if (ret) {
...
)
{
    ...
    char        args_static[FLUXED_ARGS_BYTES];
    char        return_args_static[FLUXED_ARGS_BYTES];
    CoreWindowPostEvent       *args = (CoreWindowPostEvent*) args_alloc( args_static, sizeof(CoreWindowPostEvent) );
    CoreWindowPostEventReturn *return_args;

    if (!args)
        return (DFBResult) D_OOM();

    return_args = (CoreWindowPostEventReturn*) args_alloc( return_args_static, sizeof(CoreWindowPostEventReturn) );
    ...
    ret = (DFBResult) CoreWindow_Call( obj, FCEF_NONE, CoreWindow_PostEvent, args, sizeof(CoreWindowPostEvent), return_args, sizeof(CoreWindowPostEventReturn), NULL );
    if (ret) {
        D_DERROR( ret, "%s: CoreWindow_Call( CoreWindow_PostEvent ) failed!\n", __FUNCTION__ );
        goto out;
    }

    if (return_args->result) {
        /*D_DERROR( return_args->result, "%s: CoreWindow_PostEvent failed!\n", __FUNCTION__ );*/
        ret = return_args->result;
        goto out;
    }
                                ...
                                unsigned int *ret_length )
{
    ...
    switch (method) {
        case CoreWindow_Repaint: {
         ...
         ;    // TODO: DFBRegion_debug args->left;
         ;    // TODO: DFBRegion_debug args->right;
            D_DEBUG_AT( DirectFB_CoreWindow, "  -> flags = %x\n", args->flags );
            D_DEBUG_AT( DirectFB_CoreWindow, "  -> timestamp = %lld\n", (long long) args->timestamp );

            real.Repaint( &args->left, &args->right, args->flags, args->timestamp );
#else
	            D_UNUSED
            CoreWindowRepaint       *args        = (CoreWindowRepaint *) ptr;
            ...
            return_args->result = real.Repaint( &args->left, &args->right, args->flags, args->timestamp );
    ...
    CoreWindow *source = NULL;
            ...
            if (ret) {
                 D_DERROR( ret, "%s(Bind): Looking up source by ID %u failed!\n", __FUNCTION__, args->source_id );
                 return_args->result = ret;
                 D_BREAK( "could not lookup object" );
                 return ret;
            }
    ...
    CoreWindow *source = NULL;
            ...
            ret = (DFBResult) CoreWindow_Lookup( core_dfb, args->source_id, caller, &source );
            if (ret) {
                 D_DERROR( ret, "%s(Unbind): Looking up source by ID %u failed!\n", __FUNCTION__, args->source_id );
                 return_args->result = ret;
                 D_BREAK( "could not lookup object" );
                 return ret;
            }
    ...
    CoreSurface *shape = NULL;
            ...
            if (args->shape_set) {
                ...
                if (ret) {
                     D_DERROR( ret, "%s(SetCursorShape): Looking up shape by ID %u failed!\n", __FUNCTION__, args->shape_id );
                     return_args->result = ret;
                     D_BREAK( "could not lookup object" );
                     return ret;
                }
        ...
        case CoreWindow_PostEvent: {
            D_UNUSED
            CoreWindowPostEvent       *args        = (CoreWindowPostEvent *) ptr;
            CoreWindowPostEventReturn *return_args = (CoreWindowPostEventReturn *) ret_ptr;

            D_DEBUG_AT( DirectFB_CoreWindow, "=-> CoreWindow_PostEvent\n" );

            return_args->result = real.PostEvent( &args->event );
            if (return_args->result == DFB_OK) {
            }

            *ret_length = sizeof(CoreWindowPostEventReturn);
