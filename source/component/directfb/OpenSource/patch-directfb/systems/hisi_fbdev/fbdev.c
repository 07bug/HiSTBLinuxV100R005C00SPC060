...
#include "hisi_dfb_opentv5_fbdev.c"
#endif
...
//tag, support fence sync
#ifdef CONFIG_DFB_SUPPORT_FENCE
static DFBResult system_fence_init(bool shutdown, int layer_id)
{
    int num_index = 0;
    ...
    dfb_fbdev->current_fence_index = 0;

    if (false == shutdown)
    {
       for (num_index = 0; num_index < MAX_DISPLAY_BUFFER_NUM; num_index++)
       {
          dfb_fbdev->fence_fd[num_index] = -1;
       }
    ...
    for (num_index = 0; num_index < MAX_DISPLAY_BUFFER_NUM; num_index++)
    {
       if (dfb_fbdev->fence_fd[num_index] >= 0)
       {
          close(dfb_fbdev->fence_fd[num_index]);
       }
...
#ifdef CONFIG_GFX_DFB_OPTV5_SUPPORT
     description->caps = DSCCAPS_VSYNC | DSCCAPS_POWER_MANAGEMENT | DSCCAPS_ENCODERS;
                   ...
                   CoreSurfaceBufferLock *right_lock )
{
     ...
     ret = dfb_fbdev_pan(config->source.x,left_lock->offset / left_lock->pitch + config->source.y, false);
     if (ret)
     {
        return ret;
     }
...
#ifdef CONFIG_DFB_SUPPORT_FENCE
     if (false == dfb_config->fence_sync)
#endif
     {
...
//     DFBResult                 ret;
     ...
     HIFB_HWC_LAYERINFO_S layer_info;
#endif
	 static int index = 1;
...
#ifdef CONFIG_DFB_SUPPORT_FENCE
     memset(&layer_info, 0x0, sizeof(layer_info));

     switch (config->format)
     {
          case DSPF_ARGB1555:
          case DSPF_RGBA5551:
               layer_info.eFmt = HIFB_FMT_ARGB1555;
               D_WARN("++++++++++++++ dfb_fence not support ARGB1555\n");
               break;
          case DSPF_RGB16:
               D_WARN("++++++++++++++ dfb_fence not support RGB565\n");
               layer_info.eFmt = HIFB_FMT_RGB565;
               break;
          case DSPF_RGB24:
               D_WARN("++++++++++++++ dfb_fence not support RGB888\n");
               layer_info.eFmt = HIFB_FMT_RGB888;
               break;
          case DSPF_RGB32:
          case DSPF_ARGB:
          case DSPF_ABGR:
               layer_info.eFmt = HIFB_FMT_ARGB8888;
               break;
          default:
               return DFB_UNSUPPORTED;
     }

     layer_info.stInRect.x = layer_info.stInRect.y = 0;
     layer_info.stInRect.w = var->xres;
     layer_info.stInRect.h = var->yres;
     layer_info.u32Stride  = shared->fix.line_length;
     layer_info.u32Alpha   = 0xff;
     layer_info.s32AcquireFenceFd = -1;
     layer_info.u32LayerAddr = shared->fix.smem_start + var->yoffset * shared->fix.line_length;

     if (ioctl( dfb_fbdev->fd, FBIO_HWC_REFRESH, &layer_info ) < 0)
     {
     	D_PERROR("FBIO_HWC_REFRESH ERROR==%d==\n", __LINE__);
     	return DFB_OK;
     }

#ifdef CONFIG_DFB_DISCARD_FRAME
     dfb_fbdev_quene_buffer_id(0,layer_info.u32LayerAddr,&(dfb_fbdev->current_fence_index));
     dfb_fbdev->fence_fd[dfb_fbdev->current_fence_index] = layer_info.s32ReleaseFenceFd;
#else
     dfb_fbdev->fence_fd[dfb_fbdev->current_fence_index]  = layer_info.s32ReleaseFenceFd;

     if (DLBM_TRIPLE == config->buffermode)
     {
         dfb_fbdev->current_fence_index = (dfb_fbdev->current_fence_index + 1) % MAX_DISPLAY_BUFFER_NUM;
     }
     else
     {
         dfb_fbdev->current_fence_index = (dfb_fbdev->current_fence_index + 1) % (MAX_DISPLAY_BUFFER_NUM - 1);
     }
#endif
#endif
//HISILICON add end
...
DFBResult dfb_fbdev_fence_sync_wait(int timeout, int layer_id, int buffer_id)
{
#ifndef CONFIG_DFB_DISCARD_FRAME
	int to = timeout;
    int ret = -1;
#endif
...
#ifdef CONFIG_DFB_DISCARD_FRAME
    /** no waite, discard fence **/
    if (dfb_fbdev->fence_fd[buffer_id] < 0)
    {
       return DFB_OK;
    }
    close(dfb_fbdev->fence_fd[buffer_id]);
    dfb_fbdev->fence_fd[buffer_id] = -1;
#else
    if (dfb_fbdev->current_fence_index > (MAX_DISPLAY_BUFFER_NUM - 1))
    {
        return DFB_OK;
    }

	if (dfb_fbdev->fence_fd[dfb_fbdev->current_fence_index] < 0)
    {
		return DFB_OK;
	}

    ret = ioctl(dfb_fbdev->fence_fd[dfb_fbdev->current_fence_index], SYNC_IOC_WAIT, &to);
	if (ret < 0)
    {
		D_PERROR("dfb_fbdev_fence_sync_wait error!, wait: 0x%x\n", ret);
	}

	close(dfb_fbdev->fence_fd[dfb_fbdev->current_fence_index]);
    dfb_fbdev->fence_fd[dfb_fbdev->current_fence_index] = -1;
#endif
...
DFBResult dfb_fbdev_quene_free_buffer_id(int layer_id,int *pbufferid)
{
    HI_U32 BufferNum = 0;
    ...
    if ((NULL == dfb_config) || (NULL == config))
    {
        return DFB_OK;
    }
    ...
    layer_info.eFmt = HIFB_FMT_ARGB8888;
    if (ioctl(dfb_fbdev->fd, FBIO_HWC_GETLAYERINFO, &layer_info) < 0)
    {
    	return DFB_OK;
    }
    ...
    if (DLBM_TRIPLE == config->buffermode)
    {
       BufferNum = MAX_DISPLAY_BUFFER_NUM;
    }
    else
    {
       BufferNum = MAX_DISPLAY_BUFFER_NUM - 1;
    }

    for (BufferIndex = 0; BufferIndex < BufferNum; BufferIndex++)
    {
        if (HI_FALSE == layer_info.bOutBufBusy[BufferIndex])
        {
           *pbufferid = BufferIndex;
           return DFB_OK;
        }
    }
...
DFBResult dfb_fbdev_quene_buffer_id(int layer_id,unsigned int buffer, int *pbufferid)
{
    unsigned int yoffset = 0;
    FBDevShared  *shared = dfb_fbdev->shared;
    struct fb_var_screeninfo *var = NULL;

    if ((NULL == dfb_config) || (NULL == shared))
    {
        return DFB_OK;
    }
    ...
    yoffset = (buffer - shared->fix.smem_start) / shared->fix.line_length;
    if (0 == var->yres)
    {
        *pbufferid = 0;
    }

    *pbufferid = yoffset / var->yres;

    return DFB_OK;
}


DFBResult dfb_fbdev_quene_buffnum(int layer_id, int *pbuffer_num)
{
    CoreLayerRegionConfig *config = NULL;
    ...
    *pbuffer_num = (DLBM_TRIPLE == config->buffermode) ? (3) : (2);
