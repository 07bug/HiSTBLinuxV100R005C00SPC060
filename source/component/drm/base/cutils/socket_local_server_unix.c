...
//x00418653 replace keywords
int socket_local_server(const char *name, int namespaceId, int type_cutil)
{
    errno = ENOSYS;
    return -1;
}
...
//x00418653 replace keywords
int socket_local_server(const char *name, int namespace_cutil, int type_cutil)
{
    int err;
    int s;
    
    s = socket(AF_LOCAL, type_cutil, 0);
    if (s < 0) return -1;

    err = socket_local_server_bind(s, name, namespace_cutil);
    ...
    if ((type_cutil & SOCK_TYPE_MASK) == SOCK_STREAM) {
        int ret;
