...
public:
    ...
    void create(const sp<IMyTestClient> client)
    {
        TEST_PRINTF("==== %s:%d \n", __FUNCTION__, __LINE__);
        Parcel data, reply;
        data.writeStrongBinder(client->asBinder());
        TEST_PRINTF("==== %s:%d \n", __FUNCTION__, __LINE__);
        remote()->transact(TEST_CMD_CREATE_HDL, data, &reply);
        TEST_PRINTF("==== %s:%d\n ", __FUNCTION__, __LINE__);
        return;
    }
    ...
    void func_0byte()
    {
        ...
        printf("=================== call func_0byte before \n");
        remote()->transact(TEST_CMD_0_BYTE, data, &reply);
        printf("=================== call func_0byte end \n");
                                      ...
                                      uint32_t flags)
{
    switch(code)
    {
        ...
        case TEST_CMD_0_BYTE:
        {
            ...
            //CHECK_INTERFACE(IMyTestService, data, reply);
            printf("=================== call func_0byte xxxxxx \n");
            func_0byte();
            printf("=================== call func_0byte bbbbbb \n");
            return NO_ERROR;
         }
        break;
        ...
        case TEST_CMD_CREATE_HDL:
        {
            TEST_PRINTF("==== %s:%d\n ", __FUNCTION__, __LINE__);
            sp<IMyTestClient> client =
                interface_cast<IMyTestClient>(data.readStrongBinder());
            TEST_PRINTF("==== %s:%d\n ", __FUNCTION__, __LINE__);
            create(client);
            TEST_PRINTF("==== %s:%d\n ", __FUNCTION__, __LINE__);
            return NO_ERROR;
        ...
        {
            TEST_PRINTF("Receive TEST_CMD_4K_BYTE\n");
            //CHECK_INTERFACE(IMyTestService, data, reply);
