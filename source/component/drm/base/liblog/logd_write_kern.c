...
int __android_log_print_ex(int prio, const char *tag, const char *file, int line, const char *fmt, ...)
{
    va_list ap;
    char buf[LOG_BUF_SIZE];
    int len = 0;
    ...
    tmp = ((NULL == tmp) ? file : (tmp + 1));

    snprintf(buf, LOG_BUF_SIZE - 1, "[%s:%d] ", tmp, line);
    len = strlen(buf);

    va_start(ap, fmt);
    vsnprintf(buf + len, LOG_BUF_SIZE - len - 1, fmt, ap);
    va_end(ap);
