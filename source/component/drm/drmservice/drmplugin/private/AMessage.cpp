...
status_t AReplyToken::setReply(const sp<AMessage> &reply) {
    if (mReplied) {
        HI_LOGE("trying to post a duplicate reply");
        return -EBUSY;
    }
    mReply = reply;
    mReplied = true;
...
void AMessage::deliver() {
    ...
    if (handler == NULL) {
        HI_LOGW("failed to deliver message as target handler %d is gone.", mTarget);
        return;
    }
...
status_t AMessage::post(int64_t delayUs) {
    ...
    if (looper == NULL) {
        HI_LOGW("failed to post message as target looper for handler %d is gone.", mTarget);
        return -ENOENT;
    }
...
status_t AMessage::postAndAwaitResponse(sp<AMessage> *response) {
    ...
    if (token == NULL) {
        HI_LOGE("failed to create reply token");
        return -ENOMEM;
    }
...
status_t AMessage::postReply(const sp<AReplyToken> &replyToken) {
    if (replyToken == NULL) {
        HI_LOGW("failed to post reply to a NULL token");
        return -ENOENT;
    }
    sp<ALooper> looper = replyToken->getLooper();
    if (looper == NULL) {
        HI_LOGW("failed to post reply as target looper is gone.");
        return -ENOENT;
    }
...
sp<AMessage> AMessage::FromParcel(const Parcel &parcel) {
    ...
    for (size_t i = 0; i < msg->mNumItems; ++i) {
        ...
        switch (item->mType) {
            ...
            default:
            {
                HI_LOGE("This type of object cannot cross process boundaries.");
                HI_LOGE("%s:no data type match");
            }
        }
    }
