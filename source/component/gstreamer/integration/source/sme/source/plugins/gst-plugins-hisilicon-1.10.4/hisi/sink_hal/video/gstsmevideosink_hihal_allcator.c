    ...
    gsize maxsize, gsize align, gsize offset, gsize size)
{
  ...
  slice_size = sizeof (GstMemoryHiVOMMZ) + (gsize)(maxsize);
  ...
  eMemSecRet = memset_s(&stMmzBuf, sizeof(HI_MMZ_BUF_S), 0x0, sizeof(HI_MMZ_BUF_S));
  if(eMemSecRet != 0){
      g_free(hivommz_mem);
  ...
  stMmzBuf.bufsize = (gsize)(maxsize);

//lint -e119
  flag_snp = vsnprintf_s(stMmzBuf.bufname, sizeof(stMmzBuf.bufname),
      sizeof(stMmzBuf.bufname) - 1, "VO-%d", g_atomic_int_add(&vdec_outbuf_num, 1));
  if(flag_snp <= 0){
      g_free(hivommz_mem);
...
//lint +e119
  ...
  _hivommz_init(allocator, stMmzBuf, hivommz_mem, flags, NULL, slice_size, data,
    0, align, offset, (unsigned int)(maxsize), NULL, NULL);
