  ...
  while ((msg = g_queue_pop_head (&comp->messages))) {
    ...
    switch (msg->type) {
      ...
      case GST_OMX_MESSAGE_FLUSH:{
        ...
        GST_INFO_OBJECT (comp->parent, "%s port %u flushed", comp->name,
            port->index);
      ...
      case GST_OMX_MESSAGE_PORT_ENABLE:{
        ...
        GST_INFO_OBJECT (comp->parent, "%s port %u %s", comp->name,
      ...
      case GST_OMX_MESSAGE_PORT_SETTINGS_CHANGED:{
        ...
        GST_INFO_OBJECT (comp->parent, "%s settings changed (port %u)",
      ...
      case GST_OMX_MESSAGE_BUFFER_FLAG:{
        ...
        GST_INFO_OBJECT (comp->parent, "%s port %u got buffer flags 0x%08x",
    ...
    OMX_U32 nData1, OMX_U32 nData2, OMX_PTR pEventData)
{
  ...
  switch (eEvent) {
    case OMX_EventCmdComplete:
    {
      ...
      GST_INFO_OBJECT (comp->parent, "%s %s command complete (%d)",
    ...
    case OMX_EventPortSettingsChanged:
    {
      ...
      GST_INFO_OBJECT (comp->parent, "%s settings changed (port index: %u)",
    ...
    default:
      GST_INFO_OBJECT (comp->parent, "%s unknown event 0x%08x", comp->name,
          eEvent);
      break;
  }
    ...
    OMX_BUFFERHEADERTYPE * pBuffer)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "%s port %u emptied buffer %p (%p)",
    ...
    OMX_BUFFERHEADERTYPE * pBuffer)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "%s port %u filled buffer %p (%p)", comp->name,
    ...
    const gchar * component_name, const gchar * component_role, guint64 hacks)
{
  ...
  if (component_role && !(hacks & GST_OMX_HACK_NO_COMPONENT_ROLE)) {
    ...
    GST_INFO_OBJECT (parent, "Setting component role to '%s': %s (0x%08x)",
...
OMX_ERRORTYPE
gst_omx_component_get_extension_index (GstOMXComponent * comp, OMX_STRING param_name,
    OMX_INDEXTYPE *pindex_type)
{
  OMX_ERRORTYPE err;

  g_return_val_if_fail (comp != NULL, OMX_ErrorUndefined);
  g_return_val_if_fail (param_name != NULL, OMX_ErrorUndefined);
  g_return_val_if_fail (pindex_type != NULL, OMX_ErrorUndefined);

  GST_INFO_OBJECT (comp->parent, "Getting %s get_extension_index param_name:%s",
      comp->name, param_name);

  err = OMX_GetExtensionIndex(comp->handle, param_name, pindex_type);

  GST_INFO_OBJECT (comp->parent, "Got %s param_name:%s, pindex_type:%d, err:%#x",
    comp->name, param_name, *pindex_type,err);
...
OMX_ERRORTYPE
gst_omx_port_get_channel_attributes (GstOMXPort * port,
    OMX_HISI_PARAM_CHANNELATTRIBUTES * channel_attr)
{
  GstOMXComponent *comp;
  OMX_ERRORTYPE err;
  ...
  if (channel_attr)
    err = gst_omx_component_get_parameter (comp, OMX_HisiIndexChannelAttributes,
      channel_attr);
...
OMX_ERRORTYPE
gst_omx_port_update_channel_attributes (GstOMXPort * port,
    OMX_HISI_PARAM_CHANNELATTRIBUTES * channel_attr)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;
  GstOMXComponent *comp;
  ...
  if (channel_attr) {
    err =
      gst_omx_component_set_parameter (comp, OMX_HisiIndexChannelAttributes,
      channel_attr);

    gst_omx_component_get_parameter (comp, OMX_HisiIndexChannelAttributes,
      channel_attr);

    GST_INFO_OBJECT (comp->parent, "Updated %s port %u definition: %s (0x%08x),",
...
retry:
  ...
  if (port->port_def.eDir == OMX_DirOutput && port->eos) {
    if (!g_queue_is_empty (&port->pending_buffers)) {
      GST_INFO_OBJECT (comp->parent, "%s output port %u is EOS but has "
...
gst_omx_port_release_buffer (GstOMXPort * port, GstOMXBuffer * buf)
{
  ...
  if (port->port_def.eDir == OMX_DirOutput) {
    ...
    /* for 4k bypass */
    //buf->omx_buf->nFlags = 0;
    //OMX_BUFFERFLAG_OCCOUPYFRAME = 0x20000000
    buf->omx_buf->nFlags &= OMX_BUFFERFLAG_OCCOUPYFRAME;
  ...
  /* when set CC mode = last_frame, and quit: vsink unref last gst_buf in _wr_deinit, we
   *   should not send last frame's gstomxbuf to OMX, or OMX will free last frame ignoring vo-occupy flags,
   *   Instead, we put it to pending_buffers, and have no chance to clear vo-occupy flags unless deallocate omx_buffers
   *   the scenes are as follow:
   *   1. resolution_changed: send last frame's gstomxbuf to pending_buffers, and deallocate
   *       gstomxbufs(loop in reconfig, will not acquire gstomxbuf)
   *   2. change channel in dvb:
   *        2.1 codec_changed: set vdec to NULL, send drain to vsink when _shut_down ,
   *             put last frame's gstomxbuf into pending_buf(loop has been stopped, gstomxbuf
   *             will not be used)and deallocate gstomxbufs
   *        2.2 resolution_changed: same as  resolution_changed
   *        2.3 codec and resolution not changed: same as  flush, will not in the following branch
   *   3. CC mode = last, quit:  send last frame's gstomxbuf to pending_buffers, loop acquire it, release it (dead loop), need sleep 2ms
   */
  if (port->flushing || port->disabled_pending || !port->port_def.bEnabled
    || (buf->omx_buf->nFlags & OMX_BUFFERFLAG_OCCOUPYFRAME)) {
    if(buf->omx_buf->nFlags & OMX_BUFFERFLAG_OCCOUPYFRAME){
      GST_INFO_OBJECT (comp->parent,"buf is vo_occupy, sleep 2ms for dead loop");
      g_usleep(2000);
    }else{
  ...
  if (port->port_def.eDir == OMX_DirInput) {
    ...
    GST_LOG_OBJECT(comp->parent, "FTB, comp->name:%s, omxbuf :%p,"
      "buf->omx_buf:%p, nFlags is 0x%08x", comp->name,
      buf,  buf->omx_buf, buf->omx_buf->nFlags);
    err = OMX_FillThisBuffer (comp->handle, buf->omx_buf);
  }
  GST_DEBUG_OBJECT (comp->parent, "Released buffer %p to %s port %u: %s "
      "(0x%08x),dir=%d", buf, comp->name, port->index, gst_omx_error_to_string (err),
      err,port->port_def.eDir);
    ...
    const GList * buffers, const GList * images, guint n)
{
  ...
  GST_INFO("port:%d,minbuf:actualbuf[%d:%d],n=%d", port->port_def.eDir,
    port->port_def.nBufferCountMin, port->port_def.nBufferCountActual,n);
...
gst_omx_port_deallocate_buffers_unlocked (GstOMXPort * port)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (buf->omx_buf) {
      ...
      GST_DEBUG("free comp->name is %s port is %d,"
        "OMX buf is:%p,  vo_occupy flag is %08x",
        comp->name, port->index, buf, buf->omx_buf->nFlags);
...
gst_omx_port_populate_unlocked (GstOMXPort * port)
{
  ...
  GST_INFO_OBJECT (comp->parent, "Populating %s port %d", comp->name,
      port->index);

  gst_omx_component_handle_messages (comp);

  if (port->flushing || port->disabled_pending || !port->port_def.bEnabled) {
    GST_WARNING_OBJECT (comp->parent, "%s port %u is flushing or disabled",
        comp->name, port->index);
  ...
  if (port->port_def.eDir == OMX_DirOutput && port->buffers && !port->tunneled) {
    ...
    while ((buf = g_queue_pop_head (&port->pending_buffers))) {
      ...
      //OMX_BUFFERFLAG_OCCOUPYFRAME = 0x20000000
      buf->omx_buf->nFlags &= OMX_BUFFERFLAG_OCCOUPYFRAME;
      GST_LOG_OBJECT(comp->parent, "Populate, comp->name:%s, omxbuf :%p,"
          "buf->omx_buf:%p, nFlags is 0x%08x", comp->name,
          buf, buf->omx_buf, buf->omx_buf->nFlags);
      ...
      GST_INFO_OBJECT (comp->parent, "Passed buffer %p (%p) to component %s",
  ...
  GST_INFO_OBJECT (comp->parent, "Populated %s port %u: %s (0x%08x)",
      comp->name, port->index, gst_omx_error_to_string (err), err);
  gst_omx_component_handle_messages (comp);
...
gst_omx_port_populate (GstOMXPort * port)
{
  ...
  GST_INFO("gst_omx_port_populate portdir=%d", port->port_def.eDir);
  g_mutex_lock (&port->comp->lock);
...
gst_omx_port_wait_enabled_unlocked (GstOMXPort * port, GstClockTime timeout)
{
  ...
  signalled = TRUE;
  last_error = OMX_ErrorNone;
  /* Switch the order of gst_omx_component_handle_messages and
   * gst_omx_port_update_port_definition. There is a case that "port-enable"
   * message comes between the two functions. In the former sequence, flag
   * "port->port_def.bEnabled" got from gst_omx_port_update_port_definition is
   * still not the same as flag "enabled", thus the condition of while is met.
   * The message is immediately processed by gst_omx_component_handle_messages.
   * And then gst_omx_component_wait_message never wait the message, which
   * results in timeout.
   */
  gst_omx_component_handle_messages (comp);
  gst_omx_port_update_port_definition (port, NULL);
...
gboolean
gst_omx_port_is_need_reconfig(GstOMXPort * port)
{
  GstOMXComponent *comp;
  gboolean res = FALSE;
  ...
  if(port->configured_settings_cookie != port->settings_cookie){
    res = TRUE;
    GST_WARNING_OBJECT (comp->parent, "get %s port %u need reconfig",
        comp->name, port->index);
  }else{
    GST_WARNING_OBJECT (comp->parent, "get %s port %u not need reconfig",
        comp->name, port->index);
  }
  g_mutex_unlock (&comp->lock);
...
#ifdef GST_OMX_DDP_DEC_ENABLE
  gst_omx_ddp_dec_get_type,
#endif
#ifdef GST_OMX_DRA_DEC_ENABLE
...
plugin_init (GstPlugin * plugin)
{
...
#if defined HACK_CONFIG_DIR
#ifdef __LINUX__
  env_config_dir = "/usr/data/";
#else
  env_config_dir = "/system/lib/";
#endif
#else
  ...
  for (i = 0; i < n_elements; i++) {
    ...
    GST_INFO("gst-omx register element:type-name=%s,elements[i]=%s", g_type_name (type),elements[i]);
    ...
    GST_INFO("gst-omx register element:type-name=%s", type_name);
    g_free (type_name);
  ...
  GST_INFO("gst-omx-plugin,plugin_init out");
