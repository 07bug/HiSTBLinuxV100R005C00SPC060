...
#ifdef SME_GST_OMX
#define HACK_CONFIG_DIR
#define GST_OMX_DDP_DEC_ENABLE
#define COMPUTE_OUT_PTS
#define FIX_ADD_HEADER
#endif
...
OMX_ERRORTYPE     gst_omx_component_get_extension_index (GstOMXComponent * comp, OMX_STRING param_name,OMX_INDEXTYPE *pindex_type);
...
OMX_ERRORTYPE     gst_omx_port_get_channel_attributes (GstOMXPort * port,  OMX_HISI_PARAM_CHANNELATTRIBUTES * channel_attr);
OMX_ERRORTYPE     gst_omx_port_update_channel_attributes (GstOMXPort * port, OMX_HISI_PARAM_CHANNELATTRIBUTES * channel_attr);
...
/*
@when omx-il send GST_OMX_MESSAGE_PORT_SETTINGS_CHANGED then settings_cookie++,
@at this time configured_settings_cookie != settings_cookie;
@and if plugins reconfigured then need call gst_omx_port_mark_reconfigured
@at this time port->configured_settings_cookie == settings_cookie;
@returns:if port->configured_settings_cookie != port->settings_cookie return TRUE.
@        else return FALSE;
*/
gboolean          gst_omx_port_is_need_reconfig (GstOMXPort * port);
