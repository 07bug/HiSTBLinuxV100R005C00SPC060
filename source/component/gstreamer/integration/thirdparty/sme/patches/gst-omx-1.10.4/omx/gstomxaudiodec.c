...
//#define DUMP_AUDIO_FILE

#ifdef DUMP_AUDIO_FILE
...
#ifdef DUMP_AUDIO_FILE
    if (NULL == input_file) {
        input_file = fopen("/mnt/dump/audio_input.es","wb");
    }
    if (NULL == output_file) {
        output_file = fopen("/mnt/dump/audio_output.pcm","wb");
    }
#endif
}
...
gst_omx_audio_dec_loop (GstOMXAudioDec * self)
{
  ...
  if (!gst_pad_has_current_caps (GST_AUDIO_DECODER_SRC_PAD (self)) ||
    ...
    if (acq_return == GST_OMX_ACQUIRE_BUFFER_RECONFIGURE) {
      ...
      if (err != OMX_ErrorNone) {
        if (port->flushing) {
          /* if port is in flushing state, don't treat it as error */
          GST_WARNING_OBJECT(self,
              "populate fail due to flushing, don't treat is as error");
        } else {
          goto reconfigure_error;
        }
      }
  ...
  GST_INFO_OBJECT (self, "Handling buffer: 0x%08x %" G_GUINT64_FORMAT" %d",
      (guint) buf->omx_buf->nFlags,
      (guint64) GST_OMX_GET_TICKS (buf->omx_buf->nTimeStamp), buf->omx_buf->nFilledLen);
...
#ifdef DUMP_AUDIO_FILE
    if (output_file) {
        fwrite(buf->omx_buf->pBuffer + buf->omx_buf->nOffset,1,buf->omx_buf->nFilledLen,output_file);
    }
#endif
        ...
        if (buf->omx_buf->nTickCount != 0)
          ...
          guint sample_num = buf->omx_buf->nFilledLen / self->info.bpf;
          GST_BUFFER_DURATION (outbuf) =
...
gst_omx_audio_dec_handle_frame (GstAudioDecoder * decoder, GstBuffer * inbuf)
{
  ...
  while (offset < minfo.size) {
    ...
    GST_DEBUG_OBJECT (self, "Passing frame offset %d to the component, timestamp %llu",
                        offset, (guint64) GST_OMX_GET_TICKS (buf->omx_buf->nTimeStamp));
...
#ifdef FIX_ADD_HEADER
    gboolean b_need_add_header = FALSE;
    OMX_U8 * header_buf = NULL;
    OMX_U32  header_len = 0;
    if (klass->get_header_len) {
        header_len = klass->get_header_len(self, minfo.data + offset, buf->omx_buf->nFilledLen);
        if (header_len > 0) {
            if (buf->omx_buf->nAllocLen - buf->omx_buf->nOffset < buf->omx_buf->nFilledLen + header_len) {
                // recal nFilledLen
                GST_INFO_OBJECT (self, "m_info.size %d, buf->omx_buf->nAllocLen %d - buf->omx_buf->nOffset %d,  buf->omx_buf->nFilledLen %d +  header_len %d",
                  minfo.size, buf->omx_buf->nAllocLen, buf->omx_buf->nOffset, buf->omx_buf->nFilledLen, header_len  );
                buf->omx_buf->nFilledLen -= header_len;
            }
        }
    }

    if (header_len > 0 && klass->check_need_header) {
        b_need_add_header = klass->check_need_header(self,
            minfo.data + offset, buf->omx_buf->nFilledLen,
            &header_buf, &header_len);
        if (b_need_add_header && header_buf && header_len) {
            // copy adts header to omx_buf
            memcpy(buf->omx_buf->pBuffer + buf->omx_buf->nOffset,header_buf,header_len);
        }
    }
#endif
    ...
    gst_buffer_extract (inbuf, offset,
        buf->omx_buf->pBuffer + buf->omx_buf->nOffset + header_len,
        buf->omx_buf->nFilledLen);
#else
    gst_buffer_extract (inbuf, offset,
        buf->omx_buf->pBuffer + buf->omx_buf->nOffset,
...
#ifdef DUMP_AUDIO_FILE
    if (input_file) {
        fwrite(buf->omx_buf->pBuffer + buf->omx_buf->nOffset,1,buf->omx_buf->nFilledLen,input_file);
    }
#endif
