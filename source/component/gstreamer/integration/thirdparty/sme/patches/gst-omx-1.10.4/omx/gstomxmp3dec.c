...
gst_omx_mp3_dec_class_init (GstOMXMP3DecClass * klass)
{
  ...
  /**< The original mpegversion is not used at all, and it seems have the same
   *   meaning as mpegaudioversion. To match with caps from demux, replace
   *   mpegaudioversion with mpegversion.
   **/
...
#ifdef SME_GST_OMX
  if ((!gst_structure_get_int (s, "mpegaudioversion", &mpegaudioversion) && !gst_structure_get_int (s, "mpegversion", &mpegaudioversion)) ||
#else
