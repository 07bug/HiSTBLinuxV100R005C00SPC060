...
#EXT_TYPE_GZ := tar.gz
EXT_TYPE_XZ := tar.xz
PKG_GST_BASE_NAME := gst-plugins-base-1.10.4
...
$(PRIVATE_PKG_GST_BASE_NAME_INSTALL):$(PRIVATE_PKG_GST_BASE_NAME_MAKE)
	...
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstallocators-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	...
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstapp-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstapp-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstaudio-1.0.so $(TARGET_OUT_LIB_DIR)/libgstaudio-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstaudio-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstaudio-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstfft-1.0.so $(TARGET_OUT_LIB_DIR)/libgstfft-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstfft-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstfft-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstpbutils-1.0.so $(TARGET_OUT_LIB_DIR)/libgstpbutils-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstpbutils-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstpbutils-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstriff-1.0.so $(TARGET_OUT_LIB_DIR)/libgstriff-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstriff-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstpbutils-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstrtp-1.0.so $(TARGET_OUT_LIB_DIR)/libgstrtp-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstrtp-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstrtp-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstrtsp-1.0.so $(TARGET_OUT_LIB_DIR)/libgstrtsp-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstrtsp-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstrtsp-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstsdp-1.0.so $(TARGET_OUT_LIB_DIR)/libgstsdp-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstsdp-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstsdp-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgsttag-1.0.so $(TARGET_OUT_LIB_DIR)/libgsttag-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgsttag-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgsttag-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstvideo-1.0.so $(TARGET_OUT_LIB_DIR)/libgstvideo-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstvideo-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstvideo-1.0.so
endif
...
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	...
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/bin/gst-play-1.0 $(TARGET_OUT_BIN_DIR)/
...
$(PRIVATE_PKG_GST_BASE_NAME_PREPARE):
	...
	tar -xf $(SME_THIRDPARTY_ORG_DIR)/../../../../../../third_party/open_source/$(PKG_GST_BASE_NAME).$(EXT_TYPE_XZ) -C $(SME_THIRDPARTY_MERGE_DIR)
...
$(LOCAL_MODULE)-clean:
	...
	#clear patch for plugin only
	$(NC)rm -rf $(SME_THIRDPARTY_PATCH_DIR)/$(PKG_GST_BASE_NAME)
