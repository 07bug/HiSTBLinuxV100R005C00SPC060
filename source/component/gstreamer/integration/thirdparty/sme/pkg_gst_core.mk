...
#EXT_TYPE_GZ := tar.gz
EXT_TYPE_XZ := tar.xz
PKG_GST_CORE_NAME := gstreamer-1.10.4
...
$(PRIVATE_PKG_GST_CORE_NAME_INSTALL):$(PRIVATE_PKG_GST_CORE_NAME_MAKE)
	...
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstbase-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstbase-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstcheck-1.0.so $(TARGET_OUT_LIB_DIR)/libgstcheck-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstcheck-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstcheck-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstcontroller-1.0.so $(TARGET_OUT_LIB_DIR)/libgstcontroller-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstcontroller-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstcontroller-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstnet-1.0.so $(TARGET_OUT_LIB_DIR)/libgstnet-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstnet-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstnet-1.0.so
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstreamer-1.0.so $(TARGET_OUT_LIB_DIR)/libgstreamer-1.0.so
	$(NC)cp -af $(THIRDPARTY_OBJ_DIR)/lib/libgstreamer-1.0.so*.0 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libgstreamer-1.0.so
endif
...
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	...
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/gstreamer-1.0/libgstcoretracers.so $(TARGET_OUT_PLUGIN_DIR)/libgstcoretracers.so
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_PLUGIN_DIR)/libgstcoretracers.so
endif
	...
	cp -af $(THIRDPARTY_OBJ_DIR)/bin/gst-stats-1.0 $(TARGET_OUT_BIN_DIR)/gst-stats-1.0
...
$(PRIVATE_PKG_GST_CORE_NAME_PREPARE):
	...
	tar -xf $(SME_THIRDPARTY_ORG_DIR)/../../../../../../third_party/open_source/$(PKG_GST_CORE_NAME).$(EXT_TYPE_XZ) -C $(SME_THIRDPARTY_MERGE_DIR)
...
$(LOCAL_MODULE)-clean:
	...
	$(NC)rm -rf $(TARGET_OUT_PLUGIN_DIR)/libgstcoretracers.so
	...
	$(NC)rm -rf  $(TARGET_OUT_BIN_DIR)/gst-stats-1.0
