...
$(PRIVATE_PKG_NETTLE_NAME_INSTALL):$(PRIVATE_PKG_NETTLE_NAME_MAKE)
	...
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libnettle.so.6 $(TARGET_OUT_LIB_DIR)/
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libnettle.so.6.3 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libnettle.so.6.3
endif
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libhogweed.so $(TARGET_OUT_LIB_DIR)/libhogweed.so
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libhogweed.so.4 $(TARGET_OUT_LIB_DIR)/
	cp -af $(THIRDPARTY_OBJ_DIR)/lib/libhogweed.so.4.3 $(TARGET_OUT_LIB_DIR)/
ifeq ($(STRIP_IN_RELEASE_VERSION), y)
	$(TARGET_STRIP) $(TARGET_OUT_LIB_DIR)/libhogweed.so.4.3
endif
...
$(PRIVATE_PKG_NETTLE_NAME_PREPARE):
	...
	tar -xf $(SME_THIRDPARTY_ORG_DIR)/../../../../../../third_party/open_source/$(PKG_NETTLE_NAME).$(EXT_TYPE_GZ) -C $(SME_THIRDPARTY_MERGE_DIR)
...
$(LOCAL_MODULE)-clean:
	...
	$(NC)rm -rf $(TARGET_OUT_LIB_DIR)/libnettle.so.6
	$(NC)rm -rf $(TARGET_OUT_LIB_DIR)/libnettle.so.6.3
	$(NC)rm -rf $(TARGET_OUT_LIB_DIR)/libhogweed.so
	$(NC)rm -rf $(TARGET_OUT_LIB_DIR)/libhogweed.so.4
	$(NC)rm -rf $(TARGET_OUT_LIB_DIR)/libhogweed.so.4.3
