...
static sparse_chunk_ctl_t* LOADER_BURN_New_sparseCtl(HI_U8* pucDataBuff, HI_U32 u32DataLen)
{
    ...
    COM_MemSet(pstSparseCtl, 0x00, sizeof(sparse_chunk_ctl_t));
...
static HI_S32 HI_Flash_Write_Wrapper(HI_HANDLE hFlash, HI_U64 u64Address, HI_U8* pBuf, HI_U32 u32Len, HI_U32 u32Flags, HI_U32 u32blkSZ)
{
    ...
    if (u64Address != u64AlignedWriteOffset)
    {
        ...
        COM_MemSet(pucBuf, 0x00, u32blkSZ);
...
HI_S32 LOADER_BURN_WriteData_ext4sp(FLASH_DATA_INFO_S* pstFlashDataInfo, HI_U8* pucSrcDataBuff, HI_U32 u32SrcDataLen, HI_VOID* private)
{
    ...
    if (HI_NULL_PTR == pstSparseCtl)
    {
        ...
        g_u32FlashCurWriteLen += pstSparseCtl->sparseHeader.file_hdr_sz;
...
search_chunk:
    ...
    */
    if (HI_TRUE != pstSparseCtl->bActived)
    {
        ...
        {
            COM_MemCpy(((HI_U8*)(&pstSparseCtl->chunkHeader)) + pstSparseCtl->u32HeaderOffset, pucDataBuff,
            ...
            g_u32FlashCurWriteLen += pstSparseCtl->sparseHeader.chunk_hdr_sz - pstSparseCtl->u32HeaderOffset;
        }
        else
        {
            if (0 != u32DataLen)
            {
                COM_MemCpy(&pstSparseCtl->chunkHeader, pucDataBuff, u32DataLen);
            }

            pstSparseCtl->u32HeaderOffset = u32DataLen;

            COM_DEBUG("found partial chunk header: index(%d), valid_header_offset(%d).\n",
                      pstSparseCtl->u32ChunkIndex,
                      pstSparseCtl->u32HeaderOffset);

            if (HI_NULL_PTR != s_pfnProgressCallback)
            {
                s32Ret = s_pfnProgressCallback(g_u32FlashCurWriteLen, g_u32FlashTotalWriteLen);
        ...
        if (pstSparseCtl->u32ChunkIndex < pstSparseCtl->sparseHeader.total_chunks)
        {
            ...
            switch (pstSparseCtl->chunkHeader.chunk_type)
            {
                case CHUNK_TYPE_RAW:
                {
                    ...
                    HI_U32 u32WriteDataLen = u32ChunkDataLen - pstSparseCtl->u32DataOffset < u32DataLen? u32ChunkDataLen - pstSparseCtl->u32DataOffset: u32DataLen;
                    ...
                    pstSparseCtl->u32DataOffset += u32WriteDataLen;
                    pstFlashDataInfo->u64WriteOffset += u32WriteDataLen;
                    g_u32FlashCurWriteLen += u32WriteDataLen;
                    ...
                    if (u32WriteDataLen < u32DataLen)
                    {
                        pucDataBuff += u32WriteDataLen;
                        u32DataLen  -= u32WriteDataLen;

                        pstSparseCtl->bActived = HI_FALSE;

                        goto search_chunk;
                    }
                    else if (u32WriteDataLen == u32DataLen)
                    {
                        if (u32ChunkDataLen == pstSparseCtl->u32DataOffset)
                        {
                            pstSparseCtl->bActived = HI_FALSE;
                        }
                    }
