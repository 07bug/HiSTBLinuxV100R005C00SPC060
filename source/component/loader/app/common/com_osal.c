...
HI_S32 COM_Execvp(HI_CHAR* const argv_list[])
{
    HI_S32 s32Status = HI_FAILURE;
    pid_t  child_pid = 0;
    pid_t  ret_pid = 0;

    if (HI_NULL_PTR == argv_list
        || HI_NULL_PTR == argv_list[0])
    {
        COM_PrintErrCode(COM_ERR_MEM);
        return HI_FAILURE;
    }
    ...
    if (0 == child_pid)
    {
        ...
        COM_ERROR("on error occurred in execve!\r\n");
        abort();
    }
