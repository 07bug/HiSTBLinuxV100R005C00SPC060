...
#define DOWNLOAD_FS_DEVICE_NUM              (2)
...
static HI_CHAR* s_apcDeviceName[DOWNLOAD_FS_DEVICE_NUM] =
{
    "/dev/sda",
    "/dev/sdb",
};
...
static HI_S32 fsFileCheckInUsb(const HI_CHAR* pcFileName, HI_CHAR* pcDeviceName)
{
    HI_S32 i = 0;
    ...
    for (i = 0; i < DOWNLOAD_FS_TRYTIMES; i++)
    {
        ...
        if (!FS_ISBLK(pcDeviceName, stStatBuf))
        {
    ...
    for (i = 1; i <= DOWNLOAD_FS_PARTITION_MAXNUM; i++)
    {
        ...
        snprintf(acDeviceName, sizeof(acDeviceName), "%s%d", pcDeviceName, i);
    ...
    s32Ret = fsMount(pcDeviceName, COM_CFG_D_UPGRADE_FILEPATH);
    if (HI_SUCCESS != s32Ret)
    {
        COM_PrintFuncErr(fsMount, s32Ret);
        return COM_ERR_NOEXIST;
...
static HI_S32 fsFileOpen(HI_HANDLE* phHandle, HI_CHAR* pcFilename)
{
    HI_CHAR acFullPath[PATH_MAX + 1] = {'\0'};
    FILE* pFile = HI_NULL_PTR;

    COM_DBG_FuncEnter();

    if(HI_NULL == realpath(pcFilename, acFullPath))
    {
        COM_PrintErrCode(COM_ERR_FILE);
        return COM_ERR_FILE;
    }

    pFile = fopen(acFullPath, "r");
    if (HI_NULL_PTR == pFile)
    {
...
static HI_S32 fsPartitionReset(HI_CHAR* pcPartitionName)
{
    ...
    if (HI_NULL_PTR != pcPartDes)
    {
        ...
        snprintf(acDeviceName, sizeof(acDeviceName), "/dev/mmcblk0p%d", u32Index + 1);

        argv_list[0] = "/sbin/mkfs.ext4";
        argv_list[1] = acDeviceName;
        argv_list[2] = "-F";
        argv_list[3] = NULL;

        s32Ret = COM_Execvp(argv_list);
        if (HI_SUCCESS != s32Ret)
        {
            COM_PrintFuncErr(COM_Execvp, s32Ret);
            return COM_ERR_DEVICE;
        }
...
HI_S32 DOWNLOAD_FS_Init(HI_LOADER_PARAM_S* pstLoaderParam)
{
    ...
    else if (HI_LOADER_UPGRADE_TYPE_USB == pstLoaderParam->enUpgradeType)
    {
        ...
        for (i = 0; i < DOWNLOAD_FS_DEVICE_NUM; i++)
        {
            s32Ret = fsFileCheckInUsb(acFileName, s_apcDeviceName[i]);
            if (HI_SUCCESS != s32Ret)
            {
                COM_Delayms(5000);
