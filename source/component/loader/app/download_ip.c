 ...
 ******************************************************************************
 ...
 main\1   2015-07-29  sdk  N/A               Restructure this file.
 ******************************************************************************/
...
static HI_S32 net_device_down(HI_U32 u32DevNo)
{
    ...
    HI_OSAL_Snprintf(acDevName, sizeof(acDevName), "eth%d", u32DevNo);

    argv_list[0] = "/sbin/ifconfig";
    argv_list[1] = acDevName;
    argv_list[2] = "down";
    argv_list[3] = NULL;
...
static HI_S32 InitStaticIP(HI_CHAR* pcEthDevName, HI_U32 u32Ipaddr, HI_U32 u32SubnetMask, HI_U32 u32Gateway)
{
    ...
    /* Config gateway */
    argv_list[0] = "/sbin/route";
    argv_list[1] = "add";
    argv_list[2] = "default";
    argv_list[3] = "gw";
    argv_list[4] = acGateWay;
    argv_list[5] = NULL;
...
static HI_S32 InitDynamicIP(HI_CHAR* pcEthDevName)
{
    ...
    ret_pid = waitpid(child_pid, &s32Status, 0);
...
static HI_U8* net_rtrim(HI_U8* pcString)
{
    ...
    pc = pcString + COM_StrLen((HI_CHAR*)pcString) - 1;
...
HI_S32 DOWNLOAD_IP_Init(HI_LOADER_PARAM_S* pstLoaderParam)
{
    ...
    switch (pstIpParam->enProtType)
    {
        case HI_LOADER_IPPROT_TFTP:
        {
            ...
            COM_MemSet(s_pu8Buffer, 0x00, IPDL_SIZE_BUFFER);
