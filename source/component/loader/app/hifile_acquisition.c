...
)
{
    ...
    HI_U8* pu8TempDataBlock = HI_NULL_PTR;
    ...
    if (COM_CFG_D_CNT_MODULE < pstManuInfo->u16FlashMapNum)
    {
        COM_PrintErrCode(COM_ERR_OVERFLOW);
        return COM_ERR_OVERFLOW;
    }

    for (u16Index = 0; u16Index < pstManuInfo->u16FlashMapNum; u16Index++)
    {
        stBufferInitParam.astModuleBuf[u16Index].u32ModuleID   = pstManuInfo->astFlashMap[u16Index].u32ImageID;
        stBufferInitParam.astModuleBuf[u16Index].u32ModuleSize = pstManuInfo->astFlashMap[u16Index].u32ImageLength;
    }
    ...
    pu8DataBlock = COM_MemMalloc(HIFILE_D_MIN_BLOCKSIZE);
    if (HI_NULL_PTR == pu8DataBlock)
    {
        COM_PrintErrCode(COM_ERR_MEM);
        return COM_ERR_MEM;
    }

    COM_MemSet(pu8DataBlock, 0x00, HIFILE_D_MIN_BLOCKSIZE);

    for (u16Index = 0; u16Index < pstManuInfo->u16FlashMapNum; u16Index++)
    {
        pstFlashMap = &(pstManuInfo->astFlashMap[u16Index]);

        if (pstFlashMap->u32ImageOffset < u32TotalRead)
        {
            COM_MemFreeSetNull(pu8DataBlock);
            COM_BUFFER_DeInit();
        ...
        if (pstFlashMap->u32ImageOffset > u32TotalRead)
        {
            ...
            s32Ret = DOWNLOAD_Getdata(pu8DataBlock, u32GetSize, &u32OutSize);
            if (HI_SUCCESS != s32Ret)
            {
                COM_PrintFuncErr(DOWNLOAD_Getdata, s32Ret);
        ...
        s32Ret = DOWNLOAD_Getdata(pu8DataBlock, HIFILE_D_LEN_DATAHEAD, &u32GetSize);
        if (HI_SUCCESS != s32Ret)
        {
            COM_PrintFuncErr(DOWNLOAD_Getdata, s32Ret);
        ...
        pu8TempDataBlock = pu8DataBlock;
        HI_READ_32BIT(pu8TempDataBlock, u32ImageLength, u32TotalRead);
        ...
        HI_READ_32BIT(pu8TempDataBlock, pstFlashMap->u32ImageCRC, u32TotalRead);
        ...
        while (u32ReadSize < pstFlashMap->u32ImageLength)
        {
            ...
            s32Ret = COM_BUFFER_PutData(u16Index, u32SectionIndex, pu8DataBlock, u32GetSize);
            if (HI_SUCCESS != s32Ret)
            {
