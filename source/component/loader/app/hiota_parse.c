...
(
    ...
    HI_U32                  u32DownLoadPartMaxLen,
    HI_U32*                 pu32DownLoadPartLen,
    HIOTA_UPGRADE_INFO_S*   pstUpgradeInfo
)
{
    HI_U8  i = 0;
    ...
    if ((0 == pstUpgradeInfo->u8PartitionCount)
        || (HIOTA_D_MAXNUM_PARTITION < pstUpgradeInfo->u8PartitionCount))
    {
        COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
        return COM_ERR_IMAGE_ILLEGAL;
    }
    ...
    if (u16PartDescritptionLength + u32TotalRead > u32DownLoadPartMaxLen)
    {
        COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
        return COM_ERR_IMAGE_ILLEGAL;
    }
    ...
    if (u8DownloadDescriptionLen + u32TotalRead > u32DownLoadPartMaxLen)
    {
        COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
        return COM_ERR_IMAGE_ILLEGAL;
    }
    ...
    if (u32TotalRead > u32DownLoadPartMaxLen)
    {
        COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
        return COM_ERR_IMAGE_ILLEGAL;
    }

    *pu32DownLoadPartLen = u32TotalRead;
...
)
{
    ...
    HI_SKIP_BYTES(pu8Data, 1, u32TotalRead);
    s32Ret = hiotaParseDownloadPartInfo(pu8Data, u32SectionDataLen - u32TotalRead, &u32DownloadPartInfoLen, pstUpgradeInfo);
    if (HI_SUCCESS != s32Ret)
    {
