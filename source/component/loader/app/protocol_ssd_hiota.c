...
HI_S32 PROT_SSD_HIOTA_Process(HI_VOID)
{
    ...
    COM_BUFFER_DATABLOCK_S astPartData[HIFILE_D_MAXNUM_FLASHMAP];
    HIFILE_MANUINFO_S* pstManuInfo = HI_NULL_PTR;
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    COM_MemSet(astPartData, 0x00, sizeof(astPartData));
    ...
    for (i = 0; i < pstManuInfo->u16FlashMapNum; i++)
    {
        ...
        astPartData[u32ImageNum].u32ModuleID = pstManuInfo->astFlashMap[i].u32ImageID;
        astPartData[u32ImageNum].u32DataSize = pstManuInfo->astFlashMap[i].u32ImageLength;
        astPartData[u32ImageNum].pu8DataBuff = pstManuInfo->astFlashMap[i].pu8ImageData;
        astPartData[u32ImageNum].bLastData   = HI_TRUE;

        u32ImageNum ++;
    }

    s32Ret = s_stCtrlBlock.pfnDataCallback(astPartData,u32ImageNum);
    if (HI_SUCCESS != s32Ret)
    {
