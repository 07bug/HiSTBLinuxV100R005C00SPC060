...
#include "ir.h"
#include "timer.h"
#include "hdmi_v1/hdmi_mcu_task.h"
...
void SaveRegToRam(HI_U32 u32DDRBaseAddr, HI_U32 u32DataBaseAddr, HI_U32 u32RegCount)
{
    ...
    /* add for HI3798MV300 LPDDR3 chip*/
    if ((HI_CHIP_TYPE_HI3798M == g_u8ChipType) && (HI_CHIP_VERSION_V300 == g_u16ChipId))
    {
        /* save 0x140 0x144 0x148 0x14c 0x150*/
        regAddr.val32 = u32TmpAddr;
        regData.val32 = 5;
        write_regValEx();

        regAddr.val32 = u32TmpAddr + 0x4;
        regData.val32 = u32DDRBaseAddr + 0x140;
        write_regValEx();

        for (j = 0; j < 5; j++)
        {
            regAddr.val32 = u32DDRBaseAddr + 0x140 + j * 0x4;
            read_regVal();
        ...
        regAddr.val32 = u32TmpAddr + 0x4;
        regData.val32 = u32DDRBaseAddr + 0x1d4;
        write_regValEx();

        regAddr.val32 = u32DDRBaseAddr + 0x1d4;
        read_regVal();
...
{
    if ((g_u8CaVendorId != CA_VENDOR_ID_1) || (!g_bAdvCaSupport))
    {
    ...
    /* in 98mv200, if enable cec, don't power down */
    if ((HI_CHIP_TYPE_HI3798M == g_u8ChipType) && (HI_CHIP_VERSION_V200 == g_u16ChipId))
    {
        if (g_u8CECWakeUpEnable)
        {
            LowPowerSet();
...
HI_VOID SystemResume(HI_VOID)
{
    if (!(dbgmask & 0x4))
    {
        if ((g_u8HighWakeUp != NORMAL_WAKEUP) \
            || ((HI_CHIP_TYPE_HI3798M == g_u8ChipType) && (HI_CHIP_VERSION_V200 == g_u16ChipId) && g_u8CECWakeUpEnable))
        {
            SystemPowerDown();
        }
...
HI_VOID GetDDRWakeUpParams(HI_VOID)
{
    ...
    if ((HI_CHIP_TYPE_HI3798M == g_u8ChipType) && (HI_CHIP_VERSION_V300 == g_u16ChipId))
    {
        regAddr.val32 = CFG_BASE_ADDR + SC_GEN53;
        read_regVal();
        regAddr.val32 = DATA_BASE_ADDR + 0x800 + i * 0x4;
        write_regValEx();
        i++;

        regAddr.val32 = CFG_BASE_ADDR + SC_GEN55;
        read_regVal();
        regAddr.val32 = DATA_BASE_ADDR + 0x800 + i * 0x4;
        write_regValEx();
        i++;

        regAddr.val32 = CFG_BASE_ADDR + SC_GEN57;
        read_regVal();
        regAddr.val32 = DATA_BASE_ADDR + 0x800 + i * 0x4;
        write_regValEx();
        i++;
