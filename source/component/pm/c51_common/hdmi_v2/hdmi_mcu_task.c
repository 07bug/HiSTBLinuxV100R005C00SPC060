...
#define HDMI_MCU_CPU_CEC_INFO_0_ADDR    0xf8000210  // cpu write Phy Addr
#define HDMI_MCU_CPU_CEC_INFO_1_ADDR    0xf8000214  // cpu write cecLogicAddr
#else
#define HDMI_CPU_MCU_SWITCH_INFO_ADDR   0xf8000f10
#define HDMI_MCU_CPU_CEC_INFO_0_ADDR    0xf8000f14  // cpu write Phy Addr
#define HDMI_MCU_CPU_CEC_INFO_1_ADDR    0xf8000f18  // cpu write cecLogicAddr
#endif

#define HDMI_REG_DDC_MST_CTRL_DEFAULT   0x4a0       // 80k pwd
...
{
    HI_U8           u8TxCnt       = 0;
    HI_U8    u8Phy0 = 0, u8Phy1   = 0;
    HI_BOOL         bNeedResponse = HI_FALSE;
    HDMI_INFO_S     *pstHdmiInfo  = &s_stHdmiInfo;
    CEC_MSG_FRAME_S stFrame;

    if( (pmocflag==HI_TRUE) && (pmocType==HI_UNF_PMOC_WKUP_IR) )
    {
        stFrame.enDestAddr = CEC_LOGICALADD_TV;
        stFrame.enSrcAddr  = pstHdmiInfo->enCecLogicAddr;
        stFrame.stOperand.u8Length  = 0;
        stFrame.u8Opcode   = CEC_OPCODE_TEXT_VIEW_ON;
        MCU_HDMI_CecFrameTx(&stFrame);
        stFrame.u8Opcode   = CEC_OPCODE_IMAGE_VIEW_ON;
        MCU_HDMI_CecFrameTx(&stFrame);
#if HDMI_MCU_DEBUG
        HDMI_MCU_ComPuts("wakeTV\r\n");
#endif
    }

    u8Phy0 = ((pstHdmiInfo->au8PhyAddr[0] << 4) & 0xf0) | (pstHdmiInfo->au8PhyAddr[1] & 0x0f); // [Physical Address(A.B.C.D):A B]
    u8Phy1 = ((pstHdmiInfo->au8PhyAddr[2] << 4) & 0xf0) | (pstHdmiInfo->au8PhyAddr[3] & 0x0f); // [Physical Address(A.B.C.D):C D]
    ...
    while (HI_SUCCESS == CecRxPoolRead(&stFrame) && (u8TxCnt < 3))
    {
        ...
        switch (stFrame.u8Opcode)
        {
            ...
            case CEC_OPCODE_ROUTING_CHANGE:
                if ((stFrame.stOperand.u8Data[2]&0xf0) == (u8Phy0&0xf0))
                {
                    HDMI_MCU_ComPuts("wakeup\n\r");
                    pmocflag = HI_TRUE;
                    pmocType = HI_UNF_PMOC_WKUP_CEC;
            ...
            case CEC_OPCODE_ACTIVE_SOURCE:
                if ((stFrame.stOperand.u8Data[0]&0xf0) == (u8Phy0&0xf0))
                {
                    HDMI_MCU_ComPuts("wakeup\n\r");
                    pmocflag = HI_TRUE;
                    pmocType = HI_UNF_PMOC_WKUP_CEC;
            ...
            case CEC_OPCODE_GIVE_OSD_NAME:
                ...
                stFrame.stOperand.u8Data[1] = 'i';
                stFrame.stOperand.u8Data[2] = 's';
            ...
            case CEC_OPCODE_GIVE_PHYSICAL_ADDRESS: /* wakeup */
                ...
                stFrame.stOperand.u8Data[0] = u8Phy0;
                stFrame.stOperand.u8Data[1] = u8Phy1;
                stFrame.stOperand.u8Data[2] = pstHdmiInfo->enCecLogicAddr;
                stFrame.stOperand.u8Length  = 3;
                HDMI_MCU_ComPuts("wakeup\n\r");
                pmocflag = HI_TRUE;
                pmocType = HI_UNF_PMOC_WKUP_CEC;
            ...
            case CEC_OPCODE_GIVE_DEVICE_POWER_STATUS:
                ...
                stFrame.stOperand.u8Data[0] = 0x01;
                stFrame.stOperand.u8Length  = 1;
                bNeedResponse = HI_TRUE;
                break;
            case CEC_OPCODE_GET_MENU_LANGUAGE:
                ...
                stFrame.stOperand.u8Data[1] = 'h';
                stFrame.stOperand.u8Data[2] = 'i';
                stFrame.stOperand.u8Length  = 3;
                bNeedResponse = HI_TRUE;
                break;
...
HI_VOID MCU_HDMI_TaskHandle(HI_VOID)
{
    ...
    if (pstHdmiInfo->bReReadEdid)
    {
        if (HI_SUCCESS == MCU_HDMI_DdcEdidCecAddrGet(pstHdmiInfo->au8PhyAddr, CEC_PHY_ADDR_SIZE))
        {
            pstHdmiInfo->bReReadEdid = HI_FALSE;
            pstHdmiInfo->bPhyAddrSet = HI_TRUE;
            /* restore cpu ddc mode */
            HDMI_MCU_ComRegWrite(REG_AON_DDC_MST_CTRL, HDMI_REG_DDC_MST_CTRL_DEFAULT);
#if HDMI_MCU_DEBUG
