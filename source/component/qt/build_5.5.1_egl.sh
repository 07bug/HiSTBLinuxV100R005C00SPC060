...
export QMAKE_CONFIG_FILE=$QT_PATCH_PATH/qt-everywhere-opensource-src-5.5.1/qtbase/mkspecs/devices/linux-arm-hisilicon-${GRAPHICS_DEVICE}/qmake.conf
...
func_build_main()
{
	...
	./configure -opensource -release -silent -confirm-license -prefix /${QT_PRE_PATH}-${GRAPHICS_DEVICE}  \
		...
		-device-option CROSS_COMPILE=${BUILDER_PATH}/../bin/${CROSS}- -L${BUILDER_LIB_PATH} -c++11 \
		-L${SHARE_LIB_PATH} -skip qtandroidextras -skip qtmacextras -skip qtwinextras -skip qtdoc -D EGL_FBDEV -eglfs -opengl es2
	if [ $? != 0 ] ;then
...
func_install()
{
	...
	cp -r $QT_SOURCE/resources/webgl $ROOTFS_QT_ROOT/bin
	cp $QT_SOURCE/resources/hello.html $ROOTFS_QT_ROOT/bin
	cp $QT_SOURCE/resources/run_qt5.5_egl $ROOTFS_QT_ROOT/bin
