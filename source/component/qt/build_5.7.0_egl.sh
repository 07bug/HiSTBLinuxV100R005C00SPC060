...
export BUILDER_PATH=$SDK_DIR/tools/linux/toolchains/arm-histbv320-linux/target
export SYSTEM_ROOT=$BUILDER_PATH
export CROSS=arm-histbv320-linux
...
func_build_main()
{
	...
	chmod 777 configure
	./configure --host=arm-histbv320-linux --prefix=$MY_INSTALL_PATH --enable-shared
	if [ $? != 0 ] ;then
	...
	chmod 777 configure
	CC=arm-histbv320-linux-gcc CXX=arm-histbv320-linux-g++ CFLAGS="-Wcast-align -g -O2 -I$MY_INSTALL_PATH/include" \
	CXXFLAGS=${CFLAGS} LDFLAGS+="-L$MY_INSTALL_PATH/lib -lexpat" ./configure --host=arm-histbv320-linux --prefix=$MY_INSTALL_PATH --enable-shared
	if [ $? != 0 ] ;then
	...
	./configure --host=arm-histbv320-linux --prefix=$MY_INSTALL_PATH --enable-shared --with-expat-includes=$MY_INSTALL_PATH/include \
	...
	CC=arm-histbv320-linux-gcc CXX=arm-histbv320-linux-g++ ./runConfigureICU Linux/gcc
	make -j
	make clean

	chmod 777 configure
	./configure --host=arm-histbv320-linux --prefix=$MY_INSTALL_PATH --disable-samples --disable-tests --with-cross-build=$(pwd) \
