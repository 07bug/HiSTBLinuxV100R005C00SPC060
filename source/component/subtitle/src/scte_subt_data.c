...
#define SCTE_SECTION_SEGMENT_MIN_SIZE (13)  /*Subtitle Message Format with message_body() Delineated*/
...
HI_S32 SCTE_SUBT_Data_Create(HI_VOID* hParse, HI_VOID** pphData)
{
    ...
    pstDataRecv = (SCTE_SUBT_DATA_RECV_S*)HI_MEMSUBT_MALLOC(sizeof(SCTE_SUBT_DATA_RECV_S));
...
HI_S32 SCTE_SUBT_Data_Destroy(HI_VOID* hData)
{
    ...
    HI_MEMSUBT_FREE((void*)pstDataRecv);
    pstDataRecv = HI_NULL;
...
HI_S32 SCTE_SUBT_Data_Inject(HI_VOID* hData, const HI_U8* pu8Data, HI_U32 u32DataSize)
{
    ...
    if (pu8Data[0] == 0xC6)
    {
        ...
        if ((pu8Data[3] & 0x40) && (u32DataSize >= SCTE_SECTION_SEGMENT_MIN_SIZE))
        {
