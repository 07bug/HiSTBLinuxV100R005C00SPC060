...
HI_S32 SCTE_SUBT_Display_Create(SCTE_SUBT_DISPLAY_CALLBACK_FN pfnCallback, HI_U32 u32UserData, HI_VOID** pphDisplay)
{
    ...
    pstDisplayInfo = (SCTE_SUBT_DISPALY_INFO_S*)HI_MEMSUBT_MALLOC(sizeof(SCTE_SUBT_DISPALY_INFO_S));
    ...
    pstDisplayInfo->pstDisplayParam = (SCTE_SUBT_DISPALY_PARAM_S*)HI_MEMSUBT_MALLOC(sizeof(SCTE_SUBT_DISPALY_PARAM_S));

    if (HI_NULL == pstDisplayInfo->pstDisplayParam)
    {
        HI_MEMSUBT_FREE(pstDisplayInfo);
...
HI_S32 SCTE_SUBT_Display_Destroy(HI_VOID* hDisplay)
{
    ...
    if (HI_NULL != pstDisplayInfo->pstDisplayParam)
    {
        HI_MEMSUBT_FREE((void*)pstDisplayInfo->pstDisplayParam);
        pstDisplayInfo->pstDisplayParam = HI_NULL;
    }

    HI_MEMSUBT_FREE((void*)pstDisplayInfo);
    pstDisplayInfo = HI_NULL;
