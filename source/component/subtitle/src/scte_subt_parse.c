...
#define SCTE_SUBT_CHECK_START_PIXON 2  //check start XXX bytes, if pixon then skip it
...
static HI_S32 DecompressBitmap(SCTE_SUBT_PARSE_INFO_S* pstParseInfo, HI_U8* pu8DataSrc, HI_U32 u32DataLen)
{
    ...
    HI_U32 u32CheckPixon = SCTE_SUBT_CHECK_START_PIXON;
...
static HI_S32 ParseBitmapHead(SCTE_SUBT_PARSE_INFO_S* pstParseInfo, HI_U8* pu8DataSrc, HI_U32 u32DataSize)
{
    ...
    {
        HI_MEMSUBT_FREE((void*)pstParseInfo->stOutputData.pu8SCTESubtData);
    ...
    if (pstParseInfo->stOutputData.enBackgroundStyle == SCTE_SUBT_BACKGROUD_FRAMED)
    {
        ...
        if ((u32SubtArea > SCTE_SUBT_MAX_AREA) || (0 == u32SubtArea))
        {
        ...
        pstParseInfo->stOutputData.pu8SCTESubtData = (HI_U8*)HI_MEMSUBT_MALLOC(u32SubtDataLen);
...
HI_S32 SCTE_SUBT_Parse_Create(HI_VOID* hDisplay, HI_VOID** pphParse)
{
    ...
    pstParseInfo = (SCTE_SUBT_PARSE_INFO_S*)HI_MEMSUBT_MALLOC(sizeof(SCTE_SUBT_PARSE_INFO_S));
...
HI_S32 SCTE_SUBT_Parse_Destroy(HI_VOID* hParse)
{
    ...
    HI_MEMSUBT_FREE((void*)pstParseInfo);
