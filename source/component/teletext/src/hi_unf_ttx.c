...
static const HI_CHAR s_szTTXVersion[] __attribute__((used)) = "SDK_VERSION:["\
...
#define ENABLE_SWITCHPAGE_WHEN_SUBPAGEIS0
...
static HI_U8 s_au8Language[4] = {0};
...
static HI_S32  TTX_PageContext_Init(HI_VOID** phDataParse)
{
    ...
    TTX_Mem_Memset(pstContext, 0, sizeof(TTX_PAGE_CONTEXT_S));

    if (HI_FAILURE == TTX_Parse_SetPgAddr(&(pstContext->stCurPgAddr), TTX_BAD_INV_MAGZINE, TTX_BAD_INV_PAGE, 0))
    {
        goto ERROR;
    }
    ...
    if (HI_FAILURE == TTX_Parse_SetPgAddr(&(pstContext->stIndexPgAddr), TTX_BAD_INV_MAGZINE, 0, 0))
    {
        goto ERROR;
    }
    ...
    {
        if (HI_FAILURE == TTX_Parse_SetPgAddr(&(pstContext->astLink27[j]), TTX_BAD_INV_MAGZINE, TTX_BAD_INV_PAGE, 0x3f7f))
        {
            goto ERROR;
        }
    }
    ...
    for (i = 0; i < TTX_ROW_NUM; i++)
    {
        ...
        {
            pstContext->u32aPageText[i][j] = TTX_BAD_INV_PAGE;
        }
    }
    ...
    pstContext->bCancel = HI_FALSE;
    pstContext->bShowTime = HI_TRUE;
...
static HI_S32 TTX_PageContext_DeInit(TTX_PAGE_CONTEXT_S* pstContextHead)
{
    ...
    if (HI_NULL != pstContextHead->pFlaAreaHead)   /**release the flash area */
    {
        HI_INFO_TTX("Release   ");
        TTX_Show_Release_FlashArea(pstContextHead->pFlaAreaHead);
        pstContextHead->pFlaAreaHead = HI_NULL;
    }

    TTX_Mem_Free(pstContextHead);
    pstContextHead = HI_NULL;
...
static HI_S32  TTX_CheckPage(HI_HANDLE hTTX, HI_UNF_TTX_CHECK_PARAM_S* pstCheckParam)
{
    ...
    if (HI_SUCCESS == TTX_Data_GetValidPage(&stPageAddr, TTX_NEXT_SUBPAGE, HI_NULL))
    {
...
static HI_S32  TTX_RequestPage(HI_HANDLE hTTX, const HI_UNF_TTX_PAGE_ADDR_S stPageAddr)
{
    ...
    if (HI_SUCCESS
        == TTX_Data_GetValidPage(&stTtxRequest.stReqPgAddr, TTX_NEXT_SUBPAGE, HI_NULL))
    {
    ...
    pstCurrentPoint->u32ShowTimeout = 0;
...
static HI_S32  TTX_Handle_SubpageNumKey(HI_HANDLE hTTX, HI_UNF_TTX_KEY_E enKey)
{
    ...
    if (!TTX_IsLastFigureInputted(pstInputSubcode, TTX_DIGIT_NUM_OF_SUBCODE))
    {
        if (pstCurrentPoint->bCancel)
        {
            ...
            pstCurrentPoint->bCancel = HI_FALSE;
        }
    }
...
static HI_S32  TTX_Handle_PageNumKey(HI_HANDLE hTTX, HI_UNF_TTX_KEY_E enKey)
{
    HI_U8 u8MagazineNum = 0, u8PageNum = 0;
    TTX_PARAM_S* pstParam = HI_NULL;
    TTX_PAGE_CONTEXT_S*   pstCurrentPoint = NULL;
...
static HI_S32  TTX_Handle_DirectionKey(HI_HANDLE hTTX, HI_UNF_TTX_KEY_E enKey)
{
    ...
    switch (enKey)
    {
        case  HI_UNF_TTX_KEY_PREVIOUS_PAGE:
        {
            ...
            pstCurrentPoint->bSubpageMode = HI_FALSE;
            break;
        }
        ...
        case  HI_UNF_TTX_KEY_NEXT_PAGE:
        {
            ...
            pstCurrentPoint->bSubpageMode = HI_FALSE;
            HI_FATAL_TTX("Next_Page: Mag=%d, Page=%d\n",pstCurrentPoint->stWantPgAddr.u8MagazineNum,u8PageNum);
        ...
        case  HI_UNF_TTX_KEY_PREVIOUS_SUBPAGE:
        {
            ...
            {
                pstCurrentPoint->stWantPgAddr.u16PageSubCode = TTX_MAX_SUBPAGENO;
        ...
        case  HI_UNF_TTX_KEY_NEXT_SUBPAGE:
        {
            if (pstCurrentPoint->bAutoPlay)
            {
                pstCurrentPoint->stWantPgAddr.u16PageSubCode = 1;
            }
            else
            {
                pstCurrentPoint->stWantPgAddr.u16PageSubCode++;
    ...
    //pstCurrentPoint->bAutoPlay = HI_TRUE;
    stTtxRequest.stReqPgAddr = pstCurrentPoint->stWantPgAddr;

    if (HI_SUCCESS == TTX_Data_GetValidPage(&stTtxRequest.stReqPgAddr, enSeaDirection, &pstCurrentPoint->bAutoPlay))
    {
        //printf(" 1029: bAutoPlay = %d\n", pstCurrentPoint->bAutoPlay);
    ...
    {
        //printf(" 1045: bAutoPlay = %d\n", pstCurrentPoint->bAutoPlay);

        pstCurrentPoint->stWantPgAddr = pstCurrentPoint->stCurPgAddr;

        HI_INFO_TTX("No  such page!\n");
    }
    TTX_Recv_AutoPlay(pstParam->hDataRecv, pstCurrentPoint->bAutoPlay);
...
#ifdef ENABLE_SWITCHPAGE_WHEN_SUBPAGEIS0
    ...
    switch (enKey)
    {
        ...
        case  HI_UNF_TTX_KEY_GREEN:
        {
            ...
            if (pstCurrentPoint->bSubpageMode)
            {
                ...
                HI_INFO_TTX("HI_UNF_TTX_KEY_GREEN u16PageSubCode = %d!\n", pstCurrentPoint->stWantPgAddr.u16PageSubCode);
            }
            else
            {
                pstCurrentPoint->stWantPgAddr = pstCurrentPoint->astLink27[1];
        ...
        {
            /*if (pstCurrentPoint->astLink27[5].u8MagazineNum != TTX_BAD_INV_MAGZINE)
            {
                pstCurrentPoint->stWantPgAddr = pstCurrentPoint->astLink27[5];
            }
            else*/
            {
                if (pstCurrentPoint->stIndexPgAddr.u8MagazineNum != TTX_BAD_INV_MAGZINE)
                {
                    pstCurrentPoint->stWantPgAddr = pstCurrentPoint->stIndexPgAddr;
                }
                else
                {
                    if (HI_SUCCESS != TTX_Parse_SetPgAddr(&(pstCurrentPoint->stWantPgAddr), TTX_DEFAULT_INITMAGAZINE, TTX_DEFAULT_INITPAGE,
...
static HI_S32 TTX_Handle_ControlKey(HI_HANDLE hTTX, HI_UNF_TTX_KEY_E enKey)
{
    ...
    switch (enKey)
    {
     ...
     case  HI_UNF_TTX_KEY_CANCEL:
        {
            /* CANCEL */
            pstCurrentPoint->bCancel = pstCurrentPoint->bCancel ? HI_FALSE : HI_TRUE;
            if (HI_SUCCESS != TTX_RequestPage(hTTX, pstCurrentPoint->stCurPgAddr))
            {
                HI_WARN_TTX("TTX_RequestPage failure!\n");
            }
        ...
        case  HI_UNF_TTX_KEY_TIME:
        {
            /* Time */
            pstCurrentPoint->bShowTime = pstCurrentPoint->bShowTime ? HI_FALSE : HI_TRUE;

            HI_WARN_TTX("bSubtitle = %d, bBoxPage = %d \n", pstCurrentPoint->bSubtitle, pstCurrentPoint->bBoxPage);

            if ((!pstCurrentPoint->bSubtitle) && (!pstCurrentPoint->bBoxPage))
            {
                ...
                {
                    TTX_Parse_HideTime(pstCurrentPoint);
                }
            }
...
static HI_VOID TTX_Handle_NumKey(HI_HANDLE hTTX, TTX_PAGE_CONTEXT_S* pstCurrentPoint, HI_UNF_TTX_KEY_E enKey)
{
    if (pstCurrentPoint->bSubpageMode)
    {
    ...
    {
        if (HI_SUCCESS != TTX_Handle_PageNumKey(hTTX, enKey))
        {
            HI_WARN_TTX("TTX_Handle_PageNumKey failure!\n");
        }
    }
}

static HI_VOID TTX_Handle_NoneNumKey(HI_HANDLE hTTX, HI_UNF_TTX_KEY_E enKey)
{
    switch (enKey)
    {
...
static HI_VOID TTX_ExecKeyCmd(HI_HANDLE hTTX, TTX_PAGE_CONTEXT_S* pstCurrentPoint, HI_UNF_TTX_KEY_E enKey)
{
    if (enKey <= HI_UNF_TTX_KEY_9)
    {
        TTX_Handle_NumKey(hTTX, pstCurrentPoint, enKey);
    }
    else
    {
        TTX_Handle_NoneNumKey(hTTX, enKey);
    }
...
HI_S32 HI_UNF_TTX_Create(HI_UNF_TTX_INIT_PARA_S* pstInitParam, HI_HANDLE* phTTX)
{
    ...
    if (pstInitParam->enTtxLevel >= HI_UNF_TTX_LEVEL_BUTT)
    {
        HI_WARN_TTX("Level error \n");
        return HI_FAILURE;
    }
    ...
    pstCurrentPoint->enTtxLevel = pstInitParam->enTtxLevel;
...

    {
        ...
        {
            //set time color, default yellow
            pstCurrentPoint->au32TimeColor[i] = YELLOW_COLOR;
        }
    }

    TTX_Recv_SetHightLevel(pstParam->hDataRecv, pstInitParam->enTtxLevel);
...
HI_S32 HI_UNF_TTX_SwitchContent (HI_HANDLE hTTX, HI_UNF_TTX_CONTENT_PARA_S* pstContentParam)
{
    ...
    if (((pstContentParam->stInitPgAddr.u8MagazineNum > 8) && (TTX_BAD_INV_MAGZINE != pstContentParam->stInitPgAddr.u8MagazineNum))||
        ((pstContentParam->stInitPgAddr.u8PageNum > 99) && (TTX_BAD_INV_PAGE != pstContentParam->stInitPgAddr.u8PageNum)))
    {
        HI_ERR_TTX("invalid args! [%d, %d]\n", pstContentParam->stInitPgAddr.u8MagazineNum, pstContentParam->stInitPgAddr.u8PageNum);
        return HI_FAILURE;
    }

    if (TTX_BAD_INV_PAGE != pstContentParam->stInitPgAddr.u8PageNum)
    {
    ...
    HI_WARN_TTX("u32ISO639LanCode = %d \n", pstContentParam->u32ISO639LanCode);

    if (0 != pstContentParam->u32ISO639LanCode)
    {
    ...
    {
        pstCurrentPoint->u8Language[0] = s_au8Language[0];
        pstCurrentPoint->u8Language[1] = s_au8Language[1];
        pstCurrentPoint->u8Language[2] = s_au8Language[2];
        pstCurrentPoint->u8Language[3] = '\0';
    }

    if ((pstContentParam->stInitPgAddr.u8MagazineNum == TTX_BAD_INV_MAGZINE) ||
        (pstContentParam->stInitPgAddr.u8PageNum == TTX_BAD_INV_PAGE))
    {
        if (HI_SUCCESS
...
/*        if ((HI_UNF_TTX_TTXSUBT != pstContentParam->enType) || (HI_UNF_TTX_PACKET_TYPE_RAW != pstThisElem->enPacketType))
        {
            if (HI_SUCCESS
...
*/
    ...
    switch (pstContentParam->enType)
    {
        ...
        {
            if (HI_UNF_TTX_PACKET_TYPE_RAW != pstThisElem->enPacketType)
            {
            ...
            {
                TTX_Recv_GetSubtAddr(pstThisElem, &(pstCurrentPoint->stWantPgAddr));
            }
...
HI_S32 HI_UNF_TTX_Display(HI_HANDLE hTTX, HI_HANDLE hDispalyHandle)
{
    ...
    switch (enMsgAction)
    {
        ...
        case TTX_MSG_ACTION_INPUTNUMBER:
            ...
            {
                TTX_Show_DrawNavigationSubpageMode(pstCurrentPoint, TTX_LOP_COL_NUM, HI_TRUE);
            }
            else
            {
            ...
            break;
        case TTX_MSG_ACTION_HOLD_ON:
            TTX_Parse_HoldOn(pstCurrentPoint);
            break;
        ...
        case TTX_MSG_ACTION_UPDATESUBPAGE:
            TTX_Parse_UpdateSubpageNavigation(pstCurrentPoint);
            break;
...
HI_S32 HI_UNF_TTX_Output (HI_HANDLE hTTX, HI_UNF_TTX_OUTPUT_E enOutput, HI_BOOL bEnable)
{
    ...
    if (!bEnable)
    {
        pstCurrentPoint->bTtxMix = HI_FALSE;
    }
    return HI_SUCCESS;
}
        ...
        case HI_UNF_TTX_CMD_GETPAGEADDR:           /** Get page address */
        {
            ...
            switch (pstGetpageaddr->enPageType)
            {
                case HI_UNF_TTX_PAGE_CUR:                /**<Current displaying page */
                {
                    ...
                    if ((TTX_BAD_INV_MAGZINE == pstGetpageaddr->stPageAddr.u8MagazineNum) ||
                        (TTX_BAD_INV_PAGE == pstGetpageaddr->stPageAddr.u8PageNum))
                    {
                        s32Ret = HI_FAILURE;
                    }
                }
                ...
                case  HI_UNF_TTX_PAGE_INDEX:              /**<Initial Teletext page   */
                {
                    ...
                    if (HI_SUCCESS != TTX_Data_GetValidPage(&pstGetpageaddr->stPageAddr, TTX_INDEX_PAGE, HI_NULL))
                    {
            ...
            if (TTX_BAD_INV_PAGE != pstGetpageaddr->stPageAddr.u8PageNum)
            {
                TTX_HEX_TO_DEC(pstGetpageaddr->stPageAddr.u8PageNum);
            }
...
HI_S32 HI_UNF_TTX_ResetData(HI_HANDLE hTTX)
{
    ...
    if (HI_FAILURE == TTX_Recv_ResetData(pstParam->hDataRecv))
    {
        HI_WARN_TTX("TTX_Recv_ResetData failure!\n");
    }
...
{
    HI_WARN_TTX("enter:HI_UNF_TTX_SetLanguage \n");
    ...
    //save user language
    s_au8Language[0] = pu8Language[0];
    s_au8Language[1] = pu8Language[1];
    s_au8Language[2] = pu8Language[2];
    s_au8Language[3] = '\0';


    HI_WARN_TTX("s_au8Language[0] = %c \n", s_au8Language[0]);
    HI_WARN_TTX("s_au8Language[1] = %c \n", s_au8Language[1]);
    HI_WARN_TTX("s_au8Language[2] = %c \n", s_au8Language[2]);

    //if ttx is display, set user lange to current page.
    pstParam  = (TTX_PARAM_S*)TTX_GetParamAddr(hTTX);
    if (HI_NULL != pstParam)
    {
        pstContext = (TTX_PAGE_CONTEXT_S*)pstParam->hDataParse;
