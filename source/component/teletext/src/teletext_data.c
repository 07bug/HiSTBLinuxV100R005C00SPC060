...
HI_S32 TTX_Data_Search(HI_U8 u8MagNum, HI_U8 u8PageNum, HI_U16 u16SubpageNum, TTX_PAGE_S_PTR pstTmppage)
{
    ...
    if ((pstThispage->stPage.u16subcode == u16SubpageNum) || (0 == u16SubpageNum))
    {
...
static HI_S32 TTX_Data_GetNextSubPage(TTX_PAGE_STORE_S_PTR* ppstThisPage, TTX_PAGE_STORE_S_PTR pstFirstPage, HI_U16 u16SubpageNum)
{
...
static HI_S32 TTX_Data_GetPreSubPage(TTX_PAGE_STORE_S_PTR* ppstThisPage,
                                      ...
                                      HI_U16 u16SubpageNum)
{
    ...
    while ((*ppstThisPage != NULL) && (*ppstFirstPage != NULL))
    {
        ...
        {
            pstPrevPage = (*ppstThisPage);
            (*ppstThisPage) = (*ppstThisPage)->pstNext;
        }
    }

    /*the first -> the last*/
    if ((NULL == *ppstThisPage) && (NULL != *ppstFirstPage))
    {
        if (u16SubpageNum < (*ppstFirstPage)->stPage.u16subcode)
        {
            s32Ret = HI_FAILURE;
        }
    }

    (*ppstThisPage) = pstPrevPage;
...
HI_S32 TTX_Data_GetValidPage(HI_UNF_TTX_PAGE_ADDR_S* pstPageAddr,
                                    ...
                                    HI_VOID* pUserData)
{
    ...
    if (HI_NULL == pstPageAddr)
    {
        return HI_FAILURE;
    }

    if ((pstPageAddr->u8MagazineNum > 7) || /*(*pu8PageNum > 99) ||*/ (enSearchDir > TTX_INDEX_PAGE))
    {
        return HI_FAILURE;
    }

    u16TmpMagNum  = (HI_U16)(pstPageAddr->u8MagazineNum);
    u16TmpPageNum = (HI_U16)(pstPageAddr->u8PageNum);
    ...
    if ((enSearchDir <= TTX_NEXT5_PAGE) && (enSearchDir >= TTX_NEXT_PAGE))
    {
        ...
        for (i = 0; i < TTX_MAX_PAGESTORE; i++)
        {
            ...
            if (u8NextNum == ((enSearchDir + 1) - TTX_NEXT_PAGE))
            {
                ...
                (pstPageAddr->u8MagazineNum)  = (HI_U8)(u16Page / 256);
                (pstPageAddr->u8PageNum) = u16Page % 256;
                (pstPageAddr->u16PageSubCode) = (HI_U16)pstThisPage->stPage.u16subcode;
    ...
    switch (enSearchDir)
    {
        ...
        {
            if (HI_SUCCESS == TTX_Data_GetNextSubPage(&pstThisPage, pstFirstPage, (pstPageAddr->u16PageSubCode)))
            {
                if (HI_NULL != pUserData)
                {
                    *((HI_BOOL *)pUserData) = HI_FALSE;
                }
            }
            else
            {
        ...
        {
            if (HI_SUCCESS == TTX_Data_GetPreSubPage(&pstThisPage, &pstFirstPage, (pstPageAddr->u16PageSubCode)))
            {
                if (HI_NULL != pUserData)
                {
                    *((HI_BOOL *)pUserData) = HI_FALSE;
        ...
        case TTX_THIS_PAGE:
        {
            TTX_Data_GetThisPage(&pstThisPage, pstPageAddr->u16PageSubCode);
        }
        break;
        ...
        case TTX_INDEX_PAGE:
        {
            ...
            {
                (pstPageAddr->u16PageSubCode) = s_pstDataStore->pstInitpageStore->stPage.u16subcode;
                TTX_DATA_UNLOCK();
                return HI_SUCCESS;
            }
            else
    ...
    {
        (pstPageAddr->u8MagazineNum)  = (HI_U8)(u16Page / 256);
        (pstPageAddr->u8PageNum) = (HI_U8)(u16Page % 256);
