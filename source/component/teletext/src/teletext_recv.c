...
#define TTX_MAX_SUBT_NUM  (16)

static HI_UNF_TTX_PAGE_ADDR_S g_astSubtAddr[TTX_MAX_SUBT_NUM];
static HI_U8 s_u8SubtIndex = 0;
static HI_U8 s_u8SubtNum = 0;
...
static HI_U32 TTX_RECV_GetPageAddr_Dec(HI_UNF_TTX_PAGE_ADDR_S stSubtPage)
{
    TTX_HEX_TO_DEC(stSubtPage.u8MagazineNum);
    TTX_HEX_TO_DEC(stSubtPage.u8PageNum);
    TTX_HEX_TO_DEC(stSubtPage.u16PageSubCode);

    return stSubtPage.u8MagazineNum + stSubtPage.u8PageNum + stSubtPage.u16PageSubCode;
}


static HI_VOID TTX_RECV_SortSubtAddr(HI_UNF_TTX_PAGE_ADDR_S* pPageArray)
{
    HI_U8 i = 0;
    HI_U8 j = 0;

    HI_UNF_TTX_PAGE_ADDR_S stTempPage;
    HI_U32 u32PageAddrDecI = 0;
    HI_U32 u32PageAddrDecJ = 0;

    for (i = 1; i < s_u8SubtNum; i++)
    {
        stTempPage = pPageArray[i];

        u32PageAddrDecI = TTX_RECV_GetPageAddr_Dec(stTempPage);
        u32PageAddrDecJ = TTX_RECV_GetPageAddr_Dec(pPageArray[j - 1]);

        for (j = i; j > 0 && u32PageAddrDecJ > u32PageAddrDecI; j--)
        {
            pPageArray[j] = pPageArray[j - 1];
        }

        pPageArray[j] = stTempPage;
    }

    return;
}

static HI_VOID TTX_RECV_SaveSubtAddr(HI_U8 u8Magazinenum, HI_U8 u8TmpPageNo,HI_U16 u16SubPageN)
{
    if (s_u8SubtNum >= TTX_MAX_SUBT_NUM)
    {
        return;
    }

    g_astSubtAddr[s_u8SubtNum].u8MagazineNum = u8Magazinenum;
    g_astSubtAddr[s_u8SubtNum].u8PageNum = u8TmpPageNo;
    g_astSubtAddr[s_u8SubtNum].u16PageSubCode = u16SubPageN;

    s_u8SubtNum++;

    TTX_RECV_SortSubtAddr(g_astSubtAddr);
}
    ...
    if (pstThisElem->pstFiltersPtr->u32OpenOff != TTX_REQ_UNUSED)
    {
                 ...
                 && (pstpage->u32PTS == 0) && (pstpage->u32ValidLines > 1))
        {
            ...
            else
            {
                ...
                //printf("[%s, %d]: stFilter->u16subcode = %d\n", __FUNCTION__,__LINE__, pstThisElem->pstFiltersPtr->stFilter.stReqPgAddr.u16PageSubCode);
...
static HI_VOID TTX_Recv_ReOrganisePage(HI_U8 u8MagazineNo, HI_U8 u8PageNo, TTX_PAGE_S* pstCurPage)
{
    HI_U8 i = 0;
    ...
    if (HI_SUCCESS != TTX_Data_FindPage(u8MagazineNo, u8PageNo, pstCurPage->u16subcode, &stPrePage))
    {
        return;
    }

    if (TTX_IsEraseFlagSet(pstCurPage))
    {
        return;
    }

    //HI_ERR_TTX("cur u32ValidLines = 0x%08x", pstCurPage->u32ValidLines);
    //HI_ERR_TTX("Pre u32ValidLines = 0x%08x", stPrePage.u32ValidLines);

    if (pstCurPage->u32ValidLines != stPrePage.u32ValidLines)
    {
        for (i = 1; i <= 25; i++)
        {
            if ((0 == (pstCurPage->u32ValidLines & (1 << i))) &&
                (0 != (stPrePage.u32ValidLines &  (1 << i))))
            {
                memcpy(pstCurPage->u8szLines[i], stPrePage.u8szLines[i],TTX_LINE_MEM_SIZE);
                pstCurPage->u32ValidLines |= 1 << i;
            }
        }
    }
    ...
    //HI_ERR_TTX("cur u32ExtenValidLineL = 0x%08x", pstCurPage->u32ExtenValidLineL);
    //HI_ERR_TTX("Pre u32ExtenValidLineL = 0x%08x", stPrePage.u32ExtenValidLineL);

    if (pstCurPage->u32ExtenValidLineL != stPrePage.u32ExtenValidLineL)
    {
        for (i = 0; i < 32; i++)
        {
            if ((0 == (pstCurPage->u32ExtenValidLineL & (1 << i))) &&
                (0 != (stPrePage.u32ExtenValidLineL &  (1 << i))))
            {
                memcpy(pstCurPage->u8szExtenLines[i], stPrePage.u8szExtenLines[i], TTX_LINE_MEM_SIZE);

                pstCurPage->u32ExtenValidLineL |= (1 << i);
            }
        }
    }
...
static HI_S32 TTX_Recv_ProcPreviousPage(TTX_CONTEXT_S_PTR pstThisElem, TTX_PAGE_S_PTR pstPage)
{
    ...
    if (pstPage->u32ValidLines & 0x01)
    {
        ...
        {
            TTX_RECV_SaveSubtAddr(u8Magazinenum, u8TmpPageNo, (HI_U16)u32SubPageN);
        }

        pstPage->u16subcode = (HI_U16)u32SubPageN;

        if ((1 == u8Magazinenum) && (u8TmpPageNo < 0x10))
        {
            HI_INFO_TTX("NEW_TTX: M = %x, P = %02x, S = %02x\n", u8Magazinenum,u8TmpPageNo, u32SubPageN);
        }

        if (0 == pstPage->u32PTS)
        {
            TTX_Recv_ReOrganisePage(u8Magazinenum, u8TmpPageNo, pstPage);
        }
...
static HI_S32 TTX_Recv_ParseRAWPacket(HI_VOID* hData, HI_U8* pu8DataSrc, HI_U32 u32PacketLength)
{
    ...
    while (u32ExtractPoint < u32PacketLength)    /* loop on line data */
    {
        ...
        {
            HI_INFO_TTX("[%2X, %2X] : u8PcktAddr1 = %d, u8PcktAddr2 = %d, TTX_BAD_INV_HAMMING, continue",
        ...
        if (0 == u8PacketNo)
        {
            if (s_u8PreviousIndex < 9)
            {
                ...
                //For parallel mode transmission, refresh this magazine of cached data.
                if (0 == g_s8serial)
                {
                    if (u8Magazine != s_u8PreviousIndex)
                    {
...
HI_S32 TTX_Recv_Create(HI_VOID** pphDataRecv)
{
    ...
    memset(g_astSubtAddr, 0x0, sizeof(g_astSubtAddr));
...
HI_S32 TTX_Recv_ResetData(HI_VOID* hDataRecv)
{
    TTX_CONTEXT_S_PTR pstThisElem = NULL;
...
HI_S32 TTX_Recv_SetHightLevel(HI_VOID* hDataRecv, HI_UNF_TTX_LEVEL_E enTtxLevel)
{
    TTX_CONTEXT_S_PTR pstThisElem = HI_NULL;
    ...
    pstThisElem->enTtxLevel = enTtxLevel;
...
HI_S32 TTX_Recv_GetSubtAddr(HI_VOID* hDataRecv, HI_UNF_TTX_PAGE_ADDR_S*  pstSubtPgAddr)
{
    TTX_CONTEXT_S_PTR pstThisElem = HI_NULL;
    ...
    if (s_u8SubtNum > 0)
    {
        if (s_u8SubtIndex >= s_u8SubtNum)
        {
            s_u8SubtIndex = 0;
        }

        *pstSubtPgAddr = g_astSubtAddr[s_u8SubtIndex++];
