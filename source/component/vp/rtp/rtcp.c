    ...
    for (i = 0; SOURCE_COUNT > i; ++i)
    {
        ...
        while (HI_NULL != pstSource)
        {
            ...
            if (pstSource->bIsSender)
            {
                ...
                if (ulExpected < pstSource->ulRTPPackets)
                {
                    lTotalLost = 0;
                    break;
                }
...
VP_RTP_RESULT VP_RTCP_RecvCalculateDelayAndJitter (VP_RTCP_SESSION* pstSession, HI_U32 u32Ssrc, HI_U32 u32PtsMs)
{
    ...
    {
        s32Delay = u32Time - pstSource->ulTimeOffest - (u32PtsMs / pstSource->ulTimeRatio);
        s32Delay = (s32Delay > 0) ? s32Delay : 0;
        pstSource->ulRecvDelay = RTCP_UTILS_MeanDeviation (s32Delay, pstSource->ulRecvDelay);
    ...
    if ( (0 != s32Delay) && (0 != u32Time))
    {
        u32Jitter = ((HI_U32)s32Delay > u32Time) ? ((HI_U32)s32Delay - u32Time) : (u32Time - (HI_U32)s32Delay);
    ...
    HI_DBG_RTP("Recv<-Delay:%2d~%2d,Jitter:%2d~%2d\n", s32Delay, (pstSource->ulRecvDelay / RTCP_MEAN_DEVIATION_COUNT), u32Jitter, (pstSource->ulRecvJitter / RTCP_MEAN_DEVIATION_COUNT));    /*libo@201312*/
...
VP_RTP_RESULT VP_RTCP_SendCalculateDelayAndJitter (VP_RTCP_SESSION* pstSession, HI_U32 u32PtsMs)
{
    ...
    for (i = 0; SOURCE_COUNT > i; ++i)
    {
        ...
        if ((HI_NULL != pstSource) && (pstSource->bIsRemoteRecv))
        {
            ...
            {
                s32Delay = u32Time - u32PtsMs + pstSource->ulSendNetDelay;
                s32Delay = (s32Delay > 0) ? s32Delay : 0;
                pstSource->ulSendDelay = RTCP_UTILS_MeanDeviation (s32Delay, pstSource->ulSendDelay);
            ...
            HI_DBG_RTP("Send->Delay:%2d~%2d,Jitter:%2d~%2d\n", s32Delay, (pstSource->ulSendDelay / RTCP_MEAN_DEVIATION_COUNT), u32Jitter, (pstSource->ulSendJitter / RTCP_MEAN_DEVIATION_COUNT));     /*libo@201312*/
        }
    }
