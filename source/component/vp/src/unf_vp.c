...
int VP_TIME(HI_U32 flag, char *str)
{
    ...
    if(flag == 0)
    {
        (void)gettimeofday( &g_start, NULL );
    }
    else if(flag == 1)
    {
        (void)gettimeofday( &g_end, NULL );
        tv_sec = (g_end.tv_usec >= g_start.tv_usec) ? (g_end.tv_sec - g_start.tv_sec) : (g_end.tv_sec - g_start.tv_sec - 1);
        tv_usec = (g_end.tv_usec >= g_start.tv_usec) ? (g_end.tv_usec - g_start.tv_usec) : (g_end.tv_usec + 1000000 - g_start.tv_usec);
        HI_DBG_VP("%s, start: %d.%06ds, end: %d.%06ds, diff: %d.%06ds\n",
            str, g_start.tv_sec, g_start.tv_usec, g_end.tv_sec, g_end.tv_usec, tv_sec, tv_usec);
    }
#endif
    return 0;
}

int  VP_DUMP(char* str, const unsigned char *buf, unsigned int len)
{
#ifdef DATA_DEBUG
    const unsigned int lwidth = 188*2+2;
    char strBuf[lwidth+3];
    char *p;
    unsigned int i;

    p = strBuf;
    snprintf(p, 188, "[%s]:", str);
    p += (strlen(str) +2);
    if (buf == NULL) {
        snprintf(p, 188, "*NULL*\n");
    } else if (len < (lwidth/2)) {
        for(i=0; i<len; i++, p+=2)
        ...
        *p = 0; /* end string with null char */
        ...
        for(i=0; i<(lwidth/4)-1; i++, p+=2)
            snprintf(p, 3, "%02x", buf[i]);
        snprintf(p, 6, " ... ");
        p+=5;
        for(i=len-(lwidth/4)+1; i<len; i++, p+=2)
            snprintf(p, 3, "%02x", buf[i]);
        *p = '\n';
        p++;
        *p = 0; /* end string with null char */
...
static HI_S32 VP_CheckAttr(const HI_UNF_VP_ATTR_S* pstAttr)
{
    ...
    {
        HI_ERR_VP("Only support H.264 or H.265!\n");
        return HI_ERR_VP_NOT_SUPPORT;
    }
...
static HI_VOID* VP_ScheduleCaptureYUV(HI_VOID* args)
{
    ...
    while (!g_stVPAttr[u32Chn].bUSBCamStop)
    {
        ...
        g_stVPAttr[u32Chn].stUsbCamCtrl.u32CaptureFrmCnt ++;
...
static HI_S32 VP_MuxerCreate(HI_U32 index)
{
    HI_S32 s32Ret;
    Transcoder_MuxerAttr muxer_Attr;
    Transcoder_MuxerType muxerType;
    HI_CHAR*   pstMuxerBuffer = NULL;

    CHECK_VP_INIT;

    if (MAX_VP_CHANNEL_NUM <= index)
    {
        HI_ERR_VP("Create VP failed, max number reached!\n");
        return HI_ERR_VP_NOT_SUPPORT;
    }

    muxer_Attr.audioCodec = TRANSCODER_CODEC_ID_AACLC;
    if(g_stVPAttr[index].stVencAttr.enVencType == HI_UNF_VCODEC_TYPE_H264)
    {
        muxer_Attr.videoCodec=TRANSCODER_CODEC_ID_H264;
    }
    else if(g_stVPAttr[index].stVencAttr.enVencType == HI_UNF_VCODEC_TYPE_HEVC)
    {
        muxer_Attr.videoCodec=TRANSCODER_CODEC_ID_H265;
    }
    else
    {
        HI_ERR_VP("HI_Muxer_Create not support\n");
        return HI_FAILURE;
    }

    muxerType = TRANSCODER_MUTEX_TS;
    s32Ret = HI_Muxer_Create(&g_stVPAttr[index].hMuxerHandle, muxerType,&muxer_Attr);
    if(HI_SUCCESS != s32Ret)
    {
        HI_ERR_VP("HI_Muxer_Create error, s32Ret = %d!!!\n",s32Ret);
        return HI_FAILURE;
    }
    pstMuxerBuffer = (HI_CHAR*)malloc(512 * 1024);
    if(HI_NULL == pstMuxerBuffer)
    {
        HI_ERR_VP("malloc failure\n");
        goto ERR0;
    }
    memset(pstMuxerBuffer, 0x00, (512 * 1024));
    g_stVPAttr[index].pstMuxerBuffer = pstMuxerBuffer;

    return HI_SUCCESS;
ERR0:
    HI_Muxer_Destroy(g_stVPAttr[index].hMuxerHandle);
    return HI_FAILURE;
}

static HI_S32 VP_MuxerDestory(HI_U32 index)
{
    ...
    if(g_stVPAttr[i].pLocalFs)
    {
        fclose( g_stVPAttr[i].pLocalFs);
        g_stVPAttr[i].pLocalFs = HI_NULL;
    }

    if(g_stVPAttr[i].pstMuxerBuffer)
    {
        free(g_stVPAttr[i].pstMuxerBuffer);
        g_stVPAttr[i].pstMuxerBuffer = NULL;
    }

    if(g_stVPAttr[i].hMuxerHandle !=  HI_INVALID_HANDLE &&
        g_stVPAttr[i].hMuxerHandle !=  0)
    {
        s32Ret = HI_Muxer_Destroy(g_stVPAttr[i].hMuxerHandle);
        g_stVPAttr[i].hMuxerHandle = HI_INVALID_HANDLE;
    }

    return s32Ret;
}

static HI_BOOL VP_isIframe(HI_U32 u32Chn, HI_UNF_VENC_STREAM_S * pstStream)
{
    CHECK_VP_NULL_PTR(pstStream);
    if(MAX_VP_CHANNEL_NUM <= u32Chn)
    {
        HI_ERR_VP("Parameter check failed, u32Chn = 0x%08x\n", u32Chn);
        return HI_FALSE;
    }

    if(HI_UNF_VCODEC_TYPE_H264 == g_stVPAttr[u32Chn].stVencAttr.enVencType &&
        pstStream->enDataType.enH264EType == HI_UNF_H264E_NALU_SPS)
    {
        HI_INFO_VP("VP_isIframe H264 =%d\n", pstStream->enDataType.enH264EType);
        return HI_TRUE;
    }
    else if(HI_UNF_VCODEC_TYPE_HEVC == g_stVPAttr[u32Chn].stVencAttr.enVencType &&
        pstStream->enDataType.enHEVCEType == HI_UNF_HEVCE_NALU_SPS)
    {
        HI_INFO_VP("VP_isIframe H265 =%d\n", pstStream->enDataType.enHEVCEType);
        return HI_TRUE;
    }
    return HI_FALSE;
}

static HI_S32 VP_MuxerRecord(HI_U32 u32Chn, HI_UNF_VENC_STREAM_S * pstStream)
{
    HI_S32 s32Ret = HI_FAILURE;
    HI_S32 s32MuxerLen = 512 * 1024;
    TRANSCODER_STRAM stTranStream;
    HI_CHAR  directory[PATH_MAX] = {0};
    ...
    memset(&stTranStream, 0, sizeof(TRANSCODER_STRAM));
    if(HI_UNF_VCODEC_TYPE_H264 == g_stVPAttr[u32Chn].stVencAttr.enVencType)
    {
        switch (pstStream->enDataType.enH264EType)
        {
            case HI_UNF_H264E_NALU_PSLICE:
                stTranStream.ePayloadType = PAYLOAD_VIDEO_NORMAL_FRAME;
                break;

            case HI_UNF_H264E_NALU_ISLICE:
                stTranStream.ePayloadType = PAYLOAD_VIDEO_KEY_FRAME;
                break;

            case HI_UNF_H264E_NALU_SPS:
                stTranStream.ePayloadType = PAYLOAD_VIDEO_SPS_FRAME;
                break;

            case HI_UNF_H264E_NALU_PPS:
                stTranStream.ePayloadType = PAYLOAD_VIDEO_PPS_FRAME;
                break;
            default:
                HI_ERR_VP("Unknown H264 payload type.\n");
                return HI_FAILURE;
        }
    }
    else if(HI_UNF_VCODEC_TYPE_HEVC == g_stVPAttr[u32Chn].stVencAttr.enVencType)
    {
        switch (pstStream->enDataType.enHEVCEType)
        {
            ...
            case HI_UNF_HEVCE_NALU_VPS:
                stTranStream.ePayloadType = PAYLOAD_VIDEO_VPS_FRAME;
                break;
            ...
            case HI_UNF_HEVCE_NALU_PPS:
                ...
                break;
            default:
                HI_ERR_VP("Unknown H265 payload type.\n");
                return HI_FAILURE;
        }
    }
    VP_DUMP("venc out:", pstStream->pu8Addr, 64);
    stTranStream.enDataStream.stVencStream.pu8Addr= pstStream->pu8Addr;
    stTranStream.enDataStream.stVencStream.u32SlcLen = pstStream->u32SlcLen;
    stTranStream.enDataStream.stVencStream.u32PtsMs = pstStream->u32PtsMs;
    if((HI_INVALID_HANDLE != g_stVPAttr[u32Chn].hMuxerHandle) && (0 != g_stVPAttr[u32Chn].hMuxerHandle))
    {
        HI_VP_LOCK();
        s32Ret = HI_Muxer_DoMuxer(g_stVPAttr[u32Chn].hMuxerHandle,g_stVPAttr[u32Chn].pstMuxerBuffer,&s32MuxerLen,&stTranStream);
        if(HI_SUCCESS != s32Ret)
        {
            HI_ERR_VP("HI_Muxer_DoMuxer error!!\n");
        }
        VP_DUMP("DoMuxer:", (const unsigned char *)g_stVPAttr[u32Chn].pstMuxerBuffer, 64);

        if(HI_NULL == g_stVPAttr[u32Chn].pLocalFs)
        {
            if(HI_NULL == realpath((const char*)g_stVPAttr[u32Chn].u8Directory, directory))
            {
                HI_INFO_VP("realpath file(%s)failed..[%d:%s]\n", (const char*)g_stVPAttr[u32Chn].u8Directory,  errno, strerror(errno));
                //return HI_FAILURE;
            }
            g_stVPAttr[u32Chn].pLocalFs = fopen((const char*)directory,"wb");
            HI_INFO_VP("fopen record ts file:%s\n", directory);
        }
        if(HI_NULL != g_stVPAttr[u32Chn].pLocalFs)
        {
            (HI_VOID)fwrite(g_stVPAttr[u32Chn].pstMuxerBuffer,1,(HI_U32)s32MuxerLen, g_stVPAttr[u32Chn].pLocalFs);
            fflush(g_stVPAttr[u32Chn].pLocalFs);
        }
        HI_VP_UNLOCK();
    }
    return HI_SUCCESS;
...
static HI_VOID* VP_ScheduleRtpSend(HI_VOID* args)
{
    ...
    while (!g_stVPAttr[u32Chn].bNetSendStop)
    {
        if (HI_FALSE == g_stVPAttr[u32Chn].bSendEmptyPackage)
        {
            ...
            if(stStream.bFrameEnd == HI_TRUE)
                g_stVPAttr[u32Chn].u32EncoderOutFrmCnt ++;
            g_stVPAttr[u32Chn].u32EncoderOutBytes += stStream.u32SlcLen;
            ...
            if(g_stVPAttr[u32Chn].bRecordBegin == HI_TRUE && g_stVPAttr[u32Chn].eRecordType == HI_UNF_VP_RECORD_TS)
            {
                if(HI_FALSE == bGetIFrame)
                {
                    bGetIFrame = VP_isIframe(u32Chn, &stStream);
                }
                if(HI_FALSE == bGetIFrame)
                {
                    HI_UNF_VENC_RequestIFrame(g_stVPAttr[u32Chn].hVENC);
                }
                else
                {
                    VP_MuxerRecord(u32Chn, &stStream);
                    VP_TIME(1, "ES TO TS");
                }
            }else{
                bGetIFrame = HI_FALSE;
                if(g_stVPAttr[u32Chn].pLocalFs)
                {
                    HI_INFO_VP("fclose record ts file:%s\n", g_stVPAttr[u32Chn].u8Directory);
                    fclose( g_stVPAttr[u32Chn].pLocalFs);
                    g_stVPAttr[u32Chn].pLocalFs = HI_NULL;
                }
            }
...
static HI_VOID* VP_ScheduleRtpRecv(HI_VOID* args)
{
    ...
    {
        s32Ret = RTP_AcquireES(g_stVPAttr[u32Chn].hRTP, &u32BufAddr, &u32Len, &u32PtsMs, &bFirstFU);
        if (HI_SUCCESS != s32Ret)
        {
            vp_usleep(10 * 1000);
            continue;
        ...
        if(bFirstFU)
            g_stVPAttr[u32Chn].u32DecoderInFrmCnt ++;
        g_stVPAttr[u32Chn].u32DecoderInBytes += u32Len;
...
static HI_S32 VPShowProc(HI_PROC_SHOW_BUFFER_S* pstBuf, HI_VOID* pPrivData)
{
    ...
    HI_S8 pVpStats[4][8] = {"YES", "NO", "Run", "Stop"};
    HI_S8 pVpSrcMode[3][8] = { "USBCAM", "VI", "BUTT"};
    HI_S8 pVpUSBMode[3][8] = { "YUYV", "MJPEG", "BUTT"};
    HI_S8 pVpStreamType[HI_UNF_VCODEC_TYPE_BUTT][32] = {"MPEG2", "MPEG4 DIVX4 DIVX5", "AVS", "H263", "H264",
...
HI_S32 HI_UNF_VP_Init(HI_VOID)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_VP("HI_Muxer_Init error, s32Ret = %d!!\n",ret);
    }
...
HI_S32 HI_UNF_VP_GetDefaultAttr(HI_UNF_VP_ATTR_S* pstDefAttr)
{
    ...
    pstDefAttr->stVencAttr.u32DriftRateThr = 20;
...
HI_S32 HI_UNF_VP_Create(const HI_UNF_VP_ATTR_S* pstAttr, HI_HANDLE* phVP)
{
    ...
    *phVP = (HI_ID_VP << 16) | i;
    ...
    s32Ret = VP_MuxerCreate(i);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VP("VP_MuxerInit failed, ret = 0x%08x\n", s32Ret);
        goto ERR3;
    }
    return HI_SUCCESS;

ERR3:
    (HI_VOID)HI_UNF_VENC_Destroy(g_stVPAttr[i].hVENC);
    g_stVPAttr[i].hVENC = HI_INVALID_HANDLE;
ERR2:
    (HI_VOID)HI_UNF_AVPLAY_ChnClose(g_stVPAttr[i].hAVPLAY, HI_UNF_AVPLAY_MEDIA_CHAN_VID);
ERR1:
    (HI_VOID)HI_UNF_AVPLAY_Destroy(g_stVPAttr[i].hAVPLAY);
...
HI_S32 HI_UNF_VP_Destroy(HI_HANDLE hVP)
{
    ...
    s32Ret = VP_MuxerDestory(i);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VP("VP_MuxerDeinit failed, ret = 0x%08x\n", s32Ret);
    }
...
HI_S32 HI_UNF_VP_SetAttr(HI_HANDLE hVP, const HI_UNF_VP_ATTR_S* pstAttr)
{
    ...
    g_stVPAttr[i].bIsIpv4   = pstAttr->bIsIpv4;
   // g_stVPAttr[i].bRecordBegin = pstAttr->bTsRecord;
    g_stVPAttr[i].enDecType = pstAttr->enDecType;
    //memcpy(&g_stVPAttr[i].u8TsDirectory, &pstAttr->u8TsDirectory, 256);
    memcpy(&g_stVPAttr[i].stVencAttr, &pstAttr->stVencAttr, sizeof(HI_UNF_VENC_CHN_ATTR_S));
    memcpy(&g_stVPAttr[i].stLocalNetAttr, &pstAttr->stLocalNetAttr, sizeof(HI_UNF_VP_NET_ATTR_U));
    memcpy(&g_stVPAttr[i].stRemoteNetAttr, &pstAttr->stRemoteNetAttr, sizeof(HI_UNF_VP_NET_ATTR_U));
                                 ...
                                 HI_UNF_VP_RECORD_FORMAT_E enRecordFmt, HI_CHAR * ucFullName)
{
    ...
    if(enRecordFmt >= HI_UNF_VP_RECORD_BUTT)
    {
        HI_ERR_VP("Unkown record format.\n");
        return HI_ERR_VP_NOT_SUPPORT;
    }

    if(enRecordType != HI_UNF_VP_RMT_MODE_SEND)
    {
        HI_ERR_VP("Unkown record type.\n");
        return HI_ERR_VP_NOT_SUPPORT;
    }

    if(strlen(ucFullName) >= 256)
    {
        HI_ERR_VP("invalid length.\n");
        return HI_ERR_VP_INVALID_PARA;
    }

    if(g_stVPAttr[i].bRecordBegin == HI_TRUE)
    {
        HI_ERR_VP("Recording stream, please stop current record process, then try again.\n");
        return HI_ERR_VP_NOT_SUPPORT;
    }
    HI_VP_LOCK();
    g_stVPAttr[i].eRecordType = enRecordFmt;
    memcpy(&g_stVPAttr[i].u8Directory, ucFullName, 256);
    g_stVPAttr[i].bRecordBegin = HI_TRUE;
    HI_VP_UNLOCK();
    return HI_SUCCESS;
...
HI_S32 HI_UNF_VP_StopRecord(HI_HANDLE hVP)
{
    ...
    if(g_stVPAttr[i].bRecordBegin == HI_FALSE)
    {
        HI_ERR_VP("NO recording process, please cheak again.\n");
        return HI_ERR_VP_NOT_SUPPORT;
    }
    HI_VP_LOCK();
    g_stVPAttr[i].bRecordBegin = HI_FALSE;
    g_stVPAttr[i].eRecordType = HI_UNF_VP_RECORD_BUTT;
    memset(&g_stVPAttr[i].u8Directory, 0, 256);
    HI_VP_UNLOCK();
    return HI_SUCCESS;
...
HI_S32 HI_UNF_VP_GetVideoStatics(HI_HANDLE hVP, HI_UNF_VP_VIDEO_STATICS_S *pstVideoStatics)
{
    ...
    pstVideoStatics->u32CaptureFrmCnt = g_stVPAttr[i].stUsbCamCtrl.u32CaptureFrmCnt;
    pstVideoStatics->u32EncoderOutFrmCnt = g_stVPAttr[i].u32EncoderOutFrmCnt;
    pstVideoStatics->u32EncoderOutBytes = g_stVPAttr[i].u32EncoderOutBytes;
    pstVideoStatics->u32DecoderInFrmCnt = g_stVPAttr[i].u32DecoderInFrmCnt;
    pstVideoStatics->u32DecoderInBytes = g_stVPAttr[i].u32DecoderInBytes;
    return HI_SUCCESS;
}
