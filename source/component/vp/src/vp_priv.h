...
#define HI_FATAL_VP(fmt, args...) printf("[VP FATAL]%s,%d: " fmt, __FUNCTION__, __LINE__, ## args);
#define HI_ERR_VP(fmt, args...) printf("[VP ERR]%s,%d: " fmt, __FUNCTION__, __LINE__, ## args);
#define HI_WARN_VP(fmt, args...) printf("[VP WARN]%s,%d: " fmt, __FUNCTION__, __LINE__, ## args);
#define HI_INFO_VP(fmt, args...) printf("[VP INFO]%s,%d: " fmt, __FUNCTION__, __LINE__, ## args);
#define HI_DBG_VP(fmt, args...) printf("[VP DBG]%s,%d: " fmt, __FUNCTION__, __LINE__, ## args);
#else
...
typedef struct hiVP_USBCAM_CTRL_S
{
    HI_U32       u32CaptureFrmCnt;
} VP_USBCAM_CTRL_S;
...
typedef struct hiVP_ATTR_S
{
    ...
    HI_U32           u32EncoderOutFrmCnt; /**<Total encoder output frame number*/
    HI_U32           u32EncoderOutBytes;  /**<Total encoder output bytes*/
    HI_U32           u32DecoderInFrmCnt;  /**<Total decoder input frame number*/
    HI_U32           u32DecoderInBytes;  /**<Total decoder input bytes*/
} VP_ATTR_S;
