...
int proc_get_tx_amsdu_rate_24g(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);

	if (padapter)
		RTW_PRINT_SEL(m, "tx amsdu rate 2.4G = %d Mbps\n", padapter->tx_amsdu_rate_24g);

	return 0;
}

int proc_get_tx_amsdu_rate_5g(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);

	if (padapter)
		RTW_PRINT_SEL(m, "tx amsdu rate 5G = %d Mbps\n", padapter->tx_amsdu_rate_5g);

	return 0;
}

ssize_t proc_set_tx_amsdu_rate_24g(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	char tmp[32];
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		int num = sscanf(tmp, "%d ", &amsdu_rate_24g);

		if (padapter && (num == 1)) {
			RTW_INFO("padapter->tx_amsdu_rate_24g = %x\n", amsdu_rate_24g);
			padapter->tx_amsdu_rate_24g = amsdu_rate_24g;
		}
	}
...
ssize_t proc_set_tx_amsdu_rate_5g(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	char tmp[32];
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		int num = sscanf(tmp, "%d ", &amsdu_rate_5g);

		if (padapter && (num == 1)) {
			RTW_INFO("padapter->tx_amsdu_rate_5g = %x\n", amsdu_rate_5g);
			padapter->tx_amsdu_rate_5g = amsdu_rate_5g;
		}
	}
