...
//96MV200
static int gpio_wl_reg_on = 5*8 + 6; // WL_REG_ON is input pin of WLAN module
#ifdef CUSTOMER_OOB
static int gpio_wl_host_wake = 5*8 + 1; // WL_HOST_WAKE is output pin of WLAN module
#endif
...
//MV200
#define BCM_REG_ON_GPIO     (7*8 + 2) //modify by hcm (7*8 + 3)     //BT_REG_ON gpio
#define BCM_OOB_INT_GPIO    (4*8 + 6) //(4*8 + 2)     //WL_HOST_WAKE gpio
//#endif
...
int dhd_wlan_init_gpio(void)
{
	...
	*/
	//gpio_wl_reg_on = 7*8 + 2;
#ifdef CUSTOMER_OOB
	//gpio_wl_host_wake = 4*8 + 6;
#endif
