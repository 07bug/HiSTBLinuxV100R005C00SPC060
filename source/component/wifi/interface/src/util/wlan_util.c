...
static const HI_CHAR PCI_DIR[] = "/sys/bus/pci/devices";
static const HI_CHAR MODULE_FILE[] = "/proc/modules";

HI_S32 wlan_util_insmod_module(HI_CHAR *module, HI_CHAR *module_tag, HI_CHAR *param);
...
static hi_wifi_device_info dev[] = {
    ...
    {WIFI_ID_HISI_HI1131C, WIFI_HISI_HI1131C},
    {WIFI_ID_REALTEK_RTL8812AE, WIFI_REALTEK_RTL8812AE},
    {WIFI_ID_REALTEK_RTL8188FU, WIFI_REALTEK_RTL8188FU},
    {WIFI_ID_AMPAK_AP6356S, WIFI_AMPAK_AP6356S},
    {WIFI_ID_REALTEK_RTL8822BEH, WIFI_REALTEK_RTL8822BEH},
    {WIFI_ID_ATHEROS_QCA6174, WIFI_ATHEROS_QCA6174}
};
...
HI_S32 wlan_util_get_sdio_wifi_device(HI_VOID)
{
    ...
    /* Try Hi1131C first */
    if (wlan_util_insmod_module("/kmod/sdio_detect.ko", "sdio_detect ", NULL))
    {
         //printf("fail to insmod sdio_detect.ko\n");
    } else {
         printf("success to insmod sdio_detect.ko\n");
         usleep(200000);
    }
    ...
    while ((next = readdir(dir)) != NULL) {
        ...
        snprintf(uevent_file, sizeof(uevent_file), "%s/%s/uevent", SDIO_DIR, next->d_name);
    ...
    if(ret == WIFI_HISI_HI1131C) {
        if (wlan_util_rmmod_module("sdio_detect") != 0) {
            printf("fail to rmmod sdio_detect driver\n");
            ret = -1;
        }

        printf("success to rmmod sdio_detect.ko\n");
    }
...
HI_S32 wlan_util_get_pci_wifi_device(HI_VOID)
{
    HI_S32 ret = -1;
    DIR *dir;
    struct dirent *next;
    ...
    while ((next = readdir(dir)) != NULL) {
         ...
         * DEVNUM=005
         */
        snprintf(uevent_file, sizeof(uevent_file), "%s/%s/uevent", PCI_DIR, next->d_name);
...
HI_S32 wlan_util_get_wifi_device(HI_VOID)
{
    ...
    ret = wlan_util_get_pci_wifi_device();
    if (-1 != ret)
        return ret;
...
HI_S32 wlan_util_get_interface(HI_CHAR *ifname, const HI_CHAR *dev_file)
{
    ...
    if(fh != NULL) {
        ...
        if (NULL == fgets(buff, sizeof(buff), fh)) {
