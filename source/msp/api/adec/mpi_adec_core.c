...
#define ADEC_INVALID_PTS 0xffffffff
...
static pthread_mutex_t   g_AdecApiMutex[ADEC_INSTANCE_MAXNUM]  = {[0 ... (ADEC_INSTANCE_MAXNUM-1)] = PTHREAD_MUTEX_INITIALIZER};
static pthread_mutex_t   g_AdecDataMutex[ADEC_INSTANCE_MAXNUM] = {[0 ... (ADEC_INSTANCE_MAXNUM-1)] = PTHREAD_MUTEX_INITIALIZER};
static pthread_mutex_t   g_AdecIOMutex[ADEC_INSTANCE_MAXNUM]   = {[0 ... (ADEC_INSTANCE_MAXNUM-1)] = PTHREAD_MUTEX_INITIALIZER};
...
extern HI_S32            ADEC_InitDecoderImple(ADEC_INFO_S* pstAdecAttr);
...
static HI_VOID ADECCheckScrambler(HI_HANDLE hAdec, HI_S32 s32BsConsumeBytes)
{
    ...
    if (HI_NULL == pstAdecChan->pstAdecInfo)
    {
        HI_ERR_ADEC("pstAdecChan->pstAdecInfo is null\n");
        return;
    }
...
static HI_S32 ADECInitInBuf(ADEC_STREAM_BUFFER_S* pstInBuf, HI_U32 u32PaddingSize, HI_U32 u32AdecChID)
{
    ...
    memset(pstInBuf->pu8Data, 0, (pstInBuf->u32BufSize + u32PaddingSize) * sizeof(HI_U8));
    ...
    if ((HI_S32)pstInBuf->u32BufWritePos < pstInBuf->s32BufReadPos)
    {
        ...
        if ((HI_U32)s32WrapBytes <= pstInBuf->u32BufPaddingSize)
        {
            memcpy(pstInBuf->pu8Data - s32WrapBytes, pstInBuf->pu8Data + pstInBuf->s32BufReadPos,
                   (HI_U32)s32WrapBytes);
            pstInBuf->s32BufReadPos = (0 - s32WrapBytes);
        }
    }
}
                             ...
                             HI_U32 u32AdecChID)
{
    ...
    {
        pu32Buf = (HI_U32*)HI_MPI_MMZ_Map(pstOutBuf->sAdecOutMMzBuf.phyaddr, HI_FALSE);
    ...
    memset(pu32Buf, 0, pstOutBuf->u32OutBufNum * uMaxBufSizePerFrame);
...
#endif
        pstOutBuf->pu32BaseAddr = HI_NULL;
    }
...
static HI_VOID ADECRestPTSQuet(ADEC_PTS_QUE_S* pstPTSQue)
{
    ...
    pstPTSQue->u32LastPtsMs = ADEC_INVALID_PTS;
    pstPTSQue->u32LastStorePtsMs = ADEC_INVALID_PTS;
}
...
{
    if (((pstPTSQue->ulPTSwrite + 2) % ADEC_MAX_STORED_PTS_NUM) == pstPTSQue->ulPTSread)
    {
#if 0
                          ...
                          HI_U32 u32OutSampleRate, HI_U32* pu32OrgPTS)
{
    ...
    if (((HI_U32) - 1) == FoundPtsPos)
    {
        ...
        if (ADEC_INVALID_PTS != pstPTSQue->u32LastPtsMs)
        {
            u32PtsMs = pstPTSQue->u32LastPtsMs + u32Delta;
            if (ADEC_INVALID_PTS == u32PtsMs)
            {
                u32PtsMs = 0;
            }                      /* avoid loop*/
        ...
        {
            u32PtsMs = ADEC_INVALID_PTS;   /* if the PTS of first Frame isnot valid, dont insert the PTS*/
        }
    ...
    else
    {
        ...
        ptPTS[Pos].u32PtsMs  = ADEC_INVALID_PTS;
        pstPTSQue->ulPTSread = (HI_U32)Pos;
        pstPTSQue->u32LastPtsMs = u32PtsMs;
    }
...
static HI_U32 ADECFindFixPTS(ADEC_PTS_QUE_S* pstPTSQue, HI_U32 u32Pos, HI_U32 u32PcmOutSamplesPerFrame,
                          HI_U32 u32OutSampleRate, HI_U32* pu32OrgPTS)
{
    HI_U32 u32PtsMs = 0;
    HI_U32 Pos = 0, FoundPtsPos = (HI_U32) - 1;
    HI_U32 rdPos = 0;
    HI_U32 wtPos = 0;
    ADEC_PTS* ptPTS = HI_NULL;
                            ...
                            HI_U32 u32BsSize)
{
    ...
    if (((0xffffffff) == u32PtsMs))
    {
        ...
        {
            ptPTSArrayOpt[pstPTSQue->ulPTSwrite - 1].u32EndPos =
                (ptPTSArrayOpt[pstPTSQue->ulPTSwrite - 1].u32EndPos + u32BsSize) % pstInBuf->u32Boundary;
        }
        else
        {
            ptPTSArrayOpt[ADEC_MAX_STORED_PTS_NUM - 1].u32EndPos =
                (ptPTSArrayOpt[ADEC_MAX_STORED_PTS_NUM - 1].u32EndPos + u32BsSize) % pstInBuf->u32Boundary;
        }
    }
    else  if (HI_TRUE != ADECIsPTSFull(pstPTSQue))
    {
        if (u32PtsMs < pstPTSQue->u32LastStorePtsMs)
        {
        ...
        ptPTSArrayOpt[pstPTSQue->ulPTSwrite].u32EndPos =
            (HI_U32)((HI_U32)pstInBuf->u32StreamWritePos + u32BsSize) % pstInBuf->u32Boundary;
...
static HI_S32 ADECGetStreamPTS(ADEC_CHAN_S* pstAdecChan, HI_S32* s32BufferBps, HI_U32* pInStreamPts)
{
    ...
    else
    {
        ...
        {
            u32PtsMs = (pstPTSQue->u32RecyleStorePtsMs - pstPTSQue->u32LastPtsMs) + (pstPTSQue->u32LastStorePtsMs - pstPTSQue->u32RecycleFirstPtsMs); //pts re-cycle
...
static HI_VOID ADECGetFrameTime(HI_UNF_AO_FRAMEINFO_S* pstAOFrame, HI_U32* u32FrameDurationMs)
{
    if (pstAOFrame->u32SampleRate)
    {
        ...
        {
            *u32FrameDurationMs =
                (pstAOFrame->u32PcmSamplesPerFrame * 1000) / ((HI_U32)pstAOFrame->u32SampleRate);
        }
        else
        {
            *u32FrameDurationMs =
                (((pstAOFrame->u32BitsBytesPerFrame & 0xffff) / (2 * sizeof(HI_U16))) * 1000) / ((HI_U32)pstAOFrame->u32SampleRate);
        }
    }
    else
    {
...
static HI_S32 ADECOpenDevice(ADEC_CHAN_S* pstAdecChan)
{
    HI_S32 val = 0;
    HI_S32 s32Ret = HI_SUCCESS;

    pstAdecChan->AdecDevFd = open(ADEC_DEVICE_NAME, O_RDWR, 0);
    if (-1 == pstAdecChan->AdecDevFd)
    {
        HI_FATAL_ADEC("Cannot open '%s'!\n", ADEC_DEVICE_NAME);
        return HI_ERR_ADEC_DEV_NOT_OPEN;
    }

    val = fcntl(pstAdecChan->AdecDevFd, F_GETFD);
    val |= FD_CLOEXEC;

    s32Ret = fcntl(pstAdecChan->AdecDevFd, F_SETFD, val);
    if (HI_SUCCESS != s32Ret)
    {
        s32Ret = close(pstAdecChan->AdecDevFd);
        if (HI_SUCCESS != s32Ret)
        {
            HI_WARN_ADEC("Adec Device Close failed(0x%x)\n", s32Ret);
        }
...
static HI_S32 ADECCloseDevice(ADEC_CHAN_S* pstAdecChan)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
static HI_VOID ADECGetHaSzname(ADEC_CHAN_S* pstAdecChan)
{
    ...
    if (hHaEntry)
    {
        if (hHaEntry->szName)
        {
            memcpy(pstAdecInfo->szCodecType, hHaEntry->szName, sizeof(pstAdecInfo->szCodecType));
            pstAdecInfo->szCodecType[ADEC_CODEC_NAME_LENGTH - 1] = 0;
        }
        else
        {
        ...
        {
            memcpy(pstAdecInfo->szCodecDescription, hHaEntry->pszDescription, sizeof(pstAdecInfo->szCodecDescription));
            pstAdecInfo->szCodecDescription[ADEC_CODEC_NAME_LENGTH - 1] = 0;
        }
        else
        {
...
static HI_S32 ADECInitDecoder(ADEC_INFO_S* pstAdecAttr)
{
    return ADEC_InitDecoderImple(pstAdecAttr);
}

static HI_VOID ADECDeInitDecoder(ADEC_INFO_S* pstAdecAttr)
{
    HI_HA_DECODE_S* pHaDev;

    CHECK_NULL_PTR_NORET(pstAdecAttr);

    if (HI_NULL_PTR != (pstAdecAttr->hDecoder))
    {
        if (HI_NULL_PTR != (pstAdecAttr->pHaDecoderDev))
        {
            pHaDev = pstAdecAttr->pHaDecoderDev;
            (HI_VOID)pHaDev->DecDeInit(pstAdecAttr->hDecoder);
            pstAdecAttr->hDecoder = HI_NULL_PTR;
        }
    }
}

static HI_S32 ADECResetDecoder(ADEC_INFO_S* pstAdecAttr)
{
    HI_S32 s32Ret;

    CHECK_ADEC_NULL_PTR(pstAdecAttr);

    ADECDeInitDecoder(pstAdecAttr);

    s32Ret = ADECInitDecoder(pstAdecAttr);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADEC("ADECInitDecoder failed(0x%x)\n", s32Ret);
    }
...
static HI_VOID ADECConvertHaOut2MpiOut(ADEC_CHAN_S* pstAdecChan, ADEC_OUTPUTBUF_S* ptRetBuf,
                                       HI_HADECODE_OUTPUT_S* pstAout, HI_U32 u32StreamReadPos, HI_U32 u32ConsumeBytes)
{
    ...
    if (!pstAout->u32PcmOutSamplesPerFrame)
    {
        ...
        {
            if (16 == pstAout->u32BitPerSample)
            {
                u32PcmSamplesPerFrame = (pstAout->u32BitsOutBytesPerFrame & 0xffff) / (pstAout->u32OutChannels * sizeof(HI_U16)); //only passthrough exist
            }
            else
            {
                u32PcmSamplesPerFrame = (pstAout->u32BitsOutBytesPerFrame & 0xffff) / (pstAout->u32OutChannels * sizeof(HI_U32)); //only passthrough exist
            }
        }
        else
        {
    ...
    ptRetBuf->u32PTS = ADECFindPTS(&(pstAdecChan->PTSQue), (HI_U32)u32StreamReadPos, u32PcmSamplesPerFrame,
                                   pstAout->u32OutSampleRate, &ptRetBuf->u32OrgPTS);

    if (0xffffffff == ptRetBuf->u32PTS)
    {
        HI_U32 u32EndPos = 0;
        ADEC_STREAM_BUFFER_S* pstInBuf = &(pstAdecChan->InStreamBuf);

        u32EndPos += u32ConsumeBytes;
        if (u32EndPos > pstInBuf->u32Boundary)
        {
            u32EndPos -= pstInBuf->u32Boundary;
        }

        /*
         * 1. u32BeginPos Pts is -1
         * 2. u32BeginPos ~ u32EndPos cover a valid Pts
         * then report the next valid Pts
         */
        ptRetBuf->u32PTS = ADECFindFixPTS(&(pstAdecChan->PTSQue), u32EndPos, u32PcmSamplesPerFrame,
                                   pstAout->u32OutSampleRate, &ptRetBuf->u32OrgPTS);
    }

    //pts fix: if the decoder output delay exist
    //the output pts should minus it
    if (ptRetBuf->u32PTS < pstAdecChan->u32OutputDelay)
    {
        ptRetBuf->u32PTS = 0;
    }
    else
    {
        ptRetBuf->u32PTS -= pstAdecChan->u32OutputDelay;
    }
    ...
    if (ADEC_MAX_VOLUME != pstAdecChan->u32Volume && 0 != pstAout->u32PcmOutSamplesPerFrame)
    {
...
static HI_VOID ADECGetCodecOutputDelay(ADEC_CHAN_S* pstAdecChan)
{
    HI_S32 s32Ret;
    HI_VOID* hDecoder = pstAdecChan->decAttr.hDecoder;
    HI_HA_DECODE_S* pHaDecoderDev = pstAdecChan->decAttr.pHaDecoderDev;



    switch(pstAdecChan->u32CodecID)
    {
        case HA_AUDIO_ID_AAC:
        {
            HA_CODEC_OUTOUTDELAY_GET_PARAM_S stOutputDelay = {0};
            stOutputDelay.enCmd = HA_CODEC_OUTPUTDELAY_GET_CMD;

            s32Ret = (HI_S32)pHaDecoderDev->DecSetConfig(hDecoder, (HI_VOID*)&stOutputDelay);
            if (HA_ErrorNone == s32Ret)
            {
                pstAdecChan->u32OutputDelay = stOutputDelay.u32OutputDelay;
                HI_INFO_ADEC("u32OutputDelay = %d\n", pstAdecChan->u32OutputDelay);
            }
            else
            {
                pstAdecChan->u32OutputDelay = 0;
               // HI_ERR_ADEC("Get output Delay Failed\n");
            }
            break;
        }
        case HA_AUDIO_ID_DOLBY_PLUS:
        {
            pstAdecChan->u32OutputDelay = DDP_STATIC_OUTPUT_DELAY;
            break;
        }
        default:
            pstAdecChan->u32OutputDelay = 0;
...
static HI_S32 ADECSwDecode(ADEC_CHAN_S* pstAdecChan)
{
    ...
    HI_U32 u32InDataBytes = 0, u32OutDataBytes = 0;
    ...
    CHECK_ADEC_NULL_PTR(pstAdecInfo);
    ...
    while (1)
    {
        ...
        u32StreamReadPos = ADECFixStreamReadPos(pstAdecChan, u32StreamReadPos,
                                        pstAdecChan->u32SwDecoderBytesLeft);

        /* decode one audio frame */
        u32ByteLeftBfDecode = avpkt.s32Size;
        u32InDataBytes = avpkt.s32Size + pstAdecChan->u32SwDecoderBytesLeft;
        ...
        {
            if ((0 == u32FrameConsumeBytes) && (0 != avpkt.s32Size))
            {
                pstAdecChan->u32NotConsumeByteCnt++;
                if (HI_TRUE == pstAdecChan->bPacketDecoder)
                {
            ...
            {
                pstAdecChan->u32NotConsumeByteCnt = 0;
            }

            // about 1s time & can not decode one frame, notify unsupport format
            if ((pstAdecChan->u32NotConsumeByteCnt >= ADEC_MAX_UNCONSUMEBYTECNT) && (0 == pstAdecInfo->u32FramnNm))
            {
                /* discard bytes */
                u32FrameConsumeBytes = MIN2(avpkt.s32Size, ADEC_BYTES_DISCARD_THD);
                pstAdecInfo->u32CodecUnsupportNum++;
            }

            pstAdecChan->u32SwDecoderBytesLeft = sOut.stPtsInfo.unPts.u32SwDecoderBytesLeft;
        ...
        else if (HA_ErrorNone == sRet)    /* Success */
        {
            ...
            pstAdecChan->u32SwDecoderBytesLeft = sOut.stPtsInfo.unPts.u32SwDecoderBytesLeft;
            u32OutDataBytes = avpkt.s32Size + pstAdecChan->u32SwDecoderBytesLeft;

            ADECGetCodecOutputDelay(pstAdecChan);

            ADECConvertHaOut2MpiOut(pstAdecChan, &pstOutBuf->outBuf[pstOutBuf->u32BufWriteIdx], &sOut, u32StreamReadPos, u32InDataBytes - u32OutDataBytes);
            ...
            pstAdecInfo->u32OrgChannels = sOut.u32OrgChannels;
            pstAdecInfo->u32PcmSamplesPerFrame = (sOut.u32PcmOutSamplesPerFrame & 0xffff);
            pstAdecInfo->u32BitRate = sOut.u32BitRate;
...
static HI_S32 ADECInitChannel(ADEC_CHAN_S* pstAdecChan)
{
    ...
    pstAdecChan->u32SwDecoderBytesLeft = 0;
    pstAdecChan->u32OutputDelay = 0;
    ...
    memset(pstAdecChan->pstPacketQue, 0, sizeof(ADEC_PACKET_QUE_S));
...
static HI_S32 ADECOpenChannel(ADEC_CHAN_S* pstAdecChan)
{
    ...
    s32Ret = ADECOpenDevice(pstAdecChan);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADEC("ADECOpenDevice failed(0x%x)\n", s32Ret);
        return s32Ret;
    }

    s32Ret = ioctl(pstAdecChan->AdecDevFd, DRV_ADEC_PROC_INIT, &phy_adress);
    if (HI_SUCCESS != s32Ret)
    {
        (HI_VOID)ADECCloseDevice(pstAdecChan);
        HI_ERR_ADEC("ioctl DRV_ADEC_PROC_INIT failed(0x%x)\n", s32Ret);
        return s32Ret;
    }

    pstAdecChan->pstAdecInfo = (ADEC_PROC_ITEM_S*)HI_MPI_MMZ_Map(phy_adress, HI_FALSE);
    if (HI_NULL == pstAdecChan->pstAdecInfo)
    {
        (HI_VOID)ioctl(pstAdecChan->AdecDevFd, DRV_ADEC_PROC_EXIT, &pstAdecChan->pstAdecInfo);
        (HI_VOID)ADECCloseDevice(pstAdecChan);
        HI_ERR_ADEC("HI_MPI_MMZ_Map ADEC_PROC_ITEM_S err\n");
        return HI_ERR_ADEC_NULL_PTR;
    }
    pstAdecChan->u32PhyAddr = phy_adress;
...
static HI_VOID ADECCloseChannel(ADEC_CHAN_S* pstAdecChan)
{
    ...
    {
        (HI_VOID)HI_MPI_MMZ_Unmap(pstAdecChan->u32PhyAddr);
...
static HI_S32 ADECCheckHACodecAttr(const ADEC_ATTR_S* pstAttr)
{
    ...
    if ((pstAttr->sOpenPram.sPcmformat.u32BitPerSample != 16)
        ...
        && (pstAttr->sOpenPram.sPcmformat.u32BitPerSample != 32)
       )
    {
...
static HI_S32 ADECGetInputAttr(ADEC_CHAN_S* pstAdecChan, const ADEC_ATTR_S*  pstAttr)
{
    ...
    {
        HA_CODEC_PACKETDECODER_QUERY_PARAM_S stPacketDecoder = {0};
...
static HI_S32 ADECGetAttr(HI_HANDLE hAdec, HI_U32 u32Command, HI_VOID* pstAttr)
{
    ADEC_INFO_S* ptDecAttr = HI_NULL_PTR;
    ADEC_CHAN_S* pstAdecChan = HI_NULL_PTR;
    ADEC_STREAM_BUFFER_S* pstInBuf  = HI_NULL_PTR;
    ...
    switch (u32Command)
    {
        ...
        default:
            HI_ERR_ADEC("INVALID PARAM = 0x%x\n", u32Command);
            return HI_ERR_ADEC_INVALID_PARA;
    }
    return HI_SUCCESS;
}

static HI_S32 ADECSonicCreate(ADEC_CHAN_S* pstAdecChan)
{
    HI_S32 s32Ret = HI_FAILURE;

    s32Ret = Sonic_Create(&pstAdecChan->pSonic, 8192 * 10, pstAdecChan->u32AdecChID);
    if (HI_SUCCESS != s32Ret)
    {
        HI_WARN_ADEC("Sonic_Create create failed!\n");
        return s32Ret;
    }

    pstAdecChan->bSonicCreated = HI_TRUE;

    return s32Ret;
}

static HI_S32 ADECSonicDestroy(ADEC_CHAN_S* pstAdecChan)
{
    HI_S32 s32Ret = HI_FAILURE;

    s32Ret = Sonic_Destroy(pstAdecChan->pSonic);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADEC("Sonic_Create create failed!\n");
        return s32Ret;
    }

    pstAdecChan->bSonicCreated = HI_FALSE;
    return s32Ret;
}

static HI_VOID* ADECDecThread(HI_VOID* arg)
{
    HI_U32 u32SleepCnt = 0;
    ...
    if (HI_NULL == pstAdecChan->pstAdecInfo)
    {
        HI_ERR_ADEC("adec(%d) proc info ptr isn null.\n", hAdec);
        return HI_NULL;
    }
    ...
    while (pstAdecChan->AdecThreadRun)
    {
        ...
        if (ptDecAttr->hDecoder == HI_NULL)
        {
            ...
            for (u32SleepCnt = 0; u32SleepCnt < ADEC_SYS_SLEEP_TIME; u32SleepCnt++)
            {
                if (HI_FALSE == pstAdecChan->AdecThreadRun)
                {
                    break;
                }
                stSlpTm.tv_sec = 0;
        ...
        if ((pstAdecChan->beAssigned == HI_TRUE) && (pstAdecChan->beWork == HI_TRUE))
        {
            ...
            if (HA_AUDIO_ID_BLYRAYLPCM == (HI_U32)pstAdecChan->pstAdecInfo->u32CodecID)
            {
                pstAdecChan->pstAdecInfo->u32AdecSystemSleepTime = ADEC_MIN_SLEEP_TIME;
            }
            /* shorten the exit time of this thread */
            for (u32SleepCnt = 0; u32SleepCnt < pstAdecChan->pstAdecInfo->u32AdecSystemSleepTime; u32SleepCnt++)
            {
                if (HI_FALSE == pstAdecChan->AdecThreadRun)
                {
                    break;
...
HI_S32 ADEC_Open(HI_HANDLE* phAdec)
{
    ...
    pstAdecChan->stFrameInfo.enBitDepth = HI_UNF_BIT_DEPTH_BUTT;
    pstAdecChan->stFrameInfo.enSampleRate = HI_UNF_SAMPLE_RATE_BUTT;
    pstAdecChan->stTplayAttr.bTplayEnable = HI_FALSE;
    ...
    (HI_VOID)ADECSonicCreate(pstAdecChan);
...
HI_S32 ADEC_Close(HI_HANDLE hAdec)
{
    ...
    if (HI_TRUE == pstAdecChan->bSonicCreated)
    {
        (HI_VOID)ADECSonicDestroy(pstAdecChan);
    }
...
HI_S32 ADEC_SetConfigDecoder(const HI_U32 enDstCodecID, HI_VOID* pstConfigStructure)
{
    ...
    if (pHaDev->DecSetConfig)
    {
        ...
        {
            HI_INFO_ADEC("DecSetConfig Fail(0x%x)\n", s32Ret);
            return s32Ret;
        }
    }
...
HI_S32 ADEC_GetDelayMs(HI_HANDLE hAdec, HI_U32* pDelayMs)
{
    ...
    CHECK_ADEC_HANDLE(hAdec);
    CHECK_ADEC_NULL_PTR(pDelayMs);

    ADEC_LOCK(&g_AdecApiMutex[hAdec]);
    CHECK_ADEC_CHN_NULL(g_pstAdecChan[hAdec], &g_AdecApiMutex[hAdec]);
    pstAdecChan = g_pstAdecChan[hAdec];
    CHECK_ADEC_NULL_PTR_IN_LOCK(pstAdecChan->pstAdecInfo, &g_AdecApiMutex[hAdec]);
...
HI_S32 ADEC_SetAttr(HI_HANDLE hAdec, HI_U32 u32Command, HI_VOID* pstParam)
{
    ...
    //------------1 . first  get attr -------------------
    s32Ret = ADECGetAttr(hAdec, ADEC_ATTR_ALLATTR, &stAllAttr);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADEC("ADECGetAttr ADEC_ATTR_ALLATTR failed(0x%x)\n", s32Ret);
        ADEC_UNLOCK(&g_AdecDataMutex[hAdec]);
        ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
        return s32Ret;
    }
    ...
    s32Ret = ADECCheckAttr(&stAllAttr);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADEC("ADECCheckAttr failed(0x%x)\n", s32Ret);
        ADEC_UNLOCK(&g_AdecDataMutex[hAdec]);
        ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
        return s32Ret;
    }
    ...
    s32Ret = ADECAttrProc(pstAdecChan, &stAllAttr);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADEC("ADECAttrProc failed(0x%x)\n", s32Ret);
    }
...
HI_S32 ADEC_GetAttr(HI_HANDLE hAdec, HI_U32 u32Command, HI_VOID* pstAttr)
{
    ...
    s32Ret = ADECGetAttr(hAdec, u32Command, pstAttr);
...
HI_S32 ADEC_SendStream(HI_HANDLE hAdec, const HI_UNF_STREAM_BUF_S* pstStream, HI_U32 u32PtsMs)
{
    ...
    if (ADEC_CMD_CTRL_START == pstAdecInfo->enEsCtrlState)
    {
        ...
        {
            pstAdecChan->pfEs = fopen(pstAdecInfo->esFilePath, "wb");
            if (!pstAdecChan->pfEs)
            {
                HI_ERR_ADEC("can not open file (%s)\n", pstAdecInfo->esFilePath);
            }
        }
        if (pstAdecChan->pfEs)
        {
                      ...
                      const HI_UNF_STREAM_BUF_S* pstStream2, HI_U32 u32PtsMs)
{
    ...
    if ((pstStream1->u32Size + pstStream2->u32Size) > ADEC_MAX_INPUT_BLOCK_SIZE)
    {
        HI_ERR_ADEC("stream1 u32Size(%d)+ pstStream2 u32Size(%d) exceed maxsize(%d)\n",
                    pstStream1->u32Size, pstStream2->u32Size, ADEC_MAX_INPUT_BLOCK_SIZE );
        return HI_ERR_ADEC_INVALID_PARA;
    }
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    CHECK_ADEC_CHN_NULL(g_pstAdecChan[hAdec], &g_AdecApiMutex[hAdec]);
    CHECK_ADEC_NULL_PTR_IN_LOCK(g_pstAdecChan[hAdec]->pstAdecInfo, &g_AdecApiMutex[hAdec]);
...
HI_S32 ADEC_GetBufferStatus(HI_HANDLE hAdec, ADEC_BUFSTATUS_S* pstBufStatus)
{
    ...
    if (HI_TRUE == ADECIsPTSFull(&pstAdecChan->PTSQue))
    {
        pstBufStatus->u32BufferAvailable = 0;
    }
    else if (HI_TRUE == pstAdecChan->bPacketDecoder &&
             HI_TRUE == ADECIsPacketFull(pstAdecChan->pstPacketQue))
    {
        pstBufStatus->u32BufferAvailable = 0;
    }
    else
    {
        pstBufStatus->u32BufferAvailable = MIN2(pstInBuf->u32BufFree - 1, ADEC_MAX_INPUT_BLOCK_SIZE);
    }

    pstBufStatus->u32BufferSize = pstInBuf->u32BufSize;
    pstBufStatus->u32TotDecodeFrame  = pstAdecChan->pstAdecInfo->u32FramnNm;
#if 1 //for tunneled mode temp
    pstBufStatus->u32SamplesPerFrame = pstAdecChan->pstAdecInfo->u32PcmSamplesPerFrame;
#endif
...
HI_S32 ADEC_ReceiveFrame(HI_HANDLE hAdec, HI_UNF_AO_FRAMEINFO_S* pstAOFrame, ADEC_EXTFRAMEINFO_S* pstExtInfo)
{
    ...
    pstOutBuf->u32BufReadIdx  = (pstOutBuf->u32BufReadIdx + 1) % (pstOutBuf->u32OutBufNum);
    pstAdecInfo->u32FrameRead = pstOutBuf->u32BufReadIdx;
    if ((HI_TRUE == pstAdecChan->stTplayAttr.bTplayEnable) && (HI_TRUE == pstAdecChan->bSonicCreated))
    {
        s32Ret = Sonic_Process(pstAdecChan->pSonic, pstAOFrame, pstAdecChan->stTplayAttr.u32SpeedInt, \
                               pstAdecChan->stTplayAttr.u32SpeedDeci);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_ADEC("Sonic_Process failed %d\n", s32Ret);
            pstAdecChan->pstAdecInfo->u32SpeedErrorNum++;
            ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
            ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
            return s32Ret;
        }
        if (0 == pstAOFrame->u32PcmSamplesPerFrame)
        {
            HI_WARN_ADEC("data is empty\n");
            ptOutElem->bFlag = HI_FALSE;
            ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
            ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
            return HI_ERR_ADEC_OUT_BUF_EMPTY;
    ...
    if (ADEC_CMD_CTRL_START == pstAdecChan->pstAdecInfo->enPcmCtrlState)
    {
        ...
        {
            pstAdecChan->pfPcm = fopen(pstAdecChan->pstAdecInfo->pcmFilePath, "wb");
            if (!pstAdecChan->pfPcm)
            {
                HI_ERR_ADEC("can not open file (%s)\n", pstAdecChan->pstAdecInfo->pcmFilePath);
            }
        }
        if (HI_NULL != pstAdecChan->pfPcm)
        {
...
HI_S32 ADEC_GetStatusInfo(HI_HANDLE hAdec, ADEC_STATUSINFO_S* pstStatusinfo)
{
    ...
    if (HI_TRUE == ADECIsPTSFull(&pstAdecChan->PTSQue))
    {
        pstStatusinfo->u32BufferAvailable = 0;
    }
    else if (HI_TRUE == pstAdecChan->bPacketDecoder &&
             HI_TRUE == ADECIsPacketFull(pstAdecChan->pstPacketQue))
    {
    ...
    {
        pstStatusinfo->u32BufferAvailable = MIN2(pstInBuf->u32BufFree - 1, ADEC_MAX_INPUT_BLOCK_SIZE);
    }
    ...
    pstStatusinfo->u32TotDecodeFrame = pstAdecChan->pstAdecInfo->u32FramnNm;
...
HI_S32 ADEC_GetDebugInfo(HI_HANDLE hAdec, ADEC_DEBUGINFO_S* pstDebuginfo)
{
    ...
    pstAdecInfo = pstAdecChan->pstAdecInfo;
    pstDebuginfo->u32DecFrameNum = pstAdecInfo->u32FramnNm;
    pstDebuginfo->u32ErrDecFrameNum = pstAdecInfo->u32ErrFrameNum;
...
HI_S32 ADEC_GetStreamInfo(HI_HANDLE hAdec, ADEC_STREAMINFO_S* pstStreaminfo)
{
    ...
    pstStreaminfo->enBitDepth = pstAdecInfo->enBitWidth;
    pstStreaminfo->u32Channel = pstAdecInfo->u32OrgChannels;
    pstStreaminfo->u32BitRate = pstAdecInfo->u32BitRate;
...
HI_S32 ADEC_GetAudSpectrum(HI_U16* pSpectrum, HI_U32 u32BandNum)
{
    ...
    else
    {
        ...
        /* fix by sdk@20110601: do NOT return HI_FAILURE,
...
HI_S32 ADEC_DropStream(HI_HANDLE hAdec, HI_U32 u32SeekPts)
{
    ...
    for (u32SeekPos = pstPTSQue->ulPTSread; u32SeekPos != pstPTSQue->ulPTSwrite; u32SeekPos = (u32SeekPos + 1) % (ADEC_MAX_STORED_PTS_NUM))
    {
        ...
        if (pstCurrentPTS->u32PtsMs != ADEC_INVALID_PTS)
        {
...
HI_S32 ADEC_SetCodecCmd(HI_HANDLE hAdec, HI_VOID* pstCodecCmd)
{
    ...
    CHECK_ADEC_HANDLE(hAdec);
    CHECK_ADEC_NULL_PTR(pstCodecCmd);
...
HI_S32 ADEC_CheckNewEvent(HI_HANDLE hAdec, ADEC_EVENT_S* pstNewEvent)
{
        ...
        && (pstAdecChan->stFrameInfo.enBitDepth != HI_UNF_BIT_DEPTH_BUTT))
    {
        if ((pstAdecChan->stFrameInfo.enSampleRate > 192000) || (pstAdecChan->stFrameInfo.enSampleRate < 8000)
            || ((HI_U32)pstAdecChan->stFrameInfo.enBitDepth > 32) || ((HI_U32)pstAdecChan->stFrameInfo.enBitDepth < 8))
        {
...
HI_S32 ADEC_SetTplay(HI_HANDLE hAdec, ADEC_TPLAY_S* pstTplayParam)
{
    ADEC_CHAN_S* pstAdecChan = HI_NULL_PTR;

    CHECK_ADEC_HANDLE(hAdec);
    CHECK_ADEC_NULL_PTR(pstTplayParam);

    CHECK_ADEC_SPEED(pstTplayParam->u32SpeedInt);
    ...
    memcpy(&pstAdecChan->stTplayAttr, pstTplayParam, sizeof(ADEC_TPLAY_S));
    pstAdecChan->pstAdecInfo->u32SpeedInt = pstTplayParam->u32SpeedInt;
    pstAdecChan->pstAdecInfo->u32SpeedDeci = pstTplayParam->u32SpeedDeci;
