...
HI_BOOL LinearSRC_CheckDoSRC(SRC_Linear* pLinearSrc)
{
     ...
     */
    if ((pLinearSrc->AdjustInRate == pLinearSrc->OutRate) && (0 == pLinearSrc->DisContinuity))
    {
        return HI_FALSE;  /* dont do LinearSRC */
    }

    return HI_TRUE;  /* do LinearSRC */
}
        ...
        while (insamps > 0)
        {
            while (pInst->Remainder < pInst->OutRate)
            {
                for (ch = 0; ch < Channels; ch++)
                {
                    ...
                    sum += MULSHIFT32(pcmbuf[Channels * (in + 0) + ch], pInst->IvtSFFactor * (pInst->OutRate - pInst->Remainder));
                    sum = CLIPTOSHORT(sum >> 14); /* IvtSFFactor is 2.30 format */
                    pOutpcmBuf[Channels * out + ch] = sum << 16;
                    ...
                    pOutpcmBuf[Channels * out + ch] = (HI_S32)((s64sample0 * pInst->Remainder +
                        s64sample1 * (pInst->OutRate - pInst->Remainder)) / pInst->OutRate);
#endif
                }
...
#else
                    pOutpcmBuf[Channels * out + ch] = ((pcmbuf[Channels * in + ch] * pInst->Remainder +
                    pInst->PreSample[ch] * (pInst->OutRate - pInst->Remainder)) / pInst->OutRate);
#endif
                }
...
#else
                    pOutpcmBuf[Channels * out + ch] = ((pcmbuf[Channels * (in + 1) + ch] * pInst->Remainder +
                    pcmbuf[Channels * in + ch] * (pInst->OutRate - pInst->Remainder)) / pInst->OutRate);
#endif
                }
                               ...
                               HI_S32       InSamps)
{
    ...
    {
        return LinearSRC_16bitProcessFrame(pInst, (HI_S16*)pInPcmBuf, (HI_S16*)pOutpcmBuf, InSamps);
    }

    return LinearSRC_32bitProcessFrame(pInst, (HI_S32*)pInPcmBuf, (HI_S32*)pOutpcmBuf, InSamps);
}
