...
HI_VOID* AITrackThread(HI_VOID* Arg)
{
    ...
    while (pstAiState->AiThreadRun)
    {
        ...
        stSlpTm.tv_sec = 0;
        stSlpTm.tv_nsec = AI_CHECK_DELAY_SLEEP_TIME_MS * 1000 * 1000;
        if(0 != nanosleep(&stSlpTm, NULL))
        {
            HI_ERR_AI("nanosleep err.\n");
        }
    }
    ...
    while (pstAiState->AiThreadRun)
    {
        ...
        if ((HI_FALSE == bDataFlag) && (pstAiState->u32DstNum))
        {
            ...
            else
            {
                ...
                stSlpTm.tv_sec = 0;
                stSlpTm.tv_nsec = AI_SLEEP_TIME_MS * 1000 * 1000;
                if(0 != nanosleep(&stSlpTm, NULL))
                {
                    HI_ERR_AI("nanosleep err.\n");
                }
...
static HI_S32 AIProcInit(AI_MPISTATE_S* pstAiState)
{
    ...
    pstAiState->pstAiProcInfo = (AI_PROC_INFO_S*)HI_MPI_MMZ_Map(stParam.tProcPhyAddr, HI_FALSE);
    if (HI_NULL == pstAiState->pstAiProcInfo)
    {
        HI_ERR_AI("HI_MPI_MMZ_Map AI_PROC_INFO_S failed!\n");
        (HI_VOID)ioctl(g_s32AIFd, CMD_AI_PROCDEINIT, &pstAiState->hAI);
        return HI_FAILURE;
    }

    pstAiState->u32ProcPhyAddr = stParam.tProcPhyAddr;
...
static HI_S32 AIProcDeInit(AI_MPISTATE_S* pstAiState)
{
    ...
    if (HI_NULL != pstAiState->pstAiProcInfo)
    {
        (HI_VOID)HI_MPI_MMZ_Unmap(pstAiState->u32ProcPhyAddr);
    }
...
HI_S32 HI_MPI_AI_Create(HI_UNF_AI_E enAiPort, HI_UNF_AI_ATTR_S* pstAttr, HI_HANDLE* phandle)
{
    ...
    pUserAddr = HI_MPI_MMZ_Map(stAiBufInfo.stAiBuf.tPhyBaseAddr, HI_FALSE);
...
ERR_MMAP:
    HI_MPI_MMZ_Unmap(stAiBufInfo.stAiBuf.tPhyBaseAddr);
ERR_CREAT:
...
HI_S32 HI_MPI_AI_Destroy(HI_HANDLE hAI)
{
    ...
    {
        (HI_VOID)HI_MPI_MMZ_Unmap(stAiBufInfo.stAiBuf.tPhyBaseAddr);
    }
