/******************************************************************************
  ...
  Copyright (C), 2009-2018, Hisilicon Tech. Co., Ltd.
...
#define  AVPLAY_ES_RENDER_FULL_PERCENT  98
#define  AVPLAY_ES_RENDER_HIGH_PERCENT  60
#define  AVPLAY_ES_RENDER_LOW_PERCENT    3
#define  AVPLAY_ES_RENDER_EMPTY_PERCENT  0
#define  AVPLAY_ES_RENDER_EMPTY_SIZE    (1 * 1024)
...
/*Defines commond to keep SPS/PPS info*/
#define  AVPLAY_VID_KEEP_SPS_PPS_INFO_CMD  0x29
...
#define  AVPLAY_HDCP_RESOLUTION_UNRESTRICTED  0xffff
#define  AVPLAY_HDCP_RESOLUTION_MUTE    0xfffe

#define  AVPLAY_MAX_AD_BALANCE_VALUE    100

#define  AVPLAY_CPU_SCALE_MIN_FREQ      1200000

#define  AVPLAY_AD_AUD_SYNC_STOP_PLUS       (200)
#define  AVPLAY_AD_AUD_SYNC_STOP_NEGATIVE   (-200)
#define  AVPLAY_AD_AUD_SYNC_START_PLUS      (300)
#define  AVPLAY_AD_AUD_SYNC_START_NEGATIVE  (-300)
#define  AVPLAY_AD_AUD_SYNC_PTS_MAX_DIFF    (3000)
#define  AVPLAY_FRAME_RATE_CHECK_MAXTIME    (0xffffffffffffffff)
#define  AVPLAY_FRAME_INFO_MAXTCONUT        (16)
...
#define  AVPLAY_HDR_CHN_CHECK(HDRChn) \
...
#if defined(HI_AUD_SONIC_SUPPORT)
#if    defined(CHIP_TYPE_hi3716mv450) \
    || defined(CHIP_TYPE_hi3796mv200) \
    || defined(CHIP_TYPE_hi3798cv200) \
    || defined(CHIP_TYPE_hi3798mv200) \
    ...
    #define AVPLAY_AUDIO_TPLAY_SUPPORT
#endif
#endif
    ...
    #define AVPLAY_SCENE_MODE_SUPPORT
#endif
...
#define AVPLAY_USER_DATA_EXTERN_SUPPORT
#endif
...
typedef enum
{
    ...
    AVPLAY_PROC_ADEC_AD,
    AVPLAY_PROC_AO_AD,
    AVPLAY_PROC_DMX_MAIN_TRACK,
    AVPLAY_PROC_DMX_ASSOC_TRACK,
    AVPLAY_PROC_BUTT
}AVPLAY_PROC_ID_E;

typedef enum
{
    AVPLAY_AUDIO_TYPE_NORMAL,
    AVPLAY_AUDIO_TYPE_RENDER,
    AVPLAY_AUDIO_TYPE_BUTT
}AVPLAY_AUDIO_TYPE_E;
...
typedef struct
{
    ...
    HI_U32                     AcquireADAudFrameNum;
    HI_U32                     AcquiredADAudFrameNum;
    HI_U32                     SendADAudFrameNum;
    HI_U32                     SendedADAudFrameNum;
    ...
    HI_U32                     VidFrameDropNum;
}AVPLAY_DEBUG_INFO_S;

typedef struct
{
    HI_U32  Pts;
    HI_BOOL AudSyncAdjust;
}MULTIAUD_SYNC_INFO_S;
...
typedef union
{
    ...
    HI_DRV_VIDEO_HLG_INFO_S   stHlgInfo;
}AVPLAY_HDR_METADATA_U;

typedef struct
{
    AVPLAY_HDR_TYPE_E           enHdrType;
    HI_DRV_COLOR_SPACE_E        enColorSpace;
    HI_DRV_BT2020_LUMINACE_SYS_TYPE_E  enLumaSys;
    HI_DRV_COLOUR_DESCRIPTION_INFO_S ColourDesInfo;
    AVPLAY_HDR_METADATA_U       unHdrMetadata;
}AVPLAY_HDR_INFO_S;

/***VP9 HDR***/
typedef struct tagHDR_INFO_CSC_INPUT_S
{
    HI_U8                   u8ColourPrimaries;
    HI_U8                   u8MatrixCoeffs;
    HI_BOOL                 bFullRangeFlag;
    HI_DRV_GAMMA_TYPE_E     enGammaType;
}HDR_INFO_CSC_INPUT_S;

typedef struct tagHDR_INFO_CSC_OUTPUT_S
{
    HI_DRV_COLOR_SPACE_E        enColorSpace;
    HI_DRV_BT2020_LUMINACE_SYS_TYPE_E  enLumaSys;
}HDR_INFO_CSC_OUTPUT_S;

typedef struct tagHDR_INFO_GAMMA_INPUT_S
{
    HI_U8               u8TransferCharacteristics;
    HI_U8               u8PreferredTransferCharacteristics;
}HDR_INFO_GAMMA_INPUT_S;

typedef struct tagHDR_INFO_GAMMA_OUTPUT_S
{
    HI_DRV_GAMMA_TYPE_E enGammaType;
    HI_BOOL             bBackwardCompatible;
}HDR_INFO_GAMMA_OUTPUT_S;

typedef struct tagHDR_METADATA_INFO_S
{
...
typedef struct
{
    HI_BOOL                         bJumpPlayMode;
    HI_BOOL                         bVidFirstPlay;
    HI_BOOL                         bAudFirstPlay;
    HI_U32                          IdrPts;
    HI_U32                          TargetPts;
} AVPLAY_JUMP_PLAY_INFO_S;

#ifdef AVPLAY_UVMOS_SUPPORT
typedef enum
{
    AVPLAY_DECODER_BUFFER_NORMAL = 0x0,
    AVPLAY_DECODER_BUFFER_OVERFLOW,
    AVPLAY_DECODER_BUFFER_UNDERFLOW,
    AVPLAY_DECODER_BUFFER_ABNORMAL,

    AVPLAY_DECODER_BUFFER_BUTT
}AVPLAY_DECODER_BUFFER_E;

typedef struct
{
    HI_UNF_VIDEO_FRM_STATUS_INFO_S  FrameStatus;
    struct list_head                list;
}AVPLAY_VIDEO_FRM_STATUS_INFO_S;
#endif
...
typedef struct
{
    ...
    HI_UNF_ACODEC_ATTR_S            stExternalAudAttr;
    HI_BOOL                         bCurExternalAud;
    HI_UNF_ACODEC_ATTR_S            stNormalAudAttr;
    ...
    HI_BOOL                         AvplayProcStateContinue;        /*flag for state thread continue*/
    ...
    HI_BOOL                         bStepMode;
    HI_BOOL                         bStepPlay;
    HI_BOOL                         bTplayResetMode;    /*whether reset avplay before enter tplay mode*/
    ...
    HI_UNF_AVPLAY_BUF_STATE_E       CurVidBufState;     /*the status of video es buffer when CheckBuf*/
    HI_UNF_AVPLAY_BUF_STATE_E       CurAudBufState;     /*the status of audio es buffer when CheckBuf*/
    ...
    HI_BOOL                         bKeepSPSPPSFlag;        /*is keep sps/pps info*/
    ...
    AVPLAY_AUDIO_TYPE_E             AvplayAudioType;

#ifdef HI_HDR_SUPPORT
    AVPLAY_HDR_INFO_S               stAvplayHdrInfo;
    HI_UNF_VIDEO_HDR_INFO_S         stHdrInfo;
    HI_BOOL                         bPaddingHdrInfo;
    HI_BOOL                         bSetHdrInfo;
#endif
    ...
    HI_HANDLE                       hDmxAdAud;
    HI_HANDLE                       hAdAdec;
    HI_UNF_AO_FRAMEINFO_S           AvplayAdAudFrm;
    HI_UNF_STREAM_BUF_S             AvplayAdAudEsBuf;
    HI_UNF_ES_BUF_S                 AvplayAdDmxEsBuf;
    HI_MPI_ES_EXT_DATA_S            AudEsExtData;
    SYNC_AUD_OPT_S                  AdAudSyncOpt;

    HI_BOOL                         bBoostCpuFreq;
    HI_BOOL                         bSetCpuMinFreq;
    ...
    MULTIAUD_SYNC_INFO_S            MultiAudSyncInfo[AVPLAY_MAX_DMX_AUD_CHAN_NUM];

    AVPLAY_JUMP_PLAY_INFO_S         AvplayJumpPlayInfo;
    HI_BOOL                         bAudTplay;
    HI_BOOL                         bAudTplayEnable;

    HI_BOOL                         bSetProgressive;

    HI_BOOL                         bJumpPlayFinish;
    HI_BOOL                         bQueuedVideoFrame;
    HI_BOOL                         bAcquiredVideoFrame;
    HI_BOOL                         bAcquiredAudFrame;

    HI_BOOL                         bSyncFinish;
#ifdef AVPLAY_UVMOS_SUPPORT
    HI_BOOL                         bUVMOSEnable;
    HI_BOOL                         bNormalStatus;
    HI_U32                          ListEmptyCount;
    HI_U32                          ListAvailableCount;
    HI_U32                          FrameTimeSameCount;
    HI_U32                          ValidFrameRate;     /*1/100 ms*/
    HI_U32                          CurFrameRate;       /*1/100 ms*/
    HI_U64                          LastFrameTime;      /*1/100 ms*/
    HI_UNF_AVPLAY_FRMRATE_TYPE_E    FramRateType;
    AVPLAY_VIDEO_FRM_STATUS_INFO_S  FrameInfoAvailablelistHead;
    AVPLAY_VIDEO_FRM_STATUS_INFO_S  FrameInfoEmptylistHead;
#endif
} AVPLAY_S;
...
static HI_CHAR *g_AvplayVidType[HI_UNF_VCODEC_TYPE_BUTT] =
{
    ...
    "INDEO4",  "INDEO5",     "MJPEGB",     "MVC",       "HEVC",      "DV",       "VP9",     "AVS2"
};
#endif
...
HI_VOID AVPLAY_Usleep(HI_U32 uTime)
{
    HI_S32 Ret      = HI_FAILURE;
    struct timespec stSlpTm;

    stSlpTm.tv_sec = 0;
    stSlpTm.tv_nsec = uTime * 1000;    //1000 ns = 1 us
    Ret = nanosleep(&stSlpTm, NULL);
    if(0 != Ret)
    {
        HI_ERR_AVPLAY("nanosleep err.\n");
    }
}
...
HI_S32 AVPLAY_GetStatusInfo(const AVPLAY_S *pAvplay, HI_UNF_AVPLAY_STATUS_INFO_S *pstStatusInfo)
{
    ...
    ADEC_DEBUGINFO_S             AdecDebugInfo = {0};
    VDEC_STATUSINFO_S            VdecBufStatus = {0};
    ADEC_BUFSTATUS_S             RenderBufStatus = {0};
    HI_MPI_AO_RENDER_ALLINFO_S   RenderAllInfo = {0};
    ...
    if ((AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType) && pAvplay->AudEnable)
    {
        memset(&RenderBufStatus, 0, sizeof(ADEC_BUFSTATUS_S));
        memset(&RenderAllInfo, 0, sizeof(HI_MPI_AO_RENDER_ALLINFO_S));

        (HI_VOID)HI_MPI_AO_Track_GetInfo(pAvplay->hRenderTrack, TRACK_INFO_BUFFERSTATUS, &RenderBufStatus);
        (HI_VOID)HI_MPI_AO_Track_GetInfo(pAvplay->hRenderTrack, TRACK_INFO_ALLINFO, &RenderAllInfo);
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_AUD].u32BufId = HI_UNF_AVPLAY_BUF_ID_ES_AUD;
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_AUD].u32BufSize = RenderBufStatus.u32BufferSize;
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_AUD].u32UsedSize = RenderBufStatus.u32BufferUsed;
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_AUD].u32CurrentAvailableSize = RenderBufStatus.u32BufferAvailable;
        pstStatusInfo->u32AuddFrameCount = RenderBufStatus.u32TotDecodeFrame;
        pstStatusInfo->u32AudErrorFrameCount = RenderAllInfo.u32ErrDecFrameNum;
    }
    else if ((HI_INVALID_HANDLE != pAvplay->hAdec) && pAvplay->AudEnable)
    {
        ...
        (HI_VOID)HI_MPI_ADEC_GetInfo(pAvplay->hAdec, HI_MPI_ADEC_DEBUGINFO, &AdecDebugInfo);
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_AUD].u32BufId = HI_UNF_AVPLAY_BUF_ID_ES_AUD;
        ...
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_AUD].u32CurrentAvailableSize = AdecBufStatus.u32BufferAvailable;
        ...
        pstStatusInfo->u32AudErrorFrameCount = AdecDebugInfo.u32ErrDecFrameNum;
    }
    else
    {
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_AUD].u32BufId = HI_UNF_AVPLAY_BUF_ID_ES_AUD;
        ...
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_AUD].u32CurrentAvailableSize = 0;
        ...
        pstStatusInfo->u32AudErrorFrameCount = 0;
    }

    if (   (HI_INVALID_HANDLE != pAvplay->hSyncTrack)
        && (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType) )
    {
    ...
    if ((HI_INVALID_HANDLE != pAvplay->hVdec) && pAvplay->VidEnable)
    {
        ...
        if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
        {
            ...
            pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_VID].u32CurrentAvailableSize = VidChnBuf.u32BufSize - VidChnBuf.u32UsedSize;
        ...
        pstStatusInfo->u32VidFrameDropCount = pAvplay->DebugInfo.VidFrameDropNum;
        pstStatusInfo->u32AudFrameDropCount = 0;
    }
    else
    {
        pstStatusInfo->stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_VID].u32BufSize = 0;
...
HI_VOID AVPLAY_Notify(const AVPLAY_S *pAvplay, HI_UNF_AVPLAY_EVENT_E EvtMsg, HI_VOID *pEvtPara)
{
    ...
    else
    {
        ...
        switch (EvtMsg)
        {
            ...
            case HI_UNF_AVPLAY_EVENT_VID_START_POS :
            {
                if (pEvtPara)
                {
...
HI_UNF_AVPLAY_BUF_STATE_E AVPLAY_CaclBufState(const AVPLAY_S *pAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_E enChn, HI_U32 UsedBufPercent)
{
    ...
    HI_U32                             u32AoBufTime = 0;
    ADEC_STATUSINFO_S                  stAdecStatusinfo;

    memset(&WinInfo, 0x0, sizeof(HI_DRV_WIN_PLAY_INFO_S));
    memset(&stAdecStatusinfo, 0x0, sizeof(ADEC_STATUSINFO_S));
    ...
    if (HI_UNF_AVPLAY_MEDIA_CHAN_AUD == enChn)
    {
        ...
        {
            (HI_VOID)HI_MPI_ADEC_GetInfo(pAvplay->hAdec, HI_MPI_ADEC_STATUSINFO, &stAdecStatusinfo);

            if (HI_INVALID_HANDLE != pAvplay->hSyncTrack)
            {
                (HI_VOID)HI_MPI_AO_Track_GetDelayMs(pAvplay->hSyncTrack, &u32AoBufTime);
            }
            else
            {
                u32AoBufTime = pAvplay->AudInfo.BufTime;
            }

            u32FrmTime = stAdecStatusinfo.u32UsedBufNum * pAvplay->AudInfo.FrameTime;

            if (UsedBufPercent < AVPLAY_ES_AUD_EMPTY_PERCENT)
            {
                if (u32AoBufTime + u32FrmTime <= 150)
                {
            ...
            else
            {
                ...
                else if (u32AoBufTime + u32FrmTime <= 240)
                {
                    CurBufState = HI_UNF_AVPLAY_BUF_STATE_LOW;
                }
            }
...
static HI_VOID AVPLAY_ProcAdecJumpPlay(AVPLAY_S *pAvplay)
{
    HI_S32               Ret;
    HI_U32               AoBufTime = 0;
    ADEC_EXTFRAMEINFO_S  AdecExtInfo = {0};
    ADEC_STATUSINFO_S    AdecStatusinfo = {0};
    ...
    Ret = HI_MPI_AO_Track_GetDelayMs(pAvplay->hSyncTrack, &AoBufTime);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("Call HI_MPI_AO_Track_GetDelayMs fail 0x%x\n", Ret);
        return;
    }

    Ret = HI_MPI_ADEC_GetInfo(pAvplay->hAdec, HI_MPI_ADEC_STATUSINFO, &AdecStatusinfo);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("Call HI_MPI_ADEC_GetInfo fail 0x%x\n", Ret);
        return;
    }
    ...
    if (HI_INVALID_HANDLE == pAvplay->hSyncTrack)
    {
        HI_INFO_AVPLAY("Sync track is no attach, release this aud frame\n");
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("Call HI_MPI_SYNC_AudJudge fail 0x%x\n", Ret);
    ...
    if (   (pAvplay->AudInfo.Pts == HI_INVALID_PTS)
        || (pAvplay->AudInfo.Pts < pAvplay->AvplayJumpPlayInfo.TargetPts) )
    {
        pAvplay->AvplayProcAudContinue = HI_TRUE;
        (HI_VOID)HI_MPI_ADEC_ReleaseFrame(pAvplay->hAdec, &pAvplay->AvplayAudFrm);
...
HI_VOID AVPLAY_ProcAdecToAo(AVPLAY_S *pAvplay)
{
    ...
    if (pAvplay->AvplayJumpPlayInfo.bJumpPlayMode)
    {
        AVPLAY_ProcAdecJumpPlay(pAvplay);
        return;
    }
    ...
    pAvplay->bAcquiredAudFrame = HI_FALSE;
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_ADEC_AO])
    {
        ...
        if (HI_SUCCESS == Ret)
        {
            ...
            pAvplay->bAcquiredAudFrame = HI_TRUE;
        }
        else
        {
        }
    ...
    if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_ADEC_AO])
    {
        if ((HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus) && (!pAvplay->bAudTplay))
        {
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_ADEC_AO] = HI_FALSE;
            pAvplay->AvplayProcAudContinue = HI_TRUE;
            (HI_VOID)HI_MPI_ADEC_ReleaseFrame(pAvplay->hAdec, &pAvplay->AvplayAudFrm);
        ...
        else
        {
            if (HI_INVALID_HANDLE != pAvplay->hSyncTrack)
            {
                ...
                if(HI_SUCCESS != Ret)
                {
                    HI_INFO_AVPLAY("HI_MPI_AO_Track_GetDelayMs failed Ret:0x%x .\n",Ret);
                }
                ...
                if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
                {
                    if (pAvplay->FrcParamCfg.u32PlayRate <= (1 * AVPLAY_ALG_FRC_BASE_PLAY_RATIO))
                    {
                        pAvplay->AudInfo.BufTime = AoBufTime * pAvplay->FrcParamCfg.u32PlayRate / AVPLAY_ALG_FRC_BASE_PLAY_RATIO;
                        HI_INFO_AVPLAY("Orig delay:%d ms, Dest delay:%d ms.\n", AoBufTime, pAvplay->AudInfo.BufTime);
                    }
                }
                ...
                if (HI_SUCCESS == Ret)
                {
                    ...
                    else if (SYNC_PROC_BLOCK == pAvplay->AudOpt.SyncProc)
                    {
                        return;
                    }
...
HI_S32 AVPLAY_ProcAdAudSync(AVPLAY_S *pAvplay, SYNC_AUD_OPT_S  *pstAdAudOpt)
{
    ...
    HI_S32 TargetPts = pAvplay->AvplayAudFrm.u32PtsMs;
    HI_S32 AdAudPts  = pAvplay->AvplayAdAudFrm.u32PtsMs;

    if (HI_INVALID_PTS == TargetPts || HI_INVALID_PTS == AdAudPts)
    {
        pstAdAudOpt->SyncProc = SYNC_PROC_PLAY;
        HI_INFO_AVPLAY("SYNC_PROC_REPEAT\n");
        return HI_SUCCESS;
    }

    AdPtsDiff = AdAudPts - TargetPts;

    if (abs(AdPtsDiff) > AVPLAY_AD_AUD_SYNC_PTS_MAX_DIFF)
    {
        pstAdAudOpt->SyncProc = SYNC_PROC_PLAY;
        return HI_SUCCESS;
    }

    if (   (AdPtsDiff < AVPLAY_AD_AUD_SYNC_STOP_PLUS)
        && (AdPtsDiff > AVPLAY_AD_AUD_SYNC_STOP_NEGATIVE))
    {
        pstAdAudOpt->SyncProc = SYNC_PROC_PLAY;
    }

    if (AdPtsDiff > AVPLAY_AD_AUD_SYNC_START_PLUS)
    {
        pstAdAudOpt->SyncProc = SYNC_PROC_REPEAT;
        HI_INFO_AVPLAY("SYNC_PROC_REPEAT AdAudPts:%d, TargetPts:%d\n", AdAudPts, TargetPts);
    }

    if (AdPtsDiff < AVPLAY_AD_AUD_SYNC_START_NEGATIVE)
    {
        pstAdAudOpt->SyncProc = SYNC_PROC_DISCARD;
        HI_INFO_AVPLAY("SYNC_PROC_DISCARD AdAudPts:%d, TargetPts:%d\n", AdAudPts, TargetPts);
    }

    if (SYNC_PROC_REPEAT == pstAdAudOpt->SyncProc)
    {
        if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
        {
            Ret = HI_MPI_DMX_GetPESBufferStatus(pAvplay->hDmxAdAud, &AudChnBuf);
            if (HI_SUCCESS == Ret)
            {
                ...
                if ((AudBufPercent > 85) && (AdPtsDiff > AVPLAY_AD_AUD_SYNC_PTS_MAX_DIFF))
                {
                    pstAdAudOpt->SyncProc = SYNC_PROC_PLAY;
                    HI_WARN_AVPLAY("Ad AudBufPercent > 85 play\n");
                }
            }
        }
    }
...
HI_VOID AVPLAY_ProcAdAdecToAo(AVPLAY_S *pAvplay)
{
    HI_S32                    Ret = HI_SUCCESS;
    ADEC_EXTFRAMEINFO_S       AdecExtInfo = {0};
    HI_U32                    i;

    if (!pAvplay->AudEnable || HI_INVALID_HANDLE == pAvplay->hAdAdec)
    {
        return;
    }
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_AO_AD])
    {
        pAvplay->DebugInfo.AcquireADAudFrameNum++;

        Ret = HI_MPI_ADEC_ReceiveFrame(pAvplay->hAdAdec, &pAvplay->AvplayAdAudFrm, &AdecExtInfo);
        if (HI_SUCCESS == Ret)
        {
            pAvplay->DebugInfo.AcquiredADAudFrameNum++;
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_AO_AD] = HI_TRUE;
        }
    }
    ...
    if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_AO_AD])
    {
        if ((HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus) && (!pAvplay->bAudTplay))
        {
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_AO_AD] = HI_FALSE;
            pAvplay->AvplayProcAudContinue = HI_TRUE;
            (HI_VOID)HI_MPI_ADEC_ReleaseFrame(pAvplay->hAdAdec, &pAvplay->AvplayAdAudFrm);
        }
        else
        {
            Ret = AVPLAY_ProcAdAudSync(pAvplay, &pAvplay->AdAudSyncOpt);
            if (HI_SUCCESS == Ret)
            {
                if (SYNC_PROC_DISCARD == pAvplay->AdAudSyncOpt.SyncProc)
                {
                    pAvplay->AvplayProcDataFlag[AVPLAY_PROC_AO_AD] = HI_FALSE;
                    pAvplay->AvplayProcAudContinue = HI_TRUE;
                    (HI_VOID)HI_MPI_ADEC_ReleaseFrame(pAvplay->hAdAdec, &pAvplay->AvplayAdAudFrm);
                ...
                else if (SYNC_PROC_REPEAT == pAvplay->AdAudSyncOpt.SyncProc)
                {
                    return;
                }
            }

            if (HI_INVALID_HANDLE != pAvplay->hSyncTrack && pAvplay->AdAttr.bAdEnble)
            {
                pAvplay->DebugInfo.SendADAudFrameNum++;

                /*send frame to main track*/
                Ret = HI_MPI_AO_Track_SendData_AD(pAvplay->hSyncTrack, &pAvplay->AvplayAdAudFrm);
                if (HI_SUCCESS == Ret)
                {
                    /*send frame to other track*/
                    for(i=0; i<pAvplay->TrackNum; i++)
                    {
                        if (pAvplay->hSyncTrack != pAvplay->hTrack[i])
                        {
                            (HI_VOID)HI_MPI_AO_Track_SendData_AD(pAvplay->hTrack[i], &pAvplay->AvplayAdAudFrm);
                        }
                    }

                    pAvplay->DebugInfo.SendedADAudFrameNum++;
                }
                else if (  HI_ERR_AO_OUT_BUF_FULL == Ret
                        || HI_ERR_AO_SENDMUTE == Ret
                        || HI_ERR_AO_PAUSE_STATE == Ret)
                {
                    HI_WARN_AVPLAY("call HI_MPI_AO_Track_SendData_AD failed :%#x\n", Ret);
                    return;
                }
            }
...
HI_VOID AVPLAY_ProcRenderTrackPreSync(AVPLAY_S *pAvplay)
{
    HI_S32                  Ret;
    SYNC_STAT_INFO_S        SyncInfo = {0};
    static HI_BOOL          bLastPreSyncTargetInit = HI_FALSE;

    Ret = HI_MPI_SYNC_GetSyncInfo(pAvplay->hSync, &SyncInfo);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("HI_MPI_SYNC_GetPreSyncStatus failed %#x\n", Ret);
        return;
    }

    pAvplay->bPreSyncFinish = SyncInfo.bPreSyncFinish;

    if ((HI_TRUE == SyncInfo.bPreSyncTargetInit) && (bLastPreSyncTargetInit != SyncInfo.bPreSyncTargetInit))
    {
        Ret = HI_MPI_AO_Track_SetTargetPts(pAvplay->hRenderTrack, SyncInfo.u32TargetPts);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_AO_Track_SetTargetPts failed %#x\n", Ret);
        }
    }

    bLastPreSyncTargetInit = SyncInfo.bPreSyncTargetInit;

    return;
}

HI_VOID AVPLAY_ProcRenderTrackPlay(AVPLAY_S *pAvplay)
{
    HI_S32                  Ret;

    if (HI_FALSE == pAvplay->bRenderOut)
    {
        if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
        {
             return;
        ...
        Ret = HI_MPI_AO_Track_SetSyncMode(pAvplay->hRenderTrack, HI_MPI_AO_TRACK_SYNCMODE_STARTOUTPUT);
        if (HI_SUCCESS == Ret)
        {
            pAvplay->bRenderOut = HI_TRUE;
            HI_WARN_AVPLAY("set Renker track SyncMode to START!\n");
        }
        else
        {
            HI_ERR_AVPLAY("call HI_MPI_AO_Track_SetSyncMode fail 0x%x\n", Ret);
            return;
        }
    }
...
HI_VOID AVPLAY_ProcRenderTrackDiscard(AVPLAY_S *pAvplay)
{
    HI_S32                  Ret;
    HI_U32                  u32TargetPts = 0;

    Ret = HI_MPI_AO_Track_SetSyncMode(pAvplay->hRenderTrack, HI_MPI_AO_TRACK_SYNCMODE_PAUSEOUTPUT);
    if (HI_SUCCESS == Ret)
    {
        pAvplay->bRenderOut = HI_FALSE;
        HI_WARN_AVPLAY("set Renker track SyncMode to PAUSE!\n");

        if (pAvplay->VidInfo.Pts== 0)
        {
            u32TargetPts = pAvplay->AudInfo.Pts;
        }
        else
        {
            u32TargetPts = pAvplay->VidInfo.Pts;
        }

        Ret = HI_MPI_AO_Track_SetTargetPts(pAvplay->hRenderTrack, u32TargetPts);
        if (HI_SUCCESS != Ret)
        {
            HI_INFO_AVPLAY("HI_MPI_AO_Track_DropFrame fail 0x%x\n", Ret);
            return;
        }
    }
...
HI_VOID AVPLAY_ProcRenderTrackRepeat(AVPLAY_S *pAvplay)
{
    HI_S32                  Ret;
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_INFO_AVPLAY("HI_MPI_AO_Track_SetSyncMode fail 0x%x\n", Ret);
        return;
    }
    else
    {
        ...
        Ret = HI_MPI_AO_Track_SetRepeatTargetPts(pAvplay->hRenderTrack);
        if (HI_SUCCESS != Ret)
        {
            HI_INFO_AVPLAY("HI_MPI_AO_Track_DropFrame fail 0x%x\n", Ret);
            return;
...
HI_VOID AVPLAY_ProcRenderTrackSync(AVPLAY_S *pAvplay)
{
    switch (pAvplay->AudOpt.SyncProc)
    {
        case SYNC_PROC_DISCARD :
        {
            AVPLAY_ProcRenderTrackDiscard(pAvplay);
            break;
        }

        case SYNC_PROC_REPEAT :
        {
            AVPLAY_ProcRenderTrackRepeat(pAvplay);
            break;
        }

        case SYNC_PROC_PLAY :
        {
            AVPLAY_ProcRenderTrackPlay(pAvplay);
            break;
        }

        default:
            HI_INFO_AVPLAY("Cur Audio Opt=%d\n", pAvplay->AudOpt.SyncProc);
            return;
    }
}

static HI_VOID AVPLAY_ProcRenderJumpPlay(AVPLAY_S *pAvplay)
{
    HI_S32 Ret = HI_FAILURE;
    HI_U32 Pts = HI_INVALID_PTS;

    if (pAvplay->AvplayJumpPlayInfo.bAudFirstPlay)
    {
        Ret = HI_MPI_AO_Track_SetSyncMode(pAvplay->hRenderTrack, HI_MPI_AO_TRACK_SYNCMODE_PAUSEOUTPUT);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_AVPLAY("HI_MPI_AO_Track_SetSyncMode fail 0x%x\n", Ret);
        }

        pAvplay->bRenderOut = HI_FALSE;

        Ret = HI_MPI_AO_Track_SetTargetPts(pAvplay->hRenderTrack, pAvplay->AvplayJumpPlayInfo.TargetPts);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_AVPLAY("HI_MPI_AO_Track_SetTargetPts fail 0x%x\n", Ret);
        }

        pAvplay->AvplayJumpPlayInfo.bAudFirstPlay = HI_FALSE;

        return;
    }

    Ret = HI_MPI_AO_Track_GetPts(pAvplay->hRenderTrack, &Pts);
    if (HI_SUCCESS != Ret)
    {
        HI_WARN_AVPLAY("get Pts for AO Render Track failed\n");
        return;
    }

    pAvplay->AudInfo.SrcPts    = Pts;
    pAvplay->AudInfo.Pts       = Pts;
    pAvplay->AudInfo.FrameTime = 24;
    pAvplay->AudInfo.BufTime   = 0;
    pAvplay->AudInfo.FrameNum  = 10;
    Ret = HI_MPI_SYNC_AudJudge(pAvplay->hSync, &pAvplay->AudInfo, &pAvplay->AudOpt);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("Call HI_MPI_SYNC_AudJudge fail 0x%x\n", Ret);
...
HI_VOID AVPLAY_ProcRenderTrack(AVPLAY_S *pAvplay)
{
    HI_S32                  Ret = HI_FAILURE;
    HI_U32                  Pts = HI_INVALID_PTS;
    ...
    if (pAvplay->AvplayJumpPlayInfo.bJumpPlayMode)
    {
        AVPLAY_ProcRenderJumpPlay(pAvplay);
        return;
    }
    ...
    if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
    {
        memset(&SyncStatus,0x0,sizeof(HI_UNF_SYNC_STATUS_S));
        ...
        Ret = HI_MPI_AO_Track_SetTargetPts(pAvplay->hRenderTrack, SyncStatus.u32LastVidPts);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_AVPLAY("HI_MPI_AO_Track_SetTargetPts fail 0x%x\n", Ret);
        }
        HI_WARN_AVPLAY("set Renker track pts to:%d\n", SyncStatus.u32LastVidPts);
        return;
    }
    ...
    if (HI_FALSE == pAvplay->bPreSyncFinish)
    {
        AVPLAY_ProcRenderTrackPreSync(pAvplay);
        pAvplay->bRenderOut = HI_FALSE;
        return;
    }
    else
    {
        AVPLAY_ProcRenderTrackSync(pAvplay);
        return;
    }
...
HI_VOID AVPLAY_ProcFirstDispFrame(AVPLAY_S *pAvplay)
{
    ...
    if (!pAvplay->bVidFirstQueueFrame)
    {
        ...
        if (HI_SUCCESS == Ret)
        {
            pAvplay->VidFirstFrameDispTime = WinInfo.u32DelayTime; //Using delaytime can ensure that first frame is displayed.
        }
...
HI_VOID AVPLAY_ProcVidFrameRate(AVPLAY_S *pAvplay, HI_DRV_VDEC_FRAME_S *pVdecFrameInfo)
{
   /*
    ...
    */
    ...
    Ret = AVPLAY_GetWindowByPort(pAvplay, pVdecFrameInfo->hport, &hWindow);
    if (HI_SUCCESS != Ret)
    {
        return;
    }

    Ret = HI_MPI_WIN_GetPlayInfo(hWindow, &WinInfo);
    if (HI_SUCCESS != Ret)
    {
        return;
    }

    pVdecFrameInfo->stFrameVideo.u32FrameRate = WinInfo.u32DispRate * 10;
#endif

    return;
}

HI_VOID AVPLAY_NotifyVidFrmDisplayInfo(AVPLAY_S *pAvplay, HI_HANDLE hWindow, HI_U32 u32PTS)
{
    HI_S32                                      Ret;
    HI_UNF_VIDEO_FRM_DISPLAY_TIME_S             VidfrmInfo;
    HI_DRV_WIN_PLAY_INFO_S                      WinInfo = {0};
    ...
    if(HI_SUCCESS != Ret)
    {
        HI_INFO_AVPLAY("HI_MPI_WIN_GetPlayInfo failed Ret:0x%x\n",Ret);
        return;
    }
    ...
    if(0 != Ret)
    {
        HI_ERR_AVPLAY("clock_gettime failed .\n");
        return;
    }

    VidfrmInfo.s64DisplayTime = (HI_U64)(t_start.tv_sec) * 1000000000 + (HI_U64)t_start.tv_nsec + (HI_U64)(WinInfo.u32DelayTime * 1000);
    VidfrmInfo.s64PTS = u32PTS;
    //HI_INFO_AVPLAY("--que frm info pts %d,se:%d,ns:%llu,time %llu,dl:%d\n",VidfrmInfo.s64PTS,(HI_U32)t_start.tv_sec,(HI_U64)t_start.tv_nsec,VidfrmInfo.s64DisplayTime,WinInfo.u32DelayTime);
    AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_FRAME_DISPLAY_TIME, &VidfrmInfo);
...
HI_VOID AVPLAY_ProcVidFrc(AVPLAY_S *pAvplay)
{
    ...
    if(HI_SUCCESS != Ret)
    {
        HI_INFO_AVPLAY("HI_MPI_WIN_GetPlayInfo failed 0x%x\n",Ret);
    }
...
#ifdef AVPLAY_UVMOS_SUPPORT
    pAvplay->FramRateType = FrameRate.enFrmRateType;
#endif
    ...
    if (FrameRate.enFrmRateType == HI_UNF_AVPLAY_FRMRATE_TYPE_USER)
    {
        ...
        {
            pAvplay->FrcParamCfg.u32InRate = (FrameRate.stSetFrmRate.u32fpsInteger * 1000
                                         + FrameRate.stSetFrmRate.u32fpsDecimal) * 2;
        }
        else
        {
            pAvplay->FrcParamCfg.u32InRate = FrameRate.stSetFrmRate.u32fpsInteger * 1000
                                         + FrameRate.stSetFrmRate.u32fpsDecimal;
        }
...
HI_VOID AVPLAY_ProcVidSync(AVPLAY_S *pAvplay)
{
    ...
    SYNC_STAT_INFO_S                    SyncInfo = {0};
    HI_UNF_SYNC_VID_TPLAYTIME_S         stVidPlaytime;

    memset(&WinInfo, 0x0, sizeof(HI_DRV_WIN_PLAY_INFO_S));

    /*current frame need't display(add for uvmos)*/
    if (pAvplay->CurFrmPack.stFrame[0].stFrameVideo.bDiscard)
    {
        pAvplay->VidOpt.SyncProc = SYNC_PROC_DISCARD;
        HI_WARN_AVPLAY("current frame need't display\n");
        return;
    }
    ...
    if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
    {
        if (pAvplay->FrcParamCfg.u32PlayRate <= (1 * AVPLAY_ALG_FRC_BASE_PLAY_RATIO))
        {
           WinInfo.u32DelayTime = WinInfo.u32DelayTime * pAvplay->FrcParamCfg.u32PlayRate / AVPLAY_ALG_FRC_BASE_PLAY_RATIO;
        }
    }
    ...
    {
        stVidPlaytime.u32PrivDispTime = pFrmPriv->u32PrivDispTime;
        stVidPlaytime.u32Pts = pAvplay->VidInfo.Pts;
        stVidPlaytime.enSyncProc = pAvplay->VidOpt.SyncProc;
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_VID_TPLAYTIME_SYNC, (HI_VOID *)&stVidPlaytime);
        pAvplay->VidOpt.SyncProc = stVidPlaytime.enSyncProc;
    }
    ...
    (HI_VOID)HI_MPI_SYNC_GetSyncInfo(pAvplay->hSync, &SyncInfo);
    pAvplay->bSyncFinish = SyncInfo.bSyncFinish;
...
HI_VOID AVPLAY_ProcVidPlay(AVPLAY_S *pAvplay)
{
    ...
    AVPLAY_ProcVidFrameRate(pAvplay, &pAvplay->CurFrmPack.stFrame[i]);
    ...
    if (HI_SUCCESS == Ret)
    {
        ...
        pAvplay->bQueuedVideoFrame = HI_TRUE;
        pAvplay->FrcCurPlayCnt++;
        pAvplay->DebugInfo.MasterVidStat.PlayNum++;

        pAvplay->LstFrmPack.u32FrmNum = pAvplay->CurFrmPack.u32FrmNum;
        memcpy(&pAvplay->LstFrmPack.stFrame[i], &pAvplay->CurFrmPack.stFrame[i], sizeof(HI_DRV_VDEC_FRAME_S));
    }
    else if (HI_ERR_VO_BUFQUE_FULL != Ret)
    {
        ...
        pAvplay->DebugInfo.VidFrameDropNum++;
    }
    else
    {
        ...
        for (j = 0; j < AVPLAY_MAX_SLAVE_FRMCHAN; j++)
        {
            if (hWindow == pAvplay->SlaveFrmChn[j].hWindow)
            {
                ...
                if (HI_SUCCESS == Ret)
                {
                    ...
                    pAvplay->LstFrmPack.u32FrmNum = pAvplay->CurFrmPack.u32FrmNum;
                    memcpy(&pAvplay->LstFrmPack.stFrame[j], &pAvplay->CurFrmPack.stFrame[j], sizeof(HI_DRV_VDEC_FRAME_S));
                }
                else if (HI_ERR_VO_BUFQUE_FULL != Ret)
                {
                ...
                {
                    pAvplay->AvplayProcVidContinue = HI_FALSE;
                    if((HI_FALSE == pAvplay->bSendedFrmToSlaveWin[j]) && (pAvplay->FrcCurPlayCnt >= pAvplay->FrcNeedPlayCnt))
                    {
                        Ret = HI_MPI_VDEC_ReleaseFrame(pAvplay->CurFrmPack.stFrame[i].hport,
                                                       &pAvplay->CurFrmPack.stFrame[i].stFrameVideo);
                        if (HI_SUCCESS != Ret)
...
HI_VOID AVPLAY_ProcVidRepeat(AVPLAY_S *pAvplay)
{
    ...
    AVPLAY_ProcVidFrameRate(pAvplay, &pAvplay->LstFrmPack.stFrame[i]);
...
HI_VOID AVPLAY_ProcVidDiscard(AVPLAY_S *pAvplay)
{
    ...
    if (HI_SUCCESS == Ret)
    {
        if ((0 != pAvplay->FrcNeedPlayCnt) && (HI_TRUE == pAvplay->bSyncFinish))
        {
            pAvplay->DebugInfo.VidFrameDropNum++;
        }
...
HI_VOID AVPLAY_ProcHDRPlay(AVPLAY_S *pAvplay)
{
    ...
    if (HI_TRUE == pAvplay->bDualChnPlay)
    {
        ...
        AVPLAY_ProcVidFrameRate(pAvplay, &pAvplay->CurBlFrmPack.stFrame[BlPortId]);
        AVPLAY_ProcVidFrameRate(pAvplay, &pAvplay->CurElFrmPack.stFrame[ElPortId]);
...
HI_VOID AVPLAY_ProcHDRRepeat(AVPLAY_S *pAvplay)
{
    ...
    if (HI_TRUE == pAvplay->bDualChnPlay)
    {
        ...
        AVPLAY_ProcVidFrameRate(pAvplay, &pAvplay->LstBlFrmPack.stFrame[BlPortId]);
        AVPLAY_ProcVidFrameRate(pAvplay, &pAvplay->LstElFrmPack.stFrame[ElPortId]);
...
#ifdef HI_HDR_SUPPORT
HI_VOID AVPLAY_PaddingHdrInfo2Frame(const AVPLAY_HDR_INFO_S *pstInHdrInfo, HI_DRV_VIDEO_FRAME_S *pstFrameInfo)
{
    HI_DRV_VIDEO_PRIVATE_S *pPrivFrm = (HI_DRV_VIDEO_PRIVATE_S*)pstFrameInfo->u32Priv;

    if (AVPLAY_HDR_TYPE_HDR10 == pstInHdrInfo->enHdrType)
    {
        pstFrameInfo->enSrcFrameType = HI_DRV_VIDEO_FRAME_TYPE_HDR10;
        pstFrameInfo->stColourInfo.u8ColourPrimaries = pstInHdrInfo->ColourDesInfo.u8ColourPrimaries;
        pstFrameInfo->stColourInfo.u8FullRangeFlag = pstInHdrInfo->ColourDesInfo.u8FullRangeFlag;
        pstFrameInfo->stColourInfo.u8MatrixCoeffs = pstInHdrInfo->ColourDesInfo.u8MatrixCoeffs;
        memcpy(&pstFrameInfo->unHDRInfo.stHDR10Info, &pstInHdrInfo->unHdrMetadata.stHdr10Info, sizeof(HI_DRV_VIDEO_HDR10_INFO_S));
        pPrivFrm->eColorSpace = pstInHdrInfo->enColorSpace;
    }
    else if(AVPLAY_HDR_TYPE_HLG == pstInHdrInfo->enHdrType)
    {
        pstFrameInfo->enSrcFrameType = HI_DRV_VIDEO_FRAME_TYPE_HLG;
        pstFrameInfo->stColourInfo.u8ColourPrimaries = pstInHdrInfo->ColourDesInfo.u8ColourPrimaries;
        pstFrameInfo->stColourInfo.u8FullRangeFlag = pstInHdrInfo->ColourDesInfo.u8FullRangeFlag;
        pstFrameInfo->stColourInfo.u8MatrixCoeffs = pstInHdrInfo->ColourDesInfo.u8MatrixCoeffs;
        memcpy(&pstFrameInfo->unHDRInfo.stHLGInfo, &pstInHdrInfo->unHdrMetadata.stHlgInfo, sizeof(HI_DRV_VIDEO_HLG_INFO_S));
        pPrivFrm->eColorSpace = pstInHdrInfo->enColorSpace;
    }
    /*temp for AVS2 HDR*/
    else if(AVPLAY_HDR_TYPE_SLF == pstInHdrInfo->enHdrType)
    {
        pstFrameInfo->enSrcFrameType = HI_DRV_VIDEO_FRAME_TYPE_SLF;
    }
    else
    {
        HI_ERR_AVPLAY("HDR type is not exist!\n");
    }
...
#ifdef HI_HDR_SUPPORT
        if(HI_TRUE == pAvplay->bPaddingHdrInfo)
        {
            AVPLAY_PaddingHdrInfo2Frame(&pAvplay->stAvplayHdrInfo, &(pAvplay->CurFrmPack.stFrame[0].stFrameVideo));
        }
#endif
    }
...
HI_VOID AVPLAY_QueueVidFrame(AVPLAY_S *pAvplay)
{
        ...
        || (pAvplay->FrcCurPlayCnt < pAvplay->FrcNeedPlayCnt) )
    {
        ...
        else if (SYNC_PROC_BLOCK == pAvplay->VidOpt.SyncProc)
        {
            return;
        }
    }
...
static HI_VOID AVPLAY_QueuePeekFrame(AVPLAY_S *pAvplay)
{
    HI_U32 i;

    /* 1. resume window if current status is pause*/
    if (HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->CurStatus)
    {
        if (HI_INVALID_HANDLE != pAvplay->MasterFrmChn.hWindow)
        {
    ...
    /* 3. puase window and sync if previous status is pause*/
    if (HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->CurStatus)
    {
        if (HI_INVALID_HANDLE != pAvplay->MasterFrmChn.hWindow)
        {
        ...
        (HI_VOID)HI_MPI_SYNC_Pause(pAvplay->hSync);
    }

    /* 4. peek frame finish*/
    pAvplay->AvplayJumpPlayInfo.bJumpPlayMode = HI_FALSE;

    pAvplay->bJumpPlayFinish = HI_TRUE;

    return;
}

static HI_VOID AVPLAY_ReleaseVidFrame(AVPLAY_S *pAvplay)
{
#ifdef HI_HDR_DOLBYVISION_SUPPORT
    if (pAvplay->bHDRPlay)
    {
...
HI_VOID AVPLAY_ProcVidJumpPlay(AVPLAY_S *pAvplay)
{
    HI_S32               Ret;
    HI_U32               CurVidPts = -1;

    if (pAvplay->AvplayJumpPlayInfo.bVidFirstPlay)
    {
        Ret = HI_MPI_SYNC_Resume(pAvplay->hSync);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("Call HI_MPI_SYNC_Resume fail 0x%x\n", Ret);
            return;
        }

        Ret = HI_MPI_SYNC_Seek(pAvplay->hSync, pAvplay->AvplayJumpPlayInfo.TargetPts);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("Call HI_MPI_SYNC_Seek fail 0x%x\n", Ret);
            return;
        }

        pAvplay->AvplayJumpPlayInfo.bVidFirstPlay = HI_FALSE;
    }
    ...
    CurVidPts = pAvplay->CurFrmPack.stFrame[0].stFrameVideo.u32Pts;

    if (HI_INVALID_PTS == CurVidPts)
    {
        AVPLAY_ReleaseVidFrame(pAvplay);
    }
    else if (CurVidPts < pAvplay->AvplayJumpPlayInfo.TargetPts)
    {
...
#ifdef AVPLAY_UVMOS_SUPPORT
HI_U32 AVPLAY_List_FrameInfoInit(AVPLAY_S *pAvplay)
{
    HI_U32 i = 0;
    AVPLAY_VIDEO_FRM_STATUS_INFO_S* pstFrameInfoTemp = NULL;

    if (pAvplay->ListEmptyCount || pAvplay->ListAvailableCount)
    {
        return HI_SUCCESS;
    }

    memset(&pAvplay->FrameInfoEmptylistHead, 0x0, sizeof(AVPLAY_VIDEO_FRM_STATUS_INFO_S));
    memset(&pAvplay->FrameInfoAvailablelistHead, 0x0, sizeof(AVPLAY_VIDEO_FRM_STATUS_INFO_S));

    INIT_LIST_HEAD(&pAvplay->FrameInfoEmptylistHead.list);
    INIT_LIST_HEAD(&pAvplay->FrameInfoAvailablelistHead.list);

    for (i = 0; i < AVPLAY_FRAME_INFO_MAXTCONUT; i++)
    {
        pstFrameInfoTemp = (AVPLAY_VIDEO_FRM_STATUS_INFO_S*)HI_MALLOC(HI_ID_AVPLAY, sizeof(AVPLAY_VIDEO_FRM_STATUS_INFO_S));
        if (HI_NULL != pstFrameInfoTemp)
        {
            pAvplay->ListEmptyCount++;
            memset(pstFrameInfoTemp, 0x0, sizeof(AVPLAY_VIDEO_FRM_STATUS_INFO_S));
            list_add_tail(&pstFrameInfoTemp->list, &pAvplay->FrameInfoEmptylistHead.list);
        }
    }

    pAvplay->ListAvailableCount = 0;
    HI_INFO_AVPLAY("EmptyCount:%d\n",pAvplay->ListEmptyCount);

    return HI_SUCCESS;
}

HI_U32 AVPLAY_List_FrameInfoDeInit(AVPLAY_S *pAvplay)
{
    struct list_head   *pos = HI_NULL;
    struct list_head   *n = HI_NULL;
    AVPLAY_VIDEO_FRM_STATUS_INFO_S* pstFrameInfoTemp = HI_NULL;

    HI_INFO_AVPLAY("EmptyCount:%d, AvailableCount:%d\n",pAvplay->ListEmptyCount,pAvplay->ListAvailableCount);

    list_for_each_safe(pos, n, &pAvplay->FrameInfoAvailablelistHead.list)
    {
        pstFrameInfoTemp = list_entry(pos, AVPLAY_VIDEO_FRM_STATUS_INFO_S, list);

        list_del(pos);
        HI_FREE(HI_ID_AVPLAY, pstFrameInfoTemp);

        HI_INFO_AVPLAY("FrameInfoAvailablelist Free\n");
    }

    list_for_each_safe(pos, n, &pAvplay->FrameInfoEmptylistHead.list)
    {
        pstFrameInfoTemp = list_entry(pos, AVPLAY_VIDEO_FRM_STATUS_INFO_S, list);
        ...
        HI_INFO_AVPLAY("FrameInfoEmptylist Free\n");
    }

    pAvplay->ListEmptyCount = 0;
    pAvplay->ListAvailableCount = 0;

    return HI_SUCCESS;
}

HI_S32 AVPLAY_List_FrameInfoAddTail(AVPLAY_S *pAvplay, HI_UNF_VIDEO_FRM_STATUS_INFO_S* pstFrameStatusInfo)
{
    AVPLAY_VIDEO_FRM_STATUS_INFO_S* pstFrameInfoTemp = NULL;

    if (HI_NULL == pstFrameStatusInfo || HI_NULL == pAvplay)
    {
        HI_ERR_AVPLAY("pstFrameInfo is NULL\n");
        return HI_FAILURE;
    }

    if (   (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
        || (HI_UNF_VCODEC_MODE_NORMAL != pAvplay->VdecAttr.enMode))
    {
        HI_INFO_AVPLAY("cur stat is Tplay or vdecMode isn't normal\n");
        return HI_SUCCESS;
    }

    if (!pAvplay->ListEmptyCount)
    {
        HI_INFO_AVPLAY("EmptyList has no available node\n");
        return HI_FAILURE;
    }

    pstFrameInfoTemp = get_first_item(&pAvplay->FrameInfoEmptylistHead.list, AVPLAY_VIDEO_FRM_STATUS_INFO_S, list);
    if (NULL == pstFrameInfoTemp)
    {
        HI_ERR_AVPLAY("get EmptyList first item fail\n");
        return HI_FAILURE;
    }

    list_del(pAvplay->FrameInfoEmptylistHead.list.next);
    pAvplay->ListEmptyCount--;

    memset(pstFrameInfoTemp, 0x0, sizeof(AVPLAY_VIDEO_FRM_STATUS_INFO_S));
    memcpy(&pstFrameInfoTemp->FrameStatus, pstFrameStatusInfo, sizeof(HI_UNF_VIDEO_FRM_STATUS_INFO_S));

    list_add_tail(&pstFrameInfoTemp->list, &pAvplay->FrameInfoAvailablelistHead.list);
    pAvplay->ListAvailableCount++;

    return HI_SUCCESS;
}

HI_VOID AVPLAY_CorrectFrameTime(AVPLAY_S *pAvplay)
{
    HI_U32  FrameRate = 0;
    HI_U64  TempFrameRate = 0;
    HI_DRV_VIDEO_PRIVATE_S *pstFrmPriv = HI_NULL;
    HI_DRV_VDEC_FRAME_S    *pLstVdecFrame = HI_NULL;

    pLstVdecFrame = &pAvplay->LstFrmPack.stFrame[0];
    pstFrmPriv = (HI_DRV_VIDEO_PRIVATE_S *)(pLstVdecFrame->stFrameVideo.u32Priv);

    FrameRate = pAvplay->FrcParamCfg.u32InRate;
    if ((!pAvplay->bAcquiredVideoFrame) || (!FrameRate))
    {
        return HI_SUCCESS;
    }
    ...
    if (pAvplay->FramRateType == HI_UNF_AVPLAY_FRMRATE_TYPE_USER)
    {
        FrameRate = pAvplay->FrcParamCfg.u32InRate / 10;
    }

    if (   (HI_DRV_FIELD_TOP == pstFrmPriv->eOriginField)
        || (HI_DRV_FIELD_BOTTOM == pstFrmPriv->eOriginField))
    {
        TempFrameRate = FrameRate / 2;
    }
    else
    {
    ...
    if (!pAvplay->CurFrameRate)
    {
        pAvplay->CurFrameRate = TempFrameRate;
        return;
    }

    if (pAvplay->CurFrmPack.stFrame[0].stFrameVideo.bDiscard)
    {
        pAvplay->FrameTimeSameCount = 0;
        return;
    }

    if (pAvplay->CurFrameRate == TempFrameRate)
    {
        pAvplay->FrameTimeSameCount++;
    }
    else
    {
        pAvplay->FrameTimeSameCount = 0;
    ...
    if (pAvplay->FrameTimeSameCount > 40)
    {
        pAvplay->ValidFrameRate = pAvplay->CurFrameRate;
        pAvplay->FrameTimeSameCount = 0;

        HI_INFO_AVPLAY("pAvplay->ValidFrameRate: %d\n",pAvplay->ValidFrameRate);
    }

    return;
}

HI_S32 AVPLAY_ProcReportFrameInfo(AVPLAY_S *pAvplay)
{
    HI_U64  FrameTime = 0;
    HI_U64  CurrentTime = 0;
    HI_U64  TimeCost = 0;
    ...
    FrameTime = 1000 * 100 * 100 / pAvplay->ValidFrameRate;

    clock_gettime(CLOCK_MONOTONIC, &ClockTime);
    CurrentTime = (HI_U64)(ClockTime.tv_sec) * 100000 + (HI_U64)ClockTime.tv_nsec/10000;

    if (!pAvplay->LastFrameTime)
    {
        pAvplay->LastFrameTime = CurrentTime;
        return HI_SUCCESS;
    }

    if (CurrentTime > pAvplay->LastFrameTime)
    {
        TimeCost = CurrentTime - pAvplay->LastFrameTime;
    }
    else
    {
        TimeCost = (AVPLAY_FRAME_RATE_CHECK_MAXTIME - pAvplay->LastFrameTime) + CurrentTime + 1;
    }

    if (TimeCost < FrameTime)
    {
        HI_INFO_AVPLAY("TimeCost:%lld, FrameTime:%lld\n",TimeCost, FrameTime);
        return HI_SUCCESS;
    }

    if (CurrentTime > pAvplay->LastFrameTime)
    {
        pAvplay->LastFrameTime = CurrentTime - (TimeCost - FrameTime);
    }
    else
    {
        /*one time err*/
    ...
    memset(&VidFrmStatusInfo, 0x0, sizeof(VidFrmStatusInfo));

    if (list_empty(&pAvplay->FrameInfoAvailablelistHead.list))
    {
        pAvplay->bNormalStatus = HI_FALSE;
        VidFrmStatusInfo.u32DecoderBuffer = AVPLAY_DECODER_BUFFER_UNDERFLOW;

        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_VID_FRAME_INFO, (&VidFrmStatusInfo));

        HI_INFO_AVPLAY("report frame info sys:%d, DecoderBuffer:%d\n",
                       AVPLAY_GetSysTime(), AVPLAY_DECODER_BUFFER_UNDERFLOW);

        return HI_SUCCESS;
    }

    if ((pAvplay->ListAvailableCount < 5) && (!pAvplay->bNormalStatus))
    {
        VidFrmStatusInfo.u32DecoderBuffer = AVPLAY_DECODER_BUFFER_UNDERFLOW;
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_VID_FRAME_INFO, (&VidFrmStatusInfo));
    ...
    pstFrameInfoTemp = get_first_item(&pAvplay->FrameInfoAvailablelistHead.list, AVPLAY_VIDEO_FRM_STATUS_INFO_S, list);
    if (NULL == pstFrameInfoTemp)
    {
        HI_ERR_AVPLAY("get available frameinfo item fail\n");
        return HI_FAILURE;
    }

    list_del(pAvplay->FrameInfoAvailablelistHead.list.next);
    pAvplay->ListAvailableCount--;

    pAvplay->bNormalStatus = HI_TRUE;
    AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_VID_FRAME_INFO, (&pstFrameInfoTemp->FrameStatus));

    HI_INFO_AVPLAY("report frame info sys:%d,"
        "DecoderBuffer:%d, FrmType:%d, FrmQP:%d, MaxMV:%d, MinMV:%d, AvgMV:%d\n",
        AVPLAY_GetSysTime(),
        VidFrmStatusInfo.u32DecoderBuffer,
        VidFrmStatusInfo.enVidFrmType,
        VidFrmStatusInfo.u32VidFrmQP,
        VidFrmStatusInfo.u32MaxMV,
        VidFrmStatusInfo.u32MinMV,
        VidFrmStatusInfo.u32AvgMV);
...
HI_VOID AVPLAY_ProcVidEventFrameInfo(AVPLAY_S *pAvplay)
{
    HI_DRV_VDEC_FRAME_S          *pCurVdecFrame = HI_NULL;
    HI_DRV_VDEC_FRAME_S          *pLstVdecFrame = HI_NULL;
    HI_DRV_VIDEO_PRIVATE_S       *pstFrmPriv = HI_NULL;
    HI_UNF_VIDEO_FRAME_INFO_S     VidFrm;
    HI_LD_Event_S                 LdEvent;
    HI_UNF_VIDEO_FRM_STATUS_INFO_S       VidFrmStatusInfo;
    HI_UNF_AVPLAY_HDCPSTRATEGY_PARAM_S   HdcpPara;
    HI_BOOL FlsFlag = HI_FALSE;

    pCurVdecFrame = &pAvplay->CurFrmPack.stFrame[0];
    pLstVdecFrame = &pAvplay->LstFrmPack.stFrame[0];

    AVPLAY_DRV2UNF_VidFrm(&pCurVdecFrame->stFrameVideo, &VidFrm, &VidFrmStatusInfo);

    pstFrmPriv = (HI_DRV_VIDEO_PRIVATE_S *)(pCurVdecFrame->stFrameVideo.u32Priv);
    if (   (DEF_HI_DRV_VPSS_LAST_ERROR_FLAG == pstFrmPriv->u32LastFlag)
        || (DEF_HI_DRV_VPSS_LAST_FRAME_FLAG == pstFrmPriv->u32LastFlag)
        )
    {
        FlsFlag = HI_TRUE;
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_VID_LASTFRMFLAG, &FlsFlag);
    }

    if (   (pCurVdecFrame->stFrameVideo.bIsFirstIFrame)
        || (pCurVdecFrame->stFrameVideo.DispCtrlWidth  != pLstVdecFrame->stFrameVideo.DispCtrlWidth)
        || (pCurVdecFrame->stFrameVideo.DispCtrlHeight != pLstVdecFrame->stFrameVideo.DispCtrlHeight) )
    {
        if (   (AVPLAY_HDCP_RESOLUTION_UNRESTRICTED == pCurVdecFrame->stFrameVideo.DispCtrlWidth)
            && (AVPLAY_HDCP_RESOLUTION_UNRESTRICTED == pCurVdecFrame->stFrameVideo.DispCtrlHeight))
        {
            /*hdcp strategy unrestricted*/
            HdcpPara.u32ImageWidth  = pstFrmPriv->stVideoOriginalInfo.u32Width;
            HdcpPara.u32ImageHeight = pstFrmPriv->stVideoOriginalInfo.u32Height;
        }
        else if (   (AVPLAY_HDCP_RESOLUTION_MUTE == pCurVdecFrame->stFrameVideo.DispCtrlHeight)
                 && (AVPLAY_HDCP_RESOLUTION_MUTE == pCurVdecFrame->stFrameVideo.DispCtrlWidth))
        {
            /*hdcp strategy prohibited*/
            HdcpPara.u32ImageWidth  = 0;
            HdcpPara.u32ImageHeight = 0;
        }
        else
        {
            /*hdcp strategy downscal*/
            HdcpPara.u32ImageWidth  = pCurVdecFrame->stFrameVideo.DispCtrlWidth;
            HdcpPara.u32ImageHeight = pCurVdecFrame->stFrameVideo.DispCtrlHeight;
        }

        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_HDCPSTRATEGY_CHANGE, &HdcpPara);
    }
    ...
    {
        if (HI_TRUE == pstFrmPriv->stVideoOriginalInfo.bInterlace)
        {
            VidFrm.bProgressive = HI_FALSE;
        }
    }
    else
    {
        if ((pAvplay->VidInfo.bProgressive == HI_FALSE)&&(VidFrm.bProgressive == HI_TRUE))
        {
            VidFrm.bProgressive = HI_FALSE;
        }
    }

    if (VidFrm.bProgressive == HI_FALSE)
    {
        HI_U32 u32TmpFrmRate = 0;
        u32TmpFrmRate = VidFrm.stFrameRate.u32fpsDecimal + VidFrm.stFrameRate.u32fpsInteger *1000 ;
        VidFrm.stFrameRate.u32fpsInteger = u32TmpFrmRate / 2000;
        VidFrm.stFrameRate.u32fpsDecimal = (u32TmpFrmRate / 2) % 1000;
    }
    AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_NEW_VID_FRAME, &VidFrm);

#ifdef AVPLAY_UVMOS_SUPPORT
    if(!((pAvplay->VidInfo.bProgressive == HI_FALSE)&&
      (pCurVdecFrame->stFrameVideo.bProgressive== HI_TRUE)&&
      (pAvplay->LstFrmPack.u32FrmNum != 0)&&
      (pAvplay->LstFrmPack.stFrame[0].stFrameVideo.u32FrameIndex == pCurVdecFrame->stFrameVideo.u32FrameIndex)))
    {
        VidFrmStatusInfo.u32DecoderBuffer = AVPLAY_DECODER_BUFFER_NORMAL;

        if (pCurVdecFrame->stFrameVideo.bDiscard)
        {
            VidFrmStatusInfo.u32DecoderBuffer = AVPLAY_DECODER_BUFFER_ABNORMAL;
        }

        if (pAvplay->bUVMOSEnable)
        {
            AVPLAY_List_FrameInfoAddTail(pAvplay, &VidFrmStatusInfo);
        }
    }
#endif
    ...
    LdEvent.frame  = pCurVdecFrame->stFrameVideo.u32FrameIndex;
    LdEvent.handle = pCurVdecFrame->stFrameVideo.hTunnelSrc;
    (HI_VOID)HI_SYS_GetTimeStampMs(&(LdEvent.time));
...
HI_VOID AVPLAY_ProcVdecToVo(AVPLAY_S *pAvplay)
{
    ...
    pAvplay->bJumpPlayFinish = HI_FALSE;

    if (pAvplay->AvplayJumpPlayInfo.bJumpPlayMode)
    {
        AVPLAY_ProcVidJumpPlay(pAvplay);
        return;
    }
    ...
    if (pAvplay->bUVMOSEnable)
    {
        (HI_VOID)AVPLAY_ProcReportFrameInfo(pAvplay);
        (HI_VOID)AVPLAY_CorrectFrameTime(pAvplay);
    }
#endif

    pAvplay->bQueuedVideoFrame = HI_FALSE;
    pAvplay->bAcquiredVideoFrame = HI_FALSE;
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_VDEC_VO])
    {
        ...
        pAvplay->bAcquiredVideoFrame = HI_TRUE;
...
HI_VOID AVPLAY_ProcMultiAudSync(AVPLAY_S *pAvplay, HI_U32 ChnId)
{
    HI_S32                   Ret = HI_FAILURE;
    HI_S32                   PtsDiff = 0;
    HI_U32                   CurSyncPts = HI_INVALID_PTS;
    HI_U32                   LstAudPts = pAvplay->MultiAudSyncInfo[ChnId].Pts;
    HI_U32                   AudSyncAdjust = pAvplay->MultiAudSyncInfo[ChnId].AudSyncAdjust;
    HI_U32                   AudBufPercent = 0;
    HI_UNF_ES_BUF_S          AudDmxEsBuf = {0};
    HI_UNF_SYNC_STATUS_S     SyncStatus = {0};
    HI_MPI_DMX_BUF_STATUS_S  AudChnBuf = {0};

    if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
    {
        AudSyncAdjust = HI_FALSE;
    }
    ...
    if (AudSyncAdjust)
    {
        ...
        if (HI_INVALID_PTS != SyncStatus.u32LastVidPts)
        {
            CurSyncPts = SyncStatus.u32LastVidPts;
        }
        else if (HI_INVALID_PTS != SyncStatus.u32LastAudPts)
        {
            CurSyncPts = SyncStatus.u32LastAudPts;
        }
        else
        {
            return;
        ...
        PtsDiff = CurSyncPts - LstAudPts;

        /*when in novel region check demux aud buffer*/
        if ((abs(PtsDiff) > AVPLAY_MULTIAUD_SYNC_NOVEL_REGION) && (PtsDiff < 0))
        {
            Ret = HI_MPI_DMX_GetPESBufferStatus(pAvplay->hDmxAud[ChnId], &AudChnBuf);
            if (HI_SUCCESS == Ret)
            {
                if (AudChnBuf.u32BufSize > 0)
                {
        ...
        else if ((abs(PtsDiff) > AVPLAY_MULTIAUD_SYNC_START_REGION) && (PtsDiff < 0))
        {
            return;
        }
    }

    Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAud[ChnId], &AudDmxEsBuf, HI_NULL);
    if (HI_SUCCESS != Ret)
    {
        return;
    }

    pAvplay->MultiAudSyncInfo[ChnId].Pts = AudDmxEsBuf.u32PtsMs;
    pAvplay->MultiAudSyncInfo[ChnId].AudSyncAdjust = AudDmxEsBuf.u32PtsMs == HI_INVALID_PTS ? HI_FALSE : HI_TRUE;

    HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAud[ChnId], &AudDmxEsBuf);
    pAvplay->AvplayProcAudContinue = HI_TRUE;
...
HI_VOID AVPLAY_ProcDmxToAdec(AVPLAY_S *pAvplay)
{
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ADEC])
    {
        ...
        {
            if ((i == pAvplay->CurDmxAudChn)&&(HI_FALSE == pAvplay->bCurExternalAud))
            {
                ...
                Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAud[i], &(pAvplay->AvplayDmxEsBuf), HI_NULL);
                if (HI_SUCCESS == Ret)
                {
            ...
            {
                AVPLAY_ProcMultiAudSync(pAvplay, i);
            }
        }
    }
...
HI_VOID AVPLAY_ProcDmxToAdAdec(AVPLAY_S *pAvplay)
{
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_ADEC_AD])
    {
        pAvplay->DebugInfo.AcquireAudADEsNum++;

        Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAdAud, &(pAvplay->AvplayAdDmxEsBuf), HI_NULL);
        if (HI_SUCCESS == Ret)
        {
            pAvplay->DebugInfo.AcquiredAudADEsNum++;
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_ADEC_AD] = HI_TRUE;
        }
    }
    ...
    if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_ADEC_AD])
    {
        ...
        Ret = HI_MPI_ADEC_SendStream(pAvplay->hAdAdec, &AdecEsBuf, pAvplay->AvplayAdDmxEsBuf.u32PtsMs);
        if (HI_SUCCESS == Ret)
        {
            pAvplay->DebugInfo.SendedAudADEsNum++;
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_ADEC_AD] = HI_FALSE;
...
HI_VOID AVPLAY_ProcEos(AVPLAY_S *pAvplay)
{
    ...
    if (pAvplay->AudEnable)
    {
        ...
        if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        {
            ADEC_BUFSTATUS_S    AdecBuf = {0};
        ...
        if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            (HI_VOID)HI_MPI_AO_Track_GetInfo(pAvplay->hRenderTrack, TRACK_INFO_EOSSTATEFLAG, &bAudEos);
...
HI_VOID AVPLAY_FindIFrameFromDmxEsBuf(AVPLAY_S *pAvplay)
{
    ...
    Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxVid, &VidDmxEsBuf, HI_NULL);
    if (HI_SUCCESS == Ret)
    {
    ...
    if ( !pAvplay->AudEnable && pAvplay->bAudPreEnable )
    {
        ...
        if (pAvplay->AudPreBufThreshold != 0)
        {
            ...
            else
            {
                ...
                {
                    Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAud[pAvplay->CurDmxAudChn], &AudDmxEsBuf, HI_NULL);
                    if ( HI_SUCCESS != Ret  )
                    {
                    ...
                    else
                    {
                        ...
                        pAvplay->AvplayProcStateContinue = HI_TRUE;
                    }
                }
            }
        }
...
static HI_VOID AVPLAY_GetRenderBufPercent(const AVPLAY_S * pAvplay, HI_U32 *pu32AudBufPercent, HI_U32 *pu32UsedBuff)
{
    HI_S32                             Ret = HI_SUCCESS;
    ADEC_BUFSTATUS_S                   stRenderBuffStatus = {0};
    HI_MPI_DMX_BUF_STATUS_S            AudChnBuf = {0};

    Ret = HI_MPI_AO_Track_GetInfo(pAvplay->hRenderTrack,  TRACK_INFO_BUFFERSTATUS, &stRenderBuffStatus);
    if (HI_SUCCESS != Ret)
    {
        *pu32AudBufPercent = 0;
        *pu32UsedBuff = 0;
    ...
    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        ...
        if (HI_SUCCESS == Ret)
        {
            if ((AudChnBuf.u32BufSize + stRenderBuffStatus.u32BufferSize) > 0)
            {
                *pu32AudBufPercent = (AudChnBuf.u32UsedSize + stRenderBuffStatus.u32BufferUsed) * 100 / (AudChnBuf.u32BufSize + stRenderBuffStatus.u32BufferSize);
                *pu32UsedBuff = AudChnBuf.u32UsedSize + stRenderBuffStatus.u32BufferUsed;
            }
        }
    }
    else
    {
        if (stRenderBuffStatus.u32BufferSize > 0)
        {
            *pu32AudBufPercent = stRenderBuffStatus.u32BufferUsed * 100 / stRenderBuffStatus.u32BufferSize;
            *pu32UsedBuff = stRenderBuffStatus.u32BufferUsed;
        }
        else
        {
            *pu32AudBufPercent = 50;
            *pu32UsedBuff = 0;
        }
    }
...
static HI_UNF_AVPLAY_BUF_STATE_E AVPLAY_CaclRenderBufState(const AVPLAY_S * pAvplay, HI_U32 UsedBufPercent, HI_U32 u32RenderUsedBuf)
{
    HI_UNF_AVPLAY_BUF_STATE_E CurBufState = HI_UNF_AVPLAY_BUF_STATE_NORMAL;

    if (UsedBufPercent >= AVPLAY_ES_RENDER_FULL_PERCENT)
    {
        CurBufState = HI_UNF_AVPLAY_BUF_STATE_FULL;
    }
    else if ((UsedBufPercent >= AVPLAY_ES_RENDER_HIGH_PERCENT) && (UsedBufPercent < AVPLAY_ES_RENDER_FULL_PERCENT))
    {
        CurBufState = HI_UNF_AVPLAY_BUF_STATE_HIGH;
    }
    else if (UsedBufPercent <= AVPLAY_ES_RENDER_LOW_PERCENT)
    {
        if(u32RenderUsedBuf <= AVPLAY_ES_RENDER_EMPTY_SIZE)
        {
            CurBufState = HI_UNF_AVPLAY_BUF_STATE_EMPTY;
        }
        else
...
HI_VOID AVPLAY_GetAudBufState(AVPLAY_S *pAvplay, HI_U32 *pAudBufPercent, HI_UNF_AVPLAY_BUF_STATE_E *pCurAudBufState)
{
    ...
    HI_U32                             u32RenderUsedBuff = 0;
    HI_U32                             AudBufPercent = 0;

    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        AVPLAY_GetRenderBufPercent(pAvplay, &AudBufPercent, &u32RenderUsedBuff);
        CurAudBufState = AVPLAY_CaclRenderBufState(pAvplay, AudBufPercent, u32RenderUsedBuff);
    }
    ...
    *pCurAudBufState = CurAudBufState;
    ...
    pAvplay->CurAudBufState = CurAudBufState;
...
HI_VOID AVPLAY_GetVidBufState(AVPLAY_S *pAvplay, HI_U32 *pVidBufPercent, HI_UNF_AVPLAY_BUF_STATE_E *pCurVidBufState)
{
    ...
    *pCurVidBufState = CurVidBufState;
    ...
    pAvplay->CurVidBufState = CurVidBufState;
...
HI_VOID AVPLAY_ProcCheckBuf(AVPLAY_S *pAvplay)
{
    ...
    HI_UNF_DMX_PORT_E                  PortId = HI_UNF_DMX_PORT_BUTT;
    ...
    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        Ret = HI_MPI_DMX_GetTSPortId(pAvplay->AvplayAttr.u32DemuxId, &PortId);
        if (HI_SUCCESS == Ret)
        {
            /* Ram port or Tsio port */
            if ( (PortId >= HI_UNF_DMX_PORT_RAM_0 && PortId <= HI_UNF_DMX_PORT_RAM_15)
              || (PortId >= HI_UNF_DMX_PORT_TSIO_0 && PortId <= HI_UNF_DMX_PORT_TSIO_7))
            {
...
static HI_VOID AVPLAY_DRV2UNF_VidFrm(HI_DRV_VIDEO_FRAME_S *pstDRVFrm, HI_UNF_VIDEO_FRAME_INFO_S *pstUNFFrm,HI_UNF_VIDEO_FRM_STATUS_INFO_S *pstUnfFrmStatusInfo)
{
    if (HI_NULL != pstUNFFrm)
    {
        ...
        if ((0 != pstDRVFrm->u32DisplayHorizontalSize) && (0 != pstDRVFrm->u32DisplayVerticalSize))
        {
            pstUNFFrm->u32DisplayWidth = pstDRVFrm->u32DisplayHorizontalSize;
            pstUNFFrm->u32DisplayHeight = pstDRVFrm->u32DisplayVerticalSize;
        }
        else
        {
    ...
    {
        pstUnfFrmStatusInfo->enVidFrmType = (HI_UNF_VIDEO_FRAME_TYPE_E)(pstDRVFrm->u32FrameType + 1);
        pstUnfFrmStatusInfo->u32VidFrmStreamSize = pstDRVFrm->u32FrameStreamSize;
        pstUnfFrmStatusInfo->u32VidFrmPTS = pstDRVFrm->u32Pts;
        pstUnfFrmStatusInfo->u32VidFrmQP = pstDRVFrm->u32AvgQp;
        pstUnfFrmStatusInfo->u32MaxMV = pstDRVFrm->u32MaxMV;
        pstUnfFrmStatusInfo->u32MinMV = pstDRVFrm->u32MinMV;
        pstUnfFrmStatusInfo->u32AvgMV = pstDRVFrm->u32AvgMV;
        pstUnfFrmStatusInfo->u32SkipRatio = pstDRVFrm->u32SkipRatio;
    }
...
HI_VOID AVPLAY_ProcVidEvent(AVPLAY_S *pAvplay)
{
    ...
    if (pAvplay->CurVidBufState != pAvplay->PreVidBufState)
    {
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_RNG_BUF_STATE, &pAvplay->CurVidBufState);
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_VID_BUF_STATE, &pAvplay->CurVidBufState);

        pAvplay->PreVidBufState = pAvplay->CurVidBufState;
    }
    ...
    if (VdecEvent.bNewFrame)
    {
        HI_DRV_VIDEO_FRAME_S       NewVidFrame = {0};
        HI_UNF_VIDEO_FRAME_INFO_S  NotifyVidFrm = {0};

        Ret = HI_MPI_VDEC_ReadNewFrame(pAvplay->hVdec, &NewVidFrame);
        if (HI_SUCCESS == Ret)
        {
            AVPLAY_DRV2UNF_VidFrm(&NewVidFrame, &NotifyVidFrm, HI_NULL);
            AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_NEW_PICTURE_DECODED, &NotifyVidFrm);
        }
    }
...
HI_VOID AVPLAY_ProcAudEvent(AVPLAY_S *pAvplay)
{
    ...
    HI_MPI_AO_RENDER_EVENT_S RenderEvent;
    ...
    if (pAvplay->CurAudBufState != pAvplay->PreAudBufState)
    {

        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_RNG_BUF_STATE, &pAvplay->CurAudBufState);
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_AUD_BUF_STATE, &pAvplay->CurAudBufState);

        pAvplay->PreAudBufState = pAvplay->CurAudBufState;
    }

    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        Ret = HI_MPI_AO_Track_CheckRenderNewEvent(pAvplay->hRenderTrack, &RenderEvent);
        if (HI_SUCCESS != Ret)
        {
            HI_INFO_AVPLAY("HI_MPI_AO_Track_CheckRenderNewEvent failed 0x%x\n", Ret);
            return;
        }

        if (RenderEvent.bFrameInfoChange)
        {
            AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_AUD_INFO_CHANGE, &RenderEvent.stStreamInfo);
        }

        if (RenderEvent.bUnSupportFormat)
        {
            AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_AUD_UNSUPPORT, HI_NULL);
        }

        if (RenderEvent.bStreamCorrupt)
        {
            AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_AUD_FRAME_ERR, HI_NULL);
        }

        if (RenderEvent.bNewFrame)
        {
            AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_NEW_AUD_FRAME, HI_NULL);
        }
...
HI_VOID AVPLAY_ProcSyncEvent(AVPLAY_S *pAvplay)
{
    ...
    HI_UNF_SYNC_PCR_STC_INFO_S  SyncPcrStcInfo = {0};
    ...
    Ret = HI_MPI_SYNC_GetNewPcrStcValue(pAvplay->hSync,&SyncPcrStcInfo.u64PcrValue, &SyncPcrStcInfo.u64StcValue);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("HI_MPI_SYNC_GetNewPcrStcValue failed 0x%x\n", Ret);
        return;
    }

    if ((0 != SyncPcrStcInfo.u64PcrValue)&&(0 != SyncPcrStcInfo.u64StcValue))
    {
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_GET_PCR_STC_VALUE, &SyncPcrStcInfo);
    }
...
extern HI_S32 HI_MPI_PMOC_SetCPUMinFreq(HI_BOOL bFlag, HI_U32 u32MinFreq);
...
HI_VOID *AVPLAY_StatThread(HI_VOID *Arg)
{
    ...
    while (pAvplay->AvplayThreadRun)
    {
        ...
        pAvplay->AvplayProcStateContinue = HI_FALSE;
        ...
        if (pAvplay->AvplayProcStateContinue)
        {
            continue;
        }
        ...
        if (pAvplay->AvplayThreadRun)
        {
            AVPLAY_Usleep(AVPLAY_SYS_SLEEP_TIME * 1000);
        }
    }
...
HI_VOID AVPLAY_CleanOldAudioDataInDmx(AVPLAY_S *pAvplay)
{
    HI_S32 Ret;
    HI_BOOL bMainFlag = HI_FALSE;
    HI_BOOL bAdFlag = HI_FALSE;

    while(1) //In order to avoid noise, clean old es data in demux.
    {
        Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAud[pAvplay->CurDmxAudChn], &pAvplay->AvplayDmxEsBuf, HI_NULL);
        if ((HI_FALSE == bMainFlag)&&(HI_SUCCESS == Ret)&&((pAvplay->AvplayDmxEsBuf.u32PtsMs == -1)||(pAvplay->AvplayDmxEsBuf.u32PtsMs > pAvplay->LastAudPts)))
        {
            HI_WARN_AVPLAY("release main aud pts %d\n",pAvplay->AvplayDmxEsBuf.u32PtsMs);
           (HI_VOID)HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAud[pAvplay->CurDmxAudChn], &pAvplay->AvplayDmxEsBuf);
        }
        else
        {
        ...
        {
            Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAdAud, &pAvplay->AvplayAdDmxEsBuf, HI_NULL);
            if ((HI_FALSE == bAdFlag)&&(HI_SUCCESS == Ret)&&((pAvplay->AvplayAdDmxEsBuf.u32PtsMs == -1)||(pAvplay->AvplayAdDmxEsBuf.u32PtsMs > pAvplay->LastAudPts)))
            {
                HI_WARN_AVPLAY("release ad aud pts %d\n",pAvplay->AvplayAdDmxEsBuf.u32PtsMs);
                (HI_VOID)HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAdAud, &pAvplay->AvplayAdDmxEsBuf);
            }
            else
            {
        ...
        if ((HI_TRUE == bMainFlag)&&(bAdFlag == HI_TRUE))
        {
            HI_WARN_AVPLAY("all old data has been droped!crtpts %d,lstpts %d\n ",pAvplay->AvplayDmxEsBuf.u32PtsMs,pAvplay->LastAudPts);
            if ((pAvplay->AvplayDmxEsBuf.u32PtsMs < pAvplay->LastAudPts)&&(pAvplay->AvplayDmxEsBuf.u32PtsMs != -1))
            {
                pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_MAIN_TRACK] = HI_TRUE;
            }

            if ((pAvplay->AvplayAdDmxEsBuf.u32PtsMs < pAvplay->LastAudPts)&&(pAvplay->AvplayAdDmxEsBuf.u32PtsMs != -1))
            {
                pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK] = HI_TRUE;
            }
            (HI_VOID)HI_MPI_SYNC_SetAudBackFlag(pAvplay->hSync, HI_TRUE);  //Set audio loopback flag.
            AVPLAY_Usleep(50 * 1000); //make sure set bAudBackFlag  successfully.
            break;
        }
        AVPLAY_Usleep(10*1000);
    }
    return;
}

HI_VOID AVPLAY_AudMS12Mode(AVPLAY_S *pAvplay)
{
    HI_S32 Ret;
    static HI_U32 s_u32LastPtsTime = 0;
    static HI_BOOL bAudBackFlag = HI_FALSE;
    HI_U32 thisPts = pAvplay->AvplayDmxEsBuf.u32PtsMs;
    HI_U32 thisPtsTime = AVPLAY_GetSysTime();
    HI_S32 ptsDiff = 0;
    HI_BOOL bPcrBackFlag = HI_FALSE;
    ...
    if (stSyncAttr.enSyncRef != HI_UNF_SYNC_REF_PCR)
    {
        return;
    }

    (HI_VOID)HI_MPI_SYNC_CheckPcrBackFlag(pAvplay->hSync,&bPcrBackFlag);

    if (bPcrBackFlag == HI_TRUE)   //Check pcr is loopback or not.
    {
        if ( (thisPtsTime >  s_u32LastPtsTime) && (0 != s_u32LastPtsTime))
        {
            ptsDiff = (HI_S32)(thisPtsTime - s_u32LastPtsTime);
        }
        else
        ...
        if ( ptsDiff > 800 )  //if pcr is loopback and the interval time of receiving audio es data is more than 800ms, we think auido will loop back.
        {
            HI_WARN_AVPLAY("PtsTime:%u -> %u, Diff:%d.\n ", s_u32LastPtsTime, thisPtsTime, ptsDiff);
            HI_WARN_AVPLAY("PTS:%u -> %u, PtsLess.\n ", pAvplay->LastAudPts, thisPts);

            if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_MAIN_TRACK])
            {
                (HI_VOID)HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAud[pAvplay->CurDmxAudChn], &pAvplay->AvplayDmxEsBuf);
                pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_MAIN_TRACK] = HI_FALSE;
            }

            if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK])
            {
                (HI_VOID)HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAdAud, &pAvplay->AvplayAdDmxEsBuf);
                pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK] = HI_FALSE;
            }

            AVPLAY_CleanOldAudioDataInDmx(pAvplay);
            bAudBackFlag = HI_TRUE;
            HI_WARN_AVPLAY("set aud back flag OK.\n");
            s_u32LastPtsTime = HI_INVALID_PTS;
            pAvplay->LastAudPts = HI_INVALID_PTS;
        }
        ...
        && (pAvplay->LastAudPts != HI_INVALID_PTS)
        && (thisPts != HI_INVALID_PTS)
        &&(bAudBackFlag == HI_FALSE))
        {
           HI_U32 delayms = 0;
           Ret = HI_MPI_AO_Track_GetDelayMs(pAvplay->hSyncTrack,&delayms);
           if((Ret)||(delayms >= 600))
           {
             delayms = 0;
           }
           AVPLAY_Usleep(delayms*1000);
           HI_WARN_AVPLAY("------Aud loopback is judged by pts!delayms %d lp %d p %d \n",delayms,pAvplay->LastAudPts,thisPts);
           (HI_VOID)HI_MPI_SYNC_SetAudBackFlag(pAvplay->hSync, HI_TRUE);  //Set audio loopback flag.
           AVPLAY_Usleep(50 * 1000); //make sure set bAudBackFlag  successfully.
           bAudBackFlag = HI_TRUE;
        }
...
HI_VOID AVPLAY_ProcDmxToMainTrack(AVPLAY_S *pAvplay)
{
    HI_U32               i = 0;
    HI_S32               Ret;
    HI_UNF_STREAM_BUF_S  AdecEsBuf = {0};
    ...
    if (HI_INVALID_HANDLE == pAvplay->hDmxAud[pAvplay->CurDmxAudChn])
    {
        return;
    }
            ...
            if (i == pAvplay->CurDmxAudChn)
            {
                ...
                else
                {
                    if (HI_ERR_DMX_EMPTY_BUFFER == Ret
                        && pAvplay->bSetEosFlag && !pAvplay->bSetAudEos)
                    {
                        if (HI_INVALID_HANDLE != pAvplay->hRenderTrack)
                        {
                            Ret = HI_MPI_AO_Track_SetEosFlag(pAvplay->hRenderTrack, HI_TRUE);
                            if (HI_SUCCESS != Ret)
                            {
                                HI_ERR_AVPLAY("ERR: HI_MPI_HIAO_SetEosFlag, Ret = %#x! \n", Ret);
                                return;
    ...
    if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_MAIN_TRACK])
    {
        /*Check main aud channel for loopback flag and tell sync to fund audio data.*/
        AVPLAY_AudMS12Mode(pAvplay);
        pAvplay->DebugInfo.SendAudEsNum++;

        AdecEsBuf.pu8Data = pAvplay->AvplayDmxEsBuf.pu8Buf;
        AdecEsBuf.u32Size = pAvplay->AvplayDmxEsBuf.u32BufLen;

        Ret = HI_MPI_AO_Track_SendStream(pAvplay->hRenderTrack, HI_MPI_AO_BUF_ID_ES_MAIN, &AdecEsBuf, pAvplay->AvplayDmxEsBuf.u32PtsMs, HI_NULL);
        if (HI_SUCCESS == Ret)
        {
            HI_INFO_AVPLAY("sent stream to AO Render Main Track succeed\n");
            pAvplay->DebugInfo.SendedAudEsNum++;
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_MAIN_TRACK] = HI_FALSE;
            pAvplay->AvplayProcAudContinue = HI_TRUE;
            (HI_VOID)HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAud[pAvplay->CurDmxAudChn], &pAvplay->AvplayDmxEsBuf);
        ...
        {
            if (   (Ret != HI_ERR_ADEC_IN_BUF_FULL)
                && (Ret != HI_ERR_ADEC_IN_PTSBUF_FULL)
                && (Ret != HI_ERR_AO_OUT_BUF_FULL) ) /* drop this pkg */
            {
                HI_ERR_AVPLAY("Send AD AudEs buf to ADEC fail:%#x, drop a pkg.\n", Ret);
                pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_MAIN_TRACK] = HI_FALSE;
                pAvplay->AvplayProcAudContinue = HI_TRUE;
...
HI_VOID AVPLAY_AudAssocMS12Mode(AVPLAY_S *pAvplay)
{
    HI_S32 Ret;
    static HI_U32 s_u32LastPtsTime = 0;
    static HI_U32 s_u32LastPts  = HI_INVALID_PTS;
    static HI_BOOL bDropOldDataFlag = HI_FALSE;
    HI_U32 thisPts = pAvplay->AvplayAdDmxEsBuf.u32PtsMs;
    HI_U32 thisPtsTime = AVPLAY_GetSysTime();
    HI_S32 ptsDiff = 0;
    HI_BOOL bPcrBackFlag = HI_FALSE;
    HI_UNF_SYNC_ATTR_S stSyncAttr = {0};
    ...
    if (bPcrBackFlag == HI_TRUE)   //Check pcr is loopback or not.
    {
        ...
        if ( ptsDiff > 800 )  //if pcr is loopback and the interval time of receiving audio es data is more than 1000ms, we think auido will loop back.
        {
            HI_WARN_AVPLAY("PtsTime:%u -> %u, Diff:%d.thisPts:%d\n ", s_u32LastPtsTime, thisPtsTime, ptsDiff,thisPts);
            bDropOldDataFlag = HI_TRUE;
        }

        if ((thisPts < s_u32LastPts)
        && (s_u32LastPts != HI_INVALID_PTS)
        && (thisPts != HI_INVALID_PTS))
        {
            HI_WARN_AVPLAY("PTS:%u -> %u, PtsLess.\n ", s_u32LastPts, thisPts);
            bDropOldDataFlag = HI_FALSE;
        }

        if ((pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK])&&(bDropOldDataFlag == HI_TRUE))
        {
            HI_WARN_AVPLAY("release ad aud pts %d\n",pAvplay->AvplayAdDmxEsBuf.u32PtsMs);
            (HI_VOID)HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAdAud, &pAvplay->AvplayAdDmxEsBuf);
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK] = HI_FALSE;
...
HI_VOID AVPLAY_ProcDmxToAssocTrack(AVPLAY_S *pAvplay)
{
    HI_S32               Ret;
    HI_UNF_STREAM_BUF_S  AdecEsBuf = {0};
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK])
    {
        ...
        memset(&pAvplay->AudEsExtData, 0x0, sizeof(HI_MPI_ES_EXT_DATA_S));
        Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAdAud, &pAvplay->AvplayAdDmxEsBuf, &pAvplay->AudEsExtData);
        if (HI_SUCCESS == Ret)
        {
            pAvplay->DebugInfo.AcquiredAudADEsNum++;
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK] = HI_TRUE;
    ...
    /*Check assoc aud channel for loopback flag and drop few data to avoid popping sound.*/
    AVPLAY_AudAssocMS12Mode(pAvplay);

    if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK])
    {
        HI_U8 ExtBuf[PES_PRIVATE_DATA_SIZE] = {0};
        pAvplay->DebugInfo.SendAudADEsNum++;

        AdecEsBuf.pu8Data = pAvplay->AvplayAdDmxEsBuf.pu8Buf;
        AdecEsBuf.u32Size = pAvplay->AvplayAdDmxEsBuf.u32BufLen;
        memcpy(ExtBuf, pAvplay->AudEsExtData.au8PesPrivData, PES_PRIVATE_DATA_SIZE);

        Ret = HI_MPI_AO_Track_SendStream(pAvplay->hRenderTrack, HI_MPI_AO_BUF_ID_ES_ASSOC, &AdecEsBuf, pAvplay->AvplayAdDmxEsBuf.u32PtsMs, (HI_VOID*)ExtBuf);
        if (HI_SUCCESS == Ret)
        {
            HI_INFO_AVPLAY("sent stream to AO Render Assoc Track succeed\n");
            pAvplay->DebugInfo.SendedAudADEsNum++;
            pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK] = HI_FALSE;
            pAvplay->AvplayProcAudContinue = HI_TRUE;
            (HI_VOID)HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAdAud, &pAvplay->AvplayAdDmxEsBuf);
...
static HI_VOID  AVPLAY_ProcVideoEvent(AVPLAY_S *pAvplay)
{
    if (HI_TRUE == pAvplay->bJumpPlayFinish)
    {
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_VID_START_POS, (HI_VOID *)&pAvplay->CurFrmPack.stFrame[0].stFrameVideo.u32Pts);
        pAvplay->bJumpPlayFinish = HI_FALSE;
    }

    if (HI_TRUE == pAvplay->bAcquiredVideoFrame)
    {
        AVPLAY_ProcVidEventFrameInfo(pAvplay);
        pAvplay->bAcquiredVideoFrame = HI_FALSE;
    }

    if (HI_TRUE == pAvplay->bQueuedVideoFrame)
    {
        AVPLAY_ProcFirstDispFrame(pAvplay);
        AVPLAY_NotifyVidFrmDisplayInfo(pAvplay, pAvplay->MasterFrmChn.hWindow, pAvplay->CurFrmPack.stFrame[0].stFrameVideo.u32Pts);
        pAvplay->bQueuedVideoFrame = HI_FALSE;
    }
...
HI_VOID *AVPLAY_AudDataThread(HI_VOID *Arg)
{
    ...
    while (pAvplay->AvplayThreadRun)
    {
        ...
        if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
            {
                AVPLAY_ProcDmxToMainTrack(pAvplay);
                AVPLAY_ProcDmxToAssocTrack(pAvplay);
            }

            AVPLAY_ProcRenderTrack(pAvplay);
        }

        AVPLAY_ThreadMutex_UnLock(&pAvplay->AvplayAudThreadMutex);

        if (pAvplay->bAcquiredAudFrame)
        {
...
HI_VOID *AVPLAY_VidDataThread(HI_VOID *Arg)
{
    ...
    while (pAvplay->AvplayThreadRun)
    {
        ...
        {
            AVPLAY_Usleep(AVPLAY_VIDTHREAD_SLEEP_TIME * 1000);
            continue;
        }
...
HI_VOID AVPLAY_ResetProcFlag(AVPLAY_S *pAvplay)
{
    ...
    pAvplay->CurVidBufState = HI_UNF_AVPLAY_BUF_STATE_EMPTY;
    pAvplay->CurAudBufState = HI_UNF_AVPLAY_BUF_STATE_EMPTY;
    ...
    pAvplay->bSyncFinish = HI_FALSE;

    pAvplay->DebugInfo.VidFrameDropNum = 0;
    ...
    pAvplay->VidInfo.Pts = HI_INVALID_PTS;
    pAvplay->AudInfo.Pts = HI_INVALID_PTS;
    pAvplay->AudOpt.SyncProc = SYNC_PROC_BUTT;
    pAvplay->VidOpt.SyncProc = SYNC_PROC_BUTT;
    pAvplay->stIFrame.hport = HI_INVALID_HANDLE;
    memset(&pAvplay->stIFrame.stFrameVideo, 0x0, sizeof(HI_DRV_VIDEO_FRAME_S));

    pAvplay->bSetProgressive = HI_FALSE;
...
#ifdef AVPLAY_UVMOS_SUPPORT
    pAvplay->ValidFrameRate = 2500; /*1/100 ms*/
    pAvplay->CurFrameRate = 0;
    pAvplay->FrameTimeSameCount = 0;
    pAvplay->LastFrameTime = 0;
    pAvplay->bNormalStatus = 0;
    pAvplay->FramRateType = HI_UNF_AVPLAY_FRMRATE_TYPE_BUTT;
#endif
...
HI_S32 AVPLAY_MallocVdec(AVPLAY_S *pAvplay, const HI_VOID *pPara)
{
    ...
    {
        HI_BOOL bUVMOSEEnable = HI_FALSE;
        HI_CHAR buffer[128] = {0,0,0};
        HI_CODEC_VIDEO_CMD_S stParam;

        memset(&stParam, 0x0, sizeof(stParam));

        /*VFMW_CMD_SET_UVMOS*/
        stParam.u32CmdID = 0x27;

        if (property_get("persist.sys.media.uvmos", buffer, NULL)
            && (!strcasecmp("true", buffer)))
        {
            bUVMOSEEnable = HI_TRUE;
        }

#ifdef HI_TEE_SUPPORT
        if (pAvplay->TVPAttr.bEnable == HI_TRUE)
        {
            bUVMOSEEnable = HI_FALSE;
        }
#endif
        pAvplay->bUVMOSEnable = bUVMOSEEnable;

        HI_INFO_AVPLAY("pAvplay->bUVMOSEnable:%d\n",bUVMOSEEnable);

        stParam.pPara = &bUVMOSEEnable;
        Ret = HI_MPI_VDEC_Invoke(pAvplay->hVdec, &stParam);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("HI_MPI_VDEC_Invoke cfg uvmos failed.\n");
        }
    }
#endif
...
HI_S32 AVPLAY_FreeAdec(AVPLAY_S *pAvplay)
{
    ...
    if (HI_INVALID_HANDLE != pAvplay->hAdAdec)
    {
        Ret = HI_MPI_ADEC_Close(pAvplay->hAdAdec);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("HI_MPI_ADEC_Close ad adec failed 0x%x\n", Ret);
            return Ret;
        }

        pAvplay->hAdAdec = HI_INVALID_HANDLE;
    }
    return Ret;
}
...
HI_S32 AVPLAY_GetAudType(AVPLAY_S *pAvplay, const HI_UNF_ACODEC_ATTR_S* pAdecAttr, HI_BOOL bRenderSupport, AVPLAY_AUDIO_TYPE_E* pstAudType)
{
    HI_S32          Ret = HI_SUCCESS;
    HI_BOOL         bSupport = HI_FALSE;

    if (HI_NULL == pAdecAttr)
    {
        HI_ERR_AVPLAY("pAdecAttr is NULL\n");
        return HI_ERR_AVPLAY_INVALID_PARA;
    }

    /*only normal aud support audtplay*/
    if (pAvplay->bAudTplayEnable)
    {
        *pstAudType = AVPLAY_AUDIO_TYPE_NORMAL;
        return HI_SUCCESS;
    }

    if (bRenderSupport)
    {
        Ret = HI_MPI_AO_Track_IsCodecSupport(pAdecAttr->enType, &bSupport);
        if(HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("call HI_MPI_AO_Track_IsCodecSupport Fail! Ret:0x%x\n", Ret);
            return Ret;
        }

        if(HI_TRUE == bSupport)
        {
            *pstAudType = AVPLAY_AUDIO_TYPE_RENDER;
            HI_INFO_AVPLAY("Current Audio Mode is Render.\n");
        }
        else
        {
            HI_INFO_AVPLAY("not support format, default is Normal!\n");
            *pstAudType = AVPLAY_AUDIO_TYPE_NORMAL;
        }
    }
    else
...
HI_S32 AVPLAY_SetAdecAttr(AVPLAY_S *pAvplay, const HI_UNF_ACODEC_ATTR_S *pAdecAttr)
{
    ...
    HI_S32       Ret = HI_FAILURE;
    HI_BOOL      bRenderSupport = HI_FALSE;

    Ret = HI_MPI_AO_Track_IsRenderSupport(&bRenderSupport);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("call HI_MPI_AO_Track_IsRenderSupport. ret=0x%x.\n", Ret);
        return Ret;
    }

    Ret = AVPLAY_GetAudType(pAvplay, pAdecAttr, bRenderSupport, &pAvplay->AvplayAudioType);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("call AVPLAY_GetAudType. ret=0x%x.\n", Ret);
        return Ret;
    }
    ...
    if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        ...
        memset(&AdecAttr, 0x0, sizeof(AdecAttr));
        ...
        Ret = HI_MPI_SYNC_SetMs12Flag(pAvplay->hSync, HI_FALSE);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_SYNC_SetMs12Flag failed(0x%x)!\n", Ret);
            return Ret;
        }

        if (bRenderSupport)
        {
            Ret = HI_MPI_AO_Track_SetRenderAttr(pAvplay->hSyncTrack, pAdecAttr);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_AVPLAY("HI_MPI_AO_Track_SetRenderAttr failed(0x%x)!\n", Ret);
                return Ret;
            }
        }
    ...
    {
        Ret = HI_MPI_AO_Track_SetRenderAttr(pAvplay->hRenderTrack, pAdecAttr);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_AO_Track_SetRenderAttr failed(0x%x)!\n", Ret);
            return Ret;
        ...
        Ret = HI_MPI_SYNC_SetMs12Flag(pAvplay->hSync, HI_TRUE);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_SYNC_SetMs12Flag failed(0x%x)!\n", Ret);
            return Ret;
...
HI_S32 AVPLAY_SaveExternalAudiocAttr(AVPLAY_S *pAvplay, const HI_UNF_ACODEC_ATTR_S *pAdecAttr)
{
    memcpy(&pAvplay->stExternalAudAttr,pAdecAttr,sizeof(HI_UNF_ACODEC_ATTR_S));
    return HI_SUCCESS;
}
...
HI_S32 AVPLAY_SetNormalAudPid(AVPLAY_S *pAvplay,  const HI_U32 Pid)
{
     HI_S32 Ret = HI_SUCCESS;
     HI_U32 i = 0;
     ...
     else
     {
         ...
         pAvplay->MultiAudSyncInfo[i].Pts = HI_INVALID_PTS;
         pAvplay->MultiAudSyncInfo[i].AudSyncAdjust = HI_FALSE;
         ...
         Ret = HI_MPI_AO_Track_Stop(pAvplay->hSyncTrack);
         if (HI_SUCCESS != Ret)
         {
             HI_ERR_AVPLAY("HI_MPI_AO_Track_Stop fail 0x%x\n", Ret);
         ...
         /* set adec attr may change audio type*/
        if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            (HI_VOID)HI_MPI_AO_Track_SetSyncMode(pAvplay->hRenderTrack, HI_MPI_AO_TRACK_SYNCMODE_STARTOUTPUT);
            (HI_VOID)HI_MPI_AO_Track_Start(pAvplay->hRenderTrack);

        }
        else if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        {
            (HI_VOID)HI_MPI_ADEC_Start(pAvplay->hAdec);
            (HI_VOID)HI_MPI_AO_Track_Start(pAvplay->hSyncTrack);
        }
...
HI_S32 AVPLAY_SetRenderAudPid(AVPLAY_S *pAvplay,  const HI_U32 Pid)
{
    HI_U32       i;
    HI_S32       Ret;
    ...
    if (HI_INVALID_HANDLE == pAvplay->hDmxAud[pAvplay->CurDmxAudChn])
    {
        HI_ERR_AVPLAY("aud chn is not create\n");
        return HI_FAILURE;
    }
    if (HI_INVALID_HANDLE == pAvplay->hRenderTrack)
    {
    ...
    else
    {
        ...
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_AO_Track_SetSyncMode fail 0x%x\n", Ret);
            AVPLAY_Mutex_UnLock(&pAvplay->AvplayAudThreadMutex);
            return Ret;
        }
        ...
        Ret = HI_MPI_AO_Track_Stop(pAvplay->hRenderTrack);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_AO_Track_Stop fail 0x%x\n", Ret);
            AVPLAY_Mutex_UnLock(&pAvplay->AvplayAudThreadMutex);
...
HI_S32 AVPLAY_SetPid(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_ATTR_ID_E enAttrID, const HI_U32 Pid)
{
    ...
    if (HI_UNF_AVPLAY_ATTR_ID_AUD_PID == enAttrID)
    {
        if(AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        {
             Ret = AVPLAY_SetNormalAudPid(pAvplay, Pid);
             if(Ret != HI_SUCCESS)
             {
                 HI_ERR_AVPLAY("Call AVPLAY_SetNormalAudPid failed!.\n");
                 return HI_ERR_AVPLAY_INVALID_OPT;
             }
        }
        ...
        if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            Ret = AVPLAY_SetRenderAudPid(pAvplay, Pid);
            if(Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("Call AVPLAY_SetRenderAudPid failed!.\n");
                return HI_ERR_AVPLAY_INVALID_OPT;
            }
        }
    }
    ...
    else
    {
        ...
        else if (pAvplay->CurStatus != HI_UNF_AVPLAY_STATUS_STOP)
        {
...
HI_S32 AVPLAY_SetLowDelay(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_LOW_DELAY_ATTR_S *pstAttr)
{
    ...
    if (pstAttr->bEnable)
    {
        ...
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_SYNC_GetAttr ERR, Ret=%#x\n", Ret);
            return Ret;
        }
        ...
        pAvplay->bSetProgressive = HI_TRUE;
    }
    else
    {
        /* set sync to audio */
...
HI_S32 AVPLAY_GetAdPid(const AVPLAY_S *pAvplay, HI_U32 *pPid)
{
    HI_S32       Ret = HI_SUCCESS;
    ...
    else if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        if (HI_INVALID_HANDLE == pAvplay->hRenderTrack)
        {
            HI_ERR_AVPLAY("aud chn is close, can not get aud pid.\n");
        ...
        Ret = HI_MPI_DMX_GetChannelPID(pAvplay->hDmxAdAud, pPid);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("call HI_MPI_DMX_GetChannelPID failed.\n");
        }
...
HI_S32 AVPLAY_SetAdPid(AVPLAY_S *pAvplay, const HI_U32 *u32AdPid)
{
    HI_S32                  Ret;
    HI_UNF_DMX_CHAN_ATTR_S  DmxChnAttr;
    ...
    if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        ...
        if (!pAvplay->AudEnable)
        {
            if ((*u32AdPid != DEMUX_INVALID_PID)
                && (*u32AdPid != pAvplay->AdAttr.u32AdPid))
            {
                if (HI_INVALID_HANDLE == pAvplay->hDmxAdAud)
                {
                ...
                if (HI_INVALID_HANDLE != pAvplay->hDmxAdAud)
                {
                    Ret = HI_MPI_DMX_SetChannelPID(pAvplay->hDmxAdAud, *u32AdPid);
                    if (HI_SUCCESS != Ret)
                    {
                        (HI_VOID)HI_MPI_DMX_DestroyChannel(pAvplay->hDmxAdAud);
                        pAvplay->hDmxAdAud = HI_INVALID_HANDLE;
...
HI_S32 AVPLAY_SetAdAttr(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_AD_ATTR_S *pstAttr)
{
    ...
    if (pAvplay->AudEnable)
    {
        HI_ERR_AVPLAY("aud channel is runing!\n");
        return HI_ERR_AVPLAY_INVALID_OPT;
    }

    if (HI_UNF_AVPLAY_STREAM_TYPE_TS != pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        HI_ERR_AVPLAY("ES streame type not support!\n");
        return HI_ERR_AVPLAY_INVALID_OPT;
    }

    if (abs(pstAttr->s16AdBalance) > AVPLAY_MAX_AD_BALANCE_VALUE)
    {
        HI_ERR_AVPLAY("s16AdBalance is invalid %d!\n", pstAttr->s16AdBalance);
        return HI_ERR_AVPLAY_INVALID_PARA;
    }

    if ((pstAttr->u32AdPid != DEMUX_INVALID_PID)
    ...
    /*Adapt audio driver AdBalance range:-32 ~ 32*/
    pAvplay->AdAttr.s16AdBalance = (HI_S16)(pstAttr->s16AdBalance/3.125);

    if (pAvplay->AdAttr.bAdEnble)
    {
        Ret = HI_MPI_AO_Track_IsRenderSupport(&bRender);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("call HI_MPI_AO_Track_IsRenderSupport Fail Ret:0x%x!\n", Ret);
            return Ret;
        }
        ...
        if (!bRender)
        {
            if (HI_INVALID_HANDLE == pAvplay->hAdAdec)
            {
                Ret = HI_MPI_ADEC_Open(&pAvplay->hAdAdec);
                if (Ret != HI_SUCCESS)
                {
                    HI_ERR_AVPLAY("HI_MPI_ADEC_Open failed 0x%x\n", Ret);
                    return Ret;
            ...
            if (HI_INVALID_HANDLE != pAvplay->hSyncTrack)
            {
                Ret = HI_MPI_AO_Track_SetAdBalance(pAvplay->hSyncTrack, pAvplay->AdAttr.s16AdBalance);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("HI_MPI_AO_Track_SetAdBalance failed 0x%x!\n", Ret);
                    return Ret;
                }
            }
        }
...
HI_S32 AVPLAY_StopVidChn_KeepSPSPPS(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_STOP_MODE_E enMode,HI_BOOL bKeep)
{
    HI_S32                  Ret;
...
static HI_S32 AVPLAY_StartAdAudChn(AVPLAY_S *pAvplay)
{
    HI_S32         Ret = HI_SUCCESS;
    ADEC_ATTR_S    AdecAttr;
    ...
    if (HI_INVALID_HANDLE != pAvplay->hAdAdec)
    {
        ...
        Ret = HI_MPI_ADEC_SetAllAttr(pAvplay->hAdAdec, &AdecAttr);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("HI_MPI_ADEC_GetAllAttr failed 0x%x\n", Ret);
            return Ret;
        ...
        Ret = HI_MPI_ADEC_Start(pAvplay->hAdAdec);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("HI_MPI_ADEC_Start Ad adec failed 0x%x\n", Ret);
            return Ret;
        }
    }
...
static HI_S32 AVPLAY_StartNormalAudChn(AVPLAY_S *pAvplay)
{
    HI_S32         Ret;
    HI_U32         i, j;
...
static HI_S32 AVPLAY_StartRenderAudChn(AVPLAY_S *pAvplay)
{
    HI_S32         Ret;

    /* start aud track after sync proc*/
    ...
    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        Ret = HI_MPI_AO_Track_SetTargetPts(pAvplay->hRenderTrack, 0);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_AO_Track_SetTargetPts fail 0x%x\n", Ret);
            return Ret;
        }
        HI_WARN_AVPLAY("set target Pts to Render Track succeed,pts=%d\n", 0);
    }

    Ret = HI_MPI_AO_Track_Start(pAvplay->hRenderTrack);
    if (HI_SUCCESS != Ret)
    {
        HI_INFO_AVPLAY("HI_MPI_AO_Track_Start fail 0x%x\n", Ret);
        return Ret;
    }
    HI_WARN_AVPLAY("start  AO Render Track\n");

    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        Ret = HI_MPI_DMX_OpenChannel(pAvplay->hDmxAud[pAvplay->CurDmxAudChn]);
        if (HI_SUCCESS != Ret)
        {
            HI_INFO_AVPLAY("HI_MPI_DMX_OpenChannel fail 0x%x\n", Ret);
            return Ret;
        }
...
static HI_S32 AVPLAY_StartAudChn(AVPLAY_S *pAvplay)
{
    ...
    if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        Ret = AVPLAY_StartNormalAudChn(pAvplay);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_StartNormalAudChn failed 0x%x\n", Ret);
            return Ret;
         }

        Ret = AVPLAY_StartAdAudChn(pAvplay);
        if(Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_StartAdAudChn failed 0x%x\n", Ret);
            return Ret;
        }
    }
    ...
    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        Ret = AVPLAY_StartRenderAudChn(pAvplay);
        if(Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_StartRenderAudChn failed 0x%x\n", Ret);
            return Ret;
         }
    }
...
static HI_S32 AVPLAY_StopAdAudChn(AVPLAY_S *pAvplay)
{
    HI_S32         Ret;
    ...
    if (HI_INVALID_HANDLE != pAvplay->hAdAdec)
    {
        ...
        Ret = HI_MPI_ADEC_Stop(pAvplay->hAdAdec);
        if (Ret != HI_SUCCESS)
        {
...
static HI_S32 AVPLAY_StopNormalAudChn(AVPLAY_S *pAvplay)
{
    HI_S32         Ret;
    HI_U32         i;
...
static HI_S32 AVPLAY_StopRenderAudChn(AVPLAY_S *pAvplay)
{
    HI_S32         Ret;
    ...
    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        ...
        if (HI_INVALID_HANDLE != pAvplay->hDmxAdAud)
        {
            Ret = HI_MPI_DMX_CloseChannel(pAvplay->hDmxAdAud);
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("HI_MPI_DMX_CloseChannel failed 0x%x\n", Ret);
            }
...
static HI_S32 AVPLAY_StopAudChn(AVPLAY_S *pAvplay)
{
    ...
    if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        Ret = AVPLAY_StopNormalAudChn(pAvplay);
        if(Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_StopNormalAudChn failed 0x%x\n", Ret);
            return Ret;
         }

        Ret = AVPLAY_StopAdAudChn(pAvplay);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_StopAdAudChn failed 0x%x\n", Ret);
            return Ret;
        }
    }
    ...
    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        Ret = AVPLAY_StopRenderAudChn(pAvplay);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_StopRenderAudChn failed 0x%x\n", Ret);
            return Ret;
         }
    }
...
static HI_S32 AVPLAY_StopVideo(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_STOP_OPT_S *pStop)
{
    ...
    if (pAvplay->VidEnable)
    {
        ...
        if (pAvplay->bSetCpuMinFreq)
        {
            Ret = HI_MPI_PMOC_SetCPUMinFreq(HI_FALSE, AVPLAY_CPU_SCALE_MIN_FREQ);
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("HI_MPI_PMOC_SetCPUMinFreq failed 0x%x\n", Ret);
                return Ret;
            }

            pAvplay->bSetCpuMinFreq = HI_FALSE;
        }

    #ifdef AVPLAY_UVMOS_SUPPORT
        Ret = AVPLAY_List_FrameInfoDeInit(pAvplay);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("AVPLAY_List_FrameInfoDeInit Fail\n");
        }
    #endif

        pAvplay->VidEnable = HI_FALSE;
        pAvplay->bVidPreEnable = HI_FALSE;
        pAvplay->bKeepSPSPPSFlag = HI_FALSE;
...
HI_S32 AVPLAY_StopNormalAudio(AVPLAY_S *pAvplay)
{
    HI_S32  Ret;
    HI_U32  i;
...
HI_S32 AVPLAY_StopRenderAudio(AVPLAY_S *pAvplay)
{
    HI_S32  Ret;
    ...
    {
         HI_ERR_AVPLAY("unsupport both FCC and MS12 exist now.\n");
    }
    return HI_SUCCESS;
}
...
static HI_S32 AVPLAY_StopAudio(AVPLAY_S *pAvplay)
{
    ...
    if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        Ret = AVPLAY_StopNormalAudio(pAvplay);
        if(Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_StopNormalAudChn failed 0x%x\n", Ret);
            return Ret;
    ...
    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        Ret = AVPLAY_StopRenderAudio(pAvplay);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_StopRenderAudChn failed 0x%x\n", Ret);
            return Ret;
...
HI_S32 AVPLAY_RetachWindow(AVPLAY_S *pAvplay)
{
    HI_S32 Ret;
    HI_S32 i;
    HI_HANDLE MasterWin;
    HI_HANDLE VirWin[AVPLAY_MAX_VIR_FRMCHAN];
    MasterWin = pAvplay->MasterFrmChn.hWindow;
    if (HI_INVALID_HANDLE != MasterWin)
    {
       AVPLAY_DetachWindow(pAvplay, MasterWin);
    }
    for(i = 0;i < AVPLAY_MAX_VIR_FRMCHAN;i++)
    {
        VirWin[i] = pAvplay->VirFrmChn[i].hWindow;
        if(HI_INVALID_HANDLE != VirWin[i])
        {
           AVPLAY_DetachWindow(pAvplay, VirWin[i]);
        }
    }
    Ret = AVPLAY_MallocVidBuffer(pAvplay, pAvplay->hVdec);
    if (Ret != HI_SUCCESS)
    ...
    if (MasterWin != HI_INVALID_HANDLE)
    {
      AVPLAY_AttachWindow(pAvplay,MasterWin);
    }
    for(i = 0; i < AVPLAY_MAX_VIR_FRMCHAN;i++)
    {
        if(HI_INVALID_HANDLE != VirWin[i])
        {
           AVPLAY_AttachWindow(pAvplay, VirWin[i]);
        }
    }
    return Ret;
}

HI_BOOL AVPLAY_CheckIfNeedBoostCpu(AVPLAY_S *pAvplay)
{
    if (   (HI_UNF_VCODEC_TYPE_WMV1 == pAvplay->VdecAttr.enType)
        || (HI_UNF_VCODEC_TYPE_WMV2 == pAvplay->VdecAttr.enType)
        || (HI_UNF_VCODEC_TYPE_MSMPEG4V1 == pAvplay->VdecAttr.enType)
        || (HI_UNF_VCODEC_TYPE_MSMPEG4V2 == pAvplay->VdecAttr.enType)
        || (HI_UNF_VCODEC_TYPE_RV10 == pAvplay->VdecAttr.enType)
        || (HI_UNF_VCODEC_TYPE_RV20 == pAvplay->VdecAttr.enType)
        || (HI_UNF_VCODEC_TYPE_SVQ1 == pAvplay->VdecAttr.enType)
        || (HI_UNF_VCODEC_TYPE_SVQ3 == pAvplay->VdecAttr.enType))
    {
        return HI_TRUE;
    }
...
HI_S32 AVPLAY_StartVideo(AVPLAY_S *pAvplay)
{
    ...
    if (AVPLAY_CheckIfNeedBoostCpu(pAvplay))
    {
        Ret = HI_MPI_PMOC_SetCPUMinFreq(HI_TRUE, AVPLAY_CPU_SCALE_MIN_FREQ);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("HI_MPI_PMOC_SetCPUMinFreq failed 0x%x\n", Ret);
        }

        pAvplay->bSetCpuMinFreq = HI_TRUE;
    }

#ifdef AVPLAY_UVMOS_SUPPORT
    Ret = AVPLAY_List_FrameInfoInit(pAvplay);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("AVPLAY_List_FrameInfoInit Fail\n");
    }
#endif
...
HI_VOID AVPLAY_Stop(AVPLAY_S *pAvplay)
{
    ...
    /*
     * AVPLAY reset operation can not change player status,
     * clear status info when performing a stop operation.
    */
#ifdef HI_HDR_SUPPORT
    pAvplay->bPaddingHdrInfo = HI_FALSE;
    pAvplay->bSetHdrInfo = HI_FALSE;

    memset(&pAvplay->stHdrInfo, 0x0, sizeof(HI_UNF_VIDEO_HDR_INFO_S));
    pAvplay->stHdrInfo.enHdrType = HI_UNF_VIDEO_HDR_TYPE_BUTT;
#endif

    pAvplay->AvplayJumpPlayInfo.bAudFirstPlay = HI_FALSE;
    pAvplay->AvplayJumpPlayInfo.bVidFirstPlay = HI_FALSE;
    pAvplay->AvplayJumpPlayInfo.bJumpPlayMode = HI_FALSE;
    pAvplay->AvplayJumpPlayInfo.IdrPts        = HI_INVALID_PTS;
    pAvplay->AvplayJumpPlayInfo.TargetPts     = HI_INVALID_PTS;
...
HI_S32 AVPLAY_StartNormalAudio(AVPLAY_S *pAvplay)
{
    HI_S32      Ret;
...
HI_S32 AVPLAY_StartRenderAudio(AVPLAY_S *pAvplay)
{
    HI_S32      Ret;
...
HI_S32 AVPLAY_StartAudio(AVPLAY_S *pAvplay)
{
    ...
    if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        Ret = AVPLAY_StartNormalAudio(pAvplay);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("call AVPLAY_StartNormalAudChn failed.Ret 0x%x\n",Ret);
            return Ret;
        }
    }
    ...
    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        Ret = AVPLAY_StartRenderAudio(pAvplay);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("call AVPLAY_AUDIO_TYPE_RENDER failed.Ret 0x%x\n",Ret);
            return Ret;
        }
    }
...
HI_S32  AVPLAY_SetSyncTPlay(AVPLAY_S* pAvplay)
{
    HI_S32 Ret = HI_SUCCESS;

#ifdef AVPLAY_AUDIO_TPLAY_SUPPORT
    if (pAvplay->bAudTplay)
    {
        pAvplay->DebugInfo.VidFrameDropNum = 0;
        pAvplay->bSyncFinish = HI_FALSE;
        (HI_VOID)HI_MPI_SYNC_SetSyncFinishFlag(pAvplay->hSync, HI_FALSE);
        Ret = HI_MPI_SYNC_Play(pAvplay->hSync);
        if (Ret != HI_SUCCESS)
        {
...
HI_S32 AVPLAY_Reset(AVPLAY_S *pAvplay)
{
    ...
    if (pAvplay->VidEnable)
    {
        Ret = AVPLAY_StopVidChn_KeepSPSPPS(pAvplay, HI_UNF_AVPLAY_STOP_MODE_STILL,pAvplay->bKeepSPSPPSFlag);
        if (Ret != HI_SUCCESS)
        {
            return Ret;
        }
    ...
    {
        Ret = AVPLAY_SetSyncTPlay(pAvplay);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("HI_MPI_SYNC_Play failed 0x%x\n", Ret);
        }
...
HI_S32 AVPLAY_SetMultiAud(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_MULTIAUD_ATTR_S *pAttr)
{
    ...
    memset(pAvplay->pstAcodecAttr, 0, sizeof(HI_UNF_ACODEC_ATTR_S)*pAttr->u32PidNum);
...
HI_S32 AVPLAY_SetTVP(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_TVP_ATTR_S *pstAttr)
{
    ...
    if (HI_INVALID_HANDLE == pAvplay->MasterFrmChn.hWindow)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            ...
            HI_ERR_AVPLAY("Avplay set vdec attr failed 0x%x\n", Ret);
            return Ret;
        }

        if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
        {
            if (pAvplay->DmxVidPid != DEMUX_INVALID_PID)
            {
                Ret = AVPLAY_SetPid(pAvplay, HI_UNF_AVPLAY_ATTR_ID_VID_PID,pAvplay->DmxVidPid);
                if (Ret != HI_SUCCESS)
                {
                    (HI_VOID)AVPLAY_FreeVidChn(pAvplay);
                    HI_ERR_AVPLAY("Avplay set vid pid failed 0x%x\n", Ret);
                    return Ret;
                }
            }
        }
...
static HI_VOID AVPLAY_BasicProcInfo(AVPLAY_S *pAvplay, HI_PROC_SHOW_BUFFER_S *ProcBuf)
{
    ...
    HI_PROC_Printf(ProcBuf,
                    ...
                    "CpuFreqScheTimeCnt    :%-10u   |bSetCpuMinFreq        :%-10u\n",
                    ...
                    pAvplay->DebugInfo.CpuFreqScheTimeCnt,      pAvplay->bSetCpuMinFreq
                    );
...
static HI_VOID AVPLAY_VidProcInfo(AVPLAY_S *pAvplay, HI_PROC_SHOW_BUFFER_S *ProcBuf)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        pFrameRateType = "UNKOWN";
    }

    switch (FrameRate.enFrmRateType)
    {
        case HI_UNF_AVPLAY_FRMRATE_TYPE_PTS :
            pFrameRateType = "PTS";
            break;
        case HI_UNF_AVPLAY_FRMRATE_TYPE_STREAM :
            pFrameRateType = "STREAM";
            break;
        case HI_UNF_AVPLAY_FRMRATE_TYPE_USER :
            pFrameRateType = "USER";
            break;
        case HI_UNF_AVPLAY_FRMRATE_TYPE_USER_PTS :
            pFrameRateType = "USER_PTS";
            break;
        default :
            pFrameRateType = "UNKNOWN";
            break;
    }

    if (HI_UNF_AVPLAY_FRMRATE_TYPE_USER == FrameRate.enFrmRateType)
    {
        snprintf(szFrcInRate, sizeof(szFrcInRate), "%d.%03d",
            pAvplay->FrcParamCfg.u32InRate/1000, pAvplay->FrcParamCfg.u32InRate%1000);

        snprintf(szFrcOutRate, sizeof(szFrcOutRate), "%d.%03d",
            pAvplay->FrcParamCfg.u32OutRate/1000, pAvplay->FrcParamCfg.u32OutRate%1000);
    }
    else
    {
    ...
    HI_PROC_Printf(ProcBuf,
                    ...
                    "FrameRateType         :%-10s   |ProgressiveEnable     :%-10s\n"
                    "VidFrameDropNum       :%-10d\n",
                    ...
                    pAvplay->hVdec & 0xff,
                    pFrameRateType,
                    (pAvplay->bSetProgressive) ? "TRUE" : "FALSE",
                    pAvplay->DebugInfo.VidFrameDropNum);

#ifdef HI_HDR_SUPPORT
    if (pAvplay->bPaddingHdrInfo)
    {
        switch (pAvplay->stAvplayHdrInfo.enHdrType)
        {
            case AVPLAY_HDR_TYPE_HDR10 :
                Vp9HdrType = "HDR10";
                break;
            case AVPLAY_HDR_TYPE_HLG :
                Vp9HdrType = "HLG";
                break;
            default :
                Vp9HdrType = "N/A";
    ...
    HI_PROC_Printf(ProcBuf, "VP9HDREnable          :%-10s   |VP9HDRType            :%-10s\n",
        pAvplay->bPaddingHdrInfo ? "TRUE" : "FALSE", Vp9HdrType);
#endif
...
static HI_VOID AVPLAY_AudProcInfo(AVPLAY_S *pAvplay, HI_PROC_SHOW_BUFFER_S *ProcBuf)
{
    ...
    if  (HI_FALSE == pAvplay->bCurExternalAud)
    {
        HI_PROC_Printf(ProcBuf,
                      "------------------------------AUD CHANNEL--------------------------\n"
                        ...
                        HI_UNF_STREAM_BUF_S  *pstData)
{
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_AUD == enBufId)
    {
        ...
        if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            ...
            Ret = HI_MPI_AO_Track_GetBuffer(pAvplay->hRenderTrack,HI_MPI_AO_BUF_ID_ES_MAIN, u32ReqLen, &pAvplay->AvplayAudEsBuf);
            if (Ret != HI_SUCCESS)
            {
                HI_INFO_AVPLAY("HI_MPI_AO_Track_GetBuffer failed!Ret 0x%x!\n",Ret);
                return Ret;
            }
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_AUD_AD == enBufId)
    {
        if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        {
            ...
            if (HI_INVALID_HANDLE == pAvplay->hAdAdec)
            {
                HI_WARN_AVPLAY("ad aud chn not open.\n");
                return HI_ERR_AVPLAY_INVALID_OPT;
            }

            pAvplay->DebugInfo.AcquireAudADEsNum++;

            Ret = HI_MPI_ADEC_GetBuffer(pAvplay->hAdAdec, u32ReqLen, &pAvplay->AvplayAdAudEsBuf);
            if (Ret != HI_SUCCESS)
            {
                if ((Ret != HI_ERR_ADEC_IN_BUF_FULL) && (Ret != HI_ERR_ADEC_IN_PTSBUF_FULL) )
                {
                    HI_ERR_AVPLAY("call HI_MPI_ADEC_GetADBuffer failed, Ret=0x%x.\n", Ret);
                }
        ...
        if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            ...
            Ret = HI_MPI_AO_Track_GetBuffer(pAvplay->hRenderTrack, HI_MPI_AO_BUF_ID_ES_ASSOC, u32ReqLen, &pAvplay->AvplayAdAudEsBuf);
            if (Ret != HI_SUCCESS)
            {
                return Ret;
            }
                        ...
                        HI_UNF_AVPLAY_PUTBUFEX_OPT_S *pstExOpt)
{
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_VID_HDR_DL == enBufId)
    {
        ...
        if (HI_NULL == pAvplay->AvplayElEsBuf.pu8Addr ||  HI_NULL == pAvplay->AvplayBlEsBuf.pu8Addr)
        {
            HI_ERR_AVPLAY("para invaild !\n");
            return HI_ERR_AVPLAY_INVALID_PARA;
        }
                        ...
                        HI_U32                        u32Pts)
{
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_AUD == enBufId)
    {
        ...
        if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            ...
            Ret = HI_MPI_AO_Track_PutBuffer(pAvplay->hRenderTrack, HI_MPI_AO_BUF_ID_ES_MAIN, &pAvplay->AvplayAudEsBuf, u32Pts);
            if (Ret != HI_SUCCESS)
            {
                HI_INFO_AVPLAY("HI_MPI_AO_Track_PutBuffer failed 0x%x\n", Ret);
                return Ret;
            }
           // HI_ERR_AVPLAY("put ms12 render track buffer ok!\n");
        }
    }
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_AUD_AD == enBufId)
    {
        ...
        if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        {
            if (!pAvplay->AdAttr.bAdEnble)
            {
                HI_ERR_AVPLAY("aud chn not support AD.\n");
                return HI_ERR_AVPLAY_INVALID_OPT;
            }

            if (HI_INVALID_HANDLE == pAvplay->hAdAdec)
            {
                HI_ERR_AVPLAY("ad aud chn not open.\n");
                return HI_ERR_AVPLAY_INVALID_OPT;
            }

            pAvplay->AvplayAdAudEsBuf.u32Size = u32ValidDataLen;

            Ret = HI_MPI_ADEC_PutBuffer(pAvplay->hAdAdec, &pAvplay->AvplayAdAudEsBuf, u32Pts);
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("call HI_MPI_ADEC_PutADBuffer failed.\n");
                return Ret;
            }
        }
        else if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            ...
            pAvplay->AvplayAdAudEsBuf.u32Size = u32ValidDataLen;
            Ret = HI_MPI_AO_Track_PutBuffer(pAvplay->hRenderTrack, HI_MPI_AO_BUF_ID_ES_ASSOC, &pAvplay->AvplayAudEsBuf, u32Pts);
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("HI_MPI_AO_Track_PutBuffer failed 0x%x\n", Ret);
                return Ret;
            }
        }
    }
...
#ifdef HI_HDR_SUPPORT
/*****VP9 HDR *****/
static HI_VOID AVPLAY_ColorSpaceConvert_VP9(HDR_INFO_CSC_INPUT_S *pstCscInput, HDR_INFO_CSC_OUTPUT_S *pstCscOutput)    //For HDR only
{
    if(9 == pstCscInput->u8ColourPrimaries && 9 == pstCscInput->u8MatrixCoeffs)            //BT2020NCL
    {
        pstCscOutput->enLumaSys = HI_DRV_BT2020_NON_CONSTANCE_LUMINACE_SYS;
        if(1 == pstCscInput->bFullRangeFlag)
        {
            pstCscOutput->enColorSpace = HI_DRV_CS_BT2020_YUV_FULL;
        }
        else
        {
             pstCscOutput->enColorSpace = HI_DRV_CS_BT2020_YUV_LIMITED;
        }
    }
    else if(9 == pstCscInput->u8ColourPrimaries && 10==pstCscInput->u8MatrixCoeffs)    //BT2020CL
    {
        pstCscOutput->enLumaSys = HI_DRV_BT2020_CONSTANCE_LUMINACE_SYS;
        if(1== pstCscInput->bFullRangeFlag)
        {
             pstCscOutput->enColorSpace = HI_DRV_CS_BT2020_YUV_FULL;
        }
    ...
    else if(1==pstCscInput->u8ColourPrimaries && 1==pstCscInput->u8MatrixCoeffs)    //BT709
    {
        pstCscOutput->enLumaSys = HI_DRV_BT2020_LUMINACE_SYS_BUTT;
        if(1==pstCscInput->bFullRangeFlag)
        {
             pstCscOutput->enColorSpace = HI_DRV_CS_BT709_YUV_FULL;
        }
        else
        {
             pstCscOutput->enColorSpace = HI_DRV_CS_BT709_YUV_LIMITED;
        }
    }
    else
    {
...
HI_VOID AVPLAY_GammaTypeConvert_VP9(HDR_INFO_GAMMA_INPUT_S *pstGammaInput, HDR_INFO_GAMMA_OUTPUT_S *pstGammaOutput)
{
    switch (pstGammaInput->u8TransferCharacteristics)
    {
        case 1:                            /*BT709 - SDR*/
            pstGammaOutput->enGammaType = HI_DRV_GAMMA_TYPE_1886;
            pstGammaOutput->bBackwardCompatible = HI_FALSE;
            break;
        case 16:                           /*SMPTE-ST-2084 - PQ*/
            pstGammaOutput->enGammaType = HI_DRV_GAMMA_TYPE_2084;
            pstGammaOutput->bBackwardCompatible = HI_FALSE;
            break;
...
HI_VOID AVPLAY_HDRMetadataConvert(HDR_METADATA_INFO_S *pstInMetadataInfo, HDR_METADATA_INFO_S *pstOutMetadataInfo)
{
    pstOutMetadataInfo->bMasteringAvailable = pstInMetadataInfo->bMasteringAvailable;
    pstOutMetadataInfo->stMasteringInfo.u16DisplayPrimaries_x[0] = pstInMetadataInfo->stMasteringInfo.u16DisplayPrimaries_x[0];
    pstOutMetadataInfo->stMasteringInfo.u16DisplayPrimaries_y[0] = pstInMetadataInfo->stMasteringInfo.u16DisplayPrimaries_y[0];
    pstOutMetadataInfo->stMasteringInfo.u16DisplayPrimaries_x[1] = pstInMetadataInfo->stMasteringInfo.u16DisplayPrimaries_x[1];
    pstOutMetadataInfo->stMasteringInfo.u16DisplayPrimaries_y[1] = pstInMetadataInfo->stMasteringInfo.u16DisplayPrimaries_y[1];
    pstOutMetadataInfo->stMasteringInfo.u16DisplayPrimaries_x[2] = pstInMetadataInfo->stMasteringInfo.u16DisplayPrimaries_x[2];
    pstOutMetadataInfo->stMasteringInfo.u16DisplayPrimaries_y[2] = pstInMetadataInfo->stMasteringInfo.u16DisplayPrimaries_y[2];
    pstOutMetadataInfo->stMasteringInfo.u16WhitePoint_x = pstInMetadataInfo->stMasteringInfo.u16WhitePoint_x;
    pstOutMetadataInfo->stMasteringInfo.u16WhitePoint_y = pstInMetadataInfo->stMasteringInfo.u16WhitePoint_y;
    pstOutMetadataInfo->stMasteringInfo.u32MaxDisplayMasteringLuminance = pstInMetadataInfo->stMasteringInfo.u32MaxDisplayMasteringLuminance;
    pstOutMetadataInfo->stMasteringInfo.u32MinDisplayMasteringLuminance =pstInMetadataInfo->stMasteringInfo.u32MinDisplayMasteringLuminance;

    pstOutMetadataInfo->bContentAvailable = pstInMetadataInfo->bContentAvailable;
    pstOutMetadataInfo->stContentInfo.u32MaxContentLightLevel =  pstInMetadataInfo->stContentInfo.u32MaxContentLightLevel;
    pstOutMetadataInfo->stContentInfo.u32MaxPicAverageLightLevel =  pstInMetadataInfo->stContentInfo.u32MaxPicAverageLightLevel;
    return;
}

HI_VOID AVPLAY_HDR10ConvertUnf2Drv(const HI_UNF_VIDEO_HDR10_INFO_S *pstUnfHdr10Info, HI_DRV_VIDEO_HDR10_INFO_S *pstDrvHdr10Info,HI_DRV_COLOR_SPACE_E  *penColorSpace)
{
    HI_U32                  u32Index = 0;
    HDR_INFO_CSC_INPUT_S    stCscInput = {0};
    HDR_INFO_CSC_OUTPUT_S   stCscOutput = {0};
    HDR_METADATA_INFO_S     stInMetadataInfo;
    HDR_METADATA_INFO_S     stOutMetadataInfo;

    memset(&stInMetadataInfo, 0x0, sizeof(HDR_METADATA_INFO_S));
    memset(&stOutMetadataInfo, 0x0, sizeof(HDR_METADATA_INFO_S));


    /*get colorspace info*/
    stCscInput.bFullRangeFlag = pstUnfHdr10Info->u8FullRangeFlag == 0 ? HI_FALSE : HI_TRUE;
    stCscInput.u8ColourPrimaries = pstUnfHdr10Info->u8ColourPrimaries;
    stCscInput.u8MatrixCoeffs = pstUnfHdr10Info->u8MatrixCoeffs;
    AVPLAY_ColorSpaceConvert_VP9(&stCscInput, &stCscOutput);
    *penColorSpace = stCscOutput.enColorSpace;

    /*get metadata info*/
    stInMetadataInfo.bMasteringAvailable = pstUnfHdr10Info->u8MasteringAvailable == 0 ? HI_FALSE : HI_TRUE;
    stInMetadataInfo.bContentAvailable = pstUnfHdr10Info->u8ContentAvailable == 0 ? HI_FALSE : HI_TRUE;
    for(u32Index = 0; u32Index < 3; u32Index++)
    {
        stInMetadataInfo.stMasteringInfo.u16DisplayPrimaries_x[u32Index]= pstUnfHdr10Info->stMasteringInfo.u16DisplayPrimaries_x[u32Index];
        stInMetadataInfo.stMasteringInfo.u16DisplayPrimaries_y[u32Index]= pstUnfHdr10Info->stMasteringInfo.u16DisplayPrimaries_y[u32Index];
    }
    stInMetadataInfo.stMasteringInfo.u16WhitePoint_x = pstUnfHdr10Info->stMasteringInfo.u16WhitePoint_x;
    stInMetadataInfo.stMasteringInfo.u16WhitePoint_y = pstUnfHdr10Info->stMasteringInfo.u16WhitePoint_y;
    stInMetadataInfo.stMasteringInfo.u32MaxDisplayMasteringLuminance = pstUnfHdr10Info->stMasteringInfo.u32MaxDisplayMasteringLuminance;
    stInMetadataInfo.stMasteringInfo.u32MinDisplayMasteringLuminance = pstUnfHdr10Info->stMasteringInfo.u32MinDisplayMasteringLuminance;

    stInMetadataInfo.stContentInfo.u32MaxContentLightLevel = pstUnfHdr10Info->stContentInfo.u32MaxContentLightLevel;
    stInMetadataInfo.stContentInfo.u32MaxPicAverageLightLevel = pstUnfHdr10Info->stContentInfo.u32MaxPicAverageLightLevel;

    AVPLAY_HDRMetadataConvert(&stInMetadataInfo, &stOutMetadataInfo);


    memcpy(pstDrvHdr10Info->stMasteringInfo.u16DisplayPrimaries_x,stOutMetadataInfo.stMasteringInfo.u16DisplayPrimaries_x,3 * sizeof(HI_U16));
    memcpy(pstDrvHdr10Info->stMasteringInfo.u16DisplayPrimaries_y,stOutMetadataInfo.stMasteringInfo.u16DisplayPrimaries_y,3 * sizeof(HI_U16));

    pstDrvHdr10Info->stMasteringInfo.u16WhitePoint_x = stOutMetadataInfo.stMasteringInfo.u16WhitePoint_x;
    pstDrvHdr10Info->stMasteringInfo.u16WhitePoint_y = stOutMetadataInfo.stMasteringInfo.u16WhitePoint_y;
    pstDrvHdr10Info->stMasteringInfo.u32MaxDisplayMasteringLuminance = stOutMetadataInfo.stMasteringInfo.u32MaxDisplayMasteringLuminance;
    pstDrvHdr10Info->stMasteringInfo.u32MinDisplayMasteringLuminance = stOutMetadataInfo.stMasteringInfo.u32MinDisplayMasteringLuminance;
    pstDrvHdr10Info->stContentInfo.u32MaxContentLightLevel = stOutMetadataInfo.stContentInfo.u32MaxContentLightLevel;
    pstDrvHdr10Info->stContentInfo.u32MaxPicAverageLightLevel = stOutMetadataInfo.stContentInfo.u32MaxPicAverageLightLevel;

    pstDrvHdr10Info->u8MasteringAvailable = pstUnfHdr10Info->u8MasteringAvailable;
    pstDrvHdr10Info->u8TransferCharacteristics = pstUnfHdr10Info->u8TransferCharacteristics;
    pstDrvHdr10Info->u8ContentAvailable = pstUnfHdr10Info->u8ContentAvailable;

    return;
}

HI_VOID AVPLAY_HLGConvertUnf2Drv(const HI_UNF_VIDEO_HLG_INFO_S *pstUnfHlgInfo, HI_DRV_VIDEO_HLG_INFO_S *pstDrvHlgInfo,HI_DRV_COLOR_SPACE_E  *penColorSpace)
{
    HI_U32                  u32Index = 0;
    HDR_INFO_CSC_INPUT_S    stCscInput = {0};
    HDR_INFO_CSC_OUTPUT_S   stCscOutput = {0};
    ...
    /*get colorspace info*/
    stCscInput.bFullRangeFlag = pstUnfHlgInfo->u8FullRangeFlag == 0 ? HI_FALSE : HI_TRUE;
    stCscInput.u8ColourPrimaries = pstUnfHlgInfo->u8ColourPrimaries;
    stCscInput.u8MatrixCoeffs = pstUnfHlgInfo->u8MatrixCoeffs;
    AVPLAY_ColorSpaceConvert_VP9(&stCscInput, &stCscOutput);

    *penColorSpace = stCscOutput.enColorSpace;

    stInMetadataInfo.bMasteringAvailable = pstUnfHlgInfo->u8MasteringAvailable == 0 ? HI_FALSE : HI_TRUE;
    stInMetadataInfo.bContentAvailable = pstUnfHlgInfo->u8ContentAvailable == 0 ? HI_FALSE : HI_TRUE;

    for(u32Index = 0; u32Index < 3; u32Index++)
    {
        stInMetadataInfo.stMasteringInfo.u16DisplayPrimaries_x[u32Index]= pstUnfHlgInfo->stMasteringInfo.u16DisplayPrimaries_x[u32Index];
        stInMetadataInfo.stMasteringInfo.u16DisplayPrimaries_y[u32Index]= pstUnfHlgInfo->stMasteringInfo.u16DisplayPrimaries_y[u32Index];
    }

    stInMetadataInfo.stMasteringInfo.u16WhitePoint_x = pstUnfHlgInfo->stMasteringInfo.u16WhitePoint_x;
    stInMetadataInfo.stMasteringInfo.u16WhitePoint_y = pstUnfHlgInfo->stMasteringInfo.u16WhitePoint_y;
    stInMetadataInfo.stMasteringInfo.u32MaxDisplayMasteringLuminance = pstUnfHlgInfo->stMasteringInfo.u32MaxDisplayMasteringLuminance;
    stInMetadataInfo.stMasteringInfo.u32MinDisplayMasteringLuminance = pstUnfHlgInfo->stMasteringInfo.u32MinDisplayMasteringLuminance;

    stInMetadataInfo.stContentInfo.u32MaxContentLightLevel = pstUnfHlgInfo->stContentInfo.u32MaxContentLightLevel;
    stInMetadataInfo.stContentInfo.u32MaxPicAverageLightLevel = pstUnfHlgInfo->stContentInfo.u32MaxPicAverageLightLevel;

    AVPLAY_HDRMetadataConvert(&stInMetadataInfo, &stOutMetadataInfo);

    memcpy(pstDrvHlgInfo->stMasteringInfo.u16DisplayPrimaries_x,stOutMetadataInfo.stMasteringInfo.u16DisplayPrimaries_x,3 * sizeof(HI_U16));
    memcpy(pstDrvHlgInfo->stMasteringInfo.u16DisplayPrimaries_y,stOutMetadataInfo.stMasteringInfo.u16DisplayPrimaries_y,3 * sizeof(HI_U16));

    pstDrvHlgInfo->stMasteringInfo.u16WhitePoint_x = stOutMetadataInfo.stMasteringInfo.u16WhitePoint_x;
    pstDrvHlgInfo->stMasteringInfo.u16WhitePoint_y = stOutMetadataInfo.stMasteringInfo.u16WhitePoint_y;
    pstDrvHlgInfo->stMasteringInfo.u32MaxDisplayMasteringLuminance = stOutMetadataInfo.stMasteringInfo.u32MaxDisplayMasteringLuminance;
    pstDrvHlgInfo->stMasteringInfo.u32MinDisplayMasteringLuminance = stOutMetadataInfo.stMasteringInfo.u32MinDisplayMasteringLuminance;

    pstDrvHlgInfo->stContentInfo.u32MaxContentLightLevel = stOutMetadataInfo.stContentInfo.u32MaxContentLightLevel;
    pstDrvHlgInfo->stContentInfo.u32MaxPicAverageLightLevel = stOutMetadataInfo.stContentInfo.u32MaxPicAverageLightLevel;

    pstDrvHlgInfo->u8MasteringAvailable = pstUnfHlgInfo->u8MasteringAvailable;
    pstDrvHlgInfo->u8TransferCharacteristics = pstUnfHlgInfo->u8TransferCharacteristics;
    pstDrvHlgInfo->u8ContentAvailable = pstUnfHlgInfo->u8ContentAvailable;
    return;
}

HI_VOID AVPLAY_HDRInfoConvertUnf2Avplay(const HI_UNF_VIDEO_HDR_INFO_S *pstHdrInfo, AVPLAY_HDR_INFO_S *pstAvplayHdrInfo)
{
    switch (pstHdrInfo->enHdrType)
    {
        case HI_UNF_VIDEO_HDR_TYPE_HDR10:
        {
            pstAvplayHdrInfo->enHdrType = AVPLAY_HDR_TYPE_HDR10;
            pstAvplayHdrInfo->ColourDesInfo.u8ColourPrimaries = pstHdrInfo->unHdrMetadata.stHdr10Info.u8ColourPrimaries;
            pstAvplayHdrInfo->ColourDesInfo.u8FullRangeFlag = pstHdrInfo->unHdrMetadata.stHdr10Info.u8FullRangeFlag;
            pstAvplayHdrInfo->ColourDesInfo.u8MatrixCoeffs = pstHdrInfo->unHdrMetadata.stHdr10Info.u8MatrixCoeffs;
            AVPLAY_HDR10ConvertUnf2Drv(&pstHdrInfo->unHdrMetadata.stHdr10Info, &pstAvplayHdrInfo->unHdrMetadata.stHdr10Info,&pstAvplayHdrInfo->enColorSpace);
            break;
        }

        case HI_UNF_VIDEO_HDR_TYPE_HLG:
        {
            pstAvplayHdrInfo->enHdrType = AVPLAY_HDR_TYPE_HLG;
            pstAvplayHdrInfo->ColourDesInfo.u8ColourPrimaries = pstHdrInfo->unHdrMetadata.stHlgInfo.u8ColourPrimaries;
            pstAvplayHdrInfo->ColourDesInfo.u8FullRangeFlag = pstHdrInfo->unHdrMetadata.stHlgInfo.u8FullRangeFlag;
            pstAvplayHdrInfo->ColourDesInfo.u8MatrixCoeffs = pstHdrInfo->unHdrMetadata.stHlgInfo.u8MatrixCoeffs;
            AVPLAY_HLGConvertUnf2Drv(&pstHdrInfo->unHdrMetadata.stHlgInfo, &pstAvplayHdrInfo->unHdrMetadata.stHlgInfo,&pstAvplayHdrInfo->enColorSpace);
            break;
        }

        default:
        {
            HI_ERR_AVPLAY("HDR Type is not found!\n");
            break;
        }
...
HI_S32 AVPLAY_GetStartPosition(AVPLAY_S* pAvplay, HI_UNF_AVPLAY_START_POSITION_ATTR_S* pStartPosAttr)
{
    pStartPosAttr->u32StartPosPts = pAvplay->AvplayJumpPlayInfo.TargetPts;
    pStartPosAttr->u32IdrPts = pAvplay->AvplayJumpPlayInfo.IdrPts;

    return HI_SUCCESS;
}

HI_S32 AVPLAY_SetStartPosition(AVPLAY_S* pAvplay, HI_UNF_AVPLAY_START_POSITION_ATTR_S* pStartPosAttr)
{
#ifndef AVPLAY_VID_PEEK_FRAME_SUPPORT

    HI_ERR_AVPLAY("Peek frame feature is not supported!\n");
    ...
    if (HI_INVALID_HANDLE == pAvplay->hVdec)
    {
        HI_ERR_AVPLAY("Vid channel is not open, return!\n");
        return HI_ERR_AVPLAY_INVALID_OPT;
    }
    ...
    pAvplay->AvplayJumpPlayInfo.bAudFirstPlay = HI_TRUE;
    pAvplay->AvplayJumpPlayInfo.bVidFirstPlay = HI_TRUE;
    pAvplay->AvplayJumpPlayInfo.bJumpPlayMode = HI_TRUE;
    pAvplay->AvplayJumpPlayInfo.IdrPts        = pStartPosAttr->u32IdrPts;
    pAvplay->AvplayJumpPlayInfo.TargetPts     = pStartPosAttr->u32StartPosPts;

    Ret = HI_MPI_VDEC_SetStartIDRPts(pAvplay->hVdec, pStartPosAttr->u32IdrPts);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("Call HI_MPI_VDEC_SetStartIDRPts fail 0x%x.\n", Ret);
        goto ERR_EXIT;
    }
...
HI_S32 HI_MPI_AVPLAY_Create(const HI_UNF_AVPLAY_ATTR_S *pstAvAttr, HI_HANDLE *phAvplay)
{
    ...
    pAvplay->stExternalAudAttr.enType = 0xffffffff;
    pAvplay->stNormalAudAttr.enType = 0xffffffff;
    pAvplay->bCurExternalAud = HI_FALSE;
    ...
    pAvplay->AvplayAudioType  = AVPLAY_AUDIO_TYPE_NORMAL;
    pAvplay->hRenderTrack = HI_INVALID_HANDLE;
    ...
    pAvplay->bTplayResetMode = HI_TRUE;
    AVPLAY_ResetProcFlag(pAvplay);
...
#ifdef HI_HDR_SUPPORT
    /*Initalize VP9/AVS2 HDR10 function*/
    pAvplay->bPaddingHdrInfo = HI_FALSE;
    pAvplay->bSetHdrInfo = HI_FALSE;
    memset(&pAvplay->stHdrInfo,0x0,sizeof(HI_UNF_VIDEO_HDR_INFO_S));
    pAvplay->stHdrInfo.enHdrType = HI_UNF_VIDEO_HDR_TYPE_BUTT;
...
HI_S32 HI_MPI_AVPLAY_SetAttr(HI_HANDLE hAvplay, HI_UNF_AVPLAY_ATTR_ID_E enAttrID, HI_VOID *pPara)
{
    ...
    switch (enAttrID)
    {
        ...
        case HI_UNF_AVPLAY_ATTR_ID_ADEC:
            ...
            if (HI_SUCCESS == Ret)
            {
                memcpy(&pAvplay->stNormalAudAttr, (HI_UNF_ACODEC_ATTR_S *)pPara, sizeof(HI_UNF_ACODEC_ATTR_S));
            }
            break;
        ...
        case HI_UNF_AVPLAY_ATTR_ID_AD_PID:
            Ret = AVPLAY_SetAdPid(pAvplay, (HI_U32 *)pPara);
            break;
        ...
        case HI_UNF_AVPLAY_ATTR_ID_START_POSITION:
            Ret = AVPLAY_SetStartPosition(pAvplay, (HI_UNF_AVPLAY_START_POSITION_ATTR_S *)pPara);
            break;
        case HI_UNF_AVPLAY_ATTR_ID_EXTERNAL_AUDIO:
            Ret = AVPLAY_SaveExternalAudiocAttr(pAvplay, (HI_UNF_ACODEC_ATTR_S *)pPara);
            break;
...
HI_S32 HI_MPI_AVPLAY_GetAttr(HI_HANDLE hAvplay, HI_UNF_AVPLAY_ATTR_ID_E enAttrID, HI_VOID *pPara)
{
    ...
    switch (enAttrID)
    {
        ...
        case HI_UNF_AVPLAY_ATTR_ID_AD_PID:
            Ret = AVPLAY_GetAdPid(pAvplay, (HI_U32 *)pPara);
            break;
        ...
        case HI_UNF_AVPLAY_ATTR_ID_START_POSITION:
            Ret = AVPLAY_GetStartPosition(pAvplay, (HI_UNF_AVPLAY_START_POSITION_ATTR_S *)pPara);
            break;
                                              ...
                                              HI_UNF_VIDEO_FRAME_INFO_S *pstCapPicture)
{
    ...
    if (HI_INVALID_HANDLE == pAvplay->MasterFrmChn.hWindow)
    {
        ...
        if (HI_SUCCESS != Ret)
        {
            ...
            HI_ERR_AVPLAY("HI_MPI_VDEC_SetPortType ERR, Ret=%#x\n", Ret);
...
HI_S32 HI_MPI_AVPLAY_ReleaseIFrame(HI_HANDLE hAvplay, HI_UNF_VIDEO_FRAME_INFO_S *pstCapPicture)
{
    ...
    if (HI_INVALID_HANDLE == pAvplay->MasterFrmChn.hWindow)
    {
        ...
        {
            AVPLAY_INST_UNLOCK(Id);
            HI_ERR_AVPLAY("HI_MPI_VDEC_DisablePort ERR, Ret=%#x\n", Ret);
            return Ret;
        }
...
HI_S32 HI_MPI_AVPLAY_SetDecodeMode(HI_HANDLE hAvplay, HI_UNF_VCODEC_MODE_E enDecodeMode)
{
    ...
    {
        pAvplay->VdecAttr.enMode = enDecodeMode;
    }
#endif
    ...
    )
{
    ...
    if (HI_UNF_AVPLAY_EVENT_GET_PCR_STC_VALUE == enEvent)
    {
        (HI_VOID)HI_MPI_SYNC_SetExternalPllAdjustMethod(pAvplay->hSync,HI_TRUE);
    }
...
HI_S32 HI_MPI_AVPLAY_UnRegisterEvent(HI_HANDLE hAvplay, HI_UNF_AVPLAY_EVENT_E enEvent)
{
    ...
    if (HI_UNF_AVPLAY_EVENT_GET_PCR_STC_VALUE == enEvent)
    {
        (HI_VOID)HI_MPI_SYNC_SetExternalPllAdjustMethod(pAvplay->hSync,HI_FALSE);
    }
...
HI_S32 HI_MPI_AVPLAY_FoundSupportDeoder(const HA_FORMAT_E enFormat,HI_U32 * penDstCodecID)
{
    ...
    if (HI_NULL == penDstCodecID)
    {
        HI_ERR_AVPLAY("para penDstCodecID is null.\n");
        return HI_ERR_AVPLAY_NULL_PTR;
    }
...
HI_S32 HI_MPI_AVPLAY_Start(HI_HANDLE hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_E enChn)
{
    ...
    if (HI_UNF_AVPLAY_MEDIA_CHAN_VID == enChn)
    {
        ...
        {
            AVPLAY_Play(pAvplay);
            HI_MPI_SYNC_Play(pAvplay->hSync);
        }
...
HI_S32 HI_MPI_AVPLAY_Pause(HI_HANDLE hAvplay)
{
    ...
    if (pAvplay->AudEnable)
    {
        if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        {
            ...
            HI_INFO_AVPLAY("set Normal Track to PAUSE!\n");
        }
        else if(AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
           Ret = HI_MPI_AO_Track_Pause(pAvplay->hRenderTrack);
           if (HI_SUCCESS != Ret)
           {
               HI_ERR_AVPLAY("HI_MPI_AO_Track_Pause fail 0x%x\n", Ret);
           }
           HI_WARN_AVPLAY("set Render Track to PAUSE!\n");
        }
    }
...
HI_S32 AVPLAY_SetAudTPlayEnable(AVPLAY_S* pAvplay, HI_U32 bEnable)
{
#ifdef AVPLAY_AUDIO_TPLAY_SUPPORT
    if (bEnable)
    {
        pAvplay->bAudTplayEnable = HI_TRUE;
    }
    else
    {
        pAvplay->bAudTplayEnable = HI_FALSE;
    }
...
HI_S32  AVPLAY_SetAudTPlay(AVPLAY_S* pAvplay, const HI_UNF_AVPLAY_TPLAY_OPT_S* pstTplayOpt)
{
#ifdef AVPLAY_AUDIO_TPLAY_SUPPORT
    HI_S32                  s32Ret = HI_FAILURE;
    ADEC_TPLAY_S            stTplayParam = {0};
    HI_U32                  u32SpeedInteger = 0;
    HI_U32                  u32SpeedDecimal = 0;

    if (!pAvplay->bAudTplayEnable)
    {
        pAvplay->bAudTplay = HI_FALSE;
        return HI_SUCCESS;
    }

    if (HI_TRUE != pAvplay->AudEnable)
    {
        HI_ERR_AVPLAY("Audio Not Start,can not set Tplay!\n");
        return HI_FAILURE;
    }

    if (AVPLAY_AUDIO_TYPE_NORMAL != pAvplay->AvplayAudioType)
    {
        HI_ERR_AVPLAY("Aud type is Render not support Tplay!\n");
        return HI_FAILURE;
    }

    u32SpeedInteger = pstTplayOpt->u32SpeedInteger;
    u32SpeedDecimal = pstTplayOpt->u32SpeedDecimal;

    if (   (u32SpeedInteger == 2 && u32SpeedDecimal == 0)
        || (u32SpeedInteger == 1 && u32SpeedDecimal == 500)
        || (u32SpeedInteger == 1 && u32SpeedDecimal == 250)
        || (u32SpeedInteger == 1 && u32SpeedDecimal == 100)
        || (u32SpeedInteger == 1 && u32SpeedDecimal == 50)
        || (u32SpeedInteger == 1 && u32SpeedDecimal == 0)
        || (u32SpeedInteger == 0 && u32SpeedDecimal == 950)
        || (u32SpeedInteger == 0 && u32SpeedDecimal == 900)
        || (u32SpeedInteger == 0 && u32SpeedDecimal == 500)
        || (u32SpeedInteger == 0 && u32SpeedDecimal == 250))
    {
        if (!pAvplay->bAudTplay)
        {
            if (   (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
                || (HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->CurStatus && HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->LstStatus))
            {
                s32Ret = AVPLAY_Reset(pAvplay);
                if (HI_SUCCESS != s32Ret)
                {
                    HI_ERR_AVPLAY("AVPLAY_Reset failed, Ret=0x%x.\n", s32Ret);
                    return HI_FAILURE;
                }
            }
        }

        stTplayParam.bTplayEnable = HI_TRUE;
        stTplayParam.u32SpeedInt  = pstTplayOpt->u32SpeedInteger;
        stTplayParam.u32SpeedDeci = pstTplayOpt->u32SpeedDecimal;

        s32Ret = HI_MPI_ADEC_SetTplay(pAvplay->hAdec, &stTplayParam);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_AVPLAY("set Adec tplay failed, Ret=0x%x.\n", s32Ret);
            return HI_FAILURE;
        }

        pAvplay->bAudTplay = HI_TRUE;
    }
    else
    {
        stTplayParam.bTplayEnable = HI_FALSE;
        stTplayParam.u32SpeedDeci = 0;
        stTplayParam.u32SpeedInt  = 1;
...
HI_S32  AVPLAY_AudResume(AVPLAY_S* pAvplay)
{
#ifdef AVPLAY_AUDIO_TPLAY_SUPPORT
    HI_S32       s32Ret = HI_FAILURE;
    ADEC_TPLAY_S stTplayParam = {0};
...
HI_S32 HI_MPI_AVPLAY_Tplay(HI_HANDLE hAvplay, const HI_UNF_AVPLAY_TPLAY_OPT_S *pstTplayOpt)
{
    ...
    if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
    {
        if (HI_NULL != pstTplayOpt)
        {
            Ret = AVPLAY_SetAudTPlay(pAvplay, pstTplayOpt);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_AVPLAY("Set Aud tplay speed fail!\n");
            }
        }

        (HI_VOID)AVPLAY_SetSyncTPlay(pAvplay);
    ...
    if (HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->CurStatus)
    {
        ...
        if (pAvplay->AudEnable)
        {
            ...
            {
                Ret = HI_MPI_AO_Track_Resume(pAvplay->hRenderTrack);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("HI_MPI_AO_Track_Resume fail 0x%x\n", Ret);
                }
            }
        }
...
HI_S32 HI_MPI_AVPLAY_Resume(HI_HANDLE hAvplay)
{
       ...
       )
    {
        ...
        (HI_VOID)AVPLAY_AudResume(pAvplay);
                            ...
                            HI_U32                u32TimeOutMs)
{
    ...
    if ((HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)&&(HI_FALSE == pAvplay->bCurExternalAud))
    {
                            ...
                            HI_UNF_AVPLAY_PUTBUFEX_OPT_S *pstExOpt)
{
    ...
    if(HI_NULL == pstExOpt)
    {
        HI_ERR_AVPLAY("para pstExOpt is null.\n");
        return HI_ERR_AVPLAY_NULL_PTR;
    }
...
HI_S32 HI_MPI_AVPLAY_AttachRender(HI_HANDLE hAvplay, HI_HANDLE hRender)
{
    AVPLAY_S*           pAvplay = HI_NULL;
    HI_U32              Id = GET_AVPLAY_ID(hAvplay);
    HI_S32              Ret = HI_SUCCESS;

    if (HI_INVALID_HANDLE == hRender)
    {
    ...
    AVPLAY_ThreadMutex_Lock(&pAvplay->AvplayAudThreadMutex);
    pAvplay->hRenderTrack = hRender;
    AVPLAY_ThreadMutex_UnLock(&pAvplay->AvplayAudThreadMutex);
    AVPLAY_INST_UNLOCK(Id);
    HI_WARN_AVPLAY("Attach Render(0x%x) is succeed.\n", hRender);

    return Ret;
}

HI_S32 HI_MPI_AVPLAY_DetachRender(HI_HANDLE hAvplay, HI_HANDLE hRender)
{
    AVPLAY_S*           pAvplay = HI_NULL;
    HI_U32              Id = GET_AVPLAY_ID(hAvplay);
    HI_S32              Ret = HI_SUCCESS;
    ...
    if(pAvplay->hRenderTrack != hRender)
    {
        HI_ERR_AVPLAY("para hTrack is not exist.\n");
        AVPLAY_INST_UNLOCK(Id);
        return HI_ERR_AVPLAY_INVALID_PARA;
    }
    ...
    AVPLAY_ThreadMutex_UnLock(&pAvplay->AvplayAudThreadMutex);
    AVPLAY_INST_UNLOCK(Id);
    HI_WARN_AVPLAY("Detach sound(0x%x) is succeed.\n", hRender);
...
HI_S32 HI_MPI_AVPLAY_GetStreamInfo(HI_HANDLE hAvplay, HI_UNF_AVPLAY_STREAM_INFO_S *pstStreamInfo)
{
    ...
    if (pAvplay->hAdec != HI_INVALID_HANDLE)
    {
        ...
        else
        {
            ...
            pstStreamInfo->stAudStreamInfo.enBitDepth   = AdecStreaminfo.enBitDepth;
            pstStreamInfo->stAudStreamInfo.u32Channel   = AdecStreaminfo.u32Channel;
            pstStreamInfo->stAudStreamInfo.u32BitRate   = AdecStreaminfo.u32BitRate;
        }
    }
...
#ifdef AVPLAY_UVMOS_SUPPORT
        if ((pAvplay->bUVMOSEnable) && (pAvplay->ValidFrameRate))
        {
            pstStreamInfo->stVidStreamInfo.u32fpsInteger = pAvplay->ValidFrameRate / 100;
            pstStreamInfo->stVidStreamInfo.u32fpsDecimal = pAvplay->ValidFrameRate % 100;
        }
#endif
        Ret = HI_MPI_VDEC_GetChanStatusInfo(pAvplay->hVdec, &VdecBufStatus);
        if (Ret != HI_SUCCESS)
...
HI_S32 HI_MPI_AVPLAY_PutAudPts(HI_HANDLE hAvplay, HI_U32 u32AudPts)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("call HI_MPI_SYNC_AudJudge fail Ret:0x%x.\n",Ret);
    }
    AVPLAY_ThreadMutex_UnLock(&pAvplay->AvplayAudThreadMutex);
...
HI_S32 HI_MPI_AVPLAY_Invoke(HI_HANDLE hAvplay, HI_UNF_AVPLAY_INVOKE_E enInvokeType, HI_VOID *pPara)
{
    ...
    {
        HI_CODEC_VIDEO_CMD_S *pstVidCmd = (HI_CODEC_VIDEO_CMD_S*)pPara;
        if (HI_INVALID_HANDLE == pAvplay->hVdec)
        {
...
#ifndef AVPLAY_SCENE_MODE_SUPPORT
        if (HI_UNF_AVPLAY_SET_SCENE_MODE_CMD == pstVidCmd->u32CmdID)
        {
            AVPLAY_INST_UNLOCK(Id);
            HI_ERR_AVPLAY("This chip not support scene mode\n");
            return HI_ERR_AVPLAY_NOT_SUPPORT;
        }
#endif
        if (HI_UNF_AVPLAY_GET_VDEC_HANDLE_CMD == pstVidCmd->u32CmdID)
        {
            *(HI_U32 *)(pstVidCmd->pPara) = pAvplay->hVdec;
            AVPLAY_INST_UNLOCK(Id);
            return HI_SUCCESS;
        }

        if (HI_UNF_AVPLAY_SET_VDEC_LSTFRAME_CMD == pstVidCmd->u32CmdID)
        {
            if (HI_TRUE == *(HI_BOOL *)(pstVidCmd->pPara))
            {
                Ret = HI_MPI_VDEC_SetEosFlag(pAvplay->hVdec);
                if (HI_SUCCESS != Ret)
                {
        ...
        if (pstVidCmd->u32CmdID == HI_UNF_AVPLAY_KEEP_SPS_PPS_INFO_CMD)
        {
            pAvplay->bKeepSPSPPSFlag = *(HI_BOOL*)(pstVidCmd->pPara);
            pstVidCmd->u32CmdID = AVPLAY_VID_KEEP_SPS_PPS_INFO_CMD;
        }
        ...
        if (HI_UNF_AVPLAY_SET_PROGRESSIVE_CMD == pstVidCmd->u32CmdID)
        {
            pAvplay->bSetProgressive = *(HI_U32 *)(pstVidCmd->pPara);
        }
    }
    else if (HI_UNF_AVPLAY_INVOKE_ACODEC == enInvokeType)
    {
        ...
        else if(AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            ...
            Ret = HI_MPI_AO_Track_SetRenderCmd(pAvplay->hRenderTrack, pPara);
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("ADEC_SetCodecCmd failed.\n");
            }
    ...
    else if (HI_UNF_AVPLAY_INVOKE_GET_PRIV_PLAYINFO == enInvokeType)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("HI_MPI_WIN_GetLatestFrameInfo failed Ret:0x%x\n",Ret);
            AVPLAY_INST_UNLOCK(Id);
            return Ret;
        }
    ...
    else if (HI_UNF_AVPLAY_INVOKE_SET_TPLAY_AUDIO_ENABLE == enInvokeType)
    {
        Ret = AVPLAY_SetAudTPlayEnable(pAvplay, *(HI_U32 *)pPara);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("AVPLAY_SetAudTPlayEnable failed.\n");
        }
    }
    else if (HI_UNF_AVPLAY_INVOKE_SET_TPLAY_RESET_ENABLE == enInvokeType)
    {
        if (*(HI_U32 *)pPara == 1)
        {
            pAvplay->bTplayResetMode = HI_TRUE;
        ...
        {
            pAvplay->bTplayResetMode = HI_FALSE;
        }
    }
    else
    {
        HI_ERR_AVPLAY("invoke command can not find.CMD=0x%x\n", enInvokeType);
    }
...
HI_S32 HI_MPI_AVPLAY_AcqUserData(HI_HANDLE hAvplay, HI_UNF_VIDEO_USERDATA_S *pstUserData, HI_UNF_VIDEO_USERDATA_TYPE_E *penType)
{
    ...
    if ((HI_INVALID_HANDLE == hAvplay)||(HI_NULL == pstUserData) || (HI_NULL == penType))
    {
        HI_ERR_AVPLAY("para is null.\n");
        return HI_ERR_AVPLAY_NULL_PTR;
    }
    ...
    *penType = HI_UNF_VIDEO_USERDATA_DVB1_CC;
...
HI_S32 HI_MPI_AVPLAY_RlsUserData(HI_HANDLE hAvplay, HI_UNF_VIDEO_USERDATA_S* pstUserData)
{
    HI_S32      Ret;
    HI_U32      Id = GET_AVPLAY_ID(hAvplay);
#ifdef AVPLAY_USER_DATA_EXTERN_SUPPORT
    HI_UNF_VIDEO_USERDATA_TYPE_E enType;
#endif
    AVPLAY_S   *pAvplay;

    if ((HI_INVALID_HANDLE == hAvplay)||(HI_NULL == pstUserData))
    {
        HI_ERR_AVPLAY("para is null.\n");
        return HI_ERR_AVPLAY_NULL_PTR;
    }
...
HI_S32 HI_MPI_AVPLAY_AcqUserDataEx(HI_HANDLE hAvplay, HI_UNF_VIDEO_USERDATA_TYPE_E enType, HI_UNF_VIDEO_USERDATA_S *pstUserData)
{
    ...
    Ret = HI_MPI_VDEC_AcqUserData(pAvplay->hVdec, pstUserData, &enUserType);
    if (HI_SUCCESS != Ret)
    {
        Ret = HI_ERR_AVPLAY_INVALID_OPT;
    }
...
HI_S32 HI_MPI_AVPLAY_RlsUserDataEx(HI_HANDLE hAvplay, HI_UNF_VIDEO_USERDATA_TYPE_E enType, HI_UNF_VIDEO_USERDATA_S *pstUserData)
{
    ...
    enUserType = enType;
    Ret = HI_MPI_VDEC_RlsUserData(pAvplay->hVdec, pstUserData, &enUserType);
    if (HI_SUCCESS != Ret)
    {
        Ret = HI_ERR_AVPLAY_INVALID_OPT;
    }
...
HI_S32 HI_MPI_AVPLAY_SetHDRInfo(HI_HANDLE hAvplay, HI_UNF_VIDEO_HDR_INFO_S* pstHdrInfo, HI_BOOL bPaddingHdrInfo, HI_U32 u32PtsMs)
{
#ifdef HI_HDR_SUPPORT
    HI_U32                          Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S                        *pAvplay= HI_NULL;
    ...
    if (pstHdrInfo->enHdrType < HI_UNF_VIDEO_HDR_TYPE_BUTT)
    {
        AVPLAY_HDRInfoConvertUnf2Avplay(pstHdrInfo, &pAvplay->stAvplayHdrInfo);
        pAvplay->bPaddingHdrInfo = bPaddingHdrInfo;
    }

    memcpy(&pAvplay->stHdrInfo,pstHdrInfo,sizeof(HI_UNF_VIDEO_HDR_INFO_S));

    pAvplay->bSetHdrInfo = HI_TRUE;

    HI_INFO_AVPLAY("----set hdr info prim x %d %d %d,y %d %d %d\n",
                     pAvplay->stHdrInfo.unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[0],
                     pAvplay->stHdrInfo.unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[1],
                     pAvplay->stHdrInfo.unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[2],
                     pAvplay->stHdrInfo.unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_y[0],
                     pAvplay->stHdrInfo.unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_y[1],
                     pAvplay->stHdrInfo.unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_y[2]
                     );

    HI_INFO_AVPLAY("bPaddingHdrInfo = %d\n", pAvplay->bPaddingHdrInfo);
    AVPLAY_INST_UNLOCK(Id);
...
HI_S32 HI_MPI_AVPLAY_GetHDRInfo(HI_HANDLE hAvplay, HI_UNF_VIDEO_HDR_INFO_S* pstHdrInfo)
{
#ifdef HI_HDR_SUPPORT
    HI_U32                          Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S                        *pAvplay= HI_NULL;
    ...
    stFrameVideo = pAvplay->CurFrmPack.stFrame[0].stFrameVideo;

    if (pAvplay->bSetHdrInfo)
    {
        memcpy(pstHdrInfo,&pAvplay->stHdrInfo,sizeof(HI_UNF_VIDEO_HDR_INFO_S));

        HI_INFO_AVPLAY("----get hdr info from avplay,prim x %d %d %d,y %d %d %d\n",
                         pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[0],
                         pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[1],
                         pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[2],
                         pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_y[0],
                         pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_y[1],
                         pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_y[2]
                         );
    ...
    if (HI_DRV_VIDEO_FRAME_TYPE_HDR10 == stFrameVideo.enSrcFrameType)
    {
        pstHdrInfo->enHdrType = HI_UNF_VIDEO_HDR_TYPE_HDR10;
        pstHdrInfo->unHdrMetadata.stHdr10Info.u8ColourPrimaries = stFrameVideo.stColourInfo.u8ColourPrimaries;
        pstHdrInfo->unHdrMetadata.stHdr10Info.u8FullRangeFlag = stFrameVideo.stColourInfo.u8FullRangeFlag;
        pstHdrInfo->unHdrMetadata.stHdr10Info.u8MatrixCoeffs = stFrameVideo.stColourInfo.u8MatrixCoeffs;
        pstHdrInfo->unHdrMetadata.stHdr10Info.u8ContentAvailable = stFrameVideo.unHDRInfo.stHDR10Info.u8ContentAvailable;
        pstHdrInfo->unHdrMetadata.stHdr10Info.u8MasteringAvailable = stFrameVideo.unHDRInfo.stHDR10Info.u8MasteringAvailable;
        pstHdrInfo->unHdrMetadata.stHdr10Info.u8TransferCharacteristics = stFrameVideo.unHDRInfo.stHDR10Info.u8TransferCharacteristics;
        pstHdrInfo->unHdrMetadata.stHdr10Info.stContentInfo.u32MaxContentLightLevel = stFrameVideo.unHDRInfo.stHDR10Info.stContentInfo.u32MaxContentLightLevel;
        pstHdrInfo->unHdrMetadata.stHdr10Info.stContentInfo.u32MaxPicAverageLightLevel = stFrameVideo.unHDRInfo.stHDR10Info.stContentInfo.u32MaxPicAverageLightLevel;

        memcpy(&pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo,&stFrameVideo.unHDRInfo.stHDR10Info.stMasteringInfo,sizeof(HI_UNF_VIDEO_MASTERING_DISPLAY_COLOUR_VOLUME_S));  //The two structs are the exact  same.
    }
    else if (HI_DRV_VIDEO_FRAME_TYPE_HLG == stFrameVideo.enSrcFrameType)
    {
        pstHdrInfo->enHdrType = HI_UNF_VIDEO_HDR_TYPE_HLG;
        pstHdrInfo->unHdrMetadata.stHlgInfo.u8ColourPrimaries = stFrameVideo.stColourInfo.u8ColourPrimaries;
        pstHdrInfo->unHdrMetadata.stHlgInfo.u8FullRangeFlag = stFrameVideo.stColourInfo.u8FullRangeFlag;
        pstHdrInfo->unHdrMetadata.stHlgInfo.u8MatrixCoeffs = stFrameVideo.stColourInfo.u8MatrixCoeffs;
        pstHdrInfo->unHdrMetadata.stHlgInfo.u8ContentAvailable = stFrameVideo.unHDRInfo.stHLGInfo.u8ContentAvailable;
        pstHdrInfo->unHdrMetadata.stHlgInfo.u8MasteringAvailable = stFrameVideo.unHDRInfo.stHLGInfo.u8MasteringAvailable;
        pstHdrInfo->unHdrMetadata.stHlgInfo.u8TransferCharacteristics = stFrameVideo.unHDRInfo.stHLGInfo.u8TransferCharacteristics;
        pstHdrInfo->unHdrMetadata.stHlgInfo.stContentInfo.u32MaxContentLightLevel = stFrameVideo.unHDRInfo.stHLGInfo.stContentInfo.u32MaxContentLightLevel;
        pstHdrInfo->unHdrMetadata.stHlgInfo.stContentInfo.u32MaxPicAverageLightLevel = stFrameVideo.unHDRInfo.stHLGInfo.stContentInfo.u32MaxPicAverageLightLevel;
        pstHdrInfo->unHdrMetadata.stHlgInfo.u8BackWardsCompatible = stFrameVideo.unHDRInfo.stHLGInfo.IsBackwardsCompatible;

        memcpy(&pstHdrInfo->unHdrMetadata.stHlgInfo.stMasteringInfo,&stFrameVideo.unHDRInfo.stHLGInfo.stMasteringInfo,sizeof(HI_UNF_VIDEO_MASTERING_DISPLAY_COLOUR_VOLUME_S));  //The two structs are the exact  same.
    }
    else
    {
        pstHdrInfo->enHdrType = HI_UNF_VIDEO_HDR_TYPE_BUTT;
    }

    HI_INFO_AVPLAY("----get hdr info from frame,prim x %d %d %d,y %d %d %d\n",
                     pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[0],
                     pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[1],
                     pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_x[2],
                     pstHdrInfo->unHdrMetadata.stHdr10Info.stMasteringInfo.u16DisplayPrimaries_y[0],
...
/* temp for AVS2 HDR*/
HI_S32 HI_MPI_AVPLAY_SetAvs2HDRInfo(HI_HANDLE hAvplay, HI_BOOL bPaddingHdrInfo, AVPLAY_AVS2_HDR_INFO_S *pstHdrInfo)
{
#ifdef HI_HDR_SUPPORT
    HI_U32                          Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S                        *pAvplay= HI_NULL;
    ...
    pAvplay->bPaddingHdrInfo = bPaddingHdrInfo;
    pAvplay->stAvplayHdrInfo.enHdrType = pstHdrInfo->enHdrType;
...
HI_S32 HI_MPI_AVPLAY_CheckIfSupportRandomStream(HI_HANDLE hAvplay, HI_BOOL *pbSupport)
{
    HI_S32                          s32Ret;
    HI_U32                          Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S                        *pAvplay= HI_NULL;
    HI_BOOL                         bRandStreamSupport;

    if (HI_NULL == pbSupport)
    {
        HI_ERR_AVPLAY("invalid parameter \n");
        return HI_ERR_AVPLAY_NULL_PTR;
    }
    ...
    AVPLAY_INST_LOCK_CHECK(hAvplay, Id);
    s32Ret = HI_MPI_VDEC_CheckIfSupportRandomStream(pAvplay->VdecAttr.enType,&pAvplay->VdecAttr.unExtAttr,&bRandStreamSupport);
    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("Call HI_MPI_VDEC_CheckIfSupportRandomStream failed!s32Ret 0x%x\n",s32Ret);
        AVPLAY_INST_UNLOCK(Id);
        return s32Ret;
    }

    *pbSupport = bRandStreamSupport;
    AVPLAY_INST_UNLOCK(Id);

    return HI_SUCCESS;
}

HI_S32 HI_MPI_AVPLAY_IsAudioCodecSupport(const HA_CODEC_ID_E enCodecID, HI_BOOL* pbSupport)
{
    HI_S32  Ret;
    ...
    Ret = HI_MPI_AO_Track_IsCodecSupport(enCodecID, pbSupport);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("call HI_MPI_AO_Track_IsCodecSupport failed(0x%x)\n", Ret);
    }
...
HI_S32 HI_MPI_AVPLAY_GetHandle(HI_HANDLE hAvplay, HI_MOD_ID_E enModID, HI_HANDLE *pHandle, HI_U32 *pu32Reserve)
{
    HI_U32      Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S    *pAvplay= HI_NULL;

    if ((HI_NULL == pHandle) || (HI_NULL == pu32Reserve))
    {
        HI_ERR_AVPLAY("invalid parameter \n");
        return HI_ERR_AVPLAY_NULL_PTR;
    }
    ...
    if (HI_ID_VDEC == enModID)
    {
        if (HI_INVALID_HANDLE == pAvplay->hVdec)
        {
            HI_ERR_AVPLAY("vid channel is not open! \n");
            AVPLAY_INST_UNLOCK(Id);
            return HI_FAILURE;
        }

        *pHandle = pAvplay->hVdec;
    }
    else if (HI_ID_DEMUX == enModID)
    {
        if (HI_UNF_AVPLAY_STREAM_TYPE_TS != pAvplay->AvplayAttr.stStreamAttr.enStreamType)
        {
            HI_INFO_AVPLAY("player is es mode \n");
            AVPLAY_INST_UNLOCK(Id);
            return HI_ERR_AVPLAY_INVALID_OPT;
        }
        ...
        *pHandle = pAvplay->hDmxVid;
        *pu32Reserve = pAvplay->AvplayAttr.u32DemuxId;
    }
    else
    {
        HI_ERR_AVPLAY("enModID is not support! enModID:0x%x\n",enModID);
        AVPLAY_INST_UNLOCK(Id);
        return HI_FAILURE;
    }
...
HI_S32 HI_MPI_AVPLAY_SwitchToExternalAudio(HI_HANDLE hAvplay,HI_BOOL bSwitchExternalAudio)
{
    HI_S32                          i;
    HI_U32                          Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S                        *pAvplay= HI_NULL;
    ...
    if (HI_TRUE == bSwitchExternalAudio)
    {
        if (0xffffffff == pAvplay->stExternalAudAttr.enType)
        {
            HI_ERR_AVPLAY("External audio attr has not been set before.\n");
            AVPLAY_Mutex_UnLock(&pAvplay->AvplayAudThreadMutex);
            AVPLAY_INST_UNLOCK(Id);
            return HI_FAILURE;
        }
    ...
    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        (HI_VOID)HI_MPI_AO_Track_SetSyncMode(pAvplay->hRenderTrack, HI_MPI_AO_TRACK_SYNCMODE_PAUSEOUTPUT);

        (HI_VOID)HI_MPI_AO_Track_Flush(pAvplay->hRenderTrack);

        (HI_VOID)HI_MPI_AO_Track_Stop(pAvplay->hRenderTrack);
    ...
    else if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        ...
        (HI_VOID)HI_MPI_AO_Track_Stop(pAvplay->hSyncTrack);
    }

    if (HI_TRUE == bSwitchExternalAudio)
    {
        (HI_VOID)AVPLAY_SetAdecAttr(pAvplay, &pAvplay->stExternalAudAttr);
    }
    else
    {
        (HI_VOID)AVPLAY_SetAdecAttr(pAvplay, &pAvplay->stNormalAudAttr);
    }
    ...
    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        (HI_VOID)HI_MPI_AO_Track_Start(pAvplay->hRenderTrack);
        (HI_VOID)HI_MPI_AO_Track_SetSyncMode(pAvplay->hRenderTrack, HI_MPI_AO_TRACK_SYNCMODE_STARTOUTPUT);
        HI_INFO_AVPLAY("switch to render channel\n");
    }
    else if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        (HI_VOID)HI_MPI_ADEC_Start(pAvplay->hAdec);
        (HI_VOID)HI_MPI_AO_Track_Start(pAvplay->hSyncTrack);
        HI_INFO_AVPLAY("switch to normal channel\n");
    }
    ...
    if (HI_TRUE == bSwitchExternalAudio)
    {
        pAvplay->bCurExternalAud = HI_TRUE;
    }
    else
    {
        pAvplay->bCurExternalAud = HI_FALSE;
...
HI_S32 HI_MPI_AVPLAY_STCCalibration(HI_HANDLE hAvplay, HI_S32 StcPPM)
{
    HI_S32                          Ret;
    HI_U32                          Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S                        *pAvplay= HI_NULL;
    ...
    AVPLAY_INST_LOCK_CHECK(hAvplay, Id);
    Ret = HI_MPI_SYNC_STCCalibration(pAvplay->hSync,StcPPM);
    if(Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("Call HI_MPI_SYNC_STCCalibration failed!Ret:0x%x\n",Ret);
    }
    AVPLAY_INST_UNLOCK(Id);
    return Ret;
}
