...
static HI_S32 MPIPesMemMalloc(HI_HANDLE hChannel, HI_U32 u32Size, HI_U8** pu8PesAddr)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("HI_MPI_DMX_GetChannelAttr failed!\n");
        *pu8PesAddr = HI_NULL;
        return HI_FAILURE;
    }
...
static HI_S32 MPIPesMemFree(HI_HANDLE hChannel, HI_U32 u32Size, HI_U8* pu8PesAddr)
{
    ...
    ret = HI_MPI_DMX_GetCapability(&stCap);
    if (HI_SUCCESS != ret)
    {
        return ret;
    }

    if (stCap.u32DmxNum <= u32DmxId || stCap.u32ChannelNum <= u32ChnID)
    {
        HI_ERR_DEMUX("demux channel invalid.\n");
        return HI_ERR_DMX_INVALID_PARA;
    }
...
static HI_S32 DmxReleaseBuf(HI_HANDLE hChannel, HI_U32 u32ReleaseNum, HI_UNF_DMX_DATA_S *pstBuf)
{
    ...
    if (stCap.u32DmxNum <= u32DmxId || stCap.u32ChannelNum <= u32ChId)
    {
        HI_ERR_DEMUX("demux channel invalid.\n");
        return HI_ERR_DMX_INVALID_PARA;
    }
...
static HI_S32 __DmxUnsetChanBufCb(HI_HANDLE hChannel, pthread_t *SavedSharedContext, pthread_t *SavedPrivContext)
{
    ...
    if (stCap.u32DmxNum <= DmxId || stCap.u32ChannelNum <= ChnId)
    {
        HI_ERR_DEMUX("demux channel invalid.\n");
        return HI_ERR_DMX_INVALID_PARA;
    }
...
HI_S32 HI_MPI_DMX_GetCapability(HI_UNF_DMX_CAPABILITY_S *pstCap)
{
    ...
    if (HI_SUCCESS == ret)
    {
        ...
        pstCap->u32TsioPortNum  = Param.u32TsioPortNum;
...
HI_S32 HI_MPI_DMX_GetTagPortAttr(HI_UNF_DMX_PORT_E enPortId, HI_UNF_DMX_TAG_PORT_ATTR_S *pstAttr)
{
    HI_S32  ret;
    ...
    if (HI_SUCCESS == ret)
    {
        memcpy(pstAttr, &Param.TagAttr, sizeof(HI_UNF_DMX_TAG_PORT_ATTR_S));
    }

    return ret;
}

HI_S32 HI_MPI_DMX_SetTagPortAttr(HI_UNF_DMX_PORT_E enPortId, const HI_UNF_DMX_TAG_PORT_ATTR_S *pstAttr)
{
    HI_S32 ret;
    DMX_Tag_GetAttr_S  Param;
    ...
    memcpy(&Param.TagAttr, pstAttr, sizeof(HI_UNF_DMX_TAG_PORT_ATTR_S));
    Param.enPortId = enPortId;
...
HI_S32 HI_MPI_DMX_GetTSPortId(HI_U32 u32DmxId, HI_UNF_DMX_PORT_E *penPortId)
{
    ...
    if (HI_SUCCESS == ret)
    {
        *penPortId = (HI_UNF_DMX_PORT_E)Param.PortId;
        ...
        if (DMX_PORT_MODE_TSIO == Param.PortMode)
        {
            *penPortId = (HI_UNF_DMX_PORT_E)(HI_UNF_DMX_PORT_TSIO_0 + Param.PortId);
        }
    }
...
HI_S32 HI_MPI_DMX_GetTSPortPacketNum(HI_UNF_DMX_PORT_E enPortId, HI_UNF_DMX_PORT_PACKETNUM_S *sPortStat)
{
    ...
    memset(&Param,0,sizeof(DMX_PortPacketNum_S));

    Param.enPortId = enPortId;
    ret = ioctl(g_s32DmxFd, CMD_DEMUX_PORT_GETPACKETNUM, &Param);
    if (HI_SUCCESS == ret)
    {
        memcpy(sPortStat, &Param.PortPacket, sizeof(HI_UNF_DMX_PORT_PACKETNUM_S));
    }
...
HI_S32 HI_MPI_DMX_CreateTSBuffer(HI_UNF_DMX_PORT_E enPortId, const HI_UNF_DMX_TSBUF_ATTR_S *pstTsBufAttr, HI_HANDLE *phTsBuffer)
{
    ...
    if((enPortId < HI_UNF_DMX_PORT_RAM_0) || (enPortId > HI_UNF_DMX_PORT_RAM_15))
    {
    ...
    if (HI_SUCCESS == ret)
    {
        ...
        if (HI_NULL == UsrAddr)
        {
            ...
            if (-1 == ioctl(g_s32DmxFd, CMD_DEMUX_TS_BUFFER_DEINIT, &Param.PortId))
            {
        ...
        g_stTsBuf.BufDesc[Param.PortId].VirAddr   = UsrAddr;
        g_stTsBuf.BufDesc[Param.PortId].PhyAddr   = Param.BufPhyAddr;
        g_stTsBuf.BufDesc[Param.PortId].Size      = Param.BufSize;
        g_stTsBuf.BufDesc[Param.PortId].Flag      = Param.BufFlag;

        *phTsBuffer = DMX_PORTID2BUFFERHANDLE(Param.PortId);
    }
...
HI_S32 HI_MPI_DMX_GetTSBufferHandle(HI_UNF_DMX_PORT_E enPortId, HI_HANDLE *phTsBuffer)
{
    ...
    PortId = (HI_U32)enPortId - (HI_U32)HI_UNF_DMX_PORT_RAM_0;
    if (PortId >= Cap.u32RamPortNum)
    {
        HI_ERR_DEMUX("RamPortId[%u] must be more little than ramportNum[%u] \n", PortId, Cap.u32RamPortNum);
        return HI_ERR_DMX_INVALID_PARA;
    }
...
HI_S32 HI_MPI_DMX_GetChannelPacketNum(HI_HANDLE hChannel, HI_UNF_DMX_CHAN_PACKETNUM_S *sChStat)
{
    HI_S32         ret;
    DMX_ChanPacktNum_S Param;

    MPIDmxCheckDeviceFd();
    MPIDmxCheckPointer(sChStat);

    Param.hChannel = hChannel;

    ret = ioctl(g_s32DmxFd, CMD_DEMUX_CHAN_GETPACKETNUM, &Param);
    if (HI_SUCCESS == ret)
    {
        sChStat->u32CCDiscCnt = Param.CCDisCnt;
    }
...
HI_S32 HI_MPI_DMX_AcquireEs(HI_HANDLE hChannel, HI_UNF_ES_BUF_S *pEsBuf, HI_MPI_ES_EXT_DATA_S *pEsExtData)
{
    HI_S32 ret;
    ...
    if (HI_NULL != pEsExtData)
    {
        memset(pEsExtData, 0x00, sizeof(HI_MPI_ES_EXT_DATA_S));
        memset(&Param.stEsExtData, 0x00, sizeof(HI_MPI_ES_EXT_DATA_S));
    }
    Param.hChannel = hChannel;
    ...
    if (HI_SUCCESS == ret)
    {
            ...
            && ((u32PhyAddr + Param.BufSize - g_stChanBuf.BufDesc[u32DmxId * g_stChanBuf.ChnCnt + u32ChId].PhyAddr) <= g_stChanBuf.BufDesc[u32DmxId * g_stChanBuf.ChnCnt + u32ChId].Size))
        {
            ...
            if (HI_NULL != pEsExtData)
            {
                memcpy(pEsExtData, &Param.stEsExtData, sizeof(HI_MPI_ES_EXT_DATA_S));
            }
...
/*It will return HI_SUCCESS when have ES in timeout */
HI_S32 HI_MPI_DMX_GetAcquireEsMSG(HI_HANDLE hChannel, HI_U32 u32TimeoutMs)
{
    DMX_PesBufGetEx_S Param;
    ...
    return ioctl(g_s32DmxFd, CMD_DEMUX_ES_BUFFER_GET_MSG, &Param);
}
...
HI_S32 HI_MPI_DMX_DestroyRecChn(HI_HANDLE hRecChn)
{
    ...
    if (stCap.u32DmxNum <= DmxId || stCap.u32RecChnNum <= RecId)
    {
        HI_ERR_DEMUX("demux channel invalid.\n");
        return HI_ERR_DMX_INVALID_PARA;
    }
...
HI_S32 HI_MPI_DMX_AcquireRecDataAndIndex(HI_HANDLE hRecChn, HI_MPI_DMX_REC_DATA_INDEX_S* pstRecDataIdx)
{
    ...
    return DmxAcquireRecDataAndIndex(hRecChn, pstRecDataIdx);
}
