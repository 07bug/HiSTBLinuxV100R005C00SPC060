...
HDMI_CALLBACK_NODE_S g_pstHDMICallBackList = { {0},{0} };
...
typedef struct 
{
    ...
    HI_BOOL     bCECEnable;
    pthread_t   CECTread; 
    HI_UNF_HDMI_CECCALLBACK_EXT_S stCECCallback;
}HDMI_CEC_ATTR_S;

static HDMI_CEC_ATTR_S g_stCECAttr = {0};
...
static HDMI_COMM_USER_ATTR_S g_stHdmiCommUserParam = {0};

static HDMI_CHN_USER_ATTR_S g_stHdmiChnUserParam[HI_UNF_HDMI_ID_BUTT] = { {0} };
...
{
    if ( (HI_NULL == pstDestAttr) || (HI_NULL == pstSrcAttr) )
    {
        HI_WARN_HDMI("null pointer \n");
        return ;
    }
...
static HI_S32 Hdmi_AttrCheck(HI_UNF_HDMI_ATTR_S *pstAttr)
{
    ...
    HDMI_CHECK_MAX(pstAttr->bEnableHdmi, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bEnableVideo, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->enVidOutMode, HI_UNF_HDMI_VIDEO_MODE_BUTT);
    if (pstAttr->enDeepColorMode != HI_UNF_HDMI_DEEP_COLOR_OFF)
    {
        HDMI_CHECK_MAX(pstAttr->enDeepColorMode, HI_UNF_HDMI_DEEP_COLOR_36BIT + 1);
    }//else (pstAttr->enDeepColorMode == HI_UNF_HDMI_DEEP_COLOR_OFF) is ok
    HDMI_CHECK_MAX(pstAttr->bxvYCCMode, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bEnableAudio, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bEnableAviInfoFrame, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bEnableAudInfoFrame, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bEnableSpdInfoFrame, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bEnableMpegInfoFrame, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bHDCPEnable, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bEnableVidModeAdapt, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bEnableDeepClrAdapt, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->bAuthMode, HI_TRUE + 1);
    HDMI_CHECK_MAX(pstAttr->enHDCPMode, HI_UNF_HDMI_HDCP_BUTT); 
...
static HI_S32 Hdmi_CECCommandCheck(HI_UNF_HDMI_CEC_CMD_S  *pCECCmd)
{
    ...
    HDMI_CHECK_MAX(pCECCmd->enSrcAdd, HI_UNF_CEC_LOGICALADD_BUTT);
    HDMI_CHECK_MAX(pCECCmd->enDstAdd, HI_UNF_CEC_LOGICALADD_BUTT);
    #if !defined (HDMI_FORCE_RAWDATA)
    if (CEC_OPCODE_USER_CONTROL_PRESSED == pCECCmd->u8Opcode)
    {
        HDMI_CHECK_MAX(pCECCmd->unOperand.stUIOpcode, HI_UNF_CEC_UICMD_DATA + 1);
    }
    else
    #endif
    {
        HDMI_CHECK_MAX(pCECCmd->unOperand.stRawData.u8Length, 15 + 1);//max length
    }
...
static HI_S32 Hdmi_InfoFrameCheck(HI_UNF_HDMI_INFOFRAME_S *pstInfoFrame)
{
    ...
    HDMI_CHECK_MAX(pstInfoFrame->enInfoFrameType, HI_INFOFRAME_TYPE_BUTT);
    switch (pstInfoFrame->enInfoFrameType)
    {
        case HI_INFOFRAME_TYPE_AVI:
        {
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enTimingMode, HI_UNF_ENC_FMT_BUTT);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enOutputType, HI_UNF_HDMI_VIDEO_MODE_BUTT);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.bActive_Infor_Present, HI_TRUE + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enBarInfo, HDMI_BAR_INFO_VH + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enScanInfo, HDMI_SCAN_INFO_FUTURE + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enColorimetry, HDMI_COLORIMETRY_2020_NON_CONST_LUMINOUS + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enAspectRatio, HI_UNF_HDMI_ASPECT_RATIO_BUTT);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enActiveAspectRatio, HI_UNF_HDMI_ASPECT_RATIO_BUTT);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enPictureScaling, HDMI_PICTURE_SCALING_HV + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enRGBQuantization, HDMI_RGB_QUANTIZATION_FULL_RANGE + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.bIsITContent, HI_TRUE + 1);
            //HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.u32PixelRepetition, 16);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enContentType, HDMI_CONTNET_GAME + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAVIInfoFrame.enYCCQuantization, HDMI_YCC_QUANTIZATION_FULL_RANGE + 1);
            break;
        }
        ...
        case HI_INFOFRAME_TYPE_AUDIO:
        {
            ...
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAUDInfoFrame.u32ChannelCount, HDMI_MAX_CHANNEL_COUNT +1 );
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAUDInfoFrame.enCodingType, HI_UNF_EDID_AUDIO_FORMAT_CODE_BUTT);
            ...
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAUDInfoFrame.u32ChannelAlloc, 0xFF + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAUDInfoFrame.u32LevelShift, 0x0F + 1);
            HDMI_CHECK_MAX(pstInfoFrame->unInforUnit.stAUDInfoFrame.u32DownmixInhibit, HI_TRUE + 1);
            break;
        }
...
HI_S32 HI_MPI_HDMI_SetCECCommand(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CEC_CMD_S  *pCECCmd)
{
    ...
    HDMI_CHECK_ID(enHdmi);
    HDMI_CheckChnOpen(enHdmi);
    Ret = Hdmi_CECCommandCheck(pCECCmd);
    if (Ret != HI_SUCCESS)
    {
        return Ret;
    }
    ...
    if (CEC_OPCODE_USER_CONTROL_PRESSED == pCECCmd->u8Opcode)
    {
        stCEC.CECCmd.unOperand.stRawData.u8Data[0] = pCECCmd->unOperand.stUIOpcode;
        stCEC.CECCmd.unOperand.stRawData.u8Length  = 1;
    }
    #endif
...
HI_S32 HI_MPI_HDMI_GetCECCommand(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CEC_CMD_S  *pCECCmd, HI_U32 timeout)
{
    ...
    if (CEC_OPCODE_USER_CONTROL_PRESSED == pCECCmd->u8Opcode)
    {
        pCECCmd->unOperand.stUIOpcode = pCECCmd->unOperand.stRawData.u8Data[0];
    }
    #endif
    HI_HDMI_UNLOCK();
...
HI_VOID *Hdmi_CEC_Event_Thread(HI_VOID* pParam)
{
    ...
    while(HI_TRUE == g_stCECAttr.bCECEnable)
    {
        ...
        if (Ret == HI_SUCCESS)
        {
            if (g_stCECAttr.stCECCallback.pfnCECCallBack)
            {
                g_stCECAttr.stCECCallback.pfnCECCallBack(g_stCECAttr.enHdmi, &stCECCmd, g_stCECAttr.stCECCallback.pPrivateData);
            }
        }
#if 0 /*--NO MODIFY : COMMENT BY CODINGPARTNER--*/
        else
...
HI_S32 HI_MPI_HDMI_RegCECCallBackFunc(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CECCALLBACK_EXT_S *pCECCallback)
{
    HI_S32 Ret = 0;
    
    HDMI_CheckChnOpen(enHdmi);
    HDMI_CHECK_NULL_PTR(pCECCallback);
    HDMI_CHECK_NULL_PTR(pCECCallback->pfnCECCallBack);

    if (HI_NULL != g_stCECAttr.stCECCallback.pfnCECCallBack)
    {
        HI_ERR_HDMI("CEC callback func has been registed! pfnCECCallBack %p\n",
                     g_stCECAttr.stCECCallback.pfnCECCallBack);
        return HI_FAILURE;
    }

    memset(&g_stCECAttr, 0, sizeof(HDMI_CEC_ATTR_S));
    g_stCECAttr.enHdmi = enHdmi;
    g_stCECAttr.stCECCallback.pfnCECCallBack = pCECCallback->pfnCECCallBack;
    g_stCECAttr.stCECCallback.pPrivateData = pCECCallback->pPrivateData;
    
    Ret = pthread_create(&g_stCECAttr.CECTread, HI_NULL, Hdmi_CEC_Event_Thread, HI_NULL);
...
HI_S32 HI_MPI_HDMI_UnRegCECCallBackFunc(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CECCALLBACK_EXT_S *pCECCallback)
{
    HDMI_CHECK_ID(enHdmi);
    HDMI_CHECK_NULL_PTR(pCECCallback);

    if(pCECCallback->pfnCECCallBack != g_stCECAttr.stCECCallback.pfnCECCallBack)
    {
        HI_ERR_HDMI("hand err :0x%x != 0x%x\n",pCECCallback->pfnCECCallBack, 
                     g_stCECAttr.stCECCallback.pfnCECCallBack);
        return HI_FAILURE;
    }
...
HI_S32 HI_MPI_HDMI_SetInfoFrame(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_INFOFRAME_S *pstInfoFrame)
{
    ...
    HDMI_CHECK_ID(enHdmi);
    HDMI_CheckChnOpen(enHdmi);
    Ret = Hdmi_InfoFrameCheck(pstInfoFrame);
    if (Ret != HI_SUCCESS)
    {
        return Ret;
    }
...
HI_S32 HI_MPI_HDMI_GetInfoFrame(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_INFOFRAME_TYPE_E enInfoFrameType, HI_UNF_HDMI_INFOFRAME_S *pstInfoFrame)
{
    ...
    HDMI_CHECK_ID(enHdmi);
    HDMI_CheckChnOpen(enHdmi);
    HDMI_CHECK_MAX(enInfoFrameType, HI_INFOFRAME_TYPE_BUTT);
...
HI_S32 HI_MPI_HDMI_SetDeepColor(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_DEEP_COLOR_E enDeepColor)
{
    ...
    HDMI_CHECK_ID(enHdmi);
    HDMI_CheckChnOpen(enHdmi);
    HDMI_CHECK_MAX(enDeepColor, HI_UNF_HDMI_DEEP_COLOR_BUTT);
