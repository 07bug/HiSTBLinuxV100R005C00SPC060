...
#define HDMI_SRM_MAX_SIZE               50000
...
/* private interface                                                    */
/************************************************************************/
HI_VOID Hdmi_msleep(HI_U32 u32TimeMs)
{
    HI_S32 s32Ret = HI_SUCCESS;
    struct timespec ts;

    ts.tv_sec  = u32TimeMs / 1000;
    ts.tv_nsec = (u32TimeMs % 1000) * 1000000;

    s32Ret = nanosleep(&ts, HI_NULL);
    if(HI_SUCCESS != s32Ret)
    {
        HI_ERR_HDMI("nanosleep failed.\n");
    }
...
static HI_VOID *Hdmi_CEC_EventThread(HI_VOID* pParam)
{
    ...
    while((HI_TRUE == s_stCECAttr.bCecEnable) && (s_stHdmiChnUserParam[enHdmi].bOpen) )
    {
        ...
        {
            HDMI_CEC_LOCK();
            if (s_stCECAttr.stCECCallback.pfnCECCallBack)
            {
                s_stCECAttr.stCECCallback.pfnCECCallBack(s_stCECAttr.enHdmi, &stCECCmd, s_stCECAttr.stCECCallback.pPrivateData);
            }
            HDMI_CEC_UNLOCK();
        }
    }
...
HI_S32 HI_MPI_HDMI_ComInit(void)
{
    ...
    memset(s_stHdmiChnUserParam, 0, sizeof(s_stHdmiChnUserParam));
    memset(s_stHdmiCommUserParam, 0, sizeof(s_stHdmiCommUserParam));
...
HI_S32 HI_MPI_HDMI_ComDeInit(void)
{
    ...
    memset(s_stHdmiCommUserParam, 0, sizeof(s_stHdmiCommUserParam));//clean
    /*destory mutex */
...
HI_S32 HI_MPI_HDMI_ComForceGetEDID(HI_UNF_HDMI_ID_E enHdmi, HI_U8 *u8Edid, HI_U32 *u32EdidLength)
{
    ...
    *u32EdidLength = 0;
    ...
    if( (HI_SUCCESS == s32Ret) && (HI_TRUE == EDIDData.stEdidRaw.u8EdidValid) && EDIDData.stEdidRaw.u32EdidLen )
    {
        *u32EdidLength = (EDIDData.stEdidRaw.u32EdidLen > HDMI_EDID_SIZE) ? HDMI_EDID_SIZE : EDIDData.stEdidRaw.u32EdidLen;
        memcpy(u8Edid, EDIDData.stEdidRaw.u8Edid, *u32EdidLength);
    }
    else
    {
...
HI_S32 HI_MPI_HDMI_ComReadEDID(HI_U8 *pu8Edid, HI_U32 *u32EdidLength)
{
    HI_S32 s32Ret = HI_SUCCESS;
    DRV_HDMI_EDID_RAWDATA_S EDIDData;

    HDMI_CheckChnOpen(HI_UNF_HDMI_ID_0);
    HDMI_CHECK_NULL_PTR(pu8Edid);
    HDMI_CHECK_NULL_PTR(u32EdidLength);

    *u32EdidLength = 0;
    memset(&EDIDData, 0, sizeof(DRV_HDMI_EDID_RAWDATA_S));
    EDIDData.enHdmiID = HI_UNF_HDMI_ID_0;

    HDMI_LOCK();
    s32Ret = ioctl(s_s32HdmiDevFd, CMD_HDMI_READ_EDID, &EDIDData);
    HDMI_UNLOCK();

    if( (HI_SUCCESS == s32Ret) && (HI_TRUE == EDIDData.stEdidRaw.u8EdidValid) && EDIDData.stEdidRaw.u32EdidLen)
    {
        if (EDIDData.stEdidRaw.u32EdidLen > HDMI_EDID_TOTAL_SIZE)
        {
            EDIDData.stEdidRaw.u32EdidLen = HDMI_EDID_TOTAL_SIZE;
        }

        *u32EdidLength = EDIDData.stEdidRaw.u32EdidLen;
        memcpy(pu8Edid, EDIDData.stEdidRaw.u8Edid, EDIDData.stEdidRaw.u32EdidLen);
    }
    else
    {
...
HI_S32 HI_MPI_HDMI_ComRegCECCallBackFunc(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CECCALLBACK_EXT_S *pCECCallback)
{
    HI_S32 s32Ret = 0;
    ...
    if (HI_NULL != s_stCECAttr.stCECCallback.pfnCECCallBack)
    {
        HI_ERR_HDMI("CEC callback func has been registed! You must un-register before re-register.\n");
        return HI_FAILURE;
    }

    HDMI_CEC_LOCK();
    s_stCECAttr.enHdmi = enHdmi;
    s_stCECAttr.stCECCallback.pfnCECCallBack = pCECCallback->pfnCECCallBack;
    s_stCECAttr.stCECCallback.pPrivateData = pCECCallback->pPrivateData;
...
HI_S32 HI_MPI_HDMI_ComUnRegCECCallBackFunc(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CECCALLBACK_EXT_S *pCECCallback)
{
    HDMI_CHECK_ID(enHdmi);
    HDMI_CHECK_NULL_PTR(pCECCallback);
    HDMI_CHECK_NULL_PTR(pCECCallback->pfnCECCallBack);
    ...
    HDMI_CEC_LOCK();
    if(pCECCallback->pfnCECCallBack != s_stCECAttr.stCECCallback.pfnCECCallBack)
    {
        HI_ERR_HDMI("hand err :%p != %p\n",pCECCallback,s_stCECAttr.stCECCallback.pfnCECCallBack);
        HDMI_CEC_UNLOCK();
        return HI_FAILURE;
    }
    s_stCECAttr.bCecCbReg = HI_FALSE;
    s_stCECAttr.stCECCallback.pfnCECCallBack = HI_NULL;
    s_stCECAttr.stCECCallback.pPrivateData = HI_NULL;
...
HI_S32 HI_MPI_HDMI_ComSetCECCommand(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CEC_CMD_S *pCECCmd)
{
    ...
    if (CEC_OPCODE_USER_CONTROL_PRESSED == stDrvCecCmd.CECCmd.u8Opcode)
    {
        stDrvCecCmd.CECCmd.stOperand.stRawData.u8Data[0] = stDrvCecCmd.CECCmd.stOperand.enUIOpcode;
        stDrvCecCmd.CECCmd.stOperand.stRawData.u8Length  = 1;
    }
#endif
...
HI_S32 HI_MPI_HDMI_ComCECStatus(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CEC_STATUS_S *pStatus)
{
    ...
    memcpy(pStatus->u8Network, CECStatus.stCecInfo.au8Network, sizeof(pStatus->u8Network));
    HDMI_UNLOCK();
...
HI_S32 HI_MPI_HDMI_ComCECEnable(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    if (s_stCECAttr.CECTread != HI_NULL)
    {
        HDMI_CEC_UNLOCK();
        HI_INFO_HDMI("Cec event thread has been created.\n");
        return s32Ret;
    }
...
HI_S32 HI_MPI_HDMI_ComCECDisable(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    HDMI_CEC_LOCK();
    if (s_stCECAttr.CECTread)
    {
        pthread_join(s_stCECAttr.CECTread, NULL);
        s_stCECAttr.CECTread = HI_NULL;
    }
    HDMI_CEC_UNLOCK();
...
HI_S32 HI_MPI_HDMI_ComSetSrm(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_SRM_S *pstSrm)
{
    ...
    if(   (pstSrm->u32SrmLen < (HDMI_SIZE_SRM_HEADER_DATA + HDMI_SIZE_DCP_LLC_SIGNATRUE) )
       || (pstSrm->u32SrmLen > HDMI_SRM_MAX_SIZE) )
    {
...
HI_S32 HI_MPI_HDMI_ComGetHdcpCapability(HI_UNF_HDMI_ID_E enHdmi,HI_UNF_HDMI_HDCP_CAP_S *pstHdcpCap)
{
        ...
        || (stDrvHdcpCap.stHdcpCap.enHdcp22Cap == HDMI_HDCP_ERROR))
    {
        ...
        pstHdcpCap->bHdcp22Support = HI_FALSE;
    }
    HDMI_UNLOCK();
    return s32Ret;
}
...
HI_S32 HI_MPI_HDMI_SetSuspendState(HI_UNF_HDMI_ID_E enHdmi, HI_MPI_HDMI_SUSPEND_MODE_E enSuspendMode)
{
    HI_S32                  s32Ret = HI_SUCCESS;
    DRV_HDMI_SUSPEND_STAT_S stSuspend = {0};
    HDMI_CHECK_ID(enHdmi);

    stSuspend.enHdmiID      = enHdmi;
    if (HI_MPI_HDMI_SUSPEND_MODE_FAKE == enSuspendMode)
    {
        stSuspend.enSuspend = HDMI_SUSPEND_FAKE;
    }
    else if (HI_MPI_HDMI_SUSPEND_MODE_REAL == enSuspendMode)
    {
        stSuspend.enSuspend = HDMI_SUSPEND_REAL;
    }
    else
    {
        stSuspend.enSuspend = HDMI_SUSPEND_NONE;
    }

    HDMI_LOCK();
    s32Ret = ioctl(s_s32HdmiDevFd, CMD_HDMI_SET_SUSPEND_STAT, &stSuspend);
    HDMI_UNLOCK();
    s32Ret = (s32Ret != HI_SUCCESS) ? HI_FAILURE : HI_SUCCESS ;
