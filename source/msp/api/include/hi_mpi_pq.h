...
#define PQ_API_MAX_LEVEL 100

#define PQ_API_CHECK_NULL_PTR(ptr) \
    do{\
        if (HI_NULL == ptr){\
...
#define PQ_API_CHECK_ENID_OVER_RANGE(enId, enMaxIdButt) \
    do{\
        if (enId >= enMaxIdButt){\
            HI_ERR_PQ("[%d] enId over range!\n",enId);\
            return HI_ERR_PQ_INVALID_PARA;\
        }\
    }while(0)

#define PQ_API_CHECK_NUM_OVER_RANGE(num, MAX) \
    do{\
        if (num > MAX){\
