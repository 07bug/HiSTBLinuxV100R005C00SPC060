...
static HI_BOOL JPEG_ReadIFDTree(HI_ULONG HdecHandle, JPEG_EXIF_ENTRY_S* pstEntry);
...
static HI_BOOL JPEG_GetThumbnailOffset(HI_ULONG HdecHandle, JPEG_EXIF_ENTRY_S* pstEntry);
static HI_BOOL JPEG_GetThumbnailSize(HI_ULONG HdecHandle, JPEG_EXIF_ENTRY_S* pstEntry);


static HI_BOOL JPEG_GetImageTitle(HI_ULONG HdecHandle, JPEG_EXIF_ENTRY_S* pstEntry);
static HI_BOOL JPEG_GetCustomRendered(HI_ULONG HdecHandle, JPEG_EXIF_ENTRY_S* pstEntry);
static HI_BOOL JPEG_GetDateTime(HI_ULONG HdecHandle, JPEG_EXIF_ENTRY_S* pstEntry);
...
boolean JPEG_SDEC_GetExif(j_decompress_ptr cinfo)
{
    ...
    HI_GFX_Memset(&stEntry, 0, sizeof(stEntry));
    ...
    if (!JPEG_ReadIFDTree((HI_ULONG)cinfo->client_data, &stEntry))
    {
        return HI_FALSE;
    }
    ...
    while ((u16EntryCount > 0) && (HI_TRUE == bRetVal))
    {
        ...
        switch (u16EntryTag)
        {
            ...
            case JPEG_EXIF_THUMBNAIL_OFFSET:
                bRetVal = JPEG_GetThumbnailOffset(HdecHandle, pstEntry);
                break;

            case JPEG_EXIF_THUMBNAIL_SIZE:
                bRetVal = JPEG_GetThumbnailSize(HdecHandle, pstEntry);
                break;

            case JPEG_EXIF_IMAGE_TITLE:
                bRetVal = JPEG_GetImageTitle(HdecHandle, pstEntry);
                break;

            case JPEG_EXIF_CUSTOM_RENDERED:
                bRetVal = JPEG_GetCustomRendered(HdecHandle, pstEntry);
                break;

            case JPEG_EXIF_DATE_TIME:
                bRetVal = JPEG_GetDateTime(HdecHandle, pstEntry);
                break;
...
static HI_BOOL JPEG_ReadEntryInfo(JPEG_EXIF_ENTRY_S* pstEntry)
{
    ...
    if ((NULL == pstEntry) || (NULL == pstEntry->pNextInputByte))
    {
       return HI_FALSE;
    }
...
static HI_BOOL JPEG_GetThumbnailOffset(HI_ULONG HdecHandle, JPEG_EXIF_ENTRY_S* pstEntry)
{
    ...
    if ((NULL == pJpegHandle) || (NULL == pstEntry))
    {
       return HI_FALSE;
    }
...
static HI_BOOL JPEG_GetDateTime(HI_ULONG HdecHandle, JPEG_EXIF_ENTRY_S* pstEntry)
{
    ...
    pJpegHandle->stExifInfo.u8Time[JPEG_EXIF_TIME_STR_LENGTH] = '\0';
