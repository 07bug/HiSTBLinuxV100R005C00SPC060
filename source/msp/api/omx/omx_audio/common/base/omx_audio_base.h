...
#define ADEC_FILE_SAVE 1
//#define ADEC_DYNAMIC_SAVE_SUPPORT 1
...
#undef  LOG_TAG
#define LOG_TAG             "HISI_OMXAUDIO"

#define AUDIO_DEBUG_STATE   1
#define AUDIO_DEBUG_TRACE   0


#ifndef HI_ADVCA_FUNCTION_RELEASE

#if (1 == AUDIO_DEBUG_STATE)
#define OAD_PRINT_STATE     ALOGD
#else
#define OAD_PRINT_STATE(format...)
#endif

#define OAD_PRINT_ERROR     ALOGE

#ifdef ANDROID
#define OMX_AUDIO_LOGCAT    "persist.sys.audio.omx.logcat"

HI_BOOL g_EnableLogcatOAD;

#define OAD_ALOGD_IF(cond, ...) \
    ( (__builtin_expect((cond)!=0, 0)) \
    ? ((void)ALOG(LOG_VERBOSE, LOG_TAG, __VA_ARGS__)) : (void)0 )

#define OAD_PRINT_TRACE(...)  OAD_ALOGD_IF(g_EnableLogcatOAD, __VA_ARGS__)
#else
#if (1 == AUDIO_DEBUG_TRACE)
#define OAD_PRINT_TRACE     ALOGD
#else
#define OAD_PRINT_TRACE(format...)
#endif
#endif
...
#define HA_UNUSED_PARAMETER(p) (HI_VOID)(p)
 ...
 *    bufferOwner       - ells about owner of each buffer for UseBuffer or AllocateBuffer.  add by sdk
 *                            0  =  UseBuffer
 *                           1   =  AllocateBuffer
 */
struct _BufferList
...
typedef struct
{
    ...
    pthread_t                      thread_id;
    fd_set                         rfds;
    unsigned int                   datapipe[PIPE_BUTT];
    unsigned int                   cmdspipe[PIPE_BUTT];
    ...
    HI_BOOL                        mEndOfInput;
    /* mInputBufferCnt=0: means component init or flush, need to do some SetConfig (such as aac/mp3) etc. */
    ...
    N/A
    */
    HI_S32 (*CodecFrame)(HI_VOID* pHAState, OMX_BUFFERHEADERTYPE* pInBufHdr, OMX_BUFFERHEADERTYPE* pOutBufHdr);
} HI_AUDDATATYPE;
...
#define OMX_CONF_CHECK_NULLPTR(_ptr) \
            do { \
                if (!_ptr){ \
                    return OMX_ErrorBadParameter; \
                } \
...
do {                                                     \
    ...
    _ppBufHdr = _pBufHdr;                                                             \
    if ((HI_U32)_pList.nListEnd == (_pPortDef->nBufferCountActual - 1)) \
