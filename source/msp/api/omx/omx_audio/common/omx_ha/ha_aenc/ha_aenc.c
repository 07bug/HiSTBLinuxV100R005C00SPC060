...
static HI_U32 AENCGetEncodeInDataSize(HI_HAENCODE_OPENPARAM_S* pstOpenPram)
{
    ...
    {
        return pstOpenPram->u32SamplePerFrame * pstOpenPram->u32DesiredOutChannels * sizeof(HI_U16);
    }

    return pstOpenPram->u32SamplePerFrame * pstOpenPram->u32DesiredOutChannels * sizeof(HI_U32);
}
...
HI_S32  HA_OMX_InitEncoder(HI_AUDDATATYPE* pHAData)
{
    ...
    if (HI_NULL == pstInnerBuf->pInBuffer)
    {
        ...
        if (HI_NULL == pstInnerBuf->pInBuffer)
        {
            OMX_OSAL_Trace(OMX_OSAL_TRACE_ERROR, "mall inner buffer Failed!\n");
            return HI_FAILURE;
        }
    }
    else
...
HI_S32 HA_OMX_EncodeFrame(HI_VOID* pHAState,
                          OMX_BUFFERHEADERTYPE* pInBufHdr,
                          OMX_BUFFERHEADERTYPE* pOutBufHdr)
{
    ...
    else
    {
        ...
        if (u32EncodeInDataSize > (HI_U32)pstInnerBuf->s32Insize)
        {
    ...
    if (HA_ErrorNone == s32Ret)
    {
        ...
        if (0 != pstAenc->sOpenPram.u32DesiredSampleRate && 0 != pstAenc->sOpenPram.u32DesiredOutChannels)
        {
