...
OMX_ERRORTYPE component_init(OMX_HANDLETYPE phandle, OMX_STRING comp_name)
{
    ...
    OMX_CONF_CHECK_NULLPTR(phandle);
    ...
    pHAData->sInBufList.pBufHdr = (OMX_BUFFERHEADERTYPE**)OMX_OSAL_Malloc(sizeof(OMX_BUFFERHEADERTYPE*) * pHAData->sInPortDef.nBufferCountActual);
    ...
    pHAData->sOutBufList.pBufHdr = (OMX_BUFFERHEADERTYPE**)OMX_OSAL_Malloc(sizeof(OMX_BUFFERHEADERTYPE*) * pHAData->sOutPortDef.nBufferCountActual);
    ...
    err = pthread_create(&pHAData->thread_id, HI_NULL, HI_OMX_AENC_ComponentThread, pHAData);
    if (0 != err)
    {
...
EXIT:
    ...
    {
        if (pHAData->sOutBufList.pBufHdr)
        {
            OMX_OSAL_Free(pHAData->sOutBufList.pBufHdr);
        }
        if (pHAData->sInBufList.pBufHdr)
        {
            OMX_OSAL_Free(pHAData->sInBufList.pBufHdr);
        }
        if (pHAData->hEncoder)
        {
