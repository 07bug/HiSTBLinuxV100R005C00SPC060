...
OMX_S8 channel_save_yuv(FILE* pFile, SaveYuvOption *pOption)
{
    ...
    OMX_U8* ul = NULL;
    OMX_U8* vl = NULL;
    OMX_U32 chrom_width = 0;
    OMX_U32 chrom_height = 0;

    if (!pFile || !pOption || !pOption->pstChroml || !pOption->pYaddress || !pOption->pCaddress)
    {
        DEBUG_PRINT_ERROR("%s: INVALID PARAM!\n", __func__);
        return -1;
    }

    chrom_width = pOption->Width / 2;
    chrom_height = pOption->Height / 2;
