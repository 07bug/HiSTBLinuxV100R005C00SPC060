...
#else
    pvdec_buf->bufferaddr = HI_MMZ_Map(pvdec_buf->phyaddr, 0);
...
error_exit3:
#ifndef HI_OMX_TEE_SUPPORT
    (HI_VOID)HI_MMZ_Unmap(pvdec_buf->phyaddr);
error_exit2:
#endif
       ...
       OMX_IN OMX_BUFFERHEADERTYPE *omx_bufhdr)
{
    ...
    if (puser_buf == NULL)
    {
        DEBUG_PRINT_ERROR("[FB] puser_buf is NULL\n");
    ...
    #else
        (HI_VOID)HI_MMZ_Unmap(puser_buf->phyaddr);
    #endif
...
static OMX_S8 save_this_frame(OMX_COMPONENT_PRIVATE *pcom_priv, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    if (NULL == (*ppstYuvFp))
    {
        ...
        if ((strlen(pstFrame->save_path) > (PATH_LEN - 1)) || (realpath(pstFrame->save_path, real_path) == NULL))
        {
            DEBUG_PRINT_ERROR("%s file path invalid\n", __func__);
            goto error;
        }

        *ppstYuvFp = fopen(real_path, "w");
        if (!(*ppstYuvFp))
        {
    ...
    if (NULL == (*ppstChroml))
    {
        if (pstFrame->width * pstFrame->height == 0)
        {
            DEBUG_PRINT_ERROR("%s width and height invalid\n", __func__);
            goto error1;
        }
