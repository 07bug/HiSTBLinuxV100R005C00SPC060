...
OMX_BOOL g_AndroidTest = OMX_FALSE;
#endif
OMX_BOOL g_NotRender = OMX_FALSE;
...
OMX_U32 get_time_ms(void)
{
    struct timeval tv;
    OMX_U32 ms = 0;

    (void)gettimeofday(&tv, NULL);
    ms = (OMX_U32)(tv.tv_sec * 1000 + tv.tv_usec / 1000);

    return ms;
}

OMX_ERRORTYPE omx_report_event(
...
static OMX_ERRORTYPE fill_buffer_done(OMX_COMPONENT_PRIVATE* pcom_priv, OMX_BUFFERHEADERTYPE *buffer)
{
    ...
    if (port_priv->m_port_flushing || g_NotRender)
    {
        buffer->nFilledLen = 0;
    }
    ...
    DEBUG_PRINT_TRACE("[FBD] success: useraddr: %-15p  phy: 0x%-10x  data_len: %-5ld  timestamp: %-15lld gap time:  %ld\n", buffer->pBuffer, \
            ((OMXVDEC_BUF_DESC *)buffer->pOutputPortPrivate)->phyaddr, buffer->nFilledLen, buffer->nTimeStamp,
            (pcom_priv->time_info.fbd_lasttime != 0) ? (get_time_ms() - pcom_priv->time_info.fbd_lasttime) : 0);

    pcom_priv->time_info.fbd_lasttime = get_time_ms();
...
static OMX_ERRORTYPE empty_buffer_done(OMX_COMPONENT_PRIVATE* pcom_priv, OMX_BUFFERHEADERTYPE *buffer)
{
    ...
    DEBUG_PRINT_TRACE("[EBD] success <<< useraddr: %-15p  phy: 0x%-15x gap time:  %ld\n", buffer->pBuffer, \
            ((OMXVDEC_BUF_DESC *)buffer->pInputPortPrivate)->phyaddr,
            (pcom_priv->time_info.ebd_lasttime != 0) ? (get_time_ms() - pcom_priv->time_info.ebd_lasttime) : 0);

    pcom_priv->time_info.ebd_lasttime = get_time_ms();

    return ret;
}

static OMX_ERRORTYPE use_buffer_gstpath(
       OMX_COMPONENT_PRIVATE *pcom_priv,
       ...
       OMX_IN OMX_U8* p_handle)
{
    ...
    private_handle_gst *p_private_handle = (private_handle_gst *)p_handle;
    ...
    OMX_CHECK_ARG_RETURN(p_private_handle->metadata_addr == 0);

    HI_U32 framePhyaddr = 0;
    HI_U32 metadataPhyaddr = 0;

#ifndef HI_OMX_TEE_SUPPORT
    HI_U32 frameBufSize;
#endif
    HI_U32 metadataBufSize;
    ...
    {
        DEBUG_PRINT_ERROR("[UBG] Error: buf size invalid, bytes = %ld, buf_size = %ld\n", bytes, port_priv->port_pro.buffer_size);
    ...
    {
        DEBUG_PRINT_ERROR("[UBG] Error: no Free buffer heads found\n");
        return OMX_ErrorInsufficientResources;
    }
    i = port_priv->m_cur_buf_num;

#ifdef HI_OMX_TEE_SUPPORT
    framePhyaddr  = p_private_handle->frame_addr;
#else
    ret = HI_MMZ_GetPhyaddr((HI_VOID *)p_private_handle->frame_addr, &framePhyaddr, &frameBufSize);
    if (ret < 0)
    {
        DEBUG_PRINT_ERROR("%s framePhyaddr HI_MMZ_GetPhyaddr error!\n", __func__);
        return OMX_ErrorHardware;
    }
#endif
    ret = HI_MMZ_GetPhyaddr((HI_VOID *)p_private_handle->metadata_addr, &metadataPhyaddr, &metadataBufSize);
    if (ret < 0)
    {
        DEBUG_PRINT_ERROR("%s metadataPhyaddr HI_MMZ_GetPhyaddr error!\n", __func__);
        return OMX_ErrorHardware;
    }
    ...
    if (!pomx_buf)
    {
        DEBUG_PRINT_ERROR("[UBG] Error: alloc omx buffer pomx_buf failed\n");
        return OMX_ErrorInsufficientResources;
    }
    pvdec_buf = (OMXVDEC_BUF_DESC *)malloc(sizeof(OMXVDEC_BUF_DESC));
    if (!pvdec_buf)
    {
        DEBUG_PRINT_ERROR("[UBG] Error: alloc omx buffer pvdec_buf failed\n");
    ...
    pvdec_buf->bufferaddr       = (HI_SIZE_T)p_private_handle->frame_addr;
    pvdec_buf->phyaddr          = framePhyaddr;
    pvdec_buf->buffer_len       = buf_size;
    pvdec_buf->client_data      = (HI_SIZE_T)(OMX_PTR)pomx_buf;
    pvdec_buf->buffer_type      = OMX_USE_NATIVE;
    pvdec_buf->meta_type        = OMX_ALLOCATE_WITH_META;
    //VPSS假4K输出需要额外分配buf传递信息
    if (metadataPhyaddr != (OMX_U32)-1 && metadataPhyaddr != 0)
    {
        pvdec_buf->private_phyaddr = metadataPhyaddr;
        pvdec_buf->private_len     = 4*1024;
    ...
    {
        DEBUG_PRINT_ERROR("[UBG] Error: vdec bind user buf failed\n");
        error = OMX_ErrorUndefined;
        goto error_exit2;
    }
    DEBUG_PRINT_ERROR("[UBG] channel_bind_buffer ok\n");
    ...
    *omx_bufhdr = pomx_buf;
    ...
    DEBUG_PRINT_TRACE("[UBG] framevir = 0x%x,framephy = 0x%x,size = %d,metavir = 0x%x,metaphy = 0x%x\n",
         pvdec_buf->bufferaddr,pvdec_buf->phyaddr,pvdec_buf->buffer_len, p_private_handle->metadata_addr,
         pvdec_buf->private_phyaddr);
...
error_exit1:
    free(pomx_buf);
    pomx_buf = NULL;
    DEBUG_PRINT_ERROR("[UBG] Use %s buffer %d failed\n", (port == OUTPUT_PORT_INDEX) ? "out" : "in", (int)i);
...
static void omx_setprop(void)
{
    ...
    g_AndroidTest = OMX_FALSE;

    if (property_get("sys.app.compatibility.enhance", value, NULL)
            && (!strcasecmp("true", value)))
    {
        g_AndroidTest = OMX_TRUE;
    }

    g_NotRender = OMX_FALSE;

    if (property_get("service.media.omx.notrender", value, NULL)
            && (!strcasecmp("true", value)))
    {
        g_NotRender = OMX_TRUE;
    }

    DEBUG_PRINT_TRACE("setprop logcat:%d test:%d render:%d\n", g_EnableLogcatV, g_AndroidTest, g_NotRender);
...
static OMX_S32 get_buffer_phyaddr(OMX_U32 port, OMXVDEC_DRV_CFG *drv_cfg, OMX_U8 *buffer, HI_U32 *phyaddr, eBUFFER_TYPE *buffer_type)
{
    OMX_S32 ret = HI_FAILURE;
    HI_U32 buffer_size = 0;
    OMX_BOOL isVpssBypass = OMX_FALSE;
    OMX_BOOL isOnLive = OMX_FALSE;
    ...
    isVpssBypass = drv_cfg->bVpssBypass;
    isOnLive = drv_cfg->bIsOnLive;

#ifdef HI_OMX_TEE_SUPPORT
    if ((port == OUTPUT_PORT_INDEX) && isVpssBypass)
    {
    ...
    if (isOnLive && (port == INPUT_PORT_INDEX))
    {
        *buffer_type = OMX_USER_BIGBUF;
        *phyaddr = buffer;
        ret = HI_SUCCESS;
    }
    else
    ...
    DEBUG_PRINT_ALWS("%s:%d port:%d is_bypass:%d buffer:0x%x phyaddr:0x%x size:%d\n", \
                      __func__, __LINE__, port, isVpssBypass, buffer, &phyaddr, buffer_size);
...
#ifdef HI_TVOS_SUPPORT
    ret = get_buffer_phyaddr(port, &(pcom_priv->drv_ctx.drv_cfg), buffer, &Phyaddr, &buffer_type);
    if(ret != HI_SUCCESS)
    {
        ...
        OMX_IN OMX_U32 bytes)
{
    ...
    OMXVDEC_BUF_DESC *pvdec_buf = NULL;
    p_bufffer_handle *pbuffer_handle = NULL;
    ...
    /* check alloc bytes */
    if (!pcom_priv->drv_ctx.drv_cfg.bMetaDataAlloc)
    {
        if (bytes != buf_size)
        {
    ...
    if(pcom_priv->drv_ctx.drv_cfg.bMetaDataAlloc && OUTPUT_PORT_INDEX == port)
    {
        pbuffer_handle = (p_bufffer_handle *)malloc(sizeof(p_bufffer_handle));
        if (!pbuffer_handle)
        {
            DEBUG_PRINT_ERROR("[AB] Error: alloc omx buffer pbuffer_handle failed\n");
            error = OMX_ErrorInsufficientResources;
            goto error_exit1;
        }
        memset(pbuffer_handle,  0, sizeof(p_bufffer_handle));
        pvdec_buf->private_len = METADATA_ALLOC_SIZE;
        pvdec_buf->meta_type = OMX_ALLOCATE_WITH_META;
    }
    ...
    pvdec_buf->buffer_len       = bytes;//buf_size;
...
#ifdef HI_OMX_TEE_SUPPORT
#ifdef ANDROID
    if ((port == INPUT_PORT_INDEX) && (pcom_priv->m_input_use_native_buf))
    {
        pvdec_buf->bufferaddr = (HI_SIZE_T)native_handle_create(0, 2);

        if (pvdec_buf->bufferaddr == HI_NULL)
        {
            DEBUG_PRINT_ERROR("[AB] Error: native handle create failed!\n");
            error =  OMX_ErrorUndefined;
            goto error_exit3;
        }

        ((native_handle_t *)(pvdec_buf->bufferaddr))->data[0] = pvdec_buf->phyaddr;
        ((native_handle_t *)(pvdec_buf->bufferaddr))->data[1] = pvdec_buf->buffer_len;
    }
#endif
#else
    ...
    if (pcom_priv->drv_ctx.drv_cfg.bMetaDataAlloc && OUTPUT_PORT_INDEX == port)
    {
        pvdec_buf->private_user_viraddr = (HI_SIZE_T)HI_MMZ_Map(pvdec_buf->private_phyaddr, OMX_FALSE);
        if (pvdec_buf->private_user_viraddr == HI_NULL)
        {
           DEBUG_PRINT_ERROR("[AB] Error: map buffer failed\n");
            error =  OMX_ErrorUndefined;
            goto error_exit4;
    ...
    if (port == OUTPUT_PORT_INDEX)
    {
        if(pcom_priv->drv_ctx.drv_cfg.bMetaDataAlloc)
        {
            pbuffer_handle->phyAddr = pvdec_buf->phyaddr;
            pbuffer_handle->Size = pvdec_buf->buffer_len;
            pbuffer_handle->metaDataPhyAddr = pvdec_buf->private_phyaddr;
            pbuffer_handle->metaDataSize = pvdec_buf->private_len;
            pbuffer_handle->metaDataVirAddr = pvdec_buf->private_user_viraddr;
            pomx_buf->pBuffer = (OMX_U8 *)(pbuffer_handle);
        }
        else
        {
            pomx_buf->pBuffer = (OMX_U8*)(long)pvdec_buf->bufferaddr;
...
error_exit5:
    if(pcom_priv->drv_ctx.drv_cfg.bMetaDataAlloc && OUTPUT_PORT_INDEX == port)
    {
        HI_MMZ_Unmap(pvdec_buf->private_phyaddr);
    }
...
error_exit4:
    ...
    if ((port == INPUT_PORT_INDEX) && (pcom_priv->m_input_use_native_buf))
    {
        native_handle_delete((native_handle_t *)(OMX_PTR)(pomx_buf->pBuffer));
    }
        ...
        OMX_IN OMX_BUFFERHEADERTYPE *omx_bufhdr)
{
...
#ifdef HI_TVOS_SUPPORT
        DEBUG_PRINT_ALWS("%s: flag:0x%x bypass:%d phyaddr:0x%x\n", __func__, omx_bufhdr->nFlags, pcom_priv->drv_ctx.drv_cfg.bVpssBypass, puser_buf->phyaddr);
    ...
    if (OMX_ALLOCATE_USR == puser_buf->buffer_type || OMX_ALLOCATE_SECURE == puser_buf->buffer_type)
    {
        ...
        if ((port == INPUT_PORT_INDEX) && (pcom_priv->m_input_use_native_buf))
        {
            native_handle_delete((native_handle_t *)(OMX_PTR)(puser_buf->bufferaddr));
        }
#endif
#else
        HI_MMZ_Unmap(puser_buf->phyaddr);
#endif
        if(OMX_ALLOCATE_WITH_META == puser_buf->meta_type && OUTPUT_PORT_INDEX == port)
        {
            HI_MMZ_Unmap(puser_buf->private_phyaddr);
        }
        channel_release_buffer(&pcom_priv->drv_ctx, puser_buf);
    }
    if(OMX_ALLOCATE_WITH_META == puser_buf->meta_type && OUTPUT_PORT_INDEX == port)
    {
        free((p_bufffer_handle *)(omx_bufhdr->pBuffer));
        omx_bufhdr->pBuffer = NULL;
    }
    free(puser_buf);
    puser_buf = NULL;
    if(omx_bufhdr->pPlatformPrivate)
    {
        ...
        omx_bufhdr->pPlatformPrivate = NULL;
    }
    free(omx_bufhdr);
        ...
        OMX_BOOL update_input_size)
{
    ...
    OMX_U32 max_frame_width = 0;
    OMX_U32 max_frame_height = 0;

    OMX_CHECK_ARG_RETURN(pcomp_priv == NULL);

    max_frame_width = pcomp_priv->pic_info.max_frame_width;
    max_frame_height = pcomp_priv->pic_info.max_frame_height;
...
static OMX_ERRORTYPE get_supported_profile_level(OMX_COMPONENT_PRIVATE *pcomp_priv, OMX_VIDEO_PARAM_PROFILELEVELTYPE *profileLevelType)
{
    ...
    else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_ROLE_VIDEO_DECODER_AVS2, sizeof(OMX_ROLE_VIDEO_DECODER_AVS2)))
    {
        pinfo = avs2_profile_level_list;
        max_num = COUNTOF(avs2_profile_level_list);
    }
    ...
    else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_ROLE_VIDEO_DECODER_DOLBYVISION, sizeof(OMX_ROLE_VIDEO_DECODER_DOLBYVISION)))
    {
        pinfo = dolbyhdr_profile_level_list;
        max_num = COUNTOF(dolbyhdr_profile_level_list);
    }
...
static void message_resp_output_done(OMX_COMPONENT_PRIVATE *pcom_priv, OMXVDEC_MSG_INFO *vdec_msg)
{
    ...
    if(pcom_priv->decAspects.mRange != puser_buf->dec_color_aspects.mRange
       || pcom_priv->decAspects.mPrimaries != puser_buf->dec_color_aspects.mPrimaries
       || pcom_priv->decAspects.mMatrixCoeffs != puser_buf->dec_color_aspects.mMatrixCoeffs
       || pcom_priv->decAspects.mTransfer != puser_buf->dec_color_aspects.mTransfer)
    {
        memcpy(&pcom_priv->decAspects, &puser_buf->dec_color_aspects, sizeof(OMXVDEC_DEC_COLOR_ASPECTS));
    }
...
static void message_resp_img_size_change(OMX_COMPONENT_PRIVATE *pcom_priv, OMXVDEC_MSG_INFO *vdec_msg)
{
    ...
    {
        DEBUG_PRINT_TUNNELED("Tunnel: Report image size changed: old_w: %ld, old_h:%ld, new_w:%ld, new_h:%ld\n",
                pcom_priv->pic_info.frame_width, pcom_priv->pic_info.frame_height, width, height);
    ...
    else
    {
        ...
        {
            DEBUG_PRINT_ALWS("image size changed: old_w: %ld, old_h:%ld, old stride:%ld new_w:%ld, new_h:%ld new_stide:%ld\n",
                    pcom_priv->pic_info.frame_width, pcom_priv->pic_info.frame_height, pcom_priv->pic_info.stride,
                    width, height, stride);
...
static OMX_S32 message_process(OMX_COMPONENT_PRIVATE  *pcom_priv, void* message)
{
    ...
    switch (vdec_msg->msgcode)
    {
        ...
        case VDEC_ERR_SYNTAX_ERR:
            post_event(pcom_priv, 0, vdec_msg->status_code, OMX_GENERATE_SYNTAX_ERROR);
            break;

        case VDEC_ERR_RATIO_NOTZERO:
            post_event(pcom_priv,vdec_msg->msgdata.err_size.errorratio, vdec_msg->status_code, OMX_GENERATE_ERROR_RATIO_NOT_ZEOR);
            break;
...
static OMX_ERRORTYPE component_start(OMX_COMPONENT_PRIVATE *pcom_priv)
{
    ...
    {
        if (pcom_priv->m_dec_fmt == OMX_VIDEO_CodingDolbyVision)
        {
            post_event(pcom_priv, 0, VDEC_S_SUCCESS, OMX_GENERATE_START_DONE);
            return OMX_ErrorNone;
        }
...
static OMX_S32 release_channel_safe(OMX_COMPONENT_PRIVATE *pcom_priv)
{
    OMX_S32 ret = 0;
...
#ifdef ANDROID
#ifdef ANDROID_O    // Recognize PureAndroid situation

            char value[PROPERTY_VALUE_MAX];
            if (property_get("ro.config.build.name", value, NULL) == 0)
            {
                pcom_priv->drv_ctx.drv_cfg.android_test = OMX_TRUE;
            }
#else

            pcom_priv->drv_ctx.drv_cfg.android_test = g_AndroidTest;
#endif
#else
            pcom_priv->drv_ctx.drv_cfg.android_test = OMX_FALSE;
#endif
        ...
        OMX_U32 *post_flag)
{
    ...
    if (state == OMX_StateLoaded)
    {
        ...
        if (ports_all_empty(pcom_priv))
        {
            ...
            if (release_channel_safe(pcom_priv) < 0)
            {
                DEBUG_PRINT_ERROR("ERROR: release channel failed\n");
...
#if (defined(ANDROID_N) || defined(ANDROID_O))
    if ((pcom_priv->drv_ctx.drv_cfg.TunnelModeEnable) && (pcom_priv->m_dec_fmt == OMX_VIDEO_CodingDolbyVision))
    {
        if (pcom_priv->csd.valid == 0)
        {
            if (((puser_buf->flags >> 5) & 1) || ((puser_buf->flags >> 7) & 1)) // OMX_BUFFERFLAG_SYNCFRAME or OMX_BUFFERFLAG_CODECCONFIG
            {
                ret = omx_tunnel_config_avlay_dolbyhdr(pcom_priv->m_pcomp, puser_buf);

                if (ret != OMX_ErrorNone)
                {
                    DEBUG_PRINT_ERROR("[ETB] ERROR: omx_tunnel_config_avlay_dolbyhdr failed\n");
                    ret = OMX_ErrorUndefined;
                    goto empty_error;
                }
            }
...
static void event_process(OMX_COMPONENT_PRIVATE *pcom_priv, OMX_U32 id)
{
    ...
    switch (id)
    {
        case OMX_GENERATE_COMMAND:
            ret = send_command_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_COMMAND_DONE:
            ret = command_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_ETB:
            ret = empty_this_buffer_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_EBD:
            ret = empty_buffer_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_FTB:
            ret = fill_this_buffer_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_FBD:
            ret = fill_buffer_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_FLUSH_INPUT_DONE:
            ret = flush_input_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_FLUSH_OUTPUT_DONE:
            ret = flush_output_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_START_DONE:
            ret = start_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_PAUSE_DONE:
            ret = pause_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_RESUME_DONE:
            ret = resume_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_STOP_DONE:
            ret = stop_done_proxy(pcom_priv, p1, p2);
            break;

        case OMX_GENERATE_IMAGE_SIZE_CHANGE:
            ret = image_size_change_proxy(pcom_priv, p1, p2);
            break;
        ...
        case OMX_GENERATE_SYNTAX_ERROR:
            DEBUG_PRINT_STATE("[EP] SYNTAX ERROR\n");
            omx_report_event(pcom_priv, OMX_EventMark, OMX_ErrorStreamCorrupt, 0, NULL);
            break;

        case OMX_GENERATE_ERROR_RATIO_NOT_ZEOR:
            DEBUG_PRINT_STATE("[EP] ERROR RATIO NOT ZERO:%ld\n", p1);
            omx_report_event(pcom_priv, OMX_EventMark, OMX_ErrorMbErrorsInFrame, p1, NULL);
            break;
    ...
    if (ret != OMX_ErrorNone)
    {
        DEBUG_PRINT_ERROR("%s, %d, id = %ldERROR\n", __func__, __LINE__, id);
    }
...
static OMX_S32 ports_init(OMX_COMPONENT_PRIVATE *pcom_priv)
{
    ...
    in_port->m_omx_bufhead = (OMX_BUFFERHEADERTYPE **)malloc(sizeof(OMX_BUFFERHEADERTYPE *) * DEF_MAX_IN_BUF_CNT_ON_LIVE);
    ...
    memset(in_port->m_omx_bufhead, 0, sizeof(OMX_BUFFERHEADERTYPE *) * DEF_MAX_IN_BUF_CNT_ON_LIVE);
...
#ifdef ANDROID
    if (pcomp_priv->m_output_use_native_buf)
    {
        switch (color_format)
        {
...
static void config_compress_format(OMX_COMPONENT_PRIVATE *pcomp_priv, OMX_U32 codec_index)
{
            ...
            && (pcomp_priv->m_dec_fmt != ((OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingVP6)))
    {
        pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd = codec_trans_list[codec_index].codec_type;
         //fix vts error
        if ((codec_trans_list[codec_index].codec_type == STD_Dolby_Vision)
         && (pcomp_priv->drv_ctx.drv_cfg.android_test == HI_TRUE))
        {
            pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd = (OMX_S32)STD_HEVC;
        }
    }
}
...
static OMX_ERRORTYPE get_parameter_IndexParamPortDefinition(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    if (portDefn->nPortIndex == INPUT_PORT_INDEX)
    {
        ...
        if (pcomp_priv->drv_ctx.drv_cfg.bIsOnLive)
        {
            port_priv->port_pro.max_count   = DEF_MAX_IN_BUF_CNT_ON_LIVE;
            port_priv->port_pro.buffer_size = DEFAULT_IN_BUF_SIZE_ON_LIVE;
        }
    ...
    else if (portDefn->nPortIndex == OUTPUT_PORT_INDEX)
    {
        ...
        if (pcomp_priv->drv_ctx.drv_cfg.bMetaDataAlloc && pcomp_priv->drv_ctx.drv_cfg.bVpssBypass)
        {
            portDefn->nBufferSize = DEF_MIN_OUT_BUF_SIZE;
        }
        else
        {
            portDefn->nBufferSize = port_priv->port_pro.buffer_size;
...
static OMX_ERRORTYPE set_parameter_IndexParamPortDefinition(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    if (portDefn->format.video.nFrameWidth != 0)
    {
        ...
        else
        {
            if ((pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd == STD_HEVC)
               || (pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd == STD_VP9))
            {
                portDefn->format.video.nStride = HI_OMX_GET_STRIDE(HI_OMX_ALIGN_10BIT(portDefn->format.video.nFrameWidth));
            }
            else
            {
    ...
    {
        if (pcomp_priv->m_dec_fmt == OMX_VIDEO_CodingDolbyVision)
        {
            return OMX_ErrorNone;
        }
...
static OMX_ERRORTYPE get_parameter_GoogleIndexGetAndroidNativeBufferUsage(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    if ((pusage->nPortIndex != OUTPUT_PORT_INDEX) || !pcomp_priv->m_output_use_native_buf)
    {
        DEBUG_PRINT_ERROR("Bad conditions: nPortIndex=%ld, m_use_native_buf=%d\n", pusage->nPortIndex ,pcomp_priv->m_output_use_native_buf);
...
static OMX_ERRORTYPE set_parameter_GoogleIndexEnableAndroidNativeBuffers(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    pcomp_priv->m_output_use_native_buf = penable->bEnable;
    ...
    if (pcomp_priv->pic_info.frame_width != 0)
    {
        ...
        else
        {
            ...
            if ((pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd == STD_HEVC)
               || (pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd == STD_VP9))
            {
                pcomp_priv->pic_info.stride = HI_OMX_GET_STRIDE(HI_OMX_ALIGN_10BIT(pcomp_priv->pic_info.frame_width));
            }
            else
            {
...
static OMX_ERRORTYPE set_parameter_GoogleIndexAllocateNativeHandle(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    OMX_COMPONENT_PRIVATE *pcomp_priv = (OMX_COMPONENT_PRIVATE *)pcomp->pComponentPrivate;
    struct EnableAndroidNativeBuffersParams  *penable = (struct EnableAndroidNativeBuffersParams *)param_data;
    ...
    pcomp_priv->m_input_use_native_buf = penable->bEnable;
...
static OMX_ERRORTYPE set_parameter_HISIIndexSetCallerName(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    static const OMX_U8 *xts_process[] = { "cts.media",
                                           "media.cts",
                                           "xts.media",
                                           "video.cts",
                                           "cts.videoperf",
    ...
    OMX_ERRORTYPE ret = OMX_ErrorNone;
    OMX_COMPONENT_PRIVATE *pcomp_priv = (OMX_COMPONENT_PRIVATE *)(pcomp->pComponentPrivate);
    OMX_HISI_CALLER_NAME *pCallerName = (OMX_HISI_CALLER_NAME *)(param_data);

    DEBUG_PRINT_ERROR("Caller Name: %s\n", pCallerName->strCallerName);

    pcomp_priv->drv_ctx.drv_cfg.android_test = OMX_FALSE;

    for (i = 0; i < NELEM(xts_process); i++)
    {
        if (strstr(pCallerName->strCallerName, xts_process[i]))
        {
            DEBUG_PRINT_ERROR("Set android_test TRUE!\n");
            pcomp_priv->drv_ctx.drv_cfg.android_test = OMX_TRUE;
            break;
        }
    }
...
static OMX_ERRORTYPE set_parameter_HISIIndexVideoPostProcessBypassInfo(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    pcomp_priv->drv_ctx.drv_cfg.output_view = pVpssBypassInfo->nSurfaceType;
    ret = channel_setVpssBypassInfo(&pcomp_priv->drv_ctx);

    DEBUG_PRINT_STATE("set output_view = %d\n", pcomp_priv->drv_ctx.drv_cfg.output_view);

    return ret;
}
static OMX_ERRORTYPE set_parameter_HISIIndexSetMetaDataAlloc(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
    OMX_COMPONENT_PRIVATE *pcomp_priv = (OMX_COMPONENT_PRIVATE *)(pcomp->pComponentPrivate);
    OMX_BOOL *meta_data_alloc = (OMX_BOOL *)(param_data);
    pcomp_priv->drv_ctx.drv_cfg.bMetaDataAlloc = *meta_data_alloc;

    DEBUG_PRINT_STATE("set bMetaDataAlloc = %d\n", pcomp_priv->drv_ctx.drv_cfg.bMetaDataAlloc);
    return ret;
}
#endif
...
static OMX_ERRORTYPE get_parameter_HisiIndexChannelAttributes(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    chan_attr->bIsOnlive                = pcomp_priv->drv_ctx.drv_cfg.bIsOnLive;
    //chan_attr->bIsGst                   = pcomp_priv->drv_ctx.drv_cfg.bIsGst;
...
static OMX_ERRORTYPE set_parameter_HisiIndexChannelAttributes(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    OMX_HISI_PARAM_CHANNELATTRIBUTES *chan_attr = (OMX_HISI_PARAM_CHANNELATTRIBUTES *)param_data;
    VDEC_CHAN_CFG_S *pchan_cfg = &(pcomp_priv->drv_ctx.drv_cfg.chan_cfg);
    pcomp_priv->drv_ctx.drv_cfg.bIsOnLive = chan_attr->bIsOnlive;
    //pcomp_priv->drv_ctx.drv_cfg.bIsGst    = chan_attr->bIsGst;
...
static const PARAMETER_HANDLER g_ParamerHandlerTable[] =
{
    ...
    },
    {
        ...
        NULL,
        set_parameter_GoogleIndexAllocateNativeHandle,
    },
    {
    ...
    },
    {
        ...
        NULL,
        set_parameter_HISIIndexSetCallerName,
    },
#endif
    ...
    },
    {
        OMX_HisiIndexSetMetaDataAlloc,
            NULL,
            set_parameter_HISIIndexSetMetaDataAlloc,
    },
#endif
    {
        ...
        OMX_INOUT OMX_PTR param_data)
{
    ...
    pcomp = (OMX_COMPONENTTYPE *)phandle;
    OMX_CHECK_ARG_RETURN(pcomp->pComponentPrivate == NULL);
...
#ifdef ANDROID
OMX_ERRORTYPE omx_covert_coloraspects_to_codecaspects(OMX_COMPONENT_PRIVATE *pcomp_priv, ColorAspects *pcolor_aspects)
{
    OMXVDEC_DEC_COLOR_ASPECTS *pdec_color_aspects = &pcomp_priv->decAspects;

    pcolor_aspects->mRange = pdec_color_aspects->mRange ? RangeFull : RangeLimited;

    switch(pdec_color_aspects->mPrimaries)
    {
        case 1:
        ...
        case 10:
            ...
            break;
        default:
            pcolor_aspects->mPrimaries = PrimariesUnspecified;
            break;
    }

    switch(pdec_color_aspects->mTransfer)
    {
        case 8:
        ...
        case 17:
            ...
            break;
        default:
            pcolor_aspects->mTransfer = TransferUnspecified;
            break;
    }

    switch(pdec_color_aspects->mMatrixCoeffs)
    {
        case 0:
        case 8:
        ...
        case 1:
            ...
            break;
        case 2:
            pcolor_aspects->mMatrixCoeffs = MatrixUnspecified;
            break;
        case 4:
...
OMX_ERRORTYPE omx_nontunnel_set_color_aspects(OMX_COMPONENTTYPE *pcomp, DescribeColorAspectsParams *describe_color_aspects_params)
{
    OMX_COMPONENT_PRIVATE *pcomp_priv = (OMX_COMPONENT_PRIVATE *)pcomp->pComponentPrivate;

    memcpy(&pcomp_priv->usrAspects, &describe_color_aspects_params->sAspects, sizeof(ColorAspects));

    return OMX_ErrorNone;
}

OMX_ERRORTYPE omx_nontunnel_get_color_aspects(OMX_COMPONENTTYPE *pcomp, DescribeColorAspectsParams *describe_color_aspects_params)
{
    OMX_COMPONENT_PRIVATE *pcomp_priv = (OMX_COMPONENT_PRIVATE *)pcomp->pComponentPrivate;
    ColorAspects *pcolor_aspects = &describe_color_aspects_params->sAspects;
    OMXVDEC_DEC_COLOR_ASPECTS *pdec_color_aspects = &pcomp_priv->decAspects;
    ColorAspects *pusr_color_aspects =  &pcomp_priv->usrAspects;
    ColorAspects tmpColorAspects = {0};

    omx_covert_coloraspects_to_codecaspects(pcomp_priv, &tmpColorAspects);
    pcolor_aspects->mPrimaries = tmpColorAspects.mPrimaries ? tmpColorAspects.mPrimaries : pusr_color_aspects->mPrimaries;
    pcolor_aspects->mTransfer = tmpColorAspects.mTransfer ? tmpColorAspects.mTransfer : pusr_color_aspects->mTransfer;
    pcolor_aspects->mMatrixCoeffs = tmpColorAspects.mMatrixCoeffs ? tmpColorAspects.mMatrixCoeffs : pusr_color_aspects->mMatrixCoeffs;
    pcolor_aspects->mRange = tmpColorAspects.mRange;
        ...
        OMX_INOUT OMX_PTR     config_data)
{
    ...
    switch (config_index)
    {
        ...
        {
            DescribeHDRStaticInfoParams *describe_hdr_static_info = (DescribeHDRStaticInfoParams *)config_data;
            if (describe_hdr_static_info->nPortIndex > OUTPUT_PORT_INDEX)
            {
                DEBUG_PRINT_ERROR("OMX_GoogleAndroidIndexDescribeHDRStaticInfo: Bad Port Index %d\n", describe_hdr_static_info->nPortIndex);

                return OMX_ErrorBadPortIndex;
            }

            ret = omx_tunnel_get_hdr_static_info(pcomp, describe_hdr_static_info);

            if (ret != OMX_ErrorNone)
            {
                DEBUG_PRINT_ERROR("OMX_GoogleAndroidIndexDescribeHDRStaticInfo: omx_tunnel_get_hdr_static_info failed!\n");
            }
        ...
        {
            DescribeColorAspectsParams *describe_color_aspects_params = (DescribeColorAspectsParams *)config_data;
            if (describe_color_aspects_params->nPortIndex > OUTPUT_PORT_INDEX)
            {
                DEBUG_PRINT_ERROR("OMX_GoogleAndroidIndexDescribeColorAspects: Bad Port Index %d\n", describe_color_aspects_params->nPortIndex);

                return OMX_ErrorBadPortIndex;
            }
            if (!pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable)
            {
                ret = omx_nontunnel_get_color_aspects(pcomp, describe_color_aspects_params);
            }
            else
            {
                ret = omx_tunnel_get_color_aspects(pcomp, describe_color_aspects_params);
            }
            if (ret != OMX_ErrorNone)
            {
                DEBUG_PRINT_ERROR("OMX_GoogleAndroidIndexDescribeHDRStaticInfo: omx_tunnel_get_hdr_static_info failed!\n");
        ...
        OMX_IN OMX_PTR config_data)
{
    ...
    switch (config_index)
    {
        ...
        case OMX_GoogleAndroidIndexDescribeHDRStaticInfo:
        {
            ...
            ret = omx_tunnel_set_hdr_static_info(pcomp, describe_hdr_static_info);

            if (ret != OMX_ErrorNone)
            {
                DEBUG_PRINT_ERROR("OMX_GoogleAndroidIndexDescribeHDRStaticInfo: omx_tunnel_set_hdr_static_info failed!\n");
            }
        ...
        case OMX_GoogleAndroidIndexDescribeColorAspects:
        {
            ...
            if (!pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable)
            {
                ret = omx_nontunnel_set_color_aspects(pcomp, describe_color_aspects_params);
            }
            else
            {
                ret = omx_tunnel_set_color_aspects(pcomp, describe_color_aspects_params);
            }
            if (ret != OMX_ErrorNone)
            {
                DEBUG_PRINT_ERROR("OMX_GoogleAndroidIndexDescribeHDRStaticInfo: omx_tunnel_set_hdr_static_info failed!\n");
        ...
        OMX_OUT OMX_INDEXTYPE *pindex_type)
{
...
#ifdef ANDROID
    OMX_COMPONENTTYPE *pcomp = (OMX_COMPONENTTYPE *)phandle;
    OMX_COMPONENT_PRIVATE *pcomp_priv = (OMX_COMPONENT_PRIVATE *)pcomp->pComponentPrivate;
#endif

    OMX_U32 param_length = strnlen(param_name, OMX_MAX_STRINGNAME_SIZE);
    ...
    else if (!strncmp(param_name, "OMX.google.android.index.allocateNativeHandle",\
                MAX(param_length, strlen("OMX.google.android.index.allocateNativeHandle"))))
    {
        *pindex_type = (OMX_INDEXTYPE)OMX_GoogleIndexAllocateNativeHandle;
    }
    ...
    else if (!strncmp(param_name, "OMX.HISI.index.SetMetaDataAlloc",
                      MIN(param_length, strlen("OMX.HISI.index.SetMetaDataAlloc"))))
    {
        *pindex_type = (OMX_INDEXTYPE)OMX_HisiIndexSetMetaDataAlloc;
    }
    else if (!strncmp(param_name, "OMX.google.android.index.describeHDRStaticInfo",
                      MIN(param_length, strlen("OMX.google.android.index.describeHDRStaticInfo"))))
    {
        if (!pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable)
        {
            DEBUG_PRINT_ALWS("get_extension_index: TunnelModeEnable = %d, %s not implemented!\n", pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable, param_name);

            return OMX_ErrorNotImplemented;
        }

        *pindex_type = (OMX_INDEXTYPE)OMX_GoogleAndroidIndexDescribeHDRStaticInfo;
    }
    else if (!strncmp(param_name, "OMX.google.android.index.describeColorAspects",
                      MIN(param_length, strlen("OMX.google.android.index.describeColorAspects"))))
    {
        *pindex_type = (OMX_INDEXTYPE)OMX_GoogleAndroidIndexDescribeColorAspects;
    }
    else if (!strncmp(param_name, "OMX.HISI.index.SetCallerName",\
                MAX(param_length, strlen("OMX.HISI.index.SetCallerName"))))
    {
        *pindex_type = (OMX_INDEXTYPE)OMX_HISIIndexSetCallerName;
    }
#endif
#ifdef VFMW_VPSS_BYPASS_EN
        ...
        OMX_IN OMX_BUFFERHEADERTYPE* omx_bufhdr)
{
    ...
    if ((pcom_priv->m_state == OMX_StateLoaded)
            ...
            || ((pcom_priv->m_state == OMX_StateIdle) && bit_present(&pcom_priv->m_flags, OMX_STATE_INPUT_DISABLE_PENDING))
            || ((pcom_priv->m_state == OMX_StateIdle) && bit_present(&pcom_priv->m_flags, OMX_STATE_OUTPUT_DISABLE_PENDING)))
    {
...
#ifdef ANDROID
    if (pcom_priv->m_output_use_native_buf && OUTPUT_PORT_INDEX == port)
    {
...
#endif
    {
        if (pcom_priv->drv_ctx.drv_cfg.bIsGst && OUTPUT_PORT_INDEX == port)
        {
            ret = use_buffer_gstpath(pcom_priv, bufferHdr, app_data, port, bytes, buffer);
        }
        else
        {
        ...
        OMX_IN OMX_BUFFERHEADERTYPE *buffer)
{
    ...
    OMX_CHECK_ARG_RETURN(phandle == NULL);
    OMX_CHECK_ARG_RETURN(buffer  == NULL);
    OMX_CHECK_ARG_RETURN(buffer->pInputPortPrivate == NULL);
    ...
    if ((pcom_priv->m_dec_fmt == OMX_VIDEO_CodingVP8)
        || (pcom_priv->m_dec_fmt == OMX_VIDEO_CodingVP9))
    {
    ...
    DEBUG_PRINT_TRACE("[ETB] pbuffer: %-15p phy: 0x%-15x data_len: %-5lu timestamp: %-15lld gap time:  %ld\n", \
            buffer->pBuffer, ((OMXVDEC_BUF_DESC *)buffer->pInputPortPrivate)->phyaddr, buffer->nFilledLen, buffer->nTimeStamp,
            (pcom_priv->time_info.etb_lasttime != 0) ? (get_time_ms() - pcom_priv->time_info.etb_lasttime) : 0);

    pcom_priv->time_info.etb_lasttime = get_time_ms();
        ...
        OMX_IN OMX_BUFFERHEADERTYPE *buffer)
{
    ...
    OMX_CHECK_ARG_RETURN(phandle == NULL);
    OMX_CHECK_ARG_RETURN(buffer  == NULL);
    OMX_CHECK_ARG_RETURN(buffer->pOutputPortPrivate == NULL);
    ...
    DEBUG_PRINT_TRACE("[FTB] bufhdr: %-15p  bufhdr->pBuffer: %-15p phy: 0x%-15x gap time:  %ld\n", \
            buffer, buffer->pBuffer, ((OMXVDEC_BUF_DESC *)buffer->pOutputPortPrivate)->phyaddr,
            (pcom_priv->time_info.ftb_lasttime != 0) ? (get_time_ms() - pcom_priv->time_info.ftb_lasttime) : 0);

    pcom_priv->time_info.ftb_lasttime = get_time_ms();
...
OMX_ERRORTYPE  component_deinit(OMX_IN OMX_HANDLETYPE phandle)
{
    ...
    (void)pthread_mutex_destroy(&pcom_priv->m_lock);
