...
static HI_S32 omx_tunnel_destroy_win(HI_HANDLE hWin)
{
    ...
    if (ret != HI_SUCCESS)
    {
        DEBUG_PRINT_ERROR("Tunnel: call HI_UNF_VO_DestroyWindow failed!\n");
    }
    ...
    if (ret != HI_SUCCESS)
    {
        DEBUG_PRINT_ERROR("Tunnel: call HI_UNF_VO_DeInit failed!\n");
    }
...
OMX_ERRORTYPE omx_tunnel_init(OMX_COMPONENTTYPE *pcomp, OMX_PTR pInitParam)
{
    ...
    {
        DEBUG_PRINT_ERROR("omx_tunnel_init: Bad port index %u\n", (int)pParams->nPortIndex);
    ...
    {
        DEBUG_PRINT_ERROR("omx_tunnel_init: Bad Handle %u\n", pParams->nAudioHwSync);
