...
HI_S32 HI_MPI_OTP_Init(HI_VOID)
{
    ...
    if (ret)
    {
        HI_ERR_OTP("Init otp usr proc failed ret=%d.\n", ret);
        HI_OTP_UNLOCK();
        return HI_FAILURE;
    }
#endif
...
HI_S32 HI_MPI_OTP_EnableTrustZone(HI_VOID)
{
    ...
    ret = OTP_PVItemRead(HI_OTP_SEC_WORD, (HI_U8 *)&sec_word, &sec_len);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_OTP("Failed to get scs status, return:%x\n", ret);
        return ret;
    }
    if(sec_word != 0x6EDBE953 && sec_word != 0x2A13C812)
    {
        HI_ERR_OTP("secure word check failed. sec_word:0x%x\n", sec_word);
        return HI_ERR_OTP_CHECK_FAILED;
    }
    if(sec_word == 0x2A13C812)
    {
        HI_INFO_OTP("Burn chipset to secure chipset\n");
        sec_word = 0x6EDBE953;
        ret = OTP_PVItemWrite(HI_OTP_SEC_WORD, (HI_U8 *)&sec_word, &sec_len);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_OTP("Failed to get scs status, return:%x\n", ret);
            return ret;
...
HI_S32 HI_MPI_OTP_GetPandaJtagMode(HI_UNF_OTP_JTAG_MODE_E *jtag_mode)
{
    ...
    ret = OTP_PVItemRead(HI_OTP_PANDA_JTAG_MODE, &data, &len);
    if (ret)
    {
        HI_FATAL_OTP("Fail to get panda jtag mode, return 0x%x.\n", ret);
        return ret;
    }
...
HI_S32 HI_MPI_OTP_SetRootKey(HI_UNF_OTP_ROOTKEY_E type, HI_U8 *key, HI_U32 len)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_OTP("Fail to rset otp, return:0x%x.\n", ret);
        return ret;
    }
...
HI_S32 HI_MPI_OTP_SetASCMSID (HI_U8* pu8ASCMSID, HI_U32 u32Len)
{
    ...
    return OTP_PVItemWrite(HI_OTP_ASC_MSID, pu8ASCMSID, &u32Len);
}
...
HI_S32 HI_MPI_OTP_GetASCMSID(HI_U8* pu8ASCMSID, HI_U32* pu32Len)
{
    ...
    return OTP_PVItemRead(HI_OTP_ASC_MSID, pu8ASCMSID, pu32Len);
}
...
HI_S32 HI_MPI_OTP_SetTAIDAndMSID(const HI_UNF_OTP_TA_INDEX_E enIndex, HI_U32 u32TAID, HI_U32 u32MSID)
{
    ...
    return OTP_PVItemWrite(tan_smid, data, &len);
}
...
HI_S32 HI_MPI_OTP_GetTAIDAndMSID(const HI_UNF_OTP_TA_INDEX_E enIndex, HI_U32* pu32TAID, HI_U32* pu32MSID)
{
    ...
    tan_smid = TAN_ID_DATA_E(enIndex);
    ret = OTP_PVItemRead(tan_smid, data, &len);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_OTP("OTP_PVItemRead failed,ret: %d\n", ret);
        return ret;
    }
    *pu32TAID = 0x0;
    *pu32MSID = 0x0;
...
HI_S32 HI_MPI_OTP_GetHardUARTStat(HI_UNF_OTP_UART_TYPE_E enUartType, HI_BOOL *pbDisable)
{
    ...
    ret = OTP_PVItemRead(id, &data, &len);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_OTP("Failed to get Hard Uart 2/3 Stat, return:%x\n", ret);
        return ret;
    }
...
HI_S32 HI_MPI_OTP_SetTEECtrlLock(HI_VOID)
{
    ...
    ret = OTP_DRV_PVItemLock(HI_OTP_SOC_TEE_ENABLE, HI_TRUE, &lock);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_OTP("Failed  set tee enable lock status, return:0x%X\n", ret);
        return ret;
    }
...
HI_S32 HI_MPI_OTP_GetTEECtrlLock(HI_BOOL* pbTEEEnL)
{
    ...
    ret = OTP_DRV_PVItemLock(HI_OTP_SOC_TEE_ENABLE, HI_FALSE, &lock);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_OTP("Failed  get tee enable lock status, return:0x%X\n", ret);
        return ret;
    }
