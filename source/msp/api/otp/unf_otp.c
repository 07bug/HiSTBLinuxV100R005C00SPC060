...
HI_S32 HI_UNF_OTP_SetBootMode(HI_UNF_OTP_FLASH_TYPE_E enFlashType)
{
    ...
    #else
        HI_PRINT("BOOT mode is not controlled by OTP.\n");
...
HI_S32 HI_UNF_OTP_GetPandaJtagMode(HI_UNF_OTP_JTAG_MODE_E *penJtagMode)
{
#ifdef OTP_VERSION_V100
    return HI_ERR_OTP_NOT_SUPPORT_INTERFACE;
#else
    return HI_MPI_OTP_GetPandaJtagMode(penJtagMode);
#endif
}
...
HI_S32 HI_UNF_OTP_SetASCMSID(HI_U8* pu8ASCMSID, HI_U32 u32Len)
{
    ...
    return HI_ERR_OTP_NOT_SUPPORT_INTERFACE;
#else
    return HI_MPI_OTP_SetASCMSID(pu8ASCMSID, u32Len);
#endif
}
...
HI_S32 HI_UNF_OTP_SetTEECtrlLock(HI_VOID)
{
    ...
    return HI_ERR_OTP_NOT_SUPPORT_INTERFACE;
#else
    return HI_MPI_OTP_SetTEECtrlLock();
#endif
}
...
HI_S32 HI_UNF_OTP_GetTEECtrlLock(HI_BOOL* pbTEEEnL)
{
    ...
    return HI_ERR_OTP_NOT_SUPPORT_INTERFACE;
#else
    return HI_MPI_OTP_GetTEECtrlLock(pbTEEEnL);
#endif
}
...
HI_S32 HI_UNF_OTP_DisableUART(HI_UNF_OTP_UART_TYPE_E enUartType)
{
    ...
    return HI_ERR_OTP_NOT_SUPPORT_INTERFACE;
#else
    return HI_MPI_OTP_DisableHardUART(enUartType);
#endif
}
...
HI_S32 HI_UNF_OTP_GetUARTStat(HI_UNF_OTP_UART_TYPE_E enUartType, HI_BOOL *pbDisable)
{
    ...
    return HI_ERR_OTP_NOT_SUPPORT_INTERFACE;
#else
    return HI_MPI_OTP_GetHardUARTStat(enUartType, pbDisable);
#endif
}
