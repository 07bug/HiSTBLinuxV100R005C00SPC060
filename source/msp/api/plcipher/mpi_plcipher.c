...
HI_S32 HI_MPI_PLCIPHER_SetAttr(HI_HANDLE hPLCipher, HI_UNF_PLCIPHER_ATTR_S* pstAttr)
{
    ...
    if(pstAttr->enAlg < HI_UNF_PLCIPHER_ALG_AES_IPTV || pstAttr->enAlg > HI_UNF_PLCIPHER_ALG_AES_CISSA)
    {
...
static inline HI_S32 ApiTrimDataArgs(HI_U32 SrcBuf, HI_U32 DstBuf, HI_U32 u32Len)
{
   if (u32Len < PLCIPHER_MIN_SIZE_PRE_DESC || u32Len > PLCIPHER_MAX_SIZE_PRE_DESC || u32Len % 188)
   {
       HI_ERR_PLCIPHER("data len = 0x%x is invalid, correct rang is (0x%x ~ 0x%x ), or not 188 times\n",
                       u32Len, PLCIPHER_MIN_SIZE_PRE_DESC, PLCIPHER_MIN_SIZE_PRE_DESC);
       return HI_ERR_DMX_INVALID_PARA;
   }

   if (SrcBuf % PLCIPHER_ADDR_ALIGN || DstBuf % PLCIPHER_ADDR_ALIGN)
   {
       HI_ERR_PLCIPHER("srcphyaddr[%#x] and dstphyaddr[%#x] must be %#x align\n",SrcBuf, DstBuf, PLCIPHER_ADDR_ALIGN);
       return HI_ERR_DMX_INVALID_PARA;
   }
   return HI_SUCCESS;
}
...
HI_S32 HI_MPI_PLCIPHER_Encrypt(HI_HANDLE hPLCipher, HI_VOID* pSrcBuf, HI_VOID* pDestBuf, HI_U32 u32Length)
{
    ...
    Ret = ApiTrimDataArgs(CIdata.ScrPhyAddr, CIdata.DestPhyAddr, CIdata.u32DataLength);
    if (Ret != HI_SUCCESS)
    {
        return Ret;
    }
