                               ...
                               char c, char d, char e, char f, char g, char * busnum, char* devnum)
{
    ...
    HI_OSAL_Snprintf(input, sizeof(input), patt1, a, b, c, d, e, f, g);

    if (NULL == realpath(input, file))
    {
        HI_INFO_PM("real path error: 0x%x\n", errno);
    }
    ...
    {
        memset(input, 0, sizeof(input));
        HI_OSAL_Snprintf(input, sizeof(input), patt2, busnum, devnum);
        memset(file, 0, sizeof(file));
        if (NULL == realpath(input, file))
        {
            HI_INFO_PM("1real path error: 0x%x\n", errno);
        }
        errno = 0;
        fd = open(file, O_RDWR);
        if (fd < 0)
...
static int dev_read_str(const char *patt, char* string)
{
    ...
    memset(input, 0, sizeof(input));
    HI_OSAL_Snprintf(input, sizeof(input), patt, string);

    //HI_ERR_PM("input path is 0x%s \n", input);
    if (NULL == realpath(input, file))
    {
        HI_INFO_PM("real path error: errno =0x%x\n", errno);
        return -1;
    }
...
static int dev_write_str(const char *patt, char* string, char *value)
{
    ...
    //HI_ERR_PM(" write input path is %s \n", input);
    if (NULL == realpath(input, file))
    {
        HI_INFO_PM("real path error: 0x%x \n", errno);
        return -1;
...
static int dev_read_int(const char *patt, char* string, int *value)
{
    ...
    //HI_INFO_PM(" read int input path is %s \n", input);
    if (NULL == realpath(input, file))
    {
        HI_INFO_PM("real path error: 0x%x \n", errno);
        return -1;
...
static int usb_set_remote(struct libusb * desc)
{
    DIR *sysfs_usb;
...
static int usb_get_remote(struct libusb * desc, unsigned char * intmask)
{
    DIR *sysfs_usb;
    struct dirent *dirent;
    struct libusb *lib = (struct libusb *)desc;
                ...
                if (rc != -1)
                {
                    ...
                    if (value != USB_CLASS_HUB)
                    {
                        ...
                        if (name[0]==0x31) {
                            ...
                            *intmask |= (0x01<<0);
                        } else if ((name[0]==0x33)&&(name[2]==0x31)) {
                            *intmask |= (0x01<<2);
                        }
#else
                        if (name[2] == 0x31)
