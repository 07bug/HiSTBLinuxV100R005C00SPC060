...
HI_U64 PVR_FILE_GetFileSize64(const HI_CHAR *pszFileName)
{
    ...
    if (PVR_FILE_INVALID_FILE != fd)
    {
        size += (HI_S64)pvr_lseek(fd, 0, (HI_S32)SEEK_END);
        size -= (HI_S64)nodeSize;
        pvr_close(fd);
    }
...
/*修正文件大小*/
HI_S32 PVR_FTRUNCATE64(PVR_FILE64 file, HI_U64 u64TsFileSize)
{
    PVR_FILE_S *pPvrFile = HI_NULL;
    HI_S32 pvrFd = 0;
    HI_U32 nodeIdx = 0;
    ...
    nodeIdx = (HI_U32)PVRFileGetNodeIdx(u64TsFileSize, pPvrFile->u64FileNodeSize);
    if (0 != nodeIdx)
    {
        u64TsFileSize -= (pPvrFile->u64FileNodeSize * nodeIdx);
        PVRFileGetRealFName(pPvrFile->szFileName, fNameOpen, (HI_S32)nodeIdx);
...
HI_S32 PVR_WriteStreamAlign(PVR_FILE64 fileDIO, HI_U8* pu8Buf, HI_U32 u32Len, HI_U64 u64OffsetInFile)
{
    ...
    PVRFileCheckPVRFd(pvrFd);
    PVRFileCheckPVRFdOpen(pvrFd);
    pPvrFileDIO = &g_stPvrFiles[pvrFd];
    ...
    PVRFileCheckPVRFd(pvrFd);
    PVRFileCheckPVRFdOpen(pvrFd);
    pPvrFileNew = &g_stPvrFiles[pvrFd];
