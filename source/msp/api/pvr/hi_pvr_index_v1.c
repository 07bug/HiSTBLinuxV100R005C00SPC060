...
static ssize_t PVRIndexReadIndexFromCache1(PVR_INDEX_HANDLE hIndex, HI_VOID *pBuf, HI_U32 u32ReadFrm)
{
    ...
    if (hIndex->bIsRec && !((HI_NULL != hIndex->pstLastRecordInfo) && (HI_TRUE == hIndex->pstLastRecordInfo->bAppend) && (u32ReadFrm == hIndex->pstLastRecordInfo->u32LastFrame)))
#else
    if (hIndex->bIsRec)
#endif
    {
    ...
    //检查是否在文件中
    if (!PVRIndexCheckReadFrmIsInFile1(u32ReadFrm, &hIndex->pstExtraBuffer->stFileCycInfo))
    {
    ...
    if (hIndex->stIdxReadCache.u32UsedNum == 0)
    {
        ...
        {
            u32CacheSize = (hIndex->pstExtraBuffer->stFileCycInfo.u32LastFrame - s32Start + 1)*sizeof(PVR_INDEX_ENTRY_V1_S);
        }
...
//对readcache进行边界检查，保证头部和尾部不在无效区间内
    if (PVRIndexCheckReadCacheHeadValid1(hIndex->stIdxReadCache.u32StartNum, u32ReadFrm, &hIndex->pstExtraBuffer->stFileCycInfo) &&
        PVRIndexCheckReadCacheTailValid1(hIndex->stIdxReadCache.u32StartNum + hIndex->stIdxReadCache.u32UsedNum - 1, u32ReadFrm, &hIndex->pstExtraBuffer->stFileCycInfo))
    {
...
//如果被覆盖或者没cache  中，重新读取cache

    {
        ...
        if (!PVRIndexCheckReadCacheHeadValid1((HI_U32)s32Start, u32ReadFrm, &hIndex->pstExtraBuffer->stFileCycInfo))
        {
            s32Start = hIndex->pstExtraBuffer->stFileCycInfo.u32StartFrame;
        }
        ...
        if (!PVRIndexCheckReadCacheTailValid1(u32CacheEndNum, u32ReadFrm, &hIndex->pstExtraBuffer->stFileCycInfo))
        {
        ...
        if (!PVRIndexChechBeCached1(u32ReadFrm, hIndex->stIdxReadCache.u32StartNum, hIndex->stIdxReadCache.u32UsedNum))
        {
            ...
            PVR_IDX_CACHE_UNLOCK(&(hIndex->stIdxReadCache.stCacheMutex));
            if ((hIndex->stCycMgr.u32LastFrame == u32ReadFrm) && (HI_TRUE == hIndex->bIsRec))
            {
                memcpy(pBuf, &hIndex->stLastFrame.stEntry2, sizeof(PVR_INDEX_ENTRY_V1_S));
                return sizeof(PVR_INDEX_ENTRY_V1_S);
            }
            else
            {
...
static ssize_t PVRIndexCacheReadIdx1(PVR_INDEX_HANDLE handle, PVR_FILE fd, HI_VOID* pData, size_t size, HI_U32 u32Offset)
{
    ssize_t readNum = 0;
                                        ...
                                        HI_S32 s32Num, HI_U32 u32Offset,HI_BOOL* bWriteFlag)
{
    ...
    PVR_CHECK_POINTER(pstEntry);
    PVR_CHECK_POINTER(bWriteFlag);
...
#endif
size_t PVRIndex_ReadIndex1(PVR_INDEX_ENTRY_NEW_S *pstEntryBuf, PVR_FILE fd, HI_U64 u64Offset, PVR_INDEX_HANDLE handle)
{
    size_t readSz = 0;
...
#ifndef HI_PVR_EXTRA_BUF_SUPPORT
    readSz = PVRIndexCacheReadIdx1(handle, fd, &(pstEntryBuf->stEntry2), (HI_U32)sizeof(PVR_INDEX_ENTRY_V1_S), (off_t)u64Offset);
#else
...
HI_S32 PVRIndex_GetEntryByNum1(const PVR_INDEX_HANDLE handle, PVR_INDEX_ENTRY_NEW_S *pEntry, HI_U32 u32FrameNum)
{
    ...
    readNum = PVRIndex_ReadIndex1(pEntry,handle->s32ReadFd,(u32FrameNum * sizeof(PVR_INDEX_ENTRY_V1_S)),handle);
