...
static HI_S32 PVRPlayPidSwitchProcess(PVR_PLAY_CHN_S *pChnAttr, HI_U32 u32NextIdx);
#endif
    ...
    for (i = 0; i < PVR_PLAY_MAX_CHN_NUM; i++)
    {
              ...
              || HI_UNF_PVR_PLAY_STATE_INVALID == pChnAttr->enState))
        {
            ...
            if (abs(pChnAttr->enSpeed) >= HI_UNF_PVR_PLAY_SPEED_NORMAL)
            {
                ...
                pChnAttr->u32PlaySyncTime += (s32SeedRatio * ((HI_S32)PVR_TIME_CTRL_TIMEBASE_NS)) / 1000000;
            }
            else/* slow mode */
            {
                ...
                pChnAttr->u32PlaySyncTime += (((HI_S32)PVR_TIME_CTRL_TIMEBASE_NS) / s32SeedRatio) / 1000000;
            }
        }
        ...
        if ((HI_S32)pChnAttr->u32PlaySyncTime < 0)
        {
            pChnAttr->u32PlaySyncTime = 0;
        }
    }
}
...
static HI_S32 PVRPlayShowProc(HI_PROC_SHOW_BUFFER_S* pstBuf, HI_VOID* pPrivData)
{
    ...
    for (i = 0; i < PVR_PLAY_MAX_CHN_NUM; i++)
    {
            ...
            (pChnAttr->enState != HI_UNF_PVR_PLAY_STATE_BUTT))
        {
            ...
            HI_PROC_Printf(pstBuf, "\tPlay mode         \t:%d\n", pChnAttr->stUserCfg.enTrickPlayToEndAction);
            HI_PROC_Printf(pstBuf, "\tStream type       \t:%s\n", pStreamType[u32VidType]);
            HI_PROC_Printf(pstBuf, "\tStream packet Size\t:%d\n", PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle));
...
#ifdef PVR_PID_CHANGE_SUPPORT
            HI_PROC_Printf(pstBuf, "\tPid Change Ctrl:\n");
            HI_PROC_Printf(pstBuf, "\t  ChangeTimes     \t:%u\n",pChnAttr->IndexHandle->pstPidCtrl->u32UsedNum);
            if (pChnAttr->IndexHandle->pstPidCtrl->u32IndexType == 1)
            {
                HI_PROC_Printf(pstBuf, "\t  IndexType       \t:%s\n","Video");
            }
            else if (pChnAttr->IndexHandle->pstPidCtrl->u32IndexType == 2)
            {
                HI_PROC_Printf(pstBuf, "\t  IndexType       \t:%s\n","Audio");
            }
            HI_PROC_Printf(pstBuf, "\t  CurPlayNum      \t:%u\n",pChnAttr->IndexHandle->u32CurPlayIdx);

            if (pChnAttr->IndexHandle->u32CurPlayIdx > 3)
            {
                u32PidStartNum= pChnAttr->IndexHandle->u32CurPlayIdx - 3;
            }
            for (j = u32PidStartNum; j < pChnAttr->IndexHandle->u32CurPlayIdx; j++)
            {
                pstPidInfo = &pChnAttr->IndexHandle->pstPidCtrl->stPidInfo[j];
            ...
            for (j = pChnAttr->IndexHandle->u32CurPlayIdx; j < pChnAttr->IndexHandle->pstPidCtrl->u32UsedNum; j++)
            {
                if (j - pChnAttr->IndexHandle->u32CurPlayIdx > 3)
                {
                    break;
                }
                pstPidInfo = &pChnAttr->IndexHandle->pstPidCtrl->stPidInfo[j];
        ...
        HI_U64 u64Offset)
{
    ...
    {
        ssReadCnt = PVR_PREAD64(pBuf, u32Size1, pstChn->s32DataFile, u64Offset1);
        if(-1 == ssReadCnt)
        {
            HI_ERR_PVR("read data(%u) failed!\n",u32Size1);
            return (HI_S32)ssReadCnt;
        }
        ssTotalRead = ssReadCnt;
        pTempBuf = (HI_CHAR*)pBuf + (HI_U32)ssTotalRead;
        ssReadCnt = PVR_PREAD64((HI_U8*)pTempBuf, u32Size2, pstChn->s32DataFile, u64Offset2);
        if(-1 == ssReadCnt)
        {
            HI_ERR_PVR("read data(%u) failed!\n",u32Size2);
                                     ...
                                     HI_U32 u32HeadToAdd)
{
    ...
    s32Cached = PVR_TS_CACHE_BUFFER_ReadFromBuffer(&pstExtraBufferInfo->stTsCacheBufInfo, u64ReadGlobalOffset, u32BytesSend, pBuf);
    if (0 > s32Cached)
    {
...
STATIC INLINE HI_S32 PVRPlayCheckUserCfg(HI_UNF_PVR_PLAY_ATTR_S* pUserCfg, HI_HANDLE hAvplay, HI_HANDLE hTsBuffer)
{
    HI_S32 ret;
    ...
    memset(szFileName, 0x00, PATH_MAX * sizeof(HI_CHAR));
    ...
    if (!((pUserCfg->u32FileNameLen > 0 && pUserCfg->u32FileNameLen < PVR_MAX_FILENAME_LEN)
    ...
    HI_WARN_PVR("PATH_MAX: %d\n", PATH_MAX);
    if(HI_NULL == realpath(pUserCfg->szFileName, szFileName))
    {
        HI_ERR_PVR("get real fileName failed! szFileName: %s\n", pUserCfg->szFileName);
        return HI_ERR_PVR_FILE_INVALID_FNAME;
    }
    if(strlen(szFileName) >= PVR_MAX_FILENAME_LEN)
    {
        HI_ERR_PVR("The Absolute fileName is too long, FileNameLen: %u ,MaxFileNameLen: %u\n", strlen(szFileName), PVR_MAX_FILENAME_LEN);
        return HI_ERR_PVR_FILE_INVALID_FNAME;
    }
    memset(pUserCfg->szFileName, 0x00, PVR_MAX_FILENAME_LEN * sizeof(HI_CHAR));
    snprintf(pUserCfg->szFileName, PVR_MAX_FILENAME_LEN - 1,"%s", szFileName);
...
STATIC INLINE HI_VOID PVRPlaySetTsHead(HI_U8* pTsHead, HI_U32 dataStartPos)
{
    ...
    /*invalid ts header, this is impossible*/
    if (0x47 != pTsHead[0])
    {
...
STATIC INLINE HI_VOID PVRPlayAddTsEnd(HI_U8* pBufAddr, HI_U32 dataEnd,  HI_U32 u32EndToAdd)
{
...
#else
    pLastTsHead = pBufAddr + dataEnd + u32EndToAdd - PVR_PACKET_LEN_188;
#if 0
    ...
    RVRPlaySetAdptFiled(pLastTsHead,
                        ((u32EndToAdd + PVR_TS_HEAD_SIZE) == PVR_PACKET_LEN_188) ?
    ...
    {
        if (u32EndToAdd + PVR_TS_HEAD_SIZE + PVR_TS_MIN_PD_SIZE > PVR_PACKET_LEN_188)
        {
            u32EndToAdd = PVR_PACKET_LEN_188 - (PVR_TS_HEAD_SIZE + PVR_TS_MIN_PD_SIZE);
        }

        dataInLastTs = PVR_PACKET_LEN_188 - (u32EndToAdd + PVR_TS_HEAD_SIZE + PVR_TS_MIN_PD_SIZE);
    ...
    {
        if (u32EndToAdd + PVR_TS_HEAD_SIZE > PVR_PACKET_LEN_188)
        {
            u32EndToAdd = PVR_PACKET_LEN_188 - PVR_TS_HEAD_SIZE;
        }

        dataInLastTs = PVR_PACKET_LEN_188 - (u32EndToAdd + PVR_TS_HEAD_SIZE);
...
STATIC HI_S32 PVRPlayProcCipherDecrypt(PVR_PLAY_CHN_S* pChnAttr, HI_U32 u32SrcPhyAddr, HI_U32 u32DestPhyAddr, HI_U32 u32ByteLength)
{
    ...
    /*use signed number to avoid minus value*/
    while (0 < (HI_S32)TotalLen)
    {
        ...
        i++;
#if 0   /*PVRPlaySendAframe has forbidden to send frame which larger than 10M*/
        if (i > 20)
        {
                                     ...
                                     HI_U32 u32BytesSend)
{
    ...
    HI_UNF_PVR_BUF_S stSrcBufInfo;
    HI_UNF_PVR_BUF_S stDstBufInfo;

    memset(&stStartFrame, 0, sizeof(stStartFrame));
    memset(&stEndFrame, 0, sizeof(stEndFrame));
    memset(&stSrcBufInfo, 0x00, sizeof(stSrcBufInfo));
    memset(&stDstBufInfo, 0x00, sizeof(stDstBufInfo));
    ...
    stSrcBufInfo.pu8VirAddr = pDemuxBuf->pu8Data;
    stSrcBufInfo.u32PhyAddr = u32PhyAddr;
    stSrcBufInfo.u32Len = u32BytesSend;
    stDstBufInfo.pu8VirAddr = pDemuxBuf->pu8Data;
    stDstBufInfo.u32PhyAddr = u32PhyAddr;
    stDstBufInfo.u32Len = u32BytesSend;
#ifdef HI_TEE_PVR_SUPPORT
#if defined(CHIP_TYPE_hi3798mv200) || defined(CHIP_TYPE_hi3798mv300) || defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3716mv450)
    if ((HI_TRUE == pChnAttr->bSecTsBuffer) && (HI_UNF_PVR_SECURE_MODE_TEE == pChnAttr->stUserCfg.enSecureMode))
    {
        ...
        {
            s32Ret = pChnAttr->readCallBack(&stDataAttr, pChnAttr->stReeBuf.pUserAddr,
                                            pChnAttr->stReeBuf.u32SmmuAddr,
                                            (HI_U32)u64ReadOffset, u32BytesSend);
            if (HI_SUCCESS != s32Ret)
            {
        ...
        stSrcBufInfo.pu8VirAddr = pChnAttr->stReeBuf.pUserAddr;
        stSrcBufInfo.u32PhyAddr = pChnAttr->stReeBuf.u32SmmuAddr;
    }
#endif
#endif
    ...
    {
        /*for REE and some tee pvr,src and dst is the same buffer, other tee pvr will use different buffers*/
        s32Ret = pChnAttr->stPlayCallback.pfnCallback(&stDataAttr, &stDstBufInfo,
                 &stSrcBufInfo, (HI_U32)u64ReadOffset,
...
static HI_VOID PVRPlaySetTsHeadByTsPKSize(PVR_PLAY_CHN_S* pChnAttr, HI_U8* pTsHead, HI_U32 dataStartPos)
{
    if (PVR_PACKET_LEN_188 == PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle))
    {
        PVRPlaySetTsHead(pTsHead, dataStartPos);
    }
    if (PVR_PACKET_LEN_192 == PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle))
    {
        PVRPlaySetTsHead(pTsHead + 4, dataStartPos - 4);
    }
...
STATIC INLINE HI_U32 PVRDealAppendPointData(PVR_PLAY_CHN_S *pChnAttr, HI_U8 *pu8Data, HI_U32 u32BytesSend, HI_U64 u64GlobalOffset,
                                            HI_U32 u32CipherHead, HI_U32 u32CipherEnd, HI_U32 u32HeadToAdd, HI_U32 u32EndToAdd)
{
    HI_S32 ret;
    HI_U32 u32BytesRealSend = 0;
    HI_U32 u32CurFrameSize = 0;
    HI_U32 u32CurFrameNum = 0;
    HI_BOOL bIsBoundary = HI_FALSE;
    PVR_INDEX_ENTRY_NEW_S stPrevFrame;
    PVR_INDEX_ENTRY_NEW_S stNextFrame;
    PVR_INDEX_HANDLE IndexHandle;
    HI_U64 u64FrameGlobalOffset = 0;
    HI_U32 u32FrameSize = 0;

    IndexHandle = pChnAttr->IndexHandle;
    memset(&stPrevFrame, 0x00, sizeof(stPrevFrame));
    memset(&stNextFrame, 0x00, sizeof(stNextFrame));
    u32BytesRealSend = u32BytesSend - u32CipherHead - u32CipherEnd;
    u32CurFrameSize = u32BytesSend - u32CipherHead - u32CipherEnd - u32HeadToAdd - u32EndToAdd;
    u32CurFrameNum = IndexHandle->u32ReadFrame - 1;

    ret = PVR_Index_GetFrameByNum(IndexHandle, &stPrevFrame, u32CurFrameNum - 1);
    PVR_Index_GetIndexInfo(IndexHandle, &stPrevFrame, PVR_INDEX_GLOBAL_OFFSET, &u64FrameGlobalOffset);
    PVR_Index_GetIndexInfo(IndexHandle, &stPrevFrame, PVR_INDEX_FRAME_SIZE, &u32FrameSize);
    if ((HI_SUCCESS == ret) && ((u64FrameGlobalOffset + u32FrameSize) != u64GlobalOffset))
    {
        PVRPlaySetTsHeadByTsPKSize(pChnAttr, pu8Data + u32CipherHead, u32HeadToAdd);
        HI_WARN_PVR("boundary FrameNum = %lu\n",u32CurFrameNum);
        bIsBoundary = HI_TRUE;
    }
    else
    {
        ret = PVR_Index_GetFrameByNum(IndexHandle, &stNextFrame, u32CurFrameNum + 1);
        if ((HI_SUCCESS == ret) && ((u64GlobalOffset + u32CurFrameSize) != u64FrameGlobalOffset))
        {
            PVRPlayAddTsEnd(pu8Data, u32BytesSend - u32CipherEnd - u32EndToAdd, u32EndToAdd);
            HI_WARN_PVR("boundary FrameNum = %lu\n",u32CurFrameNum);
            bIsBoundary = HI_TRUE;
        }
    }

    if (HI_FALSE == bIsBoundary)
    {
        if (u32EndToAdd != 0)
        {
            u32BytesRealSend -= PVR_INDEX_GET_TSPACKET_SIZE(IndexHandle);
        }
    }

    return u32BytesRealSend;
}
        ...
        HI_U32 u32HeadToAdd, HI_U32 u32EndToAdd)
{
    ...
    pDataBuf = stDemuxBuf.pu8Data;
#ifdef HI_TEE_PVR_SUPPORT
#if defined(CHIP_TYPE_hi3798mv200) || defined(CHIP_TYPE_hi3798mv300) || defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3716mv450)
    if ((HI_TRUE == pChnAttr->bSecTsBuffer) && (HI_UNF_PVR_SECURE_MODE_TEE == pChnAttr->stUserCfg.enSecureMode))
    {
        pDataBuf = pChnAttr->stReeBuf.pUserAddr;
    }
#endif
#endif
    /* read ts to buffer */
    u32ReadedBytes = PVRPlayReadData(pChnAttr, pDataBuf, u32BytesSend, pChnAttr->s32DataFile,
                                    u64ReadOffset, u64GlobalOffset, u32CipherHead, u32HeadToAdd);
...
#endif
        {
            PVRPlaySetTsHeadByTsPKSize(pChnAttr, stDemuxBuf.pu8Data + u32CipherHead, u32HeadToAdd);
    ...
    else /* both normal play and slow play, send all stream,  no longer patch stream, just depart it at PVR_TS_LEN size times */
    {
        ...
        if ((HI_TRUE == pChnAttr->bSecTsBuffer) && (HI_UNF_PVR_SECURE_MODE_TEE == pChnAttr->stUserCfg.enSecureMode))
        {
            if (u32EndToAdd != 0)
            {
                u32BytesRealSend -= PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle);
            }
        }
        else
#endif
#endif
        {
            u32BytesRealSend = PVRDealAppendPointData(pChnAttr, stDemuxBuf.pu8Data, u32BytesSend, u64GlobalOffset, u32CipherHead, u32CipherEnd, u32HeadToAdd, u32EndToAdd);
        }
    }
    pChnAttr->u64SendFrameNum++;
...
STATIC INLINE HI_S32 PVRPlaySendData(PVR_PLAY_CHN_S* pChnAttr, HI_U64 u64OffSet, HI_U32 u32BytesToSend, HI_U64 u64GlobalOffset)
{
    ...
    HI_U32 u32TsPKSize = PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle);

    u32HeadToAdd = (HI_U32)(u64GlobalOffset % u32TsPKSize);
    u32EndToAdd = u32TsPKSize - (HI_U32)((u64GlobalOffset + u32BytesToSend) % u32TsPKSize);
    if (u32TsPKSize <= u32EndToAdd)
    {
        u32EndToAdd = u32EndToAdd - u32TsPKSize;
    }
    ...
    if (184 <= u32EndToAdd)
    {
        ...
        u32BytesToSend = u32BytesToSend - (u32TsPKSize - u32EndToAdd);
        u32EndToAdd = 0;
    }
    u32CipherHead = (HI_U32)((u64GlobalOffset - u32HeadToAdd)  % PVR_CIPHER_PKG_LEN);

#if defined(HI_ADVCA_TYPE_VERIMATRIX_ULTRA) || defined(HI_ADVCA_TYPE_VERIMATRIX_ADVANCE)
    /*report more than one block in case IV is wrong*/
    if (u64GlobalOffset > u32HeadToAdd + PVR_CIPHER_PKG_LEN + u32CipherHead)
    {
        u32CipherHead += PVR_CIPHER_PKG_LEN;
    }
#endif
    u32CipherEnd = PVR_CIPHER_PKG_LEN - (HI_U32)((u64GlobalOffset + u32BytesToSend + u32EndToAdd) % PVR_CIPHER_PKG_LEN);
    if (PVR_CIPHER_PKG_LEN <= u32CipherEnd)
    {
        u32CipherEnd -= PVR_CIPHER_PKG_LEN;
    }
        ...
        HI_U32 u32Size)
{
    ...
    HI_U32 u32TsStartAddr = PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle) - PVR_PACKET_LEN_188;
    ...
    pu8Buffer[u32TsStartAddr] = 0x47;
    pu8Buffer[u32TsStartAddr + 1] = (HI_U8)(((u32Pid & 0x1f00) >> 8) | 0x40);
    pu8Buffer[u32TsStartAddr + 2] = (HI_U8)(u32Pid & 0xff);
    pu8Buffer[u32TsStartAddr + 3] = 0x10;
    ...
    *pu32DisplayTime = u32DisPlayTime;
    ...
    memcpy((HI_VOID*)(pu8Buffer + 4 + u32TsStartAddr), (HI_VOID*)au8PesPacket, sizeof(au8PesPacket));
...
{
    HI_S32 ret;
    HI_U32 u32Bytes2Send = PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle);
    HI_U32 u32PhyAddr;
    ...
    if ((HI_TRUE == pChnAttr->bSecTsBuffer) && (HI_UNF_PVR_SECURE_MODE_TEE == pChnAttr->stUserCfg.enSecureMode))
    {
        ...
        HI_PVR_TeeCopyDataFromRee(pChnAttr->hTsBuffer, &(pChnAttr->stReeBuf), u32PhyAddr, stStreamBuf.u32Size);
    }
    else
#endif
#endif
...
*****************************************************************************/
STATIC INLINE HI_VOID PVRPlayCheckError(PVR_PLAY_CHN_S*  pChnAttr,  HI_S32 ret)
{
    if (HI_SUCCESS == ret)
    {
        return;
    ...
    switch (ret)
    {
        case HI_ERR_PVR_FILE_TILL_END:
            ...
            pChnAttr->bSwitchNormalPlay = HI_TRUE;
            break;
...
STATIC HI_VOID PVRPlaySeektoStartFrame(PVR_PLAY_CHN_S* pChnAttr)
{
    HI_U32 u32ReadFrm;
    HI_S32 s32Ret = HI_SUCCESS;
            ...
            &&(PVR_INDEX_PAUSE_INVALID_OFFSET != pChnAttr->IndexHandle->u64PauseOffset)))
    {
        ...
        if (HI_SUCCESS == s32Ret)
        {
            ...
            pChnAttr->u32PlaySyncTime = u32DisplayTimeMs;
        }
    }
    else
    {
...
STATIC INLINE HI_BOOL PVRPlayIsTsSaved(PVR_PLAY_CHN_S* pChnAttr, PVR_INDEX_ENTRY_NEW_S* pFrameToPlay)
{
    ...
    if (pChnAttr->IndexHandle->bIsRec)
    {
        ...
        u64CurFramPosition = u64GlobalOffset + u32FrameSize + 2 * (PVR_CIPHER_PKG_LEN + PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle)) + PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle);
        if (pChnAttr->IndexHandle->u64FileSizeGlobal >= u64CurFramPosition)
        {
            return HI_TRUE;
        }
#ifdef HI_PVR_EXTRA_BUF_SUPPORT
        else if (0 != PVR_TS_CACHE_BUFFER_CheckCached(&pstExtraBufferInfo->stTsCacheBufInfo, u64CurFramPosition))
        {
            return HI_TRUE;
        }
#endif
...
STATIC INLINE HI_VOID PVRPlayAnalysisHevcStream(PVR_PLAY_CHN_S* pChnAttr)
{
    ...
    /**CNComment:HEVC格式下以下值都不会被使用,此处BFrmRefFlag设置为1为解决DTS2018030206571问题*/
...
STATIC PVR_PLAY_CHN_S* PVRPlayGetChnAttrByName(const HI_CHAR* pFileName)
{
    HI_U32 i = 0;
...
STATIC HI_S32 PVRPlayProcessEos(HI_HANDLE hAvplay, HI_UNF_AVPLAY_EVENT_E enEvent, HI_VOID* pPara)
{
    HI_U32 i = 0;
...
STATIC HI_S32 PVRPlayProcessFirstFrameDisplay(HI_HANDLE hAvplay, HI_UNF_AVPLAY_EVENT_E enEvent, HI_VOID* pPara)
{
    HI_U32 i = 0;
    PVR_PLAY_CHN_S* pChnAttr = (PVR_PLAY_CHN_S*)HI_NULL;
...
STATIC HI_UNF_SYNC_PROC_E PVRPlaySyncPlayTimeFF(PVR_PLAY_CHN_S* pChnAttr, HI_U32 u32CurPlayTimeMs, HI_U32 u32Pts)
{
    HI_UNF_SYNC_PROC_E enSyncResult = HI_UNF_SYNC_PROC_PLAY;
    HI_U32 u32ReasonableDelValue = 1000;

    if (pChnAttr->u32PlaySyncTime + u32ReasonableDelValue < u32CurPlayTimeMs)
    {
        /* 快进播放过快 *repeat*/
        enSyncResult = HI_UNF_SYNC_PROC_REPEAT;
        pChnAttr->u32RepeatFrameNum++;
        pChnAttr->u32DiscardFrameNum = 0;
        if (pChnAttr->u32RepeatFrameNum > 10)
        {
            enSyncResult = HI_UNF_SYNC_PROC_PLAY;
            pChnAttr->u32RepeatFrameNum = 0;
        }

        HI_INFO_PVR("u32PlaySyncTime: %u, CurAvplayTime: %u, PTS: %u, u32RepeatNumFF: %u\n",
                        pChnAttr->u32PlaySyncTime, u32CurPlayTimeMs, u32Pts, pChnAttr->u32RepeatFrameNum);
    }
    else if (pChnAttr->u32PlaySyncTime > u32CurPlayTimeMs + u32ReasonableDelValue)
    {
        /* 快进播放过慢*discard*/
        enSyncResult = HI_UNF_SYNC_PROC_DISCARD;
        pChnAttr->u32DiscardFrameNum++;
        pChnAttr->u32RepeatFrameNum = 0;

        if (pChnAttr->u32DiscardFrameNum > 10)
        {
            enSyncResult = HI_UNF_SYNC_PROC_PLAY;
            pChnAttr->u32DiscardFrameNum = 0;
        }

        HI_INFO_PVR("u32PlaySyncTime: %u, CurAvplayTime: %u, PTS: %u, u32DiscardNumFF: %u\n",
                        pChnAttr->u32PlaySyncTime, u32CurPlayTimeMs, u32Pts, pChnAttr->u32DiscardFrameNum);
    }

    return enSyncResult;
}

STATIC HI_U32 PVRPlaySyncPlayTimeFB(PVR_PLAY_CHN_S* pChnAttr, HI_U32 u32CurPlayTimeMs, HI_U32 u32Pts)
{
    HI_UNF_SYNC_PROC_E enSyncResult = HI_UNF_SYNC_PROC_PLAY;
    HI_U32 u32ReasonableDelValue = 1000;
    ...
    if (pChnAttr->u32PlaySyncTime + u32ReasonableDelValue < u32CurPlayTimeMs)
    {
        ...
        if (pChnAttr->u32DiscardFrameNum > 7)
        {
            enSyncResult = HI_UNF_SYNC_PROC_PLAY;
            pChnAttr->u32DiscardFrameNum = 0;
        }

        HI_INFO_PVR("u32PlaySyncTime: %u, CurAvplayTime: %u, PTS: %u, u32DiscardNumFB: %u\n",
                        pChnAttr->u32PlaySyncTime, u32CurPlayTimeMs, u32Pts, pChnAttr->u32DiscardFrameNum);
    }
    else if (pChnAttr->u32PlaySyncTime > u32CurPlayTimeMs + u32ReasonableDelValue)
    {
        ...
        pChnAttr->u32RepeatFrameNum++;
        pChnAttr->u32DiscardFrameNum = 0;
        if (pChnAttr->u32RepeatFrameNum > 5)
        {
            enSyncResult = HI_UNF_SYNC_PROC_PLAY;
            pChnAttr->u32RepeatFrameNum = 0;
        }

        HI_INFO_PVR("u32PlaySyncTime: %u, CurAvplayTime: %u, PTS: %u, u32RepeatNumFB: %u\n",
                        pChnAttr->u32PlaySyncTime, u32CurPlayTimeMs, u32Pts, pChnAttr->u32RepeatFrameNum);
    }
...
STATIC HI_S32 PVRPlaySyncPlayTime(HI_HANDLE hAvplay, HI_UNF_AVPLAY_EVENT_E enEvent, HI_VOID* pPara)
{
    HI_U32 i = 0;
    HI_U32 u32SpeedRate = 0;
    HI_U32 u32EndFrameDisPlayTime = 0;
    HI_UNF_SYNC_VID_TPLAYTIME_S *pstSyncVidInfo = (HI_UNF_SYNC_VID_TPLAYTIME_S *)pPara;
    PVR_PLAY_CHN_S* pChnAttr = (PVR_PLAY_CHN_S*)HI_NULL;
    ...
    if (PVR_PLAY_MAX_CHN_NUM == i)
    {
        HI_WARN_PVR("invalid play channel!\n");
        return HI_SUCCESS;
    }
    ...
    PVR_Index_GetIndexInfo(pChnAttr->IndexHandle, &pChnAttr->IndexHandle->stEndFrame, PVR_INDEX_DISP_TIME, &u32EndFrameDisPlayTime);
    u32SpeedRate = abs(pChnAttr->enSpeed) / HI_UNF_PVR_PLAY_SPEED_NORMAL;

    /*进度差超过一定量的时间，且时间余量足够进行帧率调整则不进行此同步，
            通过原帧率调控机制去调控*/    
    if (((u32SpeedRate >= 32) && (abs(pChnAttr->u32PlaySyncTime - pstSyncVidInfo->u32PrivDispTime) > 32*1000))
            || ((u32SpeedRate <= 16) && (abs(pChnAttr->u32PlaySyncTime - pstSyncVidInfo->u32PrivDispTime) > 16*1000)))
    {
        HI_WARN_PVR("cur sync abnormal,stop sync! u32PlaySyncTime: %u, CurAvplayTime: %u\n",
            pChnAttr->u32PlaySyncTime, pstSyncVidInfo->u32PrivDispTime);

        if (abs(u32EndFrameDisPlayTime - pstSyncVidInfo->u32PrivDispTime) > u32SpeedRate * 20 * 1000 )
        {
            return HI_SUCCESS;
        }
    }

    /*如果播放时间在1s 以内，不进行同步调整*/
    if (abs(pChnAttr->u32PlaySyncTime - pstSyncVidInfo->u32PrivDispTime) < 1 * 1000)
    {
        pChnAttr->u32DiscardFrameNum = 0;
        pChnAttr->u32RepeatFrameNum = 0;
        return HI_SUCCESS;
    ...
    if (HI_UNF_PVR_PLAY_STATE_FF == pChnAttr->enState)
    {
        pstSyncVidInfo->enSyncProc = PVRPlaySyncPlayTimeFF(pChnAttr,pstSyncVidInfo->u32PrivDispTime, pstSyncVidInfo->u32Pts);
    }
    else if (HI_UNF_PVR_PLAY_STATE_FB == pChnAttr->enState)
    {
        pstSyncVidInfo->enSyncProc = PVRPlaySyncPlayTimeFB(pChnAttr,pstSyncVidInfo->u32PrivDispTime, pstSyncVidInfo->u32Pts);
    }

    return HI_SUCCESS;
}

static HI_S32 PVRPlaySetFrmRate(HI_HANDLE hAvplay, HI_UNF_AVPLAY_FRMRATE_TYPE_E enFrmRateType, HI_U32 u32fpsInteger, HI_U32 u32fpsDecimal)
{
    HI_S32 s32Ret = HI_SUCCESS;

    if (u32fpsDecimal >= 1000)
    {
        HI_ERR_PVR("invalid frame rate. Integer: %u, Decimal: %u\n", u32fpsInteger, u32fpsDecimal);
        return HI_ERR_PVR_INVALID_PARA;
    }
    if (u32fpsInteger >= 60)
    {
        HI_WARN_PVR("Doesn't support such a large frame rate. Integer: %u, Decimal: %u\n", u32fpsInteger, u32fpsDecimal);
        u32fpsInteger = 60;
        u32fpsDecimal = 0;
    }
...
STATIC HI_S32 PVRPlaySetFrmRateAutoDetect(PVR_PLAY_CHN_S*  pChnAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;
    if ( pChnAttr->bRecordedVideoExist == HI_TRUE)
    {
...
STATIC HI_S32 PVRPlaySetDecoderSpeed(PVR_PLAY_CHN_S*  pChnAttr, HI_UNF_PVR_PLAY_SPEED_E enPlaySpeed)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
STATIC HI_S32 PVRPlaySetPlayerSpeed(PVR_PLAY_CHN_S*  pChnAttr, HI_UNF_PVR_PLAY_SPEED_E enPlaySpeed)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
STATIC HI_VOID PVRPlayProcHevc(PVR_PLAY_CHN_S*  pChnAttr, HI_UNF_PVR_PLAY_STATE_E enPlayStateNew)
{
...
*****************************************************************************/
STATIC HI_S32 PVRPlayAudSyncCtrl(PVR_PLAY_CHN_S*  pChnAttr, HI_BOOL bCtrlFlag)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
STATIC HI_S32 PVRPlaySetPauseMode(PVR_PLAY_CHN_S*  pChnAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
STATIC HI_S32 PVRPlaySetNormalMode(PVR_PLAY_CHN_S*  pChnAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_UNF_PVR_PLAY_STATE_E enCurPlayState = HI_UNF_PVR_PLAY_STATE_BUTT;
...
STATIC HI_S32 PVRPlaySetTrickMode(PVR_PLAY_CHN_S*  pChnAttr, HI_UNF_PVR_PLAY_SPEED_E enPlaySpeed)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    /*如果播放状态发生了改变,则将repeatNum 和discardNum 重置*/
    pChnAttr->u32RepeatFrameNum = 0;
    pChnAttr->u32DiscardFrameNum = 0;
...
STATIC HI_S32 PVRPlaySetStepMode(PVR_PLAY_CHN_S*  pChnAttr, HI_S32 s32Direction)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_UNF_PVR_PLAY_STATE_E enCurPlayState = HI_UNF_PVR_PLAY_STATE_BUTT;
...
STATIC HI_S32 PVRPlayControlNormalSend(PVR_PLAY_CHN_S* pChnAttr, PVR_INDEX_ENTRY_NEW_S* pstIdxEntry)
{
    ...
    if (pChnAttr->bPlayingTsNoIdx || !pChnAttr->stUserCfg.bIsClearStream)
    {
        ...
        {
            u32FrameSize = PVR_FIFO_WRITE_BLOCK_SIZE(PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle));
...
STATIC HI_S32 PVRPlayControl(PVR_PLAY_CHN_S* pChnAttr, PVR_INDEX_ENTRY_NEW_S* pstIdxEntry)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32FrameSize = 0;
    ...
    switch (pChnAttr->enState)
    {
        ...
        case HI_UNF_PVR_PLAY_STATE_FB:
        {
            ...
            {
                u32FrameSize = PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle);
                PVR_Index_SetIndexInfo(pChnAttr->IndexHandle, pstIdxEntry, PVR_INDEX_OFFSET, &(pChnAttr->u64CurReadPos));
                PVR_Index_SetIndexInfo(pChnAttr->IndexHandle, pstIdxEntry, PVR_INDEX_FRAME_SIZE, &u32FrameSize);
            }
            else
...
*****************************************************************************/
static HI_S32 PVRPlaySendAframe(PVR_PLAY_CHN_S* pChnAttr, const PVR_INDEX_ENTRY_NEW_S* pstFrame)
{
    ...
    HI_U32 u32NeedDmxBufferSize = 0;

    HI_ASSERT_RET(NULL != pstFrame);
    HI_ASSERT_RET(NULL != pChnAttr);

    u32TsPKSize = PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle);
    ...
    if ((PVR_INDEX_is_Unknownframe(pChnAttr->IndexHandle, (PVR_INDEX_ENTRY_NEW_S*)pstFrame))
        && ((HI_UNF_PVR_PLAY_STATE_FF == pChnAttr->enState) || (HI_UNF_PVR_PLAY_STATE_FB == pChnAttr->enState)))
    {
        HI_ERR_PVR("invalid frame type, we don't decode them!\n");
        return HI_SUCCESS;
    }
    ...
    u32NeedDmxBufferSize = 2 * (PVR_CIPHER_PKG_LEN + u32TsPKSize) + u32FrameSize + u32TsPKSize;

#if defined(HI_ADVCA_TYPE_VERIMATRIX_ULTRA) || defined(HI_ADVCA_TYPE_VERIMATRIX_ADVANCE)
    u32NeedDmxBufferSize += PVR_CIPHER_PKG_LEN;
#endif

    if (u32DmxFreeTsBufSize < u32NeedDmxBufferSize)
    {
...
STATIC HI_S32 PVRPlaySendStream(PVR_PLAY_CHN_S* pChnAttr, PVR_INDEX_ENTRY_NEW_S* pstPlayIdxEntry)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32DmxFreeTsBufSize = 0;
    HI_UNF_DMX_TSBUF_STATUS_S stDmxTsBufState;
...
#ifdef PVR_PID_CHANGE_SUPPORT
            if (HI_ERR_PVR_PLAY_CHANGE_PID == s32Ret)
            {
                memset(&stCurPlayFrame, 0x00, sizeof(stCurPlayFrame));
                s32Ret = PVR_Index_GetFrameByNum(pChnAttr->IndexHandle, &stCurPlayFrame, pChnAttr->stFBAttr.u32EntryFrameNum);
                if (HI_SUCCESS != s32Ret)
                {
                    HI_ERR_PVR("get frame failed! frameNum: %d \n",pChnAttr->stFBAttr.u32EntryFrameNum);
                    return s32Ret;
                }

                PVR_Index_GetIndexInfo(pChnAttr->IndexHandle, &stCurPlayFrame,PVR_INDEX_DISP_TIME, (HI_VOID *)&u32DispTimeMs);
                if (HI_TRUE == PVR_Index_CheckPidListChange(pChnAttr->IndexHandle, pChnAttr->stFBAttr.u32EntryFrameNum, u32DispTimeMs, &u32NextIdx))
                {
                    HI_WARN_PVR("u32EntryFrameNum : %d Index->CurReadFrmNum: %d \n",pChnAttr->stFBAttr.u32EntryFrameNum, pChnAttr->IndexHandle->u32ReadFrame);
                    (HI_VOID)PVRPlayPidSwitchProcess(pChnAttr, u32NextIdx);
                }
...
#ifdef PVR_PID_CHANGE_SUPPORT
        u32DispTimeMs = 0;
        u32NextIdx = pChnAttr->IndexHandle->u32CurPlayIdx;

        PVR_Index_GetIndexInfo(pChnAttr->IndexHandle, pstPlayIdxEntry,PVR_INDEX_DISP_TIME, (HI_VOID *)&u32DispTimeMs);
        if (HI_TRUE == PVR_Index_CheckPidListChange(pChnAttr->IndexHandle, pChnAttr->IndexHandle->u32ReadFrame, u32DispTimeMs, &u32NextIdx))
        {
            (HI_VOID)PVRPlayPidSwitchProcess(pChnAttr, u32NextIdx);
        }
#endif
...
STATIC HI_VOID PVRPlayCheck(PVR_PLAY_CHN_S*  pChnAttr, HI_S32 s32RetCode)
{
...
STATIC HI_S32 PVRPlayProcDecodeAbility(PVR_PLAY_CHN_S* pChnAttr)
{
    ...
    PVR_DECODEABILITY_INFO_S stPvrDecodeAbilityInfo;

    memset(&stPvrDecodeAbilityInfo, 0x00, sizeof(stPvrDecodeAbilityInfo));
    if (pChnAttr->u32UseDecodeAbility == 0)
    {
            ...
            (stStreamInfo.stVidStreamInfo.u32Width != 0))
        {
            ...
            else
            {
                ...
                stPvrDecodeAbilityInfo.u32ChnId = pChnAttr->u32chnID;
                stPvrDecodeAbilityInfo.u32DeCodeAbilityValue = u32AcqDeAbility;
                s32Ret = ioctl(g_s32PvrFd, CMD_PVR_ACQUIRE_DECODE_ABILITY, &stPvrDecodeAbilityInfo);
                if (s32Ret != HI_SUCCESS)
                {
                ...
                {
                    pChnAttr->u32UseDecodeAbility = stPvrDecodeAbilityInfo.u32DeCodeAbilityValue;
                }
            }
        }
...
STATIC HI_S32 PVRPlaySmoothInit(PVR_PLAY_CHN_S** ppChnAttr)
{
...
STATIC HI_VOID PVRPlayCreateTimer(PVR_PLAY_CHN_S** pChnAttr)
{
    ...
    if (HI_SUCCESS == PVR_Index_GetFrameByNum((*pChnAttr)->IndexHandle, &stStartEntry, (*pChnAttr)->IndexHandle->u32ReadFrame))
    {
        ...
        (*pChnAttr)->u32PlaySyncTime = u32DispPlayTimeMs;
    }
    else
    {
        (*pChnAttr)->u32CurPlayTimeMs = 0;
...
STATIC HI_VOID PVRPlayDestroyTimer(HI_VOID)
{
    HI_U32 i = 0;
...
STATIC HI_VOID PVRPlayCheckToSwitchNormal(PVR_PLAY_CHN_S *pChnAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;

    if ((HI_TRUE == pChnAttr->bSwitchNormalPlay)
            && (HI_FALSE == pChnAttr->IndexHandle->bIsRec)
            && (HI_UNF_PVR_PLAY_STATE_FB != pChnAttr->enState))
    {
        return;
    }
    ...
    if ((HI_TRUE == pChnAttr->bSwitchNormalPlay)
            && (HI_UNF_PVR_TRICKPLAYTOEND_ACTION_PLAY == pChnAttr->stUserCfg.enTrickPlayToEndAction)
            && ((HI_UNF_PVR_PLAY_STATE_FF == pChnAttr->enState)
                ||(HI_UNF_PVR_PLAY_STATE_FB == pChnAttr->enState)))
    {
        HI_INFO_PVR("===SwitchNormalPlay occured,enPlayToEndAction = %d; enState = %d; bIsRec = %d \n",pChnAttr->stUserCfg.enTrickPlayToEndAction,
                pChnAttr->enState,
                pChnAttr->IndexHandle->bIsRec);
        s32Ret = PVRPlaySetNormalMode(pChnAttr);
        if(HI_SUCCESS != s32Ret)
        {
            HI_ERR_PVR("set normal play failed! ret=%#x\n",s32Ret);
        }
    }
}
...
STATIC void* PVRPlayMainRoutine(void* args)
{
    ...
    {
        PVR_LOCK(&(pChnAttr->stPlayChnMutex));
        PVRPlayCheckToSwitchNormal(pChnAttr);
        pChnAttr->bSwitchNormalPlay = HI_FALSE;
        ...
        if (((HI_UNF_PVR_PLAY_STATE_FB == pChnAttr->enState)
                || (HI_UNF_PVR_PLAY_STATE_FF == pChnAttr->enState))
                    && (HI_FALSE == pChnAttr->bTimeShiftStartFlg))
        {
            HI_PVR_SmoothPlayTimeCtrl(&(pChnAttr->stFBAttr));
        }
...
#ifdef PVR_PID_CHANGE_SUPPORT
/*CNcomment: 回放时PID发生改变，为保证少丢失数据，需要等待上个PID的数据消耗到一定水线时才切换，
                    在这个等待过程中会将通道属性bChangePID置为HI_TRUE，进行seek，pause，resume及快进快退，逐帧播放等操作时
                    返回HI_ERR_PVR_NOT_SUPPORT。支持stop通道*/
STATIC HI_S32 PVRPlayWaitCurrentDataConsume(PVR_PLAY_CHN_S *pChnAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WaitLastPidPlayTimeMs = 0;
    HI_U32 u32CurPlayFrmDisp = 0;
    HI_U32 u32CurPlayFrmPts = 0;
    HI_U32 u32LastVidPts = 0;
    HI_U32 u32WaitAvplaySyncTime = 0;
    HI_U32 u32SpeedRate = 0;
    HI_U32 u32TimeOutMinMs = 1 * 1000; //等待时间的最小基数
    ...
    {
        u32SpeedRate = HI_UNF_PVR_PLAY_SPEED_NORMAL / abs(pChnAttr->enSpeed);
    }
    ...
    memset(&stStatusInfo, 0x00, sizeof(stStatusInfo));
    s32Ret = HI_UNF_AVPLAY_GetStatusInfo(pChnAttr->hAvplay, &stStatusInfo);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("get HI_UNF_AVPLAY_GetStatusInfo failed, force change PID! \n");
        return HI_FAILURE;
    }

    u32LastVidPts = stStatusInfo.stSyncStatus.u32LastVidPts;
    u32WaitAvplaySyncTime = abs(u32LastVidPts - u32CurPlayFrmPts);
    HI_WARN_PVR("(WPlay,TimeOut, WatiAvplay),(%d, %d, %d)\n",u32WaitLastPidPlayTimeMs, u32TimeOutMinMs, u32WaitAvplaySyncTime);
    if ((u32WaitLastPidPlayTimeMs < u32TimeOutMinMs) || (u32WaitAvplaySyncTime <= u32TimeOutMinMs) || (u32LastVidPts == -1))
    {
        return HI_SUCCESS;
    }

    while (u32WaitLastPidPlayTimeMs > u32TimeOutMinMs)
    {
        if (HI_TRUE == pChnAttr->bPlayMainThreadStop)
        {
            HI_ERR_PVR("stop channel Already!\n");
            return HI_SUCCESS;
        }
        memset(&stStatusInfo, 0x00, sizeof(stStatusInfo));
        s32Ret = HI_UNF_AVPLAY_GetStatusInfo(pChnAttr->hAvplay, &stStatusInfo);
        ...
        u32LastVidPts = stStatusInfo.stSyncStatus.u32LastVidPts;
        u32WaitAvplaySyncTime = abs(u32LastVidPts - u32CurPlayFrmPts);
        if ((u32WaitAvplaySyncTime <= u32TimeOutMinMs) || (u32LastVidPts == -1))
        {
            return HI_SUCCESS;
        }

        PVR_USLEEP(1000);
        u32WaitLastPidPlayTimeMs--;
    }
...
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32IndexLastReadFrmNum = 0;
    HI_UNF_PVR_PLAY_SPEED_E enOldSpeed = HI_UNF_PVR_PLAY_SPEED_NORMAL;
    HI_UNF_PVR_PID_CHANGE_INFO_S stOldPidInfo;
    HI_UNF_PVR_PID_CHANGE_INFO_S stNewPidInfo;

    /*check if idx is valid in case SOF and EOF*/
    if ((0 > (HI_S32)u32NextIdx) || (pChnAttr->IndexHandle->pstPidCtrl->u32UsedNum <= u32NextIdx))
    {
        HI_ERR_PVR("Should play to start or end(%u, %u)\n", u32NextIdx, pChnAttr->IndexHandle->pstPidCtrl->u32UsedNum);
        return HI_FAILURE;
    }

    /*check all(most) of the sent ts data to be consumed*/
    pChnAttr->bChangePID = HI_TRUE;
    PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
    s32Ret = PVRPlayWaitCurrentDataConsume(pChnAttr);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("wait timeout,force switch the pid!\n");
    }
    PVR_LOCK(&(pChnAttr->stPlayChnMutex));
    pChnAttr->bChangePID = HI_FALSE;
    
    /*save the play state and speed, set the channel to normal play*/
    enOldSpeed = pChnAttr->enSpeed;
    u32IndexLastReadFrmNum = pChnAttr->IndexHandle->u32ReadFrame;

    s32Ret = PVRPlaySetNormalMode(pChnAttr);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("Switch normal failed, ret = 0x%08x\n", s32Ret);
    }

    /*prepare the pid info for the callback*/
    memset(&stOldPidInfo, 0x00, sizeof(stOldPidInfo));
    memset(&stNewPidInfo, 0x00, sizeof(stNewPidInfo));

    (HI_VOID)PVR_Index_ChangeThePlayPid(pChnAttr->IndexHandle, u32NextIdx, &stOldPidInfo, &stNewPidInfo);

    /*call the callback, and  the app shall call APIs of avplay to do the pid changed. callback shall not call APIs of PVR*/
    if (HI_NULL != pChnAttr->stPlayPidChangeCB.pfnPidChangeCB)
    {
        s32Ret = pChnAttr->stPlayPidChangeCB.pfnPidChangeCB(pChnAttr->u32chnID, &stOldPidInfo, &stNewPidInfo, pChnAttr->stPlayPidChangeCB.pArgs);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_PVR("Call pid change callback failed!\n");
            return s32Ret;
        }
    }
    else
    {
        HI_ERR_PVR("pid change callback is necessary When pid was changed! \n");
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("PVRPlaySmoothInit failed, ret = 0x%08x\n", s32Ret);
    }

    s32Ret = PVRPlaySetTrickMode(pChnAttr, enOldSpeed);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("PVRPlaySetTrickMode(%u) failed, ret = 0x%08x\n", enOldSpeed, s32Ret);
    }

    PVR_Index_SetCurReadFrameNum(pChnAttr->IndexHandle, u32IndexLastReadFrmNum);
    HI_INFO_PVR("Index->CurReadFrmNum: %d \n",pChnAttr->IndexHandle->u32ReadFrame);
    HI_INFO_PVR("Switch success! \n");
    return HI_SUCCESS;
}
#endif
...
void PVRPlaySyncTrickPlayTime(PVR_PLAY_CHN_S* pChnAttr)
{
    ...
    if (u32CurFrmTimeMs > pChnAttr->u32CurPlayTimeMs)
    {
        if ((u32CurFrmTimeMs - pChnAttr->u32CurPlayTimeMs) > 1000)
        {
            ...
            pChnAttr->u32PlaySyncTime= u32CurFrmTimeMs;
        }
    }
...
HI_S32 HI_PVR_PlayInit(HI_VOID)
{
    ...
    else
    {
        ...
        for (i = 0 ; i < PVR_PLAY_MAX_CHN_NUM; i++)
        {
            ...
            PVR_MUTEX_INIT(&(pChnAttr->stPlayChnMutex), ret);
            if (HI_SUCCESS != ret)
            {
                close(g_s32PvrFd);
                for (j = 0; j < i; j++)
            ...
            memset(&pChnAttr->stUserCfg, 0, sizeof(pChnAttr->stUserCfg));
            pChnAttr->bTimeShiftStartFlg = HI_FALSE;
            pChnAttr->stPlayCallback.pfnCallback = HI_NULL;
            pChnAttr->stPlayCallback.pUserData = HI_NULL;
            pChnAttr->stPlayPidChangeCB.pfnPidChangeCB = HI_NULL;
            pChnAttr->stPlayPidChangeCB.pArgs = HI_NULL;
            pChnAttr->bChangePID = HI_FALSE;
            memset(&pChnAttr->stOriginalDmxPortAttr, 0x00, sizeof(pChnAttr->stOriginalDmxPortAttr));
            PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
        }
...
#endif
        if(HI_SUCCESS != HI_PVR_SmoothRegisterPlayCallBack(PVRPlaySendAframe, PVRPlaySetFrmRate))
        {
            HI_ERR_PVR("register play callBack failed!\n");
            return HI_FAILURE;
        }
...
HI_S32 HI_PVR_PlayDeInit(HI_VOID)
{
    ...
    else
    {
        ...
        HI_PVR_SmoothUngisterPlayCallBack();
        PVRIntfDeInitEvent();
        return HI_SUCCESS;
    }
}
...
HI_S32 HI_PVR_PlayCreateChn(HI_U32* pChn, const HI_UNF_PVR_PLAY_ATTR_S* pAttr, HI_HANDLE hAvplay, HI_HANDLE hTsBuffer)
{
    ...
    memcpy(&stPlayAttr, pAttr, sizeof(HI_UNF_PVR_PLAY_ATTR_S));
    memset(&stDMXPortAttr, 0x00, sizeof(stDMXPortAttr));

    ret = PVRPlayCheckUserCfg(&stPlayAttr, hAvplay, hTsBuffer);
    if (HI_SUCCESS != ret)
    {
        return ret;
    }
...
#endif
#endif
    memcpy(&pChnAttr->stUserCfg, &stPlayAttr, sizeof(HI_UNF_PVR_PLAY_ATTR_S));
    ...
    */
    ...
    pChnAttr->IndexHandle = PVR_Index_CreatPlay(pChnAttr->u32chnID, &stPlayAttr, &pChnAttr->bPlayingTsNoIdx);
    ...
    /* open ts file */
    pChnAttr->s32DataFile = PVR_OPEN64(stPlayAttr.szFileName, PVR_FOPEN_MODE_DATA_READ);
    ...
    ret = HI_UNF_DMX_GetTSBufferPortId(hTsBuffer, &enDmxPortId);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("Get demux port id by tsBuffer failed! ret = %x\n", ret);
        goto ErrorExit;
    }

    ret = HI_UNF_DMX_GetTSPortAttr(enDmxPortId, &stDMXPortAttr);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("Get demux TsPort attribute failed! ret = %x\n", ret);
        goto ErrorExit;
    }

    /*save original demux port attribute*/
    memcpy(&pChnAttr->stOriginalDmxPortAttr, &stDMXPortAttr, sizeof(stDMXPortAttr));
    /*config Demux port attr*/
    stDMXPortAttr.enPortType = HI_UNF_DMX_PORT_TYPE_USER_DEFINED;
    stDMXPortAttr.u32UserDefLen1 = PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle);
    stDMXPortAttr.u32UserDefLen2 = PVR_INDEX_GET_TSPACKET_SIZE(pChnAttr->IndexHandle);
    ret = HI_UNF_DMX_SetTSPortAttr(enDmxPortId, &stDMXPortAttr);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("config Demux port attribute failed! ret = %x\n", ret);
        goto ErrorExit;
    }

    pChnAttr->u64TsFileSize = PVR_FILE_GetFileSize64(stPlayAttr.szFileName);
...
HI_S32 HI_PVR_PlayStartChn(HI_U32 u32ChnID) /* pause when end of file */
{
    ...
    memset(&pChnAttr->stLastStatus, 0, sizeof(pChnAttr->stLastStatus));
    ...
    s32Ret = HI_UNF_AVPLAY_RegisterEvent64(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_FIRST_FRAME_DISPLAYED, PVRPlayProcessFirstFrameDisplay);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("registe avplay NEW_VID_FRAME event fail!\n");
        (HI_VOID)HI_UNF_AVPLAY_UnRegisterEvent(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_EOS);
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
        return HI_FAILURE;
    }

    s32Ret = HI_UNF_AVPLAY_RegisterEvent64(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_VID_TPLAYTIME_SYNC, PVRPlaySyncPlayTime);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("registe avplay NEW_VID_FRAME event fail!\n");
        (HI_VOID)HI_UNF_AVPLAY_UnRegisterEvent(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_EOS);
        (HI_VOID)HI_UNF_AVPLAY_UnRegisterEvent(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_FIRST_FRAME_DISPLAYED);
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        ...
        (HI_VOID)HI_UNF_AVPLAY_UnRegisterEvent(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_EOS);
        (HI_VOID)HI_UNF_AVPLAY_UnRegisterEvent(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_FIRST_FRAME_DISPLAYED);
        (HI_VOID)HI_UNF_AVPLAY_UnRegisterEvent(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_VID_TPLAYTIME_SYNC);
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
        return HI_FAILURE;
    }
...
HI_S32 HI_PVR_PlayStopChn(HI_U32 u32Chn, const HI_UNF_AVPLAY_STOP_OPT_S* pstStopOpt)
{
    ...
    memset(&stAvplayStatus, 0, sizeof(stAvplayStatus));
    memset(&stPvrDecodeAbilityInfo, 0, sizeof(stPvrDecodeAbilityInfo));
    pChnAttr = &g_stPvrPlayChns[u32Chn];
    PVR_LOCK(&(pChnAttr->stPlayChnMutex));
    PVR_PLAY_CHECK_CHN_INIT_UNLOCK(pChnAttr);
    ...
    if (pChnAttr->bRecordedVideoExist == HI_TRUE)
    {
        if (HI_UNF_PVR_PLAY_STATE_PAUSE != pChnAttr->enState)
        {
            (HI_VOID)HI_UNF_AVPLAY_Resume(pChnAttr->hAvplay, HI_NULL);
        }
        ...
        stPvrDecodeAbilityInfo.u32ChnId = u32Chn;
        stPvrDecodeAbilityInfo.u32DeCodeAbilityValue = pChnAttr->u32UseDecodeAbility;
        ret = ioctl(g_s32PvrFd, CMD_PVR_RELEASE_DECODE_ABILITY, &stPvrDecodeAbilityInfo);
        if (ret != HI_SUCCESS)
        {
    ...
    ret = HI_UNF_AVPLAY_UnRegisterEvent(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_FIRST_FRAME_DISPLAYED);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("unregiste avplay NEW_VID_FRAME event fail! ret=%#x\n", ret);
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
        return HI_FAILURE;
    }

    ret = HI_UNF_AVPLAY_UnRegisterEvent(pChnAttr->hAvplay, HI_UNF_AVPLAY_EVENT_VID_TPLAYTIME_SYNC);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("unregiste avplay TPLAYTIME_SYNC event fail! ret=%#x\n", ret);
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
        return HI_FAILURE;
    }
    ...
    /*reset Demux port attr*/
    ret = HI_UNF_DMX_GetTSBufferPortId(pChnAttr->hTsBuffer, &enDmxPortId);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("Get demux port id by tsBuffer failed! ret = %x\n", ret);
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
    ...
    ret = HI_UNF_DMX_SetTSPortAttr(enDmxPortId, &pChnAttr->stOriginalDmxPortAttr);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("config Demux port attribute failed! ret = %x\n", ret);
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
...
HI_S32 HI_PVR_PlayPauseChn(HI_U32 u32Chn)
{
    ...
    PVR_PLAY_CHECK_CHN(u32Chn);
    pChnAttr = &g_stPvrPlayChns[u32Chn];
    PVR_PLAY_CHECK_CHN_PID_CHANGE(pChnAttr);
...
HI_S32 HI_PVR_PlayRegisterPidChangeCallBack(HI_U32 u32ChnID, PidChangeCallBack fCallback, HI_VOID *pArgs)
{
    ...
    pChnAttr->stPlayPidChangeCB.pfnPidChangeCB= fCallback;
    pChnAttr->stPlayPidChangeCB.pArgs= pArgs;
...
HI_S32 HI_PVR_PlayUnRegisterPlayCallBack(HI_U32 u32Chn, ExtendCallBack pfnPlayCallback)
{
    ...
    PVR_PLAY_CHECK_CHN(u32Chn);
    PVR_CHECK_POINTER(pfnPlayCallback);
...
HI_S32 HI_PVR_PlayUnRegisterPidChangeCallBack(HI_U32 u32ChnID, PidChangeCallBack fCallback)
{
    ...
    PVR_LOCK(&(pChnAttr->stPlayChnMutex));
    PVR_PLAY_CHECK_CHN_INIT_UNLOCK(pChnAttr);
    if (fCallback != pChnAttr->stPlayPidChangeCB.pfnPidChangeCB)
    {
        HI_ERR_PVR("The input callback hasn't been registered!\n");
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
        return HI_ERR_PVR_INVALID_PARA;
...
HI_S32 HI_PVR_PlayGetStatus(HI_U32 u32Chn, HI_UNF_PVR_PLAY_STATUS_S* pStatus)
{
    ...
    if (HI_FALSE == pChnAttr->bTimeShiftStartFlg)
    {
        pStatus->u32CurPlayTimeInMs = u32CurFrmTimeMs;
    }
...
{
    HI_S32 ret;
    HI_CHAR szRealFileName[PATH_MAX] = {0};
    ...
    {
        HI_ERR_PVR("invalid file name,file name length exceeds 256!\n");
        return HI_ERR_PVR_FILE_INVALID_FNAME;
    }

    if(HI_NULL == realpath(pFileName, szRealFileName))
    {
        HI_ERR_PVR("get real fileName failed! szFileName: %s\n", pFileName);
        return HI_ERR_PVR_FILE_INVALID_FNAME;
    }
    if(strlen(szRealFileName) >= PVR_MAX_FILENAME_LEN)
    {
        HI_ERR_PVR("The Absolute fileName is too long, FileNameLen: %u ,MaxFileNameLen: %u\n", strlen(szRealFileName), PVR_MAX_FILENAME_LEN);
        return HI_ERR_PVR_FILE_INVALID_FNAME;
    }

    if (PVRRecCheckFileRecording(szRealFileName))
    {
        pstRecChnAttr = PVRRecGetChnAttrByName(szRealFileName);

        if (pstRecChnAttr != NULL)
        {
            PVR_Index_FlushIdxWriteCache(pstRecChnAttr->IndexHandle);
            ret = PVR_Index_PlayGetFileAttrByFileName(szRealFileName, pstRecChnAttr->IndexHandle, pAttr);
        }
        else
        {
            ret = PVR_Index_PlayGetFileAttrByFileName(szRealFileName, HI_NULL, pAttr);
        }
    }
    else
    {
        pstPlayChnAttr = (PVR_PLAY_CHN_S*)PVRPlayGetChnAttrByName(szRealFileName);

        if ((PVR_PLAY_CHN_S*)NULL != pstPlayChnAttr)
        {
            ret = PVR_Index_PlayGetFileAttrByFileName(szRealFileName, pstPlayChnAttr->IndexHandle, pAttr);
        }
        else
        {
            ret = PVR_Index_PlayGetFileAttrByFileName(szRealFileName, HI_NULL, pAttr);
