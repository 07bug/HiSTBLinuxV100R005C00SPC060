...
#ifdef HI_PVR_EXTRA_BUF_SUPPORT
    #if defined(PVR_APPEND_MODE_SUPPORT) || defined(PVR_PID_CHANGE_SUPPORT)
/**//*CNcomment: 这个接口主要是在录制时打开extra buffer时使用，因为此时文件句柄的打开参数中有O_DIRECT参数，
                            此时需要保证write 参数的512对齐，该函数就是用来保证写文件时writePos 是512对齐的*/
...
static HI_S32 PVRRecSetRecCircurarToLinear(PVR_REC_CHN_S* pRecChn)
{
    ...
    /*回绕转非回绕时并未发生回绕，直接修改bRewind属性后返回*/
...
static HI_S32 PVRRecCheckPidChange(PVR_REC_CHN_S *pstChnAttr, HI_UNF_PVR_PID_CHANGE_INFO_S *pstPidInfo)
{
    ...
    /*变更PID时只支持录制状态为RUNNING 或PAUSE 状态*/
...
//解析上次续录的信息并更新到pstPidCtrl中，并判断是否满足续录
#if defined(PVR_APPEND_MODE_SUPPORT) && defined(PVR_PID_CHANGE_SUPPORT)
...
HI_S32 HI_PVR_RecStartChn(HI_U32 u32ChnID)
{
    ...
    #ifdef HI_PVR_EXTRA_BUF_SUPPORT
    /*当开启二级缓存时(O_DIRECT)需要判断续录点是否需要对齐,如果需要则将对齐后剩余的数据写入二级缓存中*/
...
#ifdef HI_PVR_EXTRA_BUF_SUPPORT
    /*当开启二级缓存时(O_DIRECT)需要判断变更续录点是否需要对齐,如果需要则将对齐后剩余的数据写入二级缓存中*/
