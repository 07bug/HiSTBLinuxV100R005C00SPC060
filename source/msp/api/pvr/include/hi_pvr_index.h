...
/*the max times of changing pid*//**CNcomment: pid最多的变更次数*/
...
/**CNcomment: 每次PID 变更的具体信息*/
typedef struct tagPVR_INDEX_PID_INFO_S
{
    HI_U32 u32StartFrame;                           /*该段PID中的起始帧号*/
    HI_U32 u32StartDispTime;                        /*该段PID中的起始时间*/
    HI_U32 u32EndFrame;                             /*该段PID中的结束帧号*/
    HI_U32 u32EndDispTime;                          /*该段PID中的结束时间*/
    HI_U64 u64GlobalOffset;                         /*该段PID中的最后一帧的globaloffset + frameSize*/
    HI_U64 u64FileOffset;                           /*该段PID中的最后一帧在录制文件中的偏移(含frameSize)*/
    HI_U32 u32PidNum;                               /*PidList中pid的个数*/
    HI_U32 u32IndexType;                            /*该段PID中的录制索引类型*/
    HI_U32 u32IndexPid;                             /*该段PID中的索引PID*/
    HI_U32 u32VideoType;                            /*该段PID中的视频编码类型*/
    HI_U32 u32PidList[HI_UNF_PVR_REC_MAX_PID_NUM];  /*该段PID中使用的所有PID数组*/
    PVR_INDEX_INFO_S stGopInfo;                     /*该段PID的GOP信息，主要用于特技播放*/
...
{
    HI_U32 u32MagicNum;
    HI_U32 u32UsedNum;                                              /*PID变更的次数*/
    HI_U32 u32IndexType;                                            /*索引类型*/
    PVR_INDEX_PID_INFO_S stPidInfo[PVR_MAX_RECORD_PID_CHANGE_TIMES];/*记录每次PID变更的具体信息*/
...
/* <records the information about the recording of the last record *//*<CNcomment: 记录上次录制的录制信息，
            包括断电续录或者Pid变更后上一个Pid的录制都叫上次录制*/
...
{
    HI_BOOL         bOnlineLinearization;                   /*是否进行了时移切换录制*/
    HI_BOOL         bSwitchSuccess;                         /*记录时移切换录制是否成功*/
    HI_BOOL         bNewIdxProc;                            /*时移切换录制时，是否需要校正index的offset，初始值为HI_FALSE*/
    HI_BOOL         bLineared;                              /*是否进行了线性化*/
    HI_U64          u64LastWritePos;                        /*时移切换录制时，当前WritePos的值*/
    HI_U64          u64LastOffset;                          /*时移切换录制时，EndFrame 的 offset + frameSize*/
    HI_U64          u64CorrectOffset;                       /*时移切换录制时，校正后的Offset值，与bNewIdxProc相关*/
    PVR_CYC_MGR_S   stCycInfo;                              /*时移切换录制时索引中的S/E/L 信息*/
}PVR_INDEX_REWIND_CHG_INFO_S;

/**<CNcomment: PID变更的回调函数*/
typedef struct hiPVR_PID_CHANGE_CALLBACK_S
{
    PidChangeCallBack   pfnPidChangeCB;   /*回调函数*/
    HI_VOID             *pArgs;             /*回调函数参数*/
...
typedef struct hiPVR_INDEXER_S
{
    ...
    PVR_RECORD_LAST_INFO_S stAppendRecordInfo;       /*CNcomment:记录追加录制时上次录制的相关信息*/
#endif
#ifdef PVR_PID_CHANGE_SUPPORT
    HI_U32                 u32CurRecIdx;    /*录制时当前录制的PidNum*/
    HI_U32                 u32CurPlayIdx;   /*播放中当前播放的PidNum*/
    PVR_INDEX_PID_INFO_S stCurPlayPidInfo;  /*当前播放的PID信息*/
    PVR_INDEX_REC_PID_CTRL_S *pstPidCtrl;   /*录制时记录PID变更信息*/
#endif
#if defined(PVR_APPEND_MODE_SUPPORT) || defined(PVR_PID_CHANGE_SUPPORT)
    PVR_RECORD_LAST_INFO_S *pstLastRecordInfo;              /* Record the recording information pointer of the last time */
#endif

    PVR_INDEX_REWIND_CHG_INFO_S stLastFileInfo;         /*记录时移切换录制时索引的相关信息*/
...
{
    HI_U32  u32TsPacketType;                    /*记录当前录制文件的packet type*/
    HI_BOOL bTimeshiftToRec;                    /*记录该录制文件是否进行了时移切换录制*/
    HI_BOOL bLinearied;                         /*记录该录制文件是否进行了离线线性化操作*/
    HI_U32  u32StartFrame;                      /*记录时移切换录制的S/E/L信息或离线线性化时索引头中的S/E/L信息*/
    HI_U32  u32EndFrame;
    HI_U32  u32LastFrame;
    HI_U64  u64LastWritePos;                    /*记录时移切换录制时 的WritePos 值，等于PVR_INDEX_REWIND_CHG_INFO_S 中的u64LastWritePos*/
    HI_U64  u64MaxCycSize;                      /*记录时移切换录制时,回绕文件的回绕总大小*/
    HI_BOOL bSwitchSuccess;                     /*记录时移切录制是否成功*/
...
{
    PVR_FILE64              s32OldTsFd;                    /*离线线性化时，原文件的Ts文件句柄*/
    PVR_FILE                s32OldIdxFd;                   /*离线线性化时，原文件的index文件句柄*/
    PVR_FILE64              s32NewTsFd;                /*离线线性化时，目标文件的Ts文件句柄*/
    PVR_FILE                s32NewIdxFd;               /*离线线性化时，目标文件的index文件句柄*/
    HI_U32                  u32IdxPos;                  /*离线线性化时，当前已处理的帧个数*/
    PVR_INDEX_ENTRY_NEW_S   stCurEntry;                 /*离线线性化时，当前处理帧的信息*/
    PVR_INDEX_ENTRY_NEW_S   stLastEntry;                /*离线线性化时，上一处理帧的信息，用来校正当前处理帧的相关信息*/

    HI_U64                  u32DeltaLength;             /*离线线性化时，起始帧进行188k对齐处理时的deltalength*/
    HI_U64                  u64Offset1;                 /*离线线性化时，下6个属性用来记录当前处理帧的数据信息(offset，length)*/
    HI_U64                  u64Offset2;
    HI_U64                  u64Offset3;
    HI_U32                  u32Length1;
    HI_U32                  u32Length2;
    ...
    PVR_LINEAR_BUFFER_S     stLinearBuf;                /*离线线性化时，用于处理帧数据的buffer*/
}PVR_REPAIR_CTRL_S;

typedef struct tagPVR_REWIND_REPAIR_S_
{
    HI_U32                  u32StartFrame;              /*离线线性化时，原录制Index文件中的S/E/L信息*/
    HI_U32                  u32EndFrame;
    HI_U32                  u32LastFrame;
    HI_BOOL                 bTimeshiftToRec;            /*离线线性化时，原录制文件是否进行了时移切换录制*/
    HI_U32                  u32NewStartFrame;             /*离线线性化后，新文件中的S/E/L信息*/
    HI_U32                  u32NewEndFrame;
    HI_U32                  u32NewLastFrame;
    HI_U64                  u64MaxCycSize;              /*离线线性化时，原录制文件中的MaxCycSize (在线变更中的privData 中的MaxCycSize 或未在线变更文件索引头中的u64ValidSize)*/
    HI_U64                  u64LastWritePos;            /*记录时移切换录制时WritePos 的值 或者未进行在线修复的EndFrame 的 offset + frameSize*/
    HI_U32                  u32IdxVersion;              /*索引版本号*/
    HI_U32                  u32HeaderInfoLen;           /*记录索引头长度*/
    HI_U32                  u32RewindType;              /*记录原文件的回绕类型: 1: |--E--S--L|，  2: |-S----E-L|*/
}PVR_REWIND_REPAIR_S;

typedef struct tagPVR_TRUNCATE_PROC_S_
{
    HI_CHAR                             szSrcFileName[PVR_MAX_FILENAME_LEN];    /*truncate文件时，原文件的文件名*/
    PVR_FILE64                          s32OrigTsFd;                            /*truncate文件时，原文件的Ts文件句柄*/
    PVR_FILE                            s32OrigIdxFd;                           /*truncate文件时，原文件的index文件句柄*/
    HI_U32                              u32EntryVersion;                        /*原文件索引的版本号*/
    HI_U32                              u32IndexHeaderLen;                      /*原文件索引头的长度*/
    HI_U32                              u32OrigStartFrm;                        /*原文件索引中的有效startFrmNum*/
    HI_U32                              u32OrigEndFrm;                          /*原文件索引中的有效EndFrmNum*/
    HI_U32                              u32RewindLastFrm;                       /*时移切录制时的LastFrmNum*/
    HI_U64                              u64TsTruncateOffset;                    /*原Ts文件的truncate位置*/
    HI_MPI_PVR_RECFILE_TRUNCATE_INFO_S  stTruncateUserCfg;                      /*记录用户传入信息*/
}PVR_TRUNCATE_PROC_S;
