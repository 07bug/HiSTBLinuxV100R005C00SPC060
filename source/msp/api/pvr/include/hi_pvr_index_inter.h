...
#define PVR_GET_PID_INFO_OFFSET(headInfo) (sizeof(PVR_IDX_HEADER_INFO_S) + ((headInfo).u32CADataInfoLen) + ((headInfo).u32UsrDataInfoLen))
#ifdef PVR_PID_CHANGE_SUPPORT
#define PVR_GET_PRIVATE_DATA_OFFSET(headInfo)   (PVR_GET_PID_INFO_OFFSET(headInfo) + sizeof(PVR_INDEX_REC_PID_CTRL_S))
#else
#define PVR_GET_PRIVATE_DATA_OFFSET(headInfo)   (PVR_GET_PID_INFO_OFFSET(headInfo))
#endif
...
#define PVRINDEX_GET_REFCNT(u32Version, pstEntry) \
        ((0 == (u32Version)) ? 0 : (pstEntry)->stEntry2.u8RefCnt)
#define PVRINDEX_GET_REFINFO(u32Version, pstEntry, pstRefInfo) \
        do{\
            if (0 == (u32Version))\
                HI_ERR_PVR("This version entry not support RefInfo");\
            else\
                memcpy(pstRefInfo, pstEntry->stEntry2.au32RefNum, sizeof(HI_U32)*16);\
        }while(0)
...
/*read one index once, and change the name frome PVR_READ_INDEX to PVR_READ_ONE_INDEX*/
#define PVR_READ_ONE_INDEX(readSz, pstEntryBuf, fd, offset, handle)\
    do{\
        if (0 == handle->u32EntryVersion)\
            readSz = PVRIndex_ReadIndex0(pstEntryBuf,fd,offset, handle);\
        else\
            readSz = PVRIndex_ReadIndex1(pstEntryBuf,fd,offset, handle);\
    }while(0)

/*read index header info*/
#define PVR_READ_INDEX_HEADER(readSz, pstHeaderBuf, fd)\
    do{\
        readSz = PVR_READ(pstHeaderBuf, sizeof(PVR_IDX_HEADER_INFO_S), fd, 0);\
        if (readSz != sizeof(PVR_IDX_HEADER_INFO_S))\
        {\
            HI_ERR_PVR("read index header info failed!want: %u, read: %d\n", sizeof(PVR_IDX_HEADER_INFO_S), readSz);\
            return HI_ERR_PVR_FILE_CANT_READ;\
        }\
    }while(0)

#define PVR_READ_ONE_ENTRY(readSz, pstEntryBuf, fd, offset, version)\
    do{\
        if (0 != version)\
            readSz = PVR_READ(pstEntryBuf, sizeof(PVR_INDEX_ENTRY_NEW_S), fd, offset);\
        else\
            readSz = PVR_READ(pstEntryBuf, sizeof(PVR_INDEX_ENTRY_S), fd, offset);\
    }while(0)
