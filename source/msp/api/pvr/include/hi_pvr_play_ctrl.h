...
#define PVR_PLAY_CHECK_CHN_PID_CHANGE(pChnAttr)\
    do\
    {\
        if (HI_TRUE ==  pChnAttr->bChangePID)\
        {\
            HI_ERR_PVR("PID was changing,not to support this operation! \n");\
...
typedef struct tagPVR_PLAY_START_INFO_S
{
    HI_BOOL bSeek;      /*flag for seek*/
    HI_S32  s32Whence;  /*start position:SEEK_CUR,SEEK_END or SEEK_SET*/
    HI_S64 s64Offset;   /*offset from the s32Whence. Unit: ms*/
}PVR_PLAY_START_INFO_S;
...
typedef struct hiPVR_PLAY_CHN_S
{
    ...
    HI_BOOL          bSwitchNormalPlay;       /* flag for EOS report */
    ...
    PVR_PLAY_START_INFO_S  stStartSeekInfo;     /* seekinfo before starting the play*/
    ...
    PVR_PID_CHANGE_CALLBACK_S   stPlayPidChangeCB;                       /**<the callback function was called when pid changed.*//**<CNcomment: Pid变更时的播放回调函数*/
    HI_BOOL                 bChangePID;             /*判断当前播放通道是否正在切换PID*/
    HI_UNF_DMX_PORT_ATTR_S  stOriginalDmxPortAttr;   /*used to reset demux port attr when stop play chnnel*/
    HI_U32                  u32PlaySyncTime;        /*used to synchronize trick play time*/
    HI_U32                  u32DiscardFrameNum;    /*The number of dropped frames to use for recording trick play time*/
    HI_U32                  u32RepeatFrameNum;    /*Used to record the number of repeating frames at the synchronized trick play time*/
} PVR_PLAY_CHN_S;
