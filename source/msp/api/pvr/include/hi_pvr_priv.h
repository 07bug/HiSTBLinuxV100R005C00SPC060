...
/* length of packet*/
#define PVR_PACKET_LEN_188  (188)
#define PVR_PACKET_LEN_192  (192)

/*cipher align length for append record*/
#define PVR_TS_APPEND_CIPHER_BUF_LEN(u32PacketSize)     (4*(u32PacketSize))

/**  Min length for record buffer*/
#define PVR_REC_MIN_DAV_BUF_LEN(u32PacketSize)  (1024*(u32PacketSize))

/**  Max length for record buffer*/
#define PVR_REC_MAX_DAV_BUF_LEN(u32PacketSize)  (256*1024*(u32PacketSize))
...
#define PVR_PID_CHANGE_SUPPORT
#define PVR_APPEND_MODE_SUPPORT
...
/*通过Packet type 来获取packet size*/
#define PVR_GET_TSPACKET_SIZE_BY_TYPE(enPkType)    ((HI_UNF_PVR_TS_PACKET_192 == (enPkType)) ? PVR_PACKET_LEN_192:PVR_PACKET_LEN_188)

#define PVR_MUTEX_INIT(pMutex, ret)\
    do{ \
        pthread_mutex_init(pMutex, HI_NULL); \
        if (0 != pthread_mutex_lock(pMutex)) \
        { \
            HI_ERR_PVR("init mutex failed!\n"); \
            ret = HI_FAILURE; \
        } \
        else \
        { \
            ...
            { \
                HI_ERR_PVR("unlock mutex failed!\n"); \
                ret = HI_FAILURE; \
            } \
            ret = HI_SUCCESS;\
        }\
