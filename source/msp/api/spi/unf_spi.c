...
static HI_S32 SPI_CheckAttr(const HI_UNF_SPI_ATTR_S *pstAttr)
{
	...
	if((pstAttr->csCfg != HI_UNF_SPI_LOGIC_CS)&&(pstAttr->csCfg != HI_UNF_SPI_GPIO_CS))
	{
		HI_ERR_SPI("incalid para csCfg %d\n", pstAttr->csCfg);
		return HI_ERR_SPI_INVALID_PARA;
	}
	...
	if ((pstAttr->u32Dss < 4) || (pstAttr->u32Dss > 15))
	{
		HI_ERR_SPI("incalid para u32Dss %d\n", pstAttr->u32Dss);
		return HI_ERR_SPI_INVALID_PARA;
	}
