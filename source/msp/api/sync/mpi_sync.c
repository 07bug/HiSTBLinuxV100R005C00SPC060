...
#define VID_SYNC_FINISH_THRESHOLD   60
#define VID_SYNC_NOVEL_THRESHOLD   100
...
HI_VOID SYNC_ResetStatInfo(SYNC_S *pSync, SYNC_CHAN_E enChn)
{
    if (enChn == SYNC_CHAN_VID)
    {
        ...
        pSync->SyncFinish = HI_FALSE;

        pSync->SyncFinishContinueCnt = 0;
        pSync->SyncNovelContinueCnt = 0;
    }
...
HI_VOID SYNC_CheckSyncFinish(SYNC_S *pSync)
{

    if (HI_TRUE == pSync->SyncFinish)
    {
        return;
    }

    if (pSync->SyncAttr.u32PreSyncTimeoutMs > 0)
    {
        if (HI_FALSE == pSync->PreSyncFinish)
        {
            pSync->SyncFinish = HI_FALSE;
            return;
        }
    ...
    if ((HI_FALSE == pSync->AudEnable) || (HI_FALSE == pSync->VidEnable))
    {
        pSync->SyncFinish = HI_TRUE;
        return;
    }
    ...
    {
        pSync->SyncNovelContinueCnt++;
        if (pSync->SyncNovelContinueCnt >= VID_SYNC_NOVEL_THRESHOLD)
        {
            pSync->SyncFinish = HI_TRUE;
            return;
        }
    ...
    else
    {
        ...
        pSync->SyncNovelContinueCnt = 0;
        pSync->SyncFinishContinueCnt++;
    }

    if (pSync->SyncFinishContinueCnt >= VID_SYNC_FINISH_THRESHOLD)
    {
...
HI_S32 HI_MPI_SYNC_Create(HI_UNF_SYNC_ATTR_S *pstSyncAttr, HI_HANDLE *phSync)
{
    ...
    pSync = (SYNC_S *)(HI_MPI_MMZ_Map(SyncCreate.SyncPhyAddr, HI_FALSE));
    if (!pSync)
    {
    ...
    memset(pSync->pSyncMutex, 0, sizeof(pthread_mutex_t));

    Ret = pthread_mutex_init(pSync->pSyncMutex, NULL);
    if (Ret != HI_SUCCESS)
    {
        HI_FREE(HI_ID_SYNC, pSync->pSyncMutex);
        pSync->pSyncMutex = HI_NULL;
        Ret = HI_ERR_SYNC_CREATE_ERR;
        goto SYNC_UNMAP;
    }
...
SYNC_UNMAP:
    (HI_VOID)HI_MPI_MMZ_Unmap(SyncCreate.SyncPhyAddr);
...
HI_S32 HI_MPI_SYNC_Destroy(HI_HANDLE hSync)
{
    ...
    (HI_VOID)HI_MPI_MMZ_Unmap(SyncUsrAddr.SyncPhyAddr);
...
HI_S32 HI_MPI_SYNC_VidJudge(HI_HANDLE hSync, SYNC_VID_INFO_S *pVidInfo, SYNC_VID_OPT_S *pVidOpt)
{
    ...
    memset(&SyncVidJudge, 0, sizeof(SyncVidJudge));
...
HI_S32 HI_MPI_SYNC_SetExtInfo(HI_HANDLE hSync, SYNC_EXT_INFO_E enExtInfo, HI_VOID *pData)
{
    ...
    if (!pData)
    {
        HI_ERR_SYNC("para pData is null.\n");
        return HI_ERR_SYNC_NULL_PTR;
    }
...
/* for audio render sync */
HI_S32 HI_MPI_SYNC_GetSyncInfo(HI_HANDLE hSync, SYNC_STAT_INFO_S *pSyncInfo)
{
    SYNC_S             *pSync = HI_NULL;
    SYNC_USR_ADDR_S    SyncUsrAddr;
    HI_S32             Ret;
    ...
    if (HI_NULL == pSyncInfo)
    {
        HI_ERR_SYNC("para pstPreSyncInfo is null.\n");
        return HI_ERR_SYNC_NULL_PTR;
    }
    ...
    SYNC_CheckSyncFinish(pSync);

    pSyncInfo->bSyncFinish        = pSync->SyncFinish;
    pSyncInfo->bPreSyncFinish     = pSync->PreSyncFinish;
    pSyncInfo->bPreSyncTargetInit = pSync->PreSyncTargetInit;
    pSyncInfo->u32TargetPts       = pSync->PreSyncTargetTime;
    pSyncInfo->enPreSyncTarget    = pSync->PreSyncTarget;
...
HI_S32 HI_MPI_SYNC_GetNewPcrStcValue(HI_HANDLE hSync, HI_U64 *pPcrValue, HI_U64 *pStcValue)
{
    SYNC_S              *pSync = HI_NULL;
    SYNC_USR_ADDR_S     SyncUsrAddr;
    HI_S32             Ret;
    ...
    if ((!pPcrValue)||(!pStcValue))
    {
        HI_ERR_SYNC("para pcr  %p or stc %p is null.\n",pPcrValue,pStcValue);
        return HI_ERR_SYNC_NULL_PTR;
    }
    ...
    (HI_VOID)SYNC_Mutex_Lock(pSync->pSyncMutex);
    if (HI_TRUE == pSync->bPcrStcChanged)
    {
        *pPcrValue = pSync->CrtPcrValue;
        *pStcValue = pSync->CrtStcValue;
         pSync->bPcrStcChanged = HI_FALSE;
    }
    else
    {
    ...
    //HI_ERR_SYNC("P :%llu, S:%llu\n",*pPcrValue,*pStcValue);
    (HI_VOID)SYNC_Mutex_UnLock(pSync->pSyncMutex);

    return HI_SUCCESS;
}

HI_S32 HI_MPI_SYNC_SetExternalPllAdjustMethod(HI_HANDLE hSync, HI_BOOL bSetExternalPllAdjustMethod)
{
    SYNC_S              *pSync = HI_NULL;
    SYNC_USR_ADDR_S     SyncUsrAddr;
    HI_S32             Ret;
    ...
    if ((HI_TRUE != bSetExternalPllAdjustMethod)&&(HI_FALSE != bSetExternalPllAdjustMethod))
    {
        HI_ERR_SYNC("para bSetExternalPllAdjustMethod %d is invalid.\n",bSetExternalPllAdjustMethod);
        return HI_ERR_SYNC_NULL_PTR;
    }
    ...
    (HI_VOID)SYNC_Mutex_Lock(pSync->pSyncMutex);
    pSync->bExternalPllAdjust = bSetExternalPllAdjustMethod;
    (HI_VOID)SYNC_Mutex_UnLock(pSync->pSyncMutex);

    return HI_SUCCESS;
}

HI_S32 HI_MPI_SYNC_STCCalibration(HI_HANDLE hSync, HI_S32 PPM)
{
    SYNC_S              *pSync = HI_NULL;
    SYNC_USR_ADDR_S     SyncUsrAddr;
    ...
    if (abs(PPM) > 20)
    {
        HI_ERR_SYNC("para PPM %d is invalid.\n",PPM);
        return HI_ERR_SYNC_INVALID_PARA;
    }

    CHECK_SYNC_INIT();

    memset(&SyncUsrAddr, 0x0, sizeof(SyncUsrAddr));
    ...
    (HI_VOID)SYNC_Mutex_Lock(pSync->pSyncMutex);
    pSync->ExternalPPM = PPM;
    (HI_VOID)SYNC_Mutex_UnLock(pSync->pSyncMutex);

    return HI_SUCCESS;
}

HI_S32 HI_MPI_SYNC_CheckPcrBackFlag(HI_HANDLE hSync, HI_BOOL *pbPcrBackFlag)
{
    SYNC_S              *pSync = HI_NULL;
    SYNC_USR_ADDR_S     SyncUsrAddr;
    HI_S32             Ret;
    ...
    (HI_VOID)SYNC_Mutex_Lock(pSync->pSyncMutex);
    *pbPcrBackFlag = pSync->bPcrBackFlag;
    (HI_VOID)SYNC_Mutex_UnLock(pSync->pSyncMutex);

    return HI_SUCCESS;
}

HI_S32 HI_MPI_SYNC_SetAudBackFlag(HI_HANDLE hSync, HI_BOOL bAudBackFlag)
{
    SYNC_S              *pSync = HI_NULL;
    SYNC_USR_ADDR_S     SyncUsrAddr;
    HI_S32             Ret;
    ...
    (HI_VOID)SYNC_Mutex_Lock(pSync->pSyncMutex);
    pSync->bAudBackFlag = bAudBackFlag;
    pSync->PcrSyncInfo.PcrDelta = 0;
    (HI_VOID)SYNC_Mutex_UnLock(pSync->pSyncMutex);
...
HI_S32 HI_MPI_SYNC_SetMs12Flag(HI_HANDLE hSync, HI_BOOL bMs12Flag)
{
    SYNC_S              *pSync = HI_NULL;
    SYNC_USR_ADDR_S     SyncUsrAddr;
    HI_S32              Ret;
    ...
    (HI_VOID)SYNC_Mutex_Lock(pSync->pSyncMutex);
    pSync->bMs12Flag = bMs12Flag;
    (HI_VOID)SYNC_Mutex_UnLock(pSync->pSyncMutex);

    return HI_SUCCESS;
}

HI_S32 HI_MPI_SYNC_SetSyncFinishFlag(HI_HANDLE hSync, HI_BOOL Flag)
{
    SYNC_S              *pSync = HI_NULL;
    SYNC_USR_ADDR_S     SyncUsrAddr;
    HI_S32              Ret;
