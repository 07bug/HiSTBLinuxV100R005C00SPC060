...
static HI_S32 g_s32TDeRef = 0;
extern HI_S32 TDE_OSR_Init(HI_VOID);
#endif
...
#ifndef HI_BUILD_IN_BOOT
static pthread_mutex_t s_TdeMutex = PTHREAD_MUTEX_INITIALIZER;

#define TDE_MUTEX_LOCK() \
do { \
    (void)pthread_mutex_lock(&s_TdeMutex); \
}while(0)

#define TDE_MUTEX_UNLOCK() \
do { \
    (void)pthread_mutex_unlock(&s_TdeMutex); \
}while(0)
#else
#define TDE_MUTEX_LOCK()
#define TDE_MUTEX_UNLOCK()
...
HI_S32 HI_TDE2_Open(HI_VOID)
{
    ...
    g_s32TdeFd = open(g_pszTdeDevName, O_RDWR, S_IRUSR);
    if (g_s32TdeFd < 0)
    {
        TDE_MUTEX_UNLOCK();
...
#else
    if (TdeOsiBeginJob(&s32Handle, NULL) < 0)
    {
        return HI_ERR_TDE_INVALID_HANDLE;
    }
#endif
                       ...
                       TDE2_OPT_S *pstOpt)
{
    ...
    {
        HI_GFX_Memcpy(&stBlitCmd.stBackGroundRect, pstBackGroundRect, sizeof(TDE2_RECT_S));
    }
    ...
    {
        HI_GFX_Memcpy(&stBlitCmd.stForeGroundRect, pstForeGroundRect, sizeof(TDE2_RECT_S));
    }
    ...
    {
        HI_GFX_Memcpy(&stBlitCmd.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
    }
    ...
    {
        HI_GFX_Memcpy(&stBlitCmd.stOpt, pstOpt, sizeof(TDE2_OPT_S));
    }
                           ...
                           TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect)
{
    ...
    HI_GFX_Memcpy(&stQuickCopy.stSrcRect, pstSrcRect, sizeof(TDE2_RECT_S));

    HI_GFX_Memcpy(&stQuickCopy.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
...
HI_S32 HI_TDE2_QuickFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect,HI_U32 u32FillData)
{
    ...
    HI_GFX_Memcpy(&stQuickFill.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
                           ...
                           TDE2_SURFACE_S *pstDst, TDE2_RECT_S  *pstDstRect)
{
    ...
    HI_GFX_Memcpy(&stQuickResize.stSrcRect, pstSrcRect, sizeof(TDE2_RECT_S));

    HI_GFX_Memcpy(&stQuickResize.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
...

{
    ...
    HI_GFX_Memcpy(&stQuickDeflicker.stSrcRect, pstSrcRect, sizeof(TDE2_RECT_S));

    HI_GFX_Memcpy(&stQuickDeflicker.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
                           ...
                           TDE2_OPT_S *pstOpt)
{
    ...
    {
        HI_GFX_Memcpy(&stSolidDraw.stForeGroundRect, pstForeGroundRect, sizeof(TDE2_RECT_S));
    }
    ...
    {
        HI_GFX_Memcpy(&stSolidDraw.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
    }
    ...
    {
        HI_GFX_Memcpy(&stSolidDraw.stFillColor, pstFillColor, sizeof(TDE2_FILLCOLOR_S));
    }
    ...
    {
        HI_GFX_Memcpy(&stSolidDraw.stOpt, pstOpt, sizeof(TDE2_OPT_S));
    }
                      ...
                      TDE2_MBOPT_S *pstMbOpt)
{
    ...
    if ((NULL == pstMB) || (NULL == pstDst) || (NULL == pstMbOpt) || (NULL == pstDstRect) || (NULL == pstMbRect))
    {
        return HI_ERR_TDE_NULL_PTR;
    }
#ifndef HI_BUILD_IN_BOOT
                                  ...
                                  TDE2_ROP_CODE_E enRopCode_Color, TDE2_ROP_CODE_E enRopCode_Alpha)
{
    ...
    HI_GFX_Memcpy(&stBitmapMaskRop.stBackGroundRect, pstBackGroundRect, sizeof(TDE2_RECT_S));
    HI_GFX_Memcpy(&stBitmapMaskRop.stForeGroundRect, pstForeGroundRect, sizeof(TDE2_RECT_S));
    HI_GFX_Memcpy(&stBitmapMaskRop.stMaskRect, pstMaskRect, sizeof(TDE2_RECT_S));
    HI_GFX_Memcpy(&stBitmapMaskRop.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
                                 ...
                                 HI_U8 u8Alpha, TDE2_ALUCMD_E enBlendMode)
{
    ...
    HI_GFX_Memcpy(&stBitmapMaskBlend.stBackGroundRect, pstBackGroundRect, sizeof(TDE2_RECT_S));
    HI_GFX_Memcpy(&stBitmapMaskBlend.stForeGroundRect, pstForeGroundRect, sizeof(TDE2_RECT_S));
    HI_GFX_Memcpy(&stBitmapMaskBlend.stMaskRect, pstMaskRect, sizeof(TDE2_RECT_S));
    HI_GFX_Memcpy(&stBitmapMaskBlend.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
                           ...
                           TDE2_PATTERN_FILL_OPT_S *pstOpt)
{
    ...
    {
        HI_GFX_Memcpy(&stPatternFillCmd.stBackGroundRect, pstBackGroundRect, sizeof(TDE2_RECT_S));
    }
    ...
    {
        HI_GFX_Memcpy(&stPatternFillCmd.stForeGroundRect, pstForeGroundRect, sizeof(TDE2_RECT_S));
    }
    if (HI_NULL == pstDst)
    {
    ...
    {
        HI_GFX_Memcpy(&stPatternFillCmd.stDstRect, pstDstRect, sizeof(TDE2_RECT_S));
    }
    ...
    {
        HI_GFX_Memcpy(&stPatternFillCmd.stOpt, pstOpt, sizeof(TDE2_PATTERN_FILL_OPT_S));
    }
...
HI_S32 HI_TDE2_MultiBlending(TDE_HANDLE s32Handle, TDE_SURFACE_LIST_S *pstSurfaceList)
{
    ...
    if ((NULL == pstSurfaceList) || (NULL == pstSurfaceList->pDstSurface))
    {
        return HI_FAILURE;
    }

    blitnum = pstSurfaceList->u32SurfaceNum;

    HI_GFX_Memcpy(&dstSur, pstSurfaceList->pDstSurface, sizeof(TDE2_SURFACE_S));
    ...
    for (i = 0; i < blitnum;  i++)
    {
        ...
        HI_GFX_Memcpy(&srcSur, &(pstSurfaceList->pstComposor[i].stSrcSurface), sizeof(TDE2_SURFACE_S));
        HI_GFX_Memcpy(&srcRect, &(pstSurfaceList->pstComposor[i].stInRect), sizeof(TDE2_RECT_S));
        HI_GFX_Memcpy(&dstRect, &(pstSurfaceList->pstComposor[i].stOutRect), sizeof(TDE2_RECT_S));
        HI_GFX_Memcpy(&stOpt, &(pstSurfaceList->pstComposor[i].stOpt), sizeof(TDE2_OPT_S));
