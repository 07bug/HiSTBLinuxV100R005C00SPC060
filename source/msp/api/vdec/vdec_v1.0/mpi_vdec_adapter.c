...
static HI_S32 VFMW_Create(HI_HANDLE* phInst, const HI_CODEC_OPENPARAM_S * pstParam)
{
    ...
    /* Map MMZ */
    stUserAddr.u32UserAddr = (HI_U32)HI_MMZ_Map(stUsrData.stBuf.u32PhyAddr, 0);
    stUserAddr.u32PhyAddr = stUsrData.stBuf.u32PhyAddr;
...
#if (1 == HI_VDEC_USERDATA_CC_SUPPORT)
    pstVFMWInst->u32PhyAddr = stUserAddr.u32PhyAddr;
...
err:
    ...
    {
        (HI_VOID)HI_MMZ_Unmap(stUserAddr.u32PhyAddr);
    }
#endif
...
static HI_S32 VFMW_Destroy(HI_HANDLE hInst)
{
    ...
    if (pstVFMWInst)
    {
        ...
        {
            (HI_VOID)HI_MMZ_Unmap(pstVFMWInst->u32PhyAddr);
        }
#endif
...
HI_S32 VDEC_KeepSpsPpsCtrl(HI_HANDLE hInst, HI_BOOL* pParam)
{
    HI_S32 s32Ret;
...
static HI_S32 VFMW_Control(HI_HANDLE hInst, HI_U32 u32CMD, HI_VOID * pParam)
{
    ...
    case VFMW_CMD_KEEP_SPSPPS_CTRL:
        return VDEC_KeepSpsPpsCtrl(hInst,(HI_BOOL*)pParam);
    default:
...
HI_S32 VPSS_RecvFrm(HI_HANDLE hVpss, HI_DRV_VIDEO_FRAME_PACKAGE_S* pstFrameInfo)
{
    ...
    /* BEGIN: Deleted by sdk, 2013/7/19 入参使用局部变量赋值!!!!!*/
    //*pstFrameInfo = stParam.stFrame;
...
#endif
{
    ...
    /* Map MMZ */
#ifndef HI_TEE_SUPPORT
    stUserAddr.u32PhyAddr = stBuf.u32PhyAddr;
    stUserAddr.u32UserAddr = (HI_U32)HI_MMZ_Map(stBuf.u32PhyAddr, 0);
#else
    ...
    pstBufInst->u32PhyAddr = stUserAddr.u32PhyAddr;
    ...
    #ifndef HI_TEE_SUPPORT
        (HI_VOID)HI_MMZ_Unmap(pstBufInst->u32PhyAddr);
    #else
        if(HI_TRUE == pstBufAttr->bTvp)
        {
        }
...
HI_S32 VDEC_DestroyStreamBuf(HI_HANDLE hBuf)
{
    ...
    {
    #ifndef HI_TEE_SUPPORT
        s32Ret = HI_MMZ_Unmap(pstBufInst->u32PhyAddr);
    #else
