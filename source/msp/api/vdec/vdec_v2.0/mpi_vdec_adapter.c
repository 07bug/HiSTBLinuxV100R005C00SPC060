...
static const HI_U32 g_CodecTypeTable[][2] =
{
    ...
    {HI_CODEC_ID_VIDEO_AVS2,                  HI_UNF_VCODEC_TYPE_AVS2},
...
typedef struct tagVFMW_INST_S
{
    ...
    HI_U8 *pu8UserDataVirAddr[HI_VDEC_SUPPORT_USERDATA_TYPE_NUM];            /* User data MMZ user space address  */
    HI_U32 u32UserDataPhyAddr[HI_VDEC_SUPPORT_USERDATA_TYPE_NUM];            /* User data Mem phy address */
#endif
...
#if (1 == HI_VDEC_USERDATA_CC_SUPPORT)
static HI_S32 VFMW_AllocAndMapUserdataBuffer(HI_HANDLE hInst, VFMW_INST_S *pstVfMWInst, USERDATA_TYPE_E UserDataType)
{
    HI_S32 s32Ret;
    VDEC_CMD_USERDATABUF_S UserDataBuf = {0};
    VDEC_CMD_BUF_USERADDR_S UserDataUserAddr = {0};
    ...
    UserDataBuf.hHandle = VFMW_INST_HANDLE(hInst);
    UserDataBuf.stBuf.u32Size = HI_VDEC_USERDATA_CC_BUFSIZE;
    UserDataBuf.stBuf.UserDataType = UserDataType;

    s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_CHAN_USERDATAINITBUF, &UserDataBuf);

    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_VDEC("Init user data buf fail, hInst = %d, UserDataType = %d!\n", hInst, UserDataType);
        return HI_FAILURE;
    }

#ifndef HI_SMMU_SUPPORT
    UserDataUserAddr.u64UserAddr = (HI_U64)(HI_SIZE_T)HI_MPI_MMZ_Map(UserDataBuf.stBuf.u32PhyAddr, 0);
#else
    UserDataUserAddr.u64UserAddr = (HI_U64)(HI_SIZE_T)HI_MMZ_Map(UserDataBuf.stBuf.u32PhyAddr, 0);
#endif

    if (UserDataUserAddr.u64UserAddr == HI_NULL)
    {
        HI_ERR_VDEC("Map user data user space virtual address failed, hInst = %d, UserDataType = %d!\n", hInst, UserDataType);
        return HI_FAILURE;
    }

    UserDataUserAddr.hHandle = UserDataBuf.hHandle;
    UserDataUserAddr.UserDataType = UserDataType;

    s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_CHAN_USERDATASETBUFADDR, &UserDataUserAddr);

    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_VDEC("Set user data user space virtual address to kernel space failed, hInst = %d, UserDataType = %d!\n", hInst, UserDataType);
#ifndef HI_SMMU_SUPPORT
        s32Ret = HI_MPI_MMZ_Unmap(UserDataBuf.stBuf.u32PhyAddr);
#else
        s32Ret = HI_MPI_SMMU_Unmap(UserDataBuf.stBuf.u32PhyAddr);
#endif

        if (s32Ret != HI_SUCCESS)
        {
            HI_ERR_VDEC("Unmap user data user space virtual address failed, user data phyaddr = 0x%x\n", UserDataBuf.stBuf.u32PhyAddr);
        }

        pstVfMWInst->pu8UserDataVirAddr[UserDataType] = HI_NULL;
        pstVfMWInst->u32UserDataPhyAddr[UserDataType] = 0;

        return HI_FAILURE;
    }

    pstVfMWInst->pu8UserDataVirAddr[UserDataType] = (HI_U8 *)(HI_SIZE_T)(UserDataUserAddr.u64UserAddr);
    pstVfMWInst->u32UserDataPhyAddr[UserDataType] = UserDataBuf.stBuf.u32PhyAddr;
...
#if (1 == HI_VDEC_USERDATA_CC_SUPPORT)
static HI_S32 VFMW_UnmapAndFreeUserdataBuffer(HI_HANDLE hInst, VFMW_INST_S *pstVfMWInst, USERDATA_TYPE_E UserDataType)
{
    HI_S32 s32Ret;
    ...
    if (pstVfMWInst->pu8UserDataVirAddr[UserDataType] == NULL)
    {
        return HI_FAILURE;
    }

#ifndef HI_SMMU_SUPPORT
    s32Ret = HI_MPI_MMZ_Unmap(pstVfMWInst->u32UserDataPhyAddr[UserDataType]);
#else
    s32Ret = HI_MPI_SMMU_Unmap(pstVfMWInst->u32UserDataPhyAddr[UserDataType]);
#endif

    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_VDEC("Unmap user data user space virtual address failed, user data phyaddr = 0x%x\n", pstVfMWInst->u32UserDataPhyAddr[UserDataType]);
    }
...
static HI_S32 VFMW_Create(HI_HANDLE *phInst, const HI_CODEC_OPENPARAM_S *pstParam)
{
    ...
    if (NULL != pstParam)
    {
        ...
        if ((HI_CODEC_ID_VIDEO_H263 != pstParam->enID)
            ...
            && (HI_CODEC_ID_VIDEO_SORENSON != pstParam->enID))
        {
            stParam.u32DFBEn = 1;
        }
        else
    ...
    *phInst = stParam.hHandle;
    ...
    memset(pstVFMWInst, 0, sizeof(VFMW_INST_S));
    ...
    {
        s32Ret = VFMW_AllocAndMapUserdataBuffer(*phInst, pstVFMWInst, UserdataTypeIndex);
...
err1:
    ...
    for (UserdataTypeIndex--; UserdataTypeIndex >= 0; UserdataTypeIndex--)
    {
        VFMW_UnmapAndFreeUserdataBuffer(*phInst, pstVFMWInst, UserdataTypeIndex);
    }
...
static HI_S32 VFMW_Destroy(HI_HANDLE hInst)
{
    ...
    if (pstVFMWInst)
    {
        ...
        {
            VFMW_UnmapAndFreeUserdataBuffer(hInst, pstVFMWInst, UserDataType);
        }
...
static HI_S32 VFMW_GetStreamInfo(HI_HANDLE hInst, HI_CODEC_STREAMINFO_S *pstStreamInfo)
{
    ...
    pstStreamInfo->stVideo.enBitWidth = stParam.stInfo.enBitWidth;
    pstStreamInfo->stVideo.enSrcColorSpace = stParam.stInfo.enSrcColorSpace;
...
static HI_S32 VFMW_CheckEvt(HI_HANDLE hInst, HI_VOID *pParam)
{
    ...
    memset(&stParam, 0, sizeof(VDEC_CMD_EVENT_S));
...
static HI_S32 VFMW_AcqUserData(HI_HANDLE hInst, HI_VOID *pParam)
{
    ...
    stParam.hHandle = VFMW_INST_HANDLE(hInst);
    stParam.enType = *(pUserData->penType);
...
static HI_S32 VFMW_RlsUserData(HI_HANDLE hInst, HI_VOID *pParam)
{
    ...
    stParam.hHandle = VFMW_INST_HANDLE(hInst);
    stParam.stUserData.enBroadcastProfile = pUserData->pstData->enBroadcastProfile;
    stParam.stUserData.enPositionInStream = pUserData->pstData->enPositionInStream;
    stParam.stUserData.u32Pts             = pUserData->pstData->u32Pts;
    stParam.stUserData.u32SeqCnt          = pUserData->pstData->u32SeqCnt;
    stParam.stUserData.u32SeqFrameCnt     = pUserData->pstData->u32SeqFrameCnt;
    stParam.stUserData.pu8Buffer          = (HI_U64)(HI_SIZE_T)pUserData->pstData->pu8Buffer;
    stParam.stUserData.u32Length          = pUserData->pstData->u32Length;
    stParam.stUserData.bBufferOverflow    = pUserData->pstData->bBufferOverflow;
    stParam.stUserData.bTopFieldFirst     = pUserData->pstData->bTopFieldFirst;
    stParam.enType = *pUserData->penType;
...
{
    HI_S32 s32Ret;
    VDEC_CMD_SET_UVMOS_CTRL_S stParam;
    HI_BOOL *pIsUVMOSEnableCtrl = (HI_BOOL *)pParam;

    if (HI_NULL == pIsUVMOSEnableCtrl)
    {
        return HI_ERR_CODEC_INVALIDPARAM;
    }

    stParam.hHandle = VFMW_INST_HANDLE(hInst);
    stParam.bUVMOSEnable = *pIsUVMOSEnableCtrl;
    s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_CHAN_UVMOS, &stParam);

    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("Chan %d UMAPC_VDEC_CHAN_UVMOS err:%x!\n", stParam.hHandle, s32Ret);
        return HI_ERR_CODEC_OPERATEFAIL;
    }
...
HI_S32 VFMW_GetVfmwHandle(HI_HANDLE hInst, HI_VOID *pParam)
{
    HI_S32 s32Ret;
    VDEC_GET_VFMW_HANDLE stParam;
    HI_HANDLE *pVfmwHandle = (HI_HANDLE *)pParam;

    if (HI_NULL == pVfmwHandle)
    {
        return HI_ERR_CODEC_INVALIDPARAM;
    }

    stParam.hHandle = VFMW_INST_HANDLE(hInst);
    s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_GET_VFMW_HANDLE, &stParam);

    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("Chan %d Get Vfmw Handle err:%x!\n", stParam.hHandle, s32Ret);

        return HI_ERR_CODEC_OPERATEFAIL;
    }

    *pVfmwHandle = stParam.hVfmwHandle;

    return HI_SUCCESS;
}

HI_S32 VDEC_GetNewestDecodedFrame(HI_HANDLE hInst, HI_VOID *pParam)
{
    HI_S32 s32Ret;
    VDEC_CMD_VO_FRAME_S stParam;
    HI_DRV_VIDEO_FRAME_S *pFrame = (HI_DRV_VIDEO_FRAME_S *)pParam;
    ...
    stParam.hHandle = VFMW_INST_HANDLE(hInst);
    s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_CHAN_GET_NEWEST_DECODED_FRAME, &stParam);

    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_VDEC("Chan %d GetNewestDecodedFrame err:%x!\n", stParam.hHandle, s32Ret);

        return HI_ERR_CODEC_OPERATEFAIL;
    }

    *pFrame = stParam.stFrame;

    HI_INFO_VDEC("Chan %d GetNewestDecodedFrame frame OK\n", stParam.hHandle);
...
HI_S32 VDEC_KeepSpsPpsCtrl(HI_HANDLE hInst, HI_VOID *pParam)
{
    HI_S32 s32Ret;
    VDEC_CMD_KEEP_SPSPPS_CTRL_S stParam;
    ...
    stParam.hHandle = VFMW_INST_HANDLE(hInst);
    stParam.bKeepSpsPps = *((HI_BOOL *)pParam);
    s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_CHAN_KEEPSPSPPS, &stParam);

    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("Chan %d UMAPC_VDEC_CHAN_KEEPSPSPPS err:%x!\n", stParam.hHandle, s32Ret);
        return HI_ERR_CODEC_OPERATEFAIL;
    }
...
static const VFMW_CTRL_CMD_NODE g_VfmwCtrlTable[] =
{
    ...
    {VFMW_CMD_SET_UVMOS,                VDEC_SetUVMOSCtrl},
    {VFMW_CMD_GET_VFMW_HANDLE,          VFMW_GetVfmwHandle},
    {VFMW_CMD_KEEP_SPSPPS_CTRL,         VDEC_KeepSpsPpsCtrl},
    {VFMW_CMD_GET_NEWEST_DECODED_FARME, VDEC_GetNewestDecodedFrame},
...
HI_S32 VPSS_RecvFrm(HI_HANDLE hVpss, HI_VOID *pParam)
{
    ...
    /* BEGIN: Deleted by sdk, 2013/7/19 */
    //*pstFrameInfo = stParam.stFrame;
    /* END:   Deleted by sdk, 2013/7/19 */
...
HI_S32 VDEC_PutStreamBuf(HI_HANDLE hBuf, const VDEC_ES_BUF_S *pstBuf)
{
    ...
    stBufParam.stBuf.bDualLayer     = pstBuf->bDualLayer;
...
HI_S32 VDEC_CreateFrameBuf(HI_HANDLE *phBuf, VFMW_STREAMBUF_S *pstStrmBuf)
{
    ...
    if (HI_NULL == phBuf || pstStrmBuf == HI_NULL)
    {
        return HI_ERR_VDEC_INVALID_PARA;
    }
...
HI_S32 VDEC_GetBufferVirAddr(HI_HANDLE hBuf, HI_U32 u32PhyAddr, HI_U8 **ppVirAddr)
{
    ...
    if (HI_NULL == pstBufInst || ppVirAddr == HI_NULL)
    {
        return HI_FAILURE;
    }
...
{
    HI_S32 s32Ret;
    VDEC_CMD_SET_IDRPTS_S stParam;

    stParam.hHandle = hVdec;
    stParam.IDRPts = u32IDRPts;
    s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_CHAN_SETIDRPTS, &stParam);

    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("VDEC_SetStartIDRPts ,err: %x\n", s32Ret);
