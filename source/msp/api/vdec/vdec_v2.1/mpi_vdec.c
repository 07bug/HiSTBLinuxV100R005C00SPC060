...
typedef struct tagVDEC_INST_S
{
    ...
    HI_BOOL                     bNeedSetHDRAttr;
    VDEC_HDR_ATTR_S             vdec_hdr_attr;  /* VDEC HDR attribute */
#endif
...
extern HI_S32 StopDecodeFrame(HI_HANDLE hInst);

HI_U32 g_lowDelayFrameIndex[HI_VDEC_MAX_INSTANCE_NEW] = {0};
HI_U32 g_lowDelayVdecHandle[HI_VDEC_MAX_INSTANCE_NEW] = {0};
HI_U32 g_lowDelayLinePhyAddr[HI_VDEC_MAX_INSTANCE_NEW] = {0};
HI_U64 g_lowDelayLineVirAddr[HI_VDEC_MAX_INSTANCE_NEW] = {0};
...
static HI_S32 VDEC_ConvertError(HI_S32 s32Err)
{
    ...
    {
        HI_WARN_VDEC("HI_VDEC VDEC_ConvertError Num: 0x%x.\n", s32Err);
    }
    ...
    switch (s32Err)
    {
        ...
        case HI_ERR_CODEC_INPUTCORRUPT:
            return HI_ERR_VDEC_INPUT_CORRUPT;

        case HI_ERR_CODEC_NOENOUGHDATA:
            return HI_ERR_VDEC_NO_ENOUGH_DATA;
...
static HI_S32 VDEC_VFMWSpecCMD(HI_HANDLE hVdec, VFMW_CMD_E enCMD, HI_VOID *pstParam)
{
    ...
    if (pstVdec->pstCodec)
    {
        ...
        {
            s32Ret = pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), enCMD, (HI_VOID *)pstParam);
...
static HI_S32 VDEC_CreateCodec(VDEC_INST_S *pstVdec, HI_CODEC_ID_E enID)
{
    ...
    if (0 != strncmp("VFMW", CodecName, 4))
    {
        ...
        if (HI_CODEC_NeedFrameBuf(pstVdec->hCodecInst))
        {
            ...
            s32Ret = VDEC_CreateFrameBuf(&(pstVdec->hFrameBuf), &stStrmBuf);
...
#if 0
static HI_VOID VDEC_MarkCodecFrm(VDEC_INST_S *pstVdec, HI_CODEC_FRAME_S *pCodecFrm, HI_DRV_VDEC_FRAME_BUF_S *pstFrmGet)
{
    HI_U8 *pVirAddr = HI_NULL;

    pCodecFrm->stOutputAddr.u32Phy = pstFrmGet->u32PhyAddr;
    VDEC_GetBufferVirAddr(pstVdec->hFrameBuf, pstFrmGet->u32PhyAddr, &pVirAddr);
    pCodecFrm->stOutputAddr.pu8Vir = pVirAddr;
    pCodecFrm->stOutputAddr.u32Size = pstFrmGet->u32Size;

    if (HI_CODEC_INST_HANDLE(pstVdec->hCodecInst) >= HI_VDEC_MAX_INSTANCE_NEW)
    {
        HI_ERR_VDEC("[%s %d] overflow!!!\n", __func__, __LINE__);
    ...
    if (pstFrmGet->u32LineNumPhyAddr != 0)
    {
        ...
        //g_lowDelayLineVirAddr[HI_CODEC_INST_HANDLE(pstVdec->hCodecInst)] = stFrmGet.u64LineNumVirAddr;
        VDEC_GetBufferVirAddr(pstVdec->hFrameBuf, pstFrmGet->u32LineNumPhyAddr, &pVirAddr);
        g_lowDelayLineVirAddr[HI_CODEC_INST_HANDLE(pstVdec->hCodecInst)] = (HI_ULONG)pVirAddr;
...
#endif
    {
        g_lowDelayLinePhyAddr[HI_CODEC_INST_HANDLE(pstVdec->hCodecInst)] = 0;
        g_lowDelayLineVirAddr[HI_CODEC_INST_HANDLE(pstVdec->hCodecInst)] = 0;
    }
}
#endif
...
static HI_S32 VDEC_RecvInData(VDEC_INST_S *pstVdec, HI_CODEC_STREAM_S *pCodecStrm, VDEC_ES_BUF_S *pStrm, HI_UNF_ES_BUF_S *pDmxBuf)
{
    ...
    if (pstVdec->hStreamBuf != HI_INVALID_HANDLE)
    {
        ...
        /* Make stCodecStrm */
        pCodecStrm->pu8Addr = (HI_U8 *)pStrm->pu8Addr;
    ...
    {
        ret = HI_MPI_DMX_AcquireEs(pstVdec->hDmxVidChn, pDmxBuf, HI_NULL);
        ...
        /* Make stCodecStrm */
        pCodecStrm->pu8Addr = (HI_U8 *)pDmxBuf->pu8Buf;
    ...
    if (ret != HI_SUCCESS)
    {
        if (pstVdec->u32EosFlag == 1)
        {
            ...
            if (VDEC_VPSSCMD(pstVdec->hVdec, VPSS_CMD_SENDEOS, HI_NULL) != HI_SUCCESS)
            {
                HI_ERR_VDEC("HI_MPI_VDEC_SendEos failed.\n");
            }
        }
    }
...
static HI_VOID VDEC_SoftCodec_Exit(VDEC_INST_S *pstVdec, HI_CODEC_FRAME_S *pstCodecFrm, HI_U32 u32PhyAddr, HI_BOOL bGetFrameBuf)
{
    HI_S32 s32Ret;
    ...
    if (bGetFrameBuf)
    {
        ...
        stFrmPut.s32LumaPhyAddr = (HI_S32)u32PhyAddr;
        (HI_VOID)VDEC_PutFrameBuf(pstVdec->hFrameBuf, &stFrmPut);

        if (pstCodecFrm->stOutputAddr.pu8Vir != HI_NULL)
        {
            s32Ret = HI_MMZ_Unmap(u32PhyAddr);

            if (s32Ret != HI_SUCCESS)
            {
                HI_ERR_VDEC("%s, %d, unmap failed!\n", __func__, __LINE__);
            }
        }
    }
}
...
static HI_VOID *VDEC_SoftCodec(HI_VOID *phVdec)
{
    ...
    while (!pstVdec->bThreadStop)
    {
        ...
        if (!bGetFrameBuf)
        {
            ...
            if (s32Ret != HI_SUCCESS)
            {
                ...
                VDEC_UNLOCK(pstVdec->stMutex);
                (HI_VOID)nanosleep(&req, NULL);
                continue;
            }

            stCodecFrm.stOutputAddr.u32Phy = stFrmGet.u32PhyAddr;
            pVirAddr = HI_MMZ_Map(stFrmGet.u32PhyAddr, 0);
            ...
            stCodecFrm.stOutputAddr.pu8Vir = pVirAddr;
        ...
        if (pstVdec->u32EosFlag != 2)   /* If not end of stream */
        {
        ...
        if (s32Ret == HI_SUCCESS)
        {
            ...
            stFrmPut.s32LineNumPhyAddr = (HI_S32)(g_lowDelayLinePhyAddr[HI_CODEC_INST_HANDLE(pstVdec->hCodecInst)]);
            stFrmPut.u64LineNumVirAddr = g_lowDelayLineVirAddr[HI_CODEC_INST_HANDLE(pstVdec->hCodecInst)];
            ...
            if (stCodecFrm.stOutputAddr.pu8Vir != HI_NULL)
            {
                s32Ret = HI_MMZ_Unmap(stFrmGet.u32PhyAddr);
            ...
            /* Module Lowdelay, make sure complete the decoding*/
            if (g_lowDelayLinePhyAddr[HI_CODEC_INST_HANDLE(pstVdec->hCodecInst)] != 0)
            {
#if (1 == HI_VDEC_MJPEG_SUPPORT)
                StopDecodeFrame(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst));
#endif
            }
    ...
    VDEC_SoftCodec_Exit(pstVdec, &stCodecFrm, stFrmGet.u32PhyAddr, bGetFrameBuf);
...
static HI_UNF_VCODEC_CAP_LEVEL_E HI_VDEC_GetDefaultCapLevel(HI_VOID)
{
    HI_S32              ret;
    ...
    if (((HI_CHIP_TYPE_HI3716M == ChipType) && (HI_CHIP_VERSION_V410 == ChipVersion))
        || ((HI_CHIP_TYPE_HI3716M == ChipType) && (HI_CHIP_VERSION_V420 == ChipVersion)))
    {
...
HI_S32 HI_MPI_VDEC_AllocChan(HI_HANDLE *phHandle, const HI_UNF_AVPLAY_OPEN_OPT_S *pstMaxCapbility)
{
...
#endif
#ifdef HI_HDR_DOLBYVISION_SUPPORT
    pstVdec->bNeedSetHDRAttr = HI_FALSE;
#endif
    ...
    if (HI_NULL == pstMaxCapbility)
    {
        ...
        pstVdec->stOpenParam.enCapLevel = HI_VDEC_GetDefaultCapLevel();
...
HI_S32 HI_MPI_VDEC_SetChanAttr(HI_HANDLE hVdec, const HI_UNF_VCODEC_ATTR_S *pstAttr)
{
    ...
    else if (VDEC_UNF2CodecId(pstVdec->stCurAttr.enType) != enID)
    {
        ...
        if ((!pstVdec->bIsVFMW) || (!HI_CODEC_SupportDecode(pstVdec->pstCodec, enID))
            || (HI_CODEC_ID_VIDEO_H263 == enID)
            || (HI_CODEC_ID_VIDEO_SORENSON == enID)
    ...
    if (HI_TRUE == pstVdec->bNeedSetHDRAttr)
    {
        pstVdec->bNeedSetHDRAttr = HI_FALSE;

        s32Ret = VDEC_VFMWSpecCMD(hVdec, VFMW_CMD_SET_HDR_ATTR, &pstVdec->vdec_hdr_attr);
...
{
    VDEC_INST_S *vdec_inst = HI_NULL;

    VDEC_CHECK_INIT;

    if (VdecHDRAttr == HI_NULL)
    {
        return HI_ERR_VDEC_INVALID_PARA;
    }

    VDEC_FIND_INST(hVdec, vdec_inst);

    if (vdec_inst == HI_NULL)
    {
        return HI_ERR_VDEC_INVALID_PARA;
    }

    vdec_inst->vdec_hdr_attr = *VdecHDRAttr;

    if (vdec_inst->pstCodec != HI_NULL)
    {
        if (vdec_inst->bIsVFMW)
        {
            return VDEC_VFMWSpecCMD(hVdec, VFMW_CMD_SET_HDR_ATTR, (HI_VOID *)VdecHDRAttr);
        }
    }
    else
    {
        vdec_inst->bNeedSetHDRAttr = HI_TRUE;
    }
...
HI_S32 HI_MPI_VDEC_GetChanStreamInfo(HI_HANDLE hVdec, HI_UNF_VCODEC_STREAMINFO_S *pstStreamInfo)
{
    ...
    if ((pstVdec->pstCodec) && (pstVdec->pstCodec->GetStreamInfo))
    {
        ...
        pstStreamInfo->enBitWidth = (HI_UNF_PIXEL_BITWIDTH_E)(stInfo.stVideo.enBitWidth);
        pstStreamInfo->enSrcColorSpace = (HI_UNF_COLOR_SPACE_E)(stInfo.stVideo.enSrcColorSpace);
    }
    else
    {
...
HI_S32 HI_MPI_VDEC_GetPortAttr(HI_HANDLE hVdec, HI_HANDLE hPort, HI_DRV_VPSS_PORT_CFG_S *pstPortCfg)
{
    ...
    if (HI_NULL == pstPortCfg)
    {
        HI_ERR_VDEC("Bad param. %p\n", pstPortCfg);
        return HI_ERR_VDEC_INVALID_PARA;
    }
...
{
    HI_S32              s32Ret;
    VDEC_RECVFRM_OPT_S  VdecRecvOpt;
    VDEC_CHECK_INIT;
...
HI_S32 HI_MPI_VDEC_SetLowDelay(HI_HANDLE hVdec, HI_UNF_AVPLAY_LOW_DELAY_ATTR_S *pstAttr)
{
    ...
    s32Ret = VDEC_SetLowDelay(hVdec, (HI_VOID *)pstAttr);
    return s32Ret;
}
...
HI_S32 HI_MPI_VDEC_SetTVP(HI_HANDLE hVdec, HI_UNF_AVPLAY_TVP_ATTR_S *pstAttr)
{
    ...
    s32Ret = VDEC_SetTVP(hVdec, (HI_VOID *)pstAttr);
...
HI_S32 HI_MPI_VDEC_Invoke(HI_HANDLE hVdec, HI_CODEC_VIDEO_CMD_S *pstParam)
{
    ...
    {
        return VFMW_SetProgressive(hVdec, pstParam->pPara);
    }
    else if (pstParam->u32CmdID == VFMW_CMD_SET_SCENE_MODE)
    {
        return VFMW_SetSceneMode(hVdec, pstParam->pPara);
    }
    else if (pstParam->u32CmdID == VFMW_CMD_SET_DPBFULL_CTRL)
    {
        return VDEC_SetDPBFullCtrl(hVdec, pstParam->pPara);
    }
    else if (pstParam->u32CmdID == VFMW_CMD_SET_UVMOS)
    {
        return VDEC_SetUVMOSCtrl(hVdec, pstParam->pPara);
    }
...
HI_S32 HI_MPI_VDEC_GetNewestDecodedFrame(HI_HANDLE hVdec, HI_DRV_VIDEO_FRAME_S *pFrame)
{
    HI_S32 s32Ret;
    ...
    s32Ret = VDEC_VFMWSpecCMD(hVdec, VFMW_CMD_GET_NEWEST_DECODED_FARME, pFrame);
...
HI_S32 HI_MPI_VDEC_RlsUserData(HI_HANDLE hVdec, HI_UNF_VIDEO_USERDATA_S *pstUserData, HI_UNF_VIDEO_USERDATA_TYPE_E *penType)
{
    ...
    if (HI_NULL != pstVdec->pstCodec)
    {
        if (pstVdec->bIsVFMW)
        {
            ...
            return VDEC_VFMWSpecCMD(hVdec, VFMW_CMD_RLSUSERDATA, (HI_VOID *)(&stParam));
        }
    }
...
HI_S32 HI_MPI_VDEC_GetLeftStreamFrm(HI_HANDLE hVdec, HI_U32 *pLeftStreamFrm)
{
    ...
    if (pstVdec->bIsVFMW)
    {
        s32Ret = VDEC_VFMWSpecCMD(hVdec, VFMW_CMD_GET_LEFT_STREAM_FRAME, pLeftStreamFrm);
    }
    else
    {
        s32Ret = HI_FAILURE;
...
//add by sdk,for vpss bypass ,overlay will release frame buffer after vdec & vfmw instance destory
HI_S32 HI_MPI_VDEC_GlobalRelFrm(HI_DRV_VIDEO_FRAME_S *pVideoFrame)
{
    ...
    if (HI_NULL == pVideoFrame)
    {
        HI_ERR_VDEC("ERROR: null point!!\n");

        return HI_ERR_VDEC_INVALID_PARA;
    }

    s32Ret = HI_MPI_VDEC_Init();

    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("HI_MPI_VDEC_Init failed! return 0x%x\n", s32Ret);
        return HI_FAILURE;
    }

    s32Ret = VDEC_GlobalReleaseFrm(pVideoFrame);
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("HI_MPI_VDEC_DeInit failed! return 0x%x\n", s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 HI_MPI_VDEC_GetVideoBypass(HI_HANDLE hVdec, HI_BOOL *pbVideoBypass)
{
    ...
    if (HI_NULL == pbVideoBypass)
    {
        HI_ERR_VDEC("%s %d\n", __func__, __LINE__);
        return HI_ERR_VDEC_NULL_PTR;
    }

    s32Ret = VDEC_GetVideoBypassInfo(hVdec, pbVideoBypass);
...
HI_S32 HI_MPI_VDEC_SetChnTunnelMode(HI_HANDLE hVdec, HI_U32 Enable)
{
    ...
    s32Ret = VDEC_SetTunnelMode(hVdec, Enable);

    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("VDEC_SetTunnelMode err0!\n");
        return HI_FAILURE;
    }

    pstVdec->u32TunnelModeEnable = Enable;

    return s32Ret;
}

HI_S32 HI_MPI_VDEC_CheckIfSupportRandomStream(HI_UNF_VCODEC_TYPE_E enType, HI_UNF_VCODEC_EXTATTR_U *pExtAttr, HI_BOOL *pbSupport)
{
...
HI_S32 HI_MPI_VDEC_SetStartIDRPts(HI_HANDLE hVdec, HI_U32 u32IDRPts)
{
    HI_S32 s32Ret = HI_FAILURE;
    VDEC_INST_S *pstVdec = HI_NULL;
    ...
    s32Ret = VDEC_SetStartIDRPts(hVdec, u32IDRPts);
