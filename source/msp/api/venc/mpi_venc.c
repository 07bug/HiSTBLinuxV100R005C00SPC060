...
HI_S32 HI_MPI_VENC_SetAttr(HI_HANDLE hVencChn, const HI_UNF_VENC_CHN_ATTR_S* pstAttr)
{
    ...
    if (HI_SUCCESS == s32Ret)
    {
       if ((stVencChnCreate.stVeInfo.u32StrmBufPhyAddrJpeg != HI_NULL) && (s_asrVeChnVirAddr[i].pStrmBufVirAddrJpeg == HI_NULL))  /*h264->jpeg*/
       {
            ...
            s_asrVeChnVirAddr[i].pStrmBufVirAddrJpeg = HI_MPI_MMZ_Map(s_asrVeChnInfo[i].u32StrmBufPhyAddrJpeg, 0);
       ...
       {
            s32Ret = HI_MPI_MMZ_Unmap(s_asrVeChnInfo[i].u32StrmBufPhyAddrJpeg);
            if (HI_SUCCESS != s32Ret)
            {
                return s32Ret;
            }
...
HI_S32 HI_MPI_VENC_DetachInput(HI_HANDLE hVencChn/*, HI_HANDLE hSrc*/)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VENC("Venc Detach input failed!\n");
    }
...
static HI_S32 GetStreamAddr(HI_U32 HandleID, VENC_BUF_OFFSET_S *pstBufOffSet, HI_UNF_VENC_STREAM_S* pstStream)
{
    ...
    else
    {
        if (IsChipIDV500R001(stVersion.enChipTypeHardWare, stVersion.enChipVersion))
        {
            ...
            {
                if (pstStream->enDataType.enH264EType == HI_UNF_H264E_NALU_SPS)
                {
            ...
            {
                if (pstStream->enDataType.enHEVCEType == HI_UNF_HEVCE_NALU_SPS)
                {
                    pstStream->pu8Addr = (HI_U8*)(s_asrVeChnVirAddr[HandleID].pSliceBufVirAddr) + pstBufOffSet->u32StrmBufOffset[0];
                }
                else
...
HI_S32 HI_MPI_VENC_AcquireStream(HI_HANDLE hVencChn, HI_UNF_VENC_STREAM_S* pstStream, HI_U32 u32TimeoutMs)
{
    ...
    memset(&stBufOffSet, 0, sizeof(VENC_BUF_OFFSET_S));
...
HI_S32 HI_MPI_VENC_SetRateControlType(HI_HANDLE hVencChn, HI_UNF_VENC_RATECONTROL_TYPE_E enRcType)
{
    HI_S32 s32Ret, i = 0;
    VENC_INFO_RATECONTROL_S stRCType;

    memset(&stRCType, 0, sizeof(VENC_INFO_RATECONTROL_S));

    if ((enRcType != HI_UNF_VENC_RATECONTROL_TYPE_CBR) && (enRcType != HI_UNF_VENC_RATECONTROL_TYPE_AVBR))
    {
        HI_ERR_VENC("enRcType = %d not support!\n",enRcType);
        return HI_ERR_VENC_INVALID_PARA;
    }
    ...
    stRCType.hVencChn = hVencChn;
    stRCType.eRCType  = (enRcType == HI_UNF_VENC_RATECONTROL_TYPE_CBR) ? VENC_DRV_ControlRateConstant : VENC_DRV_ControlRateVariable;
    s32Ret = ioctl(g_VEncDevFd, CMD_VENC_SET_RC_TYPE, &stRCType);
    if (HI_SUCCESS != s32Ret)
    {
       HI_ERR_VENC("Drv Chan(%d) set rate control type(%d) failed! return 0x%x\n", stRCType.hVencChn, stRCType.eRCType, s32Ret);
    }

    return s32Ret;
}

static HI_UNF_VENC_RATECONTROL_TYPE_E convert_DrvToUnf_RcType(VENC_DRV_CONTROLRATETYPE_E eRCType)
{
    HI_UNF_VENC_RATECONTROL_TYPE_E retVal;

    switch (eRCType)
    {
        case VENC_DRV_ControlRateVariable:
            retVal = HI_UNF_VENC_RATECONTROL_TYPE_AVBR;
            break;
        case VENC_DRV_ControlRateConstant:
            retVal = HI_UNF_VENC_RATECONTROL_TYPE_CBR;
            break;
        default:
            retVal = HI_UNF_VENC_RATECONTROL_TYPE_BUTT;
            break;
    }
...
HI_S32 HI_MPI_VENC_GetRateControlType(HI_HANDLE hVencChn, HI_UNF_VENC_RATECONTROL_TYPE_E *penRcType)
{
    HI_S32 s32Ret, i = 0;
    VENC_INFO_RATECONTROL_S stRCType;
    ...
    if (HI_NULL == penRcType)
    {
        HI_ERR_VENC("para penRcType is NULL.\n");
        return HI_ERR_VENC_NULL_PTR;
    }
    ...
    stRCType.hVencChn = hVencChn;
    stRCType.eRCType  = VENC_DRV_ControlRateMax;

    s32Ret = ioctl(g_VEncDevFd, CMD_VENC_GET_RC_TYPE, &stRCType);
    if (HI_SUCCESS != s32Ret)
    {
       HI_ERR_VENC("Drv Chan(%d) get rate control type failed! return 0x%x\n", stRCType.hVencChn, s32Ret);
    }
    else
    {
        *penRcType = convert_DrvToUnf_RcType(stRCType.eRCType);
    }

    return s32Ret;
}

static VENC_DRV_FRMRATETYPE_E convert_UnfToDrv_FrmRateType(HI_UNF_VENC_FRMRATE_TYPE_E eUnfFrmRateType)
{
    VENC_DRV_FRMRATETYPE_E retVal;

    switch (eUnfFrmRateType)
    {
        case HI_UNF_VENC_FRMRATE_TYPE_AUTO:
            retVal = VENC_DRV_FRMRATE_AUTO;
            break;
        case HI_UNF_VENC_FRMRATE_TYPE_STREAM:
            retVal = VENC_DRV_FRMRATE_STREAM;
            break;
        case HI_UNF_VENC_FRMRATE_TYPE_USER:
            retVal = VENC_DRV_FRMRATE_USER;
            break;
        default:
            retVal = VENC_DRV_FRMRATE_BUTT;
            break;
    }
...
static HI_UNF_VENC_FRMRATE_TYPE_E convert_DrvToUnf_FrmRateType(VENC_DRV_FRMRATETYPE_E eDrvFrmRateType)
{
    HI_UNF_VENC_FRMRATE_TYPE_E retVal;

    switch (eDrvFrmRateType)
    {
        case VENC_DRV_FRMRATE_AUTO:
            retVal = HI_UNF_VENC_FRMRATE_TYPE_AUTO;
            break;
        case VENC_DRV_FRMRATE_STREAM:
            retVal = HI_UNF_VENC_FRMRATE_TYPE_STREAM;
            break;
        case VENC_DRV_FRMRATE_USER:
            retVal = HI_UNF_VENC_FRMRATE_TYPE_USER;
            break;
        default:
            retVal = HI_UNF_VENC_FRMRATE_TYPE_BUTT;
            break;
    }
...
HI_S32 HI_MPI_VENC_SetInputFrmRateType(HI_HANDLE hVencChn, HI_UNF_VENC_FRMRATE_TYPE_E enInputFrmRateType)
{
    HI_S32 s32Ret, i = 0;
    VENC_INFO_FRMRATE_S stInputFrmRateType;

    memset(&stInputFrmRateType, 0, sizeof(VENC_INFO_FRMRATE_S));

    if (enInputFrmRateType >= HI_UNF_VENC_FRMRATE_TYPE_BUTT)
    {
        HI_ERR_VENC("enRcType = %d not support!\n",enInputFrmRateType);
        return HI_ERR_VENC_INVALID_PARA;
    }
    ...
    stInputFrmRateType.hVencChn = hVencChn;
    stInputFrmRateType.eFrmRateType = convert_UnfToDrv_FrmRateType(enInputFrmRateType);
    s32Ret = ioctl(g_VEncDevFd, CMD_VENC_SET_INPUT_FRMRATE_TYPE, &stInputFrmRateType);
    if (HI_SUCCESS != s32Ret)
    {
       HI_ERR_VENC("Drv Chan(%d) set input frameRate source type(%d) failed! return 0x%x\n", stInputFrmRateType.hVencChn, stInputFrmRateType.eFrmRateType, s32Ret);
    }

    return s32Ret;
}

HI_S32 HI_MPI_VENC_GetInputFrmRateType(HI_HANDLE hVencChn, HI_UNF_VENC_FRMRATE_TYPE_E *penInputFrmRateType)
{
    HI_S32 s32Ret, i = 0;
    VENC_INFO_FRMRATE_S stInputFrmRateType;
    ...
    if (HI_NULL == penInputFrmRateType)
    {
        HI_ERR_VENC("para penInputFrmRateType is NULL.\n");
        return HI_ERR_VENC_NULL_PTR;
    }
    ...
    stInputFrmRateType.hVencChn = hVencChn;
    stInputFrmRateType.eFrmRateType  = VENC_DRV_FRMRATE_BUTT;

    s32Ret = ioctl(g_VEncDevFd, CMD_VENC_GET_INPUT_FRMRATE_TYPE, &stInputFrmRateType);
    if (HI_SUCCESS != s32Ret)
    {
       HI_ERR_VENC("Drv Chan(%d) get input frameRate source type failed! return 0x%x\n", stInputFrmRateType.hVencChn, s32Ret);
    }
    else
    {
        *penInputFrmRateType = convert_DrvToUnf_FrmRateType(stInputFrmRateType.eFrmRateType);
    }
