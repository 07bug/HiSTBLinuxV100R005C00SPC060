...
HI_S32 VO_ConvertWinAttrToMPI(HI_UNF_WINDOW_ATTR_S* pUnfAttr, HI_DRV_WIN_ATTR_S* pstMpiAttr)
{
    ...
    if (pUnfAttr->enDisp >= HI_UNF_DISPLAY2 ||
        pUnfAttr->enVideoFormat >= HI_UNF_FORMAT_RGB_BUTT)
    {
        HI_FATAL_WIN("Para pUnfAttr is invalid!\n");
        return HI_ERR_VO_INVALID_PARA;
    }
...
HI_S32 VO_ConvertWinAttrToUNF(HI_DRV_WIN_ATTR_S* pstMpiAttr, HI_UNF_WINDOW_ATTR_S* pUnfAttr)
{
    ...
    if (pstMpiAttr->enDisp >= HI_DRV_DISPLAY_2 ||
        pstMpiAttr->enDataFormat >= HI_DRV_PIX_BUTT ||
        pstMpiAttr->enARCvrs >= HI_DRV_ASP_RAT_MODE_BUTT)
    {
        HI_FATAL_WIN("Para pstMpiAttr is invalid!\n");
        return HI_ERR_VO_INVALID_PARA;
    }
...
HI_S32 HI_UNF_VO_AcquireFrame(HI_HANDLE hWindow, HI_UNF_VIDEO_FRAME_INFO_S* pstFrameinfo, HI_U32 u32TimeoutMs)
{
    ...
    while (1)
    {
        ...
        if ((u32EndTime - u32StartTime) > u32TimeoutMs)
        {
            s32Ret = HI_ERR_VO_TIMEOUT;
            break;
        }
...
#ifdef ANDROID
        s32Ret = HI_MPI_WIN_ResetFirstFrmStatus(hWindow);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_WIN("Reset win first frame status failed!\n");
            return s32Ret;
        }
#endif
...
{
    HI_DRV_WIN_SWITCH_E eFrzMode = HI_DRV_WIN_SWITCH_BUTT;
    HI_S32 s32Ret;
...
{
    HI_DRV_WIN_SWITCH_E eRstMode = HI_DRV_WIN_SWITCH_BUTT;
    HI_S32 s32Ret;
...
{
    HI_DRV_VIDEO_BUFFER_POOL_S stBufPool = {0};
    HI_S32 s32Ret;
    ...
    while(1)
    {
        ...
        if (HI_SUCCESS == s32Ret)
        {
            ...
            if (HI_SUCCESS != s32Ret)
            {
                ...
                s32Ret = HI_ERR_VO_FRAME_INFO_ERROR;
            }
...
HI_S32 HI_UNF_VO_SetWindowAlpha(HI_HANDLE hWin,HI_U32 u32Alpha)
{
    return  HI_ERR_VO_WIN_UNSUPPORT;
}

HI_S32 HI_UNF_VO_GetWindowAlpha(HI_HANDLE hWin, HI_U32 *pu32Alpha)
{
    return HI_ERR_VO_WIN_UNSUPPORT;
}
...
HI_S32 HI_UNF_VO_GetWindowPlayInfo(HI_HANDLE hWindow, HI_UNF_WIN_PLAY_INFO_S *pstPlayInfo)
{
    ...
    memset((HI_VOID *)&stPlayInfo, 0, sizeof(stPlayInfo));

    s32Ret = HI_MPI_WIN_GetPlayInfo(hWindow, &stPlayInfo);
    if (!s32Ret)
    {
        pstPlayInfo->u32Underload = stPlayInfo.u32UnderLoad;
    }
...
{
    HI_DRV_WIN_PLAY_INFO_S stWinDelay;
    HI_S32 s32Ret  = HI_FAILURE;
    ...
    s32Ret = HI_MPI_VO_GetWindowDelay(hWindow, &stWinDelay);
    ...
    *pu32DelayMs = stWinDelay.u32DelayTime;
