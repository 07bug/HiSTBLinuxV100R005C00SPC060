...
static HI_S32 OSAL_CreateTask(HI_VOID** ppTask, const HI_PCHAR pTaskName, HI_S32 (*threadfn)(HI_VOID *data))
{
    struct task_struct* pKthread;
    pKthread = kthread_run(threadfn, HI_NULL, pTaskName);
    if (IS_ERR(pKthread))
    {
        return HI_FAILURE;
    }
...
static HI_BOOL OSAL_TaskShouldStop(HI_VOID)
{
...
HI_VOID ASDP_OSAL_InterfaceInit(ADSP_OSAL_FUNC_S* pstOsalFunc)
{
    ...
    pstOsalFunc->pfun_OSAL_adsp_task_should_stop      = OSAL_TaskShouldStop;
