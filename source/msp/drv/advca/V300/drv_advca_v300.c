...
static HI_S32 DRV_ADVCA_V300_Ioctl_CMD_CA_GET_CHIPID(HI_VOID* arg)
{
    ...
    else
    {
        ...
        *pu32ChipId = ((HI_U32)au8IDBuf[0]<<24) + ((HI_U32)au8IDBuf[1]<<16) + ((HI_U32)au8IDBuf[2]<<8) + au8IDBuf[3];
    }
...
static HI_S32 DRV_ADVCA_V300_Ioctl_CMD_CA_GET_REVISION(HI_VOID* arg)
{
    ...
    str = (CA_OTP_VENDOR_NAGRA == u32VendorId) ? C1_REVISION : ADVCA_REVISION;
    ...
    strncpy(pKey->KeyBuf, str, (strlen(str)+1) < sizeof(pKey->KeyBuf) ? strlen(str) : sizeof(pKey->KeyBuf) - 1);
