...
{
    reg_set(REG_TIMER23_BASE + TIMER_LOAD + 0x20, 0);
    reg_set(REG_TIMER23_BASE + TIMER_CONTROL + 0x20,
            TIMER_EN  | TIMER_MODE | TIMER_PRE | TIMER_SIZE);
}

void timer2_start(void)
{
    reg_set(REG_TIMER23_BASE + TIMER_LOAD + 0x20, 0xffffffff);
}

unsigned long timer2_get_val(void)
{
    return 0xffffffff - reg_get(REG_TIMER23_BASE + TIMER_VALUE + 0x20);
}
