...
int DRV_ADVCA_Open(struct inode *inode, struct file *filp)
{
    ...
    if (NULL == g_pOTPExportFunctionList)
    {
        ...
        if (HI_SUCCESS != ret)
        {
            HI_FATAL_CA("HI_DRV_MODULE_GetFunction failed!\n");
            return HI_FAILURE;
        }
...
/*****************************************************************************
 Prototype    :
 Description  : C3 Module interface for Demux Chipset Pairing
                It is to set DVB level 2 Input
 Input        : None
 Output       : None
 Return Value : None
...
HI_S32 DRV_ADVCA_DecryptCipher(HI_U32 AddrID, HI_U32 *pu32DataIn)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CA("Get r2r level failed!\n");
        goto RET;
    }
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CA("HAL_ADVCA_V300_CryptR2R failed:%x\n", ret);
        goto RET;
    }

#if defined(KLAD_TYPE_TEE_VERIMATRIX_ULTRA) && defined(CHIP_TYPE_hi3798mv200)

    ret = HAL_ADVCA_V300_R2R_CALLTEE();
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CA("HAL_ADVCA_V300_CALLTEE");
        goto RET;
    }
#endif
    ret = HI_SUCCESS;
...
HI_S32 DRV_ADVCA_DecryptSecStore(HI_U32 AddrID, HI_U32 *pu32DataIn)
{
    HI_S32 ret = HI_SUCCESS;
    ...
    ret = HAL_ADVCA_V300_DecryptSecStore( pu32DataIn, AddrID);
...
HI_S32 HI_DRV_ADVCA_Crypto(DRV_ADVCA_EXTFUNC_PARAM_S stParam)
{
    ...
    else if(HI_UNF_CIPHER_CA_TYPE_MISC == stParam.enCAType)  //transfer from Demux, cipher engine
    {
        ret = DRV_ADVCA_DecryptMisc(stParam.AddrID, stParam.EvenOrOdd, stParam.pu8Data, stParam.enTarget);
    }
    ...
    else if (HI_UNF_CIPHER_CA_TYPE_SECSTORE == stParam.enCAType)
    {
        ret = DRV_ADVCA_DecryptSecStore(stParam.AddrID, (HI_U32 *)stParam.pu8Data);
    }
    else
    {
...
/*****************************************************************************
 Prototype    :
 Description  : CA Module interface for C2 high level code authentication
 Input        : None
 Output       : None
 Return Value : None
*****************************************************************************/
