...
long AI_DRV_Ioctl(struct file* file, HI_U32 cmd, unsigned long arg)
{
    ...
    {
        HI_ERR_AI("lock g_AIMutex failed\n");
        return s32Ret;
    }

    s32Ret = HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, AI_ProcessCmd);
...
static HI_VOID AI_DRV_ProcSetDelay(AI_CHN_STATE_S* state, HI_U32 u32AIDelay)
{
    HI_S32 s32Ret;
    HI_U32 u32BufDelayMs = 0;
    HI_U32 u32Ai = state->hAI & AI_CHNID_MASK;
    ...
    stGetDelayComps.bDelayMsAutoHold = HI_FALSE;
...
HI_S32 AI_DRV_WriteProc(struct file* file, const char __user* buf, size_t count, loff_t* ppos)
{
    ...
    if (AI_CMD_PROC_SET_DELAY == enProcCmd)
    {
        ...
        AI_DRV_ProcSetDelay(state, u32AIDelay);
    }
