...
static  HI_VOID AO_SND_UpdateRenderRawInfo(HI_UNF_SND_E enSound, HI_UNF_SND_HDMI_MODE_E enMode);
...
static HI_VOID AOReleaseISBBuf(SND_CARD_STATE_S* pCard)
{
    HI_S32 s32IsbNum;

    for (s32IsbNum = 0; s32IsbNum < AO_RENDER_TRACK_NUM; s32IsbNum++)
    {
        if (s_stAoDrv.astSndEntity[HI_UNF_SND_0].stISBMmz[s32IsbNum].u32StartPhyAddr)
        {
            HI_DRV_MMZ_UnmapAndRelease(&s_stAoDrv.astSndEntity[HI_UNF_SND_0].stISBMmz[s32IsbNum]);
            s_stAoDrv.astSndEntity[HI_UNF_SND_0].stISBMmz[s32IsbNum].u32StartPhyAddr = 0;
        }
    }
}
...
static HI_S32 AO_SND_AOEInit(SND_CARD_STATE_S* pCard)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_LOG_ERR("AO_ENGINE_Init failed\n");
        HI_ERR_PrintErrCode(s32Ret);
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_LOG_ERR("Alloc Aip Buf failed\n");
        HI_ERR_PrintErrCode(s32Ret);
        goto ERR_EXIT0;
    }
...
static HI_S32 AO_SND_SetHdmiMode(HI_UNF_SND_E enSound, HI_UNF_SND_OUTPUTPORT_E enOutPort, HI_UNF_SND_HDMI_MODE_E enMode)
{
    ...
    s32Ret = SND_SetOpHdmiMode(pCard, enOutPort, enMode);
    if (HI_SUCCESS == s32Ret)
    {
        AO_SND_UpdateRenderRawInfo(enSound, enMode);
    }
...
static HI_S32 AO_SND_SetArcMode(HI_UNF_SND_E enSound, HI_UNF_SND_OUTPUTPORT_E enOutPort, HI_UNF_SND_ARC_AUDIO_MODE_E enMode)
{
    ...
    s32Ret = SND_SetOpArcMode(pCard, enOutPort, enMode);
    if (HI_SUCCESS == s32Ret)
    {
        AO_SND_UpdateRenderRawInfo(enSound, enMode);
    }
...
static HI_S32 AO_SND_SetConOutputEnable(HI_UNF_SND_E enSound, HI_BOOL bEnable)
{
    HI_S32 Ret;
    SND_CARD_STATE_S* pCard = SND_CARD_GetCard(enSound);
    SND_ENGINE_STATE_S* pstEnginestate = (SND_ENGINE_STATE_S*)pCard->pSndEngine[SND_ENGINE_TYPE_PCM];
    ...
    if (!pstEnginestate)
    {
        pCard->bConOutputEnable = bEnable;
        HI_INFO_AO("pcm engine do not exist!\n");
        return HI_SUCCESS;
    }

    Ret = HAL_AOE_ENGINE_SetConOutputEnable(pstEnginestate->enEngine, bEnable);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AO("HAL_AOE_ENGINE_SetConOutputEnable failed!\n");
        return Ret;
    }

    pstEnginestate = (SND_ENGINE_STATE_S*)pCard->pSndEngine[SND_ENGINE_TYPE_SPDIF_RAW];
    if (HI_NULL != pstEnginestate)
    {
        HAL_AOE_ENGINE_SetConOutputEnable(pstEnginestate->enEngine, bEnable);
    }

    pstEnginestate = (SND_ENGINE_STATE_S*)pCard->pSndEngine[SND_ENGINE_TYPE_HDMI_RAW];
    if (HI_NULL != pstEnginestate)
    {
        HAL_AOE_ENGINE_SetConOutputEnable(pstEnginestate->enEngine, bEnable);
    }
...
static HI_S32 AO_SND_GetConOutputEnable(HI_UNF_SND_E enSound, HI_BOOL* pbEnable)
{
    SND_CARD_STATE_S* pCard = SND_CARD_GetCard(enSound);

    *pbEnable = pCard->bConOutputEnable;
    return HI_SUCCESS;
}
...
static HI_S32 AO_SND_CreateISB(AO_SND_ISB_Param_S* pstIsb)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 i = pstIsb->enSound;
    HI_U32 u32MallocSize = 0;
    HI_CHAR* MmzName[] = {"AO_MainISB", "AO_AssocISB", "AO_ExtISB", "AO_OttISB", "AO_SysISB", "AO_TtsISB"};

    if (AO_RENDER_TRACK_NUM <= pstIsb->u32Type)
    {
        HI_ERR_AO("Invalid Isb Type(0x%x)!\n", pstIsb->u32Type);
        return HI_ERR_AO_INVALID_PARA;
    }

    if (0 != s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].u32StartPhyAddr)
    {
        HI_WARN_AO("Isb Type(0x%x) had been create ISB buffer\n", pstIsb->u32Type);
        return HI_SUCCESS;
    }

    /* malloc mmz buffer */
    s32Ret = HI_DRV_MMZ_AllocAndMap(MmzName[pstIsb->u32Type], MMZ_OTHERS, pstIsb->u32BufSize, AIAO_BUFFER_ADDR_ALIGN, &s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type]);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AO("HI_DRV_MMZ_AllocAndMap(%s) failed(0x%x)!\n", MmzName[pstIsb->u32Type], s32Ret);
        return HI_ERR_AO_MALLOC_FAILED;
    }

    u32MallocSize = s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].u32Size;

    if (pstIsb->u32BufSize > u32MallocSize)
    {
        HI_ERR_AO("Err: Want alloc size(%d), but real size(%d)!\n", pstIsb->u32BufSize, u32MallocSize);
        return HI_ERR_AO_MALLOC_FAILED;
    }

    pstIsb->u32BufPhyAddr = s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].u32StartPhyAddr;

    HI_WARN_AO("SourceID(%d) malloc buf phyaddr(0x%x), size(%d)\n", pstIsb->u32Type, s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].u32StartPhyAddr, u32MallocSize);

    memset(s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].pu8StartVirAddr, 0, u32MallocSize);
    return HI_SUCCESS;
}

static HI_S32 AO_SND_DestroyISB(AO_SND_ISB_Param_S* pstIsb)
{
    HI_U32 i = pstIsb->enSound;
    ...
    HI_WARN_AO("SourceID(%d) malloc buf phyaddr(0x%x), size(%d)\n", pstIsb->u32Type, s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].u32StartPhyAddr,
               s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].u32Size);

    if (s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].u32StartPhyAddr)
    {
        HI_DRV_MMZ_UnmapAndRelease(&s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type]);
        s_stAoDrv.astSndEntity[i].stISBMmz[pstIsb->u32Type].u32StartPhyAddr = 0;
    }

    return HI_SUCCESS;
}

static HI_S32 AO_SND_SetRenderParam(HI_UNF_SND_E enSound, AO_RENDER_ATTR_S* pstRenderAttr)
{
    SND_CARD_STATE_S* pCard = SND_CARD_GetCard(enSound);

    memcpy(&pCard->stRenderAttr, pstRenderAttr, sizeof(AO_RENDER_ATTR_S));

    return HI_SUCCESS;
}

static HI_S32 AO_SND_GetRenderParam(HI_UNF_SND_E enSound, AO_RENDER_ATTR_S* pstRenderAttr)
{
    SND_CARD_STATE_S* pCard = SND_CARD_GetCard(enSound);

    memcpy(pstRenderAttr, &pCard->stRenderAttr, sizeof(AO_RENDER_ATTR_S));

    return HI_SUCCESS;
}

static HI_S32 AO_SND_SetOutputAtmosEnable(HI_UNF_SND_E enSound, HI_BOOL bEnable)
{
    HI_S32 Ret;
    SND_CARD_STATE_S* pCard = SND_CARD_GetCard(enSound);
    SND_ENGINE_STATE_S* pstEnginestate = (SND_ENGINE_STATE_S*)pCard->pSndEngine[SND_ENGINE_TYPE_PCM];
    ...
    if (!pstEnginestate)
    {
        pCard->bOutputAtmosEnable = bEnable;
        HI_INFO_AO("pcm engine do not exist!\n");
        return HI_SUCCESS;
    }

    Ret = HAL_AOE_ENGINE_SetOutputAtmosEnable(pstEnginestate->enEngine, bEnable);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AO("HAL_AOE_ENGINE_SetOutputAtmosEnable failed!\n");
        return Ret;
    }
    ...
    if (HI_NULL != pstEnginestate)
    {
        HAL_AOE_ENGINE_SetOutputAtmosEnable(pstEnginestate->enEngine, bEnable);
    }
...
static HI_S32 AO_SND_GetOutputAtmosEnable(HI_UNF_SND_E enSound, HI_BOOL* pbEnable)
{
    SND_CARD_STATE_S* pCard = SND_CARD_GetCard(enSound);

    *pbEnable = pCard->bOutputAtmosEnable;
    return HI_SUCCESS;
}

static HI_S32 AO_SND_SetConOutputStatus(struct file* file, HI_UNF_SND_E enSound, HI_BOOL bEnable)
{
    SND_CARD_STATE_S* pCard = HI_NULL;
    ...
    pCard->stRenderAttr.bContinueStatus = bEnable;
    pCard->stRenderAttr.pstConOutputFp = file;

    return HI_SUCCESS;
}

static HI_S32 AO_SND_GetConOutputStatus(HI_UNF_SND_E enSound, HI_BOOL* pbEnable)
{
    SND_CARD_STATE_S* pCard;
    ...
    pCard = SND_CARD_GetCard(enSound);
    *pbEnable = pCard->stRenderAttr.bContinueStatus;

    return HI_SUCCESS;
}

static HI_S32 AO_SND_SetOutputLatencyMode(struct file* file, HI_UNF_SND_E enSound, HI_UNF_SND_OUTPUTLATENCY_E enOutputMode)
{
    SND_CARD_STATE_S* pCard = HI_NULL;
    CHECK_AO_SNDCARD_OPEN(enSound);
    ...
    pCard->stRenderAttr.enOutputMode = enOutputMode;
    pCard->stRenderAttr.pstConOutputFp = file;

    return HI_SUCCESS;
}

static HI_S32 AO_SND_GetOutputLatencyMode(HI_UNF_SND_E enSound, HI_UNF_SND_OUTPUTLATENCY_E* penOutputMode)
{
    SND_CARD_STATE_S* pCard = HI_NULL;
    CHECK_AO_SNDCARD_OPEN(enSound);

    pCard = SND_CARD_GetCard(enSound);
    *penOutputMode = pCard->stRenderAttr.enOutputMode;
...
static HI_VOID AO_SND_UpdateRenderRawInfo(HI_UNF_SND_E enSound, HI_UNF_SND_HDMI_MODE_E enMode)
{
    ...
    pCard->stRenderAttr.bPassthruBypass = (HI_UNF_SND_HDMI_MODE_RAW == enMode) ? HI_TRUE : HI_FALSE;
}
...
static HI_S32 AO_Track_SetFifoLatency(HI_U32 u32TrackID, HI_U32 u32LatencyMs)
{
    SND_CARD_STATE_S* pCard;
    u32TrackID &= AO_TRACK_CHNID_MASK;
    pCard = TRACK_CARD_GetCard(u32TrackID);
    ...
    if (pCard)
    {
        return TRACK_SetFifoLatency(pCard, u32TrackID, u32LatencyMs);
    }
    else
    {
        return HI_ERR_AO_SOUND_NOT_OPEN;
...
static HI_S32 AO_Track_GetFifoLatency(HI_U32 u32TrackID, HI_U32* pu32LatencyMs)
{
    SND_CARD_STATE_S* pCard;
    u32TrackID &= AO_TRACK_CHNID_MASK;
    pCard = TRACK_CARD_GetCard(u32TrackID);
    ...
    if (pCard)
    {
        return TRACK_GetFifoLatency(pCard, u32TrackID, pu32LatencyMs);
    }
    else
    {
        return HI_ERR_AO_SOUND_NOT_OPEN;
...
static HI_S32 AO_Track_SetResumeThresholdMs(HI_U32 u32TrackID, HI_U32 u32ThresholdMs)
{
    SND_CARD_STATE_S* pCard;
    u32TrackID &= AO_TRACK_CHNID_MASK;
    pCard = TRACK_CARD_GetCard(u32TrackID);
    ...
    return TRACK_SetResumeThresholdMs(pCard, u32TrackID, u32ThresholdMs);
}

static HI_S32 AO_Track_GetResumeThresholdMs(HI_U32 u32TrackID, HI_U32* pu32ThresholdMs)
{
    SND_CARD_STATE_S* pCard;
    u32TrackID &= AO_TRACK_CHNID_MASK;
    pCard = TRACK_CARD_GetCard(u32TrackID);
    ...
    return TRACK_GetResumeThresholdMs(pCard, u32TrackID, pu32ThresholdMs);
}

static HI_S32 AO_Track_SetFadeAttr(HI_U32 u32TrackID, AO_TRACK_FADE_S* pstTrackFade)
{
    HI_S32 s32Ret;
    SND_CARD_STATE_S* pCard;
    u32TrackID &= AO_TRACK_CHNID_MASK;
    ...
    s32Ret = TRACK_SetFadeAttr(pCard, u32TrackID, pstTrackFade);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AO("call TRACK_SetFadeAttr failed(0x%x)\n", s32Ret);
        return s32Ret;
    }
...
static HI_S32 AO_Track_SetInfo(HI_U32 u32TrackID, HI_VIRT_ADDR_T tVirAddr)
{
    SND_CARD_STATE_S* pCard;
    u32TrackID &= AO_TRACK_CHNID_MASK;
    ...
    return TRACK_SetInfo(pCard, u32TrackID, tVirAddr);
}

static HI_S32 AO_Track_GetInfo(HI_U32 u32TrackID, HI_VIRT_ADDR_T* ptVirAddr, HI_PHYS_ADDR_T* ptPhyAddr)
{
    SND_CARD_STATE_S* pCard;
    u32TrackID &= AO_TRACK_CHNID_MASK;
    ...
    return TRACK_GetInfo(pCard, u32TrackID, ptVirAddr, ptPhyAddr);
}
...
static HI_S32 SndProcSaveAefThread(void* Arg)
{
    ...
    (HI_VOID)AO_Cast_SetAefBypass(stCastParam.hCast, pstThreadArg->bAefBypass);//ӦΪfalse
...
static HI_VOID AOContinueClose(struct file* filp)
{
    SND_CARD_STATE_S* pCard = HI_NULL;

    if (!s_stAoDrv.astSndEntity[HI_UNF_SND_0].pCard)
    {
        return;
    }

    pCard = s_stAoDrv.astSndEntity[HI_UNF_SND_0].pCard;

    if (pCard->stRenderAttr.pstConOutputFp != filp)
    {
        return;
    }

    pCard->stRenderAttr.bContinueStatus = HI_FALSE;
    pCard->stRenderAttr.enOutputMode = HI_UNF_SND_OUTPUTLATENCY_NORMAL;
}
...
static HI_S32 AO_CloseDev(HI_VOID)
{
    ...
    if (s_stAoDrv.astSndEntity[HI_UNF_SND_0].pCard)
    {
        AOContinueClose(s_stAoDrv.astSndEntity[HI_UNF_SND_0].pCard->stRenderAttr.pstConOutputFp);
    }
...
#ifdef __DPT__
                    AO_SND_UpdateTrackInfo(u32TrackIdx, HI_FALSE);
#endif
...
static HI_S32 AO_IOCTL_SndSetContinueOutput(struct file* file, HI_VOID* arg)
{
    AO_SND_ConOutput_Enable_S_PTR pstParam = (AO_SND_ConOutput_Enable_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstParam->enSound);
    return AO_SND_SetConOutputEnable(pstParam->enSound, pstParam->bConOutputEnable);
}

static HI_S32 AO_IOCTL_SndGetContinueOutput(struct file* file, HI_VOID* arg)
{
    AO_SND_ConOutput_Enable_S_PTR pstParam = (AO_SND_ConOutput_Enable_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstParam->enSound);
    return AO_SND_GetConOutputEnable(pstParam->enSound, &pstParam->bConOutputEnable);
}

static HI_S32 AO_IOCTL_SndCreateSB(struct file* file, HI_VOID* arg)
{
    AO_SND_ISB_Param_S_PTR pstParam = (AO_SND_ISB_Param_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstParam->enSound);
    return AO_SND_CreateISB(pstParam);
}

static HI_S32 AO_IOCTL_SndDestroySB(struct file* file, HI_VOID* arg)
{
    AO_SND_ISB_Param_S_PTR pstParam = (AO_SND_ISB_Param_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstParam->enSound);
    return AO_SND_DestroyISB(pstParam);
}

static HI_S32 AO_IOCTL_SndSetRenderParam(struct file* file, HI_VOID* arg)
{
    AO_SND_Render_Param_S_PTR pstParam = (AO_SND_Render_Param_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstParam->enSound);
    return AO_SND_SetRenderParam(pstParam->enSound, &pstParam->stRenderAttr);
}

static HI_S32 AO_IOCTL_SndGetRenderParam(struct file* file, HI_VOID* arg)
{
    AO_SND_Render_Param_S_PTR pstParam = (AO_SND_Render_Param_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstParam->enSound);
    return AO_SND_GetRenderParam(pstParam->enSound, &pstParam->stRenderAttr);
}

static HI_S32 AO_IOCTL_SndSetOutputAtmosEnable(struct file* file, HI_VOID* arg)
{
    AO_SND_OutputAtmos_Enable_S_PTR pstParam = (AO_SND_OutputAtmos_Enable_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstParam->enSound);
    return AO_SND_SetOutputAtmosEnable(pstParam->enSound, pstParam->bOutputAtmosEnable);
}

static HI_S32 AO_IOCTL_SndGetOutputAtmosEnable(struct file* file, HI_VOID* arg)
{
    AO_SND_OutputAtmos_Enable_S_PTR pstParam = (AO_SND_OutputAtmos_Enable_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstParam->enSound);
    return AO_SND_GetOutputAtmosEnable(pstParam->enSound, &pstParam->bOutputAtmosEnable);
}

static HI_S32 AO_IOCTL_SetConOutputStatus(struct file* file, HI_VOID* arg)
{
    AO_SND_ConOutput_Enable_S_PTR pstConOutput = (AO_SND_ConOutput_Enable_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstConOutput->enSound);
    return AO_SND_SetConOutputStatus(file, pstConOutput->enSound, pstConOutput->bConOutputEnable);
}

static HI_S32 AO_IOCTL_GetConOutputStatus(struct file* file, HI_VOID* arg)
{
    AO_SND_ConOutput_Enable_S_PTR pstConOutput = (AO_SND_ConOutput_Enable_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstConOutput->enSound);
    return AO_SND_GetConOutputStatus(pstConOutput->enSound, &pstConOutput->bConOutputEnable);
}

static HI_S32 AO_IOCTL_SetOutputLatencyMode(struct file* file, HI_VOID* arg)
{
    AO_SND_OutputLatency_Mode_S_PTR pstOutput = (AO_SND_OutputLatency_Mode_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstOutput->enSound);
    return AO_SND_SetOutputLatencyMode(file, pstOutput->enSound, pstOutput->enOutputMode);
}

static HI_S32 AO_IOCTL_GetOutputLatencyMode(struct file* file, HI_VOID* arg)
{
    AO_SND_OutputLatency_Mode_S_PTR pstOutput = (AO_SND_OutputLatency_Mode_S_PTR)arg;
    CHECK_AO_SNDCARD_OPEN(pstOutput->enSound);
    return AO_SND_GetOutputLatencyMode(pstOutput->enSound, &pstOutput->enOutputMode);
}
...
static HI_S32 AO_IOCTL_TrackSetFifoLatency(struct file* file, HI_VOID* arg)
{
    AO_Track_Fifo_Latency_S_PTR pstFifoLatency = (AO_Track_Fifo_Latency_S_PTR)arg;
    CHECK_AO_TRACK_OPEN(pstFifoLatency->hTrack);
    return AO_Track_SetFifoLatency(pstFifoLatency->hTrack, pstFifoLatency->u32LatencyMs);
}

static HI_S32 AO_IOCTL_TrackGetFifoLatency(struct file* file, HI_VOID* arg)
{
    AO_Track_Fifo_Latency_S_PTR pstFifoLatency = (AO_Track_Fifo_Latency_S_PTR)arg;
    CHECK_AO_TRACK_OPEN(pstFifoLatency->hTrack);
    return AO_Track_GetFifoLatency(pstFifoLatency->hTrack, &pstFifoLatency->u32LatencyMs);
}
...
static HI_S32 AO_IOCTL_TrackSetResumeThresholdMs(struct file* file, HI_VOID* arg)
{
    AO_Track_ResumeThreshold_S_PTR pstParam = (AO_Track_ResumeThreshold_S_PTR)arg;
    CHECK_AO_TRACK_OPEN(pstParam->hTrack);
    return AO_Track_SetResumeThresholdMs(pstParam->hTrack, pstParam->u32ThresholdMs);
}

static HI_S32 AO_IOCTL_TrackGetResumeThresholdMs(struct file* file, HI_VOID* arg)
{
    AO_Track_ResumeThreshold_S_PTR pstParam = (AO_Track_ResumeThreshold_S_PTR)arg;
    CHECK_AO_TRACK_OPEN(pstParam->hTrack);
    return AO_Track_GetResumeThresholdMs(pstParam->hTrack, &pstParam->u32ThresholdMs);
}

static HI_S32 AO_IOCTL_TrackSetFadeAttr(struct file* file, HI_VOID* arg)
{
    AO_Track_Fade_Param_S_PTR pstTrackFade = (AO_Track_Fade_Param_S_PTR)arg;
    CHECK_AO_TRACK_OPEN(pstTrackFade->hTrack);
    return AO_Track_SetFadeAttr(pstTrackFade->hTrack, &pstTrackFade->stFade);
}

static HI_S32 AO_IOCTL_TrackSetInfo(struct file* file, HI_VOID* arg)
{
    AO_Track_Info_Param_S_PTR pstTrackInfo = (AO_Track_Info_Param_S_PTR)arg;
    CHECK_AO_TRACK_OPEN(pstTrackInfo->hTrack);
    return AO_Track_SetInfo(pstTrackInfo->hTrack, pstTrackInfo->tUserVirtAddr);
}

static HI_S32 AO_IOCTL_TrackGetInfo(struct file* file, HI_VOID* arg)
{
    AO_Track_Info_Param_S_PTR pstTrackInfo = (AO_Track_Info_Param_S_PTR)arg;
    CHECK_AO_TRACK_OPEN(pstTrackInfo->hTrack);
    return AO_Track_GetInfo(pstTrackInfo->hTrack, &pstTrackInfo->tUserVirtAddr, &pstTrackInfo->tPhyAddr);
}
...
} g_fnAoIoctl[CMD_ID_AO_BUTT] =
{
    ...
    [CMD_ID_AO_SND_SETCONOUTPUTENABLE]   = {CMD_AO_SND_SETCONOUTPUTENABLE   , AO_IOCTL_SndSetContinueOutput},
    [CMD_ID_AO_SND_GETCONOUTPUTENABLE]   = {CMD_AO_SND_GETCONOUTPUTENABLE   , AO_IOCTL_SndGetContinueOutput},

    [CMD_ID_AO_SND_CREATESB]             = {CMD_AO_SND_CREATESB             , AO_IOCTL_SndCreateSB},
    [CMD_ID_AO_SND_DESTROYSB]            = {CMD_AO_SND_DESTROYSB            , AO_IOCTL_SndDestroySB},

    [CMD_ID_AO_SND_SETRENDERPARAM]       = {CMD_AO_SND_SETRENDERPARAM       , AO_IOCTL_SndSetRenderParam},
    [CMD_ID_AO_SND_GETRENDERPARAM]       = {CMD_AO_SND_GETRENDERPARAM       , AO_IOCTL_SndGetRenderParam},

    [CMD_ID_AO_SND_SETOUTPUTATMOSENABLE] = {CMD_AO_SND_SETOUTPUTATMOSENABLE , AO_IOCTL_SndSetOutputAtmosEnable},
    [CMD_ID_AO_SND_GETOUTPUTATMOSENABLE] = {CMD_AO_SND_GETOUTPUTATMOSENABLE , AO_IOCTL_SndGetOutputAtmosEnable},

    [CMD_ID_AO_SND_SETCONOUTPUTSTATUS]   = {CMD_AO_SND_SETCONOUTPUTSTATUS   , AO_IOCTL_SetConOutputStatus},
    [CMD_ID_AO_SND_GETCONOUTPUTSTATUS]   = {CMD_AO_SND_GETCONOUTPUTSTATUS   , AO_IOCTL_GetConOutputStatus},

    [CMD_ID_AO_SND_SETOUTPUTLATENCYMODE] = {CMD_AO_SND_SETOUTPUTLATENCYMODE , AO_IOCTL_SetOutputLatencyMode},
    [CMD_ID_AO_SND_GETOUTPUTLATENCYMODE] = {CMD_AO_SND_GETOUTPUTLATENCYMODE , AO_IOCTL_GetOutputLatencyMode},
...
#endif
    [CMD_ID_AO_TRACK_SETFIFOLATENCY]     = {CMD_AO_TRACK_SETFIFOLATENCY     , AO_IOCTL_TrackSetFifoLatency},
    [CMD_ID_AO_TRACK_GETFIFOLATENCY]     = {CMD_AO_TRACK_GETFIFOLATENCY     , AO_IOCTL_TrackGetFifoLatency},
    ...
    [CMD_ID_AO_TRACK_SETRESUMETHRESHOLD] = {CMD_AO_TRACK_SETRESUMETHRESHOLD , AO_IOCTL_TrackSetResumeThresholdMs},
    [CMD_ID_AO_TRACK_GETRESUMETHRESHOLD] = {CMD_AO_TRACK_GETRESUMETHRESHOLD , AO_IOCTL_TrackGetResumeThresholdMs},

    [CMD_ID_AO_TRACK_SETFADEATTR]        = {CMD_AO_TRACK_SETFADEATTR        , AO_IOCTL_TrackSetFadeAttr},
    [CMD_ID_AO_TRACK_SETINFO]            = {CMD_AO_TRACK_SETINFO            , AO_IOCTL_TrackSetInfo},
    [CMD_ID_AO_TRACK_GETINFO]            = {CMD_AO_TRACK_GETINFO            , AO_IOCTL_TrackGetInfo},
...
#endif
    memcpy(&pstSndSettings->stRenderAttr, &pCard->stRenderAttr, sizeof(AO_RENDER_ATTR_S));
...
static HI_S32 AO_SND_RestoreSettings(HI_UNF_SND_E enSound, SND_CARD_SETTINGS_S* pstSndSettings)
{
    ...
    CHECK_AO_NULL_PTR(pCard);
    CHECK_AO_NULL_PTR(pstSndSettings);
    memcpy(&pCard->stRenderAttr, &pstSndSettings->stRenderAttr, sizeof(AO_RENDER_ATTR_S));
    ...
    pCard->bConOutputEnable = pstSndSettings->bConOutputEnable;
    s32Ret = AO_SND_SetConOutputEnable(enSound, pCard->bConOutputEnable);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AO("call SetConOutputEnable failed!\n");
    }

    pCard->bOutputAtmosEnable = pstSndSettings->bOutputAtmosEnable;
    s32Ret = AO_SND_SetOutputAtmosEnable(enSound, pCard->bOutputAtmosEnable);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AO("call SetOutputAtmosEnable failed!\n");
    }
