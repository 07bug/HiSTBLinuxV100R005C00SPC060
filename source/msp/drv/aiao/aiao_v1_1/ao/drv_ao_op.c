...
HI_S32 SND_GetPortInfo(SND_CARD_STATE_S* pCard, HI_UNF_SND_OUTPUTPORT_E enOutPort, SND_PORT_KERNEL_ATTR_S* pstPortKAttr)
{
    ...
    for (u32PortNum = 0; u32PortNum < HI_UNF_SND_OUTPUTPORT_MAX; u32PortNum++)
    {
        ...
        else
        {
            ...
            enPortID = state->enPortID[state->ActiveId];
            s32Ret = HAL_AIAO_P_GetRbfAttr(enPortID, &stRbfAttr);
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_AO("call HAL_AIAO_P_GetRbfAttr failed(0x%x)\n", s32Ret);
                return s32Ret;
...
#endif
    pstSndSettings->bConOutputEnable = pCard->bConOutputEnable;
    pstSndSettings->bOutputAtmosEnable = pCard->bOutputAtmosEnable;
