 ...
 * main\1    2012-09-22   sdk     NULL         init.
 ********************************************************************************/
...
static HI_U32 UTIL_CalcFrameSize(HI_U32 Ch, HI_U32 BitDepth)
{
    HI_U32 FrameSize = 0;
    ...
    {
        case 16:
            FrameSize = Ch * sizeof(HI_U16);
            break;
        case 24:
            FrameSize = Ch * sizeof(HI_U32);
            break;
        default:
            break;
    }
...
HI_S32  HAL_AOE_AIP_Create(AOE_AIP_ID_E* penAIP, AOE_AIP_CHN_ATTR_S* pstAttr)
{
    ...
    Ret = HAL_AOE_AIP_SetAttr(enAIP, pstAttr);
    if (HI_SUCCESS != Ret)
    {
    ...
    state->aip = enAIP;
    state->bDataFromKernel = HI_FALSE;
...
HI_S32 HAL_AOE_AIP_Group_Start(HI_U32 u32StartMask)
{
    HI_U32 u32AipId = 0;
    HI_U32 u32CmdDone = 0;
    AOE_AIP_CHN_STATE_S* state[AOE_MAX_AIP_NUM] = {HI_NULL};

    u32CmdDone = iHAL_AOE_AIP_Group_SetCmd(AOE_AIP_CMD_START, u32StartMask);

    for (u32AipId = 0; u32AipId < AOE_MAX_AIP_NUM; u32AipId++)
    {
        if ((1 << u32AipId) & u32StartMask)
        {
            if ((1 << u32AipId) & u32CmdDone)
            {
                state[u32AipId] = (AOE_AIP_CHN_STATE_S*)g_AoeRm.pAip[u32AipId];
                AOEAIPFlushState(state[u32AipId]);
                state[u32AipId]->enCurnStatus = AOE_AIP_STATUS_START;
            }
            else
            {
                HI_ERR_AO("Err: AIP(%d) track start failed!\n", u32AipId);
            }
        }
    }

    if (u32CmdDone == u32StartMask)
    {
        return HI_SUCCESS;
    }
    else
...
HI_S32 HAL_AOE_AIP_SetFadeAttr(AOE_AIP_ID_E enAIP, HI_BOOL bEnable, HI_U32 u32FadeinMs, HI_U32 u32FadeOutMs)
{
    AOE_AIP_CHN_STATE_S* state = HI_NULL;
    ...
    iHAL_AOE_AIP_SetFade(enAIP, bEnable, u32FadeinMs, u32FadeOutMs);
    state->stUserAttr.stBufInAttr.bFadeEnable = bEnable;
...
HI_U32 HAL_AOE_AIP_WriteBufData(AOE_AIP_ID_E enAIP, HI_U8* pu32Src, HI_U32 u32SrcBytes)
{
    ...
    Bytes = CIRC_BUF_Write(&state->stCB, pu32Src, u32SrcBytes, state->bDataFromKernel);
    return Bytes;
}
...
HI_S32 HAL_AOE_AIP_SetMixPriority(AOE_AIP_ID_E enAIP, HI_BOOL bPriority)
{
    ...
    {
        HI_ERR_AO("iHAL_AOE_AIP_SetMixPriority Failed !\n");
    }
    return Ret;
}

HI_S32 HAL_AOE_AIP_GetFifoLatency(AOE_AIP_ID_E enAIP, HI_U32* pu32LatencyMs)
{
    AOE_AIP_CHN_STATE_S* state = HI_NULL;
    if (HI_NULL == g_AoeRm.pAip[enAIP])
    {
    ...
    state = (AOE_AIP_CHN_STATE_S*)g_AoeRm.pAip[enAIP];
    *pu32LatencyMs = state->stUserAttr.stFifoOutAttr.u32FiFoLatencyThdMs;

    return HI_SUCCESS;
}

HI_S32 HAL_AOE_AIP_SetFifoLatency(AOE_AIP_ID_E enAIP, HI_U32 u32LatencyMs)
{
    AOE_AIP_CHN_STATE_S* state = HI_NULL;
    if (HI_NULL == g_AoeRm.pAip[enAIP])
    {
    ...
    state = (AOE_AIP_CHN_STATE_S*)g_AoeRm.pAip[enAIP];
    iHAL_AOE_AIP_SetFifoLatency(enAIP, u32LatencyMs);

    state->stUserAttr.stFifoOutAttr.u32FiFoLatencyThdMs = u32LatencyMs;
...
HI_VOID HAL_AOE_AIP_SetDataSource(AOE_AIP_ID_E enAIP, HI_BOOL bDataFromKernel)
{
    ...
    state->bDataFromKernel = bDataFromKernel;
}

HI_VOID HAL_AOE_AIP_SetEos(AOE_AIP_ID_E enAIP, HI_BOOL bEos)
{
    iHAL_AOE_AIP_SetEos(enAIP, bEos);
}

HI_S32 HAL_AOE_AIP_SetResumeThresholdMs(AOE_AIP_ID_E enAIP, HI_U32 u32ThresholdMs)
{
    AOE_AIP_CHN_STATE_S* state = (AOE_AIP_CHN_STATE_S*)g_AoeRm.pAip[enAIP];

    CHECK_AIP_OPEN(enAIP);

    iHAL_AOE_AIP_SetResumeThresholdMs(enAIP, u32ThresholdMs);

    state->stUserAttr.stBufInAttr.u32ResumeThresholdMs = u32ThresholdMs;

    return HI_SUCCESS;
}

HI_S32 HAL_AOE_AIP_GetResumeThresholdMs(AOE_AIP_ID_E enAIP, HI_U32* pu32ThresholdMs)
{
    AOE_AIP_CHN_STATE_S* state = (AOE_AIP_CHN_STATE_S*)g_AoeRm.pAip[enAIP];

    CHECK_AIP_OPEN(enAIP);

    *pu32ThresholdMs = state->stUserAttr.stBufInAttr.u32ResumeThresholdMs;
...
HI_S32 HAL_AOE_ENGINE_Create(AOE_ENGINE_ID_E* penENGINE, AOE_ENGINE_CHN_ATTR_S* pstAttr)
{
    ...
    Ret = HAL_AOE_ENGINE_SetAttr(enEngine, pstAttr);
    if (HI_SUCCESS != Ret)
    {
        goto Engine_Create_ERR_EXIT;
    }
...
HI_S32 HAL_AOE_ENGINE_SetAvcAttr(AOE_ENGINE_ID_E enENGINE, HI_UNF_SND_AVC_ATTR_S* pstAvcAttr)
{
    ...
    Ret = iHAL_AOE_ENGINE_SetAvcChangeAttr(enENGINE, HI_TRUE);
    if (HI_SUCCESS != Ret)
    {
        return Ret;
    }
...
HI_S32 HAL_AOE_ENGINE_SetConOutputEnable(AOE_ENGINE_ID_E enENGINE, HI_BOOL bEnable)
{
    AOE_ENGINE_CHN_STATE_S* state = HI_NULL;
    CHECK_ENGINE_OPEN(enENGINE);
    state = (AOE_ENGINE_CHN_STATE_S*)g_AoeRm.pMix[enENGINE];
    return iHAL_AOE_ENGINE_SetConOutputEnable(enENGINE, bEnable);
}

HI_S32 HAL_AOE_ENGINE_SetOutputAtmosEnable(AOE_ENGINE_ID_E enENGINE, HI_BOOL bEnable)
{
    AOE_ENGINE_CHN_STATE_S* state = HI_NULL;
    CHECK_ENGINE_OPEN(enENGINE);
    state = (AOE_ENGINE_CHN_STATE_S*)g_AoeRm.pMix[enENGINE];
    iHAL_AOE_ENGINE_SetOutputAtmosEnable(enENGINE, bEnable);
    return HI_SUCCESS;
}
...
HI_S32 HAL_AOE_ENGINE_Group_Start(HI_U32 u32StartMask)
{
    HI_U32 u32EngineId = 0;
    HI_U32 u32CmdDone = 0;
    AOE_ENGINE_CHN_STATE_S* state[AOE_MAX_ENGINE_NUM] = {HI_NULL};

    u32CmdDone = iHAL_AOE_ENGINE_Group_SetCmd(AOE_ENGINE_CMD_START, u32StartMask);

    for (u32EngineId = 0; u32EngineId < AOE_MAX_ENGINE_NUM; u32EngineId++)
    {
        if ((1 << u32EngineId) & u32StartMask)
        {
            if ((1 << u32EngineId) & u32CmdDone)
            {
                state[u32EngineId] = (AOE_ENGINE_CHN_STATE_S*)g_AoeRm.pMix[u32EngineId];
                state[u32EngineId]->enCurnStatus = AOE_ENGINE_STATUS_START;
            }
            else
            {
                HI_ERR_AO("Err: Engine(%d) start failed!\n", u32EngineId);
            }
        }
    }
