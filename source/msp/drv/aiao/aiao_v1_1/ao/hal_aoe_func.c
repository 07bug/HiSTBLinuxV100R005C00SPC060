...
{
    *tPhyDelayAddr = g_stAoeFwInfo.tAoeRegPhyAddr + AOE_AIP_REG_OFFSET + AOE_AIP_REG_BANDSIZE * aip + offsetof(S_AIP_REGS_TYPE, AIP_STATUS0);
}
...
HI_VOID AoeAipBufSetAddr(S_AIP_REGS_TYPE* AipReg, AOE_AIP_INBUF_ATTR_S* pstBufInAttr)
{
    ...
    AipReg->AIP_EXT3_CTRL.bits.start_threshold = pstBufInAttr->u32StartThresholdMs;
    AipReg->AIP_EXT3_CTRL.bits.resume_threshold = pstBufInAttr->u32ResumeThresholdMs;
...
{
    S_AIP_REGS_TYPE* AipReg = (S_AIP_REGS_TYPE*)g_pAipReg[enAIP];
    AipReg->AIP_FIFO_ATTR.bits.fifo_latency = u32LatencyMs;
}

HI_VOID iHAL_AOE_AIP_UpdateLatency(AOE_AIP_ID_E enAIP, HI_U32 u32Datams)
{
    S_AIP_REGS_TYPE* AipReg = (S_AIP_REGS_TYPE*)g_pAipReg[enAIP];
    AipReg->AIP_STATUS0.bits.track_delayms += u32Datams;
}
...
HI_VOID iHAL_AOE_AIP_SetFade(AOE_AIP_ID_E enAIP, HI_BOOL bEnable, HI_U32 u32FadeinMs, HI_U32 u32FadeOutMs)
{
    ...
    AipReg->AIP_CTRL.bits.fade_en = bEnable;
}
...
{
    S_AIP_REGS_TYPE* AipReg = (S_AIP_REGS_TYPE*)g_pAipReg[enAIP];
    AipReg->AIP_EXT3_CTRL.bits.eos = !!bEos;
}

HI_VOID iHAL_AOE_AIP_SetResumeThresholdMs(AOE_AIP_ID_E enAIP, HI_U32 u32ThresholdMs)
{
    S_AIP_REGS_TYPE* AipReg = (S_AIP_REGS_TYPE*)g_pAipReg[enAIP];
    AipReg->AIP_EXT3_CTRL.bits.resume_threshold = u32ThresholdMs;
}
...
HI_U32 iHAL_AOE_ENGINE_Group_AckCmd(HI_U32 u32AckCmdMask)
{
    S_MIXER_REGS_TYPE* MixerReg[AOE_MAX_ENGINE_NUM] = {HI_NULL};
    HI_U32 u32CmdDoneMask = 0;
    ...
    {
        if ((1 << u32EngineId) & u32AckCmdMask)
        {
            MixerReg[u32EngineId] = (S_MIXER_REGS_TYPE*)g_pAipReg[u32EngineId];
        }
    }
    ...
    for (u32LoopOuter = 0; u32LoopOuter < 100; u32LoopOuter++)
    {
        for (u32LoopInner = 0; u32LoopInner < 8; u32LoopInner++)
        {
            for (u32EngineId = 0; u32EngineId < AOE_MAX_ENGINE_NUM; u32EngineId++)
            {
                if ((1 << u32EngineId) & u32AckCmdMask)
                {
                    if (MixerReg[u32EngineId]->ENGINE_CTRL.bits.cmd_done)
                    {
                        u32CmdDoneMask |= (1 << u32EngineId);
                    }
                }
            }
...
HI_U32 iHAL_AOE_ENGINE_Group_SetCmd(AOE_ENGINE_ID_E newcmd, HI_U32 u32SetCmdMask)
{
    ...
    for (u32EngineId = 0; u32EngineId < AOE_MAX_ENGINE_NUM; u32EngineId++)
    {
        if ((1 << u32EngineId) & u32SetCmdMask)
        {
            MixerReg[u32EngineId] = (S_MIXER_REGS_TYPE*)g_pAipReg[u32EngineId];
            MixerReg[u32EngineId]->ENGINE_CTRL.bits.cmd = newcmd;
            MixerReg[u32EngineId]->ENGINE_CTRL.bits.cmd_done = 0;
        }
    }

    return iHAL_AOE_ENGINE_Group_AckCmd(u32SetCmdMask);
}
...
{
    S_MIXER_REGS_TYPE* MixerReg = (S_MIXER_REGS_TYPE*)g_pMixerReg[enEngine];
    MixerReg->ENGINE_CTRL.bits.ena_continue_output = !!bEnable;
    return HI_SUCCESS;
}
...
{
    S_MIXER_REGS_TYPE* MixerReg = (S_MIXER_REGS_TYPE*)g_pMixerReg[enEngine];
    MixerReg->ENGINE_CTRL.bits.ena_output_atmos = !!bEnable;
}
