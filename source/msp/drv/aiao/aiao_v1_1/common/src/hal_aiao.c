...
HI_VOID    HAL_AIAO_PowerOff(HI_VOID)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_FATAL_AIAO("lock g_HalAiaoMutex failed\n");
        return;
    }
...
HI_S32    HAL_AIAO_PowerOn(HI_VOID)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_FATAL_AIAO("call AIAO_HW_PowerOn failed(0x%x)\n", Ret);
    }
...
HI_S32    HAL_AIAO_RequestIsr(HI_VOID)
{
    ...
    {
        HI_FATAL_AIAO("osal_request_irq failed irq num =%d!\n", AIAO_IRQ_NUM);
        return HI_FAILURE;
    }
...
HI_S32                  HAL_AIAO_Init(HI_VOID)
{
    ...
    if (atomic_inc_return(&atmAiaoInitCnt) == 1)
    {
        ...
        {
            HI_FATAL_AIAO("osal_request_irq failed\n");
            up(&g_HalAiaoMutex);
            return HI_FAILURE;
        }
...
HI_U32                  HAL_AIAO_GetTopIntRawStatus(HI_VOID)
{
    ...
    u32IntRaw = iHAL_AIAO_GetTopIntRawStatus();
    up(&g_HalAiaoMutex);
    return u32IntRaw;
}
...
HI_U32                  HAL_AIAO_GetTopIntStatus(HI_VOID)
{
    ...
    u32Int = iHAL_AIAO_GetTopIntStatus();
    up(&g_HalAiaoMutex);
    return u32Int;
}
...
HI_S32 HAL_AIAO_P_SetAttr(AIAO_PORT_ID_E enPortID, AIAO_PORT_ATTR_S* pstAttr)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AIAO("AIAOCheckBoardI2SOpenAttr failed(0x%x)!\n", s32Ret);
        up(&g_HalAiaoMutex);
        return s32Ret;
    }
    ...
    if (g_AIAORm.pPort[Id])
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_AIAO("iHAL_AIAO_P_SetAttr failed(0x%x)!\n", s32Ret);
            up(&g_HalAiaoMutex);
            return s32Ret;
        }
    }
...
HI_S32  HAL_AIAO_P_Open_Veri(AIAO_PORT_ID_E enPortID, const AIAO_PORT_USER_CFG_S* pstConfig)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AIAO("HAL_AIAO_P_Check failed(0x%x)!\n", s32Ret);
        return s32Ret;
    }
    ...
    if (HI_NULL != g_AIAORm.pPort[Id])
    {
        HI_ERR_AIAO("AIAO Port(%d) had been open!\n", Id);
        up(&g_HalAiaoMutex);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AIAO("iHAL_AIAO_P_Open failed(0x%x)!\n", s32Ret);
        up(&g_HalAiaoMutex);
        return s32Ret;
    }
...
{
    AIAO_I2S_BOARD_CONFIG* pstI2SBorad = HI_NULL;
...
HI_VOID HAL_AIAO_P_SetBoardI2SOpenAttr(AIAO_PORT_ID_E enPortID, const AIAO_IfAttr_S* pstNewIfAttr)
{
    ...
    if (HI_NULL == pstNewIfAttr || HI_NULL == pstOldIfAttr)
    {
        return;
    }
