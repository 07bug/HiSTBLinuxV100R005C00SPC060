...
static HI_S32 PortBufInit(AIAO_PORT_S* pstPort)
{
    ...
    HI_U32 *pu32WptrAddr = HI_NULL, *pu32RptrAddr = HI_NULL;
    ...
    if (HI_TRUE == pstConfig->bExtDmaMem)
    {
        ...
        pu8StartVirAddr = (HI_U8*)HI_NULL + pstConfig->stExtMem.tBufVirAddr;
    }
    else
    {
        // step 1.0, MMZ
...
HI_U32 iHAL_AIAO_P_WriteData(AIAO_PORT_S* pstPort, HI_U8 * pu32Src, HI_U32 u3SrcLen)
{
    ...
    Bytes = CIRC_BUF_Write(pstCB, pu32Src, u3SrcLen, HI_TRUE); // copy_from_user or pu32Src = NULL
