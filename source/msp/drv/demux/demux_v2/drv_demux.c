...
{
    .pfnDmxGetCapability      = HI_DRV_DMX_GetCapability,
    .pfnDmxGetTunerPortPktCnt = HI_DRV_DMX_TunerPortGetPacketNum,
...
#ifdef DMX_DESCRAMBLER_SUPPORT
    .pfnDmxSetDescramblerKey  = HI_DRV_DMX_SetDescramblerKey,
    .pfnDmxSetDescramblerIV   = HI_DRV_DMX_SetDescramblerIV,
#else
    .pfnDmxSetDescramblerKey  = (FN_DEMUX_SetDescramblerKey)HI_NULL,
    .pfnDmxSetDescramblerIV   = (FN_DEMUX_SetDescramblerIV)HI_NULL,
#endif
};

/* Basic Init Flag */
static HI_BOOL g_DmxBasicInitFlag = HI_FALSE;
...
HI_S32  HI_DRV_DMX_BasicInit(HI_VOID)
{
    ...
    /* Check the Basic Init Flag */
    if (HI_TRUE == g_DmxBasicInitFlag)
    {
        return HI_SUCCESS;
    }
    ...
    /* Set the Basic Init Flag */
    g_DmxBasicInitFlag = HI_TRUE;
...
HI_S32  HI_DRV_DMX_TagPortGetAttr(const HI_U32 PortId, HI_UNF_DMX_TAG_PORT_ATTR_S *pstAttr)
{
...
HI_S32  HI_DRV_DMX_TagPortSetAttr(const HI_U32 PortId, const HI_UNF_DMX_TAG_PORT_ATTR_S *pstAttr)
{
    CHECKTAGPORTID(PortId);
    CHECKPOINTER(pstAttr);
...
HI_S32 HI_DRV_DMX_AttachTsioPort(HI_U32 DmxId, HI_U32 PortId)
{
    return DMX_OsiAttachPort(DmxId, DMX_PORT_MODE_TSIO, PortId);
}
...
HI_S32 HI_DRV_DMX_AttachPort(HI_U32 DmxId, DMX_PORT_MODE_E PortMode, HI_U32 PortId)
{
    ...
    switch(PortMode)
    {
        ...
        {
            s32Ret = HI_DRV_DMX_AttachTsioPort(DmxId, PortId);
            break;
        }
...
HI_S32 HI_DRV_DMX_TunerPortGetSyncByteErrNum(const HI_U32 PortId, HI_U32 *SyncErrCnt)
{
    CHECKTUNERPORTID(PortId);
    CHECKPOINTER(SyncErrCnt);

    return DMX_OsiTunerPortGetSyncByteErrNum(PortId, SyncErrCnt);
}
...
HI_S32 HI_DRV_DMX_RamPortGetErrPacketNum(const HI_U32 PortId, HI_U32 *TsErrPackCnt)
{
    CHECKRAMPORTID(PortId);
    CHECKPOINTER(TsErrPackCnt);

    return DMX_OsiRamPortGetErrPacketNum(PortId, TsErrPackCnt);
}

HI_S32 HI_DRV_DMX_GetPortPacketNum(HI_U32 PortId, DMX_PORT_MODE_E PortMode, HI_UNF_DMX_PORT_PACKETNUM_S *pPortPacket)
{
    HI_S32 ret = HI_FAILURE;
    CHECKPOINTER(pPortPacket);

    if ((DMX_PORT_MODE_RAM != PortMode) && (DMX_PORT_MODE_TUNER != PortMode))
    {
        HI_ERR_DEMUX("Not support PortMode:0x%x!\n", PortMode);
        return HI_FAILURE;
    }

    pPortPacket->u32ErrTsPackCnt = 0;

    if (DMX_PORT_MODE_RAM == PortMode)
    {
        ret = DMX_OsiRamPortGetSyncLostCnt(PortId, &pPortPacket->u32TsSyncLossCnt);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_DEMUX("get TsSyncLossCnt fail ret:0x%x\n", ret);
            goto out;
        }
#ifdef DMX_TS_ERRINFO_PROC_SUPPORT
        ret = DMX_OsiRamPortGetSyncByteErrCnt(PortId, &pPortPacket->u32TsSyncByteErrCnt);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_DEMUX("get SyncByteErrCnt fail ret:0x%x\n", ret);
            goto out;
        }
#else
        pPortPacket->u32TsSyncByteErrCnt = pPortPacket->u32TsSyncLossCnt;
#endif
        ret = DMX_OsiRamPortGetErrPacketNum(PortId, &pPortPacket->u32ErrTsPackCnt);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_DEMUX("get ErrTsPackCnt fail ret:0x%x\n", ret);
            goto out;
        }

        pPortPacket->u32ErrTsPackCnt += pPortPacket->u32TsSyncLossCnt;
        ret = DMX_OsiPortGetIpCCDiscErrPacketNum(PortId, &pPortPacket->u32PackDisCCCnt);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_DEMUX("get DisCCCnt fail ret:0x%x\n", ret);
            goto out;
        }

        ret = DMX_OsiRamPortGetPacketNum(PortId, &pPortPacket->u32TsPackCnt);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_DEMUX("get PackCnt fail ret:0x%x\n", ret);
            goto out;
        }
    }
    else
    {
        ret = DMX_OsiTunerPortGetSyncByteErrNum(PortId, &pPortPacket->u32TsSyncByteErrCnt);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_DEMUX("get SyncByteErrCnt fail ret:0x%x\n", ret);
            goto out;
        ...
        pPortPacket->u32TsSyncLossCnt = pPortPacket->u32TsSyncByteErrCnt;
        ret = DMX_OsiPortGetDvbCCDiscErrPacketNum(PortId, &pPortPacket->u32PackDisCCCnt);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_DEMUX("get DisCCCnt fail ret:0x%x\n", ret);
            goto out;
        ...
        ret = DMX_OsiTunerPortGetPacketNum(PortId, &pPortPacket->u32TsPackCnt, &pPortPacket->u32ErrTsPackCnt);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_DEMUX("get PackCnt fail ret:0x%x\n", ret);
            goto out;
    ...
    pPortPacket->u32PackDisCCCntCA = pPortPacket->u32PackDisCCCnt;
    if (pPortPacket->u32TsPackCnt > pPortPacket->u32ErrTsPackCnt)
    {
        pPortPacket->u32TsPackDrainedCnt = pPortPacket->u32TsPackCnt - pPortPacket->u32ErrTsPackCnt;
    }
    else //if pPortPacket->u32TsPackCnt turn over
    {
        pPortPacket->u32TsPackDrainedCnt = 0;
    }

    if((pPortPacket->u32ErrTsPackCnt > pPortPacket->u32TsPackCnt)
        || (pPortPacket->u32PackDisCCCnt > pPortPacket->u32TsPackCnt)
        || (pPortPacket->u32TsPackDrainedCnt > pPortPacket->u32TsPackCnt))
    {
        ret = HI_FAILURE;
    }
...
HI_S32 HI_DRV_DMX_GetChannelPacketNum(HI_HANDLE hChannel, HI_U32 *CCDisCnt)
{
    HI_U32  DmxId, ChanId;
    CHECKPOINTER(CCDisCnt);
    DMX_CHECK_CHANHANDLE(hChannel);

    DmxId = DMX_DMXID(hChannel);
    ChanId = DMX_CHANID(hChannel);

    return DMX_OsiGetChannelPacketNum(DmxId, ChanId, CCDisCnt);
}
...
HI_S32 HI_DRV_DMX_GetAcquireEsMsg(HI_HANDLE hChannel, HI_U32 Timeout)
{
    HI_U32  ChanId, DmxId;
    ...
    return DMX_OsiReadEsRequestMsg(DmxId, ChanId, Timeout);
}
    ...
    )
{
    ...
    ret = DMX_DRV_REC_CreateChannel(RecAttr, &RecId, RecBuf);
    if (HI_SUCCESS == ret)
    {
...
HI_S32 HI_DRV_DMX_AcquireRecData(HI_HANDLE RecHandle, HI_MPI_DMX_REC_DATA_S *RecData, HI_U32 Timeout)
{
    ...
    ret = DMX_DRV_REC_AcquireRecData(DmxId, RecId, &PhyAddr, &pAddr, &Len, Timeout);
    if (HI_SUCCESS == ret)
    {
        RecData->u32DataPhyAddr = PhyAddr;
        RecData->u32Len         = Len;
        RecData->pDataAddr      = pAddr;
    }
...
static HI_S32 DMX_OsrSaveALLTs_Start(HI_U32 PortId)
{
    ...
    else if (PortId >= HI_UNF_DMX_PORT_TSIO_0 && PortId < HI_UNF_DMX_PORT_TSIO_0 + DmxCluster->Ops->GetTsioPortNum())
    {
        PortMode = DMX_PORT_MODE_TSIO;
        PortId = PortId - HI_UNF_DMX_PORT_TSIO_0;
    }
    else if (PortId >= HI_UNF_DMX_PORT_TAG_0 && PortId < HI_UNF_DMX_PORT_TAG_0 + DmxCluster->Ops->GetTagPortNum())
    {
        PortMode = DMX_PORT_MODE_TAG;
        PortId = PortId - HI_UNF_DMX_PORT_TAG_0;
    }
    ...
    ret = DMX_DRV_REC_CreateChannel(&RecAttr, &DmxAllTsRecId, &RecBuf);
    if (HI_SUCCESS != ret)
    {
...
static HI_S32 DMX_OsrSaveDmxTs_Start(HI_U32 DmxId)
{
    ...
    ret = DMX_DRV_REC_CreateChannel(&RecAttr, &DmxTsRecId, &RecBuf);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("open rec failed 0x%x\n", ret);
...
EXPORT_SYMBOL(HI_DRV_DMX_TunerPortGetSyncByteErrNum);
...
EXPORT_SYMBOL(HI_DRV_DMX_GetPortPacketNum);
