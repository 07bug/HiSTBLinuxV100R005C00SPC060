...
#define DMX_TS_PACKET_LEN_192           192
...
#define DMX_TSIO_PORT_OFFSET            0x30
...
typedef struct
{
    ...
    HI_BOOL              OqWaitEs;
    struct mutex         LockOq;
    DECLARE_BITMAP(OqReadBitmap, DMX_OQ_DEPTH);
} DMX_OQ_Info_S;
...
typedef struct hi_Pes_Priv_Data_Ctl_t
{
    HI_BOOL bValid;
    HI_U8 au8PesPrivData[PES_PRIVATE_DATA_SIZE];
}Pes_Priv_Data_Ctl_t;
...
typedef struct
{
    ...
    HI_U32                          u32DropCnt;
    HI_U32                          u32CCDiscErr;
    HI_U32                          u32OqBlkDropCnt;
    HI_BOOL                         bPesDropFlag;
    Pes_Priv_Data_Ctl_t          stPesPrivDataCtl;
} DMX_ChanInfo_S;
...
}DMX_FqBpTsio_Helper_S;

typedef struct
{
    struct mutex     Lock;
    struct list_head FqBpHead;
}DMX_TsioPort_Info_S;
...
typedef struct
{
    ...
    struct mutex            Lock;   /* the outlevel lock */
    struct mutex            BlkMapLock;
    struct work_struct      DscEndWorker;
...
/* for index receive thread arguments */
typedef struct
{
    HI_U32                      DmxId;
    HI_U32                      RecId;
}DMX_THREAD_ARGS_INFO_S;
...
typedef struct
{
    ...
    struct task_struct  *  RecIndexThread;
    DMX_THREAD_ARGS_INFO_S DmxThreadArgsInfo;
} DMX_RecInfo_S;
...
typedef struct hiDmx_Set_S
{
    ...
    HI_U32                         RamPortCnt;
    HI_UNF_DMX_TAG_PORT_ATTR_S     *TagPortInfo;
    HI_U32                         TagPortCnt;
    DMX_TsioPort_Info_S            *TsioPortInfo;
    HI_U32                         TsioPortCnt;
...
struct Dmx_Set_Operations
{
    ...
    HI_U32 (*GetTsioRawId)(const Dmx_Set_S *DmxSet, const HI_U32 TsioPortId);  /* TsioPortId is logical id */
    ...
    HI_U32 (*GetTagRawId)(const Dmx_Set_S *DmxSet, const HI_U32 TagPortId);  /* TagId is logical id */
...
typedef struct hiDmx_Set_1_Cfg_S
{
    ...
    HI_UNF_DMX_TAG_PORT_ATTR_S  TagPortInfo[DMX_TAGPORT_CNT];
    DMX_TsioPort_Info_S         TsioPortInfo[DMX_TSIOPORT_CNT];
...
typedef struct hiDmx_Set_2_Cfg_S
{
    ...
    DMX_TunerPort_Info_S        TunerPortInfo[DMX_TUNERPORT_CNT];
    DMX_RamPort_Info_S          RamPortInfo[DMX_RAMPORT_CNT];
    HI_UNF_DMX_TAG_PORT_ATTR_S  TagPortInfo[0];
    DMX_TsioPort_Info_S         TsioPortInfo[0];
...
#define TEEC_CMD_FIXUP_HEVC_INDEX          14
#define TEEC_CMD_PARSER_PES_HEADER         15
#define TEEC_CMD_GET_DISP_CONTROL          16
#define TEEC_CMD_GET_PES_HEADER_LEN        17
#define TEEC_CMD_FIXUP_AUD_SEC_BUF         18
#define TEEC_CMD_REGISTER_SECTION_SECBUF   19
#define TEEC_CMD_UNREGISTER_SECTION_SECBUF 20
#define TEEC_CMD_FIXUP_SECTION_SEC_BUF     21
#define TEEC_CMD_DESCRAMBLER_NONSEC_KEY_ACQUIRE   22
#define TEEC_CMD_DESCRAMBLER_NONSEC_KEY_RELEASE   23
#define TEEC_CMD_ATTACH_RAM_PORT           24
#define TEEC_CMD_DETACH_RAM_PORT           25
...
{
    Dmx_Cluster_S *DmxCluster = GetDmxCluster();
    HI_U32 DmxSetId = RamPortId / (DmxCluster->Ops->GetRamPortNum()) + DMX_SET_1; /* Dmx Set ID start from DMX_SET_1 */
    Dmx_Set_S *tmp= HI_NULL;
