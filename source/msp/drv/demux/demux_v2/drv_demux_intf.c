 ...
 * main\1    2010-01-25   sdk     NULL         init.
 ********************************************************************************/
...
static HI_S32 DMXProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "DmxId\tPortId\tTEICnt\tCCDiscCnt\n");
    ...
    for (DmxId = 0; DmxId < DmxCluster->Ops->GetDmxNum(); DmxId++)
    {
        ...
        HI_U32              PortId;
        DMX_Proc_TeiCCDistErrCnt TeiCCDistErrCnt = {0};
        ...
        /* Get the tei error cnt and cc disc error cnt */
        if(HI_SUCCESS != DMX_OsiGetTeiCCDiscErrCnt(DmxId, &TeiCCDistErrCnt))
        {
            HI_WARN_DEMUX("DMX_OsiGetTeiCCDiscErrCnt error.\n");
        }

        PROC_PRINT(p, "  %u\t%s\t%u\t%u\n", DmxId, PortStr, TeiCCDistErrCnt.TeiErrCnt, TeiCCDistErrCnt.CCDiscErrCnt);
    }
...

{
    switch (Type)
    {
        ...
        case HI_UNF_DMX_PORT_TYPE_SERIAL_NOSYNC_NOVALID :
            strncpy(str, "SERIAL_NOSYNC_NOVALID",len);
            break;

        case HI_UNF_DMX_PORT_TYPE_SERIAL2BIT_NOSYNC_NOVALID :
            strncpy(str, "SERIAL2BIT_NOSYNC_NOVALID",len);
            break;
        ...
        case HI_UNF_DMX_PORT_TYPE_PARALLEL_VALID_DUMMY_SYNC :
            strncpy(str, "PARALLEL_VALID_DUMMY_SYNC",len);
            break;
...
static HI_S32 DMXPortProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "\n------------------------------------------TSO port--------------------------------------------------\n");
    PROC_PRINT(p, " Id   Enable  ClkReverse  TSPortID    ClkMode     VldMode   Sync   OutputMode  BitSel  LSB   Clk   ClkDiv\n");
    ...
    for (i = 0; i < DmxCluster->Ops->GetTsoPortNum(); i++)
    {
        ...
        PROC_PRINT(p, " %d      %d         %d          %-3d      %s        %d       %s      %-2d       %s      %d   %s     %d\n",
        i,
        TSOPortAttr.bEnable,
    ...
    PROC_PRINT(p, "\n------------------------------------------------------------------RAM port-----------------------------------------------------\n");
    PROC_PRINT(p, " Id  AllTsCnt  TsChkRange Lock/lost   BufAddr      BufSize      BufUsed          Read       Write     Get(Try/Ok)     Put   Type  SyncByteErrCnt  SyncLostCnt\n");
    ...
    for (i = 0; i < DmxCluster->Ops->GetRamPortNum(); i++)
    {
        ...
        if (HI_SUCCESS == DMX_OsiRamPortGetBufInfo(i, &BufInfo))
        {
            ...
            PROC_PRINT(p, "%3u    0x%-8x%s   %u/%u     0x%-8x   0x%-8x   %-10s    0x%-8x 0x%-8x%6u/%-10u%-5u  %s      %-8u      %-8u\n",
    ...
    {
        HI_UNF_DMX_TAG_PORT_ATTR_S TagPortAttrs;
        ...
        DMX_OsiTagPortGetAttr(i, &TagPortAttrs);
...
#if defined(DMX_TSIOPORT_CNT) && (DMX_TSIOPORT_CNT > 0)
    PROC_PRINT(p, "\n----------------------TSIO port-------------------------------\n");
    PROC_PRINT(p, " TsioCnt\n");
    PROC_PRINT(p, " %u       \n", DMX_TSIOPORT_CNT);
#endif
...
static HI_S32 DMXChanProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "DmxId  ChnId  PID\tType Mod Stat  Secure  KeyId    Acquire(Try/Ok)    Release   CCDiscCnt\n");
    ...
    TraverseForEachDmxSet(DmxSet, &DmxCluster->head)
    {
        ...
        for_each_set_bit(ChanId, DmxSet->ChnBitmap, DmxSet->DmxChanCnt)
        {
            if (HI_SUCCESS == DmxGetChnInstance(DmxSet, ChanId))
            {
                ...
                PROC_PRINT(p, "  %u      %u    0x%x\t%s  %s %s\t%s\t%s  %10u/%-10u    %u    %u\n",
                            ChanInfo->DmxId,
                            ChanId,
                            ChanInfo->ChanPid,
    ...
    TraverseForEachDmxSet(DmxSet, &DmxCluster->head)
    {
        ...
        for_each_set_bit(ChanId, DmxSet->ChnBitmap, DmxSet->DmxChanCnt)
        {
            if (HI_SUCCESS == DmxGetChnInstance(DmxSet, ChanId))
            {
                ...
                if (BufInfo.BlockSize < 1024)
                {
                    PROC_PRINT(p, "  %u     %u      %u     %u   %uK   %u    %uB     %u     %u     %u%%     %u\n",
                            ChanInfo->DmxId, ChanId, DmxOqInfo->u32FQId, ChanInfo->ChanOqId, Size, BufInfo.DescDepth,
                            BufInfo.BlockSize, BufInfo.DescRead, BufInfo.DescWrite, UsedPercent, BufInfo.Overflow);
                }
                else
                {
                    PROC_PRINT(p, "  %u     %u      %u     %u   %uK   %u    %uK     %u     %u     %u%%     %u\n",
                        ChanInfo->DmxId, ChanId, DmxOqInfo->u32FQId, ChanInfo->ChanOqId, Size, BufInfo.DescDepth,
                        BufInfo.BlockSize/1024, BufInfo.DescRead, BufInfo.DescWrite, UsedPercent, BufInfo.Overflow);
                }
                DmxPutChnInstance(DmxSet, ChanId);
            }
        }
...
static HI_S32 DmxGetPortTypeAndID(const HI_UNF_DMX_PORT_E Port, DMX_PORT_MODE_E *PortMode, HI_U32 *PortId)
{
    ...
    ret = HI_DRV_DMX_GetCapability(&CapInfo);
    if (HI_SUCCESS != ret)
    {
    ...
    if((HI_MPI_DMX_PORT_E)Port == HI_MPI_DMX_PORT_RMX_0)
    {
    ...
    else if((Port >= HI_UNF_DMX_PORT_IF_0) && (Port <= HI_UNF_DMX_PORT_IF_15))
    {
    ...
    else if((Port >= HI_UNF_DMX_PORT_TSI_0) && (Port <= HI_UNF_DMX_PORT_TSI_15))
    {
    ...
    else if((Port >= HI_UNF_DMX_PORT_RAM_0) && (Port <= HI_UNF_DMX_PORT_RAM_15))
    {
    ...
    else if((Port >= HI_UNF_DMX_PORT_TSIO_0) && (Port <= HI_UNF_DMX_PORT_TSIO_7))
    {
        HI_U32 Id = (HI_U32)Port - (HI_U32)HI_UNF_DMX_PORT_TSIO_0;

        if (Id < CapInfo.u32TsioPortNum)
        {
            *PortMode   = DMX_PORT_MODE_TSIO;
            *PortId     = Id;

            return HI_SUCCESS;
        }
    }
    else if((Port >= HI_UNF_DMX_PORT_TAG_0) && (Port <= HI_UNF_DMX_PORT_TAG_7))
    {
        HI_U32 Id = (HI_U32)Port - (HI_U32)HI_UNF_DMX_PORT_TAG_0;

        if (Id < CapInfo.u32TagPortNum)
        {
            *PortMode   = DMX_PORT_MODE_TAG;
            *PortId     = Id;
...
static HI_S32 DMXPortIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        case CMD_DEMUX_PORT_GET_ATTR:
        {
            ...
            ret = DmxGetPortTypeAndID(Param->enPortId, &Param->PortMode, &Param->PortId);
            if (HI_SUCCESS != ret)
            {
                HI_ERR_DEMUX("Invalid enPortId[0x%x]\n", Param->enPortId);
                break;
            }
            ...
            else if (DMX_PORT_MODE_TUNER == Param->PortMode)
            {
                ret = HI_DRV_DMX_TunerPortGetAttr(Param->PortId, &Param->PortAttr);
            }
            else
            {
                HI_WARN_DEMUX("Portmode[0x%x] don't support get attr\n", Param->PortMode);
        ...
        case CMD_DEMUX_PORT_SET_ATTR:
        {
            ...
            {
                HI_ERR_DEMUX("Portmode[0x%x] don't support set attr\n", Param->PortMode);
                ret = HI_ERR_DMX_NOT_SUPPORT;
            }
        ...
        case CMD_DEMUX_DMX_GET_TAG_ATTR:
        {
            ...
            if (DMX_PORT_MODE_TAG != Param->PortMode)
            {
                HI_ERR_DEMUX("tagPort Invalid Unknown port 0x%x\n", Param->enPortId);
...
#else
            ret = HI_ERR_DMX_NOT_SUPPORT_TAGDEAL;
#endif
            break;
        }
        ...
        case CMD_DEMUX_PORT_GETPACKETNUM:
        {
            ...
            ret = HI_DRV_DMX_GetPortPacketNum(Param->PortId, Param->PortMode, &Param->PortPacket);
...
static HI_S32 DMXTsBufferIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        case CMD_DEMUX_TS_BUFFER_INIT:
        {
            ...
            DMX_PORT_MODE_E  PortMode = DMX_PORT_MODE_BUTT;

            ret = DmxGetPortTypeAndID(Param->enPortId, &PortMode, &Param->PortId);
            if (HI_SUCCESS != ret)
            {
                HI_ERR_DEMUX("Invalid enPortId[0x%x]\n", Param->enPortId);
                break;
...
static HI_S32 DMXChanIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_CHAN_GETPACKETNUM:
        {
            DMX_ChanPacktNum_S *Param = (DMX_ChanPacktNum_S*)arg;

            ret = HI_DRV_DMX_GetChannelPacketNum(Param->hChannel, &Param->CCDisCnt);
...
static HI_S32 DMXRecvIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_ACQUIRE_MSG:
        {
            ...
            /* support max 512 section number */
            if(CFG_HI_DEMUX_POOLBUF_SIZE / DMX_MAX_SEC_LEN < u32AcquireNum)
            {
                HI_FATAL_DEMUX("ACQUIRE MSG Num(u32AcquireNum) to many: 0x%X\n", u32AcquireNum);
                ret = HI_ERR_DMX_INVALID_PARA;
                break;
            }
        ...
        case CMD_DEMUX_RELEASE_MSG:
        {
            ...
            /* support max 512 section number */
            if(CFG_HI_DEMUX_POOLBUF_SIZE / DMX_MAX_SEC_LEN < u32ReleaseNum)
            {
                HI_FATAL_DEMUX("RELEASE MSG Num(u32ReleaseNum) to many: 0x%X\n", u32ReleaseNum);
                ret = HI_ERR_DMX_INVALID_PARA;
                break;
            }
...
static HI_S32 Compat_DMXRecvIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_COMPAT_ACQUIRE_MSG:
        {
            ...
            if(CFG_HI_DEMUX_POOLBUF_SIZE / DMX_MAX_SEC_LEN < u32AcquireNum)
            {
                HI_FATAL_DEMUX("COMPAT ACQUIRE MSG Num(u32AcquireNum) to many: 0x%X\n", u32AcquireNum);
                ret = HI_ERR_DMX_INVALID_PARA;
                break;
            }
        ...
        case CMD_DEMUX_COMPAT_RELEASE_MSG:
        {
            ...
            if(CFG_HI_DEMUX_POOLBUF_SIZE / DMX_MAX_SEC_LEN < u32ReleaseNum)
            {
                HI_FATAL_DEMUX("COMPAT RELEASE MSG Num(u32ReleaseNum) to many: 0x%X\n", u32ReleaseNum);
                ret = HI_ERR_DMX_INVALID_PARA;
                break;
            }
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_ES_BUFFER_GET:
        {
            ...
            if (HI_SUCCESS == ret)
            {
                ...
                memcpy(&pPara->stEsExtData, &stEsBuf.stEsExtData, sizeof(HI_MPI_ES_EXT_DATA_S ));
            }
        ...
        case CMD_DEMUX_ES_BUFFER_GET_MSG:
        {
            DMX_PesBufGetEx_S *pPara = (DMX_PesBufGetEx_S *)arg;

            ret = HI_DRV_DMX_GetAcquireEsMsg(pPara->hChannel,pPara->TimeoutMs);
            break;
        }
...
static HI_S32 DMXRecIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_REC_CHAN_ACQUIRE_DATA_INDEX :
        {
            ...
            if (HI_NULL == Param->RecDataIdx)
            {
                HI_ERR_DEMUX("DEMUX REC CHANACQUIRE DATA INDEX Param INVALID!\n");
                ret = HI_ERR_DMX_INVALID_PARA;
                break;
            }
        ...
        case CMD_DEMUX_REC_CHAN_RELEASE_DATA_INDEX :
        {
            ...
            if (HI_NULL == Param->RecDataIdx)
            {
                HI_ERR_DEMUX("DEMUX REC CHAN RELEASE DATA INDEX Param INVALID!\n");
                ret = HI_ERR_DMX_INVALID_PARA;
                break;
            }
...
/***********************************DMX map begin*****************************************************/
/*
*map range 0xF9C00000~0xF9C0FFFF(can map 16 page)
*/
#define PAGE_NUM_SHIFT          4 /* it depends on how many pages can be maped, 0:one page, 1:two pages, 2:four pages, 3: eight pages */
#define DMX_MAP_PAGE_NUM        (1ULL << PAGE_NUM_SHIFT)
#define DMX_MAP_MASK            (((DMX_MAP_PAGE_NUM) << PAGE_SHIFT) - 1)
...
static HI_S32 DMX_DRV_Mmap(struct file *file, struct vm_area_struct *vma)
{
    size_t size = vma->vm_end - vma->vm_start;

    /* Invalid register address of demux  */
    if ((DMX_SET_1_BASE >> PAGE_SHIFT) >> PAGE_NUM_SHIFT != vma->vm_pgoff >> PAGE_NUM_SHIFT)
    {
        HI_ERR_DEMUX("invalid page frame, vm_page_prot[%#x]\n", vma->vm_pgoff);
        return -EINVAL;
    }

    /* Invalid register address or len of demux */
    if (!_valid_mmap_phys_addr_range(vma->vm_pgoff, size, DMX_SET_1_BASE | DMX_MAP_MASK))
    {
        HI_ERR_DEMUX("invalid pgoff[%#x] or size[%#x]\n", vma->vm_pgoff, size);
        return -EINVAL;
    }
    ...
    {
        HI_FATAL_DEMUX("remap_pfn_range fail.\n");
        return -EAGAIN;
    }
...
{
    return (long)HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, DMX_OsrIoctl);
}

#ifdef CONFIG_COMPAT
static long DMX_DRV_Compat_Ioctl(struct file *file, HI_U32 cmd, unsigned long arg)
{
    return (long)HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, DMX_Compat_OsrIoctl);
}
#endif
