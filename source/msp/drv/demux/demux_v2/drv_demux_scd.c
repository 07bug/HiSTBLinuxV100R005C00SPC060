...
static inline HI_U64 PVR_SCDIndexCalcGlobalOffset(HI_BOOL bUseTimeStamp,const PVR_INDEX_SCD_S *pScData)
{
    ...
    if ( bUseTimeStamp)
    {
        offset = (pScData->u64TSCnt - pScData->u32OffsetInDavBuf - 1) * 192ULL + pScData->u16OffsetInTs + 4;
    }
    else
    {
...
static HI_S32 DmxParserSCData(DMX_RecInfo_S *RecInfo, HI_U64 ParserOffset, HI_U8 *TsRecBuf, HI_U32 TsRecBufSize, HI_U8 *SCDataBuf, HI_U32 SCDataBufLen)
{
    HI_U64 FrameGlobalOffset;
    HI_U8 *pData, *pDataSC;
    HI_U32 DestIdx, SrcIdx;
    HI_U32 u32TS_Len = 0;
    HI_U32 u32TS_offset = 0;

    FrameGlobalOffset = ParserOffset;
    pDataSC = (HI_U8*)(do_div(FrameGlobalOffset, (HI_U64)TsRecBufSize) + TsRecBuf);

    if (DMX_REC_TIMESTAMP_NONE == RecInfo->enRecTimeStamp)
    {
        u32TS_Len = DMX_TS_PACKET_LEN;
        u32TS_offset = 0;
    }
    else
    {
        u32TS_Len = DMX_TS_PACKET_LEN_192;
        u32TS_offset = 4;
    }
    ...
    for (pData = pDataSC, DestIdx = 0, SrcIdx = 0;DestIdx < SCDataBufLen;SrcIdx ++, pData ++)
    {
        ...
        if (unlikely(pData + u32TS_offset < TsRecBuf + TsRecBufSize &&  0x47 == *(pData + u32TS_offset) && 0 == do_div(FrameGlobalOffset, u32TS_Len)))
        {
            if (RecInfo->IndexPid != DmxGetPid(pData + u32TS_offset))
            {
                SrcIdx += (u32TS_Len) - 1; /* skip entire ts pkt, continue will increase SrcIdx immeidately, so here sub 1 */
                pData += (u32TS_Len) - 1;
            ...
            {
                HI_U32 SkipLen = TS_PKT_HEADER_LEN + DmxAdpFldLen(pData + u32TS_offset) + DmxPesHeaderLen(pData + u32TS_offset) + u32TS_offset;
...
static HI_S32 DmxParserFilterSCData(DMX_RecInfo_S *RecInfo, HI_U64 ParserOffset, HI_U8 *TsRecBuf, HI_U32 TsRecBufSize, HI_U8 *SCDataBuf, HI_U32 SCDataBufLen, HI_U32 *MonoParseLen, HI_U32 *RealDataLen)
{
    HI_U64 FrameGlobalOffset;
    HI_U8 *pData, *pDataSC;
    HI_U32 DestIdx, SrcIdx;
    ...
    for (pData = pDataSC, SrcIdx = 0, DestIdx = 0;DestIdx < SCDataBufLen;pData ++, SrcIdx ++)
    {
        ...
        /* according to hevc protocol , key pair('00 00 03') '03' means emulation_prevention_three_byte, need to be deleted . */
        if (DestIdx >= 3 && 0 == SCDataBuf[DestIdx-1 -2]
                         && 0 == SCDataBuf[DestIdx-1 -1] && 3 == SCDataBuf[DestIdx-1])
        {
            DestIdx--;
            continue;
        }

        /* according to hevc protocol, reach the next Start Code Prefix(0x000001), we consider it as the end of this nal unit */
        if (DestIdx >= 6 && 0 == SCDataBuf[DestIdx-1 -2]
                         && 0 == SCDataBuf[DestIdx-1 -1] && 1 == SCDataBuf[DestIdx-1])
        {
            break;
        }
    }
...
static HI_S32 DmxFixupHevcEsIndex(Dmx_Set_S *DmxSet, DMX_RecInfo_S *RecInfo, FINDEX_SCD_S *pstFidx)
{
    ...
    ret = DmxParserSCData(RecInfo, pstFidx->s64GlobalOffset, pBuf, BufLen, pDupDataBuf, HEVC_DUP_DATA_CMP_LEN);
    if (HI_SUCCESS != ret)
    {
        goto out;
    }

    /* verify start code prefix first. */
    ...
    do
    {
        ...
        HI_U32 RealParseLen = 0;

        ret = DmxParserFilterSCData(RecInfo, pstFidx->s64GlobalOffset, pBuf, BufLen, pDupDataBuf, HEVC_DUP_DATA_TOTAL_LEN, &MonoParseLen, &RealParseLen);
        if (HI_SUCCESS != ret)
        {
            goto out;
        }

        pstFidx->u32ExtraSCRealDataSize = RealParseLen;
        /* ensure pDupDataBuf is valid data. */
        DmxHalGetRecTsCnt(DmxSet, RecInfo->RecId, &TotalTsRecCnt);
        ...
        if (TotalTsRecCnt * u32TS_Len - pstFidx->s64GlobalOffset >= MonoParseLen)
        {
            break;
        }
        else
...
static HI_S32 DmxFixupSecureHevcEsIndex(Dmx_Set_S *DmxSet, DMX_RecInfo_S *RecInfo, FINDEX_SCD_S *pstFidx)
{
    ...
    ParserOffset = do_div(u64FrameGlobalOffset, (HI_U64)RecDataFq->BufSize);

    memset(&operation, 0x0, sizeof(TEEC_Operation));
    operation.started = 1;
    //operation.cancel_flag = 0;
    operation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INPUT, TEEC_VALUE_INPUT, TEEC_MEMREF_TEMP_INPUT, TEEC_MEMREF_TEMP_OUTPUT);
    ...
    operation.params[2].tmpref.buffer = (HI_VOID *)pstFidx;
    operation.params[2].tmpref.size   = sizeof(FINDEX_SCD_S);

    operation.params[3].tmpref.buffer = (HI_VOID *)CurrFrame;
    operation.params[3].tmpref.size = sizeof(HI_MPI_DMX_REC_INDEX_S);

    ret = DmxCluster->Ops->SendCmdToTA(TEEC_CMD_FIXUP_HEVC_INDEX, &operation, HI_NULL);
    if (HI_SUCCESS != ret)
    {
...
HI_S32 DmxFixupSecureHevcIndex(Dmx_Set_S *DmxSet, DMX_RecInfo_S *RecInfo, FINDEX_SCD_S *pstFidx)
{
    HI_S32 ret = HI_FAILURE;

    if (DMX_INDEX_SC_TYPE_PIC == pstFidx->u8IndexType || DMX_INDEX_SC_TYPE_PTS == pstFidx->u8IndexType)
    {
...
HI_VOID DmxRecUpdateFrameInfo(HI_U32 *Param, FRAME_POS_S *IndexInfo)
{
    ...
    switch (IndexInfo->eFrameType)
    {
        ...
        case FIDX_FRAME_TYPE_UNKNOWN :
            HI_WARN_DEMUX("FrameInfo->enFrameType = HI_UNF_FRAME_TYPE_UNKNOWN\n");
            FrameInfo->enFrameType = HI_UNF_FRAME_TYPE_UNKNOWN;
            break;
