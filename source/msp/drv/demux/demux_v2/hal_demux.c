...
HI_VOID DmxHalTSOPortSetAttr(Dmx_Set_S *DmxSet, HI_U32 PortId,HI_UNF_DMX_TSO_PORT_ATTR_S *PortAttr)
{
    ...
    tsout_cfg.bits.serial_2bit_mode = (PortAttr->enPortType == HI_UNF_DMX_PORT_TYPE_SERIAL2BIT) ? 1 : 0;
    tsout_cfg.bits.spi_mode         = (PortAttr->enPortType == HI_UNF_DMX_PORT_TYPE_SERIAL) ? 0 : 1;
    ...
    )
{
    ...
    switch (PortType)
    {
        case HI_UNF_DMX_PORT_TYPE_PARALLEL_BURST :
        {
            ...
            ts_interface.bits.sw_dummy_sync = 0;
        ...
        case HI_UNF_DMX_PORT_TYPE_SERIAL_NOSYNC_NOVALID :
        {
            ...
            ts_interface.bits.ser_2bit_rev    = 0;
            ts_interface.bits.ser_nosync      = 1;
            ts_interface.bits.sw_valid_burst  = 1;
            ts_interface.bits.sw_dummy_sync   = 0;
    ...
    /* set the fifo rate as 3(4 beat/s) to fill the tag mode or high rate mode  */
    ts_interface.bits.inf_fifo_mode = 1;
    ts_interface.bits.inf_fifo_rate = 3;
...
/***********************************************************************************
* Function      : DmxHalDvbPortSetDummy
* Description   : set the dummy for Tuner Port
* Input         : PortId
* Output        :
* Return        :
* Others: pay attention, it just used for stop rec all data
***********************************************************************************/
HI_VOID DmxHalDvbPortSetDummy(Dmx_Set_S *DmxSet, HI_U32 RawPortId, HI_BOOL DummySync)
{
    U_TS_INTERFACE  ts_interface;
    ...
    ts_interface.bits.sw_dummy_sync = ((HI_TRUE == DummySync) ? 1 : 0);
...
HI_S32 DmxHalDvbPortGetShareClk(HI_U32 PortID, HI_UNF_DMX_PORT_E *pSerialPortShareClk)
{
 ...
 || defined(CHIP_TYPE_hi3716mv420)
    ...
    /* pvr_tsi3_sel acturally is point to tsi1,the name pvr_tsi3 because logic code use this name ,in logic code ,
           tsi begin from pvr_tsi1 (inner QAM) */
    switch (PortID)
    {
        case 0x1:
            *pSerialPortShareClk = ((0x1 == PeriCrg63.bits.pvr_tsi3_sel) ? HI_UNF_DMX_PORT_TSI_0: HI_UNF_DMX_PORT_BUTT);
            break;

        case 0x2:
            *pSerialPortShareClk = ((0x1 == PeriCrg63.bits.pvr_tsi4_sel) ? HI_UNF_DMX_PORT_TSI_0: HI_UNF_DMX_PORT_BUTT);
            break;

        case 0x3:
            *pSerialPortShareClk = ((0x1 == PeriCrg63.bits.pvr_tsi5_sel) ? HI_UNF_DMX_PORT_TSI_0: HI_UNF_DMX_PORT_BUTT);
            break;
 ...
 || defined(CHIP_TYPE_hi3716mv420)
    ...
    switch (SerialPortShareClk)
    {
        ...
        case HI_UNF_DMX_PORT_TSI_0:
            if (0x1 == PortID)
            {
                PeriCrg63.bits.pvr_tsi3_sel = 0x1; /* clk_tsi1 share tsi0 clock */
            }

            if (0x2 == PortID)
            {
                PeriCrg63.bits.pvr_tsi4_sel = 0x1; /* clk_tsi2 share tsi0 clock */
            }

            if (0x3 == PortID)
            {
                PeriCrg63.bits.pvr_tsi5_sel = 0x1; /* clk_tsi3 share tsi0 clock */
            }
            break;
        ...
        default:
            if (0x1 == PortID)
            {
                PeriCrg63.bits.pvr_tsi3_sel = 0x0; /* clk_tsi1 original clock */
            }

            if (0x2 == PortID)
            {
                PeriCrg63.bits.pvr_tsi4_sel = 0x0; /* clk_tsi2 original clock */
            }

            if (0x3 == PortID)
            {
                PeriCrg63.bits.pvr_tsi5_sel = 0x0; /* clk_tsi3 original clock */
            }
...
/***********************************************************************************
* Function      : DmxHalDvbPortGetSyncByteErrCount
* Description   : Get Sync byte error count
* Input         : PortId
* Output        :
* Return        :Sync byte error count
* Others:
***********************************************************************************/
HI_U32 DmxHalDvbPortGetSyncByteErrCount(Dmx_Set_S *DmxSet, const HI_U32 PortId)
{
    U_TS_AFIFO_WFULL_STATUS afifo_wfull_status;

    afifo_wfull_status.u32 = DMX_READ_REG(DmxSet->IoBase, TS_AFIFO_WFULL_STATUS(PortId));

    return afifo_wfull_status.bits.dbg_cnt_sync_err;
}
...
/***********************************************************************************
* Function      : DmxHalDvbPortSetDummyForce
* Description   : Set Force Dummy Sync
* Input         : PortId, enable
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalDvbPortSetDummyForce(Dmx_Set_S *DmxSet, const HI_U32 RawPortId, const HI_BOOL enable)
{
    unsigned int dummy_force;
    ...
    DMX_WRITE_REG(DmxSet->IoBase, DBG_DUMMY_FORCE(RawPortId), dummy_force);

    DMX_COM_EQUAL(dummy_force, DMX_READ_REG(DmxSet->IoBase, DBG_DUMMY_FORCE(RawPortId)));
}
...
/***********************************************************************************
* Function      : DmxHalIPPortGetSyncByteErrCnt
* Description   : Set TS Sync Err Count
* Input         : PortId
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_U32 DmxHalIPPortGetSyncByteErrCnt(Dmx_Set_S *DmxSet, const HI_U32 RawRamId)
{
    U_IP_DBG_SYNC_ERR_CNT ip_sync_err_cnt;

    BUG_ON(RawRamId >= DmxSet->RamPortCnt);

    ip_sync_err_cnt.u32 = DMX_READ_REG(DmxSet->IoBase, IP_DBG_OUT2(RawRamId));

    return ip_sync_err_cnt.bits.ip_dbg_sync_err_cnt;
}

/***********************************************************************************
* Function      : DmxHalIPPortGetSyncLostCnt
* Description   : Get TS Sync Lost Count
* Input         : PortId
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_U32 DmxHalIPPortGetSyncLostCnt(Dmx_Set_S *DmxSet, const HI_U32 RawRamId)
{
    U_IP_DBG_SYNC_LOST_CNT ip_sync_lost_cnt;

    BUG_ON(RawRamId >= DmxSet->RamPortCnt);

    ip_sync_lost_cnt.u32 = DMX_READ_REG(DmxSet->IoBase, IP_DBG_SYNC0(RawRamId));

    return ip_sync_lost_cnt.bits.ip_dbg_sync_lost_cnt;
}

/***********************************************************************************
* Function      : DmxHalIPPortIsEnbled
* Description   : check the IpPort whether is enable
* Input         : PortId
* Output        :
* Return        : 1:enbale 0:disable
* Others:
***********************************************************************************/
HI_BOOL DmxHalIPPortIsEnbled(Dmx_Set_S *DmxSet, const HI_U32 RawPortId)
{
    ...
    return (ipout_en.bits.ip_stop_en == 0 ? HI_TRUE : HI_FALSE);
}
...
/***********************************************************************************
* Function      : DmxHalIPPortEnableSyncLostInt
* Description   : Enable synclost int
* Input         : PortId
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalIPPortEnableSyncLostInt(Dmx_Set_S *DmxSet, HI_U32 RawPortId)
{
    U_IP_IENA_INT ip_int_ena;

    ip_int_ena.u32 = DMX_READ_REG(DmxSet->IoBase, IP_IENA_INT(RawPortId));
    ip_int_ena.bits.ip_iena_syn_loss = 1;
...
/***********************************************************************************
* Function      : DmxHalIPPortDisableSyncLostInt
* Description   : Disable synclost int
* Input         : PortId
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalIPPortDisableSyncLostInt(Dmx_Set_S *DmxSet, HI_U32 RawPortId)
{
    U_IP_IENA_INT ip_int_ena;

    ip_int_ena.u32 = DMX_READ_REG(DmxSet->IoBase, IP_IENA_INT(RawPortId));
    ip_int_ena.bits.ip_iena_syn_loss = 0;
    DMX_WRITE_REG(DmxSet->IoBase, IP_IENA_INT(RawPortId), ip_int_ena.u32);
...
/***********************************************************************************
* Function      : DmxHalIPPortGetSyncLostIntStatus
* Description   : Get synclost int status
* Input         : PortId
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_U32 DmxHalIPPortGetSyncLostIntStatus(Dmx_Set_S *DmxSet, HI_U32 RawPortId)
{
    U_IP_IINT_INT ip_int;

    ip_int.u32 = DMX_READ_REG(DmxSet->IoBase, IP_IINT_INT(RawPortId));

    return ip_int.bits.ip_iint_syn_loss;
}

/***********************************************************************************
* Function      : DmxHalIPPortClearSyncLostIntStatus
* Description   : clear synclost int status
* Input         : PortId
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalIPPortClearSyncLostIntStatus(Dmx_Set_S *DmxSet, HI_U32 RawPortId)
{
    U_IP_IRAW_INT ip_raw_int;

    ip_raw_int.u32 = DMX_READ_REG(DmxSet->IoBase, IP_IRAW_INT(RawPortId));

    ip_raw_int.bits.ip_iraw_syn_loss = 1;
    DMX_WRITE_REG(DmxSet->IoBase, IP_IRAW_INT(RawPortId), ip_raw_int.u32);
    DMX_COM_EQUAL(ip_raw_int.u32, DMX_READ_REG(DmxSet->IoBase, IP_IRAW_INT(RawPortId)));
}
...
HI_VOID DmxHalSetRecNoTsChk(Dmx_Set_S *DmxSet, HI_U32 ChanId, HI_BOOL bEnable)
{
    U_DMX_PID_REC_BUF pid_rec_buf;
    ...
    pid_rec_buf.bits.rec_tsph_nocheck = (HI_TRUE == bEnable ? 1 : 0);
    DMX_WRITE_REG(DmxSet->IoBase, DMX_PID_REC_BUF(ChanId), pid_rec_buf.u32);
...
HI_U32 DmxHalGetPortRawId(Dmx_Set_S *DmxSet, DMX_PORT_MODE_E PortMode, HI_U32 PortId)
{
     ...
     switch(PortMode)
    {
        ...
        {
            Id = DmxSet->Ops->GetTsioRawId(DmxSet, PortId);

            BUG_ON(DMX_INVALID_PORT_ID == Id);

            Id += DMX_TSIO_PORT_OFFSET;
            break;
        }
        ...
        {
            Id = DmxSet->Ops->GetTagRawId(DmxSet, PortId);

            BUG_ON(DMX_INVALID_REMUX_ID == Id);

            Id +=  DMX_TAG_PORT_OFFSET;
...
/***********************************************************************************
* Function      :  DmxHalEnableTeiErrInt
* Description   :    Enable Tei Err Int
* Input         :
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalEnableTeiErrInt(Dmx_Set_S *DmxSet)
{
    U_ENA_TEI unEnaTei;

    unEnaTei.u32 = DMX_READ_REG(DmxSet->IoBase, ENA_TEI);
    unEnaTei.bits.ena_tei = 1;
    DMX_WRITE_REG(DmxSet->IoBase, ENA_TEI, unEnaTei.u32);

    DMX_COM_EQUAL(unEnaTei.u32, DMX_READ_REG(DmxSet->IoBase, ENA_TEI));
}

/***********************************************************************************
* Function      :  DmxHalDisableTeiErrInt
* Description   :    Disable Tei Err Int
* Input         :
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalDisableTeiErrInt(Dmx_Set_S *DmxSet)
{
    U_ENA_TEI unEnaTei;

    unEnaTei.u32 = DMX_READ_REG(DmxSet->IoBase, ENA_TEI);
    unEnaTei.bits.ena_tei = 0;
    DMX_WRITE_REG(DmxSet->IoBase, ENA_TEI, unEnaTei.u32);
...
/***********************************************************************************
* Function      :  DmxHalEnableCCDiscErrInt
* Description   :    Enable CC Disc Err Int
* Input         :
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalEnableCCDiscErrInt(Dmx_Set_S *DmxSet)
{
    U_ENA_CC_DISC unEnaCCDisc;
    HI_S32 i = 0;

    for (i = 0; i < DMX_CHANNEL_REGION_NUM; i++)
    {
        unEnaCCDisc.u32 = DMX_READ_REG(DmxSet->IoBase, ENA_CC_DISC0(i));
        unEnaCCDisc.u32 = 0xFFFFFFFF;
        DMX_WRITE_REG(DmxSet->IoBase, ENA_CC_DISC0(i), unEnaCCDisc.u32);

        DMX_COM_EQUAL(unEnaCCDisc.u32, DMX_READ_REG(DmxSet->IoBase, ENA_CC_DISC0(i)));
    }
}

/***********************************************************************************
* Function      :  DmxHalDisableCCDiscErrInt
* Description   :    Disable CC Disc Err Int
* Input         :
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalDisableCCDiscErrInt(Dmx_Set_S *DmxSet)
{
    U_ENA_CC_DISC unEnaCCDisc;
    HI_S32 i = 0;
...
/***********************************************************************************
* Function      :  DmxHalSetIPBackPressure
* Description   :    Set Demux IP port BackPressure
* Input         :
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalSetIPBackPressure(HI_VOID)
{
    HI_U32 DmxID= 0;
    U_SWITCH_BLOCK switch_block;
    Dmx_Set_S *DmxSet = HI_NULL;

    DmxSet = GetDefDmxSet();
    switch_block.u32 = DMX_READ_REG(DmxSet->IoBase, SWITCH_FAKE_EN);
    switch_block.bits.dav_buff_cfg = 0;
    for (DmxID = 0; DmxID < DMX_CNT;DmxID++)
    {
        switch_block.bits.dav_buff_cfg |= (1 << DmxID);
    }
    DMX_WRITE_REG(DmxSet->IoBase, SWITCH_FAKE_EN, switch_block.u32);
}

/***********************************************************************************
* Function      :  DmxHalCFGD2BBP
* Description   :    Set Demux enable /line to 2
* Input         :
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalCFGD2BBP(HI_VOID)
{
    U_D2B_BP_CFG unCfgD2B_BP;
    Dmx_Set_S *DmxSet = HI_NULL;

    DmxSet = GetDefDmxSet();
    unCfgD2B_BP.u32 = DMX_READ_REG(DmxSet->IoBase, D2B_BP_CFG);
    unCfgD2B_BP.bits.fp2dmx_aful_cfg = 2;
    DMX_WRITE_REG(DmxSet->IoBase,D2B_BP_CFG, unCfgD2B_BP.u32);
}
...
/***********************************************************************************
* Function      :  DmxHalGetD2BFOvflRawIntStatus
* Description  :  Get the DMX2BUF buffer overflow Raw int status of play channel
* Input          : DmxSet
* Output        :
* Return        :
* Others:
...
HI_U32 DmxHalGetD2BFOvflRawIntStatus(Dmx_Set_S *DmxSet)
{
    U_IRAW_D2B_OVFL unIntSta;

    unIntSta.u32 = DMX_READ_REG(DmxSet->IoBase, RAW_D2B_OVFL);

    return unIntSta.bits.iraw_d2b_f_ovfl;
}


/***********************************************************************************
* Function      :  DmxHalClsD2BFOvflRawIntStatus
* Description  :  Clear the DMX2BUF buffer overflow Raw int status of play channel
* Input          : DmxSet
* Output        :
* Return        :
* Others:
...
HI_VOID DmxHalClsD2BFOvflRawIntStatus(Dmx_Set_S *DmxSet)
{
    U_IRAW_D2B_OVFL unIntSta;
...
/***********************************************************************************
* Function      :  DmxHalGetD2BFOvflRawIntStatus
* Description  :  Get the DMX2BUF buffer overflow Raw int status of record channel
* Input          : DmxSet
* Output        :
* Return        :
* Others:
...
HI_U32 DmxHalGetD2BROvflRawIntStatus(Dmx_Set_S *DmxSet)
{
    U_IRAW_D2B_OVFL unIntSta;

    unIntSta.u32 = DMX_READ_REG(DmxSet->IoBase, RAW_D2B_OVFL);

    return unIntSta.bits.iraw_d2b_r_ovfl;
}

/***********************************************************************************
* Function      :  DmxHalClsD2BFOvflRawIntStatus
* Description  :  Clear the DMX2BUF buffer overflow Raw int status of record channel
* Input          : DmxSet
* Output        :
* Return        :
* Others:
...
HI_VOID DmxHalClsD2BROvflRawIntStatus(Dmx_Set_S *DmxSet)
{
    U_IRAW_D2B_OVFL unIntSta;
...
HI_S32 DmxHalSetScdRangeFilter(Dmx_Set_S *DmxSet, HI_U32 u32FltId, HI_U8 u8High, HI_U8 u8Low, HI_U8 Mask)
{
...
HI_VOID DmxHalGetPcrValue(Dmx_Set_S *DmxSet, const HI_U32 PcrId, HI_U64 *PcrVal)
{
    ...
    *PcrVal = ((value0 & 0x1ff00) >> 8) + (((value0 & 0x1) << 32) + value1) * 300;
}
...
HI_VOID DmxHalGetScrValue(Dmx_Set_S *DmxSet, const HI_U32 PcrId, HI_U64 *ScrVal)
{
    ...
    *ScrVal = ((value0 & 0x1ff00) >> 8) + (((value0 & 0x1) << 32) + value1) * 300;
}

/***********************************************************************************
* Function      :  DmxHalGetTotalTeiIntStatus
* Description   :    Get Total Tei Status
* Input         :
* Output        :
* Return        :
* Others:
...
/***********************************************************************************
* Function      :  DmxHalGetRawTeiIntInfo
...
* Others:
***********************************************************************************/
HI_VOID DmxHalGetRawTeiIntInfo(Dmx_Set_S *DmxSet, HI_U32* pu32DmxId, HI_U32* pu32ChanId)
{
    if (pu32DmxId && pu32ChanId)
    {
        U_STA_TEI unTeiIntSta;

        unTeiIntSta.u32 = DMX_READ_REG(DmxSet->IoBase, RAW_TEI);
...
* Others:
***********************************************************************************/
HI_U32 DmxHalGetTeiErrCnt(Dmx_Set_S *DmxSet, HI_U32 DmxId)
{
    U_DMX_DEBUG_STATE unDebugSta;

    BUG_ON(DmxId >= DmxSet->DmxCnt);
    unDebugSta.u32 = DMX_READ_REG(DmxSet->IoBase, DBG_DMX_STATE0(DmxId));

    return unDebugSta.bits.tei_err_cnt;
}

/***********************************************************************************
* Function      :  DmxHalGetCCDiscErrCnt
* Description   :    Clr Tei Int Status
* Input         :
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_U32 DmxHalGetCCDiscErrCnt(Dmx_Set_S *DmxSet, HI_U32 DmxId)
{
    U_DMX_DEBUG_STATE unDebugSta;
    ...
    return unDebugSta.bits.cc_disc_err_cnt;
}
...
/* add flag and gloabl variable for tsi share clock supend and resume */
#if defined(CHIP_TYPE_hi3716mv410) \
 || defined(CHIP_TYPE_hi3716mv420)
static HI_BOOL g_bSuspendFlag = HI_FALSE;
static U_PERI_CRG63 g_sPeriCrg63;
#elif defined(CHIP_TYPE_hi3798cv200)  \
   || defined(CHIP_TYPE_hi3798mv200)  \
   || defined(CHIP_TYPE_hi3798mv300)  \
   || defined(CHIP_TYPE_hi3798mv310)  \
   ...
   || defined(CHIP_TYPE_hi3716mv450)
static HI_BOOL g_bSuspendFlag = HI_FALSE;
static U_PERI_CRG64 g_sPeriCrg64;
#endif
...
HI_VOID DmxHalConfigHardware(HI_VOID)
{
    ...
    PeriCrg63.bits.pvr_tsout1_cken  = 1;
     ...
     /* when resume, recovey the original value */
    if(g_bSuspendFlag == HI_TRUE)
    {
        PeriCrg63.bits.pvr_tsi3_sel = g_sPeriCrg63.bits.pvr_tsi3_sel; /* clk_tsi3 share tsi2 clock */
        PeriCrg63.bits.pvr_tsi4_sel = g_sPeriCrg63.bits.pvr_tsi4_sel; /* clk_tsi4 share tsi2 clock */
        PeriCrg63.bits.pvr_tsi5_sel = g_sPeriCrg63.bits.pvr_tsi5_sel; /* clk_tsi5 share tsi2 clock */
...
HI_VOID DmxHalDeConfigHardware(HI_VOID)
{
    ...
    PeriCrg63.u32 = g_pstRegCrg->PERI_CRG63.u32;
     ...
     /* when supend, save the original value */
    g_sPeriCrg63.u32 = PeriCrg63.u32;
    g_bSuspendFlag = HI_TRUE;
...
HI_VOID DmxHalConfigHardware(HI_VOID)
{
    ...
    if((PeriCrg63.bits.pvr_bus_cken ==1) && (PeriCrg63.bits.pvr_dmx_cken ==1))
    {
        return;
    }

    /* only 98cv200 used external smmu, other chipset smmu clock controled by demux clock */
#if defined(CHIP_TYPE_hi3798cv200)
        /* reset the demux smmu */
        PeriCrg206.u32 = g_pstRegCrg->PERI_CRG206.u32;
        PeriCrg206.bits.pvr_smmu_srst_req = 1;
        g_pstRegCrg->PERI_CRG206.u32 = PeriCrg206.u32;

        /* open the demux smmu clock for 98cv200 */
        PeriCrg206.bits.pvr_smmu_cken   = 1;
        PeriCrg206.bits.pvr_smmu_srst_req = 0;
        g_pstRegCrg->PERI_CRG206.u32 = PeriCrg206.u32;
#endif
...
#endif
/* TSO support or not */
#if defined(DMX_TSOPORT_CNT) && (DMX_TSOPORT_CNT > 0)
    PeriCrg63.bits.pvr_ts0_cken     = 1;
    PeriCrg63.bits.pvr_tsout0_cken  = 1;
#endif
...
/* TSO support or not */
    ...
    /* init the lowpower configure for 98mv310 */
    DmxSet = GetDefDmxSet();
    if (unlikely(!DmxSet))
    {
        HI_ERR_DEMUX("Dmxset invalid.\n");
        return;
    }
    DmxHalLowPowerInit(DmxSet);
}
...
/* TSO support or not */
    ...
    /* close the demux smmu clock for 98cv200 */
    PeriCrg206.bits.pvr_smmu_cken   = 0;
    PeriCrg206.bits.pvr_smmu_srst_req = 1;
    g_pstRegCrg->PERI_CRG206.u32 = PeriCrg206.u32;
#endif

    /* when suspend, save the original value */
    g_sPeriCrg64.u32 = PeriCrg64.u32;
    g_bSuspendFlag = HI_TRUE;
...
HI_VOID DmxHalConfigHardware(HI_VOID)
{
    ...
    /* when resume, recovery the original value */
    if(g_bSuspendFlag == HI_TRUE)
    {
        PeriCrg64.bits.pvr_tsi_sel = g_sPeriCrg64.bits.pvr_tsi_sel;/* clk_tsi0,1,3 share tsi2 clock */
        g_bSuspendFlag = HI_FALSE;
...
/***********************************************************************************
* Function      : DmxHalLowPowerInit
* Description   : low power configre init
* Input          :
* Output        :
* Return        :
* Others:       :
***********************************************************************************/
HI_VOID DmxHalLowPowerInit(Dmx_Set_S *DmxSet)
{
    U_PVR_CLK_EN PvrClkEn;

    PvrClkEn.u32 = DMX_READ_REG(DmxSet->IoBase, PVR_CLK_EN);

    PvrClkEn.bits.pvr_csa3_clken = 0x0;     /* close csa3 clock */
    PvrClkEn.bits.pvr_debug_clken = 0x1;    /* open debug clock */
    PvrClkEn.bits.pvr_davcnt_clken = 0x1;   /* open davcnt clock */
    PvrClkEn.bits.pvr_ip0_6_clken = 0x0;    /* close the ramport clock */
    PvrClkEn.bits.pvr_inf_clken = 0x0;      /* close the dvb port clock */
    PvrClkEn.bits.pvr_scd_clken = 0x0;      /* close the scd clock */
    PvrClkEn.bits.mdsc_core_clk_gt_en = 0x1; /* open the dynamic clock control  */
    PvrClkEn.bits.mdsc_sm4_clk_dis = 0x1;    /* close the sm4 clock */
    PvrClkEn.bits.mdsc_asa_clk_dis = 0x1;    /* close the asa clock */

    DMX_WRITE_REG(DmxSet->IoBase, PVR_CLK_EN, PvrClkEn.u32);
    return;
}

/***********************************************************************************
* Function      : DmxHalCsa3ClkSet
* Description   : csa3 clock set
* Input          :
* Output        :
* Return        :
* Others:       :
***********************************************************************************/
HI_VOID DmxHalCsa3ClkSet(Dmx_Set_S *DmxSet, HI_BOOL ClkEn)
{
    U_PVR_CLK_EN PvrClkEn;
    ...
    if (HI_TRUE == ClkEn)
    {
        PvrClkEn.bits.pvr_csa3_clken = 0x1;
    }
    else
    {
        PvrClkEn.bits.pvr_csa3_clken = 0x0;
    }
...
/***********************************************************************************
* Function      : DmxHalIpClkSet
* Description   : Ip clock set
* Input          :  RawPortId
* Output        :
* Return        :
* Others:       :
***********************************************************************************/
HI_VOID DmxHalIpClkSet(Dmx_Set_S *DmxSet, HI_BOOL ClkEn, HI_U32 RawPortId)
{
    U_PVR_CLK_EN PvrClkEn;
    BUG_ON(RawPortId >= DmxSet->RamPortCnt);
    ...
    if (HI_TRUE == ClkEn)
    {
        PvrClkEn.bits.pvr_ip0_6_clken |= 0x1U << RawPortId;
    }
    else
    {
        PvrClkEn.bits.pvr_ip0_6_clken &= ~(0x1U << RawPortId);
...
/***********************************************************************************
* Function      : DmxHalDvbClkSet
* Description   : Dvb clock set
* Input          :
* Output        :
* Return        :
* Others:       :
***********************************************************************************/
HI_VOID DmxHalDvbClkSet(Dmx_Set_S *DmxSet, HI_BOOL ClkEn)
{
    U_PVR_CLK_EN PvrClkEn;
    ...
    if (HI_TRUE == ClkEn)
    {
        PvrClkEn.bits.pvr_inf_clken = 0x1;
    }
    else
    {
        PvrClkEn.bits.pvr_inf_clken = 0x0;
    }
...
/***********************************************************************************
* Function      : DmxHalScdClkSet
* Description   : Scd clock set
* Input          :
* Output        :
* Return        :
* Others:       :
***********************************************************************************/
HI_VOID DmxHalScdClkSet(Dmx_Set_S *DmxSet, HI_BOOL ClkEn)
{
    U_PVR_CLK_EN PvrClkEn;
    ...
    if (HI_TRUE == ClkEn)
    {
        PvrClkEn.bits.pvr_scd_clken = 0x1;
    }
    else
    {
        PvrClkEn.bits.pvr_scd_clken = 0x0;
    }
...
/***********************************************************************************
* Function      : DmxHalSms4ClkSet
* Description   : Sm4 clock set
* Input          :
* Output        :
* Return        :
* Others:       :
***********************************************************************************/
HI_VOID DmxHalSms4ClkSet(Dmx_Set_S *DmxSet, HI_BOOL ClkEn)
{
    U_PVR_CLK_EN PvrClkEn;
    ...
    if (HI_TRUE == ClkEn)
    {
        PvrClkEn.bits.mdsc_sm4_clk_dis = 0x0;
    }
    else
    {
        PvrClkEn.bits.mdsc_sm4_clk_dis = 0x1;
    }
...
/***********************************************************************************
* Function      : DmxHalAsaClkSet
* Description   : Asa clock set
* Input          :
* Output        :
* Return        :
* Others:       :
***********************************************************************************/
HI_VOID DmxHalAsaClkSet(Dmx_Set_S *DmxSet, HI_BOOL ClkEn)
{
    U_PVR_CLK_EN PvrClkEn;
    ...
    if (HI_TRUE == ClkEn)
    {
        PvrClkEn.bits.mdsc_asa_clk_dis = 0x0;
    }
    else
    {
        PvrClkEn.bits.mdsc_asa_clk_dis = 0x1;
    }
...
HI_VOID DmxHalEnFqBPTsioRamPort(Dmx_Set_S *DmxSet, HI_U32 FqId, HI_U32 TsioRamPortId)
{
    HI_U32 reg;
    ...
    /* add new cfg. */
    DMX_WRITE_REG(DmxSet->IoBase, TSIO_FQ_BP_CFG(FqId), TsioRamPortId);

    /* enable */
    if (FqId < 32) /* 0 ~ 31 */
    {
        reg = DMX_READ_REG(DmxSet->IoBase, TSIO_FQ_BP_EN0);

        reg |= (1 << FqId);

        DMX_WRITE_REG(DmxSet->IoBase, TSIO_FQ_BP_EN0, reg);
    }
    else /* 32 ~ 39 */
    {
        reg = DMX_READ_REG(DmxSet->IoBase, TSIO_FQ_BP_EN1);

        reg |= (1 << (FqId - 32));

        DMX_WRITE_REG(DmxSet->IoBase, TSIO_FQ_BP_EN1, reg);
    }
}

HI_VOID DmxHalDisFqBPTsioRamPort(Dmx_Set_S *DmxSet, HI_U32 FqId)
{
    HI_U32 reg;
    ...
    /* del cfg. */
    DMX_WRITE_REG(DmxSet->IoBase, TSIO_FQ_BP_CFG(FqId), 0);
}

HI_VOID DmxHalClrFqBPTsioRamPortStatus(Dmx_Set_S *DmxSet, HI_U32 TsioRamPortId)
{
    HI_U32 reg;

    reg = DMX_READ_REG(DmxSet->IoBase, TSIO_FQ_BP_STA);

    if (reg & (1 << TsioRamPortId))
    {
        reg |= (1 << TsioRamPortId);

        DMX_WRITE_REG(DmxSet->IoBase, TSIO_FQ_BP_STA, reg);
    }
}
...
HI_VOID DmxHalConfigRecTsTimeStamp(Dmx_Set_S *DmxSet, HI_U32 DmxId, DMX_REC_TIMESTAMP_MODE_E enRecTimeStamp)
{
    ...
    timestamp_ctrl.bits.rec_timestamp_mode |= (((enRecTimeStamp << RawDmxId * 2)) & (0x3fff));
    DMX_WRITE_REG(DmxSet->IoBase, TIMESTAMP_CTRL, timestamp_ctrl.value);
}

/***********************************************************************************
* Function      : DmxHalDeConfigRecTsTimeStamp
* Description   : DeConfig Record Ts time stamp
* Input         : DmxID,enRecTimeStamp
* Output        :
* Return        :
* Others:
***********************************************************************************/
HI_VOID DmxHalDeConfigRecTsTimeStamp(Dmx_Set_S *DmxSet, HI_U32 DmxId)
{
    HI_U32 RawDmxId = DmxSet->Ops->GetDmxRawId(DmxSet, DmxId);
    U_TIMESTAMP_CTRL timestamp_ctrl;
    ...
    timestamp_ctrl.value = DMX_READ_REG(DmxSet->IoBase, TIMESTAMP_CTRL);
    timestamp_ctrl.bits.rec_timestamp_mode &= ~(((0x3 << RawDmxId * 2)) & (0x3fff));
    DMX_WRITE_REG(DmxSet->IoBase, TIMESTAMP_CTRL, timestamp_ctrl.value);
}
 ...
 */
HI_S32 __DmxHalSetTagDealCtl(Dmx_Set_S * DmxSet, HI_BOOL bEnable, HI_UNF_DMX_PORT_E enPortId, HI_U32 u32SyncMode, HI_U32 u32TagLen)
{
    ...
    {
        PhyPortId = enPortId - HI_UNF_DMX_PORT_TSI_0 + DMX_IFPORT_CNT;
    ...
    if (HI_TRUE == bEnable)
    {
        mux_source.u32 = DMX_READ_REG(DmxSet->IoBase, TS_TAG_MUX_SOURCE);
        mux_source.bits.mux0_dvb_inf_sel = RawPortId;
        DMX_WRITE_REG(DmxSet->IoBase, TS_TAG_MUX_SOURCE, mux_source.u32);

        if (PhyPortId < DMX_TUNERPORT_CNT)
        {
            dbg_config.u32 = DMX_READ_REG(DmxSet->IoBase, TS_DBG_DVBT_INTERFACE(PhyPortId));
            dbg_config.bits.dbg_ser_len_bypass = 1;
            DMX_WRITE_REG(DmxSet->IoBase, TS_DBG_DVBT_INTERFACE(PhyPortId), dbg_config.u32);
        }
    }
    else
    {
    ...
    mux_config.u32 = DMX_READ_REG(DmxSet->IoBase, TS_TAG_MUX_CONFIG);
    mux_config.bits.mux_work_en = bEnable;
    mux_config.bits.sync_mode_sel = u32SyncMode;
    mux_config.bits.tag_length = u32TagLen;
    DMX_WRITE_REG(DmxSet->IoBase, TS_TAG_MUX_CONFIG, mux_config.u32);

    return HI_SUCCESS;
}

HI_S32 DmxHalSetTagDealCtl(Dmx_Set_S * DmxSet, HI_BOOL bEnable, HI_UNF_DMX_PORT_E enPortId, HI_U32 u32SyncMode, HI_U32 u32TagLen)
{
    ...
    return __DmxHalSetTagDealCtl(DmxSet, bEnable, enPortId, u32SyncMode, u32TagLen);
}

/*
 * set tag values
 */
HI_VOID DmxHalSetTagDealAttr(Dmx_Set_S * DmxSet, HI_U32 u32TagPortId, HI_U32 u32Low, HI_U32 u32Mid, HI_U32 u32High)
{
    DMX_WRITE_REG(DmxSet->IoBase, TS_TAG_LOW_REG(u32TagPortId), u32Low);
    DMX_WRITE_REG(DmxSet->IoBase, TS_TAG_MID_REG(u32TagPortId), u32Mid);
    DMX_WRITE_REG(DmxSet->IoBase, TS_TAG_HIGH_REG(u32TagPortId), u32High);
}

HI_S32 DmxHalSetSyncLen(Dmx_Set_S * DmxSet, HI_U32 TsiPortId, HI_U32 u32TagLen)
{
    U_TS_SYNC_LEN sync_len;

    BUG_ON(TsiPortId >= DMX_TUNERPORT_CNT);

    sync_len.u32 = DMX_READ_REG(DmxSet->IoBase, TS_SYNC_LEN(TsiPortId));
    sync_len.bits.nosync_188_len = DMX_TS_PACKET_LEN + u32TagLen;
    sync_len.bits.nosync_204_len = DMX_TS_PACKET_LEN + u32TagLen;
    sync_len.bits.sw_tsp_out_nolimit = 1;
    sync_len.bits.sw_tag_deal_syncts_sel = 1;
    DMX_WRITE_REG(DmxSet->IoBase, TS_SYNC_LEN(TsiPortId), sync_len.u32);

    return HI_SUCCESS;
}

HI_S32 DmxHalResetSyncLen(Dmx_Set_S * DmxSet, HI_U32 TsiPortId)
{
    U_TS_SYNC_LEN sync_len;
    ...
    sync_len.u32 = DMX_READ_REG(DmxSet->IoBase, TS_SYNC_LEN(TsiPortId));
    sync_len.bits.nosync_188_len = DMX_TS_PACKET_LEN;
    sync_len.bits.nosync_204_len = DMX_TS_PACKET_LEN_204;
    sync_len.bits.sw_tsp_out_nolimit = 1;
    sync_len.bits.sw_tag_deal_syncts_sel = 0;
    DMX_WRITE_REG(DmxSet->IoBase, TS_SYNC_LEN(TsiPortId), sync_len.u32);
