...
static MXL_STATUS_E mxl_waitForTunerLock(MXL_HRCLS_TUNER_ID_E tunerId/*TODO: timeout*/)
{
	...
	if ((status == MXL_SUCCESS) && (lockStatus == MXL_HRCLS_TUNER_LOCKED))
	{
		return MXL_SUCCESS;
	}
	else
...
static MXL_STATUS_E mxl_waitForChannelLock(MXL_HRCLS_CHAN_ID_E chanId/*TODO: timeout*/)
{
	...
	if ((status == MXL_SUCCESS) && (lockStatus == MXL_HRCLS_CHAN_LOCKED))
	{
		return MXL_SUCCESS;
	}
	else
...
HI_S32 mxl254_connect(HI_U32 u32TunerPort, TUNER_ACC_QAM_PARAMS_S *pstChannel)
{
	...
	if (status != MXL_SUCCESS)
	{
		HI_ERR_TUNER("[HRCLS][%d] down_interruptible FAILED\n", HRCLS_DEVICE_ID);
		return status;
	}
...
HI_S32 mxl254_init(HI_U32 u32TunerPort, HI_U8 enI2cChannel, HI_UNF_TUNER_DEV_TYPE_E enTunerDevType)
{
   ...
   ************************************************************************************/
	...
	************************************************************************************/
	...
	if (status != MXL_SUCCESS)
	{
		HI_INFO_TUNER("[HRCLS][%d] Xtal setting FAILED\n", HRCLS_DEVICE_ID);
		up(&mxl254_sem);
		return status;
	}
#if 0
	...
	************************************************************************************/
	...
	if (status != MXL_SUCCESS)
	{
		HI_INFO_TUNER("[HRCLS][%d] Version checking FAILED!\n", HRCLS_DEVICE_ID);
		up(&mxl254_sem);
		return status;
	}
	...
	************************************************************************************/
	if (devVerInfo.firmwareDownloaded == MXL_TRUE)
	{
		...
		HI_INFO_TUNER("[HRCLS][%d] Firmware already running. Forgot about h/w reset?\n", HRCLS_DEVICE_ID);
		up(&mxl254_sem);
		return MXL_FAILURE;
	}
	/************************************************************************************
	...
	************************************************************************************/
	...
	if (xptSupported == MXL_TRUE)
	{
		...
		if (status != MXL_SUCCESS)
		{
			HI_ERR_TUNER("[HRCLS][%d] MxLWare_HRCLS_API_CfgXpt FAILED\n", HRCLS_DEVICE_ID);
			up(&mxl254_sem);
			return status;
		}
	}
...
HI_S32 mxl254_get_status (HI_U32 u32TunerPort, HI_UNF_TUNER_LOCK_STATUS_E  *penTunerStatus)
{
   ...
   // MXL_HRCLS_CHAN_STATUS_E demodstatus = MXL_HRCLS_CHAN_UNLOCKED;
    ...
    MxLWare_HRCLS_API_ReqDemodAllLockStatus(HRCLS_DEVICE_ID, u32TunerPort, &qamLock, &fecLock, &mpegLock, &relock);
...
HI_S32 mxl254_get_data_sample(HI_U32 u32TunerPort, TUNER_DATA_SRC_E enDataSrc, HI_U32 u32DataLen, HI_UNF_TUNER_SAMPLE_DATA_S *pstData)
{
	MXL_STATUS_E status = MXL_FAILURE;
    HI_S16 iVal;
    HI_S16 qVal;
    ...
    {
        status = MxLWare_HRCLS_API_ReqDemodConstellationValue(u32TunerPort,u32TunerPort,&iVal,&qVal);
        if (MXL_SUCCESS != status) 
        {
           HI_ERR_TUNER("MxLWare_HRCLS_API_ReqDemodConstellationValue error!\n");
           return status;
        }
        
        pstData[i].s32DataQP = qVal;

        pstData[i].s32DataIP = iVal;
    }
    
    return HI_SUCCESS;
}

HI_S32 mxl254_get_signal_info(HI_U32 u32TunerPort, HI_UNF_TUNER_SIGNALINFO_S *pstInfo)
{
    switch(g_stTunerOps[u32TunerPort].enSigType)
	{ 
	    case HI_UNF_TUNER_SIG_TYPE_CAB:
...
HI_S32 mxl254_standby(HI_U32 u32TunerPort, HI_U32 u32Status)
{
    MXL_HRCLS_DEV_CONTEXT_T * devContextPtr = MxL_HRCLS_Ctrl_GetDeviceContext(u32TunerPort);
    ...
    {
        HI_ERR_TUNER("MxL_HRCLS_Ctrl_GetDeviceContext is NULL!\n");
        return HI_FAILURE;
    }
}
