...
static HI_VOID select_demod_hi3130(HI_U32 u32TunerPort, HI_U32 u32ResetGpioNo)
{
    HI_U8 u8DataTmp = 0;
    ...
    if ((u8DataTmp >> 5) == 1 || (u8DataTmp >> 5) == 3 || (u8DataTmp >> 5) == 4 || (u8DataTmp >> 5) == 5 || (u8DataTmp >> 5) == 6 || (u8DataTmp >> 5) == 7)
    {
        if((u8DataTmp >> 5) == 1)
        {
        ...
        else if ((u8DataTmp >> 5) == 5)
        {
            ...
            HI_INFO_TUNER("tuner port %d's demod type is hi3130v200qam outside\n", u32TunerPort);
        }
        ...
        {
            HI_INFO_TUNER("tuner port %d's demod type is hi3130v200qam inside\n", u32TunerPort);
        }
...
static HI_VOID select_demod_j83b(HI_U32 u32TunerPort, HI_U32 u32ResetGpioNo)
{
    HI_U8 u8DataTmp = 0;
#if    defined(CHIP_TYPE_hi3716mv410)   \
    || defined(CHIP_TYPE_hi3716mv420)   \
    ...
    {
        tuner_chip_reset(u32ResetGpioNo);
        hi3130v200_j83b_set_hard_reg_value(u32TunerPort);
        g_stTunerOps[u32TunerPort].tuner_set_ts_out = hi3130v200_j83b_set_ts_out;
        HI_INFO_TUNER("tuner port %d's demod type is hi3130v200qam_j83b outside\n", u32TunerPort);
    }
    else if((u8DataTmp >> 5) == 6)
    {
    ...
    {
        HI_INFO_TUNER("tuner port %d's demod type is hi3130v200qam_j83b inside\n", u32TunerPort);
    }
...
#if defined(DEMOD_DEV_TYPE_HI3130)
        g_stTunerOps[u32TunerPort].tuner_connect = hi3130v200_j83b_connect;
        g_stTunerOps[u32TunerPort].tuner_get_status = hi3130v200_j83b_get_status;
        g_stTunerOps[u32TunerPort].tuner_get_ber = hi3130v200_j83b_get_ber;
        g_stTunerOps[u32TunerPort].tuner_get_snr = hi3130v200_j83b_get_snr;
        g_stTunerOps[u32TunerPort].tuner_set_ts_type = hi3130v200_j83b_set_ts_type;
        g_stTunerOps[u32TunerPort].tuner_get_signal_strength = hi3130v200_j83b_get_signal_strength;
        g_stTunerOps[u32TunerPort].tuner_get_signal_info = hi3130v200_j83b_get_signal_info;
        g_stTunerOps[u32TunerPort].tuner_test_single_agc = hi3130v200_j83b_test_single_agc;
        g_stTunerOps[u32TunerPort].manage_after_chipreset = hi3130v200_j83b_manage_after_chipreset;
#ifdef HI_PROC_SUPPORT
        g_stTunerOps[u32TunerPort].tuner_get_registers = hi3130v200_j83b_get_registers;
#endif
        g_stTunerOps[u32TunerPort].tuner_get_rs = hi3130v200_j83b_get_rs;
        g_stTunerOps[u32TunerPort].tuner_get_freq_symb_offset = hi3130v200_j83b_get_freq_symb_offset;
        g_stTunerOps[u32TunerPort].tuner_connect_timeout = hi3130v200_j83b_connect_timeout;
...
static HI_VOID select_demod_av6211(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = avl6211_init;
...
static HI_VOID select_demod_tp5001(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = tp5001_demod_init;
...
static HI_VOID select_demod_hd2501(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = hd2501_demod_init;
...
static HI_VOID select_demod_hi3136(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = hi3136_init;
    ...
    #ifdef HI_PROC_SUPPORT
    g_stTunerOps[u32TunerPort].tuner_get_registers = hi3136_read_registers;
    #endif
}
#endif

#if defined(DEMOD_DEV_TYPE_HI3137)
static HI_VOID select_demod_hi3137(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = hi3137_init;
    ...
    #ifdef HI_PROC_SUPPORT
    g_stTunerOps[u32TunerPort].tuner_get_registers = hi3137_read_registers;
    #endif
}
#endif

#if defined(DEMOD_DEV_TYPE_MN88472)
static HI_VOID select_demod_mn88472(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = MN88472_Init;
...
static HI_VOID select_demod_mn88473(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = MN8847x_Init;
...
static HI_VOID select_demod_mxl2x4(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = mxl254_init;
    ...
    g_stTunerOps[u32TunerPort].tuner_get_data_sample = mxl254_get_data_sample;
    g_stTunerOps[u32TunerPort].tuner_get_signal_info = mxl254_get_signal_info;
    g_stTunerOps[u32TunerPort].tuner_standby = mxl254_standby;
}
#endif
...
{
    g_stTunerOps[u32TunerPort].demod_init = tda18280_init;
...
static HI_VOID select_demod_atbm888x(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = atbm888x_init;
...
static HI_VOID select_demod_mxl68x(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = mxl68x_init;
    ...
    g_stTunerOps[u32TunerPort].tuner_deinit = mxl68x_deinit;
...
static HI_VOID select_demod_avl6381(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = avl6381_init;
...
#if defined(DEMOD_DEV_TYPE_MXL251)
static HI_VOID select_demod_mxl251(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = mxl251_init;
    g_stTunerOps[u32TunerPort].tuner_connect = mxl251_connect;
    g_stTunerOps[u32TunerPort].tuner_get_status = mxl251_get_status;
    g_stTunerOps[u32TunerPort].tuner_get_ber = mxl251_get_ber;
    g_stTunerOps[u32TunerPort].tuner_get_snr = mxl251_get_snr;
    g_stTunerOps[u32TunerPort].tuner_get_signal_strength = mxl251_get_signal_strength;
    g_stTunerOps[u32TunerPort].tuner_set_ts_type = mxl251_set_ts_type;
    g_stTunerOps[u32TunerPort].tuner_connect_timeout = mxl251_connect_timeout;
}
#endif

#if defined(DEMOD_DEV_TYPE_MXL541)
static HI_VOID select_demod_mxl541(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].tuner_init = mxl541_init;
    g_stTunerOps[u32TunerPort].tuner_set_sat_attr = mxl541_set_sat_attr;
    g_stTunerOps[u32TunerPort].tuner_set_ts_type = mxl541_set_ts_type;
    g_stTunerOps[u32TunerPort].tuner_set_ts_out = mxl541_set_ts_out;
    g_stTunerOps[u32TunerPort].tuner_connect = mxl541_connect;
    g_stTunerOps[u32TunerPort].tuner_connect_timeout = mxl541_connect_timeout;
    g_stTunerOps[u32TunerPort].tuner_setfuncmode = mxl541_set_funcmode;
    g_stTunerOps[u32TunerPort].tuner_get_status = mxl541_get_status;
    g_stTunerOps[u32TunerPort].tuner_get_ber = mxl541_get_ber;
    g_stTunerOps[u32TunerPort].tuner_get_snr = mxl541_get_snr;
    g_stTunerOps[u32TunerPort].tuner_get_signal_strength  = mxl541_get_signal_strength;
    g_stTunerOps[u32TunerPort].tuner_get_signal_info = mxl541_get_signal_info;
    g_stTunerOps[u32TunerPort].tuner_get_freq_symb_offset = mxl541_get_freq_symb_offset;
    g_stTunerOps[u32TunerPort].tuner_get_powerspecdata =  mxl541_get_powerspecdata;
    g_stTunerOps[u32TunerPort].tuner_standby = mxl541_standby;
    g_stTunerOps[u32TunerPort].tuner_deinit = mxl541_deinit;
}
#endif

#if defined(DEMOD_DEV_TYPE_MXL582)
static HI_VOID select_demod_mxl582(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = mxl582_init;
    g_stTunerOps[u32TunerPort].tuner_set_sat_attr = mxl582_set_sat_attr;
    g_stTunerOps[u32TunerPort].tuner_set_ts_type = mxl582_set_ts_type;
    g_stTunerOps[u32TunerPort].tuner_connect = mxl582_connect;
    g_stTunerOps[u32TunerPort].tuner_get_status = mxl582_get_status;
    g_stTunerOps[u32TunerPort].tuner_get_ber = mxl582_get_ber;
    g_stTunerOps[u32TunerPort].tuner_get_snr = mxl582_get_snr;
    g_stTunerOps[u32TunerPort].tuner_get_signal_strength  = mxl582_get_signal_strength;
    g_stTunerOps[u32TunerPort].tuner_connect_timeout = NULL;
    g_stTunerOps[u32TunerPort].tuner_set_ter_attr = NULL;
    g_stTunerOps[u32TunerPort].tuner_get_signal_info = mxl582_get_signal_info;
    g_stTunerOps[u32TunerPort].tuner_get_powerspecdata =  NULL;
    g_stTunerOps[u32TunerPort].tuner_deinit = mxl582_deinit;
    #ifdef HI_PROC_SUPPORT
    g_stTunerOps[u32TunerPort].tuner_get_registers = NULL;
    #endif
}
#endif
...
static HI_S32 select_demod_type(HI_U32 u32TunerPort, HI_U32 u32ResetGpioNo)
{
    ...
    switch (g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        ...
        {
            select_demod_hi3130(u32TunerPort, u32ResetGpioNo);
            break;
        }
#endif
#if defined(DEMOD_DEV_TYPE_J83B)
        ...
        {
            select_demod_j83b(u32TunerPort, u32ResetGpioNo);
            break;
        }
#endif
        ...
        {
            bChipReset = HI_TRUE;
            select_demod_av6211(u32TunerPort);
            break;
        }
#endif
        ...
        {
            bChipReset = HI_TRUE;
            select_demod_tp5001(u32TunerPort);
            break;
        }
#endif
#if defined(DEMOD_DEV_TYPE_HD2501)
        ...
        {
            bChipReset = HI_TRUE;
            select_demod_hd2501(u32TunerPort);
            break;
        }
#endif
        ...
        case HI_UNF_DEMOD_DEV_TYPE_3136:
        {
            ...
            select_demod_hi3136(u32TunerPort);
            break;
        }
#endif
        ...
        {
            bChipReset = HI_TRUE;/*externel demod need chip reset, internel do not need.*/
            select_demod_hi3137(u32TunerPort);
            break;
        }
#endif
        ...
        {
            bChipReset = HI_TRUE;
            select_demod_mn88472(u32TunerPort);
            break;
        }
#endif
        ...
        {
            bChipReset = HI_TRUE;
            select_demod_mn88473(u32TunerPort);
            break;
        }
#endif
        ...
        {
            select_demod_mxl2x4(u32TunerPort);
            break;
        }
#endif
#if defined(DEMOD_DEV_TYPE_TDA18280)
        ...
        {
            select_demod_tda18280(u32TunerPort);
            break;
        }
#endif
        ...
        {
            bChipReset = HI_TRUE;
            select_demod_atbm888x(u32TunerPort);
            break;
        }
#endif
        ...
        {
            bChipReset = HI_TRUE;
            select_demod_avl6381(u32TunerPort);
            break;
        }
#endif
        ...
        {
            select_demod_mxl251(u32TunerPort);
            break;
        }
#endif
        ...
        {
            static HI_BOOL bMxl541Inited = HI_FALSE;

            if (!bMxl541Inited)
            {
                bChipReset = HI_TRUE;
                bMxl541Inited = HI_TRUE;
            }

            select_demod_mxl541(u32TunerPort);
            break;
        }
#endif
        ...
        {
            static HI_BOOL bMxl582Inited = HI_FALSE;

            if (!bMxl582Inited)
            {
                bChipReset = HI_TRUE;
                bMxl582Inited = HI_TRUE;
            }

            select_demod_mxl582(u32TunerPort);
            break;
        }
#endif
...
static HI_VOID select_tuner_mxl608(HI_U32 u32TunerPort)
{
    ...
    {
        g_stTunerOps[u32TunerPort].tuner_init = mxl608_init_tuner;
    }
...
static HI_VOID select_tuner_mxl603(HI_U32 u32TunerPort)
{
    ...
    if ((g_stTunerOps[u32TunerPort].enDemodDevType != HI_UNF_DEMOD_DEV_TYPE_3137)
     && (g_stTunerOps[u32TunerPort].enDemodDevType != HI_UNF_DEMOD_DEV_TYPE_MN88472))
    {
...
static HI_VOID select_tuner_r836(HI_U32 u32TunerPort)
{
    if (g_stTunerOps[u32TunerPort].enDemodDevType != HI_UNF_DEMOD_DEV_TYPE_3137)
    {
        g_stTunerOps[u32TunerPort].tuner_init = r836_init_tuner;
...
static HI_VOID select_tuner_tda182i5a(HI_U32 u32TunerPort)
{
    if(g_stTunerOps[u32TunerPort].enDemodDevType != HI_UNF_DEMOD_DEV_TYPE_3137)
    {
        g_stTunerOps[u32TunerPort].tuner_init = tda182i5a_init_tuner;
        g_stTunerOps[u32TunerPort].set_tuner  = tda182i5a_set_tuner_dvbc;
        g_stTunerOps[u32TunerPort].tuner_resume = tda182i5a_tuner_resume;
        g_stTunerOps[u32TunerPort].tuner_get_signal_strength = tda182i5a_get_signal_strength;
        g_stTunerOps[u32TunerPort].tuner_standby = tda182i5a_tuner_standby;
    }
}
#endif

#if defined(TUNER_DEV_TYPE_SI2144)
static HI_VOID select_tuner_si2144(HI_U32 u32TunerPort)
{
    if(g_stTunerOps[u32TunerPort].enDemodDevType != HI_UNF_DEMOD_DEV_TYPE_3137)
    {
        g_stTunerOps[u32TunerPort].tuner_init = si2144_init_tuner_dvbc;
        g_stTunerOps[u32TunerPort].set_tuner = si2144_set_tuner_dvbc;
...
static HI_VOID select_tuner_r858(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].tuner_init = r858_init_tuner;
    g_stTunerOps[u32TunerPort].set_tuner = r858_set_tuner;
    g_stTunerOps[u32TunerPort].tuner_resume = r858_tuner_resume;
    g_stTunerOps[u32TunerPort].tuner_get_signal_strength = r858_get_signal_strength;
}
#endif
...
static HI_S32 select_tuner_type(HI_U32 u32TunerPort)
{
    ...
    switch (g_stTunerOps[u32TunerPort].enTunerDevType)
    {
        ...
        {
            g_stTunerOps[u32TunerPort].tuner_init = tda18250_init_tuner;
        ...
        {
            g_stTunerOps[u32TunerPort].tuner_init = r820c_init_tuner;
        ...
        {
            g_stTunerOps[u32TunerPort].tuner_init = tda18250b_init_tuner;
        ...
        {
            g_stTunerOps[u32TunerPort].tuner_init = si2147_init_tuner;
        ...
        {
            select_tuner_mxl608(u32TunerPort);
            break;
        }
#endif
        ...
        {
            select_tuner_mxl603(u32TunerPort);
            break;
        }
#endif
        ...
        {
            select_tuner_r836(u32TunerPort);
            break;
        }
#endif
        ...
        {
            select_tuner_tda182i5a(u32TunerPort);
            break;
        }
#endif
#if defined(TUNER_DEV_TYPE_SI2144)
        ...
        {
            select_tuner_si2144(u32TunerPort);
            break;
        }
#endif
        ...
        {
            g_stTunerOps[u32TunerPort].tuner_init = r850_init_tuner;
        ...
        {
            select_tuner_r858(u32TunerPort);
            break;
        }
#endif
...
#if defined(TUNER_DEV_TYPE_MXL541)
        case HI_UNF_TUNER_DEV_TYPE_MXL541:
        {
            HI_INFO_TUNER("tuner port %d's halftuner type is MXL541\n", u32TunerPort);
            break;
        }
#endif

        case HI_UNF_TUNER_DEV_TYPE_MXL582:
    ...
    {
        s32Ret = g_stTunerOps[u32TunerPort].tuner_init(u32TunerPort);
        if(HI_SUCCESS != s32Ret)
        {
            HI_INFO_TUNER("Init_tuner error\n");
            return HI_FAILURE;
        }
    }
    return HI_SUCCESS;
                         ...
                         HI_DemodAttr_S *pstDemodAttr, HI_U32 u32ResetGpioNo)
{
    ...
    /* Added begin:sdk 2011-12-20 For AVL6211 */
    if (g_stTunerOps[u32TunerPort].demod_init)
    {
        g_stTunerOps[u32TunerPort].demod_init(u32TunerPort, g_stTunerOps[u32TunerPort].enI2cChannel, g_stTunerOps[u32TunerPort].enTunerDevType);
        //s_stTunerResumeInfo[u32TunerPort].unResumeData = unEXTTunerAttr;
    }
    /* Added begin:sdk 2011-11-28 For AVL6211 */
...
HI_S32 tuner_set_sat_attr(HI_U32 u32TunerPort, HI_UNF_TUNER_SAT_ATTR_S *pstSatTunerAttr)
{
    ...
    {
        HI_ERR_TUNER( "this device do not support this function!\n");
        return HI_SUCCESS;
    }
    ...
    switch (g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_MXL541:
        {
            g_stTunerOps[u32TunerPort].tuner_send_continuous_22K = mxl541_send_continuous_22K;
            g_stTunerOps[u32TunerPort].tuner_send_tone = mxl541_send_tone;
            g_stTunerOps[u32TunerPort].tuner_DiSEqC_send_msg = mxl541_DiSEqC_send_msg;
            g_stTunerOps[u32TunerPort].tuner_DiSEqC_recv_msg = mxl541_DiSEqC_recv_msg;
            break;
        }
#endif
        ...
        {
            g_stTunerOps[u32TunerPort].tuner_set_lnb_out = HI_NULL;
            g_stTunerOps[u32TunerPort].tuner_send_continuous_22K = mxl582_send_continuous_22K;
            //g_stTunerOps[u32TunerPort].tuner_send_tone = mxl582_send_tone;
            g_stTunerOps[u32TunerPort].tuner_DiSEqC_send_msg = mxl582_DiSEqC_send_msg;
            /*g_stTunerOps[u32TunerPort].tuner_DiSEqC_recv_msg = mxl582_DiSEqC_recv_msg;*/
    ...
    switch (pstSatTunerAttr->enDiSEqCWave)
    {
        ...
        default:
        {
            switch (pstSatTunerAttr->enLNBCtrlDev)
            {
                ...
                case HI_UNF_LNBCTRL_DEV_TYPE_A8300:
                    g_stTunerOps[u32TunerPort].tuner_lnbctrl_dev_init = a8300_init;
                    g_stTunerOps[u32TunerPort].tuner_lnbctrl_dev_standby = a8300_standby;
                    g_stTunerOps[u32TunerPort].tuner_set_lnb_out = a8300_set_lnb_out;
                    break;
#endif

#if defined(LNB_CTRL_A8297)
                case HI_UNF_LNBCTRL_DEV_TYPE_A8297:
                    g_stTunerOps[u32TunerPort].tuner_lnbctrl_dev_init = a8297_init;
                    g_stTunerOps[u32TunerPort].tuner_lnbctrl_dev_standby = a8297_standby;
                    g_stTunerOps[u32TunerPort].tuner_set_lnb_out = a8297_set_lnb_out;
                    break;
#endif
                case HI_UNF_LNBCTRL_DEV_TYPE_NONE:
                default:
                    HI_ERR_TUNER("LNB ctrl chiptype error:%d\n",pstSatTunerAttr->enLNBCtrlDev);
                    return HI_FAILURE;
            }
        ...
        case HI_UNF_TUNER_DISEQCWAVE_ENVELOPE:
        {
            switch (pstSatTunerAttr->enLNBCtrlDev)
            {
                ...
                case HI_UNF_LNBCTRL_DEV_TYPE_A8300:
                    ...
                    g_stTunerOps[u32TunerPort].tuner_lnbctrl_dev_standby = a8300_standby;
                    g_stTunerOps[u32TunerPort].tuner_set_lnb_out = a8300_set_lnb_out;
                    g_stTunerOps[u32TunerPort].tuner_send_continuous_22K = a8300_send_continuous_22K;
                    g_stTunerOps[u32TunerPort].tuner_send_tone = a8300_send_tone;
                    g_stTunerOps[u32TunerPort].tuner_DiSEqC_send_msg = a8300_DiSEqC_send_msg;
                    g_stTunerOps[u32TunerPort].tuner_DiSEqC_recv_msg = a8300_DiSEqC_recv_msg;
                    break;
#endif
                ...
                case HI_UNF_LNBCTRL_DEV_TYPE_A8297:
                    ...
                    g_stTunerOps[u32TunerPort].tuner_lnbctrl_dev_standby = a8297_standby;
                    g_stTunerOps[u32TunerPort].tuner_set_lnb_out = a8297_set_lnb_out;
                    g_stTunerOps[u32TunerPort].tuner_send_continuous_22K = a8297_send_continuous_22K;
                    g_stTunerOps[u32TunerPort].tuner_send_tone = a8297_send_tone;
                    g_stTunerOps[u32TunerPort].tuner_DiSEqC_send_msg = a8297_DiSEqC_send_msg;
                    g_stTunerOps[u32TunerPort].tuner_DiSEqC_recv_msg = a8297_DiSEqC_recv_msg;
                    break;
#endif
...
#if defined(DEMOD_DEV_TYPE_AVL6211) || defined(DEMOD_DEV_TYPE_HI3136) || defined(DEMOD_DEV_TYPE_MXL541) || defined(DEMOD_DEV_TYPE_MXL582)
...
HI_S32 tuner_get_adc_data(HI_U32 u32TunerPort, HI_U32 u32DmxPort, HI_U32 u32TimeS)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32Temp = 0;
    HI_U8 u8Str[100],u8Data=0,u8BandWidth=0,u8Statwait=0;
    ...
    if (HI_UNF_DEMOD_DEV_TYPE_3137 == g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        ...
        qam_write_byte(u32TunerPort, Hi3137_MAN_RST_CTRL1, 0x43);
        qam_read_byte(u32TunerPort, Hi3137_STATE_WAITS, &u8Statwait);
        qam_write_byte(u32TunerPort, Hi3137_STATE_WAITS, 0xff);
        qam_read_byte(u32TunerPort, Hi3137_BAND_WIDTH, &u8Data);
        u8BandWidth = u8Data >> 4;
        qam_write_byte(u32TunerPort, Hi3137_BAND_WIDTH, 0x52);
        /*****hotreset begin*****/
        qam_read_byte(u32TunerPort,Hi3137_RSTN_CTRL,&u8Data);
        u8Data = (u8Data & 0xFC) | 0x01;
        qam_write_byte(u32TunerPort,Hi3137_RSTN_CTRL,u8Data);
        tuner_mdelay(3);
        u8Data = (u8Data & 0xFC) | 0x03;
        qam_write_byte(u32TunerPort,Hi3137_RSTN_CTRL,u8Data);
        /*****hotreset end*****/
        ...
        qam_write_byte(u32TunerPort, Hi3137_MAN_RST_CTRL1, 0x5f);
        qam_write_byte(u32TunerPort, Hi3137_STATE_WAITS, u8Statwait);
        u8Data = u8BandWidth << 4;
        qam_write_byte(u32TunerPort, Hi3137_BAND_WIDTH, u8Data);
        qam_write_byte(u32TunerPort, 0x89, 0x4);
        qam_write_byte(u32TunerPort, Hi3137_MCU_HOLD, 1);
    }
    else
    {
        HI_ERR_TUNER("only hi3137 demod can record rf data.\n");
        return HI_FAILURE;
    }
    return s32Ret;
}
...
HI_S32 tuner_select_i2c(HI_U32 u32TunerPort, HI_U8 enI2cChannel, HI_U8 enTunerI2cChannel)
{
    ...
    g_stTunerOps[u32TunerPort].enTunerI2cChannel = enTunerI2cChannel;
...
HI_S32 tuner_osr_connect(PTR_TUNER_SIGNAL_S pstTunerSignal)
{
    ...
    g_stTunerOps[u32TunerPort].stCurrPara.u32TunerBandWidth = pstTunerSignal->stSignal.u32TunerBandWidth;
...
HI_S32 tuner_osr_selecttype(PTR_TUNER_ATTR_S pstTunerAttr)
{
    ...
    memset(&(g_stTunerOps[u32TunerPort]), 0, sizeof(TUNER_OPS_S));
    g_stTunerOps[u32TunerPort].enI2cChannel = pstTunerAttr->stAttr.enI2cChannel;
    g_stTunerOps[u32TunerPort].enTunerI2cChannel = pstTunerAttr->stAttr.enTunerI2cChannel;
    g_stTunerOps[u32TunerPort].enSigType = pstTunerAttr->stAttr.enSigType;

    if (HI_UNF_DEMOD_DEV_TYPE_3130I == stDemod_Attr.enDemodDevType
        || HI_UNF_DEMOD_DEV_TYPE_J83B == stDemod_Attr.enDemodDevType
        || HI_UNF_DEMOD_DEV_TYPE_3130E == stDemod_Attr.enDemodDevType)
    {
        g_stTunerOps[u32TunerPort].u32XtalClk = XTAL_CLK;
        g_stTunerOps[u32TunerPort].u32CurrQamMode = QAM_TYPE_64;
        g_stTunerOps[u32TunerPort].u8AdcType = QAM_AD_INSIDE;
        g_stTunerOps[u32TunerPort].u8AgcOutputSel = QAM_AGC_CMOS_OUTPUT;
        g_stTunerOps[u32TunerPort].u8AdcDataFmt = 1;
        g_stTunerOps[u32TunerPort].enTsType = HI_UNF_TUNER_OUTPUT_MODE_PARALLEL_MODE_A;
    }

    /* Modified begin: sdk, for outer demod configure, 2012-01-19 */
    ...
    /* Modified end: sdk, for outer demod configure, 2012-01-19 */
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 tuner_osr_selecti2cchannel(PTR_TUNER_ATTR_S pstTunerAttr)
{
    ...
    HI_U8 enTunerI2cChannel = HI_I2C_MAX_NUM_USER;
    ...
    enTunerI2cChannel = pstTunerAttr->stAttr.enTunerI2cChannel;
    if (HI_SUCCESS != tuner_select_i2c(u32TunerPort, enI2cChannel,enTunerI2cChannel))
    {
...
HI_S32 tuner_osr_get_signal_info(TUNER_SIGNALINFO_S *pstSignal)
{
    HI_U32 u32TunerPort = 0;
    HI_S32 s32Ret = 0;
...
HI_S32 tuner_osr_setfuncmode(TUNER_DATA_S *pstPara)
{
    HI_U32 u32TunerPort = 0;
    HI_S32 s32Ret = 0;
...
HI_S32 tuner_osr_setplpid(TUNER_DATABUF_S *pstPara)
{
...
HI_S32 tuner_osr_set_common_plp_id(TUNER_DATA_S *pstPara)
{
    HI_U32 u32TunerPort = 0;
    HI_S32 s32Ret = 0;
    HI_U8 u8ComPlpId = 0;
...
HI_S32 tuner_osr_set_common_plp_combination(TUNER_DATA_S *pstPara)
{
    HI_U32 u32TunerPort = 0;
    HI_S32 s32Ret = 0;
    HI_U8 u8ComPlpEna = 0;
...
HI_S32 tuner_osr_setsatattr(PTR_TUNER_SETSATATTR_S pstSetSatAttr)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_TUNER( "tuner_osr_setsatattr failed!\n");
    }
...
HI_S32 tuner_osr_setterattr(PTR_TUNER_SETTERATTR_S pstSetTerAttr)
{
    ...
    /* Modified end: sdk, for outer demod configure, 2014-02-21 */
    if (HI_SUCCESS != s32Ret)
    {
...
static HI_S32 write_sampledata(HI_U32 u32TunerPort, TUNER_SAMPLE_DATA_PARAM_S *pstSampleParam, HI_UNF_TUNER_SAMPLE_DATA_S *pstBuff)
{
    ...
    ***/
    ...
    if(TUNER_DATA_SRC_CIR_PLOT == pstSampleParam->enDataSrc)
    {
        for(i = 0; i < pstSampleParam->u32DataLen; i++)
        {
            ...
            memset(u8DataBuf, 0, sizeof(u8DataBuf));
            snprintf(u8DataBuf, sizeof(u8DataBuf), "%d ", pstBuff[i].s32DataIP);
            /***
            write file
            ***/
            if (TunerSpectrumHandle[u32TunerPort])
    ...
    else
    {
        ...
        {
            memset(u8DataBuf, 0, sizeof(u8DataBuf));
            snprintf(u8DataBuf, sizeof(u8DataBuf), "%d,%d\r\n", pstBuff[i].s32DataIP, pstBuff[i].s32DataQP);
            /***
            write file
            ***/
            if (TunerSpectrumHandle[u32TunerPort])
...
HI_S32 tuner_osr_get_sampledata(PTR_TUNER_SAMPLEDATA_S pstSampleData)
{
    ...
    HI_U32 u32Bufflen = 0;
    HI_UNF_TUNER_SAMPLE_DATA_S *pstBuff = HI_NULL;
    ...
    u32Bufflen = sizeof(HI_UNF_TUNER_SAMPLE_DATA_S) * pstSampleParam->u32DataLen;
    pstBuff = (HI_UNF_TUNER_SAMPLE_DATA_S *)HI_KMALLOC(HI_ID_TUNER, u32Bufflen, GFP_KERNEL);
    if (!pstBuff)
    {
    ...
    memset(pstBuff, 0, u32Bufflen);

    /* tuner_get_current_plp_type */
    s32Ret = g_stTunerOps[u32TunerPort].tuner_get_data_sample(u32TunerPort, pstSampleParam->enDataSrc, pstSampleParam->u32DataLen, pstBuff);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_TUNER( "\n tuner_get_data_sample failed \n");
        HI_KFREE(HI_ID_TUNER, pstBuff);
        return s32Ret;
    }

    s32Ret = copy_to_user(pstSampleParam->pstData, pstBuff, u32Bufflen);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_TUNER( "copy_to_user failed \n");
        HI_KFREE(HI_ID_TUNER, pstBuff);
        return s32Ret;
    }

    s32Ret = write_sampledata(u32TunerPort, pstSampleParam, pstBuff);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 tuner_osr_getpowerspecdata(PTR_TUNER_SPECTRUMDATA_S pstSpectrumData)
{
    ...
    u32Bufflen = sizeof(HI_S16) * stSpectrumParam->u32numOfFreqSteps;
    ps16DataBuff = (HI_S16 *)HI_KMALLOC(HI_ID_TUNER, u32Bufflen, GFP_KERNEL);
    if (!ps16DataBuff)
    {
        HI_ERR_TUNER("kmalloc failed!\n");
        return -EFAULT;
    ...
    memset(ps16DataBuff, 0, u32Bufflen);
    ...
    s32Ret = g_stTunerOps[u32TunerPort].tuner_get_powerspecdata(u32TunerPort, stSpectrumParam->u32freqStartInHz, stSpectrumParam->u32freqStepInHz, \
                                    stSpectrumParam->u32numOfFreqSteps, ps16DataBuff);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_TUNER( "\n tuner_get_powerspecdata failed \n");
        HI_KFREE(HI_ID_TUNER, ps16DataBuff);
        return s32Ret;
    }

    s32Ret = copy_to_user(stSpectrumParam->ps16powerData, ps16DataBuff, u32Bufflen);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_TUNER( "\n copy_to_user failed \n");
        HI_KFREE(HI_ID_TUNER, ps16DataBuff);
...
static HI_S32 tuner_ioctl_info(HI_U32 cmd, HI_VOID *arg)
{
...
static HI_S32 tuner_ioctl_lnb(HI_U32 cmd, HI_VOID *arg)
{
    HI_S32 s32Ret = HI_FAILURE;
...
static HI_S32 tuner_ioctl_plp(HI_U32 cmd, HI_VOID *arg)
{
    HI_S32 s32Ret = HI_FAILURE;

    switch (cmd)
    {
        case TUNER_SET_PLPNO_CMD:
        {
            TUNER_DATABUF_S *pstPara = (TUNER_DATABUF_S *)arg;
            s32Ret = tuner_osr_setplpid(pstPara);
            break;
        }
        case TUNER_SET_COMMONPLP_CMD:
        {
            TUNER_DATA_S *pstPara = (TUNER_DATA_S *)arg;
            s32Ret = tuner_osr_set_common_plp_id(pstPara);
            break;
        }
        case TUNER_SET_COMMONPLP_COMBINATION_CMD:
        {
            TUNER_DATA_S *pstPara = (TUNER_DATA_S *)arg;
            s32Ret = tuner_osr_set_common_plp_combination(pstPara);
            break;
        }
        case TUNER_GET_PLPNUM_CMD:
        {
            TUNER_DATA_S *pstPara = (TUNER_DATA_S *)arg;
            s32Ret = tuner_osr_getplpnum(pstPara);
            break;
        }
        case TUNER_GET_CURPLPTYPE_CMD:
        {
            TUNER_DATA_S *pstPara = (TUNER_DATA_S *)arg;
...
HI_S32 tuner_ioctl(struct inode *inode, struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        {
            TUNER_DATA_S *pstPara = (TUNER_DATA_S *)arg;
            s32Ret = tuner_osr_setfuncmode(pstPara);
            break;
        }
        ...
        case TUNER_GET_POWER_SPECTRUM_DATA_CMD:
        {
            s32Ret = tuner_ioctl_info(cmd, arg);
            break;
        }
        case TUNER_SET_LNBOUT_CMD:
        case TUNER_SEND_CONTINUOUS_22K_CMD:
        ...
        case TUNER_DISEQC_RECV_MSG_CMD:
        {
            s32Ret = tuner_ioctl_lnb(cmd, arg);
            break;
        }
        case TUNER_SET_PLPNO_CMD:
        case TUNER_SET_COMMONPLP_CMD:
        ...
        case TUNER_GET_GROUP_PLP_ID_CMD:
        {
            s32Ret = tuner_ioctl_plp(cmd, arg);
            break;
        }
...
HI_VOID hi3796mv200_enable_adc(HI_VOID)
{
     HI_U32 i = 30;
     ...
     unTmpPeriQamADC0.u32 = g_pstRegPeri->PERI_QAM_ADC0.u32;
     {
       ...
       unTmpPeriQamADC0.bits.qamadc_opm = 00;
     }
     g_pstRegPeri->PERI_QAM_ADC0.u32 = unTmpPeriQamADC0.u32;
     msleep(1);
     {
       /*ADC Normal Operation:PERI_QAM_ADC0[1:0]:11*/
       unTmpPeriQamADC0.bits.qamadc_opm = 3;
     }
     unTmpPeriQamADC0.bits.qamadc_selof = 0;

     g_pstRegPeri->PERI_QAM_ADC0.u32 = unTmpPeriQamADC0.u32;

     while(!g_pstRegPeri->PERI_QAM_ADC0.bits.qamadc_adcrdy)
     {
      if(!i)
      {
...
#if defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3716mv450)
HI_VOID hi3796mv200_disable_adc(HI_VOID)
{
    U_PERI_QAM_ADC0 unTmpPeriQamADC0;
...
static HI_S32 tuner_geti2cFunc(HI_VOID)
{
   ...
   if ((HI_SUCCESS != s32Ret) || !s_tuner_pGpioFunc || !s_tuner_pGpioFunc->pfnGpioDirSetBit || !s_tuner_pGpioFunc->pfnGpioWriteBit)
   {
...
static HI_VOID hi3136_resume(HI_S32 i)
{
    ...
    if (g_stTunerOps[i].demod_init)
    {
        g_stTunerOps[i].demod_init(i, g_stTunerOps[i].enI2cChannel, g_stTunerOps[i].enTunerDevType);
    }
...
HI_S32 qam_ram_collect_data(struct file * file, const char __user * buf, size_t count, loff_t *ppos)
{
    ...
    pcDataBuf = (HI_PCHAR)HI_KMALLOC(HI_ID_TUNER, sizeof(char)*RAM_SIZE*4, GFP_KERNEL);
    if(NULL == pcDataBuf)
    {
        HI_ERR_TUNER("kmalloc failed\n");
    ...
    memset(pcDataBuf, 0, sizeof(char)*RAM_SIZE*4);
    ...
    else if(1 == count)
    {
        ...
        if(u8RegVal & 0x1)
        {
            ...
            if(HI_STD_I2C_NUM >g_stTunerOps[s_u32InsideQamPort].enI2cChannel)
            {
                ...
                if(HI_SUCCESS != s32Ret)
                {
                     HI_KFREE(HI_ID_TUNER, pcDataBuf);
                     //pcDataBuf = NULL;
                     return s32Ret;
                }
            }
...
HI_S32 adc_collect_data(struct file * file, const char __user * buf, size_t count, loff_t *ppos)
{
    ...
    unTmpPeriCrg56.bits.qtc_srst_req = 0x01;

    unTmpPeriCrg56.bits.qtc_bus_cken = 0x00;
    unTmpPeriCrg56.bits.qtc_cken = 0x00;
    g_pstRegCrg->PERI_CRG56.u32 = unTmpPeriCrg56.u32;
...
static HI_S32 HI_DRV_TUNER_Connect_C(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    ...
    g_stTunerOps[u32tunerId].stCurrPara.enQamType = stSignal.enQamType;
...
static HI_S32 HI_DRV_TUNER_Connect_S(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    ...
    stSignal.u32ScrambleValue = pstConnectPara->unConnectPara.stSat.u32ScrambleValue;
...
static HI_S32 HI_DRV_TUNER_Connect_T(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    ...
    stSignal.unTer.stDVBT2Param.enChannelAttr = pstConnectPara->unConnectPara.stTer.enChannelMode;
