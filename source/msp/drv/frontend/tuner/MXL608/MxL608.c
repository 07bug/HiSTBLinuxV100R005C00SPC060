...
HI_S32 mxl608_tuner_standby(HI_U32 u32TunerPort, HI_U32 u32Status)
{
    MXL_STATUS status;

    MXL608_PWR_MODE_E powerMode = MXL608_PWR_MODE_STANDBY;
    MXL_BOOL enableLoopthrough = MXL_DISABLE;
    UINT8 standbyLt = 0;

    status = MxLWare608_API_CfgDevPowerMode(u32TunerPort, powerMode, enableLoopthrough, standbyLt);
    if(MXL_SUCCESS != status)
    {
        HI_ERR_TUNER("MxLWare608_API_CfgDevPowerMode err 0x%x\n",status);
        return status;
    }
    return HI_SUCCESS;
}
