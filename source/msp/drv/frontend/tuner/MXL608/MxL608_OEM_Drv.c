  ...
  /* If OEM data is not required, customer should treat devId as I2C slave Address */
...
#if defined (DEMOD_DEV_TYPE_HI3137) || defined (DEMOD_DEV_TYPE_ATBM888X) || defined (DEMOD_DEV_TYPE_AVL6381)
       ...
       || g_stTunerOps[devId].enDemodDevType == HI_UNF_DEMOD_DEV_TYPE_AVL6381)
    {
        ...
        for(i=0;i<10;i++)
        {
            ...
            s32Ret = tuner_i2c_send_data(g_stTunerOps[devId].enI2cChannel,g_stTunerOps[devId].u32TunerAddress,&stI2cData);
            if(!s32Ret)
                break;
        }
    ...
    if(g_stTunerOps[devId].enDemodDevType == HI_UNF_DEMOD_DEV_TYPE_MN88473)
    {
        ...
        s32Ret = tuner_i2c_send_data(g_stTunerOps[devId].enI2cChannel, (DMD_BANK_T2_)<<1 ,  &stI2cDataStr);
        if(!s32Ret)
            status = MXL_TRUE;
        else
            status = MXL_FALSE;
...

    {
        ...
        for(i=0;i<10;i++)
        {
            s32Ret = tuner_i2c_send_data(g_Mxl608I2cChnNum,g_Mxl608I2cAddr,&stI2cData);
            if(!s32Ret)
            break;
        }
        if(!s32Ret)
...
*/
        ...
        || g_stTunerOps[devId].enDemodDevType == HI_UNF_DEMOD_DEV_TYPE_AVL6381)
    {
        ...
        for (i = 0; i < 10; i++)
        {
            ...
            {
                s32Ret = HI_DRV_I2C_Read(g_stTunerOps[devId].enI2cChannel, g_stTunerOps[devId].u32TunerAddress, u32RegAddr, 2, DataPtr, 1);
            ...
            {
                s32Ret = HI_DRV_GPIOI2C_ReadExt(g_stTunerOps[devId].enI2cChannel, g_stTunerOps[devId].u32TunerAddress, u32RegAddr, 2, DataPtr, 1);
            }
    ...
    if (g_stTunerOps[devId].enDemodDevType == HI_UNF_DEMOD_DEV_TYPE_MN88473)
    {
        ...
        s32Ret = MxLWare608_OEM_WriteRegister(devId, 0xFB, RegAddr);

        buf[0] = g_stTunerOps[devId].u32TunerAddress | 0x1;

        if (HI_STD_I2C_NUM > g_stTunerOps[devId].enI2cChannel)
        {
            s32Ret = HI_DRV_I2C_Write_NoSTOP(g_stTunerOps[devId].enI2cChannel, (DMD_BANK_T2_) << 1, 0xF7, 1, buf, 1);
            s32Ret |= HI_DRV_I2C_ReadDirectly(g_stTunerOps[devId].enI2cChannel, (DMD_BANK_T2_) << 1, 0, 0, DataPtr, 1);
        }
        else
        {
            s32Ret = HI_DRV_GPIOI2C_WriteExtNoSTOP(g_stTunerOps[devId].enI2cChannel, (DMD_BANK_T2_) << 1, 0xF7, 1, buf, 1);
            s32Ret |= HI_DRV_GPIOI2C_ReadExtDirectly(g_stTunerOps[devId].enI2cChannel, (DMD_BANK_T2_) << 1, 0, 0, DataPtr, 1);
        }
        if(!s32Ret)
            status = MXL_TRUE;
        else
...
#endif
    {
        ...
        {
            s32Ret = HI_DRV_I2C_Read(g_Mxl608I2cChnNum, (HI_U8)g_Mxl608I2cAddr, u32RegAddr, 2, DataPtr, 1);
        }
        else
        {
            s32Ret = HI_DRV_GPIOI2C_ReadExt(g_Mxl608I2cChnNum, (HI_U8)g_Mxl608I2cAddr, u32RegAddr, 2, DataPtr, 1);
        }
