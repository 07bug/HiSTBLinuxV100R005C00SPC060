...
static R840_RF_Gain_TYPE R840_PreGainMode = RF_AUTO; //for test use
...
Sys_Info_Type R840_Sys_Sel(R840_Standard_Type R840_Standard)
{
	...
	switch(R840_Standard)
	{
		...
		case R840_J83B_IF_5M:
#if(R840_PULSE_GAIN==TRUE)
			R840_Sys_Info.NA_PWR_DET = 0x80;   //off
#else
			R840_Sys_Info.NA_PWR_DET = 0x00;   //on         (R30[7]=0)
#endif
			break;
...
R840_ErrCode R840_Init(HI_U32 u32TunerPort)
{
	...
	if(R840_Initial_done_flag==FALSE)
	{
          ...
          if(R840_IMR_done_flag==FALSE)
		  {
			  ...
			  if(R840_IMR(4, FALSE) != RT_Success)   //Full K node 4
				return RT_Fail;
/*
...
R840_ErrCode R840_PLL(UINT32 LO_Freq, R840_Standard_Type R840_Standard)
{
	...
	//CP current
	R840_I2C.RegAddr = 0x11;
	R840_Array[17] = (R840_Array[17] & 0xC7) | CP_CUR;  // [5:3]=CP_CUR
	R840_I2C.Data = R840_Array[17];
	if(R836_I2C_Write(&R840_I2C) != RT_Success)
	    return RT_Fail;
...
else  //R836
{
	 ...
	 else //DTV
	 {
		 ...
		 R840_I2C.RegAddr = 0x06;
#if(R840_PULSE_GAIN==TRUE)
		 R840_Array[6] = (R840_Array[6] & 0xFB) | 0x04;  //[2]=1; 1.05+/-0.25
#else
		 R840_Array[6] = (R840_Array[6] & 0xFB) | 0x00;  //[2]=0; 1.4+/-0.6
#endif
		 R840_I2C.Data = R840_Array[6];
		 if(R836_I2C_Write(&R840_I2C) != RT_Success)
			 return RT_Fail;
		 ...
		 //AGC CLK to 60hz
		 R840_I2C.RegAddr = 0x1B;
		 R840_Array[27] = (R840_Array[27] & 0xC7) | 0x30;  //R27[5:3]=110
		 R840_I2C.Data = R840_Array[27];
		 if(R836_I2C_Write(&R840_I2C) != RT_Success)
			 return RT_Fail;
#endif
...
R840_ErrCode R840_RfGainMode(R840_RF_Gain_TYPE R840_RfGainType)
{
	...
	R840_PreGainMode = R840_RfGainType;
...
R840_ErrCode R840_PulseGain(void)
{
	...
	if(FEC_Lock==0)   //0: no FEC lock
	{
		//Set NA ON
		R840_I2C.RegAddr = 0x1E;
		R840_Array[30] = (R840_Array[30] & 0x7F) | 0x00;
		R840_I2C.Data = R840_Array[30];
		if(R836_I2C_Write(&R840_I2C) != RT_Success)
			return RT_Fail;
	}
	else  //1: FEC lock
	{
		//Set NA OFF
		R840_I2C.RegAddr = 0x1E;
		R840_Array[30] = (R840_Array[30] & 0x7F) | 0x80;
		R840_I2C.Data = R840_Array[30];
		if(R836_I2C_Write(&R840_I2C) != RT_Success)
			return RT_Fail;
...
UINT8 R840_I2C_Check(void)
{
	...
	R836_I2C_Write_Len(&Dlg_I2C_Len);
...
UINT8 R840_Dump_Data(void)
{
	...
	HI_PRINT("LNA=%d, RF=%d, Mixer=%d\n", u1Gain1, u1Gain2, u1Gain3);
	HI_PRINT("RF_flag=%d, Pulse_flag=%d\n", bRfFlag, bPulseFlag);
...
HI_S32 r836_set_tuner(HI_U32 u32TunerPort, HI_U8 enI2cChannel, HI_U32 puRF)
{
   // R840_Standard_Type R840_Standard_Set;
    ...
    else
    {
        ...
        {
            R840_Info_Msg.R840_Standard = R840_DVB_C_6M;
        }
        else
        {
...
HI_S32 r836_tuner_standby(HI_U32 u32TunerPort, HI_U32 u32Status)
{
    R840_ErrCode err = RT_Success;
    R840_LoopThrough_Type R840_LoopSwitch = LT_OFF;

    R836_I2C_SetPort(u32TunerPort);

    err = R840_Standby(R840_LoopSwitch);
    if(RT_Success != err)
    {
        HI_ERR_TUNER("R840_Standby\n");
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
