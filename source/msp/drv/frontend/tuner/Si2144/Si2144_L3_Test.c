...
/* If using a Silabs Demod and > 1 tuner then undef both flags and define Si2144_DEMOD_WRAPPER_VCO (see Si2144_L2_API.c) */
#if FRONT_END_COUNT > 1
#define Si2144_USE_POSTTUNE_VCO_BLOCKING_METHOD
#undef  Si2144_USE_PRETUNE_VCO_BLOCKING_METHOD
#else
#undef Si2144_USE_POSTTUNE_VCO_BLOCKING_METHOD
#undef  Si2144_USE_PRETUNE_VCO_BLOCKING_METHOD
#endif
...
L1_Si2144_Context         *tuners[FRONT_END_COUNT];
...
int  Si2144_DisplayStatusByte              (L1_Si2144_Context *api)
{
    ...
    printf("STATUS: CTS=%d, ERR=%d, DTVINT=%d, TUNINT=%d\n", api->status->cts, api->status->err, api->status->dtvint, api->status->tunint);
...
int Si2144_demoLoop (L1_Si2144_Context *Si2144, char* choice) {
...
#ifdef    TUNER_ONLY
#ifdef    SiTRACES
    else if (strcmp_nocase(entry, "traces"   )==0) { Si2144_UserInput_traces();}
#endif /* SiTRACES */
    ...
    else if (strcmp_nocase(entry, "trace"    )==0) { Si2144_trace (Si2144); }
#endif /* TUNER_ONLY */
    ...
    else if (strcmp_nocase(entry, "Status"        )==0) { Si2144_DisplayStatusByte     (Si2144); }
    else if (strcmp_nocase(entry, "GetRevision"   )==0) { Si2144_GetRevision           (Si2144); }
#ifdef USING_DLIF_FILTER
    else if (strcmp_nocase(entry, "LoadDLIFVidFilt"   )==0) { Si2144_LoadVideofilter(Si2144,DLIF_VIDFILT_TABLE,DLIF_VIDFILT_LINES); }
#endif
   ...
   else if (stricmp(entry, "DTVTune"       )==0)
  {
    ...
    #ifdef Si2144_USE_PRETUNE_VCO_BLOCKING_METHOD
         if ((errcode =  Si2144_L2_VCO_Blocking_PreTune( tuners, fe, FRONT_END_COUNT)) != NO_Si2144_ERROR)
         {
             return errcode;
         }
    #endif // Si2144_USE_PRETUNE_VCO_BLOCKING_METHOD

    errcode=Si2144_DTVTune (Si2144, freq, Si2144->prop->dtv_mode.bw, Si2144->prop->dtv_mode.modulation, Si2144->prop->dtv_mode.invert_spectrum);
    if ((errcode != NO_Si2144_ERROR ) && (errcode != ERROR_Si2144_xTVINT_TIMEOUT))
      { printf ("Error returned from DTVTune error = %s\nPlease press <Enter> to continue\n",Si2144_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}

    #ifdef Si2144_USE_POSTTUNE_VCO_BLOCKING_METHOD
          if ((errcode =  Si2144_L2_VCO_Blocking_PostTune( tuners, fe, FRONT_END_COUNT)) != NO_Si2144_ERROR)
          {
              return errcode;
          }
    #endif // Si2144_USE_POSTTUNE_VCO_BLOCKING_METHOD
   ...
   else if (stricmp(entry,"AGCOverride"    )==0)
    {
        ...
        if      ( (strcmp_nocase(entry, "MAX")==0) ) { Si2144_AGC_Override(Si2144, Si2144_FORCE_MAX_AGC); }
        else if ( (strcmp_nocase(entry, "TOP")==0) ) { Si2144_AGC_Override(Si2144, Si2144_FORCE_TOP_AGC); }
        else if ( (strcmp_nocase(entry, "NORMAL")==0) ) { Si2144_AGC_Override(Si2144, Si2144_FORCE_NORMAL_AGC); }
        else    { printf ("Error Invalid Choice\nPlease press <Enter> to continue\n"); getc(stdin); return 1;}
    }
    else if (strcmp_nocase(entry, "m"             )==0) { Si2144_menu(1);}
    else if (strcmp_nocase(entry, "help"          )==0) { Si2144_help();}
...
int  main(int argc, char *argv[]) {
    ...
    for (fe=0; fe<FRONT_END_COUNT; fe++) {
      ...
      Si2144_L1_API_Init(front_end, tuner_addr[fe]);
      if (fe==0)
      {
 ...
 /* If using 2 tuners (or more) startup both tuners using CONFIG_I2C command to broadcast firmware download */
      ...
      if (Si2144_PowerUpUsingBroadcastI2C(tuners,FRONT_END_COUNT) != NO_Si2144_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2144's!\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("\n");
      }

#else /* use standard init for a single tuner */
      if (Si2144_Init(&(FrontEnd_Table[0])) != NO_Si2144_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2144's!\n");
