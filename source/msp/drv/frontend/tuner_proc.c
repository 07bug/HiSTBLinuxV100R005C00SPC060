...
static HI_VOID tuner_proc_get_tuner_type(HI_U32 u32TunerPort, HI_CHAR acTunerType[], HI_U32 u32ArrayLen)
{
    ...
    HI_CHAR *acTunerName[] = {"Unsupport",  "Unsupport",    "Unsupport",    "Unsupport",    "TDA18250",
                              ...
                              "R850",       "R858",         "MXL541",       "MXL582",       "unknown TUNER"};
...
HI_S32 tuner_read_constellation_data(HI_U32 u32TunerPort,HI_U32 u32DataLen)
{
    ...
    if (HI_UNF_DEMOD_DEV_TYPE_3136 != g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        HI_ERR_TUNER("only hi3136 demod can read constellation data.\n");
        return HI_SUCCESS;
    }
...
HI_VOID tuner_read_cir_plot_data(HI_U32 u32TunerPort,HI_U32 u32DataLen)
{
    ...
    if (HI_UNF_DEMOD_DEV_TYPE_3137 != g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        HI_ERR_TUNER("only hi3137 can read cirplot data.\n");
        return;
    }
        ...
        //store 0x21/0x22 register value
        qam_read_byte(u32TunerPort, Hi3137_STATE_WAITS,&u8StateWaits);
        qam_read_byte(u32TunerPort, Hi3137_MAN_RST_CTRL1,&u8RstCtrl1);
        ...
        //hot_reset
        qam_read_byte(u32TunerPort, Hi3137_RSTN_CTRL, &u8Status);
        u8Status = (u8Status & 0xFC) | 0x1;
        qam_write_byte(u32TunerPort, Hi3137_RSTN_CTRL, u8Status);
        //tuner_mdelay(2);
        u8Status = (u8Status & 0xFC) | 0x3;
        qam_write_byte(u32TunerPort, Hi3137_RSTN_CTRL, u8Status);
...
exit:
        //don't colse address increasing by itself,or errors will be occured when next catching data
        //c_stor_addrinc = 0;
        qam_read_byte(u32TunerPort, Hi3137_SDC_CTRL,&u8Status);
        u8Status &= 0xFB;
        qam_write_byte(u32TunerPort, Hi3137_SDC_CTRL,u8Status);
        ...
        //start reset
        qam_write_byte(u32TunerPort, Hi3137_STATE_WAITS,u8StateWaits);
        qam_write_byte(u32TunerPort, Hi3137_MAN_RST_CTRL1,u8RstCtrl1);
