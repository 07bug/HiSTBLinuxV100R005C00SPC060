...
HI_S32 GFX2D_HAL_HWC_Init(HI_VOID)
{
    ...
    u32NodeSize = ((sizeof(HWC_NODE_S) + 0xf) & 0xfffffff0);
                                              ...
                                              HI_GFX2D_SURFACE_S *pstDstSurface)
{
    ...
    GFX2D_CHECK_NULLPOINTER_RETURN_VALUE(pstComposeList->pstCompose, 0);
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
        ...
        if ((u8ZmeLayerCnt <= gs_stCapability.u8ZmeLayerNum) && (i != (pstComposeList->u32ComposeCnt - 1)))
        {
            continue;
        }
                                         ...
                                         HI_U8 u8NodeCnt)
{
    ...
    GFX2D_CHECK_NULLPOINTER_RETURN_VALUE(pstComposeList, 0);
    GFX2D_CHECK_NULLPOINTER_RETURN_VALUE(pstDstSurface, 0);
    GFX2D_CHECK_ARRAY_OVER_RETURN_VALUE(u8NodeCnt, (GFX2D_HAL_MAX_NODE + 1), 0);
    ...
    for (i = 0; i < u8NodeCnt; i++)
    {
        ...
        if (pstComposeList[i].u32ComposeCnt > HWC_MAX_LAYERS)
        {
            s32Ret = HI_ERR_GFX2D_NO_MEM;
            goto ERR;
        }
        ...
        if ((pstDstSurface->enFmt == HI_GFX2D_FMT_SEMIPLANAR420UV) && (pstDstSurface->u32HistogramPhyaddr != 0))
        {
            unsigned long VBaddr = 0;
                             ...
                             HI_VOID **ppNode, GFX2D_HAL_DEV_TYPE_E *penNodeType)
{
    ...
    pstNode = GFX2D_HAL_HWC_CreateNode(enDevId,stComposeList,pstDstSurface,u8NodeCnt);
    if (NULL == pstNode)
    {
        s32Ret = HI_ERR_GFX2D_UNSUPPORT;
        goto ERR;
...
{
    GFX2D_CHECK_NULLPOINTER_RETURN_NOVALUE(pstPara);
    switch (u32Index % 12)
    {
        case 0:
                               ...
                               HI_S32 *ps32ChmPara, HI_U32 u32ChmTap)
{
    ...
    GFX2D_CHECK_NULLPOINTER_RETURN_NOVALUE(pstPara);
    GFX2D_CHECK_NULLPOINTER_RETURN_NOVALUE(ps32LumPara);
    GFX2D_CHECK_NULLPOINTER_RETURN_NOVALUE(ps32ChmPara);
...
{
    HI_VOID *pVir = NULL;
    HI_S32 *ps32LumPara = NULL;
    HI_S32 *ps32ChmPara = NULL;
    ...
    LoadFilterParam((HWC_FILTERPARA_S *)pVir, ps32LumPara, HWC_FILTER_HORIZON_LUM_TAP, ps32ChmPara, HWC_FILTER_HORIZON_CHM_TAP);
...
static inline HI_VOID SetDst(HWC_NODE_S* pstNode, HI_GFX2D_SURFACE_S *pstDstSurface)
{
    ...
    if (pstDstSurface->enFmt < GFX2D_ARRAY_SIZE(gs_au32DstLayerFmt))
    {
...
{
    GFX2D_CHECK_NULLPOINTER_RETURN_NOVALUE(punHsp);
...
static HI_VOID SetZmeInfo(HWC_NODE_S *pstNode, HWC_LAYER_ID enLayerId, GFX2D_HAL_ZME_S *pstZme)
{
    ...
    if (enLayerId == HWC_LAYER_VHD)
    {
        ...
        && ((enLayerId - HWC_LAYER_G0) < (sizeof(pstNode->astGLayer) /sizeof(pstNode->astGLayer[0]))))
    {
        HWC_GLAYER_S *pstLayer = &(pstNode->astGLayer[enLayerId - HWC_LAYER_G0]);
...
{
    GFX2D_CHECK_NULLPOINTER_RETURN_VALUE(pu8LayerRefRecord, HWC_LAYER_BUTT);
    /*图形层不支持缩放，则选择视频层*/
...
static HI_S32 SelectLayerId(HI_GFX2D_COMPOSE_LIST_S *pstComposeList, HWC_LAYER_ID *penLayerId)
{
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
        ...
        {
            if (penLayerId[i] < sizeof(au8Ref)/sizeof(au8Ref[0]))
            {
                au8Ref[penLayerId[i]]++;
            }
        }
...
static inline HI_VOID SetGLayer(HWC_LAYER_ID enLayerId,HWC_NODE_S* pstNode,HI_GFX2D_COMPOSE_S *pstCompose)
{
    ...
    if ((enLayerId < HWC_LAYER_G0) || ((enLayerId - HWC_LAYER_G0) >= (sizeof(pstNode->astGLayer)/sizeof(pstNode->astGLayer[0]))))
    {
        return;
    }
    ...
    if (HI_GFX2D_SURFACE_TYPE_COLOR == pstSurface->enType)
    {
        ...
        if (pstSurface->enFmt < GFX2D_ARRAY_SIZE(gs_au32GLayerFmt))
        {
...
static inline HI_VOID SetVHDLayer(HWC_LAYER_ID enLayerId, HWC_NODE_S* pstNode, HI_GFX2D_COMPOSE_S *pstCompose)
{
    ...
    pstNode->u32VhdYSride = pstSurface->u32Stride[0] | (pstSurface->u32Stride[1] << 16);
...
{
    GFX2D_CHECK_NULLPOINTER_RETURN_NOVALUE(ppstNodeOps);
...
{
#if HWC_VERSION >= 0x200
    HI_U32 u32ErrIntStatus = 0;
#endif
    HI_U32 u32IntStatus = 0;

    GFX2D_CHECK_NULLPOINTER_RETURN_VALUE(gs_pstHWCReg,0);

#if HWC_VERSION >= 0x200
    u32ErrIntStatus = gs_pstHWCReg->u32MaskErrInt;
#endif
...
static HI_S32 CheckSurfaceStride(HI_GFX2D_SURFACE_S *pstSurface, HI_BOOL bInputSurface)
{
    ...
    if (0 == gs_stCapability.u8StrideAlign)
    {
