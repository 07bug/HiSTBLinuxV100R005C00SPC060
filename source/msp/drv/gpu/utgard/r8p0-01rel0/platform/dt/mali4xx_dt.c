...
typedef enum
{
    ...
    TEMP_REG_VALUE_HIGH = 0x22222222,
    TEMP_REG_VALUE_LOW  = 0x33333333,
} temp_reg_addr;
...
unsigned int mali_adp_get_temp_ctrl(void)
{
    ...
    if(value == TEMP_REG_VALUE_HIGH)
    {
        if(HISI_DEBUG)
            printk("mali_adp_get_temp_ctrl@ high temperature disable dvfs\n");

        return 1;
    }

#if defined(CHIP_TYPE_hi3798mv310)
    if (value == TEMP_REG_VALUE_LOW)
    {
        if(HISI_DEBUG)
            printk("mali_adp_get_temp_ctrl@ low temperature disable dvfs\n");
