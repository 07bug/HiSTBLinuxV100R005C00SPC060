...
struct mali_hw_core *hisi_sec = NULL;
...
struct mali_hw_core * hisi_sec_get(void)
{
    return hisi_sec;
}
...
_mali_osk_errcode_t mali_platform_init(void)
{
    ...
    hisi_sec = hisi_hw_core_create(HISI_SEC_BASE, SEC_REG_ADDR_SIZE, "hisi_sec");
    if (NULL == hisi_sec)
    {
        hisi_hw_core_destroy(hisi_crg);
...
int mali_secure_mode_init_hisi(void)
{
    hisi_sec_reset_disable() ;
    
    return 0 ;
}

void mali_secure_mode_deinit_hisi(void)
{
    hisi_sec_reset_disable() ;
...
int mali_gpu_reset_and_secure_mode_enable_hisi(void)
{
	hisi_sec_reset_enable() ;

	return 0 ;
}

int mali_gpu_reset_and_secure_mode_disable_hisi(void)
{
	hisi_sec_reset_disable() ;
...
_mali_osk_errcode_t mali_platform_power_mode_change(mali_power_mode power_mode, mali_bool isruntime)
{
	switch (power_mode)
	{
		...
		case MALI_POWER_MODE_DEEP_SLEEP:
			if (bPowerOff == 0)
			{
				/* if dvfs enable, we need to set the voltage and frequency to default, so that the performance is more power
				ful in most case */
				if (mali_dvfs_enable)
				{
					clk_gpu_setdefault();
...
#if  (defined(CHIP_TYPE_hi3798mv100) || defined(CHIP_TYPE_hi3796mv100)) && defined(CFG_TEMP_CTRL_CONFIG)
#define MIN_FREQ 	200000

int gpu_temperature_disable_dvfs(void)
{
	u16 temperature ;
	u32 regval;
	u8 i, j;
	...
	{		
		if(1 == temperature_disable_dvfs)
		{
			temperature_disable_dvfs = 0 ;
		}
	}
	else if(temperature > CFG_TEMP_CTRL_DOWN_THRESHOLD)
	{
		temperature_disable_dvfs = 1 ;
		gpu_dvfs_set_freq(MIN_FREQ);
	}
	
   	return temperature_disable_dvfs ;
}
#endif
...
#if (defined(CHIP_TYPE_hi3798mv100) || defined(CHIP_TYPE_hi3796mv100)) && defined(CFG_TEMP_CTRL_CONFIG)
	if( gpu_temperature_disable_dvfs() ) 
	{
		return ;
	}
#endif
