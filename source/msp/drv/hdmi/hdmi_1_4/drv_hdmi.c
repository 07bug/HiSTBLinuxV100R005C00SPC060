...
#define HDMI_CHECK_NULL_PTR(ptr)  do{\
    if (NULL == (ptr)){\
        COM_WARN("null pointer %s \n", #ptr);\
        return HI_ERR_HDMI_NUL_PTR;\
    }\
}while(0)

#define HDMI_CHECK_NULL_PTR_NORET(ptr)  do{\
    if (NULL == (ptr)){\
        COM_WARN("null pointer %s \n", #ptr);\
        return ;\
    }\
...
#define HDMI_CheckChnOpen(l_HdmiID) do{                       \
    ...
    {                                                     \
        COM_INFO("enHdmi %d is NOT open.return!\n", l_HdmiID);\
...
static HI_U32 g_HDMIWaitFlag[MAX_PROCESS_NUM] = {0};
...
HI_U32 g_u32HDMIProcid[MAX_PROCESS_NUM] = {0};


HI_S32 Hdmi_KernelInitNumClear(void)
{
    COM_INFO("g_HDMIKernelInitNum = 0\n");
...
HI_U32 DRV_HDMI_Init(HI_U32 FromUserSpace)
{
    ...
    if (bOpenAlready == HI_TRUE)
    {
        ...
        {
            COM_INFO("set bOpenAlready to TRUE.\n");
    ...
    if(bOpenAlready == HI_TRUE)
    {
        ...
        if ((g_HDMIUserInitNum > 0) || (g_HDMIKernelInitNum > 0))
        {
            ...
            COM_INFO("green channel change %d,retunr!\n",DRV_Get_IsMce2App());
            return HI_SUCCESS;   
        }
        else
        {
    ...
    if(DRV_HDMI_GetInitNum(HI_UNF_HDMI_ID_0) == 0)
    {
        ...
        memset(g_u32HDMIProcid, 0 , sizeof(HI_U32) * MAX_PROCESS_NUM);
...
HI_U32 DRV_HDMI_DeInit(HI_U32 FromUserSpace)
{
    ...
    COM_INFO("g_HDMIUserInitNum:0x%x, g_HDMIKernelInitNum:0x%x\n",
                                            g_HDMIUserInitNum,
                                            g_HDMIKernelInitNum);
...
HI_VOID hdmi_OpenNotify(HI_U32 u32ProcID,HI_UNF_HDMI_EVENT_TYPE_E event)
{
    ...
    if (DRV_Get_IsChnOpened(HI_UNF_HDMI_ID_0))
    {
        ...
        COM_INFO("u32ProcID %u,event 0x%x\n",u32ProcID,event);
...
HI_U32 DRV_HDMI_Open(HI_UNF_HDMI_ID_E enHdmi, HDMI_OPEN_S *pOpen, HI_U32 FromUserSpace,HI_U32 u32ProcID)
{
    ...
    COM_INFO("g_HDMIUserInitNum=%d,g_HDMIKernelInitNum=%d.\n",
                                          g_HDMIUserInitNum,
                                          g_HDMIKernelInitNum);
...
static HI_U32 hdmi_VideoAttrChanged(HI_UNF_HDMI_ID_E enHdmi, HDMI_VIDEO_ATTR_S *pstAttr1, HDMI_VIDEO_ATTR_S *pstAttr2,
    HI_BOOL *pVUpdate)
{
    HDMI_CHECK_NULL_PTR(pstAttr1);
    HDMI_CHECK_NULL_PTR(pstAttr2);
    HDMI_CHECK_NULL_PTR(pVUpdate);
...
{
    HDMI_ATTR_S *pstHDMIAttr = HI_NULL;

    COM_INFO(">>>in...\n");

    pstHDMIAttr = DRV_Get_HDMIAttr(enHdmi);
...
HI_U32 DRV_HDMI_GetCECAddress(HI_U8 *pPhyAddr, HI_U8 *pLogicalAddr)
{
    ...
    HDMI_CHECK_NULL_PTR(pPhyAddr);
    HDMI_CHECK_NULL_PTR(pLogicalAddr);
    if (!DRV_Get_IsCECStart(HI_UNF_HDMI_ID_0))
    {
        return HI_ERR_HDMI_DEV_NOT_OPEN;
    }
...
static HI_U32 hdmi_Create_AVI_Infoframe(HI_UNF_HDMI_AVI_INFOFRAME_VER2_S *punAVIInfoFrame, HI_U8 *pu8AviInfoFrame)
{
    ...
    hdmi_VideoIdentification_t *pstVidCode = HI_NULL;

    COM_INFO(">>>in...\n");

    HDMI_CHECK_NULL_PTR(punAVIInfoFrame);
    HDMI_CHECK_NULL_PTR(pu8AviInfoFrame);
...
static HI_U32 hdmi_Create_Audio_Infoframe(HI_UNF_HDMI_AUD_INFOFRAME_VER1_S *punAUDInfoFrame, HI_U8 *pu8AudioInfoFrame)
{
    ...
    HDMI_CHECK_NULL_PTR(punAUDInfoFrame);
    HDMI_CHECK_NULL_PTR(pu8AudioInfoFrame);
...
HI_U32 DRV_HDMI_SetInfoFrame(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_INFOFRAME_S *pstInfoFrame)
{
    ...
    {
        COM_INFO("not suport yuv422! return.\n");
        return HI_ERR_HDMI_INVALID_PARA;
    }
...
HI_U32 DRV_HDMI_ReadEvent(HI_UNF_HDMI_ID_E enHdmi,HI_U32 u32ProcID)
{
	...
	HI_U32 procID = MAX_PROCESS_NUM;

    HDMI_CHECK_ID(enHdmi);

    for (index = 0; index < MAX_PROCESS_NUM; index++)
    {
        if (u32ProcID == g_u32HDMIProcid[index])
        {
            procID = index;
            break;
        }
    ...
    if(DRV_Get_IsChnOpened(enHdmi))
    {
        ...
        COM_WARN("read start\n");
        COM_WARN("\n procID %u CurEventNo %d \n",u32ProcID,pEventList[procID].CurEventNo);
...
static void hdmi_ProcEvent(HI_UNF_HDMI_EVENT_TYPE_E event,HI_U32 u32ProcID)
{
    HI_U32 index = 0;
    HI_U32 procID = MAX_PROCESS_NUM;
    HDMI_PROC_EVENT_S *pEventList = DRV_Get_EventList(HI_UNF_HDMI_ID_0);

    COM_INFO("event:%d,g_UserCallbackFlag:%d\n",event,g_UserCallbackFlag);
    ...
    */
    ...
    if (procID == MAX_PROCESS_NUM)
    {
        COM_ERR("Invalid procID:%u in hdmi_ProcEvent\n",procID);
        return;
    }
...
void DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_TYPE_E event)
{
    ...
    pEventList = DRV_Get_EventList(HI_UNF_HDMI_ID_0);
    ...
    if (DRV_Get_IsChnOpened(HI_UNF_HDMI_ID_0))
    {
        ...
        for(u32ProcIndex = 0; u32ProcIndex < MAX_PROCESS_NUM; u32ProcIndex++)
        {
            ...
            {
                COM_INFO("proc id:%d be used\n",u32ProcIndex);
                hdmi_ProcEvent(event, g_u32HDMIProcid[u32ProcIndex]);
            }
        }
...
HI_U32 DRV_HDMI_Start(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    if(DRV_Get_IsMce2App() || DRV_Get_IsOpenedInBoot())
    {
        ...
        DRV_Set_OpenedInBoot(HI_FALSE);
        SI_EnableHdmiDevice();
        //maybe avmute on while start,so set avmute off first
...
HI_U32 DRV_HDMI_Stop(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    DRV_HDMI_CompatStopDelay(enHdmi);
...
HI_U32 DRV_HDMI_SetDeepColor(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_DEEP_COLOR_E enDeepColor)
{
    ...
    {
        COM_INFO("force to reset the whole CTRL+PHY!\n");
        SI_SW_ResetHDMITX();  
    }
...
HI_U32 DRV_HDMI_SetFormat(HI_UNF_HDMI_ID_E enHdmi, HI_DRV_DISP_FMT_E enFmt, HI_DRV_DISP_STEREO_E enStereo)
{
    ...
    COM_INFO("enEncodingFormat:%d , enStereo %d \n", enEncodingFormat,enStereo);
    ...
    //if the format change when mce to app,clear some flags,fix DTS2014082207534,add by sdk
    if (pstVidAttr->enVideoFmt != enEncodingFormat)
    {
        COM_INFO("format change when mce to app,clear bOpenMce2App & bOpenedInBoot.\n");
...
{
    HI_U32 ret = 0;
    HI_U32 u32EdidLength     = 0;
    HI_U8  u32EdidBlock[512] = {0};

    EDID_INFO(">>>in...\n");
    
    HDMI_CHECK_ID(pEDID->enHdmi);
    HDMI_CheckChnOpen(pEDID->enHdmi);
    HDMI_CHECK_NULL_PTR(pEDID);
...
HI_U32 DRV_HDMI_GetPlayStatus(HI_UNF_HDMI_ID_E enHdmi, HI_U32 *pu32Stutus)
{
    HDMI_CHECK_ID(enHdmi);
    HDMI_CheckChnOpen(enHdmi);
    HDMI_CHECK_NULL_PTR(pu32Stutus);
...
HI_U32 DRV_HDMI_LoadKey(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_LOAD_KEY_S *pstLoadKey)
{
    ...
    {
        HDCP_WARN("Invalid parameter: u32KeyLength=%d \n", pstLoadKey->u32KeyLength);
        return HI_FAILURE;
    }
    ...
    {
        /*load hdcp key */
        if (copy_from_user(stFlashEncrytedHdcpKey.u8Key, (void __user *)pstLoadKey->pu8InputEncryptedKey, pstLoadKey->u32KeyLength))
        {
            HDCP_ERR("Copy hdcp key from user failed!\n");
            return HI_FAILURE;
        }
...
HI_S32 DRV_HDMI_GetProcID(HI_UNF_HDMI_ID_E enHdmi, HI_U32 *pu32ProcID)
{
    ...
    for(u32ProcIndex = 0; u32ProcIndex < MAX_PROCESS_NUM; u32ProcIndex++)
    {
        if (g_u32HDMIProcid[u32ProcIndex] == u32CurGid)
        {
            COM_WARN("Proc ID=%u  exist!\n", u32CurGid);
            return HI_SUCCESS;
        }
        else if(HI_TRUE != pEventList[u32ProcIndex].bUsed)
        {
            *pu32ProcID = u32CurGid;
            g_u32HDMIProcid[u32ProcIndex] = u32CurGid;
...
HI_S32 DRV_HDMI_ReleaseProcID(HI_UNF_HDMI_ID_E enHdmi, HI_U32 u32ProcID)
{
    ...
    COM_INFO("u32ProcID %u\n",u32ProcID);


    for(u32ProcIndex = 0; u32ProcIndex < MAX_PROCESS_NUM; u32ProcIndex++)
    {
        if (g_u32HDMIProcid[u32ProcIndex] == u32ProcID)
        {
            g_u32HDMIProcid[u32ProcIndex] = 0;
            memset(&pEventList[u32ProcIndex], 0, sizeof(HDMI_PROC_EVENT_S));
            return HI_SUCCESS;
        }
    }
    COM_WARN("ProcID %u Already Released.\n",u32ProcID);
	COM_INFO("<<<out...\n");
...
HI_S32 DRV_HDMI_AudioChange(HI_UNF_HDMI_ID_E enHdmi, HDMI_AUDIO_ATTR_S *pstHDMIAOAttr)
{
    ...
    COM_INFO("enBitDepth:%d,u8DownSampleParm:%d,u8I2SCtlVbit:%d\n",
                                         pstHDMIAOAttr->enBitDepth,
                                         pstHDMIAOAttr->u8DownSampleParm,
                                         pstHDMIAOAttr->u8I2SCtlVbit);
    ...
    {
        COM_INFO("The same Audio Attr,return!\n");
        return HI_SUCCESS;
    }
...
{
    HI_U32 Ret                = HI_SUCCESS;
    HI_U32 u32PixelRepetition = 0;
    HI_U32 enRGBQuantization  = 0;
    ...
    HDMI_APP_ATTR_S                   *pstAppAttr      = HI_NULL;    
    HDMI_VIDEO_ATTR_S                 *pstVidAttr      = HI_NULL;
    HI_UNF_HDMI_AVI_INFOFRAME_VER2_S  *pstVIDInfoframe = HI_NULL;
...
HI_S32 DRV_HDMI_SetAPPAttr(HI_UNF_HDMI_ID_E enHdmi,HDMI_APP_ATTR_S *pstHDMIAppAttr,HI_BOOL UpdateFlag)
{
    ...
    HDMI_CHECK_NULL_PTR(pstHDMIAppAttr);
    HDMI_CHECK_ID(enHdmi);
    ...
    COM_INFO("AppAttr: bEnableHdmi=%d , bEnableAudio=%d , bEnableVideo=%d \n",
                                                  pstHDMIAppAttr->bEnableHdmi,
                                                  pstHDMIAppAttr->bEnableAudio,
                                                  pstHDMIAppAttr->bEnableVideo);
    ...
    /* copy the new app attr to local.*/
    COM_INFO("save the new app attr.\n");
...
HI_S32 DRV_HDMI_SetAOAttr(HI_UNF_HDMI_ID_E enHdmi,HDMI_AUDIO_ATTR_S *pstHDMIAOAttr,HI_BOOL UpdateFlag)
{
    ...
    || defined(CHIP_TYPE_hi3716mv420) \
    || defined(CHIP_TYPE_hi3716mv410)
    HDMI_VIDEO_ATTR_S   *pstHDMIVOAttr = HI_NULL;
#endif
    ...
    /* if not clearly indicated to be updata,then need to comfirm.*/
    if (UpdateFlag != HI_TRUE)
    {
    ...
    || defined(CHIP_TYPE_hi3716mv420) \
    || defined(CHIP_TYPE_hi3716mv410)
        pstHDMIVOAttr = DRV_Get_VideoAttr(enHdmi);
...
HI_S32 DRV_HDMI_SetVOAttr(HI_UNF_HDMI_ID_E enHdmi,HDMI_VIDEO_ATTR_S *pstHDMIVOAttr,HI_BOOL UpdateFlag)
{
    ...
    pstVidAttr = DRV_Get_VideoAttr(enHdmi);
    pstAppAttr = DRV_Get_AppAttr(enHdmi);
    Fmt = DRV_HDMI_Disp2UnfFmt(pstHDMIVOAttr->enVideoFmt);

    if (pstAppAttr->bEnableVideo != HI_TRUE)
    {
        COM_WARN("bEnableVideo Must be set to HI_TRUE!\n");
    ...
    COM_INFO("VUpdate %d,bEnableAviInfoFrame=%d,bEnableSpdInfoFrame=%d\n",
                                          VUpdate,
                                          pstAppAttr->bEnableAviInfoFrame,
                                          pstAppAttr->bEnableSpdInfoFrame);
    ...
    if ((VUpdate == HI_TRUE) && (pstAppAttr->bEnableVideo == HI_TRUE))
    {
        HI_U8 u8VideoPath[4];
...
{
    HDMI_CHECK_NULL_PTR_NORET(infor_ptr);
