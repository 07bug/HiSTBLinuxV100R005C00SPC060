...
void SI_WriteDefaultConfigToEEPROM( void)
{
    ...
    // use LONG parser
    SI_WriteByteEEPROM(EE_EDID_PARSER_ADDR, 0); 

    // 1==480p // 2==720p
    SI_WriteByteEEPROM(EE_TX_VIDEO_MODE_INDEX_ADDR, 1);   

    //ixel data is not replicated 0b01 =
    SI_WriteByteEEPROM(EE_TX_ICLK_ADDR, 0); 

    // DE disable and force HDMI mode
    SI_WriteByteEEPROM(EE_TX_DE_ENABLED_ADDR, 0);         

    //256Fs
    SI_WriteByteEEPROM(EE_TX_MCLK_INDEX_ADDR, 0x01);      

    //MCLK_EN  Internal generate
    SI_WriteByteEEPROM(EE_TX_INTERNAL_MCLK_ADDR,0x01);

    SI_WriteByteEEPROM(EE_TX_FPGA, 0xFF);
    /* hdcp set to default disable */
    SI_WriteByteEEPROM(EE_TX_HDCP, 0/*0xFF*/);      
