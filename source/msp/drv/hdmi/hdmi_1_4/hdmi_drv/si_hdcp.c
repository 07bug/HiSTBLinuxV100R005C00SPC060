...
static struct timeval TimeR0 = {0}, TimeSKVList = {0};
...
#endif
        HDCP_INFO("Enter Protocol step 2:repeater authetication\n");
        SI_timer_count();
...
void SI_ReAthentication( void )
{
    ...
    HDCP_INFO("HDCP AUTO Check RI\n");

    //if(uRSEN != BIT_RSEN)
    if(!SI_RSEN_Status())
    {
    	HDCP_INFO("Sink is in standy mode\n");
