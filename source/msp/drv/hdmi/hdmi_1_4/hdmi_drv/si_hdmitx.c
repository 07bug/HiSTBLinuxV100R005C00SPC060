...
void SI_CheckClockStable(void)
{
    ...
    {
        COM_INFO("TClock Stable:%d,return!\n", siiIsTClockStable());
        return;
    }
    ...
    while ( !siiIsTClockStable() && --TimeOut )
    {
        //wait for input pixel clock to stabilze
        DelayMS(5);         
    }
...
#if 0 /*--sdk said need cfg to 0x37--*/
...
{
    // En.SPDIF
    WriteByteHDMITXP1(AUD_MODE_ADDR, 0x03);     

    /* clock set */
    SI_WriteNValue(6144);
    
    // En. N/CTS packets
    // WriteByteHDMITXP1( ACR_CTRL_ADDR, 0x02);  
    // should set MCLK_EN(bit2) to 1,  En. N/CTS packets
    WriteByteHDMITXP1( ACR_CTRL_ADDR, 0x06);    
...
void SI_Start_HDMITX(void)
{
    ...
    {
        COM_INFO("Enable AVI InfoFrame!\n");
        SI_EnableInfoFrame(AVI_TYPE);
    }
    else
    {
        COM_INFO("Disable AVI InfoFrame!\n");
        SI_DisableInfoFrame(AVI_TYPE);
    }
    ...
    {
        COM_INFO("Enable AUD InfoFrame!\n");
        SI_EnableInfoFrame(AUD_TYPE);
    }
    else
    {
        COM_INFO("Disable AUD InfoFrame!\n");
        SI_DisableInfoFrame(AUD_TYPE);
    }
