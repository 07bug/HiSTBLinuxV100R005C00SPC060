...
static UMAP_DEVICE_S   g_hdmiRegisterData = { {0} };
...
static HI_U8 g_pDispFmtString[HI_DRV_DISP_FMT_BUTT+1][DEF_FILE_NAMELENGTH] = { {0} };

static HI_U8 g_pUnfFmtString[HI_UNF_ENC_FMT_BUTT+1][DEF_FILE_NAMELENGTH] = { {0} };
...
static HI_S32 HDMI0_Proc(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "%-20s: ","Hotplug");
    if(stHdmiStatus.bConnected)
    {
        PROC_PRINT(p, "%-20s| ", "Enable");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", "Disable");
    }
    PROC_PRINT(p, "%-20s: ","Thread");
    ...
    PROC_PRINT(p, "%-20s: ","Sink");
    if(stHdmiStatus.bSinkPowerOn)
    {
        PROC_PRINT(p, "%-20s| ", "Active");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", "Deactive");
    }
    PROC_PRINT(p, "%-20s: ","HDCP Enable");
    ...
    PROC_PRINT(p, "%-20s: ","HDCP Encryption");
    ...
    PROC_PRINT(p, "%-20s: ","Play Status");
    DRV_HDMI_GetPlayStatus(0,&u32PlayStatus);
    if(u32PlayStatus)
    {
        PROC_PRINT(p, "%-20s| ", "Start");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", "Stop");
    }
    PROC_PRINT(p, "%-20s: ","HDCP KEY CRC");
    s32Temp = DRV_ReadByte_8BA(0, TX_SLV0, 0xF9);

    if(pstAppAttr->bHDCPEnable)
    {
        PROC_PRINT(p, "%s",((s32Temp&0x03) == 0x01) ? "OK" : "Error");
    }
    else
    {
        PROC_PRINT(p, "%s", " - ");
    ...
    PROC_PRINT(p, "%-20s: ","EDID Status");
    if(DRV_Get_IsValidSinkCap(HI_UNF_HDMI_ID_0))
    {
        PROC_PRINT(p, "%-20s| ", "Valid");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", "Unvalid");
    }

    PROC_PRINT(p, "%-20s: ","CEC Status");
    ...
    PROC_PRINT(p, "%-20s: ","Output Mode");
    u32Reg = ReadByteHDMITXP1(0x2F);
    if(u32Reg & 0x01)
    {
        PROC_PRINT(p, "%-20s| ", "HDMI");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", "DVI");
    }

    PROC_PRINT(p, "%-20s: ","CEC Phy Addr");

    PROC_PRINT(p, "%02d.%02d.%02d.%02d\n", CECStatus.u8PhysicalAddr[0],
        CECStatus.u8PhysicalAddr[1],CECStatus.u8PhysicalAddr[2],CECStatus.u8PhysicalAddr[3]);


    PROC_PRINT(p, "%-20s: ","Default Mode");
    u32DefHDMIMode = DRV_Get_DefaultOutputMode(HI_UNF_HDMI_ID_0);
    if (u32DefHDMIMode >= HI_UNF_HDMI_DEFAULT_ACTION_BUTT)
    {
        COM_WARN("Invalid parameter: u32DefHDMIMode = %d \n", u32DefHDMIMode);
        u32DefHDMIMode = HI_UNF_HDMI_DEFAULT_ACTION_BUTT;
    }
    PROC_PRINT(p, "%-20s| ", g_pDefHDMIMode[u32DefHDMIMode]);
    PROC_PRINT(p, "%-20s: ","CEC Logical Addr");
    PROC_PRINT(p, "%d\n", CECStatus.u8LogicalAddr);


    PROC_PRINT(p, "%-20s: ","Force SetFmt Delay");
    if(IsForceFmtDelay())
    {
        PROC_PRINT(p, "%-20s| ", "Force");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", "Default");
    }

    PROC_PRINT(p, "%-20s: ","AVMUTE");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0xDF);  // 0x7A:0xDF
    if ( 0x10 == (u32Reg & 0x10))
    {
        PROC_PRINT(p, "%s ", "Disable");
    }
    else if ( 0x01 == (u32Reg & 0x01))
    {
        PROC_PRINT(p, "%s ", "Enable");
    }
    else
    {
        PROC_PRINT(p, "%s ", "Unknown");
    }
    PROC_PRINT(p, "\n");


    PROC_PRINT(p, "%-20s: ","Rsen Detect Mode");
    if (DRV_Get_ForcePowerState())
    {
        PROC_PRINT(p, "%-20s| ", "Force do not detect Rsen");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", "Default");
    ...
    PROC_PRINT(p, "%-20s: ","Force Mute Delay");
    if (IsForceMuteDelay())
    {
        PROC_PRINT(p, "%s ", "Force");
    }
    else
    {
        PROC_PRINT(p, "%s ", "Default");
    }
    PROC_PRINT(p, "\n");
    ...
    PROC_PRINT(p, "%-20s: ","Out Timing");

    PROC_PRINT(p, "%dx%d",w,h);
    if (w|h)
    {
    ...
    PROC_PRINT(p, "---------------- Video -------------------|---------------- Audio -------------------\n");

    PROC_PRINT(p, "%-20s: ","Video Output ");
    ...
    {
        PROC_PRINT(p, "%-20s| ", g_pDispFmtString[pstVideoAttr->enVideoFmt]);
    }
    else
    {
    ...
    PROC_PRINT(p, "%-20s: ","Input Type");
    if (pstAudioAttr->enSoundIntf < HDMI_AUDIO_INTERFACE_BUTT)
    {
        PROC_PRINT(p, "%s\n", g_pAudInputType[pstAudioAttr->enSoundIntf]);
    }
    else
    {
        PROC_PRINT(p, "%s\n", g_pAudInputType[HDMI_AUDIO_INTERFACE_BUTT]);
    }

    PROC_PRINT(p, "%-20s: ","Color Space");
    if (pstAppAttr->enVidOutMode < HI_UNF_HDMI_VIDEO_MODE_BUTT)
    {
        PROC_PRINT(p, "%-20s| ", g_pColorSpace[pstAppAttr->enVidOutMode]);
    }
    else
    {
        COM_WARN("Invalid parameter: enVidOutMode=%d \n", pstAppAttr->enVidOutMode);
    }
    PROC_PRINT(p, "%-20s: ","Sample Rate");
    PROC_PRINT(p, "%dHz\n", pstAudioAttr->enSampleRate);

    PROC_PRINT(p, "%-20s: ","DeepColor");
    if (HI_UNF_HDMI_DEEP_COLOR_OFF <= pstAppAttr->enDeepColorMode)
    {
        PROC_PRINT(p, "%-20s| ", "OFF");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", g_pDeepColor[pstAppAttr->enDeepColorMode]);
    }
    PROC_PRINT(p, "%-20s: ","Bit Depth");
    PROC_PRINT(p, "%dbit\n", pstAudioAttr->enBitDepth);


    PROC_PRINT(p, "%-20s: ","xvYCC");
    ...
    PROC_PRINT(p, "%-20s: ","Global SetFmt Delay");
    PROC_PRINT(p, "%-20d| ", GetGlobalFmtDelay());


    PROC_PRINT(p, "%-20s: ","CTS");
    ...
    PROC_PRINT(p, "%-20s: ","Global Mute Delay");
    PROC_PRINT(p, "%-20d| ",GetGlobalsMuteDelay());

    SI_GetErrCount(&HDMI_INTErrCount);
    PROC_PRINT(p, "%-20s: ","Audio FIFO Overflow");
    PROC_PRINT(p, "%-20d  ",HDMI_INTErrCount.overrun);
    PROC_PRINT(p, "\n");

    PROC_PRINT(p, "%-20s: ","TMDS Clock");
    ...
    PROC_PRINT(p, "%-20s| ", u8Buf);

    PROC_PRINT(p, "%-20s: ","Audio FIFO Underflow");
    PROC_PRINT(p, "%-20d  ",HDMI_INTErrCount.under_run);
    PROC_PRINT(p, "\n");

    u32Reg = ReadByteHDMITXP0(TX_STAT_ADDR);
    PROC_PRINT(p, "%-20s: ","Pixel Clk Stable");
    if((u32Reg & BIT_HDMI_PSTABLE)!=0)
    {
        PROC_PRINT(p, "%-20s| ","stable");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ","unstable");
    }

    PROC_PRINT(p, "%-20s: ","S/PDIF Parity Err");
    PROC_PRINT(p, "%-20d  ",HDMI_INTErrCount.spdif_par);
    PROC_PRINT(p, "\n");

    PROC_PRINT(p, "---------------------------------- Info Frame status --------------------------------\n");
    PROC_PRINT(p, "%-25s: ","AVI InfoFrame");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0x3E);  // 0x7A:0x3E
    if ( 0x03 == (u32Reg & 0x03))
    {
        PROC_PRINT(p, "%-15s| ", "Enable");
    }
    else
    {
        PROC_PRINT(p, "%-15s| ", "Disable");
    }
    PROC_PRINT(p, "%-23s: ","Gamut Metadata Packet");
    ...
    PROC_PRINT(p, "%-25s: ","AUD InfoFrame");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0x3E);  // 0x7A:0x3E
    if ( 0x30 == (u32Reg & 0x30))
    {
        PROC_PRINT(p, "%-15s| ", "Enable");
    ...
    PROC_PRINT(p, "%-23s: ","Generic Packet");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0x3F);  // 0x7A:0x3F
    if(0x03 == (u32Reg & 0x03))
    {
        PROC_PRINT(p, "%s\n", "Enable");
    ...
    PROC_PRINT(p, "%-25s: ","MPg/VendorSpec InfoFrame");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0x3E);  // 0x7A:0x3E
    if ( 0xC0 == (u32Reg & 0xC0))
    {
        PROC_PRINT(p, "%-15s| ", "Enable");
    ...
    PROC_PRINT(p, "\n");
    PROC_PRINT(p, "MPg/VendorSpec Inforframe :\n");
    ...
    PROC_PRINT(p, "\n");
    PROC_PRINT(p, "------------------------------------ Parsed InfoFrame -------------------------------\n");
    PROC_PRINT(p, "%-20s: ","Video ID Code(VIC)");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0x47);
    PROC_PRINT(p, "0x%-18x| ", u32Reg);
    PROC_PRINT(p, "%-20s: ","Colorimetry");
    ...
    PROC_PRINT(p, "%-20s: ","Pixel Repetition");
    ...
    PROC_PRINT(p, "%-20s| ", g_pPixelRep[u32Reg]);
    PROC_PRINT(p, "%-20s: ","ScanInfo");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0x44);
    u32Reg &= 0x03;
    PROC_PRINT(p, "%s\n", g_pScanInfo[u32Reg]);

    PROC_PRINT(p, "%-20s: ","Output Color Space");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0x44);
    u32Reg &= 0x60;
    u32Reg >>=5;
    PROC_PRINT(p, "%-20s| ", g_pColorSpace[u32Reg]);
    PROC_PRINT(p, "%-20s: ","AspectRatio");
    u32Reg = DRV_ReadByte_8BA(0, TX_SLV1, 0x45);
    u32Reg &= 0x30;
    if(0x00 == u32Reg)
    {
...
static HI_S32 HDMI0_Sink_Proc(struct seq_file *p, HI_VOID *v)
{
    ...
    {
        PROC_PRINT(p, "%-20s| ", "Failed");
    }
    PROC_PRINT(p, "%-20s: ","TV Manufacture Name");
    PROC_PRINT(p, "%s\n", pSinkCap->stMfrsInfo.u8MfrsName);

    PROC_PRINT(p, "%-20s: ","Source of EDID");

    if(DRV_Get_IsUserEdid(HI_UNF_HDMI_ID_0))
    {
        PROC_PRINT(p, "%-20s| ","User Setting");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ","From Sink");
    }

    PROC_PRINT(p, "%-20s: ","ProductCode");
    PROC_PRINT(p, "%d\n", pSinkCap->stMfrsInfo.u32ProductCode);

    PROC_PRINT(p, "%-20s: ","Hdmi Support");
    if(pSinkCap->bSupportHdmi)
    {
        PROC_PRINT(p, "%-20s| ", "TRUE");
    }
    else
    {
        PROC_PRINT(p, "%-20s| ", "FALSE");
    }
    PROC_PRINT(p, "%-20s: ","SerialNumber");
    PROC_PRINT(p, "%d\n", pSinkCap->stMfrsInfo.u32SerialNumber);

    PROC_PRINT(p, "%-20s: ","EDID Version");
    PROC_PRINT(p, "%d.%-18d| ", pSinkCap->u8Version,pSinkCap->u8Revision);
    PROC_PRINT(p, "%-20s: ","Week Of Manufacture");
    PROC_PRINT(p, "%d\n", pSinkCap->stMfrsInfo.u32Week);

    PROC_PRINT(p, "%-20s: ","Extend Block Num");
    PROC_PRINT(p, "%-20d| ", pSinkCap->u8ExtBlockNum);
    PROC_PRINT(p, "%-20s: ","Year Of Manufacture");
    PROC_PRINT(p, "%d\n", pSinkCap->stMfrsInfo.u32Year);

    PROC_PRINT(p, "%-20s: ","DVI Dual");
    ...
    PROC_PRINT(p, "%-20s: ","CEC PhyAddr Valid");
    ...
    PROC_PRINT(p, "%-20s: ","CEC Phy Add");
    PROC_PRINT(p, "%02x.%02x.%02x.%02x\n", pSinkCap->stCECAddr.u8PhyAddrA,
    ...
    for(i = 0,j = 0; i < HI_UNF_ENC_FMT_BUTT; i++)
    {
        ...
        {
            PROC_PRINT(p, "%s / ", g_pUnfFmtString[i]);
            j++;
            if(0 == j%6)
            {
                PROC_PRINT(p, "\n%-22s","");
    ...
    PROC_PRINT(p, "\n");
    PROC_PRINT(p, "%-20s: ","Native Format");
    if (pSinkCap->enNativeFormat < HI_UNF_ENC_FMT_BUTT)
    {
        PROC_PRINT(p, "%s\n", g_pUnfFmtString[pSinkCap->enNativeFormat]);
    }
    else
    {
        PROC_PRINT(p, "%s\n", g_pUnfFmtString[HI_UNF_ENC_FMT_BUTT]);
    }
    PROC_PRINT(p, "%-20s: ","Colorimetry");
    if(pSinkCap->stColorMetry.bxvYCC601)
    {
        PROC_PRINT(p, "%s", "xvYCC601 / ");
    }
    if(pSinkCap->stColorMetry.bxvYCC709)
    {
        PROC_PRINT(p, "%s", "xvYCC709 / ");
    }
    PROC_PRINT(p, "\n");

    PROC_PRINT(p, "%-20s: ","Color Space");
    PROC_PRINT(p, "%s", "RGB444");
    if(pSinkCap->stColorSpace.bYCbCr444)
    {
        PROC_PRINT(p, " / %s", "YCbCr444");
    }

    if(pSinkCap->stColorSpace.bYCbCr422)
    {
        PROC_PRINT(p, " / %s", "YCbCr422");
    }

    PROC_PRINT(p, "\n");

    PROC_PRINT(p, "%-20s: ","Deep Color");
    PROC_PRINT(p, "%s", "24");
    ...
    PROC_PRINT(p, "%-16s| ","Audio Fmt");
    PROC_PRINT(p, "%-15s| ","Chn");
    PROC_PRINT(p, "%-25s","samplerate");
    //PROC_PRINT(p, "%-10s","Extend");
    PROC_PRINT(p, "\n");

    for(i = 0; i < HI_UNF_EDID_MAX_AUDIO_CAP_COUNT; i++)
    {
        if ( (pSinkCap->stAudioInfo[i].enAudFmtCode) && (pSinkCap->stAudioInfo[i].enAudFmtCode <HI_UNF_EDID_AUDIO_FORMAT_CODE_BUTT) )
        {
            PROC_PRINT(p, "%-3d %-12s| ",pSinkCap->stAudioInfo[i].enAudFmtCode, g_pAudioFmtCode[pSinkCap->stAudioInfo[i].enAudFmtCode]);
            PROC_PRINT(p, "%-15d| ",pSinkCap->stAudioInfo[i].u8AudChannel);

            for (j = 0; j < MAX_SAMPE_RATE_NUM; j++)
            {
                if(pSinkCap->stAudioInfo[i].enSupportSampleRate[j] != 0)
                {
                    PROC_PRINT(p, "%d ", (pSinkCap->stAudioInfo[i].enSupportSampleRate[j]));
                }
            }
            PROC_PRINT(p, "Hz");
    ...
    PROC_PRINT(p, "Max Audio PCM channels: %d\n", pOldAudioCap->u32MaxPcmChannels);
    ...
    PROC_PRINT(p, "------------------------------ Custom Perfer Timing ---------------------------------\n");

    PROC_PRINT(p, "%-20s: ","VFB");
    PROC_PRINT(p, "%-20d| ", pSinkCap->stPerferTiming.u32VFB);

    PROC_PRINT(p, "%-20s: ","HFB");
    PROC_PRINT(p, "%d\n",  pSinkCap->stPerferTiming.u32HFB);

    PROC_PRINT(p, "%-20s: ","VBB");
    PROC_PRINT(p, "%-20d| ", pSinkCap->stPerferTiming.u32VBB);

    PROC_PRINT(p, "%-20s: ","HBB");
    PROC_PRINT(p, "%d\n",  pSinkCap->stPerferTiming.u32HBB);

    PROC_PRINT(p, "%-20s: ","VACT");
    PROC_PRINT(p, "%-20d| ", pSinkCap->stPerferTiming.u32VACT);

    PROC_PRINT(p, "%-20s: ","HACT");
    PROC_PRINT(p, "%d\n",  pSinkCap->stPerferTiming.u32HACT);

    PROC_PRINT(p, "%-20s: ","VPW");
    PROC_PRINT(p, "%-20d| ", pSinkCap->stPerferTiming.u32VPW);

    PROC_PRINT(p, "%-20s: ","HPW");
    PROC_PRINT(p, "%d\n",  pSinkCap->stPerferTiming.u32HPW);


    PROC_PRINT(p, "%-20s: ","IDV");
    ...
    PROC_PRINT(p, "%-20s: ","Image Width");
    PROC_PRINT(p, "%-20d| ", pSinkCap->stPerferTiming.u32ImageWidth);

    PROC_PRINT(p, "%-20s: ","Image Height");
    PROC_PRINT(p, "%d\n",  pSinkCap->stPerferTiming.u32ImageHeight);


    PROC_PRINT(p, "%-20s: ","Interlace");
    ...
    PROC_PRINT(p, "%-20s: ","Pixel Clock");
    PROC_PRINT(p, "%d\n",  pSinkCap->stPerferTiming.u32PixelClk);

    PROC_PRINT(p, "------------------------------------ EDID Raw Data ---------------------------------- \n");
...
HI_S32 hdmi_ProcWrite(struct file * file, const char __user * buf, size_t count, loff_t *ppos)
{
    HI_CHAR  chCmd[60] = {0};
...
#ifdef HDMI_DEBUG
    HI_CHAR  chArg3[DEF_FILE_NAMELENGTH] = {0}; // path
#endif
