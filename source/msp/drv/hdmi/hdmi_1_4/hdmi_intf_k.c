...
#define CHECK_POINTER(p) do{\
    if(HI_NULL == p){\
        COM_ERR("The pointer is null\n");\
        return HI_FAILURE;\
    }\
}while(0)

#define CHECK_POINTER_NO_RET(p) do{\
    if(HI_NULL == p){\
        COM_ERR("The pointer is null\n");\
        return;\
    }\
...
extern HI_U32 g_u32HDMIProcid[MAX_PROCESS_NUM];
...
void HDMI_IRQ_Setup(void)
{
    if (g_HDMIIrqHandle != 0)
    {
        ...
        osal_free_irq(HDMI_IRQ_NUM, "hdmi", &g_HDMIIrqHandle);
        g_HDMIIrqHandle = 0;
    }
    ...
    if (osal_request_irq(HDMI_IRQ_NUM, (irq_handler_t)HDMI_Isr, IRQF_SHARED, "hdmi", &g_HDMIIrqHandle) != 0)
    {
...
HI_S32 hdmi_Close(struct inode *inode, struct file *filp)
{
    ...
    for(u32Index = 0; u32Index < MAX_PROCESS_NUM; u32Index++)
    {
        ...
        {
            DRV_HDMI_ReleaseProcID(HI_UNF_HDMI_ID_0, g_u32HDMIProcid[u32Index]);
            break;
        }
    }
    ...
    /* when CTRL +C ,mpi close eixt.Kernel open cnt should clear. */
    Hdmi_KernelInitNumClear();
...
HI_S32 hdmi_Suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    ...
    {
    	COM_WARN("HDMI Do not setup before\n");
    	return 0;
    }
...
HI_S32 hdmi_Resume(PM_BASEDEV_S *pdev)
{
    ...
    if (0 == suspend_flag)
    {
        COM_INFO("suspend_flag = %d,return.\n",suspend_flag);
	    return 0;
    }
...
static HI_S32 hdmi_ProcessCmd(unsigned int cmd, HI_VOID *arg, HI_BOOL bUser)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_HDMI_CLOSE:
        {
            ...
            CHECK_POINTER_BREAK(phdmiClose);
        ...
        case CMD_HDMI_START:
        {
            ...
            CHECK_POINTER_BREAK(phdmiStart);
        ...
        case CMD_HDMI_SINK_CAPABILITY:
        {
            ...
            CHECK_POINTER_BREAK(phdmisinkcap);
        ...
        case CMD_HDMI_POLL_EVENT:
        {
            ...
            CHECK_POINTER_BREAK(pPollEvent);
        ...
        case CMD_HDMI_GET_INFORFRAME:
        {
            ...
            CHECK_POINTER_BREAK(pInfoframe);
        ...
        case CMD_HDMI_PREVTIMING:
        {
            ...
            CHECK_POINTER_BREAK(pstPreVideoTiming);
        ...
        case CMD_HDMI_GET_DEEPCOLOR:
        {
            ...
            CHECK_POINTER_BREAK(pDeepcolormode);
        ...
        case CMD_HDMI_SET_XVYCC:
        {
            ...
            CHECK_POINTER_BREAK(pxvYCCmode);
        ...
        case CMD_HDMI_CECSTATUS:
        {
            ...
            CHECK_POINTER_BREAK(pCECStatus);
        ...
        case CMD_HDMI_GET_HDMI_PLAYSTAUS:
        {
            ...
            CHECK_POINTER_BREAK(pPlayStaus);
		...
		case CMD_HDMI_REG_CALLBACK_FUNC:
		{
            ...
            CHECK_POINTER_BREAK(pstRegCallbackFunc);
		...
		case CMD_HDMI_LOADKEY:
		{
            ...
            CHECK_POINTER_BREAK(pstLoadKey);
        ...
        case CMD_HDMI_GET_AO_ATTR:
        {
            ...
            CHECK_POINTER_BREAK(pstGetAOAttr);
...
void hdmi_MCE_ProcHotPlug(HI_HANDLE hHdmi)
{
    ...
    HDMI_START_S             stHDMIStart    = {0};
    HDMI_STATUS_S            stHdmiStatus   = {0};
    HDMI_PORT_ATTR_S         stHDMIPortAttr = {0};
    ...
    if(DRV_Get_IsValidSinkCap(HI_UNF_HDMI_ID_0))
    {
        ...
        COM_INFO("bEnableHdmi=%d, enVidOutMode=%d\n",pstAppAttr->bEnableHdmi,pstAppAttr->enVidOutMode);
    }
    else
    {
...
{
    HI_S32 ret = 0;
    HDMI_INIT_S stHDMIInit = {0};
    HDMI_OPEN_S stHDMIOpen = {0};
    HDMI_GET_PROCID_S stHDMIProcIDOpen = {0};

    COM_INFO(">>>in...\n");

    memset(&stHDMIProcIDOpen,0,sizeof(HDMI_GET_PROCID_S));
    g_u32ProcHandle = (HI_U32)HI_NULL;
