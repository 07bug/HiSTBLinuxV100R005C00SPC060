...
{
    HDMI_EDID_MANUFACTURE_INFO_S stEdid;
    HDMI_HPD_FILLTER_S           stHpdFillter;
}COMPAT_HPD_FILLTER_S;
typedef struct{
    HDMI_EDID_MANUFACTURE_INFO_S    stEdid;
...
static COMPAT_AUDIO_NCTS_S s_astSinkAudioNCts[] =
{
    ...
    {   {"DEF" ,        0,       0,   0,    0}, HDMI_SAMPLE_RATE_48K, 297000, 6144, 297000},        //default
/* N value is modified to 6144 default, refer to hdmi_hal_ncts.c / hdmi_hal.c
    ...
    {   {"SHP" ,     4411,       0, 255, 2015}, HDMI_SAMPLE_RATE_48K, 297000, 6144, 297000},        //sharp LCD-70UF30A
    {   {"KOA" ,       48,       2,  16, 2015}, HDMI_SAMPLE_RATE_48K, 297000, 6144, 297000},        //
*/
};
...
static COMPAT_HDR_S s_stSinkHdrDebugMode[] =
{
    ...
    // 20171121: this compatibility will cause some trouble
    //  resume.start->HdrAttrSet.OE off->second open->hardwareinit->DVI(not output)
...
static COMPAT_HPD_FILLTER_S s_astSinkHpdFillter[] =
{
    /*  {Name[4],ProdCode,SerialNO,Week, Year},fillter_en, H_reshold-1 ,L_reshold-1 */
    {   {"TCL" ,        0,       0,  19, 2013}, { HI_TRUE,        100,           49} },        //TCL L40F3301B
    ...
    else
    {
            ...
            pstSinkStopDelay++, i++)
        {
            ...
            if( CompatEdidMatchChk(pstTmpManuInfo,pstSinkManuInfo) )
            {
                ...
                msleep(pstSinkStopDelay->u32StopDelay);
                break;
            }
        }
    }
...
HI_S32  DRV_HDMI_CompatHpdFillterGet(HDMI_DEVICE_ID_E enHdmiId,HDMI_HPD_FILLTER_S *pstHpd)
{
    HI_U32                          i = 0;
    HDMI_SINK_CAPABILITY_S          *pstSinkCap = HI_NULL;
    COMPAT_HPD_FILLTER_S            *pstTmpHpdCFillter = HI_NULL;
    HDMI_EDID_MANUFACTURE_INFO_S    *pstTmpManuInfo = HI_NULL ;
    HDMI_EDID_MANUFACTURE_INFO_S    *pstSinkManuInfo= HI_NULL ;
    HDMI_DEVICE_S                   *pstHdmiDev = HI_NULL;
    HI_S32                          s32Ret = HI_FAILURE;
    ...
    pstHdmiDev = GetHdmiDevice(enHdmiId);
    COMPAT_NULL_CHK(pstHdmiDev);
    COMPAT_NULL_CHK(pstHpd);

    /* default,from init or debug-cfg */
    pstHpd->bFillterEnable = pstHdmiDev->stHpdFillter.bFillterEnable;
    pstHpd->u32FillTerHigh = pstHdmiDev->stHpdFillter.u32FillTerHigh;
    pstHpd->u32FillTerLow  = pstHdmiDev->stHpdFillter.u32FillTerLow;
    ...
    else
    {
        ...
        for( i = 0, pstTmpHpdCFillter = &s_astSinkHpdFillter[0];
            pstTmpHpdCFillter && (i < HDMI_ARRAY_SIZE(s_astSinkHpdFillter));
            pstTmpHpdCFillter++, i++)
        {
            pstTmpManuInfo = &pstTmpHpdCFillter->stEdid;
            if (CompatEdidMatchChk(pstTmpManuInfo, pstSinkManuInfo))
            {
                HDMI_INFO("set s_astSinkHpdFillter[%d].\n", i);
                HDMI_MEMCPY(pstHpd,&pstTmpHpdCFillter->stHpdFillter,sizeof(HDMI_HPD_FILLTER_S));
            }
        }
    }
