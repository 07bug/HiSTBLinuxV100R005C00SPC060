                            ...
                            HDMI_EDID_STATUS_S *pstStatus)
{
    ...
    if((u32CheckSum & 0xff) != 0x00)
    {
        if(pau8Data[32] != 0x00)
        {
            ...
            //changhong-LT22610 TV's EDID checksum fail,header fail.
            //return HI_FAILURE;
        }
        else
        {
                                    ...
                                    HI_U8 u8Len)
{
    ...
    for(i = 0; i < (u8Len / 3); i++)
    {
        ...
        else
        {
            ...
            /* for some EDID,audio format may be set 0,but with a valid data block follow.We need to phase these valid data. */
            //break;
        }
...
*/
    ...
    u8Offset = 8;
    /* for progressive,may be interlaced */
    if (pstCap->bLatencyFieldsPresent && pau8Data[u8Offset] && pau8Data[u8Offset+1])
    {
        pstCap->u32VideoLatency              = (pau8Data[u8Offset] - 1) * 2;
        pstCap->u32InterlacedVideoLatency = (pau8Data[u8Offset] - 1) * 2;
        u8Offset++;
        pstCap->u32AudioLatency              = (pau8Data[u8Offset] - 1) * 2;
        pstCap->u32InterlacedAudioLatency = (pau8Data[u8Offset] - 1) * 2;
        u8Offset++;
        EDID_INFO("    %-20s : 0x%02X\n", "Video Latency", pstCap->u32VideoLatency);
        EDID_INFO("    %-20s : 0x%02X\n", "Audio Latency", pstCap->u32AudioLatency );
    }

    /* interlaced */
    if (pstCap->bLatencyFieldsPresent && pstCap->bILatencyFieldsPresent
        && pau8Data[u8Offset] && pau8Data[u8Offset+1])
    {
        pstCap->u32InterlacedVideoLatency = (pau8Data[u8Offset] - 1) * 2;
        u8Offset++;
        pstCap->u32InterlacedAudioLatency = (pau8Data[u8Offset] - 1) * 2;
    ...
    EDID_INFO("    %-20s : 0x%02X\n", "Interlaced Video Latency", pstCap->u32InterlacedVideoLatency);
    EDID_INFO("    %-20s : 0x%02X\n", "Interlaced Audio Latency", pstCap->u32InterlacedAudioLatency);
                                               ...
                                               HI_U8 u8Len)
{
    ...
    if ( (u8Len != EDID_DOLBY_VSVDB_VERSION_0_LEN)
        && (u8Len != EDID_DOLBY_VSVDB_VERSION_1_LEN)
        && (u8Len != EDID_DOLBY_VSVDB_VERSION_2_LEN) )
    {
        pstStatus->u32ParseWarn |= 1<<EDID_PARSE_WARN_BLOCKLEN_INVALID;
    ...
    if ( (u32DolbyIEEEOUI != DOLBY_IEEE_OUI)
        || (pstDolbyCap->u8VSVDBVersion > EDID_DOLBY_VSVDB_VERSION_2) )
    {
        EDID_WARN("Dolby VSVDB IEEEOUI:0x%x, Ver:%d\n", u32DolbyIEEEOUI, pstDolbyCap->u8VSVDBVersion);
        return HI_SUCCESS;
    }
    ...
    switch(pstDolbyCap->u8VSVDBVersion)
    {
        case EDID_DOLBY_VSVDB_VERSION_0:
            pstDolbyCap->unVersionCaps.stVer0.bYUV422_12Bit    = (pau8Data[3] & BIT0_MASK) ? HI_TRUE : HI_FALSE;
            pstDolbyCap->unVersionCaps.stVer0.b2160P60         = (pau8Data[3] & BIT1_MASK) ? HI_TRUE : HI_FALSE;
            pstDolbyCap->unVersionCaps.stVer0.bGlobalDimming   = (pau8Data[3] & BIT2_MASK) ? HI_TRUE : HI_FALSE;
            pstDolbyCap->unVersionCaps.stVer0.u16White_X       = (pau8Data[14] << 4) | ((pau8Data[13] >> 4) & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16White_Y       = (pau8Data[15] << 4) | (pau8Data[13] & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u8DMminorVersion = (pau8Data[19] & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u8DMmajorVersion = ((pau8Data[19] >> 4) & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16TargetMinPQ   = (pau8Data[17] << 4) | ((pau8Data[16] >> 4) & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16TargetMaxPQ   = (pau8Data[18] << 4) | (pau8Data[16] & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16Red_X         = (pau8Data[5] << 4) | ((pau8Data[4] >> 4) & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16Red_Y         = (pau8Data[6] << 4) | ((pau8Data[4]) & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16Green_X       = (pau8Data[8] << 4) | ((pau8Data[7] >> 4) & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16Green_Y       = (pau8Data[9] << 4) | (pau8Data[7] & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16Blue_X        = (pau8Data[11] << 4) | ((pau8Data[10] >> 4) & EDID_DOLBY_LOWER_NIBBLE_MASK);
            pstDolbyCap->unVersionCaps.stVer0.u16Blue_Y        = (pau8Data[12] << 4) | (pau8Data[10] & EDID_DOLBY_LOWER_NIBBLE_MASK);
            break;
        case EDID_DOLBY_VSVDB_VERSION_1:
            if (EDID_DOLBY_VSVDB_VERSION_1_LEN == u8Len) // V1 15Bytes (standard)
            {
                pstDolbyCap->unVersionCaps.stVer1.bYUV422_12Bit   = (pau8Data[3] & BIT0_MASK) ? HI_TRUE : HI_FALSE;
                pstDolbyCap->unVersionCaps.stVer1.b2160P60        = (pau8Data[3] & BIT1_MASK) ? HI_TRUE : HI_FALSE;
                pstDolbyCap->unVersionCaps.stVer1.bGlobalDimming  = (pau8Data[4] & BIT0_MASK);
                pstDolbyCap->unVersionCaps.stVer1.bColorimetry    = (pau8Data[5] & BIT0_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u8DMVersion     = ((pau8Data[3] >> 2) & EDID_DOLBY_LOWER_3BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u8LowLatency    = 0;
                pstDolbyCap->unVersionCaps.stVer1.u16TargetMinLuminance = ((pau8Data[4] >> 1) & EDID_DOLBY_LOWER_7BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u16TargetMaxLuminance = ((pau8Data[5] >> 1) & EDID_DOLBY_LOWER_7BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u16Red_X        = pau8Data[7];
                pstDolbyCap->unVersionCaps.stVer1.u16Red_Y        = pau8Data[8];
                pstDolbyCap->unVersionCaps.stVer1.u16Green_X      = pau8Data[9];
                pstDolbyCap->unVersionCaps.stVer1.u16Green_Y      = pau8Data[10];
                pstDolbyCap->unVersionCaps.stVer1.u16Blue_X       = pau8Data[11];
                pstDolbyCap->unVersionCaps.stVer1.u16Blue_Y       = pau8Data[12];
            }
            else // V1 12Bytes (low-latency)
            {
                ...
                pstDolbyCap->unVersionCaps.stVer1.u8DMVersion     = ((pau8Data[3] >> 2) & EDID_DOLBY_LOWER_3BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.bColorimetry    = (pau8Data[5] & BIT0_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u8LowLatency    = pau8Data[6] & EDID_DOLBY_LOWER_2BIT_MASK;
                pstDolbyCap->unVersionCaps.stVer1.u16TargetMinLuminance = ((pau8Data[4] >> 1) & EDID_DOLBY_LOWER_7BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u16TargetMaxLuminance = ((pau8Data[5] >> 1) & EDID_DOLBY_LOWER_7BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u16Red_X        = 0xa0 | ((pau8Data[9] >> 3) & EDID_DOLBY_LOWER_5BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u16Red_Y        = 0x40 | ((pau8Data[7] & BIT0_MASK) | ((pau8Data[8] & BIT0_MASK) << 1) | ((pau8Data[9] & EDID_DOLBY_LOWER_3BIT_MASK) << 2));
                pstDolbyCap->unVersionCaps.stVer1.u16Green_X      = 0x00 | ((pau8Data[7] >> 1) & EDID_DOLBY_LOWER_7BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u16Green_Y      = 0x80 | ((pau8Data[8] >> 1) & EDID_DOLBY_LOWER_7BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u16Blue_X       = 0x20 | ((pau8Data[6] >> 5) & EDID_DOLBY_LOWER_3BIT_MASK);
                pstDolbyCap->unVersionCaps.stVer1.u16Blue_Y       = 0x08 | ((pau8Data[6] >> 2) & EDID_DOLBY_LOWER_3BIT_MASK);
            }
            break;
        case EDID_DOLBY_VSVDB_VERSION_2:
            pstDolbyCap->unVersionCaps.stVer2.bYUV422_12Bit       = (pau8Data[3] & BIT0_MASK) ? HI_TRUE : HI_FALSE;
            pstDolbyCap->unVersionCaps.stVer2.bBackLightContol    = (pau8Data[3] & BIT1_MASK) ? HI_TRUE : HI_FALSE;
            pstDolbyCap->unVersionCaps.stVer2.bGlobalDimming      = (pau8Data[4] & BIT2_MASK) ? HI_TRUE : HI_FALSE;
            pstDolbyCap->unVersionCaps.stVer2.u8DMVersion         = ((pau8Data[3] >> 2) & EDID_DOLBY_LOWER_3BIT_MASK);
            pstDolbyCap->unVersionCaps.stVer2.u8BackLtMinLuma     = pau8Data[4] & EDID_DOLBY_LOWER_2BIT_MASK;
            pstDolbyCap->unVersionCaps.stVer2.u8Interface         = pau8Data[5] & EDID_DOLBY_LOWER_2BIT_MASK;
            pstDolbyCap->unVersionCaps.stVer2.u8YUV444RGB444_10b12b = ((pau8Data[6] & BIT0_MASK) << 1) | (pau8Data[7] & BIT0_MASK);
            pstDolbyCap->unVersionCaps.stVer2.u16TargetMinPQv2    = (pau8Data[4] >> 3)  & EDID_DOLBY_LOWER_5BIT_MASK;
            pstDolbyCap->unVersionCaps.stVer2.u16TargetMaxPQv2    = (pau8Data[5] >> 3)  & EDID_DOLBY_LOWER_5BIT_MASK;
            pstDolbyCap->unVersionCaps.stVer2.u16Red_X            = 0xa0 | ((pau8Data[8] >> 3)  & EDID_DOLBY_LOWER_5BIT_MASK);
            pstDolbyCap->unVersionCaps.stVer2.u16Red_Y            = 0x40 | ((pau8Data[9] >> 3)  & EDID_DOLBY_LOWER_5BIT_MASK);
            pstDolbyCap->unVersionCaps.stVer2.u16Green_X          = 0x00 | ((pau8Data[6] >> 1)  & EDID_DOLBY_LOWER_7BIT_MASK);
            pstDolbyCap->unVersionCaps.stVer2.u16Green_Y          = 0x80 | ((pau8Data[7] >> 1)  & EDID_DOLBY_LOWER_7BIT_MASK);
            pstDolbyCap->unVersionCaps.stVer2.u16Blue_X           = 0x20 | (pau8Data[8] & EDID_DOLBY_LOWER_3BIT_MASK);
            pstDolbyCap->unVersionCaps.stVer2.u16Blue_Y           = 0x08 | (pau8Data[9] & EDID_DOLBY_LOWER_3BIT_MASK);
            break;
        default:
            break;
    }
                                    ...
                                    HDMI_EDID_UPDATE_MODE_E enMode)
{
    ...
    if ( HDMI_EDID_UPDATE_SINK == enMode )
    {
        ...
        if (bHotplug)
        {
            s32Ret = HDMI_HAL_CALL_RET(HAL_HDMI_EdidRawDataRead, pstHdmiDev->pstHdmiHal, HDMI_EDID_TOTAL_SIZE, pau8Data);
                                                    ...
                                                    HDMI_SINK_CAPABILITY_S **pstCapDest)
{
    ...
    if ((HI_NULL == pstCapDest) || (HI_NULL == pstEdidInfo))
    {
        HDMI_ERR("\npointer is null!\n");
        return enEdidStatus;
    }
