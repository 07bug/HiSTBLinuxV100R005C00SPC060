...
HI_S32 hdmi_spd_infoframe_pack (HDMI_SPD_INFOFRAME_S *frame, HI_VOID *buffer, HI_S32 size)
{
    ...
    ptr[2] = frame->length;
    ptr[3] = 0; /* checksum */
    HDMI_MEMCPY(ptr + 4,frame->u8VendorName,sizeof(frame->u8VendorName));
    HDMI_MEMCPY(ptr + 12,frame->u8ProductDesc,sizeof(frame->u8ProductDesc));
    ptr[28] = frame->u8SrcDevInfo;
...
HI_S32 DRV_HDMI_InfoFrameSend(HDMI_INFOFRAME_S* pstInfoFrame, HDMI_INFOFRAME_ID_E enInfoFrameId, HDMI_INFOFRAME_U* uInfoframe)
{
    ...
    switch (enInfoFrameId)
    {
        ...
        case HDMI_INFOFRAME_TYPE_AVI:
        {
            ...
            bSentEnable = pstHdmiDev->stAttr.stAppAttr.bEnableAviInfoFrame && pstHdmiDev->stAttr.stAppAttr.bEnableVideo;
            break;
        }
        case HDMI_INFOFRAME_TYPE_AUDIO:
        {
            ...
            bSentEnable = pstHdmiDev->stAttr.stAppAttr.bEnableAudInfoFrame && pstHdmiDev->stAttr.stAppAttr.bEnableAudio;
            break;
        }
        ...
        {
            uInfoframe->stSpdInfoFrame.type = HDMI_INFOFRAME_TYPE_SPD;
            uInfoframe->stSpdInfoFrame.version = 1;
            uInfoframe->stSpdInfoFrame.length = HDMI_SPD_INFOFRAME_SIZE;
            hdmi_spd_infoframe_pack(&uInfoframe->stSpdInfoFrame, buffer, 32);
            bSentEnable = pstHdmiDev->stAttr.stAppAttr.bEnableSpdInfoFrame;
            break;
        }
    ...
    HDMI_HAL_CALL(HAL_HDMI_InfoframeSet, pstHdmiDev->pstHdmiHal, enInfoFrameId, buffer);
    DRV_HDMI_InfoFrameEnableSet(pstInfoFrame, enInfoFrameId, bSentEnable);
...
HI_S32 DRV_HDMI_InfoFrameEnableSet(HDMI_INFOFRAME_S* pstInfoFrame, HDMI_INFOFRAME_ID_E enInfoFrameId, HI_BOOL bEnable)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_InfoframeEnableSet, pstHdmiDev->pstHdmiHal, enInfoFrameId, bEnable);
    return HI_SUCCESS;
}
...
HI_S32 DRV_HDMI_AudioInfoFrameSend(HDMI_INFOFRAME_S* pstInfoFrame, HI_BOOL bEnable)
{
    ...
    {
        HDMI_INFO("channel refer to stream \n");
    ...
    HDMI_INFO("Code : %d .force set 0 (stream)\n",pstAOAttr->enAudioCode);
