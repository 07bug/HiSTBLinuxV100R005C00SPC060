...
static HI_S32 HdmiQuantizationStrategy(HDMI_DEVICE_S *pstHdmiDev, HDMI_APP_ATTR_S *pstUserApp)
{
    HDMI_VO_ATTR_S          *pstVoAttr = HI_NULL;
    ...
    /*RGB default -- CA format:limit range; IT format: full range
     *YUV default -- CA format:limit range; IT format: full range
     *EDID's Q & QY : if true,means that you can use limit or full range for any format,but you should set in AVI.
     */
    ...
    pstVoAttr->enOutQuantization = (pstVoAttr->enVideoTiming > HDMI_VIDEO_TIMING_640X480P_60000 ) ? HDMI_QUANTIZATION_LIMITED : HDMI_QUANTIZATION_FULL;
...
static HI_S32 HdmiColorAndBitStrategy(HDMI_DEVICE_S* pstHdmiDev, HDMI_APP_ATTR_S *pstUserApp)
{
    ...
    HDMI_INFO("bClrSpaceAdapt=%d, bDeepClrAdapt=%d\n", pstUserApp->bEnableClrSpaceAdapt, pstUserApp->bEnableDeepClrAdapt);
    HDMI_INFO("adptClrspace=%d, adptDeepclr=%d, adptClk=%u, maxClk=%u\n",pstUserApp->enOutColorSpace,
                                                pstUserApp->enDeepColorMode,
                                                pstVoAttr->u32HdmiAdaptPixClk,
                                                u32MaxTmdsClk);

    HdmiQuantizationStrategy(pstHdmiDev, pstUserApp);
    return HI_SUCCESS;
}
...
static HI_S32 HdmiCecNetWorkSet(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    {
        pstHdmiDev->stCecInfo.au8PhysicalAddr[0] = 0xf;
        pstHdmiDev->stCecInfo.au8PhysicalAddr[1] = 0xf;
        pstHdmiDev->stCecInfo.au8PhysicalAddr[2] = 0xf;
        pstHdmiDev->stCecInfo.au8PhysicalAddr[3] = 0xf;
        HDMI_WARN("EDID's cec phy address fail,Use default physical address f.f.f.f \n");
    }
...
static HI_S32 HdmiModeStrategy(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    else if(pstVoAttr->u32HdmiAdaptPixClk >= HDMI_EDID_MAX_HDMI14_TMDS_RATE)
    {
        ...
        {
            HDMI_WARN("sink not support SCDC ! bAuthMode=%d\n",pstAppAttr->bAuthMode);
        }
    }
#if 0
    ...
    /*  Start */
    HDMI_HAL_CALL(HAL_HDMI_PhyOutputEnableGet,pstHdmiDev->pstHdmiHal,&bPhyOe);
    if( !bPhyOe )
    {
        HDMI_HAL_CALL(HAL_HDMI_DataReset, pstHdmiDev->pstHdmiHal->stHalCtx.u32HdmiID, HI_FALSE,0);
    }

#ifdef HDMI_SCDC_SUPPORT
    /* oscilloscope in authmode force output scramble 20170802 */
    if ( pstSinkCap->bSupportSCDC || pstAppAttr->bAuthMode )
    {
        pstHdmiDev->stHdcpInfo.bHdcpDdcBusy  = HI_TRUE;
        HDMI_HAL_CALL(HAL_HDMI_ScdcStatusGet, pstHdmiDev->pstHdmiHal, &stScdcGet);

        HDMI_INFO("u8TmdsBitClkRatio,bSourceScrambleOn,bSinkScrambleOn = %d %d %d\n",
            stScdcGet.u8TmdsBitClkRatio,stScdcGet.bSourceScrambleOn,stScdcGet.bSinkScrambleOn);
...
static HI_S32 HdcpStrategy(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    /* load code check */
    bTmp = bHdmiStart && pstAppAttr->bHDCPEnable && (enTmpHdcpMode == HDMI_HDCP_MODE_2_2);
    if (bTmp)
    {
    ...
    /* load 1.4 key check */
    bTmp = bHdmiStart && pstAppAttr->bHDCPEnable && (enTmpHdcpMode == HDMI_HDCP_MODE_1_4) && (!pstHdcpInfo->bHdcp14LoadKey);
    if (bTmp)
    {
        /* 20170906 If no loadkey downscal-monitor can't work .Tee downscal strategy must work in hdcp1.4 sel_mode.*/
#if defined(HDMI_TEE_SUPPORT) && defined(HDMI_SUPPORT_LOGIC_SILICON)
        {
            HDMI_TEE_PARAM_S        stParam = {0};
            HDMI_MEMSET(&stParam, 0, sizeof(HDMI_TEE_PARAM_S));
        ...
        {
            HDMI_INFO("load record hdcp1.4 key!");
    ...
    if( (pstHdcpInfo->bHdcpEnable != bTmpHdcpEnable)
       || (pstHdcpInfo->enHdcpMode != enTmpHdcpMode) )
    {
        pstHdcpInfo->bHdcpAuthSuc = HI_FALSE;
    ...
    if(!pstHdcpInfo->bHdcpEnable)
    {
        pstHdcpInfo->enHdcpErrCode = HDMI_HDCP_ERR_UNDO;
    }
...
static HI_S32 HdmiAttrConstruct(HDMI_DEVICE_S * pstHdmiDev, HDMI_ATTR_S *pstAttr)
{
    ...
    pstVideoAttr->enInColorSpace = stHwStatus.stVideoStatus.enInColorSpace;
    ...
    if (stHwStatus.stInfoFrameStatus.bAVIEnable)
    {
        ...
        pstHdmiDev->stAttr.stHdrAttr.enColorimetry = (pstVideoAttr->enColorimetry < HDMI_COLORIMETRY__EXTENDED) ?
            (pstVideoAttr->enColorimetry) : (pstVideoAttr->enExtendedColorimetry + HDMI_HDR_EXTENDED_COLORIMETRY_XV_YCC_601);
        ...
        HDMI_INFO("hdr_enColorimetry: %d, enColorimetry: %d, enExtendedColorimetry: %d\n", \
            pstHdmiDev->stAttr.stHdrAttr.enColorimetry, pstVideoAttr->enColorimetry, pstVideoAttr->enExtendedColorimetry);
...
HI_S32 HpdEvent(HDMI_DEVICE_S* pstHdmiDev)
{
...
#ifdef HDMI_HDCP_SUPPORT
    /* HDCP cap */
    pstHdmiDev->stHdcpInfo.stHdcpCap.enHdcp14Cap = HDMI_HDCP_TRUE;
    pstHdmiDev->stHdcpInfo.stHdcpCap.enHdcp22Cap = HDMI_HDCP_FALSE;

    if ( (pstSinkCap->stMfrsInfo.u32Year >= 2013)
        || (HI_TRUE == pstHdmiDev->stAttr.stAppAttr.bAuthMode) )
    {
        for(u32TimeOutCnt = 0;u32TimeOutCnt < 5;u32TimeOutCnt++ )
        {
            ...
            if( HDMI_HDCP_TRUE == pstHdmiDev->stHdcpInfo.stHdcpCap.enHdcp22Cap )
            {
                break;
            }
            msleep(50);
...
static HI_S32 HdmiEventCallback(HI_VOID* pData , HDMI_EVENT_E enEvent)
{
    ...
    HDMI_HPD_FILLTER_S  stHpdFillter = {0};
#ifdef HDMI_TEE_SUPPORT
    HDMI_TEE_PARAM_S    stParam;
    HI_BOOL             bTeeEventStatus = HI_FALSE;

    HDMI_MEMSET(&stParam, 0, sizeof(stParam));
    stParam.enTeeCmd    = HI_HDMI_HDCP14_EVENT;
    stParam.pvData      = &bTeeEventStatus;
    stParam.u32Size     = sizeof(bTeeEventStatus);
#endif
    ...
    switch (enEvent)
    {
        case HDMI_EVENT_HOTPLUG:
            {
                HpdEvent(pstHdmiDev);
                DRV_HDMI_CompatHpdFillterGet(pstHdmiDev->u32HdmiDevId, &stHpdFillter);
                HDMI_HAL_CALL(HAL_HDMI_Debug, pstHdmiDev->pstHdmiHal, HDMI_DEBUG_CMD_HPD_FILLTER, &stHpdFillter);
#ifdef HDMI_TEE_SUPPORT
                stParam.enTeeCmd    = HI_HDMI_STATUS;
                bTeeEventStatus     = HI_TRUE;
        ...
        case HDMI_EVENT_HOTUNPLUG:
            {
                HDMI_HAL_CALL(HAL_HDMI_Debug, pstHdmiDev->pstHdmiHal, HDMI_DEBUG_CMD_HPD_FILLTER, &pstHdmiDev->stHpdFillter);
#ifdef HDMI_HDCP_SUPPORT
    ...
    /* not report event to user when suspend to MCU(real suspend mode); fake suspend mode need report event */
    /* when system suspend excute func switch_set_state, Will be triger wakeup source: NETLINK*/
    if ( HDMI_SUSPEND_REAL != pstHdmiDev->enSuspendState )
    {
...
HDMI_VIDEO_TIMING_E DispFmt2HdmiTiming(HDMI_DEVICE_S* pstHdmiDev, HI_U32 u32DispFmt)
{
    ...
    {
        return (HDMI_VIDEO_TIMING_E)HDMI_HAL_CALL_RET(HAL_HDMI_DispFmt2HdmiTiming, pstHdmiDev->pstHdmiHal, u32DispFmt);
    }
...
static HI_S32 HdmiDeviceInit(HDMI_DEVICE_S * pstHdmiDev)
{
    ...
    pstHpd->bFillterEnable = HI_TRUE;
    pstHpd->u32FillTerHigh = 0x63;
    pstHpd->u32FillTerLow  = 0x9;
    ...
    pstHdrAttr->enColorimetry = (pstVideoAttr->enColorimetry < HDMI_COLORIMETRY__EXTENDED) ?
            (pstVideoAttr->enColorimetry) : (pstVideoAttr->enExtendedColorimetry + HDMI_HDR_EXTENDED_COLORIMETRY_XV_YCC_601);
...
HI_S32 DRV_HDMI_Open(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bUser)
{
    ...
    if (bUser)
    {
        ...
        HDMI_INFO("Create Event Queue for process:%d %s\n", get_current()->tgid, get_current()->comm);
    }
    else
    {
    ...
    if (bHotPLug)
    {
        ...
        {
            s32Ret = HpdEvent(pstHdmiDev);
            pstHdmiDev->bOpenCapGet = (HI_SUCCESS==s32Ret)? HI_TRUE : HI_FALSE;
        }
    }
    else
    {
    ...
    else if (0 == pstHdmiDev->u32UserCnt)
    {
        HDMI_MUTEX_LOCK(pstHdmiDev->stMutexThread);
...
HI_S32 DRV_HDMI_Close(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bUser)
{
    ...
    {
        HDMI_INFO("Delete Event Queue for process:%d %s\n",get_current()->tgid, get_current()->comm);
...
#ifdef HDMI_SUPPORT_LOGIC_SILICON
    {
        HI_BOOL bEncryption = HI_FALSE;
        /* just for siliconImage Avmute state */
        HDMI_HAL_CALL(HAL_HDMI_Debug, pstHdmiDev->pstHdmiHal, HDMI_DEBUG_CMD_ENCRYPTION, &bEncryption);
    }
#endif
...
HI_BOOL IsEmiParamChange(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S* pstHwVOAttr, HDMI_APP_ATTR_S* pstHwAPPAttr)
{
    ...
    if ( (pstHwAPPAttr->enDeepColorMode == HDMI_DEEP_COLOR_OFF) || (pstHwAPPAttr->enDeepColorMode == HDMI_DEEP_COLOR_24BIT) )
    {
    ...
    {
        if (pstSwAPPAttr->enOutColorSpace != HDMI_COLORSPACE_YCbCr420)
        {
            bRet |= HI_TRUE;
        }
    }
    else if (pstSwAPPAttr->enOutColorSpace == HDMI_COLORSPACE_YCbCr420)
    {
        bRet |= HI_TRUE;
    }
    else
...
static HI_S32 CheckAudioAttr(HDMI_AO_ATTR_S *pstAOAttr)
{
    ...
    {
        HDMI_ERR("interface(%d) is invalid\n",pstAOAttr->enSoundIntf);
        return HI_FAILURE;
    }
    ...
    {
        HDMI_ERR("chanel number(%d) is invalid\n",pstAOAttr->enChanels);
        return HI_FAILURE;
    }
    ...
    switch (pstAOAttr->enSampleFs)
    {
        ...
        case HDMI_SAMPLE_RATE_768K:
            break;
        default:
            HDMI_ERR("Audio Frequency(%d) is invalid\n",pstAOAttr->enSampleFs);
            return HI_FAILURE;
    }
...
HI_S32 DRV_HDMI_AudioEnable(HDMI_DEVICE_S* pstHdmiDev,HDMI_AO_MODE_E enMode,HI_BOOL bEnable)
{
    ...
    s32Ret |= HDMI_HAL_CALL_RET(HAL_HDMI_AudioMute, pstHdmiDev->pstHdmiHal,!bTmpEnable);
...
static HI_S32 CheckVideoAttr(HDMI_VO_ATTR_S* pstVOAttr)
{
    ...
    {
        HDMI_ERR("PictureAspect(%d) is wrong\n", pstVOAttr->enPictureAspect);
        return HI_FAILURE;
    }
    ...
    {
        HDMI_ERR("InBitDepth(%d) is wrong\n", pstVOAttr->enInBitDepth);
        return HI_FAILURE;
    }
...
static HI_VOID HdmiAttrToVidcfg(HDMI_VO_ATTR_S* pstVOAttr, HDMI_APP_ATTR_S* pstAppAttr, HDMI_VIDEO_CONFIG_S* pstVideoCfg)
{
    ...
    pstVideoCfg->enInQuantization= pstVOAttr->enInQuantization;
    pstVideoCfg->enOutQuantization= pstVOAttr->enOutQuantization;
...
static HI_BOOL VOPathIsChanged(HDMI_DEVICE_S* pstHdmiDev)
{
    HDMI_HARDWARE_STATUS_S stHwStatus;
    HDMI_VO_ATTR_S *pstVOAttr = HI_NULL;
    HI_U8 u8ColorSpace = 0;
    HI_BOOL bRet = HI_FALSE;

    if (pstHdmiDev->stAttr.stHdrAttr.enHdrMode == HDMI_HDR_MODE_DOLBY_NORMAL
        || pstHdmiDev->stAttr.stHdrAttr.enHdrMode == HDMI_HDR_MODE_DOLBY_TUNNELING
        || pstHdmiDev->stAttr.stHdrAttr.enHdrMode == HDMI_HDR_MODE_CEA_861_3_AUTHEN)
    {
        return HI_FALSE;
    }
    ...
    u8ColorSpace = (stHwStatus.stInfoFrameStatus.u8AVI[4] & HDMI_AVI_COLORSPACE_MASK) >> 0x5;
    if (pstVOAttr->enInColorSpace == HDMI_COLORSPACE_YCbCr444)
    {
        if (  (u8ColorSpace == HDMI_COLORSPACE_YCbCr422)
           && ((stHwStatus.stVideoStatus.bYCbCr444_422 != HI_TRUE) || (stHwStatus.stVideoStatus.bYCbCr422_420 == HI_TRUE)))
        {
            bRet = HI_TRUE;
        }
        else if (  (u8ColorSpace == HDMI_COLORSPACE_RGB)
                && (stHwStatus.stVideoStatus.bYCbCr2RGB != HI_TRUE))
        {
            bRet = HI_TRUE;
        }
        else if (  (u8ColorSpace == HDMI_COLORSPACE_YCbCr444)
                && ((stHwStatus.stVideoStatus.bYCbCr2RGB == HI_TRUE) || (stHwStatus.stVideoStatus.bYCbCr444_422 == HI_TRUE)))
        {
            bRet = HI_TRUE;
        }
        else if (  (u8ColorSpace == HDMI_COLORSPACE_YCbCr420)
                && (stHwStatus.stVideoStatus.bYCbCr444_422 != HI_TRUE)
                && (stHwStatus.stVideoStatus.bYCbCr422_420 != HI_TRUE))
        {
            bRet = HI_TRUE;
        }
    }
    else if (pstVOAttr->enInColorSpace == HDMI_COLORSPACE_RGB)
    {
        if ((u8ColorSpace != HDMI_COLORSPACE_RGB) && (stHwStatus.stVideoStatus.bRGB2YCbCr != HI_TRUE))
        {
            bRet = HI_TRUE;
        }
        else if ((u8ColorSpace == HDMI_COLORSPACE_RGB) && (stHwStatus.stVideoStatus.bRGB2YCbCr == HI_TRUE))
        {
            bRet = HI_TRUE;
        }
    }
    ...
    if (bRet == HI_TRUE)
    {
        HDMI_INFO("VoPath colorspace change!\n");
        return HI_TRUE;
    }
...
static HI_VOID HdrModeCheck(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S *pstVOAttr, HDMI_APP_ATTR_S *pstAppAttr)
{
    ...
    HDMI_INFO("user hdr mode:%d, acture hdr mode:%d, enColorimetry:%d, enExtendedColorimetry:%d\n",
        pstHdrAttr->enUserHdrMode, pstHdrAttr->enHdrMode, pstVOAttr->enColorimetry, pstVOAttr->enExtendedColorimetry);
    return ;
}
#endif
...
HI_S32 DRV_HDMI_VOAttrSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S* pstVOAttr)
{
    ...
    HDMI_VIDEO_TIMING_E     enVideoTimingBak = HDMI_VIDEO_TIMING_BUTT;
    ...
    pstVideoAttr = &pstHdmiDev->stAttr.stVOAttr;
    /* interface HI_DRV_HDMI_SetHdrAttr may modify AVInfoFrame, will cause HW timing not changed,
       and phy not configure, so memcopy VideoAttr before backup timing */
    enVideoTimingBak = pstVideoAttr->enVideoTiming;
    memcpy(pstVideoAttr, pstVOAttr, sizeof(HDMI_VO_ATTR_S));

    HDMI_INFO("u32ClkFs: %d, u32PixelRepeat: %d\n", pstVideoAttr->u32ClkFs, pstVideoAttr->u32PixelRepeat);
    HDMI_INFO("enVideoTiming: %d, u32DispFmt: %d\n", pstVideoAttr->enVideoTiming, pstVideoAttr->u32DispFmt);
    HDMI_INFO("enStereoMode:  %d\n", pstVideoAttr->enStereoMode);
    HDMI_INFO("enInBitDepth:  %d, enInColorSpace: %d\n", pstVideoAttr->enInBitDepth, pstVideoAttr->enInColorSpace);
    HDMI_INFO("enColorimetry: %d, enExtendedColorimetry: %d\n", pstVideoAttr->enColorimetry, pstVideoAttr->enExtendedColorimetry);
    HDMI_INFO("enInQuantization: %d\n", pstVideoAttr->enInQuantization);
    HDMI_INFO("enPictureAspect:   %d, enActiveAspect: %d\n", pstVideoAttr->enPictureAspect, pstVideoAttr->enActiveAspect);
    HDMI_INFO("enPictureScaling:  %d\n", pstVideoAttr->enPictureScaling);
    HDMI_INFO("bVSyncPol: %d, bHSyncPol: %d, bDEPol: %d\n", pstVideoAttr->bVSyncPol, pstVideoAttr->bHSyncPol, pstVideoAttr->bDEPol);
    ...
    memset(&stHwAttr, 0, sizeof(HDMI_ATTR_S));
    HdmiAttrConstruct(pstHdmiDev, &stHwAttr);
    stHwAttr.stVOAttr.enVideoTiming = enVideoTimingBak;
...
static HI_BOOL HdrAttrIsChange(DRV_HDMI_HDR_S *pstAppHdrAttr, HDMI_HDR_ATTR_S *pstHdrAttr)
{
    ...
    if ( (HDMI_HDR_MODE_CEA_861_3 != pstAppHdrAttr->stHdrAttr.enHdrMode)
        && (HDMI_HDR_MODE_CEA_861_3_AUTHEN != pstAppHdrAttr->stHdrAttr.enHdrMode) )
    {
        /* below attribute must be ignore when not HDR10 mode */
        return HI_FALSE;
    }
                   ...
                   HDMI_EXTENDED_COLORIMETRY *penSdrExtColorimetryBack, HDMI_HDR_MODE_E enHdrModeBack)
{
    ...
    {
        HDMI_WARN("usermode is not HDR10/HLG.\n");
        return HI_FAILURE;
    }
...
HI_S32 DRV_HDMI_HdrAttrSet(HDMI_DEVICE_S* pstHdmiDev, DRV_HDMI_HDR_S *pstAppHdrAttr)
{
    ...
    switch(pstAppHdrAttr->stHdrAttr.enHdrMode)
    {
        ...
        case HDMI_HDR_MODE_CEA_861_3_AUTHEN:
            ...
            pstAppAttr->bEnableDRMInfoFrame = HI_TRUE;
            HDMI_INFO("Hdr auth mode is enable! \n");
            break;
        case HDMI_HDR_MODE_DOLBY_NORMAL:
        case HDMI_HDR_MODE_DOLBY_TUNNELING:
            if(pstHdrAttr->enUserHdrMode != HDMI_HDR_USERMODE_DOLBY)
...
HI_S32 DRV_HDMI_AttrSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_ATTR_S *pstAttr)
{
    ...
    HDMI_INFO("bEnableVideo: %d, bEnableAudio: %d\n", pstAttr->stAppAttr.bEnableVideo, pstAttr->stAppAttr.bEnableAudio);
    HDMI_INFO("enOutColorSpace: %d, enDeepColorMode: %d\n", pstAttr->stAppAttr.enOutColorSpace, pstAttr->stAppAttr.enDeepColorMode);
    HDMI_INFO("bxvYCCMode: %d\n", pstAttr->stAppAttr.bxvYCCMode);
    HDMI_INFO("bEnableAviInfoFrame: %d, bEnableSpdInfoFrame:  %d\n", pstAttr->stAppAttr.bEnableAviInfoFrame, pstAttr->stAppAttr.bEnableSpdInfoFrame);
    HDMI_INFO("bEnableAudInfoFrame: %d, bEnableMpegInfoFrame: %d\n", pstAttr->stAppAttr.bEnableAudInfoFrame, pstAttr->stAppAttr.bEnableMpegInfoFrame);
    HDMI_INFO("bDebugFlag:  %d\n", pstAttr->stAppAttr.bDebugFlag);
    HDMI_INFO("bHDCPEnable: %d, enHDCPMode: %d\n", pstAttr->stAppAttr.bHDCPEnable, pstAttr->stAppAttr.enHDCPMode);
    HDMI_INFO("bEnableClrSpaceAdapt: %d, bEnableDeepClrAdapt: %d\n", pstAttr->stAppAttr.bEnableClrSpaceAdapt, pstAttr->stAppAttr.bEnableDeepClrAdapt);
...
HI_S32 DRV_HDMI_StatusGet(HDMI_DEVICE_S* pstHdmiDev, HDMI_STATUS_S *pHdmiStatus)
{
    ...
    pHdmiStatus->bConnected     = stHwStatus.stCommonStatus.bHotPlug ;
    pHdmiStatus->bSinkPowerOn   = stHwStatus.stCommonStatus.bHotPlug;
...
HI_S32 DRV_HDMI_CecCmdReceive(HDMI_DEVICE_S* pstHdmiDev, HDMI_CEC_CMD_S  *pstCecCmd)
{
    ...
    HDMI_INFO("s32Ret= %d, u8Opcode=0x%x, src=%d, dst=%d\n",s32Ret, pstCecCmd->u8Opcode,pstCecCmd->enSrcAdd,pstCecCmd->enDstAdd);
...
#if defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3798mv200) || defined(CHIP_TYPE_hi3798mv300) || defined(CHIP_TYPE_hi3716mv450) || defined(CHIP_TYPE_hi3798mv310)
...
static HI_S32 HdmiCmdProcess0(HI_U32 cmd, HI_VOID *arg, HI_BOOL bUser)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_HDMI_OPEN:
        {
            ...
            HDMI_INFO("CMD_HDMI_OPEN bUser=%d %s\n",bUser,get_current()->comm);
        ...
        case CMD_HDMI_START:
        {
            ...
            HDMI_INFO("CMD_HDMI_START bUser=%d %s\n",bUser,get_current()->comm);
        ...
        case CMD_HDMI_STOP:
        {
            ...
            HDMI_INFO("CMD_HDMI_STOP bUser=%d %s\n",bUser,get_current()->comm);
        ...
        case CMD_HDMI_SET_ATTR:
        {
            ...
            HDMI_INFO("CMD_HDMI_SET_ATTR bUser=%d %s\n",bUser,get_current()->comm);
        ...
        case CMD_HDMI_SET_VO_ATTR:
        {
            ...
            HDMI_INFO("CMD_HDMI_SET_VO_ATTR bUser=%d %s\n",bUser,get_current()->comm);
        ...
        case CMD_HDMI_SET_AO_ATTR:
        {
            ...
            HDMI_INFO("CMD_HDMI_SET_AO_ATTR bUser=%d %s\n",bUser,get_current()->comm);
        ...
        case CMD_HDMI_SET_RESUME_STAT:
        {
            ...
            HDMI_MUTEX_LOCK(s_HdmiMutex);
            HDMI_INFO("CMD_HDMI_SET_RESUME_STAT bUser=%d %s,bResumeEanble=%d\n",bUser,get_current()->comm,pstRemseStat->bResumeEanble);
        ...
        case CMD_HDMI_SET_SUSPEND_STAT:
        {
            HDMI_DEVICE_S           *pstHdmiDev = HI_NULL;
            DRV_HDMI_SUSPEND_STAT_S *pstSuspendStat = HI_NULL;

            s32Ret = HI_FAILURE;
            CHECK_POINTER_BREAK(arg);
            pstSuspendStat = (DRV_HDMI_SUSPEND_STAT_S*)arg;
            pstHdmiDev = GetHdmiDevice(pstSuspendStat->enHdmiID);
            CHECK_POINTER_BREAK(pstHdmiDev);

            HDMI_MUTEX_LOCK(s_HdmiMutex);
            pstHdmiDev->enSuspendState = pstSuspendStat->enSuspend;
            HDMI_INFO("CMD_HDMI_SET_SUSPEND_STAT = %d, bUser=%d %s\n", pstHdmiDev->enSuspendState, bUser, get_current()->comm);
            s32Ret = HI_SUCCESS;
            HDMI_MUTEX_UNLOCK(s_HdmiMutex);
            break;
        }
...
static HI_S32 HdmiCmdProcess1(HI_U32 cmd, HI_VOID *arg, HI_BOOL bUser)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_HDMI_READ_EDID:
        {
            ...
            pstDrvEdidRaw->stEdidRaw.u32EdidLen = DRV_HDMI_EdidRawGet(&pstHdmiDev->stEdidInfo, pstDrvEdidRaw->stEdidRaw.u8Edid, HDMI_EDID_SIZE);
            pstDrvEdidRaw->stEdidRaw.u8EdidValid = pstDrvEdidRaw->stEdidRaw.u32EdidLen && (pstDrvEdidRaw->stEdidRaw.u32EdidLen % 128 == 0 ) ? 1 : 0;
            s32Ret = pstDrvEdidRaw->stEdidRaw.u8EdidValid ? HI_SUCCESS : HI_FAILURE;
...
HI_S32 HDMI_DRV_ModInit(HI_VOID)
{
    ...
    DRV_HDMI_SwitchDevRegister(HI_TRUE);
...
HI_VOID HDMI_DRV_ModExit(HI_VOID)
{
    ...
    DRV_HDMI_SwitchDevRegister(HI_FALSE);
