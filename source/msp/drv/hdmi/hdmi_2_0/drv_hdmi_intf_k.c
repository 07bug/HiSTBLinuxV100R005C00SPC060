...
do{\
    ...
    else if (HI_DRV_HDMI_HDR_USERMODE_HLG == uhdrmode)\
    {\
        khdrmode = HDMI_HDR_USERMODE_HLG;\
    }\
...
static HI_BOOL  bSetFormatStart = HI_FALSE;
HI_S32 HdmiExtIoctl(unsigned int cmd, void *argp);
...
HI_S32 HI_DRV_HDMI_Suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    ...
    CHECK_HDMI_PTR_NULL(pstHdmiDev->pstHdmiHal);
    ...
    while (enHdmi < HDMI_DEVICE_ID_BUTT)
    {
        ...
        ret = HdmiExtIoctl(CMD_HDMI_SET_RESUME_STAT, &stResume);
        if (ret != HI_SUCCESS)
        {
            HDMI_ERR("suspend stat set!:0x%x\n",ret);
            return ret;
        }
...
#endif
        HDMI_HAL_CALL(HAL_HDMI_HWInitSet, pstHdmiDev->pstHdmiHal, HI_FALSE);
        HDMI_INFO("HdmiExtIoctl CMD_HDMI_STOP ok! \n");

        enHdmi++;
    }

#if defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3798mv300) || defined(CHIP_TYPE_hi3798mv310) || defined(CHIP_TYPE_hi3716mv450)
    DRV_HDMI_ProdCrgDeInit();
#endif
...
HI_S32 HI_DRV_HDMI_Resume(PM_BASEDEV_S *pdev)
{
    ...
    while (enHdmi < HDMI_DEVICE_ID_BUTT)
    {
        ...
        DRV_HDMI_STATUS_S       stDrvStatus = {0};

        CHECK_HDMI_OPEN(enHdmi);

        pstHdmiDev->enSuspendState = HDMI_SUSPEND_NONE;
        HDMI_INFO("enSuspendState = %d\n", pstHdmiDev->enSuspendState);
        stDrvStatus.enHdmiID = enHdmi;
        (HI_VOID)HdmiExtIoctl(CMD_HDMI_GET_STATUS, &stDrvStatus);
        if (stDrvStatus.stHdmiStatus.bConnected)
        {
            stResume.enHdmiID = enHdmi;
            stResume.bResumeEanble = HI_TRUE;
            ret = HdmiExtIoctl(CMD_HDMI_SET_RESUME_STAT, &stResume);
            if (ret != HI_SUCCESS)
            {
                HDMI_ERR("resume stat set!:0x%x\n",ret);
                return ret;
            }
        }
...
#ifdef HDMI_ANDROID_SUPPORT
#ifdef HDMI_CEC_SUPPORT
        DRV_HDMI_EventPoolStatusGet(enHdmi, 1, &stCtrl, &stCnt);
        u32TmpHpdCnt = stCtrl.u32WritePtr;
#endif
#endif
        stOpen.enHdmiID      = enHdmi;
        stOpen.enDefaultMode = pstHdmiDev->stAttr.stAppAttr.enHdmiAction;
        ret = HdmiExtIoctl(CMD_HDMI_OPEN, &stOpen);
        if (ret != HI_SUCCESS)
        {
...
#ifdef HDMI_ANDROID_SUPPORT
#ifdef HDMI_CEC_SUPPORT
        /* in android , sent cec resume when HPD is not report. */
        DRV_HDMI_EventPoolStatusGet(enHdmi, 1, &stCtrl, &stCnt);

        if(pstHdmiDev->stCecInfo.bCecEnable && (u32TmpHpdCnt==stCtrl.u32WritePtr))
        {
            DRV_HDMI_CEC_CMD_S stDrvCecCmd = {0};
            DRV_HDMI_CEC_INFO_S stDrvCecInfo = {0};

            HdmiExtIoctl(CMD_HDMI_CEC_ENABLE, &enHdmi);

            stDrvCecCmd.enHdmiID        = enHdmi;
            stDrvCecCmd.CECCmd.enDstAdd = HDMI_CEC_LOGICALADD_TV;
            stDrvCecCmd.CECCmd.enSrcAdd = pstHdmiDev->stCecInfo.u8LogicalAddr;
            stDrvCecCmd.CECCmd.u8Opcode = 0x04;
            HdmiExtIoctl(CMD_HDMI_SET_CEC, &stDrvCecCmd);
            stDrvCecCmd.CECCmd.u8Opcode = 0x0d;
            HdmiExtIoctl(CMD_HDMI_SET_CEC, &stDrvCecCmd);
            HdmiExtIoctl(CMD_HDMI_GET_CECSTATUS, &stDrvCecInfo);
            if (stDrvCecInfo.stCecInfo.au8Network[HDMI_CEC_LOGICALADD_AUDIOSYSTEM])
            {
                stDrvCecCmd.CECCmd.enDstAdd = HDMI_CEC_LOGICALADD_AUDIOSYSTEM;
                stDrvCecCmd.CECCmd.enSrcAdd = stDrvCecInfo.stCecInfo.u8LogicalAddr;
                stDrvCecCmd.CECCmd.u8Opcode = 0x70; // system audio mode request
                stDrvCecCmd.CECCmd.stOperand.stRawData.u8Length  = 0x2;
                stDrvCecCmd.CECCmd.stOperand.stRawData.u8Data[0] = ((stDrvCecInfo.stCecInfo.au8PhysicalAddr[0] << 4) & 0xf0) | (stDrvCecInfo.stCecInfo.au8PhysicalAddr[1] & 0x0f);
                stDrvCecCmd.CECCmd.stOperand.stRawData.u8Data[1] = ((stDrvCecInfo.stCecInfo.au8PhysicalAddr[2] << 4) & 0xf0) | (stDrvCecInfo.stCecInfo.au8PhysicalAddr[3] & 0x0f);
                HdmiExtIoctl(CMD_HDMI_SET_CEC, &stDrvCecCmd);
            }
            HDMI_INFO(" drv cec resume ! u32TmpHpdCnt,stCtrl.u32WritePtr=%d,%d\n", u32TmpHpdCnt,stCtrl.u32WritePtr);
        }
#endif
#endif
        ...
        {
            HDMI_INFO("HdmiExtIoctl CMD_HDMI_START fail!Hpd=0 \n");
        }
#endif
        // report HOTUNPLUG event when wakeup no hotplug
        if (!stDrvStatus.stHdmiStatus.bConnected)
        {
            DRV_HDMI_EventPoolWrite(pstHdmiDev->u32HdmiDevId, HDMI_EVENT_HOTUNPLUG, HI_FALSE);
        }
        enHdmi++;
    }
...
HI_S32 HI_DRV_HDMI_SoftResume(HDMI_VIDEO_ATTR_S *pstVideoAttr)
{
    ...
    while (enHdmi < HDMI_DEVICE_ID_BUTT)
    {
        ...
        stOpen.enHdmiID      = enHdmi;
        stOpen.enDefaultMode = pstHdmiDev->stAttr.stAppAttr.enHdmiAction;
        s32Ret = HdmiExtIoctl(CMD_HDMI_OPEN, &stOpen);
        ...
        IntfkDisp2HdmiVoAttr(pstHdmiDev, &stDrvVoAttr.stVOAttr, pstVideoAttr);
...
#if defined(CONFIG_HDMI_STB_SDK)
    HI_S32 s32Ret = HI_FAILURE;
    s32Ret = HI_HDMI_MODULE_Register((HI_U32)HI_ID_HDMI, "HI_HDMI", (HI_VOID *)&s_stHdmiExportFuncs);
...
{
#ifndef HDMI_MASK_START
    HDMI_DEVICE_ID_E enHdmiID = HDMI_DEVICE_ID_BUTT;
    HI_U32  u32NeedHotPlugProcess = HI_TRUE;

    CHECK_HDMI_PTR_NULL(data);
    ...
    {
        HDMI_INFO("\n---HDMI kernel event(no UserCallBack): 0x%x.---\n",enEvent);
...
{
    HI_S32   s32Ret = HI_FAILURE;
    DRV_HDMI_PLAYSTATUS_S stPlayStatus = {0};
...
static HI_S32 IntfkColorSpaceConv(HDMI_VIDEO_ATTR_S *pstSrcAttr, HDMI_VO_ATTR_S *pstDestAttr)
{
    ...
    switch (pstSrcAttr->u32ColorSpace)
    {
        case HI_DRV_CS_BT601_YUV_LIMITED:
            ...
            pstDestAttr->enInQuantization  = HDMI_QUANTIZATION_LIMITED;
            pstDestAttr->enColorimetry     = HDMI_COLORIMETRY__ITU_601;
            break;
        case HI_DRV_CS_BT601_YUV_FULL:
            pstDestAttr->enInColorSpace    = HDMI_COLORSPACE_YCbCr444;
            pstDestAttr->enInQuantization  = HDMI_QUANTIZATION_FULL;
            pstDestAttr->enColorimetry     = HDMI_COLORIMETRY__ITU_601;
            break;
...
HI_S32 HI_DRV_HDMI_PreFormat(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    bSetFormatStart = ( pstHdmiDev->u32UserCnt && (pstHdmiDev->enRunState & HDMI_RUN_STATE_START) ) ? HI_TRUE : HI_FALSE ;
    HDMI_INFO(">>> in..., bUserStart=%d\n",bSetFormatStart);
#ifndef HDMI_MASK_START
    /* compatibility with UNF & kernel call HDMI_STOP & HDMI_START */
    if (bSetFormatStart)
    {
        s32Ret = HdmiExtIoctl(CMD_HDMI_STOP, &enHdmi);
        if (s32Ret != HI_SUCCESS)
...
HI_S32 HI_DRV_HDMI_SetFormat(HI_UNF_HDMI_ID_E enHdmi, HDMI_VIDEO_ATTR_S *pstVideoAttr)
{
    ...
    HDMI_INFO("enVideoFmt=%d,u32ClkFs=%d,u32PixelRepeat=%d,\n\tbVSyncPol=%d,bHSyncPol=%d,bDEPol=%d,enStereoMode=%d,\n"
                "\tenInBitDepth=%d,u32ColorSpace=%d,\n\tenBt2020Lum=%d,u32DispRatioWidth=%d,u32DispRatioHeight=%d\n",
                ...
                pstVideoAttr->u32ColorSpace,
                pstVideoAttr->enBt2020Lum,
                pstVideoAttr->u32DispRatioWidth,
                pstVideoAttr->u32DispRatioHeight);
...
#ifndef HDMI_MASK_START
    HDMI_INFO("HI_DRV_HDMI_SetFormat bSetFormatStart=%d\n",bSetFormatStart);
    if (bSetFormatStart)
    {
        s32Ret = HdmiExtIoctl(CMD_HDMI_START, &enHdmi);
        if (s32Ret != HI_SUCCESS)
        {
            HDMI_ERR("start hdmi err!:0x%x\n",s32Ret);
        }
    }
#endif
...
HI_S32 HI_DRV_HDMI_GetVideoCapability(HI_UNF_HDMI_ID_E enHdmi, HI_DRV_HDMI_VIDEO_CAPABILITY_S *pstVideoCap)
{
    ...
    if (s32Ret == HI_SUCCESS)
    {
        pstVideoCap->bYccQrangeSelectable       = stTmpVideoCap.stVideoCap.bYccQrangeSelectable;
        pstVideoCap->bRgbQrangeSelectable       = stTmpVideoCap.stVideoCap.bRgbQrangeSelectable;
        pstVideoCap->bDolbySupport              = stTmpVideoCap.stVideoCap.bDolbySupport;
        pstVideoCap->bHdrSupport                = stTmpVideoCap.stVideoCap.bHdrSupport;
        pstVideoCap->stDolbyCaps.u8VSVDBVersion = stTmpVideoCap.stVideoCap.stDolbyCaps.u8VSVDBVersion;
        HDMI_MEMCPY(&pstVideoCap->stDolbyCaps.unVersionCaps, &stTmpVideoCap.stVideoCap.stDolbyCaps.unVersionCaps, sizeof(HI_HDMI_DOLBY_CAPS_U));
        HDMI_MEMCPY(&pstVideoCap->stHdrCaps, &stTmpVideoCap.stVideoCap.stHdrCaps, sizeof(HI_HDMI_HDR_SINK_CAPS_S));
        HDMI_MEMCPY(&pstVideoCap->stColorimetry, &stTmpVideoCap.stVideoCap.stColorMetry, sizeof(HI_HDMI_COLORIMETRY_CAPS_S));
    }
...
HI_S32 HI_DRV_HDMI_Detach(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    HDMI_INFO("HI_DRV_HDMI_Detach by %s\n", get_current()->comm);
...
HI_S32 HI_DRV_HDMI_Attach(HI_UNF_HDMI_ID_E enHdmi, HDMI_VIDEO_ATTR_S *pstVideoAttr)
{
    ...
    HDMI_INFO("HI_DRV_HDMI_Attach by %s\n", get_current()->comm);
