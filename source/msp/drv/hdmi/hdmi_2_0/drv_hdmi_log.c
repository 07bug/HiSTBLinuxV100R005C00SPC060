...
HI_S32 LogCreate(LOG_CTX_S *pLogCtx)
{
    ...
    pstMem->pVirAddr = HDMI_VMALLOC(pstMem->u32MemSize + 1);
    if (HI_NULL == pstMem->pVirAddr)
    {
        OSAL_PRINTK("alloc memory for log failed!!\n");
        return HI_FAILURE;
    }
    OSAL_MEMSET(pstMem->pVirAddr, 0x0, pstMem->u32MemSize + 1);
...
static HI_VOID LogJumpOver(HI_U8 **pmem, LOG_MEM_S *pstMem, HI_S32 *pRight, HI_S32 *pLeft,HI_U32 offset)
{
...
HI_S32 LogPrintOutOfMem(HI_VOID)
{
    ...
    HI_U8      u8FuncNumBuf[LOG_HEAD_INFO_DATA_WIDTH + 2] = {0};
    HI_U8      u8StrLenBuf[LOG_HEAD_INFO_DATA_WIDTH + 1]  = {0};
    ...
    while(left >= 0)
    {
        ...
        if(right > 0)
        {
            ...
            if(HI_SUCCESS != ret)
            {
                OSAL_PRINTK("LogMemIsFull return failure!!!\n");
                continue;
            }
        ...
        if (HI_TRUE == bConditionIsValid)
        {
            ...
            LogJumpOver(&pmem, pstMem, &right, &left,LOG_FUN_NO + u32StrLen);
...
/* set log level: info?¡éwarning?¡éerror */
...
/* record into memory */
HI_VOID DRV_HDMI_LogRecord(LOG_LEVEL_E eLevel, LOG_TYPE_FUNC_E eFuncType, HI_U8 *pFuncName, const HI_CHAR *fmt, ...)
{
    HI_U8  u8Buf[LOG_MEM_BUF_SIZE] = {0};
    va_list argp = {0};
    LOG_MEM_S *pstMem = HI_NULL;
    ...
    u32Pos = 0;
    HDMI_OSAL_Snprintf(u8Buf + u32Pos, 14, "[%010lu] ", HDMI_OSAL_GetTimeInMs());
    u32Pos += 13;
    HDMI_OSAL_Snprintf(u8Buf + u32Pos, LOG_MEM_BUF_SIZE - u32Pos, "%s", pFuncName);
    u32Pos += (strlen(pFuncName) > 50) ? 50 : strlen(pFuncName);
    HDMI_OSAL_Snprintf(u8Buf + u32Pos, LOG_MEM_BUF_SIZE - u32Pos, ": ");
    u32Pos += 2;
    va_start(argp, fmt);
    HDMI_OSAL_Vsnprintf(u8Buf + u32Pos, LOG_MEM_BUF_SIZE - u32Pos, fmt, argp);
    va_end(argp);
