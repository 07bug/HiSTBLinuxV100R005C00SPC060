...
const static HI_CHAR *s_eHdcpCap2Str[] = {
    "UNDO",         //HDMI_HDCP_UNDO,
    "YES",          //HDMI_HDCP_TRUE,
    "NO",           //HDMI_HDCP_FALSE,
    "ERROR",        //HDMI_HDCP_ERROR,
};

const static HI_CHAR *s_eHdcpErr2Str[] = {
    "UNDO",                  //HDMI_HDCP_ERR_UNDO = 0x0,
    "NONE",                  //HDMI_HDCP_ERR_NONE,
    "UNHPD",                 //HDMI_HDCP_ERR_UNHPD,
    "NO_TIMMING",            //HDMI_HDCP_ERR_NO_TIMMING,
    "LOADKEY_FAIL",          //HDMI_HDCP_ERR_LOADKEY_FAIL,
    "REVOCATION_LIST",       //HDMI_HDCP_ERR_ON_REVOCATION_LIST,
    "RECEIVER_FAIL",         //HDMI_HDCP_ERR_RECEIVER_FAIL,
    "MAXDEV_EXCEEDED",       //HDMI_HDCP_ERR_MAXDEV_EXCEEDED,
    "MAXCASCADE_EXCEEDED",   //HDMI_HDCP_ERR_MAXCASCADE_EXCEEDED,
    "REPEATER_FAIL",         //HDMI_HDCP_ERR_REPEATER_FAIL,
};
#endif
...
const static HI_CHAR *s_eSuspendMode2Str[] = {
    "NONE",          //HDMI_SUSPEND_NONE = 0x0,
    "FAKE",          //HDMI_SUSPEND_FAKE = 0x1,
    "REAL",          //HDMI_SUSPEND_REAL = 0x2,
};
...
const static HI_CHAR *s_eDispFMT2Str[] = {
    ...
    "720P@59.94",       //HI_DRV_DISP_FMT_720P_59_94,
...
static HI_VOID Proc_SWStatus(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    if (HDMI_TRANSITION_NONE < pstHdmiDev->enTransitionState)
    {
        for (i = 0; i < ARRAY_SIZE(s_eTransitionSate2Str); i++)
        {
            ...
            {
                PROC_PRINT(pFile,"%-20s  ", s_eTransitionSate2Str[i]);
            }
        }
    }
    else
    {
        PROC_PRINT(pFile, "%-20s  ", "NONE");
    }
    CHECK_MAX_VALUE(u32Index, pstHdmiDev->enSuspendState, HDMI_SUSPEND_BUTT - 1);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "SuspendMode", s_eSuspendMode2Str[u32Index]);
...
//    PROC_PRINT(pFile, "-------------------------- HDCPtatus ----------------------------\n");
    ...
    CHECK_MAX_VALUE(u32Index, pstHdcpInfo->stHdcpCap.enHdcp22Cap, HDMI_HDCP_ERROR);
    PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCP2.2Support", s_eHdcpCap2Str[u32Index]);
    CHECK_MAX_VALUE(u32Index, pstHdcpInfo->stHdcpCap.enHdcp14Cap, HDMI_HDCP_ERROR);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "HDCP1.4Support",s_eHdcpCap2Str[u32Index]);

    /* line 2 */
    CHECK_MAX_VALUE(u32Index, pstHdcpInfo->enHdcpMode, HDMI_HDCP_MODE_2_2);
    PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCPMode", s_eHdcpMode2Str[u32Index]);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "HDCP1.4KeyLoad", pstHdcpInfo->bHdcp14LoadKey ? STR_YES: STR_NO);

    /* line 3 */
    PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCPEnable", pstHdcpInfo->bHdcpEnable ? STR_YES: STR_NO);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "HDCP2.2CodeLoad", pstHdcpInfo->bHdcp22LoadCode ? STR_YES: STR_NO);

    /* line 4 */
    PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCPAuthStatus", pstHdcpInfo->bHdcpAuthSuc? STR_YES: STR_NO);
    CHECK_MAX_VALUE(u32Index, pstHdcpInfo->enHdcpErrCode, HDMI_HDCP_ERR_REPEATER_FAIL);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "HDCPErrCode", s_eHdcpErr2Str[u32Index]);
...
static HI_VOID Proc_HDRStatus(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    CHECK_POINTER_NO_RET(pstHdrInfo);
...
static HI_VOID Proc_HWStatus(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    //HDMI_HAL_CALL(HAL_HDMI_HardwareStatusGet, HDMI_HARDWARE_STATUS_S stHwStatus);
...
#if defined (HDMI_SUPPORT_LOGIC_SILICON)
    if ( HI_FALSE ==  pstHdmiDev->stHdcpInfo.bHdcpEnable )
    {
...
#endif
    HDMI_HAL_CALL(HAL_HDMI_EmiStatusGet, pstHdmiDev->pstHdmiHal, &stEmiStatus);
...
{
    CHECK_POINTER_NO_RET(pFile);
    CHECK_POINTER_NO_RET(paData);
    CHECK_POINTER_NO_RET(pu32Index);
    CHECK_POINTER_NO_RET(pu32Line);
...
static HI_VOID Proc_VideoInfo(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    {
        // normal
        if (stHwStatus.stInfoFrameStatus.u8AVI[7] < CEA861FVideoCodesMax)
        {
            pTmpFmt = s_stCEA861FVideoCodesDes[stHwStatus.stInfoFrameStatus.u8AVI[7]].pu8FmtStr;
        }
        else
        {
            pTmpFmt = s_stCEA861FVideoCodesDes[0].pu8FmtStr;
        }
    }
    ...
    /* line 10 */
    PROC_PRINT(pFile, "%-15s: %-20s |", "InQuantization", pstVOAttr->enInQuantization ? "FULL" : "LIMITED");
    PROC_PRINT(pFile, "%-15s: %-20s \n", "YCCQuantization", s_eYCCQuantization2Str[unAviByte5.bits.bzYCCQuantizationRange]);

    /* line 11 */
    PROC_PRINT(pFile, "%-15s: %-20s |", "OutQuantization", pstVOAttr->enOutQuantization ? "FULL" : "LIMITED");
    ...
    /* line 19 */
    PROC_PRINT(pFile,"%-15s: %-20s |",  "InQuantization", stHwStatus.stVideoStatus.enInQuantization ? "FULL" : "LIMITED");
    PROC_PRINT(pFile, "%-15s:\n", "AVIInfoRawData");

    /* line 20 */
    PROC_PRINT(pFile,"%-15s: %-20s |",  "OutQuantization",stHwStatus.stVideoStatus.enOutQuantization ? "FULL" : "LIMITED");
    ...
    /* line 23 */
    PROC_PRINT(pFile,"%-15s: %-20s |",  "RGB2YCbCr(CSC)", stHwStatus.stVideoStatus.bRGB2YCbCr ? STR_YES: STR_NO);
    PROC_PRINT(pFile,"%-15s:\n", "VSInfoRawData");

    /* line 24 */
    PROC_PRINT(pFile,"%-15s: %-20s |",  "bCscSaturate", stHwStatus.stVideoStatus.bCscSaturate ? STR_YES: STR_NO);
    ...
    Proc_IFRawData(pFile, len, &i, &u32Line, paData);
    CHECK_MAX_VALUE(u32Index, stHwStatus.stVideoStatus.enOutBitDepth, HDMI_VIDEO_BITDEPTH_BUTT);
    PROC_PRINT(pFile,"%-15s: %-20s |",  "DeepColorMode", s_eDeepColorStr[u32Index]);

    PROC_PRINT(pFile, "%-15s: %-20s \n", "VSInfoEnable", stHwStatus.stInfoFrameStatus.bVSIFEnable ? STR_YES: STR_NO);
...
static HI_VOID Proc_HDRInfo(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    PROC_PRINT(pFile, "%-15s: %-20d \n", "AvgLightLevel", pstHdrAttr->unDescriptor.stType1.u16AverageLightLevel);
...
static HI_VOID Proc_AudioInfo(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    /* line 8 */
    PROC_PRINT(pFile, "%-15s: %-20s |", "AudioEnable", stHwStatus.stAudioStatus.bAudioEnable? STR_YES: STR_NO);
    ...
    PROC_PRINT(pFile, "%-15s: %-20s \n", "AoAppEnable", pstHdmiDev->stAudioEn.bAoAppEnable ? STR_YES : STR_NO);

    /* line 17 */
    PROC_PRINT(pFile, "%-15s: %-20u |", "N",  stHwStatus.stAudioStatus.u32RegNvalue);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "AoIfAppEnable", pstHdmiDev->stAudioEn.bAoIfAppEanble ? STR_YES : STR_NO);

    /* line 18 */
    PROC_PRINT(pFile, "%-15s: %-20u |", "Ref_N",  stHwStatus.stAudioStatus.u32RefNvalue);
    PROC_PRINT(pFile, "%-15s: %-20s \n","AoDrvEnable", pstHdmiDev->stAudioEn.bAoDrvEnable ? STR_YES : STR_NO);

    /* line 19 */
    PROC_PRINT(pFile, "%-15s: %-20s |", "AudioFIFO", STR_NA);
    PROC_PRINT(pFile, "%-15s: %-20s \n","AoSelEnable", pstHdmiDev->stAudioEn.bAoSelEanble ? STR_YES : STR_NO);

#endif
#if defined (HDMI_SUPPORT_LOGIC_HISIV100)
    /* line 15 */
    PROC_PRINT(pFile, "%-15s: %-20u |", "Ref_CTS", stHwStatus.stAudioStatus.u32RefCtsvalue);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "SwCts", stHwStatus.stAudioStatus.bSwCts ? STR_YES : STR_NO);

    /* line 16 */
    PROC_PRINT(pFile, "%-15s: %-20u |", "Reg_CTS(SW)", stHwStatus.stAudioStatus.u32RegCtsSw);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "AoIfAppEnable", pstHdmiDev->stAudioEn.bAoIfAppEanble ? STR_YES : STR_NO);

    /* line 17 */
    PROC_PRINT(pFile, "%-15s: %-20u |", "Reg_CTS(HW)", stHwStatus.stAudioStatus.u32RegCtsvalue);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "AoIfAppEnable", pstHdmiDev->stAudioEn.bAoIfAppEanble ? STR_YES : STR_NO);
...
static HI_VOID Proc_EventInfo(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    CHECK_POINTER_NO_RET(pstEvtInfo);
...
static HI_VOID Proc_EdidRawData(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    CHECK_POINTER_NO_RET(pu8EdidRaw);
...
static HI_VOID Proc_SinkBasicCap(struct seq_file *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    ...
    /* line 9 */
    PROC_PRINT(pFile, "%-20s: %-20s ", "Latency", pstCap->bLatencyFieldsPresent ? STR_YES: STR_NO);
    PROC_PRINT(pFile, "%-20s: %-20s\n", "iLatency", pstCap->bILatencyFieldsPresent ? STR_YES: STR_NO);
    /* line 10 */
    PROC_PRINT(pFile, "%-20s: %-20u ", "PVideoLatency", pstCap->u32VideoLatency);
    PROC_PRINT(pFile, "%-20s: %-20u\n", "iVideoLatency", pstCap->u32InterlacedVideoLatency);
    /* line 11 */
    PROC_PRINT(pFile, "%-20s: %-20u ", "PAudioLatency", pstCap->u32AudioLatency);
    PROC_PRINT(pFile, "%-20s: %-20u\n", "iAudioLatency", pstCap->u32InterlacedAudioLatency);

    /* line 12 */
    PROC_PRINT(pFile, "%-20s: %-20u ", "ExtBlockCnt", pstCap->u8ExtBlockNum);
    PROC_PRINT(pFile, "%-20s: %-20s \n", "Lte340Mcsc", pstCap->bSupportLte340McscScrameble ? STR_YES: STR_NO);
    /* speaker */
...
static HI_VOID Proc_SinkVideoCap(struct seq_file *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    ...
    PROC_PRINT(pFile, "\n");
    /* line 8 Quantization Range Selectable  */
    PROC_PRINT(pFile,"%-20s: %-20s\n", "YccQuanSelectable", pstCap->bYccQrangeSelectable ? STR_YES: STR_NO);
    PROC_PRINT(pFile,"%-20s: %-20s\n", "RgbQuanSelectable", pstCap->bRgbQrangeSelectable ? STR_YES: STR_NO);
...
static HI_VOID Proc_SinkHdrCap(struct seq_file *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    ...
    {
        /* line 2 */
        PROC_PRINT(pFile,"%-20s: %-20s ", "DolbySu_Y422", pstCap->stDolbyCap.unVersionCaps.stVer0.bYUV422_12Bit ? STR_YES: STR_NO);
        PROC_PRINT(pFile,"%-20s: %-20s\n", "DolbySu_2160P60", pstCap->stDolbyCap.unVersionCaps.stVer0.b2160P60 ? STR_YES: STR_NO);
        /* line 3 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyWhite_X", pstCap->stDolbyCap.unVersionCaps.stVer0.u16White_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyWhite_Y", pstCap->stDolbyCap.unVersionCaps.stVer0.u16White_Y);
        /* line 4 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DMmajorVer", pstCap->stDolbyCap.unVersionCaps.stVer0.u8DMmajorVersion);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DMminorVer", pstCap->stDolbyCap.unVersionCaps.stVer0.u8DMminorVersion);
        /* line 5 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyMinPQ", pstCap->stDolbyCap.unVersionCaps.stVer0.u16TargetMinPQ);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyMaxPQ", pstCap->stDolbyCap.unVersionCaps.stVer0.u16TargetMaxPQ);
        /* line 6 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyRed_X", pstCap->stDolbyCap.unVersionCaps.stVer0.u16Red_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyRed_Y", pstCap->stDolbyCap.unVersionCaps.stVer0.u16Red_Y);
        /* line 7 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyGreen_X", pstCap->stDolbyCap.unVersionCaps.stVer0.u16Green_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyGreen_Y", pstCap->stDolbyCap.unVersionCaps.stVer0.u16Green_Y);
        /* line 8 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyBlue_X", pstCap->stDolbyCap.unVersionCaps.stVer0.u16Blue_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyBlue_Y", pstCap->stDolbyCap.unVersionCaps.stVer0.u16Blue_Y);
    }
    else if (1 == pstCap->stDolbyCap.u8VSVDBVersion)
    {
        /* line 2 */
        PROC_PRINT(pFile,"%-20s: %-20s ", "DolbySu_Y422", pstCap->stDolbyCap.unVersionCaps.stVer1.bYUV422_12Bit ? STR_YES: STR_NO);
        PROC_PRINT(pFile,"%-20s: %-20s\n", "DolbySu_2160P60", pstCap->stDolbyCap.unVersionCaps.stVer1.b2160P60 ? STR_YES: STR_NO);
        /* line 3 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DMVersion", pstCap->stDolbyCap.unVersionCaps.stVer1.u8DMVersion);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "Colorimetry", pstCap->stDolbyCap.unVersionCaps.stVer1.bColorimetry);
        /* line 4 */
        PROC_PRINT(pFile,"%-20s: %-20d\n", "LowLatency", pstCap->stDolbyCap.unVersionCaps.stVer1.u8LowLatency);
        /* line 5 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyMinLum", pstCap->stDolbyCap.unVersionCaps.stVer1.u16TargetMinLuminance);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyMaxLum", pstCap->stDolbyCap.unVersionCaps.stVer1.u16TargetMaxLuminance);
        /* line 6 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyRed_X", pstCap->stDolbyCap.unVersionCaps.stVer1.u16Red_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyRed_Y", pstCap->stDolbyCap.unVersionCaps.stVer1.u16Red_Y);
        /* line 7 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyGreen_X", pstCap->stDolbyCap.unVersionCaps.stVer1.u16Green_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyGreen_Y", pstCap->stDolbyCap.unVersionCaps.stVer1.u16Green_Y);
        /* line 8 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyBlue_X", pstCap->stDolbyCap.unVersionCaps.stVer1.u16Blue_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyBlue_Y", pstCap->stDolbyCap.unVersionCaps.stVer1.u16Blue_Y);
    }
    else if (2 == pstCap->stDolbyCap.u8VSVDBVersion)
    {
        /* line 2 */
        PROC_PRINT(pFile,"%-20s: %-20s ", "DolbySu_Y422", pstCap->stDolbyCap.unVersionCaps.stVer2.bYUV422_12Bit ? STR_YES: STR_NO);
        PROC_PRINT(pFile,"%-20s: %-20s\n", "BackLightCtrol", pstCap->stDolbyCap.unVersionCaps.stVer2.bBackLightContol? STR_YES: STR_NO);
        /* line 3 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DMVersion", pstCap->stDolbyCap.unVersionCaps.stVer2.u8DMVersion);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "BackLtMinLuma", pstCap->stDolbyCap.unVersionCaps.stVer2.u8BackLtMinLuma);
        /* line 4 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "Interface", pstCap->stDolbyCap.unVersionCaps.stVer2.u8Interface);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "RGB/YUV444_10/12B", pstCap->stDolbyCap.unVersionCaps.stVer2.u8YUV444RGB444_10b12b);
        /* line 5 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyMinPQv2", pstCap->stDolbyCap.unVersionCaps.stVer2.u16TargetMinPQv2);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyMaxPQv2", pstCap->stDolbyCap.unVersionCaps.stVer2.u16TargetMaxPQv2);
        /* line 6 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyRed_X", pstCap->stDolbyCap.unVersionCaps.stVer2.u16Red_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyRed_Y", pstCap->stDolbyCap.unVersionCaps.stVer2.u16Red_Y);
        /* line 7 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyGreen_X", pstCap->stDolbyCap.unVersionCaps.stVer2.u16Green_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyGreen_Y", pstCap->stDolbyCap.unVersionCaps.stVer2.u16Green_Y);
        /* line 8 */
        PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyBlue_X", pstCap->stDolbyCap.unVersionCaps.stVer2.u16Blue_X);
        PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyBlue_Y", pstCap->stDolbyCap.unVersionCaps.stVer2.u16Blue_Y);
    }
}
