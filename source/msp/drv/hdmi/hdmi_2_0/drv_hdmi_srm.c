...
HI_S32  DRV_HDMI_SrmSet(HI_U8 *pu8Data,HI_U32 u32Len,HI_BOOL bFromUser)
{
    ...
    /* Sequentially increasing unique SRM numbers. Higher numbered SRMs are more recent. */
    if(pstSrmInfo->unHead1.bits.b16_SrmVersion > pstSrmTmp->unHead1.bits.b16_SrmVersion)
    {
        HDMI_ERR("error version %u(old)>=%u(new),no update!\n",pstSrmInfo->unHead1.bits.b16_SrmVersion,pstSrmTmp->unHead1.bits.b16_SrmVersion);
        goto error_fail;
    }
