        ...
        for (s32DataSize=0;s32DataSize < pstCfg->s32DataSize;s32DataSize++,pu8Data++)
        {
            ...
            for (i=0 ; (i < u32TimeOut) && (HDMI_PWD_MST_STATE_pwd_fifo_emptyGet()||(HDMI_PWD_DATA_CNT_pwd_fifo_data_cntGet()==0));i++)
            {
                ...
                if (HDMI_DDC_MST_STATE_ddc_i2c_no_ackGet() || HDMI_DDC_MST_STATE_ddc_i2c_bus_lowGet())
                {
                    ...
                    HDMI_WARN("DDC noack=%d,buslow=%d error!\n",HDMI_DDC_MST_STATE_ddc_i2c_no_ackGet() , HDMI_DDC_MST_STATE_ddc_i2c_bus_lowGet());
                    return HI_FAILURE;
                }
...
RETURN:
    ...
    /* record data */
/*
    pstTmpElem->pu8Data = (HI_U8 *)HDMI_VMALLOC(pstCfg->s32DataSize);
    ...
    {
        HDMI_ERR("ddc HDMI_VMALLOC fail\n");
    }
*/
    return s32Ret;
}
