...
#define HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,u64ReauthMs,enErrCode) do{\
    if(HI_SUCCESS != s32Ret)\
    {\
        if(pstHdcp14->stAttr.bFailStopMach){HAL_HDMI_MachStop(pstHdcp14->stInit.u32MachId);}\
        HDMI_INFO("retval=%d is not HI_SUCCESS!\n",s32Ret);\
        ...
        pstHdcp14->stStatus.enHdcpErrCode = enErrCode;\
...
int  Hdcp14LoadDebugSrcKey(void)
{
    ...
    for(i=0;i<512;i=i+4)
    {
        for(k=0;k<4;k++)
        {
            HDMI_RegWriteMap(0xf8ce0854,k);
            msleep(1);
            HDMI_RegWriteMap( u32DebugKeyAddr ,s_au32HdcpSrcKey[i+k]);
        }
...
static HI_S32 Hdcp14BksvRead(HDMI_DEVICE_ID_E enHdmiId,HI_U8 *pu8KsvData)
{
    ...
    if(stDdcCfg.s32DataSize != HDCP14_KSV_SIZE_5BYTES)
    {
        s32Ret = HDMI_HDCP_ERROR;
    }
    return s32Ret;
}
...
#else
    HI_S32           s32Ret = HI_SUCCESS;
    HI_S32           s32Eanble = HDMI_KEY_TRUE;
    HDMI_TEE_PARAM_S stParam;
    HDMI_MEMSET(&stParam, 0, sizeof(stParam));

    s32Eanble = bEnable ? HDMI_KEY_TRUE : HDMI_KEY_FALSE;

    stParam.enTeeCmd    = HI_HDMI_HDCP14_ENC_ENABLE;
    stParam.pvData      = &s32Eanble;
    stParam.u32Size     = sizeof(s32Eanble);
    s32Ret = DRV_HDMI_TeeCmdSend(enHdmiId, &stParam);
...
#else
    HI_S32           s32Ret = HI_SUCCESS;
    HI_S32           s32Enable = HDMI_KEY_FALSE;

    HDMI_TEE_PARAM_S stParam;
    HDMI_MEMSET(&stParam, 0, sizeof(stParam));
    s32Enable = bEnable ? HDMI_KEY_TRUE : HDMI_KEY_FALSE;
    stParam.enTeeCmd    = HI_HDMI_HDCP14_RPT_ENABLE;
    stParam.pvData      = &s32Enable;
    stParam.u32Size     = sizeof(s32Enable);
    s32Ret = DRV_HDMI_TeeCmdSend(enHdmiId, &stParam);
...
HI_VOID Hdcp14MachCallBack(HI_VOID *pData)
{
    ...
    static HI_BOOL       bR0FailFirst   = HI_TRUE;
    ...
    switch(pstHdcp14->stRun.enMachStatus)
    {
        ...
        case HDCP14_MACH_PI_START:
            ...
            HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,HDCP14_REAUTH_WAIT_MS_BKSV_FAIL,HDMI_HDCP_ERR_RECEIVER_FAIL);
#ifndef HDMI_TEE_SUPPORT
            s32Ret = DRV_HDMI_SrmCheck(pstHdcp14->stStatus.stPartI.au8Bksv,1);
            pstHdcp14->stAttr.bFailStopMach     = HI_TRUE;
            HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,0,HDMI_HDCP_ERR_ON_REVOCATION_LIST);
        ...
        case HDCP14_MACH_PI_BCAP:
            ...
            HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,0,HDMI_HDCP_ERR_RECEIVER_FAIL);
        ...
        case HDCP14_MACH_PI_AKSV:
            ...
            HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,HDCP14_REAUTH_WAIT_MS_AN_FAIL,HDMI_HDCP_ERR_RECEIVER_FAIL);
        ...
        case HDCP14_MACH_PI_DONE:
            ...
            else
            {
                ...
                if (bR0FailFirst)
                {
                    bR0FailFirst = HI_FALSE;
                    HDMI_ERR("hdcp14 PARTI done fail!(AR0!=BR0)\n");
                }
                HDMI_INFO("hdcp14 PARTI done fail!(AR0!=BR0)\n");
                HDCP14_FAILURE_REAUTHEN(pstHdcp14,HI_FAILURE,HDCP14_REAUTH_WAIT_MS_R0_FAIL,HDMI_HDCP_ERR_RECEIVER_FAIL);
            }
            break;
        ...
        case HDCP14_MACH_PII_START:
            ...
            if (pstHdcp14->stStatus.stPartII.unBcaps.u8.bzKsvFifoReady)
            {
                ...
                HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,0,HDMI_HDCP_ERR_REPEATER_FAIL);
                if (pstHdcp14->stStatus.stPartII.unBstatus.u16.bzMaxDevsExceeded )
                {
                    Hdcp14EncrypEnable(pstHdcp14->stInit.enHdmiId,HI_FALSE);
                    HDMI_ERR("hdcp14 MaxDevsExceeded MaxCasCadeExceeded,Fail!\n");
                    HDCP14_FAILURE_REAUTHEN(pstHdcp14,HI_FAILURE,HDCP14_REAUTH_WAIT_MS_ENCYP,HDMI_HDCP_ERR_MAXDEV_EXCEEDED);
                    break;
                }
                if ( pstHdcp14->stStatus.stPartII.unBstatus.u16.bzMaxCasCadeExceeded )
                {
                    Hdcp14EncrypEnable(pstHdcp14->stInit.enHdmiId,HI_FALSE);
                    HDMI_ERR("hdcp14 MaxDevsExceeded MaxCasCadeExceeded,Fail!\n");
                    HDCP14_FAILURE_REAUTHEN(pstHdcp14,HI_FAILURE,HDCP14_REAUTH_WAIT_MS_ENCYP,HDMI_HDCP_ERR_MAXCASCADE_EXCEEDED);
                    break;
                }
                ...
                HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,0,HDMI_HDCP_ERR_REPEATER_FAIL);
#ifndef HDMI_TEE_SUPPORT
                s32Ret = DRV_HDMI_SrmCheck(pstHdcp14->stStatus.stPartII.pu8BksvListData,pstHdcp14->stStatus.stPartII.unBstatus.u16.bzDeviceCnt);
                pstHdcp14->stAttr.bFailStopMach     = HI_TRUE;
                HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,0,HDMI_HDCP_ERR_REPEATER_FAIL);
                pstHdcp14->stAttr.bFailStopMach     = HI_FALSE;
#endif
        ...
        case HDCP14_MACH_PIII_START:
            if (pstHdcp14->stInit.pfnEventCallBack && pstHdcp14->stInit.pvEventData)
            {
                pstHdcp14->stStatus.enHdcpErrCode = HDMI_HDCP_ERR_NONE;
        ...
        case HDCP14_MACH_PIII_RI_MONITOR:
            ...
            HDCP14_FAILURE_REAUTHEN(pstHdcp14,s32Ret,HDCP14_REAUTH_WAIT_MS_Ri_FAIL,HDMI_HDCP_ERR_RECEIVER_FAIL);
            break;
...
HI_S32  HAL_HDMI_Hdcp14Support(HDMI_DEVICE_ID_E enHdmiId, HDMI_HDCP_STAT_EN *penHdcp14Cap)
{
    //HI_U8           au8Bksv[HDCP14_KSV_SIZE_5BYTES];
    HDCP14_INFO_S   *pstHdcp14 = HI_NULL;

    pstHdcp14 = Hdcp14InfoGet(enHdmiId);
    HDCP14_NULL_CHK(penHdcp14Cap);
    HDCP14_NULL_CHK(pstHdcp14);
    HDCP14_INIT_CHK(pstHdcp14);

    //s32Ret = Hdcp14BksvRead(enHdmiId,au8Bksv);
    *penHdcp14Cap = HDMI_HDCP_TRUE;
