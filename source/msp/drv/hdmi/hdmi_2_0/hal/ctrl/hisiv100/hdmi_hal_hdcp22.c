...
#define HDCP22_CHK_FAILURE_STOP(pstHdcp22,s32Ret,enErrCode) do{\
    if (!pstHdcp22){HDMI_ERR("pstHdcp22 null!\n");return;};\
    if(HI_SUCCESS != s32Ret){\
        HDMI_WARN("retval=%d is not HI_SUCCESS! return failure!\n", s32Ret);\
        HAL_HDMI_MachStop(pstHdcp22->stInit.u32MachId);\
        HDCP22_AuthStop();\
        pstHdcp22->stAuth.enHdcpErrCode = enErrCode;\
        if(pstHdcp22->stInit.pfnEventCallBack)\
...
do { \
    ...
    HDMI_RegWriteMap(BASE_ADDR_mcu_dram + addr*4, value); \
    } \
    else { \
...
do { \
    if (HI_NULL != g_Hdcp22McuDramVirAddr) { \
        *pvalue = HDMI_RegReadMap(BASE_ADDR_mcu_dram + addr*4);\
    } \
    else { \
        HDMI_ERR("g_Hdcp22McuDramVirAddr is null!!!\n"); \
    } \
...
HI_VOID HDCP22_IntrProcess(HDCP22_INFO_S *pstHdcp22)
{
    ...
    HDCP22_NULL_CHK_NORET(pstHdcp22);
    HDCP22_NULL_CHK_NORET(pstHdcp22->stInit.pfnEventCallBack);
...
#ifndef HDMI_TEE_SUPPORT
        s32Ret = DRV_HDMI_SrmCheck(&(pstHdcp22->stAuth.u8RecvId[0]), 1);
        HDCP22_CHK_FAILURE_STOP(pstHdcp22,s32Ret,HDMI_HDCP_ERR_ON_REVOCATION_LIST);
#endif
    }
    ...
    if (HI_TRUE == uIntrB0.bits.bRevidListReady)
    {
        ...
        if(pstHdcp22->stAuth.stRxInfo.bMaxDevsExceeded)
        {
            pstHdcp22->stAuth.enHdcpErrCode = HDMI_HDCP_ERR_MAXDEV_EXCEEDED;
            pstHdcp22->stInit.pfnEventCallBack(pstHdcp22->stInit.pvEventData, HDMI_EVENT_HDCP_FAIL);
            pstHdcp22->stRun.enMachStatus = HDCP22_MACH_AUTH_EN;
        }
        if(pstHdcp22->stAuth.stRxInfo.bMaxCascadeExceeded)
        {
            pstHdcp22->stAuth.enHdcpErrCode = HDMI_HDCP_ERR_MAXCASCADE_EXCEEDED;
            pstHdcp22->stInit.pfnEventCallBack(pstHdcp22->stInit.pvEventData, HDMI_EVENT_HDCP_FAIL);
            pstHdcp22->stRun.enMachStatus = HDCP22_MACH_AUTH_EN;
        }
        HDCP22_RecvIdListGet(&(pstHdcp22->stAuth.u8RecvIdList[0]), pstHdcp22->stAuth.stRxInfo.u8DevCnt);
#ifndef HDMI_TEE_SUPPORT
        s32Ret = DRV_HDMI_SrmCheck(&(pstHdcp22->stAuth.u8RecvIdList[0]), pstHdcp22->stAuth.stRxInfo.u8DevCnt);
        HDCP22_CHK_FAILURE_STOP(pstHdcp22,s32Ret,HDMI_HDCP_ERR_ON_REVOCATION_LIST);
#endif
    }
    ...
    {
        pstHdcp22->stAuth.enHdcpErrCode = HDMI_HDCP_ERR_RECEIVER_FAIL;
        pstHdcp22->stInit.pfnEventCallBack(pstHdcp22->stInit.pvEventData, HDMI_EVENT_HDCP_FAIL);
        pstHdcp22->stRun.enMachStatus = HDCP22_MACH_AUTH_EN;
    }
    ...
    {
        pstHdcp22->stAuth.enHdcpErrCode = HDMI_HDCP_ERR_NONE;
    ...
    switch(pstHdcp22->stRun.enMachStatus)
    {
        ...
        case HDCP22_MACH_WAIT_START:
            if ( HAL_HDMI_MachMsGet()-(*pu64StartStamp) > pstHdcp22->stAttr.u64StartWaitMs )
            {
                ...
                if (HI_SUCCESS != DRV_HDMI_TeeCmdSend(pstHdcp22->stInit.enHdmiId, &stParam))
                {
                    HDMI_ERR("tee start fail!\n");
                }
#else
                HDCP22_AuthStart();
#endif
...
HI_S32  HAL_HDMI_Hdcp22McuCodeLoad(HDMI_DEVICE_ID_E enHdmiId)
{
    ...
    pstHdcp22->stRun.bMcuLoad = (g_8051Code[0] == HDMI_RegReadMap(BASE_ADDR_mcu_pram) ) ? HI_TRUE : HI_FALSE ;
    pstHdcp22->stRun.bMcuLoad &= (g_8051Code[1] == HDMI_RegReadMap(BASE_ADDR_mcu_pram + 4) ) ? HI_TRUE : HI_FALSE ;
    ...
    if(!pstHdcp22->stRun.bMcuLoad)
    {
        ...
        for(u32Cnt = 0;(u32Cnt < HDMI_ARRAY_SIZE(g_8051Code)); u32Cnt++)
        {
                ...
                u32MaxWriteTimes--)
            {
                ...
                HDMI_RegWriteMap((BASE_ADDR_mcu_pram  + u32Cnt*4), u32Data);
                u32Data = HDMI_RegReadMap(BASE_ADDR_mcu_pram  + u32Cnt*4);
                s32Ret = HI_SUCCESS;
            }
...
HI_S32 HAL_HDMI_Hdcp22AuthStatusGet(HDMI_DEVICE_ID_E enHdmiId, HDCP22_AUTH_STATUS_S *pstAuthStauts)
{
    ...
    HDMI_MEMCPY(pstAuthStauts, &(pstHdcp22->stAuth), sizeof(HDCP22_AUTH_STATUS_S));
...
HI_S32  HAL_HDMI_Hdcp22Support(HDMI_DEVICE_ID_E enHdmiId, HDMI_HDCP_STAT_EN *penHdcp22Cap)
{
    HI_U8           u8DdcByte = 0;
    HI_S32          s32Ret = HI_SUCCESS;
    HDCP22_INFO_S   *pstHdcp22 = HI_NULL;
    ...
    HDCP22_NULL_CHK(penHdcp22Cap);
    pstHdcp22 = HDCP22_InfoGet(enHdmiId);
    HDCP22_NULL_CHK(pstHdcp22);
    HDCP22_INIT_CHK(pstHdcp22);

    *penHdcp22Cap = HDMI_HDCP_UNDO;
    ...
    {
        *penHdcp22Cap = u8DdcByte ? HDMI_HDCP_TRUE : HDMI_HDCP_FALSE;
        s32Ret = HI_SUCCESS;
    }
    else
    {
        *penHdcp22Cap = HDMI_HDCP_ERROR;
        s32Ret = HI_FAILURE;
    }
    return s32Ret;
