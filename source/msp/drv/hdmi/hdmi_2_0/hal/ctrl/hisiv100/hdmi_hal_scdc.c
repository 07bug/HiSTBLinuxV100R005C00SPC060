...
#endif
        /*poll status*/
        s32DdcErr = ScdcScrambleGet(enHdmiId,&stScramble);

        if(s32DdcErr)
        {
            u32DdcErrCnt++;
            HDMI_WARN("scdc ddcErr %d time!\n",u32DdcErrCnt);

            if(u32DdcErrCnt >= 2)
            {
                HDMI_WARN("force exit scdc!\n\n");
                s32DdcErr    = 0;
                u32DdcErrCnt = 0;
                break;
            }
...
HI_S32 HAL_HDMI_ScdcSupportGet(HDMI_DEVICE_ID_E enHdmiId,HI_BOOL *pbSupport)
{
    ...
    if ( s32Ret==stDdcCfg.s32DataSize )//||specification define (u8DdcByte == SCDC_HDMI20_VERSION) || (u8DdcByte == 0) )
    {
