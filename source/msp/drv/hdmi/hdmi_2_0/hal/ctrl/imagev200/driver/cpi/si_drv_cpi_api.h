...
typedef struct _SiiCpiStatus_t
{
    ...
    uint8_t         cecError;
    uint16_t        msgId;
    uint32_t        u32WriteQueCnt; /* count of write to queue */
    uint32_t        u32SendCnt;     /* count of send */
    bool_t          bIsSending;     /* whether the command is sending. to avoid the interrupt report abnormally, and the queue full. */
} SiiCpiStatus_t;
