...
#ifndef HDMI_BUILD_IN_BOOT
int SiiDrvTxEdidGet(SiiInst_t inst, SiiEdid_t* pEdid)
{
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);
...
void SiiDrvTxPwdSrst(SiiInst_t inst)
{
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);
    ...
    SII_LIB_LOG_DEBUG1(pObj,"pwd srst & resent avi! \n");
    SiiDrvCraWrReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__PWD_SRST,BIT_MSK__PWD_SRST__REG_SW_RST);
    udelay(10);
    SiiDrvCraClrBit8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__PWD_SRST,BIT_MSK__PWD_SRST__REG_SW_RST );

    SiiDrvCraWrReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__TPI_INFO_FSEL, BIT_ENUM__TPI_INFO_FSEL__AVI);
    SiiDrvCraPutBit8(sCraInstGet(pObj), sBaseAddrGet(pObj)| REG_ADDR__TPI_INFO_EN, 0xC0, 0xC0);
    return ;
}
...
void SiiDrvTxScdcConfig(SiiInst_t inst, HDMI_SCDC_STATUS_S *pstScdcStatus)
{
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);

    SiiModHdmiTxSet(pObj->instHdmiTx, SII_MOD_HDMI_TX_OPCODE__SCDC_CONFIG, (void *)pstScdcStatus);
}
#endif
...
void SiiDrvTxAvMuteSet(SiiInst_t inst, SiiAvmuteCfg_s *pstAvmute)
{
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);
    SiiModHdmiTxSet(pObj->instHdmiTx, SII_MOD_HDMI_TX_OPCODE__AVMUTE, pstAvmute);
}
...
void SiiDrvTxNullPacketOnOffSet(SiiInst_t inst, bool_t onOff)
{
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);

    SII_LIB_LOG_DEBUG1(pObj, "null packet onOff: %s\n", onOff ? "ON":"OFF");
    SiiModHdmiTxSet(pObj->instHdmiTx, SII_MOD_HDMI_TX_OPCODE__NULL_PACKET, &onOff);
}
...
void SiiDrvTxHdcpErrCodeGet(SiiInst_t inst, SiiDrvHdcpErrCode *pstHdcpErrCode)
{
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);
    SiiModHdcpGet(pObj->instHdcp, SII_MOD_HDCP_OPCODE__HDCP_ERR_CODE, pstHdcpErrCode);
}
...
int SiiDrvTxHdcpCapabilityGet(SiiInst_t inst, SiiHdcpCaps_t *pstHdcpSupport)
{
    int     s32Ret = 0;
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);
    s32Ret = SiiModHdmiTxGet(pObj->instHdmiTx, SII_MOD_HDMI_TX_OPCODE__HDCP_CAPABILITY, pstHdcpSupport);
    return s32Ret;
}
...
void SiiDrvTxHdcpEncryption(SiiInst_t inst, bool_t bEnable)
{
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);
    SiiModHdcpSet(pObj->instHdcp, SII_MOD_HDCP_OPCODE__HDCP_PROTECTION, &bEnable);
}
#endif
...
void SiiDrvTxDitherConfig(SiiInst_t inst, SiiDitherCfg_t *pstDitherMode)
{
    TxObj_t* pObj = (TxObj_t*)SII_LIB_OBJ_PNTR(inst);

    SiiModVidpathSet(pObj->instVideoPath, SII_MOD_VIDPATH_DITHER_CONFIG, pstDitherMode);
