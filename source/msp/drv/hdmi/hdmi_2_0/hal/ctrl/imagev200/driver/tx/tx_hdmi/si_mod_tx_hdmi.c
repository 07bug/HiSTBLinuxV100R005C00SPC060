...
static void sAvMuteSet(HdmiTxObj_t* pObj, SiiAvmuteCfg_s *pstAvMute);
#ifndef HDMI_BUILD_IN_BOOT
...
static int sHdcpCapabilityGet(HdmiTxObj_t* pObj);
#endif
...
static void sNullPacketEnable(HdmiTxObj_t* pObj, bool_t bEnable);
...
bool_t SiiModHdmiTxSet(SiiInst_t inst, SiiModHdmiTxInternalOpcode_t opcode, const void *inData)
{
    ...
    case SII_MOD_HDMI_TX_OPCODE__AVMUTE:
        {
            SiiAvmuteCfg_s *pstAvmute = (SiiAvmuteCfg_s *)inData;
            pObj->bAvMute = pstAvmute->bMuteSet;
            sAvMuteSet(pObj, pstAvmute);
            break;
        }
    ...
    case SII_MOD_HDMI_TX_OPCODE__SCDC_CONFIG:
        {
            HDMI_SCDC_STATUS_S * pstScdcStatus = (HDMI_SCDC_STATUS_S *)inData;
            SiiDrvTxScdcSet(pObj->scdcInst, pstScdcStatus);
...
#endif
#endif
    case SII_MOD_HDMI_TX_OPCODE__NULL_PACKET:
        {
            sNullPacketEnable(pObj, *(bool_t*)inData);
            break;
        }
...
//-------------------------------------------------------------------------------------------------
int SiiModHdmiTxGet(SiiInst_t inst, SiiModHdmiTxInternalOpcode_t opcode, void *outData)
{
    int             s32Ret = 0;
    HdmiTxObj_t* pObj = (HdmiTxObj_t*)SII_LIB_OBJ_PNTR(inst);
    //uint16_t cecAddr = 0;
    ...
    case SII_MOD_HDMI_TX_OPCODE__HDCP_CAPABILITY:
        s32Ret = sHdcpCapabilityGet(pObj);
...
static void sPlugStatusUpdate(HdmiTxObj_t* pObj)
{
    ...
    else
    {
        ...
        #ifdef HDMI_HDCP_SUPPORT
        pObj->stHdcpCaps.enHdcp14 = HDMI_HDCP_UNDO;
        pObj->stHdcpCaps.enHdcp22 = HDMI_HDCP_UNDO;
        #endif
...
static int sHdcpCapabilityGet(HdmiTxObj_t* pObj)
{
    ...
    int         s32Ret = 0;
    ddcComErr_t ddcErr = SI_TX_DDC_ERROR_CODE_NO_ERROR;

#ifndef HDMI_BUILD_IN_BOOT
    HDMI_MUTEX_LOCK(pObj->pConfig->stDdcLock);
#endif

    pObj->stHdcpCaps.enHdcp14 = Sii_HDMI_HDCP_TRUE;

    pObj->stHdcpCaps.enHdcp22 = Sii_HDMI_HDCP_UNDO;
    ddcErr = sUpdateEdid(pObj, 0xFF, 0x50, &dsHdcp_2_2_Supported, 1);
    if(SI_TX_DDC_ERROR_CODE_NO_ERROR == ddcErr)
    {
        dsHdcp_2_2_Supported = (dsHdcp_2_2_Supported & BIT_MASK_HDCP_2_2_SUPPORT) >> 2;
        SII_LIB_LOG_DEBUG1(pObj, "HDCP2.2 ddc read success,hdcp22supprot=%d!\n", dsHdcp_2_2_Supported);
        if(dsHdcp_2_2_Supported)
        {
            pObj->stHdcpCaps.enHdcp22 = Sii_HDMI_HDCP_TRUE;
        }
        else
        {
            pObj->stHdcpCaps.enHdcp22 = Sii_HDMI_HDCP_FALSE;
        }
    }
    else
    {
        SII_LIB_LOG_WARN(pObj, "HDCP2.2 ddc read fail!ddcErr=%d!\n",ddcErr);
        pObj->stHdcpCaps.enHdcp22 = Sii_HDMI_HDCP_ERROR;
        s32Ret |= -1;
    }
...
static void sPutTMDSOnOff(HdmiTxObj_t* pObj, bool_t onOff)
{
    ...
    /* 20150617: sdk, fixed the snmsung UA65F900AJ TV display issue(1080P_30, 2160P_30), have two things must do for this TV.
...
static void sReset(HdmiTxObj_t* pObj, SiiResetType_t resetSrc, uint8_t reset)
{
    ...
    // sdk, Because the reset software(SII_RESET_TYPE__SOFTWARE) can chage the reg REG_ADDR__CEC_CONFIG_CPI to default value,
    ...
    SiiDrvCraSetBit8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__CEC_DEBUG_3, BIT_MSK__CEC_DEBUG_3__CEC_REG_NACK_HDR);
}
#ifndef HDMI_BUILD_IN_BOOT
...
static void sHdmiHardWareStatusGet(HdmiTxObj_t* pObj,  SiiHdmiStatus_t *hdmiStatus)
{
    ...
    if(SiiDrvCraRdReg8(craInst,  baseAddr | REG_ADDR__P2T_CTRL) & BIT_MSK__P2T_CTRL__REG_DC_PKT_EN)
    {
        hdmiStatus->outBitDepth = (SiiDrvCraRdReg8(craInst,  baseAddr | REG_ADDR__P2T_CTRL) & BIT_MSK__P2T_CTRL__REG_PACK_MODE);
    }
    else
    {
        hdmiStatus->outBitDepth = HDMI_DEEP_COLOR_OFF;
    }
...
static void sHardwareInit(HdmiTxObj_t* pObj)
{
    ...
    or it will  disturb other device cec communication,such as ONKYU TX-RZ800 & Samsung UA55HU6000JXXZ;
    when enable cec ,set 0 in bit cec_reg_nack_hdr, and it must clear cec rx-fifo whenever recieve an message.*/
    SiiDrvCraSetBit8(craInst, baseAddr | REG_ADDR__CEC_DEBUG_3, BIT_MSK__CEC_DEBUG_3__CEC_REG_NACK_HDR);
}
...
static void sAvMuteSet(HdmiTxObj_t* pObj, SiiAvmuteCfg_s *pstAvMute)
{
    ...
    else
    {
        ...
        if (pstAvMute->bMuteRptEn)
        {
            if (pstAvMute->u32RptCnt >= TIMER_START__AVMUTE_CNT_MAX)
            {
                u16Timer = TIMER_START__AVMUTE_CLR_STOP_MAX;
                SII_LIB_LOG_DEBUG1(pObj, "u16Timer overflow!\n");
            }
            else
            {
                u16Timer = pstAvMute->u32RptCnt * 17;   // 1/60 ms per frame
            }

            SiiLibSeqTimerStop(pObj->timerAvmute);
            if (u16Timer > 0)
            {
                SiiLibSeqTimerStart(pObj->timerAvmute, u16Timer, 0);
            }
        }
...
static void sNullPacketEnable(HdmiTxObj_t* pObj, bool_t bEnable)
{
    ...
    if (bEnable)
    {
        SiiDrvCraSetBit8(craInst, baseAddr | REG_ADDR__P2T_CTRL, BIT_MSK__P2T_CTRL__REG_NULL_PKT_EN);
    }
    else
    {
        SiiDrvCraClrBit8(craInst, baseAddr | REG_ADDR__P2T_CTRL, BIT_MSK__P2T_CTRL__REG_NULL_PKT_EN);
    }
}
