...
#define OPTM_TABL_LEN                61

const HI_S32 OPTM_SIN_TABLE[OPTM_TABL_LEN] = {
...
const HI_U32 OPTM_COS_TABLE[OPTM_TABL_LEN] = {
...
const static OPTM_CSC_TABLE_S *OPTM_CSC_MatrixTbl[4][4] =
{
    /* ALG_CS_eUnknown = 0 */
    {
...
{
    HI_S32 s32ContrstAdj = 0, s32SatAdj = 0;
    HI_S32 s32Rgain = 0, s32Ggain = 0, s32Bgain = 0;
    HI_U32 u32Hue = 0;

    HIFB_CHECK_NULLPOINTER_UNRETURN(pstCscDrvPara);
    HIFB_CHECK_NULLPOINTER_UNRETURN(pMatrixOri);
    HIFB_CHECK_NULLPOINTER_UNRETURN(pMatrixDst);
    ...
    HIFB_CHECK_ARRAY_OVER_UNRETURN(u32Hue, OPTM_TABL_LEN);
...
static HI_VOID OPTM_CalcCscTmprtMat(OPTM_ALG_CSC_DRV_PARA_S *pstCscDrvPara, OPTM_CSC_TABLE_S *pMatrixOri, OPTM_CSC_TABLE_S *pMatrixDst)
{
    ...
    HI_GFX_Memset(&stYCbCrColrTemp, 0x0,sizeof(stYCbCrColrTemp));
...
HI_VOID OPTM_CalcCscCoef_YCbCrtoYCbCr(OPTM_ALG_CSC_DRV_PARA_S *pstCscDrvPara, const OPTM_CSC_TABLE_S *pMatrixOri, OPTM_CSC_TABLE_S *pMatrixDst)
{
    ...
    HI_GFX_Memset(&DstCSCTableTmp, 0x0,sizeof(DstCSCTableTmp));
...
HI_VOID OPTM_CalcCscMatrix(OPTM_ALG_CSC_DRV_PARA_S *pstCscDrvPara, OPTM_ALG_CSC_RTL_PARA_S *pstCscRtlPara)
{
    ...
    HIFB_CHECK_NULLPOINTER_UNRETURN(pstCscDrvPara);
    HIFB_CHECK_NULLPOINTER_UNRETURN(pstCscRtlPara);

    HI_GFX_Memset(&stCscTbl, 0x0,sizeof(stCscTbl));
    HI_GFX_Memset(&stAlgCSIn, 0x0,sizeof(stAlgCSIn));
    HI_GFX_Memset(&stAlgCSOut, 0x0,sizeof(stAlgCSOut));
    GetCscType(pstCscDrvPara->eInputCS, &stAlgCSIn);
    GetCscType(pstCscDrvPara->eOutputCS, &stAlgCSOut);

    HIFB_CHECK_ARRAY_OVER_UNRETURN(stAlgCSIn.eCsType, 4);
    HIFB_CHECK_ARRAY_OVER_UNRETURN(stAlgCSOut.eCsType, 4);
...
HI_VOID GetCscType(OPTM_COLOR_SPACE_E eCSDrv, OPTM_ALG_CS_S *pstCsAlg)
{
    HIFB_CHECK_NULLPOINTER_UNRETURN(pstCsAlg);
...
static inline HI_VOID DRV_CSC_CalcYCbCrDc(OPTM_ALG_CSC_DRV_PARA_S *pstCscDrvPara, OPTM_ALG_CSC_RTL_PARA_S *pstCscRtlPara)
{
    ...
    HIFB_CHECK_NULLPOINTER_UNRETURN(pstCscDrvPara);
    HIFB_CHECK_NULLPOINTER_UNRETURN(pstCscRtlPara);
    tmpContrst = (HI_S32)pstCscDrvPara->u32Contrst;
    tmpBright  = (HI_S32)pstCscDrvPara->u32Bright - 50;
    ...
    else
    {
        ...
        {
            case ALG_CS_RANGE_FULL:
                pstCscRtlPara->s32CscDcIn_2 = tmpBright *50 / tmpContrst;
                pstCscRtlPara->s32CscDcIn_1 = -128;
                pstCscRtlPara->s32CscDcIn_0 = -128;
                break;
            case ALG_CS_RANGE_LMTD:
                pstCscRtlPara->s32CscDcIn_2 = -16 + (tmpBright *50 / tmpContrst);
                pstCscRtlPara->s32CscDcIn_1 = -128;
                pstCscRtlPara->s32CscDcIn_0 = -128;
                break;
            default:
