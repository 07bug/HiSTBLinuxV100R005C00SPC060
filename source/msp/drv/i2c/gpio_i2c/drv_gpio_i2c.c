...
static HI_S32 DRV_GPIOI2C_ReadData_SiLabs(HI_U32 I2cNum, HI_U8 devaddress, HI_U32 address, HI_U32 addresslen,
                                   HI_BOOL bSendRegAddress, HI_U8 *pData, HI_U32 DataLen)
{
    HI_U32 i = 0;
    //HI_U32 ret; //TC MODIFY
...
HI_S32 HI_DRV_GPIOI2C_ReadExt(HI_U32 I2cNum, HI_U8 devaddress, HI_U32 address, HI_U32 addresslen, HI_U8 *pData,
                              HI_U32 DataLen)
{
    if((addresslen > 4) || (DataLen > HI_I2C_MAX_LENGTH) || (HI_NULL == pData))
    {
...
HI_S32 HI_DRV_GPIOI2C_Read_SiLabs(HI_U32 I2cNum, HI_U8 devaddress, HI_U32 address, HI_U32 addresslen, HI_U8 *pData,
                              HI_U32 DataLen)
{
     if((addresslen > 4) || (DataLen > HI_I2C_MAX_LENGTH) || (HI_NULL == pData))
    {
    ...
    return DRV_GPIOI2C_ReadData_SiLabs(I2cNum, devaddress, address, addresslen, HI_TRUE, pData, DataLen);
}
            ...
            if ((TmpGpioClkNo == u32SCLGpioNo) && (TmpGpioSDANo == u32SDAGpioNo))
            {
                ...
                *pu32I2cNum = g_stI2cGpio[i].I2cNum;
                HI_INFO_GPIOI2C("SCLGpioNo.%d and SDAGpioNo.%d have been created for I2c channel[%d] !\n",
                                        u32SCLGpioNo, u32SDAGpioNo, *pu32I2cNum);
                return HI_SUCCESS;
            }
...
{
    if ( HI_STD_I2C_NUM > I2cNum  || I2cNum >= HI_I2C_MAX_NUM)
    {
...
long HI_DRV_GPIOI2C_Ioctl(void *file, unsigned int cmd, unsigned long arg)
{
    long    Ret;
    HI_U8  *pData = HI_NULL;
    I2C_DATA_S I2cData = {0};
    I2C_GPIO_S I2cGpio = {0};
    ...
    switch (cmd)
    {
        case CMD_I2C_WRITE:
            ...
            if (I2cData.DataLen > HI_I2C_MAX_LENGTH  || 0 == I2cData.DataLen || I2cData.I2cRegCount > 4)
            {
                HI_ERR_GPIOI2C("invalid para\n");
            ...
            if(HI_NULL == I2cData.pData)
            {
                HI_ERR_GPIOI2C("I2cData.pData is null pointer\n");
                Ret = HI_ERR_I2C_INVALID_PARA;
                break;
            }
            ...
            memset(pData, 0, I2cData.DataLen);
...
EXPORT_SYMBOL(HI_DRV_GPIOI2C_Read_SiLabs);
