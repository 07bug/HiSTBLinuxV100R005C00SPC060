        ...
        HI_U32 regAddrSize, HI_U8 *pData, HI_U32 dataSize, HI_BOOL bWithStop)
{
    ...
    if ((dataSize > HI_I2C_MAX_LENGTH) || (0 ==  dataSize))
    {
        HI_ERR_I2C("para dataSize is invalid\n");
        return HI_ERR_I2C_INVALID_PARA;
    }
    ...
    memset(msgbuf, 0, regAddrSize + dataSize);
        ...
        HI_U32 regAddrSize, HI_U8 *pData, HI_U32 dataSize, HI_U8 u8Mode)
{
    ...
    if ((dataSize > HI_I2C_MAX_LENGTH) || (0 == dataSize))
    {
        HI_ERR_I2C("dataSize is invalid!\n");
        return HI_ERR_I2C_INVALID_PARA;
    }
...
int I2C_DRV_Read_2Stop(HI_U32 channel, HI_U8 devAddr, HI_BOOL bSendSlave,
                 HI_U32 regAddr, HI_U32 regAddrSize,
                 HI_U8 *pData, HI_U32 dataSize)
{
    HI_S32 ret;
...
HI_S32 I2C_Ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case CMD_I2C_READ:
        ...
        if(HI_NULL == I2cData.pData) {
            HI_ERR_I2C("I2cData.pData is null pointer\n");
            HI_KFREE(HI_ID_I2C, pData);
            return HI_ERR_I2C_INVALID_PARA;
        }
...
static HI_S32 I2C_DRV_DummyAttachAdapter(struct device *dev, void *dummy)
{
    struct i2c_adapter *adap;
        ...
        atomic_add(1, &hi_i2c_dummy.client[adap->nr].adapter->dev.kobj.kref.refcount);
    return 0;
}
...
HI_S32 HI_DRV_I2C_Read_2Stop(HI_U32 channel, HI_U8 devAddr,
        HI_U32 regAddr, HI_U32 regAddrSize, HI_U8 *pData, HI_U32 dataSize)
{
    HI_S32 ret;
    ...
    ret = I2C_DRV_Read_2Stop(channel, devAddr, HI_TRUE, regAddr, regAddrSize,
            pData, dataSize);
    HI_INFO_I2C("ret=0x%x, channel=%d, DevAddr=0x%x, RegAddr=0x%x, "
            "Num=%d, Len=%d\n", ret, channel, devAddr, regAddr,
            regAddrSize, dataSize);
...
/* Added end: sdk 20120131, for avl6211 demod */
