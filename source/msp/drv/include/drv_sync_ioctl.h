...
typedef struct
{
    HI_S32      PpmSum;         /* ppm sum */
    HI_S32      PpmNumber;      /* ppm number */
    HI_S32      PpmValid;       /* PpmValid = PpmSum/PpmNumber */
}SYNC_PPMINFO_S;

typedef struct
{
    HI_U64                     StcLastValue;        /* last stc value*/
    HI_S32                     StcDeltaSum;         /* stc  delta sum value */
    HI_BOOL                    StcAdjustFail;
    HI_BOOL                    StcAdjustInitFlag;
}SYNC_STCINFO_S;
...
typedef struct hiSYNC_PCR_INFO_S
{
    ...
    SYNC_PPMINFO_S             PpmInfo;             /* PPM calculate */
    SYNC_STCINFO_S             StcInfo;

    HI_S32                     PcrDeltaSum;
    HI_S32                     PcrDeltaNumber;
    HI_U64                     PcrLastValue;        /* last  pcr value*/
    HI_BOOL                    StcAdjustReferFlag;
}SYNC_PCRINFO_S;
...
typedef struct hi_SYNC_INFO_S
{
    HI_U32              u32TargetPts;         /* amendatory pts */
    HI_BOOL             bPreSyncTargetInit;
    HI_BOOL             bPreSyncFinish;
    HI_BOOL             bSyncFinish;
    SYNC_CHAN_E         enPreSyncTarget;
}SYNC_STAT_INFO_S;
...
typedef struct hiSYNC_S
{
    ...
    HI_BOOL                    bUseStopRegion;       /*use stop region or not*/
    HI_BOOL                    bFirstSyncStateReport;

    HI_BOOL                    bPcrStcChanged;       /*pcr and stc changed or not*/
    HI_BOOL                    bExternalPllAdjust;   /*use external pll adjusting method or not*/
    HI_S32                     ExternalPPM;          /*External PPM config by user*/
    HI_U64                     CrtPcrValue;          /*current pcr value*/
    HI_U64                     CrtStcValue;          /*current stc value*/

    HI_U32                     AudFundBeginTime;
    HI_BOOL                    bMs12Flag;
    HI_BOOL                    bPcrBackFlag;
    HI_BOOL                    bVidBackFlag;
    HI_BOOL                    bAudBackFlag;

    HI_BOOL                    SyncFinish;        /* finish flag of synchronization */

    HI_U32                     SyncFinishContinueCnt;
    HI_U32                     SyncNovelContinueCnt;
#ifndef __KERNEL__
...
typedef struct hiSYNC_PCR_STC_INFO_S
{
    HI_U64       PcrValue;
    HI_U64       StcValue;
}SYNC_PCR_STC_INFO_S;
