...
#define ADEC_CODEC_NAME_LENGTH            32
...
#define CHECK_ADEC_SPEED(SPEEDINT) \
    do                                               \
    {                                                \
        if ((ADEC_SPEED_INT_SLOW != SPEEDINT)   &&   \
            (ADEC_SPEED_INT_NORMAL != SPEEDINT) &&   \
            (ADEC_SPEED_INT_FAST != SPEEDINT))       \
        {                                            \
...
typedef struct
{
    ...
    HI_CHAR          szCodecType[ADEC_CODEC_NAME_LENGTH];
    HI_CHAR          szCodecDescription[ADEC_CODEC_NAME_LENGTH];
    HI_U32           u32CodecID;
    HI_BOOL          enFmt;
    ...
    HI_U32           u32SaveEsCnt;
    HI_CHAR          esFilePath[ADEC_PATH_MAX];
	HI_CHAR          pcmFilePath[ADEC_PATH_MAX];
