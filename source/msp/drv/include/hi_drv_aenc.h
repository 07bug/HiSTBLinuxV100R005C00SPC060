...
typedef struct hiAENC_PROC_ITEM_S
{
    ...
    HI_U32                  u32SaveEsCnt;
    HI_CHAR                 filePath[AENC_PATH_MAX];
} AENC_PROC_ITEM_S;

/* use macro to check parameter */
#define  HI_MPI_AENC_RetUserErr(DrvErrCode, hAenc) \
    do                                                  \
    {                                                   \
        HI_S32 retvalerr;                               \
...
#define CHECK_AENC_HANDLE(hAenc) \
    do                                                  \
    {                                                   \
        if ((HI_S32)(hAenc) >= AENC_INSTANCE_MAXNUM)    \
        {                                               \
...
#define CHECK_AENC_CHN_OPEN_RET(hAenc) \
    do { \
        ...
        if (HI_NULL == g_pstAencChan[hAenc]) \
        { \
            AENC_CHAN_UNLOCK(hAenc); \
            return HI_FAILURE; \
        } \
