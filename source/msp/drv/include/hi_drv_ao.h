...
#define CHECK_AO_CAST_HANDLE(hCast) \
    do \
    { \
        if ((hCast & 0xffff0000) != (HI_ID_AO << 16)) \
        { \
            HI_ERR_AO("hCast(0x%x) is not ao handle!\n", hCast); \
            return HI_ERR_AO_INVALID_PARA; \
        } \
        if ((hCast & 0xff00) != (HI_ID_CAST << 8)) \
        { \
            HI_ERR_AO("hCast(0x%x) is not cast handle!\n", hCast); \
            return HI_ERR_AO_INVALID_PARA; \
        } \
        if (AO_MAX_CAST_NUM <= (hCast & AO_CAST_CHNID_MASK)) \
        { \
...
typedef struct
{
    ...
    HI_CHAR        aCommPath[AO_COMM_PATH_MAX_LENGTH];
    HI_BOOL        bContinueStatus;
    HI_UNF_SND_OUTPUTLATENCY_E enOutputMode;
    struct file*   pstConOutputFp;
} AO_RENDER_ATTR_S;
