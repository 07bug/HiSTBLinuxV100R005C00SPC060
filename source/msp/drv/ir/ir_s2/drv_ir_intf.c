...
static void ir_setwork(unsigned long arg, struct ir_priv *ir)
{
    int ret;
...
static int ir_request_resource(struct ir_priv *ir)
{
    ...
    ret = -EFAULT;
    if (osal_request_irq(ir->irq, ir->callbacks.isr_handle, IRQF_SHARED, "ir_s2", (void *)ir))
    {
        goto irq;
    }
    else
    {
        if(HI_SUCCESS != HI_DRV_SYS_SetIrqAffinity(HI_ID_IR, ir->irq, "ir_s2"))
        {
...
static void ir_free_resource(struct ir_priv *ir)
{
    ...
    osal_free_irq(ir->irq, "ir_s2", (void *)ir);
    HI_KFREE(HI_ID_IR, ir->symbol_buf);
    HI_KFREE(HI_ID_IR, ir->key_buf);
}
