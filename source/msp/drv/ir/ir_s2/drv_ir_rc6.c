...
static void rc6_keyup_proc(unsigned long i)
{
    ...
    if (rc6_data[i].has_last_key && last_key->key_stat != KEY_STAT_UP) {
...
void rc6_exit(void)
{
    int i;
    for (i = 0; i < MAX_RC6_INFR_NR; i++)
        del_timer_sync(&rc6_timer[i]);
}

int rc6_process_states(struct rc6_ir *data, struct ir_signal signal)
{
...
void rc6_process_out(struct ir_priv *ir, struct ir_protocol *ip,
        struct rc6_ir *rc6, struct ir_buffer *wr, int *pcnt)
{
    rc6->has_last_key = 1;
        ...
        struct ir_buffer *rd, struct ir_buffer *wr)
{
    ...
    del_timer_sync(&rc6_timer[ip->priv]);
    /* if a repeat key. */
	flag2 = rc6->has_last_key && (rc6->this_key.lower == rc6->last_key.lower
