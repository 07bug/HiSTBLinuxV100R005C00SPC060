...
static HI_VOID IR_SetClk(bool bFlag)
{
    HI_U32 * pIRRest = HI_NULL_PTR;
    HI_U32 u32RegVal = 0;

    pIRRest = ioremap_nocache(IR_CLK_REG, 0x4);
...
HI_S32 IR_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    memset(IrCodeString, 0, sizeof(IrCodeString));
...
HI_VOID IR_Config(HI_VOID)
{
    ...
    WRITE_REG(g_IrRegAddr + IR_INTM, 0x70000);
    WRITE_REG(g_IrRegAddr + IR_START, 0x00);

    return;
}

HI_VOID Ir_ClearInt(HI_VOID)
{

    WRITE_REG(g_IrRegAddr + IR_INTC, IR_INTC_SYMB_RCV);
    WRITE_REG(g_IrRegAddr + IR_INTC, IR_INTC_TIME_OUT);
    WRITE_REG(g_IrRegAddr + IR_INTC, IR_INTC_OVERRUN);
...
{
    WRITE_REG(g_IrRegAddr + IR_INTM, 0x7000f);
...
HI_S32 IR_Open(struct inode *inode, struct file *filp)
{
    ...
    if (1 == atomic_inc_return(&g_IrCount))
    {
        ...
        Ret = osal_request_irq(IR_IRQ_NO, (irq_handler_t)Ir_Isr, IRQF_SHARED, "ir_std", (HI_VOID*)&hiirIRQ);
        if (Ret != HI_SUCCESS)
        {
        ...
        {
            if(HI_SUCCESS != HI_DRV_SYS_SetIrqAffinity(HI_ID_IR, IR_IRQ_NO, "ir_std"))
            {
...
HI_S32 IR_Close(struct inode *inode, struct file *filp)
{
    ...
    {
        osal_free_irq(IR_IRQ_NO, "ir_std", (HI_VOID*)&hiirIRQ);
