...
static HI_BOOL g_bSlectType = HI_FALSE;
 ...
 *-----------------------------------------------------------------*/
HI_S32 KEYLEDOpen(KeyLed_Status_E enKeyLedStatus, atomic_t* count)
{
    HI_S32 ret = 0;

    if (1 == atomic_inc_return(count))
    {
        if (keyledflag & enKeyLedStatus)
        {
            ret = HI_SUCCESS;
        }
        else
        {
            ret = (enKeyLedStatus & KeyON) ?
                        g_stKeyLedOpt.KEYLED_KEY_Open():
                        g_stKeyLedOpt.KEYLED_LED_Open();
            if (ret == HI_SUCCESS)
            {
                keyledflag |= enKeyLedStatus;
            }
...
HI_S32 KEYLEDClose(KeyLed_Status_E enKeyLedStatus, atomic_t* count)
{
    HI_S32 ret = 0;
...
#ifdef  KEYLED_STANDARD
HI_S32 KEYLEDSetMode(HI_KEYLED_MODE_S *pKeyLedMode)
{
    HI_S32 ret = 0;
    ...
    if (HI_NULL != g_stKeyLedOpt.KEYLED_SetMode)
    {
        if (keyledflag & KeyON || keyledflag & LedON)
        {
            ...
            {
                ret = g_stKeyLedOpt.KEYLED_SetMode(*pKeyLedMode);
            }
            else
            {
                ret = HI_SUCCESS;
...
HI_S32 KEYLEDDisplayMulitled(HI_KEYLED_DISPLAY_S *pKeyLedDisplay)
{
    HI_S32 ret = 0;
    ...
    if (HI_NULL != g_stKeyLedOpt.KEYLED_LED_DisplayLED)
    {
        ...
        if ((HI_CHIP_TYPE_HI3716M == enChipType) && (HI_CHIP_VERSION_V300 == enChipVersion))
        {
            ret = g_stKeyLedOpt.KEYLED_LED_DisplayLED(*pKeyLedDisplay);
        }
        else
        {
            ret = HI_SUCCESS;
...
HI_S32 KEYLEDSetLock(HI_U32 *uLock)
{
    HI_S32 ret = 0;

    CHECK_LED_OPEN();
    if (g_stKeyLedOpt.KEYLED_LED_SetLockIndicator)
        ret = g_stKeyLedOpt.KEYLED_LED_SetLockIndicator((HI_BOOL)*uLock);
    else
    {
...
HI_S32 KEYLEDIoctl(struct inode *inode, struct file * file, HI_U32 cmd, HI_VOID * arg)
{
    ...
    KeyLed_Status_E  enKeyLedStatus = 0;
    HI_UNF_KEYLED_TIME_S display_time;
    HI_U32 RepTimeMs = 0;
    memset(&display_time , 0 ,sizeof(HI_UNF_KEYLED_TIME_S));
    ...
    if((cmd != HI_KEYLED_SELECT_CMD)&&(g_bSlectType ==  HI_FALSE))
    {
    ...
    case HI_KEYLED_SELECT_CMD:
        ...
        if(g_stKeyLedProcInfo.enKeyLedType > HI_UNF_KEYLED_TYPE_BUTT)
            return HI_ERR_KEYLED_INVALID_PARA;

        ret = KEYLEDSelectType(*(HI_UNF_KEYLED_TYPE_E*)arg);
        break;

    case HI_KEYLED_KEY_OPEN_CMD:

        enKeyLedStatus = KeyON;
        ret =  KEYLEDOpen(enKeyLedStatus, &g_KeyOpenCount);
        break;

    case HI_KEYLED_KEY_CLOSE_CMD:

        enKeyLedStatus = KeyON;
        ret =  KEYLEDClose(enKeyLedStatus, &g_KeyOpenCount);
        break;

    case HI_KEYLED_KEY_RESET_CMD:

        CHECK_KEY_OPEN();
        ret = g_stKeyLedOpt.KEYLED_KEY_Reset();
        break;
    ...
    case HI_KEYLED_KEY_GET_VALUE_CMD:
        ...
        ret = g_stKeyLedOpt.KEYLED_KEY_GetValue(&pstPara->u32KeyStatus, &pstPara->u32KeyCode);
        break;
    ...
    case HI_KEYLED_SET_BLOCK_TIME_CMD:
        ...
        ret = g_stKeyLedOpt.KEYLED_KEY_SetBlockTime(*(HI_U32*)arg);
        break;
    ...
    case HI_KEYLED_SET_IS_KEYUP_CMD:
        ...
        ret = g_stKeyLedOpt.KEYLED_KEY_IsKeyUp(*(HI_U32*)arg);
        break;
        ...
        ret = g_stKeyLedOpt.KEYLED_KEY_IsRepKey(*(HI_U32*)arg);
        break;
    ...
    case HI_KEYLED_SET_REPKEY_TIME_CMD:
        ...
        RepTimeMs = *(HI_U32*)arg;
        if(RepTimeMs < 108)
        {
            RepTimeMs = 108;
        }
        else if (RepTimeMs > 65536)
        {
            RepTimeMs = 65536;
        }

        ret = g_stKeyLedOpt.KEYLED_KEY_SetRepTime(RepTimeMs);
        break;

    case HI_KEYLED_LED_OPEN_CMD:

        enKeyLedStatus = LedON;
        ret =  KEYLEDOpen(enKeyLedStatus, &g_LedOpenCount);
        break;

    case HI_KEYLED_LED_CLOSE_CMD:

        enKeyLedStatus = LedON;
        ret =  KEYLEDClose(enKeyLedStatus, &g_LedOpenCount);
        break;
    ...
    case HI_KEYLED_DISPLAY_CODE_CMD:
        ...
        ret = g_stKeyLedOpt.KEYLED_LED_Display(*(HI_U32*)arg);
        break;
    ...
    case HI_KEYLED_DISPLAY_TIME_CMD:
        ...
        display_time = *(HI_UNF_KEYLED_TIME_S*)arg;
        if((display_time.u32Hour > 23) || (display_time.u32Minute >59))
            return HI_ERR_KEYLED_INVALID_PARA;

        g_stKeyLedProcInfo.stLedTime = *(HI_UNF_KEYLED_TIME_S*)arg;
        ret = g_stKeyLedOpt.KEYLED_LED_DisplayTime(*(HI_UNF_KEYLED_TIME_S*)arg);
        break;
    ...
    case HI_KEYLED_SET_FLASH_PIN_CMD:
        ...
        if((*(HI_UNF_KEYLED_LIGHT_E*)arg > 6) || (*(HI_UNF_KEYLED_LIGHT_E*)arg < 1))
            return HI_ERR_KEYLED_INVALID_PARA;

        ret = g_stKeyLedOpt.KEYLED_LED_SetFlashPin(*(HI_UNF_KEYLED_LIGHT_E*)arg);
        break;
    ...
    case HI_KEYLED_CONFIG_FLASH_FREQ_CMD:
        ...
        if((*(HI_UNF_KEYLED_LEVEL_E*)arg < 1) || (*(HI_UNF_KEYLED_LEVEL_E*)arg > 5))
            return  HI_ERR_KEYLED_INVALID_PARA;

        ret = g_stKeyLedOpt.KEYLED_LED_SetFlashFreq(*(HI_UNF_KEYLED_LEVEL_E*)arg);
        break;
    ...
    case HI_KEYLED_DISPLAY_MULITLED_CMD:
        ...
        ret = KEYLEDDisplayMulitled((HI_KEYLED_DISPLAY_S *)arg);
        break;
    ...
    case HI_KEYLED_SET_MODE_CMD:
        ...
        ret = KEYLEDSetMode((HI_KEYLED_MODE_S *)arg);
        break;
#endif
...
static long KEYLED_Ioctl(struct file * file, HI_U32 cmd, unsigned long arg)
{
    ...
    ret = HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, KEYLEDIoctl);
