...
#define  GPIO_CLOCK_SET(val)\
    do {\
            if (s_pGpioFunc && s_pGpioFunc->pfnGpioWriteBit)\
                s_pGpioFunc->pfnGpioWriteBit(CLK_CT1642, val);\
    } while(0)

#define  GPIO_DATA_SET(val)\
    do {\
            if (s_pGpioFunc && s_pGpioFunc->pfnGpioWriteBit)\
                s_pGpioFunc->pfnGpioWriteBit(DAT_CT1642, val);\
    } while(0)

#define  GPIO_KEY_GET(val)\
    do {\
           if (s_pGpioFunc && s_pGpioFunc->pfnGpioReadBit)\
               s_pGpioFunc->pfnGpioReadBit(KEY_CT1642, &val);\
    } while(0)\
...
HI_S32 keyled_ct1642_gpio_dirset(HI_VOID)
{
    ...
    HI_ERR_KEYLED("s_pGpioFunc or s_pGpioFunc->pfnGpioDirSetBit is NULL! \n");
    return  HI_FAILURE;
}
