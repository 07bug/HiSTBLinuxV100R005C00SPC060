...
static HI_VOID keyled_disable_key(HI_VOID)
{
    ...
    /* add by sdk for DTS2012102304374 */
    msleep(1);
...
static HI_S32 keyled_set_flashpin(HI_UNF_KEYLED_LIGHT_E pin)
{
    ...
    /* add by sdk for DTS2012102401972 */
...
HI_S32 KEYLED_Init_CT1642(HI_VOID)
{
    ...
    err = osal_request_irq(KEYLED_IRQ, keyled_interrupt_handler, IRQF_SHARED, "keyled_ct1642", (HI_VOID*)&hkeyledIRQ);
    if (0 != err)
    {
    ...
    {
        if(HI_SUCCESS != HI_DRV_SYS_SetIrqAffinity(HI_ID_KEYLED, KEYLED_IRQ, "keyled_ct1642"))
        {
...
HI_S32 KEYLED_DeInit_CT1642(HI_VOID)
{
    ...
    /* release irq */
    osal_free_irq(KEYLED_IRQ, "keyled_ct1642", (HI_VOID*)&hkeyledIRQ);
