...
#define  GPIO_CLOCK_SET(val)\
     do {\
            if (s_pGpioFunc && s_pGpioFunc->pfnGpioWriteBit)\
                s_pGpioFunc->pfnGpioWriteBit(CLK_PT6961, val);\
     } while(0)

#define  GPIO_STB_SET(val)\
     do {\
            if (s_pGpioFunc && s_pGpioFunc->pfnGpioWriteBit)\
                s_pGpioFunc->pfnGpioWriteBit(STB_PT6961, val);\
     } while(0)

#define  GPIO_DIN_SET(val)\
     do {\
            if (s_pGpioFunc && s_pGpioFunc->pfnGpioWriteBit)\
                s_pGpioFunc->pfnGpioWriteBit(DIN_PT6961, val);\
     } while(0)

#define  GPIO_DOUT_GET(val)\
     do {\
            if (s_pGpioFunc && s_pGpioFunc->pfnGpioReadBit)\
                s_pGpioFunc->pfnGpioReadBit(DOUT_PT6961, &val);\
     } while (0)
...
HI_S32 keyled_pt6961_gpio_dirset(HI_VOID)
{
    ...
    {
        HI_ERR_KEYLED("s_pGpioFunc or s_pGpioFunc->pfnGpioDirSetBit is null pointer\n");
        return HI_FAILURE;
    }
