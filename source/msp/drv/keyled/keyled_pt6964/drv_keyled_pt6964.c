...
#define  GPIO_CLOCK_SET(val)\
    do {\
          if (s_pGpioFunc && s_pGpioFunc->pfnGpioWriteBit)\
              s_pGpioFunc->pfnGpioWriteBit(CLK_PT6964, val);\
    } while(0)

#define  GPIO_STB_SET(val)\
    do {\
          if (s_pGpioFunc && s_pGpioFunc->pfnGpioWriteBit)\
              s_pGpioFunc->pfnGpioWriteBit(STB_PT6964, val);\
    } while(0)
...
HI_S32 keyled_pt6964_rx_byte(HI_U8 *u8data)
{
    ...
    if((!s_pGpioFunc) || (!s_pGpioFunc->pfnGpioDirSetBit) || (!s_pGpioFunc->pfnGpioReadBit))
    {
        HI_ERR_KEYLED("null pointer!");
        return HI_FAILURE;
    }
