...
HI_S32 KEYLED_Init(HI_VOID)
{
    ...
    ret = osal_request_irq(KEYLED_IRQ, keyled_interrupt_handler, IRQF_SHARED, "keyled_std", (HI_VOID*)&hkeyledIRQ);
    if (0 != ret)
    {
    ...
    {
        if(HI_SUCCESS != HI_DRV_SYS_SetIrqAffinity(HI_ID_KEYLED, KEYLED_IRQ, "keyled_std"))
        {
...
HI_S32 KEYLED_DeInit(HI_VOID)
{
    ...
    /* release irq */
    osal_free_irq(KEYLED_IRQ, "keyled_std", (HI_VOID*)&hkeyledIRQ);
...
HI_S32 keyled_display_mulitled(HI_U8 LedNum, HI_U8 *u8Data)
{
    ...
    if (LedNum > 5)
    {
        HI_ERR_KEYLED("LedNum is too large!\n");
    ...
    if (5 == LedNum)
    {
        ...
        memcpy(&SuspendSaveVal.LedData, u8Data, sizeof(SuspendSaveVal.LedData));
        SuspendSaveVal.LedDisNum = sizeof(SuspendSaveVal.LedData);
    }
    else
    {
