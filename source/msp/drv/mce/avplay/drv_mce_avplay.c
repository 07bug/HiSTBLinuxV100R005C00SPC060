...
*/
    ...
    Ret = HI_DRV_ADEC_Reset(pAvplay->hAdec);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_MCE("call HI_DRV_ADEC_Reset failed.\n");
        return Ret;
    }
    return HI_SUCCESS;
}
...
HI_VOID DRV_AVPLAY_ProcVdecToVo(MCE_AVPLAY_S *pAvplay)
{
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_VDEC_VO])
    {
        ...
        sysTime = MCE_GetSysTime();
        HI_INFO_MCE("=====Receive a new frame, sys=%u, id=%u, pts=%u=====\n",
            sysTime, pAvplay->CurFrmPack.stFrame[0].stFrameVideo.u32FrameIndex,
    ...
    HI_INFO_MCE("sys:%u, frm:%d, need:%u, cur:%u, sync:%u, delay:%u\n",
        sysTime,pAvplay->CurFrmPack.stFrame[0].stFrameVideo.u32FrameIndex, pAvplay->FrcNeedPlayCnt,
...
HI_S32 HI_DRV_AVPLAY_Create(const HI_UNF_AVPLAY_ATTR_S *pstAvAttr, HI_HANDLE *phAvplay)
{
    ...
    if (HI_NULL == pstAvAttr || HI_NULL == phAvplay)
    {
    ...
    g_stAvplay[i].pAvplayThreadInst = kthread_create(DRV_AVPLAY_ProcThread, &(g_stAvplay[i]), "fastplay");
    if (IS_ERR(g_stAvplay[i].pAvplayThreadInst))
    {
        HI_ERR_MCE("create fastplay thread error\n");
...
HI_S32 HI_DRV_AVPLAY_GetSyncVdecHandle(HI_HANDLE hAvplay, HI_HANDLE *phVdec, HI_HANDLE *phSync)
{
    ...
    if (HI_NULL == phSync || HI_NULL == phVdec)
    {
        HI_ERR_MCE("invalid parameter \n");
        return HI_ERR_AVPLAY_NULL_PTR;
    }
...
HI_S32 HI_DRV_AVPLAY_CheckIfSupportRandomStream(HI_HANDLE hAvplay, HI_BOOL *pbSupport)
{
    HI_S32                          s32Ret;
    MCE_AVPLAY_S                    *pAvplay= HI_NULL;
    ...
    s32Ret = HI_DRV_VDEC_CheckIfSupportRandomStream(pAvplay->VdecAttr.enType, &pAvplay->VdecAttr.unExtAttr, &bRandStreamSupport);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_MCE("Call HI_DRV_VDEC_CheckIfSupportRandomStream failed!s32Ret 0x%x\n",s32Ret);
        DRV_AVPLAY_UNLOCK(&(pAvplay->AvplayMutex));
        return s32Ret;
    }
