...
HI_S32 MCE_Suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    ...
    pMce->TsThreadStatus = HI_MCE_THREAD_SLEEP;

    if (HI_INVALID_HANDLE != pMce->hAvplay)
    {
        (HI_VOID)HI_DRV_AVPLAY_Stop(pMce->hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_VID|HI_UNF_AVPLAY_MEDIA_CHAN_AUD, HI_NULL);
    }
#endif

    HI_PRINT("MCE suspend OK\n");
...
HI_S32 MCE_Resume(PM_BASEDEV_S *pdev)
{
    ...
    if (HI_INVALID_HANDLE != pMce->hAvplay)
    {
        (HI_VOID)HI_DRV_AVPLAY_Start(pMce->hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_AUD|HI_UNF_AVPLAY_MEDIA_CHAN_VID);
    }
#endif

    HI_PRINT("MCE resume OK\n");
