...
static HI_S32 channel_insert_in_raw_list(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_S *pstream)
{
    HI_S32 is_find = 0;
    unsigned long  flags;
    OMXVDEC_BUF_S *pbuf = HI_NULL;
...
static HI_S32 channel_insert_in_yuv_list(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_S *pframe)
{
    HI_S32 is_find = 0;
    unsigned long  flags;
    OMXVDEC_BUF_S *pbuf = HI_NULL;
...
HI_S32 channel_create_inst(struct file *fd, OMXVDEC_DRV_CFG *pcfg)
{
    ...
    if (fd == NULL || pcfg == NULL)
    {
        OmxPrint(OMX_FATAL, "%s error!\n", __func__);
...
HI_S32 channel_release_inst(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    for (i = 0; i < pchan->max_in_buf_num && i<DEF_MAX_OUT_BUF_CNT; i++)
    {
