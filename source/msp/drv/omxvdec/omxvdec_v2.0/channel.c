...
static HI_U32 channel_insert_addr_table(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    pbuf->meta_type   = puser_buf->meta_type;
...
static HI_U32 channel_delete_addr_table(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    {
        /* unmap kernel virtual address */
        if ((puser_buf->meta_type == OMX_ALLOCATE_WITH_META) && (puser_buf->dir == PORT_DIR_OUTPUT))//gstpath :pchan->bIsGst
        {
...
HI_VOID channel_init_inst_config(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_DRV_CFG *pcfg)
{
    ...
    pchan->bMetaDataAlloc = pcfg->bMetaDataAlloc;
    OmxPrint(OMX_INFO, "%s chan bMeta %d\n", __func__,pchan->bMetaDataAlloc);
...
static HI_S32 channel_alloc_inst_memory(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_DRV_CFG *pcfg)
{
    ...
    if (pcfg->cfg_inbuf_num > DEF_MAX_IN_BUF_CNT_MAX || pcfg->cfg_outbuf_num > DEF_MAX_OUT_BUF_CNT)
    {
        OmxPrint(OMX_FATAL, "%s cfg num is too big! input:%d output:%d\n", __func__, pcfg->cfg_inbuf_num, pcfg->cfg_outbuf_num);
        return HI_FAILURE;
    }
    ...
    #ifdef HI_OMX_TEE_SUPPORT
        ret = decoder_allocbuf_by_vfmw(pchan, "SEC_OMXVDEC_LAST_FRAME", HI_NULL, LAST_FRAME_BUF_SIZE, 4, &pchan->decoder_eos_buf);
    #else
...
static HI_VOID channel_free_inst_memory(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    {
    #ifdef HI_OMX_TEE_SUPPORT
        decoder_releasebuf_by_vfmw(&pchan->decoder_eos_buf);
    #else
...
HI_S32 channel_release_inst(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    spin_lock_irqsave(&omxvdec->stream_lock, flags);
    spin_lock_irqsave(&pchan->chan_lock, flags);
    state = pchan->state;
    spin_unlock_irqrestore(&pchan->chan_lock, flags);
    spin_unlock_irqrestore(&omxvdec->stream_lock, flags);
    ...
    if (pchan->TunnelModeEnable != 1)
    {
        ...
        for (i=0; i<pchan->max_out_buf_num && i<DEF_MAX_OUT_BUF_CNT; i++)
        {
           ...
           if (pbuf->status != BUF_STATE_INVALID)
           {
               ...
               HI_DRV_OMXVDEC_UnmapAndRelease(&stBuffer);
               /*gstpath omx allocat meta buffer,need to release*/
               if (pbuf->meta_type == OMX_ALLOCATE_WITH_META)
               {
                   stBuffer.u32StartPhyAddr = pbuf->private_phy_addr;
                   stBuffer.pu8StartVirAddr = (HI_U8*)pbuf->private_vaddr;
                   HI_DRV_OMXVDEC_UnmapAndRelease(&stBuffer);
               }
           }
        }
    ...
    for (i=0; i<pchan->max_in_buf_num && i<DEF_MAX_IN_BUF_CNT; i++)
    {
       ...
       /*buf_type is OMX_USER_BIGBUF, in buf is allocated by gst, no need to release*/
       if (pbuf->buf_type == OMX_USER_BIGBUF)
       {
           break;
       }
       if (pbuf->status != BUF_STATE_INVALID)
...
HI_S32 channel_alloc_buf(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    if ((puser_buf->is_sec) && (puser_buf->dir == 0))
    {
    #ifdef HI_OMX_TEE_SUPPORT
        ret = decoder_allocbuf_by_vfmw(pchan, bufname, HI_NULL, buf_size, puser_buf->align, &TmpBuf);
    #else
    ...
    if(puser_buf->meta_type == OMX_ALLOCATE_WITH_META && puser_buf->dir == 1 && puser_buf->private_len != 0)
    {
        ...
        memcpy(bufname, "OMXVDEC_METADATA", sizeof("OMXVDEC_METADATA"));
        buf_size = (puser_buf->private_len + align - 1) & ~(align - 1);
        ret = HI_DRV_OMXVDEC_Alloc(bufname, HI_NULL, buf_size, puser_buf->align, &MetaBuf, 0);
        if (ret != HI_SUCCESS)
        {
            HI_DRV_OMXVDEC_Release(&TmpBuf,puser_buf->is_sec);
            OmxPrint(OMX_FATAL, "alloc  failed\n");
            return HI_FAILURE;
        }
        puser_buf->private_phyaddr = MetaBuf.u32StartPhyAddr;
        puser_buf->private_len = MetaBuf.u32Size;
    }
...
{
    MMZ_BUFFER_S   TmpBuf = {0};
    MMZ_BUFFER_S   MetaBuf = {0};
    ...
    {
    #ifdef HI_OMX_TEE_SUPPORT
        decoder_releasebuf_by_vfmw(&TmpBuf);
    #else
        HI_DRV_OMXVDEC_Release(&TmpBuf, puser_buf->is_sec);
    #endif
    }
    ...
    {
        if(puser_buf->private_phyaddr != NULL)
        {
            MetaBuf.u32StartPhyAddr = (HI_U32)puser_buf->private_phyaddr;
            MetaBuf.u32Size         = puser_buf->private_len;
            HI_DRV_OMXVDEC_Release(&MetaBuf, 0);
        }
    }
...
HI_S32 channel_global_release_frame(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    if (0 != pframe->private_vaddr)
    {
        ...
        OmxPrint(OMX_INFO, "%s puser_buf:%p pframe:%p pstFramePriv:%p phyaddr_y:0x%x\n", __func__, \
                            puser_buf, pframe, pstFramePriv, pstFramePriv->stBufAddr[0].u32PhyAddr_YHead);
