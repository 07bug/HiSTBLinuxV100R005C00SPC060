...
static HI_BOOL decoder_check_codec_support(HI_U32 enType)
{
    ...
    {
        OmxPrint(OMX_FATAL, "SYS_GetVideoCodecSupport fail\n");
        return HI_FALSE;
    }
...
HI_S32 decoder_record_occoupied_frame(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    ret = (pVfmwFunc->pfnVfmwControl)( pchan->decoder_id, VDEC_CID_SET_IMG_OUTPUT_INFO, &s32ImgOutputEn, sizeof(s32ImgOutputEn));
    if (VDEC_OK != ret)
    {
        OmxPrint(OMX_ERR, "Chan %d VDEC_CID_SET_IMG_OUTPUT_INFO (enable = %d) err!\n",  pchan->decoder_id,s32ImgOutputEn);
    }

    ret = (pVfmwFunc->pfnVfmwControl)( pchan->decoder_id, VDEC_CID_REPORT_OCCUQIED_FRM, &stReportFrmRec, sizeof(stReportFrmRec));
    if (VDEC_OK != ret)
    {
    ...
    else
    {
        ...
        for (i = 0; i < stReportFrmRec.specialFrameNum ; i++)
        {
            ...
            if (HI_SUCCESS == decoder_find_special2Normal_Index(pchan, stSpecialFrmInfo.frmBufRec.u32StartPhyAddr, &index))
            {
                ...
                pchan->dfs.single_buf[index].own_by_decoder = HI_FALSE;
            }
            else
            {
...
static HI_S32 decoder_event_arrange_frame_buffer(OMXVDEC_CHAN_CTX  *pchan, HI_VOID *pargs)
{
    ...
    else if (pchan->bVpssBypass)
    {
        pchan->dfs.need_frm_num += MAX(g_TestDispNum, 6);
    }
    else
    {
        pchan->dfs.need_frm_num += g_TestDispNum;
    }
    ...
    ret = (pVfmwFunc->pfnVfmwControl)(pchan->decoder_id, VDEC_CID_SET_FRAME_BUFFER_NUM, (HI_VOID *)&need_frm_num, sizeof(need_frm_num));
    if(ret != VDEC_OK)
    {
...
static HI_S32 decoder_event_syntax_error(OMXVDEC_CHAN_CTX *pchan, HI_VOID *pargs)
{
    OmxPrint(OMX_INFO, "%s: decoder report syntax error!\n", __func__);
    message_queue(pchan->msg_queue, VDEC_ERR_SYNTAX_ERR, VDEC_S_SUCCESS, HI_NULL);

    return HI_SUCCESS;
}

static HI_S32 decoder_event_error_ratio_notzero(OMXVDEC_CHAN_CTX *pchan, HI_VOID *pargs)
{
    OmxPrint(OMX_INFO, "%s: decoder report error ratio :%d\n", __func__, *(HI_U32*)pargs);
    message_queue(pchan->msg_queue, VDEC_ERR_RATIO_NOTZERO, VDEC_S_SUCCESS, pargs);
...
static HI_S32 decoder_event_handler(HI_S32 chan_id, HI_S32 event_type, HI_VOID *pargs)
{
    ...
    switch (event_type)
    {
        ...
        case EVNT_DECSYNTAX_ERR:
            ret = decoder_event_syntax_error(pchan, pargs);
            break;

        case EVNT_RATIO_NOTZERO:
            ret = decoder_event_error_ratio_notzero(pchan, pargs);
            break;
...
static HI_S32 decoder_get_stream(HI_S32 chan_id, STREAM_DATA_S *stream_data)
{
    ...
    spin_lock_irqsave(&g_OmxVdec->stream_lock, flags);

    pchan = channel_find_inst_by_channel_id(g_OmxVdec, chan_id);
    if (pchan == HI_NULL)
    {
        spin_unlock_irqrestore(&g_OmxVdec->stream_lock, flags);
    ...
    list_for_each_entry(pbuf, &pchan->raw_queue, list)
    {
        ...
        if (pchan->is_tvp)
        {
            ...
            stream_data->SmmuAddr             = pbuf->phy_addr + pbuf->offset;
        #else
...
static HI_S32 decoder_release_stream(HI_S32 chan_id, STREAM_DATA_S *stream_data)
{
    ...
    {
    #ifdef HI_SMMZ2_SUPPORT
        stream_phy_addr = stream_data->SmmuAddr;
    #else
        stream_phy_addr = stream_data->PhyAddr;
    #endif
    }
    else
    {
    ...
    OmxPrint(OMX_INBUF, "VFMW release stream: PhyAddr = 0x%08x, Len = %d\n",
                        stream_phy_addr, stream_data->Length);
...
HI_S32 decoder_create_inst(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_DRV_CFG *pdrv_cfg)
{
    ...
    {
    #ifdef HI_SMMZ2_SUPPORT
        stOption.s32ScdInputMmuEn = SCD_INPUT_MMZ;
    #else
    ...
    ret = decoder_check_codec_support(pchan->protocol);
    if (ret == HI_FALSE)
    {
        OmxPrint(OMX_FATAL, "%s protocol not support: %d, 0x%x\n", __func__, pchan->protocol, g_EnSysCodec);
        goto error2;
    }
...
HI_S32 decoder_allocbuf_by_vfmw(OMXVDEC_CHAN_CTX *pchan, const char *bufname, char *zone_name, HI_U32 size, int align, MMZ_BUFFER_S *psMBuf)
{
    ...
    stVdecBuf.hHandle = pchan->channel_id;
    strncpy(stVdecBuf.aszName, bufname, 16);
    ret = (pVfmwFunc->pfnVfmwControl)(HI_INVALID_HANDLE, VDEC_CID_ALLOC_SEC_BUF, &stVdecBuf, sizeof(stVdecBuf));

    if (ret != VDEC_OK)
    {
        OmxPrint(OMX_ALWS, "alloc smmz2 es buf error %d\n", ret);
        return HI_FAILURE;
    }

    psMBuf->u32StartPhyAddr = stVdecBuf.MmuAddr;
    psMBuf->u32Size = stVdecBuf.Length;
...
HI_VOID decoder_releasebuf_by_vfmw(MMZ_BUFFER_S *psMBuf)
{
    ...
    stVdecBuf.MmuAddr = psMBuf->u32StartPhyAddr;
    stVdecBuf.Length = psMBuf->u32Size;
#ifdef HI_SMMZ2_SUPPORT
    stVdecBuf.eMode = MEM_SMMZ2;
#else
    stVdecBuf.eMode = MEM_MMU_MMU;
#endif
    stVdecBuf.Tvp = HI_TRUE;
    ret = (pVfmwFunc->pfnVfmwControl)(HI_INVALID_HANDLE, VDEC_CID_FREE_SEC_BUF, &stVdecBuf, sizeof(stVdecBuf));

    if (ret != VDEC_OK)
    {
        OmxPrint(OMX_ALWS,"release smmz2 es buf error %d\n", ret);
        return;
    }
