...
HI_S32  message_queue(msg_queue_s *queue, HI_U32 msgcode, HI_U32 status, HI_VOID *priv)
{
    ...
    if (priv)
    {
        ...
        else if (VDEC_ERR_RATIO_NOTZERO == msgcode)
        {
            memcpy(&msg->msg_info.msgdata, priv, sizeof(OMXVDEC_ERR_SIZE));
        }
        else
        {
...
HI_S32 message_dequeue(msg_queue_s *queue, OMXVDEC_MSG_INFO *pmsg_info)
{
    ...
    ret = wait_event_interruptible_timeout(queue->wait, message_pending(queue),msecs_to_jiffies(10));
