...
static HI_S32 processor_report_frame(OMXVDEC_CHAN_CTX *pchan, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    user_buf.dec_color_aspects.mRange       = pstFrame->stColourInfo.u8FullRangeFlag;
    user_buf.dec_color_aspects.mPrimaries   = pstFrame->stColourInfo.u8ColourPrimaries;
    user_buf.dec_color_aspects.mMatrixCoeffs = pstFrame->stColourInfo.u8MatrixCoeffs;
    user_buf.dec_color_aspects.mTransfer    = pstFrame->unHDRInfo.stHDR10Info.u8TransferCharacteristics;
...
static HI_U32 processor_get_stride(HI_U32 protocol, HI_U32 width, HI_BOOL is_native_output)
{
    ...
    else
    {
        if ((protocol == STD_HEVC) || (protocol == STD_VP9))
        {
            stride = HI_OMX_GET_STRIDE(HI_OMX_ALIGN_10BIT(width));
        }
        else
        {
...
static HI_S32 processor_get_frame_buffer(OMXVDEC_CHAN_CTX *pchan, HI_VOID *pstArgs)
{
    ...
    {
        OmxPrint(OMX_WARN, "%s state err! port_enable_flag = %d\n", __func__,pchan->port_enable_flag);
        return HI_FAILURE;
    }
    ...
    /*bMetaDataAlloc is only set by Gstreamer,when bypass,buf_len is 4KB */
    if (pchan->bMetaDataAlloc && pchan->bVpssBypass)
    {
        ExpectedSize = 4 * 1024;
    }
    else
...
static HI_S32 processor_report_last_frame(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    do
    {
        ...
        if (HI_SUCCESS == ret)
        {
            ...
            /*report eos for gst when move vpss off from omx*/
            pVideoPri->bByPassVpss = pchan->bVpssBypass;
            stLastFrame.u32Width   = pchan->out_width;
            stLastFrame.u32Height  = pchan->out_height;
            stLastFrame.bSecure    = pchan->is_tvp;
            if (stFrameStore.PrivateVirAddr != HI_NULL)
            {
                memcpy(stFrameStore.PrivateVirAddr, &stLastFrame, sizeof(HI_DRV_VIDEO_FRAME_S));
            }
...
HI_S32 processor_create_inst(OMXVDEC_CHAN_CTX *pchan, OMX_PIX_FORMAT_E color_format)
{
    ...
    if (pchan->android_test == HI_TRUE)
    {
        g_DeInterlaceEnable = HI_FALSE;
    }

    stVpssCfg.bProgRevise = g_DeInterlaceEnable;
    /*set bAlwaysFlushSrc true or bProgRevise false when lowdelay*/
    if ( HI_TRUE == pchan->bLowdelay || HI_TRUE == g_FastOutputMode)
    {
        stVpssCfg.bAlwaysFlushSrc = HI_TRUE;
    }
    OmxPrint(OMX_INFO, "%s dei %d,bAlwaysFlushSrc %d\n", __func__, g_DeInterlaceEnable,stVpssCfg.bAlwaysFlushSrc);
...
HI_S32 processor_set_bypass(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    if (ret == HI_SUCCESS)
    {
        stVpssPortCfg.bPassThrough = pchan->output_view; //0: force not bypass; 1: auto; 2: force bypass;
        if (pchan->android_test)
        {
            stVpssPortCfg.bPassThrough = 0;
        }
        ret = (pVpssFunc->pfnVpssSetPortCfg)(pchan->port_id, &stVpssPortCfg);
    }
    else
