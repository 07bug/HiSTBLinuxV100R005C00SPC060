...
static HI_S32 task_alloc_buf_for_slot(OMXVDEC_CHAN_CTX *pchan, DSF_SINGLE_BUF_S *pBufSlot, FRAME_BUF_S *pstFrameBuf, ALLOC_BUF_TYPE_E type)
{
    ...
    if (type == ALLOC_FRM)
    {
        ...
        if(g_MapFrmEnable == HI_TRUE)
	    {
	        ret = HI_DRV_OMXVDEC_AllocAndMap(buf_name, HI_NULL, NeededAllocSize, 0, &TmpMMZBuf);
	    }
	    else
	    {
    ...
    if (type == ALLOC_FRM)
    {
        ...
        {
            pBufSlot->frm_buf_type = (g_MapFrmEnable == HI_TRUE)?ALLOC_BY_MMZ : ALLOC_BY_MMZ_UNMAP;
        }
    }
    else
    {
        ...
        {
            pBufSlot->pmv_buf_type = ALLOC_BY_MMZ;
        }
    }
...
static HI_S32 task_handle_situation_1(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    for (i=0; i<lack_num; i++)
    {
        ...
        pBufSlot->own_by_decoder   = HI_TRUE;

        pchan->dfs.alloc_frm_num++;

        /* Config this buffer to decoder */
        ret = decoder_command_handler(pchan, DEC_CMD_CONFIG_MEM, &stFrameBuf, sizeof(stFrameBuf));
        if (ret != HI_SUCCESS)
        {
