...
extern HI_U32   g_SegSize;
extern HI_BOOL  g_RawMoveEnable;
...
static OMXVDEC_CHAN_CTX *omxvdec_ioctl_get_chan_context(struct file *fd, OMXVDEC_IOCTL_MSG *pMsg)
{
    ...
    {
        OmxPrint(OMX_FATAL, "%s, invalid parameter\n", __func__);
        return HI_NULL;
    }
...
static HI_S32 omxvdec_proc_cmd_set_dispnum(HI_CHAR CmdStr[DBG_CMD_NUM][DBG_CMD_LEN])
{
    ...
    if (Data > 16)
    {
        OmxPrint(OMX_ALWS, "set DispNum: %d out of range\n", Data);
        return HI_FAILURE;
    }
...
static HI_S32 omxvdec_proc_cmd_set_segsize(HI_CHAR CmdStr[DBG_CMD_NUM][DBG_CMD_LEN])
{
    ...
    {
        OmxPrint(OMX_ALWS, "set seg size: %d out of range\n", Data);
        return HI_FAILURE;
    }
...
HI_S32 omxvdec_write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    {
        OmxPrint(OMX_ALWS, "%s, parameter invalid!\n", __func__);
        return HI_FAILURE;
    }
...
static HI_S32 omxvdec_read_proc(struct seq_file *p, HI_VOID *v)
{
    ...
    if (p == HI_NULL)
    {
        OmxPrint(OMX_ALWS, "%s, Invalid parameter\n", __func__);
        return -1;
    }

    if (HI_NULL == g_OmxVdec)
    {
        OmxPrint(OMX_ALWS, "%s, g_OmxVdec = NULL\n", __func__);
        return -1;
    }
