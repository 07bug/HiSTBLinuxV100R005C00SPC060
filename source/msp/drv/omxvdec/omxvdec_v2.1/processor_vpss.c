...
/*================ GLOBAL VALUE ================*/
HI_BOOL  g_FrameRateLimit    = HI_FALSE;
...
static HI_S32 processor_get_frame_buffer(OMXVDEC_CHAN_CTX *pchan, HI_VOID *pstArgs)
{
    ...
    if (OutFrame.PhyAddr != 0)
    {
        ...
        pVpssFrm->u32PrivDataPhyAddr = (HI_U32)OutFrame.PrivatePhyAddr; // if PrivatePhyAddr equals -1, that means not allocate metadata
    ...
    if (ret != HI_SUCCESS)
    {
        if (pchan->last_frame_info[0] == DECODER_REPORT_LAST_FRAME)
        {
                ...
                || (pchan->last_frame_info[1]) == REPORT_LAST_FRAME_FAIL)
            {
                ...
                ret = processor_report_last_frame(pchan); // output fake last frame
...
HI_S32 processor_release_image(OMXVDEC_CHAN_CTX *pchan, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    stImage.image_width          = pstFrame->u32Height;
    stImage.image_height         = pstFrame->u32Width;
    ...
    if (ret != HI_SUCCESS)
    {
        ...
        ret = pchan->image_ops.global_release_image(&stImage);
    }
...
HI_S32 processor_create_inst(OMXVDEC_CHAN_CTX *pchan, OMX_PIX_FORMAT_E color_format)
{
    ...
    g_DeInterlaceEnable = pchan->android_test ? HI_FALSE : HI_TRUE;
    ...
    stVpssPortCfg.s32OutputWidth        = 0; // width and height are zero which means self-adapted
