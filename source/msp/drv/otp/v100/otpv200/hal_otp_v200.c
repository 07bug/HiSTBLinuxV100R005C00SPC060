...
HI_U32 HAL_OTP_V200_ReadFromBuffer(HI_U32 addr)
{
    if (addr + 4 > OTP_SIZE)
    {
        HI_FATAL_OTP("addr 0x%08x large than otp size 0x%08x\n", addr + 4, OTP_SIZE);
        return HI_FAILURE;
    }
...
/******* proc function begin ********/
    ...
    if(DRV_OTP_Is_FakeOTPBuffer_Flag() == HI_SUCCESS)  //Fake test
    {
        HI_U32 u32VALUE = 0;
        //Get data from fake buffer
        u32VALUE = DRV_OTP_FakeOTP_ReadByte(addr + 0x00)
...
/******* proc function begin ********/
    ...
    if(DRV_OTP_Is_FakeOTPBuffer_Flag() == HI_SUCCESS)  //Fake test
    {
        ...
        //Get data from fake buffer
        u8VALUE = tdata & 0xFF;
        (HI_VOID)DRV_OTP_FakeOTP_WriteByte(addr + 0x00, u8VALUE);
        u8VALUE = (tdata >> 8) & 0xFF;
        (HI_VOID)DRV_OTP_FakeOTP_WriteByte(addr + 0x01, u8VALUE);
        u8VALUE = (tdata >> 16) & 0xFF;
        (HI_VOID)DRV_OTP_FakeOTP_WriteByte(addr + 0x02, u8VALUE);
        u8VALUE = (tdata >> 24) & 0xFF;
        (HI_VOID)DRV_OTP_FakeOTP_WriteByte(addr + 0x03, u8VALUE);
...
/******* proc function begin ********/
    ...
    if(DRV_OTP_Is_FakeOTPBuffer_Flag() == HI_SUCCESS)  //Fake test
    {
        HI_U8 u8VALUE = 0;
        //Get data from fake buffer
        u8VALUE = DRV_OTP_FakeOTP_ReadByte(addr + 0x00);
        u8VALUE |= (1 << bit_pos);
        return DRV_OTP_FakeOTP_WriteByte(addr, u8VALUE);
    }
#endif
