...
HI_S32  PDM_TransTvFomat(HI_UNF_ENC_FMT_E enSrcFmt, HI_UNF_ENC_FMT_E * penHdFmt, HI_UNF_ENC_FMT_E * penSdFmt)
{
    switch (enSrcFmt)
    {
...
HI_S32  PDM_TransVgaFomat(HI_UNF_ENC_FMT_E enSrcFmt, HI_UNF_ENC_FMT_E * penHdFmt, HI_UNF_ENC_FMT_E * penSdFmt)
{
    switch (enSrcFmt)
    {
...
HI_VOID PDM_TransFomat(HI_UNF_ENC_FMT_E enSrcFmt, HI_UNF_ENC_FMT_E *penHdFmt, HI_UNF_ENC_FMT_E *penSdFmt)
{
    ...
    Ret = PDM_TransTvFomat(enSrcFmt, penHdFmt, penSdFmt);
    if (Ret == HI_SUCCESS)
    {
        return;
    }

    Ret = PDM_TransVgaFomat(enSrcFmt, penHdFmt, penSdFmt);
    if (Ret == HI_SUCCESS)
    {
        return;
    }
...
HI_VOID PDM_GetDispIntfParam(HI_DISP_PARAM_S *pstDispParam, HI_DB_KEY_S   *pstKey,HI_DB_TABLE_S  *pstTable)
{
    HI_S32                  Ret;
...
HI_VOID PDM_GetDispColorAndBLightParam(HI_DISP_PARAM_S *pstDispParam, HI_DB_KEY_S   *pstKey,HI_DB_TABLE_S  *pstTable)
{
    HI_S32 Ret;
...
HI_VOID PDM_GetDispRectParam(HI_DISP_PARAM_S *pstDispParam, HI_DB_KEY_S   *pstKey,HI_DB_TABLE_S  *pstTable)
{
    HI_S32 Ret ;
...
HI_S32 DRV_PDM_GetDispParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam)
{
    ...
    HI_DB_TABLE_S           stTable;
    HI_DB_KEY_S             stKey;
    HI_U8                   *pBasePhyAddr = HI_NULL;
    HI_U8                   *pBaseVirAddr = HI_NULL;
    HI_U32                  u32BaseLen;
    HI_U32                  Version;
    ...
    PDM_GetDispIntfParam(pstDispParam, &stKey, &stTable);

    PDM_GetDispColorAndBLightParam(pstDispParam, &stKey, &stTable);

    PDM_GetDispRectParam(pstDispParam, &stKey, &stTable);
...
HI_S32 DRV_PDM_GetMceParam(HI_MCE_PARAM_S *pstMceParam)
{
    ...
    HI_DB_TABLE_S   stTable;
    HI_DB_KEY_S     stKey;
    HI_U8           *pMceParaPhyAddr = HI_NULL;
    HI_U8           *pMceParaVirAddr = HI_NULL;
...
HI_S32 DRV_PDM_GetMceData(HI_U32 u32Size, HI_U8 **pAddr)
{
    ...
    HI_U8       *pMceDataPhyAddr = HI_NULL;
    HI_U8       *pMceDataVirAddr = HI_NULL;
        ...
        || (HI_NULL == pMceDataVirAddr))
    {
        ...
        /*The address read is the first address of the Fastplay partition, and the front 8k data is the configuration data area,
        so the starting address of the valid data need to offset 8k data size on the first address*/
        pMceDataVirAddr += MCE_DEF_PLAYPARAM_SIZE;
...
HI_VOID  PDM_GetSoundDac0AndSpidfParam(HI_UNF_PDM_SOUND_PARAM_S *pstSoundParam, HI_DB_TABLE_S   *pstTable,  HI_DB_KEY_S    *pstKey, HI_U32 *pu32Volume)
{
    HI_S32 Ret ;
...
HI_S32 DRV_PDM_GetSoundParam(HI_UNF_SND_E enSound, HI_UNF_PDM_SOUND_PARAM_S *pstSoundParam)
{
    ...
    PDM_GetSoundDac0AndSpidfParam(pstSoundParam, &stTable,  &stKey, &u32Volume);
...
static HI_S32 DRV_PDM_GetReserveMem(const HI_CHAR *BufName, HI_U32 *pReseverAddr, HI_U32 *pReseverBufSize)
{
    HI_U32 BufIndex = 0;

    DRV_PDM_LOCK(&g_PdmGlobal.PdmMutex);

    for (BufIndex = 0; BufIndex < g_PdmGlobal.u32BufNum; BufIndex++)
    {
        if (0 == strncmp(g_PdmGlobal.stBufInfo[BufIndex].as8BufName, BufName, strlen(BufName)))
        {
            break;
        }
    }

    if (BufIndex >= g_PdmGlobal.u32BufNum)
    {
        DRV_PDM_UNLOCK(&g_PdmGlobal.PdmMutex);
        return HI_FAILURE;
    }

    if (g_PdmGlobal.stBufInfo[BufIndex].bRelease)
    {
        DRV_PDM_UNLOCK(&g_PdmGlobal.PdmMutex);
        return HI_SUCCESS;
    }
...
HI_S32 HI_DRV_PDM_GetDispParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam)
{
    ...
    {
        HI_ERR_PDM("pstDispParam is null!\n");
...
{
    if (HI_NULL == pstSoundParam)
    {
        HI_ERR_PDM("pstSoundParam is null!\n");
        return HI_ERR_PDM_PTR_NULL;
    }
...
HI_S32 HI_DRV_PDM_GetSoundParamEx(HI_UNF_SND_E enSound, HI_UNF_PDM_SOUND_PARAM_S * pstSoundParam, HI_BOOL *pbVolumeExistInBase)
{
    ...
    if ((HI_NULL == pbVolumeExistInBase) || (HI_NULL == pstSoundParam))
    {
...
{
    if (HI_NULL == pMceParam)
    {
        HI_ERR_PDM("pMceParam is null!\n");
        return HI_ERR_PDM_PTR_NULL;
    }
...
HI_S32 HI_DRV_PDM_GetReserveMem(const HI_CHAR *BufName, HI_U32 *pReseverAddr, HI_U32 *pReseverBufSize)
{
    if ((NULL == BufName) || (NULL == pReseverAddr) || (NULL == pReseverBufSize))
    {
        HI_ERR_PDM("ptr is null!\n");
        return HI_FAILURE;
    }

    return DRV_PDM_GetReserveMem(BufName,pReseverAddr,pReseverBufSize);
}

HI_S32 HI_DRV_PDM_GetData(const HI_CHAR *BufName, HI_U8 **pDataAddr, HI_U32 *pu32DataLen)
{
    if ((HI_NULL == pDataAddr) || (HI_NULL == BufName) || (HI_NULL ==  pu32DataLen))
    {
        HI_ERR_PDM("ptr is null!\n");
        return HI_FAILURE;
    }
