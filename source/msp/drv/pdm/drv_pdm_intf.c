...
PDM_EXPORT_FUNC_S g_PdmExportFuncs =
{
    ...
    .pfnPDM_GetReserveMem     = HI_DRV_PDM_GetReserveMem,
...
static HI_VOID PDM_DispProcRead(struct seq_file *p)
{
    HI_S32 Ret;
    HI_U32 i;
    HI_U32 j;
...
HI_S32 PDM_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        {
            HI_U8* pDataAddr = NULL;
            HI_U32 DataLen = 0;
            PDM_GET_DATA_S *pBufferInfo = NULL;

            pBufferInfo = (PDM_GET_DATA_S *)arg;

            if (NULL == pBufferInfo || NULL == pBufferInfo->BufAddr)
            {
                HI_ERR_PDM("para is invalid!\n");
                return HI_ERR_PDM_PTR_NULL;
            }

            Ret = HI_DRV_PDM_GetData(pBufferInfo->BufName, &pDataAddr, &DataLen);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PDM("DRV_PDM_GetData Err! Ret:0x%x\n",Ret);
                return Ret;
            }

            if (DataLen > pBufferInfo->BufLenth)
            {
                DataLen = pBufferInfo->BufLenth;
            }

            if (copy_to_user((void __user *)pBufferInfo->BufAddr, pDataAddr, DataLen))
            {
        ...
        {
            PDM_RESERVE_MEM_INFO_S *pReserveMemInfo = NULL;
            pReserveMemInfo = (PDM_RESERVE_MEM_INFO_S *)arg;

            if (NULL == pReserveMemInfo)
            {
                HI_ERR_PDM("pReserveMemInfo is NULL!\n");
                return HI_ERR_PDM_PTR_NULL;
            }

            Ret = HI_DRV_PDM_GetReserveMem(pReserveMemInfo->BufName, &pReserveMemInfo->PhyAddr, &pReserveMemInfo->Size);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PDM("HI_DRV_PDM_GetReserveMem fail 0x%x\n", Ret);
                return Ret;
            }
...
#ifdef CONFIG_COMPAT
HI_S32 PDM_COMPAT_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
    HI_S32  Ret = HI_SUCCESS;
    ...
    switch (cmd)
    {
        case CMD_PDM_COMPAT_GET_DATA:
        {
            HI_U8* pDataAddr = NULL;
            HI_U32 DataLen = 0;
            PDM_COMPAT_GET_DATA_S *pBufferInfo = NULL;

            pBufferInfo = (PDM_COMPAT_GET_DATA_S *)arg;

            if (NULL == pBufferInfo || NULL == compat_ptr(pBufferInfo->BufAddr))
            {
                HI_ERR_PDM("para is invalid!\n");
                return HI_ERR_PDM_PTR_NULL;
            }
            ...
            if (copy_to_user(compat_ptr(pBufferInfo->BufAddr), pDataAddr, DataLen))
            {
...
static long PDM_DRV_COMPAT_Ioctl(struct file *ffile, unsigned int cmd, unsigned long arg)
{
    ...
    Ret = HI_DRV_UserCopy(ffile->f_path.dentry->d_inode, ffile, cmd, arg, PDM_COMPAT_Ioctl);
