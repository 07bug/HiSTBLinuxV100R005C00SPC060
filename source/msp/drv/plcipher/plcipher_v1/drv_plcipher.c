...
HI_S32 HI_DRV_PLCIPHER_Resume(HI_VOID)
{
    ...
    memset(&stAttr, 0, sizeof(HI_UNF_PLCIPHER_ATTR_S));
...
HI_S32 HI_DRV_PLCIPHER_CreateHandle(HI_HANDLE* phPLCipher)
{
    ...
    *phPLCipher = HI_HANDLE_MAKEHANDLE(HI_ID_PLCIPHER, 0, i);
    ...
    /* clear key and iv, init */
    memset(&stPlAttr, 0x00, sizeof(stPlAttr));
    stPlAttr.CIHandle = *phPLCipher;

    stPlAttr.bIsCWOrIV = HI_TRUE;   //set key
    stPlAttr.enEvenOdd = HI_UNF_PLCIPHER_KEY_EVEN;
    HI_DRV_PLCIPHER_SetKeyIV(&stPlAttr);

    stPlAttr.bIsCWOrIV = HI_TRUE;   //set key
    stPlAttr.enEvenOdd = HI_UNF_PLCIPHER_KEY_ODD;
    HI_DRV_PLCIPHER_SetKeyIV(&stPlAttr);

    stPlAttr.bIsCWOrIV = HI_FALSE;   //set IV
    stPlAttr.enEvenOdd = HI_UNF_PLCIPHER_KEY_EVEN;
    HI_DRV_PLCIPHER_SetKeyIV(&stPlAttr);
...
HI_S32 HI_DRV_PLCIPHER_GetAttr(HI_HANDLE hPLCipher, HI_UNF_PLCIPHER_ATTR_S* pstAttr)
{
    ...
    pstAttr->enEvenOdd = g_stSoftChn[softChnId].enKeyEvenOdd;
...
HI_S32 HI_DRV_PLCIPHER_SetAttr(HI_HANDLE hPLCipher, HI_UNF_PLCIPHER_ATTR_S* pstAttr)
{
    ...
    g_stSoftChn[softChnId].enKeyEvenOdd = pstAttr->enEvenOdd;

    /* Set the even odd type for keyladdder encrypt */
    HAL_PLCipher_SetCWAttr(softChnId, pstAttr->enEvenOdd, pstAttr->enAlg, HI_TRUE);
    up(&g_PLCipherMutexKernel);
...
HI_S32 DRV_PLCIPHER_DealData(HI_BOOL bEncryptOrDecrypt,  PLCIPHER_DATA_S *pstCIData)
{
    ...
    /* DTS2018030901063: Resolve probabilistic encryption failures */
    HAL_PLCipher_ClrIntState();
...
HI_S32 HI_DRV_PLCIPHER_Encrypt(PLCIPHER_DATA_S *pstCIData)
{
    ...
    if (0 != (pstCIData->u32DataLength%(PLCIPHER_MIN_SIZE_PRE_DESC)))
    {
        HI_ERR_PLCIPHER("The data length must be an integer multiple of %d, len:%d\n", PLCIPHER_MIN_SIZE_PRE_DESC, pstCIData->u32DataLength);
        return HI_ERR_PLCIPHER_INVALID_PARA;
    }
