...
HI_S32 PLCIPHER_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    for(i = 0; i < PLCIPHER_CHAN_NUM; i++)
    {
        ...
        DRV_PLCIPHER_GetDESCStatus(&stDescStatus);
        PROC_PRINT(p, "\n---------------Desc Information-------------\n");
        PROC_PRINT(p, "InDescWPtr    InDescRPtr   OutDescWPtr    OutDescRPtr   Addr(InDesc/OutDesc)   Valid(In/Out)    Depth\n");
        PROC_PRINT(p, "    %d            %d          %d               %d             %08x/%08x     %d/%d           %d\n",
                    stDescStatus.InDescWPtr,
                    stDescStatus.InDescRPtr,
                    stDescStatus.OutDescWPtr,
                    stDescStatus.OutDescRPtr,
                    stDescStatus.InDescStartAddr,
                    stDescStatus.OutDescStartAddr,
                    stDescStatus.InDescValidNum,
                    stDescStatus.OutDescValidNum,
                    stDescStatus.DescDepth);
    }
...
HI_S32 PLCIPHER_DRV_ModInit(HI_VOID)
{
    ...
    g_u32PLCipherRegBase = ioremap_nocache(PLCIPHER_REG_BASE_ADDR_PHY, 0x1000);
