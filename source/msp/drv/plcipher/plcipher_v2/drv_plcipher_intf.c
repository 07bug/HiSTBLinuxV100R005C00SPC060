...
HI_S32 PLCIPHER_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    for(i = 0; i < PLCIPHER_CHAN_NUM; i++)
    {
        ...
        else if (HI_UNF_PLCIPHER_ALG_AES_CISSA == pstPLCipherChanInfo->enAlg)
        {
            strncpy(Alg, "AES_CISSA", sizeof(Alg));
        }
        else
        {
...
HI_S32 PLCIPHER_DRV_ModInit(HI_VOID)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_FATAL_PLCIPHER("HI_DRV_PLCIPHER_Init failed\n");
        goto out3;
    }
#ifdef MODULE
...
#endif
    iounmap((HI_VOID *)g_u32PLCipherRegBase);
out2:
