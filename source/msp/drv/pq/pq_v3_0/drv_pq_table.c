...
HI_S32 PQ_MNG_UpdatePhyList(HI_U32 u32Addr, HI_U8 u8Lsb, HI_U8 u8Msb, HI_U8 u8SourceMode, HI_U8 u8OutputMode, HI_U32 u32Value)
{
    ...
    for (i = 0; i < sg_u32PhyListNum; i++)
    {
        ...
        if ((PQ_SOURCE_MODE_NO != sg_pstPhyReg[i].u8SourceMode) && (u8SourceMode != sg_pstPhyReg[i].u8SourceMode))
        {
            continue;
        }
        if ((PQ_SOURCE_MODE_NO != sg_pstPhyReg[i].u8OutputMode) && (u8OutputMode != sg_pstPhyReg[i].u8OutputMode))
        {
            continue;
        }
...
HI_S32 PQ_MNG_LoadPhyList(PQ_REG_TYPE_E enRegType, PQ_SOURCE_MODE_E enSourceMode, PQ_OUTPUT_MODE_E enOutputMode, PQ_BIN_MODULE_E enModule)
{
    ...
    PQ_CHECK_OVER_RANGE_RE_FAIL(enModule, PQ_BIN_MODULE_BUTT);
    ...
    for (i = 0; i < sg_u32PhyListNum; i++)
    {
        ...
        if ((REG_TYPE_VPSS == enRegType) && (PQ_HAL_IsVpssReg(u32Addr) != HI_SUCCESS))
        {
            continue;
        }

        if ((REG_TYPE_VDP == enRegType) && (PQ_HAL_IsVdpReg(u32Addr) != HI_SUCCESS))
        {
            continue;
        }
        ...
        if (PQ_HAL_IsVpssReg(u32Addr) == HI_SUCCESS)
        {
            s32Ret = PQ_MNG_SetVpssReg(u32Addr, u8Lsb, u8Msb, u32Value);
        }
        else if (PQ_HAL_IsVdpReg(u32Addr) == HI_SUCCESS)
        {
...
HI_VOID PQ_MNG_PrintTable(PQ_PRN_TABLE_TYPE_E enType, HI_U32 u32PriAddr)
{
    ...
    PQ_CHECK_NULL_PTR_RE_NULL(sg_pstPhyReg);
...
HI_S32 PQ_MNG_GetArraySoftTable(HI_U32 u32Lut, HI_U32 *pArray, HI_U32 u32Num,
                                PQ_SOURCE_MODE_E enSourceMode, PQ_OUTPUT_MODE_E enOutputMode, HI_U32 u32DefaultArray[])
{
    HI_S32 i;
    HI_U32 u32Addr, u32Value;
    ...
    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstPhyReg);
    PQ_CHECK_NULL_PTR_RE_FAIL(pArray);
    PQ_CHECK_OVER_RANGE_RE_FAIL(enSourceMode, PQ_SOURCE_MODE_BUTT);
    PQ_CHECK_OVER_RANGE_RE_FAIL(enOutputMode, PQ_OUTPUT_MODE_BUTT);
...
HI_S32 PQ_MNG_SetReg(HI_PQ_REGISTER_S *pstAttr, PQ_SOURCE_MODE_E enSourceMode, PQ_OUTPUT_MODE_E enOutputMode)
{
    ...
    {
        if ((PQ_SOURCE_MODE_NO != sg_pstPhyReg[s32Num].u8SourceMode) \
        ...
        if ((PQ_OUTPUT_MODE_NO != sg_pstPhyReg[s32Num].u8OutputMode) \
    ...
    if (PQ_HAL_IsVpssReg(pstAttr->u32RegAddr) == HI_SUCCESS)
    {
        s32Ret = PQ_MNG_SetVpssReg(pstAttr->u32RegAddr, pstAttr->u8Lsb, \
                                   pstAttr->u8Msb, pstAttr->u32Value);
    }
    else if (PQ_HAL_IsVdpReg(pstAttr->u32RegAddr) == HI_SUCCESS)
    {
        s32Ret = PQ_MNG_SetVdpReg(pstAttr->u32RegAddr, pstAttr->u8Lsb, \
                                  pstAttr->u8Msb, pstAttr->u32Value);
    }
    else if (PQ_HAL_IsSpecialReg(pstAttr->u32RegAddr) == HI_SUCCESS)
    {
...
HI_S32 PQ_MNG_GetReg(HI_PQ_REGISTER_S *pstAttr)
{
    ...
    for (i = 0; i < PHY_REG_MAX; i++)
    {
        ...
        if ((PQ_SOURCE_MODE_NO != u8SourceMode) && (u8SourceMode != pstAttr->u8SourceMode))
        {
            continue;
        }

        if ((PQ_OUTPUT_MODE_NO != u8OutputMode) && (u8OutputMode != pstAttr->u8OutputMode))
        {
            continue;
        }
