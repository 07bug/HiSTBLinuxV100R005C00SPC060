...
HI_VOID PQ_HAL_VdpZmeRegCfg(HI_U32 u32LayerId, ALG_VZME_RTL_PARA_S *pstZmeRtlPara, HI_BOOL bFirEnable)
{
    ...
    /*
        有三种场景：
        一、16:9 到 12:9   只有水平缩小
        二、16:12 到 16:9  只有垂直缩小
        三、小窗口         [(3840/3,2160/3),(3840,2160)]
        ...
        else if(水平垂直同时缩，且水平缩小到3300~3840时)
        {
            先垂直后水平 && 垂直缩放倍数<=2.20
        }
        else(水平和垂直同时缩放)
        {
            先水平后垂直 && 垂直缩放比例限制：4400/ow(输出宽度)
        }
        */
    u32InW  = pstZmeRtlPara->u32ZmeWIn;
    u32InH  = pstZmeRtlPara->u32ZmeHIn;
    u32OutW = pstZmeRtlPara->u32ZmeWOut;
    u32OutH = pstZmeRtlPara->u32ZmeHOut;

    if (VDP_ZME_LAYER_V0 == u32LayerId)
    {
        /* add for 4K zoom out; Only consider Hor First Scene ,else all Ver first */
        if ((u32OutW < u32InW) && (u32InH == u32OutH))
        {
            PQ_HAL_VID_SetZmeHfirOrder(u32LayerId, HI_FALSE); /* Hor first */
        }
        else if ((u32OutW < u32InW) && (u32OutH < u32InH) && (u32OutW < 3300))
        {
            PQ_HAL_VID_SetZmeHfirOrder(u32LayerId, HI_FALSE); /* Hor first */
        }
        else
        {
            PQ_HAL_VID_SetZmeHfirOrder(u32LayerId, HI_TRUE); /* Ver first */
        }
    }
    else if (VDP_ZME_LAYER_V3 == u32LayerId)
    {
    ...
    */
    ...
    /* In 98MV100 4KPIP Bug V1:Input format 422, Output format 422 Close Chroma Upsampling */
    if (VDP_ZME_LAYER_V1 == u32LayerId)
    {
        if ((VDP_PROC_FMT_SP_422 == pstZmeRtlPara->u8ZmeYCFmtIn) && (VDP_PROC_FMT_SP_422 == pstZmeRtlPara->u8ZmeYCFmtOut))
        {
            PQ_HAL_VID_SetZmeInFmt(u32LayerId, VDP_PROC_FMT_SP_422);
            PQ_HAL_VID_SetZmeOutFmt(u32LayerId, VDP_PROC_FMT_SP_422);
            PQ_HAL_VID_SetZmeHorLumEn(u32LayerId, HI_FALSE);
            PQ_HAL_VID_SetZmeVerLumEn(u32LayerId, HI_FALSE);
            PQ_HAL_VID_SetZmeHorChmEn(u32LayerId, HI_FALSE);
            PQ_HAL_VID_SetZmeVerChmEn(u32LayerId, HI_FALSE);
        }
        else
        {
...
static HI_VOID PQ_HAL_SR_SetZmeCoef(HI_U32 u32Data, VDP_SR_ZME_MODE_E enMode, VDP_SR_ZME_COEF_S *pstVdpSrZmeCoef)
{
    ...
    if (enMode == VDP_SR_ZME_MODE_HOR)
    {
        ...
        SR_HCOEF5.bits.hccoef00 = pstVdpSrZmeCoef->s16HCCoef0[0];
        SR_HCOEF5.bits.hccoef01 = pstVdpSrZmeCoef->s16HCCoef0[1];
        ...
        SR_HCOEF6.bits.hccoef03 = pstVdpSrZmeCoef->s16HCCoef0[3];
        SR_HCOEF6.bits.hccoef04 = pstVdpSrZmeCoef->s16HCCoef0[4];
        ...
        SR_HCOEF7.bits.hccoef06 = pstVdpSrZmeCoef->s16HCCoef0[6];
        SR_HCOEF7.bits.hccoef07 = pstVdpSrZmeCoef->s16HCCoef0[7];
        ...
        SR_HCOEF8.bits.hccoef11 = pstVdpSrZmeCoef->s16HCCoef1[1];
        SR_HCOEF8.bits.hccoef12 = pstVdpSrZmeCoef->s16HCCoef1[2];
        ...
        SR_HCOEF9.bits.hccoef14 = pstVdpSrZmeCoef->s16HCCoef1[4];
        SR_HCOEF9.bits.hccoef15 = pstVdpSrZmeCoef->s16HCCoef1[5];
        ...
        SR_HCOEF10.bits.hccoef17 = pstVdpSrZmeCoef->s16HCCoef1[7];
...
/* Explanation:
   The unit of the pointer pu32Addr and variable s32CoefAttr is 4 Bytes, stArray.s32CoefAttr is the start address,
   In function VpssZmeTransCoefAlign, u32Size and stArray.u32Size unit is byte, 256 is the number of Byte，so pu32Addr + 256 / 4

   Sharpen filter coefficient addressing from 256, So The size of every Vpss ZME filter coefficient are same, Which is 256,
   no matter it is 8 tap, 6 tap, 4 tap, even 2 tap
   */
static HI_S32 VpssZmeLoadCoefHL(VZME_COEF_RATIO_E enCoefRatio, HI_U32 *pu32Addr)
{
    ZME_COEF_BITARRAY_S stArray;

    /* load luma horizontal zoom coef */
    VpssZmeTransCoefAlign(g_pVZmeVpssCoef[enCoefRatio][VZME_VPSS_COEF_8T32P_LH], VZME_TAP_8T32P, &stArray);

    PQ_CHECK_NULL_PTR_RE_FAIL(pu32Addr);
    PQ_CHECK_NULL_PTR_RE_FAIL(pu32Addr + stArray.u32Size);

    PQ_SAFE_MEMCPY(pu32Addr, stArray.s32CoefAttr, stArray.u32Size); /* function PQ_SAFE_MEMCPY unit is Byte */
    //pu32Addr += stArray.u32Size;
    pu32Addr += 256 / 4;

    /* for Sharpen(LTI&CTI) High frequency scaling; load LTI horizontal zoom coef */
    VpssZmeTransCoefAlign(g_pVDtiZmeCoef[enCoefRatio][VDTI_COEF_8T32P_LH], VZME_TAP_8T32P, &stArray);
...
static HI_S32 VpssZmeLoadCoefHC(VZME_COEF_RATIO_E enCoefRatio, HI_U32 *pu32Addr)
{
    ZME_COEF_BITARRAY_S stArray;

    /* load chroma horizontal zoom coef */
    VpssZmeTransCoefAlign(g_pVZmeVpssCoef[enCoefRatio][VZME_VPSS_COEF_4T32P_CH], VZME_TAP_4T32P, &stArray);
    ...
    /* for Sharpen(LTI&CTI) High frequency scaling; load CTI horizontal zoom coef */
    VpssZmeTransCoefAlign(g_pVDtiZmeCoef[enCoefRatio][VDTI_COEF_4T32P_CH], VZME_TAP_4T32P, &stArray);
...
static HI_S32 VpssZmeLoadCoefVL(VZME_COEF_RATIO_E enCoefRatio,  HI_U32 *pu32Addr)
{
    ZME_COEF_BITARRAY_S stArray;

    /* for General Scaling; load luma vertical zoom coef */
    VpssZmeTransCoefAlign(g_pVZmeVpssCoef[enCoefRatio][VZME_VPSS_COEF_6T32P_LV], VZME_TAP_6T32P, &stArray);
    ...
    /* for Sharpen(LTI&CTI) High frequency scaling; load LTI vertical zoom coef */
    VpssZmeTransCoefAlign(g_pVDtiZmeCoef[enCoefRatio][VDTI_COEF_2T32P_LV], VZME_TAP_2T32P, &stArray);
...
static HI_S32 VpssZmeLoadCoefVC(VZME_COEF_RATIO_E enCoefRatio,  HI_U32 *pu32Addr)
{
    ZME_COEF_BITARRAY_S stArray;

    /* for General Scaling; load chroma vertical zoom coef */
    VpssZmeTransCoefAlign(g_pVZmeVpssCoef[enCoefRatio][VZME_VPSS_COEF_4T32P_CV], VZME_TAP_4T32P, &stArray);
    ...
    /* for Sharpen(LTI&CTI) High frequency scaling; load CTI vertical zoom coef */
    VpssZmeTransCoefAlign(g_pVDtiZmeCoef[enCoefRatio][VDTI_COEF_2T32P_CV], VZME_TAP_2T32P, &stArray);
...
{
    HI_U32 u32NumSize;
    /* HScalar also use the Horizontal Coef */

    PQ_CHECK_NULL_PTR_RE_NULL(pu32CurAddr);
    PQ_CHECK_NULL_PTR_RE_NULL(pstAddrTmp);

    /* HL8 and LTI HL8 */
    /* Unit is Byte 256 +17*3*4 =460 For the requirement Of 128bits align, it must exact division by 16, carry to 464 */
    u32NumSize = 464;
    VpssZmeLoadCoefHL(VZME_COEF_1, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHL8_1 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4; /* pu32CurAddr Unit is 4 Bytes */

    VpssZmeLoadCoefHL(VZME_COEF_E1, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHL8_E1 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHL(VZME_COEF_075, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHL8_075 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHL(VZME_COEF_05, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHL8_05 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHL(VZME_COEF_033, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHL8_033 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHL(VZME_COEF_025, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHL8_025 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHL(VZME_COEF_0, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHL8_0 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    /* HC4 and CTI HC4 */
    u32NumSize = 400; /* 256+17*2*4=392 carry to 400 */
    VpssZmeLoadCoefHC(VZME_COEF_1, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHC4_1 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHC(VZME_COEF_E1, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHC4_E1 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHC(VZME_COEF_075, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHC4_075 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHC(VZME_COEF_05, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHC4_05 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHC(VZME_COEF_033, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHC4_033 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHC(VZME_COEF_025, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHC4_025 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefHC(VZME_COEF_0, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrHC4_0 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    /* VL6 and LTI VL2 */
    u32NumSize = 336; /* 256+17*1*4 = 324 Carry to 336 */
    VpssZmeLoadCoefVL(VZME_COEF_1, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVL6_1 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVL(VZME_COEF_E1, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVL6_E1 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVL(VZME_COEF_075, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVL6_075 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVL(VZME_COEF_05, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVL6_05 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVL(VZME_COEF_033, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVL6_033 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVL(VZME_COEF_025, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVL6_025 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVL(VZME_COEF_0, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVL6_0 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;
    ...
    /* VC4 and CTI VC2 */
    u32NumSize = 336; /* 256+17*1*4 = 324 carry to 336 */
    VpssZmeLoadCoefVC(VZME_COEF_1, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVC4_1 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVC(VZME_COEF_E1, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVC4_E1 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVC(VZME_COEF_075, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVC4_075 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVC(VZME_COEF_05, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVC4_05 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVC(VZME_COEF_033, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVC4_033 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVC(VZME_COEF_025, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVC4_025 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;

    VpssZmeLoadCoefVC(VZME_COEF_0, pu32CurAddr);
    pstAddrTmp->u32ZmeCoefAddrVC4_0 = u32PhyAddr;
    u32PhyAddr  += u32NumSize;
    pu32CurAddr += u32NumSize / 4;
...
static HI_S32 PQ_HAL_VPSS_SetZmeEnable(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, REG_ZME_MODE_E eMode, HI_BOOL bEnable)
{
    switch (ePort)
    {
        case VPSS_REG_HD:
            ...
            {
                pstReg->VPSS_VHD_HSP.bits.hlmsc_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_HSP.bits.hchmsc_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERL) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_VSP.bits.vlmsc_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERC) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_VSP.bits.vchmsc_en = bEnable;
            }
            break;
        case VPSS_REG_STR:
            if ((eMode == REG_ZME_MODE_HORL ) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_HSP.bits.hlmsc_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_HSP.bits.hchmsc_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERL) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_VSP.bits.vlmsc_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERC) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_VSP.bits.vchmsc_en = bEnable;
            }
            break;
        case VPSS_REG_HSC:
            if ((eMode == REG_ZME_MODE_HORL ) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_CTRL.bits.hlmsc_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_CTRL.bits.hchmsc_en = bEnable;
            }
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmeInSize(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, HI_U32 u32Height, HI_U32 u32Width)
{
    switch (ePort)
    {
        ...
        case VPSS_REG_STR:
        case VPSS_REG_HD:
            pstReg->VPSS_VHD_ZMEIRESO.bits.ih = u32Height - 1;
            pstReg->VPSS_VHD_ZMEIRESO.bits.iw = u32Width - 1;
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmeOutSize(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, HI_U32 u32Height, HI_U32 u32Width)
{
    ...
    {
        case VPSS_REG_HD:
            pstReg->VPSS_VHD_ZMEORESO.bits.oh = u32Height - 1;
            pstReg->VPSS_VHD_ZMEORESO.bits.ow = u32Width - 1;
            break;
        case VPSS_REG_STR:
            pstReg->VPSS_STR_ZMEORESO.bits.oh = u32Height - 1;
            pstReg->VPSS_STR_ZMEORESO.bits.ow = u32Width - 1;
            break;
        case VPSS_REG_HSC:
            pstReg->VPSS_HSCL_ORESO.bits.hscl_ow = u32Width - 1;
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmeFirEnable(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, REG_ZME_MODE_E eMode, HI_BOOL bEnable)
{
    switch (ePort)
    {
        case VPSS_REG_HD:
            if ((eMode ==  REG_ZME_MODE_HORL ) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_HSP.bits.hlfir_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_HSP.bits.hchfir_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERL) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_VSP.bits.vlfir_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERC) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_VSP.bits.vchfir_en = bEnable;
            }
            break;
        case VPSS_REG_STR:
            if ((eMode == REG_ZME_MODE_HORL ) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_HSP.bits.hlfir_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_HSP.bits.hchfir_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERL) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_VSP.bits.vlfir_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERC) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_VSP.bits.vchfir_en = bEnable;
            }
            break;
        case VPSS_REG_HSC:
            if ((eMode == REG_ZME_MODE_HORL ) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_CTRL.bits.hlfir_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_CTRL.bits.hchfir_en = bEnable;
            }
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmeMidEnable(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, REG_ZME_MODE_E eMode, HI_BOOL bEnable)
{
    switch (ePort)
    {
        case VPSS_REG_HD:
            if ((eMode == REG_ZME_MODE_HORL) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_HSP.bits.hlmid_en = bEnable;
            }
            if ((eMode ==  REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_HSP.bits.hchmid_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERL) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_VSP.bits.vlmid_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERC) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_VHD_VSP.bits.vchmid_en = bEnable;
            }
            break;
        case VPSS_REG_STR:
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_HSP.bits.hchmid_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_HORL) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_HSP.bits.hlmid_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERL) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_VSP.bits.vlmid_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_VERC) || (eMode == REG_ZME_MODE_VER) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_STR_VSP.bits.vchmid_en = bEnable;
            }
            break;
        case VPSS_REG_HSC:
            if ((eMode == REG_ZME_MODE_HORL ) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_CTRL.bits.hlmid_en = bEnable;
            }
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_CTRL.bits.hchmid_en = bEnable;
            }
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmePhase(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, REG_ZME_MODE_E eMode, HI_S32 s32Phase)
{
    switch (ePort)
    {
        case VPSS_REG_HD:
            ...
            {
                pstReg->VPSS_VHD_HCOFFSET.bits.hor_coffset = s32Phase;
            }
            else if (eMode == REG_ZME_MODE_HORL)
            {
                pstReg->VPSS_VHD_HLOFFSET.bits.hor_loffset = s32Phase;
            }
            else if (eMode == REG_ZME_MODE_VERC)
            {
                pstReg->VPSS_VHD_VOFFSET.bits.vchroma_offset = s32Phase;
            }
            else if (eMode == REG_ZME_MODE_VERL)
            {
                pstReg->VPSS_VHD_VOFFSET.bits.vluma_offset = s32Phase;
            }
            else
            {
        ...
        case VPSS_REG_STR:
            if (eMode == REG_ZME_MODE_HORC)
            {
                pstReg->VPSS_STR_HCOFFSET.bits.hor_coffset = s32Phase;
            }
            else if (eMode == REG_ZME_MODE_HORL)
            {
                pstReg->VPSS_STR_HLOFFSET.bits.hor_loffset = s32Phase;
            }
            else if (eMode == REG_ZME_MODE_VERC)
            {
                pstReg->VPSS_STR_VOFFSET.bits.vchroma_offset = s32Phase;
            }
            else if (eMode == REG_ZME_MODE_VERL)
            {
                pstReg->VPSS_STR_VOFFSET.bits.vluma_offset = s32Phase;
            }
            else
            {
                HI_ERR_PQ("Error,PQ_HAL_VPSS_SetZmePhase error\n");
        ...
        case VPSS_REG_HSC:
            if ((eMode == REG_ZME_MODE_HORL ) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_HLOFF.bits.hscl_loffset = s32Phase;
            }
            if ((eMode == REG_ZME_MODE_HORC) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_HCOFF.bits.hscl_coffset = s32Phase;
            }
            break;
        default:
            HI_ERR_PQ("Error,PQ_HAL_VPSS_SetZmePhase error\n");
...
static HI_S32 PQ_HAL_VPSS_SetZmeRatio(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, REG_ZME_MODE_E eMode, HI_U32 u32Ratio)
{
    switch (ePort)
    {
        case VPSS_REG_HD:
            ...
            {
                pstReg->VPSS_VHD_HSP.bits.hratio = u32Ratio;
            }
            else
            {
                pstReg->VPSS_VHD_VSR.bits.vratio = u32Ratio;
            }
            break;
        case VPSS_REG_STR:
            if (eMode == REG_ZME_MODE_HOR)
            {
                pstReg->VPSS_STR_HSP.bits.hratio = u32Ratio;
            }
            else
            {
                pstReg->VPSS_STR_VSP.bits.vratio = u32Ratio;
            }
            break;
        case VPSS_REG_HSC:
            if ((eMode == REG_ZME_MODE_HORL ) || (eMode == REG_ZME_MODE_HOR) || (eMode == REG_ZME_MODE_ALL))
            {
                pstReg->VPSS_HSCL_CTRL.bits.hratio = u32Ratio;
            }
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmeHfirOrder(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, HI_BOOL bVfirst)
{
    switch (ePort)
    {
        case VPSS_REG_HD:
            ...
            break;
        case VPSS_REG_STR:
            //pstReg->VPSS_STR_HSP.bits.hfir_order = bVfirst;
            break;
        case VPSS_REG_HSC:
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmeInFmt(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, HI_DRV_PIX_FORMAT_E eFormat)
{
    ...
    switch (ePort)
    {
        case VPSS_REG_STR:
        case VPSS_REG_HD:
            pstReg->VPSS_VHD_VSP.bits.zme_in_fmt = u32Format;
            break;
        case VPSS_REG_HSC:
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmeOutFmt(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, HI_DRV_PIX_FORMAT_E eFormat)
{
    ...
    {
        case VPSS_REG_HD:
            pstReg->VPSS_VHD_VSP.bits.zme_out_fmt = u32Format;
            break;
        case VPSS_REG_STR:
            pstReg->VPSS_STR_VSP.bits.zme_out_fmt = u32Format;
            break;
        case VPSS_REG_HSC:
            break;
        default:
...
static HI_S32 PQ_HAL_VPSS_SetZmeCoefAddr(HI_U32 u32HandleNo, VPSS_REG_PORT_E ePort, S_CAS_REGS_TYPE *pstReg, REG_ZME_MODE_E eMode, HI_U32 u32Addr)
{
    switch (ePort)
    {
        case VPSS_REG_HD:
            if (eMode == REG_ZME_MODE_HORC)
            {
                pstReg->VPSS_VHD_ZME_CHADDR.bits.vhd_scl_ch = u32Addr;
            }
            else if (eMode == REG_ZME_MODE_HORL)
            {
                pstReg->VPSS_VHD_ZME_LHADDR.bits.vhd_scl_lh = u32Addr;
            }
            else if (eMode == REG_ZME_MODE_VERC)
            {
                pstReg->VPSS_VHD_ZME_CVADDR.bits.vhd_scl_cv = u32Addr;
            }
            else if (eMode == REG_ZME_MODE_VERL)
            {
                pstReg->VPSS_VHD_ZME_LVADDR.bits.vhd_scl_lv = u32Addr;
            }
            else
            {
        ...
        case VPSS_REG_STR:
            if (eMode == REG_ZME_MODE_HORC)
            {
                pstReg->VPSS_STR_ZME_CHADDR.bits.str_scl_ch = u32Addr;
            }
            else if (eMode == REG_ZME_MODE_HORL)
            {
                pstReg->VPSS_STR_ZME_LHADDR.bits.str_scl_lh = u32Addr;
            }
            else if (eMode == REG_ZME_MODE_VERC)
            {
                pstReg->VPSS_STR_ZME_CVADDR.bits.str_scl_cv = u32Addr;
            }
            else if (eMode == REG_ZME_MODE_VERL)
            {
                pstReg->VPSS_STR_ZME_LVADDR.bits.str_scl_lv = u32Addr;
            }
            else
            {
                HI_ERR_PQ("Error,PQ_HAL_VPSS_SetZmeCoefAddr error\n");
        ...
        case VPSS_REG_HSC:
            if (eMode == REG_ZME_MODE_HORC)
            {
                pstReg->VPSS_HSCL_CADDR.bits.hscl_caddr = u32Addr;
            }
            else if (eMode == REG_ZME_MODE_HORL)
            {
                pstReg->VPSS_HSCL_LADDR.bits.hscl_laddr = u32Addr;
            }
            break;
        default:
            HI_ERR_PQ("Error,PQ_HAL_VPSS_SetZmeCoefAddr error\n");
...
HI_VOID PQ_HAL_VpssZmeRegCfg(HI_U32 u32LayerId, S_CAS_REGS_TYPE *pstReg, ALG_VZME_RTL_PARA_S *pstZmeRtlPara, HI_BOOL bFirEnable)
{
    ...
    /* Modify start sdk 2014-12-10; for logic when LTI and CTI enable, it must set to fither mode
       Another 4K mode in drv_pq.c can read reg vlaue,close LTI enable
       */
    pstZmeRtlPara->bZmeMdHL = 1;
    pstZmeRtlPara->bZmeMdHC = 1;
    pstZmeRtlPara->bZmeMdVL = 1;
    pstZmeRtlPara->bZmeMdVC = 1;
    /* Modify end sdk 2014-12-10 */
