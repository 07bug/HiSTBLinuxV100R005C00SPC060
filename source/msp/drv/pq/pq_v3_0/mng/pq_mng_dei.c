...
*****************************************************************************/
static HI_S32 PQ_MNG_DEIGlobalMotionGain(HI_S32 s32WidthY, HI_S32 s32HeightY, HI_U8 u8BitDepth, MotionHist_S *pstMotionHist)
{
    HI_U32 gain;
...
HI_S32 PQ_MNG_GetGlobalMotion(MOTION_INPUT_S *pstMotionInput)
{
    ...
    s32Ret = PQ_HAL_GetInterlaceMotionEn(pstMotionInput->u32HandleNo, &bIglbEn);
    if (HI_SUCCESS != s32Ret)
    {
        return HI_FAILURE;
    }
...
HI_S32 PQ_MNG_InitDei(HI_VOID)
{
    ...
    s32Ret = PQ_MNG_InitPhyList(u32HandleNo, REG_TYPE_VPSS, PQ_SOURCE_MODE_NO, PQ_OUTPUT_MODE_NO, PQ_BIN_MODULE_DEI);
    if (HI_SUCCESS != s32Ret)
    {
...
{
    PQ_CHECK_NULL_PTR_RE_FAIL(pu32Gain);
