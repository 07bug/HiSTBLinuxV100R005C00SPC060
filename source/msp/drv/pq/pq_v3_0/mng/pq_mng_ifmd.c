...
static HI_S32 PQ_MNG_GetIfmdSoftTable(HI_BOOL bInit)
{
    ...
    g_u32IPDetect     = PQ_MNG_GetSoftTable(PQ_SOFT_IP_DETECT,    PQ_SOURCE_MODE_NO, PQ_OUTPUT_MODE_NO, IP_DECTECT_INTERLACE_PRIMARY);
#if defined(CHIP_TYPE_hi3716mv410) || defined(CHIP_TYPE_hi3716mv420)
    g_u32FrmRateThr_L = PQ_MNG_GetSoftTable(PQ_SOFT_FRMRATETHR_L, PQ_SOURCE_MODE_NO, PQ_OUTPUT_MODE_NO, PLAYBACK_FRMRATETHR_L);
    g_u32FrmRateThr_H = PQ_MNG_GetSoftTable(PQ_SOFT_FRMRATETHR_H, PQ_SOURCE_MODE_NO, PQ_OUTPUT_MODE_NO, PLAYBACK_FRMRATETHR_H);
#endif
...
HI_S32 PQ_MNG_InitIfmd(PQ_PARAM_S *pstPqParam)
{
    ...
    s32Ret = PQ_MNG_InitPhyList(u32HandleNo, REG_TYPE_VPSS, PQ_SOURCE_MODE_NO, PQ_OUTPUT_MODE_NO, PQ_BIN_MODULE_FMD);
    if (HI_SUCCESS != s32Ret)
    {
