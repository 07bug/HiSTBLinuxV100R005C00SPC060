...
static HI_VOID pq_table_GetBootargs(HI_VOID)
{
    ...
    {
        PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_COLOR, PQ_SOURCE_MODE_UHD, HI_FALSE);
    }
...
HI_S32 PQ_TABLE_InitPQTable(PQ_PARAM_S *pstPqParam, HI_BOOL bPqParaUseTable)
{
...
HI_S32 PQ_TABLE_SetVdpReg(HI_U32 u32Addr, HI_U8 u8Lsb, HI_U8 u8Msb, HI_U32 u32Value)
{
    ...
    if (PQ_HAL_CheckVdpValid() == HI_SUCCESS)
    {
...
HI_S32 PQ_TABLE_SetVpssReg(HI_U32 u32Addr, HI_U8 u8Lsb, HI_U8 u8Msb, HI_U32 u32Value)
{
    ...
    {
        if (PQ_HAL_CheckVpssValid(i) == HI_FAILURE)
        {
            continue;
        }
...
HI_S32 PQ_TABLE_SetVdpRegCfgByCoef(HI_U32 u32Addr, HI_U8 u8Lsb, HI_U8 u8Msb, HI_U32 u32Value)
{
    HI_U32 u32RegValue = 0;
    HI_U32 u32OffsetAddr =  (u32Addr & REG_OFFSET_ADDR_MASK) - 0x7004;

    u32RegValue = *((HI_U32 *)((HI_VOID *)(g_stSharpCfgBuff.pu8StartVirAddr) + u32OffsetAddr));

    PQ_HAL_SetU32ByBit(&u32RegValue, u8Msb, u8Lsb, u32Value);

    *(HI_U32 *)((HI_VOID *)(g_stSharpCfgBuff.pu8StartVirAddr) + u32OffsetAddr) = u32RegValue;

    return HI_SUCCESS;
}

/* 特殊配置寄存器方式: 通过系数方式配置寄存器 */
static HI_S32 pq_table_SetVdpRegCfgByCoefByModule(HI_U32 u32Module, HI_U32 u32Addr, HI_U8 u8Lsb, HI_U8 u8Msb, HI_U32 u32Value)
{
    if ((HI_PQ_MODULE_SHARPNESS != u32Module) || (0x7000 == u32Addr))
    {
        return HI_FAILURE;
    }

    PQ_TABLE_SetVdpRegCfgByCoef(u32Addr, u8Lsb, u8Msb, u32Value);

    return HI_SUCCESS;
}
#endif

HI_S32 PQ_TABLE_LoadPhyList(PQ_REG_TYPE_E enRegType,
                            PQ_SOURCE_MODE_E enSourceMode,
                            PQ_OUTPUT_MODE_E enOutputMode,
                            HI_U32 u32HandleNo)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 i;
    ...
    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstPhyReg);
    ...
    for (i = 0; i < sg_u32PhyListNum; i++)
    {
        ...
        if ((PQ_SOURCE_MODE_NO != u8SourceMode) && (u8SourceMode != enSourceMode))
        {
            continue;
        }

        if ((PQ_OUTPUT_MODE_NO != u8OutputMode) && (u8OutputMode != enOutputMode))
        {
            continue;
        }
...
#if defined(SHARPEN_CFG_STYLE_COEF)
            /* sharpen set cfg by coef */
            s32Ret = pq_table_SetVdpRegCfgByCoefByModule(u32Module, u32Addr, u8Lsb, u8Msb, u32Value);
            if (s32Ret == HI_SUCCESS)
            {
                continue;
            }
...
HI_S32 PQ_TABLE_LoadPhyListOfAlg(HI_PQ_MODULE_E enModule,
                                 PQ_SOURCE_MODE_E enSourceMode,
                                 PQ_OUTPUT_MODE_E enOutputMode)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
HI_S32 PQ_TABLE_InitPhyList(HI_U32 u32ID,
                            HI_PQ_MODULE_E enModule,
                            PQ_SOURCE_MODE_E enSourceMode,
                            PQ_OUTPUT_MODE_E enOutputMode)
{
    ...
    for (i = u32StartPos; i <= u32EndPos; i++)
    {
        ...
        else if (REG_TYPE_VDP == PQ_COMM_GetAlgTypeID(enModule))
        {
            ...
            PQ_CHECK_NULL_PTR_RE_FAIL(pVdpVirReg);
...
#if defined(SHARPEN_CFG_STYLE_COEF)
            /* sharpen set cfg by coef */
            if (HI_SUCCESS != pq_table_SetVdpRegCfgByCoefByModule(u32Module, u32Addr, u8Lsb, u8Msb, u32Value))
            {
                continue;
            }
#endif
...
HI_U32 PQ_TABLE_GetSoftTable(HI_U32 u32Lut,
                             PQ_SOURCE_MODE_E enSourceMode,
                             PQ_OUTPUT_MODE_E enOutputMode,
                             HI_U32 u32DefaultValue)
{
    HI_U32 i;
    ...
    for (i = 0; i < sg_u32SoftListNum;  i++)
    {
        ...
        if (HI_FAILURE == PQ_HAL_IsSpecialReg(u32Addr))
        {
            break;
        }
...
HI_VOID pq_table_ProcessInitSoftTable(HI_U32 u32Addr, HI_U32 u32Value)
{
    ...
    HI_PQ_COLORTEMPERATURE_S stTemperature = {50, 50, 50};
    ...
    switch (u32Addr)
    {
        case PQ_SOFT_COLOR_TEMP_R:
            ...
            {
                GET_ALG_FUN(HI_PQ_MODULE_CSC)->GetColorTemp(&stTemperature);
                stTemperature.u32Red = u32Value;
        ...
        case PQ_SOFT_COLOR_TEMP_G:
            ...
            {
                GET_ALG_FUN(HI_PQ_MODULE_CSC)->GetColorTemp(&stTemperature);
                stTemperature.u32Green = u32Value;
                GET_ALG_FUN(HI_PQ_MODULE_CSC)->SetColorTemp(&stTemperature);
            }
            break;
        case PQ_SOFT_COLOR_TEMP_B:
            ...
            {
                GET_ALG_FUN(HI_PQ_MODULE_CSC)->GetColorTemp(&stTemperature);
                stTemperature.u32Blue = u32Value;
                GET_ALG_FUN(HI_PQ_MODULE_CSC)->SetColorTemp(&stTemperature);
            }
            break;
        case PQ_SOFT_MODULE_FMD_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_FMD, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_FMD_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_FMD, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_FMD_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_FMD, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_TNR_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_TNR, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_TNR_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_TNR, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_TNR_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_TNR, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_SNR_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_SNR, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_SNR_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_SNR, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_SNR_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_SNR, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_VDP4KSNR_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_VDP4KSNR, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_VDP4KSNR_FHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_VDP4KSNR, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_VDP4KSNR_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_VDP4KSNR, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_DB_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DB, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_DB_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DB, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_DB_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DB, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_DR_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DR, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_DR_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DR, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_DR_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DR, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_DM_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DM, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_DM_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DM, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_DM_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DM, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_SHARPN_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_SHARPNESS, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_SHARPN_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_SHARPNESS, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_SHARPN_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_SHARPNESS, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_DCI_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DCI, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_DCI_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DCI, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_DCI_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DCI, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_ACM_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_COLOR, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_ACM_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_COLOR, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_ACM_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_COLOR, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_CSC_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_CSC, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_CSC_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_CSC, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_CSC_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_CSC, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        case PQ_SOFT_MODULE_DS_SD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DS, PQ_SOURCE_MODE_SD, u8Value);
            break;
        case PQ_SOFT_MODULE_DS_HD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DS, PQ_SOURCE_MODE_HD, u8Value);
            break;
        case PQ_SOFT_MODULE_DS_UHD:
            PQ_COMM_SetMoudleCtrl(HI_PQ_MODULE_DS, PQ_SOURCE_MODE_UHD, u8Value);
            break;
        default:
        {
            break;
...
HI_S32 PQ_TABLE_InitSoftTable(HI_BOOL bInit)
{
    ...
    if (bInit == HI_TRUE)
    {
        for (i = 0; i < HI_PQ_MODULE_BUTT; i++)
        {
            ...
            PQ_COMM_SetMoudleCtrl(i, PQ_SOURCE_MODE_NO, HI_TRUE);
            PQ_COMM_SetMoudleCtrl(i, PQ_SOURCE_MODE_SD, HI_TRUE);
            PQ_COMM_SetMoudleCtrl(i, PQ_SOURCE_MODE_HD, HI_TRUE);
            PQ_COMM_SetMoudleCtrl(i, PQ_SOURCE_MODE_UHD, HI_TRUE);
        }
    }

    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstSoftReg);
...
HI_S32 PQ_TABLE_FindBinPos(HI_PQ_MODULE_E enModule, HI_U32 *pu32StartPos, HI_U32 *pu32EndPos)
{
    ...
    PQ_CHECK_OVER_RANGE_RE_FAIL(enModule, HI_PQ_MODULE_BUTT);
    PQ_CHECK_NULL_PTR_RE_FAIL(pu32StartPos);
    PQ_CHECK_NULL_PTR_RE_FAIL(pu32EndPos);
...
HI_S32 PQ_TABLE_SetTableRegDefault(HI_VOID)
{
    ...
    PQ_CHECK_NULL_PTR_RE_FAIL(pstPhyRegDefault);
    PQ_CHECK_NULL_PTR_RE_FAIL(pstSoftRegDefault);
    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstPhyReg);
    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstSoftReg);
