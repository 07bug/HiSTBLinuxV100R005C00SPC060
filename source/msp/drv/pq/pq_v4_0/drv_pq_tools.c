...
static HI_S32 pq_tools_UpdatePhyList(HI_U32 u32Addr, HI_U8 u8Lsb, HI_U8 u8Msb, HI_U8 u8SourceMode, HI_U8 u8OutputMode, HI_U32 u32Value)
{
    ...
    PQ_CHECK_NULL_PTR_RE_FAIL(pstPhyReg);
    ...
    for (i = 0; i < u32PhyListNum; i++)
    {
        ...
        if ((PQ_SOURCE_MODE_NO != pstPhyReg[i].u8SourceMode) && (u8SourceMode != pstPhyReg[i].u8SourceMode))
        {
            continue;
        }
        if ((PQ_SOURCE_MODE_NO != pstPhyReg[i].u8OutputMode) && (u8OutputMode != pstPhyReg[i].u8OutputMode))
        {
            continue;
        }
...
static HI_S32 pq_tools_SetSoftTable(HI_U32 u32Lut, PQ_SOURCE_MODE_E enSourceMode, PQ_OUTPUT_MODE_E enOutputMode, HI_U32 u32Value)
{
    ...
    PQ_CHECK_NULL_PTR_RE_FAIL(pstSoftReg);
...
HI_S32 PQ_TOOLS_SetRegister(HI_PQ_REGISTER_S *pstAttr)
{
    ...
    if (PQ_HAL_IsSpecialReg(pstAttr->u32RegAddr) == HI_SUCCESS)
    {
    ...
    {
        if ((PQ_SOURCE_MODE_NO != pstPhyReg[s32Num].u8SourceMode)
        ...
        if ((PQ_OUTPUT_MODE_NO != pstPhyReg[s32Num].u8OutputMode)
    ...
    if (HI_FALSE == REGADDR_IS_ALIGN_4BYTE(pstAttr->u32RegAddr))
    {
        HI_ERR_PQ("RegAddr is not align to 4 bytes! %x\n", pstAttr->u32RegAddr);
        return HI_FAILURE;
    }
