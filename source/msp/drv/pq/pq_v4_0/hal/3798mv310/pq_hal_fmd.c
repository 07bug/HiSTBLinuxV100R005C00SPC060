...
{
    HI_U32 i, j;
    HI_U32 au32MotionHistTmp[32] = {0};         /* output of logic, 32bit unsigned */
    HI_U32 u32RatioDiva = 8;
    HI_U32 u32RatioDivb = 10;
    S_VPSSWB_REGS_TYPE *p;

    PQ_CHECK_NULL_PTR_RE_FAIL(pIfmdTotalHardPara);
    PQ_CHECK_NULL_PTR_RE_FAIL(pstIfmdCalcInfo);

    if (HI_HAL_FMD_PQ_MCDI_SPLIT_MODE_5 == pstIfmdCalcInfo->enSplitMode)
    {
        for (i = 0; i < 5; i++)
        {
            p = (S_VPSSWB_REGS_TYPE *)((HI_ULONG)pstIfmdCalcInfo->pstIfmdHardOutReg + sizeof(S_STT_REGS_TYPE) * i);

            if (i == 0)
            {
                pIfmdTotalHardPara->stFmdRtlStatPara.frmHstBin.HISTOGRAM_BIN_1 = p->VPSS_PDHISTBIN[0].bits.hsit_bin * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.frmHstBin.HISTOGRAM_BIN_2 = p->VPSS_PDHISTBIN[1].bits.hsit_bin * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmHstBin.HISTOGRAM_BIN_3 = p->VPSS_PDHISTBIN[2].bits.hsit_bin * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmHstBin.HISTOGRAM_BIN_4 = p->VPSS_PDHISTBIN[3].bits.hsit_bin * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.frmITDiff           = p->VPSS_PDFRMITDIFF.bits.frm_it_diff * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc.PCC_BWD      = p->VPSS_PDPCCBWD0.bits.pcc_bwd_0 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc.PCC_FWD      = p->VPSS_PDPCCFWD0.bits.pcc_fwd_0 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc.PCC_FFWD     = p->VPSS_PDPCCFFWD0.bits.pcc_ffwd_0 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc.PCC_CRSS     = p->VPSS_PDPCCCRSS0.bits.pcc_crss_0 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc.PCC_FWD_TKR  = (p->VPSS_PDPCCFWDTKR0.bits.pcc_fwd_tkr_0 * u32RatioDiva / u32RatioDivb);

                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc.PCC_BWD_TKR  = p->VPSS_PDPCCBWDTKR0.bits.pcc_bwd_tkr_0 * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc2.PCC_BWD     = p->VPSS_PDPCCBWD1.bits.pcc_bwd_1 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc2.PCC_FWD     = p->VPSS_PDPCCFWD1.bits.pcc_fwd_1 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc2.PCC_FFWD    = p->VPSS_PDPCCFFWD1.bits.pcc_ffwd_1 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc2.PCC_CRSS    = p->VPSS_PDPCCCRSS1.bits.pcc_crss_1 * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc2.PCC_FWD_TKR = p->VPSS_PDPCCFWDTKR1.bits.pcc_fwd_tkr_1 * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.frmPcc2.PCC_BWD_TKR = p->VPSS_PDPCCBWDTKR1.bits.pcc_bwd_tkr_1 * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.frmUm.match_UM      = p->VPSS_PDUMMATCH0.bits.match_um * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmUm.nonmatch_UM   = p->VPSS_PDUMNOMATCH0.bits.nomatch_um * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmUm.match_UM2     = p->VPSS_PDUMMATCH1.bits.match_um * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.frmUm.nonmatch_UM2  = p->VPSS_PDUMNOMATCH1.bits.nomatch_um * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.lasiStat.lasiCnt14  = p->VPSS_PDLASICNT140.bits.lasi_14_cnt_0 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.lasiStat.lasiCnt32  = p->VPSS_PDLASICNT320.bits.lasi_32_cnt_0 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.lasiStat.lasiCnt34  = p->VPSS_PDLASICNT340.bits.lasi_34_cnt_0 * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.lasiStat2.lasiCnt14 = p->VPSS_PDLASICNT141.bits.lasi_14_cnt_1 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.lasiStat2.lasiCnt32 = p->VPSS_PDLASICNT321.bits.lasi_32_cnt_1 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stFmdRtlStatPara.lasiStat2.lasiCnt34 = p->VPSS_PDLASICNT341.bits.lasi_34_cnt_1 * u32RatioDiva / u32RatioDivb;

                pIfmdTotalHardPara->stFmdRtlStatPara.SceneChangeInfo.iCHD = p->VPSS_PDICHD.bits.ichd * u32RatioDiva / u32RatioDivb;

                for (j = 0; j < 16; j++)
                {
                    pIfmdTotalHardPara->stFmdRtlStatPara.StillBlkInfo.BlkSad[j] = p->VPSS_PDSTLBLKSAD[j].bits.stlblk_sad * u32RatioDiva / u32RatioDivb;
                }
                pIfmdTotalHardPara->stMadRtlStatPara.u32SmallMtnNum = p->iglb_mtn_num0.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32SmallMtnSum = p->iglb_mtn_sum0.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32MidMtnNum0  = p->iglb_mtn_num1.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32MidMtnSum0  = p->iglb_mtn_sum1.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32MidMtnNum1  = p->iglb_mtn_num2.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32MidMtnSum1  = p->iglb_mtn_sum2.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32MidMtnNum2  = p->iglb_mtn_num3.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32MidMtnSum2  = p->iglb_mtn_sum3.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32LargeMtnNum = p->iglb_mtn_num4.u32 * u32RatioDiva / u32RatioDivb;
                pIfmdTotalHardPara->stMadRtlStatPara.u32LargeMtnSum = p->iglb_mtn_sum4.u32 * u32RatioDiva / u32RatioDivb;
                ...
                PQ_SAFE_MEMCPY((HI_VOID *) & au32MotionHistTmp[0], \
                               (HI_VOID *) & (p->iglb_mtn_bin0.u32), sizeof(pIfmdTotalHardPara->stMadRtlStatPara.u32MotionHist));

                for (j = 0; j < 32; j++)
                {
                    pIfmdTotalHardPara->stMadRtlStatPara.u32MotionHist[j] = au32MotionHistTmp[j] * u32RatioDiva / u32RatioDivb;
                }
            }
            else
            {
    ...
    else
    {
        ...
        PQ_SAFE_MEMCPY((HI_VOID *) & pIfmdTotalHardPara->stMadRtlStatPara.u32MotionHist[0], \
                       (HI_VOID *) & (pstIfmdCalcInfo->pstIfmdHardOutReg->iglb_mtn_bin0.u32), sizeof(pIfmdTotalHardPara->stMadRtlStatPara.u32MotionHist));
    }
