...
typedef struct
{
    HI_S16 LRWAdjPixRange;          //s9,[-256,255];   default value:3
    HI_S16 LRWAdjPixRange10bit;     //s11,[-1024,1023]; default value:12
    HI_U8  DRDirBlendStr; //u4,[0,8];  default value:7;      (HI_U16)DM_Blend(CtrstDiffStr, MinAngCtrstStr, pDrParam->DRDirBlendStr, 8);
    HI_U16 alphaActHVrange;  //u15,[0,20460]; default value:4800
    HI_U16 alphaActHVratio;  //u11,[0,2047]; default value:109
    HI_U16 winActClipLimit; //u16,[0,40926]; default value:6400
    HI_U16 winActLimit;     //u16,[0,40926]; default value:3200
    HI_U16 winActRatio;     //u11,[0,2047]; default value:82
    HI_U16 AngStr1MinusFac; //u13,[0,8191]; default value:16
    HI_U8  DMCondAppDREn;   //u1,[0,1]; default value:1
    HI_U8  u8DRDMMergeMode;  //u3,[0,7]; default value:7
    HI_U8  u8DRDMBlendStr;   //u4,[0,15];  default value:8;  15:v500 DR alone; 0:s3 DM alone; (0,15): blenidng of DR in v500 & DM in S3;
} DR_API_REG_S;
...
typedef struct
{
    ...
    HI_U8 u8DirStrLut[21];    //u4,[0,15];    default value @ HI_S32 DM_InitReg(DM_INF_S *pInf) function in dm_api.c
    ...
    HI_U16 TransWid;   //u8,[0,255]; default value:30
    HI_U16 TransWid10bit;   //u10,[0,1023]; default value:120
    HI_U16 TransBand[31];  //u6,[0,63];  default value @ HI_S32 DM_InitReg(DM_INF_S *pInf) function in dm_api.c
    /* for test */
    HI_U16 AngCtrstPun[26];  //u5,[0,31];   default value @ HI_S32 DM_InitReg(DM_INF_S *pInf) function in dm_api.c
    /*add  96m2*/
    HI_U16 DirStrLun[26];  //u4,[0,31];   default value @ HI_S32 DM_InitReg(DM_INF_S *pInf) function in dm_api.c
