...
static HI_BOOL sg_bHdrProcessEn = HI_FALSE;
...
static HI_S32 PQ_MNG_ACMLUTInit(PQ_PARAM_S *pstPqParam, HI_BOOL bDefault)
{
    ...
    pstAcmModeRecmd = (void *)PQ_KMALLOC(sizeof(PQ_ACM_LUT_S), GFP_KERNEL);
...
/* Init Color */
HI_S32 PQ_MNG_InitColor(PQ_PARAM_S *pstPqParam, HI_BOOL bParaUseTableDefault)
{
    HI_S32 s32Ret = HI_FAILURE;
    HI_U32 u32ID = 0;
    ...
    s32Ret = PQ_MNG_ACMLUTInit(pstPqParam, bParaUseTableDefault);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    s32Ret = PQ_TABLE_InitPhyList(u32ID, HI_PQ_MODULE_COLOR, PQ_SOURCE_MODE_HD, PQ_OUTPUT_MODE_HD);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 PQ_MNG_SetColorGainLevel(HI_U32 u32GainLevel)
{
    ...
    //PQ_HAL_SetACMEn(VDP_LAYER_VP0, HI_TRUE);
...
HI_S32 PQ_MNG_SetACMGain(HI_VOID)
{
    ...
    PQ_CHECK_NUM_OVER_RANGE_RE_FAIL(enSourceMode, PQ_SOURCE_MODE_BUTT);
...
HI_S32 PQ_MNG_SetFleshToneLevel(HI_PQ_FLESHTONE_E enGainLevel)
{
    ...
    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstAcmCoef);
    pstColorParam = (void *)PQ_KMALLOC(sizeof(PQ_ACM_LUT_S), GFP_ATOMIC);
...
HI_S32 PQ_MNG_GetSixBaseColorLevel(HI_PQ_SIX_BASE_COLOR_S *pstSixBaseColorOffset)
{
...
HI_S32 PQ_MNG_SetSixBaseColorLevel(HI_PQ_SIX_BASE_COLOR_S *pstSixBaseColorOffset)
{
    HI_S32 s32Ret = HI_FAILURE;
...
HI_S32 PQ_MNG_GetACMLumaTbl(HI_PQ_COLOR_LUT_S *pstColorData)
{
    ...
    s32Ret = PQ_HAL_GetACMLumaTbl((COLOR_LUT_S *) pstColorData);
...
HI_S32 PQ_MNG_GetACMHueTbl(HI_PQ_COLOR_LUT_S *pstColorData)
{
    ...
    s32Ret =  PQ_HAL_GetACMHueTbl((COLOR_LUT_S *) pstColorData);
...
HI_S32 PQ_MNG_GetACMSatTbl(HI_PQ_COLOR_LUT_S *pstColorData)
{
    ...
    s32Ret = PQ_HAL_GetACMSatTbl((COLOR_LUT_S *) pstColorData);
...
HI_S32 PQ_MNG_SetAcmDemoEn(HI_BOOL bOnOff)
{
...
HI_S32 PQ_MNG_GetAcmDemoEn(HI_BOOL *pbOnOff)
{
...
HI_S32 PQ_MNG_SetAcmEn(HI_BOOL bOnOff)
{
    PQ_SOURCE_MODE_E enSourceMode = PQ_COMM_GetSourceMode();
    ...
    if (HI_TRUE == sg_bHdrProcessEn)
    {
        bOnOff = HI_FALSE;
    }

    PQ_HAL_SetACMEn(VDP_LAYER_VP0, bOnOff);
...
HI_S32 PQ_MNG_GetAcmEn(HI_U32 *pu32OnOff)
{
...
static HI_S32 PQ_MNG_GetHdrAcmCfg(HI_PQ_XDR_FRAME_INFO *pstXdrFrameInfo, HI_PQ_HDR_CFG *pstPqHdrCfg)
{
    ...
    /* sdr->sdr(not 2020) : use hdr adjust para */
    if ((HI_DRV_VIDEO_FRAME_TYPE_SDR == pstXdrFrameInfo->enSrcFrameType)
        && (HI_PQ_DISP_TYPE_NORMAL == pstXdrFrameInfo->enDispType))
    {
        sg_bHdrProcessEn = HI_FALSE;
        return HI_SUCCESS;
    }
    else
    {
        sg_bHdrProcessEn = HI_TRUE;
        s32Ret = PQ_MNG_SetAcmEn(HI_FALSE);
    }
...
static stPQAlgFuncs stACMFuncs =
{
    ...
    .SetEnable            = PQ_MNG_SetAcmEn,
    .GetEnable            = PQ_MNG_GetAcmEn,
    .SetDemo              = PQ_MNG_SetAcmDemoEn,
    .GetDemo              = PQ_MNG_GetAcmDemoEn,
