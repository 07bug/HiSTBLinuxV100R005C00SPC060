...
HI_S32 PQ_MNG_SetDCIWgtLut(HI_PQ_DCI_WGT_S *pstDciCoef)
{
    ...
    s32Ret = PQ_HAL_SetDCIWgtLut((DCI_WGT_S *)pstDciCoef);
...
HI_S32 PQ_MNG_GetDCIWgtLut(HI_PQ_DCI_WGT_S *pstDciCoef)
{
    ...
    s32Ret = PQ_HAL_GetDCIWgtLut((DCI_WGT_S *)pstDciCoef);
...
HI_S32 PQ_MNG_GetDCIHistgram(HI_PQ_DCI_HISTGRAM_S *pstDciHist)
{
    ...
    s32Ret = PQ_HAL_GetDCIHistgram((DCI_HISTGRAM_S *)pstDciHist);
...
HI_S32 PQ_MNG_SetDciBSCoef(HI_PQ_DCI_BS_LUT_S *pstDciCoef)
{
    ...
    memcpy(stBsLut.s16BSDelta, pstDciCoef->s16BSDelta, sizeof(stBsLut.s16BSDelta));

    s32Ret = PQ_HAL_SetDciBSCoef(&stBsLut);
...
HI_S32 PQ_MNG_GetDciBSCoef(HI_PQ_DCI_BS_LUT_S *pstDciCoef)
{
    ...
    memcpy(pstDciCoef->s16BSDelta, stBsLut.s16BSDelta, sizeof(pstDciCoef->s16BSDelta));
...
static HI_S32 PQ_MNG_DCILUTInit(HI_BOOL bDefault)
{
    ...
    memset(&stDciLut, 0, sizeof(stDciLut));
    memcpy(stDciLut.s16HistCoef0, HistCoef0, sizeof(stDciLut.s16HistCoef0));
    memcpy(stDciLut.s16HistCoef1, HistCoef1, sizeof(stDciLut.s16HistCoef1));
    memcpy(stDciLut.s16HistCoef2, HistCoef2, sizeof(stDciLut.s16HistCoef2));
    memcpy(stDciLut.s16YDivCoef , YdivCoef,  sizeof(stDciLut.s16YDivCoef ));
    memcpy(stDciLut.s32DciCfgReg, DciCfgReg, sizeof(stDciLut.s32DciCfgReg));

    if (HI_TRUE == bDefault)
    {
        memcpy(stDciLut.s16WgtCoef0, WgtCoef0, sizeof(stDciLut.s16WgtCoef0));
        memcpy(stDciLut.s16WgtCoef1, WgtCoef1, sizeof(stDciLut.s16WgtCoef1));
        memcpy(stDciLut.s16WgtCoef2, WgtCoef2, sizeof(stDciLut.s16WgtCoef2));

        memcpy(pstDciLut->s16WgtCoef0, WgtCoef0, sizeof(pstDciLut->s16WgtCoef0));
        memcpy(pstDciLut->s16WgtCoef1, WgtCoef1, sizeof(pstDciLut->s16WgtCoef1));
        memcpy(pstDciLut->s16WgtCoef2, WgtCoef2, sizeof(pstDciLut->s16WgtCoef2));

        pstDciLut->u16Gain0 = sg_u32DCILevelGain[PQ_SOURCE_MODE_HD][DCI_GLOBAL_GAIN_0];
        pstDciLut->u16Gain1 = sg_u32DCILevelGain[PQ_SOURCE_MODE_HD][DCI_GLOBAL_GAIN_1];
        pstDciLut->u16Gain2 = sg_u32DCILevelGain[PQ_SOURCE_MODE_HD][DCI_GLOBAL_GAIN_2];
    }
    else
    {
        memcpy(stDciLut.s16WgtCoef0, pstDciLut->s16WgtCoef0, sizeof(stDciLut.s16WgtCoef0));
        memcpy(stDciLut.s16WgtCoef1, pstDciLut->s16WgtCoef1, sizeof(stDciLut.s16WgtCoef1));
        memcpy(stDciLut.s16WgtCoef2, pstDciLut->s16WgtCoef2, sizeof(stDciLut.s16WgtCoef2));
    }

    s32Ret = PQ_HAL_SetDciLut(&stDciLut);

    PQ_HAL_SetDCIlevel(PQ_HAL_LAYER_VID0, sg_u32DCILevelGain[PQ_SOURCE_MODE_HD][DCI_GLOBAL_GAIN_0], \
                       sg_u32DCILevelGain[PQ_SOURCE_MODE_HD][DCI_GLOBAL_GAIN_1], \
                       sg_u32DCILevelGain[PQ_SOURCE_MODE_HD][DCI_GLOBAL_GAIN_2]);
    return s32Ret;
}
...
static HI_S32 PQ_MNG_DciBSLutInit(HI_BOOL bDefault)
{
    ...
    memset(&stDciBsLut, 0, sizeof(stDciBsLut));
    memcpy(stDciBsLut.s16VertOffset, VertOffset, sizeof(stDciBsLut.s16VertOffset));

    if (HI_TRUE == bDefault)
    {
        memcpy(stDciBsLut.s16BSDelta, BSDelta, sizeof(stDciBsLut.s16BSDelta));
        memcpy(pstDciBSLut->s16BSDelta, BSDelta, sizeof(pstDciBSLut->s16BSDelta));
    }
    else
    {
        memcpy(stDciBsLut.s16BSDelta, pstDciBSLut->s16BSDelta, sizeof(stDciBsLut.s16BSDelta));
    }
...
/* 初始化DCI模块 */
HI_S32 PQ_MNG_InitDCI(PQ_PARAM_S *pstPqParam, HI_BOOL bParaUseTableDefault)
{
    HI_S32 s32Ret = HI_FAILURE;
    HI_U32 u32ID = 0;
    ...
    s32Ret = PQ_MNG_DCILUTInit(bParaUseTableDefault);
    ...
    s32Ret = PQ_MNG_DciBSLutInit(bParaUseTableDefault);
    ...
    s32Ret = PQ_TABLE_InitPhyList(u32ID, HI_PQ_MODULE_DCI, PQ_SOURCE_MODE_HD, PQ_OUTPUT_MODE_HD);
...
HI_S32 PQ_MNG_SetDCILevelGain(HI_U32 u32DCILevelGain)
{
    ...
    if (enSourceMode >= PQ_SOURCE_MODE_BUTT)
    {
        enSourceMode = PQ_SOURCE_MODE_NO;
    }
...
{
    PQ_CHECK_NULL_PTR_RE_FAIL(pu32DCILevelGain);
...
{
    PQ_CHECK_NULL_PTR_RE_FAIL(pbSceneChange);
...
HI_S32 PQ_MNG_CalDCIScd(HI_VOID)
{
    ...
    if (sg_bCtrlSceneChange == HI_TRUE)
    {
        ...
        else
        {
            if (u32CurFrmNum >= 8)
            {
                ...
                if ((s32HistSAD[0] > DCI_SCD_SADThrsh) &&
                    (s32HistSAD[0] > ((s32HistSAD[1] + s32HistSAD[2] + s32HistSAD[3] + s32HistSAD[4] + s32HistSAD[5] + s32HistSAD[6]) / 6)))
                {
                    bSceneChange = HI_TRUE;
                    s32Count++;
                }
    ...
    PQ_HAL_UpdataDCIScd(PQ_HAL_LAYER_VID0, HI_TRUE);
...
HI_S32 PQ_MNG_SetDciDemoModeCoor(HI_U32 u32XPos)
{
    ...
    s32Ret = PQ_HAL_SetDCIDemoPos(PQ_VDP_LAYER_VID0, u32XPos);
