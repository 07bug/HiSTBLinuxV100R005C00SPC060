...
static ALG_DEI_S sg_stPQDEIInfo;
...
static HI_VOID PQ_MNG_InitSoftWareAlgParm(HI_Mc_DeiParam_S *mParm, HI_DEI_Variable_S *deiVariable)
{
    ...
    mParm->K_C_VERTW = 15;//8;   // [0,15]  //modify by sdk 2015.7.8
    ...
    */
    ...
    deiVariable->stINTERPINFO.MC_EDGE_CORING = 0; //256;
    deiVariable->stINTERPINFO.MC_EDGE_SCALE  = 6;//32; //64;
    ...
    g_pstDeiSwAlgPara.TH_L_VMVSUM   = 20;  //no register
    ...
    g_pstDeiSwAlgPara.s32GlbFrmIndex[0] = 0;
    g_pstDeiSwAlgPara.s32GlbFrmIndex[1] = 0;
    g_pstDeiSwAlgPara.s32GlbFrmIndex[2] = 0;
    g_pstDeiSwAlgPara.s32GlbFrmIndex[3] = 0;
}
...
HI_S32 PQ_MNG_DEI_ParaInit(HI_VOID)
{
    ...
    stDeiPara = (DEI_PARAMETER_S *)PQ_KMALLOC(sizeof(DEI_PARAMETER_S), GFP_KERNEL);
    PQ_CHECK_NULL_PTR_RE_FAIL(stDeiPara);
    ...
    /* global motion */
    stDeiPara->u32SmallmotionThd = sg_stPQDEIInfo.stMotionCalc.u32SmallMotionThd;
    stDeiPara->u32LargemotionThd = sg_stPQDEIInfo.stMotionCalc.u32LargeMotionThd;
    stDeiPara->bIgblEn = 1;
...
HI_S32 PQ_MNG_DEI_MotionParaInit(HI_VOID)
{
    ...
    sg_stPQDEIInfo.stMotionCalc.u32SmallMotionThd        = 16; /* 32; */
    sg_stPQDEIInfo.stMotionCalc.u32LargeMotionThd        = 192;/* 32; 256; */
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[0]   = 16; /* 32; */
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[1]   = 64; /* 48; */
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[2]   = 80; /* 96; 112; */
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[3]   = 144;/* 208; */
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[4]   = 255;
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[5]   = 255;
    sg_stPQDEIInfo.stMotionCalc.as16GlobalMotionSlope[0] = 8;
    sg_stPQDEIInfo.stMotionCalc.as16GlobalMotionSlope[1] = 0;  /* 4; */
    sg_stPQDEIInfo.stMotionCalc.as16GlobalMotionSlope[2] = 8;  /* 4; */
    sg_stPQDEIInfo.stMotionCalc.as16GlobalMotionSlope[3] = 0;
    sg_stPQDEIInfo.stMotionCalc.u16MaxGlobalMotionRatio  = 128;
    sg_stPQDEIInfo.stMotionCalc.u16MinGlobalMotionRatio  = 16;
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[0] = 16;
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[1] = ALG_CLIP3(0, 255, sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[0] +
            ((sg_stPQDEIInfo.stMotionCalc.as16GlobalMotionSlope[0] * (sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[1] - sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[0]) + HalfValue) >> Shift));
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[2] = ALG_CLIP3(0, 255, sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[1] +
            ((sg_stPQDEIInfo.stMotionCalc.as16GlobalMotionSlope[1] * (sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[2] - sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[1]) + HalfValue) >> Shift));
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[3] = ALG_CLIP3(0, 255, sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[2] +
            ((sg_stPQDEIInfo.stMotionCalc.as16GlobalMotionSlope[2] * (sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[3] - sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[2]) + HalfValue) >> Shift));
    sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[4] = ALG_CLIP3(0, 255, sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionRatio[3] +
            ((sg_stPQDEIInfo.stMotionCalc.as16GlobalMotionSlope[3] * (sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[4] - sg_stPQDEIInfo.stMotionCalc.au16GlobalMotionThd[3]) + HalfValue) >> Shift));
...
HI_S32 PQ_MNG_ALG_GetMcDeiParam(MOTION_INPUT_S *pstMotionInput)
{
    ...
    PQ_CHECK_NULL_PTR_RE_FAIL(pstMotionInput);
    PQ_CHECK_NULL_PTR_RE_FAIL(pstMotionInput->pstMotionReg);
    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstMcParm);
    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstDeiVar);
    ...
    au32MotionData[3] = pstSTTReg->iglb_mtn_num3.bits.iglb_mtn_num3;
    au32MotionData[4] = pstSTTReg->iglb_mtn_num4.bits.iglb_mtn_num4;
    ...
    au32MotionData[8] = pstSTTReg->iglb_mtn_sum3.bits.iglb_mtn_sum3;
    au32MotionData[9] = pstSTTReg->iglb_mtn_sum4.bits.iglb_mtn_sum4;
#else
    au32MotionData[0] = pstSTTReg->iglb_mtn_num0.bits.iglb_mtn_num0;
    au32MotionData[1] = pstSTTReg->iglb_mtn_num1.bits.iglb_mtn_num1;
    au32MotionData[2] = pstSTTReg->iglb_mtn_num2.bits.iglb_mtn_num2;
    ...
    PQ_MNG_ALG_RGMV_HVCHK(&nVmvSum, &nHmvSum, pstMotionInput->pRGMV, pstMotionInput->u32Width, \
                          pstMotionInput->u32Height, pstMotionInput->stride, pstMotionInput->enSplitMode);
    ...
    else
    {
        ...
        K_Y_MCW = 16;//64;/*TODO:0410 MCWEIGHT TOO LARGE*/
        K_C_MCW = 8;//64;/*TODO:0410 MCWEIGHT TOO LARGE*/
    }
    ...
    nK_FR_MCW = MIN2(16, MAX2(0, sg_stPQDEIInfo.s32FRCountMC - 60) * 48 / 8);
    ...
    sg_stPQDEIInfo.s32FRCountMC = (sg_stPQDEIInfo.s32FRCountMC == 63) ? sg_stPQDEIInfo.s32FRCountMC : (sg_stPQDEIInfo.s32FRCountMC + 1);

    /*TODO:solve first frm lasi problem*/
    /*1、得到FrmIndex，循环，由于特殊码流检测需要4帧时间，因此统计这个过程*/
    for (u32Cnt = 3; u32Cnt > 0; u32Cnt--)
    {
        g_pstDeiSwAlgPara.s32GlbFrmIndex[u32Cnt] = g_pstDeiSwAlgPara.s32GlbFrmIndex[u32Cnt - 1];
    }

    g_pstDeiSwAlgPara.s32GlbFrmIndex[0] = pstMotionInput->u32FrameIndex;

    if (pstMotionInput->u32Width <= 720 && pstMotionInput->u32Height <= 576 && \
        ((g_pstDeiSwAlgPara.s32GlbFrmIndex[3] == 0) && (g_pstDeiSwAlgPara.s32GlbFrmIndex[2] == 1) && \
         (g_pstDeiSwAlgPara.s32GlbFrmIndex[1] == 2) && (g_pstDeiSwAlgPara.s32GlbFrmIndex[0] == -3)) && \
        (sg_stPQDEIInfo.u32StreamId != 0x66) && (sg_stPQDEIInfo.u32StreamId != 0x67) && (sg_stPQDEIInfo.u32StreamId != 0x68) && (sg_stPQDEIInfo.u32StreamId != 0x69)
       )
    {
        sg_stPQDEIInfo.s32FRCountMC = 0;/*if seek or start, make cnt = 0*/
    }

    sg_pstMcParm->K_C_VERTW = 15 * sg_pstMcParm->K_Y_VERTW / 16; /* 15; 8; [0,15] */

    /* special para */
#ifdef PQ_YDJC_MCDI_SPLIT_DEFAULT
    if (pstMotionInput->u32Height == 576)
    {
        u32R0_MC   = 246;
    }
    else if (pstMotionInput->u32Height == 480)
    {
        u32R0_MC   = 192;
    }
    else if ((pstMotionInput->u32Height == 1080) && (HI_MNG_PQ_MCDI_SPLIT_MODE_5 == pstMotionInput->enSplitMode))
    {
        u32R0_MC   = 230;
    }
    else
    ...
    sg_pstDeiVar->stINTERPINFO.MC_EDGE_SCALE = 0;/*todo*/
#else
    u32R0_MC = 0;
    sg_pstDeiVar->stINTERPINFO.MC_EDGE_SCALE = 6;/*todo:keep org para*/
#endif

    PQ_HAL_SetSoftRegByReso(pstMotionInput->u32HandleNo, u32R0_MC);
...
HI_S32 PQ_MNG_SetDeiParam(HI_PQ_WBC_INFO_S *pstVpssWbcInfo)
{
    ...
    stMotionInput.enSplitMode  = pstVpssWbcInfo->enSplitMode;
    stMotionInput.u32FrameIndex = pstVpssWbcInfo->u32FrameIndex;

    //PQ_MNG_ALG_GetGlobalMotion(&stMotionInput);
    PQ_MNG_ALG_GetMcDeiParam(&stMotionInput);

    /* stream Id : 2 */
    if (0x68 == sg_stPQDEIInfo.u32StreamId)
    {
        PQ_HAL_SetDeiStreamId2(pstVpssWbcInfo->u32HandleNo);
    }
...
/* Init DEI */
HI_S32 PQ_MNG_InitDei(PQ_PARAM_S *pstPqParam, HI_BOOL bParaUseTableDefault)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32HandleNo = 0;

    if (HI_TRUE == sg_stPQDEIInfo.bInit)
    {
        return HI_SUCCESS;
    }

    sg_stPQDEIInfo.bEnable              = HI_TRUE;
    sg_stPQDEIInfo.s32FRCountMC         = 0;
    sg_stPQDEIInfo.bDemoEnable          = HI_FALSE;
    sg_stPQDEIInfo.eDemoMode            = PQ_DEMO_ENABLE_R;
    sg_stPQDEIInfo.u32StreamId          = 0;
    sg_stPQDEIInfo.u32StreamIdSetMode  = HI_TRUE; //Default Para On
    ...
    if (HI_SUCCESS != s32Ret)
    {
        ...
        sg_stPQDEIInfo.bInit = HI_FALSE;

        return HI_FAILURE;
    }

    sg_pstMcParm = (HI_Mc_DeiParam_S *)PQ_KMALLOC(sizeof(HI_Mc_DeiParam_S), GFP_KERNEL);
    PQ_CHECK_NULL_PTR_RE_FAIL(sg_pstMcParm);
    memset(sg_pstMcParm, 0, sizeof(HI_Mc_DeiParam_S));

    sg_pstDeiVar = (HI_DEI_Variable_S *)PQ_KMALLOC(sizeof(HI_DEI_Variable_S), GFP_KERNEL);
    if (HI_NULL == sg_pstDeiVar)
    {
        PQ_KFREE_SAFE(sg_pstMcParm);
    ...
    s32Ret = PQ_TABLE_InitPhyList(u32HandleNo, HI_PQ_MODULE_DEI, PQ_SOURCE_MODE_NO, PQ_OUTPUT_MODE_NO);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PQ("dei InitPhyList error\n");

        PQ_KFREE_SAFE(sg_pstMcParm);
        PQ_KFREE_SAFE(sg_pstDeiVar);
...
{
    if (HI_FALSE == sg_stPQDEIInfo.bInit)
    {
        return HI_SUCCESS;
    }
...
HI_S32 PQ_MNG_SetDeiDemoEn(HI_BOOL bOnOff)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32HandleNo;

    sg_stPQDEIInfo.bDemoEnable = bOnOff;
...
HI_S32 PQ_MNG_GetDeiDemoEn(HI_BOOL *pbOnOff)
{
    PQ_CHECK_NULL_PTR_RE_FAIL(pbOnOff);
    *pbOnOff = sg_stPQDEIInfo.bDemoEnable;
    return HI_SUCCESS;
}
...
HI_S32 PQ_MNG_SetDeiDemoMode(PQ_DEMO_MODE_E enMode)
{
    ...
    sg_stPQDEIInfo.eDemoMode = enMode;
...
HI_S32 PQ_MNG_GetDeiDemoMode(PQ_DEMO_MODE_E *penMode)
{
    ...
    *penMode = sg_stPQDEIInfo.eDemoMode;
    return HI_SUCCESS;
}
...
HI_S32 PQ_MNG_SetMotionCompensateEn(HI_BOOL bOnOff)
{
    ...
    PQ_HAL_SetMotionCompensateEn(u32HandleNo, bOnOff);
...
HI_S32 PQ_MNG_SetDeiStreamId(HI_U32 u32StreamId)
{
    sg_stPQDEIInfo.u32StreamId  = u32StreamId; //special para


    return HI_SUCCESS;
}

HI_S32 PQ_MNG_SetDeiStreamIdSetEnable(HI_U32 u32StreamIdSetMode)
{
    sg_stPQDEIInfo.u32StreamIdSetMode = u32StreamIdSetMode;
...
static stPQAlgFuncs stDeiFuncs =
{
    ...
    .SetDemo            = PQ_MNG_SetDeiDemoEn,
    .GetDemo            = PQ_MNG_GetDeiDemoEn,
    ...
    .SetStreamId        = PQ_MNG_SetDeiStreamId,
    .SetStreamSetON   = PQ_MNG_SetDeiStreamIdSetEnable,
};
