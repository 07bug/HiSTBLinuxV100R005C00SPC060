...
HI_S32 PQ_MNG_InitLCACM(PQ_PARAM_S *pstPqParam, HI_BOOL bParaUseTableDefault)
{
    ...
    s32Ret = PQ_HAL_InitLcacm(pstPqParam, bParaUseTableDefault);
    if (HI_SUCCESS != s32Ret)
    {
        sg_stPQLCACMInfo.bInit = HI_FALSE;
...
HI_S32 PQ_MNG_Tool_SetHdrAcmMode(HI_PQ_HDR_ACM_MODE_S *pstMode)
{
    HI_S32 s32Ret = HI_SUCCESS;

    PQ_CHECK_NULL_PTR_RE_FAIL(pstMode);

    s32Ret = PQ_HAL_Tool_SetHdrAcmMode(pstMode);

    return s32Ret;
}

HI_S32 PQ_MNG_Tool_GetHdrAcmMode(HI_PQ_HDR_ACM_MODE_S *pstMode)
{
    HI_S32 s32Ret = HI_SUCCESS;

    PQ_CHECK_NULL_PTR_RE_FAIL(pstMode);

    s32Ret = PQ_HAL_Tool_GetHdrAcmMode(pstMode);

    return s32Ret;
}

HI_S32 PQ_MNG_Tool_SetHdrAcmRegCfg(HI_PQ_HDR_ACM_REGCFG_S *pstCfg)
{
    HI_S32 s32Ret = HI_SUCCESS;

    PQ_CHECK_NULL_PTR_RE_FAIL(pstCfg);

    s32Ret = PQ_HAL_Tool_SetHdrAcmRegCfg(pstCfg);

    return s32Ret;
}

HI_S32 PQ_MNG_Tool_GetHdrAcmRegCfg(HI_PQ_HDR_ACM_REGCFG_S *pstCfg)
{
    HI_S32 s32Ret = HI_SUCCESS;

    PQ_CHECK_NULL_PTR_RE_FAIL(pstCfg);

    s32Ret = PQ_HAL_Tool_GetHdrAcmRegCfg(pstCfg);
