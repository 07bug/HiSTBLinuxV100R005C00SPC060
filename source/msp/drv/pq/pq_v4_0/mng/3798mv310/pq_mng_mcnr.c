...
static int Alpha_LUT[20][32] =    /* adjust for MCNR when demo on , when noise is very large in 98mv200*/
{
    ...
    /*  5 */ { 7,  8, 10, 11, 13, 16, 18, 21, 24, 26, 27, 28, 29, 30, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31}, // 5
    /*  6 */ { 4,  4,  5,  6,  9, 12, 14, 16, 18, 21, 24, 28, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31}, // 8
    /*  7 */ { 2,  3,  4,  5,  7,  9, 12, 15, 19, 24, 28, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31}, // 8
    /*  8 */ { 2,  3,  4,  4,  5,  7, 10, 14, 19, 24, 28, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31}, // 8
    /*  9 */ { 2,  3,  4,  4,  5,  7,  9, 12, 17, 22, 27, 29, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31}, // 8
...
static int Strong_Alpha_LUT[32] =
{
    4 ,  4,  4,  4,  5,  5,  5,  6,
    6 ,  7,  8,  9, 10, 11, 12, 13,
...
static HI_S32 pq_mng_MCNR_EstimationGlobalMotionMCME(MCNR_INPUT_INFO *pInfIn, HI_VOID *pRGMV, HI_U32 u32Stride)
{
...
#if defined(SPLITE_MCNR_ALG_ON)
    u32SplitCnt = SPLITE_MCNR_CNT;//(pInfIn->YWidth + PQ_MCNR_SPLIT_WTH - 1) / PQ_MCNR_SPLIT_WTH;
    u32FirstWth = 9 * PQ_RGMV_WTH * 2 + PQ_RGMV_WTH * 2; //640
    u32MidWth   = (u32SplitCnt > 2) ? (6 * PQ_RGMV_WTH * 2 + PQ_RGMV_WTH * 4) : 0; //512
    u32LastWth  = (u32SplitCnt > 1) ? ((pInfIn->YWidth - (u32SplitCnt - 2) * 6 * PQ_RGMV_WTH * 2  - 9 * PQ_RGMV_WTH * 2) + PQ_RGMV_WTH * 2) : 0;
    ...
    hRGNum = (pInfIn->YHeight + 9) / 16; // 这里的需要检视确认是否是+5/8 还是+9/16
    ...
    for (nRow = 0; nRow < hRGNum; nRow++)
    {
        for (nCol = 0; nCol < wRGNum; nCol++)
        {
            ...
            */
            ...
            else  //1920 分支, 分5块分支: 9+1, 1+6+1, 1+6+1, 1+9
            {
                u32RgmvStride = (nCol < 9) ? u32StrideFirst : ((nCol < 21) ? u32StrideMid : u32StrideLast);
                u32RgmvBlkOffset = (nCol < 9) ? 0 : 8;
                u32RgmvBlkIdx = (nCol < 9) ? (nCol % 9) : ((nCol < 21) ? ((nCol - 9) % 6) : ((nCol - 21) % 9));
                u32NodeAddrRmgvOffset = (((nCol < 9) ? 0 : 1) * hRGNum * u32StrideFirst + (((nCol / 6) > 1) ? (nCol / 6 - 1) : 0) * hRGNum * u32StrideMid) / 4;
    ...
    if (pInfIn->YWidth < 1920)
    {
        ...
        memcpy(g_McnrTimingInfo.Inf_ME_HMVCnt, Inf_ME_HMVCntTmp, sizeof(g_McnrTimingInfo.Inf_ME_HMVCnt));
        memcpy(g_McnrTimingInfo.Inf_ME_VMVCnt, Inf_ME_VMVCntTmp, sizeof(g_McnrTimingInfo.Inf_ME_VMVCnt));
    ...
    else
    {
        ...
        {
            PQ_CHECK_ZERO_RE_FAIL(meanPreGM);
...
HI_S32 pq_mng_MCND_getNoiseLevel(MCNR_INPUT_REG *pReg, MCNR_INPUT_INFO *pInfIn)
{
    ...
    if ((0 == pInfIn->YWidth) || (0 == pInfIn->YHeight))
    {
        flatRatio = 1;
        flatStr   = 1;
    }
    else if (pInfIn->bInterlace == 1)
    {
...
static HI_S32 pq_mng_MCNR_UpdateSNRRegisters(HI_U32 u32SceneChange, SNR_OUTPUT_REG *pRegOut)
{
    ...
    pRegOut->MotionAlpha = NR_CLIP3(8, 32, SNR_Motion_Ratio);
...
HI_S32 PQ_MNG_UpdateMCNRCfg(HI_PQ_WBC_INFO_S *pstVpssWbcInfo)
{
    ...
    s32Ret = pq_mng_MCNR_UpdateSoftWareCfg(u32HandleNo, pRGMV, u32Stride, \
                                           ...
                                           bInterlace, u32SceneChange, enSplitMode);
