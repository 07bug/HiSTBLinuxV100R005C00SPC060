...
HI_S32 PQ_MNG_InitDering(PQ_PARAM_S *pstPqParam, HI_BOOL bParaUseTableDefault)
{
    ...
    s32Ret = PQ_TABLE_InitPhyList(u32ID, HI_PQ_MODULE_DR, PQ_SOURCE_MODE_SD, PQ_OUTPUT_MODE_HD);
...
HI_S32 PQ_MNG_EnableDering(HI_BOOL bOnOff)
{
    ...
    if (REG_TYPE_VPSS == PQ_COMM_GetAlgTypeID(HI_PQ_MODULE_DR))
    {
        for (u32HandleNo = 0; u32HandleNo < VPSS_HANDLE_NUM; u32HandleNo++)
        {
            if (HI_SUCCESS == PQ_HAL_CheckVpssValid(u32HandleNo))
            {
                PQ_HAL_EnableDering(u32HandleNo, bOnOff);
            }
        }
    }
    else
    {
        PQ_HAL_EnableDering(u32Vid, bOnOff);
    }
