...
HI_S32 PQ_MNG_InitDeshoot(PQ_PARAM_S *pstPqParam, HI_BOOL bParaUseTableDefault)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32VID = 0;

    /* single DS */
    s32Ret = PQ_TABLE_InitPhyList(u32VID, HI_PQ_MODULE_DS, PQ_SOURCE_MODE_HD, PQ_OUTPUT_MODE_HD);
...
HI_S32 PQ_MNG_EnableDeshoot(HI_BOOL bOnOff)
{
    ...
    if (REG_TYPE_VPSS == PQ_COMM_GetAlgTypeID(HI_PQ_MODULE_DS))
    {
        for (u32HandleNo = 0; u32HandleNo < VPSS_HANDLE_NUM; u32HandleNo++)
        {
            if (HI_SUCCESS == PQ_HAL_CheckVpssValid(u32HandleNo))
            {
                PQ_HAL_EnableDeshoot(u32HandleNo, bOnOff);
            }
        }
    }
    else
    {
        PQ_HAL_EnableDeshoot(u32Vid, bOnOff);
    }
