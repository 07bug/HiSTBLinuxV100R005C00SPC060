...
HI_S32 PWM_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
	...
	memset(&stAttrCmd, 0, sizeof(stAttrCmd));
	
	if(HI_NULL == arg)
	{
		HI_FATAL_PWM("invalid para\n");
		return HI_FAILURE;
	}
    ...
    switch (cmd)
    {
        ...
        {
			if(copy_from_user(&stAttrCmd, (PWM_ATTR_CMD_PARA_S __user *)arg, sizeof(PWM_ATTR_CMD_PARA_S)))
			{
				HI_ERR_PWM("copy data from user fail\n");
				return HI_FAILURE;
			}

            Ret = PWM_GetAttr(stAttrCmd.enPwm, &stAttrCmd.stPwmAttr);

			if(copy_to_user((PWM_ATTR_CMD_PARA_S __user *)arg, &stAttrCmd, sizeof(PWM_ATTR_CMD_PARA_S)))
			{
				HI_ERR_PWM("copy data to user fail\n");
				return HI_FAILURE;
			}
...
static long PWM_DRV_Ioctl(struct file *ffile, unsigned int cmd, unsigned long arg)
{
    ...
    Ret = HI_DRV_UserCopy(ffile->f_path.dentry->d_inode, ffile, cmd, arg, PWM_Ioctl);
