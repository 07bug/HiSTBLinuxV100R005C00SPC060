...
static HI_U32 hsciIRQ[HI_UNF_SCI_PORT_BUTT];
...
HI_VOID SCI_InitSetupPara(HI_UNF_SCI_PORT_E enSciPort)
{
    ...
    g_SciPara[enSciPort].SciState.SciSetupPara.enRxParity = SCI_PARITY_ODD;
    g_SciPara[enSciPort].SciState.SciSetupPara.enTxParity = SCI_PARITY_ODD;
#else
    ...
    if (0 == g_SciPara[enSciPort].SciAttr.Frequency)
    {
...
HI_VOID SCI_DefaultConfig(HI_UNF_SCI_PORT_E enSciPort)
{
    ...
    ***************************/
    ...
    SCI_HAL_SetBlockTimeoutEnable(enSciPort, HI_TRUE);
...
HI_VOID SCI_CfgT0Para(HI_UNF_SCI_PORT_E enSciPort)
{
    SCI_SETUP_PARA_S      *pSciSetupPara;
...
HI_VOID SCI_RxTide(HI_UNF_SCI_PORT_E enSciPort)
{
    ...
    {
        HI_ERR_SCI("SCI parity failed.RegData:0x%x\n",RegData);
        if (g_SciPara[enSciPort].SciState.enSciCrtState == HI_UNF_SCI_STATUS_READATR)
        {
            HI_ERR_SCI("SCI Parity failed!\n");
#if defined(HI_ADVCA_TYPE_NAGRA)
            SCI_SetState(enSciPort, HI_UNF_SCI_STATUS_ATR_ERROR);
            return;
#endif
        }
    }
...
HI_S32 SCI_SetAtrByte(HI_UNF_SCI_PORT_E enSciPort, HI_U32 AtrMask)
{
...
#if defined(HI_ADVCA_TYPE_NAGRA)
        if ((SCI_ClkRate[g_SciPara[enSciPort].SciState.SciArtPara.enClkRate] == 0)||(SCI_BitRate[g_SciPara[enSciPort].SciState.SciArtPara.enBitRate] == 0))
        {
            HI_FATAL_SCI("Can not support this TA1 CLK %d Bit %d\n",g_SciPara[enSciPort].SciState.SciArtPara.enClkRate,g_SciPara[enSciPort].SciState.SciArtPara.enBitRate);
            return HI_FAILURE;
        }
        else if ((SCI_ClkRate[g_SciPara[enSciPort].SciState.SciArtPara.enClkRate] == 372)&&(SCI_BitRate[g_SciPara[enSciPort].SciState.SciArtPara.enBitRate] == 1))
        {
            HI_ERR_SCI("Set block 11\n");
            SCI_HAL_SetBlockGuard(enSciPort, SCI_DFT_BLKGUARD_TIME - 11);
        }
#endif
        g_SciPara[enSciPort].SciState.SciSetupPara.ClkRate =
    ...
    if (AtrMask & (0x1 << 4))
    {
        if (g_SciPara[enSciPort].SciState.SciArtPara.ChGuard < 255)
        {
            ...
            {
                SCI_HAL_SetCharGuard(enSciPort, g_SciPara[enSciPort].SciState.SciArtPara.ChGuard);
            }
        }
    }
...
HI_VOID SCI_AtrParamSet(HI_UNF_SCI_PORT_E enSciPort, HI_U8 AtrData)
{
    ...
    case 0xA1:        /*character A1*/
    {
        ...
        if (g_SciPara[enSciPort].SciState.SciArtPara.enClkRate)
        {
            ...
            if ((Fi > 0) && (Di > 0))
            {
                g_SciPara[enSciPort].SciState.SciArtPara.CharacterTime = SCI_DFT_CHAR_TIME * Di * 372 / Fi;
                g_SciPara[enSciPort].SciState.SciArtPara.BlockTime = SCI_DFT_BLOCK_TIME * Di * 372 / Fi;
            }
            else
            {
                g_SciPara[enSciPort].SciState.SciArtPara.CharacterTime = SCI_DFT_CHAR_TIME;
                g_SciPara[enSciPort].SciState.SciArtPara.BlockTime = SCI_DFT_BLOCK_TIME;
            }
            g_SciPara[enSciPort].SciState.SciSetupPara.CharTimeout =
            g_SciPara[enSciPort].SciState.SciArtPara.CharacterTime;
    ...
    {
        if (g_SciPara[enSciPort].SciState.enSciProtocol == HI_UNF_SCI_PROTOCOL_T1)
        {
            HI_U32 Value;
        ...
        if (g_SciPara[enSciPort].SciState.SciArtPara.enClkRate)
        {
            ...
            {
#if defined(HI_ADVCA_TYPE_NAGRA)
                 g_SciPara[enSciPort].SciState.SciArtPara.BlockTime = (1 << Value) * 960 * 372 * Di / Fi + 12;
#else
            ...
            {
#if defined(HI_ADVCA_TYPE_NAGRA)
                g_SciPara[enSciPort].SciState.SciArtPara.BlockTime = (1 << Value) * 960 + 12;
#else
        ...
        Value = AtrData & 0xf;
#if defined(HI_ADVCA_TYPE_NAGRA)
        g_SciPara[enSciPort].SciState.SciArtPara.CharacterTime = (1 << Value) + 12;
        g_SciPara[enSciPort].SciState.SciSetupPara.CharTimeout = (1 << Value) + 12;
#else
...
HI_S32 SCI_AtrProcessFormat(HI_UNF_SCI_PORT_E enSciPort, HI_U8 AtrData)
{
    ...
    if (5 == g_SciPara[enSciPort].SciState.SciArtPara.Current)
    {
        ...
        if ((0 == g_SciPara[enSciPort].SciState.SciArtPara.NumHist)
            && (HI_UNF_SCI_PROTOCOL_T0 == g_SciPara[enSciPort].SciState.enSciProtocol))
        {
...
HI_S32 SCI_AtrProcessEnd(HI_UNF_SCI_PORT_E enSciPort, HI_U8 AtrData)
{
    ...
    Ret = SCI_SetAtrByte(enSciPort, g_SciPara[enSciPort].SciState.SciArtPara.AtrMask);
    if (Ret == HI_SUCCESS)
    {
...
#if defined(HI_ADVCA_TYPE_NAGRA)
    if ((g_SciPara[enSciPort].SciState.enSciCrtState  ==  HI_UNF_SCI_STATUS_READY)&&(g_SciPara[enSciPort].SciState.SciArtPara.NumHist == 0))
    {
        HI_ERR_SCI("error ATR historybyte.AtrData:%#x,drop it!\n",AtrData);
        return 0;
    }

    if ((g_SciPara[enSciPort].SciState.enSciCrtState  ==  HI_UNF_SCI_STATUS_ATR_ERROR))
    {
...
#if (defined(CHIP_TYPE_hi3716mv410) && defined(HI_ADVCA_TYPE_NAGRA))   \
    || (defined(CHIP_TYPE_hi3716mv420) && defined(HI_ADVCA_TYPE_NAGRA))
    g_pstRegIO->ioshare_18.u32 = 0x131;
    g_pstRegIO->ioshare_19.u32 = 0x131;
    g_pstRegIO->ioshare_20.u32 = 0x131;
    g_pstRegIO->ioshare_21.u32 = 0x131;
    g_pstRegIO->ioshare_22.u32 = 0x171;
#endif

    memset(&stPortAttr, 0, sizeof(SCI_PORT_ATTR_S));
...
HI_S32 SCI_Open(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_PROTOCOL_E enSciProtocol, HI_U32 Frequency)
{
    ...
    SCI_PORT_ATTR_S stPortAttr;
    hsciIRQ[enSciPort] = (HI_U32)enSciPort;
    ...
    snprintf(s8Buff[enSciPort], sizeof(s8Buff[0]), "sci%d", enSciPort);

    Ret = osal_request_irq(stPortAttr.u32IrqNo, SCI_Isr, IRQF_SHARED, s8Buff[enSciPort], (HI_VOID*)&hsciIRQ[enSciPort]);
    if (Ret != HI_SUCCESS)
    {
    ...
    {
        if(HI_SUCCESS != HI_DRV_SYS_SetIrqAffinity(HI_ID_SCI, stPortAttr.u32IrqNo, s8Buff[enSciPort]))
        {
            HI_FATAL_SCI("HI_DRV_SYS_SetIrqAffinity failed.\n");
        }
    }

    SCI_HAL_EnableInt(enSciPort, (SCI_INT_CARDIN)|(SCI_INT_CARDOUT));
...
HI_S32 SCI_Close(HI_UNF_SCI_PORT_E enSciPort)
{
    ...
    osal_free_irq(stPortAttr.u32IrqNo, s8Buff[enSciPort], (HI_VOID*)&hsciIRQ[enSciPort]);
...
HI_S32 SCI_GetAtr(HI_UNF_SCI_PORT_E enSciPort, HI_U8 *pAtrBuf, HI_U32 AtrBufSize, HI_U8 *pAtrDataLen)
{
    ...
    if (HI_NULL == pAtrBuf || HI_NULL == pAtrDataLen)
    {
        HI_ERR_SCI("pAtrBuf or pAtrDataLen is NULL pointer\n");
        return HI_ERR_SCI_NULL_PTR;
    }

    memset(AtrTmpData, 0, SCI_ATR_BUF_LEN*sizeof(HI_U8));
    Ret = down_interruptible(&g_SciPara[enSciPort].SciSem);
...
HI_S32 SCI_GetStatus(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_STATUS_E *penState)
{
    ...
    if(HI_NULL == penState)
    {
         HI_ERR_SCI("penState is NULL pointer!\n");
         return HI_ERR_SCI_NULL_PTR;
    }
    Ret = down_interruptible(&g_SciPara[enSciPort].SciSem);
...
HI_S32 SCI_PPS_GetRecBuf(HI_UNF_SCI_PORT_E enSciPort, HI_U8* pRecBuf, HI_U32* pDataLen)
{
    ...
    if(HI_NULL == pRecBuf)
    {
        HI_ERR_SCI("pRecBuf is NULL pointer!\n");
        return HI_ERR_SCI_NULL_PTR;
    }
...
HI_S32 SCI_ConfVcc(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_LEVEL_E enSciLevel)
{
    ...
    if (enSciLevel >= HI_UNF_SCI_LEVEL_BUTT)
    {
            HI_ERR_SCI("enSciLevel(%d) is invalid para\n", enSciLevel);
            return HI_ERR_SCI_INVALID_PARA;
    }
...
HI_S32 SCI_ConfClkMode(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_MODE_E enClkMode)
{
    ...
    {
        HI_ERR_SCI("enClkMode(%d) is invalid para\n", enClkMode);
        return HI_ERR_SCI_INVALID_PARA;
    }
...
HI_S32 SCI_SendData(HI_UNF_SCI_PORT_E enSciPort, HI_U8 *pDataBuf, HI_U32 BufSize, HI_U32 *pDataLen, HI_U32 TimeoutMs)
{
    ...
    if (BufSize > sizeof(g_SciDataBuf[0].DataBuf))
    {
        HI_ERR_SCI("BufSize is to large\n");
        return HI_FAILURE;
    }
    Ret = down_interruptible(&g_SciPara[enSciPort].SciSem);
...
HI_S32 SCI_SendData(HI_UNF_SCI_PORT_E enSciPort, HI_U8 *pDataBuf, HI_U32 BufSize, HI_U32 *pDataLen, HI_U32 TimeoutMs)
{
    ...
    if (BufSize > SCI_READ_BUF_LEN)
    {
        HI_ERR_SCI("Bufsize is too large\n");
        return HI_ERR_SCI_INVALID_PARA;
    }

    if (HI_NULL == pDataBuf)
    {
        HI_ERR_SCI("pDataBuf is NULL\n");
        return HI_ERR_SCI_NULL_PTR;
    }
...
HI_S32 SCI_ReceiveData(HI_UNF_SCI_PORT_E enSciPort, HI_U8 *pDataBuf, HI_U32 BufSize, HI_U32 *pDataLen, HI_U32 TimeoutMs)
{
    ...
    {
        HI_ERR_SCI("pDataBuf or pDataLen is NULL\n");
        return HI_ERR_SCI_NULL_PTR;
    }

    if (BufSize > SCI_READ_BUF_LEN)
    {
        HI_ERR_SCI("BufSize if too long\n");
        return HI_ERR_SCI_INVALID_PARA;
    }
...
#if defined(HI_ADVCA_TYPE_NAGRA)
            if (g_SciPara[enSciPort].ErrType == SCI_INT_BLKTOUT)
            {
...
HI_S32 SCI_Switch(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_PROTOCOL_E enSciProtocol, HI_U32 Frequency)
{
    ...
    {
        HI_ERR_SCI("enSciProtocol(%d) is invalid para\n", enSciProtocol);
        return HI_ERR_SCI_INVALID_PARA;
    }
...
HI_S32 SCI_ExtSetBaud( HI_UNF_SCI_PORT_E enSciPort, HI_U32 ClkRate, HI_U32 BitRate)
{
    ...
    CHECK_PORTID();
     ...
     if ((ClkRate < 372) || (ClkRate > 2048))
    {
        HI_ERR_SCI("para u32ClkRate is invalid.\n");
        return HI_ERR_SCI_INVALID_PARA;
    }

    if ((BitRate < 1) || (BitRate > 32) || ((1 != BitRate) && (BitRate % 2) != 0))
    {
...
irqreturn_t SCI_Isr(HI_S32 irq, HI_VOID *dev_id)
{
    ...
    HI_U8 ii = 0;
    SCI_PORT_ATTR_S stPortAttr;
    HI_UNF_SCI_PORT_E enSciPort = *(HI_UNF_SCI_PORT_E *)dev_id;
    ...
    while (IntState)
    {
        ...
        if (IntState & SCI_INT_BLKTOUT)
        {

            SCI_HAL_ClearInt(enSciPort, SCI_INT_BLKTOUT);
#if defined(HI_ADVCA_TYPE_NAGRA)
            g_SciPara[enSciPort].ErrType = SCI_INT_BLKTOUT;
#endif
            HI_ERR_SCI("sci%d block transfer timeout.\n", enSciPort);
