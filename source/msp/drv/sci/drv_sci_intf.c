...
HI_S32 SCI_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    memset(&SciAtrBuf,0,sizeof(ATR_BUF_S));
    memset(&SciPara,0,sizeof(SCI_PARA_S));
...
HI_S32 SCI_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
    ...
    if (HI_NULL == arg)
    {
	    HI_ERR_SCI("invalid para\n");
	    return -EAGAIN;
    }

    if (down_interruptible(&g_SciMutex))
    {
        return -EAGAIN;
    }
...
static HI_S32 SCI_DRV_Open(struct inode *inode, struct file *filp)
{
    ...
    memset(pSciDevState, 0, sizeof(SCI_DEV_STATE_S));
...
static long SCI_DRV_Ioctl(struct file *ffile, unsigned int cmd, unsigned long arg)
{
    ...
    Ret = (long)HI_DRV_UserCopy(ffile->f_path.dentry->d_inode, ffile, cmd, arg, SCI_Ioctl);
...
static long SCI_DRV_Compat_Ioctl(struct file *ffile, unsigned int cmd, unsigned long arg)
{
    ...
    Ret = (long)HI_DRV_UserCopy(ffile->f_path.dentry->d_inode, ffile, cmd, arg, SCI_Compat_Ioctl);
