...
HI_S32 SPI_Write(HI_SIZE_T arg)
{
    HI_U8  *pData = NULL;
    SPI_DATA_S SSPData;
    SPI_DATAEX_S __user *pSSPData = (SPI_DATAEX_S __user *)arg;
...
HI_S32 SPI_Read(HI_SIZE_T arg)
{
    ...
    SPI_DATA_S __user *pSSPData = (SPI_DATA_S __user *)arg;
    if (HI_NULL == pSSPData) {
		HI_ERR_SPI("invalid para\n");
		return HI_ERR_SPI_COPY_DATA_ERR;
    }

    memset(&SSPData, 0, sizeof(SSPData));
    if (copy_from_user(&SSPData, pSSPData, sizeof(SPI_DATA_S))) {
        HI_INFO_SPI("ssp read copy data from user fail!\n");
        return HI_ERR_SPI_COPY_DATA_ERR;
    }
...
HI_S32 SPI_ReadEx(HI_SIZE_T arg)
{
    ...
    SPI_DATAEX_S __user *pEXData = (SPI_DATAEX_S __user *)arg;
    if(HI_NULL == pEXData) {
		HI_ERR_SPI("invalid para\n");
		return HI_ERR_SPI_COPY_DATA_ERR;
    }
...
HI_S32 HI_DRV_SPI_Ioctl(struct file *file, HI_U32 cmd, HI_SIZE_T arg)
{
    ...
    memset(&SSPFForm, 0, sizeof(SSPFForm));
    memset(&SSPBLEND, 0, sizeof(SSPBLEND));
    memset(&SSPLOOP, 0, sizeof(SSPLOOP));
    ...
    case  CMD_SPI_SET_ATTR:
        ...
        if (copy_from_user(&SSPFForm, (SPI_FFORM_S __user *)arg, sizeof(SPI_FFORM_S))) {
    ...
    case  CMD_SPI_GET_ATTR:
        ...
        error = HI_DRV_SPI_GetFrom(SSPFForm.devId,&(SSPFForm.mode),&(SSPFForm.spo),
           &(SSPFForm.sph),&(SSPFForm.dss));
        if (HI_SUCCESS == error) {
            if (copy_to_user((SPI_FFORM_S __user *)arg, &SSPFForm,sizeof(SPI_FFORM_S))) {
    ...
    case CMD_SPI_SET_BLEND:
        ...
        if (copy_from_user(&SSPBLEND, (SPI_BLEND_S __user *)arg, sizeof(SPI_BLEND_S))) {
    ...
    case CMD_SPI_GET_BLEND:
        ...
        if (copy_to_user((SPI_BLEND_S __user *)arg, &SSPBLEND,sizeof(SPI_BLEND_S))) {
    ...
    case CMD_SPI_SET_LOOP:
        ...
        if (copy_from_user(&SSPLOOP, (SPI_LOOP_S __user *)arg, sizeof(SPI_LOOP_S))) {
