...
HI_BOOL SYNC_CheckAdjustFinish(SYNC_S *pSync, SYNC_CHAN_E enChn, HI_S32 AudPcrDiff, HI_S32 VidPcrDiff)
{
    HI_S32 PcrLagThreshold = 0;
    HI_S32 PcrLeadThreshold = 0;
...
{
    HI_BOOL bAdjustOk = HI_FALSE;

    //adjust ok
    bAdjustOk = SYNC_CheckAdjustFinish(pSync, enChn, AudPcrDiff, VidPcrDiff);
    if (HI_TRUE == bAdjustOk)
    {
        return;
    }
    ...
    else if (SYNC_CHAN_VID == enChn)
    {
        ...
        if (abs(VidPcrDiff) < PCR_LAG_STOP_THRESHOLD)
        {
            ...
            if ((HI_TRUE == pSync->PcrSyncInfo.PcrAudSyncOK)||(abs(pSync->PcrSyncInfo.AudPcrDiff) < PCR_LEAD_ADJUST_THRESHOLD))
            {
    ...
    if (SYNC_BUF_STATE_HIGH != pSync->CrtBufStatus.VidBufState)
    {
        if (VidAudDiff > pSync->SyncAttr.stSyncNovelRegion.s32VidPlusTime)
        {
            if (pSync->SyncAttr.stSyncNovelRegion.bSmoothPlay || pSync->VidFirstPts == HI_INVALID_PTS)
            {
                ...
                if (SYNC_CHAN_VID == enChn)
                {
                    // TODO:sdk usage of VidPlayCnt VidDiscardCnt VidRepeatCnt
                    pSync->VidRepPlayCnt++;
                }
            }
            else
...
HI_BOOL SYNC_PcrAdjustStartRegionOK(SYNC_S *pSync, SYNC_CHAN_E enChn, HI_S32 AudPcrDiff, HI_S32 VidPcrDiff)
{
    HI_S32 PcrLagThreshold,PcrLeadThreshold;

    if (((pSync->AudFirstCome)&&(!pSync->VidFirstCome))||((!pSync->AudFirstCome)&&(pSync->VidFirstCome))) //if only aud or vid is exist,make sure audpcrdiff or vidpcrdiff in -100~100.
    {
        PcrLagThreshold = PCR_LAG_STOP_THRESHOLD;
        PcrLeadThreshold = PCR_LEAD_STOP_THRESHOLD;
    }
...
HI_VOID SYNC_PcrSyncStartRegionAdjust(SYNC_S *pSync, SYNC_CHAN_E enChn, HI_S32 AudPcrDiff, HI_S32 VidPcrDiff)
{
    if (!pSync->VidSyndAdjust)
    {
        if (HI_TRUE == SYNC_PcrAdjustStartRegionOK(pSync,enChn,AudPcrDiff,VidPcrDiff))
        {
           return;
        }
    }
...
HI_VOID SYNC_VIDProcessBack(SYNC_S *pSync, SYNC_VID_INFO_S *pVidInfo, SYNC_VID_OPT_S *pVidOpt)
{
    if ((HI_FALSE == pSync->bMs12Flag)||(HI_UNF_SYNC_REF_PCR != SYNC_GetSyncAdjustRefMode(pSync)))
    {
        return;
    }

    if (HI_FALSE == pSync->bVidBackFlag)
    {
        if (HI_TRUE == pSync->bPcrBackFlag)  //If pcr is loopback, make vid play freely.
        {
    ...
    {
        if(HI_FALSE == pSync->bPcrBackFlag)
        {
        ...
        if(HI_FALSE == pSync->bAudBackFlag)
        {
            pVidOpt->SyncProc = SYNC_PROC_CONTINUE;
            HI_WARN_SYNC("vid repeat1 pts %d dl %d af %d pf %d\n",pVidInfo->Pts,pVidInfo->DelayTime,pSync->bAudBackFlag,pSync->bPcrBackFlag);
        }
        else
        {
            if (pSync->VidAudDiff < pSync->SyncAttr.stSyncStartRegion.s32VidNegativeTime)
            {
                HI_WARN_SYNC("vid discard1 diff %d pts %d vl %d dl %d\n",pSync->VidAudDiff,pVidInfo->Pts,SYNC_GetLocalTime(pSync, SYNC_CHAN_VID),pVidInfo->DelayTime);
                pVidOpt->SyncProc = SYNC_PROC_DISCARD;
            }
            else if (pSync->VidAudDiff > pSync->SyncAttr.stSyncStartRegion.s32VidPlusTime)
            {
                HI_WARN_SYNC("vid discard1 diff %d pts %d vl %d dl %d\n",pSync->VidAudDiff,pVidInfo->Pts,SYNC_GetLocalTime(pSync, SYNC_CHAN_VID),pVidInfo->DelayTime);
                pVidOpt->SyncProc = SYNC_PROC_REPEAT;
            }
            else
            {
                HI_WARN_SYNC("vid continue diff %d pts %d vl %d dl %d\n",pSync->VidAudDiff,pVidInfo->Pts,SYNC_GetLocalTime(pSync, SYNC_CHAN_VID),pVidInfo->DelayTime);
                pVidOpt->SyncProc = SYNC_PROC_CONTINUE;
            }
        }
        if (pSync->CrtBufStatus.VidBufState == SYNC_BUF_STATE_HIGH)
        {
            pVidOpt->SyncProc = SYNC_PROC_PLAY;
            pSync->bAudBackFlag = HI_FALSE;
            pSync->bPcrBackFlag = HI_FALSE;
            pSync->bVidBackFlag = HI_FALSE;
            HI_WARN_SYNC("+++VID ap:%d,vp:%d\n",pSync->PcrSyncInfo.AudPcrDiff,pSync->PcrSyncInfo.VidPcrDiff);
            pSync->PcrSyncInfo.PcrDelta = (pSync->VidAudDiff > 0) ? pSync->PcrSyncInfo.AudPcrDiff : pSync->PcrSyncInfo.VidPcrDiff;
        }
    }
    return;
...
HI_VOID SYNC_VidProc(HI_HANDLE hSync, SYNC_VID_INFO_S *pVidInfo, SYNC_VID_OPT_S *pVidOpt)
{
    ...
    if ((HI_UNF_SYNC_REF_PCR == pSync->SyncAttr.enSyncRef)&&(pSync->VidLastPts > pVidInfo->Pts)&&(pSync->VidLastPts - pVidInfo->Pts > 10000)&&(pSync->VidLastPts != -1)&&(pVidInfo->Pts != -1))
    {
        HI_WARN_SYNC("==========We think vid back!lp %d p %d\n",pSync->VidLastPts,pVidInfo->Pts);
    ...
    *pVidOpt = pSync->VidOpt;
    SYNC_VIDProcessBack(pSync,pVidInfo,pVidOpt);
...
HI_VOID SYNC_AudProcessBack(SYNC_S *pSync, SYNC_AUD_INFO_S *pAudInfo, SYNC_AUD_OPT_S *pAudOpt)
{
    HI_U32 Curtime = 0;
    if ((HI_FALSE == pSync->bMs12Flag)||(HI_UNF_SYNC_REF_PCR != SYNC_GetSyncAdjustRefMode(pSync)))
    {
    ...
    if (HI_FALSE == pSync->bAudBackFlag)
    {
        if (HI_TRUE == pSync->bPcrBackFlag)   //If pcr is loopback,make audio play freely.
        {
    ...
    {
        if((HI_FALSE == pSync->bVidBackFlag)||(HI_FALSE == pSync->bPcrBackFlag)) //aud or pcr is not loopback, waiting...
        {
            HI_WARN_SYNC("aud repeat1\n");
            pAudOpt->SyncProc = SYNC_PROC_REPEAT;
        }
        else
        {
            if (pSync->AudFundBeginTime == HI_INVALID_TIME)
            {
                pSync->AudFundBeginTime = SYNC_GetSysTime();
            }

            if ((abs(pAudInfo->Pts - pSync->PcrSyncInfo.PcrLast) > 10000)&&(pAudInfo->Pts > pSync->PcrSyncInfo.PcrLast))
            {
                HI_WARN_SYNC("Drop redundant aud data. pts %d\n",pAudInfo->Pts);  //if aud is loopback,drop old data..
                pAudOpt->SyncProc = SYNC_PROC_DISCARD;
                return;
            }
            pAudOpt->SyncProc = SYNC_PROC_REPEAT; //In order to save new audio data, remain all new data until reach waterline.
        }

        Curtime = SYNC_GetSysTime();
        if ((pSync->CrtBufStatus.AudBufState >= SYNC_BUF_STATE_NORMAL)||((pSync->AudFundBeginTime != HI_INVALID_TIME)&&(Curtime - pSync->AudFundBeginTime > 1000))) //Make sure audio is enough when start playing.
        {
            HI_U32 PcrLoacalTime = SYNC_GetLocalTime(pSync, SYNC_CHAN_PCR);
            pAudOpt->SyncProc = SYNC_PROC_REPEAT;
            pSync->bAudBackFlag = HI_FALSE;
            pSync->bPcrBackFlag = HI_FALSE;
            pSync->bVidBackFlag = HI_FALSE;
            HI_WARN_SYNC("++++++++++++++AUD exit,ap:%d,vp:%d,dlt %d ct %d fb %d AudState=%d\n",pSync->PcrSyncInfo.AudPcrDiff,pSync->PcrSyncInfo.VidPcrDiff,pSync->PcrSyncInfo.PcrDelta,Curtime,pSync->AudFundBeginTime, pSync->CrtBufStatus.AudBufState);
            pSync->AudFundBeginTime = HI_INVALID_TIME;
            pSync->PcrSyncInfo.PcrDelta += pSync->PcrSyncInfo.AudPcrDiff;
            SYNC_SetLocalTime(pSync,SYNC_CHAN_PCR,PcrLoacalTime+pSync->PcrSyncInfo.PcrDelta);
        }
    }
    return;
}
...
HI_VOID SYNC_AudProc(HI_HANDLE hSync, SYNC_AUD_INFO_S *pAudInfo, SYNC_AUD_OPT_S *pAudOpt)
{
    ...
    *pAudOpt = pSync->AudOpt;
    ...
    SYNC_AudProcessBack(pSync,pAudInfo,pAudOpt); //check audio is loopback or not.
...
HI_VOID SYNC_PcrProc(HI_HANDLE hSync, HI_U64 PcrValue, HI_U64 StcValue)
{
    ...
    */
    ...
    do_div(PcrValueConvert, 300);
    PcrTime = (HI_U32) ((PcrValueConvert >> 1) & 0xffffffff);
    PcrTime = PcrTime / 45;
    ...
    {
        if (pSync->PcrSyncInfo.PcrLast - PcrTime> 10000) //Check pcr is loopback or not.
        {
            HI_WARN_SYNC("==========We think PCR back!\n");
    ...
    if (pSync->PcrSyncInfo.PcrSeriesCnt >= PTS_SERIES_COUNT)
    {
        ...
        if (SYNC_PCR_ADJUST_SCR == pSync->PcrSyncInfo.enPcrAdjust)
        {
            ...
            if(abs(PcrLocalTime - PcrTime - pSync->PcrSyncInfo.PcrDelta ) < 300)  //keep pcr uniformly continuous when Network jitter occurs.
            {
               pSync->PcrSyncInfo.PcrAdjustDeltaOK = HI_TRUE;
               return;
            }
