...
static HI_S32 SYNC_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    {
        PROC_PRINT(p,"__________Hisilicon STC________________\n");
        PROC_PRINT(p,"StcInitFlag                 :%-10d\n"
                          "StcAdjustFail               :%-10d\n"
                          "StcAdjustRefer              :%-10d\n"
                          "PcrStcDiff                  :%-10d\n",

                        pSync->PcrSyncInfo.StcInfo.StcAdjustInitFlag,
                        pSync->PcrSyncInfo.StcInfo.StcAdjustFail,
                        pSync->PcrSyncInfo.StcAdjustReferFlag,
                        pSync->PcrSyncInfo.PpmInfo.PpmValid
                        );
    }
#endif
...
static HI_S32 SYNC_ProcWrite(struct file * file, const char __user * buf, size_t count, loff_t *ppos)
{
    SYNC_S            *pSync;
...
static long SYNC_COMPAT_DRV_Ioctl(struct file *ffile, unsigned int cmd, unsigned long arg)
{
    ...
    Ret = HI_DRV_UserCopy(ffile->f_path.dentry->d_inode, ffile, cmd, arg, SYNC_COMPAT_Ioctl);
...
#ifdef HI_SYNC_PROC_SUPPORT
    if (HI_SUCCESS != SYNC_IntfRegister(&SYNC_ProcPara))
    {
