...
HI_S32 SYNC_CheckId(SYNC_USR_ADDR_S *pSyncUsrAddr, struct file *file)
{
    ...
    pSyncUsrAddr->SyncPhyAddr = g_SyncGlobalState.SyncInfo[pSyncUsrAddr->SyncId].SyncPhyAddr;
    ...
    switch (cmd)
    {
        ...
        case CMD_SYNC_COMPAT_CHECK_ID:
        {
            ...
            if (HI_SUCCESS == Ret)
            {
                ...
                pSyncCompatUsrAddr->SyncPhyAddr = SyncUsrAddr.SyncPhyAddr;
            }
...
static HI_VOID DRV_SYNC_ResetStatInfo(SYNC_S *pSync, SYNC_CHAN_E enChn)
{
    ...
    if ((!pSync->VidEnable) && (!pSync->AudEnable))
    {
        ...
        pSync->PcrSyncInfo.StcInfo.StcAdjustFail     = HI_FALSE;
        pSync->PcrSyncInfo.StcInfo.StcAdjustInitFlag = HI_FALSE;

        pSync->bPcrBackFlag = HI_FALSE;
        pSync->bVidBackFlag = HI_FALSE;
        pSync->bAudBackFlag = HI_FALSE;
        pSync->bMs12Flag = HI_FALSE;
        ...
        pSync->bExternalPllAdjust = HI_FALSE;
    }
...
HI_S32 HI_DRV_SYNC_AudJudge(HI_HANDLE hSync, SYNC_AUD_INFO_S *pAudInfo, SYNC_AUD_OPT_S *pAudOpt)
{
    ...
    if ((HI_NULL == pAudInfo) || (HI_NULL == pAudOpt))
    {
        HI_ERR_SYNC("ERR: ptr is null!\n");
        return HI_FAILURE;
    }
...
HI_S32 HI_DRV_SYNC_VidJudge(HI_HANDLE hSync, SYNC_VID_INFO_S *pVidInfo, SYNC_VID_OPT_S *pVidOpt)
{
    ...
    if ((HI_NULL == pVidInfo) || (HI_NULL == pVidOpt))
    {
        HI_ERR_SYNC("ERR: ptr is null!\n");
        return HI_FAILURE;
    }
