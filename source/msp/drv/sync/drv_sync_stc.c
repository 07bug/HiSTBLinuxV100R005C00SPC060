...
static HI_U32 g_EpllSrcValueInt  = 0;
static HI_U32 g_EpllSrcValueFrac = 0;
#endif
...
HI_S32 SYNC_PllTuneFreqCrgInit(HI_VOID)
{
    ...
    g_EpllSrcValueInt  = CrgFbdiv;
    g_EpllSrcValueFrac = CrgFrac;
#endif
...
#ifdef  AIAO_SRC_PLL_HAS_CREECTED
        CrgFbdiv = g_EpllSrcValueInt & (0xfff);
        CrgFrac = g_EpllSrcValueFrac & (0xffffff);
#else
...
HI_VOID SYNC_VpllTuneFreq(HI_U32 VpllFbdiv, HI_U32 VpllFrac)
{
    ...
    {
        udelay(10);
        if (!g_pstRegCrg->PERI_CRG179.bits.vpll_tune_busy)
        {
            break;
        }
    ...
    if (Count >= 20)
    {
        HI_ERR_SYNC("wait for tun vpll overtime!\n");
    }

    g_pstRegCrg->PERI_CRG144.u32 = 0x1;
}
...
HI_VOID SYNC_EpllTuneFreq(HI_U32 EpllFbdiv, HI_U32 EpllFrac)
{
    ...
    {
        udelay(10);
        if (!g_pstRegCrg->PERI_CRG171.bits.epll_tune_busy)
        {
            break;
        }
    ...
    if (Count >= 20)
    {
        HI_ERR_SYNC("wait for tun epll overtime!\n");
    }

    g_pstRegCrg->PERI_CRG124.u32 = 0x1;
}
...
#ifdef AIAO_SRC_PLL_HAS_CREECTED
    EpllFbdiv = g_EpllSrcValueInt & (0xfff);
    EpllFrac = g_EpllSrcValueFrac & (0xffffff);
#else
...
HI_VOID SYNC_HpllTuneFreq(HI_U32 HpllFbdiv, HI_U32 HpllFrac)
{
    ...
    {
        udelay(10);
        if (!g_pstRegCrg->PERI_CRG177.bits.hpll_tune_busy)
        {
            break;
        }
    ...
    if (Count >= 20)
    {
        HI_ERR_SYNC("wait for tun hpll overtime!\n");
    }

    g_pstRegCrg->PERI_CRG139.u32 = 0x1;
}
...
{
    HI_U32 VpllTurneBusy = 0;
    HI_U32 EpllTurneBusy = 0;
    HI_U32 HpllTurneBusy = 0;
    HI_S32 VpllAdjust = 0;
    HI_S32 EpllAdjust = 0;
    HI_S32 HpllAdjust = 0;
    ...
    HI_U32 EpllTargetFbdiv = 0;
    HI_U32 EpllTargetFrac  = 0;
    HI_U32 HpllTargetFbdiv = 0;
    HI_U32 HpllTargetFrac  = 0;

    HI_S32 VpllDeltaFrac = 0;
    HI_S32 EpllDeltaFrac = 0;
...
HI_VOID SYNC_PllTuneFreqAbsolute(SYNC_S *pSync, HI_S32 PPM)
{
    ...
    SYNC_CalculateDeltaFrac(EpllFbdiv, EpllFrac, PPM, &EpllDeltaFrac);
    SYNC_CalculateDeltaFrac(HpllFbdiv, HpllFrac, PPM, &HpllDeltaFrac);

    /* update pll Target Frequency */
    SYNC_CalculateTargetFreq(&VpllFbdiv, &VpllFrac, VpllDeltaFrac);
    SYNC_CalculateTargetFreq(&EpllFbdiv, &EpllFrac, EpllDeltaFrac);
