...
#define TDE_CALC_MAX_SLICE_NUM(bFF, vstep, in_w) \
    ((HI_FALSE == (bFF) && TDE_NO_SCALE_STEP == (vstep)) ? (1) : ((in_w) / 118 + 5))
...
#define TDE_CHECK_SURFACEPARA(pstSurface, pstRect)\
    do{\
        if((TDE_MAX_SURFACE_PITCH < pstSurface->u32Stride)\
           ||(TDE_MAX_RECT_WIDTH < pstSurface->u32Width)\
           ||(TDE_MAX_RECT_HEIGHT < pstSurface->u32Height)\
...
STATIC INLINE HI_U32  TdeOsiMbCalcHStep(HI_U32 u32Wi, HI_U32 u32Wo,
                                        TDE2_MB_COLOR_FMT_E enInFmt,
                                        TDE2_MB_COLOR_FMT_E enOutFmt, HI_BOOL bCbCr);
...
STATIC INLINE HI_U32 TdeOsiCalcOverLapSlice(TDE_SLICE_INFO_S *pSliceInfo, HI_U32 u32MaxNum, HI_U32 u32BmpXi, HI_U32 u32BmpWi,
                                            HI_U32 u32BmpXo, HI_U32 u32BmpWo,
                                            TDE_FILTER_OPT *pstFilterOpt);
...
STATIC HI_S32 TdeOsiColorConvert(TDE2_FILLCOLOR_S *pstFillColor, TDE2_SURFACE_S *pstSur, HI_U32 *pu32OutColor)
{
    ...
    if ( ((TDE2_COLOR_FMT_CLUT1 <= pstFillColor->enColorFmt) && (pstFillColor->enColorFmt <= TDE2_COLOR_FMT_A8))
                                           ...
                                           HI_U32 u32OutWidth, HI_U32 u32OutHeight)
{
        ...
        || (TDE_MAX_RECT_HEIGHT < u32OutHeight))
    {
        ...
        {
            TDE_FATAL("Can not support resize!\n");
            return HI_ERR_TDE_INVALID_PARA;
        }
    }

    if (   (((HI_U64)u32OutWidth * TDE_MAX_MINIFICATION_H) < u32InWidth)
        || ((((HI_U64)u32OutHeight * TDE_MAX_MINIFICATION_V) < u32InHeight)))
    {
...
static HI_S32 TdeOsiSetBlend(TDE_HWNode_S *pstHWNode, TDE2_ALUCMD_E enAluCmd, TDE2_BLEND_OPT_S stBlendOpt, TDE_DRV_ALU_MODE_E *enAluMode, HI_BOOL bCheckBlend)
{
    ...
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(enAluMode, HI_FAILURE);
...
STATIC INLINE HI_S32 TdeOsiSetClutOpt(TDE2_SURFACE_S *pClutSur, TDE2_SURFACE_S *pOutSur, TDE_CLUT_USAGE_E *penClutUsage,
                                      HI_BOOL bClutReload, TDE_HWNode_S *pstHWNode)
{
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pClutSur, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pOutSur, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(penClutUsage, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstHWNode, HI_FAILURE);
    ...
    if (((HI_U8 *) - 1 != pClutSur->pu8ClutPhyAddr) && (HI_NULL != pClutSur->pu8ClutPhyAddr))
    {
        ...
        *penClutUsage = TdeOsiGetClutUsage(pClutSur->enColorFmt, pOutSur->enColorFmt);
        if (TDE_CLUT_CLUT_BYPASS > *penClutUsage)
        {
            ...
            if (0 == stClutCmd.u32PhyClutAddr)
            {
                TDE_ERROR("Alloc u32PhyClutAddr failed!\n");
                TDE_FREE((HI_VOID *)pMmzClut);
                return HI_ERR_TDE_NO_MEM;
            }
...
STATIC INLINE HI_VOID TdeOsiSetFilterOptAdjInfo(TDE_DRV_SURFACE_S *pInSur, TDE_DRV_SURFACE_S *pOutSur,
                                                TDE_FILTER_OPT *pstFilterOpt, TDE_CHILD_SCALE_MODE_E enScaleMode)
{
    TDE_CHECK_NULLPOINTER_RETURN_NOVALUE(pInSur);
    TDE_CHECK_NULLPOINTER_RETURN_NOVALUE(pOutSur);
    TDE_CHECK_NULLPOINTER_RETURN_NOVALUE(pstFilterOpt);
...
{
    TDE_CHECK_NULLPOINTER_RETURN_NOVALUE(pstConv);
...
{
    return ((TDE2_COLOR_FMT_CLUT4 == enFmt) ? 4 : -1);
}

STATIC HI_S32 TdeOsiGetbppByFmt2(TDE2_COLOR_FMT_E enFmt)
{
    return ((TDE2_COLOR_FMT_CLUT2 == enFmt) ? 2 : -1);
}
...
STATIC HI_S32 TdeOsiGetbppByFmt(TDE2_COLOR_FMT_E enFmt)
{
    ...
    BppDepth = TdeOsiGetbppByFmt16(enFmt);
    if (16 == BppDepth)
    {
        return 16;
    }
    BppDepth = TdeOsiGetbppByFmt24(enFmt);
    if (24 == BppDepth)
    {
        return 24;
    }
    BppDepth = TdeOsiGetbppByFmt32(enFmt);
    if (32 == BppDepth)
    {
        return 32;
    }

    BppDepth = TdeOsiGetbppByFmt8(enFmt);
    if (8 == BppDepth)
    {
        return 8;
    }

    BppDepth = TdeOsiGetbppByFmt1(enFmt);
    if (1 == BppDepth)
    {
        return 1;
    }
    BppDepth = TdeOsiGetbppByFmt2(enFmt);
    if (2 == BppDepth)
    {
        return 2;
    }
    BppDepth = TdeOsiGetbppByFmt4(enFmt);
    if (4 == BppDepth)
    {
        return 4;
    }
                                ...
                                TDE_SCANDIRECTION_S *pstDstDirection)
{
    ...
    if (enMirror && bFilter)
    {
        TDE_ERROR("Could not support VF/FF/HF and Mirror\n!");
        return -1;
    }
    ...
    else
    {
        ...
        {
            TDE_ERROR("bits per pixel less than 0!\n");
            return -1;
        }
...
STATIC HI_S32 TdeOsiGetInterRect(TDE2_RECT_S *pRect1, TDE2_RECT_S *pRect2, TDE2_RECT_S *pInterRect)
{
    ...
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pRect1, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pRect2, HI_FAILURE);
...
STATIC INLINE HI_BOOL TdeOsiIsRect1InRect2(TDE2_RECT_S *pRect1, TDE2_RECT_S *pRect2)
{
    ...
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pRect1, HI_FALSE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pRect2, HI_FALSE);
    ...
    if (  (pRect1->s32Xpos >= pRect2->s32Xpos) && (pRect1->s32Ypos >= pRect2->s32Ypos)
          && (s32Right1 <= s32Right2) && (s32Bottom1 <= s32Bottom2))
    {
        return HI_TRUE;
    }
        ...
        TDE2_RECT_S  *pstDstRect, TDE2_MBOPT_S *pMbOpt, TDE_MBPARA_S *stMbPara)
{
    ...
    TDE2_RECT_S stOutRect = {0};
    HI_S32 s32Ret = 0;
    TDE2_DEFLICKER_MODE_E enDeflickerMode = TDE2_DEFLICKER_MODE_NONE;

    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstMB, HI_FAILURE);
    //TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstHWNode, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstDstRect, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pMbOpt, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(stMbPara, HI_FAILURE);
    ...
    {
        TdeOsiListPutPhyBuff(stMbPara->u32WorkBufNum);
        TDE_ERROR("mb high quality pass1 fail\n!");
        return s32Ret;
    }
    ...
    {
        TdeOsiListPutPhyBuff(stMbPara->u32WorkBufNum);
        TDE_ERROR("mb high quality pass2 fail\n!");
        return s32Ret;
    }
        ...
        TDE2_RECT_S  *pstDstRect, TDE2_MBOPT_S *pMbOpt, TDE_MBPARA_S *stMbPara)
{
    ...
    {
        TdeOsiListPutPhyBuff(stMbPara->u32WorkBufNum);
        TDE_ERROR("mb middle quality pass1 fail\n!");
        return s32Ret;
    }
                                     ...
                                     TDE2_MBOPT_S *pMbOpt)
{
    ...
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstMB, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstMbRect, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstDst, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstDstRect, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pMbOpt, HI_FAILURE);
    ...
    if (TDE2_MB_COLOR_FMT_JPG_YCbCr400MBP == pstMB->enMbFmt)
    {
        ...
        {
            TDE_ERROR("TdeOsiConvertMbSurface error!\n");
            return HI_ERR_TDE_INVALID_PARA;
        }
    ...
    {
        TDE_ERROR("could not transfer mb paras!\n");
        return HI_ERR_TDE_INVALID_PARA;
    }
                                      ...
                                      TDE_PIC_MODE_E enPicMode, TDE2_MBOPT_S *pMbOpt)
{
    ...
    HI_BOOL bNeedNotZoom = HI_FALSE;

    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pS1Tmp, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pS2Tmp, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstMB, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstMbRect, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstDst, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstDstRect, HI_FAILURE);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pMbOpt, HI_FAILURE);
    bNeedNotZoom = (  (TDE2_MBRESIZE_NONE == pMbOpt->enResize) || ((pstMbRect->u32Height == pstDstRect->u32Height)
    ...
    if (TDE2_MBRESIZE_QUALITY_HIGH == pMbOpt->enResize)
    {
        ...
        pS1Tmp->u32PhyAddr = TdeOsiListGetPhyBuff(0, pS1Tmp->u32Pitch * pS1Tmp->u32Height);
        ...
        pS2Tmp->u32PhyAddr = TdeOsiListGetPhyBuff(pS1Tmp->u32Pitch * pS1Tmp->u32Height, pS2Tmp->u32Pitch * pS2Tmp->u32Height);
        if (0 == pS2Tmp->u32PhyAddr)
        {
            return 0;
        }
                                       ...
                                       TDE2_MB_COLOR_FMT_E enInFmt, TDE2_MB_COLOR_FMT_E enOutFmt, HI_BOOL bCbCr)
{
    ...
    {
        TDE_ERROR("Can't calc Hstep from format:%d to format:%d\n", enInFmt, enOutFmt);
        //return 0;
    }
#ifndef HI_BUILD_IN_BOOT
    ...
    if (bCbCr && TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP == enOutFmt)
    {
        ...
        {
            TDE_CHECK_EQUAL_RETURN_VALUE(0, (u32Wo - 1), 0);
                                       ...
                                       TDE2_MB_COLOR_FMT_E enInFmt, TDE2_MB_COLOR_FMT_E enOutFmt, HI_BOOL bCbCr)
{
    ...
    if (enInFmt != enOutFmt && TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP != enOutFmt)
    {
        TDE_ERROR("Can't calc Vstep from format:%d to format:%d\n", enInFmt, enOutFmt);
        //return 0;
    }
    ...
    if (bCbCr && TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP == enOutFmt)
    {
        ...
        {
            TDE_CHECK_EQUAL_RETURN_VALUE(0, (u32Ho - 1), 0);
...
#define TDE_GET_STARTX_BY_SCANINFO(hScan, sliceW, x) \
    ...
    HI_U32 u32OverLapPix = 0;
    HI_U32 n = 1;

    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pSliceInfo, 0);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstFilterOpt, 0);
    u32OverLapPix = (pstFilterOpt->bEvenStartInX && pstFilterOpt->bEvenStartOutX) ? 4 : 2;
    ...
    else
    {
        ...
        TDE_CHECK_EQUAL_RETURN_VALUE(0, pstFilterOpt->u32Bppi, 0);
        ...
        for (n = 1; n <= u32MaxNum + 1; n++) /* u32MaxNum protect form memory is not outside */
        {
            ...
            pSliceInfo[n].u32SliceXi  = TDE_GET_STARTX_BY_SCANINFO(pstFilterOpt->stSrcDire.enHScan, pSliceInfo[n].u32SliceWi, u32Xi);
            pSliceInfo[n].s32SliceHOfst = pstFilterOpt->s32HOffset;
            pSliceInfo[n].u32SliceWo = pSliceInfo[n].u32SliceWi;
            pSliceInfo[n].u32SliceXo = TDE_GET_STARTX_BY_SCANINFO(pstFilterOpt->stDstDire.enHScan, pSliceInfo[n].u32SliceWo, u32Xo);
...
STATIC INLINE HI_VOID TdeOsiCalcOverLapSliceWi(TDE_OVERLAPSLICE_DATUM *stOverSlapSliceDatum, TDE_FILTER_OPT *pstFilterOpt)
{
    ...
    else
    {
        ...
        TDE_CHECK_EQUAL_RETURN_NOVALUE(pstFilterOpt->u32Bppi, 0);
...
STATIC INLINE HI_VOID TdeOsiCalcOverLapSliceWo( TDE_OVERLAPSLICE_DATUM *stOverSlapSliceDatum, TDE_FILTER_OPT *pstFilterOpt)
{
    ...
    else
    {
        ...
        {
            TDE_CHECK_EQUAL_RETURN_NOVALUE(pstFilterOpt->u32HStep, 0);
                                       ...
                                       TDE_FILTER_OPT *pstFilterOpt)
{
    ...
    TDE_INFO("slice calc u32MaxSliceNum: %d!\n", u32MaxSliceNum);
#ifndef HI_BUILD_IN_BOOT
    ...
    {
        TDE_INFO("one slice block for filter!\n");
        u32SliceNum = 1;
    ...
    {
        TDE_INFO("NoLap slice block for filter!\n");
    ...
    {
        TDE_INFO("OverLap slice block for filter!\n");
        ...
        TDE_INFO("slice real u32SliceNum: %d!\n", u32SliceNum);
        stChildInfo.u64Update = 0x1F;
    }
...
STATIC void TdeOsiGetHUpdateInfo(UpdateConfig *reg, UpdateInfo *info, int scaler_en)
{
    ...
    HI_S32 ratio;
    HI_S32 dratio;
    TDE_INFO("update_start:%d, update_hstop:%d, ori_in_width:%d\n", \
             update_hstart, update_hstop, reg->ori_in_width);

    TDE_CHECK_EQUAL_RETURN_NOVALUE((reg->zme_out_width - 1), 0);
    TDE_CHECK_EQUAL_RETURN_NOVALUE((reg->ori_in_width - 1), 0);
        ...
        HI_S32 update_vstop, HI_S32 ratio, HI_S32 dratio )
{
    ...
    if (update_vstop > (reg->ori_in_height - 3) && update_vstop < reg->ori_in_height)
    {
        ...
        TDE_INFO("update_vstop:%d zme_voutstop:%d\n", update_vstop, zme_voutstop);
    }
    else
    {
        /*the update outstreatched area didn't exceed the down limit bordline*/
    ...
    info->zme_in_height = zme_vinstop - zme_vinstart + 1;
    info->zme_instart_h = zme_vinstart;
    TDE_INFO("zme_voutstart:%d, zme_voutstop:%d\n", zme_voutstart, zme_voutstop);
    info->zme_outstart_h = zme_voutstart;
    info->zme_out_height = zme_voutstop - zme_voutstart + 1;
...
STATIC void TdeOsiGetVUpdateInfo(UpdateConfig *reg, UpdateInfo *info, int scaler_en, int deflicker_en)
{
    ...
    HI_S32 ratio = 0;
    HI_S32 dratio = 0;
    TDE_INFO("update_vstart:%d, update_vstop:%d, ori_in_height:%d, ratio:%d£¬dratio:%d\n", \
             update_vstart, update_vstop, reg->ori_in_height, ratio, dratio);

    TDE_CHECK_EQUAL_RETURN_NOVALUE((reg->zme_out_height - 1), 0);
    TDE_CHECK_EQUAL_RETURN_NOVALUE((reg->ori_in_height - 1), 0);

    ratio = (HI_S32)(4096 * (reg->ori_in_height - 1) / (reg->zme_out_height - 1) + 1 / 2);
    dratio = 4096 * (reg->zme_out_height - 1) / (reg->ori_in_height - 1);
                                  ...
                                  TDE2_RECT_S *pInRect, TDE2_RECT_S  *pOutRect, TDE2_DEFLICKER_MODE_E enDeflickerMode, TDE_FILTER_OPT *pstFilterOpt)
{
    ...
    TDE_INFO("InHeight:%d, InWidth:%d, OutHeight:%d, OutWidth:%d,UpdateX:%d, UpdateY:%d,\
    ...
    if (bLocalDeflicker)
    {
        ...
        TDE_INFO("zme_instart_w:%d, zme_instart_h:%d, zme_in_width:%d, \
...
{
    HI_S32 s32Ret = 0;
    HI_BOOL bInvalidOperationArea = 0;
        ...
        TDE2_OPT_S      *pstOpt)
{
    ...
    {
        TDE_ERROR("pTmpSrc2Rect is NULL!\n");
    ...
    if (TDE_FRAME_PIC_MODE == enPicMode)
    {
        ...
        switch (enAdjColorFmt)
        {
            ...
            case TDE_DRV_COLOR_FMT_YCbCr420MB:
            {
                ...
                pstDrvCbCr->u32Height =
                    TDE_ADJ_SIZE_BY_START_P(pstMbRect->s32Ypos, pstMbRect->u32Height);
                TDE_INFO("W:0x%x, H:0x%x\n", pstDrvCbCr->u32Width, pstDrvCbCr->u32Height);
            }
            break;
            ...
            break;
            default:
                TDE_ERROR("Unspport color format!\n");
                return -1;
        }
                                ...
                                TDE_HWNode_S *pstHwNode)
{
    ...
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstForeGround, HI_ERR_TDE_NULL_PTR);
    ...
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstHwNode, HI_ERR_TDE_NULL_PTR);
                                  ...
                                  TDE_HWNode_S *pstHwNode)
{
    ...
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstMbOpt, HI_ERR_TDE_NULL_PTR);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pstHwNode, HI_ERR_TDE_NULL_PTR);
...
STATIC HI_S32 TdeOsiSetColorKey(TDE2_SURFACE_S *pstForeGround , TDE2_SURFACE_S *pstBackGround , TDE_HWNode_S *pstHWNode , TDE2_COLORKEY_U unColorKeyValue, TDE2_COLORKEY_MODE_E enColorKeyMode,  TDE_CLUT_USAGE_E enClutUsage)
{
    ...
    {
        TdeHalFreeNodeBuf(pstHWNode);
        TDE_FATAL("Unknown fmt category!\n");
        return HI_ERR_TDE_INVALID_PARA;
    }
                        ...
                        TDE2_SURFACE_S *pstDst, TDE2_RECT_S  *pstDstRect, TDE2_MBOPT_S *pstMbOpt)
{
    ...
    u32CbCrHeight = TDE_MAX_RECT_HEIGHT;
    if ((pstMB->enMbFmt == TDE2_MB_COLOR_FMT_MP1_YCbCr420MBP) || (pstMB->enMbFmt == TDE2_MB_COLOR_FMT_MP2_YCbCr420MBP)\
        ...
        || (pstMB->enMbFmt == TDE2_MB_COLOR_FMT_JPG_YCbCr422MBVP))
    {
        u32CbCrHeight = TDE_MAX_RECT_HEIGHT / 2;
    }
    ...
    for (n = 0; n < j; n++)
    {
        ...
        for (m = 0; m < i; m++)
        {
            if ((i - 1) == m)
            {
                pstMbRect->u32Width = u32Width - m * TDE_MAX_RECT_WIDTH;
            }
            else
            {
                pstMbRect->u32Width = TDE_MAX_RECT_WIDTH;
            }
            ...
            if ((j - 1) == n)
            {
                pstMbRect->u32Height = u32Height - n * TDE_MAX_RECT_HEIGHT;
            }
            else
            {
                pstMbRect->u32Height = TDE_MAX_RECT_HEIGHT;
...
HI_BOOL TdeOsiCheckOverlap(TDE2_SURFACE_S *pstSur1, TDE2_RECT_S *pstRect1, TDE2_SURFACE_S *pstSur2, TDE2_RECT_S *pstRect2)
{
    ...
    TDE_INFO("u32Rect1StartPhy:%x, u32Rect1EndPhy:%x, u32Rect2StartPhy:%x, \
