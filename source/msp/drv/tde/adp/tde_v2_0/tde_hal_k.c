...
HI_S32 TdeHalInit(HI_U32 u32BaseAddr)
{
    ...
    {
        HI_GFX_LOG_PrintFuncErr(wmeminit, HI_FAILURE);
        goto TDE_INIT_ERR;
    }
    ...
    {
        wmemterm();
        HI_GFX_LOG_PrintFuncErr(TdeHalInitParaTable, HI_FAILURE);
        goto TDE_INIT_ERR;
    }
...
HI_S32 TdeHalNodeExecute(HI_U32 u32NodePhyAddr, HI_U64 u64Update, HI_BOOL bAqUseBuff)
{
    ...
    {
        HI_GFX_LOG_PrintFuncErr(TdeHalCtlIsIdleSafely, HI_FAILURE);
        return HI_FAILURE;
    }
                                ...
                                TDE_DRV_ALU_MODE_E enAlu, TDE_DRV_COLORFILL_S *pstColorFill)
{
    ...
    switch (enMode)
    {
        ...
        case TDE_NORM_FILL_2OPT:/*signal color with bitmap operation and blit*//*CNcomment:单色和位图运算操作搬移*/
        {
            ...
            TDE_CHECK_ARRAY_OVER_RETURN_VALUE(enAlu, TDE_BUTT + 1, HI_FAILURE);
...
HI_S32 TdeHalNodeSetBlend(TDE_HWNode_S *pHWNode, TDE2_BLEND_OPT_S *pstBlendOpt)
{
    ...
    {
        TDE_CHECK_ARRAY_OVER_RETURN_VALUE(pstBlendOpt->eBlendCmd, TDE2_BLENDCMD_BUTT + 1, HI_FAILURE);
...
STATIC HI_U32 TdeHalGetColorKeyMode(TDE2_COLORKEY_COMP_S *stColorKey)
{
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(stColorKey, 0);
                                ...
                                TDE2_RECT_S *pInRect, TDE2_RECT_S  *pOutRect, TDE2_FILTER_MODE_E enFilterMode)
{
    ...
    TDE_CHECK_NULLPOINTER_RETURN_NOVALUE(pNode);
    TDE_CHECK_NULLPOINTER_RETURN_NOVALUE(pInRect);
    TDE_CHECK_NULLPOINTER_RETURN_NOVALUE(pOutRect);

    TDE_CHECK_ARRAY_OVER_RETURN_NOVALUE(enInFmt, TDE2_COLOR_FMT_BUTT + 1);
    ...
    TDE_CHECK_EQUAL_RETURN_NOVALUE(0, pOutRect->u32Width);
    ...
    TDE_CHECK_EQUAL_RETURN_NOVALUE(0, pOutRect->u32Height);
    ...
    if ((pOutRect->u32Width * 16) <= pInRect->u32Width)
    {
        ...
        TDE_CHECK_EQUAL_RETURN_NOVALUE(0, pNode->SRC1_HPZME.bits.src1_hpzme_mode + 1);
                                ...
                                TDE2_RECT_S *pInRect, TDE2_RECT_S  *pOutRect, HI_BOOL bDefilicker, TDE2_FILTER_MODE_E enFilterMode)
{
    ...
    if ((pOutRect->u32Width * 16) <= pInRect->u32Width)
    {
        ...
        TDE_CHECK_EQUAL_RETURN_NOVALUE(0, pNode->SRC2_HPZME.bits.src2_hpzme_mode + 1);
    ...
    if ((pOutRect->u32Height * 16) <= pInRect->u32Height)
    {
        ...
        TDE_CHECK_EQUAL_RETURN_NOVALUE(0, u32TempHeightScaleCoff);
...
#define OrgHfCoef Tde6x32Coef
    ...
    {
        TDE_ERROR("Alloc Deflicker vertical coef failed\n");
        TDE_FREE(pHfCoef);
        TDE_FREE(pVfCoef);
        return HI_FAILURE;
    }
    ...
    if (0 == s_stParaTable.u32DeflickerVfCoefAddr)
    {
        TDE_ERROR("Alloc u32DeflickerVfCoefAddr failed\n");
        TDE_FREE(pHfCoef);
        TDE_FREE(pVfCoef);
...
#else
    /** can not use low power, reset would be dead by set 0x0f006466**/
    TDE_WRITE_REG(s_pu32BaseVirAddr, TDE_MISCELLANEOUS, 0x03006466);
...
HI_S32 TDE_OSR_Init(HI_VOID)
{
    ...
    {
        HI_GFX_LOG_PrintFuncErr(TdeHalInit,HI_FAILURE);
        return -1;
    }
    TdeOsiListInit();
    HI_GFX_LOG_FuncExit(gs_TdeDebugInterApi);
...
struct seq_file *TdeHalNodePrintInfo(struct seq_file *p, HI_U32 *pu32CurNode)
{
    ...
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(p, p);
    TDE_CHECK_NULLPOINTER_RETURN_VALUE(pu32CurNode, p);
