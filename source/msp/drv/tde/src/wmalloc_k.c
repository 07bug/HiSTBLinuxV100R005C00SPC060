...
HI_BOOL gs_TdeDebugInterApi = HI_FALSE;
...
#define PRINTMEMINFO() do\
    {\
        TDE_INFO("-------------------------------------------------------------------\n");\
        TDE_INFO("\tUnitSize    \tMemBlockNum    \tFreeMemBlockNum\n");\
...
static HI_VOID *mallocUnit(UNIT_SIZE_E eUnitSize)
{
    ...
    TDE_INFO("eUnitSize %d, free units:%d, first free unit:%d\n", pBlock->nUnitSize, pBlock->nFree, pBlock->nFirst);
...
static HI_S32 freeUnit(UNIT_SIZE_E eUnitSize, HI_VOID *ptr)
{
    ...
    *(HI_U16 *)ptr = pBlock->nFirst; /* point to next unit can be assigned */
    ...
    TDE_INFO("eUnitSize : %d,first free unit : %d, free units : %d\n", pBlock->nUnitSize, pBlock->nFirst, pBlock->nFree);
...
static HI_S32 wfree(HI_VOID *ptr)
{
    ...
    TDE_ERROR("Free mem failed!vir:%p, phy:%x\n", ptr, wgetphy(ptr));
...
HI_S32 wmeminit(void)
{
    ...
    {
        TDE_FATAL("malloc mempool buffer failed!\n");
        return HI_FAILURE;
    }
#else
