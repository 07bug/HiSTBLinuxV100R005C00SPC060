...
typedef struct tagBUFMNG_INST_S
{
    ...
    HI_U32 u32DataNum;         /* For stream mode, it is undecoded packet number.  For frame mode, it is undecoded frame number, support BUFMNG_NOT_END_FRAME_BIT.*/
...
HI_S32 BUFMNG_Create(HI_HANDLE hBuf, BUFMNG_INST_CONFIG_S* pstConfig)
{
    ...
    if ((HI_INVALID_HANDLE == hBuf) || (HI_NULL == pstConfig))
    {
...
HI_S32 BUFMNG_SaveYuv(HI_S32 Handle, HI_DRV_VIDEO_FRAME_S *pstFrame, HI_UNF_VCODEC_TYPE_E enType)
{
    ...
    if (HI_DRV_PIX_FMT_NV21 == pstFrame->ePixFormat)
    {
        ...
        len = vfs_write(VdecSaveYuvFile, U_Array, chroma_len, &VdecSaveYuvFile->f_pos);
        if (len != chroma_len)
        {
            HI_ERR_BUFMNG("%s write u failed.\n", __func__);
        }

        len = vfs_write(VdecSaveYuvFile, V_Array, chroma_len, &VdecSaveYuvFile->f_pos);
        if(len != chroma_len)
        {
    ...
    else
    {
        ...
        if (!bVcmpFlag)
        {
            ...
            len = vfs_write(VdecSaveYuvFile, U_Array, pstFrame->u32Width*pstFrame->u32Height/4, &VdecSaveYuvFile->f_pos);
            ...
            len = vfs_write(VdecSaveYuvFile, V_Array, pstFrame->u32Width * pstFrame->u32Height / 4, &VdecSaveYuvFile->f_pos);
            if (len != pstFrame->u32Width*pstFrame->u32Height/4)
            {
