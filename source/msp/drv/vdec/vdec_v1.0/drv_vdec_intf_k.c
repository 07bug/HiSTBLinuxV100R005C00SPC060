/******************************************************************************
  ...
  Copyright (C), 2006-2017, Hisilicon Tech. Co., Ltd.
...
#define HI_VIDEO_MAX_VDH_BUF 200

//#define __VDEC_ADD_EXT_EOP__   //add by sdk
...
#define HI_VDEC_FRAME_SIZE_1080P    (3300 * 1024)
...
#elif defined(HI_VIDEO_MAX_DISP_FRAME_NUM)
        ...
        for (i = 0; i < pstBufVpssInst->u32BufNum; i++)
        {
            ...
            if (HI_SUCCESS != s32Ret)
            {
                ...
                list_for_each_safe(pos, n, &(pstBufVpssInst->stVpssBufAvailableList))
                {
                    ...
                    HI_DRV_MMZ_UnmapAndRelease(&(pstTarget->MMZBuf_meta));//DRM: meta内存为非安内存 sdk
                    list_del_init(pos);
...
static HI_S32 BUFMNG_VPSS_RelBuffer(HI_HANDLE hVpss, HI_HANDLE hPort, HI_DRV_VIDEO_FRAME_S *pstImage)
{
    ...
    else
    {
        ...
        list_for_each_safe(pos, n, &(pstVpssChan->stPort[j].stBufVpssInst.stVpssBufUnAvailableList))
        {
            ...
            if (pstImage->stBufAddr[0].u32PhyAddr_Y == pstTarget->MMZBuf_frm.u32StartPhyAddr)
            {
...
HI_S32 VDEC_FindVpssHandleByVdecHandle(HI_HANDLE hVdec, HI_HANDLE *phVpss)
{
    ...
    VDEC_ASSERT_RETURN(phVpss != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
static HI_S32 VDEC_EventHandle(HI_S32 s32ChanID, HI_S32 s32EventType, HI_VOID *pArgs)
{
    ...
    switch (s32EventType)
    {
        ...
        case EVNT_NEED_ARRANGE:                             //获取码流信息,为帧存划分做准备.sdk//
            if (pArgs == HI_NULL)
            {
                HI_ERR_VDEC("Param is invalid\n");
                break;
        ...
        case EVNT_ARRANGE_FRAME_BUF:
            ...
            pstChan->u32RefFrameNum = ((HI_U32 *)pArgs)[0];
            pstChan->u32FrameSize = ((HI_U32 *)pArgs)[1];
            pstChan->u32PMVNum = ((HI_U32 *)pArgs)[2];
            pstChan->u32PMVSize = ((HI_U32 *)pArgs)[3];

            //VPSS在对标清的流做去隔行操作时，会占用3帧
            if (((((HI_U32 *)pArgs)[4]) <= 1920) && ((((HI_U32 *)pArgs)[5]) <= 1088))
            {
                pstChan->u32RefFrameNum += 3;
            }
...
static HI_S32 VDEC_VpssEventHandle(HI_HANDLE hVdec, HI_DRV_VPSS_EVENT_E enEventID, HI_VOID *pstArgs)
{
    ...
    if ((HI_NULL == pstArgs) && (VPSS_EVENT_NEW_FRAME != enEventID))
    {
        HI_ERR_VDEC("%s %d args err! event id:%d\n", __func__, __LINE__, enEventID);
        return HI_FAILURE;
    }
    ...
    if(pstFrame == HI_NULL)
    {
        HI_ERR_VDEC("%s %d args err! Failed to applicate space!", __func__, __LINE__);
        return HI_FAILURE;
    }
    ...
    switch (enEventID)
    {
        ...
        case VPSS_EVENT_GET_FRMBUFFER:
            ...
            hVpss = (HI_HANDLE)PORTHANDLE_TO_VPSSID(hPort);
            memset(&stMMZ_Buffer, 0, sizeof(stMMZ_Buffer));
            ...
            ((HI_DRV_VPSS_FRMBUF_S *)pstArgs)->u32StartPhyAddr = stMMZ_Buffer.MMZBuf_frm.u32StartPhyAddr; //sdk
            ((HI_DRV_VPSS_FRMBUF_S *)pstArgs)->pu8StartVirAddr = stMMZ_Buffer.MMZBuf_frm.pu8StartVirAddr;
            ((HI_DRV_VPSS_FRMBUF_S *)pstArgs)->u32PrivDataPhyAddr = stMMZ_Buffer.MMZBuf_meta.u32StartPhyAddr; //sdk
...
static HI_S32 VDEC_SetAttr(VDEC_CHANNEL_S *pstChan)
{
    ...
    {
        if ((HI_U32)pstCfg->s32CtrlOptions & HI_UNF_VCODEC_CTRL_OPTION_SIMPLE_DPB || pstChan->bLowdelay)
        {
...
static HI_S32 VDEC_CreateStrmBuf(HI_DRV_VDEC_STREAM_BUF_S *pstBuf)
{
 ...
 #ifdef HI_TEE_SUPPORT
    if (stBufInstCfg.bTvp == HI_TRUE)
    {
        hVdec = pstBuf->hHandle & 0xff;
        pstChan = s_stVdecDrv.astChanEntity[hVdec].pstChan;
        if (pstChan != HI_NULL)
        {
            pstChan->stEsBuf.u32StartPhyAddr = pstBuf->u32PhyAddr;
            pstChan->stEsBuf.u32Size = pstBuf->u32Size;
        }
    }
 #endif
    return HI_SUCCESS;
...
#ifdef __VDEC_ADD_EXT_EOP__ //add by sdk
...
HI_S32 HI_VDEC_Chan_VORlsFrame(HI_HANDLE hPort, HI_DRV_VIDEO_FRAME_S* pstVpssFrame)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
         HI_INFO_VDEC("VDEC_Chan_VORlsFrame err !\n");
        return HI_FAILURE;
    }
    return s32Ret;
}
...
HI_S32 HI_DRV_VDEC_GetFrmPackingType(HI_HANDLE hVdec, HI_UNF_VIDEO_FRAME_PACKING_TYPE_E *penFramePackType)
{
    ...
    hVdec = hVdec & 0xff;
    VDEC_ASSERT_RETURN(hVdec < HI_VDEC_MAX_INSTANCE_NEW, HI_ERR_VDEC_INVALID_PARA);
    VDEC_ASSERT_RETURN(penFramePackType != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
HI_S32 HI_DRV_VDEC_GetVideoFrameInfo(HI_HANDLE hVdec, HI_UNF_AVPLAY_VIDEO_FRAME_INFO_S *pstVideoFrameInfo)
{
    ...
    hVdec = hVdec & 0xff;
    VDEC_ASSERT_RETURN(hVdec < HI_VDEC_MAX_INSTANCE_NEW, HI_ERR_VDEC_INVALID_PARA);
    VDEC_ASSERT_RETURN(pstVideoFrameInfo != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
#ifdef Hi3716MV410
HI_S32 VDEC_AllocPMVMMZMem(HI_S32 chanID)
{
    ...
    MMZ_BUFFER_S memRecord = {0};

    pstChan = s_stVdecDrv.astChanEntity[chanID].pstChan;

    PMVSize = pstChan->u32PMVSize * pstChan->u32PMVNum;

    ret = HI_DRV_MMZ_AllocAndMap("VFMW_PMV_BUF", "VFMW", PMVSize, 0, &memRecord);
    if (ret == HI_SUCCESS)
    {
        pstChan->stPMVMMZBuf.u32Size = memRecord.u32Size;
        pstChan->stPMVMMZBuf.u32StartPhyAddr = memRecord.u32StartPhyAddr;
        pstChan->stPMVMMZBuf.pu8StartVirAddr = memRecord.pu8StartVirAddr;
    }
    else
    {
        HI_ERR_VDEC("alloc VFMW_PMV_BUF failed\n");
        return HI_FAILURE;
    }

    ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_ALLOC_PMV_TO_DECODER, (HI_VOID *)&memRecord);
...
static HI_S32 VDEC_RlsFrm(VDEC_CHANNEL_S *pstChan, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    {
        HI_ERR_VDEC("pstPrivInfo == NULL\n");
        return HI_FAILURE;
    }

    if (pstStatInfo == HI_NULL)
    {
        return HI_FAILURE;
    }

    if (pstImgInft == HI_NULL || pstImgInft->release_image == HI_NULL)
    {
        return HI_FAILURE;
    }
...
static HI_S32 VDEC_Chan_InitParam(HI_HANDLE hHandle, HI_UNF_AVPLAY_OPEN_OPT_S *pstCapParam)
{
    ...
    pstChan->stOption.s32DelayTime = 0;//sdk 动态分配帧存时，若分配不到内存vfmw等待的时间
    pstChan->stOption.u32SelfAdaptionDFS = 1;//sdk 自适应帧存
    pstChan->stOption.u32CfgFrameNum = 0;//sdk 配置帧数
    pstChan->stOption.u32NeedMMZ = 1;//sdk是否使用MMZ做为预分配不足时的补充
    pstChan->stOption.u32MaxMemUse = -1;//sdk最大可用帧存
    pstChan->bNeedAlloc = HI_FALSE;//sdk 记录该通道是否已经分配过帧存了
...
HI_S32 VDEC_Chan_FindPreMMZ(MMZ_BUFFER_S *pstMMZBuffer)
{
    ...
    VDEC_ASSERT_RETURN(pstMMZBuffer != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
                ...
                if ( st_VdecChanPreUsedMMZInfo[i].u32StartPhyAddr == pstMMZBuffer->u32StartPhyAddr)
                {
                    ...
                    if (0 == i)
                    {
                        if (0 == st_VdecChanPreUsedMMZInfo[i + 1].u32NodeState && st_VdecChanPreUsedMMZInfo[i + 1].u32Size > 0)
                        {
                            ...
                            g_VdecPreMMZNodeNum = (g_VdecPreMMZNodeNum > 0) ? (g_VdecPreMMZNodeNum - 1) : 0;
                        }
                        else
                        {
                            st_VdecChanPreUsedMMZInfo[i].u32NodeState = 0;
...
#endif
    {
        if (1 != pstChan->stOption.u32DynamicFrameStoreAllocEn) //sdk
        {
...
static HI_S32 VDEC_Chan_Reset(HI_HANDLE hHandle, HI_DRV_VDEC_RESET_TYPE_E enType)
{
    ...
    if (HI_INVALID_HANDLE != pstChan->hChan)
    {
        ...
        if(pstChan->bKeepSpsPps)
        {
            ...
            s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_RESET_CHAN_WITH_OPTION, &stResetOption);
            if (VDEC_OK != s32Ret)
            {
...
HI_S32 HI_DRV_VDEC_SetCtrlInfo(HI_HANDLE hHandle, HI_UNF_AVPLAY_CONTROL_INFO_S *pstCtrlInfo)
{
    ...
    //该改动为临时版本，和zhouzhong(sdk)沟通后，后续PVR会在上层配置u32DispOptimizeFlag
    //那时需要删除此处代码
...
HI_S32 VDEC_Ioctl(struct inode *inode, struct file  *filp, unsigned int cmd, void *arg)
{
    ...
    switch (cmd)
    {
        ...
        case UMAPC_VDEC_CHAN_KEEPSPSPPS:
            if (HI_NULL == arg)
            {
                HI_ERR_VDEC("CMD %p Bad arg!\n", (HI_VOID *)cmd);
                return HI_ERR_VDEC_INVALID_PARA;
    ...
    switch (cmd)
    {
        ...
        case UMAPC_VDEC_CHAN_ALLOC:
        {
            ...
            s_stVdecDrv.astChanEntity[hHandle].u32DynamicFsEn = ((VDEC_CMD_ALLOC_S *)arg)->u32DFBEn; //sdk
        ...
        case UMAPC_VDEC_CHAN_EVNET_NEWFRAME:
        {
            ...
            HI_HANDLE hVdecHandle = pstFrameCmd->hHandle & 0xff;
            if(hVdecHandle >= HI_VDEC_MAX_INSTANCE_NEW)
            {
                s32Ret = HI_FAILURE;
                HI_ERR_VDEC("UMAPC_VDEC_CHAN_EVNET_NEWFRAME return:%d\n", s32Ret);
                break;
            }

            if(HI_NULL == s_stVdecDrv.astChanEntity[pstFrameCmd->hHandle & 0xff].pstChan)
            {
                s32Ret = HI_FAILURE;
                HI_ERR_VDEC("UMAPC_VDEC_CHAN_EVNET_NEWFRAME return:%d\n", s32Ret);
                break;
...
static SINT32 VDEC_FindPreMMZNode(HI_HANDLE hHandle, MMZ_BUFFER_S *pstMMZbuffer)
{
    ...
    if (i >= g_VdecPreMMZNodeNum)
    {
        for (i = 0; i < g_VdecPreMMZNodeNum && i < HI_VDEC_MAX_PRENODE_NUM; i++)
        {
...
static HI_S32 VDEC_ReleaseMemForDecoder(HI_HANDLE hHandle)
{
    ...
    if (pstChan->bPMVMMZallocated == 1)
    {
        HI_DRV_MMZ_UnmapAndRelease(&pstChan->stPMVMMZBuf);
        pstChan->bPMVMMZallocated = 0;
    }
...
static SINT32 VDEC_AllocMemForDecoder(HI_HANDLE hHandle)
{
    ...
    VDEC_CHANNEL_S *pstChan = HI_NULL;
    VDEC_CHAN_FRAME_STORE_PARAM_S stFsParam;
    MMZ_BUFFER_S stMMZFrmBuffer;
    MMZ_BUFFER_S stMMZPMVBuffer;
    FRAME_BUF_S  stFrameBuf;
    MMZ_BUFFER_S stPMVBuf;
    UINT32 u32NeededFrameNum = 0;

    HI_S32 i = 0;

    pstChan = s_stVdecDrv.astChanEntity[hHandle].pstChan;
    memset(&stMMZFrmBuffer, 0, sizeof(stMMZFrmBuffer));
    memset(&stMMZPMVBuffer, 0, sizeof(stMMZPMVBuffer));
    memset(&stFsParam, 0, sizeof(stFsParam));
    memset(&stPMVBuf, 0, sizeof(stPMVBuf));
    ...
    if ((1 == pstChan->stOption.u32DynamicFrameStoreAllocEn) && (HI_TRUE == pstChan->bNeedAlloc))
    {
        ...
        if(s32Ret == HI_FAILURE)
        {
            HI_ERR_VDEC("%s, VDEC_RlsPreMMZNode Failed!\n", __FUNCTION__);
        }

        s32Ret = VDEC_ReleaseMemForDecoder(hHandle);
        if(s32Ret == HI_FAILURE)
        {
            HI_ERR_VDEC("%s, VDEC_ReleaseMemForDecoder Failed!\n", __FUNCTION__);
        }
        ...
        stMMZFrmBuffer.u32Size = (pstChan->u32FrameSize) * (pstChan->needFrameNum);//计算需要的帧存大小
        stMMZPMVBuffer.u32Size = (pstChan->u32PMVSize) * (pstChan->u32PMVNum);//计算需要的PMV大小

        if (0) //VDEC_FindPreMMZNode(hHandle, &stMMZPMVBuffer) == HI_SUCCESS) //找到了合适的预分配节点
        {
            stPMVBuf.u32Size = (pstChan->u32PMVSize) * (pstChan->u32PMVNum);
            stPMVBuf.u32StartPhyAddr = stMMZPMVBuffer.u32StartPhyAddr;
            stPMVBuf.pu8StartVirAddr = stMMZPMVBuffer.pu8StartVirAddr;

            s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_ALLOC_PMV_TO_DECODER, (HI_VOID *)&stPMVBuf);
            if (s32Ret != HI_SUCCESS)
            {
                HI_ERR_VDEC("alloc PMV buffer from PREMem failed\n");
                return HI_FAILURE;
            }
        }
        else
        {
            s32Ret = VDEC_AllocPMVMMZMem(hHandle);
            if (s32Ret != HI_SUCCESS)
            {
                HI_ERR_VDEC("VDEC_AllocPMVMMZMem alloc PMV buffer failed\n");
                return HI_FAILURE;
            }
            pstChan->bPMVMMZallocated = 1;
        }

        VDEC_FindPreMMZNode(hHandle, &stMMZFrmBuffer);
        pstChan->preAllocFrameNum = (pstChan->stVDHMMZBuf.u32Size)/ pstChan->u32FrameSize;
        pstChan->mmzAllocFrameNum = (pstChan->needFrameNum > pstChan->preAllocFrameNum)
                                    ? (pstChan->needFrameNum - pstChan->preAllocFrameNum) : 0;

        pstChan->u8NeedMMZ = pstChan->stOption.u32NeedMMZ;

        if (HI_SUCCESS != VDEC_AllocPREMem(hHandle, pstChan->preAllocFrameNum))
        {
            HI_ERR_VDEC("alloc frame buffer from PREMem error\n");
...
{
    VDEC_ASSERT_RETURN(pstMMZBuf != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
static HI_S32 VDEC_OpenDev(HI_VOID)
{
    ...
    /**********************Creat Vdec task //sdk start*************************/
    if (HI_NULL == s_stVdecDrv.pVdecTask) //sdk
    {
...
#ifdef Hi3716MV410
    TotalFrmNum = RefFrameNum + DispFrameNum;
...
HI_S32 HI_DRV_VDEC_AllocChan(HI_HANDLE *phHandle, HI_UNF_AVPLAY_OPEN_OPT_S *pstCapParam)
{
    ...
    VDEC_ASSERT_RETURN(phHandle != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
HI_S32 HI_DRV_VDEC_CreateStrmBuf(HI_HANDLE hVdec, HI_DRV_VDEC_STREAM_BUF_S *pstBuf)
{
    ...
    hVdec = hVdec & 0xff;
    VDEC_ASSERT_RETURN(hVdec < HI_VDEC_MAX_INSTANCE_NEW, HI_ERR_VDEC_INVALID_PARA);
    VDEC_ASSERT_RETURN(pstBuf != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
HI_S32 HI_DRV_VDEC_ResetChan(HI_HANDLE hHandle)
{
    ...
    return VDEC_Chan_Reset(hHandle, VDEC_RESET_TYPE_ALL);
}
...
HI_S32 HI_DRV_VDEC_Chan_RecvVpssFrmBuf(HI_HANDLE hVdec, HI_DRV_VIDEO_FRAME_PACKAGE_S *pstFrm)
{
    ...
    hVdec = hVdec & 0xff;
    VDEC_ASSERT_RETURN(hVdec < HI_VDEC_MAX_INSTANCE_NEW, HI_ERR_VDEC_INVALID_PARA);
    VDEC_ASSERT_RETURN(pstFrm != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
{
    VDEC_ASSERT_RETURN(pstFrmRate != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
HI_S32 HI_DRV_VDEC_GetCap(VDEC_CAP_S *pstCap)
{
    ...
    VDEC_ASSERT_RETURN(pstCap != HI_NULL, HI_ERR_VDEC_INVALID_PARA);
...
VDEC_CHANNEL_S *HI_DRV_VDEC_DRV_GetChan(HI_HANDLE hHandle)
{
    ...
    hHandle = hHandle & 0xff;
    VDEC_ASSERT_RETURN(hHandle < HI_VDEC_MAX_INSTANCE_NEW, HI_NULL);
...
HI_S32 HI_DRV_VDEC_RegisterEventCallback(HI_HANDLE hHandle, EventCallBack fCallback)
{
    hHandle = hHandle & 0xff;
    VDEC_ASSERT_RETURN(hHandle < HI_VDEC_MAX_INSTANCE_NEW, HI_ERR_VDEC_INVALID_PARA);
    VDEC_ASSERT_RETURN(fCallback != HI_NULL, HI_ERR_VDEC_INVALID_PARA);

    s_stVdecDrv.astChanEntity[hHandle].eCallBack = fCallback;
    return HI_SUCCESS;
}
...
HI_S32 HI_DRV_VDEC_RegisterDmxHdlCallback(HI_U32  hHandle, GetDmxHdlCallBack fCallback, HI_U32 dmxID)
{
    ...
    s_stVdecDrv.astChanEntity[hHandle].pstChan->u32DmxID = dmxID;
    s_stVdecDrv.astChanEntity[hHandle].DmxHdlCallBack = fCallback;
    return HI_SUCCESS;
}
