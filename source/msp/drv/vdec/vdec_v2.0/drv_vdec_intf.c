...
#include <asm/io.h>
//#include <mach/hardware.h>  //@sdk 64bit
...
extern HI_SYS_VIDEO_CODEC_E g_enSysCodec;
                                 ...
                                 const char __user * buffer, size_t count, loff_t *ppos)
{
    ...
    else if (0 == strncmp(dat1, "maskyuvlock", strlen("maskyuvlock")))
    {
        strncpy(MaskName, "maskyuvlock", strlen("maskyuvlock"));
        SetMaskBit(dat2, MaskName, 3);
    }
    ...
    if (pstChan)
    {
        ...
        if (pstChan->stOption.u32DynamicFrameStoreAllocEn == 1)
        {
            PROC_PRINT(p,"-------------------Dynamic Frame Store Information-------------------\n"
                         ...
                         (pstChan->stVFMWReportFrameInfo.u32AllocFrmEndTime - pstChan->stVFMWReportFrameInfo.u32AllocFrmStartTime)
                         );//sdk
        }
