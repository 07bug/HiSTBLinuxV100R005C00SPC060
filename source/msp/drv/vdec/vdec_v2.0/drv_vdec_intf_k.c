/******************************************************************************
  ...
  Copyright (C), 2006-2011, Hisilicon Tech. Co., Ltd.
...
#ifdef CHIP_TYPE_hi3798mv200
static UINT32 isVp9Enhanced = 0;
static UINT32 g_OrgCoreVoltVal = 0;
#define CHIP_TYPE_FF (1)
#define CHIP_TYPE_TT (2)
#define CHIP_TYPE_SS (3)
#define VP9_IMGSIZE_8BIT (12587040)
#endif
...
typedef struct tagVDEC_CHAN_ENTITY_S
{
    ...
    VDEC_VPSSCHANNEL_S *pstVpssChan;    /* Channel structure pointer for vpss*/
    ...
    HI_BOOL             bO5;
    HI_U32              u32DynamicFsEn; /*Dynamic frame store flag*///sdk
...
typedef struct
{
    ...
    struct task_struct*     pVdecTask; //sdk
...
#define DEFAULT_EOP_EN    (HI_FALSE)
#else
...
#if defined(HI_VIDEO_MAX_DISP_FRAME_NUM)
static HI_S32 RefFrameNum  = 6;
static HI_S32 DispFrameNum = HI_VIDEO_MAX_DISP_FRAME_NUM;
#else
static HI_S32 RefFrameNum  = 6;
static HI_S32 DispFrameNum = 4;
#endif
...
HI_S32 HI_DRV_VDEC_AllocBuf(BUFMNG_INST_CONFIG_S *pstConfig, VDEC_BUFFER_S* pstVDECAllocBuf);
HI_S32 HI_DRV_VDEC_ReleaseBuf(VDEC_BUFFER_S* pstVDECBuf);
#endif
...
/*********************************** Code ************************************/
#ifdef CHIP_TYPE_hi3798mv200
static HI_VOID VDEC_SetVp9Enhance(HI_U8 Value)
{
    HI_S32 s32Ret;
    UINT8 *pCORE_VOLT;
    GPU_EXT_FUNC_S* g_pGpuFunc;
    PMOC_EXT_FUNC_S* g_pPmocFunc;
    if (Value == HI_TRUE)
    {
        s32Ret = HI_DRV_MODULE_GetFunction(HI_ID_GPU, (HI_VOID**)&g_pGpuFunc);
        if ((s32Ret == HI_SUCCESS) && (g_pGpuFunc != HI_NULL))
        {
            g_pGpuFunc->pfnGPUSetMaxFreq(600000);
        }
        pCORE_VOLT = ioremap_nocache( 0xf8a23020, 0x10000) ;
        g_OrgCoreVoltVal = *(UINT32*)pCORE_VOLT;

        s32Ret = HI_DRV_MODULE_GetFunction(HI_ID_PM, (HI_VOID**)&g_pPmocFunc);
        if ((s32Ret == HI_SUCCESS) && (g_pPmocFunc != HI_NULL))
        {
            g_pPmocFunc->pfnPmocSetCPUFreq(1, 1200000);
        }
        isVp9Enhanced = 1;
    }
    else
    {
        pCORE_VOLT = ioremap_nocache( 0xf8a23020, 0x10000) ;
        *(UINT32*)pCORE_VOLT = g_OrgCoreVoltVal;
        s32Ret = HI_DRV_MODULE_GetFunction(HI_ID_GPU, (HI_VOID**)&g_pGpuFunc);
        if ((s32Ret == HI_SUCCESS) && (g_pGpuFunc != HI_NULL))
        {
            g_pGpuFunc->pfnGPUSetMaxFreq(800000);
        }
        s32Ret = HI_DRV_MODULE_GetFunction(HI_ID_PM, (HI_VOID**)&g_pPmocFunc);
        if ((s32Ret == HI_SUCCESS) && (g_pGpuFunc != HI_NULL))
        {
            g_pPmocFunc->pfnPmocSetCPUFreq(0, 1200000);
        }
        isVp9Enhanced = 0;
    }
...
static HI_VOID VDEC_Vp9EnhanceProcess(VDEC_CHANNEL_S* pstChan, HI_BOOL bDestroyChan)
{
    if (pstChan->bNeedVp9Enhance == HI_TRUE)
    {
        if (isVp9Enhanced == 0)
        {
            VDEC_SetVp9Enhance(HI_TRUE);

        }
        else
        {
            if (bDestroyChan == HI_TRUE)
            {
                VDEC_SetVp9Enhance(HI_FALSE);
            }
        }
    }
    return;
...
HI_VOID VDEC_InitFrmBufRecSpinLock(CHAN_FRAME_BUF_RECORD_LOCK_S* pIntrMutex)
{
    spin_lock_init(&pIntrMutex->irq_lock);
    pIntrMutex->isInit = HI_TRUE;
...
/* 中断互斥加锁(关中断且加锁)*/
HI_VOID VDEC_FrmBufRecSpinLockIRQ(CHAN_FRAME_BUF_RECORD_LOCK_S* pIntrMutex)
{
    if (pIntrMutex->isInit == HI_FALSE)
    {
        spin_lock_init(&pIntrMutex->irq_lock);
...
/* 中断互斥解锁(开中断且去锁)*/
HI_VOID VDEC_FrmBufRecSpinUnLockIRQ(CHAN_FRAME_BUF_RECORD_LOCK_S* pIntrMutex)
{
    if (pIntrMutex->isInit == HI_TRUE)
    {
        spin_unlock_irqrestore(&pIntrMutex->irq_lock, pIntrMutex->irq_lockflags);
...
HI_VOID VDEC_InitChanControlStateLock(CHAN_CONTROL_STATE_LOCK_S* pIntrMutex)
{
    spin_lock_init(&pIntrMutex->irq_lock);
    pIntrMutex->isInit = HI_TRUE;
...
HI_VOID VDEC_ChanControlStateSpinLockIRQ(CHAN_CONTROL_STATE_LOCK_S* pIntrMutex)
{
    if (pIntrMutex->isInit == HI_FALSE)
    {
        spin_lock_init(&pIntrMutex->irq_lock);
...
HI_VOID VDEC_ChanControlStateSpinUnlockIRQ(CHAN_CONTROL_STATE_LOCK_S* pIntrMutex)
{
    if (pIntrMutex->isInit == HI_TRUE)
    {
        spin_unlock_irqrestore(&pIntrMutex->irq_lock, pIntrMutex->irq_lockflags);
...
static HI_S32 BUFMNG_VPSS_Init(BUFMNG_VPSS_INST_S* pstBufVpssInst)
{
    ...
    if (HI_DRV_VDEC_BUF_VDEC_ALLOC_MANAGE == pstBufVpssInst->enFrameBuffer)
    {
        for (i = 0; i < pstBufVpssInst->u32BufNum; i++)
        {
            ...
            {
                //HI_VFREE_BUFMNG(pstBufNode);//申请失败不能释放，需要把以前的都释放
            ...
            if (HI_SUCCESS != s32Ret)
            {
                ...
                list_for_each_safe(pos, n, &(pstBufVpssInst->stVpssBufAvailableList))
                {
                    pstTarget = list_entry(pos, BUFMNG_VPSS_NODE_S, node);
                    HI_DRV_VDEC_UnmapAndRelease(&(pstTarget->VDECBuf_frm));
                    HI_DRV_VDEC_UnmapAndRelease(&(pstTarget->VDECBuf_meta));//DRM: meta内存为非安内存 sdk
                    list_del_init(pos);
                    HI_VFREE_BUFMNG(pstTarget);
                }
...
static HI_S32 VDEC_FindVdecHandleByVpssHandle(VPSS_HANDLE hVpss, HI_HANDLE* phVdec)
{
    ...
    {
        if (HI_INVALID_HANDLE == s_stVdecDrv.astChanEntity[i].pstVpssChan->hVdec)
        {
...
static HI_S32 VDEC_FindVdecHandleByPortHandle(HI_HANDLE hPort, HI_HANDLE* phVdec)
{
    ...
    for (i = 0; i < HI_VDEC_MAX_INSTANCE_NEW; i++)
    {
        for (j = 0; j < VDEC_MAX_PORT_NUM; j++)
        {
            ...
            if (hPort == s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].hPort &&
                HI_TRUE == s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].bEnable)
            {
                break;
            }
        }
...
HI_S32 VDEC_FindVpssHandleByVdecHandle(HI_HANDLE hVdec, HI_HANDLE* phVpss)
{
    ...
    {
        *phVpss = s_stVdecDrv.astChanEntity[i].pstVpssChan->hVpss;
        return HI_SUCCESS;
    }
}
...
static HI_S32 VDEC_CheckVpssPortOutFrameStatus(HI_HANDLE hVdec, HI_VOID* pstArgs)
{
    ...
    pstVpssChan = s_stVdecDrv.astChanEntity[hVdec].pstVpssChan;
...
static HI_S32 VDEC_InsertTmpListIntoPortList(HI_HANDLE hVdec, HI_HANDLE hPort)
{
    ...
    pListLock = &(s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].stPortList.stPortFrameListLock);

    VDEC_SpinLockIRQ(pListLock);
    i = s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].s32PortTmpListPos - 1;
    ...
    /*s32LastValidPos = s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].s32PortTmpListPos;  */
    pstVdecPortFrameList = &(s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].stPortList.stVdecPortFrameList);
    ...
    for (; i >= 0; i--)
    {
        ...
        memcpy(&(pstVdecPortFrameListNode->stPortOutFrame), &(s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].astPortTmpList[i].stPortOutFrame), sizeof(HI_DRV_VIDEO_FRAME_S));
        pnode = &(pstVdecPortFrameListNode->node);
        list_add_tail(pnode, pstVdecPortFrameList);
    }

    /*memcpy(&(s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].astPortTmpList[0]),
           &(s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].astPortTmpList[s32LastValidPos]),
           sizeof(VDEC_PORT_FRAME_LIST_NODE_S));*/
    s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].s32PortTmpListPos = 0;
...
static HI_S32 VDEC_InsertFrameIntoTmpList(HI_HANDLE hVdec, HI_HANDLE hPort, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    ...
    s32Index = s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].s32PortTmpListPos;
    ...
    memcpy(&(s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].astPortTmpList[s32Index].stPortOutFrame),
           pstFrame, sizeof(HI_DRV_VIDEO_FRAME_S));

    s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].s32PortTmpListPos ++;
...
static HI_S32 VDEC_RecvPortFrameFromVpss(HI_HANDLE hVdec, HI_HANDLE hPort, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    ...
    hPortTmp = s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[hPort].hPort;
...
static HI_S32 VDEC_Event_NewImage(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, HI_VOID* pArgs)
{
    ...
    if (pstChan->enCurState == VDEC_CHAN_STATE_RUN)
    {
        if (s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32Speed < 0
            || HI_TRUE == s_stVdecDrv.astChanEntity[hHandle].pstChan->bLowdelay
            || HI_TRUE == s_stVdecDrv.astChanEntity[hHandle].pstChan->bWakeUpVpss)
        {
            (void)(s_stVdecDrv.pVpssFunc->pfnVpssSendCommand)
                (s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss,
...
static HI_S32 VDEC_Event_FindIFrame(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, HI_VOID* pArgs)
{
    ...
    if (1 == pstChan->stStatInfo.u32TotalVdecParseIFrame)
    {
        ...
        if (s_stVdecDrv.astChanEntity[hHandle].bO5 == HI_TRUE && s_stVdecDrv.astChanEntity[hHandle].eCallBack)
        {
...
static HI_S32 VDEC_Event_ArrangeFrameBuf(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, HI_VOID* pArgs)
{
    ...
    pstChan->stVFMWReportFrameInfo.u32AllocFrmStartTime = VDEC_GetTimeInMs();
    ...
    else
    {
        ...
        BUFMNG_SpinLockIRQ(&s_stVdecDrv.stRemainFrmList.stIrq);
        {
            ...
            for (i = 0; i < stReportFrmRec.specialFrameNum ; i++)
            {
                ...
                #ifdef HI_TEE_SUPPORT
                    stSpecialFrmInfo.bIsSecFrm = (pstChan->bTvp == HI_TRUE) ? HI_TRUE : HI_FALSE;
                #endif
    ...
    if (s_stVdecDrv.astChanEntity[hHandle].bO5 == HI_TRUE && s_stVdecDrv.astChanEntity[hHandle].eCallBack)
    {
        s_stVdecDrv.astChanEntity[hHandle].eCallBack(hHandle, (HI_U32*)pArgs, VIDDEC_EVT_STREAM_INFO_CHANGE);
    }
...
static HI_S32 VDEC_Event_RecordFrameBufUser(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, HI_VOID* pArgs)
{
    HI_U32 RecordPhyAddr = 0;
    HI_U32 SlotLen = 0;
    HI_U32 FrameNum = 0;
    HI_U32 frame_index = 0;
    HI_U32 buffer_record_index = 0;

    VDEC_ASSERT_RETURN(pstChan != HI_NULL, HI_FAILURE);
    VDEC_ASSERT_RETURN(pArgs != HI_NULL, HI_FAILURE);

    RecordPhyAddr = (HI_U32)((HI_U32*)pArgs)[0];
    SlotLen = (HI_U32)((HI_U32*)pArgs)[1];
    FrameNum = (HI_U32)((HI_U32*)pArgs)[2];

    for(frame_index = 0; frame_index < FrameNum; frame_index++)
    {
        for(buffer_record_index = 0; buffer_record_index < VDEC_MAX_BUFFER_RECORD; buffer_record_index++)
        {
            if (HI_FALSE == pstChan->stFrameBufferRecord[buffer_record_index].bConfigured && HI_FALSE == pstChan->stFrameBufferRecord[buffer_record_index].bAvailable)
            {
                pstChan->stFrameBufferRecord[buffer_record_index].stFrameBuffer.u32StartPhyAddr = RecordPhyAddr + (HI_U32)(frame_index * SlotLen);
                pstChan->stFrameBufferRecord[buffer_record_index].bConfigured = HI_TRUE;
                pstChan->stFrameBufferRecord[buffer_record_index].bAvailable = HI_FALSE;
                break;
            }
        }
    }
...
static const VDECEVENT_COMMAND_NODE g_EVENT_CommandTable[] =
{
    ...
    {EVNT_RECORD_FRAME_BUF_USER,            VDEC_Event_RecordFrameBufUser},
...
HI_S32 FastbackwardCheckForNewIFrame(HI_VDEC_PRIV_FRAMEINFO_S* pstPrivInfo, HI_HANDLE hHandle)
{
    ...
    if ((HI_FALSE != s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32GetFirstVpssFrameFlag)
...
HI_S32 FastbackwardCheckForNewGop(HI_VDEC_PRIV_FRAMEINFO_S* pstPrivInfo, HI_HANDLE hHandle, HI_S32 i)
{
    ...
    if ((HI_TRUE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32GetFirstVpssFrameFlag)
        && ((pstPrivInfo->s32GopNum) != (s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32PortLastFrameGopNum))
        &&(0 < s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32PortTmpListPos)
        )
    {
        return 1;
    }
...
static HI_VOID VDEC_FastbackwardHandle(HI_HANDLE hVdec, HI_HANDLE hHandle, VDEC_VPSSCHANNEL_S* pstVpssChan, HI_VOID* pstArgs)
{
    ...
    for (i = 0; i < VDEC_MAX_PORT_NUM; i++)
    {
        ...
        {
            if (HI_INVALID_HANDLE != s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].hPort
                && (HI_TRUE == pstVpssChan->stPort[i].bEnable))
            {
                ...
                if (HI_SUCCESS == s32Ret)
                {
                    if (HI_TRUE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stControlInfo.u32DispOptimizeFlag)
                    {
                        if (s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].u32LastFrameIndex == pstFrame->u32FrameIndex)
                        {
                            s32Ret = (s_stVdecDrv.pVpssFunc->pfnVpssRelPortFrame)(s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].hPort, pstFrame);
                    ...
                    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].u32LastFrameIndex = pstFrame->u32FrameIndex;
                    pstPrivInfo = (HI_VDEC_PRIV_FRAMEINFO_S*)(((HI_DRV_VIDEO_PRIVATE_S*)(pstFrame->u32Priv))->u32Reserve);
                    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32RecvNewFrame = 1;
                }
                else
                {
                    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32RecvNewFrame = 0;
                    continue;
                }
            }
            else
        ...
        if (HI_FALSE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stControlInfo.u32BackwardOptimizeFlag)
        {
            /*if get a new I frame then insert the tmpFrames to portFrameList from tail to head*/
            ...
            {
                if (s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32RecvNewFrame)
                {
                    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32GetFirstVpssFrameFlag = HI_TRUE;
                    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->eLastFrameFormat = pstPrivInfo->s32FrameFormat;
                }
            }
        }
        else
        {
            ...
            //else
            {
                if (s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32RecvNewFrame)
                {
                    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32GetFirstVpssFrameFlag = HI_TRUE;
                    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32PortLastFrameGopNum = pstPrivInfo->s32GopNum;
                }
                                      ...
                                      HI_VOID* pstArgs)
{
    ...
    ((HI_DRV_VPSS_FRMBUF_S*)pstArgs)->u32StartPhyAddr = stMMZ_Buffer.VDECBuf_frm.u32StartPhyAddr;  //sdk
    ((HI_DRV_VPSS_FRMBUF_S*)pstArgs)->pu8StartVirAddr = stMMZ_Buffer.VDECBuf_frm.pu8StartVirAddr;
    ((HI_DRV_VPSS_FRMBUF_S*)pstArgs)->u32PrivDataPhyAddr = stMMZ_Buffer.VDECBuf_meta.u32StartPhyAddr;  //sdk
                                ...
                                HI_VOID* pstArgs)
{
    ...
    if (HI_DRV_VDEC_BUF_USER_ALLOC_MANAGE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->enFrameBuffer
      ||HI_DRV_VDEC_BUF_VDEC_ALLOC_MANAGE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->enFrameBuffer)
    {
        if (pstArgs == HI_NULL)
        {
        ...
        for (i = 0; i < VDEC_MAX_PORT_NUM; i++)
        {
            if (hPort == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].hPort)
            {
                break;
            }
        }
        ...
        if (HI_DRV_VPSS_BUF_USER_ALLOC_MANAGE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].bufferType)
        {
            hPort = ((HI_DRV_VPSS_FRMINFO_S*)pstArgs)->hPort;
            hVpss = (HI_HANDLE)PORTHANDLE_TO_VPSSID(hPort);
...
static HI_VOID GetCMPInfo(VDEC_CHANNEL_S *pstChan, HI_CHIP_TYPE_E enChipType, HI_CHIP_VERSION_E enChipVersion)
{
    ...
    if (pstChan->enSceneMode == HI_UNF_AVPLAY_SCENE_MODE_VIRTUAL_REAL)
    {
        EnVcmp = 2;
    }
...
#ifdef CHIP_TYPE_hi3798mv200
    HI_DRV_SYS_GetFlag(&b98mv200Vp9Burst);
    if ((b98mv200Vp9Burst == 1)
    && (enChipType == HI_CHIP_TYPE_HI3798M) && (enChipVersion == HI_CHIP_VERSION_V200))
    {
        stVdecChanCfg.b98mv200Vp9Burst = HI_TRUE;
    }
#endif
...
static HI_S32 VDEC_SeekPTS(HI_HANDLE hHandle, HI_U32* pu32SeekPts, HI_U32 u32Gap)
{
    ...
    {
        if (HI_INVALID_HANDLE != s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss)
        {
            /*Stop VPSS*/
            s32Ret = (s_stVdecDrv.pVpssFunc->pfnVpssSendCommand)(s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss, HI_DRV_VPSS_USER_COMMAND_STOP, HI_NULL);
            ...
            /*Reset VPSS*/
            s32Ret = (s_stVdecDrv.pVpssFunc->pfnVpssSendCommand)(s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss, HI_DRV_VPSS_USER_COMMAND_RESET, HI_NULL);
    ...
    *(pu32SeekPts) = pTmpUserData[0];
    ...
    /*start VPSS*/
    s32RetStart = (s_stVdecDrv.pVpssFunc->pfnVpssSendCommand)(s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss, HI_DRV_VPSS_USER_COMMAND_START, HI_NULL);
...
HI_S32 VDEC_GetEsFromDmx(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, STREAM_DATA_S* pstPacket)
{
    ...
    if (s32Ret != HI_SUCCESS)
    {
        /**HI_ERR_DMX_INVALID_PARA indicates the DMX channel is invalid**
         **set the DMX channel to HI_INVALID_HANDLE to stop the readstream**
         **come here, which will prevent the demux from printing handle error log**/
    ...
    if (pstChan->bTvp == HI_TRUE)
    {
    #ifdef HI_SMMZ2_SUPPORT
        pstPacket->SmmuAddr = vidEsBuf.u32BufPhyAddr;
    #else
                         ...
                         HI_BOOL bUserSpace)
{
    ...
    {
#ifdef HI_SMMZ2_SUPPORT
        pstPacket->SmmuAddr = stEsBuf.u32PhyAddr;
#else
...
static HI_S32 VDEC_GotIDRPacket(HI_HANDLE hHandle, STREAM_DATA_S *pstPacket, HI_BOOL bUserSpace)
{
    HI_S32 s32Ret;
    VDEC_CHANNEL_S* pstChan = HI_NULL;
    HI_U32 u32PtsThd = 40;
...
static HI_S32 VDEC_RecvStrmBuf(HI_HANDLE hHandle, STREAM_DATA_S* pstPacket, HI_BOOL bUserSpace)
{
    ...
    {
        s32Ret = VDEC_GotIDRPacket(hHandle, pstPacket, bUserSpace);
        if (s32Ret == HI_SUCCESS)
        {
...
OUT:
         ...
         (pstChan->OmxTunnelMode != 1))
    {
        ...
        if (pstChan->bTvp == HI_TRUE)
        {
            pstPacket->SmmuAddr = pstChan->stEOSBuffer.u32StartPhyAddr;
        }
        else
        {
...
static HI_S32 VDEC_RlsStrmBuf(HI_HANDLE hHandle, STREAM_DATA_S* pstPacket, HI_BOOL bUserSpace)
{
    ...
    if ((pstPacket->PhyAddr == pstChan->stEOSBuffer.u32StartPhyAddr) || (pstPacket->SmmuAddr == pstChan->stEOSBuffer.u32StartPhyAddr))
    {
        VDEC_CHAN_USE_UP(&s_stVdecDrv.astChanEntity[hHandle]);
        return HI_SUCCESS;
    }
    ...
    if ((pstChan->hDmxVidChn != HI_INVALID_HANDLE) && s_stVdecDrv.pDmxFunc && s_stVdecDrv.pDmxFunc->pfnDmxReleaseEs)
    {
        ...
        {
        #ifdef HI_SMMZ2_SUPPORT
            vidEsBuf.u32BufPhyAddr = pstPacket->SmmuAddr;
        #else
...
static HI_S32 VDEC_ConvertWinInfo(HI_HANDLE hPort, HI_DRV_VPSS_PORT_CFG_S* pstVpssPortCfg, HI_DRV_WIN_PRIV_INFO_S* pstWinInfo)
{
    ...
    for (i = 0; i < VDEC_MAX_PORT_NUM; i++)
    {
        if (s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[i].hPort == hPort)
        {
            break;
        }
    }
    ...
    if ( HI_DRV_VPSS_BUF_USER_ALLOC_MANAGE == s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stPort[i].bufferType)
    {
...
static HI_S32 VDEC_Chan_CreateVpss(HI_HANDLE hVdec, HI_HANDLE* phVpss)
{
    ...
    /*保存vpss句柄*/
    s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->hVpss = *phVpss;
    up(&s_stVdecDrv.stSem);


    s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->eFramePackType = HI_UNF_FRAME_PACKING_TYPE_BUTT;
    s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->enFrameBuffer = HI_DRV_VPSS_BUF_VPSS_ALLOC_MANAGE;//HI_DRV_VDEC_BUF_USER_ALLOC_MANAGE;//;

    s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->enVideoBypass = VDEC_VPSS_BYPASSMODE_BUTT;
...
HI_S32 VDEC_Chan_DestroyVpss(HI_HANDLE hVdec)
{
    ...
    {
        s32Ret = (s_stVdecDrv.pVpssFunc->pfnVpssSendCommand)(s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->hVpss, HI_DRV_VPSS_USER_COMMAND_STOP, NULL);
        if (HI_SUCCESS != s32Ret)
        {
        ...
        s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->hVpss = HI_INVALID_HANDLE;
        s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stControlInfo.u32BackwardOptimizeFlag = HI_FALSE;
        s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->stControlInfo.u32DispOptimizeFlag = HI_FALSE;
        s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->s32GetFirstIFrameFlag = HI_FALSE;
        s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->s32GetFirstVpssFrameFlag = HI_FALSE;
        s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->eLastFrameFormat = VDEC_FRAME_BUTT;
        //s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->s32LastFrameGopNum = -1;
        s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->s32ImageDistance = 0;
    }
...
static HI_S32 VDEC_Chan_ResetVpss(HI_HANDLE hVpss)
{
    ...
    for (i = 0; i < VDEC_MAX_PORT_NUM; i++)
    {
        pListLock = &(s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].stPortList.stPortFrameListLock);
        pstVpssChan = s_stVdecDrv.astChanEntity[hHandle].pstVpssChan;
...
static HI_S32 VDEC_Chan_SetExtBuffer(HI_HANDLE hVpss, VDEC_BUFFER_ATTR_PRI_S* pstBufferAttr)
{
    ...
    /*网络播放场景，bufferType == HI_DRV_VPSS_BUF_USER_ALLOC_MANAGE 时,start vpss 的时机需要设置在配置了bypass策略之后 add by sdk */
    ...
    if (HI_DRV_VDEC_BUF_USER_ALLOC_MANAGE == s_stVdecDrv.astChanEntity[i].pstVpssChan->enFrameBuffer)
    {
        //只支持每次配的buf大小一样
...
static HI_S32 VDEC_Chan_SetFrameBufferMode(HI_HANDLE hVpss, VDEC_FRAMEBUFFER_MODE_E enFrameBufferMode)
{
    ...
    {
        s_stVdecDrv.astChanEntity[i].pstVpssChan->enFrameBuffer = HI_DRV_VDEC_BUF_VPSS_ALLOC_MANAGE;
    }

    if (VDEC_BUF_USER_ALLOC_MANAGE == enFrameBufferMode)
    {
        s_stVdecDrv.astChanEntity[i].pstVpssChan->enFrameBuffer = HI_DRV_VDEC_BUF_USER_ALLOC_MANAGE;
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[0].bufferType = HI_DRV_VPSS_BUF_USER_ALLOC_MANAGE;
        BUFMNG_VPSS_Init(&(s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[0].stBufVpssInst));
    }
...
static HI_S32 VDEC_Chan_CheckAndDelBuffer(HI_HANDLE hVpss, VDEC_BUFFER_INFO_S* pstBufInfo)
{
    ...
    {
        HI_ERR_VDEC("next is null error\n");
...
static HI_S32 VDEC_Chan_SetResolution(HI_HANDLE hVpss, VDEC_RESOLUTION_ATTR_S* pstResolution)
{
    ...
    else
    {
        for (j = 0; j < VDEC_MAX_PORT_NUM; j++)
        {
            if (HI_INVALID_HANDLE != s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].hPort)
            {
                break;
            }
        }
        ...
        hPort = s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].hPort;
...
static HI_S32 VDEC_Chan_SetFrmPackingType(HI_HANDLE hVpss, HI_UNF_VIDEO_FRAME_PACKING_TYPE_E eFramePackType)
{
    ...
    s_stVdecDrv.astChanEntity[i].pstVpssChan->eFramePackType = eFramePackType;
...
static HI_S32 VDEC_Chan_GetFrmPackingType(HI_HANDLE hVpss, HI_UNF_VIDEO_FRAME_PACKING_TYPE_E* penFramePackType)
{
    ...
    *penFramePackType = s_stVdecDrv.astChanEntity[i].pstVpssChan->eFramePackType;
...
VOID VDEC_GetColorSpace_Type(IMAGE* pstImage, HI_DRV_COLOR_SPACE_E* pColorSpace)
{
...
HI_DRV_COLOR_SPACE_E VDEC_GetColorSpace(HI_UNF_VCODEC_TYPE_E enType, IMAGE* pstImage)
{
    ...
    switch (enType)
    {
        case HI_UNF_VCODEC_TYPE_HEVC:
        case HI_UNF_VCODEC_TYPE_H264:
            VDEC_GetColorSpace_Type(pstImage, &ColorSpace);
            break;
...
static HI_VOID ConvertFrm_GetBitWidth(HI_UNF_VCODEC_TYPE_E enType,
                                      ...
                                      HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    if (HI_UNF_VCODEC_TYPE_HEVC == enType || HI_UNF_VCODEC_TYPE_VP9 == enType || HI_UNF_VCODEC_TYPE_AVS2 == enType)
    {
...
static HI_VOID ConvertFrm_GetVP6ExtAttr(HI_UNF_VCODEC_TYPE_E enType,
                                          ...
                                          HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    if ((HI_UNF_VCODEC_TYPE_VP6 == enType)
     || (HI_UNF_VCODEC_TYPE_VP6F == enType)
     || (HI_UNF_VCODEC_TYPE_VP6A == enType))
    {
                                           ...
                                           HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    if (HI_TRUE == pstChan->bLowdelay)
    {
        ...
        if (HI_UNF_VCODEC_TYPE_HEVC == pstChan->stCurCfg.enType
         || HI_UNF_VCODEC_TYPE_H264 == pstChan->stCurCfg.enType
         || HI_UNF_VCODEC_TYPE_VP9 == pstChan->stCurCfg.enType
...
{
    pstFrame->u32FrameStreamSize = pstImage->FrameStreamSize;
    pstFrame->u32AvgQp = pstImage->AvgQp;
    pstFrame->u32FrameType = pstImage->format.frame_type;
    pstFrame->u32MaxMV = pstImage->MaxMV;
    pstFrame->u32MinMV = pstImage->MinMV;
    pstFrame->u32AvgMV = pstImage->AvgMV;
    pstFrame->u32SkipRatio = pstImage->SkipRatio;

    HI_INFO_VDEC("FrameStreamSize = %d, AvgQp = %d, FrameType = %d, MaxMV = %d, MinMV = %d, AvgMV = %d, SkipRatio = %d, bDiscard: %d\n",
                               ...
                               IMAGE* pstImage, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    ...
    /*对于h265场流，当码流信息不含宽高比信息时，设置此时的宽高比为分辨率的宽比2倍高，例如，当显示宽高
      为1920:540的时候，其宽高比为1920:1080*/
...
HI_S32 VDEC_DelFrmBufRec(HI_S32 chanID, UINT32 u32BufIndex)
{
    ...
    {
        HI_ERR_VDEC("invalid param pstChan=%p, u32BufIndex=%d!\n", pstChan, u32BufIndex);
        return HI_FAILURE;
    }
...
static HI_S32 VDEC_RecvFastBackFrm(HI_HANDLE hHandle, IMAGE_INTF_S* pstImgInft, IMAGE* pstImage)
{
    ...
    if (VDEC_OK == s32Ret)
    {
        if (HI_FALSE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stControlInfo.u32BackwardOptimizeFlag)
        {
            if (HI_FALSE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32GetFirstIFrameFlag)
            {
                if (VDEC_CheckGetFirstIFrameFlag(pstImage) == HI_TRUE)
                {
                    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32GetFirstIFrameFlag = HI_TRUE;
                    return HI_SUCCESS;
                }
                else
                {
        ...
        else
        {
            if (HI_TRUE == pstImage->DispEnableFlag)
            {
                if (HI_FALSE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32GetFirstIFrameFlag)
                {
                    //if (pstImage->GopNum !=  s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32LastFrameGopNum)
                    {
                        s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32GetFirstIFrameFlag = HI_TRUE;
                        //s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32LastFrameGopNum = pstImage->GopNum;
                        s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32ImageDistance = pstImage->DistanceBeforeFirstFrame;

                        if (0 != s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32ImageDistance)
                        {
                            s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32ImageDistance --;
                            s32Ret = pstImgInft->release_image(pstImgInft->image_provider_inst_id, pstImage);
                ...
                else
                {
                    ...
                    {
                        s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32ImageDistance = pstImage->DispFrameDistance;
                        return HI_SUCCESS;
                    }
                }
            }
...
static HI_S32 VDEC_O5InfoCheck(HI_HANDLE hHandle, VDEC_CHANNEL_S* pstChan, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    HI_U32 u32StreamInfo[7] = {0};
    HI_BOOL bReportFlag = HI_FALSE;

    if (s_stVdecDrv.astChanEntity[hHandle].bO5 == HI_FALSE)
    {
        return HI_SUCCESS;
    }

    if (pstChan->stLastFrm.u32FrameRate != pstFrame->u32FrameRate && pstChan->stLastFrm.u32FrameRate != 0)
    {
        bReportFlag = HI_TRUE;
    }
    if (pstChan->stLastFrm.enSrcFrameType != pstFrame->enSrcFrameType)
    {
        bReportFlag = HI_TRUE;
    }
    if (((pstFrame->u32Width != pstChan->stLastFrm.u32Width) && (pstChan->stLastFrm.u32Width != 0)) ||
        ((pstFrame->u32Height != pstChan->stLastFrm.u32Height) && (pstChan->stLastFrm.u32Height != 0)))
    {
        bReportFlag = HI_TRUE;
    }

    if (bReportFlag == HI_TRUE)
    {
        u32StreamInfo[0] = pstFrame->u32Pts;
        u32StreamInfo[1] = (pstFrame->enSrcFrameType == HI_DRV_VIDEO_FRAME_TYPE_SDR) ? HI_FALSE : HI_TRUE;
        u32StreamInfo[2] = (pstFrame->enSrcFrameType == HI_DRV_VIDEO_FRAME_TYPE_SDR) ? HI_FALSE : pstFrame->enSrcFrameType;
        u32StreamInfo[3] = 0;//uiBitrate not support
        u32StreamInfo[4] = pstFrame->u32FrameRate;
        u32StreamInfo[5] = pstFrame->u32Width;
        u32StreamInfo[6] = pstFrame->u32Height;

        if (s_stVdecDrv.astChanEntity[hHandle].eCallBack)
        {
            s_stVdecDrv.astChanEntity[hHandle].eCallBack(hHandle, (HI_U32*)u32StreamInfo, VIDDEC_EVT_STREAM_INFO_CHANGE);
        }
    }
        ...
        for (u32Index = 0 ; u32Index < 4; u32Index++)
        {
            ...
            switch (u32ID)
            {
                case VDEC_USERDATA_IDENTIFIER_DVB1:
                    ...
                    {
                        ret = USRDATA_Put(hHandle, pstUsrData, HI_UNF_VIDEO_USERDATA_DVB1_CC);
                        if (s_stVdecDrv.astChanEntity[hHandle].bO5 == HI_TRUE && ret == HI_SUCCESS && s_stVdecDrv.astChanEntity[hHandle].eCallBack)
                        {
                            s_stVdecDrv.astChanEntity[hHandle].eCallBack(hHandle, NULL, VIDDEC_EVT_CC_DATA);
                        }
                    }
                    break;
...
static HI_S32 VDEC_DiscardFrm_VR(VDEC_CHANNEL_S* pstChan, IMAGE* pstImage)
{
    IMAGE_INTF_S* pstImgInft = HI_NULL;
    HI_S32 s32Ret = HI_FAILURE;

    if (pstChan == HI_NULL || pstImage == HI_NULL)
    {
        HI_ERR_VDEC("bad param\n");
        return HI_FAILURE;
    }

    pstImgInft = &pstChan->stImageIntf;
    if (pstImage->frame_rate > 45000 && pstImage->image_width > 1920 && pstImage->image_height > 1088)
    {
        if (pstImage->seq_img_cnt % 2 == 0)
        {
            s32Ret = pstImgInft->release_image(pstImgInft->image_provider_inst_id, pstImage);
            if (s32Ret == VDEC_OK)
            {
                pstImage->frame_rate /= 2;
                pstChan->stStatInfo.u32DiscardFrmNum++;
                return HI_SUCCESS;
            }
        }
        pstImage->frame_rate /= 2;
...
static HI_S32 VDEC_RecvFrm(HI_HANDLE hHandle, VDEC_CHANNEL_S* pstChan, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    ...
    if (s_stVdecDrv.astChanEntity[hHandle].bO5 == HI_TRUE && s_stVdecDrv.astChanEntity[hHandle].pstChan->stCurCfg.enMode == HI_UNF_VCODEC_MODE_FIRST_I &&
        pstChan->stStatInfo.u32VdecToVpssFrameOK >= 1)
    {
#ifdef VFMW_VPSS_BYPASS_EN
        BUFMNG_SpinUnLockIRQ(&s_stVdecDrv.stRemainFrmList.stIrq);
#endif
    ...
    if (pstChan->enSceneMode == HI_UNF_AVPLAY_SCENE_MODE_VIRTUAL_REAL)
    {
        s32Ret = VDEC_DiscardFrm_VR(pstChan, &stImage);
        if (s32Ret == HI_SUCCESS)
        {
            return HI_FAILURE;
        }
    ...
    {
        stImage.frame_rate = g_CHAN_FRAME_RATE[hHandle] * 1000;
    }


    s32FrameRate = stImage.frame_rate / 1000;

    if (s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32Speed < 0)
    {
        s32FrameRate = 60;
        s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_SET_FRAME_RATE, &s32FrameRate, sizeof(s32FrameRate));
    ...
    VDEC_O5InfoCheck(hHandle, pstChan, pstFrame);
    ...
    pstStatInfo->u32VdecToVpssFrameOK++;
...
HI_S32 HI_DRV_VDEC_RlsFrmBufWithoutHandle(HI_DRV_VIDEO_FRAME_S* pstFrm)
{
    ...
    s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(HI_INVALID_HANDLE, VDEC_CID_GET_CHAN_ID_BY_MEM, &u32Yaddr, sizeof(u32Yaddr));

    if (VDEC_OK != s32Ret)
    {
        HI_INFO_VDEC("VMFW GET_CHAN_ID_BY_MEM err!\n");
        return HI_FAILURE;
    }
...
static HI_S32 VDEC_Chan_setAllocEn(HI_HANDLE hHandle, HI_U32 set_dat)
{
    VDEC_CHANNEL_S*  pstChan = HI_NULL;
    if (hHandle == HI_INVALID_HANDLE || hHandle >= HI_VDEC_MAX_INSTANCE_NEW)
    {
        return HI_FAILURE;
    }
    if (s_stVdecDrv.astChanEntity[hHandle].pstChan == HI_NULL)
    ...
    if ((pstChan->stCurCfg.enType != HI_UNF_VCODEC_TYPE_H263)
            && (pstChan->stCurCfg.enType != HI_UNF_VCODEC_TYPE_MJPEG)
            && (pstChan->stCurCfg.enType != HI_UNF_VCODEC_TYPE_SORENSON))
    {
        pstChan->stOption.u32DynamicFrameStoreAllocEn = set_dat;
    }
    return HI_SUCCESS;
}
...
static HI_S32 VDEC_Chan_InitParam(HI_HANDLE hHandle, HI_UNF_AVPLAY_OPEN_OPT_S* pstCapParam)
{
    ...
    /*prevent the Chan from being cteated again*/
    if(pstChan->isInitAlready == HI_TRUE)
    {
        HI_ERR_VDEC("Chan:%d has been inited\n", hHandle);
        goto err0;
    }
    ...
    pstChan->stOption.u32DynamicFrameStoreAllocEn = ((MaskCtrlWord & 0x1) == 1) ? 0 : s_stVdecDrv.astChanEntity[hHandle].u32DynamicFsEn; //sdk 使能动态分配功能
    pstChan->stOption.s32ExtraFrameStoreNum = PLUS_FS_NUM + 1;//sdk PLUS_FS_NUM + 1:流畅播放防止阻塞
    pstChan->stOption.s32DelayTime = 0;
    pstChan->stOption.u32CfgFrameNum = 0;//sdk 配置帧数
    pstChan->stOption.u32MaxMemUse = -1;//sdk最大可用帧存
    ...
    pCapLevel_Handler = VDEC_CapLEVEL_Handler(pstChan->enCapToFmw);
    ...
    pstChan->stOption.s32ExtraFrameStoreNum = DispFrameNum;
    ...
    if ((1 != pstChan->stOption.u32DynamicFrameStoreAllocEn) && (CAP_LEVEL_MVC_FHD == pstChan->enCapToFmw))
    {
    ...
    pstChan->isInitAlready = HI_TRUE;
    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVdec = hHandle;
...
static HI_S32 VDEC_Chan_AllocHandle(HI_HANDLE* phHandle, struct file* pstFile)
{
    ...
    s_stVdecDrv.astChanEntity[i].pstVpssChan->hVdec = i;
    s_stVdecDrv.astChanEntity[i].pstVpssChan->hVpss = HI_INVALID_HANDLE;
    s_stVdecDrv.astChanEntity[i].pstVpssChan->stControlInfo.u32BackwardOptimizeFlag = HI_FALSE;
    s_stVdecDrv.astChanEntity[i].pstVpssChan->stControlInfo.u32DispOptimizeFlag = HI_FALSE;
    s_stVdecDrv.astChanEntity[i].pstVpssChan->s32GetFirstIFrameFlag = HI_FALSE;
    s_stVdecDrv.astChanEntity[i].pstVpssChan->eLastFrameFormat = VDEC_FRAME_BUTT;
    //s_stVdecDrv.astChanEntity[i].pstVpssChan->s32LastFrameGopNum = -1;
    s_stVdecDrv.astChanEntity[i].pstVpssChan->s32Speed = 1024;
    s_stVdecDrv.astChanEntity[i].pstVpssChan->s32ImageDistance = 0;

    for (j = 0; j < VDEC_MAX_PORT_NUM; j++)
    {
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].hPort = HI_INVALID_HANDLE;
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].bEnable = HI_FALSE;
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].enPortType = VDEC_PORT_TYPE_BUTT;
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].s32PortTmpListPos = 0;
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].s32GetFirstVpssFrameFlag = HI_FALSE;
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].s32RecvNewFrame = HI_FALSE;
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].s32PortLastFrameGopNum = -1;
        s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].u32LastFrameIndex = -1;
        INIT_LIST_HEAD(&s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].stPortList.stVdecPortFrameList);
        s32Ret = VDEC_InitSpinLock(&s_stVdecDrv.astChanEntity[i].pstVpssChan->stPort[j].stPortList.stPortFrameListLock);
...
static HI_S32 VDEC_Chan_FreeHandle(HI_HANDLE hHandle)
{
    ...
    //}
    /*Free chan before Free handle*/
    pstChan = s_stVdecDrv.astChanEntity[hHandle].pstChan;
    if(NULL != pstChan && pstChan->hChan != HI_INVALID_HANDLE)
    {
        s32Ret = VDEC_Chan_Free(hHandle);
        if(s32Ret != HI_SUCCESS)
        {
            HI_ERR_VDEC("Chan:%d Call VDEC_Chan_Free fail!\n", hHandle);
            return HI_FAILURE;
        }
    }
    ...
    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVdec = HI_INVALID_HANDLE;
...
static HI_S32 VDEC_Chan_GetMemSize(HI_HANDLE hHandle)
{
    ...
    ((HI_U64*)as8TmpBuf)[1] = (HI_U64)(HI_SIZE_T)(&pstChan->stOption);  //@sdk 64bit
    s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(HI_INVALID_HANDLE, VDEC_CID_GET_CHAN_MEMSIZE, as8TmpBuf, sizeof(as8TmpBuf));
...
HI_S32 VDEC_Chan_AllocMemForTEE(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle)
{
...
#ifdef HI_TEE_SUPPORT
    BUFMNG_INST_CONFIG_S stBufConfig;
#endif
    if (pstChan->stMemSize.ScdDetailMem > 0)
    {
    #ifdef HI_TEE_SUPPORT
        stBufConfig.bTvp = HI_TRUE;
        stBufConfig.hVdec = hHandle;
        stBufConfig.u32Size = HI_VDEC_EOS_MEM_SIZE;
        strncpy(stBufConfig.aszName, "VDEC_Sec_Eos", 16);
        Ret = HI_DRV_VDEC_AllocBuf(&stBufConfig, &pstChan->stEOSBuffer);
    #else
        Ret = HI_DRV_VDEC_Alloc_TVP("VDEC_Sec_Eos", HI_NULL, HI_VDEC_EOS_MEM_SIZE, 0, &pstChan->stEOSBuffer);
    #endif
        if (HI_SUCCESS != Ret)
        {
    ...
    pstChan->stOption.MemDetail.ChanMemEs.Length   = pstChan->stEsBuf.u32Size;
    pstChan->stOption.MemDetail.ChanMemEs.PhyAddr  = pstChan->stEsBuf.u32StartPhyAddr;
    pstChan->stOption.MemDetail.ChanMemEs.VirAddr  = HI_NULL;
...
HI_S32 VDEC_Chan_AllocVFMWMem(HI_HANDLE hHandle)
{
...
#endif
    {
        ...
        if (u32VDHSize > 0)
        {
            ...
            if (1 != pstChan->stOption.u32DynamicFrameStoreAllocEn) //sdk 采用动态分配的方案，预分配在这里由于不知道真正需要的帧存大小，所以不能去预分节点。
            {
...
#endif
            {
                if (1 == pstChan->stOption.u32DynamicFrameStoreAllocEn) //如果采用动态帧存的方案,则在这里不需要了整块申请了 sdk//
                {
                    pstChan->stVDHMMZBuf.u32Size = 0;
                    pstChan->stVDHMMZBuf.u32StartPhyAddr = HI_NULL;
                    pstChan->stVDHMMZBuf.pu8StartVirAddr = HI_NULL;
...
HI_S32 VDEC_Chan_FreeVFMWMem(HI_HANDLE hHandle)
{
    ...
    if (HI_TRUE == pstChan->bTvp)
    {
        ...
        {
        #ifdef HI_TEE_SUPPORT
            HI_DRV_VDEC_ReleaseBuf(&pstChan->stEOSBuffer);
        #else
            HI_DRV_VDEC_Release_TVP(&pstChan->stEOSBuffer);
        #endif
        }
...
static HI_S32 VDEC_Chan_Create(HI_HANDLE hHandle)
{
    ...
    {
    #ifdef HI_SMMZ2_SUPPORT
        pstChan->stOption.s32ScdInputMmuEn = SCD_INPUT_MMZ;
    #else
...
static HI_S32 VDEC_Rls_Frm_Occupied_by_Vdec(HI_HANDLE hHandle)
{
    ...
    /* Check and get pstChan pointer */
    if (HI_NULL == s_stVdecDrv.astChanEntity[hHandle].pstChan || HI_NULL == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan)
    {
        HI_ERR_VDEC("Chan %d not init!\n", hHandle);
        return HI_FAILURE;
    }
...
static HI_S32 VDEC_Chan_Destroy(HI_HANDLE hHandle)
{
    ...
    if (isVp9Enhanced == 1)
    {
        VDEC_Vp9EnhanceProcess(pstChan, HI_TRUE);
    }
#endif
    ...
    if (HI_INVALID_HANDLE != pstChan->hChan)
    {
        ...
        if (VDEC_OK != s32Ret)
        {
            ...
            /*if not return, VFMW MEM will be released when the vfmw chan still exists*
             *the vfmw will visit the mempry where it is invalid now, will lead to kernel panic*/
            return HI_FAILURE;
        }
...
HI_S32 HI_DRV_VDEC_ChanStart(HI_HANDLE hHandle)
{
    ...
    {
        s32Ret = (s_stVdecDrv.pVpssFunc->pfnVpssSendCommand)(s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss, HI_DRV_VPSS_USER_COMMAND_START, NULL);
...
HI_S32 HI_DRV_VDEC_ChanStop(HI_HANDLE hHandle)
{
    ...
    if (HI_INVALID_HANDLE != s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss)
    {
        s32Ret = (s_stVdecDrv.pVpssFunc->pfnVpssSendCommand)(s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss, HI_DRV_VPSS_USER_COMMAND_STOP, NULL);
    ...
    //{
        ...
        //return HI_FAILURE;
    //}
    /*reset vpss before VFMW decode channel destroy(reset VDH) change by sdk*/
    if (HI_INVALID_HANDLE != s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss)
    {
        VDEC_Chan_ResetVpss(s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss);
    }

    if (pstChan->OmxTunnelMode != 1)
    {
        /* Destroy VFMW decode channel */
        s32Ret = VDEC_Chan_Destroy(hHandle);
        if (HI_SUCCESS != s32Ret)
...
static HI_S32 VDEC_Chan_Reset(HI_HANDLE hHandle, HI_DRV_VDEC_RESET_TYPE_E enType)
{
    ...
    if (HI_INVALID_HANDLE != pstChan->hChan)
    {
        ...
        if ((pstChan->bKeepSpsPps) || (pstChan->OmxTunnelMode))
        {
            stResetOption.s32KeepBS      = 0;
            stResetOption.s32KeepSPSPPS  = 1;
            stResetOption.s32KeepFSP     = 1;
            s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_RESET_CHAN_WITH_OPTION, &stResetOption, sizeof(stResetOption));
            if (VDEC_OK != s32Ret)
            {
                HI_ERR_VDEC("Chan %d RESET_CHAN WITHOPTION err!\n", pstChan->hChan);
            }
        }
        else
        {
...
HI_S32 DRV_VDEC_GetChanAttr(HI_HANDLE hHandle, VDEC_VCODEC_ATTR_S* pstCfgParam)
{
    ...
    //   *pstCfgParam = s_stVdecDrv.astChanEntity[hHandle].pstChan->stCurCfg;  //@sdk for 64bit
...
HI_S32 HI_DRV_VDEC_GetChanStatusInfo(HI_HANDLE hHandle, VDEC_STATUSINFO_S* pstStatus)
{
    ...
    for (i = 0; i < VDEC_MAX_PORT_NUM; i++)
    {
        if (VDEC_PORT_TYPE_MASTER == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].enPortType)
        {
            hMasterPortHandle = s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].hPort;
            break;
        }
        else if (VDEC_PORT_TYPE_VIRTUAL == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].enPortType)
        {
            hVirPortHandle = s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].hPort;
        }
    }
    ...
    if (HI_INVALID_HANDLE != hPortHandle)
    {
        ...
        if (HI_DRV_VPSS_BUF_VPSS_ALLOC_MANAGE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].bufferType)
        {
        ...
        else if (HI_DRV_VPSS_BUF_USER_ALLOC_MANAGE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].bufferType)
        {
            pstStatus->u32VfmwTotalDispFrmNum = s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].stBufVpssInst.u32AvaiableFrameCnt;
        }
    }
...
HI_S32 HI_DRV_VDEC_SetColorSpace(HI_HANDLE hHandle, HI_U32 u32UsrColorSpace)
{
    ...
    hVpss = s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->hVpss;
...
HI_S32 HI_DRV_VDEC_SetSceneMode(HI_HANDLE hHandle, HI_UNF_AVPLAY_SCENE_MODE_E eSceneMode)
{
    ...
    if (eSceneMode != HI_UNF_AVPLAY_SCENE_MODE_VIRTUAL_REAL)
    {
    ...
    else
    {
        ...
        if (s32Ret == HI_SUCCESS)
        {
            stVpssPortCfg.bPassThrough = HI_FALSE;
            pstVpssChan->enVideoBypass = VDEC_VPSS_BYPASSMODE_DISABLE;

            if (HI_SUCCESS != (s_stVdecDrv.pVpssFunc->pfnVpssSetPortCfg)(pstVpssChan->stPort[0].hPort, &stVpssPortCfg))
            {
                HI_ERR_VDEC("pfnVpssSetPortCfg set vpssbypass failed!\n");
                return HI_FAILURE;
            }
        }
        else
        ...
        if (s_stVdecDrv.astChanEntity[hHandle].pstChan != NULL)
        {
            s_stVdecDrv.astChanEntity[hHandle].pstChan->enSceneMode = eSceneMode;
        }
        else
        {
            HI_ERR_VDEC("vdec chan %d not init\n", hHandle);
            return HI_FAILURE;
        }
    }
...
HI_S32 HI_DRV_VDEC_SetTrickMode(HI_HANDLE hHandle, HI_UNF_AVPLAY_TPLAY_OPT_S* pstOpt)
{
    ...
    if (s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32Speed != s32Speed)
    {
        for (i = 0; i < VDEC_MAX_PORT_NUM; i++)
        {
            s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32RecvNewFrame = HI_FALSE;
            //s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32PortTmpListPos = 0;
            s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32GetFirstVpssFrameFlag = 0;
            s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].s32PortLastFrameGopNum = -1;
            s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].u32LastFrameIndex = -1;
        }
    }
    ...
    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->s32Speed = s32Speed;
#if 1

    if ((s32Speed > 1024) && (HI_TRUE == s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stControlInfo.u32DispOptimizeFlag))
    {
...
HI_S32 HI_DRV_VDEC_SetCtrlInfo(HI_HANDLE hHandle, HI_UNF_AVPLAY_CONTROL_INFO_S* pstCtrlInfo)
{
    ...
    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stControlInfo.u32BackwardOptimizeFlag = pstCtrlInfo->u32BackwardOptimizeFlag;
    s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stControlInfo.u32DispOptimizeFlag = pstCtrlInfo->u32DispOptimizeFlag;
...
static HI_S32 VDEC_Ioctl_ChanAlloc(struct file*  filp, unsigned int cmd, void* arg)
{
    ...
    if (((VDEC_CMD_ALLOC_S *)arg)->u32DFBEn > 1)  //sdk
    {
        ((VDEC_CMD_ALLOC_S *)arg)->u32DFBEn = 0;
    }

    if (((VDEC_CMD_ALLOC_S *)arg)->u32TunnelModeEnable > 1) //sdk
    {
        ((VDEC_CMD_ALLOC_S *)arg)->u32TunnelModeEnable = 0;
    }

    s_stVdecDrv.astChanEntity[hHandle].u32DynamicFsEn = ((VDEC_CMD_ALLOC_S *)arg)->u32DFBEn;
...
static HI_S32 VDEC_Ioctl_CreatEsBuffer(struct file*  filp, unsigned int cmd, void* arg)
{
    ...
    {
        HI_ERR_VDEC("create size=%d es buf fail!\n", ((HI_DRV_VDEC_STREAM_BUF_S*)arg)->u32Size);
        return HI_ERR_VDEC_INVALID_PARA;
    }
...
static HI_S32 VDEC_Ioctl_RcvVpssFrm(struct file*  filp, unsigned int cmd, void* arg)
{
    ...
    if ( s32Ret == HI_SUCCESS)
    {
        if (s_stVdecDrv.astChanEntity[hVdec].pstVpssChan->s32Speed < 0)
        {
...
static HI_S32 VDEC_ReleaseMemForDecoder(HI_HANDLE hHandle)
{
    ...
    for (i = 0; i < VDEC_MAX_BUFFER_RECORD; i++)
    {
...
#endif
        {
            ...
            {
                HI_DRV_VDEC_ReleaseMem(&pstChan->stFrameBufferRecord[i].stFrameBuffer, pstChan->bTvp, g_bMapFrmEnable);
            }
            ...
            {
                HI_DRV_VDEC_ReleaseMem(&pstChan->stFrameBufferRecord[i].stPmvBuffer, pstChan->bTvp, HI_TRUE);
            }
...
static HI_S32 VDEC_SetUseableBufferToDecoder(HI_HANDLE hHandle)
{
    ...
    for (i = 0; i < VDEC_MAX_BUFFER_RECORD; i++)
    {
        if (HI_FALSE == pstChan->stFrameBufferRecord[i].bConfigured && HI_TRUE == pstChan->stFrameBufferRecord[i].bAvailable)
        {
            ...
            s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_ALLOC_MEM_TO_DECODER, (HI_VOID *)&stFrameBuf, sizeof(stFrameBuf));
            if (s32Ret == HI_SUCCESS)
            {
...
static HI_VOID GetAllocMemInfo(VDEC_CHANNEL_S* pstChan, VDEC_AllocMem_Info_S *pstAllocMemInfo)
{
    ...
    {
        if (pstChan->stVFMWReportFrameInfo.u32ImageNum + pstChan->stOption.s32ExtraFrameStoreNum >= pstChan->stVFMWConfiguredBufferNum.u32ImageNum)
        {
...
static HI_S32 CheckIsNeedAllocMem(HI_S32 ChanID, VDEC_CHANNEL_S* pstChan, VDEC_AllocMem_Info_S *pstAllocMemInfo)
{
    if ((1 == pstChan->stOption.u32DynamicFrameStoreAllocEn)
        && (pstAllocMemInfo->u32NeededFrameNum > 0)
        && (pstChan->stVFMWReportFrameInfo.u32ImageNum > 0)
        && (g_ChanControlState[ChanID] == NORMAL_CHAN))
    {
        return 1;
    }
                             ...
                             FRAME_BUF_S   *pstFrameBuf)
{
    ...
    {
        //预分配分配不到的话直接从系统里面分配
        s32Ret = HI_DRV_VDEC_AllocMem("VFMW_FRM_BUF", NULL, pstMemBuffer->u32Size, 0, pstMemBuffer, pstChan->bTvp, g_bMapFrmEnable);
                                ...
                                FRAME_BUF_S   *pstFrameBuf)
{
    ...
    if (pstAllocMemInfo->u32NeededPmvNum != 0)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            ...
            if (HI_TRUE == pstChan->bTvp)
            {
                s32Ret = HI_DRV_VDEC_AllocMem("VFMW_PMV_BUF", NULL, pstMemBuffer->u32Size, 0, pstMemBuffer, pstChan->bTvp, HI_TRUE);
            }
            else
        #endif
            {
                ...
                if (HI_SUCCESS == s32Ret)
                {
                    s32Ret = HI_DRV_VDEC_MapCache(pstMemBuffer);
                }
            }
                               ...
                               FRAME_BUF_S   *pstFrameBuf)
{
    ...
    if (CheckIsAllMemReady(pstAllocMemInfo))
    {
        ...
        s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_ALLOC_MEM_TO_DECODER, (HI_VOID *)pstFrameBuf, sizeof(FRAME_BUF_S));
        if (s32Ret == HI_SUCCESS)
        {
            if (u32Index < VDEC_MAX_BUFFER_RECORD)
            {
            ...
            if (pstChan->stVFMWConfiguredBufferNum.u32ImageNum == 1)
            {
                pstChan->stVFMWReportFrameInfo.u32AllocFrmEndTime = VDEC_GetTimeInMs();
            }
...
static HI_S32 VDEC_AllocMemForDecoder(HI_HANDLE hHandle)
{
    ...
    if (pstChan->stVFMWReportFrameInfo.u32AllocFrameOnly == 1)
    {
        ...
        for (u32Index = 0; u32Index < VDEC_MAX_BUFFER_RECORD; u32Index++)
        {
            ...
            {
                HI_DRV_VDEC_ReleaseMem(&pstChan->stFrameBufferRecord[u32Index].stFrameBuffer, pstChan->bTvp, g_bMapFrmEnable);
    ...
    if (CheckIsNeedAllocMem(hHandle, pstChan, &stAllocMemInfo))
    {
...
static SINT32 VdecTaskFunc(VOID* param)   //sdk
{
    UINT32 i = 0;
...
#ifdef CHIP_TYPE_hi3798mv200
    HI_CHIP_TYPE_E          enChipType      = HI_CHIP_TYPE_HI3716C;
    HI_CHIP_VERSION_E       enChipVersion   = HI_CHIP_VERSION_V100;
    HI_U32                  b98mv200Vp9burst = 0;
#endif
    ...
    while (1)
    {
        ...
        for (i = 0; i < HI_VDEC_MAX_INSTANCE_NEW; i++)
        {
            ...
            if ((HI_NULL != pstChan) && (VDEC_CHAN_STATE_RUN == pstChan->enCurState))
            {
                ...
                &&(enChipType == HI_CHIP_TYPE_HI3798M) && (enChipVersion == HI_CHIP_VERSION_V200))
                {
                    if (pstChan->stCurCfg.enType == HI_UNF_VCODEC_TYPE_VP9
                    && pstChan->stVFMWReportFrameInfo.u32ImageSize >= VP9_IMGSIZE_8BIT)
                    {
                        pstChan->bNeedVp9Enhance = HI_TRUE;
                    }
                    else
                    {
                        pstChan->bNeedVp9Enhance = HI_FALSE;
                    }
                    VDEC_Vp9EnhanceProcess(pstChan, HI_FALSE);
                }
#endif
            ...
            for (j = 0; j<num && num>0; j++)
            {
               ...
               if ( HI_SUCCESS == s32Ret )
               {
                   ...
                   else
                   {
                       ...
                       s_stVdecDrv.stRemainFrmList.stSpecialFrmRec[u32Index].frmBufRec.u32StartPhyAddr);
                   }
               }
            }
        }
...
HI_S32 VDEC_DRV_FreeAll(HI_HANDLE hHandle)
{
    ...
    if (HI_INVALID_HANDLE != hVpss)
    {
        for (i = 0; i < VDEC_MAX_PORT_NUM; i++)
        {
            hPort = s_stVdecDrv.astChanEntity[hHandle].pstVpssChan->stPort[i].hPort;
...
static HI_S32 VDEC_VpssChan_Init(HI_VOID)
{
    HI_U32 i;
    HI_VOID *pstVpss = HI_NULL;

    pstVpss = HI_VMALLOC_VDEC(HI_VDEC_MAX_INSTANCE_NEW * sizeof(VDEC_VPSSCHANNEL_S));
    if (pstVpss == HI_NULL)
    {
        HI_ERR_VDEC("Vmalloc VPSSCHANNEL error\n");
        return HI_FAILURE;
    }

    for (i = 0; i < HI_VDEC_MAX_INSTANCE_NEW; i++)
    {
        s_stVdecDrv.astChanEntity[i].pstVpssChan = (VDEC_VPSSCHANNEL_S *)(pstVpss + i * sizeof(VDEC_VPSSCHANNEL_S));
    }

    return HI_SUCCESS;
}

static HI_VOID VDEC_VpssChan_Deinit(HI_VOID)
{
    HI_U32 i;
    HI_VOID *pstVpss = HI_NULL;

    pstVpss = (HI_VOID *)(s_stVdecDrv.astChanEntity[0].pstVpssChan);
    if (pstVpss != HI_NULL)
    {
        for (i = 0; i < HI_VDEC_MAX_INSTANCE_NEW; i++)
        {
            s_stVdecDrv.astChanEntity[i].pstVpssChan = HI_NULL;
        }

        HI_VFREE_VDEC(pstVpss);
        pstVpss = HI_NULL;
    }
}
...
static HI_S32 VDEC_OpenDev(HI_VOID)
{
    ...
    for (i = 0; i < HI_VDEC_MAX_INSTANCE_NEW; i++)
    {
        ...
        if (s_stVdecDrv.astChanEntity[i].pstVpssChan != HI_NULL)
        {
            memset(s_stVdecDrv.astChanEntity[i].pstVpssChan, 0, sizeof(VDEC_VPSSCHANNEL_S));
            s_stVdecDrv.astChanEntity[i].pstVpssChan->hVdec = HI_INVALID_HANDLE;
            s_stVdecDrv.astChanEntity[i].pstVpssChan->hVpss = HI_INVALID_HANDLE;
        }
        else
        {
            HI_ERR_VDEC("pstVpssChan not init\n");
            return HI_FAILURE;
        }
        s_stVdecDrv.astChanEntity[i].pstChan = HI_NULL; //sdk
        s_stVdecDrv.astChanEntity[i].u32DynamicFsEn = 0;//sdk
    }
...
HI_S32 VDEC_DRV_Init(HI_VOID)
{
    ...
    if (VDEC_VpssChan_Init() != HI_SUCCESS)
    {
        HI_ERR_VDEC("Init pstVpssChan in vdec error\n");
        return HI_FAILURE;
    }
...
{
    HI_S32 s32Ret;
    s32Ret = VDEC_Chan_setAllocEn((*phHandle) & 0xff, set_dat);//设置动态帧存
    return s32Ret;
}
...
HI_S32 HI_DRV_VDEC_AllocChan(HI_HANDLE* phHandle, HI_UNF_AVPLAY_OPEN_OPT_S* pstCapParam)
{
    ...
    //dynamic frame store in mce
    s_stVdecDrv.astChanEntity[(*phHandle)& 0xff].u32DynamicFsEn = 1;
...
HI_S32 HI_DRV_VDEC_SeekPTS(HI_HANDLE hHandle, HI_U32* pu32SeekPts, HI_U32 u32Gap)
{
    /* check input parameters */
    HI_S32 s32Ret;
    if (HI_NULL == pu32SeekPts)
    {
        HI_ERR_VDEC("Bad param!\n");
        return HI_ERR_VDEC_INVALID_PARA;
    }

    s32Ret = VDEC_SeekPTS(hHandle, pu32SeekPts, u32Gap);

    if (s32Ret == VDEC_OK)
    {
        if (s_stVdecDrv.astChanEntity[hHandle].bO5 == HI_TRUE && s_stVdecDrv.astChanEntity[hHandle].eCallBack)
        {
            s_stVdecDrv.astChanEntity[hHandle].eCallBack(hHandle, NULL, VIDDEC_EVT_PTS_MATCH);
        }
    }
...
HI_S32 HI_DRV_VDEC_SetO5Flag(HI_HANDLE hHandle, HI_BOOL flag)
{
    ...
    down(&s_stVdecDrv.stSem);
    s_stVdecDrv.astChanEntity[hHnd].bO5= flag;
    up(&s_stVdecDrv.stSem);
...
HI_S32 HI_DRV_VDEC_GlobalRelFrm(HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    ...
    if ( HI_SUCCESS != s32Ret )
    {
        ...
        s32Ret = HI_DRV_VDEC_RlsFrmBufWithoutHandle(pstFrame);
        if (s32Ret != HI_SUCCESS)
        {
            HI_INFO_VDEC("HI_DRV_VDEC_RlsFrmBufWithoutHandle err!\n");
        }
...
HI_S32 HI_DRV_VDEC_AllocBuf(BUFMNG_INST_CONFIG_S *pstConfig, VDEC_BUFFER_S* pstVDECAllocBuf)
{
    ...
    HI_HANDLE hVdec;
    VDEC_CHANNEL_S* pstChan = HI_NULL;
    VDEC_MEM_S stVdecBuf;

    hVdec = pstConfig->hVdec & 0xff;

    pstChan = s_stVdecDrv.astChanEntity[hVdec].pstChan;
    if (pstChan == HI_NULL)
    {
        HI_ERR_VDEC("pstChan vdec: 0x%x, is null\n", hVdec);
        return s32Ret;
    }

    stVdecBuf.Length = pstConfig->u32Size;
#ifdef HI_SMMZ2_SUPPORT
    stVdecBuf.eMode = MEM_SMMZ2;
#else
    stVdecBuf.eMode = MEM_MMU_MMU;
#endif
    stVdecBuf.Tvp = HI_TRUE;
    stVdecBuf.hHandle = pstConfig->hVdec;
    strncpy(stVdecBuf.aszName, pstConfig->aszName, 16);
    s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(HI_INVALID_HANDLE, VDEC_CID_ALLOC_SEC_BUF, &stVdecBuf, sizeof(stVdecBuf));

    if (s32Ret != VDEC_OK)
    {
        HI_ERR_VDEC("alloc smmz2 es buf error %d\n", s32Ret);
        return s32Ret;
    }

    pstVDECAllocBuf->u32StartPhyAddr = stVdecBuf.MmuAddr;
    pstVDECAllocBuf->u32Size = stVdecBuf.Length;
...
HI_S32 HI_DRV_VDEC_ReleaseBuf(VDEC_BUFFER_S* pstVDECBuf)
{
    ...
    stVdecBuf.MmuAddr = pstVDECBuf->u32StartPhyAddr;
    stVdecBuf.Length = pstVDECBuf->u32Size;
#ifdef HI_SMMZ2_SUPPORT
    stVdecBuf.eMode = MEM_SMMZ2;
#else
    stVdecBuf.eMode = MEM_MMU_MMU;
#endif
    stVdecBuf.Tvp = HI_TRUE;
    s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(HI_INVALID_HANDLE, VDEC_CID_FREE_SEC_BUF, &stVdecBuf, sizeof(stVdecBuf));

    if (s32Ret != VDEC_OK)
    {
        HI_ERR_VDEC("release smmz2 es buf error %d\n", s32Ret);
        return s32Ret;
    }
...
EXPORT_SYMBOL(HI_DRV_VDEC_SetTrickMode);
EXPORT_SYMBOL(HI_VDEC_Chan_setAllocEn);
EXPORT_SYMBOL(HI_DRV_VDEC_ResetChanStatInfo);
EXPORT_SYMBOL(HI_DRV_VDEC_SeekPTS);
EXPORT_SYMBOL(HI_DRV_VDEC_SetO5Flag);
#ifdef HI_TEE_SUPPORT
EXPORT_SYMBOL(HI_DRV_VDEC_SetTvp);
#endif
