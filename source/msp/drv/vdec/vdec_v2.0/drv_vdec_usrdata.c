...
HI_S32 USRDATA_Alloc(HI_HANDLE hHandle, HI_DRV_VDEC_USERDATABUF_S *pstBuf)
{
    ...
    if ((hHandle >= HI_VDEC_MAX_INSTANCE_NEW)
      ||(pstBuf == HI_NULL)
      ||(pstBuf->UserDataType >= VDEC_USRDATA_TYPE_INDEX_BUTT)||(g_stUsrData[hHandle][pstBuf->UserDataType].bUsed))
    {
...
{
    if (hHandle >= HI_VDEC_MAX_INSTANCE_NEW || UserDataType >= VDEC_USRDATA_TYPE_INDEX_BUTT || UserDataType < 0 || (u64Addr == 0))
    {
        return HI_FAILURE;
    }
...
HI_S32 USRDATA_Acq(HI_HANDLE hHandle, VDEC_VIDEO_USERDATA_S *pstUsrData, HI_UNF_VIDEO_USERDATA_TYPE_E *penType)
{
    ...
    else
    {
        *penType = stBuf.u32Marker;
        if (stBuf.pu8KnlVirAddr == HI_NULL)
        {
            return HI_FAILURE;
        }
...
HI_S32 USRDATA_Rls(HI_HANDLE hHandle, VDEC_VIDEO_USERDATA_S* pstUsrData, HI_UNF_VIDEO_USERDATA_TYPE_E *penType)
{
    ...
    if (hHandle >= HI_VDEC_MAX_INSTANCE_NEW || pstUsrData == HI_NULL || penType == HI_NULL )
    {
        return HI_FAILURE;
    }
...
HI_S32 USRDATA_DRV_Rls(HI_HANDLE hHandle, VDEC_VIDEO_USERDATA_S* pstUsrData, HI_UNF_VIDEO_USERDATA_TYPE_E *penType)
{
    HI_S32 s32Ret;
    BUFMNG_BUF_S stBuf;
    USERDATA_TYPE_E VdecUserDataType;
    ...
    stBuf.pu8KnlVirAddr = (HI_U8*)(HI_SIZE_T)(pstUsrData->u64KnlAddress - sizeof(VDEC_VIDEO_USERDATA_S));
...
HI_S32 USRDATA_Put(HI_HANDLE hHandle, USRDAT* pstUsrData, HI_UNF_VIDEO_USERDATA_TYPE_E enType)
{
    ...
    BUFMNG_BUF_S stBuf;
    VDEC_VIDEO_USERDATA_S *pstPutData = HI_NULL;
    VDEC_CHANNEL_S *pstChan = HI_NULL;
    USERDATA_TYPE_E VdecUserDataType;

    if (hHandle >= HI_VDEC_MAX_INSTANCE_NEW || pstUsrData == HI_NULL)
    {
        return HI_FAILURE;
    }

    memset(&stBuf, 0, sizeof(stBuf));
    memset(&VdecUserDataType, 0, sizeof(VdecUserDataType));
    ...
    pstPutData->u64KnlAddress = (HI_U64)(HI_SIZE_T)(stBuf.pu8KnlVirAddr + sizeof(VDEC_VIDEO_USERDATA_S));
...
EXPORT_SYMBOL(USRDATA_DRV_Rls);
EXPORT_SYMBOL(USRDATA_DeInit);
