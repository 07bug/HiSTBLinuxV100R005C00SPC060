...
#define BUFMNG_ASSERT_RETURN(Condition, RetValue)                  \
    ...
    {                                                                  \
        if (!(Condition))                                                \
        {                                                              \
            HI_WARN_BUFMNG("[%s %d]assert warning\n",__func__,__LINE__);\
            return RetValue;                                       \
        }                                                              \
    }while(0)

#define BUFMNG_ASSERT(Condition)                                   \
    do                                                                 \
    {                                                                  \
        if (!(Condition))                                                \
        {                                                              \
...
{
    BUFMNG_ASSERT_RETURN(bufname != HI_NULL, HI_FAILURE);
    BUFMNG_ASSERT_RETURN(psMBuf != HI_NULL, HI_FAILURE);
    BUFMNG_ASSERT_RETURN(size > 0, HI_FAILURE);
    BUFMNG_ASSERT_RETURN(size < MAX_VDEC_BUFFER, HI_FAILURE);
...
HI_S32 BUFMNG_GetStatus(HI_HANDLE hBuf, BUFMNG_STATUS_S *pstStatus)
{
    ...
    BUFMNG_ASSERT_RETURN(pstStatus != HI_NULL, HI_ERR_BM_INVALID_PARA);
    BUFMNG_FIND_INST(hBuf, pstInst);
    BUFMNG_ASSERT_RETURN(pstInst != HI_NULL, HI_ERR_BM_INVALID_PARA);
...
HI_S32 BUFMNG_OpenFile(HI_S32 Handle, HI_S8 *FilePath, HI_S8 Flag)
{
    ...
    BUFMNG_ASSERT_RETURN(FilePath != HI_NULL, HI_FAILURE);
...
HI_S32 BUFMNG_SaveRaw(HI_S32 Handle, HI_S8 *Addr, HI_S32 Length)
{
    ...
    if (HI_NULL == VdecSaveRawFile || Handle != VdecRawChanNum || Addr == HI_NULL)
    {
...
HI_S32 BUFMNG_SaveRPU(HI_S32 Handle, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (g_VdecSaveRPUFile == HI_NULL || g_VdecRPUChanNum != Handle || pstFrame == HI_NULL)
    {
    ...
    oldfs = get_fs();
    set_fs(KERNEL_DS);
    len = vfs_write(g_VdecSaveRPUFile, Addr, Length, &g_VdecSaveRPUFile->f_pos);
...
HI_S32 SaveYuvNV21(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_U8 *Y_Addr, HI_U8 *C_Addr, mm_segment_t oldfs)
{
    ...
    {
        len = vfs_write(VdecSaveYuvFile, Y_Addr, pstFrame->u32Width, &VdecSaveYuvFile->f_pos);
    ...
    len = vfs_write(VdecSaveYuvFile, U_Array,
    ...
    len = vfs_write(VdecSaveYuvFile, V_Array,
                    pstFrame->u32Height * pstFrame->u32Width / 4, &VdecSaveYuvFile->f_pos);
...
HI_S32 WriteFile(HI_U8 *Array, HI_U32 Size, mm_segment_t oldfs, HI_BOOL bVcmpFlag)
{
    ...
    len = vfs_write(VdecSaveYuvFile, Array, Size, &VdecSaveYuvFile->f_pos);
...
static HI_VOID ConvertYPIXForNV21TileCmpOff(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_U8 *Y_Addr, VDEC_BUFFER_S *pstFrmBuf)
{
    HI_U32 i = 0, j = 0, k = 0;
    HI_U8 *dst, *src;
    HI_U32 Stride = 0;
    ...
    for (i = 0; i < pstFrame->u32Height; i++)
    {
        for (j = 0; j < pstFrame->u32Width; j += 64)
        {
            if (pstFrame->enBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT)
            {
                ...
                src2b = Yaddress_2b + (Stride_2b / 32) * 2 * (i / 2) + (i % 2 * 16) + (j / 4) * 2;
    ...
    for (i = 0; i < pstFrame->u32Height / 2; i++)
    {
        for (j = 0; j < pstFrame->u32Width; j += 64)
        {
            if (pstFrame->enBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT)
            {
                ...
                src2b = Caddress_2b + (Stride_2b / 32) * 2 * (i / 2) + (i % 2 * 16) + (j / 4) * 2;
                memcpy(dst2b, src2b, 64 / 4);
...
HI_S32 BUFMNG_SaveYuv(HI_S32 Handle, HI_DRV_VIDEO_FRAME_S *pstFrame, HI_UNF_VCODEC_TYPE_E enType)
{
    ...
    {
        if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21_TILE)
        {
            SaveYUVNV21TileCmpOff(pstFrame, Y_Addr, C_Addr, &stFrmBuf, oldfs);
        }
        else if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21_TILE_CMP)
        {
            SaveYUVNV21TileCmpOn(pstFrame, Y_Addr, C_Addr, &stFrmBuf, oldfs);
        }
        else
        {
            HI_ERR_VDEC("%s, %d, unsupported Pixel format: %d\n", __func__, __LINE__, pstFrame->ePixFormat);
        }
