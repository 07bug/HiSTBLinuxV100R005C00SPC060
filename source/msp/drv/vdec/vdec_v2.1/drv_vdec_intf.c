...
#define DEFAULT_COMPRESS_STRATEGY VDEC_COMPRESS_ADJUST

VDEC_COMPRESS_E GlbCompStrategy = DEFAULT_COMPRESS_STRATEGY;
...
static HI_S32 VDEC_DRV_CtrlReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "%-35s:%d\n", "GlbCompStrategy", GlbCompStrategy);
    PROC_PRINT(p, "%-35s:%d\n", "MapFrmEnable",    g_bMapFrmEnable);
    ...
    PROC_PRINT(p, "echo  compress    on/off/self      -- forced compress on, off or self-adaption\n");
...
static HI_S32 VDEC_DRV_GetProcArg(HI_CHAR  *chCmd, HI_CHAR  *chArg, HI_U32 u32ArgIndex)
{
    ...
    while (chCmd[u32Count] != 0 && chCmd[u32Count] != '\n')
    {
        ...
        if (u32NewFlag == 1)
        {
            ...
            {
                case 1:
                    if (u32Count >= VDEC_MAX_PROC_ARGS_SIZE)
                    {
                        return HI_FAILURE;
                    }
...
static HI_VOID SetCompressionStrategy(VDEC_COMPRESS_E CompressStrategy)
{

    if (CompressStrategy >= VDEC_COMPRESS_BUTT)
    {
        return;
    }

    GlbCompStrategy = CompressStrategy;
}
...
static HI_VOID SaveYUV(HI_CHAR *pdat2, HI_CHAR *pdat3, HI_HANDLE handle)
{
    ...
    if (BUFMNG_CheckFile(handle, 1) == HI_FALSE)
    {
        ...
        {
            SetCompressionStrategy(VDEC_FORCE_NO_COMPRESS);
        }
    }
    else if (BUFMNG_CheckFile(handle, 1) == HI_TRUE)
    {
        ...
        {
            SetCompressionStrategy(DEFAULT_COMPRESS_STRATEGY);
        }
    }
                                     ...
                                     const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (file == HI_NULL || buffer == HI_NULL || ppos == HI_NULL)
    {
        HI_FATAL_VDEC("parameter invalid!\n");
        return -1;
    }
    ...
    Ret = VDEC_DRV_GetProcArg(buf, dat1, 1);

    if (Ret == HI_FAILURE)
    {
        HI_ERR_VDEC("ERROR: parameter 1 Error!\n");
        return -1;
    }

    Ret = VDEC_DRV_GetProcArg(buf, dat2, 2);

    if (Ret == HI_FAILURE)
    {
        HI_ERR_VDEC("ERROR: parameter 2 Error!\n");
        return -1;
    }

    Ret = VDEC_DRV_GetProcArg(buf, dat3, 3);

    if (Ret == HI_FAILURE)
    {
        HI_ERR_VDEC("ERROR: parameter 3 Error!\n");
        return -1;
    }
    ...
    if (0 == strncmp(dat1, "help", 5))
    {
        ...
        HI_DRV_PROC_EchoHelper("echo  compress    on/off/self      -- forced compress on, off or self-adaption\n");
        HI_DRV_PROC_EchoHelper("echo  setfps      handle   fps     -- set frame rate\n");
    ...
    else if (0 == strncmp(dat1, "compress", 8))
    {
        if (!strncmp(dat2, "on", 3))
        {
            SetCompressionStrategy(VDEC_FORCE_COMPRESS);
            HI_DRV_PROC_EchoHelper("Set global compression strategy to force compress on\n");
        }
        else if (!strncmp(dat2, "off", 4))
        {
            SetCompressionStrategy(VDEC_FORCE_NO_COMPRESS);
            HI_DRV_PROC_EchoHelper("Set global compression strategy to force_no_compress off\n");
        }
        else if (!strncmp(dat2, "self", 5))
        {
            SetCompressionStrategy(VDEC_COMPRESS_ADJUST);
            HI_DRV_PROC_EchoHelper("Set global compression strategy to compress self-adaption\n");
        }
        else
        {
            HI_DRV_PROC_EchoHelper("Invalid command\n");
        }
    }
    else if (strncmp(dat1, "map_frm", 7) == 0)
    {
        if (strncmp(dat2, "on", 2) == 0)
        {
            g_bMapFrmEnable = HI_TRUE;
            HI_DRV_PROC_EchoHelper("Enable map frame buffer.\n");
        }
        else if (strncmp(dat2, "off", 3) == 0)
        {
            g_bMapFrmEnable = HI_FALSE;
            HI_DRV_PROC_EchoHelper("Disable map frame buffer.\n");
        }
        else
        {
                                 ...
                                 const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (q == NULL)
    {
        HI_FATAL_VDEC("FATAL:  file->private_data is null!\n");
        return -1;
    }
    ...
    if (pProcItem == NULL)
    {
        HI_FATAL_VDEC("FATAL: q->private is null!\n");
        return -1;
    }
    ...
    s32Ret = VDEC_DRV_GetProcArg(buf, dat1, 1);
    ...
    s32Ret = VDEC_DRV_GetProcArg(buf, dat2, 2);
    ...
    s32Ret = VDEC_DRV_GetProcArg(buf, dat3, 3);
    ...
    else if (strncmp(dat1, "low_delay_stat", 14) == 0)
    {
        ...
        {
            s32Ret = (pfnVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_START_LOWDLAY_CALC, &hHandle, sizeof(hHandle));
        ...
        {
            s32Ret = (pfnVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_STOP_LOWDLAY_CALC, &hHandle, sizeof(hHandle));
                               ...
                               VDEC_CHANNEL_S *pstChan)
{
    ...
    PROC_PRINT(p, "--------------------------Picture Information------------------------\n"
               ...
               pstStatInfo->u32VdecDecErrFrame,//sdk
...
static HI_VOID ReadFrameType(struct seq_file *p, HI_DRV_VIDEO_FRAME_TYPE_E enSrcFrameType)
{
...
static HI_S32 VDEC_DRV_ReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    VDEC_CHANNEL_S *pstChan;
    VDEC_CHAN_STATINFO_S *pstStatInfo;
    DRV_PROC_ITEM_S *pstProcItem = NULL;
    ...
    if (pstProcItem == NULL)
    {
        HI_FATAL_VDEC("FATAL: pstProcItem is null!\n");
        return -1;
    }
    ...
    if (pstChan)
    {
        ...
        PROC_PRINT(p,
                   ...
                   (1 == pstChan->stOption.u32DFSEn) ? "Enable" : "Disable",
        ...
        ReadFrameType(p, pstChan->stLastFrm.enSrcFrameType);
        PROC_PRINT(p, "=======================================================================\n");
