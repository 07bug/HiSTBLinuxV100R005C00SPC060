...
HI_S32 USRDATA_Alloc(HI_HANDLE hHandle, HI_DRV_VDEC_USERDATABUF_S *pstBuf)
{
    ...
    if ((hHandle >= HI_VDEC_MAX_INSTANCE_NEW)
        || (pstBuf == HI_NULL)
        || (pstBuf->UserDataType >= VDEC_USRDATA_TYPE_INDEX_BUTT) || (pstBuf->UserDataType < 0)
        || (g_stUsrData[hHandle][pstBuf->UserDataType].bUsed))
    {
...
{
    if ((hHandle >= HI_VDEC_MAX_INSTANCE_NEW) || (UserDataType >= VDEC_USRDATA_TYPE_INDEX_BUTT) || UserDataType < VDEC_USRDATA_TYPE_CC)
    {
        return HI_FAILURE;
    }
