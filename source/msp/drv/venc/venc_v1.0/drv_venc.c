...
static HI_S32 CheckPicSize(HI_U32 Width, HI_U32 Height, HI_UNF_VCODEC_TYPE_E VencType)
{
    HI_U32 MinHeight;
    HI_U32 MaxHeight;
    HI_U32 MinWidth;
...
static HI_UNF_VCODEC_CAP_LEVEL_E GetMinCapLevel(HI_UNF_VCODEC_TYPE_E VencType)
{
...
static HI_S32 CheckCaplevel(const HI_UNF_VENC_CHN_ATTR_S *pstAttr, HI_U32 *BufMinSize)
{
    ...
    MinCapLevel = GetMinCapLevel(pstAttr->enVencType);
...
static HI_S32 CheckConfig(const HI_UNF_VENC_CHN_ATTR_S *pstAttr)
{
    ...
    Ret = CheckPicSize(pstAttr->u32Width, pstAttr->u32Height, pstAttr->enVencType);
    if(Ret != HI_SUCCESS)
    {
        return Ret;
    }
                      ...
                      VENC_CHN_INFO_S *pstVeInfo,HI_BOOL bOMXChn, struct file  *pfile)
{
    ...
    s32Ret = CheckConfig(pVencUnfAttrs);
...
HI_S32 VENC_DRV_DetachInput(VeduEfl_EncPara_S* hVencChn, HI_HANDLE hSrc, HI_MOD_ID_E enModId )
{
    ...
    if (pstVenc->bEnable)
    {
        ...
        //return HI_ERR_VENC_CHN_INVALID_STAT;
        if (VENC_DRV_StopReceivePic(hVencChn) != HI_SUCCESS)
        {
    ...
    if (VENC_DRV_EflDetachInput(hVencChn, &stSrcInfo) != HI_SUCCESS)
    {
        return HI_FAILURE;
    }
...
#ifdef VENC_SUPPORT_JPGE
static HI_S32 VENC_DRV_GetJpgeStream(VeduEfl_NALU_S *pstVeduPacket, VENC_STREAM_S *pstStream, VeduEfl_EncPara_S *hVencChn,
                              HI_U8 *StartVirAddr, HI_U32 StreamBufLen, VENC_BUF_OFFSET_S *pstBufOffSet)
{
    ...
    while (((s32Ret != HI_SUCCESS) || (pstVeduPacket->bFrameEnd != HI_TRUE)) && (RecycleCnt < 1000));
    ...
    if ((pstVeduPacket->SlcLen[0] + pstVeduPacket->SlcLen[1]) > (StreamBufLen - u32StreamTemp))
    {
        HI_ERR_VENC("jpeg 2nd Get stream failed, DstBufLen= %d(need %d), not enough\n", StreamBufLen,(pstVeduPacket->SlcLen[0] + pstVeduPacket->SlcLen[1]));
...
HI_S32 VENC_DRV_ReleaseStream(VeduEfl_EncPara_S* hVencChn, VENC_INFO_ACQUIRE_STREAM_S* pstAcqStrm)
{
    ...
    stStream.VirAddr = (HI_VIRT_ADDR_T)(pstEncChnPara->stCycBuf.pBase + pstAcqStrm->H264_stream_off);

    if ((HI_VIRT_ADDR_T)g_stVencChn[u32VeChn].stChnPacket.pVirtAddr[0] == stStream.VirAddr)
    {
       memcpy(&stVeduPacket, &g_stVencChn[u32VeChn].stChnPacket, sizeof(VeduEfl_NALU_S));
       memset(&g_stVencChn[u32VeChn].stChnPacket, 0, sizeof(VeduEfl_NALU_S));
    }
    else
    {
        stVeduPacket.pVirtAddr[0] = (HI_VOID*)stStream.VirAddr;
        stVeduPacket.SlcLen[0]    = stStream.u32SlcLen;
        stVeduPacket.pVirtAddr[1] = stVeduPacket.pVirtAddr[0]+ stVeduPacket.SlcLen[0];
        stVeduPacket.SlcLen[1]    = 0;
        stVeduPacket.PTS0         = stStream.u32PtsMs;
        stVeduPacket.PTS1         = 0;
        stVeduPacket.bFrameEnd    = (HI_U32)stStream.bFrameEnd;
        stVeduPacket.NaluType     = stStream.enDataType.enH264EType;
    }
...
HI_S32 VENC_DRV_SetRateControlType(VeduEfl_EncPara_S* EncHandle, VENC_DRV_CONTROLRATETYPE_E enRCType)
{
    ...
    if (enRCType == VENC_DRV_ControlRateVariable)
    {
        HI_ERR_VENC("Chan(%p) just support CBR, not support avbr!\n",EncHandle);
...
HI_S32 VENC_DRV_GetRateControlType(VeduEfl_EncPara_S* EncHandle, VENC_DRV_CONTROLRATETYPE_E *penRCType)
{
    ...
    /*current chip just support CBR*/
    *penRCType = VENC_DRV_ControlRateConstant;
...
static HI_S32 GetCapInfoH265(HI_UNF_VENC_CAP_INFO_S *pstCapInfo)
{
    ...
    enMinCapLevel = GetMinCapLevel(pstCapInfo->enCodecType);
...
HI_S32 HI_DRV_VENC_GetUserHndByKerHnd(VeduEfl_EncPara_S* hVencHandle, HI_HANDLE* hVencUsrHandle)
{
    ...
    for (i = 0; i < HI_VENC_MAX_CHN; i++)
    {
        if ((g_stVencChn[i].hVEncHandle == hVencHandle) && (hVencHandle != NULL))
        { break; }
    }
