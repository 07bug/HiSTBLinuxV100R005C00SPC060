...
HI_S32 VENC_DRV_GetMessage_OMX(VeduEfl_EncPara_S* hVencChn, venc_msginfo* pmsg_info )
{
   ...
   if (msg_info.msgcode == VENC_MSG_RESP_INPUT_DONE)   //FBD
   {
       ...
       {
           if (HI_DRV_SYS_GetTimeStampMs(&TmpTime_dbg[GetMsg_EBD_ID][5])!= HI_SUCCESS)
           {
   ...
   if(msg_info.msgcode == VENC_MSG_RESP_OUTPUT_DONE)
   {
       ...
       {
           if (HI_DRV_SYS_GetTimeStampMs(&TmpTime_dbg[GetMsg_FBD_ID][6]!= HI_SUCCESS)
           {
               HI_ERR_VENC("Get Time Stamp Ms err!\n");
           }
           GetMsg_FBD_ID++;
...
HI_S32 VENC_DRV_FlushPort_OMX(VeduEfl_EncPara_S* hVencChn, HI_U32 u32PortIndex, HI_U32 InterFlag)
{
   HI_S32 s32Ret;
   HI_S32 s32VeChn = 0;
   ...
   s32Ret = VENC_DRV_EflFlushPort_OMX(hVencChn,u32PortIndex,InterFlag);
   return s32Ret;
}

HI_S32 VENC_DRV_MMZ_Map_OMX(VeduEfl_EncPara_S* hVencChn, HI_MMZ_BUF_S* pMMZbuf)
{
   HI_S32 s32Ret;
...
HI_S32 VENC_DRV_MMZ_UMMap_OMX(VeduEfl_EncPara_S* hVencChn, HI_MMZ_BUF_S* pMMZbuf)
{
   HI_S32 s32Ret;
