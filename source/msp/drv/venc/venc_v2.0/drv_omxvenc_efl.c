...
Others     :
******************************************************************************/
HI_S32 VENC_DRV_EflMMapToKernel_OMX(VeduEfl_EncPara_S *pEncPara, HI_MMZ_BUF_S* pMMZbuf)
{
    HI_S32 s32Ret = 0,i = 0;
    HI_U32 u32ChnID = 0;
    ...
    if (HI_SUCCESS == s32Ret)
    {
        ...
        for (i = 0; i < OMX_OUTPUT_PORT_BUFFER_MAX; i++)
        {
            ...
            {
                pEncPara->OmxOutputBuf[i].pu8StartVirAddr = sMBufVenc.pu8StartVirAddr;
                pEncPara->OmxOutputBuf[i].u32StartPhyAddr = pMMZbuf->phyaddr;
                pEncPara->OmxOutputBuf[i].u32Size         = pMMZbuf->bufsize;
        ...
        {
            HI_ERR_VENC("i = %d, pEncPara->OmxOutputBuf[] not idle node!\n",i);
        }
    }
    else
    {
...
HI_S32 VENC_DRV_EflUMMapToKernel_OMX(VeduEfl_EncPara_S *pEncPara, HI_MMZ_BUF_S* pMMZbuf)
{
    HI_S32 i = 0;
    HI_U32 u32ChnID = 0;
    VENC_BUFFER_S sMBufVenc   = {0};
    ...
    for (i = 0; i < OMX_OUTPUT_PORT_BUFFER_MAX; i++)
    {
        if (pEncPara->OmxOutputBuf[i].u32StartPhyAddr == sMBufVenc.u32StartPhyAddr)
        {
            sMBufVenc.pu8StartVirAddr = pEncPara->OmxOutputBuf[i].pu8StartVirAddr;
            memset(&pEncPara->OmxOutputBuf[i], 0, sizeof(VENC_BUFFER_S));
    ...
    if (i == OMX_OUTPUT_PORT_BUFFER_MAX)
    {
        HI_ERR_VENC("i = %d, pEncPara->OmxOutputBuf[] can't find Phyaddr(0x%x)\n",i,sMBufVenc.u32StartPhyAddr);
        return HI_FAILURE;
    }
