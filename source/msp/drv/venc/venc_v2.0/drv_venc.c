...
static HI_S32 Convert_FrameStructure(HI_UNF_VIDEO_FRAME_INFO_S* pstUnfImage, HI_DRV_VIDEO_FRAME_S* pstDrvImage)
{
    ...
    pstDrvImage->u32FrameRate = pstUnfImage->stFrameRate.u32fpsInteger * 1000 + pstUnfImage->stFrameRate.u32fpsDecimal;
...
static HI_S32 CheckPicSize(HI_U32 Width, HI_U32 Height, HI_UNF_VCODEC_TYPE_E VencType, HI_BOOL bOMXChn)
{
    HI_U32 MinHeight;
    HI_U32 MaxHeight;
    HI_U32 MinWidth;
...
static HI_UNF_VCODEC_CAP_LEVEL_E GetMaxCapLevel(HI_UNF_VCODEC_TYPE_E VencType)
{
    ...
    MaxTotalMB = (D_VENC_ALIGN_UP(MaxWidth, VENC_MB_WIDTH) >> VENC_MB_POWER) * (D_VENC_ALIGN_UP(MaxHeight, VENC_MB_WIDTH) >> VENC_MB_POWER);

    if (MaxTotalMB > VENC_MB_NUM_FULLHD)
    {
        MaxCapLevel = HI_UNF_VCODEC_CAP_LEVEL_4096x4096;
    }
    else if (MaxTotalMB > VENC_MB_NUM_720P)
    {
...
static HI_UNF_VCODEC_CAP_LEVEL_E GetMinCapLevel(HI_BOOL bOMXChn, HI_UNF_VCODEC_TYPE_E VencType)
{
    ...
    MinTotalMB = (D_VENC_ALIGN_UP(MinWidth, VENC_MB_WIDTH) >> VENC_MB_POWER) * (D_VENC_ALIGN_UP(MinHeight, VENC_MB_WIDTH) >> VENC_MB_POWER);
    ...
    else if (MinTotalMB > VENC_MB_NUM_QCIF)
    {
        MinCapLevel = HI_UNF_VCODEC_CAP_LEVEL_CIF;
    }
    else
    {
...
static HI_S32 GetBufMinSize(HI_UNF_VCODEC_CAP_LEVEL_E enCapLevel, HI_U32 *BufMinSize)
{
    switch (enCapLevel)
    {
        case HI_UNF_VCODEC_CAP_LEVEL_QCIF:
            *BufMinSize = BUF_MIN_SIZE_QCIF;
            break;

        case HI_UNF_VCODEC_CAP_LEVEL_CIF:
            *BufMinSize = BUF_MIN_SIZE_CIF;
            break;

        case HI_UNF_VCODEC_CAP_LEVEL_D1:
            *BufMinSize = BUF_MIN_SIZE_D1;
            break;

        case HI_UNF_VCODEC_CAP_LEVEL_720P:
            *BufMinSize = BUF_MIN_SIZE_720P;
            break;

        case HI_UNF_VCODEC_CAP_LEVEL_FULLHD:
            *BufMinSize = BUF_MIN_SIZE_FULLHD;
            break;
...
static HI_S32 CheckCaplevel_JpegExt(const HI_UNF_VENC_CHN_ATTR_S *pstAttr, HI_U32 *BufMinSize)
{
    HI_U32 TotalMB;
    ...
    switch (pstAttr->enCapLevel)
    {
        ...
        case HI_UNF_VCODEC_CAP_LEVEL_1280x2160:
            ...
            *BufMinSize = BUF_MIN_SIZE_2Kx1K;
            break;

        case HI_UNF_VCODEC_CAP_LEVEL_2160x2160:
            if (TotalMB > VENC_MB_NUM_2Kx2K)
            {
                HI_ERR_VENC("Picture Width(%u) or Heigth(%u) too large for enCapLevel(%d) .\n",
                            pstAttr->u32Width, pstAttr->u32Height, pstAttr->enCapLevel);
        ...
        case HI_UNF_VCODEC_CAP_LEVEL_2160x4096:
            ...
            *BufMinSize = BUF_MIN_SIZE_4Kx2K;
            break;

        case HI_UNF_VCODEC_CAP_LEVEL_4096x4096:
            if (TotalMB > VENC_MB_NUM_4Kx4K)
            {
                HI_ERR_VENC("Picture Width(%u) or Heigth(%u) too large for enCapLevel(%d) .\n",
                            pstAttr->u32Width, pstAttr->u32Height, pstAttr->enCapLevel);
...
static HI_S32 CheckCaplevel(const HI_UNF_VENC_CHN_ATTR_S *pstAttr, HI_U32 *BufMinSize, HI_BOOL bOMXChn)
{
    ...
    TotalMB = (D_VENC_ALIGN_UP(pstAttr->u32Height, VENC_MB_WIDTH) >> VENC_MB_POWER) * (D_VENC_ALIGN_UP(pstAttr->u32Width, VENC_MB_WIDTH) >> VENC_MB_POWER);
    MinCapLevel = GetMinCapLevel(bOMXChn, pstAttr->enVencType);
    MaxCapLevel = GetMaxCapLevel(pstAttr->enVencType);
    ...
    switch (pstAttr->enCapLevel)
    {
        ...
        default:
            ...
            {
                if (CheckCaplevel_JpegExt(pstAttr, BufMinSize) != HI_SUCCESS)
                {
                   return HI_ERR_VENC_INVALID_PARA;
                }
            }
...
static HI_S32 CheckConfig(const HI_UNF_VENC_CHN_ATTR_S *pstAttr, HI_BOOL bOMXChn)
{
    ...
    Ret = CheckPicSize(pstAttr->u32Width, pstAttr->u32Height,pstAttr->enVencType, bOMXChn);
    if(Ret != HI_SUCCESS)
    {
        return Ret;
    }
...
static HI_S32 Convert_DrvFrameStructure(HI_DRV_VIDEO_FRAME_S* pstDrvImage, HI_UNF_VIDEO_FRAME_INFO_S* pstUnfImage)
{
    ...
    pstUnfImage->stFrameRate.u32fpsInteger = pstDrvImage->u32FrameRate / 1000;
    pstUnfImage->stFrameRate.u32fpsDecimal = pstDrvImage->u32FrameRate % 1000;
...
HI_BOOL VENC_DRV_CheckChnStateSafe(HI_U32 ChanId)
{
    ...
    if ((g_stVencChn[ChanId].enChanState == VEDU_STATE_DESTORYING)
        || (g_stVencChn[ChanId].enChanState == VEDU_STATE_BUTT))
    {
        return HI_FALSE;
    }
    else
                      ...
                      VENC_CHN_INFO_S *pstVeInfo,HI_BOOL bOMXChn, struct file  *pfile)
{
    ...
    HI_CHAR YUVFileName[FILE_NAME_LEN] = {""};
    HI_CHAR StreamFileName[FILE_NAME_LEN] = {""};
    HI_U32 SlcSize = 0;
    const HI_U32 NoUseBufSize = 64;
    ...
    EncCfg.IsAVBR        = (pstAttr->ControlRateType != VENC_DRV_ControlRateConstant) ? 1 : 0;
    EncCfg.CapLevel      = (pVencUnfAttrs->enCapLevel <= HI_UNF_VCODEC_CAP_LEVEL_FULLHD)? pVencUnfAttrs->enCapLevel: (HI_UNF_VCODEC_CAP_LEVEL_FULLHD + 1)  ;
    ...
    if (pstEncChnPara->CurStrmBufSize > (pVencUnfAttrs->u32StrmBufSize - NoUseBufSize) * BIT_NUM_BYTE)
    {
         pstEncChnPara->CurStrmBufSize = (pVencUnfAttrs->u32StrmBufSize - NoUseBufSize) * BIT_NUM_BYTE;
    }
    pstEncChnPara->CurStrmBufSize = D_VENC_ALIGN_UP(pstEncChnPara->CurStrmBufSize / BIT_NUM_BYTE, VEDU_MMZ_ALIGN);
    ...
    g_stVencChn[i].enChanState             = VEDU_STATE_CREATE;
    ...
    memcpy(g_stVencChn[i].stProcWrite.YUVFileName, YUVFileName, FILE_NAME_LEN);
    s32Ret = HI_OSAL_Snprintf(StreamFileName, sizeof(StreamFileName), "venc_proc_chn%02d.h264", i);
    if (0 == s32Ret)
    {
        HI_ERR_VENC("HI_OSAL_Snprintf failed!\n");
    }
    memcpy(g_stVencChn[i].stProcWrite.StreamFileName, StreamFileName, FILE_NAME_LEN);
...
HI_S32 VENC_DRV_DestroyChn( VeduEfl_EncPara_S* hVencChn)
{
    ...
    g_stVencChn[u32VeChn].enChanState = VEDU_STATE_DESTORYING;
    ...
    {
        spin_lock_irqsave(&g_SendFrame_Lock[u32VeChn], flags);
        g_stVencChn[u32VeChn].enChanState = VEDU_STATE_STOP;
        spin_unlock_irqrestore(&g_SendFrame_Lock[u32VeChn], flags);
        return s32Ret;
    }
    ...
    g_stVencChn[u32VeChn].enChanState = VEDU_STATE_BUTT;
    spin_unlock_irqrestore(&g_SendFrame_Lock[u32VeChn], flags);
...
HI_S32 VENC_DRV_AttachInput(VeduEfl_EncPara_S* hVencChn, HI_HANDLE hSrc, HI_MOD_ID_E enModId )
{
    ...
    if (HI_SUCCESS == ret)
    {
        ...
        HI_INFO_VENC("VENC%d attchInputOK, srcHdl:%#x.\n", u32VeChn, pstVenc->hSource);
    }
...
HI_S32 VENC_DRV_DetachInput(VeduEfl_EncPara_S* hVencChn, HI_HANDLE hSrc, HI_MOD_ID_E enModId )
{
    ...
    if (pstVenc->bEnable)
    {
        if (VENC_DRV_StopReceivePic(pstEncChnPara) != HI_SUCCESS)
        {
    ...
    HI_INFO_VENC("VENC%d dettchInputOK, srcHdl:%#x.\n", u32VeChn, pstVenc->hSource);
...
static HI_S32 WaitForStream(VeduEfl_EncPara_S *hVencChn, HI_U32 u32TimeoutMs)
{
...
static HI_S32 AcquireVencStream(VeduEfl_EncPara_S *hVencChn, HI_U32 u32TimeoutMs, VENC_STREAM_S* pstStream, VENC_BUF_OFFSET_S *pstBufOffSet)
{
    ...
    s32Ret = VENC_DRV_EflAcqReadBuffer(pstEncChnPara, &stVeduPacket);
    if (s32Ret != HI_SUCCESS)
    {
        return s32Ret;
    }
...
static HI_S32 ReleaseVencStream(VeduEfl_EncPara_S *hVencChn, VENC_INFO_ACQUIRE_STREAM_S* pstAcqStrm)
{
    ...
    s32Ret = VENC_DRV_EflRlsReadBuffer(pstEncChnPara, &stVeduPacket);
...
HI_S32 VENC_DRV_StartReceivePic(VeduEfl_EncPara_S* EncHandle)
{
    ...
    if (HI_SUCCESS == s32Ret)
    {
        ...
        g_stVencChn[u32VeChn].enChanState = VEDU_STATE_START;
                                       ...
                                       VeduEfl_EncPara_S* EncHandle, OPTM_VENC_CHN_S* pstVenc)
{
    ...
    else  /* jpeg*/
    {
        ...
        if ((pstAttr->u32InputFrmRate > HI_VENC_MAX_fps) || (pstAttr->u32InputFrmRate < pstAttr->u32TargetFrmRate))
        {
            return HI_ERR_VENC_INVALID_PARA;
        }
...
{
    VeduEfl_EncPara_S*  pstEncChnPara = EncHandle;
    HI_U32 FrameRate = HI_VENC_MAX_fps;
        ...
        || (pstVenc->stChnUserCfg.u32Width != pstAttr->u32Width))
    {
        ...
        {
        #ifdef VENC_SUPPORT_ATTACH_VO
            if (pstVenc->enSrcModId == HI_ID_VO)
            {
                FrameRate = HI_VENC_MAX_fps * 100;
            }
        #endif
            if ((pstEncChnPara->stSrcInfo.pfChangeInfo)(pstEncChnPara->stSrcInfo.handle,
                                                        pstAttr->u32Width,
                                                        pstAttr->u32Height,
                                                        FrameRate) != HI_SUCCESS)
            {
...
HI_S32 VENC_DRV_SetRateControlType(VeduEfl_EncPara_S* EncHandle, VENC_DRV_CONTROLRATETYPE_E enRCType)
{
    ...
    if ((enRCType != VENC_DRV_ControlRateVariable) && (enRCType != VENC_DRV_ControlRateConstant))
    {
        HI_ERR_VENC("Chan(%p) set RateControl type(%d) not support!\n",EncHandle,enRCType);

        return HI_ERR_VENC_INVALID_PARA;
    }

    VENC_DRV_EflRequestIframe(pstEncChnPara);

    memset(&RcAttrCfg, 0, sizeof(VeduEfl_RcAttr_S));
    RcAttrCfg.BitRate = pstEncChnPara->stRc.BitRate;
    RcAttrCfg.OutFrmRate = pstEncChnPara->stRc.VoFrmRate;
    RcAttrCfg.InFrmRate = pstEncChnPara->stRc.ViFrmRate;
    RcAttrCfg.MaxQp  = pstEncChnPara->stRc.MaxQp;
    RcAttrCfg.MinQp  = pstEncChnPara->stRc.MinQp;    

    /*Set RateControl Mode*/
    pstEncChnPara->stRc.IsAVBR = (enRCType != VENC_DRV_ControlRateConstant) ? 1 : 0;
    pstEncChnPara->stRc.MaxTimeOfP = (pstEncChnPara->stRc.IsAVBR)? 200 : 6;

    if (HI_SUCCESS != VENC_DRV_EflRcAttrInit(pstEncChnPara, &RcAttrCfg))
    {
...
HI_S32 VENC_DRV_GetRateControlType(VeduEfl_EncPara_S* EncHandle, VENC_DRV_CONTROLRATETYPE_E *penRCType)
{
    ...
    D_VENC_GET_CHN(u32VeChn, EncHandle);
    D_VENC_CHECK_CHN(u32VeChn);
    D_VENC_CHECK_PTR(penRCType);

    *penRCType = (pstEncChnPara->stRc.IsAVBR == 1) ? VENC_DRV_ControlRateVariable : VENC_DRV_ControlRateConstant;
...
HI_S32 VENC_DRV_SetInFrmRateType(VeduEfl_EncPara_S* EncHandle, VENC_DRV_FRMRATETYPE_E enInFrmRateType)
{
    ...
    if (enInFrmRateType >= VENC_DRV_FRMRATE_BUTT)
    {
        HI_ERR_VENC("Chan(%p) set RateControl type(%d) not support!\n",EncHandle,enInFrmRateType);

        return HI_ERR_VENC_INVALID_PARA;
    }

    pstEncChnPara->InputFrmRateTypeConfig = enInFrmRateType;
...
HI_S32 VENC_DRV_GetInFrmRateType(VeduEfl_EncPara_S* EncHandle, VENC_DRV_FRMRATETYPE_E *penInFrmRateType)
{
    ...
    D_VENC_GET_CHN(u32VeChn, EncHandle);
    D_VENC_CHECK_CHN(u32VeChn);
    D_VENC_CHECK_PTR(penInFrmRateType);

    if (pstEncChnPara->InputFrmRateTypeConfig != VENC_DRV_FRMRATE_BUTT)
    {
        *penInFrmRateType = pstEncChnPara->InputFrmRateTypeConfig;
    }
    else if (pstEncChnPara->InputFrmRateTypeDefault != VENC_DRV_FRMRATE_BUTT)
    {
        *penInFrmRateType = pstEncChnPara->InputFrmRateTypeDefault;
    }
    else
    {
        /*both InputFrmRateTypeDefault and InputFrmRateTypeConfig is invalid, return UseMode*/
        *penInFrmRateType = VENC_DRV_FRMRATE_USER;
    }
...
static HI_S32 GetCapInfoH265(HI_UNF_VENC_CAP_INFO_S *pstCapInfo)
{
    ...
    enMaxCapLevel = GetMaxCapLevel(pstCapInfo->enCodecType);
    ...
    enMinCapLevel = GetMinCapLevel(0, pstCapInfo->enCodecType);
...
static HI_S32 GetCapInfoJPGE(HI_UNF_VENC_CAP_INFO_S *pstCapInfo)
{
    ...
    pstCapInfo->stMaxReso.enCapLevel = enMaxCapLevel;
    pstCapInfo->stMaxReso.u32Height  = HI_JPEG_MAX_HEIGTH;
    pstCapInfo->stMaxReso.u32Width   = HI_JPEG_MAX_WIDTH;
    pstCapInfo->stMaxReso.u32MaxBitRate = HI_VENC_MAX_bps;
    pstCapInfo->stMaxReso.u32MinBitRate = HI_VENC_MIN_bps;
    pstCapInfo->stMaxReso.u32MaxFrmRate = HI_VENC_MAX_fps;
    pstCapInfo->stMaxReso.u32MinBufSize = MaxResoBufMinSize;
    ...
    pstCapInfo->stMinReso.enCapLevel = enMinCapLevel;
    pstCapInfo->stMinReso.u32Height  = HI_JPEG_MIN_HEIGTH;
    pstCapInfo->stMinReso.u32Width   = HI_JPEG_MIN_WIDTH;
    pstCapInfo->stMinReso.u32MaxBitRate = HI_VENC_MAX_bps;
    pstCapInfo->stMinReso.u32MinBitRate = HI_VENC_MIN_bps;
    pstCapInfo->stMinReso.u32MaxFrmRate = HI_VENC_MAX_fps;
    pstCapInfo->stMinReso.u32MinBufSize = MinResoBufMinSize;
...
#ifndef VENC_DPT_ONLY
static HI_S32 VENC_DRV_ReleaseStreamIntar(VeduEfl_EncPara_S* hVencChn, VENC_STREAM_S* pstStream)
{
    HI_U32 u32VeChn = 0;
    HI_S32 s32Ret = 0;
    VeduEfl_NALU_S stVeduPacket;
...
HI_S32 HI_DRV_VENC_Create(HI_HANDLE* phVencChn, HI_UNF_VENC_CHN_ATTR_S* pstAttr, HI_BOOL bOMXChn, struct file*  pfile)
{
    ...
    if (phVencChn == NULL || pstAttr == NULL)
    {
