...
#define D_VENC_CALC_PME_SIZE(PicWidth,PicHeight)      (((PicWidth + 31) / 32) * ((PicHeight + 31) / 32)* 128)
#define D_VENC_CALC_PMEINFO_SIZE(Protocol,PicHeight)  ((Protocol == VEDU_H265)? (((PicHeight + 63) / 64 + 1) * 128) : (((PicHeight + 15) / 16 + 1) * 32))
#define D_VENC_CALC_MV_SIZE(PicWidth,PicHeight)       (((PicWidth + 31) / 32) * ((PicHeight + 31) / 32)* 32)
...
HI_BOOL IsChipIDV500R001(HI_CHIP_TYPE_E enChipType, HI_CHIP_VERSION_E enChipVersion)
{
    if (((enChipType == HI_CHIP_TYPE_HI3798M) && (enChipVersion == HI_CHIP_VERSION_V300)) ||
        ...
        ((enChipType == HI_CHIP_TYPE_HI3798M) && (enChipVersion == HI_CHIP_VERSION_V310)) ||
        ((enChipType == HI_CHIP_TYPE_HI3716M) && (enChipVersion == HI_CHIP_VERSION_V450)) ||
        ((enChipType == HI_CHIP_TYPE_HI3798M_H) && (enChipVersion == HI_CHIP_VERSION_V300)))
    {
        return HI_TRUE;
    }
    else
...
static HI_U32 H264e_MakeSlcHdr(VeduEfl_EncPara_S *pEncPara, const VeduEfl_H264e_SlcHdr_S* pSlcHdr)
{
    HI_U32 code   = 0;
    HI_U32 buf[8] = {0};
    HI_S32 bits, i;
    HI_U32 bitPara;
    HI_U32 *pHdrBuf     = pEncPara->stHal.SlcHdrStream;
    HI_U32 *pReorderBuf = pEncPara->stHal.ReorderStream;
    HI_U32 *pMarkBuf    = pEncPara->stHal.MarkingStream;
    tBitStream BS1;
    tBitStream BS2;
    tBitStream BS3;
    ...
    {
        ue_vlc(bits, code, pEncPara->idr_pic_id & 0xF);
...
static HI_U32 H265e_MakeSliceHead(VeduEfl_EncPara_S* pEncPara, const VeduEfl_H265e_SlcHdr_S *pSlcHdr)
{
    ...
    {
        VENC_DRV_BsPutBits31(&BS1, 0, 1);       //no_output_of_prior_pics_flag = 0    u(1)
    }

    VENC_DRV_BsPutBits31(&BS1, 1, 1);           //slice_pic_parameter_set_id=0         ue(v)
    ...
    for (i = 0; bits > 0; i++, bits -= 32)
    {
        pEncPara->stHal.slchdr_part1 = BS1.bBigEndian ? buf[i] : REV32(buf[i]);
        if (bits < 32) { pEncPara->stHal.slchdr_part1 >>= (32 - bits); }
    }
    pEncPara->stHal.slchdr_size_part1 = BS1.totalBits;

    // part2
    VENC_DRV_BsOpenBitStream(&BS2, buf);

    ue_vlc(bits, code, pSlcHdr->eSliceType);
    VENC_DRV_BsPutBits31(&BS2, code, bits);                     //slice_type    ue(v)

    if ( 2 != pSlcHdr->eSliceType )
    {
        VENC_DRV_BsPutBits31(&BS2, pSlcHdr->uiPocLsb, 16);      //pic_order_cnt_lsb         u(16)
        VENC_DRV_BsPutBits31(&BS2, 1, 1);                       //short_term_ref_pic_set_sps_flag=1     u(1)

        if ( pSlcHdr->bSpsTemporalMvpEnableFlag )
        {
            VENC_DRV_BsPutBits31(&BS2, 1, 1);                   //enable_temporal_mvp_flag=1        u(1)
        }
    }

    VENC_DRV_BsPutBits31(&BS2, pSlcHdr->bSliceSaoLuma, 1);      //slice_sao_luma_flag                  u(1)
    VENC_DRV_BsPutBits31(&BS2, pSlcHdr->bSliceSaoChroma, 1);    //slice_sao_chroma_flag                u(1)

    if ( pSlcHdr->eSliceType != 2 )
    {
        VENC_DRV_BsPutBits31(&BS2, 1, 1);                       //num_ref_idx_active_override_flag = 1    u(1)

        ue_vlc(bits, code,  pSlcHdr->num_ref_idx_l0_active_minus1);
        VENC_DRV_BsPutBits31(&BS2, code, bits);                 //num_ref_idx_l0_active_minus1

        if( pSlcHdr->eSliceType == 0)
        {
            VENC_DRV_BsPutBits31(&BS2, 1, 1);                   //num_ref_idx_l1_active_minus1
        }

        if( pSlcHdr->eSliceType == 0)
        {
            VENC_DRV_BsPutBits31(&BS2, 0, 1);                   //mvd_l1_zero_flag
        }

        VENC_DRV_BsPutBits31(&BS2, 0, 1);                       //cabac_init_flag

        if (pSlcHdr->bSpsTemporalMvpEnableFlag)
        {
            if( pSlcHdr->eSliceType == 0)
            {
                VENC_DRV_BsPutBits31(&BS2, pSlcHdr->collocated_from_l0_flag, 1);
            }
            ...
            {
                ue_vlc(bits, code, 0);
                VENC_DRV_BsPutBits31(&BS2, code, bits);
            }
        }

        ue_vlc(bits, code, pSlcHdr->five_minus_max_num_merge_cand);
        VENC_DRV_BsPutBits31(&BS2, code, bits);                             //five_minus_max_num_merge_cand=3         ue(v)

    }

    se_vlc(bits, code, pSlcHdr->iSliceQPDelta);
    VENC_DRV_BsPutBits31(&BS2, code, bits);                                 //slice_qp_delta     se(v)

    H265e_MakeSliceHead_DBSAO(pEncPara, pSlcHdr, &BS2);
    ...
    for (i = 0; bits > 0; i++, bits -= 32)
    {
        pEncPara->stHal.CABAC_SLCHDR_PART2_SEG[i] = BS2.bBigEndian ? buf[i] : REV32(buf[i]);
        if (bits < 32) { pEncPara->stHal.CABAC_SLCHDR_PART2_SEG[i] >>= (32 - bits); }
    }
    pEncPara->stHal.slchdr_size_part2 = BS2.totalBits;

    return (HI_U32)BS2.totalBits;
}
...
HI_VOID SetStreamInfo(VeduEfl_EncPara_S *pEncPara, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
    ...
    pstStreamInfo->Protocol = pEncPara->Protocol;
#ifdef VENC_SUPPORT_JPGE
    pstStreamInfo->pstJpgMMZBuf = &pEncPara->JpgMMZBuf;
#endif
    pstStreamInfo->IFrmInsertBySaveStream = pEncPara->IFrmInsertBySaveStrm;
}
...
static HI_VOID SetStrmMngInputInfo(VeduEfl_EncPara_S* pEncPara, STRMMNG_INPUT_INFO_S* pstInputInfo)
{
    ...
    pstInputInfo->ExtFlag = pEncPara->stHal.ExtFlag;
    pstInputInfo->ExtFillLen = pEncPara->stHal.ExtFillLen;
    ...
    pstInputInfo->RecodeCnt    = pEncPara->stRc.RecodeCnt;
#ifdef VENC_SUPPORT_JPGE
...
static HI_VOID GetStreamInfo(VeduEfl_EncPara_S* pEncPara, STRMMNG_STREAM_INFO_S* pstStreamInfo)
{
    ...
    pEncPara->IFrmInsertBySaveStrm = pstStreamInfo->IFrmInsertBySaveStream;
}
...
static HI_VOID JPGE_MakeJFIF(VeduEfl_EncPara_S* EncHandle)
{
    ...
    if (STRM_JPGEWriteHeader(pEncPara->hStrmMng, &stInputInfo, &stStreamInfo) != HI_SUCCESS)
    {
...
static HI_VOID JPGE_RemoveJFIF(VeduEfl_EncPara_S* EncHandle)
{
    ...
    if (STRM_JPGERemoveHeader(pEncPara->hStrmMng, &stInputInfo, &stStreamInfo) != HI_SUCCESS)
    {
...
static HI_VOID VENC_DRV_EflSetRegCfgData(VENC_HAL_CFG_S *pRegCfg, VeduEfl_EncPara_S *pEncPara)
{
    HI_U32 i = 0;
    ...
    pRegCfg->CtrlCfg.TimeOutEn = 2;

    pRegCfg->CtrlCfg.RegConfigMode = VENC_REG_CONFIG_MODE;/*0: cfg mode 1: ddr mode*/
   ...
   /*get RcCfg default info*/
    ...
    pRegCfg->RcCfg.bAVBR = pEncPara->stRc.IsAVBR;
   ...
   /*get StreamCfg info*/
    ...
    pRegCfg->ExtCfg.ExtFlag = pEncPara->stHal.ExtFlag;
    pRegCfg->ExtCfg.ExtFillLen = pEncPara->stHal.ExtFillLen;
...
static HI_S32 VENC_DRV_EflCfgRegVenc(VeduEfl_EncPara_S* EncHandle)
{
    VENC_HAL_CFG_S RegCfg;
    VeduEfl_EncPara_S* pEncPara = EncHandle;

    memset(&RegCfg, 0, sizeof(VENC_HAL_CFG_S));
    VENC_DRV_BoardInit();
    VENC_DRV_EflSetRegCfgData(&RegCfg, pEncPara);
    VENC_HAL_CfgReg(&RegCfg);

    return HI_SUCCESS;
}

static HI_VOID VENC_DRV_EflGetReadBackData(VENC_HAL_READ_S* pReadBack, Vedu_Hal_S* pstHal, VeduEfl_Rc_S* pstRc)
{
    HI_U32 i = 0;
    ...
    pstRc->MeanQP  = pReadBack->MeanQP;
    pstRc->NumIMBCurFrm = pReadBack->NumIMBCurFrm;

    pstRc->MhbBits = pReadBack->MhbBits;
    pstRc->TxtBits = pReadBack->TxtBits;
    pstRc->MadiVal = pReadBack->MadiVal;
    pstRc->MadpVal = pReadBack->MadpVal;
...
static HI_VOID MakeSliceHeader(VeduEfl_EncPara_S *pEncPara)
{
    if (pEncPara->Protocol == VEDU_H264)
    {
        ...
        memset(&pEncPara->stHal.SlcHdrStream, 0, sizeof(HI_U32)*4);
        memset(&pEncPara->stHal.ReorderStream, 0, sizeof(HI_U32)*2);
        memset(&pEncPara->stHal.MarkingStream, 0, sizeof(HI_U32)*2);

        pEncPara->stHal.SlcHdrBits = H264e_MakeSlcHdr(pEncPara, &SlcHdr);
    }
...
static HI_VOID GetHalInfo(VeduEfl_EncPara_S *pEncPara, VeduEfl_EncIn_S *pEncIn)
{
    ...
    pEncPara->stHal.ExtFlag = pEncPara->stImage_OMX.flags;
    pEncPara->stHal.ExtFillLen = pEncPara->stImage_OMX.data_len;
...
static HI_S32 GetFrameWriteBuf(VeduEfl_EncPara_S *pEncPara)
{
    ...
    if (ret != HI_SUCCESS)
    {
        pEncPara->CurStrmBufSize = stStreamInfo.CurStrmBufSize;
        return ret;
    }
...
static HI_S32 VENC_DRV_EflStartOneFrameVenc(VeduEfl_EncPara_S *EncHandle, VeduEfl_EncIn_S *pEncIn )
{
    HI_U32 ret = HI_SUCCESS;
    VeduEfl_EncPara_S *pEncPara  = EncHandle;
    ...
    if (ret != HI_SUCCESS)
    {
        pEncPara->stStat.StartOneFrameFail++;
    }
    else
    {
        pEncPara->bFirstSlcForLowDly = HI_TRUE;
    }
...
static HI_VOID PutFrameWriteBuffer(VeduEfl_EncPara_S *pEncPara)
{
    ...
    HI_INFO_VENC("Current Buffer Size= %d Output PicSize = %d  TooFewBufferSkip = %d, pts = 0x%x\n",
        stStreamInfo.pstBuf[0].u32Size,stInputInfo.SliceLength[0],stStreamInfo.TooFewBufferSkip,pEncPara->stImage_OMX.timestamp0);
...
static HI_VOID PutJPGEWriteBuffer(VeduEfl_EncPara_S *pEncPara)
{
    ...
    if (STRM_PutJPGEWriteBuffer(pEncPara->hStrmMng, &stInputInfo, &stStreamInfo) != HI_SUCCESS)
    {
...
static HI_VOID VENC_DRV_EflInitCfg(VeduEfl_EncPara_S *pEncPara, VeduEfl_EncCfg_S *pEncCfg)
{
    ...
    /* init RC para */
    pEncPara->stRc.IsAVBR     = pEncCfg->IsAVBR;
    pEncPara->stRc.IsIntraPic = 1;

    pEncPara->stRc.MinTimeOfP = 3;
    pEncPara->stRc.MaxTimeOfP = (pEncPara->stRc.IsAVBR)? 200 : 6;
...
static HI_VOID GetRcnAndMVAddr(Vedu_Hal_S *pstHal, HI_U32 LumaSize, HI_U32 BusRcnBuf, HI_U32 CapLevel, HI_U32 Protocol)
{
    HI_U32 j = BusRcnBuf + LumaSize * 3;
    HI_U32 PmeBufSize = 0;
    HI_U32 PmeInfoBufSize = 0;
    HI_U32 MvBufSize = 0;
    HI_U32 PicWidth = 0;
    ...
    {
        case VEDU_CAP_LEVEL_QCIF:
            PicWidth = D_VENC_ALIGN_UP(176 , VEDU_MMZ_ALIGN);
            break;

        case VEDU_CAP_LEVEL_CIF:
            PicWidth = D_VENC_ALIGN_UP(352 , VEDU_MMZ_ALIGN);
            break;

        case VEDU_CAP_LEVEL_D1:
            PicWidth = D_VENC_ALIGN_UP(720 , VEDU_MMZ_ALIGN);
            break;

        case VEDU_CAP_LEVEL_720P:
            PicWidth = D_VENC_ALIGN_UP(1280 , VEDU_MMZ_ALIGN);
            break;

        case VEDU_CAP_LEVEL_1080P:
            PicWidth = D_VENC_ALIGN_UP(1920 , VEDU_MMZ_ALIGN);
            break;
    ...
    /*pic encode support turn 90 degrees, CapLevel:1080P not only support 1920x1088 but also 1088x1920,
      so set PicHeight as same as PicWidth here!*/
    PmeBufSize     = D_VENC_CALC_PME_SIZE(PicWidth,PicWidth);
    MvBufSize      = D_VENC_CALC_MV_SIZE(PicWidth,PicWidth);
    PmeInfoBufSize = D_VENC_CALC_PMEINFO_SIZE(Protocol,PicWidth);
...
static HI_U32 CalcMeMvBufferSize(HI_U32 CapLevel, HI_U32 Protocol)
{
    HI_U32 PmeBufSize = 0;
    HI_U32 PmeInfoBufSize = 0;
    HI_U32 MvBufSize = 0;
    ...
    TotalSize = PmeBufSize*2 + MvBufSize*2 + PmeInfoBufSize*3;
...
HI_S32 VENC_DRV_EflCreateVenc(VeduEfl_EncPara_S **pEncHandle, VeduEfl_EncCfg_S *pEncCfg)
{
    ...
    HI_U32 StrmBufExtLen = 0;
    HI_U32 MeMvBufSize = 0;
    STRMMNG_ALLOC_INFO_S  stBufferAllocInfo;
    STRMMNG_STREAM_INFO_S stStreamInfo;
    ...
    pEncPara->hStrmMng = (pEncPara->hStrmMng & 0xffff0000) | (pEncCfg->PrivHandle & 0xffff);
    ...
    MeMvBufSize = CalcMeMvBufferSize(pEncCfg->CapLevel, pEncCfg->Protocol);

    BitBufSize = STRM_GetBitBufSize(pEncPara->hStrmMng, &pEncCfg->streamBufSize, pEncCfg->SlcSplitEn, LumaSize, &StrmBufExtLen);
    if (BitBufSize == 0)
    {
        goto error_1;
    }
    ...
    memset(&stBufferAllocInfo, 0, sizeof(STRMMNG_ALLOC_INFO_S));
    stBufferAllocInfo.Protocol = pEncCfg->Protocol;
    stBufferAllocInfo.BitBufSize = BitBufSize;
    stBufferAllocInfo.LumaSize   = LumaSize;
    stBufferAllocInfo.MeMvSize   = MeMvBufSize;
    if (STRM_AllocStreamBuf(pEncPara->hStrmMng, &pEncPara->StreamMMZBuf, &stBufferAllocInfo) != HI_SUCCESS)
    {
        goto error_1;
    }
    ...
    pstHal->RegConfigMode = VENC_REG_CONFIG_MODE;
    ...
    GetRcnAndMVAddr(pstHal, LumaSize, BusRcnBuf, pEncCfg->CapLevel, pEncCfg->Protocol);
    ...
    else
    {
        ...
        pEncPara->FrameDequeue = VENC_DRV_MngQueueInit(MAX_VEDU_QUEUE_NUM, HI_FALSE, HI_FALSE);
    ...
    SetStreamInfo(pEncPara, &stStreamInfo);
    stStreamInfo.StrmBufExtLen = StrmBufExtLen;
...
HI_S32 VENC_DRV_EflDestroyVenc( VeduEfl_EncPara_S* EncHandle )
{
    ...
    if (pEncPara->OMXChn)
    {
        ...
        {
            if ( 0 != pEncPara->OmxOutputBuf[i].pu8StartVirAddr)
            {
               sMBufVenc_temp.pu8StartVirAddr = pEncPara->OmxOutputBuf[i].pu8StartVirAddr;
...
static HI_S32 VENC_DRV_EflRcGetAttr( HI_UNF_VENC_CHN_ATTR_S *pChnUserCfg, VeduEfl_EncPara_S* EncHandle, VeduEfl_RcAttr_S* pRcAttr )
{
    VeduEfl_EncPara_S* pEncPara = EncHandle;
...
HI_S32 VENC_DRV_EflRcAttrInit(VeduEfl_EncPara_S* EncHandle, VeduEfl_RcAttr_S* pRcAttr)
{
    ...
    pEncPara->stRc.MinStillPercent = 5;
    pEncPara->stRc.MaxStillQP      = 30;
    if (pEncPara->stRc.IsAVBR)
    {
        pEncPara->stRc.MaxQp     = 51;
        pEncPara->stRc.MinQp     = 24;
    }
    else
    {
...
static HI_U32 GetFrmRate_UserConfig( VeduEfl_EncPara_S* pEncPara, HI_U32 u32ChnID)
{
    ...
    if (pEncPara->stRc.LastSecFrameCntForUsr >= newViFrmRate)
    {
...
static HI_VOID UpdateInputFrmRate( VeduEfl_EncPara_S* pEncPara, HI_U32 newViFrmRate, HI_UNF_VENC_CHN_ATTR_S *pstSrcUserCfg)
{
    ...
    if ((newViFrmRate != pEncPara->stRc.ViFrmRate) && (newViFrmRate <= FrmRateCntBufNum))
    {
        ...
        if (pstSrcUserCfg->u32TargetFrmRate > pEncPara->stRc.ViFrmRate)
        {
            pEncPara->stRc.VoFrmRate = pEncPara->stRc.ViFrmRate;
        }
        else
        {
            pEncPara->stRc.VoFrmRate = pstSrcUserCfg->u32TargetFrmRate;
        }
...
static HI_S32 VENC_DRV_EflSetResolution(VeduEfl_EncPara_S* EncHandle, HI_U32 FrameWidth, HI_U32 FrameHeight, HI_U32 u32ChnID)
{
    ...
    VENC_DRV_EflRcGetAttr(&(g_stVencChn[u32ChnID].stChnUserCfg), pEncPara, &RcAttr);
...
static HI_S32 VENC_DRV_EflCheckOmxImgCfg(const venc_user_buf *pstPreImage,HI_U32 yuvStoreType)
{
    HI_BOOL flag = 0;

    flag |= (pstPreImage->picWidth > HI_VENC_MAX_WIDTH)||(pstPreImage->picWidth < HI_OMXVENC_MIN_WIDTH);
    flag |= (pstPreImage->picHeight > HI_VENC_MAX_HEIGTH)||(pstPreImage->picHeight < HI_OMXVENC_MIN_HEIGTH);
...
static HI_VOID VENC_DRV_EflUpdateScanType(VeduEfl_EncPara_S *pEncPara)
{
...
static HI_S32 GetLocalChannelInfo_X(VeduEfl_EncPara_S* pEncPara, HI_U32 u32ChnID)
{
   ...
   if (pEncPara->stHal.PTS0 == pEncPara->stImage.u32Pts)
   {
    ...
    if (VENC_DRV_EflRcFrmRateCtrl(pEncPara, pEncPara->stStat.PutFrameNumOK) != HI_SUCCESS)
    {
        PutChannelImageForError(pEncPara);
        pEncPara->stHal.PTS0 = pEncPara->stImage.u32Pts;
    ...
    if (VENC_DRV_EflCheckImgCfg(pEncPara->Protocol, &pEncPara->stImage, pEncPara->stHal.YuvStoreType) != HI_SUCCESS)
    {
...
static HI_S32 GetLocalChannelInfo_Y(VeduEfl_EncPara_S* pEncPara, HI_U32 u32ChnID)
{
    pEncPara->stStat.GetFrameNumTry++;
    if (!pEncPara->stSrcInfo.pfGetImage_OMX)
    {
        HI_ERR_VENC("pEncPara->stSrcInfo.pfGetImage_OMX is Null!\n");
        return HI_FAILURE;
    }
    ...
    if ((pEncPara->stImage_OMX.picWidth != pEncPara->stRc.PicWidth)
        || (pEncPara->stImage_OMX.picHeight != pEncPara->stRc.PicHeight))
    {
        HI_ERR_VENC("Not Support Resolution change!!! %dx%d ==> %dx%d\n",
            pEncPara->stRc.PicWidth,pEncPara->stRc.PicHeight,
    ...
    /* check the picture resolving power ,stride ,addr info first*/
    if (VENC_DRV_EflCheckOmxImgCfg(&pEncPara->stImage_OMX, pEncPara->stHal.YuvStoreType) != HI_SUCCESS)
    {
        HI_ERR_VENC("stImg cfg erro!!\n");
        pEncPara->stStat.PutFrameNumTry++;
        if (VENC_DRV_EflPutMsg_OMX(pEncPara->MsgQueue_OMX, VENC_MSG_RESP_INPUT_DONE, HI_SUCCESS , &(pEncPara->stImage_OMX)) != HI_SUCCESS)
...
static HI_S32 VENC_DRV_EflQueryChn_Y(HI_U32 u32ChnID, VeduEfl_EncIn_S *pEncIn)
{
    ...
    if (pEncPara->bNotEnoughBuf != HI_TRUE)
    {
        if (GetLocalChannelInfo_Y(pEncPara, u32ChnID) != HI_SUCCESS)
        {
            return HI_FAILURE;
        }
    }
...
static VeduEfl_EncPara_S* VENC_DRV_EflQueryChn_H264(VeduEfl_EncIn_S* pEncIn, HI_BOOL IsIsr)
{
    ...
    for (u32StartQueryNo = 0;u32StartQueryNo < HI_VENC_MAX_CHN;u32StartQueryNo++)
    {
        ...
        if ((IsIsr == HI_TRUE) && (g_stVencChn[u32ChnID].stProcWrite.bSaveYUVFileRun))
        {
            continue;
        }
    ...
    else
    {
        if (g_stVencChn[u32ChnID].stProcWrite.bSaveYUVFileRun && pEncPara)
        {
            if (pEncPara->OMXChn)
            {
                VENC_DRV_DbgWriteYUV_OMX(&pEncPara->stImage_OMX, g_stVencChn[u32ChnID].stProcWrite.YUVFileName);
            }
            else
            {
...
{
#ifdef HI_SMMU_SUPPORT
    VENC_HAL_READ_SMMU_S ReadBack = {0};
...
static HI_VOID VENC_DRV_EncNextFrm(HI_U32 u32VeChn, VeduEfl_EncIn_S *pEncIn)
{
    ...
    pEncPara = VENC_DRV_EflQueryChn_H264( pEncIn,  HI_TRUE);
...
static HI_S32 VEDU_DRV_RCProcessOfRecoding(VeduEfl_EncPara_S *pEncPara, VeduEfl_EncIn_S *EncIn)
{
    STRMMNG_INPUT_INFO_S stInputInfo;
    STRMMNG_STREAM_INFO_S stStreamInfo;
...
static HI_VOID Venc_ISR( HI_VOID )
{
    ...
    if (ReadBack.VencSliceInt)
    {
         ...
         if (pEncPara->bFirstSlcForLowDly)
         {
            VENC_DRV_DbgLowdelayEvent(u32VeChn,EVENT_VENC_FRM_OUT, pEncPara->stImage.u32FrameIndex,pEncPara->stImage.hTunnelSrc);
            pEncPara->bFirstSlcForLowDly = HI_FALSE;
         }
    ...
    if (ReadBack.VencEndOfPic)
    {
...

         {
            ...
            if (pEncPara->stRc.PicBits != 0 || ReadBack.VencBufFull || ReadBack.VencPbitOverflow)
            {
                ...
                if (((ReadBack.VencBufFull == 1) || (ReadBack.VencPbitOverflow == 1)) && pEncPara->stRc.RecodeCnt)
                {
                    HI_ERR_VENC("recode time = %d still bufferFull(%d) or VencPbitOverflow(%d)!\n",pEncPara->stRc.RecodeCnt,ReadBack.VencBufFull,ReadBack.VencPbitOverflow);
                }

            #if RC_RECODE_USE
                pEncPara->stRc.CheckBufSize = pEncPara->CurStrmBufSize;
        ...
        if(pEncPara->OMXChn && pEncPara->PrependSpsPpsEnable &&
            ((g_stVencChn[u32VeChn].stProcWrite.bTimeModeRun == HI_TRUE) ||
            (g_stVencChn[u32VeChn].stProcWrite.bFrameModeRun == HI_TRUE)))
        {
            pEncPara->PrependSpsPpsEnable = HI_FALSE;
        }
...
#ifdef VENC_SUPPORT_JPGE
        {
            ...
            if (pEncPara != NULL)
            {
                ...
                if (HI_INVALID_HANDLE != g_stVencChn[u32ChnID].hJPGE)  /*JPGE CHANNEL*/
                {
                    ...
                    HI_DBG_VENC("prepare to encode!Wp = %x, Rp = %x\n",*pEncPara->StrmBufWpVirAddr,*pEncPara->StrmBufRpVirAddr);
                    ...
                    {
                        HI_DBG_VENC("encode ==>success!!!!Wp = %x, Rp = %x, bufferLen = %x\n",*pEncPara->StrmBufWpVirAddr,*pEncPara->StrmBufRpVirAddr,pEncPara->stHal.StrmBufSize);
...
#endif
        /* if ipfree, sleep */
        s32Ret = VENC_DRV_OsalWaitEvent(&g_VENC_Event, msecs_to_jiffies(15));
...
static HI_VOID EflTask_Stream_ListBuf(VeduEfl_EncPara_S* pEncPara, venc_user_buf *pstStreamBuf, HI_VOID* venc_stream_addr)
{
    ...
    if (STRM_GetWrittenLength(pEncPara->hStrmMng) > 0)
    {
        s32Ret = VENC_DRV_EflAcqReadBuffer(pEncPara, &Nalu);
        DataLen = Nalu.SlcLen[0];
        if ((DataLen > 0) && (DataLen <= SLICE_SIZE_LIMIT) && (pstStreamBuf->buffer_size >= DataLen) && (s32Ret == HI_SUCCESS))
        {
        ...
        else
        {
            ...
            if(VENC_DRV_EflPutMsg_OMX(pEncPara->MsgQueue_OMX, VENC_MSG_RESP_FLUSH_OUTPUT_DONE, s32Ret, pstStreamBuf) != HI_SUCCESS)
            {
                HI_ERR_VENC("put msg omx err!\n");
            }
        ...
        if (VENC_DRV_EflRlsReadBuffer(pEncPara, &Nalu) != HI_SUCCESS)
        {
        ...
        {
            pstStreamBuf->flags |= OMXVENC_BUFFERFLAG_EOS;
            if (Nalu.ExtFillLen == 0)
            {
                pstStreamBuf->data_len = 0;
            }
        }

        if (VENC_DRV_EflPutMsg_OMX(pEncPara->MsgQueue_OMX, VENC_MSG_RESP_OUTPUT_DONE, s32Ret , pstStreamBuf) != HI_SUCCESS)
        {
            HI_ERR_VENC("put msg omx err!\n");
        }
        pEncPara->stStat.FbdCnt++;
...
static HI_VOID EflTask_Stream_LoopBuf(VeduEfl_EncPara_S* pEncPara, venc_user_buf *pstStreamBuf, HI_VOID* venc_stream_addr)
{
    ...
    do
    {
        if (STRM_GetWrittenLength(pEncPara->hStrmMng) > 0)
        {
            ...
            s32Ret = VENC_DRV_EflAcqReadBuffer(pEncPara, &Nalu);
            if (Check_StreamBuf_Size(&Nalu, pstStreamBuf->buffer_size) == HI_TRUE)
            {
                if (Nalu.SlcLen[0] > 0)
                {
...
static HI_VOID VENC_DRV_EflTask_Stream( HI_VOID )
{
    ...
    while (!VeduIpCtx.StopTask)
    {
        ...
        if (EncHandle != NULL)
        {
            ...
            s32Ret = down_interruptible(&g_VencMutex);
            if (s32Ret != HI_SUCCESS)
            {
            ...
            for (i = 0; i < OMX_OUTPUT_PORT_BUFFER_MAX; i++)
            {
                if (pEncPara->OmxOutputBuf[i].u32StartPhyAddr == StreamBuf.bufferaddr_Phy)
                {
                    findBuf = 1;
                    break;
                }
        ...
        else
        {
           ...
           //msleep(10);
           s32Ret = VENC_DRV_OsalWaitEvent(&g_VENC_Event_stream, msecs_to_jiffies(10));
           if (0 != s32Ret)
           {
               HI_INFO_VENC("wait stream time out!\n");
           }
...
HI_S32 VENC_DRV_EflAcqReadBuffer(VeduEfl_EncPara_S *phVencChn, VeduEfl_NALU_S *pstVeduPacket)
{
    HI_S32 s32Ret = HI_FAILURE;
    STRMMNG_STREAM_INFO_S stStreamInfo;
    ...
    SetStreamInfo(phVencChn, &stStreamInfo);
    s32Ret = STRM_AcqReadBuffer(phVencChn->hStrmMng, &stStreamInfo, pstVeduPacket);

    phVencChn->stStat.StreamTotalByte = stStreamInfo.StreamTotalByte;

    if (s32Ret == HI_SUCCESS)
    {
        if (VENC_DRV_DbgSaveStream(pstVeduPacket, phVencChn) != HI_SUCCESS)
        {
...
HI_S32 VENC_DRV_EflRlsReadBuffer(VeduEfl_EncPara_S *phVencChn, VeduEfl_NALU_S *pstVeduPacket)
{
    HI_S32 s32Ret = HI_FAILURE;
    STRMMNG_STREAM_INFO_S stStreamInfo;

    SetStreamInfo(phVencChn, &stStreamInfo);
    s32Ret = STRM_RlsReadBuffer(phVencChn->hStrmMng, &stStreamInfo, pstVeduPacket);
...
#if (defined(VENC_96MV200_CONFIG) || defined(VENC_BINARY_COMPATIBLE))
        DRV_VENC_PDT_V500R001_Init();
#else
        HI_ERR_VENC("Current SDK Version VENC firmware not support binary compatible!\n");
#endif
    }
    else
    {
        VeduIpCtx.IsChipIDV500R001 = HI_FALSE;
#if (defined(VENC_98MV200_CONFIG) || defined(VENC_BINARY_COMPATIBLE))
