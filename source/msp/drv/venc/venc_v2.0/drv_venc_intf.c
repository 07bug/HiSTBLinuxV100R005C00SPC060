...
static HI_S32 VENC_Ioctl_MmzMap(HI_VOID * arg)
{
    ...
    Ret = VENC_DRV_MMZ_Map_OMX(g_stVencChn[u32Index].hVEncHandle, &(pMMBInfo.stVencBuf));
    up(&g_VencMutex);
    if (HI_SUCCESS == Ret)
    {
...
static HI_S32 VENC_Ioctl_MmzUmmap(HI_VOID * arg)
{
    ...
    Ret = VENC_DRV_MMZ_UMMap_OMX(g_stVencChn[u32Index].hVEncHandle, &(pMMBInfo.stVencBuf));
    up(&g_VencMutex);
