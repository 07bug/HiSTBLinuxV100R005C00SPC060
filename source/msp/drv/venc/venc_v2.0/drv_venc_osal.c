...
static HI_VOID* VencKernelRegisterMap(unsigned long PhyAddr, unsigned long size)
{
    return ioremap (PhyAddr, size);
}

static HI_VOID VencKernelRegisterUnMap(HI_VOID* VirAddr)
{
    iounmap(VirAddr);
    return;
}
...
{
    HI_U32 u32ptaddr = 0;
    HI_U32 u32err_rdaddr = 0;
    HI_U32 u32err_wraddr = 0;

    HI_DRV_SMMU_GetPageTableAddr(&u32ptaddr, &u32err_rdaddr, &u32err_wraddr);

    *pSmmuPageBaseAddr = u32ptaddr;
    *pSmmuErrReadAddr  = u32err_rdaddr;
    *pSmmuErrWriteAddr = u32err_wraddr;
}
#endif
...
/************************************************************************/
static VOID *OSAL_VmallocVirMem(SINT32 Size)
{
    return vmalloc(Size);
}
/************************************************************************/
...
/************************************************************************/
static VOID OSAL_VfreeVirMem(VOID *p)
{
    if (p)
    {
        vfree(p);
...
{
    venc_Osal_Func_Ptr_S.pfun_Osal_printk = printk;
    venc_Osal_Func_Ptr_S.pfun_Osal_MemSet = (FN_OSAL_MemSet)memset;
    venc_Osal_Func_Ptr_S.pfun_Osal_MemCpy = (FN_OSAL_MemCpy)memcpy;
    venc_Osal_Func_Ptr_S.pfun_Osal_ioremap = VencKernelRegisterMap;
    venc_Osal_Func_Ptr_S.pfun_Osal_iounmap = VencKernelRegisterUnMap;
    venc_Osal_Func_Ptr_S.pfun_Osal_AllocVirMem = OSAL_VmallocVirMem;
    venc_Osal_Func_Ptr_S.pfun_Osal_FreeVirMem  = OSAL_VfreeVirMem;
}
