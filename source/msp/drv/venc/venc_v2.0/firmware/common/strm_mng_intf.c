...
static HI_S32 GetConfigInfo(HI_HANDLE hStrmMng, STRMMNG_STREAM_INFO_S *pstStreamInfo, STRMMNG_INST_CONFIG_S *pstConfig)
{
    ...
    else if (hStrmMng == STRM_MODE_RECYCLE)
    {
        ...
        pstConfig->pu8KnlVirAddr     = pstStreamInfo->StreamMMZBuf.pu8StartVirAddr + VEDU_MMZ_ALIGN;
        pstConfig->u32RsvByte        = 64;
        pstConfig->u32StrmExtLen     = pstStreamInfo->StrmBufExtLen;
    }

    return HI_SUCCESS;
}

HI_S32 STRM_Create(HI_HANDLE hHandle, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
    HI_S32 Ret = 0;
    HI_U32 StrmMode  = 0;
    HI_U32 PrivateID = 0;
...
HI_U32 STRM_GetBitBufSize(HANDLE hHandle, HI_U32 *pStrmBufSize, HI_U32 SlcSplitEn, HI_U32 LumaSize, HI_U32 *pExtLen)
{
    HI_S32 BitBufSize = 0;
    HI_U32 StrmMode  = 0;
    HI_U32 PrivateID = 0;

    if ((pStrmBufSize == HI_NULL) || (pExtLen == HI_NULL))
    {
        return 0;
    }
    ...
    else
    {
        *pStrmBufSize = D_VENC_ALIGN_UP(*pStrmBufSize, VEDU_MMZ_ALIGN);
        ...
        BitBufSize = g_venc_strm_fun_ptr[StrmMode].pfun_STRM_GetBitBufSize(PrivateID, *pStrmBufSize, SlcSplitEn, LumaSize, pExtLen);
    }

    return BitBufSize;
}

HI_S32 STRM_AllocStreamBuf(HANDLE hHandle, VENC_BUFFER_S *pStreamMMZBuf, STRMMNG_ALLOC_INFO_S *pAllocInfo)
{
    HI_S32 Ret = 0;
    HI_U32 StrmMode  = 0;
    HI_U32 PrivateID = 0;
    ...
    {
        Ret = g_venc_strm_fun_ptr[StrmMode].pfun_STRM_AllocStreamBuf(PrivateID, pStreamMMZBuf, pAllocInfo);
    }
