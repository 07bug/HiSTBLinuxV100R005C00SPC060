...
HI_S32 ListBuf_GetStrmState(HI_HANDLE hBuf, STRMMNG_State_S* pStrmStatInfo)
{
    ...
    if (pstInst == HI_NULL || pStrmStatInfo == HI_NULL)
    {
...
HI_VOID ListBuf_DeInit(HI_VOID)
{
    ...
    if (!list_empty(head))
    {
        ...
        s_stBMParam.IsInit = 0;
        LISTBUF_UNLOCK(s_stBMParam.stSem);
    }
}

HI_S32 ListBuf_Create(HI_HANDLE hBuf, STRMMNG_INST_CONFIG_S *pstConfig)
{
    LISTBUF_INST_S *pstInst;
    struct list_head* pos;
    struct list_head* n;
    ...
    memset(pstInst, 0, sizeof(LISTBUF_INST_S));
...
static HI_S32 GetOneWriteBuffer(HI_HANDLE hBuf, STRMMNG_BUF_S *pstBuf)
{
    ...
    HI_U32 u32TailFree = 0;
    HI_U32 u32HeadFree = 0;
...
get_attr:
    ...
    memset(pstBlock, 0, sizeof(LISTBUF_BLOCK_S));
...
static HI_S32 GetParaSetWriteBuffer(HI_HANDLE hHandle, STRMMNG_INPUT_INFO_S *pstInputInfo, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
    ...
    if ((pstInputInfo->PrependSpsPpsEnable == HI_TRUE) || (pstInputInfo->GetFrameNumOK == 1) || (pstStreamInfo->IFrmInsertBySaveStream))
    {
        if (pstInputInfo->Protocol == VEDU_H264)
        {
            ...
            pstStreamInfo->ParaSetLen = D_VENC_ALIGN_UP(HeaderRealLength, VEDU_MMZ_ALIGN);
            ...
            stBuf.u32NaluType = H264_NALU_TYPE_SPS;
        }
        else
        {
            ...
            stBuf.u32NaluType = H265_NALU_TYPE_SPS;/*para set only use sps as Nalu Type*/
        }
...
static HI_VOID GetStrmBufInfoForSlice(STRMMNG_BUF_S *pstBuf, STRMMNG_INPUT_INFO_S *pstInputInfo, STRMMNG_STREAM_INFO_S *pstStreamInfo, HI_U32 BufNum)
{
    ...
    pstBuf->u32Size = D_VENC_ALIGN_UP(MaxBufSize, VEDU_MMZ_ALIGN);
    pstBuf->u32InvldByte = pstBuf->u32Size - MaxBufSize;
    pstStreamInfo->CurStrmBufSize = D_VENC_ALIGN_UP(CheckBufSize, VEDU_MMZ_ALIGN);

    HI_INFO_VENC("pstInputInfo->TargetBits = %d, pstBuf->u32Size = %d,CurStrmBufSize = %d\n",pstInputInfo->TargetBits,pstBuf->u32Size,pstStreamInfo->CurStrmBufSize);
...
static HI_S32 PutOneWriteBuffer(HI_HANDLE hBuf, STRMMNG_BUF_S *pstBuf)
{
    ...
    if (IS_WRITING(pstBlock) && (pstBuf->u32Addr == pstBlock->u32Addr))
    {
        ...
        pstBlock->u32ExtFlag = pstBuf->ExtFlag;
        pstBlock->u32ExtFillLen = pstBuf->ExtFillLen;
...
static HI_VOID PutSliceStrmBufNormal(HI_HANDLE hBuf, STRMMNG_INPUT_INFO_S *pstInputInfo, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
    ...
    for (i = SlicIdx; i < BufNum; i++)
    {
        ...
        if (RealSliceSize)
        {
            ...
            pstBuf[i].ExtFlag   = pstInputInfo->ExtFlag;
            pstBuf[i].ExtFillLen = pstInputInfo->ExtFillLen;
...
static HI_S32 ListBuf_PutWriteBufSlices(HI_HANDLE hBuf, STRMMNG_INPUT_INFO_S *pstInputInfo, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
...
static HI_S32 ListBuf_PutWriteBufSlicesAb(HI_HANDLE hBuf, STRMMNG_INPUT_INFO_S* pstInputInfo, STRMMNG_STREAM_INFO_S* pstStreamInfo)
{
...
static HI_S32 AcqOneReadBuffer(HI_HANDLE hBuf, STRMMNG_BUF_S *pstBuf)
{
    ...
    pstBuf->ExtFlag    = pstBlock->u32ExtFlag;
    pstBuf->ExtFillLen = pstBlock->u32ExtFillLen;
                                     ...
                                     VENC_STREAM_S* pstStream, VENC_BUF_OFFSET_S *pstBufOffSet)
{
    ...
    LISTBUF_ASSERT_RETURN((pstVeduPacket != NULL), HI_ERR_LISTBUF_INVALID_PARA);
    LISTBUF_ASSERT_RETURN((pstStream != NULL), HI_ERR_LISTBUF_INVALID_PARA);
    LISTBUF_ASSERT_RETURN((pstBufOffSet != NULL), HI_ERR_LISTBUF_INVALID_PARA);
...
{
    STRMMNG_BUF_S stBuf = {0};
    HI_S32 s32Ret;

    LISTBUF_ASSERT_RETURN((pstStreamInfo != NULL), HI_ERR_LISTBUF_INVALID_PARA);
    LISTBUF_ASSERT_RETURN((pstVeduPacket != NULL), HI_ERR_LISTBUF_INVALID_PARA);
    ...
    pstVeduPacket->PTS1         = stBuf.u32Pts1Ms;
    pstVeduPacket->ExtFlag      = stBuf.ExtFlag;
    pstVeduPacket->ExtFillLen   = stBuf.ExtFillLen;
...
static HI_S32 ListBuf_GetFreeLength(LISTBUF_INST_S* pstInst, HI_U32 NaluType)
{
    LISTBUF_BLOCK_S* pstBlockTail = HI_NULL;
    LISTBUF_BLOCK_S* pstBlockRead = HI_NULL;
    HI_U32 u32ReadOffset;
...
static HI_S32 ListBuf_RlsReadBuffer(HI_HANDLE hBuf, STRMMNG_BUF_S *pstBuf)
{
    HI_U32 u32PhyAddr;
    LISTBUF_INST_S *pstInst   = HI_NULL;
    LISTBUF_BLOCK_S *pstBlock = HI_NULL;
...
{
    HI_U32 NaluType = 0;
    HI_VOID*  pBase = HI_NULL;
    VENC_STREAM_S  stStream;

    LISTBUF_ASSERT_RETURN((pstStreamInfo != NULL), HI_ERR_LISTBUF_INVALID_PARA);
    LISTBUF_ASSERT_RETURN((pstAcqStrm != NULL), HI_ERR_LISTBUF_INVALID_PARA);
    LISTBUF_ASSERT_RETURN((pstVeduPacket != NULL), HI_ERR_LISTBUF_INVALID_PARA);

    pBase = pstStreamInfo->StreamMMZBuf.pu8StartVirAddr + VEDU_MMZ_ALIGN;
    stStream = pstAcqStrm->stStream;

    if (Protocol == VEDU_H264)
    {
        if (stStream.enDataType.enH264EType == HI_UNF_H264E_NALU_SPS)
        {
    ...
    {
        if (stStream.enDataType.enHEVCEType == HI_UNF_HEVCE_NALU_SPS)
        {
            stStream.VirAddr  = (HI_VIRT_ADDR_T)(pstStreamInfo->SliceBuf.pu8StartVirAddr + pstAcqStrm->H264_stream_off);
        }
        else
...
HI_S32 ListBuf_CheckBufferSize(HI_HANDLE hBuf, HI_U32 StrmBufSize, HI_U32 CurStrmBufSize)
{
    ...
    memset(&stBuf, 0, sizeof(STRMMNG_BUF_S));
    stBuf.u32Size = D_VENC_ALIGN_UP(CurStrmBufSize, VEDU_MMZ_ALIGN);

    if (stBuf.u32Size > StrmBufSize)
    {
        stBuf.u32Size = D_VENC_ALIGN_DOWN(StrmBufSize, VEDU_MMZ_ALIGN);
    }
...
HI_U32 ListBuf_GetBitBufSize(HI_HANDLE hBuf, HI_U32 StreamBufSize, HI_U32 SlcSplitEn, HI_U32 LumaSize, HI_U32 *pExtLen)
{
    return StreamBufSize;
}

HI_S32 ListBuf_AllocStreamBuf(HI_HANDLE hBuf, VENC_BUFFER_S* pStreamMMZBuf, STRMMNG_ALLOC_INFO_S *pAllocInfo)
{
    ...
    AllocBufSize = pAllocInfo->LumaSize * 3 + pAllocInfo->BitBufSize + pAllocInfo->MeMvSize;
...
{
    HI_S32 s32Ret = HI_FAILURE;
    const HI_U32 BitBufSize = 4096;

    s32Ret = HI_DRV_VENC_MMZ_AllocAndMap("VENC_ParaSetBuf", HI_NULL, BitBufSize, VEDU_MMZ_ALIGN, pSliceMMZBuf);
...
HI_S32 ListBuf_AllocParaSetBufAddr(HI_HANDLE hBuf, VENC_BUFFER_S *pParaSetBuf, HI_U32 *pParaSetADDR)
{
    ...
    LISTBUF_ASSERT_RETURN(pParaSetBuf != HI_NULL, HI_ERR_LISTBUF_INVALID_PARA);
    LISTBUF_ASSERT_RETURN(pParaSetADDR != HI_NULL, HI_ERR_LISTBUF_INVALID_PARA);
    ...
    memset(pParaSetBuf->pu8StartVirAddr, 0, BitBufSize);
