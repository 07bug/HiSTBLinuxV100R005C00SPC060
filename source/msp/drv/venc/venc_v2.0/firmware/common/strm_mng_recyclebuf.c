...
#define RECYCLEBUF_ASSERT_RETURN(Condition, RetValue)                  \
do                                                                 \
{                                                                  \
    if (!Condition)                                                \
    {                                                              \
...
{
    if (!pstInputInfo->RecodeCnt)
    {
        if ((pstInputInfo->PrependSpsPpsEnable == HI_TRUE) || (pstInputInfo->GetFrameNumOK == 1) || (pstStreamInfo->IFrmInsertBySaveStream))
        {
            ...
            pstStreamInfo->IFrmInsertBySaveStream = 0;
        }
    }
...
static HI_S32 GetStreamWriteBuffer(VALG_CRCL_BUF_S* pstStrBuf, STRMMNG_INPUT_INFO_S *pstInputInfo, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
    ...
    *(pstStreamInfo->StrmBufWpVirAddr) = pstStrBuf->u32WrHead;
    ...
    pstStreamInfo->CurStrmBufSize = D_VENC_ALIGN_UP(pstInputInfo->TargetBits * BUF_SIZE_MULT / 8, VEDU_MMZ_ALIGN);
...
static HI_S32 SkpBitStream(VALG_CRCL_BUF_S *pstStrBuf, STRMMNG_STREAM_INFO_S *pstStreamInfo, VeduEfl_NALU_S* pNalu)
{
    HI_U32 u32InputLen = 0;

    if (IsBufVaild(pstStrBuf) != HI_SUCCESS)
    {
        HI_WARN_VENC("===> SkpBitStream Error1!InputLen = %d, WH = %x,WT = %x,RH = %x, RT = %x, Len = %d\n",
            u32InputLen,pstStrBuf->u32WrHead,pstStrBuf->u32WrTail,pstStrBuf->u32RdHead,pstStrBuf->u32RdTail,pstStrBuf->u32BufLen);
        return HI_FAILURE;
    }
    ...
    {
        HI_WARN_VENC("===> SkpBitStream Error2!pNalu->pVirtAddr[0]-64 = %x-64 = %x, RdTail = %x\n",pNalu->pVirtAddr[0],((HI_U8*)pNalu->pVirtAddr[0] - 64),GetBufRdTail(pstStrBuf));
        return HI_FAILURE;
    }
    ...
    {
        HI_WARN_VENC("===> SkpBitStream Error3!InputLen = %d, WH = %x,WT = %x,RH = %x, RT = %x, Len = %d\n",
            u32InputLen,pstStrBuf->u32WrHead,pstStrBuf->u32WrTail,pstStrBuf->u32RdHead,pstStrBuf->u32RdTail,pstStrBuf->u32BufLen);
        return HI_FAILURE;
    }

    HI_DBG_VENC("===> SkpBitStream success!InputLen = %d, WH = %x,WT = %x,RH = %x, RT = %x\n",
        u32InputLen,pstStrBuf->u32WrHead,pstStrBuf->u32WrTail,pstStrBuf->u32RdHead,pstStrBuf->u32RdTail);

    return HI_SUCCESS;
}


static HI_S32 GetBitStream(VALG_CRCL_BUF_S *pstStrBuf, HI_U32 StrmBufExtLen, STRMMNG_STREAM_INFO_S *pstStreamInfo, VeduEfl_NALU_S* pNalu)
{
    ...
    if ((pstStreamInfo->Protocol != VEDU_JPGE) && (pNaluHdr->Type != 1) && (pNaluHdr->Type != 5) && (pNaluHdr->Type != 7) && (pNaluHdr->Type != 8)
        &&(pNaluHdr->Type != 32) && (pNaluHdr->Type != 33) && (pNaluHdr->Type != 34))
    {
    ...
    if (pNalu->SlcLen[1] > 0)
    {
        if (pNalu->SlcLen[1] < StrmBufExtLen)
        {
            memcpy((pNalu->pVirtAddr[0] + pNalu->SlcLen[0]),pNalu->pVirtAddr[1],pNalu->SlcLen[1]);
        }
        else
        {
            HI_ERR_VENC("SlcLen[1] = %d larger than u32StrmBufExtLen(%d) can't copy! The stream maybe lost!\n",pNalu->SlcLen[1], StrmBufExtLen);
            if (SkpBitStream(pstStrBuf, pstStreamInfo, pNalu) != HI_SUCCESS)
            {
                HI_ERR_VENC("Call SkpBitStream failed!\n");
            }
            return HI_FAILURE;
        }
    }

    HI_DBG_VENC("success===>WH = %x, WT = %x, RH = %x, RT = %x, Size = %d\n",pstStrBuf->u32WrHead,pstStrBuf->u32WrTail,pstStrBuf->u32RdHead,pstStrBuf->u32RdTail,pstStrBuf->u32BufLen);
    return HI_SUCCESS;
}
...
HI_VOID RecycleBuf_DeInit(HI_VOID)
{
    ...
    if (s_stRBParam.IsInit)
    {
        ...
        s_stRBParam.IsInit = 0;
    }
}

HI_S32 RecycleBuf_Create(HI_HANDLE hHandle, STRMMNG_INST_CONFIG_S *pstConfig)
{
    HI_S32 Ret = 0;
    RECYCLEBUF_INST_S *pstInst;
    struct list_head* pos;
    ...
    pstInst->u32StrmBufExtLen = pstConfig->u32StrmExtLen;
...
HI_S32 RecycleBuf_PutWriteBuf(HI_HANDLE hHandle, STRMMNG_INPUT_INFO_S *pstInputInfo, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
    ...
    if (pstInst == HI_NULL || pstInputInfo == HI_NULL || pstStreamInfo == HI_NULL)
    {
        return HI_ERR_VENC_INVALID_PARA;
    }
...
HI_S32 RecycleBuf_AcqReadBuffer(HI_HANDLE hHandle, STRMMNG_STREAM_INFO_S *pstStreamInfo, VeduEfl_NALU_S* pNalu)
{
    ...
    if (pstInst == HI_NULL || pstStreamInfo == HI_NULL || pNalu == HI_NULL)
    {
        return HI_ERR_VENC_INVALID_PARA;
    }

    RECYCLEBUF_SPIN_LOCK(pstInst->stSpinLock, ulFlags);
    ret = GetBitStream(&pstInst->stRecycleBuf, pstInst->u32StrmBufExtLen, pstStreamInfo, pNalu);
    RECYCLEBUF_SPIN_UNLOCK(pstInst->stSpinLock, ulFlags);
                                  ...
                                  VENC_STREAM_S* pstStream, VENC_BUF_OFFSET_S* pstBufOffSet)
{
    ...
    if (pstInst == HI_NULL || pstVeduPacket == HI_NULL || pstStream == HI_NULL || pstBufOffSet == HI_NULL)
    {
        return HI_ERR_VENC_INVALID_PARA;
    }
                                  ...
                                  VENC_INFO_ACQUIRE_STREAM_S* pstAcqStrm, VeduEfl_NALU_S* pstVeduPacket)
{
    ...
    if (pstInst == HI_NULL || pstAcqStrm == HI_NULL || pstVeduPacket == HI_NULL)
    {
        return HI_ERR_VENC_INVALID_PARA;
    }
...
HI_S32 RecycleBuf_GetRlsStrmInfoIntar(HI_HANDLE hHandle, VENC_STREAM_S *pstStream, VeduEfl_NALU_S* pstVeduPacket)
{
    ...
    if (pstInst == HI_NULL || pstStream == HI_NULL || pstVeduPacket == HI_NULL)
    {
        return HI_ERR_VENC_INVALID_PARA;
    }
...
HI_S32 RecycleBuf_CheckBufSize(HI_HANDLE hHandle, HI_U32 StrmBufSize, HI_U32 CurStrmBufSize)
{
    ...
    if (FreeLen < D_VENC_ALIGN_DOWN(CheckBufSize, VEDU_MMZ_ALIGN))
    {
        RECYCLEBUF_SPIN_UNLOCK(pstInst->stSpinLock, ulFlags);
...
HI_U32 RecycleBuf_GetBitBufSize(HI_HANDLE hHandle, HI_U32 StreamBufSize, HI_U32 SlcSplitEn, HI_U32 LumaSize, HI_U32 *pExtLen)
{
    HI_U32 BitBufSize = 0;
    ...
    if (SlcSplitEn)
    {
        StrmBufExtLen = D_VENC_ALIGN_UP((LumaSize * 3 / 2 / 8 ), VEDU_MMZ_ALIGN);
    }
    else
    {
        StrmBufExtLen = D_VENC_ALIGN_UP((LumaSize * 5 / 3 ), VEDU_MMZ_ALIGN);
    }
    BitBufSize = StreamBufSize + StrmBufExtLen;
    *pExtLen   = StrmBufExtLen;

    return BitBufSize;
}

HI_S32 RecycleBuf_AllocStreamBuf(HI_HANDLE hHandle, VENC_BUFFER_S *pStreamMMZBuf, STRMMNG_ALLOC_INFO_S *pAllocInfo)
{
    HI_S32 s32Ret = HI_FAILURE;
    HI_U32 AllocBufSize = 0;

    if (pStreamMMZBuf == HI_NULL || pAllocInfo == HI_NULL)
    {
        return HI_FAILURE;
    }

#ifdef VENC_SUPPORT_JPGE
    if (pAllocInfo->Protocol == VEDU_JPGE)
    {
        AllocBufSize = pAllocInfo->BitBufSize;
        s32Ret = HI_DRV_VENC_MMZ_AllocAndMap("VENC_JpegBuf", HI_NULL, AllocBufSize, VEDU_MMZ_ALIGN, pStreamMMZBuf);
...
#endif
    {
        AllocBufSize = pAllocInfo->BitBufSize + pAllocInfo->LumaSize* 3 + pAllocInfo->MeMvSize;

        s32Ret = HI_DRV_VENC_AllocAndMap("VENC_SteamBuf", HI_NULL, AllocBufSize, VEDU_MMZ_ALIGN, pStreamMMZBuf);
...
HI_S32 RecycleBuf_WriteHeader(HI_HANDLE hHandle, STRMMNG_INPUT_INFO_S* pstInputInfo, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
    ...
    if (pstInst == HI_NULL || pstStreamInfo == HI_NULL || pstInputInfo == HI_NULL)
    {
        return HI_ERR_VENC_INVALID_PARA;
    }

    JfifHdrLen = pstInputInfo->SlcSplitEn ? 698 : 698 - 6;

    RECYCLEBUF_SPIN_LOCK(pstInst->stSpinLock, ulFlags);

    pstStrBuf = &pstInst->stRecycleBuf;

    HI_DBG_VENC("prepare to write! pBase = %p,Wp = %x,Rp = %x, WH = %x,WT = %x,RH = %x, RT = %x, bufferLen = %x\n",pstStrBuf->pBase, *(pstStreamInfo->StrmBufWpVirAddr), *(pstStreamInfo->StrmBufRpVirAddr),
        pstStrBuf->u32WrHead,pstStrBuf->u32WrTail,pstStrBuf->u32RdHead,pstStrBuf->u32RdTail,pstStrBuf->u32BufLen);

    memset(&NaluHdr, 0, sizeof(VeduEfl_NaluHdr_S));
    NaluHdr.PacketLen  = 64 + D_VENC_ALIGN_UP(JfifHdrLen , VEDU_MMZ_ALIGN);
    NaluHdr.InvldByte  = D_VENC_ALIGN_UP(JfifHdrLen , VEDU_MMZ_ALIGN) - JfifHdrLen;
    NaluHdr.bLastSlice = 0;
    NaluHdr.PTS0       = pstInputInfo->PTS0;
    NaluHdr.PTS1       = pstInputInfo->PTS1;

    BufWrite(pstStrBuf, &NaluHdr, 64);
    BufWrite(pstStrBuf, pstInputInfo->pJfifHdr , D_VENC_ALIGN_UP(JfifHdrLen, VEDU_MMZ_ALIGN));

    //BufUpdateWp( pstStrBuf );    updata Wp when hardware encode success!

    *(pstStreamInfo->StrmBufRpVirAddr) = pstStrBuf->u32RdTail;
    *(pstStreamInfo->StrmBufWpVirAddr) = pstStrBuf->u32WrHead;
    HI_DBG_VENC("Wp = %x,Rp = %x, WH = %x,WT = %x,RH = %x, RT = %x\n",pstStrBuf->u32WrHead,pstStrBuf->u32RdTail,
        pstStrBuf->u32WrHead,pstStrBuf->u32WrTail,pstStrBuf->u32RdHead,pstStrBuf->u32RdTail);
...
HI_S32 RecycleBuf_JPGERemoveHeader(HI_HANDLE hHandle, STRMMNG_INPUT_INFO_S *pstInputInfo, STRMMNG_STREAM_INFO_S *pstStreamInfo)
{
    ...
    JfifHdrLen = pstInputInfo->SlcSplitEn ? 698 : 698 - 6;
    u32Len     = 64 + D_VENC_ALIGN_UP(JfifHdrLen, VEDU_MMZ_ALIGN);

    RECYCLEBUF_SPIN_LOCK(pstInst->stSpinLock, ulFlags);

    pstStrBuf = &pstInst->stRecycleBuf;

    if (pstStrBuf->u32WrTail == pstStrBuf->u32RdHead)
    {
        HI_WARN_VENC("JPEG header is already read, can't remove the header,u32WrTail = %d,u32RdHead = %d\n",pstStrBuf->u32WrTail,pstStrBuf->u32RdHead);
        pstStrBuf->u32WrHead = pstStrBuf->u32WrTail;
        RECYCLEBUF_SPIN_UNLOCK(pstInst->stSpinLock, ulFlags);
        return HI_FAILURE;
    }
...
HI_S32 RecycleBuf_GetJpgeStream(HI_HANDLE hHandle, STRMMNG_STREAM_INFO_S *pstStreamInfo, VeduEfl_NALU_S *pstVeduPacket, VENC_STREAM_S *pstStream, VENC_BUF_OFFSET_S *pstBufOffSet)
{
    ...
    RECYCLEBUF_ASSERT_RETURN(pstStreamInfo != HI_NULL, HI_ERR_VENC_INVALID_PARA);
    RECYCLEBUF_ASSERT_RETURN(pstVeduPacket != HI_NULL, HI_ERR_VENC_INVALID_PARA);
    RECYCLEBUF_ASSERT_RETURN(pstStream != HI_NULL, HI_ERR_VENC_INVALID_PARA);
    RECYCLEBUF_ASSERT_RETURN(pstBufOffSet != HI_NULL, HI_ERR_VENC_INVALID_PARA);
    ...
    pVirAddr = (HI_VOID*)pstStreamInfo->pstJpgMMZBuf->pu8StartVirAddr;
    StreamBufLen = pstStreamInfo->pstJpgMMZBuf->u32Size;
    ...
    do
    {
        ...
        if (s32Ret == HI_SUCCESS && pstVeduPacket->bFrameEnd == 0)
        {
            RecycleCnt++;
            HI_ERR_VENC("===>while recycle to STRM_AcqReadBuffer success!s32Ret = %d,bFrameEnd = %d, RecycleCnt = %d,\n",s32Ret,pstVeduPacket->bFrameEnd,RecycleCnt);
        }
    }
