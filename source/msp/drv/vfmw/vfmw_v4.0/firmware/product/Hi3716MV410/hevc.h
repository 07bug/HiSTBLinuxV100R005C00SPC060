...
* Date                       Author                       Change
* ====                       ======                       ======
* 2014/03/01          sdk sdk                Original.
* 2014/07/01                sdk                 Modify to vfmw.
*
* Dependencies:
* Linux OS
...
//HEVC syntax performance optimization enable //sdk
...
#define HEVC_MAX_DPB_PIC_BUF          (6)             //²Î¿¼Ð­ÒéÎÄµµA.4
...
#endif
//macros
#if !defined (CFG_HEVC_MAX_CU_DEPTH)              //sdk
...
/*Ð¡ÓÚÕâ¸öÖµÐèÒªÆ´°ü£¬·ñÔò¿ÉÄÜÂëÁ÷½âÎö²»ÍêÕû*/
...
{
    /************** VAHB *************/
    SINT32     total_image_num;       // ×Ü¹²·ÖÅäµÄÍ¼Ïñ¸öÊý
    SINT32     total_pmvblk_num;      // ×Ü¹²·ÖÅäµÄPMV¿é¸öÊý

    UADDR      vahb_phy_addr;         // VAHBÎïÀíµØÖ·£¬1024¶ÔÆë
...
typedef struct
{
    ...
    UINT32 nesting_max_temporal_id_plus1[HEVC_MAX_NESTING_NUM_LAYER];   //sdk  20141028
...
typedef struct
{
    ...
    UINT32       slice_type;
    SINT32       picid_refidx0;     // µ±Ç°sliceÔÚlist0ÖÐµÚÒ»¸öÔªËØ¶ÔÓ¦µÄÖ¡´æ²ÛÎ»ºÅ£¬¸ºÊýÎÞÐ§£¬P slice repair
    SINT32       picid_nearpoc;     // µ±Ç°sliceÔÚdpbÖÐÕÒµ½µÄpoc¾àÀë×î½üµÄÖ¡´æµÄ²ÛÎ»ºÅ£¬¸ºÊýÎÞÐ§£¬I slice repair
...
enum NalUnitType
{
    ...
    NAL_UNIT_EOPIC,                    // NAL_UNIT_UNSPECIFIED_48, ×Ô¶¨ÒåÒ»Ö¡Í¼Ïñ½áÊøÌØÊânalÀàÐÍ
    NAL_UNIT_EOSTREAM,                 // NAL_UNIT_UNSPECIFIED_49, ×Ô¶¨ÒåÂëÁ÷½áÊøÌØÊânalÀàÐÍ
...
typedef struct
{
    ...
    UINT8   bIsInsertEOPICNal;          //ÊÇ·ñÊÇ´úÂë²åÈëeopicnal»¹ÊÇÕæÕýÂëÁ÷Àï´æÔÚnaltype=30µÄnal
    ...
    UINT32  prev_pic_parameter_set_id;  //ÓÃÓÚÅÐ¶ÏÊÇ·ñÊÇÐÂµÄÒ»Ö¡Í¼Ïñ,ÌØ±ðÊÇµÚÒ»¸öslice±»¶ªÆú(»òÕß½â´í)µÄÇé¿öÏÂ

    UINT32  scalingListSize[4];
    UINT32  scalingListSizeX[4];
    UINT32  sigLastScanCG32x32[64];
    UINT32  qmatrix[4][6][16];          // 4 types:4x4 8x8 16x16 32x32, ¶ÔÓÚ4x4£¬16¸öbytesÆ´ÔÚ4wordsÖÐ£¬ÆäÓà64¸öbytesÆ´ÔÚ16wordsÖÐ
    ...
    //Slice中存储的是Slice中pic_parameter_set_id数值，以后需要更换名字
    ...
    UINT32     u32SCDWrapAround; //sdk
    HEVC_NALU_S *pLastNal;//sdk
    /*add by sdk*/
