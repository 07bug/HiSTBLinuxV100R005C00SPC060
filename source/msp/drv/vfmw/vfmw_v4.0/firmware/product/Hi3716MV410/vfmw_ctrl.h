...
typedef struct hiVFMW_CHAN_MEM_S
{
    ...
    UINT32          u32DynamicAllocEn; /*是否为动态帧存方式sdk*/
    VFMW_CHAN_FRM_BUF_S *pstChanFrmBuf;//sdk
} VFMW_CHAN_MEM_S;
//sdk
...
#ifdef Hi3716MV410    
    MEM_RECORD_S stPMVBuffer;
#endif
    SYNTAX_CTX_S   stSynCtx;//此成员必须放在最后一个定义,创建通道时即使mvc support，但是配置的能力集不是mvc相关的，该结构体内存也只分配次大的CTX所需，以达到内存裁剪目的。 sdk
...
SINT32 VCTRL_SetDiscardPicParam(SINT32 ChanID, VDEC_DISPIC_PARAM_S *pArgs);    //add by sdk, 2012.11.20
...
SINT32 VCTRL_SetPMVBufToChan(SINT32 ChanID, PMV_BUF_S *PMVBuf);
#endif
