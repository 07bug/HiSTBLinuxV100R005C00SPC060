...
/***********************************************************************
      VFMW的版本号
 ***********************************************************************/
#define  VFMW_VERSION_NUM       (2018070900)
...
// 20130911: BVT 可能设置的最小SCD SEG BUF 为 32*1024，所以得更新相关判断条件
#ifdef VFMW_BVT_SUPPORT
...
#ifdef Hi3716MV410
/* type=120 上报码流信息分配帧存 */
#define REPORT_ARRANGE_FRAME_BUF(chan_id, stVfmwFrameSizeInfo, Width, Height)                     	\
    do{                                                         \
        if( NULL != g_event_report )                            \
        {                                                       \
            UINT32  para[6];                                    \
            para[0] = stVfmwFrameSizeInfo.u32ImageNum;\
            para[1] = stVfmwFrameSizeInfo.u32ImageSize;\
            para[2] = stVfmwFrameSizeInfo.u32PmvNum;\
            para[3] = stVfmwFrameSizeInfo.u32PmvSize;\
            para[4] = (UINT32)(Width);\
            para[5] = (UINT32)(Height);\
...
#define VFMW_ASSERT_RET(Condition, Ret)                                  \
do                                                                       \
{                                                                        \
    if (!(Condition))                                                    \
    {                                                                    \
