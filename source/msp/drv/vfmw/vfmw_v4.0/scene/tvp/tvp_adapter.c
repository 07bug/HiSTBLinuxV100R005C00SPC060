...
VOID TVP_VDEC_Release_Stream(HI_S32 ChanID)
{
    STREAM_DATA_S *pStrRawPacket;
...
/********清相应通道的callback信息******/
VOID TVP_VDEC_Release_Callback(HI_S32 ChanID)
{
    HI_S32 Tail;
...
HI_S32 TVP_VDEC_Control( HI_S32 ChanID, VDEC_CID_E eCmdID, HI_VOID *pArgs )
{
    ...
    {
        TVP_VDEC_ASSERT((ChanID >=0 && ChanID < TOTAL_MAX_CHAN_NUM), "Chan is not create\n");
    ...
    if (IsSecFlag)
    {
        ...
        switch (eCmdID)
        {
            ...
            case VDEC_CID_GET_CHAN_DETAIL_MEMSIZE_WITH_OPTION:
                ...
                {
                    SecPrint(PRN_FATAL, "pArgs Invalid Param\n");
                    goto Exit_Entry;
                }
                break;
...
HI_S32 TVP_VDEC_TrustDecoderInit(VDEC_OPERATION_S *pArgs)
{
    ...
    TEEC_UUID svc_id = {0x3c2bfc84, 0xc03c, 0x11e6,
