...
static UINT8 VFMW_GetTVPFlag(SINT32 ChanID, VDEC_CID_E eCmdID, VOID *pArgs)
{
    ...
    else
    {
        ...
        else if (VDEC_CID_ALLOC_SEC_BUF == eCmdID || VDEC_CID_FREE_SEC_BUF == eCmdID)
        {
            SecFlag = 1;
        }
    }
...
SINT32 VFMW_GetImage(SINT32 ChanID, IMAGE *pImage)
{
...
#endif
    {
        ...
        if (ret == VDEC_OK)
        {
            vfmw_Osal_Func_Ptr_S.pfun_Osal_SemaDown(G_SAVEYUV_SEM,ChanID);
            YUV_DumpImage(ChanID, pImage);
            vfmw_Osal_Func_Ptr_S.pfun_Osal_SemaUp(G_SAVEYUV_SEM,ChanID);
        }
    }
