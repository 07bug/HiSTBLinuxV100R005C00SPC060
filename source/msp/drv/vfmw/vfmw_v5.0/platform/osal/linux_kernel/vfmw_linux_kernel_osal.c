...
SINT32 OSAL_CreateTask( VOID *pParam, SINT8 TaskName[], VOID *pTaskFunction )
{
    ...
    *pTask = kthread_create(pTaskFunction, (VOID *)NULL, TaskName);
    ...
    scheduler_param.sched_priority = 99;
    sched_setscheduler(*pTask, SCHED_FIFO, &scheduler_param);
...
VOID KernelMunmapMMZ(MEM_RECORD_S *pMemRec)
{
    MMZ_BUFFER_S stMemBuf;
    memset(&stMemBuf, 0, sizeof(MMZ_BUFFER_S));
...
#else
    vfmw_Osal_Func_Ptr_S.pfun_Osal_vfmw_proc_create  = NULL;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_vfmw_proc_destroy = NULL;
#endif
