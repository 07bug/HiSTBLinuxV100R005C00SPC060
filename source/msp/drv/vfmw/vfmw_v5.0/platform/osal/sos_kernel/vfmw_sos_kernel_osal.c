...
UINT8 *SOS_Register_Map(UINT32 PhyAddr, SINT32 Length)
{
...
VOID SOS_Mem_UnmapMMZ(MEM_RECORD_S *pMemRec)
{
    TEE_MMZ_BUFFER_S stMemBuf;

    memset(&stMemBuf, 0, sizeof(TEE_MMZ_BUFFER_S));
    stMemBuf.pu8StartVirAddr = pMemRec->VirAddr;
    stMemBuf.u32StartPhyAddr = pMemRec->PhyAddr;
    DRV_TEE_MMZ_UnmapCpu(&stMemBuf);
}

SINT32 SOS_Mem_MapSmmuFromMMZ(UADDR *pSmmuPhyAddr, UADDR MMZPhyAddr, SINT32 Size)
{
    SINT32 ret;
    TEE_MMZ_BUFFER_S stMmzBuf;
    TEE_SMMU_BUFFER_S stSMMUBuf;
    ...
    *pSmmuPhyAddr = 0;
    ...
    stMmzBuf.u32StartPhyAddr = MMZPhyAddr;
    stMmzBuf.u32Size = Size;

    DRV_TEE_MMZ_MapToSmmu(&stMmzBuf, &stSMMUBuf);
    if (ret != HI_SUCCESS)
    {
        return SOS_ERR;
    }

    *pSmmuPhyAddr = stSMMUBuf.u32StartSmmuAddr;

    return SOS_OK;
}

SINT32 SOS_Mem_UnMapSmmuOfMMZ(UADDR SmmuPhyAddr, UADDR MMZPhyAddr)
{
    SINT32 ret;
    TEE_SMMU_BUFFER_S stSMMUBuf;
    ...
    stSMMUBuf.u32StartSmmuAddr = SmmuPhyAddr;

    ret = DRV_TEE_MMZ_UmapFromSmmu(&stSMMUBuf, MMZPhyAddr);
    if (ret != HI_SUCCESS)
    {
        return SOS_ERR;
    }
...
VOID SOS_Flush_Cache(VOID *ptr, UADDR phy_addr, UINT32 len)
{
#ifdef HI_SMMU_SUPPORT
    DRV_TEE_MEM_FlushCache(ptr, len);
#endif
}

SINT32 SOS_Get_MMzAddrFromSmmu(UADDR *pMMZPhyAddr, UADDR SmmuPhyAddr, SINT32 Size)
{
    TEE_SMMU_BUFFER_S SecSmmuBuf;
    TEE_MMZ_BUFFER_S MMZBuf;
    SINT32 ret;

    memset(&MMZBuf, 0, sizeof(TEE_MMZ_BUFFER_S));
    SecSmmuBuf.u32StartSmmuAddr = SmmuPhyAddr;
    SecSmmuBuf.u32Size = Size;
    ret = DRV_TEE_SMMU_GetMMZAddr(&SecSmmuBuf, &MMZBuf);
    if (ret != HI_SUCCESS)
    {
        return SOS_ERR;
    }
    *pMMZPhyAddr = MMZBuf.u32StartPhyAddr;
    return SOS_OK;
}
...
VOID OSAL_InitVfmwInterface(VOID)
{
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_KernelFlushCache = SOS_Flush_Cache;
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_MapSmmuFromMMZ = SOS_Mem_MapSmmuFromMMZ;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_UnMapSmmuOfMMZ = SOS_Mem_UnMapSmmuOfMMZ;
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_GetMMZAddrFromSmmu = SOS_Get_MMzAddrFromSmmu;
...
static SINT32 SOS_VfmwIoctl(const UINT32 cmd, VOID *pIoctlArgs, size_t size)
{
    ...
    switch (cmd)
    {
        ...
        case TEE_VFMW_IOCTL_WRITE_PROC:
            ...
            break;
        case TEE_VFMW_IOCTL_Parse_CSD:
            res = SEC_VFMW_Parse_CSD(pArgs->arg0, (SINT32)pArgs->arg1);
            break;
        default:
