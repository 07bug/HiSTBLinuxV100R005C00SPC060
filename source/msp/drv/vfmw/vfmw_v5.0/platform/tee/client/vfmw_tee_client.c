...
#define PROC_CMD_SET_PRINT         (0x000)
#define PROC_CMD_SELECT            (0x900)
#define PROC_CMD_SET_SEG_BYPASS    (0x902)
...
static UINT8             g_SecSegBypass         = 0x1;
#ifndef  HI_ADVCA_FUNCTION_RELEASE
...
VOID TVP_VDEC_ConvertCfgToSecure(SINT32 ChanID, VDEC_CHAN_CFG_S *pChanCfg, VDEC_CHAN_CFG_SECURE_S *pSecureCfg)
{
    ...
    pSecureCfg->hVdecHandle = pChanCfg->hVdecHandle;
...
VOID TVP_VDEC_ConvertCfgToNs(VDEC_CHAN_CFG_SECURE_S *pSecureCfg, VDEC_CHAN_CFG_S *pChanCfg)
{
    ...
    pChanCfg->hVdecHandle = pSecureCfg->hVdecHandle;
}
...
VOID TVP_VDEC_ConvertImageHdrInfoToNs(SINT32 ChanID, IMAGE_SECURE *pSecureImage, IMAGE *pImage)
{
    ...
    pImage->stHDRInput.SlHDR1MetadataAvailable = pSecureImage->TechnicolorInfo.IsValid;
...
VOID TVP_VDEC_ConvertImageToNs(SINT32 ChanID, IMAGE_SECURE *pSecureImage, IMAGE *pImage)
{
    ...
    pImage->eVidStd = pSecureImage->eVidStd;
}
...
VDEC_CID_SECURE_E TVP_VDEC_ConvertCmdToSecure(VDEC_CID_E eCmdID)
{
    ...
    switch (eCmdID)
    {
        ...
        case VDEC_CID_REPORT_DEMUX_RELEASE:
            ...
            break;
        case VDEC_CID_ALLOC_SEC_BUF:
            eResultCmd = VDEC_CID_SEC_ALLOC_SEC_BUF;
            break;
        case VDEC_CID_FREE_SEC_BUF:
            eResultCmd = VDEC_CID_SEC_FREE_SEC_BUF;
            break;
        default:
            break;
    }
...
VOID TVP_VDEC_ConvertOptionToSecure(SINT32 ChanID, VDEC_CHAN_OPTION_S *pChanNsOption, VDEC_SECURE_CHAN_OPTION_S *pChanSecOption)
{
    ...
    OSAL_FP_memcpy(&(pChanSecOption->MemDetail), &(pChanNsOption->MemDetail), sizeof(VDEC_SECURE_CHAN_MEM_DETAIL_S));
    ...
    pChanSecOption->s32ScdInputMmuEn = pChanNsOption->s32ScdInputMmuEn;
}
...
VOID TVP_VDEC_ConvertOptionToNS(SINT32 ChanID, VDEC_SECURE_CHAN_OPTION_S *pChanSecOption, VDEC_CHAN_OPTION_S *pChanNsOption)
{
    ...
    OSAL_FP_memcpy(&(pChanNsOption->MemDetail), &(pChanSecOption->MemDetail), sizeof(VDEC_SECURE_CHAN_MEM_DETAIL_S));
    ...
    pChanNsOption->s32ScdInputMmuEn = pChanSecOption->s32ScdInputMmuEn;
}
#endif
    ...
    else
    {
        ...
        else
        {
            switch (*p_option)
            {
                ...
                case PROC_CMD_SET_SEG_BYPASS:
                    g_SecSegBypass = (*p_value == 0)? 0: 1;
                    SecPrint(PRN_ALWS, "Set SecSegBypass = %d\n", g_SecSegBypass);
                    break;
...
#endif
    PROC_PRINT(p, "PrintTime        :%-7d  | SecSegBypass        :%d\n",     g_bPrintTime,    g_SecSegBypass);
    PROC_PRINT(p, "\n");
...
SINT32 TVP_VDEC_TrustDecoderInit(VDEC_OPERATION_S *pArgs)
{
    ...
    stInitArgs.ShareData.seg_bypass_enbale = g_SecSegBypass;
