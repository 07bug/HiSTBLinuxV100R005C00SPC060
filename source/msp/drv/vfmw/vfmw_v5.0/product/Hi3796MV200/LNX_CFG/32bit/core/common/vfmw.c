...
SINT32 VDEC_InitSecureShareData(VDH_SHARE_DATA_S *ShareData)
{
    ...
    VCTRL_WriteProc(0x902, ShareData->seg_bypass_enbale);
...
VOID VDEC_InitGlobalResource(VOID)
{
    ...
    OSAL_FP_sema_init(G_VFMW_SEM, 0xFFFFFFFF);
    for (ChanID = 0; ChanID < MAX_CHAN_NUM; ChanID++)
    {
...
SINT32 VDEC_Init(VDEC_OPERATION_S *pArgs)
{
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_SemaDown(G_VFMW_SEM, 0xFFFFFFFF);
...
OK_EXIT:
    if (g_VfmwInitCount < 0x7FFFFFFF)
    {
        ...
        vfmw_Osal_Func_Ptr_S.pfun_Osal_SemaUp(G_VFMW_SEM, 0xFFFFFFFF);
