...
typedef struct
{
    ...
    VDMDRV_ATTACH_S VdmAttachStr;    //add 描述vdh是否与某个通道强绑定
} VDMDRV_EXT_S;
...
#ifdef VFMW_MODULE_LOWDLY_SUPPORT
    //add  fordsp
    VDMDRV_CHAN_INT_STATE ChanIntState;/*用来表示vdh中断状态的标志，1:同时检测到行号中断和解码完成中断*/
#endif
    VOID *pDecParam;
...
typedef struct
{
	...
	UINT32    ErrFrame_Flag;      //logic decode err flag
...
typedef struct
{
    ...
    UINT32    UVStride;
    UINT32    DecSliceNum;
    UINT32    Error_Flag; //logic decode err flag
