...
#define  VFMW_MODIFICATION_DATE       "2018-07-09-00"
...
#define  DEFAULT_PRINT_ENABLE   (0x0)          // 默认关闭
#define  DEFAULT_PRINT_DEVICE   (DEV_SCREEN)   // 当前仅支持 'DEV_SCREEN'，即打印到屏幕
...
#define REPORT_RECORD_FRAME_BUF_USER(ChanID, PhyAddr, SlotLen, FrameNum)  \
    do{                                                         \
        if(NULL != g_event_report)                              \
        {                                                       \
            UINT32  para[3];                                    \
            para[0] = (UINT32)PhyAddr;                          \
            para[1] = (UINT32)SlotLen;                          \
            para[2] = (UINT32)FrameNum;                         \
            g_event_report(ChanID, EVNT_RECORD_FRAME_BUF_USER, (UINT8*)para, sizeof(para)); \
        }                                                       \
    }while(0)
