...
/*============== LOCAL DEFINITION =============*/
#define VDH_PROC_NAME        "vfmw_distributor"
...
static VOID Set_SecureWaitTime(TASK_ARRAY_S *pTaskArray, TASK_MEM_S **pTaskMem, UINT32 *pSecureWaitTime)
{
    if (pTaskArray->Tail != pTaskArray->Head)
    {
        *pTaskMem = &pTaskArray->Task[pTaskArray->Tail];
        ...
        if (1 == (*pTaskMem)->InUsed)
        {
            ...
            {
                *pSecureWaitTime = OSAL_GetTimeInMs() - (*pTaskMem)->Task.StartTime;
            }
        }
    }
}
...
SINT32 VDH_GetTask_To_Process(MODULE_TYPE_E Type, SINT32 ModuleID, TASK_PARAM_S *pTaskParam)
{
    ...
    Set_SecureWaitTime(pTaskArray, &pTaskMem, &SecureWaitTime);
...
static VOID Scd_Power_Manage(VDH_COMMON_CTX_S *pCtx)
{
    UINT32 i;
...
static VOID Mfde_Power_Manage(VDH_COMMON_CTX_S *pCtx)
{
    UINT32 i;
...
static VOID Bpd_Power_Manage(VDH_COMMON_CTX_S *pCtx)
{
    UINT32 i;
...
VOID VDH_Module_Management(VOID)
{
    ...
    // Scd Power Manage
    Scd_Power_Manage(pCtx);

    // Mfde Power Manage
    Mfde_Power_Manage(pCtx);

    // Bpd Power Manage
    Bpd_Power_Manage(pCtx);
...
static VOID VDH_Proc_AnalysisSeperateParams(UINT8  *buf, UINT8 *str, size_t count, UINT32 *index)
{
    UINT32 j = 0;
    UINT32 i = *index;
...
SINT32 VDH_Write_Proc(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    {
        /* dat1 */
        VDH_Proc_AnalysisSeperateParams(buf, str1, count, &i);

        /* dat2 */
        VDH_Proc_AnalysisSeperateParams(buf, str2, count, &i);
