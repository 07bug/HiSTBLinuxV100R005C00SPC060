...
static VOID PTS_Info_CalFrmTime(SINT32 ChanID, PTS_CHAN_INFO_S *pstPtsInfo, UINT32 u32NumPass, UINT64 u64PtsPass)
{
...
static VOID PTS_Info_HandleValidSrcPTS(SINT32 ChanID, IMAGE *pstImage)
{
    ...
    if (FRD_INVALID_PTS_64 != pstPtsInfo->u64LastSrcPts)
    {
        ...
        if (u32NumPass)
        {
            ...
            {
                PTS_Info_CalFrmTime(ChanID, pstPtsInfo, u32NumPass, u64PtsPass);
            }
            else
            {
...
static SINT32 PTS_Info_GuessFirstFrameRate(SINT32 ChanID, UINT32 u32SrcFrameRate, IMAGE *pstImage)
{
    ...
    OSAL_FP_memset(&stFrmRate, 0x0, sizeof(PTS_FRMRATE_S));
...
static VOID PTS_Info_GetRoundFrameRate(SINT32 ChanID, IMAGE *pstImage)
{
    UINT32 u32RoundRate = 0;
...
static VOID PTS_Info_GetFirstFrameRate(SINT32 ChanID, IMAGE *pstImage, FRD_ALG_INFO_S *pPtsInfo, UINT32 u32FrameRate)
{
    UINT32 u32SrcFrameRate;
    UINT32 u32RoundRateHiplayer = 0;
    UINT32 u32RoundRate = 0;
    PTS_FRMRATE_S stFrmRate;
    SINT32 s32Ret = 0;
    ...
    PTS_Info_GetRoundFrameRate(ChanID, pstImage);
...
static VOID PTS_Info_FrameRateDetect(SINT32 ChanID, IMAGE *pstImage)
{
    ...
    PTS_Info_GetFirstFrameRate(ChanID, pstImage, pPtsInfo, u32FrameRate);
...
static VOID PTS_Info_ProcCommandHanlder(UINT8 pCmd[PTS_CMD_NUM][PTS_CMD_LEN])
{
    ...
    //Parse command
    if (!OSAL_FP_strncmp(pCmd[CmdNum], PTS_CMD_SET_SAMPLE_NUM, OSAL_FP_strlen(PTS_CMD_SET_SAMPLE_NUM)))
    {
