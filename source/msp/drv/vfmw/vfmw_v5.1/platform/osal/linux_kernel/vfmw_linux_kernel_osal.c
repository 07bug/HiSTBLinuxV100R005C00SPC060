...
OSAL_IRQ_SPIN_LOCK g_MemLock;
OSAL_IRQ_SPIN_LOCK g_VoQueLock;
OSAL_IRQ_SPIN_LOCK g_FspLock;
OSAL_IRQ_SPIN_LOCK g_DestroyLock;

/* Mutext */
OSAL_TASK_MUTEX g_IntEvent;
OSAL_TASK_MUTEX g_WaitScdStopWorking;
OSAL_TASK_MUTEX g_PostProcessWaitQueue;
...
SINT32 OSAL_StopTask(VOID *pParam)
{
    OSAL_TASK *pTask = (OSAL_TASK *)pParam;
...
UINT8 *KernelRegisterMap(UADDR PhyAddr, SINT32 Length)
{
    return (UINT8 *)ioremap_nocache(PhyAddr, Length);
}
...
{
    struct file *filp = filp_open(filename, flags | O_LARGEFILE, mode);
...
VOID *OSAL_KmallocVirMem(SINT32 Size)
{
    return HI_KMALLOC(HI_ID_VFMW, Size, GFP_ATOMIC);
}

VOID OSAL_KfreeVirMem(VOID *p)
{
    if (p)
    {
        HI_KFREE(HI_ID_VFMW, p);
    }
}
...
SINT32 KernelMemFree(MEM_DESC_S *pMemDesc)
{
    ...
    stSMMUBuf.u32StartSmmuAddr = (UADDR)pMemDesc->PhyAddr;
    ...
    stMBuf.u32StartPhyAddr  = (UADDR)pMemDesc->PhyAddr;
...
VOID KernelGetPageTableAddr(UADDR *pu32ptaddr, UADDR *pu32err_rdaddr, UADDR *pu32err_wraddr)
{
#ifdef HI_SMMU_SUPPORT
...
OSAL_IRQ_SPIN_LOCK *GetSpinLockByEnum(SpinLockType spinLockType)
{
    ...
    switch (spinLockType)
    {
        ...
        case G_MEM_LOCK:
            pOsalSpinLock = &g_MemLock;
            break;

        case G_VOQUE_LOCK:
            pOsalSpinLock = &g_VoQueLock;
            break;

        case G_FSP_LOCK:
            pOsalSpinLock = &g_FspLock;
            break;

        case G_DESTROY_LOCK:
            pOsalSpinLock = &g_DestroyLock;
            break;
...
SINT32 Osal_InitWaitQue(MutexType mutextType, SINT32 initVal)
{
    ...
    switch (mutextType)
    {
        ...
        case G_VIR_PP_WQ:
            retVal = OSAL_InitEvent(&g_PostProcessWaitQueue, initVal);
            break;
...
SINT32 Osal_WakeupWaitQue(MutexType mutexType)
{
    ...
    switch (mutexType)
    {
        ...
        case G_VIR_PP_WQ:
            retVal = OSAL_GiveEvent(&g_PostProcessWaitQueue);
            break;
...
SINT32 Osal_WaitWaitQue(MutexType mutexType, SINT32 condition, SINT32 waitTimeInMs)
{
    ...
    switch (mutexType)
    {
        ...
        case G_VIR_PP_WQ:
            retVal = OSAL_WaitEvent(&g_PostProcessWaitQueue, waitTimeInMs);
            break;
...
VOID OSAL_InitVfmwInterface(VOID)
{
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_StopTask = OSAL_StopTask;
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_KmallocVirMem = OSAL_KmallocVirMem;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_KfreeVirMem  = OSAL_KfreeVirMem;
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_ListAddTail       = (FN_OSAL_ListAddTail)list_add_tail;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_ListAdd           = (FN_OSAL_ListAdd)list_add;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_ListDel           = (FN_OSAL_ListDel)list_del;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_ListIsEmpty       = (FN_OSAL_ListIsEmpty)list_empty;

    vfmw_Osal_Func_Ptr_S.pfun_Osal_seq_printf = (FN_OSAL_SeqPrintf)seq_printf;
}
