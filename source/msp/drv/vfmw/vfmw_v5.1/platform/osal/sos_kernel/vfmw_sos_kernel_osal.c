...
SOS_IRQ_SPIN_LOCK g_MemLock;
SOS_IRQ_SPIN_LOCK g_VoQueLock;
SOS_IRQ_SPIN_LOCK g_FspLock;
SOS_IRQ_SPIN_LOCK g_DestroyLock;
...
SOS_IRQ_SPIN_LOCK *GetSpinLockByEnum(SpinLockType spinLockType)
{
    ...
    switch (spinLockType)
    {
        ...
        case G_MEM_LOCK:
            pSosSpinLock = &g_MemLock;
            break;

        case G_VOQUE_LOCK:
            pSosSpinLock = &g_VoQueLock;
            break;

        case G_FSP_LOCK:
            pSosSpinLock = &g_FspLock;
            break;

        case G_DESTROY_LOCK:
            pSosSpinLock = &g_DestroyLock;
            break;
...
VBOOL SOS_VFMW_CheckUUID(VOID)
{
    VBOOL isValid = VDEC_FALSE;
    SINT32 s32Ret = VDEC_ERR;
...
VOID SOS_GetPageTableAddr(UADDR *pu32ptaddr, UADDR *pu32err_rdaddr, UADDR *pu32err_wraddr)
{
#ifdef HI_SMMU_SUPPORT
...
VOID OSAL_InitVfmwInterface(VOID)
{
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_strlcpy  = NULL;//(FN_OSAL_strlcpy)strlcpy;;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_strlen   = (FN_OSAL_strlen)strlen;

    vfmw_Osal_Func_Ptr_S.pfun_Osal_msleep = SOS_udelay;
