...
#define CHECK_HANDLE(ViHandle) \
    do { \
        if(((ViHandle >> 16) & 0xff) != HI_ID_VI) \
        { \
            HI_ERR_VI("VI handle(%#x) is invalid\n", (ViHandle)); \
            return HI_ERR_VI_INVALID_PARA; \
        } \
        if (MAX_VI_CHN <= ((ViHandle) & 0xff)) \
        { \
            HI_ERR_VI("VI handle(%#x) is invalid, channel error.\n", (ViHandle)); \
            return HI_ERR_VI_CHN_NOT_EXIST; \
        } \
        if (HI_UNF_VI_BUTT <= (HI_UNF_VI_E)(((ViHandle) & 0xff00) >> 8))\
        { \
            HI_ERR_VI("VI handle(%#x) is invalid, port error.\n", (ViHandle)); \
            return HI_ERR_VI_CHN_NOT_EXIST; \
        } \
        if(g_ViDrv[((ViHandle) & 0xff00) >> 8][(ViHandle) & 0xff].hVi != ViHandle) \
        { \
            HI_ERR_VI("VI handle(%#x) is invalid, not created.\n", (ViHandle)); \
            return HI_ERR_VI_INVALID_PARA; \
        }\
    } while (0)
