...
HI_S32 DispPrepareHDMI( HI_DRV_DISP_INTF_ID_E enHDMIId)
{
    ...
    for ( i = HI_DRV_DISPLAY_0; i < HI_DRV_DISPLAY_2 ; i++)
    {
        ...
        if ((!DispGetHdmiFunction()))
        {
            ...
            DispCheckNullPointer(s_pstHDMIFunc->pfnHdmiDetach);

            s_pstHDMIFunc->pfnHdmiDetach(HI_UNF_HDMI_ID_0);

#if defined (HI_HDMI_SUPPORT_2_0)
            DispCheckNullPointer(s_pstHDMIFunc->pfnHdmiClose);
...
HI_S32 DispPrepareInft(HI_DRV_DISP_INTF_S* pstIntf)
{
    switch (pstIntf->eID)
    {
        ...
        case HI_DRV_DISP_INTF_RGB0:
            (HI_VOID)DispPrepareVDAC(pstIntf->u8VDAC_Y_G);
            (HI_VOID)DispPrepareVDAC(pstIntf->u8VDAC_Pb_B);
            (HI_VOID)DispPrepareVDAC(pstIntf->u8VDAC_Pr_R);
            break;
        ...
        case HI_DRV_DISP_INTF_HDMI2:
            (HI_VOID)DispPrepareHDMI(pstIntf->eID);
            break;
        ...
        case HI_DRV_DISP_INTF_LCD2:
            (HI_VOID)DispPrepareLCD_BT1120(pstIntf->eID);
            break;
...
HI_BOOL DispCheckIntfExist(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_INTF_S* pstIntf)
{
    ...
    HI_DRV_DISP_INTF_S* pstIntf2 = HI_NULL;
    ...
    if ( DispGetInitParam(enDisp, &stInitParam) == HI_SUCCESS)
    {
        ...
        for (enIf = HI_DRV_DISP_INTF_YPBPR0; enIf < HI_DRV_DISP_INTF_ID_MAX; enIf++)
        {
            if (stInitParam.stIntf[enIf].eID != HI_DRV_DISP_INTF_ID_MAX)
            {
                ...
                DISP_PRINT(">>>>>>>>>> intf %d id=%d\n", enIf, stInitParam.stIntf[enIf].eID);
            }
        }
...
HI_VOID DispDeInitDisplay(HI_DRV_DISPLAY_E enDisp)
{
    ...
    if (pstDisp->hCast)
    {
        (HI_VOID)DISP_SetCastEnable(pstDisp->hCast, HI_FALSE);
        (HI_VOID)DISP_DestroyCast(pstDisp->hCast);
    }
...
HI_S32 DispSetColor(DISP_S * pstDisp)
{
    ...
    (HI_VOID)pstDisp->pstIntfOpt->PF_SetChnBgColor(pstDisp->enDisp,
...
HI_S32 DispSetEnable(DISP_S * pstDisp, HI_BOOL bEnable)
{
    ...
    for (u = 0; u < HI_DRV_DISP_INTF_ID_MAX; u++)
    {
        if (pstDisp->stSetting.stIntf[u].bOpen)
        {
            (HI_VOID)pfOpt->PF_SetIntfEnable2(pstDisp->enDisp, &pstDisp->stSetting.stIntf[u], bEnable);
        }
    }
...
HI_S32 DISP_GetVersion(HI_DRV_DISP_VERSION_S * pstVersion)
{
    ...
    // return version
    (HI_VOID)DISP_HAL_GetVersion(pstVersion);
...
HI_S32 DISP_Init(HI_VOID)
{
    ...
    for (eDisp = HI_DRV_DISPLAY_0; eDisp < HI_DRV_DISPLAY_2; eDisp++)
    {
        ...
        if (pstDisp->stSetting.stIntf[HI_DRV_DISP_INTF_HDMI0].bOpen)
        {
            ...
            (HI_VOID)DISP_GetFormat(eDisp, &enStereo, &enEncFmt);
            ...
            (HI_VOID)DispGetFmtTiming(enEncFmt, &stTiming);
        ...
        if ( pstDisp->bEnable ||  pstDisp->bBaseExist )
        {
            ...
            {
                DispCheckNullPointer(s_pstHDMIFunc->pfnHdmiInit);
                DispCheckNullPointer(s_pstHDMIFunc->pfnHdmiOpen);
...
HI_S32 DISP_Suspend(HI_VOID)
{
    ...
    if (DISP_DEVICE_STATE_OPEN == s_s32DisplayGlobalFlag)
    {
        for (enD = HI_DRV_DISPLAY_0; enD < HI_DRV_DISPLAY_BUTT; enD++)
        {
            ...
            {
                (HI_VOID)DISP_SetEnable(enD, HI_FALSE);
            }
        }

        (HI_VOID)DISP_ISR_Suspend();
...
HI_S32 DISP_Resume(HI_VOID)
{
    ...
    if (DISP_DEVICE_STATE_SUSPEND == s_s32DisplayGlobalFlag)
    {
        ...
        for (enD = HI_DRV_DISPLAY_0; enD < HI_DRV_DISPLAY_BUTT; enD++)
        {
            ...
            {
                (HI_VOID)DISP_SetEnable(enD, HI_TRUE);
            }
...
HI_S32 DISP_GetDACAttr(HI_DRV_VDAC_ATTR_S * pDACAttr)
{
    DISP_INTF_OPERATION_S *pfHal = DISP_HAL_GetOperationPtr();
    DispCheckNullPointer(pfHal);
    DispCheckNullPointer(pfHal->PF_GetDACAttr);
...
HI_S32 DISP_Open(HI_DRV_DISPLAY_E enDisp)
{
    ...
    (HI_VOID)DISP_ISR_OpenChn(enDisp);

    // set display info for first time
    (HI_VOID)DISP_ISR_SetDispInfo(enDisp, &pstDisp->stDispInfo);
    ...
    (HI_VOID)DISP_SetLayerZorder(enDisp, HI_DRV_DISP_LAYER_GFX, HI_DRV_DISP_ZORDER_MOVETOP);

#ifndef __DISP_PLATFORM_BOOT__

    (HI_VOID)DRV_CGMS_Init(enDisp, pstDisp->stSetting.enFormat, &(pstDisp->stCgmsInfo));
    (HI_VOID)DRV_MCVN_Init(enDisp, pstDisp->stSetting.enFormat);
...
HI_S32 DISP_Close(HI_DRV_DISPLAY_E enDisp)
{
...
#ifndef __DISP_PLATFORM_BOOT__
    (HI_VOID)DRV_MCVN_DeInit(enDisp);
    (HI_VOID)DRV_CGMS_DeInit(&(pstDisp->stCgmsInfo));
#endif
    ...
    // s4.2 delete isr
    (HI_VOID)DISP_ISR_CloseChn(enDisp);
#endif
...
#ifdef DISP_DEBUG_TEST_SET_FORMAT_TIME
        if (HI_DRV_SYS_GetTimeStampMs(&t0))
        {
            DISP_ERROR("get sys time failed, not init.\n");
            return HI_FAILURE;
        }
                        ...
                        HI_DRV_DISP_CALLBACK_S * pstCB)
{
    ...
    pstCB->eCallBackPrior = HI_DRV_DISP_CALLBACK_PRIORTY_HIGH;
