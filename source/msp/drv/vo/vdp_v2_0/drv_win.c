...
HI_S32 DRV_WIN_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    DISP_DOWN_INTERRUPTIBLE(&g_VoMutex);
    pProcItem = p->private;
    ...
    */
    ...
    if (g_bWinPrivateInformation)
    {
        ...
        PROC_PRINT(p, "%-19s:%-4d                |\n", "Layer Dcmp Cap", pstProcInfo->stWinInfoForDeveloper.bLayerOfWinHasDcmpCapbility);
        PROC_PRINT(p, "%-19s:%-4d                |\n", "Win Expect Cmp", pstProcInfo->stWinInfoForDeveloper.bDcmpExpectbyWin);
        PROC_PRINT(p, "%-19s:%-4d                |\n", "Vpss Frame CmpInfo", pstProcInfo->stWinInfoForDeveloper.bCmpVpssGive);
...
HI_S32 WinProcCmdProcee(HI_HANDLE hWin, HI_CHAR *pArg1,HI_CHAR *pArg2)
{
    ...
    CHECK_NULL_POINTER(pArg1);
    CHECK_NULL_POINTER(pArg2);
...
HI_S32 WIN_AddToProc(HI_HANDLE hWindow)
{
    ...
    //HI_ERR_WIN("WIN_AddToProc win index=0x%x\n", u32Index);
...
HI_S32 WIN_RemFromProc(HI_HANDLE hWindow)
{
    ...
    Ret = WinGetProcIndex(hWindow, &u32Index);
    //HI_ERR_WIN("WIN_RemFromProc win index=0x%x\n", u32Index);
    if (!Ret)
    {
        HI_OSAL_Snprintf(ProcName, 12 ,"win%04x", (HI_U32)(u32Index & WINDOW_INDEX_MASK));
...
HI_S32 WIN_ProcessCmd(unsigned int cmd, HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_WIN_GET_OUT_TYPE_AND_ENGINE:
        {
            WIN_OUT_TYPE_AND_ENGINE_S *pVoWinOutTypeAndEngine;
            pVoWinOutTypeAndEngine = (WIN_OUT_TYPE_AND_ENGINE_S*)arg;
            pVoWinOutTypeAndEngine->enDispOutType = HI_DRV_DISP_TYPE_NORMAL;
            pVoWinOutTypeAndEngine->enXdrEngine = DRV_XDR_ENGINE_SDR;
            Ret = HI_SUCCESS;
            break;
        }
        case CMD_WIN_RESET_FIRST_FRAME_STATUS:
        {
            Ret = HI_SUCCESS;
            break;
        }
...
HI_S32 DRV_WIN_Ioctl(struct inode *inode, struct file  *file, unsigned int cmd, HI_VOID *arg)
{
    ...
    //HI_ERR_WIN("zzzzzzzzzzzzzzz WIN cmd = 0x%x\n", cmd);
    Ret = WIN_ProcessCmd(cmd, arg, pstWinState);
