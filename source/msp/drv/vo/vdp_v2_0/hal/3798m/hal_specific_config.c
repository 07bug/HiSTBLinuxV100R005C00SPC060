...
HI_S32 Chip_Specific_SetWbc3DInfo(DISP_WBC_E eWbc, HI_DISP_DISPLAY_INFO_S *pstDispInfo, HI_RECT_S *in)
{
    ...
    stRect.u32DYL = (HI_U32)in->s32X + in->s32Height;
    stRect.u32DXL = (HI_U32)in->s32Y + in->s32Width;
...
HI_S32 Chip_Specific_WinDoFurtherZmeProcess(HI_U32 u32LayerId, WIN_HAL_PARA_S *pstPara)
{
    ...
    if ((pstPara->bSecondHorZmeEnable) || (pstPara->bSecondVerZmeEnable))
    {
		...
		/*when sr opened, the v0's zme's buffer has a limit up to 1920.
		 *else it can use all the mem ,because sr does not work.
		 * */
		VDP_VID_SetSrZmeMemMode(VDP_LAYER_VID0,HI_FALSE);
    ...
    else
    {
		...
		if (u32LayerId == VDP_LAYER_VID0)
		{
			 ...
			 *else it can use all the mem ,because sr does not work.
			 * */
			VDP_VID_SetSrZmeMemMode(VDP_LAYER_VID0,HI_TRUE);
...
HI_S32 Chip_Specific_SetLayerCapability(VIDEO_LAYER_CAPABILITY_S *pstVideoLayerCap)
{
    ...
    pstVideoLayerCap[eId].bSR      = HI_TRUE;
    ...
    pstVideoLayerCap[eId].stLayerProcInfo.bDci = HI_TRUE;
    pstVideoLayerCap[eId].stLayerProcInfo.bSR  = HI_TRUE;
#endif
...
HI_S32 Chip_Specific_CBM_SetDispZorder(HI_DRV_DISPLAY_E enDisp)
{
    ...
    memset(MixArray,0 ,sizeof(MixArray));
