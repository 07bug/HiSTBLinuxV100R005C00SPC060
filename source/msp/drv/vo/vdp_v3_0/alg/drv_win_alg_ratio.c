...
HI_VOID VDP_ALG_RATIO_CropedAspect(HI_RECT_S *pInWnd,HI_RECT_S *pCropedWnd,HI_U32 *AspectRatioW, HI_U32 *AspectRatioH)
{
    ...
    if (HI_NULL == pInWnd       || HI_NULL == pCropedWnd ||
        HI_NULL == AspectRatioW || HI_NULL == AspectRatioH)
    {
        WIN_ERROR("Pointer is null !\n");
        return;
    }
    ...
    VDP_ALG_RATIO_CorrectAspectRatioW_H(&a_w,&a_h);
...
HI_S32 VDP_ALG_RATIO_RatioProcess(VDP_ALG_RATIO_DRV_PARA_S *pstDrvPara,VDP_ALG_RATIO_OUT_PARA_S *pstOutPara)
{
    ...
    VDP_ALG_RATIO_CorrectAspectRatioW_H(&ar_w, &ar_h);
    ...
    VDP_ALG_RATIO_CorrectAspectRatioW_H(&sr_w, &sr_h);
    ...
    {
        VDP_ALG_RATIO_LetterBox(ar_w, ar_h, &(pstDrvPara->stOutWnd), u32pixr1_out); \
        pstOutPara->u32ZmeW = pstDrvPara->stOutWnd.s32Width;
        pstOutPara->u32ZmeH = pstDrvPara->stOutWnd.s32Height;
    ...
    else //  HI_UNF_VO_ASPECT_CVRS_PAN_SCAN == enCvrsMode  ||  HI_UNF_VO_ASPECT_CVRS_COMBINED == enCvrsMode
    {
        ...
        if (0 == ar_w || 0 == u32pixr1_out || 0 == ar_h)
        {
            WIN_ERROR("Denominator may be zero !\n");
            return HI_FAILURE;
        }
        ...
        pstOutPara->stOutWnd.s32X = pstOutPara->stOutWnd.s32X & VDP_WIDTH_ALIGN;
        pstOutPara->stOutWnd.s32Y = pstOutPara->stOutWnd.s32Y & VDP_HEIGHT_ALIGN;
...
{
    CHECK_NULL_POINTER(pstAspDrvPara);
    CHECK_NULL_POINTER(pstScreen);
    CHECK_NULL_POINTER(pstAspCfg);
