...
HI_S32	DispCastSetFramePTS(HI_DRV_VIDEO_FRAME_S *pstCastFrame,MIRA_GET_PTS_E pts_flag)
{
    ...
    if(!pts_flag) {
        ...
        if(HI_SUCCESS != DISP_OS_GetTime(&pstPrivFrame->u32Pts0))
        {
            return HI_FAILURE;
        }
        ...
        if(HI_SUCCESS != DISP_OS_GetTime(&pstPrivFrame->u32Pts1))
        {
            return HI_FAILURE;
        }
    }
...
HI_S32 DispCastSetFrameInfo(DISP_CAST_S *pstCast, HI_DRV_VIDEO_FRAME_S *pstCurFrame)
{
    ...
    if(HI_SUCCESS != DISP_OS_GetTime(&pstCast->stAttr.stFrameDemo.u32Pts))
    {
        return HI_FAILURE;
    }
...
HI_S32 DISP_Acquire_Snapshot(HI_DRV_DISPLAY_E enDisp, HI_VOID **snapshotHandle, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    *snapshotHandle  = 0;
    ...
    /* set pts. */
    if(HI_SUCCESS != DISP_OS_GetTime(&u32Pts))
    {
        return HI_FAILURE;
    }
