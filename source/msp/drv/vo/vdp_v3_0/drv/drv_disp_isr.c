...
#define  ISR_EXECUTE_TIME 900

HI_S32 HI_DRV_SYS_GetTimeStampUs(HI_U64 *pu32TimeMs)
{
    HI_U64 u64TimeNow;
...
HI_S32 DISP_ISR_Main(HI_S32 irq, HI_VOID *dev_id)
{
	...
	HI_U64 input_time = 0, output_time = 0;
	HI_DRV_SYS_GetTimeStampUs(&input_time);
    ...
    {
		HI_DRV_SYS_GetTimeStampUs(&output_time);
		if ((output_time - input_time) > ISR_EXECUTE_TIME)
		{
			DISP_WARN("Isr use time :%lld us, more than ISR_EXECUTE_TIME!\n", (output_time - input_time));
		}
    ...
    for(i = 1; i >= 0; i--)
    {
        ...
        for (eIntType = HI_DRV_DISP_C_INTPOS_0_PERCENT; eIntType<HI_DRV_DISP_C_TYPE_BUTT; eIntType++)
        {
            ...
            if( s_DispIntTable[enDisp][HI_DRV_DISP_C_INTPOS_0_PERCENT] & u32IntState )
            {
                ...
                (HI_VOID)HI_DRV_SYS_GetTimeStampMs((HI_U32 *)&Ct);
