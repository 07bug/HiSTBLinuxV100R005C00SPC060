...
{
    HI_U32 i = 0;
    HI_DRV_VIDEO_FRAME_S *pstFrame_wbc = HI_NULL;

    memset((HI_VOID *)&stWbcIsogenyProcInfor, 0, sizeof(WIN_WBCISOGENY_PROC_INFOR_S));

    pstFrame_wbc = (HI_DRV_VIDEO_FRAME_S *)DISP_MALLOC(sizeof(HI_DRV_VIDEO_FRAME_S));
    if (!pstFrame_wbc)
    {
        return HI_ERR_VO_MALLOC_FAILED;
    }
    ...
    {
        memset((HI_VOID *)pstFrame_wbc, 0, sizeof(HI_DRV_VIDEO_FRAME_S));

        pstFrame_wbc->bProgressive = HI_TRUE;
        pstFrame_wbc->ePixFormat = HI_DRV_PIX_FMT_NV61_2X1;

        pstFrame_wbc->stBufAddr[0].u32PhyAddr_Y =  stMBuf_WBC.u32StartPhyAddr
                + i * WBC_MAX_STRIDE * WBC_RECT_HEIGHT * 2;
        pstFrame_wbc->stBufAddr[0].u32Stride_Y  =  WBC_MAX_STRIDE;
        pstFrame_wbc->stBufAddr[0].u32PhyAddr_C =  pstFrame_wbc->stBufAddr[0].u32PhyAddr_Y
                + WBC_MAX_STRIDE * WBC_RECT_HEIGHT;

        pstFrame_wbc->stBufAddr[0].u32Stride_C =  WBC_MAX_STRIDE;

        /*insert the frame to  wbc's fullist, notice, i think this is should be full state. */
        if (HI_SUCCESS != WbcInit_Insert_Empty_Node(&stWbcConfigInfor.stWinBP, pstFrame_wbc))
        {
            DISP_FREE(pstFrame_wbc);
            return HI_FAILURE;
        }
    }
...
HI_VOID WinWbc_DestroyBuffer(HI_VOID)
{
    ...
    (HI_VOID)WinBuf_Destroy(&stWbcConfigInfor.stWinBP);
    memset((HI_VOID *)&stWbcConfigInfor, 0, sizeof(HI_DRV_WIN_WBC_Linker_S));
}
...
HI_S32 WinLinker_GetNewNodeToWrite(HI_BOOL bLowDelayMode)
{
    ...
    if (bLowDelayMode)
    {
        s32Ret = DispBuf_GetEmptyNode(&stWbcConfigInfor.stWinBP.stBuffer, &pstNode);
        if (!pstNode || HI_SUCCESS != s32Ret)
        {
...
HI_S32 Win_GetWbcinputSize(VDP_LAYER_CONN_E eWbcConnection,
                           ...
                           HI_RECT_S *pstSdWbcInputRect)
{
    ...
    {
        WIN_FATAL("Para eWbcConnection is invalid !\n");
...
HI_S32 Wbc_ReleaseOneFrame(WB_POOL_S *pstWinBP, DISP_BUF_NODE_S *pstWBNode)
{
    ...
    WIN_CHECK_NULL_RETURN(pstWinBP);
    WIN_CHECK_NULL_RETURN(pstWBNode);
                              ...
                              HI_U32 u32Fidelity)
{
    ...
    WIN_CHECK_NULL_RETURN(pstHdWinInputSize);
    WIN_CHECK_NULL_RETURN(pstHdWinOutputSize);
    ...
    if (HI_NULL == pstDispHd || HI_NULL == pstDispSd)
    {
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != Win_Get_WbcPointAndOutSize(&stHdWinInputSize,
            pstHdWinOutputSize,
    ...
    if (HI_SUCCESS != Win_GetWbcinputSize(eWbcConnection,
                                          &stHdWinInputSize,
    ...
    WinHd_Decide_WbcInputOutput_FieldMode(pstWin,
                                          pstSlvWin,
                                          bWbcInputFrameMode,
                                          pstInfo,
                                          &eWbcFieldMode_Out,
    ...
    Win_SetLowdelayMode_AndSwitch(pstDispHd->u32RefreshRate, pstDispSd->u32RefreshRate);
    bLowdelay = WinSd_Chn_Get_LowdelayMode();

    /*get empty node from wbc's list,  when wbc finished, insert it to sd's buffer.*/
    if (HI_SUCCESS != WinLinker_GetNewNodeToWrite(bLowdelay))
    {
...
HI_S32 Win_ConfigWbc(HI_DRV_WIN_WBC_Linker_S *pstWbcLinker)
{
    ...
    (HI_VOID)stFunction.PF_SetWbcPixFmt(DISP_WBC_Video, HI_DRV_PIX_FMT_NV61_2X1);

    pstFramePrivate = (HI_DRV_VIDEO_PRIVATE_S *)&pstWbcLinker->pstCurrentFrame->u32Priv[0];

    pstWbcLinker->eSrcCS = pstFramePrivate->eColorSpace;
    pstWbcLinker->eDstCS =   HI_DRV_CS_BT601_YUV_LIMITED;
    /*all the size not think about field or frame.*/
    (HI_VOID)stFunction.PF_SetWbcIORect(DISP_WBC_Video,
    ...
    {
        (HI_VOID)stFunction.PF_SetWbc3DInfo(DISP_WBC_Video, &pstWbcLinker->stDispInfo, &pstWbcLinker->stinputSize);
    }
    else
    {
        /*wbc located on v0.*/
        (HI_VOID)stFunction.PF_SetWbc3DInfo(DISP_WBC_Video, &pstWbcLinker->stDispInfo, &pstWbcLinker->stDispInfo.stFmtResolution);
    }

    VDP_WBC_SetConnection(VDP_LAYER_WBC_HD0, pstWbcLinker->eWbcConnection);

    /*configure wbc csc.*/
    (HI_VOID)stFunction.PF_SetWbcColorSpace(DISP_WBC_Video, pstWbcLinker->eSrcCS , pstWbcLinker->eDstCS);
    ...
    (HI_VOID)stFunction.PF_SetWbcAddr(DISP_WBC_Video, &stAddr);
    (HI_VOID)stFunction.PF_SetWbcEnable(DISP_WBC_Video, HI_TRUE);
    (HI_VOID)stFunction.PF_UpdateWbc(DISP_WBC_Video);
    (HI_VOID)stFunction.FP_GetChnBottomFlag(HI_DRV_DISPLAY_1, &bBtm, &u32Vcnt);
...
HI_S32 LinkerBuf_DelUsedNode(HI_U8 *pu8NodePtr)
{
    ...
    if (HI_NULL == u32Partfinish_UsedArray[u32UsedArray_ReadPos])
    {
        WIN_ERROR("u32Partfinish_UsedArray[u32UsedArray_ReadPos] is null.\n");
        return HI_FAILURE;
    }
...
HI_VOID WbcLinker_Proc(WINDOW_S *pstWin)
{
    HI_DRV_VIDEO_FRAME_S *pstFrame = HI_NULL;
...

    {
        ...
        if (stWbcConfigInfor.stWinBP.pstConfig)
        {
            ...
            if (pstWbcFramePrivate->bFrameValid == HI_TRUE)
            {
                ...
                (HI_VOID)WinBuf_PutNewFrame(&pstWin->stBuffer.stWinBP, pstFrame);
                Wbc_CalculateSpeed();
        ...
        HI_BOOL bBtm)
{
    ...
    else if (pstFieldMatch)
    {
        ...
        while (!DispBuf_GetFullNode(&pstWinBP->stBuffer, &pstCurrentNode))
        {
            ...
            (HI_VOID)DispBuf_DelFullNode(&pstWinBP->stBuffer, pstCurrentNode);
...
HI_DRV_VIDEO_FRAME_S *WinSd_Search_FieldMatchFrame(WINDOW_S *pstWin)
{
    ...
    DISP_INTF_OPERATION_S stFunction;
    (HI_VOID)DISP_HAL_GetOperation(&stFunction);
    (HI_VOID)stFunction.FP_GetChnBottomFlag(0, &bBtm, &u32Vcnt);
...
HI_DRV_VIDEO_FRAME_S *WinSd_Search_NewestFrame(WINDOW_S *pstWin)
{
    ...
    {
        (HI_VOID)DispBuf_DelFullNode(&pstWin->stBuffer.stWinBP.stBuffer, pstCurrentNode);
        /* release disp buffer*/
                                    ...
                                    const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    if (!pstWin->bEnable || pstWin->bMasked)
    {
        ...
        (HI_VOID)WinSd_ResetBuffer(pstWin);
