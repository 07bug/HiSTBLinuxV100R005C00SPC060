...
HI_S32 DRV_WIN_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    if (nRet)
    {
        HI_ERR_WIN("down_interruptible failed !\n");
        return HI_FAILURE;
    }
    pProcItem = p->private;
...
HI_S32 WIN_ProcessCmd(unsigned int cmd, HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_WIN_SET_ALPHA:
        {
            ...
            if (HI_SUCCESS == Ret)
            {
                Ret = WIN_SetWindowAlpha(pstWinAlpha->hWindow,pstWinAlpha->enDisp,pstWinAlpha->u32Alpha);
            }
            break;
        }
