...
extern HI_VOID VDP_WBC_SetZmeUse  (VDP_LAYER_WBC_E enLayer, VDP_WBC_ZME_USE_E stZmeUseBy);
HI_S32 SetDefault(HI_U32 eLayer)
{
    ...
    VDP_VID_SetReqCtrl(uHid, 1);
    VDP_VID_SetPreReadEnable(uHid, 1);
    ...
    VDP_WBC_SetConnection(VDP_LAYER_WBC_HD0,VDP_LAYER_CONN_VIDBZME);
    VDP_WBC_SetZmeUse(VDP_LAYER_WBC_HD0,VDP_WBC_ZME_USE_WBC_VP);
    s_stVideoLayer[eLayer].bInitial = HI_TRUE;

    return HI_SUCCESS;
}

extern HI_S32 PF_GetIsogenyMode(ISOGENY_MODE_E *peIsogeny_mode);
HI_S32 SetAllLayerDefault(HI_VOID)
{
    ...
    PF_GetIsogenyMode(&eIsogeny_mode);

    if (eIsogeny_mode != ISOGENY_WBC_MODE)
    {
...
HI_S32 WinHalSetPixFmt_MPW(HI_U32 u32LayerId, WIN_HAL_PARA_S *pstPara)
{
    ...
    else if (eFmt == HI_DRV_PIX_FMT_NV21_TILE)
    {
        VDP_VID_SetInDataFmt(u32LayerId, VDP_VID_IFMT_SP_TILE);
        VDP_VID_SetInDataUVOrder(u32LayerId, 0);
        VDP_VID_SetZmeInFmt_Define1(u32LayerId, VDP_PROC_FMT_SP_420);
    }
    else if (eFmt == HI_DRV_PIX_FMT_NV21_TILE_CMP)
    {
        VDP_VID_SetInDataFmt(u32LayerId, VDP_VID_IFMT_SP_TILE);
        VDP_VID_SetInDataUVOrder(u32LayerId, 0);
        VDP_VID_SetZmeInFmt_Define1(u32LayerId, VDP_PROC_FMT_SP_420);
...
extern  HI_U32 FP_GetChnBottomFlag(HI_DRV_DISPLAY_E enDisp, HI_BOOL *pbBtm, HI_U32 *pu32Vcnt);
...
HI_S32 WinHalSetRect_MPW(HI_U32 u32LayerId, WIN_HAL_PARA_S *pstPara, HI_S32 s32exl)
{
    ...
    HI_BOOL bBtm = 0, bFirlterEnable = HI_FALSE;
    ...
    if (u32LayerId == VDP_LAYER_VID0)
    {
        if (pstPara->bFieldRead)
        {
            VDP_VID_SetReadMode(u32LayerId, VDP_RMODE_INTERFACE,VDP_RMODE_INTERFACE);
        }
        else
        {
            VDP_VID_SetReadMode(u32LayerId, VDP_RMODE_PROGRESSIVE,VDP_RMODE_PROGRESSIVE);
        }
    }
    else if (u32LayerId == VDP_LAYER_VID3)
    {
        VDP_DATA_RMODE_E  eFmode  = 0;

        eFmode  = VDP_RMODE_INTERFACE;
        VDP_VID_SetReadMode(u32LayerId, eFmode,eFmode);
    }
    ...
    stZmeI.bZmeFrmFmtIn  = pstPara->bFieldRead ? 0:1;
    stZmeI.bZmeFrmFmtOut = pstPara->bFieldOut ? 0:1;

    (HI_VOID)DISP_HAL_GetOperation(&stFunction);
    stFunction.FP_GetChnBottomFlag(pstPara->enDisp, &bBtm, &u32Vcnt);

    stZmeI.bZmeBFIn  = pstPara->bFieldRead ? (1 - bBtm):0;
    stZmeI.bZmeBFOut = pstPara->bFieldOut ? (1 - bBtm):0;
...
HI_S32 WinHalGetExtractLineNum(HI_U32 u32LayerId,
                                         HI_U32 u32InHeight,
                                         HI_U32 u32OutHeight,
                                         HI_BOOL bFieldRead,
                                         HI_BOOL bFieldOutput,
                                         HI_U32  u32ZoomInScale)
{
    HI_S32 s32exl = 1;

    u32InHeight = bFieldRead ? u32InHeight/2:u32InHeight;
    u32OutHeight = bFieldOutput ? u32OutHeight/2:u32OutHeight;
    ...
    while((u32InHeight / s32exl)  > (u32OutHeight * u32ZoomInScale * VIDEO_ZOOM_IN_VERTICAL_MAX))
    {
        s32exl = s32exl * 2;
    }
...
HI_S32 WinHalSetFrame_MPW(HI_U32 u32LayerId, WIN_HAL_PARA_S *pstPara)
{
    ...
    //printk("\nhal-%d--%d \n",u32LayerId,pstPara->u32Alpha);

    if ((VDP_LAYER_VID0 == u32LayerId)
        ||(VDP_LAYER_VID1 == u32LayerId))
    {
        VDP_VP_SetLayerGalpha(0,pstPara->u32Alpha);
    }
    else if (VDP_LAYER_VID3 == u32LayerId)
    {
        VDP_VID_SetLayerGalpha(u32LayerId, pstPara->u32Alpha);
    }
    ...
    (HI_VOID)DISP_HAL_GetOperation(&stFunction);
    u32ZoomInScale = stFunction.PF_GetFmtZoomInScale(pstPara->pstDispInfo->eFmt);

    s32exl = WinHalGetExtractLineNum(u32LayerId,
                                     pstPara->stIn.s32Height,
                                     pstPara->stVideo.s32Height,
                                     pstPara->bFieldRead,
                                     pstPara->bFieldOut,
                                     u32ZoomInScale);
...
//HI_PRINT("in id=%d, halid=%d\n", eLayer, eLayerHalId);
    // get mixv setting
    nMaxLayer = Chip_Specific_GetMixvMaxNumvber(eMixId);

//HI_PRINT("eMixId=%d, nMaxLayer=%d\n", eMixId, nMaxLayer);
    if (nMaxLayer <= 1)
    {
        return HI_SUCCESS;
    }
...
HI_S32 VideoLayer_Init(HI_VOID)
{
    ...
    s_stVieoLayerFunc.WinHalGetExtractLineNum  =  WinHalGetExtractLineNum;
