...
{                                \
    if (HI_NULL == (ptr))                    \
    {                            \
        HI_ERR_DISP("WIN Input null pointer in %s!\n", __FUNCTION__); \
        return HI_ERR_VO_NULL_PTR;  \
    }                             \
}

static HI_BOOL CheckWindowOverlap(WindowInfor_S stWindowBase,WindowInfor_S stWindowIn)
{
...
static HI_VOID RegisterWindowOverlap(WindowMappingResult_S *winMappingResult,
...
static HI_S32 judgeWhetherZorderMatch(WindowMappingResult_S *winMappingResult,
                             HI_U32 numofWindow,
...
static HI_VOID CalcWindowOverlapNum(WindowInfor_S *winCoordinate,HI_U32 *calcWindowNum,HI_U32  numofWindow)
{
    HI_U32 u32X;
    HI_U32 u32Y;
...
static HI_VOID GetMaxWindowOverlapPos(HI_U32 *calcWindowNum,HI_U32 numofWindow,HI_U32 *maxOverlapValue,HI_U32 *maxOverlapWindow)
{
    HI_U32 u32X;
...
static HI_U32 GetWindowArea(WindowInfor_S stWindowInfo)
{
    return ((HI_U32)(stWindowInfo.x_end - stWindowInfo.x_start))*((HI_U32)(stWindowInfo.y_end - stWindowInfo.y_start));

}

static HI_VOID GetMaxWindowAreaPos(WindowInfor_S *winCoordinate,HI_U32 numofWindow,HI_U32 *maxAreaWindow)
{
    HI_U32 u32X;
    HI_U32 u32MaxArea;
    HI_U32 u32MaxZorder;    
    ...
    *maxAreaWindow = 0;
    ...
    for (i = 0; i < numofWindow; i++)
    {
        ...
        {
            /* If one of max-area window is on V0, take it as "maxAreaWindow" and return */
            if ( V0 == winCoordinate[i].u32VideoLayer)
            {
                *maxAreaWindow = i;
...
static HI_VOID ClacLayerMapping(WindowInfor_S *winCoordinate,WindowMappingResult_S *winMappingResult,HI_U32 numofWindow,HI_U32 u32VoWinPos);
...
使用及约束；
   ...
   7） 此算法只是做窗口到通道的映射，以及V1上窗口x值由小到大的排序（输出编号）
       不能改变窗口的位置；
*/
WindowMappingResult_S g_winMappingResult_tmp[MAX_ALG_SUPPORT_NUM] = {{0}};
WindowInfor_S g_stWinInfor[MAX_ALG_SUPPORT_NUM] = {{0}};
                  ...
                  HI_U32  numofWindow)
{
    ...
    CHECK_NULL_POINTER(winCoordinate);
    CHECK_NULL_POINTER(winMappingResult);

    memset((void*)g_winMappingResult_tmp, 0, sizeof(WindowMappingResult_S) * MAX_ALG_SUPPORT_NUM);
    if (1 == numofWindow)
    {
    ...
    memset((HI_VOID*)calcWindowNumArr,0,MAX_ALG_SUPPORT_NUM * sizeof(HI_U32));
    ...
    else
    {
        ...
        {
            //判断下两个窗体的order顺序
            memcpy((void*)g_winMappingResult_tmp, (void*)winMappingResult, numofWindow*sizeof(WindowMappingResult_S));
            s32Ret = GetMaxZorderByOverlapWindow(winTmpCoordinate,
                                        numofWindow,
                                        u32FirstMaxOverlapWindow,
                                        &u32VoWinPos,
                                        g_winMappingResult_tmp);
             if (s32Ret)
                goto ALG_EXIT;
                             ...
                             HI_U32  numofWindow)
{
    ...
    CHECK_NULL_POINTER(winZorderInfor);

    if (numofWindow > MAX_ALG_SUPPORT_NUM)
    {
        return HI_FAILURE;
    }
