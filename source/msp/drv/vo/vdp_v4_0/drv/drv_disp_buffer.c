...
HI_S32 BP_SetFrameTunnulAddr(HI_U32* pu32TunnelAddr)
{
    HI_U8* pu8Buffer =  HI_NULL;

    if(HI_NULL == pu32TunnelAddr)
    {
        return HI_FAILURE;
    }
    ...
    pu8Buffer = HI_KMALLOC(HI_ID_VO, DISP_CAST_LOWDELAY_MEM_LENGTH, GFP_ATOMIC);
    ...
    *pu32TunnelAddr = virt_to_phys(pu8Buffer);

    return HI_SUCCESS;
}

HI_S32 BP_CheckCreatePara(HI_U32 u32BufNum, BUF_ALLOC_S* pstAlloc)
{
...
HI_S32 BP_GetBufStride(HI_DRV_PIX_FORMAT_E eDataFormat, HI_U32 u32BufWidth)
{
    HI_U32 BufStride = 0;

    if ((eDataFormat == HI_DRV_PIX_FMT_NV21)
          || (eDataFormat == HI_DRV_PIX_FMT_NV12)
          || (eDataFormat == HI_DRV_PIX_FMT_NV16_2X1)
          || (eDataFormat == HI_DRV_PIX_FMT_NV61_2X1)
       )
    {
        BufStride = (u32BufWidth + 15) & 0xFFFFFFF0ul;
    }
    else if (  (eDataFormat == HI_DRV_PIX_FMT_YUYV)
               || (eDataFormat == HI_DRV_PIX_FMT_YVYU)
               || (eDataFormat == HI_DRV_PIX_FMT_UYVY)
            )
    {
        BufStride = ( (u32BufWidth * 2) + 15) & 0xFFFFFFF0ul;
    }
    else
    {
        BufStride =  (u32BufWidth + 15) & 0xFFFFFFF0ul;
...
HI_S32 BP_Create(HI_U32 u32BufNum, BUF_ALLOC_S* pstAlloc, BUF_POOL_S* pstBP)
{
    ...
    if (HI_FALSE == BP_CheckCreatePara(u32BufNum, pstAlloc))
    {
        return HI_ERR_DISP_INVALID_PARA;
    }
    ...
    if (HI_FALSE == pstAlloc->bFbAllocMem)
    {
    ...
    {
        pstBP->stAlloc.u32BufStride = BP_GetBufStride(pstAlloc->eDataFormat, pstAlloc->u32BufWidth);
    }
    ...
    if (pstBP->enMemType == BUF_MEM_FB_SUPPLY)
    {
        ...
        /*we allocate one more 4k for storing tunnel line number. The addr must be 4k aligned*/
        BufSize = ((BufSize + DISP_CAST_FRAME_BUFFER_4K_ALIGNED - 1) / DISP_CAST_FRAME_BUFFER_4K_ALIGNED)
            * DISP_CAST_FRAME_BUFFER_4K_ALIGNED;

        for (u = 0; u < u32BufNum; u++)
        {
            BufName[8] = (HI_CHAR)('0' + u);

            nRet = DISP_OS_MMZ_Alloc((const char*)BufName,
                VDP_MEM_TYPE_MMZ,
                BufSize,
                DISP_CAST_FRAME_BUFFER_4K_ALIGNED,
                &pstBP->pstBufQueue[u].stMem);

            if (nRet)
            {
                DISP_ERROR(" BP_Create DISP_OS_MMZ_Alloc Get Frame Buffer Fail u =%d\n", u);
                break;
            }
            nRet = BP_SetFrameTunnulAddr(&pstBP->pstBufQueue[u].stFrame.u32TunnelPhyAddr);
            if (nRet)
            {
                DISP_ERROR(" BP_Create Get Tunnul Buffer Fail u =%d\n", u);
                break;
            }
...
HI_S32 BP_Destroy(BUF_POOL_S* pstBP)
{
    ...
    if ((pstBP->enMemType == BUF_MEM_FB_SUPPLY)
        && (HI_NULL != pstBP->pstBufQueue))
    {
            for (u = 0; u < pstBP->u32BufNum; u++)
            {
                if(pstBP->pstBufQueue[u].stMem.u32StartPhyAddr)
                {
                    DISP_OS_MMZ_Release(&pstBP->pstBufQueue[u].stMem);
                    pstBP->pstBufQueue[u].stMem.u32StartPhyAddr = 0;
                }

                if(pstBP->pstBufQueue[u].stFrame.u32TunnelPhyAddr)
                {
                  HI_KFREE(HI_ID_VO, phys_to_virt(pstBP->pstBufQueue[u].stFrame.u32TunnelPhyAddr));
                  pstBP->pstBufQueue[u].stFrame.u32TunnelPhyAddr = 0;
                }
            }
    }
...
HI_S32 BP_ReAllocBuf(BUF_POOL_S* pstBP, HI_U32 u32BufId)
{
    ...
    pstBP->pstBufQueue[index].stMem.u32StartPhyAddr = 0;
    ...
    nRet = DISP_OS_MMZ_Alloc((const char*)BufName,
                             ...
                             BufSize,
                             DISP_CAST_FRAME_BUFFER_4K_ALIGNED,
                             &pstBP->pstBufQueue[index].stMem);

    if (nRet)
    {
       DISP_ERROR(" BP_Create DISP_OS_MMZ_Alloc Get Frame Buffer Fail index =%d\n", index);
       pstBP->enMemType = BUF_MEM_FB_SUPPLY;
        goto _REALLOCONE__ERR_EXIT__;
    }
    ...
    if ( (pstBP->enMemType == BUF_MEM_FB_SUPPLY) || (pstBP->enMemType == BUF_MEM_USER_SUPPLY))
    {
        ...
        pstBP->pstBufQueue[index].stFrame.stBufAddr[0].u32PhyAddr_Y = pstBP->pstBufQueue[index].stMem.u32StartPhyAddr;
        pstBP->pstBufQueue[index].stFrame.stBufAddr[0].u32PhyAddr_C = pstBP->pstBufQueue[index].stMem.u32StartPhyAddr + Coffset;
...
HI_S32 BP_CreateBlackFrame(HI_VOID)
{
    ...
    {
        DISP_INFO("black frame has been created!\n");
        return HI_SUCCESS;
    }

    DISP_MEMSET(&s_stBlackFrame, 0, sizeof(BUF_S));

    nRet = DISP_OS_MMZ_Alloc((const char*)"VDP_BlackFrame", VDP_MEM_TYPE_MMZ, BufSize, VDP_MEM_ALIGN, &s_stBlackFrame.stMem);
    if (nRet)
    {
    ...
    nRet = DISP_OS_MMZ_Map(&s_stBlackFrame.stMem);
    if (HI_SUCCESS != nRet)
    {
        DISP_ERROR("Map black frame memory failed\n");
        DISP_OS_MMZ_Release(&s_stBlackFrame.stMem);
        return nRet;
    }

    pY  = (HI_UCHAR*)(s_stBlackFrame.stMem.pu8StartVirAddr);
    DISP_MEMSET(pY, 0x10, BufSize / 2);
    pUV = (HI_UCHAR*)(s_stBlackFrame.stMem.pu8StartVirAddr + BLACK_FRAME_WIDTH * BLACK_FRAME_HEIGHT);
    DISP_MEMSET(pUV, 0x80, BufSize / 2);

    (HI_VOID)DISP_OS_MMZ_UnMap(&s_stBlackFrame.stMem);
