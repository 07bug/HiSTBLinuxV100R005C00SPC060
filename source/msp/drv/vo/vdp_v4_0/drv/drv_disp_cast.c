...
{
    CHECK_NULL_POINTER(pstCfg);
    CHECK_NULL_POINTER(pstInfo);
    ...
    if (pstCfg->bCrop)
    {
        ...
        if ((pstCfg->stCropRect.u32BottomOffset + pstCfg->stCropRect.u32TopOffset)
                > (pstInfo->stFmtResolution.s32Height - 64)
                || (pstCfg->stCropRect.u32LeftOffset + pstCfg->stCropRect.u32RightOffset)
                > (pstInfo->stFmtResolution.s32Width - 64))
        {
                                    ...
                                    DISP_CAST_ATTR_S *pstAttr)
{
    ...
    pstAttr->bCrop = (HI_S32)pstCfg->bCrop;
    if  (pstAttr->bCrop)
    {
        pstAttr->stCropRect = pstCfg->stCropRect;
    }
...
{
    CHECK_NULL_POINTER(pstCast);
    CHECK_NULL_POINTER(u32BufId);
    CHECK_NULL_POINTER(frameInfo);
                       ...
                       DISP_CAST_S** cast_ptr)
{
    ...
    // set attr
    if (HI_SUCCESS != CastSetFrameDemoPartA(pstCast, pstCfg, &pstCast->stAttr))
    {
...
HI_S32	DispCastSetFramePTS(HI_DRV_VIDEO_FRAME_S *pstCastFrame,MIRA_GET_PTS_E pts_flag)
{
    ...
    if (HI_DRV_SYS_GetTimeStampMs(&u32CurTime))
    {
    	DISP_WARN("get sys time failed, not init.\n");
        return HI_FAILURE;
    }

    pstCastFrame->u32Pts    = u32CurTime;
    pstCastFrame->u32SrcPts = u32CurTime;
                        ...
                        HI_U32 *pu32BufId)
{
    ...
    s32Ret = BP_GetIDFromFrmInfo(&(cast_ptr->stBP),pstCastFrame,pu32BufId);
...
HI_S32 DISP_CastReleaseFrame(DISP_CAST_S *cast_ptr, HI_DRV_VIDEO_FRAME_S *pstCastFrame)
{
    ...
    nRet = Cast_AddNode2ReallocateList(u32BufId, HI_FALSE, bNeedReAllocate);
    if (HI_SUCCESS != nRet)
    {
                             ...
                             HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    pstPriv->eColorSpace = eColorSpace;
                               ...
                               const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    pstCast->stAttr.stIn.s32X = pstCast->stAttr.stCropRect.u32LeftOffset;
    pstCast->stAttr.stIn.s32Y = pstCast->stAttr.stCropRect.u32TopOffset;

    pstCast->stAttr.stIn.s32Width = pstInfo->stDispInfo.stFmtResolution.s32Width
                                    - pstCast->stAttr.stCropRect.u32LeftOffset
                                    - pstCast->stAttr.stCropRect.u32RightOffset;

    pstCast->stAttr.stIn.s32Height = pstInfo->stDispInfo.stFmtResolution.s32Height
                                    - pstCast->stAttr.stCropRect.u32TopOffset
                                    - pstCast->stAttr.stCropRect.u32BottomOffset;



#if defined(CHIP_TYPE_hi3798cv200)
    if ((pstInfo->stDispInfo.eFmt >= HI_DRV_DISP_FMT_PAL  )
        && (pstInfo->stDispInfo.eFmt <= HI_DRV_DISP_FMT_1440x480i_60 ))
    {
        pstCast->stAttr.stIn.s32X = pstCast->stAttr.stIn.s32X*2;
        pstCast->stAttr.stIn.s32Width = pstCast->stAttr.stIn.s32Width*2;
    }
#endif
    ...
    if (HI_SUCCESS != DispSetFrameDemoPartB(cast_ptr, Rate, pstCast->stAttr.eOutColorSpace, &pstCast->stAttr.stFrameDemo))
    {
...
{
    HI_DRV_VIDEO_PRIVATE_S *pstPrivFrame;
    ...
    pstPrivFrame = (HI_DRV_VIDEO_PRIVATE_S *)&(pstCast->stAttr.stFrameDemo.u32Priv[0]);
    pstPrivFrame->u32FrmCnt   = pstCast->u32FrameCnt;
    pstPrivFrame->u32BufferID = pstCast->u32LastCfgBufId;

    if (HI_DRV_SYS_GetTimeStampMs(&pstCast->stAttr.stFrameDemo.u32Pts))
    {
    	DISP_WARN("get sys time failed, not init.\n");
        return HI_FAILURE;
    }
...
HI_S32 DispCastSendTask(DISP_CAST_S *pstCast)
{
    ...
    pstCast->stIntfOpt.PF_SetWbcIORect(pstCast->eWBC, &pstCast->stDispInfo, &pstAttr->stIn, &stOut);

    pstCast->stIntfOpt.PF_SetWbc3DInfo(pstCast->eWBC, &pstCast->stDispInfo, &pstAttr->stIn, &stOut);
    // config csc
    pstCast->stIntfOpt.PF_SetWbcColorSpace(pstCast->eWBC, pstAttr->eInColorSpace, HI_DRV_CS_BT709_YUV_LIMITED);
    ...
    if (pstCast->bLowDelay)
    {
        pU32TunnelVirAddr = (HI_U32*)phys_to_virt(pstAttr->stFrameDemo.u32TunnelPhyAddr);
    ...
    pstCast->stIntfOpt.PF_SetWbcSmmuBypass(pstCast->eWBC, HI_FALSE);
    #else
    pstCast->stIntfOpt.PF_SetWbcSmmuBypass(pstCast->eWBC, HI_TRUE);
    #endif
    // set enable
...
HI_VOID DISP_CastPushFrame(HI_VOID *cast_ptr, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
...
#ifdef CHIP_TYPE_hi3798mv310
    HI_BOOL bInterruptValid = HI_FALSE;
    HI_U32 u32WbcLineNum = 0;

    nRet = DISP_HAL_GetPortIntValidAndWbcLineNum(&bInterruptValid, &u32WbcLineNum);
    if ((HI_SUCCESS != nRet) || (HI_FALSE == bInterruptValid))
    {
...

    {
        ...
        if (HI_SUCCESS != Cast_PushFrameToBackMod(pstCast))
        {
...
HI_VOID DISP_CastCB_GenarateFrame(HI_VOID *cast_ptr, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    {
        pstCast->stIntfOpt.PF_SetWbcEnable(pstCast->eWBC, HI_FALSE);
        pstCast->stIntfOpt.PF_SetWbcLowDlyEnable(DISP_WBC_00, HI_FALSE);
        pstCast->stIntfOpt.PF_UpdateWbc(pstCast->eWBC);
        return;
    }
    ...
    if (pstInfo->eEventType == HI_DRV_DISP_C_VT_INT)
    {
        ...
        (HI_VOID)Cast_ReturnFrameToList(pstCast);

        pstCast->u32TaskCount++;

        if (0 == pstCast->u32Periods || 0 == pstCast->stDispInfo.u32RefreshRate)
        {
        ...
        (HI_VOID)DispCastSetFrameInfo(pstCast, &stCurFrame);

        (HI_VOID)DispCastSetFramePTS(&pstCast->stAttr.stFrameDemo, MIRA_SET_CREATE_PTS);

        pstCast->stAttr.stFrameDemo.u32Pts += 100000 / pstCast->stDispInfo.u32RefreshRate;
        pstCast->stAttr.stFrameDemo.u32SrcPts = pstCast->stAttr.stFrameDemo.u32Pts;

        (HI_VOID)DispCastSendTask(pstCast);
...
HI_S32 DISP_Acquire_Snapshot(HI_DRV_DISPLAY_E enDisp, HI_VOID **snapshotHandle, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    HI_BOOL  bBufAlloc = 0;
    HI_RECT_S stOutputSize = {0};
    HI_U32  u32PixelRepeatTimes = 0;
    ...
    *snapshotHandle  = 0;
    ...
    stOutputSize = stInfo.stFmtResolution;

    if (stOutputSize.s32Width > VDP_DHD0_WBC_WIDTH_MAX)
    {
        stOutputSize.s32Width = VDP_DHD0_WBC_WIDTH_MAX;
    }

    if (stOutputSize.s32Height > VDP_DHD0_WBC_HEIGHT_MAX)
    {
        stOutputSize.s32Height = VDP_DHD0_WBC_HEIGHT_MAX;
    }

    stAlloc.bFbAllocMem = HI_TRUE;
    stAlloc.eDataFormat = HI_DRV_PIX_FMT_NV21;
    stAlloc.u32BufHeight = stOutputSize.s32Height;
    stAlloc.u32BufWidth =  stOutputSize.s32Width;
    ...
    pstVideoFrame->u32Width  = stOutputSize.s32Width;
    pstVideoFrame->u32Height = stOutputSize.s32Height;
    ...
    pstVideoFrame->stDispRect = stOutputSize;
    ...
    (HI_VOID)stFunc.PF_GetPixelRepeatTimesAccordingFmt(HI_DRV_DISPLAY_1,
                                              stInfo.eFmt,
                                              &u32PixelRepeatTimes);

    stInfo.stPixelFmtResolution.s32Width = stInfo.stPixelFmtResolution.s32Width * u32PixelRepeatTimes;
    stInfo.stPixelFmtResolution.s32X = stInfo.stPixelFmtResolution.s32X * u32PixelRepeatTimes;

    stFunc.PF_SetWbcIORect(eWBC, &stInfo, &stInfo.stPixelFmtResolution, &stOutputSize);

    stFunc.PF_SetWbc3DInfo(eWBC, &stInfo, &stInfo.stPixelFmtResolution, &stOutputSize);

    // config csc
    stFunc.PF_SetWbcColorSpace(eWBC, stInfo.eColorSpace, HI_DRV_CS_BT709_YUV_LIMITED);

    // config addr
    stFunc.PF_SetWbcAddr(eWBC, &(pstVideoFrame->stBufAddr[0]));

#ifdef	CFG_VDP_MMU_SUPPORT
    stFunc.PF_SetWbcSmmuBypass(eWBC, HI_FALSE);
#else
    stFunc.PF_SetWbcSmmuBypass(eWBC, HI_TRUE);
#endif
    ...
    /* set pts. */
    if (HI_DRV_SYS_GetTimeStampMs(&u32Pts))
    {
...
HI_S32 DISP_Release_Snapshot(HI_DRV_DISPLAY_E enDisp, HI_VOID* snapshotHandle, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    CHECK_NULL_POINTER(snapshotHandle);
