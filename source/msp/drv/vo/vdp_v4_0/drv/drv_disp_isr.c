...
HI_S32 Disp_ISR_CheckDelayTimeMS(HI_DRV_DISPLAY_E enDisp, HI_U32 u32ISRDelayTimeMS)
{
    DISP_ISR_CHN_S *pstChn;

    pstChn = &s_DispISRMngr.stDispChn[enDisp];

    pstChn->stISRDebug.u32ISRDelayTimeMS = u32ISRDelayTimeMS;

    DISP_WARN("set disp%d delay time %dms\n",enDisp,u32ISRDelayTimeMS);
    return HI_SUCCESS;
...
static HI_BOOL CheckLowbandFlag(HI_VOID)
{
    ...
    if (HI_NULL == pfOpt->PF_GetUnmaskedIntState)
    {
        return HI_FALSE;
    }
...
HI_S32 DISP_ISR_Init(HI_VOID)
{
    ...
    Disp_ISR_CheckDelayTimeMS(HI_DRV_DISPLAY_0 ,DEF_DISP_ISR_DELAY_TIME_MS );
    Disp_ISR_CheckDelayTimeMS(HI_DRV_DISPLAY_1 ,DEF_DISP_ISR_DELAY_TIME_MS );
...
{
    if ( (s_DispISRMngrInitFlag < 0) || (enDisp >= HI_DRV_DISPLAY_2))
    {
        return HI_FAILURE;
    }
...
static HI_S32 DISP_ISR_SearchNode(DISP_ISR_CHN_S *pstChn, HI_DRV_DISP_CALLBACK_TYPE_E eType,
                            ...
                            HI_DRV_DISP_CALLBACK_S *pstCB)
{
    ...
    DispCheckNullPointer(pstCB->pfDISP_Callback);
    ...
    {
        DISP_ERROR("Para eType invalid !\n");
        return HI_FAILURE;
    }

    if (pstCB->eCallBackPrior >= HI_DRV_DISP_CALLBACK_PRIORTY_BUTT)
    {
        DISP_ERROR("Para pstCB invalid:%x,%x,%x!\n",  pstCB, pstCB->pfDISP_Callback, pstCB->eCallBackPrior);
        return HI_FAILURE;
    }
    ...
    s32Ret = DISP_ISR_SearchNullNode(pstChn, eType);
    if (s32Ret < 0)
    {
                              ...
                              HI_DRV_DISP_CALLBACK_S *pstCB)
{
    ...
    s32Ret = DISP_ISR_SearchNode(pstChn, eType, pstCB);
    if (s32Ret < 0)
    {
...
HI_S32 DISP_ISR_SetEvent(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_CALLBACK_EVENT_E eEvent)
{
    ...
    if (eEvent >= HI_DRV_DISP_C_EVENT_BUTT)
    {
        DISP_ERROR("Para eEvent invalid !\n");
        return HI_FAILURE;
    }
...
HI_S32 DISP_ISR_SetDispInfo(HI_DRV_DISPLAY_E enDisp, HI_DISP_DISPLAY_INFO_S *pstDispInfo)
{
    ...
    DispCheckID(enDisp);
    DispCheckNullPointer(pstDispInfo);
    ...
    for (i = (HI_S32) HI_DRV_DISPLAY_1; i >= 0; i--)
    {
        ...
        for (eIntType = HI_DRV_DISP_C_INTPOS_0_PERCENT; eIntType < HI_DRV_DISP_C_TYPE_BUTT; eIntType++)
        {
            ...
            if ( s_DispIntTable[enDisp][HI_DRV_DISP_C_INTPOS_0_PERCENT] & u32IntState )
            {
                ...
                HI_U32 u32FinishSysTime;
                HI_U32 u32PeriodTime;
                HI_U32 u32CircleTime;
                HI_U32 u32LeftTime;

                HI_DRV_SYS_GetTimeStampMs((HI_U32 *)&u32StartSysTime);

                // get top and bottom flag
                pfOpt->FP_GetChnBottomFlag(enDisp, &bBtm, &vcnt);
                pfOpt->PF_GetCurrentVbiTimeInfor(enDisp, pstDisp->stCBInfo.stDispInfo.eFmt, &u32CircleTime, &u32LeftTime);

                pstDisp->stCBInfo.stDispInfo.bIsBottomField = bBtm;
                pstDisp->stCBInfo.stDispInfo.u32Vline = vcnt;
                u32PeriodTime = u32StartSysTime - pstDisp->stCBInfo.stDispInfo.u32LastSysTime  ;
                pstDisp->stCBInfo.stDispInfo.u32LastSysTime = u32StartSysTime;

                DISP_ISR_Execute_ByPriorty(pstDisp, eIntType);

                HI_DRV_SYS_GetTimeStampMs((HI_U32 *)&u32FinishSysTime);

                if ((HI_DRV_DISPLAY_1 == enDisp) && (u32PeriodTime > (u32CircleTime + pstDisp->stISRDebug.u32ISRDelayTimeMS)))
                {
                    DISP_WARN("use-%d,set-%d-isr delay:%dms,line-(%d,%d)\n", (u32FinishSysTime - u32StartSysTime), pstDisp->stISRDebug.u32ISRDelayTimeMS, (u32PeriodTime - u32CircleTime), pstDisp->stCBInfo.stDispInfo.u32LastVline, pstDisp->stCBInfo.stDispInfo.u32Vline);
                }

                pstDisp->stCBInfo.stDispInfo.u32LastVline = pstDisp->stCBInfo.stDispInfo.u32Vline ;
