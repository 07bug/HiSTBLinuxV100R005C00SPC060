...
typedef struct tagDISP_SETTING_S
{
    ...
    HI_DRV_COLOR_SPACE_E enFmtColorSpace;   /* follow to fmt color space. */
    HI_DRV_COLOR_SPACE_E enOutputColorSpace; /* actual output color space in every interrupt. */
    HI_DRV_DISP_COLOR_SPACE_E enUserSetColorSpace; /* user set color space mode. */
...
typedef struct tagDISP_DEBUG_S
{
    HI_BOOL bStopSendHDMIInfo;
}DISP_DEBUG_S;
...
typedef struct tagDISP_S
{
    ...
    /***********HDR proc info************/
    DRV_DISP_DOLBY_PLAY_INFO_S  stDolbyInfo;

    DISP_XDR_PLAY_INFO_S    stXdrPlayInfo;
    DISP_XDR_PLAY_INFO_S    stXdrPrePlayInfo;
    DISP_XDR_PROC_INFO_S    stXdrProcInfo;
    DISP_DEBUG_S    stDispDebug;
#endif
}DISP_S;
...
typedef struct tagDISP_COMMON_CONTROL_S
{
