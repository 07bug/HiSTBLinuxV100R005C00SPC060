...
HI_S32 VIR_BUFFER_GetFrm(VIR_BUFFER_S *pstBuffer,HI_DRV_VIDEO_FRAME_S *pstFrm)
{
    ...
    switch (enType)
    {
        ...
        case MUTUAL_TYPE_SINKACTIVE:
            ...
            if ((u32Tail != u32Head) && (u32Head < DEF_VIR_BUFFER_LENGTH))
            {
...
HI_S32 WIN_VIR_SendAttrToSource(VIRTUAL_S *pstVirWin)
{
    ...
    if (pstVirWin->stSrcInfo.pfSendWinInfo != HI_NULL)
    {
        ...
        if ((pstVirWin->stAttrBuf.stOutRect.s32Height == 0 && pstVirWin->stAttrBuf.stOutRect.s32Width == 0)
                || pstVirWin->stAttrBuf.stOutRect.s32Width > 1920
                || pstVirWin->stAttrBuf.stOutRect.s32Height > 1088)
        {
        ...
        {
            stWinPriv.stOutRect.s32Height = pstVirWin->stAttrBuf.stOutRect.s32Height;
            stWinPriv.stOutRect.s32Width = pstVirWin->stAttrBuf.stOutRect.s32Width;
        }
        ...
        stWinPriv.enARCvrs = HI_DRV_ASP_RAT_MODE_FULL; //virtual win fix to ASP-FULL
        ...
        WinCheckNullPointer(pstVirWin->stSrcInfo.pfSendWinInfo);
...
HI_S32 WIN_VIR_SetSize(VIRTUAL_S *pstVirWin,HI_U32 u32Width,HI_U32 u32Height)
{
    ...
    pstVirWin->stAttrBuf.stOutRect.s32Width = pstVirWin->u32Width;
    pstVirWin->stAttrBuf.stOutRect.s32Height = pstVirWin->u32Height;
    WIN_VIR_SendAttrToSource(pstVirWin);
