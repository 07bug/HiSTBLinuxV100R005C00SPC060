...
static WIN_SHARE_INFO_S s_stWinShareInfo = {0};
static WIN_INFO_S       s_stMaxWinInfo = {0}; /* record max win info. */
...
HI_VOID WIN_SHARE_ResetShareInfo(HI_VOID)
{
    ...
    memset(&s_stWinShareInfo,0x0,sizeof(s_stWinShareInfo));
    for (u32Index = 0; u32Index < WINDOW_MAX_NUMBER; u32Index++)
    {
        s_stWinShareInfo.stWinInfo[u32Index].hWinHandle = HI_INVALID_HANDLE;
    }
...
HI_VOID WIN_SHARE_SetWinInfo(WIN_INFO_S *pstWinInfo)
{
    ...
    s_stWinShareInfo.stWinInfo[u32WinIndex] = *pstWinInfo;
...
{
    WIN_SHARE_INNER_SetValidWinNum(&s_stWinShareInfo);
    *pstWinShareInfo = s_stWinShareInfo;

    return;
}
HI_VOID WIN_SHARE_GetMaxEnableWinIndex(WIN_SHARE_INFO_S *pstWinShareInfo, HI_U32 *pu32MaxWinIndex)
{
    ...
    for (u32Index = 0; u32Index < WINDOW_MAX_NUMBER; u32Index++)
    {
            ...
            && (pstWinShareInfo->stWinInfo[u32Index].bWinEnable))
        {
            u64Size = ((HI_U64)(pstWinShareInfo->stWinInfo[u32Index].stDisp.s32Width)) *
                      ((HI_U64)(pstWinShareInfo->stWinInfo[u32Index].stDisp.s32Height));
            ...
            if (u64MaxSize == u64Size)
            {
                if (VDP_LAYER_VID0 == pstWinShareInfo->stWinInfo[u32Index].u32LayerId)
                {
                     u32MaxWinIndex = u32Index;
                }
            }
...
HI_VOID WIN_SHARE_SetMaxWinInfo(WIN_INFO_S *pstMaxWinInfo)
{
    s_stMaxWinInfo = *pstMaxWinInfo;

    return;
}

HI_VOID WIN_SHARE_ResetMaxWinInfo(HI_VOID)
{
    memset(&s_stMaxWinInfo, 0x0, sizeof(s_stMaxWinInfo));

    s_stMaxWinInfo.hWinHandle = HI_INVALID_HANDLE;

    return;
}

HI_VOID WIN_SHARE_GetMaxWinHandle(HI_HANDLE *pHandle)
{
    *pHandle = s_stMaxWinInfo.hWinHandle;

    return;
}

HI_VOID WIN_SHARE_GetMaxWinOutType(HI_DRV_DISP_OUT_TYPE_E *penOutType)
{
    *penOutType = s_stMaxWinInfo.enOutType;

    return;
}

HI_VOID WIN_SHARE_GetMaxWinOutColorSpace(HI_DRV_COLOR_SPACE_E *penOutColorSpace)
{

    *penOutColorSpace = s_stMaxWinInfo.enOutCsc;
...
HI_VOID WIN_SHARE_GetFrameColorSpace(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_DRV_COLOR_SPACE_E *penColorSpace)
{
    ...
    if (HI_DRV_PIX_FMT_RGB24==pstFrame->ePixFormat || HI_DRV_PIX_FMT_ARGB8888==pstFrame->ePixFormat)
    {
        *penColorSpace = HI_DRV_CS_BT709_RGB_FULL;
    }
...
HI_S32  WIN_SHARE_GetXdrLibStatus(WIN_SHARE_XDR_LIB_STATUS_S *pstXdrLibStatus)
{
    if (HI_NULL == pstXdrLibStatus)
    {
        HI_ERR_WIN("pstXdrLibStatus is NULL\n");
        return HI_ERR_VO_NULL_PTR;
    }

    pstXdrLibStatus->bExistDolbyLib = WIN_GetDolbyLibStatus();
    pstXdrLibStatus->bExistHdr10Lib = HI_TRUE;
    pstXdrLibStatus->bExistHlgLib   = HI_TRUE;
    pstXdrLibStatus->bExistSlfLib   = HI_TRUE;
    pstXdrLibStatus->bExistJTPLib   = HI_TRUE;
