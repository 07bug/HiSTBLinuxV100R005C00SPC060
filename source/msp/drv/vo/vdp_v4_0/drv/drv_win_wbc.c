...
HI_BOOL  ISOGENY_GetHDWinEnable(HI_VOID)
{
    ...
    if ((HI_TRUE == stWnHDFrameInfor.bV0Mute) &&
        ...
        (HI_FALSE == stWnHDFrameInfor.bV1RegionEnable))
    {
        return HI_FALSE;
    }
                              ...
                              HI_RECT_S* pstSdOutRect)
{
    ...
    stSdOutputRectSize.s32X += stSdDispInfo.stOffsetInfo.u32Left;
    stSdOutputRectSize.s32Width = stSdOutputRectSize.s32Width - ( stSdDispInfo.stOffsetInfo.u32Left +  stSdDispInfo.stOffsetInfo.u32Right);
    stSdOutputRectSize.s32Y += stSdDispInfo.stOffsetInfo.u32Top;
    stSdOutputRectSize.s32Height = stSdOutputRectSize.s32Height - ( stSdDispInfo.stOffsetInfo.u32Top +  stSdDispInfo.stOffsetInfo.u32Bottom);
...
static HI_VOID WinTransferSdCfgInfoToWinInfo(WIN_HAL_PARA_S *pstWinPara,
                                                      WIN_INFO_S     *pstWinInfo)
{
    ...
    pstWinInfo->bWinEnable    = pstWinPara->bRegionEnable;
    ...
    pstWinInfo->enOutCsc = pstWinPara->eOutPutColorSpace;
...
HI_VOID ISR_SlaveWinConfigFrame( HI_DRV_VIDEO_FRAME_S* pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S* pstInfo)
{
    ...
    memset(&stWinInfo, 0x0, sizeof(WIN_INFO_S));
    ...
    WinTransferSdCfgInfoToWinInfo(&stLayerPara, &stWinInfo);

    (HI_VOID)pstFunc->PF_SetSdLayerCfg(VDP_LAYER_VID3, &stWinInfo);
...
HI_VOID ISOGBNY_ConfigWbcConvert(WIN_WBC_CONFIG_S* pWBCConfig, ISOGBNY_WBC_CFG_S* pstWBC0Cfg)
{
    ...
    memcpy(&pWBCConfig->stSdWinOutputSize, &pstWBC0Cfg->stSdWinOutputSize, sizeof(HI_RECT_S));
...
HI_S32 ISOGBNY_ConfigWbc0(ISOGBNY_WBC_CFG_S* pstWBC0Cfg, HI_DRV_VIDEO_FRAME_S* pstCfgFrame)
{
    ...
    if ((pstWBC0Cfg->bWbcEnable == HI_FALSE)  || (HI_NULL == pstCfgFrame))
    {
        ...
        ISOGBNY_ConfigWbcConvert(&st_tmpWbcConfig, pstWBC0Cfg);
    ...
    if (HI_SUCCESS != pstFunc->PF_SetWbcParam((WIN_WBC_CONFIG_S*)pstWBC0Cfg))
    {
        WIN_ERROR("Call PF_SetWbcParam failed!\n");
        return HI_FAILURE;
    }
...
HI_VOID ISR_CallbackIsogenyProcess(HI_VOID* hDst, const HI_DRV_DISP_CALLBACK_INFO_S* pstInfo)
{
    ...
    if ( HI_FALSE == ISOGENY_GetHDWinEnable())
    {
        ...
        stIsogenyInfo.stSlaveCfg.u32RecCount = 0;
        goto _ISOGENY_COLSED_EXIT;
    }
    ...
    if (sizeof(HI_DRV_VIDEO_FRAME_S) > (DISP_BUF_DATA_SIZE * sizeof(HI_U32)))
    {
        WIN_ERROR("Buffer too small to copy!\n");
        goto _ISOGENY_EXIT;
    }
...
_ISOGENY_COLSED_EXIT:
    ...
    (HI_VOID)ISOGBNY_ConfigWbc0(&stIsogenyInfo.stWBC0Cfg, HI_NULL);
    ISOGENY_CleanHDWinFrameInfo();
...
HI_VOID WIN_GetIsogenyPocInfor(ISOGBNY_PROC_INFO_S* pstIsogenyProcInfo)
{
    ...
    pstIsogenyProcInfo->bIsogbnyInit = stIsogenyInfo.bIsogenyIint;
...
HI_S32 ISOGENY_ProcRead(struct seq_file* p, HI_VOID* v)
{
    ...
    if (HI_NULL != stIsogenyProcInfo.stSlaveProcInfo.pstSalveCurrentNode)
    {
        ...
        if (HI_NULL != stIsogenyProcInfo.stSlaveProcInfo.pstSalveCfgFrame)
        {
...
HI_S32 ISOGBNY_Init(HI_VOID)
{
    ...
    stIsogenyInfo.bCloseIsogeny = HI_FALSE;
    stIsogenyInfo.bIsogenyIint = HI_TRUE;
...
HI_S32 ISOGBNY_DeInit(HI_VOID)
{
    ...
    stIsogenyInfo.bIsogenyIint = HI_FALSE;
    stIsogenyInfo.bCloseIsogeny = HI_TRUE;
    stIsogenyInfo.enWBCForcePoint = WBC_WB_POINT_AUTO;
    stIsogenyInfo.enWBForceField = WBC_WB_FIELD_AUTO;
