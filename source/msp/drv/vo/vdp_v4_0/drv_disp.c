...
HI_U8 *g_pDispXdrEngineTypeString[DRV_XDR_ENGINE_BUTT] = {
    "AUTO",
    "SDR",
    "HDR10",
    "HLG",
...
static HI_S32 DISPProcessCmdSetXdrEngineType(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser);
static HI_S32 DISPProcessCmdGetXdrEngineType(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser);
...
static const stDISPProcessCmdFunc s_aDISPCmdFuncTab[] =
{
    ...
    {CMD_DISP_SET_XDR_ENGINE, DISPProcessCmdSetXdrEngineType},
    {CMD_DISP_GET_XDR_ENGINE, DISPProcessCmdGetXdrEngineType},
        ...
        if (g_pDispProcLevel >= DISP_PROC_EXTREN_XDR_INFO)
        {
            ...
            for (u32Index = 0; u32Index < WINDOW_MAX_NUMBER; u32Index++)
            {
                ...
                if (WIN_SHARE_HandleValid(pstWinInfo->hWinHandle))
                {
                    ...
                    PROC_PRINT(p, "%-20s:%s\n", "bRegionMute", g_pDispYNString[pstWinInfo->bRegionMute]);
            ...
            if ((XDR_WORK_MODE_DOLBY == s_DispAttr.stXdrProcInfo.stXdrPlayInfo.enXdrWorkMode))
            {
...
*/
    ...
    HI_DRV_PROC_EchoHelper("echo engine  sdr/hdr10/dolby/hlg     > /proc/msp/dispX\n");
    HI_DRV_PROC_EchoHelper("------------------------------------------------------\n");
    HI_DRV_PROC_EchoHelper("echo output sdr/cert_sdr/dolby/hdr10/cert_hdr10/hlg/auto/follow_first_frm > /proc/msp/disp1\n");
}
...
HI_S32 DispProcSetOutputType(HI_DRV_DISPLAY_E enDisp, HI_CHAR *pArg2)
{
    ...
    else if (0 == HI_OSAL_Strncmp(pArg2, "follow_first_frm", strlen("follow_first_frm")))
    {
        nRet = DISP_SetDispOutput(enDisp, HI_DRV_DISP_TYPE_AUTO_FOLLOW_FIRSTFRAME);
    }
    else
    {
        nRet = HI_FAILURE;
...
HI_S32 DispProcSetXdrEngineType(HI_DRV_DISPLAY_E enDisp, HI_CHAR *pArg2)
{
    HI_S32  nRet = HI_SUCCESS;
    if (0 == HI_OSAL_Strncmp(pArg2, "sdr", strlen("sdr")))
    {
        nRet = DISP_SetXdrEngineType(enDisp, DRV_XDR_ENGINE_SDR);
    }
    else if (0 == HI_OSAL_Strncmp(pArg2, "dolby", strlen("dolby")))
    {
        nRet = DISP_SetXdrEngineType(enDisp, DRV_XDR_ENGINE_DOLBY);
    }
    else if (0 == HI_OSAL_Strncmp(pArg2, "hdr10", strlen("hdr10")))
    {
        nRet = DISP_SetXdrEngineType(enDisp, DRV_XDR_ENGINE_HDR10);
    }
    else if (0 == HI_OSAL_Strncmp(pArg2, "hlg", strlen("hlg")))
    {
        nRet = DISP_SetXdrEngineType(enDisp, DRV_XDR_ENGINE_HLG);
    }
    else if (0 == HI_OSAL_Strncmp(pArg2, "auto", strlen("auto")))
    {
        nRet = DISP_SetXdrEngineType(enDisp, DRV_XDR_ENGINE_AUTO);
    }
    else
    {
        nRet = HI_FAILURE;
...
static const DISP_PROC_COMMAND DispProcCmds[] =
{
    ...
    {"engine",          DispProcSetXdrEngineType},
...
HI_S32 DISP_ExtOpen(HI_DRV_DISPLAY_E enDisp, DRV_DISP_STATE_S *pDispState, HI_BOOL bUser)
{
    ...
    if (HI_TRUE != pDispState->bDispOpen[enDisp])
    {
        ...
        if (0 < g_DispAllCountState.DispOpenNum[enDisp])
        {
...
HI_S32 HI_DRV_DISP_SetAlpha(HI_DRV_DISPLAY_E enDisp, HI_U32 u32Alpha)
{
    HI_S32 Ret;
    DISP_ALPHA_S stDispAlpha = {0};

    DispCheckID(enDisp);
    stDispAlpha.u32ALpha= u32Alpha;
    stDispAlpha.enDisp = enDisp;
    Ret = HI_DRV_DISP_Process(CMD_DISP_SET_ALPHA, &stDispAlpha);
    return Ret;
}

HI_S32 HI_DRV_DISP_GetAlpha(HI_DRV_DISPLAY_E enDisp, HI_U32* pu32Alpha)
{
    HI_S32 Ret;
    DISP_ALPHA_S stDispAlpha = {0};
    ...
    Ret = HI_DRV_DISP_Process(CMD_DISP_GET_ALPHA, &stDispAlpha);

    *pu32Alpha=stDispAlpha.u32ALpha;
...
HI_S32 DRV_DISP_Resume(PM_BASEDEV_S *pdev)
{
    ...
    if (Disp_GetFastbootupFlag() == DISP_FASTBOOTUP_FLAG)
    {
        ...
        else if (DispCheckIntfExistByType(HI_DRV_DISPLAY_1, &stIntf))
        {
         ...
         DispCheckNullPointer(pstHDMIFunc->pfnHdmiSoftResume);
...
static HI_S32 DISPProcessCmdSetFormat(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,stero-%d,format-%d-ret -0x%x\n",pDispFormat->enDisp,pDispFormat->enStereo,pDispFormat->enFormat,Ret);
...
static HI_S32 DISPProcessCmdSetScreenOFFSet(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,l,r,t,b,(%d,%d,%d,%d)-ret -0x%x\n",
        pOffset->enDisp,
        pOffset->stScreenOffset.u32Left,
        pOffset->stScreenOffset.u32Right,
        pOffset->stScreenOffset.u32Top,
        pOffset->stScreenOffset.u32Bottom,
        Ret);
...
static HI_S32 DISPProcessCmdSetDevRatio(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,H-(%d),V-(%d),-ret -0x%x\n",
            pDispAspectRatio->enDisp,
            pDispAspectRatio->u32ARHori,
            pDispAspectRatio->u32ARVert,
            Ret);
...
static HI_S32 DISPProcessCmdAcquireSnapshot(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    if (pDispState->hSnapshot[pstFrame->enDispLayer] != HI_NULL)
    {
        return HI_FAILURE;
    }
    ...
    {
        pDispState->hSnapshot[pstFrame->enDispLayer] = HI_NULL;
    }
...
{
    HI_S32 Ret = HI_SUCCESS;
    DISP_XDR_ENGINE_S *pstDispXdrEngine = (DISP_XDR_ENGINE_S*)arg;
    Ret = DISP_SetXdrEngineType(pstDispXdrEngine->enDisp, pstDispXdrEngine->enXdrEngine);
...
static HI_S32 DISPProcessCmdGetXdrEngineType(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    HI_S32 Ret = HI_SUCCESS;
    DISP_XDR_ENGINE_S *pstDispXdrEngine = (DISP_XDR_ENGINE_S*)arg;
    Ret = DISP_GetXdrEngineType(pstDispXdrEngine->enDisp, &pstDispXdrEngine->enXdrEngine);
...
EXPORT_SYMBOL(HI_DRV_DISP_GetFormat);
EXPORT_SYMBOL(HI_DRV_DISP_SetAlpha);
EXPORT_SYMBOL(HI_DRV_DISP_GetAlpha);
