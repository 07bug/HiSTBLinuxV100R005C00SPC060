...
HI_S32 GetPdmDispParam(HI_DRV_PDM_DISP_PARAM_S *pstPdmDispParam)
{
#ifndef __DISP_PLATFORM_BOOT__
    ...
    else
    {
        ...
        {
            DISP_ERROR("DISP_get PDM func failed!\n");
            return HI_FAILURE;
        }

        nRet  = pst_PDMFunc->pfnPDM_GetDispParam(HI_UNF_DISPLAY0, &pstPdmDispParam->stDispParam0);
        nRet |= pst_PDMFunc->pfnPDM_GetDispParam(HI_UNF_DISPLAY1, &pstPdmDispParam->stDispParam1);
        if (nRet)
        {
            DISP_WARN("Got no disp params, may not burned!\n");
            pstPdmDispParam->bGetDispParam = HI_FALSE;
            return HI_FAILURE;
        }

        nRet  = DISP_CheckParam(HI_UNF_DISPLAY0, &(pstPdmDispParam->stDispParam0));
        nRet |= DISP_CheckParam(HI_UNF_DISPLAY1, &(pstPdmDispParam->stDispParam1));
        if (nRet)
        {
        ...
        nRet = HI_DRV_PDM_GetDispParam(HI_UNF_DISPLAY0, &(pstPdmDispParam->stDispParam0));
        nRet |= HI_DRV_PDM_GetDispParam(HI_UNF_DISPLAY1, &(pstPdmDispParam->stDispParam1));
        if (nRet)
        {
    ...
    pstPdmDispParam->bGetDispParam = HI_TRUE;
    return HI_SUCCESS;
}


HI_S32 DispGetInitParam(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_INIT_PARAM_S* pstSetting, HI_DRV_PDM_DISP_PARAM_S *pstPdmDispParam)
{
    HI_DRV_DISP_FMT_E  enFormat = HI_DRV_DISP_FMT_BUTT;
    HI_DISP_PARAM_S *pstDispParam;
    HI_S32 i, j;

    if ((HI_NULL == pstPdmDispParam) || (pstPdmDispParam->bGetDispParam != HI_TRUE))
    {
        return HI_FAILURE;
    }
    ...
    {
        pstDispParam = &(pstPdmDispParam->stDispParam0);
    }
    else
    {
        pstDispParam = &(pstPdmDispParam->stDispParam1);
    }
    ...
    if (HI_DRV_DISPLAY_1 == (HI_DRV_DISPLAY_E)pstPdmDispParam->stDispParam0.enSrcDisp)
    {
        /*attach  mode*/
        if (HI_DRV_DISPLAY_0 == enDisp)
        {
            enFormat = DISP_GetEncFmt(pstPdmDispParam->stDispParam1.enFormat);
            ...
            pstSetting->u32VirtScreenWidth  = pstPdmDispParam->stDispParam0.u32VirtScreenWidth;
            pstSetting->u32VirtScreenHeight = pstPdmDispParam->stDispParam0.u32VirtScreenHeight;
            pstSetting->stOffsetInfo        = *((HI_DRV_DISP_OFFSET_S *)&pstPdmDispParam->stDispParam1.stOffsetInfo);
        }
        ...
        if (HI_DRV_DISPLAY_1 == enDisp)
        {
            ...
            pstSetting->u32VirtScreenWidth  = pstPdmDispParam->stDispParam1.u32VirtScreenWidth;
            pstSetting->u32VirtScreenHeight = pstPdmDispParam->stDispParam1.u32VirtScreenHeight;
            pstSetting->stOffsetInfo        = *((HI_DRV_DISP_OFFSET_S*)&pstPdmDispParam->stDispParam1.stOffsetInfo);
        }

        pstSetting->u32Brightness = pstPdmDispParam->stDispParam1.u32Brightness;
        pstSetting->u32Contrast   = pstPdmDispParam->stDispParam1.u32Contrast;
        pstSetting->u32Saturation = pstPdmDispParam->stDispParam1.u32Saturation;
        pstSetting->u32HuePlus    = pstPdmDispParam->stDispParam1.u32HuePlus;
        pstSetting->bGammaEnable  = pstPdmDispParam->stDispParam1.bGammaEnable;
        pstSetting->stBgColor.u8Red     = pstPdmDispParam->stDispParam1.stBgColor.u8Red;
        pstSetting->stBgColor.u8Green   = pstPdmDispParam->stDispParam1.stBgColor.u8Green;
        pstSetting->stBgColor.u8Blue    = pstPdmDispParam->stDispParam1.stBgColor.u8Blue;
    }
    else
    {
