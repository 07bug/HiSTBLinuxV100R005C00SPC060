...
static long DISP_FileIoctl(struct file *ffile, unsigned int cmd, unsigned long arg)
{
    ...
    Ret = (long)HI_DRV_UserCopy(ffile->f_path.dentry->d_inode,ffile, cmd, arg, DRV_DISP_Ioctl);
...
static long DISP_Compat_FileIoctl(struct file *ffile, unsigned int cmd, unsigned long arg)
{
    ...
    Ret = (long)HI_DRV_UserCopy(ffile->f_path.dentry->d_inode,ffile, cmd, arg, DRV_DISP_Compat_Ioctl);
...
HI_S32 VDP_DRV_ModInit(HI_VOID)
{
    ...
    if (HI_SUCCESS != DRV_DISP_Init2())
    {
        HI_FATAL_DISP("DRV_DISP_Init2 failed.\n");
        return HI_FAILURE;
    }
