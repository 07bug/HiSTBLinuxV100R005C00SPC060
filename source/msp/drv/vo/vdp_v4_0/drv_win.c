...
static HI_U32 u32StartTime;

#define SET_CONSUME_START_MS() ({ \
        DISP_OS_GetTime(&u32StartTime); \
    })
#define GET_CONSUME_END_MS() ({         \
        HI_U32 b = 0; \
        DISP_OS_GetTime(&b); \
        (b-u32StartTime);\
    })
...
HI_U8 *g_pWinFieldModeString[HI_DRV_FIELD_BUTT+1] =
{
    "Top",
    "Bottom",
    "All",
...
HI_U8 *g_pXdrSrcFrmType[HI_DRV_VIDEO_FRAME_TYPE_BUTT] = {
    "SDR",
    "Dolby",
    "Dolby EL",
    "HDR10",
...
HI_U8 *g_pOutputTypeString[HI_DRV_DISP_TYPE_BUTT] = {
    "SDR",
    "SDR CERT",
    "AUTO MODE",
    "DOLBYVISION",
...
static HI_S32 WINProcessCmdGetOutTypeAndEngine( HI_VOID *arg, WIN_STATE_S *pstWinState);
...
static HI_S32 WINProcessCmdResetFirstFrameStatus( HI_VOID *arg, WIN_STATE_S *pstWinState);
...
static const stWINProcessCmdFunc s_aWINCmdFuncTab[] =
{
    ...
    {CMD_WIN_GET_OUT_TYPE_AND_ENGINE, WINProcessCmdGetOutTypeAndEngine},
    ...
    {CMD_WIN_RESET_FIRST_FRAME_STATUS, WINProcessCmdResetFirstFrameStatus},
};
...
static HI_VOID Win_GetProcBufStatus(struct seq_file *p, WIN_PROC_INFO_S *pstProcInfo)
{
    ...
    if (HI_NULL == p || HI_NULL == pstProcInfo)
    {
        HI_ERR_WIN("Para pointer is null!\n");
        return;
    }
...
HI_S32 DRV_WIN_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    WIN_PROC_INFO_S *pstProcInfo = HI_NULL;
    HI_HANDLE hWin;
    ...
    if (nRet)
    {
        WIN_ERROR("down_interruptible failed !\n");
        return HI_FAILURE;
    }
    ...
    */
    ...
    if ( (pstProcInfo->enType != HI_DRV_WIN_ACTIVE_SLAVE))
    {
        ...
        PROC_PRINT(p, "%-19s:%-20s", "OpenHdr", g_pWinYNString[!pstProcInfo->stAttr.bCloseHdrPath]);
        PROC_PRINT(p, "%-19s:%-20s\n", "|Frame Type", g_pXdrSrcFrmType[pstNewFrame->enSrcFrameType]);
    ...
    {
        PROC_PRINT(p, "----------------AUTO FOLLOW TO FIRST FRAME MODE STATUS--------------\n");
        PROC_PRINT(p, "%-19s:%-20s\n", "bSenceChange", g_pWinYNString[pstProcInfo->stFirstFrmStatus.bSenceChange]);
        PROC_PRINT(p, "%-19s:%-20s\n", "FirstFrm ColorSpace", g_pVDPColorSpaceString[pstProcInfo->stFirstFrmStatus.enOutColorSpace]);
        PROC_PRINT(p, "%-19s:%-20s\n", "FirstFrm OutType", g_pOutputTypeString[pstProcInfo->stFirstFrmStatus.enDispOutType]);
        ...
        switch (pstNewFrame->enSrcFrameType)
        {
            ...
            case HI_DRV_VIDEO_FRAME_TYPE_HLG:
                PROC_PRINT(p,
                           ...
                           pstNewFrame->stColourInfo.u8MatrixCoeffs,
                           pstNewFrame->stColourInfo.u8FullRangeFlag,
                           pstNewFrame->unHDRInfo.stHLGInfo.u8TransferCharacteristics,
                           pstNewFrame->unHDRInfo.stHLGInfo.u8MasteringAvailable,
                           pstNewFrame->unHDRInfo.stHLGInfo.stMasteringInfo.u32MaxDisplayMasteringLuminance,
                           pstNewFrame->unHDRInfo.stHLGInfo.stMasteringInfo.u32MinDisplayMasteringLuminance,
                           pstNewFrame->unHDRInfo.stHLGInfo.u8ContentAvailable
                          );
                break;
            ...
            case HI_DRV_VIDEO_FRAME_TYPE_SLF:
                PROC_PRINT(p,
                           ...
                           pstNewFrame->stColourInfo.u8MatrixCoeffs,
                           pstNewFrame->stColourInfo.u8FullRangeFlag,
                           pstNewFrame->unHDRInfo.stSLFInfo.u8TransferCharacteristics,
                           pstNewFrame->unHDRInfo.stSLFInfo.u8MasteringAvailable,
                           pstNewFrame->unHDRInfo.stSLFInfo.stMasteringInfo.u32MaxDisplayMasteringLuminance,
                           pstNewFrame->unHDRInfo.stSLFInfo.stMasteringInfo.u32MinDisplayMasteringLuminance,
                           pstNewFrame->unHDRInfo.stSLFInfo.u8ContentAvailable
                          );
    ...
    if (HI_TRUE != bVirtual)
    {
        ...
        PROC_PRINT(p, "Overlay Lowdelay stat: OmxDone %u, OverlayQueue %u, WinGet %u, WinConfig %u, Total %u, cost %u, toDisp %u\n",
                   ...
                   (pstProcInfo->stLowdelayStat.u32WinConfigTime - pstProcInfo->stLowdelayStat.u32WinGetFrameTime),
                   pstWin->stBuffer.stWinBP.stBuffer.stDispNodeDelayTime.u32NodeConfigLeftTime);
...
HI_S32 WinProcSetASP(HI_HANDLE hWin, HI_CHAR *pArg2)
{
    ...
    if( (stWinAttr.enARCvrs <= HI_DRV_ASP_RAT_MODE_COMBINED))
    {
                         ...
                         const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    if (HI_NULL == file ||
        ...
        HI_NULL == ppos)
    {
        HI_ERR_WIN("Input parameter ptr is null!\n");
        return -EFAULT;
    }
...
HI_S32 WIN_RemFromState(WIN_STATE_S *pst2WinState, HI_BOOL bVirtual, HI_HANDLE hWin)
{
    ...
    CHECK_NULL_POINTER(pst2WinState);
...
HI_S32 WIN_CreateExt(WIN_CREATE_S *pVoWinCreate, WIN_STATE_S *pst2WinState)
{
    ...
    CHECK_NULL_POINTER(pVoWinCreate);
    CHECK_NULL_POINTER(pst2WinState);
    CHECK_DISP_ID(pVoWinCreate->WinAttr.enDisp);

    if (pVoWinCreate->WinAttr.enARCvrs >= HI_DRV_ASP_RAT_MODE_BUTT)
    {
        HI_ERR_WIN("para pVoWinCreate->WinAttr.enARCvrs is invalid.\n");
        return HI_ERR_VO_INVALID_PARA;
    }

    if (pVoWinCreate->WinAttr.enDataFormat >= HI_DRV_PIX_BUTT)
    {
        HI_ERR_WIN("para pVoWinCreate->WinAttr.enARCvrs is invalid.\n");
        return HI_ERR_VO_INVALID_PARA;
    }
    ...
    Ret = WIN_AddToState(pst2WinState, pVoWinCreate->WinAttr.bVirtual, pVoWinCreate->WinAttr.enDisp, pVoWinCreate->hWindow);
    if ( HI_SUCCESS != Ret)
    {
        goto __ERR_EXIT_REM_PORC__;
    }
...
HI_S32 WIN_DestroyExt(HI_HANDLE hWindow, WIN_STATE_S *pstWinState)
{
    ...
    CHECK_NULL_POINTER(pstWinState);
...
static HI_S32 WINProcessCmdCreate( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    SET_CONSUME_START_MS();

    pVoWinCreate = (WIN_CREATE_S *)arg;

    Ret = WIN_CreateExt(pVoWinCreate, pstWinState);
    WIN_WARN("creat win0x%x,v(%d) out->(%d,%d,%d,%d).user -%dms,ret-%d\n", \
...
static HI_S32 WINProcessCmdDestroy( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    Ret = WIN_CheckHanlde(hWindow, pstWinState);
    ...
    if (HI_SUCCESS == Ret)
    {
        ...
        Ret = WIN_DestroyExt(hWindow, pstWinState);

    }
    WIN_WARN("destroy win0x%xd--ues-%dms,ret %d.\n", hWindow, GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdSetEnable( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("set win0x%x, enable->%d,ues-%dms,ret=%d\n", pWinEnable->hWindow, pWinEnable->bEnable, GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdSetAttr( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("set attr win0x%x, out->(%d,%d,%d,%d),user-%dms,ret-%d\n", pVoWinAttr->hWindow, \
...
static HI_S32 WINProcessCmdSetZorder( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("set zorder win0x%x, ZFlag->%d-user-%dms,ret-%d\n", pVoWinZorder->hWindow, pVoWinZorder->eZFlag, GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdSetSource( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("set source win0x%x, hSrc->%d,use-%dms,ret-%d\n", pVoWinAttach->hWindow, stWinSrc.hSrc, GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdFreeze( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("set Freeze win0x%x, enable->%d,mode-%d,use-%dms,ret-%d\n", pVoWinFreeze->hWindow, pVoWinFreeze->bEnable,pVoWinFreeze->eMode, GET_CONSUME_END_MS(), Ret);
...
{
    HI_S32 Ret = HI_SUCCESS;
    WIN_XDR_FRAME_S   *pVoWinFrame;
    pVoWinFrame = (WIN_XDR_FRAME_S *)arg;
    Ret = WIN_CheckHanlde(pVoWinFrame->hWindow, pstWinState);
    if (HI_SUCCESS == Ret)
    {
        Ret = WIN_QueueFrame(pVoWinFrame->hWindow, pVoWinFrame);
    }
...
{
    HI_S32 Ret = HI_SUCCESS;
    WIN_XDR_FRAME_S    *pVoWinDoviFrm;
    pVoWinDoviFrm = (WIN_XDR_FRAME_S *)arg;
    Ret = WIN_CheckHanlde(pVoWinDoviFrm->hWindow, pstWinState);
    if (HI_SUCCESS == Ret)
    {
        Ret = WinQueueDolbyFrame(pVoWinDoviFrm->hWindow, pVoWinDoviFrm);
    }
...
static HI_S32 WINProcessCmdReset( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("win reset win0x%x, %s,use-%dms,ret=%d\n", pVoWinReset->hWindow,
             (HI_DRV_WIN_SWITCH_LAST == pVoWinReset->eMode ? "last" : "black"),
             GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdPause( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("win pause win0x%x, bEnable->%d,use-%dms\n", pVoWinPause->hWindow, pVoWinPause->bEnable,GET_CONSUME_END_MS());
...
static HI_S32 WINProcessCmdGetHandle( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    DISP_MEMSET(&stWinArray, 0, sizeof(stWinArray));
...
static HI_S32 WINProcessCmdCaptureStart( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    Ret = WIN_CheckHanlde(capture_info->hWindow, pstWinState);
    if (HI_SUCCESS != Ret)
    {
        return HI_FAILURE;
    }
    ...
    if (enDisp >= HI_DRV_DISPLAY_2 || u32WinIndex >= DEF_MAX_WIN_NUM_ON_SINGLE_DISP)
    {
    ...
    WinCheckWindow(capture_info->hWindow, pstWin);
    ...
    WIN_WARN("win capture start win0x%x,addr->0x%x,user-%dms,ret %d\n", capture_info->hWindow, capture_info->CapPicture.stBufAddr[0].u32PhyAddr_Y, GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdCaptureRelease( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("win capture release win0x%x,addr->0x%x,user-%dms,ret %d\n", capture_info->hWindow, capture_info->CapPicture.stBufAddr[0].u32PhyAddr_Y, GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdCaptureFree( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("win capture free win0x%x,addr->0x%x,user-%dms,ret %d\n", capture_info->hWindow, capture_info->CapPicture.stBufAddr[0].u32PhyAddr_Y, GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdSetRotation( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("win set rotation win0x%x,rotation->0x%x,use-%dms,-ret %d\n", pVoWinRotation->hWindow, pVoWinRotation->enRotation, GET_CONSUME_END_MS(), Ret);
...
static HI_S32 WINProcessCmdSetFlip( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_WARN("win set flip win0x%x,rotation->0x%x,ser-%dms,ret %d\n", pVoWinFlip->hWindow, pVoWinFlip->bHoriFlip, GET_CONSUME_END_MS(), Ret);
...
{
    HI_S32 Ret = HI_SUCCESS;
    WIN_OUT_TYPE_AND_ENGINE_S *pVoWinOutTypeAndEngine;
    pVoWinOutTypeAndEngine = (WIN_OUT_TYPE_AND_ENGINE_S*)arg;

    Ret = WIN_POLICY_GetOutputTypeAndEngine(pVoWinOutTypeAndEngine->hWindow,
                                  pVoWinOutTypeAndEngine->enFrmType,
                                  &pVoWinOutTypeAndEngine->enDispOutType,
                                  &pVoWinOutTypeAndEngine->enXdrEngine);
...
static HI_S32 WINProcessCmdResetFirstFrameStatus( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    if (HI_SUCCESS == Ret)
    {
        Ret = WinResetFirstFrameStatus(*((HI_HANDLE *)arg));
    }
...
HI_S32 WIN_ProcessCmd(unsigned int cmd, HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    CHECK_NULL_POINTER(arg);
    CHECK_NULL_POINTER(pstWinState);

    for (u32Index = 0; (u32Index < sizeof(s_aWINCmdFuncTab) / sizeof(stWINProcessCmdFunc)); u32Index++)
    {
        if ((s_aWINCmdFuncTab[u32Index].cmd) == cmd &&
            (HI_NULL != s_aWINCmdFuncTab[u32Index].pWIN_ProcessCmdFunc))
        {
            Ret = s_aWINCmdFuncTab[u32Index].pWIN_ProcessCmdFunc(arg, pstWinState);
...
HI_S32 DRV_WIN_Resume(PM_BASEDEV_S *pdev)
{
    ...
    if (HI_SUCCESS != WIN_Resume())
    {
        HI_FATAL_WIN("Call WIN_Resume failed.\n");
        up(&g_VoMutex);
        return HI_FAILURE;
    }
...
HI_S32 HI_DRV_WIN_Reset(HI_HANDLE hWindow, HI_DRV_WIN_SWITCH_E enSwitch)
{
    ...
    if (enSwitch >= HI_DRV_WIN_SWITCH_BUTT)
    {
        HI_FATAL_WIN("Para enSwitch is invalid!\n");
        return HI_ERR_VO_INVALID_PARA;
    }
...
HI_S32 Make_CaptureFrame(HI_DRV_VIDEO_FRAME_S *pFrameinfo, HI_DRV_VIDEO_FRAME_S *pCapFrameinfo)
{
    ...
    if (datalen > VDP_MEM_MAX)
    {
        HI_ERR_WIN("the size (%d) more than MAX\n", datalen);
        return HI_FAILURE;
    }
...
HI_S32 HI_DRV_WIN_QFrame(HI_HANDLE hWindow, HI_DRV_VIDEO_FRAME_S *pFrame)
{
    ...
    pstWinXdrFrm = HI_KMALLOC(HI_ID_VO, sizeof(WIN_XDR_FRAME_S), GFP_KERNEL);
    ...
    memset(pstWinXdrFrm, 0, sizeof(WIN_XDR_FRAME_S));
    pstWinXdrFrm->hWindow = hWindow;
    pstWinXdrFrm->stBaseFrm = *pFrame;
    /* get disp output type and xdrEngine. */
    Ret = WIN_POLICY_GetOutputTypeAndEngine(hWindow,
                                           pFrame->enSrcFrameType,
    ...
    if (Ret != HI_SUCCESS)
    {
        ...
        pstWinXdrFrm = HI_NULL;
        return Ret;
    }

    Ret = DRV_WIN_Process(CMD_WIN_QU_FRAME, pstWinXdrFrm);
...
HI_S32 HI_DRV_WIN_SetZorder(HI_HANDLE hWin, HI_DRV_DISP_ZORDER_E ZFlag)
{
    ...
    if (ZFlag >= HI_DRV_DISP_ZORDER_BUTT)
    {
        HI_FATAL_WIN("Para ZFlag is invalid!\n");
        return HI_ERR_VO_INVALID_PARA;
    }
...
HI_S32 HI_DRV_WIN_Freeze(HI_HANDLE hWin, HI_BOOL bEnable,  HI_DRV_WIN_SWITCH_E eRst)
{
    ...
    if (eRst >= HI_DRV_WIN_SWITCH_BUTT)
    {
        HI_FATAL_WIN("Para eRst is invalid!\n");
        return HI_ERR_VO_INVALID_PARA;
    }
...
HI_S32 WIN_DRV_DestroyAll(WIN_STATE_S *pst2WinState)
{
    ...
    WinCheckNullPointer(pst2WinState);
...
HI_S32 HI_DRV_WIN_DeInit(HI_VOID)
{
    ...
    if (HI_SUCCESS != WIN_DRV_DestroyAll(&g_VoModState))
    {
        WIN_FATAL("Call WIN_DRV_DestroyAll faild.\n");
        up(&g_VoMutex);
        return HI_FAILURE;
    }
    ...
    {
        if (HI_SUCCESS != WIN_DeInit())
        {
            WIN_FATAL("Call WIN_DeInit faild.\n");
            up(&g_VoMutex);
            return HI_FAILURE;
        }
    }
...
{
    if (HI_SUCCESS != HI_DRV_WIN_DeInit())
    {
        WIN_FATAL("Call HI_DRV_WIN_DeInit faild.\n");
    }

    if (HI_SUCCESS != HI_DRV_MODULE_UnRegister(HI_ID_VO))
    {
        WIN_FATAL("Call HI_DRV_MODULE_UnRegister faild.\n");
    }
...
HI_S32 WIN_DRV_Close(struct inode *finode, struct file  *ffile)
{
    ...
    if (HI_SUCCESS != WIN_DRV_DestroyAll(pVoState))
    {
        HI_FATAL_WIN("Call WIN_DRV_DestroyAll failed!\n");
        up(&g_VoMutex);
        return HI_FAILURE;
    }
    ...
    if (atomic_dec_and_test(&g_VoCount))
    {
        if (HI_SUCCESS != WIN_DeInit())
        {
            HI_FATAL_WIN("Call WIN_DeInit failed!\n");
            up(&g_VoMutex);
            return HI_FAILURE;
        }
    }
...
EXPORT_SYMBOL(HI_DRV_WIN_GetInfo);
//#endif
