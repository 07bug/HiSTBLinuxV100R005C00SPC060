...
HI_S32 VDP_ConfigCheck(HI_U32 type)
{
    ...
    for (kk = 0; kk < (sizeof(wbc) / 4 - 1); kk++)
    {
        ...
        {
            if (WBC_GP0_CTRL.bits.wbc_en == 1)
            {
                VDP_ASSERT((WBC_GP0_CTRL.bits.mode_out != 0));
            }
        }
    }
    ...
    */
    ...
    return HI_SUCCESS;
    //CBM_ATTR.u32  = VDP_RegRead(&(pVdpReg->CBM_ATTR.u32));

    //cout << "CBM_ATTR.u32:" << (CBM_ATTR.u32)<< endl;
...
HI_S32 VDP_Sys_ConfigCheck(HI_VOID)
{
    ...
    for (enChan = 0; enChan <= 0x1; enChan++)
    {
        ...
        // VDP_ASSERT(DHD0_CTRL.bits.iop != VDP_RMODE_INTERLACE);
    ...
    VDP_ASSERT(VOAXICTRL.bits.m0_outstd_rid0 >= 1 );
    VDP_ASSERT(VOAXICTRL.bits.m1_outstd_rid0 >= 1 );
    VDP_ASSERT(VOAXICTRL.bits.m0_outstd_rid1 >= 1 );
    VDP_ASSERT(VOAXICTRL.bits.m1_outstd_rid1 >= 1 );

    VDP_ASSERT(VOAXICTRL.bits.m0_wr_ostd >= 1);
    VDP_ASSERT(VOAXICTRL.bits.m1_wr_ostd >= 1);
    ...
    if (g0_en == 1 && g1_en == 1)
        //VDP_ASSERT(VOCTRL.bits.m0_arb_mode == 0xb);
#else
    if (g0_en == 1 && g1_en == 1 && g3_en == 1)
    {
        VDP_ASSERT(VOCTRL.bits.m0_arb_mode == 0x0);
    }
#endif
...
#ifdef VDP_HI3798MV200
    VDP_ASSERT(g0_req_ctrl <= 4);
    VDP_ASSERT(g1_req_ctrl <= 4);
    //        VDP_ASSERT(g4_req_ctrl <= 2);

    VDP_ASSERT(v0_req_ctrl <= 4);
    VDP_ASSERT(v1_req_ctrl <= 2);
    VDP_ASSERT(v3_req_ctrl <= 1);
#else
    for (i = 0; i <= 2; i++)
    {
        V0_CTRL.u32  = VDP_RegRead(((HI_ULONG) & (pVdpReg->V0_CTRL.u32) + vid[i] * VID_OFFSET));
...
HI_S32 VDP_VID_ConfigCheck(HI_U32 u32Data)
{
    ...
    vid_data_width = ((v0_data_width == 0) || (V0_CTRL.bits.ifmt == VDP_VID_IFMT_SP_TILE_64)) ? 8 : 10;
    ...
    {
        HI_U32 u32Num = 0;
        U_V0_P0CTRL      REGION_V0_P0CTRL;
        for (i = 1; i <= 0x8000; i = (i << 1))
        {
            REGION_V0_P0CTRL.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->V0_P0CTRL.u32) + u32Data * VID_OFFSET + u32Num * REGION_OFFSET));
            if (REGION_V0_P0CTRL.bits.region_en == 0x1)
            {
                ...
                VDP_ASSERT(V0_P0STRIDE.bits.surface_stride >= (region_w * vid_data_width / 8 + 15) / 16 * 16); //B02, multi_region nonsupport 10bit
                VDP_ASSERT(V0_P0STRIDE.bits.surface_cstride >= (region_w * vid_data_width / 8 + 15) / 16 * 16);
    ...
    if (V0_CTRL.bits.ifmt == VDP_VID_IFMT_SP_TILE_64)
    {
        ...
        cout << hex << "V0_NADDR = " << V0_NADDR.u32 << endl;
        cout << hex << "V0_NCADDR = " << V0_NCADDR.u32 << endl;
        ...
        VDP_ASSERT(!(V0_P0STRIDE.bits.surface_stride < V0_IRESO.bits.iw * vid_data_width / 8));
        VDP_ASSERT(!(V0_P0STRIDE.bits.surface_cstride < V0_IRESO.bits.iw * vid_data_width / 8));
    ...
    if (v0_zme2_en == 1)
    {
        VDP_ASSERT(((V0_ZME2_VSP.bits.zme_in_fmt == VDP_PROC_FMT_SP_422) || (V0_ZME2_VSP.bits.zme_in_fmt == VDP_PROC_FMT_SP_420)));

        if (DB_HDR_CTRL.bits.hdr_en == 1)
        {
            VDP_ASSERT(V0_ZME2_VSP.bits.zme_out_fmt == VDP_PROC_FMT_SP_420);
        }
        else
        {
    ...
    //ACC assertion
    //------------------------------------------------------------------------------------
    if (VHDACCTHD1.bits.acc_en)
    {
        VDP_ASSERT(disp_mode != VDP_DISP_MODE_FP);
    }
...
#endif
    if ((V0_ZME2_VSP.bits.vlmsc_en == 0x1) || (V0_ZME2_VSP.bits.vchmsc_en == 0x1))
    {
        VDP_ASSERT((V0_DLPOS.bits.disp_ylpos - V0_DFPOS.bits.disp_yfpos) >= pVdpReg->V0_SRORESO.bits.oh );
        VDP_ASSERT(V0_DLPOS.bits.disp_ylpos >= (pVdpReg->V0_SRORESO.bits.oh + V0_VFPOS.bits.video_yfpos));
    ...
    if ((V0_ZME2_HSP.bits.hlmsc_en == 0x1) || (V0_ZME2_HSP.bits.hchmsc_en == 0x1))
    {
        VDP_ASSERT(V0_DLPOS.bits.disp_xlpos >= (pVdpReg->V0_SRORESO.bits.ow + V0_VFPOS.bits.video_xfpos));
        VDP_ASSERT((V0_DLPOS.bits.disp_xlpos - V0_DFPOS.bits.disp_xfpos) >= pVdpReg->V0_SRORESO.bits.ow );
...
{
    HI_U32 wbc_ireso_w = 1920;
    HI_U32 wbc_ireso_h = 1080;
    U_WBC_DHD0_CTRL    WBC_DHD0_CTRL;
    ...
    WBC_DHD0_HPZME.u32        = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_DHD0_HPZME.u32)     + u32Data * WBC_OFFSET)) ;
    WBC_DHD0_FCROP.u32        = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_DHD0_FCROP.u32)     + u32Data * WBC_OFFSET)) ;
    WBC_DHD0_LCROP.u32        = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_DHD0_LCROP.u32)     + u32Data * WBC_OFFSET)) ;
    ...
    if (WBC_DHD0_CTRL.bits.wbc_en == 1)
    {
        if (WBC_DHD_LOCATE.bits.wbc_dhd_locate == 3)
        {
            VDP_ASSERT(GP0_CTRL.bits.depremult_en == 1);
        }
    }
#endif
    //---------------------------------------------------------
    ...
    else if (u32Data == 3)  // WBC_VP
    {
        if (WBC_DHD0_CTRL.bits.wbc_en == HI_TRUE)
        {
            //cout << "assert:" << WBC_DHD0_HPZME.bits.hpzme_en << endl;
            //cout << "assert:" << WBC_DHD0_CTRL.bits.wbc_point_sel << endl;
            //cout << "assert:" << WBC_DHD0_LCROP.bits.wlcrop << endl;
            //cout << "assert:" << WBC_DHD0_FCROP.bits.wfcrop << endl;
            //if((WBC_DHD0_HPZME.bits.hpzme_en = 1) && (WBC_DHD0_CTRL.bits.wbc_point_sel == VDP_LAYER_CONN_VIDBZME))
            //{
            //    VDP_ASSERT((WBC_DHD0_LCROP.bits.wlcrop-WBC_DHD0_FCROP.bits.wfcrop+1) % 8 == 0x0);
            //}
...
HI_S32 VDP_GFX_ConfigCheck(HI_U32 u32Data)
{
    ...
    U_MIXG0_MIX_BYPASS   st_mixg0_mix_bypass;
    U_G0_CTRL            st_g0_ctrl;
    U_G0_CTRL            st_g1_ctrl;
    ...
    st_mixg0_mix_bypass.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->MIXG0_MIX_BYPASS.u32) + 0 * GFX_OFFSET));
    st_g0_ctrl.u32     = VDP_RegRead(((HI_ULONG) & (pVdpReg->G0_CTRL.u32) + 0 * GFX_OFFSET));
    st_g1_ctrl.u32     = VDP_RegRead(((HI_ULONG) & (pVdpReg->G0_CTRL.u32) + 1 * GFX_OFFSET));
#ifdef VDP_HI3796MV200
    if (st_g0_ctrl.bits.surface_en && st_g1_ctrl.bits.surface_en && (st_mixg0_mix_bypass.bits.mix1_bypass_mode == 1))
    {
        VDP_ASSERT(st_mixg0_mix_bypass.bits.layer0_bypass_en == 0);
    }
#endif
    ...
    if (G0_CTRL.bits.surface_en == 0x1)
    {
        ...
        if ((DHD0_CTRL.bits.disp_mode == VDP_DISP_MODE_TAB))
        {
            ...
            if (u32Data == VDP_LAYER_GFX0)
            {
                ...
                VDP_ASSERT(((G0_IRESO.bits.ih + 1) % 2) == 0);
            }
...
HI_S32 VDP_MMU_AddrCheck (HI_VOID)
{
    ...
    U_V0_SMMU_BYPASS1      V0_SMMU_BYPASS1      ;
    U_V0_SMMU_BYPASS2      V0_SMMU_BYPASS2      ;
    U_V0_SMMU_BYPASS3      V0_SMMU_BYPASS3      ;
    U_G0_SMMU_BYPASS0      G0_SMMU_BYPASS0      ;
    ...
    for (ii = 0; ii < 4; ii++)
    {
        ...
        if (vid_en == 1)
        {
            ...
            V0_SMMU_BYPASS1.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->V0_SMMU_BYPASS1.u32) + ii * VID_OFFSET));
            V0_SMMU_BYPASS2.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->V0_SMMU_BYPASS2.u32) + ii * VID_OFFSET));
            V0_SMMU_BYPASS3.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->V0_SMMU_BYPASS3.u32) + ii * VID_OFFSET));
            ...
            HI_U32 region_l_bypass[64] = {0};
            HI_U32 region_c_bypass[64] = {0};
            HI_U32 u32mmu_bypass = 0;

            HI_U32 jj = 0;
            for (jj = 0; jj < VID_REGION_NUM; jj++)
            {
                HI_U32 l_bits = 0;
                HI_U32 c_bits = 0;
                ...
                if (jj < 16)
                {
                    u32mmu_bypass = V0_SMMU_BYPASS0.u32;
                }
                else if (jj < 32)
                {
                    u32mmu_bypass = V0_SMMU_BYPASS1.u32;
                }
                else if (jj < 48)
                {
                    u32mmu_bypass = V0_SMMU_BYPASS2.u32;
                }
                else if (jj < 64)
                {
                    u32mmu_bypass = V0_SMMU_BYPASS3.u32;
                }

                l_bits = (jj % 16) * 2;
                c_bits = (jj % 16) * 2 + 1;

                region_l_bypass[jj] = (u32mmu_bypass & (1 << l_bits)) >> l_bits;
                region_c_bypass[jj] = (u32mmu_bypass & (1 << c_bits)) >> c_bits;
#if MMU_ADDR_CHECK_DBG
                cout << dec << "region_l_bypass[" << jj << "] = " << region_l_bypass[jj] << endl;
                cout << dec << "region_c_bypass[" << jj << "] = " << region_c_bypass[jj] << endl;
#endif
            }

            if (ii == 0) //v0 addr judge
            {
            ...
            if (multi_en == 1) //multi region
            {
                if (smmu_glb_bypass == 0)
                {
                    for (HI_U32 jj = 0; jj < VID_REGION_NUM; jj++)
                    {
                        ...
                        V0_P0CTRL.u32  = VDP_RegRead(((HI_ULONG) & (pVdpReg->V0_P0CTRL.u32 ) + ii * VID_OFFSET + jj * REGION_OFFSET));
                        region_en = V0_P0CTRL.bits.region_en;

#if MMU_ADDR_CHECK_DBG
                        cout << dec << "Region No.  " << jj << endl;
                        cout << dec << "region_en   = " << region_en << endl;
                        cout << hex << "L addr      = " << V0_P0LADDR.u32 << endl;
                        cout << hex << "C addr      = " << V0_P0CADDR.u32 << endl;
                        cout << dec << "l_bypass    = " << region_l_bypass[jj] << endl;
                        cout << dec << "c_bypass    = " << region_c_bypass[jj] << endl;
#endif
                        if (region_en == 1)
                        {
        ...
        if (gfx_en == 1)
        {
            ...
            if (smmu_glb_bypass == 0)
            {
                if (disp_mode != 0)
                {
                    ...
                    {
                        VDP_ASSERT(G0_ADDR.bits.surface_addr < 0x80000000 && G0_ADDR.bits.surface_addr > 0x100000 && G0_STRIDE.bits.surface_stride <= 0x400);
                    }
                    else
                    {
                    ...
                    {
                        VDP_ASSERT(G0_NADDR.bits.surface_addr < 0x80000000 && G0_NADDR.bits.surface_addr > 0x100000 && G0_STRIDE.bits.surface_stride <= 0x400);
                    }
                    else
                    {
