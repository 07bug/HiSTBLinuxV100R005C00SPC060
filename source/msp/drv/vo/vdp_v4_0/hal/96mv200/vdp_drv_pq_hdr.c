...
//#include "vdp_drv_coef.h"
//lut for hdr
#include "vdp_dm_lutpq2l.h"

#include "vdp_dm_typefxp.h"
#include "vdp_dm_lutl2gx.h"
#include "vdp_dm_lutl2ga.h"
#include "vdp_dm_lutl2gb.h"
#include "vdp_dm_lutl2pqx.h"
#include "vdp_dm_lutl2pqa.h"
#include "vdp_dm_lutl2pqb.h"
...
HI_S32 VDP_DRV_SetDmGCvmCoef(HI_S16 *ptmLutI, HI_S16 *ptmLutS, HI_S16 *psmLutI, HI_S16 *psmLutS)
{
...
#endif
#if VDP_CBB_FPGA
    HI_U8   *addr = gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_GCVM] ;//
#else
    HI_U8   *addr ;//
    ...
    {
        VDP_PRINT("PTR of GCvm is HI_NULL  please check it !!! \n");
        return HI_FAILURE;
    }
...
HI_S32 VDP_DRV_SetDmCvmCoef(HI_S16 *ptmLutI, HI_S16 *ptmLutS, HI_S16 *psmLutI, HI_S16 *psmLutS)
{
...
#endif
#if VDP_CBB_FPGA
    HI_U8   *addr = gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_CVM] ;//
#else
    HI_U8   *addr ;//
    ...
    {
        VDP_PRINT("PTR of Cvm is HI_NULL  please check it !!! \n");
        return HI_FAILURE;
    }
...
HI_VOID VDP_DRV_SetDmDegammaCoef (DmKsFxp_t *pKs)
{
    ...
    if (HI_NULL == input_addr
        || HI_NULL == output_addr)
    {
        VDP_PRINT("PTR of Degamma is HI_NULL  please check it !!! \n");
        return ;
    }
...
HI_VOID VDP_DRV_SetDmgammaCoef(DmKsFxp_t *pKs)
{
    ...
    if (HI_NULL == input_addr
        || HI_NULL == output_addr)
    {
        VDP_PRINT("PTR of Gamma is HI_NULL  please check it !!! \n");
        return ;
    }

    //addr = gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_GDM_GAMMA];//
