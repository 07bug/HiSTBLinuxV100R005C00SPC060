...
extern HI_S16 gs16TmIHlg2Sdr[512] ;
extern HI_S16 gs16TmSHlg2Sdr[512] ;
extern HI_S16 gs16SmIHlg2Sdr[512] ;
extern HI_S16 gs16SmSHlg2Sdr[512] ;

extern HI_S16 TMLutI_hlg2hdr10[512] ;
extern HI_S16 TMLutS_hlg2hdr10[512] ;
extern HI_S16 SMLutI_hlg2hdr10[512] ;
extern HI_S16 SMLutS_hlg2hdr10[512] ;

extern HI_U32 gu16G_IM_GmmLutHlg2Hlg[64] ;//   // hlg400

extern  HI_U32 gu16G_IM_GmmLutHlg2SdrYuv[64] ;
...
extern int16_t  TMLutI_hdr102hlg[TM_LUT_MAX_SIZE];
extern int16_t  TMLutS_hdr102hlg[TM_LUT_MAX_SIZE];
extern int16_t  SMLutI_hdr102hlg[TM_LUT_MAX_SIZE];
extern int16_t  SMLutS_hdr102hlg[TM_LUT_MAX_SIZE];
...
HI_S32 VDP_DRV_SetHisiVidBaseSetting(VDP_HISI_HDR_CFG_S  *stHdrCfg)
{
    ...
    HI_PQ_ZME_COEF_RATIO_E enHiPqZmeCoefRatio = HI_PQ_ZME_COEF_1;

    VDP_RECT_S stOutReso = {0};
    VDP_RECT_S stVidReso = {0};
    VDP_RECT_S stDispReso = {0};

    VDP_RECT_S stVp0DispReso = {0};
    VDP_RECT_S stVp0VidReso = {0};
    VDP_RECT_S stInReso = {0};
    VDP_DRAW_MODE_E enDrawMode = VDP_DRAW_MODE_1;

    if (HI_NULL == stHdrCfg)
    {
        VDP_PRINT("VDP_DRV_SetHisiVidBaseSetting is NULL !!!\n");
        return HI_FAILURE;
    }

    stInReso = stHdrCfg->stVidInReso;
    ...
    if (stInReso.u32Hgt * 10 <= stOutReso.u32Hgt * 24)
    {
        enDrawMode = VDP_DRAW_MODE_1;
    }
    else if (stInReso.u32Hgt * 10 <= stOutReso.u32Hgt * 40)
    {
        enDrawMode = VDP_DRAW_MODE_2;
    }
    else if (stInReso.u32Hgt * 10 <= stOutReso.u32Hgt * 60)
    {
        enDrawMode = VDP_DRAW_MODE_4;
    }
    else
    {
        enDrawMode = VDP_DRAW_MODE_8;
    }

    if (enDrawMode == VDP_DRAW_MODE_2)
    {
        stInReso.u32Hgt /= 2;
    }
    else if (enDrawMode == VDP_DRAW_MODE_4)
    {
        stInReso.u32Hgt /= 4;
    }
    else if (enDrawMode == VDP_DRAW_MODE_8)
    {
        stInReso.u32Hgt /= 8;
    }


    VDP_VID_SetInReso                  (VDP_LAYER_VID0, stInReso);
    VDP_VID_SetOutReso                 (VDP_LAYER_VID0, stOutReso);
    VDP_VID_SetDrawMode                (VDP_LAYER_VID0, enDrawMode, enDrawMode);
    ...
    //  if((stHdrCfg->stBlReso.u32Wth != stHdrCfg->stBlReso.u32Wth) || (stHdrCfg->stBlReso.u32Hgt != stHdrCfg->stBlReso.u32Hgt))
    {
        ...
        VDP_VID_SetZmeHorRatio         (VDP_LAYER_VID0, (stInReso.u32Wth / 2 * ZME_HPREC) / stOutReso.u32Wth * 2);
        VDP_VID_SetZmeVerRatio         (VDP_LAYER_VID0, (stInReso.u32Hgt * ZME_VPREC) / stOutReso.u32Hgt);
        ...
        if (HI_NULL == gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_ZMEH]
            || HI_NULL == gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_ZMEV]
...
HI_S32 VDP_DRV_SetHisiHdrMode(VDP_HISI_HDR_CFG_S  *stHdrCfg, HI_PQ_HDR_CFG *pstHisiHdrTmCfg)
{
    ...
    if (stHdrCfg->enHdrMode >= VDP_HIHDR_CFG_MODE_BUTT)
    {
        VDP_PRINT("enHiHdrMode arange Error !!! \n");
        return HI_FAILURE;
    }
    ...
    else if (stHdrCfg->enHdrMode == VDP_HIHDR_HLG_IN_HLG_OUT)
    {
        bl_en                = 0;
        cvm_sel              = 0;
        edr_ds_en            = 0;
    ...
    VDP_HDR_SetVDmEn               (1);
    if (stHdrCfg->enHdrMode == VDP_HIHDR_HDR10_IN_HDR10_OUT || stHdrCfg->enHdrMode == VDP_HIHDR_HDR10_IN_SDR_OUT  ||  stHdrCfg->enHdrMode == VDP_HIHDR_HLG_IN_HDR10_OUT)
    {
    ...
    else  if (stHdrCfg->enHdrMode == VDP_HIHDR_HLG_IN_HLG_OUT)
    {
        ...
        if (stHdrCfg->bGfxEn)
        {
            VDP_DRV_SetGdmgammaCoef(gu16G_IM_GmmLutHlg2Hlg);
            VDP_HDR_SetGdmParaUpd(VDP_LAYER_GP0, VDP_GP_PARA_HDR_GMM);
            VDP_DRV_SetHisiGdmHlgInHlgOut(stHdrCfg);
        }
    ...
    else if (stHdrCfg->enHdrMode == VDP_HIHDR_HDR10_IN_HLG_OUT )//need to add para
    {
        ...
        VDP_DRV_SetHisigammaCoef(HisiHdr10l2pqLutX, HisiHdr10l2pqLutA, HisiHdr10l2pqLutB);
#else
        VDP_DRV_SetHisiDegammaCoefHlgScene(HisiHdr10_1024Lut, 1024);
        VDP_DRV_SetHisigammaCoefHlgScene(HisiHdr10l2pqLutX, HisiHdr10l2pqLutA, HisiHdr10l2pqLutB);

#endif

        //VDP_DRV_SetHisiDmCvmCoef(pstHisiHdrTmCfg->s16V_TMLutI,pstHisiHdrTmCfg->s16V_TMLutS,pstHisiHdrTmCfg->s16V_SMLutI,pstHisiHdrTmCfg->s16V_SMLutS);
        //VDP_DRV_SetHisiDmCvmCoef(TMLutI_Hlg,TMLutS_Hlg,SMLutI_Hlg,SMLutS_Hlg);
        // VDP_DRV_SetHisiDmCvmCoef(TMLutI_hlg2hdr10, TMLutS_hlg2hdr10, SMLutI_hlg2hdr10, SMLutS_hlg2hdr10);
        ...
        //VCVM1
        VDP_HDR_SetVCvmLms2iptEn       ( cvm1_lms2ipt_en   );
        VDP_HDR_SetVCvmIpt2lmsEn       ( cvm1_ipt2lms_en   );//by
        VDP_HDR_SetVCvmLms2rgbEn       ( cvm1_lms2rgb_en   );//by
        VDP_HDR_SetVCvmRgb2yuvEn       ( cvm1_rgb2yuv_en   );//by
        ...
        if (stHdrCfg->bGfxEn)
        {
            VDP_DRV_SetGdmgammaCoef(gu16G_IM_GmmLutHlg2Hlg);
            VDP_HDR_SetGdmParaUpd(VDP_LAYER_GP0, VDP_GP_PARA_HDR_GMM);
            VDP_DRV_SetHisiGdmHdr10InHlgOut(stHdrCfg);
        }
    ...
    else if (stHdrCfg->enHdrMode == VDP_HIHDR_HLG_IN_HDR10_OUT )
    {
        ...
        if (stHdrCfg->bGfxEn)
        {
            VDP_DRV_SetGdmgammaCoef(gdm_gmm_hihdrlut);
            VDP_HDR_SetGdmParaUpd(VDP_LAYER_GP0, VDP_GP_PARA_HDR_GMM);
            VDP_DRV_SetHisiGdmHlgInHdr10Out(stHdrCfg);
        }
    ...
    else if (stHdrCfg->enHdrMode == VDP_HIHDR_HLG_IN_SDR_OUT)
    {
        ...
        //VDP_DRV_SetHisiDmCvmCoef(gs16TmIHlg2Sdr, gs16TmSHlg2Sdr, gs16SmIHlg2Sdr, gs16SmSHlg2Sdr);
        VDP_DRV_SetHisiDmCvmCoef(pstHisiHdrTmCfg->stPQHdrTm.s16aTMLutI, pstHisiHdrTmCfg->stPQHdrTm.s16aTMLutS, pstHisiHdrTmCfg->stPQHdrTm.s16aSMLutI, pstHisiHdrTmCfg->stPQHdrTm.s16aSMLutS);
        ...
        if (stHdrCfg->bGfxEn)
        {
            VDP_DRV_SetGdmgammaCoef(gu16G_IM_GmmLutHlg2SdrYuv);
            VDP_HDR_SetGdmParaUpd(VDP_LAYER_GP0, VDP_GP_PARA_HDR_GMM);
            VDP_DRV_SetHisiGdmHlgInSdrOut(stHdrCfg);
        }
        VDP_HDR_SetGDmEn               ( 0            );
    ...
    VDP_VID_SetLayerGalpha(VDP_LAYER_VID0, stHdrCfg->u32Alpha);
...
HI_VOID VDP_DRV_HdrRegClean(HI_VOID)
{
    ...
    VDP_VID_SetMuteEnable(VDP_LAYER_VID1, HI_FALSE);
