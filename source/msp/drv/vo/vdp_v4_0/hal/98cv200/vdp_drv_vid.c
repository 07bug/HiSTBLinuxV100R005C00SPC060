...
HI_S32 VDP_DRV_SetVidLayer(HI_U32 u32LayerId, VDP_LAYER_VID_INFO_S *pstInfo)
{
    ...
    HI_BOOL bPalNtscZme2Close = HI_FALSE;
    //-------for ec csc---------//
    HI_U32 u32Pre   = 0;
    HI_U32 u32DcPre  = 0;
    ...
    {
        VDP_DRV_SetLayerLetterboxBkg(u32LayerId, pstInfo->bRegionEn);
    }
    ...
    else if ((u32LayerId == VDP_LAYER_VID1) && (pstInfo->bRegionEn == HI_FALSE))
    {
        ...
        /* V1 enable or disable will influence the letterbox color of V0 layer. */
        VDP_DRV_SetLayerLetterboxBkg(u32LayerId, VDP_VID_CheckRegionState(u32LayerId));
    ...
    if ((u32LayerId == VDP_LAYER_VID0) || (u32LayerId == VDP_LAYER_VID1))
    {
        //printk("w-%d\n",pstInfo->stVdpDispInfo.stFmtResolution.u32Wth);
            ...
            || ((pstInfo->stVdpDispInfo.bInterlace  == 1) && (pstInfo->stVdpDispInfo.stFmtResolution.u32Wth == 720 ) && (pstInfo->stVdpDispInfo.stFmtResolution.u32Hgt  == 480)))
        {
            ...
            if (pstInfo->stSrcInfo.stSrcRect.u32Wth > 1920 || pstInfo->stSrcInfo.stSrcRect.u32Hgt > 1080 )
            {
                bPalNtscZme2Close = HI_TRUE;
            }
        }
    }
    ...
    stZmeInfo.stZme2InRect   = pstInfo->stZme2InRect;
    ...
    memcpy(&stZmeInfo.stZmeDrvPara, &pstInfo->stZmeDrvPara,  sizeof(VDP_VZME_DRV_PARA_S));
    ...
    if ((u32LayerId == VDP_LAYER_VID0) || (u32LayerId == VDP_LAYER_VID1))
    {
        ...
        {
            if ((u32LayerId == VDP_LAYER_VID0) && (HI_FALSE == bPalNtscZme2Close))
            {
    ...
    if (VDP_LAYER_VID3 != u32LayerId)
    {
        ...
        VDP_VP_SetLayerGalpha          (VDP_LAYER_VP0, pstInfo->u32AllAlpha);
        //printk("vp-- %d\n",pstInfo->u32AllAlpha);
    }

    //printk("lay%d-- %d\n",u32LayerId,pstInfo->u32Alpha);
    VDP_VID_SetLayerGalpha(u32LayerId, pstInfo->u32Alpha);
    ...
    if (VDP_LAYER_VID3 != u32LayerId)
    {
        stColorSpaceInfo.bSourceIsSdr  = HI_TRUE;
        stColorSpaceInfo.enInClrSpace  = pstInfo->enInClrSpace;
        stColorSpaceInfo.enOutClrSpace = pstInfo->enOutClrSpace;

        /* 8bit->10bit */
        pstInfo->stMixvColor.u32BkgY = pstInfo->stMixvColor.u32BkgY << 2;
        pstInfo->stMixvColor.u32BkgU = pstInfo->stMixvColor.u32BkgU << 2;
        pstInfo->stMixvColor.u32BkgV = pstInfo->stMixvColor.u32BkgV << 2;

        /* set the mixv background color under RGB colorspace.NOTE:mixv is immediate register. */
        VDP_CBM_SetMixerBkg(VDP_CBM_MIXV0, pstInfo->stMixvColor);

        if(HI_TRUE == pstInfo->bR2YClosed)
        {
            ...
            pstInfo->stMixvColor.u32BkgY = 0x80;
            pstInfo->stMixvColor.u32BkgU = 0x200;
            pstInfo->stMixvColor.u32BkgV = 0x200;
                                 ...
                                 VDP_VID_IFMT_E   enInZmeFmt)
{
    ...
    if ((0 == pstZmeInfo->stInRect.u32Wth)
        || (0 == pstZmeInfo->stInRect.u32Hgt))
    {
...
HI_S32 VDP_DRV_SetVidZme(HI_U32 u32LayerId, VDP_DRV_ZME_INFO_S *pstZmeInfo)
{
    ...
    if (pstZmeInfo->stInRect.u32Hgt >= stZme1Out.u32Hgt)
    {
    ...
    if (pstZmeInfo->stInRect.u32Wth >= stZme1Out.u32Wth)
    {
    ...
    VDP_VID_SetZmeFirEnable            (u32LayerId, VDP_ZME_MODE_HORL, pstZmeInfo->stZmeDrvPara.bZmeFirHL);
    VDP_VID_SetZmeFirEnable            (u32LayerId, VDP_ZME_MODE_HORC, pstZmeInfo->stZmeDrvPara.bZmeFirHC);
    VDP_VID_SetZmeMidEnable            (u32LayerId, VDP_ZME_MODE_HOR, pstZmeInfo->stZmeDrvPara.bZmeMedH);
    ...
    VDP_VID_SetZmeFirEnable            (u32LayerId, VDP_ZME_MODE_VERL, pstZmeInfo->stZmeDrvPara.bZmeFirVL);
    VDP_VID_SetZmeFirEnable            (u32LayerId, VDP_ZME_MODE_VERC, pstZmeInfo->stZmeDrvPara.bZmeFirVC);
    VDP_VID_SetZmeMidEnable            (u32LayerId, VDP_ZME_MODE_VER, pstZmeInfo->stZmeDrvPara.bZmeMedV);

    if ((3840 <= pstZmeInfo->stFmtResolution.u32Wth)
        && (2160 == pstZmeInfo->stFmtResolution.u32Hgt)
        && (1920 < pstZmeInfo->stInRect.u32Wth)
        && (1080 < pstZmeInfo->stInRect.u32Hgt))
    {
...
#ifdef VDP_PQ_STRATEGY
    pstZmeInfo->stZmeDrvPara.u32ZmeFrmWIn  = u32ZmeWth;
    pstZmeInfo->stZmeDrvPara.u32ZmeFrmWOut = stZme1Out.u32Wth;
    pstZmeInfo->stZmeDrvPara.u32ZmeFrmHIn  = u32ZmeInResoHeight;
    pstZmeInfo->stZmeDrvPara.u32ZmeFrmHOut = stZme1Out.u32Hgt;
    ...
    pstZmeInfo->stZmeDrvPara.u8ZmeYCFmtIn  = u32InFmtTmp;
    pstZmeInfo->stZmeDrvPara.u8ZmeYCFmtOut = 0;
    pstZmeInfo->stZmeDrvPara.u8ZmeYCFmtIn  = u32InFmtTmp;
    pstZmeInfo->stZmeDrvPara.u8ZmeYCFmtOut = 0;
    VdpZmeComnSet(&pstZmeInfo->stZmeDrvPara, &pstZmeRtlPara);
#else
    //s32ZmePhase = 0;
#endif
    ...
    {
        VDP_VID_SetZmeVerRatio         (u32LayerId, ZME_VPREC);
    }
    else
    {

        if (0 == stZme1Out.u32Hgt)
        {
            VDP_PRINT("Denominator Is Zero !!!!\n");
            return HI_FAILURE;
        }
...
HI_S32 VDP_DRV_SetVidCsc(HI_U32 u32LayerId, VDP_CLR_SPACE_E enInClr, VDP_CLR_SPACE_E enOutClr)
{
    ...
    if ((VDP_LAYER_VID3 == u32LayerId)
        && ((VDP_CLR_SPACE_YUV_2020_L == enInClr)
            || (VDP_CLR_SPACE_YUV_2020   == enInClr)
            || (VDP_CLR_SPACE_RGB_2020_L == enInClr)
            || (VDP_CLR_SPACE_RGB_2020   == enInClr)))
    {
        enInClr = VDP_CLR_SPACE_YUV_709_L;
    }

    //printk("--in--%d-out %d---\n",enInClr,enOutClr);
    if ((VDP_CLR_SPACE_BUTT <= enInClr) || (VDP_CLR_SPACE_BUTT <= enOutClr))
    {
        VDP_PRINT("enInClr->%d or enOutClr->%d value is invalid!\n",enInClr, enOutClr);
        return HI_FAILURE;
    }
    ...
    switch(enInClr)
    {
        ...
        case VDP_CLR_SPACE_YUV_601_L:
            enInClr = VDP_CLR_SPACE_YUV_601_L;
            break;
    ...
    if ((VDP_CLR_SPACE_YUV_601 == enOutClr) || (VDP_CLR_SPACE_YUV_601_L == enOutClr))
    {
        enOutClr = VDP_CLR_SPACE_YUV_601_L;
    }
    else if ((VDP_CLR_SPACE_YUV_709 == enOutClr) || (VDP_CLR_SPACE_YUV_709_L == enOutClr))
    {
        enOutClr = VDP_CLR_SPACE_YUV_709_L;
    }
    else if ((VDP_CLR_SPACE_YUV_2020 == enOutClr) || (VDP_CLR_SPACE_YUV_2020_L == enOutClr))
    {
        enOutClr = VDP_CLR_SPACE_YUV_2020_L;
    }
    else
    {
    ...
    else if (enInClr == VDP_CLR_SPACE_YUV_709 && enOutClr == VDP_CLR_SPACE_YUV_601_L)
    {
        enCscMode  = VDP_CSC_YUV2YUV_709_F_601_L   ;
        enPqCscMode = HI_PQ_CSC_YUV2YUV_709_F_601_L;
    }
    ...
    else if (enInClr == VDP_CLR_SPACE_RGB_709 && enOutClr == VDP_CLR_SPACE_YUV_601_L)
    {
        enCscMode = VDP_CSC_RGB2YUV_601             ; /* RGB       -> YCbCr 601 */
        enPqCscMode = HI_PQ_CSC_RGB2YUV_601             ; /* RGB       -> YCbCr 601 */
    }
    ...
    {
        enCscMode = VDP_CSC_YUV2YUV              ; /* YCbCr L      -> YCbCr L */
        enPqCscMode = HI_PQ_CSC_YUV2RGB_2020_601_L              ; /* YCbCr L      -> YCbCr L */
        enCsc2Mode = VDP_CSC_RGB2YUV_601;
    ...
    else
    {
            ...
            && (enPqCscMode != HI_PQ_CSC_YUV2RGB_2020_709_L))
        {
            ...
            VDP_HDR_SetVDmImapYuv2rgbscale2p (13);//here let this be 13 to fix bt2020 pip color issues.
        }
    }
