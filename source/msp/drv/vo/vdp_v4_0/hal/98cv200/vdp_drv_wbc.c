...
HI_S32 VDP_DRV_SetWbcZme(HI_U32 u32LayerId, VDP_DRV_WBC_ZME_INFO_S *pstZmeInfo)
{
    ...
    //WBC zme setting
    //--------------------------------------------
    VDP_WBC_SetZmeEnable           (u32LayerId, VDP_ZME_MODE_HOR, HI_TRUE);
    VDP_WBC_SetZmeEnable           (u32LayerId, VDP_ZME_MODE_VER, HI_TRUE);
    ...
    {
        VDP_WBC_SetHorZmeEnable        (u32LayerId, HI_TRUE);
        VDP_WBC_SetHorZmeMidEnable     (u32LayerId, HI_TRUE);
        VDP_WBC_SetHorZmeFirEnable     (u32LayerId, HI_TRUE);
        u32Ration = 2;
    }
    else
    {
        VDP_WBC_SetHorZmeEnable        (u32LayerId, HI_FALSE);
    }


    if ((pstZmeInfo->enWbcVpConnPoint == VDP_LAYER_CONN_VIDBZME) && (u32LayerId == VDP_LAYER_WBC_VP0))
    {
    ...
    {
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_HOR, 0);
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_VERL, 0);
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_VERC, 0);
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_VERBL, 0);
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_VERBC, 0);
    }
    else
    {
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_HOR, 0);
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_VERL, pstZmeRtlPara.s32ZmeOffsetVL);
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_VERC, pstZmeRtlPara.s32ZmeOffsetVC);
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_VERBL, pstZmeRtlPara.s32ZmeOffsetVLBtm);
        VDP_WBC_SetZmePhase            (u32LayerId, VDP_ZME_MODE_VERBC, pstZmeRtlPara.s32ZmeOffsetVCBtm);
    }

    if (0 == pstZmeInfo->stOutRect.u32Wth
        || 0 == pstZmeInfo->stOutRect.u32Hgt)
    {
        VDP_PRINT("Denominator Is Zero !!!!\n");
        return HI_FAILURE;
    }

    VDP_WBC_SetZmeHorRatio         (u32LayerId, ((pstZmeInfo->stInRect.u32Wth / u32Ration)*ZME_HPREC) / pstZmeInfo->stOutRect.u32Wth);
    VDP_WBC_SetZmeVerRatio         (u32LayerId, (pstZmeInfo->stInRect.u32Hgt * ZME_VPREC) / pstZmeInfo->stOutRect.u32Hgt);
    ...
    if (HI_NULL == gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_WBC_ZMEH]
        || HI_NULL == gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_WBC_ZMEV]
        || HI_NULL == gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_ZME_TMP])
    {
        VDP_PRINT("wbczme coef PTR Is NUll Please check it !!! \n");
        return HI_FAILURE;
    }
    ...
    DRV_PQ_GetVdpZmeCoef(enRationVL, HI_PQ_ZME_COEF_TYPE_LV, HI_PQ_ZME_TAP_4T32P, (HI_S16 *)(gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_ZME_TMP]));
    ...
    VDP_WBC_SetZmeCoefAddr(u32LayerId, VDP_WBC_PARA_ZME_HOR , (gstVdpCoefBufAddr.u32CoefPhyAddr[VDP_COEF_BUF_WBC_ZMEH]));
    VDP_WBC_SetZmeCoefAddr(u32LayerId, VDP_WBC_PARA_ZME_VER , (gstVdpCoefBufAddr.u32CoefPhyAddr[VDP_COEF_BUF_WBC_ZMEV]));
    VDP_WBC_SetParaUpd(u32LayerId, VDP_WBC_PARA_ZME_HOR);
    VDP_WBC_SetParaUpd(u32LayerId, VDP_WBC_PARA_ZME_VER);
...
HI_S32 VDP_DRV_SetWbcDitherMode(HI_U32 u32LayerId, VDP_WBC_DITHER_MODE_E enDitherMode)
{
    ...
    {
        VDP_WBC_SetDitherEnable(u32LayerId, HI_TRUE);
        VDP_WBC_SetDitherRoundEnable(u32LayerId, HI_TRUE); //must to 1 自动比对必须为1；
        VDP_WBC_SetDitherMode(u32LayerId, 1); //must to 1 自动比对必须为1；
    }
    else
    {
        ;
...
HI_S32 VDP_DRV_SetWbcLayer(HI_U32 u32LayerId, VDP_LAYER_WBC_INFO_S *pstInfo)
{
    ...
    VDP_RECT_S st_crop = {0};

    if (HI_NULL == pstInfo)
    {
        VDP_PRINT("VDP_LAYER_WBC_INFO_S is NULL !!!\n");
        return HI_FAILURE;
    }

    st_crop = pstInfo->stInRect;

    memset(&stZmeInfo, 0x0, sizeof(VDP_DRV_WBC_ZME_INFO_S));


    VDP_WBC_SetEnable              (u32LayerId, pstInfo->bWbcLayerEn);
    ...
    {
        VDP_WBC_SetProToInterEnable(u32LayerId, HI_TRUE);
    }
    else
    {
        VDP_WBC_SetProToInterEnable(u32LayerId, HI_FALSE);
    }


    //VDP_WBC_SetCscEnable(u32LayerId, pstInfo->bCscEnable);
    //VDP_WBC_SetCscMode (u32LayerId, pstInfo->enCscMode);


    //VDP_WBC_SetOutIntf             (enLayerId, pstCfg->enReadMode);
    VDP_WBC_SetOutFmt              (u32LayerId, pstInfo->enOutFmt);
    if (u32LayerId == VDP_LAYER_WBC_VP0)
    {
        VDP_WBC_SetLayerAddr           (u32LayerId, pstInfo->stAddr[VDP_ADDR_WBCVP0].u32LumAddr, pstInfo->stAddr[VDP_ADDR_WBCVP0].u32ChmAddr, pstInfo->stAddr[VDP_ADDR_WBCVP0].u32LumStr, pstInfo->stAddr[VDP_ADDR_WBCVP0].u32ChmStr);
    }
    else if (u32LayerId == VDP_LAYER_WBC_HD0)
    {
        VDP_WBC_SetLayerAddr           (u32LayerId, pstInfo->stAddr[VDP_ADDR_WBCDHD0].u32LumAddr, pstInfo->stAddr[VDP_ADDR_WBCDHD0].u32ChmAddr, pstInfo->stAddr[VDP_ADDR_WBCDHD0].u32LumStr, pstInfo->stAddr[VDP_ADDR_WBCDHD0].u32ChmStr);
    }

    VDP_WBC_SetCropPos             (u32LayerId, st_crop);
    VDP_WBC_SetOutIntf             (u32LayerId, pstInfo->u32OutIop);
    VDP_WBC_SetOutReso             (u32LayerId, pstInfo->stOutRect);

    //VDP_WBC_SetDitherMode          (enLayerId, VDP_DITHER_ROUND_8);
    VDP_DRV_SetWbcDitherMode(u32LayerId, pstInfo->enWbcDitherMode);

    VDP_WBC_SetThreeMd                 (u32LayerId, 0);
    VDP_WBC_SetSmmuCBypass(u32LayerId, !pstInfo->bSmmu);
    VDP_WBC_SetSmmuLBypass(u32LayerId, !pstInfo->bSmmu);
    ...
    if (u32LayerId == VDP_LAYER_WBC_HD0 )
    {
        VDP_WBC_SetCdsEnable           (u32LayerId, HI_TRUE);
        VDP_WBC_SetCdsFirEnable        (u32LayerId, HI_TRUE);
        VDP_WBC_SetCdsMidEnable        (u32LayerId, HI_TRUE);
        VDP_WBC_SetCdsCoef             (u32LayerId, s32CdsCoef);
    }
    else if (u32LayerId == VDP_LAYER_WBC_VP0)
    {
        VDP_WBC_SetWbcVpConnection(u32LayerId, pstInfo->enWbcVpConnPoint);
        ...
        {
            VDP_WBC_SetCdsEnable           (u32LayerId, HI_FALSE);
        }
    ...
    VDP_DRV_SetWbcZme(u32LayerId, &stZmeInfo);
    VDP_WBC_SetRegUp(u32LayerId);
