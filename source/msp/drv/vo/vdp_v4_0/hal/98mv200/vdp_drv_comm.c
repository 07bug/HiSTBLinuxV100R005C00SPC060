...
#if (VDP_CBB_FPGA &&  !defined(__DISP_PLATFORM_BOOT__))
    HI_S32 nRet;
    nRet = HI_DRV_MMZ_AllocAndMap(HAL_VDP_DDR_NAME, HI_NULL, u32Size, 0, (MMZ_BUFFER_S *)stVdpMmzBuffer);
    if (nRet != 0)
    {
...
HI_S32 VDP_DRV_CoefBufAddrDistribute(VDP_COEF_ADDR_S *stVdpCoefBufAddr)
{
    ...
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_ZME_V1_H]  = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_ZME2V] + COEF_SIZE_ZMEV        ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_ZME_V1_V]  = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_ZME_V1_H] + COEF_SIZE_ZMEH  ;


    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_ZME_TMP] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_ZME_V1_V] + COEF_SIZE_ZMEV        ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_C6_V0]  = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_ZME_TMP] + COEF_SIZE_ZME_TMP        ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_C6_V1]  = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_C6_V0] + COEF_SIZE_C6_V0;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_C6_VP0] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_C6_V1] + COEF_SIZE_C6_V1        ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_WBC_ZMEH]  = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_C6_VP0] + COEF_SIZE_C6_VP0;
    ...
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_VDM_DEGAMMA] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_WBC_ZMEV]  + COEF_SIZE_WBC_ZMEV           ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_VDM_GAMMA] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_VDM_DEGAMMA] + COEF_SIZE_VDM_DEGAMMA      ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_VDM_TONEMAP] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_VDM_GAMMA] + COEF_SIZE_VDM_GAMMA       ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_GDM_GAMMA2] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_VDM_TONEMAP] + COEF_SIZE_GDM_GAMMA2      ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_GDM_TONEMAP] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_GDM_GAMMA2] + COEF_SIZE_GDM_TONEMAP       ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_GCVM] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_GDM_TONEMAP] + COEF_SIZE_GDM_TONEMAP ;
    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_HDR_METADATA] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_GCVM] + COEF_SIZE_GDM_TONEMAP ;

    stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_MUTE_V1_L] = stVdpCoefBufAddr->pu8CoefVirAddr[VDP_COEF_BUF_HDR_METADATA] + COEF_SIZE_HDR_METADATA ;
    ...
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_ZME_V1_H]  = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_ZME2V] + COEF_SIZE_ZMEV        ;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_ZME_V1_V]  = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_ZME_V1_H] + COEF_SIZE_ZMEH  ;



    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_ZME_TMP]  = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_ZME_V1_V] + COEF_SIZE_ZMEV        ;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_C6_V0]  = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_ZME_TMP] + COEF_SIZE_ZME_TMP        ;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_C6_V1]  = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_C6_V0] + COEF_SIZE_C6_V0;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_C6_VP0] = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_C6_V1] + COEF_SIZE_C6_V1        ;

    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_WBC_ZMEH]  = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_C6_VP0] + COEF_SIZE_C6_VP0;
    ...
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_VDM_DEGAMMA]  = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_WBC_ZMEV] + COEF_SIZE_WBC_ZMEV   ;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_VDM_GAMMA] = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_VDM_DEGAMMA] + COEF_SIZE_VDM_DEGAMMA        ;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_VDM_TONEMAP] = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_VDM_GAMMA] + COEF_SIZE_VDM_GAMMA         ;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_GDM_GAMMA2] = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_VDM_TONEMAP] + COEF_SIZE_GDM_GAMMA2        ;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_GDM_TONEMAP] = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_GDM_GAMMA2] + COEF_SIZE_GDM_TONEMAP         ;

    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_GCVM] = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_GDM_TONEMAP] + COEF_SIZE_GDM_TONEMAP ;
    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_HDR_METADATA] = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_GCVM] + COEF_SIZE_GDM_TONEMAP ;


    stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_MUTE_V1_L] = stVdpCoefBufAddr->u32CoefPhyAddr[VDP_COEF_BUF_HDR_METADATA] + COEF_SIZE_HDR_METADATA ;
