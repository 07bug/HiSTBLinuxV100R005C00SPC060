...
HI_S32 VDP_DRV_GetHiHdrCfg(HI_U32 u32LayerId, XDP_LAYER_VID_INFO_S *pstInfoOut, HI_PQ_HDR_CFG  *pstPqHdrCfg, HIHDR_SCENE_MODE_E enSceneMode, DmCfg_t  *pstDmCfg)
{
    ...
    if (HI_NULL == pstPqHdrCfg || u32LayerId == VDP_LAYER_VID3 || HI_NULL == pstDmCfg)
    {
        return HI_FAILURE;
    }
...
HI_S32 XDP_DRV_SetVidBkg(HI_U32 u32LayerId, XDP_LAYER_VID_INFO_S *pstInfoOut)
{
    ...
    if (HI_TRUE == pstInfoOut->stXdpLayerExtern.bCloseY2R)
    {
        VDP_VID_SetVdmVdmRgb2yuvEn(VDP_LAYER_VID0, HI_FALSE);
        pstInfoOut->stXdpBkgCfg.stMixvColor.u32BkgY = 0x80;
        pstInfoOut->stXdpBkgCfg.stMixvColor.u32BkgU = 0x200;
        pstInfoOut->stXdpBkgCfg.stMixvColor.u32BkgV = 0x200;
...
HI_S32 XDP_DRV_SetVidCsc(HI_U32 u32LayerId, XDP_CSC_CFG_S  stXdpCscCfg[HI_PQ_CSC_TYPE_BUUT])
{
    ...
    if (u32LayerId == VDP_LAYER_VID3)
    {
        ...
        VDP_VID_SetCscCoef            (u32LayerId, stXdpCscCfg[HI_PQ_CSC_TUNING_V3].stPqCscCoef);
        VDP_VID_SetCscDcCoef          (u32LayerId, stXdpCscCfg[HI_PQ_CSC_TUNING_V3].stPqCscDc);
    }
    else
    {
