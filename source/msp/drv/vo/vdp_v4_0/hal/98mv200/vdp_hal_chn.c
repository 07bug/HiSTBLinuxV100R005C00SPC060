...
HI_U32  VDP_DISP_SetReadCheckSumY  (HI_VOID)
{
    ...
    //U_INTF1_CHKSUM_Y INTF1_CHKSUM_Y;
    VDP_PRINT("line : u32Checksum == 0x%08x\n", u32Checksum);

    u32Checksum = VDP_RegRead(((HI_ULONG) & (pVdpReg->INTF_CHKSUM_Y.u32))); // + VDP_RegRead((&(pVdpReg->INTF1_CHKSUM_Y.u32)));
...
HI_U32  VDP_DISP_SetReadCheckSumU  (HI_VOID)
{
    ...
    //U_INTF1_CHKSUM_U INTF1_CHKSUM_U;

    u32Checksum = VDP_RegRead(((HI_ULONG) & (pVdpReg->INTF_CHKSUM_U.u32))) ; //+ VDP_RegRead((&(pVdpReg->INTF1_CHKSUM_U.u32)));
    return u32Checksum;
}
HI_U32  VDP_DISP_SetReadCheckSumV  (HI_VOID)
{
    ...
    //U_INTF1_CHKSUM_V INTF1_CHKSUM_V;

    u32Checksum = VDP_RegRead(((HI_ULONG) & (pVdpReg->INTF_CHKSUM_V.u32))); // + VDP_RegRead((&(pVdpReg->INTF1_CHKSUM_V.u32)));
    return u32Checksum;
}
...
HI_VOID VDP_RST_FILE(char *fpName)
{
    ...
    strncpy(fpSumRst, FILE_PATH, sizoef(fpSumRst) - 1);
    fpSumRst[79] = '\0';
