...
#if EDA_TEST
    u_apb_drv->apb_write(a, b); //cfg to rtl with apb
    *Reg = b;       // ENV cfg
#else
    *Reg = b;       // ENV cfg
#endif
...
{
#if( EDA_TEST & !BT_TEST  & !VDP_C_COMPILE)
    HI_U32 Val = 0;
    u_ahb_drv->read_ahb(a, &Val);
    //cout << "ReadWord  : AHB addr =" ;
    //cout << hex << setw(8) << setfill('0') << a ;
    //cout << " : ReadData  = " ;
    //cout << hex << setw(8) << setfill('0') << Val << endl;
...
HI_VOID VDP_SetArbMode(HI_U32 u32Data, HI_U32 u32bMode)
{
    ...
    if (u32Data == VDP_MASTER0)
    {
        ...
        // old mode -> 0:Poll Mode,1:GFX First Mode
        // m0_arb_mode[3]  : 0->Arb 1->Poll
        // m0_arb_mode[2:0]: 0->Arb 3->Poll
        VOCTRL.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->VOCTRL.u32)));
...
HI_VOID VDP_CBM_GetMixerBkg(VDP_CBM_MIX_E u32mixer_id, VDP_BKG_S *pstBkg)
{
    ...
    if (u32mixer_id == VDP_CBM_MIX0)
    {
        /* DHD0  mixer link */
        CBM_BKG1.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->CBM_BKG1.u32)));
        pstBkg->u32BkgY = CBM_BKG1.bits.cbm_bkgy1;
        pstBkg->u32BkgU = CBM_BKG1.bits.cbm_bkgcb1;
        pstBkg->u32BkgV = CBM_BKG1.bits.cbm_bkgcr1;
    }
    else if (u32mixer_id == VDP_CBM_MIX1)
    {
        CBM_BKG2.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->CBM_BKG2.u32)));
        pstBkg->u32BkgY = CBM_BKG2.bits.cbm_bkgy2;
        pstBkg->u32BkgU = CBM_BKG2.bits.cbm_bkgcb2;
        pstBkg->u32BkgV = CBM_BKG2.bits.cbm_bkgcr2;
    }
    else if (u32mixer_id == VDP_CBM_MIX2)
    {
        /* DHD1 mixer link */
        CBM_BKG3.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->CBM_BKG3.u32)));
        pstBkg->u32BkgY = CBM_BKG3.bits.cbm_bkgy3;
        pstBkg->u32BkgU = CBM_BKG3.bits.cbm_bkgcb3;
        pstBkg->u32BkgV = CBM_BKG3.bits.cbm_bkgcr3;
    }
#if SD0_EN
    else if (u32mixer_id == VDP_CBM_MIX2)
    {
        ...
        /* DSD mixer link */
        MIXDSD_BKG.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->MIXDSD_BKG.u32)));
        pstBkg->u32BkgY = MIXDSD_BKG.bits.mixer_bkgy;
        pstBkg->u32BkgU = MIXDSD_BKG.bits.mixer_bkgcb;
        pstBkg->u32BkgV = MIXDSD_BKG.bits.mixer_bkgcr;
    ...
    else if (u32mixer_id == VDP_CBM_MIXG0)
    {
        ...
        /* G0 mixer link */
        MIXG0_BKG.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->MIXG0_BKG.u32)));
        pstBkg->u32BkgY = MIXG0_BKG.bits.mixer_bkgy;
        pstBkg->u32BkgU = MIXG0_BKG.bits.mixer_bkgcb;
        pstBkg->u32BkgV = MIXG0_BKG.bits.mixer_bkgcr;

        MIXG0_BKALPHA.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->MIXG0_BKALPHA.u32)));
        pstBkg->u32BkgA = MIXG0_BKALPHA.bits.mixer_alpha;
    }
    else if (u32mixer_id == VDP_CBM_MIXV0)
    {
        U_MIXV0_BKG MIXV0_BKG;
        MIXV0_BKG.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->MIXV0_BKG.u32)));
        pstBkg->u32BkgY = MIXV0_BKG.bits.mixer_bkgy;
        pstBkg->u32BkgU = MIXV0_BKG.bits.mixer_bkgcb;
        pstBkg->u32BkgV = MIXV0_BKG.bits.mixer_bkgcr;
    }
#endif
    else
    {
        VDP_PRINT("Error! VDP_CBM_GetMixerBkg() Select Wrong mixer ID\n");
    }
...
HI_VOID VDP_SetTwoChnDbgEnable(HI_U32 bEnable)
{
    ...
    DHD0_CTRL.bits.twochn_debug = bEnable;//in HiFoneB02 twochn_mode = 1 only
...
HI_VOID VDP_SetGdcArbMode(HI_U32 u32Data)
{
    ...
    VOCTRL.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->VOCTRL.u32)));
    VOCTRL.bits.grc_arb_mode = u32Data;
    VDP_RegWrite(((HI_ULONG) & (pVdpReg->VOCTRL.u32)), VOCTRL.u32);
}
