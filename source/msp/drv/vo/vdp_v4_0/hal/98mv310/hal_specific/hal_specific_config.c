...
HI_S32 DISP_HAL_GetPortIntValidAndWbcLineNum(HI_BOOL *pbInterruptValid, HI_U32 *pu32WbcLineNum)
{
    ...
    HI_U32 u32PartFnsLineNum = 0;
    HI_BOOL bVblank = HI_FALSE;

    VDP_DISP_GetDHD0VblankState(&bVblank);
    VDP_WBC_GetWbcDHD0StateLineNum(pu32WbcLineNum);
    VDP_WBC_GetWbcDHD0PartFnsLineNum(&u32PartFnsLineNum);

    if((HI_TRUE == bVblank) || (*pu32WbcLineNum < u32PartFnsLineNum))
    {
        *pbInterruptValid = HI_FALSE;
    }
    else
    {
        *pbInterruptValid = HI_TRUE;
    }
#endif
