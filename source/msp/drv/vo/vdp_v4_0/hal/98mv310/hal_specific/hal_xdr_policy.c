...
static DRV_DISP_XDR_HARDWARE_CAP_S g_stXdrHardwareCap = {HI_TRUE,HI_TRUE,HI_FALSE,HI_FALSE,HI_FALSE};

#else
static DRV_DISP_XDR_HARDWARE_CAP_S g_stXdrHardwareCap = {HI_FALSE,HI_FALSE,HI_FALSE,HI_FALSE,HI_FALSE};
#endif
...
HI_BOOL XDR_JudgeWinRectCover(HI_RECT_S *pstUpwinOutRect,HI_RECT_S *pstDowinOutRect )
{
	HI_RECT_S stUpwinOutRect = *pstUpwinOutRect;
	HI_RECT_S stDowinOutRect = *pstDowinOutRect;

	if ((stUpwinOutRect.s32X <= stDowinOutRect.s32X)
        && (stUpwinOutRect.s32Y <= stDowinOutRect.s32Y)
        && ((stUpwinOutRect.s32X + stUpwinOutRect.s32Width) >= (stDowinOutRect.s32X + stDowinOutRect.s32Width))
        && ((stUpwinOutRect.s32Y + stUpwinOutRect.s32Height) >= (stDowinOutRect.s32Y + stDowinOutRect.s32Height)))
    {
...
HI_VOID XDR_SPECIFIC_AdjustWinMask( WIN_SHARE_INFO_S *pstWinShareInfo )
{
    HI_U32  i = 0;
    HI_BOOL  bCover = HI_FALSE;

    WIN_INFO_S   *pTmpstWinInfo = HI_NULL;
    WIN_INFO_S   *pTmpstWinLayerV0 = HI_NULL;

    for (i = 0; i < WINDOW_MAX_NUMBER; i++)
    {
        pTmpstWinInfo = &pstWinShareInfo->stWinInfo[i];
        if (HI_NULL != pTmpstWinInfo)
        {
            if ((HI_TRUE == pTmpstWinInfo->bRegionEnable)
                && (VDP_LAYER_VID0 == pTmpstWinInfo->u32LayerId )
                && (0 != pTmpstWinInfo->u32Zorder ))
            {
                pTmpstWinLayerV0 = pTmpstWinInfo;
            }
        }
    }
    ...
    for (i = 0; i < WINDOW_MAX_NUMBER; i++)
    {
        ...
        if (HI_NULL != pTmpstWinInfo)
        {
            if ((HI_NULL != pTmpstWinLayerV0) && (HI_TRUE == pTmpstWinInfo->bRegionEnable))
            {
                if ( (pTmpstWinInfo->u32LayerId == VDP_LAYER_VID1))
                {
                    bCover = XDR_JudgeWinRectCover(&pTmpstWinLayerV0->stDisp, &pTmpstWinInfo->stDisp);
                    pTmpstWinInfo->bRegionEnable = (HI_TRUE == bCover) ? HI_FALSE : HI_TRUE;
                }
            }
        }
    }
                                       ...
                                       HI_DRV_COLOR_SPACE_E    enFmtOutColorCsc)
{
    ...
    XDR_SPECIFIC_AdjustWinMask(pstWinShareInfo);
