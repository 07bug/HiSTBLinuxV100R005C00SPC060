...
HI_VOID VDP_DATE_SetSignal(HI_DRV_DISP_INTF_ID_E enIntfId,DISP_VENC_E eDate, HI_BOOL bRGBSync)
{
    ...
    if (HI_DRV_DISP_INTF_YPBPR0 == enIntfId)
    {
        if (DISP_VENC_HDATE0 ==  eDate )
        {
            HI_ERR_DISP("98mv310 don't support Hdate\n");
        }
        else if (DISP_VENC_SDATE0 == eDate )
        {
            U_DATE_COEFF0 DATE_COEFF0;
...
HI_VOID VDP_VDAC_SetLink(DISP_VENC_E eDate, HI_U32 uVdac, HI_DRV_DISP_VDAC_SIGNAL_E signal)
{
    ...
    //HI_ERR_DISP("VDP_VDAC_SetLink date=%d, dac=%d, sig=%d\n", eDate, uVdac, signal);
...
HI_VOID SDATE_HDATE_VBI_SetCgmsTypeAEn(DISP_VENC_E enChanel,HI_BOOL bEnTypeA)
{
    ...
    {
        HI_INFO_DISP("98mv310 don't support Hdate\n");
    }
...
HI_S32 SDATE_MVCN_DATE_SetMcvn(HI_U32 *pu32Value)
{
   //VDP_MVCN_DATE_SetMcvn
    ...
    HI_INFO_DISP("DATE_COEFF2-6= %x %x %x %x %x %x\n",
