...
static HI_VOID ADAPTER_TransferAddrToHalAddr(HI_DRV_FRAME_TYPE_E eFrmType,
...
static HI_BOOL ADAPTER_IsLastDolbyFrame(HI_DRV_VIDEO_FRAME_S *pstCurrentFrm,
...
static HI_VOID ADAPTER_TransferBitWidth(HI_DRV_PIXEL_BITWIDTH_E enBitWidth,
...
static HI_S32  ADAPTER_GetZmeStrategyFromPq(HI_U32 u32LayerId,
...
static HI_VOID ADAPTER_TransferZmeStrategy(HI_BOOL bMute,
...
static HI_S32 ADAPTER_TransferWinInfoToHalCfg(WIN_INFO_S *pstWinInfo,
        VDP_LAYER_VID_INFO_S *pstVidInfo)
{
    ...
    if (!pstWinInfo->bRegionMute && pstWinInfo->bRegionEnable)
    {
        ...
        {
            DISP_ERROR("pstWinInfo->pCurrentFrame is null.\n");
            return HI_FAILURE;
        }
    ...
    pstVidInfo->stMuteColor.u32BkgY  = 0x0;
    pstVidInfo->stMuteColor.u32BkgU  = 0x0;
    pstVidInfo->stMuteColor.u32BkgV  = 0x0;
#else
...
{
    VDP_LAYER_VID_INFO_S stVidInfo = {0};
...
HI_VOID ADAPTER_SetGfxXdrMode(HI_DRV_DISP_OUT_TYPE_E enDispOutType, HI_DRV_COLOR_SPACE_E enOutColorSpace)
{
    //not support.
    return;
}
...
//**************************Hisi Path*******************************************
//******************************************************************************
static HI_VOID ADAPTER_TransferToHalReso(HI_RECT_S *pstDrvReso, VDP_RECT_S *pstHalReso)
{
...
static HI_VOID ADAPTER_TransferToHalAddr(HI_DRV_VID_FRAME_ADDR_S *pstDrvAddr,
...
static HI_VOID ADAPTER_GetHiHdrMode(HI_DRV_VIDEO_FRAME_TYPE_E enFrmType,
        ...
        case HI_DRV_VIDEO_FRAME_TYPE_HDR10:
            switch (enDispType)
            {
                ...
                case HI_DRV_DISP_TYPE_HLG:
                    *penHdrMode = VDP_HIHDR_HDR10_IN_HLG_OUT;
                    break;
                default:
            ...
            switch (enDispType)
            {
                ...
                case HI_DRV_DISP_TYPE_HDR10:
                    *penHdrMode = VDP_HIHDR_HLG_IN_HDR10_OUT;
                    break;
                case HI_DRV_DISP_TYPE_HLG:
                    *penHdrMode = VDP_HIHDR_HLG_IN_HLG_OUT;
                    break;
                default:
                    DISP_ERROR("Unsupport mode.\n");
                    *penHdrMode = VDP_HIHDR_HLG_IN_SDR_OUT;
...
static HI_VOID ADAPTER_TransferWinInfoToHiHdrCfg(WIN_INFO_S *pstWinInfo, VDP_HISI_HDR_CFG_S *pstHdrCfg)
{
    HI_BOOL  bUvorder = HI_FALSE, bCmpFmt = HI_FALSE, bLostCmp = HI_FALSE;
    ...
    pstHdrCfg->u32Alpha = (pstWinInfo->u32Alpha * LOGIC_MAX_ALPHA) / VDP_VIDEO_MAX_ALPHA;
    pstHdrCfg->u32AllAlpha = (pstWinInfo->u32AllAlpha * LOGIC_MAX_ALPHA) / VDP_VIDEO_MAX_ALPHA;
...
#ifdef VDP_DOLBY_HDR_SUPPORT
static HI_VOID ADAPTER_GetDolbyMode(HI_DRV_VIDEO_FRAME_S  *pstFrame,
                                    HI_DRV_DISP_OUT_TYPE_E enDispType,
...
static HI_VOID ADAPER_TransferWinInfoToDolbyCfg(WIN_INFO_S *pstWinInfo,
        VDP_HDR_CFG_S *pstHdrCfg)
{
    ...
    else
    {
        ...
        if (HI_NULL == pstWinInfo->pstDispInfo)
        {
            DISP_ERROR("pstWinInfo->pstDispInfo is null.\n");
            return;
        }
                               ...
                               HI_BOOL  bDolbyPath)
{
        ...
        && (HI_NULL != pstWinInfo->pCurrentFrame))
    {
            ...
            || (VDP_LAYER_VID1 == pstWinInfo->u32LayerId))
        {
            ...
            pstVidInfo->stXdpZmeCfg.stZme2InRect.u32Wth  =  stZmeOut.stStrategyLayerId.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME1].u32FrmOutWidth;
            pstVidInfo->stXdpZmeCfg.stZme2InRect.u32Hgt  =  stZmeOut.stStrategyLayerId.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME1].u32FrmOutHeight;

            pstVidInfo->stXdpZmeCfg.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME2].u32FrmInWidth   = stZmeOut.stStrategyLayerId.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME2].u32FrmInWidth;
            pstVidInfo->stXdpZmeCfg.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME2].u32FrmInHeight  = stZmeOut.stStrategyLayerId.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME2].u32FrmInHeight;
            pstVidInfo->stXdpZmeCfg.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME2].u32FrmOutWidth  = stZmeOut.stStrategyLayerId.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME2].u32FrmOutWidth;
            pstVidInfo->stXdpZmeCfg.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME2].u32FrmOutHeight = stZmeOut.stStrategyLayerId.stZmeFmt[HI_PQ_V0_ZME_NODE_ZME2].u32FrmOutHeight;
    ...
    else
    {
        if (pstWinInfo->bRegionMute)
        {
            stZmeIn.stLayerStrategy.u32FmtIn   = CBB_VID_IFMT_SP_420;
            stZmeIn.stLayerStrategy.u32FrmWIn  = pstWinInfo->stIn.s32Width;
            stZmeIn.stLayerStrategy.u32FrmHIn  = pstWinInfo->stIn.s32Height;
            stZmeIn.stLayerStrategy.eFrostLevel = HI_PQ_FROST_LEVEL_CLOSE;
...
HI_VOID ADAPTER_SetLayerCfg(HI_U32 u32LayerId,
                            ...
                            HI_BOOL bConfigWholeLayer)
{
    static  XDP_LAYER_VID_INFO_S stLayerInfoIn = {{0}};
    HI_PQ_HDR_CFG  *pstPqHdrCfg = HI_NULL;
    HI_U32 u32Index = 0;
    ...
    ADAPTER_TransferWinInfoToHalCfg(pstWinInfo, &stLayerInfoIn, HI_FALSE);
    ...
    if (HI_NULL != pstWinInfo->pCurrentFrame)
    {
        ...
        if (DRV_XDR_ENGINE_DOLBY == pstDispBufNode->enXdrEngine)
        {
            ...
            stLayerInfoIn.stXdpSysCfg.bMuteEn = HI_TRUE;
        }
    }
...
HI_VOID ADAPTER_SetGfxXdrMode(HI_DRV_DISP_OUT_TYPE_E enDispOutType, HI_DRV_COLOR_SPACE_E enOutColorSpace)
{
    HI_PQ_GFXHDR_HDR_INFO  stGfxHdrInfo = {0};
    HI_PQ_GFXHDR_CFG       stPqGfxHdrCfg = {0};
    VDP_CLR_SPACE_E        enHalOutColorSpace = VDP_CLR_SPACE_BUTT;

    stGfxHdrInfo.enSrcFrameType = HI_DRV_VIDEO_FRAME_TYPE_SDR;
    stGfxHdrInfo.enInCS         = HI_DRV_CS_BT709_YUV_LIMITED;
    switch (enDispOutType)
    {
        case HI_DRV_DISP_TYPE_NORMAL:
            stGfxHdrInfo.enDispType = HI_PQ_DISP_TYPE_NORMAL;
            break;
        case HI_DRV_DISP_TYPE_HDR10:
            stGfxHdrInfo.enDispType = HI_PQ_DISP_TYPE_HDR10;
            break;
        case HI_DRV_DISP_TYPE_HLG:
            stGfxHdrInfo.enDispType = HI_PQ_DISP_TYPE_HLG;
            break;
        default:
            HI_ERR_DISP("Unsupport DispOutType->[%d],Change to SDR out.\n", enDispOutType);
            stGfxHdrInfo.enDispType = HI_PQ_DISP_TYPE_NORMAL;
            break;
    }
    stGfxHdrInfo.enOutCS        = enOutColorSpace;

    DRV_PQ_GetGfxHdrCfg(&stGfxHdrInfo, &stPqGfxHdrCfg);
    enHalOutColorSpace = XDR_COLOR_SPACE_TRANSFER(enOutColorSpace);

    VDP_DRV_SetHisiGfxHdr(&stPqGfxHdrCfg, enHalOutColorSpace);
    return;
}

#ifdef VDP_DOLBY_HDR_SUPPORT
static HI_VOID ADAPTER_TransferElAddr(HI_DRV_VID_FRAME_ADDR_S *pstDrvAddr,
                                      VDP_ADDR_S              *pstCbbAddr,
                                      HI_BOOL                  bSecure,
                                      HI_BOOL                  bSmmu)
{
...
static HI_VOID ADAPER_TransferWinInfoToDolbyCfg(WIN_INFO_S *pstWinInfo,
        XDP_LAYER_VID_INFO_S *pstLayerInfoIn)
{
    DISP_BUF_NODE_S *pstDispBufNode = HI_NULL;
    HI_DRV_VIDEO_FRAME_S   *pstBlFrm = pstWinInfo->pCurrentFrame;
    HI_DRV_VIDEO_FRAME_S   *pstElFrm = HI_NULL;

    ADAPTER_TransferWinInfoToHalCfg(pstWinInfo, pstLayerInfoIn, HI_TRUE);
    ...
    pstLayerInfoIn->stXdpSrcCfg.bCompatibleSource = \
            pstBlFrm->unHDRInfo.stDolbyInfo.bCompatible;

    pstDispBufNode = container_of((HI_U32 *)pstBlFrm, DISP_BUF_NODE_S, u32Data[0]);

    if (HI_FALSE == pstDispBufNode->bValidData2)
    {
        /* no el frame,just return. */
        return;
    }
    ...
    /* transfer el addr and stride. */
    ADAPTER_TransferElAddr(&pstElFrm->stBufAddr[0],
                           &pstLayerInfoIn->stXdpSrcCfg.stAddr[VDP_ADDR_EL],
                           pstElFrm->bSecure,
                           pstLayerInfoIn->stXdpSrcCfg.bSmmuEn);
    pstLayerInfoIn->stXdpSrcCfg.stElSrcRect.u32X = 0;
    pstLayerInfoIn->stXdpSrcCfg.stElSrcRect.u32Y = 0;
    pstLayerInfoIn->stXdpSrcCfg.stElSrcRect.u32Wth = pstElFrm->u32Width;
    pstLayerInfoIn->stXdpSrcCfg.stElSrcRect.u32Hgt = pstElFrm->u32Height;

    //============================bitwidth===========================
    pstLayerInfoIn->stXdpSrcCfg.enElDataWidth = XDR_BITWIDTH_TRANSFER(pstElFrm->enBitWidth);
    pstLayerInfoIn->stXdpSrcCfg.enElInFmt = XDR_PIXEL_FMT_TRANSFER(pstElFrm->ePixFormat);

#if defined(CHIP_TYPE_hi3798cv200)
    /*****************************************************
      | EL zme strategy.
      ******************************************************/
    if ((pstElFrm->u32Width != pstWinInfo->stDisp.s32Width)
        || (pstElFrm->u32Height != pstWinInfo->stDisp.s32Height))
    {
        if ((pstElFrm->u32Width == (pstWinInfo->stDisp.s32Width / 2) )
            && (pstElFrm->u32Height == (pstWinInfo->stDisp.s32Height / 2) )
           )
        {
            pstLayerInfoIn->stXdpSysCfg.bMuteEn = HI_FALSE;
        }
        else
        {
            pstLayerInfoIn->stXdpSysCfg.bMuteEn = HI_TRUE;
            //HI_ERR_DISP("EL reso invalid.\n");
        }
    }
#endif
...
HI_VOID ADAPTER_SetDolbyCfg(WIN_INFO_S *pstWinInfo, DRV_DISP_DOLBY_PLAY_INFO_S *pstDolbyInfo)
{
    ...
    /* step1. transfer to hal cfg. NOTE: null frame may carefully process. */
    ADAPER_TransferWinInfoToDolbyCfg(pstWinInfo, &stLayerInfoIn);

    /* step2. config logic path. */
    VDP_DRV_SetHdrMode(&stLayerInfoIn,
                       &pstDolbyInfo->stDolbyCoeff.stCompCfg,
                       &pstDolbyInfo->stDolbyCoeff.stDmKs);
