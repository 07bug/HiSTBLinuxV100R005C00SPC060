                          ...
                          HI_U32 u32PortCnt)
{
    ...
    VPSS_DBG_INFO("IN W %d H %d  W %d H %d\n",
                           ...
                           VPSS_HAL_INFO_S *pstHalInfo)
{
    ...
    for (u32Count = 0; (u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER) && (u32PortAvailable > 0); u32Count ++)
    {
        ...
        if (pstHalPort->bEnable && pstHalPort->bConfig == HI_FALSE)
        {
            ...
            VPSS_DBG_INFO("step2: lbx valid X Y W H %d %d %d %d\n", (HI_U32)pstHalPort->stVideoRect.s32X,
                          (HI_U32)pstHalPort->stVideoRect.s32Y,
                          pstHalPort->stVideoRect.s32Width,
                          pstHalPort->stVideoRect.s32Height);
            ...
            VPSS_DBG_INFO("step2: set W H Yaddr YS CS %d %d  %#x %d %d\n",
                          pstOutFrm->u32Width, pstOutFrm->u32Height,
                          pstOutAddr->u32PhyAddr_Y, pstOutAddr->u32Stride_Y, pstOutAddr->u32Stride_C);
...

{
    ...
    VPSS_DBG_INFO("dei in w h prog %d %d %d Yaddr:%#x ystride:%d\n",
                              ...
                              HI_U32 u32AppVir, HI_BOOL bRotateY, HI_U32 u32PortId)
{
    ...
    VPSS_DBG_INFO("Rota in w h prog %d %d %d Yaddr:%#x ystride:%d\n",
                  pstHalInfo->stInInfo.u32Width,
                  pstHalInfo->stInInfo.u32Height,
                  pstHalInfo->stInInfo.bProgressive,
                  pstCur->u32PhyAddr_Y,
    ...
    VPSS_REG_EnPort(u32AppVir, enPort, HI_TRUE);
    VPSS_DBG_INFO("Rota out w h %d %d yaddr %#x stride %d\n",
                  pstOutFrm->u32Width, pstOutFrm->u32Height,
                  pstOutAddr->u32PhyAddr_Y, pstOutAddr->u32Stride_Y);
...
HI_S32 VPSS_HAL_DumpReg(VPSS_IP_E enIP, VPSS_HAL_TASK_NODE_E enTaskNodeId)
{
    HI_U32 i;
    HI_U32 *pu32Reg;
