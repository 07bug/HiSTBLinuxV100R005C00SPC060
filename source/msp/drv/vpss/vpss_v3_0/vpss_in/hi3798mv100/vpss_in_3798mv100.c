...
HI_S32 VPSS_IN_Refresh_V1(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    if (pstImage->bSecure != pstEntity->bSecure)
    {
        ...
        pstEntity->pstHisInfo =
            VPSS_VMALLOC(sizeof(VPSS_HIS_INFO_S));
        if (HI_NULL != pstEntity->pstHisInfo)
        {
            (HI_VOID)VPSS_HIS_Init(pstEntity->pstHisInfo, pstEntity->bSecure);
        }
        else
        {
            VPSS_ERROR("Malloc HisInfo fail\n");
        }
    }
    ...
    switch (enType)
    {
        ...
        case VPSS_IN_INFO_WBCREG_VA:
            if (HI_DRV_BUF_ADDR_MAX != enLR)
            {
                ...
                s32Ret = VPSS_STTINFO_SttWbcGetAddr(pstEntity->pstSttWbc[enLR],
                                                    &u32stt_w_phy_addr,
                                                    pu32stt_w_vir_addr);
            }
            else
        ...
        case VPSS_IN_INFO_FIELD:
                ...
                || HI_DRV_BUF_ADDR_RIGHT == enLR)
            {
                ...
                s32Ret = VPSS_IMG_GetFieldAddr(pstEntity->pstSrcImagesList,
                                               pstFieldAddr,
                                               enLR);
            }
            else
        ...
        case VPSS_IN_INFO_HIS_ADDR:
            if (HI_DRV_BUF_ADDR_MAX == enLR)
            {
                ...
                s32Ret = VPSS_HIS_GetAddr(pstEntity->pstHisInfo,
                                          pstHisAddr);
            }
            else
            {
                            ...
                            HI_VOID *pstInfo)
{
    ...
    switch (enType)
    {
        case VPSS_SET_INFO_INPUT_CFG:
        {
            ...
            pstHalInfo->u32stt_w_vir_addr = 0xffffff; //98m don't have stt info, to fill a address
            ...
            switch (pstHalInfo->enNodeType)
            {
                ...
                case VPSS_HAL_NODE_2D_H265_STEP2_DEI:
                    (HI_VOID)stInIntf.pfnGetInfo(pstEntity,
                                                 VPSS_IN_INFO_FIELD,
...
HI_S32 VPSS_IN_CompleteImage_V1(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    if (HI_SUCCESS != VPSS_IMG_Complete(pstEntity->pstSrcImagesList))
    {
        VPSS_ERROR("Call VPSS_IMG_Complete failed.\n");
        return HI_FAILURE;
    }
