                         ...
                         ALG_RATIO_OUT_PARA_S *pstAspCfg)
{
    ...
    VPSS_CHECK_NULL(pstAspDrvPara);
    VPSS_CHECK_NULL(pstScreen);
    VPSS_CHECK_NULL(pstAspCfg);
...
HI_S32 ALG_ASP_Process(ALG_RATIO_DRV_PARA_S *pstDrvPara, ALG_RATIO_OUT_PARA_S *pstOutPara)
{
    ...
    VPSS_CHECK_NULL(pstDrvPara);
    VPSS_CHECK_NULL(pstOutPara);
                                 ...
                                 ALG_ASP_PARA_S *pstAspPara)
{
    ...
    else
    {
        ...
        pOutWnd->s32Y = (pOutWnd->s32Y + (pOutWnd->s32Height - v_h) / 2) & 0xfffffffeul;
                               ...
                               ALG_ASP_PARA_S *pstAspPara)
{
    ...
    */
    VPSS_CHECK_ZERO_RETURN_NULL(s32SrcImg_asp_w);
    VPSS_CHECK_ZERO_RETURN_NULL(s32Screen_skew);
    VPSS_CHECK_ZERO_RETURN_NULL(s32SrcImg_asp_h);
    ...
    VPSS_CHECK_ZERO_RETURN_NULL(s32Crop_in_w);
    VPSS_CHECK_ZERO_RETURN_NULL(s32Crop_in_h);
    ...
    pstOutWin_crop->s32X = s32Croped_pixel_w / 2;
    pstOutWin_crop->s32Y = s32Croped_pixel_h / 2;

    VPSS_PIXLE_DOWNALIGN_W(pstOutWin_crop->s32X, bFourPixOneClk);
    VPSS_PIXLE_DOWNALIGN_H(pstOutWin_crop->s32Y, bFourPixOneClk);

    s32Croped_pixel_w = pstOutWin_crop->s32X * 2;
    s32Croped_pixel_h = pstOutWin_crop->s32Y * 2;
    ...
    HI_ASSERT(((pstOutWin_crop->s32X * 2) + pstOutWin_crop->s32Width) == pstDrvWin_in->s32Width);
    HI_ASSERT(((pstOutWin_crop->s32Y * 2) + pstOutWin_crop->s32Height) == pstDrvWin_in->s32Height);
    ...
    VPSS_PIXLE_UPALIGN_W(pstOutPara->u32ZmeW, bFourPixOneClk);
    VPSS_PIXLE_UPALIGN_H(pstOutPara->u32ZmeH, bFourPixOneClk);
                                ...
                                ALG_ASP_PARA_S *pstAspPara)
{
    ...
    */
    ...
    VPSS_CHECK_ZERO_RETURN_NULL(s32ExpH);
    VPSS_CHECK_ZERO_RETURN_NULL(s32ExpW);
