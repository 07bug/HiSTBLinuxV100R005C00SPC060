...
HI_S32 HI_DRV_VPSS_Suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    ...
    VPSS_OSAL_IntfTime_Start(VPSS_INTF_SUSPEND);

    for (s32InstId = 0; s32InstId < VPSS_INSTANCE_MAX_NUMB; s32InstId++)
    {
        pstInstance = VPSS_CTRL_GetInstance(s32InstId);
        if (HI_NULL != pstInstance)
        {
            s32Ret = VPSS_CTRL_Pause(s32InstId);
            if (HI_SUCCESS != s32Ret)
            {
                VPSS_ERROR("VPSS_CTRL_Pause inst %d fail!\n", s32InstId);
                return s32Ret;
            }
        }
    }

    VPSS_HAL_SetClockEn(VPSS_IP_0, HI_FALSE);

    VPSS_OSAL_IntfTime_Stop(VPSS_INTF_SUSPEND);
...
HI_S32 HI_DRV_VPSS_Resume(PM_BASEDEV_S *pdev)
{
    ...
    VPSS_OSAL_IntfTime_Start(VPSS_INTF_RESUME);
    VPSS_HAL_SetClockEn(VPSS_IP_0, HI_TRUE);
    for (s32InstId = 0; s32InstId < VPSS_INSTANCE_MAX_NUMB; s32InstId++)
    {
        pstInstance = VPSS_CTRL_GetInstance(s32InstId);
        if (HI_NULL != pstInstance)
        {
            s32Ret = VPSS_CTRL_Resume(s32InstId);
            if (HI_SUCCESS != s32Ret)
            {
                VPSS_ERROR("VPSS_CTRL_Resume inst %d fail!\n", s32InstId);
                return s32Ret;
            }
        }
    }

    VPSS_OSAL_IntfTime_Stop(VPSS_INTF_RESUME);
...
HI_VOID VPSS_DRV_Exit(HI_VOID)
{
...
#if defined(ALGBUF_MANAGE_MODE2)
    {
        ...
        {
            VPSS_AlgBuf_Free(&g_VpssAlgBuf[index]);
        }
    }
#endif
}
...
{
    VPSS_HANDLE hInst;
    VPSS_OSAL_IntfTime_Start(VPSS_INTF_CREATE_INST);
    ...
    {
        *hVPSS = hInst;
        VPSS_OSAL_IntfTime_Stop(VPSS_INTF_CREATE_INST);
        return HI_SUCCESS;
    }
    else
    {
...
{
    HI_S32 s32Ret;
    VPSS_OSAL_IntfTime_Start(VPSS_INTF_DESTRORY_INST);
    ...
    up(&g_VpssMutex);
    VPSS_OSAL_IntfTime_Stop(VPSS_INTF_DESTRORY_INST);
...
HI_S32  HI_DRV_VPSS_CreatePort(VPSS_HANDLE hVPSS, HI_DRV_VPSS_PORT_CFG_S *pstVpssPortCfg, VPSS_HANDLE *phPort)
{
    ...
    VPSS_OSAL_IntfTime_Start(VPSS_INTF_CREATE_PORT);
    ...
    if (pstInstance)
    {
        ...
        VPSS_OSAL_IntfTime_Stop(VPSS_INTF_CREATE_PORT);
        return s32Ret;
    }
    else
    {
...
HI_S32  HI_DRV_VPSS_DestroyPort(VPSS_HANDLE hPort)
{
    ...
    HI_S32 s32Ret;
    VPSS_HANDLE hVPSS;
    VPSS_OSAL_IntfTime_Start(VPSS_INTF_DESTRORY_PORT);
    ...
    VPSS_CTRL_Resume(hVPSS);
    VPSS_OSAL_IntfTime_Stop(VPSS_INTF_DESTRORY_PORT);
