...
PF_NODE_CFG_FUNC pfSetNodeInfo[VPSS_HAL_NODE_BUTT] =
{
    ...
    VPSS_HAL_Set_2DField_Node, //Last2Field
    VPSS_HAL_Set_2DField_Node, //Last2Field
    VPSS_HAL_Set_2DField_Node, //VPSS_HAL_NODE_2D_DETILE_FIRST3FIELD
    VPSS_HAL_Set_2DField_Node, //VPSS_HAL_NODE_2D_DETILE_FIRST3FIELD_virtual
...
HI_U8 *g_pcHalNodeString[] =
{
    ...
    NUM_2_STRING(2D_DETILE_FIRST3FIELD),
    NUM_2_STRING(2D_DETILE_FIRST3FIELD_VIRTUAL),
...
#if 0
HI_VOID VPSS_HAL_DumpRegRTLFmt(VPSS_IP_E enIP, VPSS_HAL_NODE_TYPE_E enNodeId)
{
    HI_ULONG ulRegBase;
    VPSS_HAL_CTX_S *pstHalCtx = HI_NULL;
    HI_U32 reg_size = sizeof(S_VPSS_REGS_TYPE) / 4;
    HI_U32 reg_count;
    HI_U32 vpss_node;

    pstHalCtx = &stHalCtx[enIP][0];

    HI_PRINT("NODE(%d): phyaddr: %lx  vir:%lx \n", enNodeId, (HI_ULONG)(pstHalCtx->au32AppPhy[enNodeId]),
             (HI_ULONG)(pstHalCtx->apu8AppVir[enNodeId]));

    for (reg_count = 0; reg_count < reg_size ; reg_count++)
    {
        if (reg_count == (int)0x314 / 4 || reg_count == (int)0x318 / 4 || reg_count == (int)0x31c / 4
            || reg_count == (int)0x324 / 4 || reg_count == (int)0x344 / 4 || reg_count == (int)0xf000 / 4)
        {
        ...
        vpss_node = *((HI_U32 *)(ulRegBase + (reg_count * 4)));
        HI_PRINT("%04x    %08x \n" , (4 * reg_count), vpss_node);
    }
}
#endif
...
HI_VOID VPSS_Hal_PrintHalInfo(struct seq_file *p, S_VPSS_REGS_TYPE *pstNodeReg)
{
    ...
    PROC_PRINT(p,
               ...
               VPSS_PROC_LEVEL5"Alg     tnr:%u snr:%u dei:%u mcdi:%u rgme:%u ifmd:%u igbm:%u"
               " db:%d blk_det:%u mcnr:%u mcnrmeds:%u meds:%u"
               VPSS_PROC_LEVEL5"VHD0    4pc_en :%u bitw:%u   format:%u  rota_en:%u flip_en:%u cts_en:%u  tunl_en:%u"
               ...
               pstNodeReg->VPSS_SNR_CTRL.bits.db_en,
               pstNodeReg->VPSS_CTRL.bits.blk_det_en,
               pstNodeReg->VPSS_TNR_CONTRL.bits.mcnr_en,
               pstNodeReg->VPSS_TNR_CONTRL.bits.mcnr_meds_en,
               pstNodeReg->VPSS_CTRL.bits.meds_en,
...
HI_S32 VPSS_HAL_SetClockEn(VPSS_IP_E enIP, HI_BOOL bClockEn)
{
    ...
    if (bClockEn)
    {
        ...
        VPSS_DRV_Set_VPSS_MISCELLANEOUS_CLKGT((S_VPSS_REGS_TYPE *)pstHalCtx->pu8BaseRegVir, 1, 1);
        VPSS_DRV_Set_VPSS_MISCELLANEOUS_OUTSTANDING((S_VPSS_REGS_TYPE *)pstHalCtx->pu8BaseRegVir, 0x6, 0x6);
        VPSS_DRV_Set_VPSS_MISCELLANEOUS_BUSLEN((S_VPSS_REGS_TYPE *)pstHalCtx->pu8BaseRegVir, 0x0);
        VPSS_DRV_Set_VPSS_MISCELLANEOUS_INITTIMER((S_VPSS_REGS_TYPE *)pstHalCtx->pu8BaseRegVir, 0x64);
...
HI_S32 VPSS_HAL_CfgAXI(VPSS_IP_E enIP)
{
    ...
    stAxiCfg.stBusCtrl.u32BurstLen    = AXI_BURST_LEN_16;
    stAxiCfg.stBusCtrl.u32Split128    = 0;
    stAxiCfg.stBusCtrl.u32Split256    = 0;
                              ...
                              HI_BOOL bNeedRota)
{
    switch ( enTaskNodeId )
    {
...
#endif
            {
                if ((pstHalInfo->stInInfo.bProgressive) || (pstHalInfo->stInInfo.u32Width < 128))
                {
        ...
        case VPSS_HAL_TASK_NODE_2D_LAST2FIELD:
            ...
            break;
        case VPSS_HAL_TASK_DETILE_FIRST3FIELD:
            VPSS_HAL_ENABLE_NODE(VPSS_HAL_NODE_2D_DETILE_FIRST3FIELD);
            if (bNeedVirtualNode)
            {
                VPSS_HAL_ENABLE_NODE(VPSS_HAL_NODE_2D_DETILE_FIRST3FIELD_VIRTUAL);
            }
            if (bNeedRota)
            {
                if (HI_SUCCESS == VPSS_HAL_AllocateRotaBuf(enIP, pstHalInfo))
...
#if 0 //keep rwzb picture quality better.
    //RWZB
    if (pstHalInfo->stRwzbInfo.u32IsRwzb > 0)
    {
        // VPSS_DRV_Set_VPSS_DIECTRL_outsel(pstVpssNode, HI_TRUE, HI_TRUE);
        VPSS_Die_SetDieOutSelL(pstVpssNode, HI_TRUE);
        VPSS_Die_SetDieOutSelC(pstVpssNode, HI_TRUE);
    }
#endif
...
HI_BOOL VPSS_HAL_NeedVPZMEEnable(VPSS_HAL_NODE_TYPE_E enNodeType)
{
    if ( (enNodeType == VPSS_HAL_NODE_2D_FIELD)
         ...
         || (enNodeType == VPSS_HAL_NODE_2D_DETILE_FIRST3FIELD)
         || (enNodeType == VPSS_HAL_NODE_2D_DETILE_FIRST3FIELD_VIRTUAL)
       )
    {
        return HI_TRUE;
    }
...
#ifdef VPSS_SUPPORT_DICNT
    stDiCntCfg.u32WPhyAddr  = pstHalInfo->stMcdeiStInfo.stDiCntCfg.u32WriteAddr;
    stDiCntCfg.u32RPhyAddr  = pstHalInfo->stMcdeiStInfo.stDiCntCfg.u32ReadAddr;
    stDiCntCfg.u32Stride    = pstHalInfo->stMcdeiStInfo.stDiCntCfg.u32Stride;
    stDiCntCfg.u32Width     = pstHalInfo->stMcdeiStInfo.stDiCntCfg.u32Width;
    stDiCntCfg.u32Height    = pstHalInfo->stMcdeiStInfo.stDiCntCfg.u32Height - 1;
    VPSS_CBB_INFO_SetConfig(stRegAddr, CBB_INFO_DI_CNT, (HI_VOID *)&stDiCntCfg, HI_NULL);
#endif
...
HI_S32 VPSS_HAL_SetPortCfg(VPSS_IP_E enIP, CBB_REG_ADDR_S stRegAddr, VPSS_HAL_INFO_S *pstHalInfo)
{
    ...
    for (u32PortId = 0; u32PortId < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32PortId++)
    {
        ...
        if ((pstHalPort->bEnable)  && (pstHalPort->bConfig == HI_FALSE))
        {
            ...
            VPSS_PIXLE_DOWNALIGN_W(pstCropRect->s32Width, b4PixelClk);
            VPSS_PIXLE_DOWNALIGN_H(pstCropRect->s32Height, b4PixelClk);//2pixel align whatever 4p/c is enable or disable
            VPSS_PIXLE_DOWNALIGN_W(pstCropRect->s32X, b4PixelClk);
...
6：10bit；
其它：保留。
注：bank456可以自适应，3种场景可以统一配bank4。
*/
HI_S32 VPSS_HAL_GetRamBank(VPSS_HAL_FRAME_S *pstInInfo, HI_BOOL bRotaEnable)
{
    HI_S32 s32RamBank = 0;

    if (HI_FALSE == pstInInfo->bProgressive)
    {
        if (pstInInfo->u32Width <= 960)
        {
            s32RamBank = 0;
        }
        else
    ...
    if (HI_TRUE == bRotaEnable)
    {
        s32RamBank = 4;
        return s32RamBank;
    }

    if (HI_DRV_PIXEL_BITWIDTH_8BIT == pstInInfo->enBitWidth)
    {
        if (pstInInfo->u32Width > 1920)
        {
            s32RamBank = 5;
        }
    }
...
HI_S32 VPSS_HAL_Set_2DFrame_Node(VPSS_IP_E enIP, VPSS_HAL_INFO_S *pstHalInfo, HI_U32 *pu32AppVir, HI_U32 u32AppPhy)
{
    ...
    CBB_ALG_INFO_S      stMedsInfo;
    CBB_ALG_INFO_S      stFMTCInfo;
    CBB_INFO_S          sttInfoCfg;
    HI_BOOL             bMcnrMeds_en;
    HI_BOOL             bRgmeEnable;
    ...
    VPSS_MEM_CLEAR(stMedsInfo);
    VPSS_MEM_CLEAR(stFMTCInfo);
    ...
    stMedsInfo.bEnable  = HI_FALSE;
    bRfrEnableFlag      = HI_TRUE;



    if (pstHalInfo->stInInfo.u32Width >= 256)
    {
        stMedsInfo.bEnable  = HI_TRUE;
    }
    ...
    {
        stMcTnrInfo.bEnable = HI_FALSE;
        bMcnrMeds_en = HI_FALSE;
    }
    else
    {
        if (pstHalInfo->stInInfo.u32Width < 128)
        {
            bRgmeEnable = HI_FALSE;
        }
    }

    if (HI_TRUE == bRgmeEnable)
    {
        if (pstHalInfo->stInInfo.u32Width < 256)
        {
            bMcnrMeds_en = HI_FALSE;
    ...
    VPSS_Sys_SetRamBank((S_VPSS_REGS_TYPE *)pu32AppVir, VPSS_HAL_GetRamBank(&(pstHalInfo->stInInfo), HI_FALSE));
    VPSS_Tnr_SetMcnrMedsEn((S_VPSS_REGS_TYPE *)pu32AppVir, bMcnrMeds_en);

    stDBInfo.u32ParaAddr = u32AppPhy + VPSS_REG_SIZE_CALC(VPSS_CTRL, VPSS_SNR_CTRL) - sizeof(HI_U32);
    ...
    VPSS_AlgCfg_BaseFunc(stRegAddr, CBB_ALG_MEDS, (HI_VOID *)&stMedsInfo, HI_NULL);


    /* snr, tnr */
    //In 98mv200 all of SNR,DB,DM use DB'coef addr(VPSS_SNR_CTRL).
    stSnrInfo.u32ParaAddr = u32AppPhy + VPSS_REG_SIZE_CALC(VPSS_CTRL, VPSS_SNR_CTRL) - sizeof(HI_U32);
    ...
    stFMTCInfo.u32ParaAddr = u32AppPhy + VPSS_REG_SIZE_CALC(VPSS_CTRL, VPSS_FMTC_GAMMA_CSC_CTRL) - sizeof(HI_U32);
    stFMTCInfo.bEnable = VPSS_HAL_VIDEOPHONE_SCENE(pstHalInfo->enSceneMode);
    VPSS_AlgCfg_BaseFunc(stRegAddr, CBB_ALG_FMTC, (HI_VOID *)&stFMTCInfo, HI_NULL);
    ...
    if (pstHalInfo->stRwzbInfo.u32EnRwzb)
    {
        ...
        {
            VPSS_HAL_RWZB_Calc(pstHalInfo, pstNext2); //use wbc data(has been detiled)
        }
    }
...
HI_S32 VPSS_HAL_Set_2DDei_Node(VPSS_IP_E enIP, VPSS_HAL_INFO_S *pstHalInfo, HI_U32 *pu32AppVir, HI_U32 u32AppPhy)
{
    ...
    if (HI_TRUE == stMcDeiAlgCfg.bEnable)
    {
        if (pstHalInfo->stInInfo.u32Width > 960)
        {
            bMcnrMeds_en = HI_TRUE;
    ...
    stSnrInfo.u32Ymotionmode = stTnrInfo.u32Ymotionmode;
    stSnrInfo.u32ParaAddr = u32AppPhy + VPSS_REG_SIZE_CALC(VPSS_CTRL, VPSS_SNR_CTRL) - sizeof(HI_U32);
    VPSS_AlgCfg_BaseFunc(stRegAddr, CBB_ALG_SNR, (HI_VOID *)&stSnrInfo, HI_NULL);
    ...
    stFMTCInfo.u32ParaAddr = u32AppPhy + VPSS_REG_SIZE_CALC(VPSS_CTRL, VPSS_FMTC_GAMMA_CSC_CTRL) - sizeof(HI_U32);
    stFMTCInfo.bEnable = VPSS_HAL_VIDEOPHONE_SCENE(pstHalInfo->enSceneMode);
    if (HI_TRUE == stFMTCInfo.bEnable)
    {
        VPSS_ERROR("FMTC do not support interlace frame(dei)\n");
    }
    VPSS_AlgCfg_BaseFunc(stRegAddr, CBB_ALG_FMTC, (HI_VOID *)&stFMTCInfo, HI_NULL);
...
HI_S32 VPSS_HAL_Set_2DField_Node(VPSS_IP_E enIP, VPSS_HAL_INFO_S *pstHalInfo, HI_U32 *pu32AppVir, HI_U32 u32AppPhy)
{
    ...
    stTnrInfo.bEnable   = HI_FALSE;
    bRfrEnableFlag      = HI_TRUE;  //must open in first 3 field, we need dei-ref relationship foundition
    stMcTnrInfo.bEnable = HI_FALSE;
    ...
    if (HI_DRV_PIXEL_BITWIDTH_8BIT == pstHalInfo->stInInfo.enBitWidth)
    {
        if (pstHalInfo->stInInfo.u32Width > 1920)
        {
            s32RamBank = 5;
    ...
    VPSS_Sys_SetRamBank((S_VPSS_REGS_TYPE *)pu32AppVir, s32RamBank);
    VPSS_Snr_SetSnrEn((S_VPSS_REGS_TYPE *)pu32AppVir, HI_FALSE);
...
HI_S32 VPSS_HAL_SetRotateNode(VPSS_IP_E enIP, VPSS_HAL_INFO_S *pstHalInfo, HI_U32 *pu32AppVir, HI_U32 u32AppPhy, HI_BOOL bRotateY, HI_U32 u32PortId)
{
    ...
    stTunnelIn.bEnable = HI_FALSE; //disable tunnel in when rota is enable.
    VPSS_CBB_FUNC_SetConfig(stRegAddr, CBB_FUNC_TUNNEL_IN, (HI_VOID *)&stTunnelIn, HI_NULL);
    ...
    VPSS_Sys_SetRamBank((S_VPSS_REGS_TYPE *)pu32AppVir, VPSS_HAL_GetRamBank(&(pstHalInfo->stInInfo), HI_TRUE));
        ...
        HI_U32 *pu32AppVir, HI_U32 u32AppPhy)
{
    ...
    VPSS_HAL_FRAME_S        *pstInInfo;
    HI_PQ_ZME_PARA_IN_S     *pstZmeDrvPara;
    VPSS_HAL_FRAME_S        stInInfoStep1;
    ...
    stInInfoStep1 = pstHalInfo->stInInfo;
    stInInfoStep1.bProgressive = HI_TRUE;

    VPSS_Sys_SetRamBank((S_VPSS_REGS_TYPE *)pu32AppVir, VPSS_HAL_GetRamBank(&(stInInfoStep1), HI_FALSE));
...
HI_S32 VPSS_HAL_Set_2DDei_Last2Field_Node(VPSS_IP_E enIP, VPSS_HAL_INFO_S *pstHalInfo, HI_U32 *pu32AppVir, HI_U32 u32AppPhy)
{
    ...
    stFrameCfg.u32Width         = pstHalInfo->stInInfo.u32Width;
    stFrameCfg.u32Height        = pstHalInfo->stInInfo.u32Height ;
    stFrameCfg.bProgressive     = HI_FALSE; //field-in-field-out need bfield=1,dei=0
