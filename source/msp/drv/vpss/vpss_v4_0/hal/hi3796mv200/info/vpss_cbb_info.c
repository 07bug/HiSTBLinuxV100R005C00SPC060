...
{
    if ((HI_NULL == pstSttReg) || (HI_NULL == pstInfoStt))
    {
        VPSS_ERROR("Invalid ptr! %p %p\n", pstSttReg, pstInfoStt);
        return;
    }

    if (HI_NULL == pstInfoStt->pu8RVirAddr)
    {
        VPSS_ERROR("Invalid ptr! pu8RVirAddr=NULL\n");
        return;
    }
