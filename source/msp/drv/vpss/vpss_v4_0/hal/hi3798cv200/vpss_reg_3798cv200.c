...
HI_S32 VPSS_REG_SetClockEn(HI_BOOL bClkEn)
{
    if (bClkEn)
    {
        ...
        //0:333Mhz, 1:400Mhz, 2:300Mhz, Must Be same with VPSS_LOGIC_CLOCK_SEL
        VPSS_WRITE_REG_SAFE(g_pstRegCrg->PERI_CRG60.bits.vpss_clk_sel, VPSS_LOGIC_CLOCK_SEL, "VPSS clock select timeout!\n");
        udelay(1);
...
HI_S32 VPSS_REG_ResetAppReg(HI_U32 *pu32AppAddr, VPSS_REG_S *pstPqReg)
{
    ...
    if ((HI_NULL == pu8FlushAddr) || (HI_NULL == pstPqReg))
    {
        VPSS_ERROR("Null addr %p %p\n", pu8FlushAddr, pstPqReg);
        return HI_FAILURE;
    }
...
{
    HI_U32 u32VPSS_PNEXT;
    HI_S32 s32RetryCnt;

    s32RetryCnt = 0;
    u32VPSS_PNEXT = 0;

    while ((u32VPSS_PNEXT != (HI_U32)(long)pu32PhyAddr) && (s32RetryCnt < 10))
    {
        u32VPSS_PNEXT = VPSS_REG_RegRead((volatile HI_U32 *)((unsigned long) & (pstVpssRegs->VPSS_PNEXT)));
        u32VPSS_PNEXT = (HI_U32)((unsigned long)pu32PhyAddr);
        VPSS_REG_RegWrite((volatile HI_U32 *)((unsigned long) & (pstVpssRegs->VPSS_PNEXT)), u32VPSS_PNEXT);
        u32VPSS_PNEXT = VPSS_REG_RegRead((volatile HI_U32 *)((unsigned long) & (pstVpssRegs->VPSS_PNEXT)));
        s32RetryCnt++;

        if (u32VPSS_PNEXT != (HI_U32)(long)pu32PhyAddr)
        {
            VPSS_FATAL("retry:%d (exp %x  real %x)\n", s32RetryCnt, pu32PhyAddr, u32VPSS_PNEXT);
        }
    }
...
HI_VOID VPSS_DRV_Set_VPSS_START(S_VPSS_REGS_TYPE *pstVpssRegs)
{
    ...
    VPSS_START.u32 = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_START.u32));
...
HI_S32 VPSS_REG_GetPFCNT(HI_U32 *pu32AppAddr, HI_U32 *pu32Int)
{
    ...
    VPSS_PFCNT = VPSS_REG_RegRead((volatile HI_U32 *) & (pstReg->VPSS_PFCNT));
