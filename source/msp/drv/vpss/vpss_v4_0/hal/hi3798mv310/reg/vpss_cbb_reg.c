...
#define VPSS_READ_REG_SAFE(reg, val, errlog) \
    { \
        HI_S32 s32Timeout = 0; \
        while (val != reg) \
        { \
...
{
    if ((HI_NULL == pstVpssRegs) || (HI_NULL == pstPqReg))
    {
        VPSS_ERROR("Null addr %p %p\n", pstVpssRegs, pstPqReg);
        return HI_FAILURE;
    }
    ...
    VPSS_REG_RegWrite((volatile HI_U32 *) & (pstVpssRegs->VPSS_PNEXT), 0x0);

    {
        CBB_REG_ADDR_S          stRegAddr = {0};
        CBB_BUS_CTRL_S          stBusCtrl = {0};

        stRegAddr.pu8BaseVirAddr = (HI_U8 *)pstVpssRegs; //use node vir addr
        stRegAddr.pu8NodeVirAddr = (HI_U8 *)pstVpssRegs; //use node vir addr

        stBusCtrl.enBurstLen = VPSS_REQ_LEN_16BURST;
        stBusCtrl.enBurstNum = VPSS_REQ_NUM_1;

        VPSS_CBB_BASE_Operating(stRegAddr, CBB_BASEOPT_CONFIG_BUS, &stBusCtrl, HI_NULL);
    }
...
HI_S32 VPSS_DRV_SetClockEn(HI_BOOL bClkEn)
{
    ...
    S_VPSS_REGS_TYPE *pstVpssReg;

    pstHalCtx = &stHalCtx[VPSS_IP_0][0];
    VPSS_HAL_CHECK_INIT(pstHalCtx->bInit);
    VPSS_CHECK_NULL(pstHalCtx->pu8BaseRegVir);

    pstVpssReg = (S_VPSS_REGS_TYPE *)pstHalCtx->pu8BaseRegVir;

    if (bClkEn)
    {
        VPSS_WRITE_REG_SAFE(g_pstRegCrg->PERI_CRG60.bits.vpss_cken, 1, "VPSS clock enable timeout!\n");
        s32Ret = VPSS_DRV_Reset(HI_TRUE);
        VPSS_READ_REG_SAFE(pstVpssReg->VPSS_VERSION.bits.version, 0x0, "VPSS reset timeout!\n");
        s32Ret |= VPSS_DRV_Reset(HI_FALSE);
        VPSS_READ_REG_SAFE(pstVpssReg->VPSS_VERSION.bits.version, 0x4000, "VPSS un-reset timeout!\n");
    }
    else
    {
        s32Ret = VPSS_DRV_Reset(HI_TRUE);
...
HI_S32 VPSS_DRV_Reset(HI_BOOL bReset)
{
    HI_S32 s32Timeout = 100;
    if (bReset)
    {
        g_pstRegCrg->PERI_CRG60.bits.vpss_srst_req = 1;
        while (0 < s32Timeout--)
        {
            if (g_pstRegCrg->PERI_CRG60.bits.vpss_srst_req == 1)
            {
                break;
                // else
            }
    ...
    {
        g_pstRegCrg->PERI_CRG60.bits.vpss_srst_req = 0;
        while (0 < s32Timeout--)
        {
            if (g_pstRegCrg->PERI_CRG60.bits.vpss_srst_req == 0)
            {
                break;
            }
        }
...
//addr=0x4bc
HI_VOID VPSS_DRV_Set_VPSS_TRANS_INFO(S_VPSS_REGS_TYPE *pstVpssRegs , HI_U32 trans_en ,
                                     HI_U32 trans_size, HI_U32  trans_prot, HI_U32 bypass)
{
    ...
    VPSS_TRANS_INFO.bits.trans_prot = trans_prot;
    VPSS_TRANS_INFO.bits.trans_bypass = bypass; //0,smmu 1,mmz
...
HI_S32 VPSS_DRV_SetTimeOut(S_VPSS_REGS_TYPE *pstVpssRegs, HI_U32 u32Data)
{
    ...
    VPSS_REG_RegWrite((volatile HI_U32 *) & (pstVpssRegs->VPSS_TIMEOUT), VPSS_TIMEOUT);
...
HI_VOID VPSS_DRV_SetIntMask( S_VPSS_REGS_TYPE *pstVpssRegs, HI_U32 u32Mask )
{
    ...
    while ((VPSS_INTMASK.u32 != u32Mask) && (s32RetryCnt++ < 10))
    {
        VPSS_INTMASK.u32 = u32Mask;
    ...
    //addr=0x4
    HI_VOID VPSS_DRV_Set_VPSS_CTRL2(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 mode)
    {
        U_VPSS_CTRL2 VPSS_CTRL2;

        VPSS_CTRL2.u32 = VPSS_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_CTRL2.u32));
        VPSS_CTRL2.bits.ram_bank = mode;
    ...
    HI_VOID VPSS_DRV_Set_VPSS_DIECTRL_maonly(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 ma_only )
    {
        U_VPSS_DIECTRL VPSS_DIECTRL;
        VPSS_DIECTRL.u32 = VPSS_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_DIECTRL.u32));
        VPSS_DIECTRL.bits.ma_only           = ma_only           ;
    ...
    HI_VOID VPSS_DRV_Set_VPSS_DIELMA2_recmode(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 rec_mode_en)
    {
        U_VPSS_DIELMA2 VPSS_DIELMA2;
        VPSS_DIELMA2.u32 = VPSS_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_DIELMA2.u32));
        VPSS_DIELMA2.bits.rec_mode_en               = rec_mode_en               ;
    ...
    HI_VOID VPSS_DRV_Set_VPSS_DIEHISMODE_hismot_ppreinfo(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 his_motion_en , HI_U32 ppre_info_en)
    {
        U_VPSS_DIEHISMODE VPSS_DIEHISMODE;
        VPSS_DIEHISMODE.u32 = VPSS_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_DIEHISMODE.u32));
        VPSS_DIEHISMODE.bits.his_motion_en              = his_motion_en              ;
    ...
    //addr=0x2100
#if 1
    HI_VOID VPSS_DRV_Set_VPSS_SNR_CTRL(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 dbm_out_mode , HI_U32 mad_mode , HI_U32 det_hy_en , HI_U32 dbm_demo_mode , HI_U32 dbm_demo_en , HI_U32 db_lum_hor_en , HI_U32 nr_en , HI_U32 dm_en , HI_U32 db_en )
    {
        U_VPSS_SNR_CTRL VPSS_SNR_CTRL;

        VPSS_SNR_CTRL.u32 = VPSS_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SNR_CTRL.u32));
        ...
        VPSS_SNR_CTRL.bits.snr_en        = nr_en         ;
        VPSS_SNR_CTRL.bits.dm_en         = dm_en         ;
        VPSS_SNR_CTRL.bits.db_en         = db_en         ;

        VPSS_RegWrite((volatile HI_U32 *) & (pstVpssRegs->VPSS_SNR_CTRL.u32), VPSS_SNR_CTRL.u32);

        return;
    }

    HI_VOID VPSS_DRV_Set_VPSS_SNR_CTRL_db(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 db_en )
    {
        U_VPSS_SNR_CTRL VPSS_SNR_CTRL;
    ...
    HI_VOID VPSS_DRV_Set_VPSS_SNR_CTRL_dm(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 dm_en )
    {
        U_VPSS_SNR_CTRL VPSS_SNR_CTRL;
    ...
    HI_VOID VPSS_DRV_Set_VPSS_SNR_CTRL_nr(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 nr_en )
    {
        U_VPSS_SNR_CTRL VPSS_SNR_CTRL;
    ...
    //addr=0x3800
    HI_VOID VPSS_DRV_Set_VPSS_TNR_CONTRL(S_VPSS_REGS_TYPE * pstVpssRegs , HI_U32 motion_mode , HI_U32 mcnr_rgme_mode , HI_U32 marketcoor , HI_U32 marketmode , HI_U32 marketmodeen , HI_U32 sharemotionen , HI_U32 mcnr_meds_en , HI_U32 cnren , HI_U32 ynren , HI_U32 nren )
    {
        ...
        VPSS_TNR_CONTRL.bits.motion_mode    = motion_mode     ;
    ...
    {
        U_VPSS_SMMU_INTSTAT_S VPSS_SMMU_INTSTAT_S;
        U_VPSS_SMMU_INTSTAT_S VPSS_SMMU_INTSTAT_NS;
        ...
        *pu32NoSecureState = VPSS_SMMU_INTSTAT_NS.u32;
        ...
        {
            HI_U32 VPSS_SMMU_FAULT_ADDR_PTW_NS, VPSS_SMMU_FAULT_ID_PTW_NS, VPSS_SMMU_FAULT_PTW_NUM;
            HI_U32 VPSS_SMMU_FAULT_ADDR_RD_NS, VPSS_SMMU_FAULT_TLB_RD_NS, VPSS_SMMU_FAULT_ID_RD_NS;
            HI_U32 VPSS_SMMU_FAULT_ADDR_WR_NS, VPSS_SMMU_FAULT_TLB_WR_NS, VPSS_SMMU_FAULT_ID_WR_NS;

            VPSS_SMMU_FAULT_ADDR_PTW_NS  = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ADDR_PTW_NS.u32));
            VPSS_SMMU_FAULT_ID_PTW_NS    = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ID_PTW_NS.u32));
            VPSS_SMMU_FAULT_PTW_NUM      = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_PTW_NUM.u32));
            VPSS_SMMU_FAULT_ADDR_RD_NS   = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ADDR_RD_NS.u32));
            VPSS_SMMU_FAULT_TLB_RD_NS    = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_TLB_RD_NS.u32));
            VPSS_SMMU_FAULT_ID_RD_NS     = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ID_RD_NS.u32));

            VPSS_SMMU_FAULT_ADDR_WR_NS   = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ADDR_WR_NS.u32));
            VPSS_SMMU_FAULT_TLB_WR_NS    = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_TLB_WR_NS.u32));
            VPSS_SMMU_FAULT_ID_WR_NS     = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ID_WR_NS.u32));

            if (0x0 != VPSS_SMMU_INTSTAT_NS.bits.ints_tlbinvalid_rd_stat)
            {
                HI_PRINT("VPSS_SMMU_READ_ERR MODULE : %s  NSSTATE : %#x ReadAddr:%#x\n",
                         HI_MOD_VPSS,
                         VPSS_SMMU_INTSTAT_NS.u32,
                         VPSS_SMMU_FAULT_ADDR_RD_NS);
            }

            if (0x0 != VPSS_SMMU_INTSTAT_NS.bits.ints_tlbinvalid_wr_stat)
            {
                HI_PRINT("VPSS_SMMU_WRITE_ERR MODULE : %s  NSSTATE : %#x WriteAddr:%#x\n",
                         HI_MOD_VPSS,
                         VPSS_SMMU_INTSTAT_NS.u32,
                         VPSS_SMMU_FAULT_ADDR_WR_NS);
            }

            HI_PRINT("VPSS_SMMU_FAULT_ADDR_PTW_NS:%x\nVPSS_SMMU_FAULT_ID_PTW_NS:%u\nVPSS_SMMU_FAULT_PTW_NUM:%u\nVPSS_SMMU_FAULT_TLB_RD_NS:%#x\nVPSS_SMMU_FAULT_ID_RD_NS:%u\nVPSS_SMMU_FAULT_TLB_WR_NS:%#x\nVPSS_SMMU_FAULT_ID_WR_NS:%u\n"
                     ,
                     VPSS_SMMU_FAULT_ADDR_PTW_NS,
                     VPSS_SMMU_FAULT_ID_PTW_NS,
                     VPSS_SMMU_FAULT_PTW_NUM,
                     VPSS_SMMU_FAULT_TLB_RD_NS,
                     VPSS_SMMU_FAULT_ID_RD_NS,
                     VPSS_SMMU_FAULT_TLB_WR_NS,
                     VPSS_SMMU_FAULT_ID_WR_NS
                    );
        ...
        {
            HI_U32 VPSS_SMMU_FAULT_ADDR_PTW_S, VPSS_SMMU_FAULT_ID_PTW_S, VPSS_SMMU_FAULT_PTW_NUM;
            HI_U32 VPSS_SMMU_FAULT_ADDR_RD_S, VPSS_SMMU_FAULT_TLB_RD_S, VPSS_SMMU_FAULT_ID_RD_S;
            HI_U32 VPSS_SMMU_FAULT_ADDR_WR_S, VPSS_SMMU_FAULT_TLB_WR_S,   VPSS_SMMU_FAULT_ID_WR_S;
            HI_U32 VPSS_SMMU_SCB_TTBR;

            VPSS_SMMU_FAULT_ADDR_PTW_S   = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ADDR_PTW_S.u32));
            VPSS_SMMU_FAULT_ID_PTW_S     = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ID_PTW_S.u32));
            VPSS_SMMU_FAULT_PTW_NUM      = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_PTW_NUM.u32));
            VPSS_SMMU_FAULT_ADDR_RD_S    = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ADDR_RD_S.u32));
            VPSS_SMMU_FAULT_TLB_RD_S     = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_TLB_RD_S.u32));
            VPSS_SMMU_FAULT_ID_RD_S      = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ID_RD_S.u32));
            VPSS_SMMU_SCB_TTBR           = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_SCB_TTBR.u32));
            VPSS_SMMU_FAULT_ADDR_WR_S    = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ADDR_WR_S.u32));
            VPSS_SMMU_FAULT_TLB_WR_S     = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_TLB_WR_S.u32));
            VPSS_SMMU_FAULT_ID_WR_S      = VPSS_REG_RegRead((volatile HI_U32 *) & (pstVpssRegs->VPSS_SMMU_FAULT_ID_WR_S.u32));


            if (0x0 != VPSS_SMMU_INTSTAT_S.bits.ints_tlbinvalid_rd_stat)
            {
                HI_PRINT("VPSS_SMMU_READ_ERR MODULE : %s  SSTATE : %#x ReadAddr:%#x\n",
                         HI_MOD_VPSS,
                         VPSS_SMMU_INTSTAT_S.u32,
                         VPSS_SMMU_FAULT_ADDR_RD_S);
            }

            if (0x0 != VPSS_SMMU_INTSTAT_S.bits.ints_tlbinvalid_wr_stat)
            {
                HI_PRINT("VPSS_SMMU_WRITE_ERR MODULE : %s  SSTATE : %#x WriteAddr:%#x\n",
                         HI_MOD_VPSS,
                         VPSS_SMMU_INTSTAT_S.u32,
                         VPSS_SMMU_FAULT_ADDR_WR_S);
            }

            HI_PRINT(
                "VPSS_SMMU_FAULT_ADDR_PTW_S:%x\nVPSS_SMMU_FAULT_ID_PTW_S:%u\nVPSS_SMMU_FAULT_PTW_NUM:%u\nVPSS_SMMU_FAULT_TLB_RD_S:%#x\nVPSS_SMMU_FAULT_ID_RD_S:%u\nVPSS_SMMU_FAULT_TLB_WR_S:%#x\nVPSS_SMMU_FAULT_ID_WR_S:%u\nVPSS_SMMU_SCB_TTBR:%x\n"
                ,
                VPSS_SMMU_FAULT_ADDR_PTW_S,
                VPSS_SMMU_FAULT_ID_PTW_S,
                VPSS_SMMU_FAULT_PTW_NUM,
                VPSS_SMMU_FAULT_TLB_RD_S,
                VPSS_SMMU_FAULT_ID_RD_S,
                VPSS_SMMU_FAULT_TLB_WR_S,
                VPSS_SMMU_FAULT_ID_WR_S,
                VPSS_SMMU_SCB_TTBR
            );
        }
