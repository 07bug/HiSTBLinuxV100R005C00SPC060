...
//#define ALGBUF_MANAGE_MODE1 //两种模式2选一，或者不选。 模式1:在newImg时，申请一块SMMU buffer
#define ALGBUF_MANAGE_MODE2 //模式2:模块加载后 申请一块VPSS算法所需SMMU buffer
#endif
#define VPSS_WBCINFO_COPY_ALL
...
#define VPSS_KFREE(ptr)         {HI_KFREE(HI_ID_VPSS, ptr); ptr=HI_NULL;}
#define VPSS_VMALLOC(fmt...)    HI_VMALLOC(HI_ID_VPSS, fmt)
#define VPSS_VFREE(ptr)         {HI_VFREE(HI_ID_VPSS, ptr); ptr=HI_NULL;}
#define VPSS_VFREE_SAFE(ptr)    {if(HI_NULL!=ptr) {HI_VFREE(HI_ID_VPSS, ptr);ptr=HI_NULL;}}
...
#define VPSS_CHECK_ZERO_RETURN_FAILURE(para)\
    do\
    {\
        if (0 == para)\
        {\
            VPSS_ERROR("Para is Zero \n");\
            return HI_FAILURE;\
        }\
    }while(0)

#define VPSS_CHECK_ZERO_RETURN_NULL(para)\
    do\
    {\
        if (0 == para)\
        {\
...
#define VPSS_CHECK_OVER_RANGE_RE_FAIL(ID, MaxID) \
    do{\
        if (ID >= MaxID){\
            VPSS_ERROR("[%d] over range!\n",ID);\
            return HI_VPSS_RET_OVERFLOW;\
        }\
    }while(0)
...
#define VPSS_CHECK_ENUM(value, least, biggest) do { \
        if ((value) >= (biggest)) \
        { \
            VPSS_ERROR("Invalid value:%d, normal range is [%d,%d]\n", (value), (least), (biggest)); \
            return HI_FAILURE; \
        } \
    } while (0);

#define VPSS_IS_VALID_ENUM(value, least, biggest) do { \
        if ((value) >= (biggest)) \
        { \
            VPSS_ERROR("Invalid value:%d, normal range is [%d,%d)\n", (value), (least), (biggest)); \
            return HI_FALSE; \
...
typedef enum hiVPSS_MCDI_SPLIT_MODE_E
{
    VPSS_MCDI_SPLIT_MODE_0 = 0, /* Mcdi Split Mode */
    VPSS_MCDI_SPLIT_MODE_1 = 1, /* Mcdi Split Mode */
    VPSS_MCDI_SPLIT_MODE_2 = 2, /* Mcdi Split Mode */
    VPSS_MCDI_SPLIT_MODE_3 = 3, /* Mcdi Split Mode */
    VPSS_MCDI_SPLIT_MODE_4 = 4, /* Mcdi Split Mode */
    VPSS_MCDI_SPLIT_MODE_5 = 5, /* Mcdi Split Mode */

    VPSS_MCDI_SPLIT_MODE_BUTT
} VPSS_MCDI_SPLIT_MODE_E;
...
#define VPSS_FRAME_NO_NORMAL    0xFFFFFFFD
#define VPSS_FRAME_NO_LAST2     (VPSS_FRAME_NO_NORMAL + 1)
#define VPSS_FRAME_NO_LAST1     (VPSS_FRAME_NO_LAST2 + 1)
...
#define VPSS_HEIGHT_ALIGN_4PIXELCLK 0XFFFFFFFE
#define VPSS_WIDTH_ALIGN_4PIXELCLK  0XFFFFFFFE
#else
...
#define VPSS_PIXLE_UPALIGN_W(w, fourpixelclk) {\
        if (fourpixelclk) {\
            (w) = (((HI_U32)(w)+(0XFFFFFFFF - VPSS_WIDTH_ALIGN_4PIXELCLK)) & (VPSS_WIDTH_ALIGN_4PIXELCLK));\
        } else {\
            (w) = (((HI_U32)(w)+(0XFFFFFFFF - VPSS_WIDTH_ALIGN)) & (VPSS_WIDTH_ALIGN));\
        }}

#define VPSS_PIXLE_UPALIGN_H(h, fourpixelclk) {\
        if (fourpixelclk) {\
            (h) = (((HI_U32)(h)+(0XFFFFFFFF - VPSS_WIDTH_ALIGN_4PIXELCLK)) & (VPSS_HEIGHT_ALIGN_4PIXELCLK));\
        } else {\
            (h) = (((HI_U32)(h)+(0XFFFFFFFF - VPSS_WIDTH_ALIGN)) & (VPSS_HEIGHT_ALIGN));\
        }}
...
#define VPSS_HDR_TYPE_NEED_BYPASS_VPSS(type) (HI_FALSE)
...
//#define VPSS_SUPPORT_OUT_TUNNEL_DBG
#define VPSS_OUT_TUNNEL_FINISHLINE  30  //30%

#define VPSS_TUNNEL_DBG_INFO(fmt...)  // printk(fmt)
//#define VPSS_INTF_TIMESTAMP
...
#define VPSS_SUPPORT_SDDEI_TMPBUF
#else
...
#define VPSS_SUPPORT_SPLIT_MCDI
#endif

#if defined(CHIP_TYPE_hi3798mv310)
#define VPSS_SUPPORT_BLKMT
#define VPSS_SUPPORT_DICNT
#define VPSS_SUPPORT_TB_SAME_ADDR
#define VPSS_SUPPORT_10BIT_DITHER  //8+2bit in, before zme,dither to 8bit
#define HI_VPSS_SUPPORT_OUTTUNNEL_MMZ
#define VPSS_SUPPORT_SPLIT_MCDI_MANR
#define VPSS_SUPPORT_SPLIT_MCDI
#define VPSS_NOT_SUPPORT_MCNR_FMT(fmt) (HI_FALSE)
...
#define HI_VPSS_SUPPORT_OUTTUNNEL_SMMU
#else
#define HI_VPSS_SUPPORT_OUTTUNNEL_MMZ
#endif
...
#define VPSS_FRAME_MAX_HEIGHT           4096
