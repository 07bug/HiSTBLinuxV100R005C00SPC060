                    ...
                    HI_DRV_VPSS_BUFLIST_CFG_S *pstBufListCfg)
{
    ...
    VPSS_SAFE_MEMCPY( pstBufCfg, pstBufListCfg,
        ...
        if (pstNode != HI_NULL)
        {
            ...
            switch (pstBufCfg->eBufType)
            {
                case HI_DRV_VPSS_BUF_VPSS_ALLOC_MANAGE:
                    ...
                    pstExtBuf->u32StartPhyAddr = 0;
                    pstExtBuf->pu8StartVirAddr = 0;
                    pstExtBuf->u32Size = 0;
...
HI_S32 VPSS_FB_DelInit(VPSS_FB_INFO_S *pstFrameList)
{
    ...
    list_for_each_safe(pos, n, &(pstFrameList->stEmptyFrmList))
    {
        ...
        pstExtBuf = &(pstTarget->stBuffer.stExtBuf);
...
#if 0 //in new flow, this case shoule be normal
...
HI_S32 VPSS_FB_GetFulFrmBuf(VPSS_FB_INFO_S *pstFrameList, HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *pchFile)
{
    ...
    if (pstNextNode != &(pstFrameList->stFulFrmList))
    {
        ...
        {
            VPSS_SAFE_MEMCPY(pstFrame, pstFrm, sizeof(HI_DRV_VIDEO_FRAME_S));
        ...
        else
        {
            if (pstNextNode->next != &(pstFrameList->stFulFrmList))
            {
                ...
                /* 将左眼的输出地址拷贝到右眼 */
                VPSS_SAFE_MEMCPY(&pstFrm->stBufAddr[0], &(pstLeftNode->stOutFrame.stBufAddr[0]), sizeof(HI_DRV_VID_FRAME_ADDR_S));
...
HI_S32 VPSS_FB_RelFulFrmBuf(VPSS_FB_INFO_S *pstFrameList, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
         ...
         pos = n, n = pos->next)
    {
            ...
            && (pstBuf->stMemBuf.u32StartPhyAddr != 0))
        {
            ...
            {
                VPSS_WARN("Rel Error BufferId %d FrmId %d RelCount %d "
                          "Frame Yaddr0:0x%p Yaddr2:0x%p BufferFrm Yaddr:0x%p \n",
                          pstTarget->stOutFrame.u32FrameIndex,
                          pstFrame->u32FrameIndex, u32RelCount,
                          pstFrame->stBufAddr[0].u32PhyAddr_Y,
                          pstFrame->stBufAddr[1].u32PhyAddr_Y,
                          pstBuf->stMemBuf.u32StartPhyAddr
                         );
    ...
    if ((s32GetFrm == HI_FAILURE) && (u32RelCount == 0))
    {
        VPSS_WARN("Can't Get RelFrm %d ,RelCount %d\n",
                  pstFrame->u32FrameIndex, u32RelCount);
    }
    else
    {
...
VPSS_FB_NODE_S *VPSS_FB_GetEmptyFrmBufNoMmz(VPSS_FB_INFO_S *pstFrameList, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (pstTarget)
    {
        ...
        VPSS_SAFE_MEMCPY(&(pstTarget->stOutFrame), pstFrame,
                                       ...
                                       HI_U32 u32ExtBufSize)
{
    ...
    if (pstTarget)
    {
        ...
        if (pstFrameList->stBufListCfg.eBufType == HI_DRV_VPSS_BUF_VPSS_ALLOC_MANAGE)
        {
            ...
            if (HI_SUCCESS != VPSS_OSAL_CalBufSize(&u32BufSize, &u32BufStride,
                                                   u32Height, u32Width, ePixFormat, enOutBitWidth))
            {
                ...
                pstMemBuf->u32StartPhyAddr = 0;
                pstMemBuf->pu8StartVirAddr = 0;
                (HI_VOID)VPSS_FB_AddEmptyFrmBuf(pstFrameList, pstTarget, VPSS_FB_TYPE_NORMAL);
                VPSS_FATAL("Calc Buffer size Failed.ePixFormat %d\n", ePixFormat);
                return HI_NULL;
            }
            ...
            if (pstExtBuf->u32Size != u32ExtBufSize)
            {
                if (u32ExtBufSize == 0)
                {
                    if (pstExtBuf->u32Size != 0 )
                    {
                        VPSS_OSAL_FreeMem(pstExtBuf);
                    }
                    pstExtBuf->u32Size = 0;
                ...
                else
                {
                    ...
                    pstExtBuf->pu8StartVirAddr = 0;
                    //fix to ree buffer
                    s32Ret = VPSS_OSAL_AllocateMem(VPSS_MEM_FLAG_NORMAL, u32ExtBufSize,
                                                   HI_NULL,
                                                   "VPSS_MetaBuf",
                                                   pstExtBuf);
                    ...
                    if (HI_SUCCESS != s32Ret)
                    {
                        ...
                        VPSS_FB_AddEmptyFrmBuf(pstFrameList, pstTarget, VPSS_FB_TYPE_NORMAL);
                        VPSS_FATAL("Dynamic Alloc Ext Buffer Failed.BufSize %#x\n", u32ExtBufSize);
                        return HI_NULL;
                    }
                }
            }
...
HI_S32 VPSS_FB_AddFulFrmBuf(VPSS_FB_INFO_S *pstFrameList, VPSS_FB_NODE_S *pstFBNode)
{
    ...
    if (pstFBNode->stOutFrame.bIsFirstIFrame && 0 == pstFrameList->u32FirstIFrameCnt)
    {
        HI_DRV_STAT_Event(STAT_EVENT_VPSSOUTFRM, 0);
        pstFrameList->u32FirstIFrameCnt++;
    }
    return HI_SUCCESS;
}
...
HI_BOOL VPSS_FB_CheckIsAvailable(VPSS_FB_INFO_S *pstFrameList, HI_U32  u32AvailableNum)
{
    ...
    else if ((DEF_VPSS_3D_BUFFER_DEPEND_NUM == u32AvailableNum)
...
HI_S32 VPSS_FB_Reset(VPSS_FB_INFO_S *pstFrameList)
{
    ...
    {
        VPSS_FATAL("Reset Error\n");
        u32RelCount = DEF_HI_DRV_VPSS_PORT_BUFFER_MAX_NUMBER;
    }
...
HI_S32 VPSS_FB_GetState(VPSS_FB_INFO_S *pstFrameList, VPSS_FB_STATE_S *pstFbState)
{
    ...
    {
        pstFbState->pstTarget_1 = list_entry(pstFrameList->pstTarget_1, VPSS_FB_NODE_S, node);
        pstFbNode = list_entry(pstFrameList->pstTarget_1, VPSS_FB_NODE_S, node);
        pstFbState->u32OutRate = pstFbNode->stOutFrame.u32FrameRate;
    }
    else
    {
        pstFbState->pstTarget_1 = &(pstFrameList->stFulFrmList);
    }
...
HI_S32 VPSS_FB_RlsExtBuffer(VPSS_FB_INFO_S *pstFrameList)
{
    ...
    list_for_each_safe(pos, n, &(pstFrameList->stExtFrmList))
    {
        ...
        if (u32RlsCnt >= DEF_HI_DRV_VPSS_PORT_BUFFER_MAX_NUMBER)
        {
            VPSS_FATAL("RlsExtBuffer Error\n");
            break;
        }
