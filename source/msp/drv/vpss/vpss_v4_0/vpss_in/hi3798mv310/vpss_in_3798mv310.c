...
HI_BOOL VPSS_IN_CheckImage_V3(HI_DRV_VIDEO_FRAME_S *pstImage)
{
    ...
    //resolution change.
    if (pstPriv->u32LastFlag == DEF_HI_DRV_VPSS_FAKE_FLAG)
    {
        pstPriv->u32LastFlag = DEF_HI_DRV_VPSS_LAST_ERROR_FLAG;
    }
    ...
    VPSS_IS_VALID_ENUM(pstImage->enSrcFrameType, HI_DRV_VIDEO_FRAME_TYPE_SDR, HI_DRV_VIDEO_FRAME_TYPE_BUTT);
    VPSS_IS_VALID_ENUM(pstImage->enFieldMode, HI_DRV_FIELD_TOP, HI_DRV_FIELD_BUTT);
    VPSS_IS_VALID_ENUM(pstImage->enBufValidMode, HI_DRV_FIELD_TOP, HI_DRV_FIELD_BUTT);
    VPSS_IS_VALID_ENUM(pstImage->enBitWidth, HI_DRV_PIXEL_BITWIDTH_8BIT, HI_DRV_PIXEL_BITWIDTH_BUTT);
    VPSS_IS_VALID_ENUM(pstImage->enCodecType, HI_DRV_STD_START_RESERVED, HI_DRV_STD_END_RESERVED);
    VPSS_IS_VALID_ENUM(pstImage->enTBAdjust, HI_DRV_VIDEO_TB_PLAY, HI_DRV_VIDEO_TB_BUTT);
...
HI_VOID VPSS_IN_CorrectProgByCfg(HI_DRV_VPSS_PRODETECT_E enProgInfo, HI_DRV_VIDEO_FRAME_S *pImage)
{
        ...
        else if (pImage->u32Height <= 576)
        {
            if ( (240 >= pImage->u32Height) && (320 >= pImage->u32Width) )
            {
                pImage->bProgressive = HI_TRUE;
                     ...
                     || ((pImage->u32Width == 520) && (pImage->u32Height == 360)))
            {
            ...
            else
            {
                if (HI_UNF_VCODEC_TYPE_HEVC == pstVdecPriv->entype)
                {
                    ...
                    {
                        return; //trust h265's prog=true attr.
                    }
                }
    ...
    else if (enProgInfo == HI_DRV_VPSS_PRODETECT_OFF)
    {
        VPSS_INFO("ProgProdetect is off\n");
    }
    else
    {
...
HI_S32 VPSS_IN_ReviseImage(VPSS_IN_ENTITY_S *pstEntity, HI_DRV_VIDEO_FRAME_S *pImage)
{
    ...
    /* too small imgs do not support madi */
    if ((pImage->u32Width < 128) || (pImage->u32Height < 64))
    {
        pImage->bProgressive = HI_TRUE;
    }
    ...
    if (pstEntity->stStreamInfo.u32StreamH != 0)
    {
        if ((pstEntity->stStreamInfo.u32StreamH != pImage->u32Height) || (pstEntity->stStreamInfo.u32StreamW != pImage->u32Width))
        {
            if (pImage->u32Height == 1080 && pImage->u32Width == 1920)
            {
                pstEntity->stStreamInfo.u32RealTopFirst = DEF_TOPFIRST_BUTT;
                pImage->bTopFieldFirst = 0;
            }
        }
    }
    if (pImage->u32Height == 720 && pImage->u32Width == 1280)
    {
        pImage->bTopFieldFirst = 1;
    }
#endif
    ...
    if ((pImage->eFrmType == HI_DRV_FT_SBS) || (pImage->eFrmType == HI_DRV_FT_TAB))
    {
        ...
        VPSS_SAFE_MEMCPY(&(pImage->stBufAddr_LB[1]), &(pImage->stBufAddr_LB[0]),
                         sizeof(HI_DRV_VID_FRAME_ADDR_S));
    }
    ...
    //donot support 10bit_i and 3d-10bit
    if (HI_DRV_PIXEL_BITWIDTH_10BIT == pImage->enBitWidth)
    {
        if ((HI_TRUE != pImage->bProgressive) || (HI_DRV_FT_NOT_STEREO != pImage->eFrmType))
        {
...
HI_S32 VPSS_IN_UpdateStreamInfo(VPSS_IN_ENTITY_S *pstEntity, HI_DRV_VIDEO_FRAME_S *pstSrcImage)
{
    ...
    {
        if (pstStreamInfo->bH265Interlace == HI_FALSE)
        {
            pstStreamInfo->u32IsNewImage = HI_TRUE;
        }
    }
...
HI_S32 VPSS_IN_NewImgInit(VPSS_IN_ENTITY_S *pstEntity, HI_DRV_VIDEO_FRAME_S *pstImage)
{
    ...
    VPSS_NRMADINFO_S *pstNrMadInfo;
    VPSS_MCDEI_INFO_S *pstMcdeiInfo;
    VPSS_MCDI_SPLIT_INFO_S stSplitInfo;
    unsigned long flags;
    HI_BOOL     bDEISecure;
    HI_DRV_VIDEO_PRIVATE_S *pstPriv;
    HI_VDEC_PRIV_FRAMEINFO_S *pstVdecPriv;
    ...
    HI_S32 s32Ret;
#if defined(ALGBUF_MANAGE_MODE2)
    HI_U32 u32InstanceId = pstEntity->u32InstanceId;
#endif
...
#endif
#if defined(ALGBUF_MANAGE_MODE2)
    pstWbcInfo->u32InstanceId = u32InstanceId;
    pstMcdeiInfo->stRgmeInfo.u32InstanceId = u32InstanceId;
    pstMcdeiInfo->stPrjhInfo.u32InstanceId = u32InstanceId;
    pstMcdeiInfo->stPrjvInfo.u32InstanceId = u32InstanceId;
    pstMcdeiInfo->stBlendWbcInfo.u32InstanceId = u32InstanceId;
    pstMcdeiInfo->stRgmeWbcInfo.u32InstanceId = u32InstanceId;
    pstMcdeiInfo->stBlkmvInfo.u32InstanceId = u32InstanceId;
#ifdef VPSS_SUPPORT_BLKMT
    pstMcdeiInfo->stBlkmtInfo.u32InstanceId = u32InstanceId;
#endif
#ifdef VPSS_SUPPORT_DICNT
    pstMcdeiInfo->stDiCntInfo.u32InstanceId = u32InstanceId;
#endif
    pstDieStInfo->u32InstanceId = u32InstanceId;
    pstNrMadInfo->u32InstanceId = u32InstanceId;
    psttWbc->u32InstanceId = u32InstanceId;
#endif
    ...
    stWbcAttr.enBitWidth = HI_DRV_PIXEL_BITWIDTH_8BIT; //8bit for rwzb
    ...
    pstEntity->bSplitEnable = VPSS_HAL_SupportMcSplit(pstEntity->enSplitMode,
                              pstImage->u32Width,
                              pstImage->bProgressive);

    stWbcAttr.bSplitEnable = pstEntity->bSplitEnable;
    stWbcAttr.enSplitMode = pstEntity->enSplitMode;
    stNrAttr.bSplitEnable = pstEntity->bSplitEnable;
    stNrAttr.enSplitMode = pstEntity->enSplitMode;

    stSplitInfo.bProgressive = pstImage->bProgressive;
    stSplitInfo.bSplitEnable = pstEntity->bSplitEnable;
    stSplitInfo.enSplitMode = pstEntity->enSplitMode;
    ...
    else
    {
        ...
        if (HI_DRV_FT_NOT_STEREO == pstImage->eFrmType)
        {
            ...
            s32InitListRet |= VPSS_INST_McDeiInit(pstMcdeiInfo, pstImage, &stSplitInfo);
        ...
        else
        {
            ...
            s32InitListRet |= VPSS_INST_McDeiInit((&pstEntity->stMcdeiInfo[1]), pstImage, &stSplitInfo);
#endif
        }
    }
...
HI_S32 VPSS_IN_Refresh_V3(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    */
...
#if 0
    if ((pstImage->u32FrameIndex > 0) && (pstImage->u32FrameIndex % 10 == 0))
    {
        HI_DRV_VIDEO_PRIVATE_S *pstPriv;
        pstPriv = (HI_DRV_VIDEO_PRIVATE_S *) & (pstImage->u32Priv[0]);
        pstPriv->u32LastFlag = DEF_HI_DRV_VPSS_LAST_ERROR_FLAG;
    ...
    if (HI_TRUE == pstImage->bIsFirstIFrame)
    {
...

{
...
#ifdef VPSS_WBCINFO_COPY_ALL
    HI_DRV_VID_FRAME_ADDR_S stBufAddr_tmp[HI_DRV_BUF_ADDR_MAX];
    HI_DRV_VID_FRAME_ADDR_S stBufAddrLB_tmp[HI_DRV_BUF_ADDR_MAX];
    HI_DRV_FRAME_TYPE_E eFrmType_tmp;
    HI_U32 u32Width_tmp;
    HI_U32 u32Height_tmp;
    HI_DRV_PIX_FORMAT_E ePixFormat_tmp;
    HI_BOOL bProgressive_tmp;
    HI_BOOL bCompressd_tmp;
    HI_DRV_PIXEL_BITWIDTH_E bitwidth_tmp;
    HI_BOOL bTopFieldFirst_tmp;
#endif
    ...
    if ((HI_SUCCESS != s32Ret) || (HI_NULL == pstWbc))
    {
        VPSS_FATAL("GetInfo VPSS_IN_INFO_WBC fail! Errcode:0x%x\n", s32Ret);
        return s32Ret;
    }
#ifdef VPSS_WBCINFO_COPY_ALL
    eFrmType_tmp = pstWbc->eFrmType;
    u32Width_tmp = pstWbc->u32Width;
    u32Height_tmp = pstWbc->u32Height;
    ePixFormat_tmp =  pstWbc->ePixFormat;
    bProgressive_tmp = pstWbc->bProgressive;
    VPSS_SAFE_MEMCPY(stBufAddr_tmp, pstWbc->stBufAddr, sizeof(stBufAddr_tmp));
    VPSS_SAFE_MEMCPY(stBufAddrLB_tmp, pstWbc->stBufAddr_LB, sizeof(stBufAddrLB_tmp));
    bCompressd_tmp = pstWbc->bCompressd;
    bitwidth_tmp = pstWbc->enBitWidth;
    bTopFieldFirst_tmp = pstWbc->bTopFieldFirst;
    VPSS_SAFE_MEMCPY(pstWbc, pstCur, sizeof(HI_DRV_VIDEO_FRAME_S));
    pstWbc->eFrmType = eFrmType_tmp;
    pstWbc->u32Width = u32Width_tmp;
    pstWbc->u32Height = u32Height_tmp;
    pstWbc->ePixFormat = ePixFormat_tmp;
    pstWbc->bProgressive = bProgressive_tmp;
    VPSS_SAFE_MEMCPY(pstWbc->stBufAddr, stBufAddr_tmp, sizeof(stBufAddr_tmp));
    VPSS_SAFE_MEMCPY(pstWbc->stBufAddr_LB, stBufAddrLB_tmp, sizeof(stBufAddrLB_tmp));
    pstWbc->bCompressd = bCompressd_tmp;
    pstWbc->enBitWidth = bitwidth_tmp;
    pstWbc->bTopFieldFirst = bTopFieldFirst_tmp;
#else
    ...
    pstWbc->enFieldMode = pstCur->enFieldMode;
    ...
    if (pstEntity->stWbcInfo[enLR].stWbcAttr.enMode < 2)
    {
        VPSS_ERROR("Invalid wbc mode:%d\n", pstEntity->stWbcInfo[enLR].stWbcAttr.enMode);
        return HI_FAILURE;
    }

    s32RefCount = pstEntity->stWbcInfo[enLR].stWbcAttr.enMode - 2;

    if (s32RefCount < (VPSS_WBC_MODE_BUTT - 2))
    {
        if (pstRef[s32RefCount] != HI_NULL)
        {
            //VPSS_ERROR("pstRef[i]  %d %#x\n",s32RefCount,pstRef[s32RefCount]->stBufAddr[0].u32PhyAddr_Y);
            VPSS_IN_CopyHalFrameInfo(pstRef[s32RefCount], &pstHalInfo->stInRefInfo[s32RefCount], enLR);

            for (i = (s32RefCount - 1); i >= 0; i--)
            {
                if (pstRef[i] != HI_NULL)
                {
                               ...
                               HI_VOID *pstInfo)
{
    ...
    switch (pstHalInfo->enNodeType)
    {
        ...
        case VPSS_HAL_NODE_2D_DEI_FIRST_3FIELD:
        case VPSS_HAL_NODE_2D_DEI_FIRST_3FIELD_VIRTUAL:
        ...
        case VPSS_HAL_NODE_2D_H265_STEP2_DEI:
            ...
            pstHalInfo = (VPSS_HAL_INFO_S *)pstInfo;
            pstMcdeiInfo = &pstEntity->stMcdeiInfo[enLR];
            stSplitInfo.bProgressive = pstEntity->stSrcImage.bProgressive;
            stSplitInfo.bSplitEnable = pstEntity->bSplitEnable;
            stSplitInfo.enSplitMode = pstEntity->enSplitMode;

            VPSS_INST_SetHalMcdeiInfo(pstMcdeiInfo, pstHalInfo, enLR, pstSrcImage, &stSplitInfo);
            break;
                            ...
                            HI_VOID *pstInfo)
{
    ...
    VPSS_CHECK_NULL(pstEntity);
    VPSS_CHECK_ENUM(enLR, HI_DRV_BUF_ADDR_LEFT, HI_DRV_BUF_ADDR_MAX);
    VPSS_CHECK_VERSION(pstEntity->enVersion, VPSS_VERSION_V4_0);
    ...
    switch (enType)
    {
        ...
        case VPSS_SET_INFO_PTS:
        {
            ...
            if (HI_FALSE == pstEntity->stStreamInfo.u32StreamProg)
            {
                ...
                {
#ifdef VPSS_WBCINFO_COPY_ALL
                    w_tmp = pstFrm->u32Width;
                    h_tmp = pstFrm->u32Height;
                    VPSS_SAFE_MEMCPY(stBufAddr_tmp, pstFrm->stBufAddr, sizeof(stBufAddr_tmp));
                    VPSS_SAFE_MEMCPY(stBufAddrLB_tmp, pstFrm->stBufAddr_LB, sizeof(stBufAddrLB_tmp));
                    VPSS_SAFE_MEMCPY(pstFrm, pstRef[1], sizeof(HI_DRV_VIDEO_FRAME_S));
                    VPSS_SAFE_MEMCPY(pstFrm->stBufAddr, stBufAddr_tmp, sizeof(stBufAddr_tmp));
                    VPSS_SAFE_MEMCPY(pstFrm->stBufAddr_LB, stBufAddrLB_tmp, sizeof(stBufAddrLB_tmp));
                    pstFrm->u32Width = w_tmp;
                    pstFrm->u32Height = h_tmp;
#else
