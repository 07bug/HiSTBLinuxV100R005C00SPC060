...
HI_S32 VPSS_SRC_DeInit(VPSS_SRC_S *pstSrc)
{
    ...
    s32Ret = VPSS_SRC_ReleaseFullSrcList(pstSrc);

    if (HI_SUCCESS != s32Ret)
    {
        VPSS_FATAL("VPSS_SRC_ReleaseFullSrcList fail!\n");
    }


    if (pstSrc->u32PutSrcCount != pstSrc->u32ReleaseSrcCount)
    {
        VPSS_FATAL("DEINIT get count %d is not equal to release count %d\n",
                   pstSrc->u32PutSrcCount, pstSrc->u32ReleaseSrcCount);
    }
#if 0
    else
    {
        VPSS_FATAL("DEINIT get count %d == release count %d\n",
                   pstSrc->u32PutSrcCount, pstSrc->u32ReleaseSrcCount);
    }
#endif
...
HI_S32 VPSS_SRC_Reset(VPSS_SRC_S *pstSrc)
{
    ...
    if (!pstSrc->bInit)
    {
        VPSS_WARN("VPSS src reset failed(not init).\n");
        return HI_FAILURE;
    }
    ...
    if (pstSrc->u32PutSrcCount != pstSrc->u32ReleaseSrcCount)
    {
        VPSS_FATAL("RESET get count %d is not equal to release count %d\n",
                   pstSrc->u32PutSrcCount, pstSrc->u32ReleaseSrcCount);
    }
#if 0
    else
    {
        VPSS_FATAL("RESET get count %d == release count %d\n",
                   pstSrc->u32PutSrcCount, pstSrc->u32ReleaseSrcCount);
    }
#endif
    pstSrc->pstTarget_1 = &(pstSrc->stFulSrcList);
...
HI_S32 VPSS_SRC_ReleaseFullSrcList(VPSS_SRC_S *pstSrc)
{
    VPSS_SRC_NODE_S *pstFstNode;
...
HI_S32 VPSS_SRC_PutImage(VPSS_SRC_S *pstSrc, VPSS_SRC_DATA_S *pstData)
{
    ...
    if (!pstSrc->bInit)
    {
        VPSS_WARN("VPSS_SRC_PutImage Delint.\n");
        return HI_FAILURE;
    }
    ...
    if (bProgressive && bFieldAll)
    {
        ...
        VPSS_SAFE_MEMCPY(&(pstDstNode->stSrcData), pstData, sizeof(VPSS_SRC_DATA_S));
    ...
    else if (!bProgressive && bFieldAll)
    {
        ...
        VPSS_SAFE_MEMCPY(&(pstFstDstNode->stSrcData), pstData, sizeof(VPSS_SRC_DATA_S));
        ...
        VPSS_SAFE_MEMCPY(&(pstSndDstNode->stSrcData), pstData, sizeof(VPSS_SRC_DATA_S));
        ...
        switch (pstData->ePixFormat)
        {
            ...
            {
#if 0  // tile format, top and bottom all cfg addr of top
            ...
            default:
            {
                ...
                pstReviseNode->stSrcData.stBufAddr[0].u32PhyAddr_Y = u32BaseAddr;

                u32BaseAddr = pstReviseNode->stSrcData.stBufAddr[0].u32PhyAddr_C;
                u32BaseStride =  pstReviseNode->stSrcData.stBufAddr[0].u32Stride_C;

                pstReviseNode->stSrcData.stBufAddr[0].u32PhyAddr_C = u32BaseAddr;
#else
                u32BaseAddr = pstReviseNode->stSrcData.stBufAddr[0].u32PhyAddr_Y;
                u32BaseStride =  pstReviseNode->stSrcData.stBufAddr[0].u32Stride_Y;
    ...
    else //(!bProgressive && !bFieldAll)
    {
        ...
        VPSS_DBG_INFO("bProgressive enBufValidMode  %d %d\n",  bProgressive,  pstData->enBufValidMode);
        pstDstNode = VPSS_SRC_DelEmptyNode(pstSrc);
        if (HI_NULL == pstDstNode)
        {
            return HI_FAILURE;
...
HI_S32 VPSS_SRC_GetProcessImage(VPSS_SRC_S *pstSrc, VPSS_SRC_DATA_S **pstData)
{
    ...
    if (!pstSrc->bInit)
    {
        VPSS_WARN("VPSS_SRC_GetProcessImage Delint.\n");
        return HI_FAILURE;
    }
...
HI_S32 VPSS_SRC_MoveNext(VPSS_SRC_S *pstSrc, HI_U32 u32NextCount)
{
    if (!pstSrc->bInit)
    {
        VPSS_WARN("VPSS_SRC_GetPreImgInfo Delint.\n");
        return HI_FAILURE;
    }
    ...
    {
        VPSS_WARN("VPSS_SRC_MoveNext error.\n");
        return HI_FAILURE;
    }
...
HI_S32 VPSS_SRC_CompleteImage(VPSS_SRC_S *pstSrc, HI_VOID *arg)
{
    ...
    if (!pstSrc->bInit)
    {
        VPSS_WARN("VPSS_SRC_CompleteImage Delint.\n");
        return HI_FAILURE;
    }
...
HI_S32 VPSS_SRC_CorrectListOrder(VPSS_SRC_S *pstImgInfo, HI_BOOL bTopFirst)
{
         ...
         pos = pos->next, n = pos->next)
    {
        ...
        if (pstImageList->pstTarget_1 != pos)
        {
            ...
            s64Tmp = pstCurFieldNode->stSrcData.s64OmxPts;
            pstCurFieldNode->stSrcData.s64OmxPts =
                pstPreFieldNode->stSrcData.s64OmxPts;
            pstPreFieldNode->stSrcData.s64OmxPts = s64Tmp;
        }
...
HI_S32 VPSS_SRCIN_Init(VPSS_SRCIN_S *pSrcIn)
{
    ...
    if (pSrcIn->bInit)
    {
        VPSS_WARN("SRCIN has been Inited\n");
        return HI_SUCCESS;
    }
...
HI_S32 VPSS_SRCIN_DeInit(VPSS_SRCIN_S *pSrcIn)
{
    ...
    if (!pSrcIn->bInit)
    {
        VPSS_WARN("SRCIN has been DeInited\n");
        return HI_SUCCESS;
    }
...
HI_S32 VPSS_SRCIN_SendImage(VPSS_SRCIN_S *pSrcIn, VPSS_SRC_DATA_S *pstData)
{
    ...
    VPSS_SAFE_MEMCPY(&pstDateTmp->stSrcData, pstData, sizeof(VPSS_SRC_DATA_S));
...
HI_S32 VPSS_SRCIN_CallImage(VPSS_SRCIN_S *pSrcIn, VPSS_SRC_DATA_S *pstData)
{
    ...
    {
        VPSS_WARN("SRCIN No Node to Call\n");
        VPSS_OSAL_UpSpin(&pSrcIn->stSrcInLock, &u32Flag);
        return HI_FAILURE;
    }
    ...
    list_del(pstListTmp);
    pstDateTmp = (VPSS_SRC_NODE_S *)list_entry(pstListTmp, VPSS_SRC_NODE_S, node);
    VPSS_SAFE_MEMCPY(pstData, &pstDateTmp->stSrcData, sizeof(VPSS_SRC_DATA_S));
...
HI_S32 VPSS_SRCIN_GetImage(VPSS_SRCIN_S *pSrcIn, VPSS_SRC_DATA_S *pstData)
{
    ...
    {
        VPSS_WARN("SRCIN No Node to Get\n");
        VPSS_OSAL_UpSpin(&pSrcIn->stSrcInLock, &u32Flag);
        return HI_FAILURE;
    }
