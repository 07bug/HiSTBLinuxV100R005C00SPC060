    ...
    switch (pstCmd->hDbgPart)
    {
        case DEF_DBG_SRC_ID:
            ...
            pstInstDbg->unInfo.bits.end_index = pstInstDbg->unInfo.bits.start_index +
                                                ((((HI_U32)pstCmd->hDbgValue) & 0xFFFF) > 10000 ? 10000 : (((HI_U32)pstCmd->hDbgValue) & 0xFFFF));
        ...
        case DEF_DBG_PORT2_ID:
            ...
            VPSS_CHECK_OVER_RANGE_RE_FAIL(u32Count, DEF_HI_DRV_VPSS_PORT_MAX_NUMBER);
...
HI_S32 VPSS_DBG_ReplyDbgCmd_SaveYUV(VPSS_DBG_S *pstDbg, VPSS_DEBUG_E enCmd, HI_VOID *para1, HI_VOID *para2)
{
    ...
    HI_S8 chFile[DEF_FILE_NAMELENGTH] = {0};
    HI_U32 u32Hour = 0;
    HI_U32 u32Minute = 0;
    HI_U32 u32Second = 0;
    HI_U16 u16StartIdx;
    ...
    VPSS_CHECK_NULL(pstDbg);
    VPSS_CHECK_OVER_RANGE_RE_FAIL(enCmd, DBG_BUTT);
    VPSS_CHECK_NULL(para1);
    VPSS_CHECK_NULL(para2);
    ...
    switch (u32DbgPart)
    {
        ...
        case DEF_DBG_PORT2_ID:
            ...
            chFile[DEF_FILE_NAMELENGTH - 1] = '\0';
...
HI_VOID VPSS_DBG_PrintFrameInfo(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_BOOL bSrcFrame, HI_U32 u32StartIndex, HI_U32 u32EndIndex)
{
    ...
    if (HI_TRUE == bSrcFrame)
    {
        VPSS_PROC_MSG_PRINT("Src Frame : Index %d Type %d Format %d W %d H %d Prog %d topfirst %d FieldMode %d enBufValidMode %d\n"
                            ...
                            "            AspW %u AspH %u  DispCtrlW %u DispCtrlH %u \n"
                            ...
                            pstFrame->DispCtrlWidth,
                            pstFrame->DispCtrlHeight,
    ...
    else
    {
        VPSS_PROC_MSG_PRINT("Out Frame : Index %d Type %d Format %d W %d H %d LW %d LH %d SrcPTS %d PTS %d OMXPTS %lld Rate %d Cnt %d Fidelity %d\n"
                            "            u32LastFlag %d,oriField %d,BitWidth %d,u32TunnelPhyAddr %x AspW %d AspH %d DispCtrlW %u DispCtrlH %u\n",
                            pstFrame->u32FrameIndex,
                            pstFrame->eFrmType,
                            pstFrame->ePixFormat,
                            pstFrame->u32Width,
...
HI_VOID VPSS_CTRL_ProcPrintTimeStamp(struct seq_file *p, VPSS_INSTANCE_S *pstInstance)
{
    ...
    for (u32Count = 0; u32Count < VPSS_PROC_TIMESTAMP_MAXCNT; u32Count++)
    {
        ...
        ulTotalTime = pstInstance->aulFinishTime[u32Count] - pstInstance->aulBeginTime[u32Count];
        bTunnelEnable = pstInstance->bOutLowDelay;
        ...
        {
            VPSS_PROC_INFO_PRINT(p,
                                 VPSS_PROC_LEVEL4 "TaskN+%d ListOrder %d"
                                 VPSS_PROC_LEVEL4 "InstBegin            : %lu"
                                 VPSS_PROC_LEVEL4 "InstCreat            : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "InstReady            : %lu(+%lums)"
                                 ...
                                 ,
                                 u32Count, pstInstance->au32RunOrder[u32Count],
                                 pstInstance->aulBeginTime[u32Count],
                                 pstInstance->aulCreatTime[u32Count],
                                 pstInstance->aulCreatTime[u32Count] - pstInstance->aulBeginTime[u32Count],

                                 pstInstance->aulReadyTime[u32Count],
                                 pstInstance->aulReadyTime[u32Count] - pstInstance->aulCreatTime[u32Count],

                                 pstInstance->aulLogicStartTime[u32Count],
                                 pstInstance->aulLogicStartTime[u32Count] - pstInstance->aulReadyTime[u32Count],
        ...
        {
            VPSS_PROC_INFO_PRINT(p,
                                 VPSS_PROC_LEVEL4 "TaskN+%d Order%d"
                                 VPSS_PROC_LEVEL4 "InstBegin            : %lu"
                                 VPSS_PROC_LEVEL4 "InstCreat            : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "InstReady            : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "LogicStart           : %lu(+%lums)"
...
HI_S32 VPSS_CTRL_ProcPrint_V2(struct seq_file *p, HI_VOID *v)
{
    ...
    VPSS_PORT_PRC_S *pstPortPrc[DEF_HI_DRV_VPSS_PORT_MAX_NUMBER] = {0};
    VPSS_PORT_S *pstPort;
    ...
    VPSS_PROC_INFO_PRINT(p,   VPSS_PROC_LEVEL1
                         ...
                         ,
                         pstInstance->ID,
                         VPSS_PROC_PRINT_ENUM(pstInstance->enState, INSTANCE_STATE_BUTT, g_pInstState),
                         VPSS_PROC_PRINT_YN_COL(pstInstance->stInEntity.stTransFbInfo.bLogicTransState),
                         g_pAlgModeString[pstInEntity->bAlwaysFlushSrc],
                         pcSourceName,
    ...
    if (pstDbgCtrl->stInstDbg.eProcPrintLevel >= PROC_PRINT_LEVEL2)
    {
        VPSS_PROC_INFO_PRINT(p, VPSS_PROC_LEVEL3 "TimeStamp(Inst:%d)", pstInstance->ID);
        VPSS_PROC_INFO_PRINT(p, VPSS_PROC_LEVEL2 "Statistic Info");
        VPSS_CTRL_ProcPrintTimeStamp(p, pstInstance);
        VPSS_PROC_INFO_PRINT(p, VPSS_PROC_LEVEL3 "Running Inst:%d", VPSS_CTRL_GetInstanceCnt(VPSS_IP_0));
#ifdef VPSS_INTF_TIMESTAMP
        VPSS_PROC_INFO_PRINT(p, VPSS_PROC_LEVEL3 "Interface Time Info");
        VPSS_OSAL_IntfTime_Print(p);
#endif
    }
    if (pstDbgCtrl->stInstDbg.eProcPrintLevel >= PROC_PRINT_LEVEL1)
    {
    ...
    if (pstDbgCtrl->stInstDbg.eProcPrintLevel >= PROC_PRINT_LEVEL1)
    {
...
#endif
        {
            if (HI_TRUE == pstInstance->abNodeVaild[u32Count])
            {
                ...
                pstNodeReg = HI_NULL;
                if (HI_SUCCESS == VPSS_HAL_GetNodeAddr(VPSS_IP_0, pstVpssHalInfo, u32Count, (HI_U8 **)&pstNodeReg, &pNodeName))
                {
...
HI_S32 VPSS_CTRL_ProcPrint_V1(struct seq_file *p, HI_VOID *v)
{
    ...
    VPSS_PROC_INFO_PRINT(p,
                         ...
                         (INSTANCE_STATE_STOP == pstInstance->enState) ? "stop" : "working",
                         ...
                         /*alg config*/
                         g_pAlgModeString[pstPortPrc[0]->bTunnelEnable],
...
#else
    return HI_SUCCESS;
#endif  // #ifdef HI_VPSS_PROC_SUPPORT
}
    ...
    if (DBG_BUTT == stDbgCmd.enDbgType)
    {
        if (!HI_OSAL_Strncmp(chArg1, DEF_DBG_CMD_SETBYPASS, DEF_FILE_NAMELENGTH))
        {
            ...
            if (!HI_OSAL_Strncmp(chArg2, DEF_DBG_SET_ON, DEF_FILE_NAMELENGTH))
            {
                ...
                VPSS_PROC_MSG_PRINT("Set vpss working in bypass mode!\n");
            }
            else
            {
                VPSS_PROC_MSG_PRINT("Set vpss working in normal mode!\n");
            }
        ...
        else if (!HI_OSAL_Strncmp(chArg1, DEF_DBG_CMD_RESET, DEF_FILE_NAMELENGTH))
        {
            ...
            if (!HI_OSAL_Strncmp(chArg2, DEF_DBG_SET_ON, DEF_FILE_NAMELENGTH))
            {
                ...
                VPSS_PROC_MSG_PRINT("Reset logic when vpss is idle!\n");
            }
            else
            {
                VPSS_PROC_MSG_PRINT("Keep logic running when vpss is idle!\n");
            }
