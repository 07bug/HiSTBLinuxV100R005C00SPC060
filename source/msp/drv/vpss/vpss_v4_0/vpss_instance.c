         ...
         u32Count ++)
    {
        ...
        if (pstPort->s32PortId != VPSS_INVALID_HANDLE)
        {
               ...
               )
            {
                ...
                {
                    VPSS_CHECK_ZERO_RETURN_FAILURE(u32LevelW);
                    VPSS_CHECK_ZERO_RETURN_FAILURE(u32LevelH);
                ...
                VPSS_CHECK_ZERO_RETURN_FAILURE(u32TmpW);
                VPSS_CHECK_ZERO_RETURN_FAILURE(u32TmpH);
                VPSS_CHECK_ZERO_RETURN_FAILURE(pstPort->s32OutputHeight);
...
HI_VOID VPSS_INST_RevisePortOutRect_Rotation(VPSS_INSTANCE_S *pstInstance)
{
    ...
    if (NULL == pstInstance)
    {
        VPSS_FATAL("null pointer pstInstance! \n");
        return;
    }
    pstStreamInfo = &(pstInstance->stInEntity.stStreamInfo);
    ...
    {
        pstPort = &(pstInstance->stPort[u32Count]);
        if ((pstPort->bEnble == HI_TRUE) && (pstPort->s32PortId != VPSS_INVALID_HANDLE))
        {
            if ((pstStreamInfo->u32StreamProg == HI_FALSE) &&
                (pstPort->enRotation == HI_DRV_VPSS_ROTATION_90 ||
                 pstPort->enRotation == HI_DRV_VPSS_ROTATION_270) &&
                (pstPort->s32OutputHeight > 2048))
            {
                pstPort->s32OutputHeight = 2048;
            }
        }
...
HI_VOID VPSS_INST_RevisePortOutRect_3d (VPSS_INSTANCE_S *pstInstance)
{
    ...
    pstStreamInfo = &(pstInstance->stInEntity.stStreamInfo);
    s32OutRectW = pstStreamInfo->u32StreamW;
    s32OutRectH = pstStreamInfo->u32StreamH;
    for (u32Count = 0; u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32Count++)
    {
        pstPort = &(pstInstance->stPort[u32Count]);
        pstPortCfg = &(pstInstance->stUsrPortCfg[u32Count]);
        if (HI_DRV_FT_SBS == pstStreamInfo->eStreamFrmType || HI_DRV_FT_TAB == pstStreamInfo->eStreamFrmType)
        {
            if (pstPort->s32PortId != VPSS_INVALID_HANDLE)
            {
                if (s32OutRectW > VPSS_FHD_WIDTH)
                {
                    pstPort->s32OutputWidth  = VPSS_FHD_WIDTH;
                    pstPort->s32OutputHeight = VPSS_FHD_HEIGHT;
                }
            }
        }
...
#endif
static HI_S32 VPSS_INST_ELZmeAndBitWidthPolicy(HI_U32 u32ElW, HI_U32 u32ElH,
        HI_U32 u32UserCfgOutW, HI_U32 u32UserCfgOutH, HI_U32 *ps32OutW, HI_U32 *ps32OutH,
        HI_DRV_PIXEL_BITWIDTH_E enElBitWidth,
        HI_DRV_PIXEL_BITWIDTH_E *penBitWidth)
{
#ifdef VPSS_ZME_WIDTH_MAX_FHD
    if ((u32UserCfgOutW > VPSS_FHD_WIDTH) || (u32UserCfgOutH > VPSS_FHD_HEIGHT))
    {
        *ps32OutW = u32UserCfgOutW / 2;
        *ps32OutH = u32UserCfgOutH / 2;

        if ((*ps32OutW > VPSS_FHD_WIDTH) || (*ps32OutH > VPSS_FHD_HEIGHT))
        {
            *ps32OutW = VPSS_FHD_WIDTH;
            *ps32OutH = VPSS_FHD_HEIGHT;
        }

        return HI_SUCCESS;
    }
#endif
    if  (((u32ElW > VPSS_FHD_WIDTH ) || ( u32ElH > VPSS_FHD_HEIGHT))
         || ((u32ElW <= u32UserCfgOutW / 2) && (u32ElH <= u32UserCfgOutH / 2)))
    {
        *ps32OutW = u32UserCfgOutW / 2;
        *ps32OutH = u32UserCfgOutH / 2;
    }
    ...
    {
        *ps32OutW = u32UserCfgOutW;
        *ps32OutH = u32UserCfgOutH;
    }

    *penBitWidth = enElBitWidth;
#ifndef VPSS_ZME_SUPPORT_10BIT
    if ((u32ElW != *ps32OutW ) || (u32ElH != *ps32OutH ))
    {
        *penBitWidth = HI_DRV_PIXEL_BITWIDTH_8BIT;
    }
#endif

    //printk("e(%d,%d),o(%d,%d),c(%d,%d)\n",u32ElW,u32ElH,*ps32OutW,*ps32OutH,u32UserCfgOutW,u32UserCfgOutH);
    return HI_SUCCESS;
}
HI_S32 VPSS_INST_ELFrameResoAndBitwidthAdjust(VPSS_INSTANCE_S *pstInstance)
{
    HI_U32 u32Count;
    VPSS_PORT_S *pstPort;
    HI_DRV_VPSS_PORT_CFG_S *pUserCfgstPort;
    VPSS_IN_STREAM_INFO_S *pstStreamInfo;
    HI_S32 s32Ret;
    HI_DRV_VIDEO_FRAME_S *pstCur;
    VPSS_IN_INTF_S stInIntf;
    ...
    if ((HI_SUCCESS != s32Ret) || (HI_NULL == pstCur))
    {
        VPSS_ERROR("VPSS_SRC_GetProcessImage failed!\n");
        return HI_FAILURE;
    }
    if (HI_DRV_VIDEO_FRAME_TYPE_DOLBY_EL != pstCur->enSrcFrameType)
    {
        return HI_SUCCESS;
    }
    for (u32Count = 0;
         ...
         u32Count ++)
    {
        pUserCfgstPort = &(pstInstance->stUsrPortCfg[u32Count]);
        pstPort = &(pstInstance->stPort[u32Count]);
        if (pstPort->s32PortId != VPSS_INVALID_HANDLE)
        {
            (HI_VOID)VPSS_INST_ELZmeAndBitWidthPolicy(pstCur->u32Width,
                    pstCur->u32Height,
                    pUserCfgstPort->s32OutputWidth,
                    pUserCfgstPort->s32OutputHeight,
                    &pstPort->s32OutputWidth,
                    &pstPort->s32OutputHeight,
                    pstCur->enBitWidth,
...
HI_S32 VPSS_INST_Reset(VPSS_INSTANCE_S *pstInstance)
{
    ...
    s32Ret = VPSS_CTRL_Pause(pstInstance->ID);
    if (HI_SUCCESS != s32Ret)
    {
        return s32Ret;
    }
    ...
    {
        pstInstance->aulBeginTime[u32Count] = 0;
        pstInstance->aulCreatTime[u32Count] = 0;
        pstInstance->aulReadyTime[u32Count] = 0;
...
HI_BOOL VPSS_INST_CheckAllDone(VPSS_INSTANCE_S *pstInstance)
{
     ...
     */
    s2Ret = HI_FAILURE;
    if (HI_TRUE == pstInEntity->stSrc.bInit)
    {
...
HI_S32 VPSS_INST_SetState(VPSS_INSTANCE_S *pstInstance, VPSS_INSTANCE_STATE_E enState)
{
    ...
    if (INSTANCE_STATE_STOP == enState)
    {
        pstInstance->eNextState = INSTANCE_STATE_STOP;
    }
    else if (INSTANCE_STATE_IDLE == enState)
    {
        pstInstance->eNextState = INSTANCE_STATE_IDLE;
    }
    else
    {
        pstInstance->eNextState = INSTANCE_STATE_IDLE;
...
HI_S32 VPSS_INST_Init(VPSS_INSTANCE_S *pstInstance, HI_DRV_VPSS_CFG_S *pstVpssCfg)
{
    ...
    pstInstance->ID = 0;
    pstInstance->hDst = 0;
    pstInstance->enState = INSTANCE_STATE_STOP;
    pstInstance->enLastState = INSTANCE_STATE_BUTT;
    pstInstance->eNextState = INSTANCE_STATE_STOP;
    ...
    VPSS_OSAL_InitEvent(&(pstInstance->stEventInstanceStop), EVENT_UNDO, EVENT_UNDO);
    ...
    {
        VPSS_HAL_INFO_S *pstVpssHalInfo;
        ...
        s32Ret = HI_DRV_SMMU_AllocAndMap("VPSS_LowDelay_mmu",
                                         4096, 0, &pstVpssHalInfo->stLowDelayBuf_MMU);

        VPSS_DBG_INFO("VPSS alloc LowDelayBuf smmu:%#x vir:%#x\n",
                      pstVpssHalInfo->stLowDelayBuf_MMU.u32StartSmmuAddr,
                      pstVpssHalInfo->stLowDelayBuf_MMU.pu8StartVirAddr);

#else
        s32Ret = HI_DRV_MMZ_AllocAndMap("VPSS_LowDelay_mmz", HI_NULL,
                                        4096, 0, &pstVpssHalInfo->stLowDelayBuf_MMZ);

        VPSS_DBG_INFO("VPSS alloc LowDelayBuf mmz:%#x vir:%#x\n",
                      pstVpssHalInfo->stLowDelayBuf_MMZ.u32StartPhyAddr,
                      pstVpssHalInfo->stLowDelayBuf_MMZ.pu8StartVirAddr);
#endif
...
HI_S32 VPSS_INST_DelInit(VPSS_INSTANCE_S *pstInstance)
{
    ...
    VPSS_IN_ENTITY_S *pstInEntity = HI_NULL;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_S32 s32NodeId = 0;
    VPSS_IN_INTF_S stInIntf;
    ...
    {
        HI_S32 PortId;
        VPSS_MEM_S *pstExtBuf;
        VPSS_MEM_S *pstMemBuf;

        for (PortId = 0; PortId < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER * 2; PortId++)
        {
            pstMemBuf = HI_NULL;
            pstExtBuf = HI_NULL;
            pstPort = &(pstInstance->stPort[PortId / 2]);

            if (pstPort->stFrmInfo.stBufListCfg.eBufType == HI_DRV_VPSS_BUF_VPSS_ALLOC_MANAGE)
            {
                if (HI_NULL != pstInstance->pstFrmNode[PortId])
                {
                    pstMemBuf = &(pstInstance->pstFrmNode[PortId]->stBuffer.stMemBuf);
                    pstExtBuf = &(pstInstance->pstFrmNode[PortId]->stBuffer.stExtBuf);

                    if (pstMemBuf->u32StartPhyAddr != 0 || pstMemBuf->pu8StartVirAddr != 0)
                    {
                        //VPSS_FATAL("Instance mem free:%#x\n", pstMemBuf->u32StartPhyAddr);
                        VPSS_OSAL_FreeMem(pstMemBuf);
                    }

                    if (pstExtBuf->u32StartPhyAddr != 0 || pstExtBuf->pu8StartVirAddr != 0)
                    {
                        //VPSS_FATAL("Instance extmem free:%#x\n", pstExtBuf->u32StartPhyAddr);
                        VPSS_OSAL_FreeMem(pstExtBuf);
                    }
                }
            }
        }
    ...
    {
#ifdef VPSS_HAL_WITH_CBB
        VPSS_HAL_FreeRotaBuf(&(pstInstance->stVpssHalInfo));
#else
        VPSS_BUFFER_S *pstVpssBuf = HI_NULL;
        for (u32Count = 0; u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32Count ++)
        {
#ifdef ZME_2L_TEST
            if ( pstInstance->pstFrmNodeRoBuf[u32Count] != HI_NULL)
            {
                pstVpssBuf = &(pstInstance->pstFrmNodeRoBuf[u32Count]->stBuffer);
                if (pstVpssBuf->stMemBuf.u32Size != 0)
                {
                    ...
                    VPSS_VFREE(pstInstance->pstFrmNodeRoBuf[u32Count]);
                    pstInstance->pstFrmNodeRoBuf[u32Count] = HI_NULL;
                }

            }

            if ( pstInstance->pstFrmNodeZME1L != HI_NULL)
            {
                pstVpssBuf = &(pstInstance->pstFrmNodeZME1L->stBuffer);
                if (pstVpssBuf->stMemBuf.u32Size != 0)
                {
                    (HI_VOID)VPSS_OSAL_FreeMem(&(pstVpssBuf->stMemBuf));
                    pstVpssBuf->u32Stride = 0;
    ...
    {
        if ((0x0 != pstVpssHalInfo->stNodeList[s32NodeId].u32AppPhy)
            && (HI_NULL != pstVpssHalInfo->stNodeList[s32NodeId].pu8AppVir))
        {
            VPSS_INFO("Free node: inst:%d node:%d phy:%#x vir:%p\n",
                      pstInstance->ID,
                      s32NodeId,
                      pstVpssHalInfo->stNodeList[s32NodeId].u32AppPhy,
                      pstVpssHalInfo->stNodeList[s32NodeId].pu8AppVir);

            HI_DRV_MMZ_UnmapAndRelease(&pstVpssHalInfo->stNodeList[s32NodeId].stRegBuf);
            memset(&pstVpssHalInfo->stNodeList[s32NodeId].stRegBuf, 0, sizeof(MMZ_BUFFER_S));
            pstVpssHalInfo->stNodeList[s32NodeId].u32AppPhy = 0x0;
            pstVpssHalInfo->stNodeList[s32NodeId].pu8AppVir = HI_NULL;
        }
        else if ((0x0 == pstVpssHalInfo->stNodeList[s32NodeId].u32AppPhy)
                 && (HI_NULL == pstVpssHalInfo->stNodeList[s32NodeId].pu8AppVir))
        {
        }
        else
        {
            VPSS_FATAL("Invalid addr phy:%#x vir:%p\n",
                       pstVpssHalInfo->stNodeList[s32NodeId].u32AppPhy,
                       pstVpssHalInfo->stNodeList[s32NodeId].pu8AppVir);
        }
    }
...
#ifdef VPSS_SUPPORT_OUT_TUNNEL
#ifdef HI_VPSS_SUPPORT_OUTTUNNEL_SMMU
    HI_DRV_SMMU_UnmapAndRelease(&pstVpssHalInfo->stLowDelayBuf_MMU);
    memset(&(pstVpssHalInfo->stLowDelayBuf_MMU), 0x0, sizeof(pstVpssHalInfo->stLowDelayBuf_MMU));
#else
    HI_DRV_MMZ_UnmapAndRelease(&pstVpssHalInfo->stLowDelayBuf_MMZ);
    memset(&(pstVpssHalInfo->stLowDelayBuf_MMZ), 0x0, sizeof(pstVpssHalInfo->stLowDelayBuf_MMZ));
#endif
#endif

#ifdef VPSS_SUPPORT_SDDEI_TMPBUF
    VPSS_HAL_SDDeiBuffer_Free(pstVpssHalInfo);
#endif
...
HI_U32 VPSS_INST_GetDefPortCfg(HI_DRV_VPSS_PORT_CFG_S *pstPortCfg)
{
    ...
    pstPortCfg->stBufListCfg.u32BufNumber = HI_VPSS_MAX_BUFFER_NUMB * 2;
#else
                                  ...
                                  HI_VOID *pArgs)
{
    ...
    switch ( eCommand )
    {
        case HI_DRV_VPSS_USER_COMMAND_RESET:
            VPSS_OSAL_IntfTime_Start(VPSS_INTF_RESET);
            VPSS_INST_Reset(pstInstance);
            VPSS_OSAL_IntfTime_Stop(VPSS_INTF_RESET);
            break;
        ...
        case HI_DRV_VPSS_USER_COMMAND_START:
            (HI_VOID)VPSS_INST_SetState(pstInstance, INSTANCE_STATE_IDLE);
            break;
...
HI_S32 VPSS_INST_RelPortFrame(VPSS_INSTANCE_S *pstInstance, VPSS_HANDLE hPort, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    VPSS_PORT_S *pstPort;
    VPSS_FB_INFO_S *pstFrameList;
#if (defined(CHIP_TYPE_hi3798cv200) || defined(CHIP_TYPE_hi3798mv200) || defined(CHIP_TYPE_hi3798mv300) || defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3716mv450) || defined(CHIP_TYPE_hi3798mv310))
...
HI_S32 VPSS_INST_CheckByPass(VPSS_INSTANCE_S *pstInstance)
{
    ...
    for (u32Count = 0; u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32Count++)
    {
            ...
            && pstPort->bEnble == HI_TRUE)
        {
            ...
            else if (pstFrameList->stBufListCfg.eBufType == HI_DRV_VPSS_BUF_VPSS_ALLOC_MANAGE)
            {
                (HI_VOID)VPSS_FB_CheckFulListAvailable(pstFrameList,
                                                       &bFrmListAvailable);
                if (bFrmListAvailable == HI_TRUE && pstInstance->pfUserCallBack != HI_NULL)
                {
                    pstInstance->pfUserCallBack(pstInstance->hDst,
                                                VPSS_EVENT_NEW_FRAME, HI_NULL);
                }
            }
            else
            {
...
HI_S32 VPSS_INST_CheckSplitState(VPSS_INSTANCE_S *pstInstance)
{
    HI_U32 u32Count;
    VPSS_PORT_S *pstPort;
    ...
    pstInstance->bSplitEnable = HI_FALSE;

    s32Ret = DRV_PQ_GetMcdiSplitMode(pstInstance->ID, &(pstInstance->enSplitMode)); //同样0/1表示不分块, 5表示当前支持的5分块模式; 当前仅支持handleNo传入为0; 枚举由PQ定义;
    if (HI_SUCCESS != s32Ret)
    {
        VPSS_ERROR("call DRV_PQ_GetMcdiSplitMode fail:%d\n", s32Ret);
        return s32Ret;
    }

    if (HI_DRV_PQ_MCDI_SPLIT_MODE_5 == pstInstance->enSplitMode)
    {
        for (u32Count = 0; u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32Count++)
        {
            pstPort = &((pstInstance)->stPort[u32Count]);
        ...
        if ((HI_TRUE == bRotation)  &&
            (HI_TRUE == VPSS_HAL_SupportMcSplit(pstInstance->enSplitMode,
                                                pstInstance->stInEntity.stSrcImage.u32Width,
                                                pstInstance->stInEntity.stSrcImage.bProgressive)
            )
           )
        {
            pstInstance->bSplitEnable = HI_FALSE;
            ...
            VPSS_WARN("Do not support this case: rota\n");
            return HI_FAILURE;
        }
        else
        {
            pstInstance->enSplitMode = (HI_DRV_PQ_MCDI_SPLIT_MODE_5 == pstInstance->enSplitMode)
    ...
    pstInstance->stInEntity.enSplitMode = (VPSS_MCDI_SPLIT_MODE_E)pstInstance->enSplitMode;
    //pstInstance->stInEntity.bSplitEnable = pstInstance->bSplitEnable;
...
HI_S32 VPSS_INST_CheckUndoImage(VPSS_INSTANCE_S *pstInstance)
{
    ...
    s32Ret = VPSS_INST_CheckSplitState(pstInstance);
    if (HI_SUCCESS != s32Ret)
    {
        return s32Ret;
    }
    ...
    */
    ...
    if (pstEntity->stStreamInfo.u32IsNewImage == HI_TRUE)
    {
        ...
        (HI_VOID)DRV_PQ_UpdateVpssPQ(pstInstance->ID, &stVpssPqInfo, &pstInstance->stPqRegData, &pstInstance->stPqWbcReg[0], &pstInstance->stPQModule);
    }

    pstInstance->bSplitEnable = pstInstance->stInEntity.bSplitEnable;
#ifdef VPSS_SUPPORT_SPLIT_MCDI
    pstInstance->stVpssHalInfo.bSplitEnable = pstInstance->bSplitEnable;
    pstInstance->stVpssHalInfo.enSplitMode = (VPSS_MCDI_SPLIT_MODE_E)pstInstance->enSplitMode;
#endif
    return s32Ret;
}
...
HI_S32 VPSS_INST_CheckInstAvailable(VPSS_INSTANCE_S *pstInstance)
{
    ...
    if (s32BufIsEnough != 0)
    {
        ...
        if (eBufStratagy == HI_DRV_VPSS_BUFFUL_PAUSE
            || eBufStratagy == HI_DRV_VPSS_BUFFUL_BUTT)
        {
            VPSS_WARN("Inst %d OUT Buf Is FULL.\n", pstInstance->ID);
            return HI_FAILURE;
        }
    }
    else
                                  ...
                                  HI_DRV_VIDEO_FRAME_S *pstFrm, HI_DRV_BUF_ADDR_E enBufLR)
{
...
#endif
    /* keep pts equal to src frame, this function will copy all info from CUR
    to pstFrm, any info need modifid should be put after this function ! */
    ...
    if ((0 == pstInst->stInEntity.stWbcInfo[0].u32CompleteCount)
        && (HI_TRUE == pstInst->bSplitEnable))
    {
        pstFrm->u32Height = pstFrm->u32Height / 2;
    }
#endif
...
HI_VOID VPSS_INST_GetInCrop(VPSS_INSTANCE_S *pstInst, HI_U32 PortId, HI_RECT_S *pstInCropRect)
{
    ...
    if (pstInst->stProcCtrl.bUseCropRect)
    {
        ...
        {
            VPSS_WARN("u32LeftOffset(%d) add u32RightOffset(%d) is too large(%d)\n"
    ...
    else
    {
        ...
        {
            VPSS_WARN("s32X(%d)  s32Y(%d) s32Width(%d) s32Height(%d) is invaild\n",
        ...
        {
            VPSS_WARN("s32X(%d) add s32Width(%d) is too large  u32SrcW(%d)n"
    ...
    else
    {
        ...
        if ((HI_DRV_FT_NOT_STEREO != pstInst->stSrcImage.eFrmType)
            && ((pstInCropRect->s32X != pInRct->s32X)
                ...
                || (pstInCropRect->s32Height != pInRct->s32Height)))
        {
            VPSS_WARN("3d not support crop,pInRct s32X(%d), s32Y(%d)\n"
...
HI_BOOL VPSS_INST_CheckPassThroughForOMX(VPSS_INSTANCE_S *pstInstance, HI_DRV_VIDEO_FRAME_S *pstInImage)
{
    ...
    {
        pstPort = &(pstInstance->stPort[u32Count]);
        if (pstPort->s32PortId != HI_INVALID_HANDLE) //ignore port enable state
        {
