...
HI_U32 VPSS_INST_McDeiGetMedsWidth(HI_DRV_VIDEO_FRAME_S *pstSrcImage, VPSS_MCDI_SPLIT_INFO_S *pstSplitInfo)
{
    HI_U32 u32StreamW;

    u32StreamW = pstSrcImage->u32Width;

    if (VPSS_HAL_SupportMcSplit(pstSplitInfo->enSplitMode, u32StreamW, pstSplitInfo->bProgressive))
    {
        return u32StreamW;
    }
    else
...
HI_BOOL VPSS_INST_McDeiGetMedsEn(HI_DRV_VIDEO_FRAME_S *pstSrcImage, VPSS_MCDI_SPLIT_INFO_S *pstSplitInfo)
{
    HI_U32 u32StreamW;
...
HI_S32 VPSS_INST_McDeiInit(VPSS_MCDEI_INFO_S *pstMcdeiInfo, HI_DRV_VIDEO_FRAME_S
                           *pstSrcImage, VPSS_MCDI_SPLIT_INFO_S *pstSplitInfo)
{
    HI_U32 u32StreamW;
    HI_U32 u32StreamH;
    ...
    stSttAttr.u32Width = VPSS_INST_McDeiGetMedsWidth(pstSrcImage, pstSplitInfo);
    stSttAttr.u32Height = u32StreamH;
    stSttAttr.bProgressive = pstSrcImage->bProgressive;
    stSttAttr.enSplitMode = pstSplitInfo->enSplitMode;
    stSttAttr.bSplitEnable = pstSplitInfo->bSplitEnable;
    ...
    {
        stSttAttr.u32Height = u32StreamH * 2; //下面是按照场模式计算, 高度固定/2, 所以逐行时上层*2
    }
    ...
    pstMcdeiInfo->bMcNrEnable = VPSS_INST_McNrSupport(pstSrcImage)
                                || VPSS_HAL_SupportMcSplit(pstSplitInfo->enSplitMode, pstSrcImage->u32Width, pstSplitInfo->bProgressive);
    ...
    stWbcBlendAttr.bProgressive = pstSrcImage->bProgressive;
    stWbcBlendAttr.bSplitEnable = pstSplitInfo->bSplitEnable;
    stWbcBlendAttr.enSplitMode = pstSplitInfo->enSplitMode;

    stWbcRgmeAttr.u32Width = VPSS_INST_McDeiGetMedsWidth(pstSrcImage, pstSplitInfo);
    ...
    stWbcRgmeAttr.bProgressive = pstSrcImage->bProgressive;
#if defined(CHIP_TYPE_hi3798mv200) || defined(CHIP_TYPE_hi3798mv300) || defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3716mv450) || defined(CHIP_TYPE_hi3798mv310)
    ...
    stWbcRgmeAttr.bSplitEnable = pstSplitInfo->bSplitEnable;
    stWbcRgmeAttr.enSplitMode = pstSplitInfo->enSplitMode;
...
HI_S32 VPSS_INST_SetHalMcdeiInfo(VPSS_MCDEI_INFO_S *pstMcdeiInfo, VPSS_HAL_INFO_S *pstHalInfo,
                                 HI_DRV_BUF_ADDR_E enLR, HI_DRV_VIDEO_FRAME_S
                                 *pstSrcImage, VPSS_MCDI_SPLIT_INFO_S *pstSplitInfo)
{
    HI_S32 i;
    ...
    pstHalInfo->stMcdeiRfrInfo.bMedsEn = VPSS_INST_McDeiGetMedsEn(pstSrcImage, pstSplitInfo);
