...
VPSS_Intf_TimeStamp_S g_astVPSS_Intf_Time[VPSS_INTF_BUTT] =
{
    {"reset", 0},
    {"pause", 0},
    {"start", 0},
...
HI_S32 VPSS_OSAL_WRITEYUV_10BIT_NV21(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *str)
{
...
HI_S32 VPSS_OSAL_WRITEYUV_10BIT_NV61(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *str)
{
    unsigned char *ptr = HI_NULL;
    unsigned char *ptr_bk = HI_NULL;
    FILE *fp = HI_NULL;
...
HI_S32 VPSS_OSAL_WRITEYUV_10BIT_NV21_CMP(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *str)
{
...
{
    char cYUVFileName[DEF_FILE_NAMELENGTH] = {0};
    ...
    HI_OSAL_Snprintf(cYUVFileName, DEF_FILE_NAMELENGTH, "%s/%s", s_VpssSavePath, pchFile);
    cYUVFileName[DEF_FILE_NAMELENGTH - 1] = '\0';

    if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21
        || pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV12)
    {
        VPSS_OSAL_WRITEYUV_10BIT_NV21(pstFrame, cYUVFileName);
    }
    else if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV61_2X1
             || pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV16_2X1)
    {
        VPSS_OSAL_WRITEYUV_10BIT_NV61(pstFrame, cYUVFileName);
    }
    else if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21_CMP
             || pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV12_CMP)
    {
        VPSS_OSAL_WRITEYUV_10BIT_NV21_CMP(pstFrame, cYUVFileName);
    }
    else
    {
...
HI_S32 VPSS_OSAL_WRITEYUV_8BIT_NV21(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *str)
{
    unsigned char *ptr = HI_NULL;
    unsigned char *ptr_bk = HI_NULL;
    FILE *fp = HI_NULL;
    ...
    if (pstFrame->u32Width > pstFrame->stBufAddr[0].u32Stride_Y)
    {
        VPSS_FATAL("Invalid stride:%d width:%d\n", pstFrame->stBufAddr[0].u32Stride_Y, pstFrame->u32Width);
        return HI_FAILURE;
    }
...
HI_S32 VPSS_OSAL_WRITEYUV_8BIT_NV21_TILE(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *str)
{
    FILE *fp = HI_NULL;
...
HI_S32 VPSS_OSAL_WRITEYUV_8BIT_NV61_2X1(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *str)
{
    unsigned char *ptr = HI_NULL;
    unsigned char *ptr_bk = HI_NULL;
    FILE *fp = HI_NULL;
    ...
    else
    {
        ...
        for (i = 0; i < pstFrame->u32Height; i++)
        {
            VPSS_SAFE_MEMCPY(pu8Ydata, ptr_bk, sizeof(HI_U8)*pstFrame->stBufAddr[0].u32Stride_Y);

            if (pstFrame->u32Width != VPSS_OSAL_fwrite(pu8Ydata, pstFrame->u32Width, fp))
            {
                VPSS_FATAL("line %d: fwrite fail!\n", __LINE__);
            }
            ptr_bk += pstFrame->stBufAddr[0].u32Stride_Y;
        }
        ...
        for (i = 0; i < pstFrame->u32Height; i++)
        {
            for (j = 0; j < pstFrame->u32Width / 2; j++)
            {
                ...
                {
                    pu8Vdata[i * pstFrame->u32Width / 2 + j] = ptr_bk[2 * j];
                    pu8Udata[i * pstFrame->u32Width / 2 + j] = ptr_bk[2 * j + 1];
                }
                else
                {
                    pu8Udata[i * pstFrame->u32Width / 2 + j] = ptr_bk[2 * j];
                    pu8Vdata[i * pstFrame->u32Width / 2 + j] = ptr_bk[2 * j + 1];
                }
            }
            ptr_bk += pstFrame->stBufAddr[0].u32Stride_C;
        }
        VPSS_OSAL_fwrite(pu8Udata, pstFrame->u32Width * pstFrame->u32Height  / 2, fp);
...
HI_S32 VPSS_OSAL_WRITEYUV_8BIT_NV21_CMP(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *str)
{
    HI_U32 i, j;
    HI_U8 u8YhFileName[] = "/mnt/cmp_y.head";
    HI_U8 u8YdFileName[] = "/mnt/cmp_y.dat";
...
RELEASE_RESOURCE:
    ...
    if (HI_NULL != pu8Y_VAddr_bk)
    {
        VPSS_OSAL_MEMUnmap(pstFrame->bSecure, pstFrame->stBufAddr[0].u32PhyAddr_Y, pu8Y_VAddr);
    }

    if (HI_NULL != pu8C_VAddr_bk)
    {
        VPSS_OSAL_MEMUnmap(pstFrame->bSecure, pstFrame->stBufAddr[0].u32PhyAddr_C, pu8C_VAddr);
    }
...
HI_S32 VPSS_OSAL_WRITEYUV_8BIT(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_CHAR *pchFile)
{
    ...
    if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21
        || pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV12)
    {
        VPSS_OSAL_WRITEYUV_8BIT_NV21(pstFrame, cYUVFileName);
    }
    else if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21_TILE
             || pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV12_TILE)
    {
        VPSS_OSAL_WRITEYUV_8BIT_NV21_TILE(pstFrame, cYUVFileName);
    }
    else if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV61_2X1
             || pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV16_2X1)
    {
        VPSS_OSAL_WRITEYUV_8BIT_NV61_2X1(pstFrame, cYUVFileName);
    }
    else if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21_CMP
             || pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV12_CMP)
    {
        VPSS_OSAL_WRITEYUV_8BIT_NV21_CMP(pstFrame, cYUVFileName);
    }
    else
    {
        VPSS_FATAL("PixFormat %d can't saveyuv\n", pstFrame->ePixFormat);
        ...
        || pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV12)
    {
        ...
        for (i = 0; i < pstFrame->u32Height / 2; i++)
        {
            ...
            for (j = 0; j < pstFrame->u32Width * 2; j += 4)
            {
                if (pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21)
                {
                    pu8Udata_bk[(j / 2)] = pu8UVdata_2Byte[j + 2];
                    pu8Udata_bk[(j / 2) + 1] = pu8UVdata_2Byte[j + 3];

                    pu8Vdata_bk[(j / 2)] = pu8UVdata_2Byte[j];
                    pu8Vdata_bk[(j / 2) + 1] = pu8UVdata_2Byte[j + 1];
                }
                else
                {
                    pu8Udata_bk[(j / 2)] = pu8UVdata_2Byte[j];
                    pu8Udata_bk[(j / 2) + 1] = pu8UVdata_2Byte[j + 1];

                    pu8Vdata_bk[(j / 2)] = pu8UVdata_2Byte[j + 2];
                    pu8Vdata_bk[(j / 2) + 1] = pu8UVdata_2Byte[j + 3];
                }
            }

            pu8Udata_bk += pstFrame->u32Width;
            pu8Vdata_bk += pstFrame->u32Width;
        }


        VPSS_OSAL_fwrite(pu8Udata, pstFrame->u32Width * pstFrame->u32Height / 2, fpOutFile);
        VPSS_OSAL_fwrite(pu8Vdata, pstFrame->u32Width * pstFrame->u32Height / 2, fpOutFile);
...
SAVE_EL_FAIL:
    ...
    VPSS_VFREE_SAFE(pu8Udata);
    VPSS_VFREE_SAFE(pu8Vdata);
...
HI_S32 VPSS_OSAL_CalBufSize(HI_U32 *pSize, HI_U32 *pStride, HI_U32 u32HeightOri, HI_U32 u32Width, HI_DRV_PIX_FORMAT_E ePixFormat, HI_DRV_PIXEL_BITWIDTH_E  enOutBitWidth)
{
    HI_U32 u32RetSize = 0;
    HI_U32 u32RetStride = 0;
    HI_U32 u32Height = 0;

    //when output height is align up to 4 in HAL, outbuffer should be align up to 4 too.
    u32Height = (u32HeightOri + 3) & 0xFFFFFFFC;
...
HI_S32 VPSS_OSAL_GetVpssVersion(VPSS_VERSION_E *penVersion, VPSS_CAPABILITY_U *penCapability)
{
    ...
    /*
        V4 includes 98cv200; (98mv200, mv300); (96mv200 3716mv450);(98mv310,98mv300H);
        V3 includes others chip type: such as (98mv100 3796mv100 hi3716dv100);(3716mv410,420);etc.
    */
    if ( (enChipType == HI_CHIP_TYPE_HI3798C && enChipVersion == HI_CHIP_VERSION_V200 )
...
{
    HI_U32 u32PageIndex = 0;
    HI_U32 u32PageOffset = 0;
    HI_U32 u32PhyAddr = 0;
    HI_U32 *pu32Page;
    HI_U32 u32High20Bit, u32Low12Bit;
    HI_U32 u32PageTabBaseAddr;
    HI_U32 u32err_rdaddr;
    HI_U32 u32err_wraddr;

    HI_DRV_SMMU_GetPageTableAddr(&u32PageTabBaseAddr, &u32err_rdaddr, &u32err_wraddr);

    if (u32PageTabBaseAddr == 0)
    {
        VPSS_FATAL("u32PageTabBaseAddr = 0x%08x Error!\n", u32PageTabBaseAddr);
    }

    u32PageOffset = u32SmmuAddr / 4096 * 4;
    u32PageIndex = u32PageTabBaseAddr + u32PageOffset;
    pu32Page = (HI_U32 *)phys_to_virt(u32PageIndex);

    if (HI_NULL != pu32Page)
    {
        u32High20Bit = ((*pu32Page) & 0xfffff000);
        u32Low12Bit = u32SmmuAddr & 0xfff;
        u32PhyAddr = u32High20Bit + u32Low12Bit;
    }
    else
    {
        VPSS_FATAL("pu32Page = 0x%08x Error!\n", (HI_U32)(long)pu32Page);
    }
...
{
    VPSS_Intf_TimeStamp_S *pstIntf;
    HI_U32 u32Idx;

    for (u32Idx = 0; u32Idx < VPSS_INTF_BUTT; u32Idx++)
    {
        pstIntf = &g_astVPSS_Intf_Time[u32Idx];
        pstIntf->u64Max = 0;
        pstIntf->u64Min = 0xffffffff;
        pstIntf->u32Total = 0;
        pstIntf->u64Cur = 0;
        pstIntf->u64Start = 0;
        pstIntf->u32Count = 0;
        pstIntf->u32MaxAtCount = 0;
        pstIntf->u32MinAtCount = 0;
        pstIntf->enState = VPSS_TIMESTAMP_STATE_INIT;
    }
    return;
}
...
{
    VPSS_Intf_TimeStamp_S *pstIntf;
    if (eIntf < VPSS_INTF_BUTT)
    {
        pstIntf = &g_astVPSS_Intf_Time[eIntf];
        if ((pstIntf->enState != VPSS_TIMESTAMP_STATE_INIT) && (pstIntf->enState !=
                VPSS_TIMESTAMP_STATE_STOP))
        {
            VPSS_ERROR("timestamp state error intf %d state %d\n", eIntf, pstIntf->enState);
            return;
        }
        pstIntf->enState = VPSS_TIMESTAMP_STATE_START;
        pstIntf->u64Start = VPSS_OSAL_GetTimeUS();
    }
    return;
}
HI_VOID VPSS_OSAL_IntfTime_Stop(VPSS_Intf_Name_E eIntf)
{
    ...
    if (eIntf < VPSS_INTF_BUTT)
    {
        ...
        if (pstIntf->enState != VPSS_TIMESTAMP_STATE_START)
        {
            VPSS_ERROR("timestamp state error intf %d state %d\n", eIntf, pstIntf->enState);
            return;
        }
        pstIntf->enState = VPSS_TIMESTAMP_STATE_STOP;

        u64CurTime = VPSS_OSAL_GetTimeUS();
        u64DiffTime = (pstIntf->u64Start < u64CurTime) ? (u64CurTime - pstIntf->u64Start) : 1000;
        pstIntf->u64Cur = u64DiffTime;
        pstIntf->u32Count++;
        pstIntf->u32Total += u64DiffTime;
        if (pstIntf->u64Max < u64DiffTime)
        {
            pstIntf->u64Max = u64DiffTime;
            pstIntf->u32MaxAtCount = pstIntf->u32Count;
        }
        if (pstIntf->u64Min > u64DiffTime)
        {
            pstIntf->u64Min = u64DiffTime;
            pstIntf->u32MinAtCount = pstIntf->u32Count;
        }
    }
    return;
}
HI_VOID VPSS_OSAL_IntfTime_Print(struct seq_file *p)
{
    ...
    for (u32Idx = 0; u32Idx < VPSS_INTF_BUTT; u32Idx++)
    {
        ...
        VPSS_PROC_INFO_PRINT(p,
                             VPSS_PROC_LEVEL4"-------- %s --------"
                             VPSS_PROC_LEVEL4"Max     : %llu"
                             VPSS_PROC_LEVEL4"Min     : %llu"
                             VPSS_PROC_LEVEL4"Avg     : %u"
                             VPSS_PROC_LEVEL4"Cur     : %llu"
                             VPSS_PROC_LEVEL4"Count   : %u"
                             VPSS_PROC_LEVEL4"Max at  : %u"
                             VPSS_PROC_LEVEL4"Min at  : %u",
                             pstIntf->ucFuncName,
                             pstIntf->u64Max,
                             pstIntf->u64Min,
                             (pstIntf->u32Count > 0) ?
                             (pstIntf->u32Total / pstIntf->u32Count) : 0,
                             pstIntf->u64Cur,
                             pstIntf->u32Count,
                             pstIntf->u32MaxAtCount,
                             pstIntf->u32MinAtCount
                            );
    }
    return;
}
