...
HI_VOID VPSS_ST_RGME_CalBufSize(HI_U32 *pSize, HI_U32 *pStride, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    {
        u32Stride = HI_ALIGN_8BIT_YSTRIDE(((u32FirstWth + 33) / 64 * 16 + 15) & 0xfffffff0);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE(((u32MidWth + 33) / 64 * 16 + 15) & 0xfffffff0) * ((u32SplitCnt > 2) ? (u32SplitCnt - 2) : 0);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE(((u32LastWth + 33) / 64 * 16 + 15) & 0xfffffff0);
    }
    else
    {
    ...
    *pSize = u32Stride * (u32Height / 2 + 5) / 8;
    ...
    if (VPSS_HAL_SupportMcSplit(pstAttr->enSplitMode, u32Width, pstAttr->bProgressive))
    {
        *pSize = u32Stride * ((u32Height + VPSS_OVERLAP_HEIGHT) / 2 + 5) / 8;
    }
#endif
...
HI_S32 VPSS_ST_RGME_Init(VPSS_ST_RGME_S *pstStRgme, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    u32TmpInstanceId = pstStRgme->u32InstanceId;
    memset(pstStRgme, 0, sizeof(VPSS_ST_RGME_S));
    pstStRgme->u32InstanceId = u32TmpInstanceId;
    pstStRgme->u32Width = pstAttr->u32Width;
    pstStRgme->u32Height = pstAttr->u32Height;
    pstStRgme->bProgressive = pstAttr->bProgressive;
    ...
    else
    {
        ...
        s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTRGME, "VPSS_SttRgmeBuf", u32TotalBuffSize, 0, &(pstStRgme->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStRgme->u32InstanceId, VPSS_ALG_STTRGME);
        s32Ret = VPSS_AlgBuf_Get(&(g_VpssAlgBuf[pstStRgme->u32InstanceId]), VPSS_ALG_STTRGME, "VPSS_SttRgmeBuf", u32TotalBuffSize, 0, &(pstStRgme->stMMUBuf));
#else
        s32Ret = HI_DRV_SMMU_AllocAndMap("VPSS_SttRgmeBuf", u32TotalBuffSize, 0, &(pstStRgme->stMMUBuf));
#endif

        if ((HI_FAILURE == s32Ret) || (HI_NULL == pstStRgme->stMMUBuf.pu8StartVirAddr))
        {
            VPSS_FATAL("VPSS Strgme Alloc memory failed.\n");
            return HI_FAILURE;
        }
    ...
    if ((HI_FAILURE == s32Ret) || (HI_NULL == pstStRgme->stMMZBuf.pu8StartVirAddr))
    {
        VPSS_FATAL("VPSS Strgme Alloc memory failed.\n");
        return HI_FAILURE;
    }
...
HI_S32 VPSS_ST_RGME_DeInit(VPSS_ST_RGME_S *pstStRgme)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &pstStRgme->stMMUBuf, VPSS_ALG_STTRGME);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStRgme->u32InstanceId, VPSS_ALG_STTRGME);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[pstStRgme->u32InstanceId], &pstStRgme->stMMUBuf, VPSS_ALG_STTRGME);
#else
...
HI_VOID VPSS_ST_BLKMV_CalBufSize(HI_U32 *pSize, HI_U32 *pStride, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    if (VPSS_HAL_SupportMcSplit(pstAttr->enSplitMode, u32Width, pstAttr->bProgressive))
    {
        u32Stride = HI_ALIGN_8BIT_YSTRIDE(((u32FirstWth + 7) / 8 * 4 + 15) / 16 * 16);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE(((u32MidWth + 7) / 8 * 4 + 15) / 16 * 16) * ((u32SplitCnt > 2) ? (u32SplitCnt - 2) : 0);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE(((u32LastWth + 7) / 8 * 4 + 15) / 16 * 16);
    }
    else
    {
    ...
    *pSize = u32Stride * (u32Height / 2 + 3) / 4;
    ...
    if (VPSS_HAL_SupportMcSplit(pstAttr->enSplitMode, u32Width, pstAttr->bProgressive))
    {
        *pSize = u32Stride * ((u32Height + VPSS_OVERLAP_HEIGHT) / 2 + 3) / 4;
    }
#endif
    *pSize = HI_ALIGN_8BIT_YSTRIDE(*pSize);
    *pStride = u32Stride;
...
HI_S32 VPSS_ST_BLKMV_Init(VPSS_ST_BLKMV_S *pstStBlkmv, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    u32TmpInstanceId = pstStBlkmv->u32InstanceId;
    memset(pstStBlkmv, 0, sizeof(VPSS_ST_BLKMV_S));
    pstStBlkmv->u32InstanceId = u32TmpInstanceId;
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTBLKMV, "VPSS_SttBlkmvBuf", u32TotalBuffSize, 0, &(pstStBlkmv->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStBlkmv->u32InstanceId, VPSS_ALG_STTBLKMV);
        s32Ret = VPSS_AlgBuf_Get(&g_VpssAlgBuf[pstStBlkmv->u32InstanceId], VPSS_ALG_STTBLKMV, "VPSS_SttBlkmvBuf", u32TotalBuffSize, 0, &(pstStBlkmv->stMMUBuf));
#else
        s32Ret = HI_DRV_SMMU_AllocAndMap( "VPSS_SttBlkmvBuf", u32TotalBuffSize, 0, &(pstStBlkmv->stMMUBuf));
#endif
        if ((HI_FAILURE == s32Ret) || (HI_NULL ==
                                       pstStBlkmv->stMMUBuf.pu8StartVirAddr))
        {
            VPSS_FATAL("VPSS Stblkmv Alloc memory failed.\n");
            return HI_FAILURE;
        }
    ...
    if ((HI_FAILURE == s32Ret) || (HI_NULL == pstStBlkmv->stMMZBuf.pu8StartVirAddr))
    {
        VPSS_FATAL("VPSS Stblkmv Alloc memory failed.\n");
        return HI_FAILURE;
    }
...
HI_S32 VPSS_ST_BLKMV_DeInit(VPSS_ST_BLKMV_S *pstStBlkmv)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &pstStBlkmv->stMMUBuf, VPSS_ALG_STTBLKMV);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStBlkmv->u32InstanceId, VPSS_ALG_STTBLKMV);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[pstStBlkmv->u32InstanceId], &pstStBlkmv->stMMUBuf, VPSS_ALG_STTBLKMV);
#else
...
HI_S32 VPSS_ST_BLKMT_Init(VPSS_ST_BLKMT_S *pstStBLKMT, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    u32TmpInstanceId = pstStBLKMT->u32InstanceId;
    memset(pstStBLKMT, 0, sizeof(VPSS_ST_BLKMT_S));
    pstStBLKMT->u32InstanceId = u32TmpInstanceId;
...
#if defined(ALGBUF_MANAGE_MODE1)
        s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTBLKMT, "VPSS_SttBLKMTBuf", u32TotalBuffSize, 0, &(pstStBLKMT->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStBLKMT->u32InstanceId, VPSS_ALG_STTBLKMT);
        s32Ret = VPSS_AlgBuf_Get(&g_VpssAlgBuf[pstStBLKMT->u32InstanceId], VPSS_ALG_STTBLKMT, "VPSS_SttBLKMTBuf", u32TotalBuffSize, 0, &(pstStBLKMT->stMMUBuf));
#else
...
HI_S32 VPSS_ST_BLKMT_DeInit(VPSS_ST_BLKMT_S *pstStBLKMT)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &pstStBLKMT->stMMUBuf, VPSS_ALG_STTBLKMT);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStBLKMT->u32InstanceId, VPSS_ALG_STTBLKMT);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[pstStBLKMT->u32InstanceId], &pstStBLKMT->stMMUBuf, VPSS_ALG_STTBLKMT);
#else
...
HI_VOID VPSS_ST_DICNT_CalBufSize(HI_U32 *pSize, HI_U32 *pStride, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    *pSize = u32Stride * u32Height / 2;
    ...
    if (VPSS_HAL_SupportMcSplit(pstAttr->enSplitMode, u32Width, pstAttr->bProgressive))
    {
        u32Stride = HI_ALIGN_8BIT_YSTRIDE((((u32FirstWth + 3) / 4 * 4 + 7) / 8 + 15) / 16 * 16);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE((((u32MidWth + 3) / 4 * 4 + 7) / 8 + 15) / 16 * 16) * ((u32SplitCnt > 2) ? (u32SplitCnt - 2) : 0);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE((((u32LastWth + 3) / 4 * 4 + 7) / 8 + 15) / 16 * 16);
    }
    *pSize = u32Stride * (u32Height + VPSS_OVERLAP_HEIGHT) ;
...
HI_S32 VPSS_ST_DICNT_Init(VPSS_ST_DICNT_S *pstStDICNT, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    u32TmpInstanceId = pstStDICNT->u32InstanceId;
    memset(pstStDICNT, 0, sizeof(VPSS_ST_DICNT_S));
    pstStDICNT->u32InstanceId = u32TmpInstanceId;
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTDICNT, "VPSS_SttDICNTBuf", u32TotalBuffSize, 0, &(pstStDICNT->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStDICNT->u32InstanceId, VPSS_ALG_STTDICNT);
        s32Ret = VPSS_AlgBuf_Get(&g_VpssAlgBuf[pstStDICNT->u32InstanceId], VPSS_ALG_STTDICNT, "VPSS_SttDICNTBuf", u32TotalBuffSize, 0, &(pstStDICNT->stMMUBuf));
#else
...
HI_S32 VPSS_ST_DICNT_DeInit(VPSS_ST_DICNT_S *pstStDICNT)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &pstStDICNT->stMMUBuf, VPSS_ALG_STTDICNT);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStDICNT->u32InstanceId, VPSS_ALG_STTDICNT);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[pstStDICNT->u32InstanceId], &pstStDICNT->stMMUBuf, VPSS_ALG_STTDICNT);
#else
...
HI_VOID VPSS_ST_PRJH_CalBufSize(HI_U32 *pSize, HI_U32 *pStride, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    if (VPSS_HAL_SupportMcSplit(pstAttr->enSplitMode, u32Width, pstAttr->bProgressive))
    {
        u32Stride = HI_ALIGN_8BIT_YSTRIDE((((u32FirstWth + 33) / 64 * 256) + 15) & 0xFFFFFFF0);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE((((u32MidWth + 33) / 64 * 256) + 15) & 0xFFFFFFF0) * ((u32SplitCnt > 2) ? (u32SplitCnt - 2) : 0);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE((((u32LastWth + 33) / 64 * 256) + 15) & 0xFFFFFFF0);
    }
    else
    {
...
HI_S32 VPSS_ST_PRJH_Init(VPSS_ST_PRJH_S *pstStPrjh, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    u32TmpInstanceId = pstStPrjh->u32InstanceId;
    memset(pstStPrjh, 0, sizeof(VPSS_ST_PRJH_S));
    pstStPrjh->u32InstanceId = u32TmpInstanceId;
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTPRJH, "VPSS_SttRgmeBuf", u32TotalBuffSize, 0, &(pstStPrjh->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStPrjh->u32InstanceId, VPSS_ALG_STTPRJH);
        s32Ret = VPSS_AlgBuf_Get(&g_VpssAlgBuf[pstStPrjh->u32InstanceId], VPSS_ALG_STTPRJH, "VPSS_SttRgmeBuf", u32TotalBuffSize, 0, &(pstStPrjh->stMMUBuf));
#else
...
HI_S32 VPSS_ST_PRJH_DeInit(VPSS_ST_PRJH_S *pstStPrjh)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &pstStPrjh->stMMUBuf, VPSS_ALG_STTPRJH);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStPrjh->u32InstanceId, VPSS_ALG_STTPRJH);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[pstStPrjh->u32InstanceId], &pstStPrjh->stMMUBuf, VPSS_ALG_STTPRJH);
#else
...
HI_VOID VPSS_ST_PRJV_CalBufSize(HI_U32 *pSize, HI_U32 *pStride, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    if (VPSS_HAL_SupportMcSplit(pstAttr->enSplitMode, u32Width, pstAttr->bProgressive))
    {
        u32Stride = HI_ALIGN_8BIT_YSTRIDE((((u32FirstWth + 33) / 64 * 64) + 15) & 0xFFFFFFF0);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE((((u32MidWth + 33) / 64 * 64) + 15) & 0xFFFFFFF0) * ((u32SplitCnt > 2) ? (u32SplitCnt - 2) : 0);
        u32Stride += HI_ALIGN_8BIT_YSTRIDE((((u32LastWth + 33) / 64 * 64) + 15) & 0xFFFFFFF0);
    }
    else
    {
...
HI_S32 VPSS_ST_PRJV_Init(VPSS_ST_PRJV_S *pstStPrjv, VPSS_MC_ATTR_S *pstAttr)
{
    ...
    u32TmpInstanceId = pstStPrjv->u32InstanceId;
    memset(pstStPrjv, 0, sizeof(VPSS_ST_PRJV_S));
    pstStPrjv->u32InstanceId = u32TmpInstanceId;
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTPRJV, "VPSS_SttprjvBuf", u32TotalBuffSize, 0, &(pstStPrjv->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStPrjv->u32InstanceId, VPSS_ALG_STTPRJV);
        s32Ret = VPSS_AlgBuf_Get(&g_VpssAlgBuf[pstStPrjv->u32InstanceId], VPSS_ALG_STTPRJV, "VPSS_SttprjvBuf", u32TotalBuffSize, 0, &(pstStPrjv->stMMUBuf));
#else
...
HI_S32 VPSS_ST_PRJV_DeInit(VPSS_ST_PRJV_S *pstStPrjv)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &pstStPrjv->stMMUBuf, VPSS_ALG_STTPRJV);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstStPrjv->u32InstanceId, VPSS_ALG_STTPRJV);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[pstStPrjv->u32InstanceId], &pstStPrjv->stMMUBuf, VPSS_ALG_STTPRJV);
#else
