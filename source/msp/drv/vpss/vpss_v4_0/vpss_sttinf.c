...
#endif
/*Die Code*/
HI_VOID VPSS_STTINFO_CalDieBufSize(HI_U32 *pSize, HI_U32 *pStride, HI_U32 u32Width, HI_U32 u32Heigth, VPSS_MCDI_SPLIT_INFO_S *pstSplitInfo)
{
    ...
    if (VPSS_HAL_SupportMcSplit(pstSplitInfo->enSplitMode, u32Width, pstSplitInfo->bProgressive))
    {
        (*pStride) = HI_ALIGN_8BIT_YSTRIDE(((u32FirstWth + 3) / 4 * 2 + 15) / 16 * 16);
        (*pStride) += HI_ALIGN_8BIT_YSTRIDE(((u32MidWth + 3) / 4 * 2 + 15) / 16 * 16) * ((u32SplitCnt > 2) ? (u32SplitCnt - 2) : 0);
        (*pStride) += HI_ALIGN_8BIT_YSTRIDE(((u32LastWth + 3) / 4 * 2 + 15) / 16 * 16);
    }
    else
    {
    ...
    *pSize = (*pStride) * u32Heigth / 2;
    ...
    if (VPSS_HAL_SupportMcSplit(pstSplitInfo->enSplitMode, u32Width, pstSplitInfo->bProgressive))
    {
        *pSize = (*pStride) * (u32Heigth + VPSS_OVERLAP_HEIGHT) / 2 ;
    }
#endif
#else
                            ...
                            VPSS_MCDI_SPLIT_INFO_S *pstSplitInfo)
{
    ...
    u32TmpInstanceId = pstDieStInfo->u32InstanceId;
    memset(pstDieStInfo, 0, sizeof(VPSS_DIESTINFO_S));
    pstDieStInfo->u32InstanceId = u32TmpInstanceId;
    ...
    VPSS_STTINFO_CalDieBufSize(&(pstDieStInfo->u32DieInfoSize),
                               &(pstDieStInfo->u32Stride), u32Width, u32Heigth, pstSplitInfo);
    ...
    else
    {
        ...
        s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTDIE, "VPSS_SttDieBuf", u32TotalBuffSize, 0, &(pstDieStInfo->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstDieStInfo->u32InstanceId, VPSS_ALG_STTDIE);
        s32Ret = VPSS_AlgBuf_Get(&g_VpssAlgBuf[pstDieStInfo->u32InstanceId], VPSS_ALG_STTDIE, "VPSS_SttDieBuf", u32TotalBuffSize, 0, &(pstDieStInfo->stMMUBuf));
#else
        s32Ret = HI_DRV_SMMU_AllocAndMap( "VPSS_SttDieBuf", u32TotalBuffSize, 0, &(pstDieStInfo->stMMUBuf));
#endif
        if ((HI_FAILURE == s32Ret) || (HI_NULL ==
                                       pstDieStInfo->stMMUBuf.pu8StartVirAddr))
        {
            VPSS_FATAL("VPSS STTDie Alloc memory failed.\n");
            return HI_FAILURE;
        }
    ...
    if ((HI_FAILURE == s32Ret) || (HI_NULL == pstDieStInfo->stMMZBuf.pu8StartVirAddr))
    {
        VPSS_FATAL("VPSS STTDie Alloc memory failed.\n");
        return HI_FAILURE;
    }
...
HI_S32 VPSS_STTINFO_DieDeInit(VPSS_DIESTINFO_S *pstDieStInfo)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &pstDieStInfo->stMMUBuf, VPSS_ALG_STTDIE);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstDieStInfo->u32InstanceId, VPSS_ALG_STTDIE);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[pstDieStInfo->u32InstanceId], &pstDieStInfo->stMMUBuf, VPSS_ALG_STTDIE);
#else
...
HI_VOID VPSS_STTINFO_CalNrBufSize(HI_U32 *pSize, HI_U32 *pStride, VPSS_NR_ATTR_S *pstAttr)
{
    ...
    VPSS_HAL_CalcSplitWth(u32Width, &u32FirstWth, &u32MidWth, &u32LastWth, &u32SplitCnt);
    if (HI_TRUE == pstAttr->bSplitEnable)
    {
        (*pStride) = HI_ALIGN_8BIT_YSTRIDE(((u32FirstWth * 5 + 7) / 8 + 15) & 0xfffffff0);
        (*pStride) += HI_ALIGN_8BIT_YSTRIDE((((u32MidWth * 5 + 7) / 8 + 15) & 0xfffffff0)) * ((u32SplitCnt > 2) ? (u32SplitCnt - 2) : 0);
        (*pStride) += HI_ALIGN_8BIT_YSTRIDE(((u32LastWth * 5 + 7) / 8 + 15) & 0xfffffff0);
    }
    else
    {
    ...
    *pSize = (*pStride) * pstAttr->u32Height;
    ...
    if (HI_TRUE == pstAttr->bSplitEnable)
    {
        *pSize = (*pStride) * (pstAttr->u32Height + VPSS_OVERLAP_HEIGHT) ;
    }
#endif
...
HI_S32 VPSS_STTINFO_NrInit(VPSS_NRMADINFO_S *pstNrMadInfo, VPSS_NR_ATTR_S *pstAttr)
{
    ...
    u32TmpInstanceId = pstNrMadInfo->u32InstanceId;
    memset(pstNrMadInfo, 0, sizeof(VPSS_NRMADINFO_S));
    pstNrMadInfo->u32InstanceId = u32TmpInstanceId;
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTNR, "VPSS_SttNrBuf_SMMU", u32TotalBuffSize, 0, &(pstNrMadInfo->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstNrMadInfo->u32InstanceId, VPSS_ALG_STTNR);
        s32Ret = VPSS_AlgBuf_Get(&g_VpssAlgBuf[pstNrMadInfo->u32InstanceId], VPSS_ALG_STTNR, "VPSS_SttNrBuf_SMMU", u32TotalBuffSize, 0, &(pstNrMadInfo->stMMUBuf));
#else
        s32Ret = HI_DRV_SMMU_AllocAndMap( "VPSS_SttNrBuf_SMMU", u32TotalBuffSize, 0, &(pstNrMadInfo->stMMUBuf));
#endif
        if ((HI_FAILURE == s32Ret) || (HI_NULL ==
                                       pstNrMadInfo->stMMUBuf.pu8StartVirAddr))
        {
            VPSS_FATAL("VPSS Nr Alloc memory failed.\n");
            return HI_FAILURE;
        }
    ...
    if ((HI_FAILURE == s32Ret) || (HI_NULL ==
                                   pstNrMadInfo->stMMZBuf.pu8StartVirAddr))
    {
        VPSS_FATAL("VPSS Nr Alloc memory failed.\n");
        return HI_FAILURE;
    }
...
HI_S32 VPSS_STTINFO_NrDeInit(VPSS_NRMADINFO_S *pstNrMadInfo)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &pstNrMadInfo->stMMUBuf, VPSS_ALG_STTNR);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(pstNrMadInfo->u32InstanceId, VPSS_ALG_STTNR);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[pstNrMadInfo->u32InstanceId], &pstNrMadInfo->stMMUBuf, VPSS_ALG_STTNR);
#else
...
HI_S32 VPSS_STTINFO_SttWbcInit(VPSS_STTWBC_S *psttWbc)
{
...
#if defined(ALGBUF_MANAGE_MODE1)
    s32Ret = VPSS_AlgBuf_Malloc(&g_VpssAlgBuf, VPSS_ALG_STTWBC, "VPSS_SttWbcBuf", VPSS_STTWBC_SIZE, 0, &(psttWbc->stMMUBuf));
#elif defined(ALGBUF_MANAGE_MODE2)
    VPSS_ALG_INSTANCEID_CHECK(psttWbc->u32InstanceId, VPSS_ALG_STTWBC);
    s32Ret = VPSS_AlgBuf_Get(&g_VpssAlgBuf[psttWbc->u32InstanceId], VPSS_ALG_STTWBC, "VPSS_SttWbcBuf", VPSS_STTWBC_SIZE, 0, &(psttWbc->stMMUBuf));
#else
    s32Ret = HI_DRV_SMMU_AllocAndMap( "VPSS_SttWbcBuf", VPSS_STTWBC_SIZE, 0, &(psttWbc->stMMUBuf));
#endif

    if ((HI_FAILURE == s32Ret) || (HI_NULL ==
                                   psttWbc->stMMUBuf.pu8StartVirAddr))
    {
    ...
    if ((HI_FAILURE == s32Ret) || (HI_NULL ==
                                   psttWbc->stMMZBuf.pu8StartVirAddr))
    {
        VPSS_FATAL("VPSS WBC Alloc memory failed.\n");
        return HI_FAILURE;
    }
...
HI_S32 VPSS_STTINFO_SttWbcDeInit(VPSS_STTWBC_S *psttWbc)
{
    ...
    {
#if defined(ALGBUF_MANAGE_MODE1)
        VPSS_AlgBuf_Free(&g_VpssAlgBuf, &psttWbc->stMMUBuf, VPSS_ALG_STTWBC);
#elif defined(ALGBUF_MANAGE_MODE2)
        VPSS_ALG_INSTANCEID_CHECK(psttWbc->u32InstanceId, VPSS_ALG_STTWBC);
        VPSS_AlgBuf_Rel(&g_VpssAlgBuf[psttWbc->u32InstanceId], &psttWbc->stMMUBuf, VPSS_ALG_STTWBC);
#else
