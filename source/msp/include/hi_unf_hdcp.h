...
\param[in]: pu8ToolAesKey. The AES Key used by tool to encrypt the original hdcp key.  CNcomment: HDCP工具加密原始HDCP Key时使用的AES密钥，该接口将先使用这个密钥解密再使用硬件密钥加密. CNend
\param[in]: u32ToolAesKeyLen. The AES Key length used by tool to encrypt the original hdcp key.  CNcomment: HDCP工具加密原始HDCP Key时使用的AES密钥长度。 CNend
...
HI_S32  HI_UNF_HDCP_EncryptHDCPKey(HI_UNF_HDCP_HDCPKEY_S stHdcpKey,
                                   HI_BOOL bIsUseHdcpRootKey,
                                   HI_U8 u8OutEncryptKey[332],
                                   HI_U8 *pu8ToolAesKey,
