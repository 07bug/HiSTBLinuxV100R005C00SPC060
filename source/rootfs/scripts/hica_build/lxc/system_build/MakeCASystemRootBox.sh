...
if [ $# -lt 4 ]
then
    echo -e "usage: ./MakeCASystemRootBox.sh outdir mode toolchain strip_status"
    echo -e "        for example: ./MakeCASystemRootBox.sh /home/user/HiSTBLinuxV100R002/out/hi3716mv410/hi3716m41dma_ca_debug/rootbox DEBUG arm-histbv300-linux STRIP_OPEN"
    exit 1
fi
...
if [ $toolchain_name == "arm-histbv310-linux" ]; then
	...
	lib_to_cp+=" libdl-*.so "
	lib_to_cp+=" libdl.so.* "
	lib_to_cp+=" libstdc++.so.* "
	lib_to_cp+=" libpthread-*.so "
	lib_to_cp+=" libpthread.so.* "
	lib_to_cp+=" libgcc_s.so.* "
	lib_to_cp+=" libc-*.so "
	lib_to_cp+=" libc.so.* "
	lib_to_cp+=" libm-*.so "
	lib_to_cp+=" libm.so.* "
	lib_to_cp+=" ld-*.so "
	lib_to_cp+=" ld-linux.so.* "
	lib_to_cp+=" libutil-*.so "
   	lib_to_cp+=" libutil.so.* "
elif [ $toolchain_name == "arm-histbv320-linux" ]; then
   	echo "toolchain_name is arm-histbv320-linux"
	lib_to_cp=" librt-*.so "
	lib_to_cp+=" librt.so.* "
	lib_to_cp+=" libcrypt-*.so "
	lib_to_cp+=" libcrypt.so.* "
	...
	lib_to_cp+=" libm.so.* "
	lib_to_cp+=" ld-*.so "
	lib_to_cp+=" ld-linux-aarch64.so.* "
	lib_to_cp+=" ld-linux-armhf.so.* "
	lib_to_cp+=" libutil-*.so "
	lib_to_cp+=" libutil.so.* "
fi
...
for ff in $lib_to_cp ; do
    $SCP -r -d $rootbox_dir/lib/$ff 		$rootbox_dir/home/stb/lib
done
...
if [ $mode == "RELEASE" ]; then
    ...
    if [ "ss$lxc_enable_net" != "ssyes" ]; then
...
if [ -x $etc_dir/lxc_create_lxc2.sh ];then
   $etc_dir/lxc_create_lxc2.sh $mode $rootbox_dir lxc2 $toolchain_name ${lxc_enable_net}
fi
if [ -x $etc_dir/lxc_create_lxc1.sh ];then
   $etc_dir/lxc_create_lxc1.sh $mode $rootbox_dir lxc1 $toolchain_name ${lxc_enable_net}
fi
if [ -x $etc_dir/lxc_create_lxc0.sh -a "ss$lxc_enable_net" = "ssyes" ];then
   $etc_dir/lxc_create_lxc0.sh $mode $rootbox_dir lxc0 $toolchain_name ${lxc_enable_net}
fi
