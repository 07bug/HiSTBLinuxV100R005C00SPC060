...
static int is_not_valid_fname(const char *path)
{
	if (NULL == path) {
...
static int read_a_file(int buflen, uint8_t *buffer, const char *path)
{
	...
	if ((buflen < 0) || (buflen > HASH_FILE_MAX_SIZE) || (NULL == buffer) || (NULL == path)) {
		tloge("read a file, param is error\n");
		return -2;
	}
	if (is_not_valid_fname(path))
		return -5;
...
static int tee_set_native_ca_hash(const char *xml_flie)
{
	...
	if (NULL == xml_flie) {
		tloge("set native ca hash: error xml file inut\n");
		return -1;
	}
	buflen = get_file_size(xml_flie);
	if ((buflen <= 0) || (buflen > HASH_FILE_MAX_SIZE)) {
		/*tlogw("auth xml file(%s) not exist or length illegal: %d\n", xml_flie, buflen);*/
		return -1;
	}
