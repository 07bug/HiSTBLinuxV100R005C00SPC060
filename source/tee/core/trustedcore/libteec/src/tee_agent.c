...
/*DTS2014040405270 chown sec_stroage_data sdk */
...
#define TTF_FILE_PATH	"/vendor/etc/DroidSansFallback.ttf"
...
typedef enum {
	sec_nv_info,/* add by sdk for bootloaderlock status in nv partition */
	sec_get_time,
} misc_cmd_t;
...
struct sec_storage_t {
	...
	int32_t ret2;   /* just for fread, judge end-of-file or error
				fwrite: judge the sendor is SSA or SLOG */
	uint32_t userid;
	uint32_t magic;
	uint32_t error;
	union args1 {
...
struct misc_control_t {
	...
	int32_t magic;
	union args2 {
		/* add by sdk for bootloader lock status in nv partition */
		struct {
...
/*DTS2014040405270 chown sec_stroage_data sdk begin */
/*
...
/*DTS2014040405270 chown sec_stroage_data sdk end */
...
int mkdir_r(const char *path)
{
	...
	while (i < FILE_NAME_MAX_BUF && path[i] != '\0') {
		i++;
	}
	if (i >= FILE_NAME_MAX_BUF) {
...
static int join_file_name(const char *name, char *path)
{
	...
	if (name == strstr(name, SFS_PARTITION_TRANSIENT)) {
		...
		if (system_support_multi_users() && t_userid != 0) {
			...
			rc = strncat_s(path, FILE_NAME_MAX_BUF, SFS_PARTITION_USER_SYMLINK,
			...
			rc = strncat_s(path, FILE_NAME_MAX_BUF, user_path, strlen(user_path) + 1);
			if (EOK != rc) {
				tloge("strncat_s failed %d\n", rc);
				return -1;
			}

			rc = strncat_s(path, FILE_NAME_MAX_BUF,	name + strlen(SFS_PARTITION_TRANSIENT),
			...
			rc = strncat_s(path, FILE_NAME_MAX_BUF,	name, strlen(name) + 1);
			if (EOK != rc) {
				tloge("strncat_s failed %d\n", rc);
				return -1;
			}
...
static void open_work(struct sec_storage_t *trans_control)
{
	...
	FILE *pFile;
	char name_buff[FILE_NAME_MAX_BUF] = {0};
	errno_t rc = EOK;
	...
	if (!join_file_name((char *)(trans_control->args.open.name), name_buff)) {
		if (trans_control->cmd == sec_create) {
			...
			rc = strncpy_s(trans_control->args.open.mode,
				sizeof(trans_control->args.open.mode), "w+", sizeof("w+"));
			if (EOK != rc) {
				tloge("strncpy_s failed %d\n", rc);
				trans_control->ret = -1;
			...
			if (is_file_exist(name_buff) == 0) {
				...
				tlogd("file(%s) not exist, open failed\n", name_buff);
				trans_control->ret = -1;
				trans_control->error = ENOENT;
				return;
			}
...
static void close_work(struct sec_storage_t *trans_control)
{
	...
	if (find_open_file(trans_control->args.close.fd, &sel_file)) {
		if (0 != fflush(sel_file->file)) {
			tloge("fflush file failed: %s\n", strerror(errno));
		}
		if (0 != fsync(fileno(sel_file->file))) {
...
static void write_work(struct sec_storage_t *trans_control)
{
	...
	if (find_open_file(trans_control->args.write.fd, &sel_file)) {
		...
		if (SEC_WRITE_SSA == trans_control->ret2) {
			/* */
...
{
	int ret;
	char name_buff[FILE_NAME_MAX_BUF] = {0}, name_buff2[FILE_NAME_MAX_BUF] = {0};
...
{
	int ret;
	char from_path[FILE_NAME_MAX_BUF] = {0}, to_path[FILE_NAME_MAX_BUF] = {0};
...
static void *thread_misc_work(void *control)
{
	...
	while (1) { /*lint !e716 */
		...
		switch (trans_control->cmd) {
		case sec_nv_info:/* add by sdk for bootloaderlock status in nv partition */
...
static int check_partition_ready(const char *mnt_dir)
{
	FILE* fp;
	...
	{
		if (0 == strncmp(mentry->mnt_dir, mnt_dir, strlen(mnt_dir))) {
			find_flag = 1;
			break;
		}
	}
...
static unsigned int get_file_size(const char *filename)
{
	...
	if(stat(filename, &statbuff) < 0) {
		tloge("Failed to get the size of ttf file.\n");
		return file_size;
	}

	file_size = (unsigned int)statbuff.st_size;

	return file_size;
}

static void *thread_load_ttf_work(void *dummy)
{
	unsigned int ttf_file_size;

	(void)dummy;
	tlogi("now start to load ttf into memory\n");
	...
	ttf_file_size = get_file_size(TTF_FILE_PATH);
	if (0 == ttf_file_size) {
		tloge("the size of ttf_file is error.\n");
		close(fd);
		return NULL;
	}

	ret = ioctl(fd, (int)TC_NS_CLIENT_IOCTL_LOAD_TTF_FILE, /*lint !e835 */
		ttf_file_size);
	if (ret) {
		tloge("failed to get ttf buff.\n");
	}
...
#ifndef WITH_DEVCHIP_PLATFORM
	tlogi("now start thread_load_ttf_work.\n");
	ret = pthread_create(&th_load_ttf, NULL, thread_load_ttf_work, NULL);
	if (ret) {
	    tloge("create th_load_ttf thread failed, ret is %d\n", ret);
	}
#endif
#ifdef TEE_SUPPORT_SOCKET
...
#endif
#ifndef WITH_DEVCHIP_PLATFORM
	ret = pthread_join(th_load_ttf, NULL);
	if (ret) {
		tloge("join thread th_load_ttf failed.\n");
	}
#endif
	if (th_fs_create_flag == 1) {
