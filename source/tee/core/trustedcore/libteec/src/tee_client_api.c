...
static int recv_file_descriptor(int socketfd)
{
	...
	if (res <= 0) {
		tloge("recv msg error ret %x, errno %x\n", res, errno);
		return -1;
	}
...
static void do_try_remove_session_from_list(TEEC_Context *context, TEEC_Session *session)
{
	TEEC_Session *session_entry = NULL;
	TEEC_Session *tmp_entry = NULL;

	if (!context || !session) {
		tloge("Bad params, context or session is NULL\n");
		return;
	}

	list_for_each_entry_safe(session_entry, tmp_entry, &context->session_list, head) {/*lint !e413 *//*lint !e666 *//*lint !e48 *//*lint !e40 *//*lint !e64 *//*lint !e666 *//*lint !e40 *//*lint !e666 *//*lint !e666 */
		if (session_entry == session) {
			if (session->head.prev && session->head.next) {
				list_remove(&session->head);
			} else {
				tloge("session may been memseted, the list seem be broken.\n");
			}
		}
	}
}
...
static int ca_daemon_connect()
{
	...
	if (-1 == (s = socket(AF_UNIX, SOCK_STREAM, 0))) {
		tlogd("can't open stream socket, errno=%d\n", errno);
		return -1;
	}
	...
	rc = strncpy_s(remote.sun_path, sizeof(remote.sun_path),
	...
	if (-1 == connect(s, (struct sockaddr *)&remote, len)) { /*lint !e713 */
		tlogd("connect() failed, errno=%d\n", errno);
		close(s);
		return -1;
	}
	...
	if (fd > 0) {
		...
		tloge("not connected , ret %x, errno %x\n", fd, errno);
				...
				uint32_t *returnOrigin)
{
	...
	if (ret == 0) {
		...
		do_try_remove_session_from_list(context, session);
...
void TEEC_ReleaseSharedMemory(TEEC_SharedMemory *sharedMem)
{
	...
	/* First, check parameters is valid or not */
	if (NULL == sharedMem || NULL == sharedMem->context){
			...
			TEEC_Operation *operation)
{
	...
	rc = memcpy_s(cli_context->uuid, sizeof(cli_context->uuid), (uint8_t *)&service_id,
			sizeof(TEEC_UUID));
	if (EOK != rc) {
		return;
	}
...
#else
				= ((unsigned long long)tmpref->buffer) >> 32;
#endif
...
#else
				= ((unsigned long long)&tmpref->size) >> 32;
#endif
		} else if ((param_type[param_cnt] == TEEC_MEMREF_WHOLE)
			|| (param_type[param_cnt] == TEEC_MEMREF_PARTIAL_INPUT)
			|| (param_type[param_cnt] == TEEC_MEMREF_PARTIAL_OUTPUT)
...
#else
					= ((unsigned long long)&memref->parent->size) >> 32;
#endif
			} else {
...
#else
					= ((unsigned long long)&memref->size) >> 32;
#endif
			}
...
#else
					= ((unsigned long long)memref->parent->buffer) >> 32;
#endif
			} else {
				cli_context->params[param_cnt].memref.buffer
...
#else
				= ((unsigned long long)&ionref->ion_share_fd) >> 32;
#endif
			cli_context->params[param_cnt].value.b_addr
...
#else
				= ((unsigned long long)&ionref->ion_size) >> 32;
#endif
		} else {
			/* if type is none, ignore it */
		}
