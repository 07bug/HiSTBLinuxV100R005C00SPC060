/*
 * FileName:
 * Author:         sdk  Version: 0.0.1  Date: 2013-5-20
 * Description:
 * Version:
 * Function List:
 *                 1.
...
#ifndef CFG_HI_TEE_TA_LOAD_PATH
#define CFG_HI_TEE_TA_LOAD_PATH "/data"
#endif
const char *IMAGE_PATH = CFG_HI_TEE_TA_LOAD_PATH;
#else
	...
	const char *loadfile)
{
	...
	if (ret) {
		TEEC_Error("fstat error\n");
		goto FREAD_ERR;
	}

	total_img_len = (unsigned int)file_stat.st_size;

	if (total_img_len > MAX_IMAGE_LEN) {
		TEEC_Error("file is too large to load\n");
		goto FREAD_ERR;
	}
#endif
...
#ifdef WITH_DEVCHIP_PLATFORM
			(uint32_t)((((unsigned long)file_buf_pt) >> 16) >>16 );
#else
			(uint32_t)((((unsigned long long)file_buf_pt) >> 16) >> 16 );
#endif
...
EXIT_ERR:
	...
	for (index = 0; index < alloc_cnt; index++) {
		/*the bits to move at one time can't be greater or equal to the bits of the system, or the result will be random*/
#ifdef WITH_DEVCHIP_PLATFORM
		unsigned long int file_buf_h_tmp = (((unsigned long)register_buf->file_buf_h) << 16) << 16; /*lint !e511 */
#else
		unsigned long long int file_buf_h_tmp = (((unsigned long long)register_buf->file_buf_h) << 16) << 16; /*lint !e511 */
#endif
		file_buf_pt = (char *)(register_buf->file_buf |file_buf_h_tmp );
		/*[false alarm]*/
