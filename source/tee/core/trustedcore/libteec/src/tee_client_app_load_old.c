...
#endif
const char *IMAGE_PATH_OLD = CFG_HI_TEE_TA_LOAD_PATH;
#else
	...
	const TEEC_UUID *srv_uuid)
{
	...
	int32_t len;
	char temp_fname[MAX_FILE_PATH_LEN + MAX_FILE_NAME_LEN + MAX_FILE_EXT_LEN + 1]={0};
	...
	const char *loadfile)
{
	...
	if ((ret != 1) ||
		(image_head.context_len > MAX_IMAGE_LEN)||
