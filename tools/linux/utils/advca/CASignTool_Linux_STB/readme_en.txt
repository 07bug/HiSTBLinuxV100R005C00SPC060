...
4. This tool is used as follows:
  ...
  -ov sets the version of Secure OS.
  ...
  Signing TA, TA certificate and TA root certificate.
  
  9> ./CASignTool 15 -R pemFilePath -ot b/t -O destFilePath
  Transform the key format. The .pem format can be transformed to binary format or text format.
  -R sets the path of the PEM file, -ot sets the format of the output file, b means binary, t means txt, -O sets the path of the output file.
  
  10> ./CASignTool 16 mergeImage.cfg -r srcDirPath -O destFilePath
  Merge image.
  -r sets the dirctory of the source file, -O sets the path of the output file.
  In the config file, the parameter ImageNumber indicates the number of images to be signed. When a signed image is added, the parameters ImageXXX is added. XXX indicates the number of indexes, and increases in sequence. 
...
6. During the boot image signature, the supported chips include Hi3798 CV200, Hi3798M V100, Hi3716M V420, Hi3716M V410, Hi3796M V100, Hi3796M V200, Hi3798M V200, Hi3716D V110, Hi3716D V100, Hi3798M V300 and Hi3798M V310. The chips can be configured in the .cfg file. For non-boot images, you do not need consider the chip type.
