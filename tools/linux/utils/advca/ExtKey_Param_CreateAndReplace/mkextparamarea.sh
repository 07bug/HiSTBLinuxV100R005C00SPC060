...
function usage ()
{
	echo "Usage: ${selfname} ExtCfgParamFile FBLSize SecureOSSize SBLSize FBLDataSize SBLVerifyFlag SBLDecryptFlag"
	echo "  ExtCfgParamFile      Chipset configuration parameter."
	echo "                   used to verify Ext Params Area and SBL"
    echo "  FBLSize          FBL partition size in hex."
	...
	echo "Example:"
	echo "  ${selfname} reg.bin 0x10 0x10 0x10 0x10 1 0"
	echo ""
	exit 0
}
###############################################################################
...
function run ()
{
	...
	dd if=/dev/zero of=${outputfile} bs=1 seek=${ExtCfgParamFileLen}  count=${reservedsize0}

	len=`expr length $FBLSize`
	FBLSize=`expr substr $FBLSize 3 $(($len-2))`
    loop=$((10-$len))
    for((i=1;i<=$loop;i++))
	do
		FBLSize="0"$FBLSize
	...
	byte0=`expr substr $FBLSize 7 2`
	if [ -z "$byte0" ]; then
			byte0=0
	fi

	byte1=`expr substr $FBLSize 5 2`
	if [ -z "$byte1" ]; then
			byte1=0
	fi

	byte2=`expr substr $FBLSize 3 2`
    if [ -z "$byte2" ]; then
		byte2=0
	fi

	byte3=`expr substr $FBLSize 1 2`
	if [ -z "$byte3" ]; then
		byte3=0
	fi
	...
	dd if=/dev/zero of=${outputfile} bs=1 seek=${rsasigoffset}  count=${reservedsize1}
...
SBLVerifyFlag=${6}
SBLDecryptFlag=${7}
ExtCfgParamFileLen=9600
reservedsize0=76
rsasigoffset=$((${ExtCfgParamFileLen} + ${reservedsize0} + 4 + 16))
echo 'rsasigoffset' ${rsasigoffset}
reservedsize1=0
