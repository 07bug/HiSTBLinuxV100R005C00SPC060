...
3、注意事项
	...
	4、关于miniboot等不能烧写的问题，工具端已做了兼容性修改更新，但必须将BurnConfig.ini中的内配置项ProgrammerFileName配置为相应的文件名以及提供programmer.bin文件（由客户提供有烧录命令的boot版本，文件名必须与配置项ProgrammerFileName保持一致）。其它烧写不需要programmer.bin文件，BurnConfig.ini文件必须将其内配置项ProgrammerFileName配置为空或注释掉该配置项。
